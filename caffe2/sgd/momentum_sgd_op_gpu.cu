...
__global__ void SparseMomentumSGDKernel(
  ...
  CUDA_1D_KERNEL_LOOP(i, N) {
    ...
    if (!nesterov) {
      const float adjusted_gradient =
          LR * gradIn[gradIdx] + momentum * param_mom[paramIdx];
      ...
      const float adjusted_gradient =
          (1 + momentum) * mom_new - momentum * mom_old;
      gradOut[gradIdx] = adjusted_gradient;
      param[paramIdx] -= adjusted_gradient;
    }
  }
...
REGISTER_CUDA_OPERATOR(
    MomentumSGDUpdate,
    MomentumSGDUpdateOp<float, CUDAContext>);
REGISTER_CUDA_OPERATOR(
    SparseMomentumSGDUpdate,
    SparseMomentumSGDUpdateOp<float, CUDAContext>);
