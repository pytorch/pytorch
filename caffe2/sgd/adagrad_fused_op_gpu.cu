...
__global__ void gradient_mean_kernel(
  ...
  for (int i = threadIdx.x; i < post; i += blockDim.x) {
    grad_out[group * post + i] = lengths[group] > 0
        ? grad_in[group * post + i] / lengths[group]
        : grad_in[group * post + i];
  }
}
...
class CUDASparseAdagradFusedWithSparseLengthsSumGradientOp final
 ...
 public:
  ...
  bool DoRunWithType2() {
    ...
    if (is_mean) {
      auto block_size = post <= maxThreads ? post : maxThreads;
      gradient_mean_kernel<T>
          <<<len_length, block_size, 0, context_.cuda_stream()>>>(
              grad, lengths, grad_buffer_data, post);
    }
