...
void adagrad_update<HIPContext>(
    ...
    const float* lr,
    HIPContext* context) {
  hipLaunchKernelGGL(AdagradUpdate, dim3(CAFFE_GET_BLOCKS(static_cast<int>(N))), dim3(CAFFE_HIP_NUM_THREADS), 0, context->hip_stream(), static_cast<int>(N), w, g, h, nw, nh, epsilon, decay, lr);
}
...
bool RowWiseSparseAdagradOp<float, HIPContext>::DoRunWithType() {
  ...
  hipLaunchKernelGGL(RowWiseSparseAdagradKernel, dim3(min(static_cast<const int>(GRAD_M), CAFFE_MAXIMUM_NUM_BLOCKS)), dim3(CAFFE_HIP_NUM_THREADS), 0, context_.hip_stream(),
