...
void adam_update<HIPContext>(
    ...
    const float* lr,
    HIPContext* context) {
  hipLaunchKernelGGL(AdamUpdate, dim3(CAFFE_GET_BLOCKS(static_cast<int>(N))), dim3(CAFFE_HIP_NUM_THREADS), 0, context->hip_stream(), 
...
void adam_compute<HIPContext>(
    ...
    const float* lr,
    HIPContext* context) {
  hipLaunchKernelGGL(AdamCompute, dim3(CAFFE_GET_BLOCKS(static_cast<int>(N))), dim3(CAFFE_HIP_NUM_THREADS), 0, context->hip_stream(), 
...
__global__ void SparseAdamKernel(
  ...
  HIP_1D_KERNEL_LOOP(i, N) {
    ...
    param[paramIdx] += lr[0] * correction * m1n / (sqrtf(m2n) + epsilon);
  }
}
...
bool SparseAdamOp<float, HIPContext>::DoRunWithType() {
  ...
  hipLaunchKernelGGL(SparseAdamKernel<SIndex>, dim3(CAFFE_GET_BLOCKS(N)), dim3(CAFFE_HIP_NUM_THREADS), 0, context_.hip_stream(), 
          N,
          grad_slice_sz,
          beta1_,
          beta2_,
