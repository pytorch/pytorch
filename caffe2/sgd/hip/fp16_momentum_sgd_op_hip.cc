...
__global__ void FP16MomentumSGDKernel(int N,
                                      const half2* g,
                                      const half2* m,
                                      half2* ng,
                                      half2* nm,
                                      ...
                                      half2* param)
{
    ...
    if(!nesterov)
    {
        HIP_1D_KERNEL_LOOP(i, n)
        {
            ...
            const half2 adjusted_gradient = __hfma2(LR, ng[i], __hmul2(momentum, m[i]));
            ...
            if(i == 0 && (N % 2))
            {
                ...
                const half adjusted_gradient_half = __hfma(
                    __high2half(LR), ng_half[N - 1], __hmul(__high2half(momentum), m_half[N - 1]));
    ...
    else
    {
        HIP_1D_KERNEL_LOOP(i, n)
        {
            ...
            if(i == 0 && (N % 2))
            {
                ...
                const half mi_new_half = __hfma(__high2half(LR), ng_half[N - 1], mom_mi_half);
                nm_half[N - 1]         = mi_new_half;
                ng_half[N - 1] =
                    __hsub(__hfma(mi_new_half, __high2half(momentum), mi_new_half), mom_mi_half);
                if(param)
                {
...
__global__ void FP16MomentumSGDFP32Kernel(int N,
                                          const half2* g,
                                          const half2* m,
                                          half2* ng,
                                          half2* nm,
...
template <>
void fp16_momentum_sgd_update<HIPContext>(int N,
                                          const float16* g,
                                          const float16* m,
                                          float16* ng,
                                          float16* nm,
...
REGISTER_HIP_OPERATOR(FP16MomentumSGDUpdate, FP16MomentumSGDUpdateOp<float16, HIPContext>);
