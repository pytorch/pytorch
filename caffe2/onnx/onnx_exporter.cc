...
bool OnnxExporter::IsDenyListed(const caffe2::Argument& arg) {
  ...
  if (arg.has_i()) {
    const auto it = kDenyListInt.find(arg.name());
    if (it != kDenyListInt.end()) {
      return it->second.count(arg.i());
    }
  } else if (arg.has_s()) {
    const auto it = kDenyListString.find(arg.name());
    if (it != kDenyListString.end()) {
