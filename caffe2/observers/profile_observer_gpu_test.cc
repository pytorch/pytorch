...
TEST(CostObserverTest, TestLSTMUnit) {
  ...
  CAFFE_ENFORCE(ob);
  auto cost_per_op_type = ob->getAggregatedOpTypeCost();
  CAFFE_ENFORCE(cost_per_op_type["LSTMUnit"].flops == D * N);
  CAFFE_ENFORCE(
      cost_per_op_type["LSTMUnit"].bytes_read == D * N * sizeof(float));
  CAFFE_ENFORCE(
      cost_per_op_type["LSTMUnit"].bytes_written == D * N * sizeof(float));
  net->DetachObserver(ref);
}
