...
bool VideoInputOp<Context>::Prefetch() {
  try {
    ...
    if (!std::is_same<Context, CPUContext>::value) {
      if (get_rgb_) {
        prefetched_clip_rgb_on_device_.CopyFrom(prefetched_clip_rgb_, true);
      }
      if (get_optical_flow_) {
      ...
      if (get_video_id_) {
        prefetched_video_id_on_device_.CopyFrom(prefetched_video_id_, true);
      }
      if (get_start_frame_) {
  ...
  try {
    ...
    if (get_start_frame_) {
      ...
      if (std::is_same<Context, CPUContext>::value) {
        ...
        start_frame_output->CopyFrom(prefetched_start_frame_on_device_, true);
      }
    }
  } catch (const std::exception& exc) {
