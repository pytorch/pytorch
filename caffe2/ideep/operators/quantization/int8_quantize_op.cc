...
class IDEEPInt8QuantizeOp final : public IDEEPOperator {
 public:
  ...
  IDEEPInt8QuantizeOp(const OperatorDef& operator_def, Workspace* ws)
    ...
    if (HasArgument("output_order")) {
      output_fmt_ = static_cast<iformat>(
    ...
    output_dt_ = zero_point_ == 0 ? idtype::u8 : idtype::s8;
  ...
  bool RunOnDevice() override {
    ...
    CAFFE_ENFORCE(X.ndims() == 4, "Only support 4 dims");
    CAFFE_ENFORCE(X.get_data_type() == idtype::f32, "Not support data type");

    auto* Y = Output(0);
    Y->init({X.get_dims(), output_dt_,
        output_fmt_ != iformat::format_undef
        ? output_fmt_ : X.get_public_format()});
 ...
 private:
  ...
  iscale Y_scales_;
  idtype output_dt_;
  iformat output_fmt_ {iformat::format_undef};
