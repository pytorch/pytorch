...
class TestParallelFcOperator(hu.HypothesisTestCase):
    def _run_test(self, n, m, k, multi_dim, engine, tracing, gc, dc):
        dtype = np.float32
    ...
    def test_fc(self, n, m, k, multi_dim, engine, tracing, gc, dc):
        # Tracing for net type parallel needed for TBB doesn't work yet.
        assume(engine == "INTRA_OP_PARALLEL" or not tracing)
        self._run_test(n, m, k, multi_dim, engine, tracing, gc, dc)
