...
#include "numa_all_reduce_op_avx.h"
...
#ifdef _MSC_VER
      // Some version of MSC doesn't have posix_memalign and aligned_alloc.
      push_bufs_[input_id] = static_cast<float*>(malloc(len * sizeof(float)));
#else
      // We could use aligned_alloc which is C++ standard but sanitizer produces
      // false positives (it forces size should be a multiple of alignment).
      CAFFE_ENFORCE_EQ(
          posix_memalign(
              (void**)&push_bufs_[input_id], 4096, len * sizeof(float)),
          0);
#endif
