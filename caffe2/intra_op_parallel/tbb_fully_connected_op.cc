...
bool TBBFullyConnectedGradientOp::RunOnDevice() {
  ...
  tracing::TracerGuard task_tracer_guard =
      tracing::TracerGuard::getCurrentTracerGuard()
      ? *tracing::TracerGuard::getCurrentTracerGuard()
      : tracing::TracerGuard(); // The default non-enabled tracer guard

  ::tbb::parallel_for(
      0,
      num_tasks,
      [this, num_tasks, &task_tracer_guard](size_t task_id) {
#ifdef CAFFE2_USE_MKL
        int old_mkl_num_threads = mkl_set_num_threads_local(1);
#endif
        tracing::TracerGuard worker_tracer_guard = task_tracer_guard;
