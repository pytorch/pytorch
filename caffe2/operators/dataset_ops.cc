...
class TreeWalker {
 ...
 private:
  inline const auto& input(int32_t idx) const {
    return inputs_[idx]->Get<TensorCPU>();
  }

  inline const auto& field(int idx) const {
    return cursor_.it.fields().at(idx);
  }

  inline auto lengthIdx(int fieldId) const {
    return field(fieldId).lengthFieldId + 1;
  }

  inline auto offset(int fieldId) const {
    return prevOffsets_[lengthIdx(fieldId)];
  }

  auto fieldDim(int fieldId) const {
    auto tensorDim = input(fieldId).dims();
    tensorDim[0] = sizes_[lengthIdx(fieldId)];
    return tensorDim;
  }

  auto* fieldPtr(int fieldId) const {
