...
void GetTopK(
  ...
  std::vector<std::pair<T, TIndex>> vec;
  vec.reserve(k);
  for (TIndex i = 0; i < k; ++i) {
    vec.emplace_back(*src_ptr, i);
    src_ptr += stride;
  }
  std::priority_queue<
      std::pair<T, TIndex>,
...
bool TopKOp<T, Context>::RunOnDevice() {
  ...
  const TIndex prev_dims = std::accumulate(
      input_dims.cbegin(),
      input_dims.cbegin() + axis_,
      TIndex(1),
      std::multiplies<TIndex>());
  const TIndex next_dims = std::accumulate(
      input_dims.cbegin() + axis_ + 1,
      input_dims.cend(),
      TIndex(1),
      std::multiplies<TIndex>());
  const TIndex src_offset_stride = input_dims[axis_] * next_dims;
  const TIndex dst_offset_stride = k_ * next_dims;
  TIndex src_offset = 0;
  TIndex dst_offset = 0;
  for (TIndex i = 0; i < prev_dims; ++i) {
    for (TIndex j = 0; j < next_dims; ++j) {
      GetTopK(
          input_data,
          input_dims[axis_],
          k_,
...
bool TopKGradientOp<T, Context>::RunOnDevice() {
  ...
  const TIndex next_dims = std::accumulate(
      ...
      TIndex(1),
      std::multiplies<TIndex>());
  const TIndex src_offset_stride = k * next_dims;
  const TIndex dst_offset_stride = origin_dims[axis_] * next_dims;
  TIndex src_offset = 0;
  TIndex dst_offset = 0;
  for (TIndex i = 0; i < prev_dims; ++i) {
    for (TIndex j = 0; j < next_dims; ++j) {
