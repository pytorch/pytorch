...
class expand_dims_cpu final : public c10::OperatorKernel {
 public:
  void operator()(
    ...
    if (!initialized_) {
      ...
      CAFFE_ENFORCE(dims_.get(0) >= 0, "Dimension ids must be non-negative.");
      initialized_ = true;
    }
 ...
 private:
  torch::ListPtr<int64_t> dims_ = c10::make_list<int64_t>();
