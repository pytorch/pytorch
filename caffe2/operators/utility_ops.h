...
class SumGradientOp : public Operator<Context> {
 public:
  ...
  bool DoRunWithType() {
    // legacy support
    if (InputSize() == 1)
      return DoRunWithTypeLegacy<DstType>();
 ...
 private:
  // This is for backward compatibility, when SumOp does not support broadcast
  template <typename DstType>
  bool DoRunWithTypeLegacy() {
    CAFFE_ENFORCE_EQ(InputSize(), 1);

    const auto& dY = Input(0);
    for (auto i = 0; i < OutputSize(); i++) {
      auto* dX = Output(i, dY.sizes(), at::dtype<DstType>());
      dX->CopyFrom(dY, true /*async*/);
    }
