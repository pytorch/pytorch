...
__global__ void LRNComputeDiffNCHW(const int nthreads, const T* bottom_data,
  ...
  CUDA_1D_KERNEL_LOOP(index, nthreads) {
    // find out the local offset
    const auto w = index % width;
    const auto h = (index / width) % height;
    const auto n = index / width / height;
    const auto offset = (n * channels * height + h) * width + w;
    const auto step = height * width;
    bottom_data += offset;
    top_data += offset;
    scale += offset;
    top_diff += offset;
