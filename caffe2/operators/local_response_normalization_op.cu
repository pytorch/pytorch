...
namespace {
template <typename T>
__global__ void LRNFillScaleNCHW(
    const int nthreads,
    const T* in,
    const int num,
    const int channels,
...
template <typename T>
__global__ void LRNFillScaleNHWC(
    const int nthreads,
    const T* in,
    const int num,
    const int height,
...
// kernel.
template <typename T>
__global__ void LRNComputeOutput(
    const int nthreads,
    const T* in,
    const T* scale,
...
template <typename T>
__global__ void LRNComputeDiffNCHW(
    const int nthreads,
    const T* bottom_data,
    const T* top_data,
    const T* scale,
  ...
  CUDA_1D_KERNEL_LOOP(index, nthreads) {
    ...
    while (head < post_pad) {
      accum_ratio +=
          top_diff[head * step] * top_data[head * step] / scale[head * step];
      ++head;
    }
    // until we reach size, nothing needs to be subtracted
    while (head < size) {
...
template <typename T>
__global__ void LRNComputeDiffNHWC(
    const int nthreads,
    const T* bottom_data,
    const T* top_data,
    const T* scale,
  ...
  CUDA_1D_KERNEL_LOOP(index, nthreads) {
    ...
    for (int i = -pre_pad; i < size - pre_pad; ++i) {
      if (c + i >= 0 && c + i < channels) {
        accum_ratio +=
            top_diff[index + i] * top_data[index + i] / scale[index + i];
      }
    }
...
bool LRNOp<float, CUDAContext>::RunOnDeviceWithOrderNCHW() {
  ...
  int n_threads = N * H * W;
  LRNFillScaleNCHW<float>
      <<<CAFFE_GET_BLOCKS(n_threads),
         ...
         context_.cuda_stream()>>>(
          ...
          scale_data);
  n_threads = X.numel();
  LRNComputeOutput<float>
      <<<CAFFE_GET_BLOCKS(n_threads),
         CAFFE_CUDA_NUM_THREADS,
         0,
         context_.cuda_stream()>>>(n_threads, Xdata, scale_data, -beta_, Ydata);
  return true;
}
