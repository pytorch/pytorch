...
class MIOpenSoftmaxOp final : public Operator<HIPContext>
{
    ...
    ~MIOpenSoftmaxOp() { MIOPEN_ENFORCE(miopenDestroyTensorDescriptor(desc_)); }
    ...
    bool DoRunWithType()
    {
        ...
        {
            MIOPEN_ENFORCE(
                miopenSet4dTensorDescriptor(desc_, miopenTypeWrapper<T>::type, N, D, 1, 1));
            dims_ = X.dims();
        }
        MIOPEN_ENFORCE(miopenSoftmaxForward(miopen_wrapper_.inline_miopen_handle(),
                                            &alpha_,
                                            desc_,
                                            X.template data<T>(),
                                            &beta_,
...
class MIOpenSoftmaxGradientOp final : public Operator<HIPContext>
{
    ...
    ~MIOpenSoftmaxGradientOp() { MIOPEN_ENFORCE(miopenDestroyTensorDescriptor(desc_)); }
    ...
    bool DoRunWithType()
    {
        ...
        MIOPEN_ENFORCE(miopenSoftmaxBackward(miopen_wrapper_.inline_miopen_handle(),
                                             &alpha_,
                                             desc_,
                                             Y.template data<T>(),
                                             desc_,
