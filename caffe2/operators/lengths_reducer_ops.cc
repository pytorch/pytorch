...
training stage, which allows to use larger batches or train efficiently in a case of limited resources.
    ...
    .Output(
        3,
        "core2_output",
        "intermediate mm result from core2 for backward path")
...
OPERATOR_SCHEMA(TTSparseLengthsSumGradient).NumInputs(10).NumOutputs(3);
...
class GetTTSparseLengthsGradient : public GradientMakerBase {
  ...
  vector<OperatorDef> GetGradientDefs() override {
    ...
    return SingleGradientDef(
        "TTSparseLengthsSumGradient",
        "",
        // CORE0, CORE1, CORE2, INDICES, LENGTHS, CORE0_output, CORE1_output,
        // CORE2_output, indices, dY
        vector<string>{
            I(0), I(1), I(2), I(3), I(4), O(1), O(2), O(3), O(4), GO(0)},
