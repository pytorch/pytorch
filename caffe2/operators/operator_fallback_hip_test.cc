...
{
    public:
    IncrementByOneOp(const OperatorDef& def, Workspace* ws) : Operator<CPUContext>(def, ws) {}
    bool RunOnDevice()
    {
...
OPERATOR_SCHEMA(IncrementByOne).NumInputs(1).NumOutputs(1).AllowInplace({{0, 0}});
...
{
    OperatorDef op_def =
        CreateOperatorDef("IncrementByOne", "", vector<string>{"X"}, vector<string>{"X"});
    Workspace ws;
