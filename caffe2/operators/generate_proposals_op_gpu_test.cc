...
static void AddLinSpacedInput(
  ...
  tmp_vec.setLinSpaced(min_val, max_val);
...
TEST(GenerateProposalsGPUTest, TestEmpty) {
  if (!HasCudaGPU())
    return;
  Workspace ws;
  OperatorDef def;
...
TEST(GenerateProposalsGPUTest, TestRealDownSampled) {
  ...
  Tensor rois_cpu(rois, CPU);
  auto rois_data = Eigen::Map<const ERMatXf>(
      rois_cpu.data<float>(), rois.dim(0), rois.dim(1));
  ...
  Tensor rois_probs_cpu(rois_probs, CPU);
  auto rois_probs_data = ConstEigenVectorArrayMap<float>(
      rois_probs_cpu.data<float>(), rois.dim(0));
  EXPECT_NEAR(
