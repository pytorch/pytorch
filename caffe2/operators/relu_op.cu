...
__global__ void ReluCUDAKernel<half2>(const int N, const half2* X, half2* Y) {
  ...
  if (i < N) {
    ...
    // There are explicit cast to float here, because it may otherwise cause
    // ambiguity on ROCm and can be triggered sometimes:
    //
    ...
    Y[i] = __floats2half2_rn(
        xx.x > 0.0f ? static_cast<float>(xx.x) : 0.0f,
...
__global__ void ReluGradientCUDAKernel<half2>(
  ...
  if (i < N) {
    ...
    //
    //   error: conditional expression is ambiguous; 'const
    //   hip_impl::Scalar_accessor<float, Native_vec_, 1>' can be converted to
    //   'float' and vice versa

    dX[i] = __floats2half2_rn(
        yy.x > 0.0f ? static_cast<float>(dy.x) : 0.0f,
