#include "caffe2/operators/quantized/int8_aabb_roi_proposals_op.h"
...
template <>
bool Int8AABBRoIProposalsOp<CPUContext>::RunOnDevice() {
  /* Validate inputs */
  ...
  CAFFE_ENFORCE_EQ(input_scores_tensor.t.dim(), 4, input_scores_tensor.t.dim());
  const auto num_images = input_scores_tensor.t.size(0);
  const auto num_anchors = order_ == StorageOrder::NCHW
      ? input_scores_tensor.t.size(1)
      : input_scores_tensor.t.size(3);
  const auto features_height = order_ == StorageOrder::NCHW
      ? input_scores_tensor.t.size(2)
      : input_scores_tensor.t.size(1);
  ...
  CAFFE_ENFORCE_EQ(
      input_deltas_tensor.t.sizes(),
      (vector<int64_t>{
          num_images, features_height, features_width, num_anchors * 4}));
  const uint8_t* input_deltas_ptr = input_deltas_tensor.t.data<uint8_t>();
  const int32_t input_deltas_zero_point = input_deltas_tensor.zero_point;
  const float input_deltas_scale = input_deltas_tensor.scale;

  const Tensor& anchors_tensor = Input(2);
  CAFFE_ENFORCE_EQ(anchors_tensor.sizes(), (vector<int64_t>{num_anchors, 4}));
  CAFFE_ENFORCE(anchors_tensor.template IsType<int16_t>());
  const int16_t* anchors_ptr = anchors_tensor.data<int16_t>();

  const Tensor& image_info_tensor = Input(3);
  CAFFE_ENFORCE_EQ(image_info_tensor.sizes(), (vector<int64_t>{num_images, 2}));
  CAFFE_ENFORCE(image_info_tensor.template IsType<uint16_t>());
  ...
  Tensor* output_boxes_tensor =
      Output(1, {max_total_proposals, 4}, at::dtype<uint16_t>());
  int8::Int8TensorCPU* output_scores_tensor = nullptr;
  ...
  if (OutputSize() > 1) {
    output_scores_tensor = Outputs()[2]->template GetMutable<int8::Int8TensorCPU>();
    output_scores_tensor->t.Resize(max_total_proposals);

    output_scores_tensor->zero_point = input_scores_tensor.zero_point;
    output_scores_tensor->scale = input_scores_tensor.scale;
  ...
  for (int image_idx = 0; image_idx < num_images; image_idx++) {
    ...
    std::vector<uint8_t> ordered_scores;
    ...
    for (int roi_idx : score_ordered_proposals) {
      ...
      const AABB anchor_aabb(
          float(x) * roi_stride_ + float(anchors_ptr[anchor_idx * 4 + 0]) * 0.125f,
          float(y) * roi_stride_ + float(anchors_ptr[anchor_idx * 4 + 1]) * 0.125f,
          float(x) * roi_stride_ + float(anchors_ptr[anchor_idx * 4 + 2]) * 0.125f,
          float(y) * roi_stride_ + float(anchors_ptr[anchor_idx * 4 + 3]) * 0.125f);

      const float dx = (int32_t(image_deltas[roi_idx * 4 + 0]) - input_deltas_zero_point) * input_deltas_scale;
      const float dy = (int32_t(image_deltas[roi_idx * 4 + 1]) - input_deltas_zero_point) * input_deltas_scale;
      const float dw = (int32_t(image_deltas[roi_idx * 4 + 2]) - input_deltas_zero_point) * input_deltas_scale;
      const float dh = (int32_t(image_deltas[roi_idx * 4 + 3]) - input_deltas_zero_point) * input_deltas_scale;
    ...
    for (size_t outputIdx : outputIndices) {
      *output_boxes_ptr++ = uint16_t(std::min(std::max(ordered_boxes[outputIdx].x1 * 8.0f, 0.0f), 65535.0f));
      *output_boxes_ptr++ = uint16_t(std::min(std::max(ordered_boxes[outputIdx].y1 * 8.0f, 0.0f), 65535.0f));
      *output_boxes_ptr++ = uint16_t(std::min(std::max(ordered_boxes[outputIdx].x2 * 8.0f, 0.0f), 65535.0f));
      *output_boxes_ptr++ = uint16_t(std::min(std::max(ordered_boxes[outputIdx].y2 * 8.0f, 0.0f), 65535.0f));
  ...
  output_scores_tensor->t.ShrinkTo(num_output_rois);

  return true;
}

REGISTER_CPU_OPERATOR(Int8AABBRoIProposals, Int8AABBRoIProposalsOp<CPUContext>);

OPERATOR_SCHEMA(Int8AABBRoIProposals)
    .NumInputs(4)
    .NumOutputs(2, 3)
    .SetDoc(R"DOC(
...
bounding boxes.
    ...
    .Input(
        ...
        "size (num_images, num_anchors, H, W)")
    .Input(
        1,
        "deltas",
        "Bounding box deltas from conv layer, "
        "size (num_images, num_anchors * 4, H, W), "
        "format (dx, dy, dw, dh)")
    .Input(
        2,
        "anchors",
...
SHOULD_NOT_DO_GRADIENT(Int8AABBRoIProposals);
