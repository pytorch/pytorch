...
template <>
template <typename T>
C10_EXPORT bool GeluFunctor<CPUContext>::operator()(const int N, const T* X, T* Y, CPUContext* context) const {
  if (fast_gelu) {
...
template <>
template <typename T>
C10_EXPORT bool GeluGradientFunctor<CPUContext>::Forward(
