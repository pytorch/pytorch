...
is marked as omitted by setting its 'has_{name}' argument to False.
    ...
    .Arg(
        "save_scopes",
        "Whether to save the scopes across iterations, as in "
        "for backprop")
    .Arg(
        "disable_scopes",
        "Do not create new scopes. Use this only if you're "
    ...
    .Input(
        0,
        "max_trip_count",
        "Number of iterations to go out to. Used if "
        "the flag has_trip_count is True.")
    .Input(
        1,
        "first_iter_condition",
        "Dynamic condition value for the first "
