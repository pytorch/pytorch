...
__global__ void PackSegmentsKernel(const Data_T* data_ptr,
                                   const T* lengths_ptr,
...
template <typename T, typename Data_T>
__global__ void UnpackSegmentsKernel(const Data_T* data_ptr,
                                     const T* lengths_ptr,
                                     const T* lengths_cum_sum,
                                     const T max_length,
                                     const int64_t num_seq,
...
template <typename T>
int64_t int_array_sum(const T* dev_array,
                      int64_t num_items,
                      Tensor<HIPContext>& dev_buffer,
                      Tensor<HIPContext>& dev_sum,
...
template <typename T>
T array_max(const T* dev_array,
            int64_t num_items,
            Tensor<HIPContext>& dev_max_buffer,
            Tensor<HIPContext>& dev_max,
...
template <typename T>
void array_prefix_sum_exclusive(const T* dev_array,
...
{
    return DispatchHelper<TensorTypes2<char, int32_t, int64_t, float>, T>::call(this, Input(DATA));
}
...
bool PackSegmentsOp<HIPContext>::DoRunWithType2()
{
    ...
    const T max_length = array_max<T>(
        lengths_ptr, num_seq, dev_buffer_, dev_max_length_, host_max_length_, context_);
...
bool UnpackSegmentsOp<HIPContext>::DoRunWithType2()
{
    ...
    const int64_t num_cell = int_array_sum<T>(
        lengths_ptr, num_seq, dev_buffer_, dev_num_cell_, host_num_cell_, context_);

    // create output tensor
    auto shape = data.dims();
    CAFFE_ENFORCE(shape[0] == lengths.dim(0), "LENGTH should match DATA in dimension 0");
