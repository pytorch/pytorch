...
bool GatherOp<HIPContext>::DoRunWithType() {
  ...
  hipLaunchKernelGGL((GatherKernel), dim3(std::min(N, CAFFE_MAXIMUM_NUM_BLOCKS)), dim3(CAFFE_HIP_NUM_THREADS), 0, context_.hip_stream(), src_base, out, idxs, N, block_size);
  return true;
}
...
void ScatterAssignOp<HIPContext>::DoScatterAssign(
    ...
    TIndex K,
    TIndex block_size) {
  hipLaunchKernelGGL((scatter_assign_kernel), dim3(std::min(K, static_cast<TIndex>(CAFFE_MAXIMUM_NUM_BLOCKS))), dim3(CAFFE_HIP_NUM_THREADS), 0, context_.hip_stream(), data, idxs, slicesData, N, K, block_size);
}
...
bool RangeOp<HIPContext>::DoRunOnDevice(
  ...
  int N = output->size();
  hipLaunchKernelGGL((RangeKernel), dim3(CAFFE_GET_BLOCKS(N)), dim3(CAFFE_HIP_NUM_THREADS), 0, context_.hip_stream(), N, output->mutable_data<T>(), start, step);
  return true;
}
