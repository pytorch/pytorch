...
__global__ void
ComputeIndicesKernel(const int numMasks, const int maskSize, int* indices, bool* const masks[])
{
...
__global__ void FillValuesKernel(const int numMasks,
                                 const int maskSize,
...
{
    public:
    BooleanUnmaskOp(const OperatorDef& def, Workspace* ws) : Operator<HIPContext>(def, ws) {}
