...
__global__ void WeightedSampleKernel(const int batch_size,
...
bool WeightedSampleOp<float, HIPContext>::RunOnDevice()
{
    ...
    if(batch_size > 0 && weights_dim > 0)
    {
        ...
        {
            auto& in_val = Input(1);
            CAFFE_ENFORCE_EQ(in_weights.dims(),
                             in_val.dims(),
                             "The sampling weights tensor and "
                             "the sampling values tensor must "
        ...
        HIPRAND_ENFORCE(
            hiprandGenerateUniform(context_.hiprand_generator(), unif_samples_data, batch_size));

        hipLaunchKernelGGL((hip_ops::WeightedSampleKernel),
