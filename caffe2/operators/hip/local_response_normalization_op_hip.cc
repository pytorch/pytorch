...
bool LRNOp<float, HIPContext>::RunOnDeviceWithOrderNCHW() {
  ...
  int n_threads = N * H * W;
  hipLaunchKernelGGL((LRNFillScaleNCHW<float>), dim3(CAFFE_GET_BLOCKS(n_threads)), dim3(CAFFE_HIP_NUM_THREADS), 0, context_.hip_stream(), 
      n_threads, Xdata, N, C, H, W, size_, alpha_ / size_, bias_, scale_data);
  n_threads = X.size();
  hipLaunchKernelGGL((LRNComputeOutput<float>), dim3(CAFFE_GET_BLOCKS(n_threads)), dim3(CAFFE_HIP_NUM_THREADS), 0, context_.hip_stream(), 
...
bool LRNOp<float, HIPContext>::RunOnDeviceWithOrderNHWC() {
  ...
  hipLaunchKernelGGL((LRNFillScaleNHWC<float>), dim3(CAFFE_GET_BLOCKS(n_threads)), dim3(CAFFE_HIP_NUM_THREADS), 0, context_.hip_stream(), 
...
bool LRNGradientOp<float, HIPContext>::RunOnDeviceWithOrderNCHW() {
  ...
  hipLaunchKernelGGL((LRNComputeDiffNCHW<float>), dim3(CAFFE_GET_BLOCKS(n_threads)), dim3(CAFFE_HIP_NUM_THREADS), 0, context_.hip_stream(), 
...
bool LRNGradientOp<float, HIPContext>::RunOnDeviceWithOrderNHWC() {
  ...
  hipLaunchKernelGGL((LRNComputeDiffNHWC<float>), dim3(CAFFE_GET_BLOCKS(X.size())), dim3(CAFFE_HIP_NUM_THREADS), 0, context_.hip_stream(), 
      static_cast<const int>(X.size()), X.data<float>(), Y.data<float>(), scale_data,
      dY.data<float>(),
