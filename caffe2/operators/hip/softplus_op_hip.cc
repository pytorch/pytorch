...
{
    HIP_1D_KERNEL_LOOP(i, N) { Y[i] = log(exp(X[i]) + 1.0f); }
}

template <typename T>
__global__ void SoftplusGradientKernel(const int N, const T* Y, const T* dY, T* dX)
{
...
REGISTER_HIP_OPERATOR(SoftplusGradient, SoftplusGradientOp<float, HIPContext>);
} // namespace caffe2
