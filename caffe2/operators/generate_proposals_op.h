...
struct GenerateProposalsArguments {
  GenerateProposalsArguments(const OperatorBase& op)
    : spatial_scale_(op.GetSingleArgument<float>("spatial_scale", 1.0 / 16)),
      feat_stride_(1.0 / spatial_scale_),
      rpn_pre_nms_topN_(op.GetSingleArgument<int>("pre_nms_topN", 6000)),
      rpn_post_nms_topN_(op.GetSingleArgument<int>("post_nms_topN", 300)),
      rpn_nms_thresh_(op.GetSingleArgument<float>("nms_thresh", 0.7f)),
      rpn_min_size_(op.GetSingleArgument<float>("min_size", 16)),
      correct_transform_coords_(
          op.GetSingleArgument<bool>("correct_transform_coords", false)),
      angle_bound_on_(op.GetSingleArgument<bool>("angle_bound_on", true)),
      angle_bound_lo_(op.GetSingleArgument<int>("angle_bound_lo", -90)),
      angle_bound_hi_(op.GetSingleArgument<int>("angle_bound_hi", 90)),
      clip_angle_thresh_(
          op.GetSingleArgument<float>("clip_angle_thresh", 1.0)) {}
...
class GenerateProposalsOp final : public Operator<Context> {
 public:
  ...
  GenerateProposalsOp(const OperatorDef& operator_def, Workspace* ws)
      : Operator<Context>(operator_def, ws), args_(*this) {}
