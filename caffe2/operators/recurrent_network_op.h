 ...
 public:
  ...
  RecurrentNetworkOp(const OperatorDef& operator_def, Workspace* ws)
    ...
    detail::AddApplyLinkOps(
        links_, timestep_, def().device_option(), &stepNetDef_);
 ...
 public:
  ...
  void AddParamGradientAccumulationOps() {
    ...
    for (const auto& param : params_) {
      auto g = param.cellGradient.empty() ? param.grad : param.cellGradient;
      OperatorDef opdef;
      opdef.set_type("Sum");
      opdef.add_input(param.accGrad);
      opdef.add_input(g);
      opdef.add_output(param.accGrad);
      opdef.mutable_device_option()->CopyFrom(def().device_option());
      stepNetDef_.add_op()->CopyFrom(opdef);
      stepNetDef_.add_external_input(param.accGrad);
    }
  }
...
template <typename T, class Context>
class RNNAccumulateInputGradientOp : public Operator<Context> {
 public:
  RNNAccumulateInputGradientOp(const OperatorDef& def, Workspace* ws)
