...
#endif
      {
        ...
        for (; j < j_end_aligned; j += VLEN) {
          ...
          __m256 transformed_v = _mm256_fmadd_ps(
              acc_v,
              _mm256_set1_ps(1.0 / out_qparams_.scale),
...
template class SumDNNLowPOp<uint8_t, false>;
template class SumDNNLowPOp<uint8_t, true>;
template class SumDNNLowPOp<uint16_t, false>;
template class SumDNNLowPOp<uint16_t, true>;
