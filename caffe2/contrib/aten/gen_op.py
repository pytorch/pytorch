...
parser.add_argument(
    "--yaml_dir", default="aten/src/ATen/ATen", help="where ATen yaml files are"
)
...
def supports(o, factory_methods):
    ...
    for ret in o["returns"]:
        if not value_has_tensors(ret) and ret["type"] not in RETURN_MAP:
            print(
                "Skipping {} Because of Ret: {} ({})".format(
    ...
    for arg in o["arguments"]:
        if not value_has_tensors(arg) and arg["type"] not in ARGUMENT_MAP:
            print(
                "Skipping {} Because of Arg: {} ({}) ".format(
...
def required_attribute_names(o):
    return sorted(
        [
            ...
            if not value_has_tensors(a) and "default" not in a
        ]
    )
...
def main() -> None:
    decls = yaml.load(
        read(os.path.join(args.yaml_dir, "Declarations.yaml")), Loader=Loader
    )
    factory_methods = find_factory_methods(decls)
    filtered = [
        expanded
        for o in decls
        for expanded in expand(o)
        if supports(expanded, factory_methods)
    ]
    ...
    for o in filtered:
        ...
        static_tensor_inputs = sum(
            arg["type"] not in TENSORLIST_TYPE and value_is_tensor_type(arg)
            for arg in o["arguments"]
        )
        ...
        if has_tensorlist:
            tensorlist_idx = [
                ...
                if arg["type"] in TENSORLIST_TYPE
            ][0]
        ...
        for i, arg in enumerate(o["arguments"]):
            ...
            view_length = (
                "InputSize()"
                if has_tensorlist and i < tensorlist_idx
                else static_tensor_inputs
            )
            if (
                arg["type"] == "at::TensorList"
                or arg["type"] == "const at::ITensorListRef &"
            ):
                # NOTE: do not advance real_inputs here. After this we will
                # switch to indexing the "stack" from the end
                env["statements"].append(
                    "auto {} = peekSlice({}, InputSize() - {}, InputSize());".format(
                        arg["name"], real_inputs, static_tensor_inputs
                    )
                )
                ...
                # switch to indexing the "stack" from the end
                env["statements"].append(
                    "auto {} = peekSliceOptionals({}, InputSize() - {}, InputSize());".format(
                        arg["name"], real_inputs, static_tensor_inputs
                    )
                )
        ...
        key += 1
    write(
        os.path.join(args.install_dir, args.output_prefix + "aten_op.h"),
