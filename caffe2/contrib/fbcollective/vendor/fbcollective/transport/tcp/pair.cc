...
void Pair::connect(const Address& peer) {
  ...
  if (self_.ss_.ss_family == AF_INET) {
    ...
    FBC_ENFORCE(false, "unknown sa_family");
  }

  FBC_ENFORCE_NE(rv, 0, "cannot connect to self");
  ...
  FBC_ENFORCE_NE(fd_, -1, "socket: ", strerror(errno));

  // Connect to peer
  rv = ::connect(fd_, (struct sockaddr*)&peer_.ss_, addrlen);
  if (rv == -1 && errno != EINPROGRESS) {
    FBC_ENFORCE_NE(rv, -1, "connect: ", strerror(errno));
  }
...
bool Pair::write(Op& op) {
  ...
  FBC_ENFORCE_NE(rv, -1, "writev: ", strerror(errno));
  op.nwritten_ += rv;
  if (rv < nbytes) {
    return false;
  }

  FBC_ENFORCE_EQ(rv, nbytes);
  return true;
}
...
bool Pair::read(Op& op) {
  for (;;) {
    ...
    if (op.nread_ == sizeof(op.preamble_)) {
      FBC_ENFORCE_NE(op.preamble_.length_, 0);
    }
...
void Pair::handleEvents(int events) {
  ...
  FBC_ENFORCE(false, "Unexpected state: ", state_);
}
...
void Pair::handleListening() {
  ...
  FBC_ENFORCE_NE(rv, -1, "accept: ", strerror(errno));
...
void Pair::handleConnecting() {
  ...
  FBC_ENFORCE_NE(rv, -1);
  FBC_ENFORCE_EQ(optval, 0, "SO_ERROR: ", strerror(optval));
...
std::unique_ptr<::fbcollective::transport::Buffer>
Pair::createSendBuffer(int slot, void* ptr, size_t size) {
  auto buffer = new Buffer(this, slot, ptr, size);
  return std::unique_ptr<::fbcollective::transport::Buffer>(buffer);
}
