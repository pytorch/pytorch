...
bool OnnxifiTransformer::supportOpOnnx(
  ...
  try {
    ...
    if (denylisted_ops.count(pos)) {
      LOG(INFO) << "Skipping denylisted op " << op.type() << " at pos " << pos;
      return false;
    }
  ...
  for (const auto& op : net.op()) {
    ...
    if (op.type() == "Gather") {
      ...
      auto supported = opts_.use_onnx
          ? supportOpOnnx(op, &exporter, *denylisted_ops, backend_id)
          : supportOpC2(op, shape_hints, weights, *denylisted_ops, backend_id);
    ...
    if (!check.empty()) {
      ...
      denylisted_ops->emplace(it->second);
      // We know that current op is not going to be supported. Might as well
      // denylist it too
      denylisted_ops->emplace(
          ArgumentHelper::GetSingleArgument<OperatorDef, int>(op, kNetPos, -1));
    }
  }
}

void OnnxifiTransformer::denylistCpuPartition(
    const NetDef& net,
    std::unordered_set<int>* denylisted_ops) const {
...
void OnnxifiTransformer::applyFilteringRules(
  ...
  tieGatherAndSparseLengthsWeightedSumOps(
      net, shape_hints, weights, denylisted_ops);
  denylistCpuPartition(net, denylisted_ops);
}
...
NetDef OnnxifiTransformer::TransformViaC2(
  ...
  auto c2_supports = [this,
    ...
    return supportOpC2(op, shape_hints, weights, denylisted_ops, backend_id);
  };
...
NetDef OnnxifiTransformer::TransformViaOnnx(
  ...
  onnx::OnnxExporter exporter(nullptr);
  auto onnx_supports = [this, &exporter, &denylisted_ops, backend_id](
                           const caffe2::OperatorDef& op) {
    return supportOpOnnx(op, &exporter, denylisted_ops, backend_id);
  };
...
void OnnxifiTransformer::transform(
  ...
  std::unordered_set<int> new_denylisted_ops(
      denylisted_ops.begin(), denylisted_ops.end());
  applyFilteringRules(*pred_net, shape_hints, weights, &new_denylisted_ops);

  // Transform the net
  NetDef net_opt = opts_.use_onnx
      ? TransformViaOnnx(
            ws, pred_net, weights, new_denylisted_ops, &shape_hints)
      : TransformViaC2(pred_net, weights, new_denylisted_ops, shape_hints);
