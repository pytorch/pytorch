  ...
  if (!FLAGS_onnxifi_shape_hints.empty()) {
    ...
    for (const auto& hint : hints) {
      ...
      if (kv.size() == 2) {
        ...
        if (valid) {
          // We treat hinted shapes as BATCH. If there are shape hints on blobs
          // in the workspace, since they are already inserted as CONSTANT, it
          // will take effect here. For SEQ typed tensors, there are only a few
          // of them and they will be handled by BoundShapeInferencer.
          std::vector<TensorBoundShape_DimType> dimType(
              input.dims_size(), TensorBoundShape_DimType_CONSTANT);
          if (dimType.size()) {
            dimType[0] = TensorBoundShape_DimType_BATCH;
          }
          more_shape_hints.emplace(
              std::piecewise_construct,
              std::forward_as_tuple(kv.front()),
              std::forward_as_tuple(dimType, input));
        }
      } else {
        CAFFE_THROW("Cannot parse shape hint: ", hint);
      }
