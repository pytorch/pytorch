...
// The list in in the form of "0-3,5,6-7" which means, we will block ops
...
void onnxifi(
  ...
  if (!opts.use_onnx) {
    auto blocked_ops = ParseBlockListOps(FLAGS_onnxifi_blocklist_ops);
    for (const auto& op : net->op()) {
      if (blocked_ops.count(op.type())) {
