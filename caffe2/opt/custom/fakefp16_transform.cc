...
void fakeFp16Transform(NetDef* net) {
  ...
  auto denylist_pos = glow::ParseNetPositionList(FLAGS_onnxifi_blacklist);
  auto denylist_type = glow::ParseBlackListOps(FLAGS_onnxifi_blacklist_ops);
  ...
  for (int i = 0; i < net->op().size(); ++i) {
    ...
    if (denylist_pos.count(net_pos) || denylist_type.count(op->type())) {
      continue;
    }
