...
class CAFFE2_API OnnxifiTransformer final : public BackendTransformerBase {
 public:
  ...
  void transform(
      ...
      const std::unordered_set<int>& skiplisted_ops) override;
 ...
 private:
  ...
  NetDef TransformViaC2(
      ...
      const std::unordered_set<int>& skiplisted_ops,
      const ShapeInfoMap& shape_hints);
  ...
  void tieGatherAndSparseLengthsWeightedSumOps(
      ...
      const ShapeInfoMap& shape_hints,
      const std::unordered_set<std::string>& weights,
      std::unordered_set<int>* skiplisted_ops) const;

  // For net with partitioning info, skiplist ops that are supposed to run on
  // CPU, whose partition info will contain empty device_id list.
  void skiplistCpuPartition(
      const NetDef& net,
      std::unordered_set<int>* skiplisted_ops) const;
