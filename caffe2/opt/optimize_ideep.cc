...
void preConvertFiltersFormat(repr::NNModule* nn, caffe2::Workspace* ws) {
  for (auto node_pair : repr::nn::dataIterator<repr::Conv>(nn->dataFlow)) {
    ...
    auto* filter = getTensor<itensor>(blob);
    if (filter == nullptr) {
      assert(filter && "Filter not a IDEEP tensor");
      continue;
    }
...
void OptimizeForIdeep(repr::NNModule* nn, caffe2::Workspace* ws, bool training_mode) {
  ...
  // Must be called behind all optimizations
  // because the internal format of filters have been changed.
