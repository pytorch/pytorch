...
void TvmTransformer::transform(
  ...
  NetDef net_opt =
      applyTvmTransform(pred_net, weights, denylisted_ops, shape_hints);
...
bool TvmTransformer::canConvertFullGraph(
  ...
  for (const auto& op : net.op()) {
    ...
    if (denylisted_ops.count(pos) || supported_ops.count(op.type()) == 0) {
      return false;
    }
  }
  return true;
...
NetDef TvmTransformer::applyTvmTransform(
  ...
  auto tvm_supports = [&denylisted_ops, &shape_hints, &profiling_based_jit](
                          const caffe2::OperatorDef& op) {
    const auto& supported_ops = getSupportedOps();
    try {
      // If the op position is denylisted, return false
      int pos =
          ArgumentHelper::GetSingleArgument<OperatorDef, int>(op, kNetPos, -1);
      if (denylisted_ops.count(pos)) {
        LOG(INFO) << "Denylisting op" << op.type() << " at position " << pos;
        return false;
      }
...
void tvmTransform(
  ...
  ts.transform(ws, net, weight_names, shape_hints, denylisted_ops);
}
