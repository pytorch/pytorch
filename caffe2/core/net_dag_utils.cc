...
void prune(int node_idx, std::vector<OpGraphNode>& nodes) {
  ...
  while (!nodes_stack.empty()) {
    ...
    CAFFE_ENFORCE((unsigned)curr < ancestors.size(), "Out of bound access");
    if (ancestors[curr]) {
...
std::vector<OpGraphNode> prepareChainGraphNodes(
  ...
  for (size_t chain_idx = 0; chain_idx < execution_chains.size(); ++chain_idx) {
  ...
  for (size_t op_idx = 0; op_idx < operator_nodes.size(); ++op_idx) {
