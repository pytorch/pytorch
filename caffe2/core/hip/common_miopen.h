...
/**
 * miopenTypeWrapper is a wrapper class that allows us to refer to the miopen
 * type in a template function. The class is specialized explicitly for
 * different data types below.
 */
template <typename T>
 ...
 * miopenTensorDescriptor_t, allowing us to do descriptor change as-needed
 * during runtime.
 */
 ...
 public:
  ...
  inline miopenTensorDescriptor_t Descriptor(
    ...
    MIOPEN_ENFORCE(miopenSet4dTensorDescriptor(
        desc_, type, dims_[0], dims_[1], dims_[2], dims_[3]));
    if (changed)
      *changed = true;
    return desc_;
  }
