...
class ThreadLocalHIPObjects
{
    ...
    hipStream_t GetStream(int gpu, int stream_id)
    {
        ...
        if(!gpu_streams[stream_id])
        {
            ...
            HIP_ENFORCE(hipStreamCreateWithFlags(&gpu_streams[stream_id], hipStreamNonBlocking));
        }
    ...
    rocblas_handle GetHandle(int gpu, int stream_id)
    {
        ...
        if(!gpu_handles[stream_id])
        {
            ...
            // caution.
            ROCBLAS_ENFORCE(
                rocblas_set_pointer_mode(gpu_handles[stream_id], rocblas_pointer_mode_host));
            ROCBLAS_ENFORCE(rocblas_set_stream(gpu_handles[stream_id], GetStream(gpu, stream_id)));
        }
    ...
    miopenHandle_t GetMiopenHandle(int gpu, int stream_id)
    {
        ...
        if(!gpu_handles[stream_id])
        {
            ...
            MIOPEN_ENFORCE(miopenSetStream(gpu_handles[stream_id], GetStream(gpu, stream_id)));
        }
        return gpu_handles[stream_id];
    }
...
class HIPContext final
{
    ...
    inline hipStream_t hip_stream() const { return hip_stream(gpu_id_, stream_id_); }
    ...
    rocblas_handle get_rocblas_handle() { return hip_objects_.GetHandle(gpu_id_, stream_id_); }

    miopenHandle_t miopen_handle() { return hip_objects_.GetMiopenHandle(gpu_id_, stream_id_); }
    ...
    hiprandGenerator_t& hiprand_generator()
    {
        if(!hiprand_generator_)
        {
            ...
            HIPRAND_ENFORCE(hiprandSetPseudoRandomGeneratorSeed(hiprand_generator_, random_seed_));
    ...
    inline void CopyBytes(size_t nbytes, const void* src, void* dst)
    {
        ...
        HIP_ENFORCE(hipMemcpyAsync(
            dst, src, nbytes, hipMemcpyDefault, hip_objects_.GetStream(gpu_id_, stream_id_)));
    }
...
template <>
inline void CPUContext::CopyBytes<HIPContext, CPUContext>(size_t nbytes, const void* src, void* dst)
{
    HIPContext context(GetGPUIDForPointer(src));
    context.CopyBytes<HIPContext, CPUContext>(nbytes, src, dst);
}
template <>
inline void CPUContext::CopyBytes<CPUContext, HIPContext>(size_t nbytes, const void* src, void* dst)
{
