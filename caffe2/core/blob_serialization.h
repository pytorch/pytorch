...
/*
 * Get an empty Tensor from the TensorProto given the meta data in proto (data type
 * and size of the Tensor) without actually filling in the data.
 *
 * We need this function because we want to construct a fully initialized Tensor in the
 * beginning instead of keeping partially initialized Tensor around the process.
 * Consider the case when we have a Tensor that is split into multiple protos during serialization,
 * in deserialization, we have to fill the Tensor in multiple calls to Deserialize,
 * therefore we need to create a new Tensor with the correct size and data type before the call to Deserialize, because
 * otherwise we will have to check whether the function call is the first call to initialize
 * the underlying Tensor, which makes the function stateful and complicated.
 *
 * The legacy code get away with this problem by passing in a partially initialized Tensor
 * and use Resize and mutable_data to set the correct size, data type and allocate
 * memory for the Tensor, so the state is encoded in these function calls. e.g. mutable_data
 * will allocate memory on the first call and it will return a pointer to the allocated
...
class CAFFE2_API TensorDeserializer : public BlobDeserializerBase {
 public:
   ...
   * Tensor, each call will fill part of the Tensor given the segment begin and end
   * information in proto, therefore we have to pass in the Tensor pointer rather
   * than create a new Tensor everytime.
   *
