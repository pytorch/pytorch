...
class CAFFE2_API OperatorBase : public Observable<OperatorBase> {
 public:
  ...
  inline Tensor
  XOutputTensor(int idx, at::IntListRef dims, at::TensorOptions options) {
  ...
  inline Tensor*
  OutputTensor(int idx, at::IntListRef dims, at::TensorOptions options) {
    if (isLegacyOperator()) {
      CAFFE_ENFORCE_WITH_CALLER(
          options.device_opt() != c10::nullopt,
...
class Operator : public OperatorBase {
 public:
  ...
  Tensor XOutput(int idx, at::IntListRef dims, at::TensorOptions options) {
  ...
  ///
  Tensor* Output(int idx, at::IntListRef dims, at::TensorOptions options) {
    // We'll default device to the device of the current Operator Context
    if (options.device_opt() == c10::nullopt) {
