...
class CAFFE2_API Tensor final {
 ...
 public:
   ...
   */
  explicit Tensor(at::IntListRef dims, DeviceType type) : Tensor(type) {
  ...
  explicit Tensor(at::IntListRef dims, at::Device device): Tensor(device) {
    Resize(dims);
  }
  ...
  inline at::IntListRef sizes() const {
  ...
  inline at::IntListRef strides() const {
 ...
 */
CAFFE2_API void ReinitializeTensor(Tensor* t, at::IntListRef dims, at::TensorOptions options);
...
CAFFE2_API Tensor empty(at::IntListRef dims, at::TensorOptions options);
...
// types are supported
template <typename T>
Tensor TensorCPUFromValues(at::IntListRef dims, at::ArrayRef<T> values) {
