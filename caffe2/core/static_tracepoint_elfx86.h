...
#define CAFFE_SDT_ASM_3(a, b, c) \
  CAFFE_SDT_S(a) "," CAFFE_SDT_S(b) "," CAFFE_SDT_S(c) "\n"
...
#define CAFFE_SDT_PROBE(provider, name, n, arglist) \
  __asm__ __volatile__(CAFFE_SDT_NOTE_CONTENT(      \
      provider,                                     \
      name,                                         \
      CAFFE_SDT_ARG_TEMPLATE_##n)::CAFFE_SDT_OPERANDS_##n arglist)
