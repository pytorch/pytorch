...
def OpAlmostEqual(op_a, op_b, ignore_fields=None):
    ...
    assert all(
        isinstance(f, text_type) for f in ignore_fields
...
def Caffe2TensorToNumpyArray(tensor):
    if tensor.data_type == caffe2_pb2.TensorProto.FLOAT:
        ...
        return np.asarray(tensor.int64_data, dtype=np.int64).reshape(tensor.dims)
...
def MakeArgument(key, value):
    ...
    if type(value) is float:
        ...
        argument.floats.extend(v.item() if type(v) is np.float_ else v for v in value)
    elif iterable and all(
        type(v) in integer_types or type(v) in [bool, np.int_] for v in value
    ):
        argument.ints.extend(v.item() if type(v) is np.int_ else v for v in value)
    elif iterable and all(
        isinstance(v, binary_type) or isinstance(v, text_type) for v in value
    ):
        argument.strings.extend(
            v.encode("utf-8") if isinstance(v, text_type) else v for v in value
        )
    ...
    if not init_net.BlobIsDefined(iter):
        ...
        with core.DeviceScope(core.DeviceOption(caffe2_pb2.CPU)):
            iteration = init_net.ConstantFill(
                [], iter, shape=[1], value=iter_val, dtype=core.DataType.INT64
            )
    ...
    for k in dir(cls):
        if k == k.upper():
            ...
            if isinstance(v, string_types):
                assert v not in list(enum.values()), (
