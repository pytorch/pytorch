...
class Caffe2Backend(Backend):
    ...
    def optimize_onnx(input, init=False, predict=False):
        ...
        try:
            ...
            warnings.warn("OptimizerWarning: optimizer module not found in ONNX version {}".format(onnx.__version__), stacklevel=1)
    ...
    def prepare(cls, model, device='CPU', raw_values_dict=None, **kwargs):
        ...
        for imp in model.opset_import:
            if not imp.HasField("domain") or imp.domain == "":
                ...
                if imp.version > cls._known_opset_version:
                    warnings.warn("This version of onnx-caffe2 targets ONNX operator set version {}, but the model we are trying to import uses version {}.  We will try to import it anyway, but if the model uses operators which had BC-breaking changes in the intervening versions, import will fail.".format(cls._known_opset_version, imp.version), stacklevel=1)
            else:
                warnings.warn("Unrecognized operator set {}".format(imp.domain), stacklevel=1)
        ...
        try:
            ...
            warnings.warn("ShapeInferenceWarning: Inferred shape and existing shape differ in rank", stacklevel=1)
    ...
    def _onnx_model_to_caffe2_net(cls, onnx_model, device, opset_version, include_initializers):
        ...
        try:
            ...
            warnings.warn("ShapeInferenceWarning: utils module not found in ONNX version {}".format(onnx.__version__), stacklevel=1)
