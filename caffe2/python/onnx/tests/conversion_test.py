...
class TestConversion(TestCase):
    ...
    def test_caffe2_to_onnx_value_info(self):
        ...
        six.assertRaisesRegex(
            self, Exception, "value info", self._run_command, caffe2_to_onnx, args
        )
    ...
    def test_onnx_to_caffe2_zipfile(self):
        ...
        graph_def = helper.make_graph(
            ...
            initializer=[
                helper.make_tensor(
                    "W", TensorProto.FLOAT, [3, 2], b"__EXTERNAL", raw=True
                )
            ],
        )
        model_def = helper.make_model(graph_def, producer_name="onnx-to-caffe2-test")
    ...
    def _make_fake_loop_op(self, body_nodes, input_types, output_types):
        ...
        graph_inputs = [
            helper.make_tensor_value_info("i", TensorProto.INT32, ()),
    ...
    def test_convert_end2end(self):
        ...
        predict_net.op.extend(
            [core.CreateOperator("FC", inputs=[x, w, b], outputs=[y], axis=2)]
        )
        ...
        init_net.op.extend(
            [
                core.CreateOperator(
                    "GivenTensorFill", [], [w], values=w_val, shape=w_val.shape
                ),
                core.CreateOperator(
                    "GivenTensorFill", [], [b], values=b_val, shape=b_val.shape
                ),
            ]
        )
        ...
        for _ in range(5):
            self._run_command(
                ...
                [
                    ...
                    onnx_model_f.name,
                    "--value-info",
                    json.dumps({x: (TensorProto.FLOAT, (1, 3, 2))}),
                ],
                catch_exceptions=False,
            )
