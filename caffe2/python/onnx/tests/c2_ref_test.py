...
class TestCaffe2Basic(TestCase):
    ...
    def test_check_arguments(self):
        ...
        with six.assertRaisesRegex(
            self, RuntimeError, "Don't know how to map unexpected argument (foo|bar)"
        ):
    ...
    def test_elementwiselinear(self):
        ...
        predict_net.op.extend(
            [
                core.CreateOperator(
                    "ElementwiseLinear", inputs=["X", "W", "B"], outputs=["Y"], axis=3
                )
            ]
        )
    ...
    def test_mergedim(self):
        ...
        predict_net.external_output[:] = ["Y"]
        predict_net.op.extend(
            [core.CreateOperator("MergeDim", inputs=["X"], outputs=["Y"])]
        )
        ws, c2_outputs = c2_native_run_net(
            init_net=None, predict_net=predict_net, inputs=[X]
        )
    ...
    def test_slice(self):
        ...
        predict_net.op.extend(
            [
                core.CreateOperator(
                    "Slice", inputs=["X"], outputs=["Y"], starts=starts, ends=ends
                )
            ]
        )
        ws, c2_outputs = c2_native_run_net(
    ...
    def test_cast(self):
        ...
        for to_type in [
            ...
            predict_net.external_output[:] = ["Y"]
            predict_net.op.extend(
                [core.CreateOperator("Cast", inputs=["X"], outputs=["Y"], to=to_type)]
            )
            ws, c2_outputs = c2_native_run_net(
                init_net=None, predict_net=predict_net, inputs=[X]
            )
...
class TestCaffe2End2End(TestCase):
    ...
    def _test_net(self, net_name, input_blob_dims=(1, 3, 224, 224), decimal=7):
        ...
        model = c2_onnx.caffe2_net_to_onnx_model(
            predict_net=c2_predict_net, init_net=c2_init_net, value_info=value_info
        )
    ...
    @unittest.skipIf(os.environ.get("JENKINS_URL"), "Taking too long to download!")
    def test_vgg16(self):
