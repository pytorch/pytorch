...
class TestFrontendSSAConversion(TestCase):
    def test_ssa(self):
        ...
        net.op.extend(
            [
                ...
                core.CreateOperator("Add", ["Y", "s"], ["Y"], broadcast=True),
            ]
        )
        ...
        init_net.op.extend(
            [
                core.CreateOperator(
                    "GivenTensorFill", [], ["W"], values=W, shape=W.shape
                ),
                core.CreateOperator(
                    "GivenTensorFill", [], ["b"], values=b, shape=b.shape
                ),
                core.CreateOperator(
                    "GivenTensorFill", [], ["s"], values=s, shape=s.shape
                ),
            ]
        )
        ...
        _, orig_output = c2_native_run_net(
            predict_net=net, init_net=init_net, inputs=[X]
        )
    ...
    def test_idempotence(self):
        ...
        net.op.extend(
            [
                core.CreateOperator("FC", ["X", "W", "b"], ["Y"]),
                core.CreateOperator("Add", ["Y", "s"], ["Z"], broadcast=True),
            ]
        )
