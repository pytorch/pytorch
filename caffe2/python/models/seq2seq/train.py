...
def prepare_batch(batch):
    ...
    return Batch(
        encoder_inputs=np.array(batch_encoder_inputs, dtype=np.int32).transpose(),
        encoder_lengths=np.array(encoder_lengths, dtype=np.int32),
        decoder_inputs=np.array(batch_decoder_inputs, dtype=np.int32).transpose(),
        decoder_lengths=np.array(decoder_lengths, dtype=np.int32),
        targets=np.array(batch_targets, dtype=np.int32).transpose(),
        target_weights=np.array(batch_target_weights, dtype=np.float32).transpose(),
    )
...
class Seq2SeqModelCaffe2(object):
    def _build_model(self, init_params):
        ...
        self.norm_clipped_sparse_grad_update(
            model, scope="norm_clipped_sparse_grad_update"
        )
        self.model = model
    ...
    def _build_shared(self, model):
        ...
        with core.DeviceScope(core.DeviceOption(caffe2_pb2.CPU)):
            ...
            self.global_step = model.AddParam(
                name="global_step", init_value=0, trainable=False
            )
            self.start_time = model.AddParam(
                name="start_time", init_value=time.time(), trainable=False
            )
    ...
    def model_build_fun(self, model, forward_only=False, loss_scale=None):
        ...
        targets, _ = model.net.Reshape(
            [targets], ["targets", "targets_old_shape"], shape=[-1]
        )
        target_weights, _ = model.net.Reshape(
            [target_weights], ["target_weights", "target_weights_old_shape"], shape=[-1]
        )
        ...
        total_loss_scalar = model.net.Mul(
            [loss_per_word, num_words], "total_loss_scalar"
        )
        ...
        with core.NameScope(scope):
            ...
            for i, param in enumerate(params):
                ...
                grad_squared_sum = model.net.SumElements(
                    grad_squared, "grad_{}_squared_sum".format(i)
                )
    ...
    def __init__(
        self, model_params, source_vocab_size, target_vocab_size, num_gpus=1, num_cpus=1
    ):
    ...
    def save(self, checkpoint_path_prefix, current_step):
        ...
        checkpoint_config_path = os.path.join(
            os.path.dirname(checkpoint_path_prefix), "checkpoint"
        )
        ...
        for source_sentence, target_sentence in zip(source, target):
            ...
            if (
                ...
                parallel_sentences.append(
                    (numerized_source_sentence, numerized_target_sentence)
                )
...
def run_seq2seq_model(args, model_params=None):
    source_vocab = seq2seq_util.gen_vocab(args.source_corpus, args.unk_threshold)
    target_vocab = seq2seq_util.gen_vocab(args.target_corpus, args.unk_threshold)
    ...
    with Seq2SeqModelCaffe2(
        ...
        for i in range(args.epochs):
            ...
            for batch in batches:
                total_loss += model_obj.step(batch=batch, forward_only=False)
            logger.info("\ttraining loss {}".format(total_loss))
            total_loss = 0
            for batch in batches_eval:
                total_loss += model_obj.step(batch=batch, forward_only=True)
...
def main():
    ...
    encoder_layer_configs = [
        dict(num_units=args.encoder_cell_num_units)
    ...
    decoder_layer_configs = [
        dict(num_units=args.decoder_cell_num_units)
    ...
    run_seq2seq_model(
        ...
        model_params=dict(
            ...
            optimizer_params=dict(learning_rate=args.learning_rate),
