...
def _weighted_sum(model, values, weight, output_name):
    ...
    return model.net.WeightedSum(values_weights_flattened, output_name)
...
class Seq2SeqModelCaffe2EnsembleDecoder(Seq2SeqModelCaffe2EnsembleDecoderBase):
    ...
    def _build_decoder(
        ...
        with core.NameScope(scope):
            ...
            embedded_tokens_t_prev = step_model.net.Gather(
                [decoder_embeddings, previous_tokens], "embedded_tokens_t_prev"
            )
        ...
        state_configs = [
            BeamSearchForwardOnly.StateConfig(
                ...
                state_prev_link=BeamSearchForwardOnly.LinkConfig(
                    blob=state_prev, offset=0, window=1
                ),
                state_link=BeamSearchForwardOnly.LinkConfig(
                    blob=state, offset=1, window=1
                ),
            )
            for initial_state, state_prev, state in zip(
                initial_states, states_prev, states
            )
        ]
        ...
        with core.NameScope(scope):
            ...
            if use_attention:
                ...
                attention_weights = step_model.net.ConstantFill(
                    [self.encoder_inputs], "zero_attention_weights_tmp_1", value=0.0
                )
                attention_weights = step_model.net.Transpose(
                    attention_weights, "zero_attention_weights_tmp_2"
                )
    ...
    def __init__(self, translate_params):
        ...
        num_decoders_blob = self.model.param_init_net.ConstantFill(
            [], "num_decoders_blob", value=1 / float(len(output_log_probs)), shape=[1]
        )
...
def run_seq2seq_beam_decoder(args, model_params, decoding_params):
    ...
    for line in sys.stdin:
        ...
        translation, alignment, _ = decoder.decode(
            numerized_source_sentence, 2 * len(numerized_source_sentence) + 5
        )
        print(" ".join([inversed_target_vocab[tid] for tid in translation]))


def main():
    parser = argparse.ArgumentParser(description="Caffe2: Seq2Seq Translation")
    parser.add_argument(
        "--source-corpus",
        type=str,
        default=None,
