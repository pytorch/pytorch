...
def rnn_unidirectional_layer(
    ...
    with core.NameScope(scope):
        initial_cell_state = model.param_init_net.ConstantFill(
            [], "initial_cell_state", shape=[num_units], value=0.0
        )
        initial_hidden_state = model.param_init_net.ConstantFill(
            [], "initial_hidden_state", shape=[num_units], value=0.0
        )
...
def rnn_bidirectional_layer(
    ...
    with core.NameScope(scope):
        reversed_inputs = model.net.ReversePackedSegs(
            [inputs, input_lengths], ["reversed_inputs"]
        )
    ...
    with core.NameScope(scope):
        outputs_bw = model.net.ReversePackedSegs(
            [outputs_bw, input_lengths], ["outputs_bw"]
        )
    ...
    if return_sequence_output:
        with core.NameScope(scope):
            outputs, _ = model.net.Concat(
                [outputs_fw, outputs_bw], ["outputs", "outputs_dim"], axis=2
            )
    else:
        outputs = None
...
def build_embeddings(model, vocab_size, embedding_size, name, freeze_embeddings):
    embeddings = model.param_init_net.GaussianFill(
        [], name, shape=[vocab_size, embedding_size], std=0.1
    )
    ...
    with core.NameScope(scope or ""):
        if num_gpus == 0:
            embedded_encoder_inputs = model.net.Gather(
                [embeddings, inputs], ["embedded_encoder_inputs"]
            )
        else:
            with core.DeviceScope(core.DeviceOption(caffe2_pb2.CPU)):
                embedded_encoder_inputs_cpu = model.net.Gather(
                    [embeddings, inputs], ["embedded_encoder_inputs_cpu"]
                )
            embedded_encoder_inputs = model.CopyCPUToGPU(
                embedded_encoder_inputs_cpu, "embedded_encoder_inputs"
            )
    ...
    with core.NameScope(scope or ""):
        if num_gpus == 0:
            embedded_decoder_inputs = model.net.Gather(
                [embeddings, inputs], ["embedded_decoder_inputs"]
            )
        else:
            with core.DeviceScope(core.DeviceOption(caffe2_pb2.CPU)):
                embedded_decoder_inputs_cpu = model.net.Gather(
                    [embeddings, inputs], ["embedded_decoder_inputs_cpu"]
                )
            embedded_decoder_inputs = model.CopyCPUToGPU(
                embedded_decoder_inputs_cpu, "embedded_decoder_inputs"
            )
...
def output_projection(
    model, decoder_outputs, decoder_output_size, target_vocab_size, decoder_softmax_size
):
    ...
    output_projection_w = model.param_init_net.XavierFill(
        [], "output_projection_w", shape=[target_vocab_size, decoder_output_size]
    )

    output_projection_b = model.param_init_net.XavierFill(
        [], "output_projection_b", shape=[target_vocab_size]
    )
    model.params.extend([output_projection_w, output_projection_b])
    output_logits = model.net.FC(
        [decoder_outputs, output_projection_w, output_projection_b], ["output_logits"]
    )
