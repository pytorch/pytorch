...
class Seq2SeqBeamSearchTest(test_util.TestCase):
    def _build_seq2seq_model(
        ...
        model_obj = Seq2SeqModelCaffe2(
            training_params, source_vocab_size, target_vocab_size, num_gpus
        )
        model_obj.initialize_from_scratch()

        checkpoint_path_prefix = os.path.join(tmp_dir, "checkpoint")
        checkpoint_path = model_obj.save(
            checkpoint_path_prefix=checkpoint_path_prefix, current_step=0
        )
    ...
    def _run_compare_train_inference(self, model_params):
        ...
        translate_params = dict(
            ...
            decoding_params=dict(beam_size=3, word_reward=0, unk_reward=0),
        )
        ...
        for _ in range(3):
            encoder_inputs = np.random.random_integers(
                low=3, high=19, size=encoder_lengths  # after GO_ID (1) and EOS_ID (2)
            )
            targets, _, beam_model_score = beam_decoder_model.decode(
                encoder_inputs, decoder_lengths
            )
            ...
            workspace.FeedBlob(
                "targets", np.array([targets]).transpose().astype(dtype=np.int32)
            )
            workspace.FeedBlob(
                "target_weights",
            ...
            np.testing.assert_almost_equal(
                beam_model_score, train_model_score, decimal=4
            )

    def test_attention(self):
        model_params = dict(
            attention="regular",
            decoder_layer_configs=[dict(num_units=32)],
            encoder_type=dict(
                encoder_layer_configs=[dict(num_units=16)],
    ...
    def test_2layer_attention(self):
        model_params = dict(
            attention="regular",
            decoder_layer_configs=[dict(num_units=32), dict(num_units=32)],
            encoder_type=dict(
                encoder_layer_configs=[dict(num_units=16), dict(num_units=32)],
                use_bidirectional_encoder=True,
            ),
            encoder_embedding_size=8,
            decoder_embedding_size=8,
    ...
    def test_multi_decoder(self):
        model_params = dict(
            ...
            encoder_type=dict(
                encoder_layer_configs=[dict(num_units=32)],
