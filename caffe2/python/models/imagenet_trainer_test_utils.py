...
def test_shared_grads(
    ...
    (shapes, types) = workspace.InferShapesAndTypes(
        ...
        {data_blob: [4, 3, 227, 227], label_blob: [4]},
    )
...
def test_forward_only(
    create_model, last_out_blob, data_blob="gpu_0/data", num_labels=1000
):
    model = cnn.CNNModelHelper(order="NCHW", name="test", cudnn_exhaustive_search=True)
    with core.NameScope("gpu_0"):
        data = model.net.AddExternalInput(data_blob)
        create_model(
            model, data, num_input_channels=3, num_labels=num_labels, is_test=True
        )
...
def test_forward_only_fast_simplenet(
    ...
    optim_proto = memonger.optimize_inference_fast(
        model.net.Proto(),
        set([data_blob, last_out_blob]).union(set(model.net.Proto().external_input)),
    )
