...
class TestLayers(LayersTestCase):
    ...
    def testFilterMetricSchema(self):
        ...
        self.assertEqual(
            ...
            schema.Struct(
                ("a", schema.Struct(("b", schema.Scalar()), ("c", schema.Scalar()))),
                ("d", schema.Scalar()),
            ),
        )
        ...
        self.assertEqual(
            self.model.metrics_schema,
            schema.Struct(
                ("a", schema.Struct(("b", schema.Scalar()))), ("d", schema.Scalar())
            ),
        )
    ...
    def testAddOutputSchema(self):
        ...
        expected_output_schema = schema.Struct(
            ("struct", schema.Struct()), ("scalar", schema.Scalar(np.float64))
        )
        self.assertEqual(self.model.output_schema, expected_output_schema)
    ...
    def testFCWithoutBias(self):
        ...
        init_ops = self.assertNetContainOps(
            train_init_net, [OpSpec("UniformFill", None, None)]
        )

        mat_mul_spec = OpSpec(
            "MatMul",
            [self.model.input_feature_schema.float_features(), init_ops[0].output[0]],
    ...
    def testPairwiseSimilarityWithAllEmbeddings(self):
        ...
        self.assertNetContainOps(
            train_net,
            [OpSpec("BatchMatMul", None, None), OpSpec("Flatten", None, None)],
        )
    ...
    def testSamplingTrain(self):
        ...
        init_ops = self.assertNetContainOps(
            train_init_net,
            [OpSpec("UniformFill", None, None), OpSpec("UniformFill", None, None)],
        )
    ...
    @given(
        X=hu.arrays(dims=[5, 2]), num_to_collect=st.integers(min_value=1, max_value=10)
    )
    ...
    @given(
        X=hu.arrays(dims=[5, 2]), num_to_collect=st.integers(min_value=3, max_value=3)
    )
    def testReservoirSamplingWithID(self, X, num_to_collect):
        ID = np.array([1, 2, 3, 1, 2], dtype=np.int64)
        input_record = self.new_record(
            schema.Struct(
                ("record", schema.Struct(("dense", schema.Scalar()))),
        ...
        reservoir_input = schema.Struct(
            ("data", packed_record), ("object_id", input_record.object_id)
        )
    ...
    def testGatherRecord(self):
        ...
        gathered_items = workspace.FetchBlob(list(gathered_record.sparse.list.items()))
    ...
    def testFunctionalLayerWithOutputDtypes(self):
        loss = self.model.AveragedLoss(
            self.model.input_feature_schema, 1, output_dtypes=(np.float32, (1,))
        )
        self.assertEqual(1, len(loss.field_types()))
        self.assertEqual(np.float32, loss.field_types()[0].base)
    ...
    def testConv(self):
        ...
        init_ops = self.assertNetContainOps(
            train_init_net,
            [OpSpec("XavierFill", None, None), OpSpec("ConstantFill", None, None)],
        )
        conv_spec = OpSpec(
            "Conv",
            [
