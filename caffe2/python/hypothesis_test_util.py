...
hypothesis.settings.load_profile(
    "sandcastle" if is_sandcastle() else os.getenv("CAFFE2_HYPOTHESIS_PROFILE", "dev")
)
...
def segment_ids(size, is_sorted):
    ...
    if is_sorted:
        return arrays([size], dtype=np.int32, elements=st.booleans()).map(
...
def lengths(size, min_segments=None, max_segments=None, **kwargs):
    ...
    return (
        st.integers(min_value=max(min_segments - 1, 0), max_value=max_segments - 1)
        .flatmap(
            lambda num_borders: hypothesis.extra.numpy.arrays(
                np.int32, num_borders, elements=st.integers(min_value=0, max_value=size)
            )
        )
...
def segmented_tensor(
    ...
    **kwargs
    ...
    data_dims_ = st.tuples(gen_empty, data_dims_).map(
        lambda pair: ([0] if pair[0] else []) + pair[1]
    )
    return data_dims_.flatmap(
    ...
    **kwargs
    ...
    all_dims_ = st.tuples(gen_empty, data_dims_).flatmap(
        lambda pair: st.tuples(
            st.just(pair[1]),
            (
                st.integers(min_value=1, max_value=pair[1][0])
                if not pair[0]
                else st.just(0)
            ),
        )
    )
    return all_dims_.flatmap(
        lambda dims: st.tuples(
            arrays(dims[0], dtype, elements),
            arrays(
                dims[1],
                dtype=itype,
                elements=st.integers(min_value=0, max_value=dims[0][0] - 1),
            ),
...
device_options = _device_options_no_hip + (
    [hip_do] if workspace.has_hip_support else []
)
...
gcs_no_hip = dict(
    gc=st.sampled_from(_device_options_no_hip), dc=st.just(_device_options_no_hip)
)
...
def runOpBenchmark(device_option, op, inputs, input_device_options=None, iterations=10):
    ...
    with temp_workspace():
        ...
        for (n, b) in zip(op.input, inputs):
            workspace.FeedBlob(
                n, b, device_option=_input_device_options.get(n, device_option)
            )
...
class HypothesisTestCase(test_util.TestCase):
    ...
    def assertGradientChecks(
        ...
        self.assertTrue(
            res, "Gradient check failed for input " + str(op.input[outputs_to_check])
        )

    def _assertGradReferenceChecks(
        self, op, inputs, ref_outputs, output_to_grad, grad_reference, threshold=1e-4
    ):
        ...
        for (n, ref) in zip(op.input, grad_ref_outputs):
            ...
            if not grad_names:
                ...
                np.testing.assert_allclose(
                    ...
                    atol=threshold,
                    rtol=threshold,
                    err_msg="Gradient {0} (x) is not matching the reference (y)".format(
                        val_name
                    ),
                )
                if ref_indices is not None:
                    ...
                    np.testing.assert_allclose(
                        indices, ref_indices, atol=1e-4, rtol=1e-4
                    )

    def _assertInferTensorChecks(
        self, name, shapes, types, output, ensure_output_is_inferred=False
    ):
        self.assertTrue(
    ...
    def assertReferenceChecks(
        ...
        with temp_workspace():
            ...
            if not (
                isinstance(reference_outputs, tuple)
            ...
            if grad_reference is not None:
                assert output_to_grad is not None, (
                    "If grad_reference is set," "output_to_grad has to be set as well"
                )
    ...
    def assertValidationChecks(
        ...
        if as_kwargs:
            assert len(set(list(op.input) + list(op.output))) == len(op.input) + len(
                op.output
        ...
        with temp_workspace():
            ...
            if as_kwargs:
                validator(
                    **dict(
                        list(zip(list(op.input) + list(op.output), inputs + outputs))
                    )
                )
            else:
