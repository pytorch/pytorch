...
class TestQuantize8bits(hu.HypothesisTestCase):
    def test_quantize_op(self):
        ...
        input_data = np.float32(
            np.asarray([[801.0, 786, 235.2, 2353.3434], [5.0, 11.0, 9.0, -2.0]])
        )
    ...
    def test_quantize_tensor_with_const_row_op(self):
        ...
        input_data = np.float32(
            np.asarray([[801.0, 786, 235.2, 2353.3434], [9.0, 9.0, 9.0, 9.0]])
        )
        workspace.FeedBlob("input_data", input_data)
        workspace.RunOperatorOnce(op)
        op1 = core.CreateOperator(
    ...
    def test_SparseSegmentUint8(self):
        ...
        l = init_net.ConstantFill(
            [], ["l"], shape=[isize // 10], value=10, dtype=core.DataType.INT32
        )
        net.FloatToRowwiseQuantized8Bits([d], ["quantized_data", "scale_bias"])
        net.Rowwise8BitQuantizedToFloat(
            ["quantized_data", "scale_bias"], ["dequantized_data"]
        )

        # SparseLengthsWeightedSum
        net.SparseLengthsWeightedSum(
            ["dequantized_data", w, i, l], ["PositionWeighted_0"], engine="fp16"
        )
        net.SparseLengthsWeightedSum8BitsRowwise(
            ["quantized_data", w, i, l, "scale_bias"], ["PositionWeighted_1"]
        )
        ...
        net.SparseLengthsSum8BitsRowwise(
            ["quantized_data", i, l, "scale_bias"], ["Sum_1"]
        )
        ...
        net.SparseLengthsMean8BitsRowwise(
            ["quantized_data", i, l, "scale_bias"], ["Mean_1"]
        )
        ...
        np.testing.assert_array_almost_equal(
            PositionWeighted_1, ground_truth_posw, decimal=5
        )
        ...
        np.testing.assert_array_almost_equal(
            Gathered_1, ground_truth_gathered, decimal=5
        )
