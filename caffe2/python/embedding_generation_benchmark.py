...
def generate_embedding_table(vocab_size, embedding_size):
    log.info(
        "Generating embedding table with dimensions {}".format(
...
def create_model(args, queue, embedding_table, embedding_size):
    ...
    if args.implementation == "sinusoid":
        model.net.SinusoidPositionEncoding(
            [input_blob], ["output"], embedding_size=embedding_size
        )
    else:
...
def GetArgumentParser():
    ...
    parser.add_argument(
        "--data_size", type=int, default=10000, help="Number of sequences to generate"
    )
    parser.add_argument(
        "--seq_length", type=int, default=128, help="Max sequence length"
    )
    parser.add_argument(
        "--iters_to_report",
        type=int,
    ...
    parser.add_argument(
        "--implementation", type=str, default="sinusoid", help="'table' or 'sinusoid'"
    )
    return parser
...
if __name__ == "__main__":
    ...
    workspace.GlobalInit(
        ["caffe2", "--caffe2_log_level=0", "--caffe2_print_blob_sizes_at_exit=0"]
        + extra_args
    )
