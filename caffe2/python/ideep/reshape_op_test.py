...
class TestReShapeOps(TestCase):
    ...
    def test_int64_reshape_input(self):
        _test_reshape(
            old_shape=(4, 2, 1), new_shape=(2, 4), arg_shape=False, shape_dtype=np.int64
        )
    ...
    def test_in_place(self):
        ...
        _test_reshape(
            old_shape=(4, 2, 1), new_shape=(-1, 8), in_place=True, arg_shape=False
        )

    def test_zero_dim(self):
        _test_reshape(
            old_shape=(4, 2, 1), new_shape=(0, 0, 0), expected_shape=(4, 2, 1)
        )
        _test_reshape(
            old_shape=(4, 2, 1),
            new_shape=(0, 0, 0),
        ...
        _test_reshape(
            old_shape=(4, 2, 1), new_shape=(0, 2, 1), expected_shape=(4, 2, 1)
        )
        _test_reshape(
            old_shape=(4, 2, 1),
            new_shape=(0, 2, 1),
    ...
    def test_zero_dim_and_missing_dim(self):
        _test_reshape(
            old_shape=(4, 2, 1), new_shape=(0, -1, 0), expected_shape=(4, 2, 1)
        )
        _test_reshape(
            old_shape=(4, 2, 1),
            new_shape=(0, -1, 0),
    ...
    def test_input_shape_changes(self):
        ...
        with core.DeviceScope(device_opt):
            workspace.FeedBlob(
                "input_blob", np.array(np.random.rand(10, 20, 10), dtype=np.float32)
            )
            net = core.Net("mynet")
            z, _ = net.Reshape(
                "input_blob", ["z_reshape", "dummy_size"], shape=(-1, 10)
            )
            workspace.CreateNet(net)
            workspace.RunNet(net)
            workspace.FeedBlob(
                "input_blob", np.array(np.random.rand(10, 40, 10), dtype=np.float32)
            )
            workspace.RunNet(net)
        ...
        with core.DeviceScope(device_opt):
            ...
            if arg_shape:
                op = core.CreateOperator(
                    "Reshape", [blob_in], [blob_out, "old_shape"], shape=new_shape
                )
            else:
                op = core.CreateOperator(
                    "Reshape", [blob_in, "new_shape"], [blob_out, "old_shape"]
                )
                workspace.FeedBlob(
                    "new_shape",
                    np.asarray(new_shape, dtype=shape_dtype),
                    core.DeviceOption(caffe2_pb2.CPU, 0),
                )
