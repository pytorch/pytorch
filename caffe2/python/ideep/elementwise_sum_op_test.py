...
class ElementwiseSumTest(hu.HypothesisTestCase):
    ...
    def test_elementwise_sum_fallback(self,
        ...
        if (inplace):
            Xs = [np.random.randint(10, size=(batch_size, input_channels, size, size)).astype(
                np.int32) for _ in range(inputs)]
        else:
            Xs = [np.random.rand(batch_size, input_channels, size, size).astype(
                np.float32) for _ in range(inputs)]
    ...
    @given(size=st.integers(7, 9),
           ...
           inplace=st.booleans(),
           **mu.gcs)
    def test_elementwise_add_fallback(self,
                                      size,
                                      input_channels,
                                      batch_size,
                                      inplace,
        ...
        X0 = np.random.rand(batch_size, input_channels, size).astype(np.float32)
        X1 = np.random.rand(size).astype(np.float32)
        inputs = [X0, X1]
