...
class ConvTest(hu.HypothesisTestCase):
    @given(stride=st.integers(1, 3),
           ...
           #There is a bug in mkl-dnn if size is 10,13,16...,so work around here.
           size=st.integers(8, 9),
    ...
    def test_convolution(self, stride, pad, kernel, size,
        ...
        if training_mode:
            ...
            dY = workspace.FetchBlob('Y')
            op_grad = core.CreateOperator(
                "ConvGradient",
                ["X", "w", "dY"],
                ["dw", "db", "dX"] if use_bias else ["dw", "dX"],
                stride=stride,
                pad=pad,
                kernel=kernel,
                group=group,
            ...
            inputs = [X, w, dY]
            self.assertDeviceChecks(dc, op_grad, inputs, [0, 1, 2] if use_bias else [0, 1])
    ...
    @given(pad=st.integers(0, 3),
           ...
           training_mode=st.booleans(),
           **mu.gcs)
    def test_winograd_convolution(self, pad, size,
                                  input_channels, output_channels,
        ...
        op = core.CreateOperator(
            ...
            kernel=kernel,
            training_mode=training,
            conv_algorithm=conv3x3_winograd_algorithm
        )
        X = np.random.rand(
