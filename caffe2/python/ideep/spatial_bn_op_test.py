...
class TestSpatialBN(hu.HypothesisTestCase):
    ...
    def test_spatialbn_test_mode(
        self, size, input_channels, batch_size, seed, order, epsilon, inplace, gc, dc
    ):
    ...
    def test_spatialbn_train_mode(
        ...
        self.assertDeviceChecks(
            dc, op, [X, scale, bias, running_mean, running_var], [0, 1, 2, 3]
        )
    ...
    def test_spatialbn_train_mode_gradient_check(
        self, size, input_channels, batch_size, seed, order, epsilon, gc, dc
    ):
        op = core.CreateOperator(
            "SpatialBN",
            ["X", "scale", "bias", "mean", "var"],
