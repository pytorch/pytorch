...
class FcTest(hu.HypothesisTestCase):
    ...
    def test_fc_with_axis(self, n, m, c, h, w, axis, gc, dc):
        ...
        op0 = core.CreateOperator(
            "FC", ["X", "W", "b"], ["Y"], axis=axis, device_option=dc[0]
        )

        op0_bw = core.CreateOperator(
            "FCGradient", ["X", "W", "dY"], ["dW", "db"], axis=axis, device_option=dc[0]
        )
        ...
        op1 = core.CreateOperator(
            "FC", ["X", "W", "b"], ["Y"], axis=axis, device_option=dc[1]
        )

        op1_bw = core.CreateOperator(
            "FCGradient", ["X", "W", "dY"], ["dW", "db"], axis=axis, device_option=dc[1]
        )
    ...
    def test_fc_with_axis_w(self, n, o, i, h, w, axis_w, gc, dc):
        ...
        op0 = core.CreateOperator(
            "FC", ["X", "W", "b"], ["Y"], axis_w=axis_w, device_option=dc[0]
        )
        ...
        op1 = core.CreateOperator(
            "FC", ["X", "W", "b"], ["Y"], axis_w=axis_w, device_option=dc[1]
        )
    ...
    def test_int8_fc_4_dims(self, n, m, k, gc, dc):
        ...
        w_bytes = (
            np.rint([w[i, ...] / w_scale[i] for i in range(w.shape[0])]).astype(np.int8)
            + w_zero_point
        )
        ...
        b_zero_point = 0
        b_bytes = np.rint([b[i] / b_scale[i] for i in range(b.shape[0])]).astype(
            np.int32
        )
