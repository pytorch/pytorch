...
class ConvFusionTest(hu.HypothesisTestCase):
    ...
    def test_convolution_relu_fusion(self, stride, pad, kernel, size,
        ...
        self.assertEqual(len(net.Proto().op), 1, "Expected only 1 op but got {}".format(net.Proto().op))
        self.assertEqual(net.Proto().op[0].type, "ConvFusion", "Expected only 1 ConvFusion but got {}".format(net.Proto().op))
    ...
    def test_convolution_sum_fusion(self, stride, pad, kernel, size,
        ...
        net.Proto().CopyFrom(old_net)
        optimizeForIDEEP(net)
        self.assertEqual(len(net.Proto().op), 2, "Expected only 2 ops but got {}".format(net.Proto().op))
        self.assertEqual(net.Proto().op[1].type, "ConvFusion", "Expected 2nd op to be ConvFusion but got {}".format(net.Proto().op))
    ...
    def test_convolution_bn_folding(
        ...
        optimizeForIDEEP(net)
        self.assertEqual(len(net.Proto().op), 1, "Expected only 1 op but got {}".format(net.Proto().op))
        self.assertEqual(net.Proto().op[0].type, "Conv", "Expected only Conv but got {}".format(net.Proto().op))
        workspace.RunOperatorOnce(net.Proto().op[0])
