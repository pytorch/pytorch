...
from caffe2.python import brew, core, model_helper, workspace
    ...
    def test_preConvert(self, input_channels, batch_size):
        ...
        with core.DeviceScope(device_opt):
            train_model = model_helper.ModelHelper(
                name="test_train", arg_scope=arg_scope
            )
            ...
            X = (
                np.random.rand(batch_size, input_channels, 28, 28).astype(np.float32)
                - 0.5
            )
            ...
            workspace.FeedBlob("X", X)
            workspace.FeedBlob("label", label)
            for blob in list(blob_dict.keys()):
                workspace.FeedBlob(blob, blob_dict[blob])
            workspace.CreateNet(train_model.net, overwrite=True)
            workspace.RunNet(train_model.net)
            for blob in list(output_dict.keys()):
                output_dict_cosim[blob] = workspace.FetchBlob(blob)

            for blob in list(output_dict.keys()):
                if not np.allclose(
                    output_dict[blob], output_dict_cosim[blob], atol=0.001, rtol=0.0001
                ):
