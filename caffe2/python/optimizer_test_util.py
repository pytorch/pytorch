...
class OptimizerTestBase(object):
    ...
    def _createDense(self, dtype=core.DataType.FLOAT):
        ...
        initializer = (
            Initializer if dtype == core.DataType.FLOAT else PseudoFP16Initializer
        )
    ...
    def testDense(self):
        ...
        np.testing.assert_allclose(
            perfect_model[np.newaxis, :], workspace.FetchBlob("fc_w"), atol=1e-2
        )
    ...
    def testSparse(self):
        ...
        perfect_model = np.array([2, 6, 5, 0, 1]).astype(np.float32)
        np.random.seed(123)  # make test deterministic
        data = np.random.randint(2, size=(20, perfect_model.size * DUPLICATION)).astype(
            np.float32
        )
        ...
        for indices_type in [np.int32, np.int64]:
            ...
            for _ in range(2000):
                ...
                indices = np.repeat(np.arange(perfect_model.size), DUPLICATION)[
                    data[idx] == 1
                ]
                if indices.size == 0:
                    continue
                workspace.FeedBlob(
                    "indices", indices.reshape((indices.size,)).astype(indices_type)
                )
                workspace.FeedBlob("label", np.array(label[idx]).astype(np.float32))
                workspace.RunNet(model.net.Proto().name)

            np.testing.assert_allclose(
                perfect_model, workspace.FetchBlob("w"), atol=1e-2
            )
        self.check_optimizer(optimizer)
