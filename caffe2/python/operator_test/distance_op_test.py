...
class DistanceTest(serial.SerializedTestCase):
    ...
    def test_cosine_similarity(self, n, dim, gc, dc):
        ...
        cos = np.divide(
            ...
            np.multiply(
                np.linalg.norm(X, axis=1) + kEps, np.linalg.norm(Y, axis=1) + kEps
            ),
        )
        np.testing.assert_allclose(
            self.ws.blobs[("cos")].fetch(), cos, rtol=1e-4, atol=1e-4
        )
        self.assertGradientChecks(
            gc, cos_op, [X, Y], 0, [0], stepsize=1e-2, threshold=1e-2
        )
        self.assertGradientChecks(
            gc, cos_op, [X, Y], 1, [0], stepsize=1e-2, threshold=1e-2
        )

    @serial.given(
        inputs=hu.tensors(n=2, min_dim=1, max_dim=2, dtype=np.float32), **hu.gcs
    )
    ...
    def test_L1_distance(self, n, dim, gc, dc):
        ...
        self.ws.run(op)
        np.testing.assert_allclose(
            self.ws.blobs[("l1_dist")].fetch(),
            [np.linalg.norm(x - y, ord=1) for x, y in zip(X, Y)],
            rtol=1e-4,
            atol=1e-4,
        )
    ...
    def test_L2_distance(self, n, dim, gc, dc):
        ...
        self.ws.run(l2_op)
        np.testing.assert_allclose(
            self.ws.blobs[("l2_dist")].fetch(),
            np.square(X - Y).sum(axis=1) * 0.5,
            rtol=1e-4,
            atol=1e-4,
        )
        self.assertGradientChecks(
            gc, l2_op, [X, Y], 0, [0], stepsize=1e-2, threshold=1e-2
        )
        self.assertGradientChecks(
            gc, l2_op, [X, Y], 1, [0], stepsize=1e-2, threshold=1e-2
        )
