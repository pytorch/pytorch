...
class TestCounterOps(TestCase):
    def test_stats_ops(self):
        ...
        for _ in range(2):
            workspace.RunOperatorOnce(
                core.CreateOperator("StatRegistryUpdate", ["k", "v"], [])
            )
        workspace.RunOperatorOnce(
            core.CreateOperator("StatRegistryExport", [], ["k2", "v2", "t2"])
        )

        workspace.RunOperatorOnce(
            core.CreateOperator("StatRegistryCreate", [], ["reg"])
        )
        workspace.RunOperatorOnce(
            core.CreateOperator("StatRegistryUpdate", ["k2", "v2", "reg"], [])
        )

        workspace.RunOperatorOnce(
            core.CreateOperator("StatRegistryExport", ["reg"], ["k3", "v3", "t3"])
        )
