...
def map_rois_to_fpn_levels(
    rois, k_min, k_max, roi_canonical_scale, roi_canonical_level
):
    ...
    target_lvls = np.floor(
        roi_canonical_level + np.log2(s / roi_canonical_scale + 1e-6)
    )
...
class TestCollectAndDistributeFpnRpnProposals(serial.SerializedTestCase):
    ...
    def _create_input(
        ...
        for lvl in range(rpn_num_levels):
            rpn_roi = roi_canonical_scale * np.random.rand(proposal_count, 5).astype(
                np.float32
            )
