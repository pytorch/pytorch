...
class TorchIntegration(hu.HypothesisTestCase):
    ...
    def test_bbox_transform(
        ...
        torch.testing.assert_close(box_out, a)
    ...
    def test_sparse_to_dense_mask(self, dim_1, dim_2, dim_3):
        ...
        torch.testing.assert_close(output, a)
        torch.testing.assert_close(presence_mask, b)
    ...
    def test_generate_proposals(self, A, H, W, img_count):
        ...
        torch.testing.assert_close(rois, a)
        torch.testing.assert_close(rois_probs, b)
    ...
    def test_inference_lstm(
        ...
        torch.testing.assert_close(output, a)
        torch.testing.assert_close(hidden, b)
        torch.testing.assert_close(cell, c)
    ...
    def test_generate_proposals_cuda(self, A, H, W, img_count):
        ...
        torch.testing.assert_close(rois, a.cpu())
        torch.testing.assert_close(rois_probs, b.cpu())
    ...
    def _test_roi_align(self, N, C, H, W, device):
        ...
        torch.testing.assert_close(roi_feature_ref, roi_feature.cpu())
    ...
    def test_collect_and_distribute_fpn_rpn_proposals_op(self, roi_counts):
        ...
        torch.testing.assert_close(rois, all_outputs[0])
    ...
    def _test_lengths_op(self, inputs, ref_op_name, torch_op, device):
        ...
        torch.testing.assert_close(expected_output, actual_output.cpu())
    ...
    def test_piecewise_linear_op(self, binary_input):
        ...
        torch.testing.assert_close(torch.tensor(expected_output), actual_output)
    ...
    def test_alias_with_name_is_in_place(self):
        ...
        x[1] = 6
        torch.testing.assert_close(x, torch.tensor([3., 6.]).to(device=device))
        # y should also change because y is alias of x
        torch.testing.assert_close(y, torch.tensor([3., 6.]).to(device=device))
    ...
    def test_copy_between_cpu_and_gpu(self):
        ...
        torch.testing.assert_close(x_gpu, x_gpu_ref)
        x_cpu = torch.ops._caffe2.CopyGPUToCPU(x_gpu)
        torch.testing.assert_close(x_cpu, x_cpu_ref)
    ...
    def test_merge_id_lists(self, lengths_0, lengths_1):
        ...
        torch.testing.assert_close(expected_merged_lengths, output_merged_lengths)
        torch.testing.assert_close(expected_merged_values, output_merged_values)
    ...
    def test_pack_segments(self):
        ...
        torch.testing.assert_close(s, unpacked_tensor)
