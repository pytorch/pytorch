...
def gru_unit(*args, **kwargs):
    ...
    if sequence_lengths:
        seq_lengths = (np.ones(shape=(N, D)) * seq_lengths.reshape(N, 1)).astype(
            np.int32
        )
...
def gru_reference(
    ...
    for t in range(T):
        ...
        (hidden_t,) = gru_unit(
            hidden_t_prev, gates_out_t, seq_lengths, t, drop_states=drop_states
        )
        hidden[t + 1] = hidden_t
...
def _prepare_gru_unit_op(
    ...
    with scope.NameScope("test_name_scope"):
        if sequence_lengths:
            hidden_t_prev, gates_t, seq_lengths, timestep = model.net.AddScopedExternalInputs(
                "hidden_t_prev", "gates_t", "seq_lengths", "timestep"
            )
        else:
            hidden_t_prev, gates_t, timestep = model.net.AddScopedExternalInputs(
        ...
        workspace.FeedBlob(
            gates_t, generate_input_state(n, 3 * d).astype(np.float32), device_option=gc
        )
...
class GRUCellTest(serial.SerializedTestCase):
    ...
    def test_gru_unit_op(
        self, seed, input_tensor, fwd_only, drop_states, sequence_lengths, gc, dc
    ):
        np.random.seed(seed)
        ...
        workspace.FeedBlob(
            "test_name_scope/external/recurrent/i2h", input_tensor, device_option=gc
        )
        print(str(net.Proto()))
        op = net._net.op[-1]
    ...
    def gru_base(
        ...
        d = d // 3
        ref = partial(
            ref, drop_states=drop_states, linear_before_reset=linear_before_reset
        )
        with core.DeviceScope(gc):
            net = _prepare_rnn(
                t,
