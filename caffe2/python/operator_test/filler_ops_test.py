...
class TestFillerOperator(serial.SerializedTestCase):
    ...
    def test_shape_error(self, gc, dc):
        op = core.CreateOperator(
            "GaussianFill", [], "out", shape=32, mean=0.0, std=1.0  # illegal parameter
        )
        exception = False
        try:
        ...
        op = core.CreateOperator(
            "ConstantFill", [], "out", shape=[], value=2.0  # scalar
        )
        exception = False
    ...
    def test_uniform_fill_using_arg(self, gc, dc):
        ...
        output_blob = net.UniformIntFill(
            [], ["output_blob"], shape=shape, min=min_v, max=max_v
        )
    ...
    def test_diagonal_fill_op_float(self, shape, gc, dc):
        value = 2.5
        op = core.CreateOperator(
            "DiagonalFill", [], "out", shape=shape, value=value  # scalar
        )
    ...
    def test_msra_fill_op(self, gc, dc):
        op = core.CreateOperator(
            "MSRAFill", [], "out", shape=[15, 5, 3]  # sample odd dimensions
        )
        for device_option in dc:
            op.device_option.CopyFrom(device_option)
