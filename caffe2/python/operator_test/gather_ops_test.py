...
class TestGatherOps(serial.SerializedTestCase):
    ...
    @given(batch_num=st.integers(1, 40),
           rows_num=st.integers(1, 6),
           index_num=st.integers(1, 20),
           index_num2=st.integers(1, 20),
           **hu.gcs)
    def test_gather_ops_axis2_ind3D(self, batch_num, rows_num, index_num, index_num2, gc, dc):
        data = np.random.random((batch_num, rows_num, 5, 4)).astype(np.float32)
        ind = np.random.randint(5, size=(index_num, index_num, index_num2)).astype('int32')
        op = core.CreateOperator(
            'Gather',
            ['data', 'ind'],
            ['output'],
    ...
    # Test Gather with match_outer == true, with 3D ind
    @given(batch_num=st.integers(1, 40),
           rows_num=st.integers(1, 6),
           index_num=st.integers(1, 20),
           axis2_num=st.integers(1, 30),
           **hu.gcs)
    def test_gather_op_match_outer_axis2(self, batch_num, rows_num, axis2_num, index_num, gc, dc):
        data = np.random.random((batch_num, rows_num, axis2_num)).astype(np.float32)
        ind = np.random.randint(axis2_num, size=(batch_num, rows_num, index_num)).astype('int32')
        op = core.CreateOperator(
            'Gather',
            ['data', 'ind'],
            ['output'],
    ...
    # Test Gather with match_outer == true, with 4D ind
    @given(batch_num=st.integers(1, 40),
           rows_num=st.integers(1, 6),
           index_num=st.integers(1, 20),
           axis2_num=st.integers(1, 30),
           **hu.gcs)
    def test_gather_op_match_outer_axis2_data4D(self, batch_num, rows_num, axis2_num, index_num, gc, dc):
    ...
    # ran this test for 20+ times, they all PASS!
    # log here: https://fb.quip.com/7BpVAAiQIyGj
    @given(batch_num=st.integers(1, 5),
           rows_num=st.integers(1, 5),
           index_num=st.integers(1, 5),
           index_num2=st.integers(1, 5),
           axis2_num=st.integers(1, 5),
           **hu.gcs)
    def test_gather_op_match_outer_axis2_data4D_ind4D_small(
    ...
    @given(batch_num=st.integers(1, 30),
           ...
           **hu.gcs)
    def test_gather_op_match_outer_axis2_data4D_ind4D_large(
        self, batch_num, rows_num, axis2_num, index_num, index_num2, gc, dc
    ):
        data = np.random.random((batch_num, rows_num, axis2_num, 5)).astype(np.float32)
        ind = np.random.randint(axis2_num, size=(batch_num, rows_num, index_num, index_num2)).astype('int32')
