...
class TestReduceOps(serial.SerializedTestCase):
    def run_reduce_op_test_impl(self, op_name, X, axes, keepdims, ref_func, gc, dc):
        if axes is None:
            op = core.CreateOperator(op_name, ["X"], ["Y"], keepdims=keepdims)
        else:
            op = core.CreateOperator(
                op_name, ["X"], ["Y"], axes=axes, keepdims=keepdims
            )
    ...
    def run_reduce_op_test(self, op_name, X, keepdims, num_axes, ref_func, gc, dc):
        self.run_reduce_op_test_impl(op_name, X, None, keepdims, ref_func, gc, dc)

        num_dims = len(X.shape)
        if num_dims < num_axes:
            self.run_reduce_op_test_impl(
                op_name, X, list(range(num_dims)), keepdims, ref_func, gc, dc
            )
        else:
            for axes in it.combinations(list(range(num_dims)), num_axes):
                self.run_reduce_op_test_impl(
    ...
    def test_reduce_min(self, X, keepdims, num_axes, gc, dc):
        ...
        np.random.shuffle(X)
        X = X.reshape(X_dims)
        self.run_reduce_op_test("ReduceMin", X, keepdims, num_axes, np.min, gc, dc)
    ...
    def test_reduce_max(self, X, keepdims, num_axes, gc, dc):
        ...
        np.random.shuffle(X)
        X = X.reshape(X_dims)
        self.run_reduce_op_test("ReduceMax", X, keepdims, num_axes, np.max, gc, dc)
    ...
    def test_reduce_sum(self, n, m, k, t, keepdims, num_axes, gc, dc):
        ...
        self.run_reduce_op_test("ReduceSum", X, keepdims, num_axes, np.sum, gc, dc)
    ...
    def test_reduce_mean(self, X, keepdims, num_axes, gc, dc):
        self.run_reduce_op_test("ReduceMean", X, keepdims, num_axes, np.mean, gc, dc)
    ...
    def test_reduce_l1(self, n, m, k, keepdims, num_axes, gc, dc):
        ...
        np.random.shuffle(X)
        X = X.reshape((m, n, k))
        self.run_reduce_op_test("ReduceL1", X, keepdims, num_axes, getNorm(1), gc, dc)
    ...
    def test_reduce_l2(self, n, m, k, keepdims, num_axes, gc, dc):
        ...
        self.run_reduce_op_test("ReduceL2", X, keepdims, num_axes, getNorm(2), gc, dc)
...
class TestReduceFrontReductions(serial.SerializedTestCase):
    def grad_variant_input_test(self, grad_op_name, X, ref, num_reduce_dim):
        ...
        grad_op1 = core.CreateOperator(
            grad_op_name, ["dY", "shape"], ["dX1"], num_reduce_dim=num_reduce_dim
        )
    ...
    def max_op_test(self, op_name, num_reduce_dim, gc, dc, in_data, in_names, ref_max):
        ...
        op = core.CreateOperator(
            op_name, in_names, ["outputs"], num_reduce_dim=num_reduce_dim
        )

        self.assertReferenceChecks(
            device_option=gc, op=op, inputs=in_data, reference=ref_max
        )
        ...
        grad_op = core.CreateOperator(
            op_name + "Gradient", grad_in_names, ["dX"], num_reduce_dim=num_reduce_dim
        )
        self.assertDeviceChecks(dc, grad_op, grad_in_data, [0])

    def reduce_op_test(
        self, op_name, op_ref, in_data, in_names, num_reduce_dims, device
    ):
        op = core.CreateOperator(
            op_name, in_names, ["outputs"], num_reduce_dim=num_reduce_dims
        )

        self.assertReferenceChecks(
            device_option=device, op=op, inputs=in_data, reference=op_ref
        )
