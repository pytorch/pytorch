...
def create_test(output_dir, width, height, default_bound, minsize, crop, means,
    ...
    with env.begin(write=True) as txn:
        while index < count:
            ...
            image_tensor.data_type = 4  # string data  # type: ignore[assignment, assignment]
            image_tensor.string_data.append(img_str.getvalue())
            img_str.close()

            label_tensor = tensor_protos.protos.add()
            label_tensor.data_type = 2  # int32 data  # type: ignore[assignment, assignment]
            ...
            if label_type == 0:
                ...
                weight_tensor.data_type = 1  # float weights  # type: ignore[assignment, assignment]
                binary_labels = np.random.randint(2, size=num_labels)
            ...
            if output1:
                ...
                output1_tensor.data_type = 1  # float data  # type: ignore[assignment, assignment]
                output1_tensor.float_data.append(output1)

            output2 = []
            if output2_size:
                output2_tensor = tensor_protos.protos.add()
                output2_tensor.data_type = 2  # int32 data  # type: ignore[assignment, assignment]
            ...
            if not do_default_bound:
                ...
                bounding_tensor.data_type = 2  # int32 data  # type: ignore[assignment, assignment]
