...
class TestElementwiseOps(hu.HypothesisTestCase):
    ...
    def _test_binary_op(
            self, op_name, np_ref, n, m, k, l, bias, test_grad, gc, dc):
        op = core.CreateOperator(
            op_name,
            ["A", "B"],
            ["C"],
        ...
        A = np.random.rand(n, m, k, l).astype(np.float32) + bias
        B = np.random.rand(n, m, k, l).astype(np.float32) + bias
        ...
        A = np.random.rand(k, l).astype(np.float32) + bias
        B = np.random.rand(n, m, k, l).astype(np.float32) + bias
        self._run_single_test(op, ref, A, B, test_grad, gc, dc)
        ...
        A = np.random.rand(m, 1, l).astype(np.float32) + bias
        B = np.random.rand(n, m, k, l).astype(np.float32) + bias
        self._run_single_test(op, ref, A, B, test_grad, gc, dc)

        A = np.random.rand(1, m, 1, l).astype(np.float32) + bias
        B = np.random.rand(n, 1, k, 1).astype(np.float32) + bias
        self._run_single_test(op, ref, A, B, test_grad, gc, dc)

    @given(n=st.integers(1, 5), m=st.integers(1, 5), k=st.integers(1, 5),
           l=st.integers(1, 5), **hu.gcs)
    def test_add(self, n, m, k, l, gc, dc):
        self._test_binary_op("Add", np.add, n, m, k, l, -0.5, True, gc, dc)

    @given(n=st.integers(1, 5), m=st.integers(1, 5), k=st.integers(1, 5),
           l=st.integers(1, 5), **hu.gcs)
    def test_sub(self, n, m, k, l, gc, dc):
        self._test_binary_op("Sub", np.subtract, n, m,
                             k, l, -0.5, True, gc, dc)

    @given(n=st.integers(1, 5), m=st.integers(1, 5), k=st.integers(1, 5),
           l=st.integers(1, 5), **hu.gcs)
    def test_mul(self, n, m, k, l, gc, dc):
        self._test_binary_op("Mul", np.multiply, n, m,
                             k, l, -0.5, True, gc, dc)

    @given(n=st.integers(1, 5), m=st.integers(1, 5), k=st.integers(1, 5),
           l=st.integers(1, 5), **hu.gcs)
    def test_div(self, n, m, k, l, gc, dc):
        self._test_binary_op("Div", np.divide, n, m, k, l, 1.0, True, gc, dc)

    def _test_bitwise_binary_op(self, op_name, np_ref, n, m, k, l, gc, dc):
        op = core.CreateOperator(
            op_name,
            ["A", "B"],
            ["C"],
        ...
        A = np.random.randint(128, size=(n, m, k, l))
        B = np.random.randint(128, size=(n, m, k, l))
        ...
        A = np.random.randint(128, size=(k, l))
        B = np.random.randint(128, size=(n, m, k, l))
        self._run_single_test(op, ref, A, B, False, gc, dc)
        ...
        A = np.random.randint(128, size=(m, 1, l))
        B = np.random.randint(128, size=(n, m, k, l))
        self._run_single_test(op, ref, A, B, False, gc, dc)

        A = np.random.randint(128, size=(1, m, 1, l))
        B = np.random.randint(128, size=(n, 1, k, 1))
        self._run_single_test(op, ref, A, B, False, gc, dc)

    @given(n=st.integers(1, 5), m=st.integers(1, 5), k=st.integers(1, 5),
           l=st.integers(1, 5), **hu.gcs)
    def test_bitwise_and(self, n, m, k, l, gc, dc):
        self._test_bitwise_binary_op(
            "BitwiseAnd", np.bitwise_and, n, m, k, l, gc, dc)

    @given(n=st.integers(1, 5), m=st.integers(1, 5), k=st.integers(1, 5),
           l=st.integers(1, 5), **hu.gcs)
    def test_bitwise_or(self, n, m, k, l, gc, dc):
        self._test_bitwise_binary_op(
            "BitwiseOr", np.bitwise_or, n, m, k, l, gc, dc)

    @given(n=st.integers(1, 5), m=st.integers(1, 5), k=st.integers(1, 5),
           l=st.integers(1, 5), **hu.gcs)
    def test_bitwise_xor(self, n, m, k, l, gc, dc):
        self._test_bitwise_binary_op(
            "BitwiseXor", np.bitwise_xor, n, m, k, l, gc, dc)
