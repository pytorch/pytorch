...
def compute_regular_attention_logits(
    ...
    weighted_hidden_t = (
        np.dot(hidden_t, weighted_decoder_hidden_state_t_w.T)
        + weighted_decoder_hidden_state_t_b
    )
    attention_v = attention_v.reshape([-1])
    return np.sum(
        attention_v * np.tanh(weighted_encoder_outputs + weighted_hidden_t), axis=2
    )
...
def compute_dot_attention_logits(
    ...
    attention_logits_t = np.sum(
        np.matmul(encoder_outputs_for_dot_product, hidden_t_for_dot_product), axis=2
    )
...
def lstm_with_attention_reference(
    ...
    encoder_outputs_for_dot_product = np.transpose(
        encoder_outputs_transposed, [0, 2, 1]
    )
    ...
    for t in range(decoder_input_length):
        ...
        gates_input = np.concatenate(
            (hidden_t_prev, attention_weighted_encoder_context_t_prev), axis=2
        )
        ...
        attention_weighted_encoder_context[t + 1] = np.sum(
            (encoder_outputs * attention_weights_t.reshape([-1, batch_size, 1])), axis=0
        )
    return (
        hidden[1:],
...
def _prepare_attention(
    ...
    for layer_id, d in enumerate(dim_out):
        h, c = model.net.AddExternalInputs(
            "hidden_init_{}".format(layer_id), "cell_init_{}".format(layer_id)
        )
    ...
    with scope.NameScope("test_name_scope"):
        (
            ...
            "input_blob", "seq_lengths", "encoder_outputs", "weighted_encoder_outputs"
        )
...
class RNNCellTest(hu.HypothesisTestCase):
    ...
    @given(
        ...
        dim_out=st.lists(
            elements=st.integers(min_value=1, max_value=3), min_size=1, max_size=3
        ),
    ...
    def test_unroll_lstm(self, input_tensor, dim_out, outputs_with_grads, **kwargs):
        ...
        gradient_checker.NetGradientChecker.CompareNets(
            nets, outputs, outputs_with_grads, inputs_with_grads=inputs[0]
        )
    ...
    def test_unroll_attention(
        ...
        for state_name, unrolled_state_name in zip(
            net["initial_states"], unrolled["initial_states"]
        ):
    ...
    def lstm_base(
        ...
        self.assertReferenceChecks(
            hu.cpu_do, op, inputs, ref, outputs_to_check=list(range(4))
        )
    ...
    def test_lstm_with_regular_attention(
        ...
        self.lstm_with_attention(
            partial(rnn_cell.LSTMWithAttention, attention_type=AttentionType.Regular),
            encoder_output_length,
            encoder_output_dim,
            decoder_input_length,
            decoder_state_dim,
    ...
    def test_lstm_with_recurrent_attention(
        ...
        self.lstm_with_attention(
            partial(rnn_cell.LSTMWithAttention, attention_type=AttentionType.Recurrent),
            encoder_output_length,
            encoder_output_dim,
            decoder_input_length,
            decoder_state_dim,
    ...
    def test_lstm_with_dot_attention_same_dim(
        ...
        self.lstm_with_attention(
            partial(rnn_cell.LSTMWithAttention, attention_type=AttentionType.Dot),
            encoder_output_length,
            encoder_output_dim,
            decoder_input_length,
            decoder_state_dim,
    ...
    def test_lstm_with_coverage_attention(
        ...
        self.lstm_with_attention(
            partial(
                rnn_cell.LSTMWithAttention, attention_type=AttentionType.SoftCoverage
            ),
            encoder_output_length,
            encoder_output_dim,
            decoder_input_length,
    ...
    def lstm_with_attention(
        ...
        workspace.FeedBlob(
            decoder_input_blob,
            np.random.randn(
                decoder_input_length, batch_size, decoder_state_dim * 4
            ).astype(np.float32),
        )
        workspace.FeedBlob(
            "external/LSTMWithAttention/encoder_outputs_transposed",
            np.random.randn(
                batch_size, encoder_output_dim, encoder_output_length
            ).astype(np.float32),
        )
        workspace.FeedBlob(
            "external/LSTMWithAttention/weighted_encoder_outputs",
            np.random.randn(
                encoder_output_length, batch_size, encoder_output_dim
            ).astype(np.float32),
        )
        workspace.FeedBlob(
    ...
    def test_multi_lstm(
        self, input_length, dim_in, max_num_units, num_layers, batch_size
    ):
        model = ModelHelper(name="external")
        (input_sequence, seq_lengths) = model.net.AddExternalInputs(
            "input_sequence", "seq_lengths"
        )
        ...
        seq_lengths_val = np.random.randint(
            1, input_length + 1, size=(batch_size)
        ).astype(np.int32)
        input_sequence_val = np.random.randn(input_length, batch_size, dim_in).astype(
            np.float32
        )
