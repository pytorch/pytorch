...
class TestPairWiseLossOps(serial.SerializedTestCase):
    @given(
        X=hu.arrays(dims=[2, 1], elements=st.floats(min_value=0.0, max_value=10.0)),
        label=hu.arrays(
            dims=[2, 1],
    ...
    @given(
        ...
        dY=hu.arrays(dims=[1], elements=st.floats(min_value=1, max_value=10)),
        **hu.gcs_cpu_only
    )
    def test_pair_wise_loss_gradient(self, X, label, dY, gc, dc):
        ...
        np.testing.assert_allclose(
            np.asscalar(dx[0]),
            np.asscalar(
                0.5 * dY[0] * (up_output_pred[0] - down_output_pred[0]) / delta
            ),
            rtol=1e-2,
            atol=1e-2,
        )
