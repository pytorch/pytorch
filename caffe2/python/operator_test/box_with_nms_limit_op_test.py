...
def gen_multiple_boxes(centers, scores, count, num_classes):
    ...
    for cc, ss in zip(centers, scores):
        ...
        ret_scores = (
            np.vstack((ret_scores, cur_sc)) if ret_scores is not None else cur_sc
        )
...
class TestBoxWithNMSLimitOp(serial.SerializedTestCase):
    ...
    def test_detections_per_im(self, det_per_im, gc):
        ...
        op = get_op(
            2, 3, {"score_thresh": 0.5, "nms": 0.9, "detections_per_im": det_per_im}
        )
    ...
    def test_multiclass(
        ...
        scores[:, scores_bg_class_id] = np.random.rand(scores.shape[0]).astype(
            np.float32
        )
        ...
        # [1, 1, 1, 2, 2, 2, 3, 3, 3, ...]
        gt_classes = np.tile(
            np.array(list(range(1, num_classes)), dtype=np.float32),
            (gt_boxes.shape[0], 1),
        ).T.flatten()
    ...
    def test_detections_per_im_same_thresh(self, det_per_im, gc):
        ...
        def verify(inputs, outputs):
            ...
            # check classes
            np.testing.assert_allclose(outputs[2], gt_classes, atol=1e-4, rtol=1e-4)
    ...
    def test_detections_per_im_same_thresh_multiclass(self, num_classes, gc):
        ...
        def verify(inputs, outputs):
            ...
            self.assertTrue(
                ...
                or np.allclose(outputs[1], gt_boxes[1, :], atol=1e-4, rtol=1e-4)
            )
