...
class TestFeatureMapsOps(TestCase):
    def test_merge_single_scalar_feature_tensors(self):
        op = core.CreateOperator(
            ...
            ["in1", "in1_presence", "in2", "in2_presence"],
        ...
        np.testing.assert_array_equal(
            workspace.FetchBlob("out_lengths"), np.array([2, 1], dtype=np.int32)
        )
        np.testing.assert_array_equal(
            workspace.FetchBlob("out_keys"), np.array([11, 12, 12], dtype=np.int64)
        )
        np.testing.assert_array_equal(
    ...
    def test_merge_single_scalar_feature_tensors_gradient(self):
        op = core.CreateOperator(
            ...
            ["in1_presence", "in2_presence", "in3_presence", "out_values_grad"],
        ...
        # Input 4.
        workspace.FeedBlob(
            "out_values_grad", np.array([0.1, 1.1, 1.2, 2.3], dtype=np.float)
        )

        workspace.RunOperatorOnce(op)

        np.testing.assert_array_equal(
            workspace.FetchBlob("in1_grad"), np.array([0.1, 0], dtype=np.float)
        )
        np.testing.assert_array_equal(
            workspace.FetchBlob("in2_grad"), np.array([1.1, 1.2], dtype=np.float)
        )
        np.testing.assert_array_equal(
            workspace.FetchBlob("in3_grad"), np.array([0, 2.3], dtype=np.float)
        )
    ...
    def test_merge_single_scalar_feature_tensors_gradient_with_strings(self):
        ...
        # Input 4.
        workspace.FeedBlob(
            "out_values_grad", np.array(["0.1", "1.1", "1.2", "2.3"], dtype=np.unicode_)
        )

        workspace.RunOperatorOnce(op)

        np.testing.assert_array_equal(
            workspace.FetchBlob("in1_grad"), np.array(["0.1", ""], dtype=np.bytes_)
        )
        np.testing.assert_array_equal(
            workspace.FetchBlob("in2_grad"), np.array(["1.1", "1.2"], dtype=np.bytes_)
        )
        np.testing.assert_array_equal(
            workspace.FetchBlob("in3_grad"), np.array(["", "2.3"], dtype=np.bytes_)
        )
    ...
    def test_merge_single_list_feature_tensors(self):
        op = core.CreateOperator(
            ...
            ["out_lengths", "out_keys", "out_values_lengths", "out_values_values"],
            feature_ids=[11, 12],
        )
        ...
        workspace.FeedBlob(
            "in2_values", np.array([12.1, 12.2, 12.3, 12.4], dtype=np.float)
        )
        workspace.FeedBlob("in2_presence", np.array([True, True], dtype=np.bool))
    ...
    def _test_merge_single_list_or_map_feature_tensors_gradient(self, op_name):
        op = core.CreateOperator(
            ...
            ["in1_values_grad", "in2_values_grad"],
        )
    ...
    def test_merge_multi_scalar_feature_tensors(self):
        ...
        np.testing.assert_array_equal(
            workspace.FetchBlob("out_lengths"), np.array([3, 3], dtype=np.int32)
        )
        np.testing.assert_array_equal(
    ...
    def test_merge_multi_scalar_feature_tensors_gradient(self):
        op = core.CreateOperator(
            ...
            ["in1_lengths", "in2_lengths", "out_values_grad"],
            ["in1_values_grad", "in2_values_grad"],
        )
    ...
    def test_merge_multi_map_feature_tensors(self):
        ...
        workspace.FeedBlob("in1_values_lengths", np.array([2, 2, 2], dtype=np.int32))
        workspace.FeedBlob(
            "in1_values_keys", np.array([111, 112, 121, 122, 131, 132], dtype=np.int64)
        )
        workspace.FeedBlob(
            "in1_values_values",
            np.array([11.1, 11.2, 12.1, 12.2, 13.1, 13.2], dtype=np.float),
        ...
        workspace.FeedBlob("in2_values_lengths", np.array([2, 2, 2], dtype=np.int32))
        workspace.FeedBlob(
            "in2_values_keys", np.array([141, 142, 151, 152, 161, 162], dtype=np.int64)
        )
        workspace.FeedBlob(
            "in2_values_values",
            np.array([14.1, 14.2, 15.1, 15.2, 16.1, 16.2], dtype=np.float),
    ...
    def _test_merge_multi_list_or_map_feature_tensors_gradient(self, op_name):
        op = core.CreateOperator(
            ...
            ["in1_values_values_grad", "in2_values_values_grad"],
        )
