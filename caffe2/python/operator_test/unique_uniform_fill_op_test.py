...
class TestUniqueUniformFillOp(hu.HypothesisTestCase):
    @given(
        ...
        avoid=st.lists(st.integers(1, 1000), min_size=1, max_size=100, unique=True),
        dtypes=st.sampled_from(
            [(np.int32, core.DataType.INT32), (np.int64, core.DataType.INT64)]
        ),
    ...
    def test_unique_uniform_int_fill(self, r, avoid, dtypes, s, gc, dc):
        ...
        net.UniqueUniformFill(
            ["X", "AVOID"], ["Y"], min=1, max=r, input_as_shape=True, dtype=dtypes[1]
        )
