...
class TestIndexOps(TestCase):
    def _test_index_ops(self, entries, dtype, index_create_op):
        workspace.RunOperatorOnce(
            core.CreateOperator(index_create_op, [], ["index"], max_elements=10)
        )
        ...
        query2 = np.array(
            [entries[5], entries[4], entries[0], entries[6], entries[7]], dtype=dtype
        )
        ...
        with tempfile.NamedTemporaryFile() as tmp:
            ...
            workspace.RunOperatorOnce(
                core.CreateOperator("IndexGet", ["index", "query3"], ["result3"])
            )
