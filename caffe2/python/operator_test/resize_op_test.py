...
class TestResize(hu.HypothesisTestCase):
    ...
    @given(height_scale=st.floats(0.25, 4.0) | st.just(2.0),
           ...
           seed=st.integers(0, 65535),
           **hu.gcs)
    def test_nearest_NHWC(self, height_scale, width_scale, height, width,
                     num_channels, batch_size, seed,
                     gc, dc):
        ...
        X = np.random.rand(
            batch_size, height, width, num_channels).astype(np.float32)
    ...
    @given(height_scale=st.floats(0.25, 4.0) | st.just(2.0),
           ...
           seed=st.integers(0, 65535),
           **hu.gcs)
    def test_nearest_grad_NHWC(self, height_scale, width_scale, height, width,
                          num_channels, batch_size, seed, gc, dc):
