...
class TestInstanceNorm(serial.SerializedTestCase):
    ...
    def _get_op(
        self, device_option, store_mean, store_inv_stdev, epsilon, order, inplace=False
    ):
    ...
    def test_instance_norm_gradients(
        self, gc, dc, N, C, H, W, order, store_mean, store_inv_stdev, epsilon, seed
    ):
        np.random.seed(seed)
    ...
    def test_instance_norm_layout(
        self, gc, dc, N, C, H, W, store_mean, store_inv_stdev, epsilon, seed
    ):
        # force store_inv_stdev if store_mean to match existing forward pass
        # implementation
        store_inv_stdev |= store_mean
    ...
    def test_instance_norm_reference_check(
        ...
        def ref(input_blob, scale_blob, bias_blob):
            ...
            inv_stdev_blob = 1.0 / np.sqrt(
                input_blob.reshape((N, C, -1)).var(axis=2) + epsilon
            )
    ...
    def test_instance_norm_model_helper(
        ...
        brew.instance_norm(
            model, "input", "output", C, epsilon=epsilon, order=order, is_test=is_test
        )
