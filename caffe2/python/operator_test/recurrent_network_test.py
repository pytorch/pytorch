...
class RecurrentNetworkTest(serial.SerializedTestCase):
    ...
    def _convolution_1d(
        self, model, inputs, conv_window, conv_filter, conv_bias, output_name, left_pad
    ):
        if left_pad:
        ...
        inputs_transposed = model.net.Transpose(
            inputs, "inputs_transposed", axes=[1, 0, 2]
        )
        # [batch_size, 1, inputs_length, state_size]
        inputs_transposed_4d = model.net.ExpandDims(
            inputs_transposed, "inputs_transposed_4d", dims=[1]
        )
        ...
        output_transposed = model.net.Squeeze(
            output_transposed_4d, output_name + "_transposed", dims=[1]
        )
    ...
    def test_stateful_convolution_forward_only(
        self, sequence_length, conv_window, batch_size, state_size
    ):
        """
    ...
    """
        ...
        initial_output_state = model.param_init_net.ConstantFill(
            [], "initial_output_state", value=0.0, shape=[1, batch_size, state_size]
        )
        step_model = ModelHelper(name="step_model", param_model=model)
        (fake_input_t, timestep, input_state_t_prev) = step_model.net.AddExternalInputs(
            "fake_input_t", "timestep", "input_state_t_prev"
        )
        conv_filter = step_model.param_init_net.XavierFill(
            [], "conv_filter", shape=[state_size, 1, conv_window, state_size]
        )
        conv_bias = step_model.param_init_net.ConstantFill(
            [], "conv_bias", shape=[state_size], value=0.0
        )
        step_model.params.extend([conv_filter, conv_bias])
        input_state_t = step_model.net.UniformFill(
            [], "input_state_t", min=-1.0, max=1.0, shape=[1, batch_size, state_size]
        )
