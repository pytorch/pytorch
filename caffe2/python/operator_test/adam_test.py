...
class TestAdam(hu.HypothesisTestCase):
    ...
    def ref_adam(param, mom1, mom2, grad, LR, ITER, beta1, beta2, epsilon,
        ...
        assert (
            output_effective_lr + output_update <= 1
        ), "can't set both output_effective_lr and output_update to be true."

        if output_effective_lr or output_update:
            assert output_grad, "must set output_grad to be true "
            "if either output_effective_lr or output_effective_lr is set True"
