...
class TestBatchBucketize(serial.SerializedTestCase):
    ...
    def test_batch_bucketize_example(self, gc, dc):
        op = core.CreateOperator(
            "BatchBucketize", ["FEATURE", "INDICES", "BOUNDARIES", "LENGTHS"], ["O"]
        )
        float_feature = np.array(
            [
                [1.42, 2.07, 3.19, 0.55, 4.32],
        ...
        def ref(float_feature, indices, boundaries, lengths):
            output = np.array(
                [[2, 1, 1], [2, 1, 1], [1, 0, 0], [0, 2, 1], [2, 3, 0]], dtype=np.int32
            )
            return (output,)
    ...
    def test_batch_bucketize(self, x, seed, gc, dc):
        ...
        indices = np.random.choice(list(range(d)), d - 3, replace=False)
        indices.sort()
        boundaries = []
        for i in range(d - 3):
