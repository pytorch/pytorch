...
class TestkMaxPoolingOps(hu.HypothesisTestCase):
    @given(N=st.integers(min_value=0, max_value=10),
           K=st.integers(min_value=1, max_value=10),
           **hu.gcs_cpu_only)
    def test_kmax_pooling_op(self, N, K, gc, dc):
        lens = np.random.randint(low=1, high=2 * K + 1, size=N).astype(np.int32)
        X = []
        for i in lens:
            X.extend(map(lambda x: x / 100.0, range(0, 6 * i, 6)))
        X = np.array(X, dtype=np.float32)
        op = core.CreateOperator("kMaxPooling", ["X", "Y"], ["values", "indices"], k=K)

        def kmax_pooling(X, lens):
            N, si = lens.shape[0], 0
            values, indices = [], []
            for i in range(N):
        ...
        self.assertReferenceChecks(gc, op, [X, lens], kmax_pooling)
        self.assertGradientChecks(gc, op, [X, lens], 0, [0])

    @given(N=st.integers(min_value=0, max_value=10),
           K=st.integers(min_value=1, max_value=10),
           **hu.gcs_cpu_only)
    def test_kmax_pooling_empty_op(self, N, K, gc, dc):
