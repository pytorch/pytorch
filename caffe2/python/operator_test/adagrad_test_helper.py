...
def adagrad_sparse_test_helper(
    ...
    assert (
        output_effective_lr + output_effective_lr_and_update <= 1
    ), "can't set both output_effective_lr and output_effective_lr_and_update to be true."
    ...
    def ref_sparse(
        ...
        #update.fill(np.nan)
        effective_lr = np.zeros(param.shape)
        update = np.zeros(param.shape)
