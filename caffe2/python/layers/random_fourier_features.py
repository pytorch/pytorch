...
class RandomFourierFeatures(ModelLayer):
    ...
    def __init__(
        ...
        **kwargs
        ...
        assert self.output_dims >= 1, (
            "Expected output dimensions >= 1, got %s" % self.output_dims
        )

        self.output_schema = schema.Scalar(
            (np.float32, (self.output_dims,)), self.get_next_blob_reference("output")
        )
        ...
        w_init = (
            w_init if w_init else ("GaussianFill", {"mean": 0.0, "std": 1.0 / sigma})
        )
    ...
    def add_ops(self, net):
        ...
        cosine_arg = net.FC(
            self.input_record.field_blobs() + [self.w, self.b],
