    ...
    def __init__(
        ...
        **kwargs
        ...
        self.output_schema = schema.Struct(
            ...
            ("loss", schema.Scalar(np.float32, self.get_next_blob_reference("loss"))),
        )

    def initialize_label_smoothing_constants(self):
        assert self.label_smoothing_matrix is not None
        self.label_smoothing_matrix = np.array(self.label_smoothing_matrix).astype(
            np.float32
        )
        ...
        self.label_dim = self.model.add_global_constant(
            "%s_label_dim" % self.name, array=label_dim, dtype=np.dtype(np.int64)
        )
    ...
    def add_ops(self, net):
        ...
        if self.label_smoothing_matrix is not None:
            ...
            if self.input_record.label.field_types()[0].base != np.int32:
                label = [
                    net.Cast(
                        label, net.NextScopedBlob("int32_label"), to=core.DataType.INT32
                    )
                ]
