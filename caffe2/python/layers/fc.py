    ...
    def __init__(
        ...
        **kwargs
        ...
        if self.output_dim_vec is None or len(self.output_dim_vec) == 1:
            weight_shape = (
                [input_dims, output_dims] if transposed else [output_dims, input_dims]
            )
            ...
            for idx, output_dim in enumerate(self.output_dim_vec):
                weight_shape = (
                    [input_dims, output_dim] if transposed else [output_dim, input_dims]
                )
                self.w_vec.append(
                    self.create_param(
                        param_name="w_sub_{}".format(idx),
                        shape=weight_shape,
                        initializer=weight_init,
                        optimizer=weight_optim,
                        regularizer=weight_reg,
        ...
        self.output_schema = schema.Scalar(
            (np.float32, output_shape), self.get_next_blob_reference("output")
        )
