...
class SparseDropoutWithReplacement(ModelLayer):
    def __init__(
        ...
        **kwargs
        ...
        super(SparseDropoutWithReplacement, self).__init__(
        ...
        assert self.dropout_prob_train >= 0 and self.dropout_prob_train <= 1.0, (
            "Expected 0 <= dropout_prob_train <= 1, but got %s"
            % self.dropout_prob_train
        )
        assert self.dropout_prob_eval >= 0 and self.dropout_prob_eval <= 1.0, (
            "Expected 0 <= dropout_prob_eval <= 1, but got %s" % dropout_prob_eval
        )
        assert self.dropout_prob_predict >= 0 and self.dropout_prob_predict <= 1.0, (
            "Expected 0 <= dropout_prob_predict <= 1, but got %s" % dropout_prob_predict
        )
        assert (
            self.dropout_prob_train > 0
            or self.dropout_prob_eval > 0
            or self.dropout_prob_predict > 0
    ...
    def _add_ops(self, net, ratio):
        input_values_blob = list(self.input_record.items())
        input_lengths_blob = self.input_record.lengths()

        output_lengths_blob = self.output_schema.lengths()
        output_values_blob = list(self.output_schema.items())

        net.SparseDropoutWithReplacement(
            [input_values_blob, input_lengths_blob],
            [output_values_blob, output_lengths_blob],
            ratio=ratio,
