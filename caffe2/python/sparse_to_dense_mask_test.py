...
class TestSparseToDenseMask(TestCase):
    ...
    def test_sparse_to_dense_mask_float(self):
        ...
        workspace.FeedBlob(
            'values',
            np.array([1, 2, 3, 4, 5, 6, 7], dtype=np.float))  # type: ignore[attr-defined]
        workspace.FeedBlob('default', np.array(-1, dtype=np.float))  # type: ignore[attr-defined]
        workspace.FeedBlob('lengths', np.array([3, 4], dtype=np.int32))
        workspace.RunOperatorOnce(op)
        output = workspace.FetchBlob('output')
        expected = np.array([[-1, 1, 3], [6, 7, -1]], dtype=np.float)  # type: ignore[attr-defined]
    ...
    def test_sparse_to_dense_mask_invalid_inputs(self):
        ...
        workspace.FeedBlob(
            'values',
            np.array([1, 2, 3, 4, 5, 6], dtype=np.float))  # type: ignore[attr-defined]
        workspace.FeedBlob('default', np.array(-1, dtype=np.float))  # type: ignore[attr-defined]
    ...
    def test_sparse_to_dense_mask_subtensor(self):
        ...
        workspace.FeedBlob('default', np.array([[-1, 0]], dtype=np.float))  # type: ignore[attr-defined]
        workspace.FeedBlob('lengths', np.array([2, 3], dtype=np.int32))
        workspace.RunOperatorOnce(op)
        output = workspace.FetchBlob('output')
        expected = np.array([
            [[[-1, 0]], [[1, -1]], [[-1, 0]], [[-1, 0]]],
            [[[4, -4]], [[5, -5]], [[-1, 0]], [[3, -3]]]], dtype=np.float)  # type: ignore[attr-defined]
        self.assertEqual(output.shape, expected.shape)
        np.testing.assert_array_equal(output, expected)
    ...
    def test_sparse_to_dense_mask_empty_lengths(self):
        ...
        workspace.FeedBlob('values', np.array([1, 2, 3], dtype=np.float))  # type: ignore[attr-defined]
        workspace.FeedBlob('default', np.array(-1, dtype=np.float))  # type: ignore[attr-defined]
        workspace.RunOperatorOnce(op)
        output = workspace.FetchBlob('output')
        expected = np.array([-1, 1, 3], dtype=np.float)  # type: ignore[attr-defined]
        self.assertEqual(output.shape, expected.shape)
        np.testing.assert_array_equal(output, expected)
    ...
    def test_sparse_to_dense_mask_presence_mask(self):
        ...
        workspace.FeedBlob('values', np.array([11, 12, 13], dtype=np.float))  # type: ignore[attr-defined]
        workspace.FeedBlob('default', np.array(-1, dtype=np.float))  # type: ignore[attr-defined]
        ...
        expected_output = np.array([[11, -1], [-1, 12]], dtype=np.float)  # type: ignore[attr-defined]
