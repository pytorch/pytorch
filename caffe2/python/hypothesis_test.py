...
def _tensor_and_indices(
    min_dim=1, max_dim=4, dtype=np.float32, elements=None, **kwargs
):
    ...
    return st.tuples(data_dims_, original_dim).flatmap(
        lambda pair: st.tuples(
            ...
            hu.arrays(
                pair[0][0],
                dtype=np.int64,
                elements=st.integers(min_value=0, max_value=pair[1] - 1),
            ),
        )
    )
    ...
    @given(
        inputs=dtypes().flatmap(
            lambda dtype: hu.tensors(
                n=2, dtype=dtype, elements=hu.elements_of_type(dtype, filter_=filter_)
            )
        ),
...
def _test_binary_broadcast(
    name, ref, filter_=None, gcs=hu.gcs, allow_inplace=False, dtypes=_dtypes
):
    @given(
        inputs=dtypes().flatmap(
            lambda dtype: _tensor_and_prefix(
                dtype=dtype, elements=hu.elements_of_type(dtype, filter_=filter_)
            )
        ),
...
class TestOperators(hu.HypothesisTestCase):
    ...
    def test_div(self):
        ...
        _test_binary(
            "Div", ref, filter_=non_zero, test_gradient=False, dtypes=div_dtypes
        )(self)
    ...
    @given(
        device_options=st.lists(
            min_size=2, max_size=4, elements=st.sampled_from(hu.expanded_device_options)
        ),
    ...
    def test_random_seed_behaviour(self, device_options, set_seed):
        ...
        def run(do):
            ...
            op = core.CreateOperator(
                "XavierFill", [], ["Y"], device_option=do, shape=[2]
            )
            ws.run(op)
    ...
    @unittest.skipIf(
        not workspace.has_gpu_support, "Skipping test due to no gpu present."
    )
    @given(
    ...
    def test_recurrent(
        ...
        H = np.random.randn(num_layers, N, hidden_size * num_directions).astype(
            np.float32
        )
        C = (
            np.random.randn(num_layers, N, hidden_size * num_directions).astype(
                np.float32
            )
            if rnn_mode == "lstm"
            else np.empty((1,)).astype(np.float32)
        )  # unused in GRU
        inputs = [X, H, C, W]
        input_idxs = (
            [i for (i, _) in enumerate(inputs)] if rnn_mode == "lstm" else [0, 1, 3]
        )  # ignore C
        for input_idx in input_idxs:
            self.assertGradientChecks(
                device_option, op, inputs, input_idx, [0], stepsize=0.01, threshold=0.01
            )
    ...
    def test_depth_concat(self, ndim, axis, add_axis, num_inputs, gc, dc):
        ...
        op = core.CreateOperator(
            "Concat", input_names, ["Y", "Y_dims"], axis=axis, add_axis=add_axis
        )
        self.assertDeviceChecks(dc, op, inputs, [0])
        for i in range(num_inputs):
    ...
    @given(
        X=hu.arrays(dims=[5, 2], elements=st.floats(min_value=1.0, max_value=10.0)),
        **hu.gcs_cpu_only
    )
    def test_last_n_windows(self, X, gc, dc):
        ...
        collect_net.LastNWindowCollector(
            ["output", "next", "input"], ["output", "next"], num_to_collect=7
        )
    ...
    def test_momentum_sgd(self, inputs, in_place, momentum, nesterov, lr, gc, dc):
        ...
        op = core.CreateOperator(
            "MomentumSGD",
            ["grad", "m", "lr"],
            ["grad" if in_place else "grad_o", "m" if in_place else "m_o"],
            momentum=momentum,
    ...
    def test_ftrl_sgd(
        self, inputs, in_place, alpha, beta, lambda1, lambda2, engine, gc, dc
    ):
        var, n, z, grad = inputs
        n = np.abs(n)
        ...
        op = core.CreateOperator(
            "Ftrl",
            ["var", "nz", "grad"],
            ["var" if in_place else "var_o", "nz" if in_place else "nz_o"],
            alpha=alpha,
            beta=beta,
            lambda1=lambda1,
            lambda2=lambda2,
    ...
    def _dense_gftrl(alpha, beta, lambda1, lambda2, w, nz, g):
        ...
        w = (
            z
            * ((lambda1 * np.sqrt(output_dim)) / z_norms - 1)
    ...
    def test_sparse_ftrl_sgd(
        self, inputs, alpha, beta, lambda1, lambda2, engine, gc, dc
    ):
        var, n, z, grad = inputs
    ...
    def test_ftrl_sgd_send_alpha_by_input(
        self, inputs, in_place, alpha, beta, lambda1, lambda2, engine, gc, dc
    ):
        var, n, z, grad = inputs
        n = np.abs(n)
    ...
    def test_sparse_ftrl_sgd_send_alpha_by_input(
        self, inputs, alpha, beta, lambda1, lambda2, engine, gc, dc
    ):
        var, n, z, grad = inputs
        # generate fake subset manually because hypothesis is too complicated :)
        ...
        def ftrl(w, nz, i, g, alpha):
            sw, snz = self._dense_ftrl_send_alpha_by_input(
                beta, lambda1, lambda2, w[i], nz[i], g, alpha
            )
            w[i] = sw
            nz[i] = snz
            return (w, nz)
    ...
    @given(
        target_probabilities=hu.arrays(
            dims=[10],
            elements=st.floats(
                allow_nan=False, allow_infinity=False, min_value=0.01, max_value=1
            ),
        ),
        **hu.gcs
    )
    def test_perplexity(self, target_probabilities, gc, dc):
        ...
        self.assertReferenceChecks(
            device_option=gc, op=op, inputs=[target_probabilities], reference=op_ref
        )
    ...
    def test_multi_class_accuracy(self, prediction, labels, gc, dc):
        op = core.CreateOperator(
            "MultiClassAccuracy", ["prediction", "labels"], ["accuracies", "amounts"]
        )
        ...
        self.assertReferenceChecks(
            device_option=gc, op=op, inputs=[prediction, labels], reference=op_ref
        )
    ...
    @given(
        lengths=st.lists(
            st.integers(min_value=1, max_value=10), min_size=0, max_size=10
        ),
    ...
    def test_lengths_to_weights(self, lengths, power, gc, dc):
        op = core.CreateOperator(
            "LengthsToWeights", ["lengths"], ["weights"], power=power
        )
    ...
    @given(
        input_tensor=hu.arrays(
            dims=[10], elements=st.floats(allow_nan=False, allow_infinity=False)
        ),
        **hu.gcs
    )
    def test_abs(self, input_tensor, gc, dc):
        ...
        self.assertReferenceChecks(
            device_option=gc, op=op, inputs=[input_tensor], reference=abs_ref
        )

    @given(
        input_tensor=hu.arrays(
            dims=[10], elements=st.floats(min_value=-10, max_value=10)
        ),
        **hu.gcs
    )
    def test_cos(self, input_tensor, gc, dc):
        ...
        self.assertReferenceChecks(
            device_option=gc, op=op, inputs=[input_tensor], reference=cos_ref
        )
    ...
    def test_sin(self, input_tensor, gc, dc):
        ...
        self.assertReferenceChecks(
            device_option=gc, op=op, inputs=[input_tensor], reference=sin_ref
        )
    ...
    def test_exp(self, input_tensor, gc, dc):
        ...
        self.assertReferenceChecks(
            device_option=gc, op=op, inputs=[input_tensor], reference=exp_ref
        )

    @given(
        input_tensor=hu.arrays(
            dims=[10], elements=st.floats(min_value=1, max_value=10000)
        ),
        **hu.gcs_cpu_only
    )
    def test_log(self, input_tensor, gc, dc):
        ...
        self.assertReferenceChecks(
            device_option=gc, op=op, inputs=[input_tensor], reference=log_ref
        )
        self.assertGradientChecks(gc, op, [input_tensor], 0, [0])

    def test_blobs_dequeue_timeout(self):
        op = core.CreateOperator(
            "CreateBlobsQueue", [], ["queue"], capacity=5, num_blobs=1
        )
        self.ws.run(op)
        t = time.time()
    ...
    @given(
        num_threads=st.integers(1, 10),  # noqa
        num_elements=st.integers(1, 100),
        capacity=st.integers(1, 5),
        num_blobs=st.integers(1, 3),
        do=st.sampled_from(hu.device_options),
    )
    def test_blobs_queue_threading(
        self, num_threads, num_elements, capacity, num_blobs, do
    ):
        """
        ...
        xs = [
            np.random.randn(num_elements, 5).astype(np.float32)
        ...
        def enqueue(t):
            while True:
                ...
                op = core.CreateOperator(
                    "EnqueueBlobs", ["queue"] + feed_blobs, feed_blobs, device_option=do
                )
                try:
                    elems = q.get_nowait()
        ...
        threads = [
            threading.Thread(target=enqueue, args=(t,)) for t in range(num_threads)
        ]
        ...
        for n in range(num_elements):
            ...
            op = core.CreateOperator(
                "DequeueBlobs", ["queue"], dequeue_blobs, device_option=do
            )
            self.ws.run(op)
        for thread in threads:
            thread.join()
        ...
        ys = [
            np.vstack(
                [
                    self.ws.blobs["y_{}_{}".format(i, n)].fetch()
                    for n in range(num_elements)
                ]
            )
            for i in range(num_blobs)
        ...
        for i in range(num_blobs):
            ...
            for j in range(num_elements):
                ...
                self.assertTrue(
                    any(np.array_equal(xs[i][j], ys[i][k]) for k in range(num_elements))
                )

    @given(
        num_producers=st.integers(1, 10),
        num_consumers=st.integers(1, 10),
        capacity=st.integers(1, 5),
        num_blobs=st.integers(1, 3),
        do=st.sampled_from(hu.device_options),
    )
    def test_safe_blobs_queue(
        self, num_producers, num_consumers, capacity, num_blobs, do
    ):
        ...
        for i in range(num_consumers):
            ...
            consumer_steps.append(
                core.execution_step(name, [net1, net2], should_stop_blob=status)
            )
    ...
    def test_weighted_sample_blobs_queue(
        ...
        init_net = core.Net("init_net")
        queues = [
            init_net.CreateBlobsQueue([], 1, capacity=capacity, num_blobs=num_blobs)
        ...
        for i in range(num_queues):
            ...
            step = core.execution_step(
                name,
                [
                    core.execution_step("producer_%d" % i, [net], num_iter=num_iter),
    ...
    def test_dag_net_forking(self, net_type, num_workers, gc, dc):
        ...
        def run():
            ...
            for input_blob in input_blobs:
                self.ws.create_blob(input_blob).feed(
                    np.random.randn(n, d).astype(np.float32), device_option=gc
                )
        ...
        for output in outputs[1:]:
            ...
            self.assertAlmostEqual(np.sum(np.square(output)), 91.81752, delta=1e-2)

    @given(
        input=hu.tensor(min_dim=2, max_dim=6),
    ...
    def test_has_elements(self, x, y, gc, dc):
        ...
        self.assertReferenceChecks(
            gc, op, [x, y], lambda x, y: (len(x) > 0 or len(y) > 0,)
        )
    ...
    def test_should_stop_as_criteria_net_execution_step(self, initial_iters, max_iters):
        ...
        plan.AddStep(
            core.execution_step(
                "step", [criteria_net, net], should_stop_blob=core.BlobReference("stop")
            )
        )
        self.ws.run(plan)
    ...
    def test_disabled_execution_step(self):
        ...
        steps = [
            core.execution_step(
                "step_1", nets[0], should_stop_blob=core.BlobReference("should_stop_1")
            ),
            core.execution_step(
                "step_2", nets[1], should_stop_blob=core.BlobReference("should_stop_2")
            ),
        ...
        for i, _ in enumerate(nets):
            self.assertEqual(
                self.ws.blobs["output_{}".format(i + 1)].fetch()[0], expected[i]
            )
    ...
    def test_atomic_iter_with_concurrent_steps(
        ...
        if num_iters * num_nets > 0:
            ...
            self.assertEqual(
                num_iters * num_nets, stat_val[list(stats_key).index(atomic_iter_key)]
            )
    ...
    def test_logit(self, a, eps, a_grad, eps_grad, gc, dc):
        ...
        self.assertGradientChecks(
            gc, op_grad, [a_grad], 0, [0], threshold=0.04, stepsize=2e-3
        )

    @given(
        a=hu.tensor(elements=st.floats(allow_nan=True)),
    ...
    @given(
        data=_dtypes(dtypes=[np.int32, np.int64, np.float32, np.bool]).flatmap(
            lambda dtype: hu.tensor(
                min_dim=1, dtype=dtype, elements=hu.elements_of_type(dtype)
            )
        ),
        has_input=st.booleans(),
        has_extra_shape=st.booleans(),
        extra_shape=st.lists(min_size=1, max_size=5, elements=st.integers(1, 5)),
        **hu.gcs
    )
    def test_constant_fill(self, data, has_input, has_extra_shape, extra_shape, gc, dc):
        ...
        if has_input:
            if has_extra_shape:
                ...
                op = core.CreateOperator(
                    "ConstantFill", ["X"], ["Y"], dtype=enum_type, value=value
                )
        else:
            op = core.CreateOperator(
                "ConstantFill",
    ...
    def test_elman_recurrent_network(self, t, n, d):
        ...
        backward_mapping = {str(k): str(v) for k, v in viewitems(backward_mapping)}
        ...
        link_internal, link_external, link_offset = list(zip(*links))
        backward_links = [
            ("hidden_t_prev_grad", "hidden_grad", 0),
            ("hidden_t_grad", "hidden_grad", 1),
            ("gates_t_grad", "input_grad", 0),
        ]
        backward_link_internal, backward_link_external, backward_link_offset = list(
            zip(*backward_links)
        )
        ...
        op = core.CreateOperator(
            ...
            initial_recurrent_state_ids=[inputs.index(i) for i in recurrent_inputs],
    ...
    def test_space_to_batch(self, n, c, h, w, pad, block_size, gc, dc):
        ...
        op = core.CreateOperator(
            "SpaceToBatch", ["X"], ["Y"], pad=pad, block_size=block_size
        )
        self.assertDeviceChecks(dc, op, [X], [0])
        self.assertGradientChecks(gc, op, [X], 0, [0])
    ...
    def test_batch_to_space(self, n, c, h, w, pad, block_size, gc, dc):
        ...
        op = core.CreateOperator(
            "BatchToSpace", ["X"], ["Y"], pad=pad, block_size=block_size
        )
        self.assertDeviceChecks(dc, op, [X], [0])
        self.assertGradientChecks(gc, op, [X], 0, [0])
    ...
    def test_scale(self, X, in_place, scale, gc, dc):
        op = core.CreateOperator(
            "Scale", ["X"], ["Y" if not in_place else "X"], scale=scale
        )
        self.assertDeviceChecks(dc, op, [X], [0])
        self.assertGradientChecks(gc, op, [X], 0, [0])
    ...
    def test_same_pad_image(
        self, pad, size, input_channels, batch_size, order, mode, gc, dc
    ):
        assume(size > pad)

        op = core.CreateOperator(
            "PadImage", ["X"], ["Y"], pad=pad, mode=mode, order=order
        )
        if order == "NHWC":
            X = (
                np.random.rand(batch_size, size, size, input_channels).astype(
                    np.float32
                )
                - 0.5
            )
            ...
            X = (
                np.random.rand(batch_size, input_channels, size, size).astype(
                    np.float32
                )
                - 0.5
            )
    ...
    def test_pad_image(
        ...
        if order == "NHWC":
            ...
            def numpy_pad_ref(x):
                return (
                    np.pad(x, ((0, 0), (pad_t, pad_b), (pad_l, pad_r), (0, 0)), mode),
                )
            ...
            def numpy_pad_ref(x):
                return (
                    np.pad(x, ((0, 0), (0, 0), (pad_t, pad_b), (pad_l, pad_r)), mode),
                )
    ...
    def test_instance_norm(
        self, size, input_channels, batch_size, order, epsilon, gc, dc
    ):
        op = core.CreateOperator(
            "InstanceNorm", ["X", "scale", "bias"], ["Y"], order=order, epsilon=epsilon
        )
        np.random.seed(1701)
        ...
        self.assertReferenceChecks(
            gc, op, [X, scale, bias], ref_nchw if order == "NCHW" else ref_nhwc
        )
    ...
    def test_sparse_to_dense(self, inp, gc, dc):
        first_dim, X, I = inp
        if X.dtype != np.dtype("float32") and gc.device_type in {
            caffe2_pb2.CUDA,
            caffe2_pb2.HIP,
        }:
    ...
    def test_dot_product_with_padding(self, N, M, K, pad_value, gc, dc):
        ...
        op = core.CreateOperator(
            "DotProductWithPadding", ["X", "Y"], "out", pad_value=pad_value
        )
    ...
    def test_accumulate_histogram_op(self, N, M, num_buckets, gc, dc):
        ...
        def histogram(X):
            ...
            Y[(X >= lower_bound) & (X < upper_bound)] = (
                (X[(X >= lower_bound) & (X < upper_bound)] - lower_bound) / segment + 1
            ).astype(np.int32)
