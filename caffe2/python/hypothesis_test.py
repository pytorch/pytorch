...
class TestOperators(hu.HypothesisTestCase):
    ...
    @given(dtype=st.sampled_from([np.float32, np.float64, np.int32, np.bool_]))
    @settings(deadline=1000)
    ...
    def test_cast(self, a, src, dst, use_name, gc, dc):
        ...
        if src in ftypes and dst not in ftypes and dst is not np.bool_:
            info = np.iinfo(dst)
    ...
    @given(data=_dtypes(dtypes=[np.int32, np.int64, np.float32, np.bool_]).
    ...
    def test_constant_fill(self, data, has_input, has_extra_shape, extra_shape,
        ...
        # in opt mode, np.bool_ is converted into np.bool_
