...
class AdagradOptimizer(Optimizer):
    ...
    def _process_pruning_options(self, pruning_options):
        ...
        if self.mask_tensor is not None:
            ...
            self.use_mask = True
        if (
            self.mask_db_path is not None
            or self.mask_db_type is not None
            ...
            assert self.mask_blob_name is not None, (
                "when mask is provided through db, "
                "db path, db type, and blob name are all needed"
            )
    ...
    def _run(self, net, param_init_net, param_info):
        ...
        if self.use_mask is True:
            if self.mask_tensor is not None:
                ...
                raise NotImplementedError(
                    "If mask is used, it needs to be provided through a numpy array or a db file"
                )
