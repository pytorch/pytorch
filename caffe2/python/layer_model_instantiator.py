...
def shrink_output_schema(net, out_schema):
    ...
    return schema.from_column_list(
        [col_name for ok, col_name in zip(exists, out_schema.field_names()) if ok],
        [col_type for ok, col_type in zip(exists, out_schema.field_types()) if ok],
        [col_blob for ok, col_blob in zip(exists, out_schema.field_blobs()) if ok],
        [col_meta for ok, col_meta in zip(exists, out_schema.field_metadata()) if ok],
    )
...
def generate_training_nets(model, include_tags=None):
    ...
    model.apply_post_grad_net_modifiers(
        train_net, train_init_net, grad_map, modify_output_record=True
    )
    ...
    model.apply_final_net_modifiers(
        train_net, train_init_net, grad_map, modify_output_record=True
    )
