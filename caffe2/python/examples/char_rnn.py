...
class CharRNN(object):
    ...
    def CreateModel(self):
        ...
        input_blob, seq_lengths, hidden_init, cell_init, target = model.net.AddExternalInputs(
    ...
    def TrainModel(self):
        ...
        while True:
            ...
            input = np.zeros([self.seq_length, self.batch_size, self.D]).astype(
                np.float32
            )
            ...
            if num_iter % self.iters_to_report == 0:
                ...
                log.debug(
                    "Loss since last report: {}".format(last_n_loss / last_n_iter)
                )
...
@utils.debug
def main():
    parser = argparse.ArgumentParser(description="Caffe2: Char RNN Training")
    parser.add_argument(
        "--train_data",
        type=str,
        default=None,
    ...
    parser.add_argument(
        "--gpu", action="store_true", help="If set, training is going to use GPU 0"
    )
