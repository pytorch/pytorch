...
def Parallelize(
    ...
    max_concurrent_distributed_ops = min(
        max_concurrent_distributed_ops, num_workers - 1
    )
    ...
    has_parameter_updates = (
        param_update_builder_fun is not None or optimizer_builder_fun is not None
    )
    assert not (
        param_update_builder_fun is not None and optimizer_builder_fun is not None
    ...
    model_helper_obj._device_grouped_blobs = _GroupByDevice(
        model_helper_obj, devices, model_helper_obj.params, non_datapar_params
    )

    # computed params
    computed_params_grouped = _GroupByDevice(
        model_helper_obj, devices, model_helper_obj.GetComputedParams(""), []
    )
    model_helper_obj._device_grouped_blobs.update(computed_params_grouped)

    model_helper_obj._param_names = list(
        viewkeys(model_helper_obj._device_grouped_blobs)
    )
    model_helper_obj._computed_param_names = list(viewkeys(computed_params_grouped))
    ...
    gradients_grouped = _GroupByDevice(
        model_helper_obj, devices, grads_ordered, non_datapar_grads
    )
    ...
    assert not (
        optimize_gradient_memory and dynamic_memory_management
...
def Parallelize_BMUF(
    ...
    model_parameter_names = list(viewkeys(model_helper_obj._device_grouped_blobs))
    ...
    if reset_momentum_sgd:
        momentum_ops = [
            op
            for op in model_helper_obj.net.Proto().op
...
def _AddBarrierToModelNets(model, barrier_net_timeout_sec):
    if model._rendezvous is not None and model._rendezvous["engine"] == "GLOO":
        ...
        model._barrier_net = _CreateBarrierNet(
            model, model._barrier_init_net, "pre_training", barrier_net_timeout_sec
        )
...
def _CreateBarrierNet(model, init_net, name_prefix, timeout_sec):
    ...
    barrier_net.Barrier(
        inputs=[comm_world], outputs=[], engine=model._rendezvous["engine"]
    )
    return barrier_net
...
def Synchronize(model, timeout_sec=_DEFAULT_BARRIER_NET_TIMEOUT_SEC):
    warnings.warn(
        "The Synchronize API has been deprecated.  We now have a "
...
def _ForEachDevice(
    devices, f, device_type, device_prefix, scoped=False, *args, **kwargs
):
    for device in devices:
...
def ExtractPredictorNet(model, inputs, outputs, device):
    ...
    (predictor_net, export_blobs) = model_helper.ExtractPredictorNet(
        ...
        device=device,
        renames={
            a: b for (a, b) in zip(prefix_inputs + prefix_outputs, inputs + outputs)
        },
    )
...
def GetCheckpointParams(model):
    ...
    first_gpu_blobs = {
        b
        for b in all_blobs
        if str(b).startswith("{}_{}/".format(model._device_prefix, model._devices[0]))
    }
            ...
            if name not in model._device_grouped_blobs:
                grouped = {
                    d: core.BlobReference(
                        "{}_{}{}{}".format(
                            model._device_prefix, d, scope._NAMESCOPE_SEPARATOR, name
                        )
                    )
                    for d in devices
                }
...
def GetLearningRateBlobNames(model):
    ...
    if model._optimizer is not None:
        if (
            ...
            return [
                model._optimizer.get_gpu_blob_name("lr", gpu, "")
...
def _Broadcast(devices, model, net, param, use_nccl=False):
    ...
    for dev_idx in devices[1:]:
        if _IsGPUBlob(model, param):
            ...
            device_opt = (
                core.DeviceOption(caffe2_pb2.IDEEP, 0)
                if _IsIDEEPBlob(model, param)
                else core.DeviceOption(caffe2_pb2.CPU, 0)
            )
        with core.DeviceScope(device_opt):
            net.Copy(
    ...
    def sumN(*dev_indices):
        ...
        with core.DeviceScope(device_opt):
            for i, peer in enumerate(devices):
                ...
                if (
                    p2p_access_pattern is not None
                    and p2p_access_pattern.size
                    and not p2p_access_pattern[devices[0], peer]
                ):
                    # Copy from peer to d0
                    blobs[i] = model.Copy(
                        blobs[i], "gpu_{}/{}_gpu{}_copy".format(devices[0], param, peer)
                    )
...
def AddBlobSync(model, blobs, net=None):
    ...
    _SyncAllParams(
        model._devices, model, model.param_init_net, net, model._rendezvous, set(blobs)
    )
...
def AddDistributedBlobSync(model, blobs):
    ...
    comm_world = _CreateOrCloneCommonWorld(
        model.param_init_net, "blob_sync_cw_" + synth_name, rendezvous=model._rendezvous
    )

    model.net.Allreduce(
        inputs=[comm_world] + blobs, outputs=blobs, engine=model._rendezvous["engine"]
    )
...
def _SyncAllParamsDistributed(
    ...
    if model._broadcast_context is None:
        model._broadcast_context = CollectivesConcurrencyControl(
            "broadcast", max_concurrent_distributed_ops, init_net, rendezvous
        )
    ...
    for param_name in sorted(unique_param_names):
        ...
        device_opt = (
            gpu_device_opt
            if _IsGPUBlob(model, param_name)
...
def _AllReduceBlobs(
    ...
    if rendezvous is None or rendezvous["num_shards"] <= 1:
        ...
        _AllReduceBlobsDistributed(
            blob_names, devices, model, net, rendezvous, max_concurrent_distributed_ops
        )
...
def _PruneParametersForSharing(model):
    ...
    model.param_to_grad = {
        p: model.param_to_grad[p] for p in model.param_to_grad if p in paramset
    }
...
def _AllReduceBlobsDistributed(
    ...
    context = CollectivesConcurrencyControl(
        "allreduce", max_concurrent_distributed_ops, model.param_init_net, rendezvous
    )
    ...
    for blob_name in blob_names:
        ...
        if rendezvous["engine"] == "GLOO":
            ...
            allreduce(
                blobs_group, gpu_direct=(rendezvous.get("transport", None) == "ibverbs")
            )
        else:
            ...
            with core.DeviceScope(master_device_opt):
                ...
                net.NCCLAllreduce(
                    blobs_group, blobs_group, control_input=nccl_control_blob
                )
...
def _AllReduceBlobsSingleHost(blob_names, devices, model, net, use_nccl):
    ...
    for blob_name in blob_names:
        ...
        assert len(blobs_group) == len(
            devices
        ), "Each GPU from {}, should have a copy of {}.".format(devices, blob_name)
        ...
        if _IsGPUBlob(model, blob_name):
            ...
            assert not isinstance(
                blobs_group[0], core.GradientSlice
...
def _GroupByDevice(model, devices, params, non_data_params):
    ...
    for _i, p in enumerate(params):
        assert isinstance(p, core.BlobReference) or isinstance(
            p, core.GradientSlice
        ...
        if isinstance(p, core.BlobReference):
            ...
            assert (
                "{}_{}/".format(model._device_prefix, gpuid) in p.GetNameScope()
            ...
            assert (
                "{}_{}/".format(model._device_prefix, gpuid) in p.indices.GetNameScope()
            ), "Indices {} expected to have namescope '{}_{}'".format(
                str(p), model._device_prefix, gpuid
            )
            assert (
                "{}_{}/".format(model._device_prefix, gpuid) in p.values.GetNameScope()
...
def _ValidateParams(params):
    ...
    if len(params) > len(set_params):
        ...
        assert len(params) == len(set_params), "Duplicate entries in params: {}".format(
            dupes
        )
...
def _ComputeBlobsToSync(model):
    ...
    if model._shared_model:
        ...
        for op in model.param_init_net.Proto().op:
            dp_outputs = [
                o for o in op.output if o.startswith("{}_".format(model._device_prefix))
            ]
...
def _AddDynamicMemoryOptimization(model, blobs_to_keep, devices):
    ...
    model.net._net = memonger.release_blobs_when_used(
        model.net.Proto(), blobs_to_keep_all_devices
    )
...
def OptimizeGradientMemory(model, input_shapes, excluded_blobs, recycle_activations):
    ...
    if input_shapes is not None:
        ...
        (shapes, types) = workspace.InferShapesAndTypes(
            [model.param_init_net, model.net], input_shapes_all_devices
        )
    else:
        shapes = None
...
def _CreateOrCloneCommonWorld(
    net, common_world_blob, rendezvous, name=None, timeout_sec=None
):
    ...
    if existing is not None:
        comm_world = net.CloneCommonWorld(
            [existing], common_world_blob, name=name, engine=rendezvous["engine"]
        )
    else:
        kwargs = dict()
...
def _RunComparison(model, blob_name, device=None):
    ...
    with core.DeviceScope(device):
        ...
        for i in range(rendezvous["num_shards"]):
            assert gather_arr[i] == baseline, "allcompare failed on shard {}.".format(
                rendezvous["shard_id"]
            )
...
def _InterleaveOps(model):
    ...
    assert num_devices * num_ops_per_dev == len(
        orig_ops
    ...
    for j in range(num_ops_per_dev):
        ...
        for d in model._devices:
            ...
            # Sanity
            assert ops[d][j].type == tp, "Type mismatch {} / {}".format(
                tp, ops[d][j].type
            )
    ...
    for op in orig_ops:
        if op.type != "SpatialBN" and op.type != "SpatialBNGradient":
            if spatial_bn_phase:
                ...
                new_ops.append(
                    core.CreateOperator(
                        "Sum", sums_blobs, input_blob_name + "_sums_combined"
                    )
                )
                new_ops.append(
                    core.CreateOperator(
                        "Sum", sumsq_blobs, input_blob_name + "_sumsq_combined"
                    )
                )
            ...
            injected_ops.append(
                core.CreateOperator(
                    "ChannelStats", name, [name + "_sums", name + "_sumsq"]
                )
            )
            ...
            injected_ops.append(
                core.CreateOperator(
                    ...
                    [op.input[0], op.input[3], op.input[4], op.input[2]],
    ...
    for op in orig_ops:
        if op.type != "SpatialBN" and op.type != "SpatialBNGradient":
            ...
            device_option = core.DeviceOption(
                model._device_type, op.device_option.device_id
            )
            injected_ops.append(
                core.CreateOperator(
                    "ChannelStats",
