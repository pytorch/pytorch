    ...
    **kwargs
    ...
    if requested_engine is not None:
        if use_cudnn and requested_engine != "CUDNN":
            raise ValueError(
                "When use_cudnn=True, the only engine you can specify is " '"CUDNN"'
            )
    ...
    if group != 1:
        kwargs["group"] = group
    if is_nd:
        return model.net.Conv(inputs, blob_out, kernels=kernels, order=order, **kwargs)
    else:
        if isinstance(kernel, list):
            ...
            return model.net.Conv(
                inputs, blob_out, kernel=kernel, order=order, **kwargs
            )
...
def conv_transpose(
    ...
    **kwargs
    ...
    weight_shape = (
        ...
        if order == "NCHW"
        else [dim_in, kernel, kernel, dim_out]
    )
    ...
    return model.net.ConvTranspose(
        [blob_in, weight, bias], blob_out, kernel=kernel, order=order, **kwargs
    )
...
def group_conv_deprecated(
    ...
    **kwargs
    ...
    for i in range(group):
        if model.init_params:
            weight = model.param_init_net.__getattr__(weight_init[0])(
                [], blob_out + "_gconv_%d_w" % i, shape=weight_shape, **weight_init[1]
            )
            if use_bias:
                bias = model.param_init_net.__getattr__(bias_init[0])(
                    [],
        ...
        conv_blobs.append(
            splitted_blobs[i].Conv(
                inputs, blob_out + "_gconv_%d" % i, kernel=kernel, order=order, **kwargs
            )
        )
    concat, concat_dims = model.net.Concat(
        conv_blobs, [blob_out, "_" + blob_out + "_concat_dims"], order=order
    )
    return concat
