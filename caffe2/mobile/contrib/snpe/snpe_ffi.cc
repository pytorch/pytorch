  ...
  do {                                                                 \
    if (!(condition)) {                                                \
      throw std::runtime_error(                                        \
          std::string("Exception in SNPE: ") + std::string(__FILE__) + \
...
struct SNPEContext {
 public:
  SNPEContext(
    ...
    SNPE_ENFORCE(zdl::SNPE::SNPEFactory::isRuntimeAvailable(
        zdl::DlSystem::Runtime_t::GPU));

    dnn_ = snpeBuilder
               .setOutputLayers({}) // Just the last one is fine.
    ...
    inputTensor_ =
        zdl::SNPE::SNPEFactory::getTensorFactory().createTensor(inputDims_);

    SNPE_ENFORCE(
        dnn_->getOutputLayerNames() &&
        (*dnn_->getOutputLayerNames()).size() >= 1);
  }

  const zdl::DlSystem::Optional<zdl::DlSystem::TensorShape>& getInputDims()
      const {
    return inputDims_;
  };

  const std::vector<std::vector<size_t>>& run(
    ...
    // Copy input data.
    memcpy(
        inputTensor_->begin().dataPointer(),
        inputData,
        (count * sizeof(float)));
    ...
    for (auto name : outputTensors_.getTensorNames()) {
      ...
      outputDims_.push_back(
          std::vector<size_t>(dims, dims + outputTensor->getShape().rank()));
    }
  ...
  void copyOutputTo(float* outputData) {
    const auto& outputTensor =
        outputTensors_.getTensor(*outputTensors_.getTensorNames().begin());
    SNPE_ENFORCE(outputTensor);
    memcpy(
        outputData,
        outputTensor->begin().dataPointer(),
        (outputTensor->getSize() * sizeof(float)));
  }
...
bool snpe_has_gpu() {
  return zdl::SNPE::SNPEFactory::isRuntimeAvailable(
      zdl::DlSystem::Runtime_t::GPU);
}
