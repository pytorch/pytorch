...
class SNPEOp final : public Operator<CPUContext> {
 public:
  SNPEOp(const OperatorDef& def, Workspace* ws)
      : Operator<CPUContext>(def, ws),
        model_buffer_(
            OperatorBase::GetSingleArgument<string>("model_buffer", "")),
        input_name_(
            OperatorBase::GetSingleArgument<string>("input_name", "data")) {
    CAFFE_ENFORCE(
        gSNPELocation() != "",
        "SNPE library \"",
        gSNPELocation(),
    ...
    handle_ = deleted_unique_ptr<void>(
        dlopen(snpe_ffi.str().c_str(), RTLD_LAZY), [](void* handle) {
          if (handle) {
            dlclose(handle);
          }
        });
...
#define X(n)                                            \
  ...
  CAFFE_ENFORCE(n##_f, dlerror());
    ...
    ctx_ = deleted_unique_ptr<void>(
        snpe_create_f(
            reinterpret_cast<const unsigned char*>(model_buffer_.data()),
            model_buffer_.length(),
            input_name_.c_str()),
        [this](void* ctx) {
          if (ctx) {
            X(snpe_destroy);
  ...
  bool RunOnDevice() override {
    ...
    if (Input(0).ndim() != dimSize) {
      if (dimSize == 3 && dimSize == Input(0).ndim() - 1 &&
          Input(0).dim32(0) == 1) {
        const int C = Input(0).dim32(1);
        const int H = Input(0).dim32(2);
        const int W = Input(0).dim32(3);
        if (dims[0] != C || dims[1] != H || dims[2] != W) {
          CAFFE_THROW(
              "Input size must match what SNPE expects, which in this case is: ",
              dims[0],
              " ",
              dims[1],
              " ",
      ...
      for (auto i = 0; i < Input(0).ndim(); ++i) {
        CAFFE_ENFORCE_EQ(
            dims[i],
            Input(0).dim32(i),
            "SNPE input dimension is not compatible.");
      }
    }
    ...
    snpe_run_f(
        ctx_.get(), Input(0).data<float>(), Input(0).size(), &dims, &dimSize);
