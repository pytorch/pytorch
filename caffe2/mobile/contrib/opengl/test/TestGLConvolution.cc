...
void AddNoiseInput(const std::vector<caffe2::int64_t>& shape,
...
double BenchOp(const std::string& typ,
  ...
  AddNoiseInput(std::vector<caffe2::int64_t>{1, inputC, inH, inW}, "X", ws);
  if (transposed) {
    AddNoiseInput(std::vector<caffe2::int64_t>{inputC, outputC, kH, kW}, "W", ws);
  } else {
    AddNoiseInput(std::vector<caffe2::int64_t>{outputC, inputC, kH, kW}, "W", ws);
  }
  AddNoiseInput(std::vector<caffe2::int64_t>{outputC}, "B", ws);
...
static double BenchGLConvolution(int input_channels,
  ...
  AddNoiseInput(
      std::vector<caffe2::int64_t>{1, input_channels, input_height, input_width}, "X_cpu", ws);
  if (transposed) {
    AddNoiseInput(
        std::vector<caffe2::int64_t>{input_channels, output_channels, kernel_height, kernel_width},
        "W",
        ws);
  } else {
    AddNoiseInput(
        std::vector<caffe2::int64_t>{output_channels, input_channels, kernel_height, kernel_width},
        "W",
        ws);
  }
  AddNoiseInput(std::vector<caffe2::int64_t>{output_channels}, "b", ws);
