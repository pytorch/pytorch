  ...
  GLConvolution(
      ...
      : GLFilter(
            ...
            {{"KERNEL_SIZE_X", c10::to_string(_geometry.kernel_size.x)},
             ...
             {"INPUT_TILE_WIDTH",
              c10::to_string(_geometry.input_tile_size.x)},
             {"INPUT_TILE_HEIGHT",
              c10::to_string(_geometry.input_tile_size.y)},
             {"OUTPUT_TILE_WIDTH",
              c10::to_string(_geometry.output_tile_size.x)},
             {"OUTPUT_TILE_HEIGHT",
              c10::to_string(_geometry.output_tile_size.y)},
             {"INPUT_TILE_X",
              c10::to_string(_geometry.input_tile_grid_size.x)},
             {"OUTPUT_TILE_X",
              c10::to_string(_geometry.output_tile_grid_size.x)},
             {"INPUT_TILE_CHUNK_SIZE",
              c10::to_string(_input_tile_chunk_size)},
             ...
             {"TRANSPOSED_CONVOLUTION",
              c10::to_string(_geometry.transposed)},
             {"BOUNDS_CHECK_MODE",
 ...
 private:
  ...
  const std::vector<binding*> input_bindings(
    ...
    for (int i = 0; i < input_batch_size; i++) {
      bindings.push_back(
          inputData[i] =
              new binding{"inputData[" + c10::to_string(i) + "]"});
    }
