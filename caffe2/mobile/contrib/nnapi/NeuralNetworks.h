...
typedef enum {
   ...
   * * 0: A 4-D tensor, of shape [batches, height, width, depth], specifying the
   * input.
   * * 1: An INT32 value, specifying the implicit padding scheme, has to be one
   * of the
   ...
   *
   * Inputs:
   * * 0 ~ n-1: The list of n input tensors, of shape [D0, D1, ..., Daxis(i),
   * ..., Dm]. For inputs of {@link ANEURALNETWORKS_TENSOR_QUANT8_ASYMM} type,
   * all input tensors must have the same scale and zeroPoint.
   ...
   * be of {@link ANEURALNETWORKS_TENSOR_INT32}, with zeroPoint of 0 and
   ...
   * be of {@link ANEURALNETWORKS_TENSOR_INT32}, with zeroPoint of 0 and
   *      bias_scale == input_scale * filter_scale.
   * * 3: An INT32 value, specifying the implicit padding scheme, has to be one
   * of the
   *      {@link PaddingCode} values.
   ...
   * Outputs:
   * * 0: The output 4-D tensor, of shape [batches, out_height, out_width,
   * depth_out]. For output tensor of {@link
   * ANEURALNETWORKS_TENSOR_QUANT8_ASYMM} type, the following condition must be
   ...
   *
   * Outputs:
   * * 0: The output tensor, of the same type and dimensions as the input
   * tensor.
   */
   ...
   *      For output tensor of {@link ANEURALNETWORKS_TENSOR_QUANT8_ASYMM} type,
   ...
   *
   * Inputs:
   * * 0: Lookups. A 1-D {@link ANEURALNETWORKS_TENSOR_INT32} tensor with shape
   * [ k ].
   * * 1: Keys. A 1-D {@link ANEURALNETWORKS_TENSOR_INT32} tensor with shape [ n
   * ]; Keys and Values pair represent a map, i.e., the ith element in Keys
   * (Keys[i]) is the key to select the ith sub-tensor in Values (Values[i]),
   * where 0 <= i <= n-1. Keys tensor *MUST* be sorted in ascending order.
   ...
   * and scale 1.0f. A non-zero byte represents True, a hit. A zero indicates
   * otherwise.
   */
   ...
   *      For output tensor of {@link ANEURALNETWORKS_TENSOR_QUANT8_ASYMM} type,
   * the following condition must be satisfied: output_scale > input1_scale *
   * input2_scale.
   */
   ...
   *
   * Resized images will be distorted if their output aspect ratio is not the
   * same as input aspect ratio.
   *
   * Supported tensor types:
   * * {@link ANEURALNETWORKS_TENSOR_FLOAT32}
   *
   ...
   * * 1: A FLOAT32 value, specifying the positive scaling factor for the
   * exponent, beta.
   *
   * Outputs:
   * * 0: The output tensor of same shape as input0.
 ...
 *    <li>Complete the compilation with {@link
 * ANeuralNetworksCompilation_finish}.</li> <li>Use the compilation as many
 * times as needed with {@link ANeuralNetworksExecution_create}.</li>
 ...
 * complete with {@link ANeuralNetworksEvent_wait}.</li> <li>Destroy the
 * execution with
 *        {@link ANeuralNetworksExecution_free}.</li></ul></p>
 *
 * <p>An execution cannot be modified once {@link
 * ANeuralNetworksExecution_startCompute} has been called on it.</p>
 *
