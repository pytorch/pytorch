...
int main(int argc, char** argv) {
  ...
  CAFFE_CHECK(ReadProtoFromFile(caffe2::FLAGS_init_net, &net_def));
  CAFFE_CHECK(workspace->RunNetOnce(net_def));
  CAFFE_CHECK(ReadProtoFromFile(caffe2::FLAGS_net, &net_def));
