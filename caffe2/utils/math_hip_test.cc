...
void executeGpuBinaryOpTest(
    ...
    std::function<
        void(int N0, int N1, const float* src0, const float* src1, float* dst, HIPContext* context)>
        operation,
    std::function<float(int)> correct_output)
{
    ...
    {
        math::Set<float, HIPContext>(1, input0(i), tensorx0->mutable_data<float>() + i, &context);
    }
    for(int i = 0; i < shapex1; i++)
    {
        math::Set<float, HIPContext>(1, input1(i), tensorx1->mutable_data<float>() + i, &context);
    }
    operation(shapex0,
              shapex1,
...
TEST(MathUtilGPUTest, testAddStripedBatch)
{
    ...
    for(int j = 0; j < 9; j++)
    {
        ...
        for(int k = 0; k < 33; k++)
        {
            math::Set<float, HIPContext>(
                33, 1.0 + j + k, tensorx->mutable_data<float>() + j * stripe + k * 25, &context);
            tot[k] += 1.0 + j + k;
        }
    }
    ...
    math::Set<float, HIPContext>(stripe, 0.0, tensory->mutable_data<float>(), &context);

    math::AddStripedBatch<float, HIPContext>(stripe,
...
TEST(MathUtilGPUTest, testCopyVector)
{
    executeGpuBinaryOpTest(
        ...
        [](int N0,
           ...
           const float* /*src1*/,
           float* dst,
           HIPContext* context) { math::CopyVector<float, HIPContext>(N0, src0, dst, context); },
        [](int i) { return 5.0f - i; });
}
