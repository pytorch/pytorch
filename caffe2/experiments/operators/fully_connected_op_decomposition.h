...
class FullyConnectedOpDecomp final : public Operator<Context> {
 public:
  ...
  bool RunOnDevice() override {
    ...
    CHECK_EQ(K, V.dim32(0));
    CHECK_EQ(N, b.dim32(0));
    if (X.ndim() > 1) {
      Y->Resize(vector<TIndex>{M, N});
      multi_buffer_.Resize(vector<TIndex>{M, middle});
    } else {
      Y->Resize(vector<TIndex>{N});
      multi_buffer_.Resize(vector<TIndex>{middle});
    }
...
class FullyConnectedDecompGradientOp : public Operator<Context> {
 public:
  ...
  bool RunOnDevice() override {
    ...
    // Compute dU
    // first compute X * V
    du_buffer_.Resize(vector<TIndex>{N, middle});
    ...
    // Compute dV
    // first compute dY * U
    dv_buffer_.Resize(vector<TIndex>{M, middle});
    ...
    if (OutputSize() == 4) {
      auto* dX = Output(3);
      dX->ResizeLike(X);
      dx_buffer_.Resize(vector<TIndex>{M, middle});
