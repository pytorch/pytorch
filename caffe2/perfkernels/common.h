...
#define AVX512_DO(funcname, ...)                       \
  ...
  if (GetCpuId().avx512f() && GetCpuId().avx512dq() && \
      GetCpuId().avx512vl()) {                         \
    return caffe2::##funcname##__avx512(__VA_ARGS__);  \
  }
