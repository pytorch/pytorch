import torch
# A = torch.rand(256, 128).cuda()
# B = torch.rand(128, 64).cuda()
# torch.cusparselt_linear(A, B)
...
for output_channel, input_channel in zip(output_channels, input_channels):
    activation = torch.rand(num_batches, first_activation_dim, input_channel, device="cuda", dtype=torch.bfloat16)
