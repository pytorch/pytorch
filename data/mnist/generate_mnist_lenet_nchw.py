...
with open('mnist_lenet_nchw.pbtxt', 'w') as fid:
...
plan.AddStep(core.ExecutionStep("init", init_net))
plan.AddStep(core.ExecutionStep("train", train_net, 1000))
with open('mnist_lenet_nchw_gpu.pbtxt', 'w') as fid:
  fid.write(str(plan.Proto()))

# CUDNN version: it is fine to mark every operator to use engine cudnn, because
# if an operator does not have the cudnn engine, we will just fall back to the
# default option.
init_net.RunAllOnGPU(use_cudnn=True)
train_net.RunAllOnGPU(use_cudnn=True)
plan = core.Plan("mnist_lenet")
plan.AddNets([init_net, train_net])
plan.AddStep(core.ExecutionStep("init", init_net))
plan.AddStep(core.ExecutionStep("train", train_net, 1000))
with open('mnist_lenet_nchw_cudnn.pbtxt', 'w') as fid:
  fid.write(str(plan.Proto()))
