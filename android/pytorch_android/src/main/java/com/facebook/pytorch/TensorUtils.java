...
public final class TensorUtils {
  ...
  public static Tensor bitmapToFloatTensorTorchVisionForm(final Bitmap bitmap, int x, int y,
    ...
    final int dims[] = new int[]{1, 3, height, width};
    return Tensor.newFloatTensor(dims, floatArray);
  }

  public static Tensor imageYUV420CenterCropToFloatTensorTorchVisionForm(
      final Image image, final int tensorWidth, final int tensorHeight) {
    if (image.getFormat() != ImageFormat.YUV_420_888) {
      throw new IllegalArgumentException("Image format " + image.getFormat() + " != ImageFormat" +
          ".YUV_420_888");
    }
    ...
    float tx = (float) ccSize / tensorWidth;
    float ty = (float) ccSize / tensorHeight;
    int uvRowStride = uRowStride >> 1;

    int cSize = tensorHeight * tensorWidth;
    final int tensorInputOffsetG = cSize;
    final int tensorInputOffsetB = 2 * ccSize;
    ...
    for (int x = 0; x < tensorWidth; x++) {
      for (int y = 0; y < tensorHeight; y++) {
        ...
        // scaling as nearest
        final int ccX = (int) Math.floor(x * tx);
        final int ccY = (int) Math.floor(y * ty);

        // turn clockwise 90 degrees
        final int srcX = ccY + offsetX;
        final int srcY = (ccSize - 1) - ccX + offsetY;
    ...
    final int dims[] = new int[]{1, 3, tensorHeight, tensorHeight};
    return Tensor.newFloatTensor(dims, floatArray);
  }
