...
for ALG_ID in range(4):
    print(f"CuSparseLt v0.5.0 | alg_id: {ALG_ID}")
    total_sp24 = 0
    total_dense = 0
    for count, input_shape, weight_shape in [
        ...
        print(f"input: {list(input.shape)} | weight: {list(weight.shape)}")

        total_sp24  += count * benchmark_fn("  gemm@sp24", torch._cslt_sparse_mm, weight_sp24, input.T, transpose_result=True, alg_id=ALG_ID)
