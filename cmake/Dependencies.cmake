...
macro(disable_ubsan)
  if(CMAKE_C_FLAGS MATCHES ${UBSAN_FLAG} OR CMAKE_CXX_FLAGS MATCHES
                                            ${UBSAN_FLAG})
...
if(USE_CUDA)
  ...
  if(CAFFE2_USE_CUDA)
    ...
    # torch::cudart is dealt with separately, due to CUDA_ADD_LIBRARY design
    # reason (it adds CUDA_LIBRARIES itself).
    ...
    if(SCCACHE_EXECUTABLE)
      # Using RSP/--options-file renders output noncacheable by sccache as they
      # fall under `multiple input files` non-cacheable rule
      set(CMAKE_CUDA_USE_RESPONSE_FILE_FOR_INCLUDES 0)
      set(CMAKE_CUDA_USE_RESPONSE_FILE_FOR_LIBRARIES 0)
      set(CMAKE_CUDA_USE_RESPONSE_FILE_FOR_OBJECTS 0)
    endif()
  else()
    message(WARNING "Not compiling with CUDA. Suppress this warning with "
                    "-DUSE_CUDA=OFF.")
...
if(USE_ASAN OR USE_TSAN)
  ...
  if(USE_ASAN)
    if(TARGET Sanitizer::address)
      ...
      message(
        WARNING "Not ASAN found. Suppress this warning with -DUSE_ASAN=OFF.")
  ...
  if(USE_TSAN)
    if(TARGET Sanitizer::thread)
      ...
      message(
        WARNING "Not TSAN found. Suppress this warning with -DUSE_TSAN=OFF.")
...
set_property(
  CACHE BLAS PROPERTY STRINGS
                      "ATLAS;BLIS;Eigen;FLAME;Generic;MKL;OpenBLAS;vecLib;APL")
...
if(NOT INTERN_BUILD_MOBILE)
  ...
  if(NOT
     (ATLAS_FOUND
      ...
      OR APL_FOUND))
    message(
      WARNING "Preferred BLAS (" ${BLAS}
              ") cannot be found, now searching for a general BLAS library")
    find_package(BLAS)
    if(NOT BLAS_FOUND)
      set(USE_BLAS 0)
    endif()
...
if(NOT AT_MKL_ENABLED)
  ...
  if(NOT EXISTS "${POCKETFFT_INCLUDE_DIR}")
    message(
      FATAL_ERROR
        "pocketfft directory not found, expected ${POCKETFFT_INCLUDE_DIR}")
  elseif(NOT EXISTS "${POCKETFFT_INCLUDE_DIR}/pocketfft_hdronly.h")
    message(
      FATAL_ERROR "pocketfft headers not found in ${POCKETFFT_INCLUDE_DIR}")
  endif()
...
# ---[ Dependencies NNPACK and family (QNNPACK, PYTORCH_QNNPACK, and XNNPACK)
# can download and compile their dependencies in isolation as part of their
# build.  These dependencies are then linked statically with PyTorch.  To avoid
# the possibility of a version mismatch between these shared dependencies,
# explicitly declare our intent to these libraries that we are interested in
# using the exact same source dependencies for all.

if(USE_NNPACK
   OR USE_PYTORCH_QNNPACK
   OR USE_XNNPACK)
  set(DISABLE_NNPACK_AND_FAMILY OFF)

  # Sanity checks - Can we actually build NNPACK and family given the
  # configuration provided? Disable them and warn the user if not.
  ...
  if(IOS)
    ...
    if(NOT IOS AND NOT (CMAKE_SYSTEM_NAME MATCHES
                        "^(Android|Linux|Darwin|Windows)$"))
      message(
        WARNING
    ...
    if(NOT IOS AND NOT (CMAKE_SYSTEM_PROCESSOR MATCHES
                        "^(i686|AMD64|x86_64|armv[0-9].*|arm64|aarch64)$"))
      message(
        WARNING
...
set(CONFU_DEPENDENCIES_SOURCE_DIR
    ${PROJECT_BINARY_DIR}/confu-srcs
    CACHE PATH "Confu-style dependencies source directory")
set(CONFU_DEPENDENCIES_BINARY_DIR
    ${PROJECT_BINARY_DIR}/confu-deps
    CACHE PATH "Confu-style dependencies binary directory")

# ---[ pthreadpool Only add a dependency on pthreadpool if we are on a mobile
# build or are building any of the libraries in the {Q/X}NNPACK family.
if(INTERN_BUILD_MOBILE OR NOT DISABLE_NNPACK_AND_FAMILY)
  ...
  if(NOT TARGET pthreadpool)
    if(USE_SYSTEM_PTHREADPOOL)
      ...
      set_property(TARGET pthreadpool PROPERTY IMPORTED_LOCATION
                                               "${PTHREADPOOL_LIBRARY}")
...
if(NOT CMAKE_SYSTEM_PROCESSOR MATCHES "^(s390x|ppc64le)$")
  # ---[ Caffe2 uses cpuinfo library in the thread pool ---[ But it doesn't
  # support s390x/powerpc and thus not used on s390x/powerpc
  if(NOT TARGET cpuinfo AND USE_SYSTEM_CPUINFO)
    ...
    set_target_properties(cpuinfo PROPERTIES IMPORTED_LOCATION
                                             "${CPUINFO_LIBRARY}")
  elseif(NOT TARGET cpuinfo)
    if(NOT DEFINED CPUINFO_SOURCE_DIR)
      set(CPUINFO_SOURCE_DIR
          "${CMAKE_CURRENT_LIST_DIR}/../third_party/cpuinfo"
          CACHE STRING "cpuinfo source directory")
    endif()
    ...
    # We build static version of cpuinfo but link them into a shared library for
    # Caffe2, so they need PIC.
...
if(USE_PYTORCH_QNNPACK)
  if(NOT DEFINED PYTORCH_QNNPACK_SOURCE_DIR)
    set(PYTORCH_QNNPACK_SOURCE_DIR
        "${PROJECT_SOURCE_DIR}/aten/src/ATen/native/quantized/cpu/qnnpack"
        CACHE STRING "QNNPACK source directory")
  endif()

  if(NOT TARGET pytorch_qnnpack)
    set(PYTORCH_QNNPACK_BUILD_TESTS
        OFF
        CACHE BOOL "")
    set(PYTORCH_QNNPACK_BUILD_BENCHMARKS
        OFF
        CACHE BOOL "")
    set(PYTORCH_QNNPACK_LIBRARY_TYPE
        "static"
        CACHE STRING "")
    add_subdirectory("${PYTORCH_QNNPACK_SOURCE_DIR}"
                     "${CONFU_DEPENDENCIES_BINARY_DIR}/pytorch_qnnpack")
    # We build static versions of QNNPACK and pthreadpool but link them into a
    # shared library for Caffe2, so they need PIC.
    set_property(TARGET pytorch_qnnpack PROPERTY POSITION_INDEPENDENT_CODE ON)
    set_property(TARGET cpuinfo PROPERTY POSITION_INDEPENDENT_CODE ON)
    # QNNPACK depends on gemmlowp headers
    target_include_directories(pytorch_qnnpack
                               PRIVATE "${CAFFE2_THIRD_PARTY_ROOT}/gemmlowp")
  endif()
...
if(USE_NNPACK)
  ...
  if(NNPACK_FOUND)
    if(TARGET nnpack)
      # ---[ NNPACK is being built together with Caffe2: explicitly specify
      # dependency
...
if(USE_XNNPACK AND NOT USE_SYSTEM_XNNPACK)
  if(NOT DEFINED XNNPACK_SOURCE_DIR)
    set(XNNPACK_SOURCE_DIR
        "${CAFFE2_THIRD_PARTY_ROOT}/XNNPACK"
        CACHE STRING "XNNPACK source directory")
  endif()

  if(NOT DEFINED XNNPACK_INCLUDE_DIR)
    set(XNNPACK_INCLUDE_DIR
        "${XNNPACK_SOURCE_DIR}/include"
        CACHE STRING "XNNPACK include directory")
  endif()
  ...
  if(NOT TARGET XNNPACK OR NOT TARGET microkernels-prod)
    ...
    # Disable ARM BF16 and FP16 vector for now; unused and causes build failures
    # because these new ISA features may not be supported on older compilers
    ...
    set(XNNPACK_ENABLE_ARM_I8MM
        OFF
        CACHE BOOL "")

    # Disable avxvnni int8
    set(XNNPACK_ENABLE_AVXVNNIINT8
        OFF
        CACHE BOOL "")

    # Older MSVC versions don't support AVX512FP. TODO Minimum version support?
    if(CMAKE_C_COMPILER_ID STREQUAL "MSVC")
      set(XNNPACK_ENABLE_AVX512FP16
          OFF
          CACHE BOOL "")
    endif()

    # Conditionally disable AVX512AMX, as it requires Clang 11 or later. Note
    # that XNNPACK does conditionally compile this based on GCC version. Once it
    # also does so based on Clang version, this logic can be removed.
    if(CMAKE_C_COMPILER_ID STREQUAL "Clang")
      if(CMAKE_C_COMPILER_VERSION VERSION_LESS "11")
        set(XNNPACK_ENABLE_AVX512AMX
            OFF
            CACHE BOOL "")
      endif()
    endif()
    ...
    # Object libraries within XNNPACK which must be PIC to successfully link
    # this static libXNNPACK with pytorch
    set(__caffe2_CMAKE_POSITION_INDEPENDENT_CODE_FLAG
        ${CMAKE_POSITION_INDEPENDENT_CODE})
    set(CMAKE_POSITION_INDEPENDENT_CODE ON)

    if(WIN32)
      # Disable libm dependency explicitly to avoid symbol conflict for XNNPACK
      # as Windows runtime has provided the math functions - #134989
      set(XNNPACK_BUILD_WITH_LIBM
          OFF
          CACHE BOOL "")
    endif()
    ...
    set(CMAKE_POSITION_INDEPENDENT_CODE
        ${__caffe2_CMAKE_POSITION_INDEPENDENT_CODE_FLAG})
  endif()
...
if(BUILD_TEST
   OR BUILD_MOBILE_BENCHMARK
   OR BUILD_MOBILE_TEST)
  ...
  # For gtest, we will simply embed it into our test binaries, so we won't need
  # to install it.
  set(INSTALL_GTEST
      OFF
...
if(USE_FBGEMM)
  ...
  if(NOT DEFINED FBGEMM_SOURCE_DIR)
    set(FBGEMM_SOURCE_DIR
        "${CAFFE2_THIRD_PARTY_ROOT}/fbgemm"
        CACHE STRING "FBGEMM source directory")
  endif()
  if(NOT CAFFE2_COMPILER_SUPPORTS_AVX512_EXTENSIONS)
    message(WARNING "A compiler with AVX512 support is required for FBGEMM. "
                    "Not compiling with FBGEMM. "
                    "Turn this warning off by USE_FBGEMM=OFF.")
    set(USE_FBGEMM OFF)
  endif()
  if(NOT CMAKE_SIZEOF_VOID_P EQUAL 8)
    message(WARNING "x64 operating system is required for FBGEMM. "
                    "Not compiling with FBGEMM. "
                    "Turn this warning off by USE_FBGEMM=OFF.")
    set(USE_FBGEMM OFF)
  endif()
  if(USE_FBGEMM AND NOT TARGET fbgemm)
    set(FBGEMM_BUILD_TESTS
        OFF
        CACHE BOOL "")
    set(FBGEMM_BUILD_BENCHMARKS
        OFF
        CACHE BOOL "")
    if(MSVC AND BUILD_SHARED_LIBS)
      set(FBGEMM_LIBRARY_TYPE
          "shared"
          CACHE STRING "")
    else()
      set(FBGEMM_LIBRARY_TYPE
    ...
    if(USE_ASAN)
      set(USE_SANITIZER
          "address,undefined"
          CACHE STRING "-fsanitize options for FBGEMM")
    endif()
    ...
    # Disabling autovec in fbgemm due to large library size causing symbol
    # relocation issues, which is only allowed in static builds. Long-term
    # solution involves modularizing fbgemm targets.
...
if(USE_FBGEMM)
  ...
  message(WARNING "Turning USE_FAKELOWP off as it depends on USE_FBGEMM.")
...
if(USE_ITT)
  ...
  if(ITT_FOUND)
    ...
    message(
      WARNING "Not compiling with ITT. Suppress this warning with -DUSE_ITT=OFF"
    )
    set(USE_ITT
        OFF
        CACHE BOOL "" FORCE)
...
# ---[ Python Interpreter If not given a Python installation, then use the
...
# ---[ EIGEN Due to license considerations, we will only use the MPL2 parts of
# Eigen.
...
if(USE_SYSTEM_EIGEN_INSTALL)
  ...
  if(EIGEN3_FOUND)
    ...
    execute_process(
      COMMAND ${Python_EXECUTABLE} ../tools/optional_modules.py checkout_eigen
...
if(BUILD_PYTHON)
  ...
  find_package(
    ...
    OPTIONAL_COMPONENTS ${PYTHON_COMPONENTS})
else()
  ...
  if(Python_Development.Module_FOUND)
    if(USE_NUMPY)
      if(NOT Python_NumPy_FOUND)
        message(
          WARNING
            "NumPy could not be found. Not building with NumPy. Suppress this warning with -DUSE_NUMPY=OFF"
        )
    ...
    message(
      WARNING
        "Python dependencies not met. Not compiling with python. Suppress this warning with -DBUILD_PYTHON=OFF"
    )
...
if(USE_SYSTEM_PYBIND11)
  ...
  set(pybind11_INCLUDE_DIRS
      ${CMAKE_CURRENT_LIST_DIR}/../third_party/pybind11/include)
  install(
    DIRECTORY ${pybind11_INCLUDE_DIRS}
...
target_include_directories(pybind::pybind11 SYSTEM
...
if(NOT OpenTelemetryApi_FOUND)
  ...
  set(OpenTelemetryApi_INCLUDE_DIRS
      ${CMAKE_CURRENT_LIST_DIR}/../third_party/opentelemetry-cpp/api/include)
endif()
message(STATUS "opentelemetry api include dirs: "
               "${OpenTelemetryApi_INCLUDE_DIRS}")
add_library(opentelemetry::api INTERFACE IMPORTED)
target_include_directories(opentelemetry::api SYSTEM
                           INTERFACE ${OpenTelemetryApi_INCLUDE_DIRS})
...
if(USE_MPI)
  ...
  if(MPI_CXX_FOUND)
    ...
    message(
      WARNING "Not compiling with MPI. Suppress this warning with -DUSE_MPI=OFF"
    )
...
if(USE_OPENMP AND NOT TARGET caffe2::openmp)
  ...
  if(OPENMP_FOUND)
    ...
    if(OpenMP_CXX_LIBRARIES)
      message(
        STATUS "Will link against OpenMP libraries: ${OpenMP_CXX_LIBRARIES}")
    endif()
    ...
    if(MSVC AND OpenMP_CXX_LIBRARIES MATCHES ".*libiomp5md\\.lib.*")
      target_compile_definitions(caffe2::openmp
                                 INTERFACE _OPENMP_NOFORCE_MANIFEST)
...
if(USE_CUDNN)
  if(CUDNN_VERSION VERSION_LESS 8.5)
    message(
      FATAL_ERROR
        "PyTorch needs CuDNN-8.5 or above, but found ${CUDNN_VERSION}. Builds are still possible with `USE_CUDNN=0`"
    )
  endif()
  set(CUDNN_FRONTEND_INCLUDE_DIR
      ${CMAKE_CURRENT_LIST_DIR}/../third_party/cudnn_frontend/include)
  target_include_directories(torch::cudnn
                             INTERFACE ${CUDNN_FRONTEND_INCLUDE_DIR})
endif()
...
if(USE_SYSTEM_NVTX)
  ...
  find_path(
    nvtx3_dir
    NAMES nvtx3
    PATHS "${PROJECT_SOURCE_DIR}/third_party/NVTX/c/include"
    NO_DEFAULT_PATH)
endif()
...
if(USE_ROCM)
  ...
  if(PYTORCH_FOUND_HIP)
    ...
    if(USE_NCCL AND NOT USE_SYSTEM_NCCL)
      message(INFO
              "Forcing USE_SYSTEM_NCCL to ON since it's required by using RCCL")
    ...
    message(
      "TORCH_HIP_VERSION=${TORCH_HIP_VERSION} is added as a compiler defines")
    ...
    # needed for compat with newer versions of hip-clang that introduced C++20
    # mangling rules
    list(APPEND HIP_HIPCC_FLAGS -fclang-abi-compat=17)

    set(HIP_CLANG_FLAGS ${HIP_CXX_FLAGS})
    # Ask hcc to generate device code during compilation so we can use host
    # linker to link.
    ...
    set(Caffe2_HIP_INCLUDE $<INSTALL_INTERFACE:include> ${Caffe2_HIP_INCLUDE})
    # This is needed for library added by hip_add_library (same for
    # hip_add_executable)
    hip_include_directories(${Caffe2_HIP_INCLUDE})

    set(Caffe2_PUBLIC_HIP_DEPENDENCY_LIBS hip::amdhip64 MIOpen hiprtc::hiprtc
    )# libroctx will be linked in with MIOpen

    # Math libraries
    list(
      APPEND
      Caffe2_PUBLIC_HIP_DEPENDENCY_LIBS
      roc::hipblas
      roc::rocblas
      hip::hipfft
      hip::hiprand
      ...
      roc::hipblaslt
      roc::rocsolver)
    # hipsparselt is an optional component that will eventually be enabled by
    # default.
    if(hipsparselt_FOUND)
      list(APPEND Caffe2_PUBLIC_HIP_DEPENDENCY_LIBS roc::hipsparselt)
    endif()

    # ---[ Kernel asserts Kernel asserts is disabled for ROCm by default. It can
    # be turned on by turning on the env USE_ROCM_KERNEL_ASSERT to the build
    # system.
...
if(USE_XCCL)
  if(NOT USE_XPU)
    message(
      WARNING
        "Not using XPU, so disabling USE_XCCL. Suppress this warning with -DUSE_XCCL=OFF."
    )
...
if(USE_DISTRIBUTED AND USE_TENSORPIPE)
  if(MSVC)
    ...
    # Suppress warning to unblock libnop compilation by clang-17 See
    # https://github.com/pytorch/pytorch/issues/151316
    target_compile_options_if_supported(
      tensorpipe -Wno-missing-template-arg-list-after-template-kw)
    if(CMAKE_VERSION VERSION_GREATER_EQUAL "4.0.0")
      unset(CMAKE_POLICY_VERSION_MINIMUM)
    endif()
    ...
    elseif(USE_ROCM)
      ...
      # Not yet... list(APPEND Caffe2_HIP_DEPENDENCY_LIBS tensorpipe_hip)
    endif()
  endif()
endif()
  ...
  if(NOT CMAKE_SIZEOF_VOID_P EQUAL 8)
    ...
    if(NOT USE_SYSTEM_GLOO)
      if(USE_DISTRIBUED AND USE_TENSORPIPE)
        ...
        set_target_properties(uv_a PROPERTIES INTERFACE_INCLUDE_DIRECTORIES
                                              "${_include_dirs}")
      endif()
      set(GLOO_USE_CUDA_TOOLKIT
          ON
          CACHE BOOL "" FORCE)

      # Disable NCCL/RCCL since we don't use Gloo+NCCL, make sure to re-enable
      # it!
      ...
      # of PROJECT_SOURCE_DIR with/without conda system. The reason is that gloo
      # generates a new config.h in the binary directory.
      ...
      # need to use Gloo_INCLUDE_DIRS over third_party/gloo to find Gloo's
    ...
    # Add explicit dependency since NCCL is built from third_party. Without
    # dependency, make -jN with N>1 can fail if the NCCL build hasn't finished
    # when CUDA targets are linked.
...
if(USE_SNPE AND ANDROID)
  if(SNPE_LOCATION AND SNPE_HEADERS)
    message(STATUS "Using SNPE location specified by -DSNPE_LOCATION: "
                   ${SNPE_LOCATION})
    message(STATUS "Using SNPE headers specified by -DSNPE_HEADERS: "
                   ${SNPE_HEADERS})
...
if(CAFFE2_CMAKE_BUILDING_WITH_MAIN_REPO AND NOT INTERN_DISABLE_ONNX)
  ...
  # If linking local protobuf, make sure ONNX has the same protobuf patches as
  # Caffe2 and Caffe proto. This forces some functions to not be inline and
  # instead route back to the statically-linked protobuf.
  if(CAFFE2_LINK_LOCAL_PROTOBUF)
    set(ONNX_PROTO_POST_BUILD_SCRIPT
  ...
  if(NOT USE_SYSTEM_ONNX)
    # In mobile build we care about code size, and so we need drop everything
    # (e.g. checker) in onnx but the pb definition.
    if(ANDROID OR IOS)
...
if(USE_KINETO
   AND INTERN_BUILD_MOBILE
   AND USE_LITE_INTERPRETER_PROFILER)
...
if(NOT INTERN_BUILD_MOBILE)
  ...
  # ############################################################################
  # Flags When using MSVC Detect CUDA architecture and get best NVCC flags
  # finding cuda must be first because other things depend on the result
  #
  # NB: We MUST NOT run this find_package if NOT USE_CUDA is set, because
  # upstream FindCUDA has a bug where it will still attempt to make use of
  # NOTFOUND compiler variables to run various probe tests.  We could try to fix
  # this, but since FindCUDA upstream is subsumed by first-class support for
  # CUDA language, it seemed not worth fixing.
  ...
  if(MSVC)
    ...
    string(
      ...
      " -Xcompiler=/wd4819,/wd4503,/wd4190,/wd4244,/wd4251,/wd4275,/wd4522")
  else()
    if(WERROR)
      ...
      if(CMAKE_CXX_COMPILER_ID STREQUAL "GNU"
        ...
        string(APPEND CMAKE_CUDA_FLAGS
               " -Xcompiler -Werror -Xcompiler -Wno-error=sign-compare ")
      endif()
    endif()
  endif()
  ...
  # use cub in a safe manner, see: https://github.com/pytorch/pytorch/pull/55292
  string(APPEND CMAKE_CUDA_FLAGS " -DCUB_WRAPPED_NAMESPACE=at_cuda_detail")

  message(
    STATUS "Found CUDA with FP16 support, compiling with torch.cuda.HalfTensor")
  string(
    APPEND
    CMAKE_CUDA_FLAGS
    " -DCUDA_HAS_FP16=1"
  ...
  if(NOT GENERATOR_IS_MULTI_CONFIG)
    if(${CMAKE_BUILD_TYPE} STREQUAL "Release")
      ...
      string(REGEX REPLACE "[-/]DNDEBUG" "" CMAKE_CXX_FLAGS ""
                           ${CMAKE_CXX_FLAGS})
    endif()
  endif()
  string(REGEX REPLACE "[-/]DNDEBUG" "" CMAKE_C_FLAGS_DEBUG ""
                       ${CMAKE_C_FLAGS_DEBUG})
  string(REGEX REPLACE "[-/]DNDEBUG" "" CMAKE_CXX_FLAGS_DEBUG ""
  ...
  elseif(USE_MAGMA)
    message(WARNING "Not compiling with MAGMA. Suppress this warning with "
                    "-DUSE_MAGMA=OFF.")
    caffe2_update_option(USE_MAGMA OFF)
  endif()
  ...
  if(USE_MKLDNN)
    if(NOT CMAKE_SIZEOF_VOID_P EQUAL 8)
      message(WARNING "x64 operating system is required for MKLDNN. "
...
# CMAKE_CXX_FLAGS in ways that break feature checks. Since we already know `fmt`
# is compatible with a superset of the compilers that PyTorch is, it shouldn't
# be too bad to just disable the checks.
...
# ---[ Kineto edge profiler depends on KinetoProfiler but it only does cpu
# profiling. Thus we dont need USE_CUDA/USE_ROCM
if(USE_KINETO
   AND INTERN_BUILD_MOBILE
   AND NOT (BUILD_LITE_INTERPRETER AND USE_LITE_INTERPRETER_PROFILER))
...
if(USE_KINETO
   AND INTERN_BUILD_MOBILE
   AND USE_LITE_INTERPRETER_PROFILER
   AND (USE_CUDA OR USE_ROCM))
  message(
    FATAL_ERROR "Mobile build with profiler does not support CUDA or ROCM")
endif()

if(USE_KINETO)
  if(NOT USE_CUDA)
    set(LIBKINETO_NOCUPTI
        ON
        CACHE STRING "" FORCE)
  else()
    set(LIBKINETO_NOCUPTI
  ...
  if(LIBKINETO_NOCUPTI
     ...
     AND LIBKINETO_NOXPUPTI)
    message(STATUS "Using CPU-only version of Kineto")
  endif()

  set(CAFFE2_THIRD_PARTY_ROOT
      "${PROJECT_SOURCE_DIR}/third_party"
      CACHE STRING "")
  set(KINETO_SOURCE_DIR
      "${CAFFE2_THIRD_PARTY_ROOT}/kineto/libkineto"
      CACHE STRING "")
  set(KINETO_BUILD_TESTS
      OFF
      CACHE BOOL "")
  set(KINETO_LIBRARY_TYPE
      "static"
      CACHE STRING "")
  ...
  if(NOT LIBKINETO_NOCUPTI)
    ...
    find_library(
      CUPTI_LIBRARY_PATH ${CUPTI_LIB_NAME}
      PATHS ${CUDA_SOURCE_DIR} ${CUDA_SOURCE_DIR}/extras/CUPTI/lib64
            ${CUDA_SOURCE_DIR}/lib ${CUDA_SOURCE_DIR}/lib64
      NO_DEFAULT_PATH)

    find_path(
      CUPTI_INCLUDE_DIR cupti.h
      PATHS ${CUDA_SOURCE_DIR}/extras/CUPTI/include ${CUDA_INCLUDE_DIRS}
            ${CUDA_SOURCE_DIR} ${CUDA_SOURCE_DIR}/include
      NO_DEFAULT_PATH)
    ...
    if(CUPTI_LIBRARY_PATH AND CUPTI_INCLUDE_DIR)
      ...
      # I've only tested this sanity check on Linux; if someone runs into this
      # bug on another platform feel free to generalize it accordingly
      if(NOT USE_CUPTI_SO AND UNIX)
        ...
        set(CMAKE_REQUIRED_LINK_OPTIONS
            "-Wl,--whole-archive,${CUPTI_LIBRARY_PATH},--no-whole-archive")
  ...
  int main() {
    ...
    return 1;
      ...
      message(
        STATUS "Could not find CUPTI library, using CPU-only Kineto build")
      set(LIBKINETO_NOCUPTI
          ON
          CACHE STRING "" FORCE)
    endif()
...
target_include_directories(
  httplib SYSTEM INTERFACE ${PROJECT_SOURCE_DIR}/third_party/cpp-httplib)
...
include_directories(moodycamel SYSTEM INTERFACE
