...
TORCH_LIBRARY_IMPL(aten, FT_BATCHED_KEY, m) {
  ...
  STOP_DECOMPOSE(reflection_pad3d_backward);
  ...
  STOP_DECOMPOSE(conj_physical);
  STOP_DECOMPOSE(resolve_conj);
  STOP_DECOMPOSE(bilinear);
  STOP_DECOMPOSE(tensordot);
  STOP_DECOMPOSE(addr);
  ...
  STOP_DECOMPOSE(std_mean);
  STOP_DECOMPOSE(std_mean.correction_names);
  STOP_DECOMPOSE(std_mean.dim);
  ...
  STOP_DECOMPOSE(var_mean);
  STOP_DECOMPOSE(var_mean.correction_names);
  STOP_DECOMPOSE(var_mean.dim);
  ...
  STOP_DECOMPOSE(aminmax);
  STOP_DECOMPOSE(corrcoef);
  STOP_DECOMPOSE(cov);
  STOP_DECOMPOSE(cumulative_trapezoid.dx);
  STOP_DECOMPOSE(cumulative_trapezoid.x);
  STOP_DECOMPOSE(fused_moving_avg_obs_fake_quant);
  STOP_DECOMPOSE(histogram.bin_ct);
  STOP_DECOMPOSE(histogram.bins_tensor);
  STOP_DECOMPOSE(reflection_pad3d);
  ...
  STOP_DECOMPOSE(special_polygamma);
  ...
  STOP_DECOMPOSE(fliplr);
  STOP_DECOMPOSE(flipud);
  ...
  STOP_DECOMPOSE(adaptive_avg_pool1d);
  STOP_DECOMPOSE(adaptive_avg_pool3d);
  ...
  STOP_DECOMPOSE(_det_lu_based_helper);
