...
def main():
    ...
    device = "cuda"
    lstm = LSTM(32).to(device)
    comp_lstm = torch.compile(lstm)

    x = torch.rand(5, 32).to(device)
    torch._dynamo.mark_dynamic(x, 0)
    h_0 = torch.rand(1, 32).to(device)
    c_0 = torch.rand(1, 32).to(device)
    ...
    print("Passed, yay!")
    x2 = torch.rand(8, 32).to(device)  # now try with different seqlen
    assert torch.all(
