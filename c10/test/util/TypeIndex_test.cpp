...
namespace test_top_level_name {
  static_assert(string_view::npos != get_fully_qualified_type_name<Dummy>().find("Dummy"), "");
}

namespace test_nested_name {
  struct Dummy final {};

  static_assert(string_view::npos != get_fully_qualified_type_name<Dummy>().find("test_nested_name::Dummy"), "");
}

namespace test_type_template_parameter {
  template<class T> struct Outer final {};
  struct Inner final {};

  static_assert(string_view::npos != get_fully_qualified_type_name<Outer<Inner>>().find("test_type_template_parameter::Outer"), "");
  static_assert(string_view::npos != get_fully_qualified_type_name<Outer<Inner>>().find("test_type_template_parameter::Inner"), "");
}

namespace test_nontype_template_parameter {
  template<size_t N> struct Class final {};

  static_assert(string_view::npos != get_fully_qualified_type_name<Class<38474355855>>().find("38474355855"), "");
}
...
namespace test_type_computations_are_resolved {
  ...
  static_assert(string_view::npos != get_fully_qualified_type_name<typename Type<int>::type>().find("int"), "");
  static_assert(string_view::npos != get_fully_qualified_type_name<typename Type<int>::type>().find("*"), "");

  // but with remove_pointer applied, there is no '*' in the type name anymore
  static_assert(string_view::npos == get_fully_qualified_type_name<typename std::remove_pointer<typename Type<int>::type>::type>().find("*"), "");

  struct Functor final {
    std::string operator()(int64_t a, const Type<int>& b) const;
  };
  static_assert(get_fully_qualified_type_name<std::string(int64_t, const Type<int>&)>() ==
      get_fully_qualified_type_name<typename c10::guts::infer_function_traits_t<Functor>::func_type>(), "");
}

namespace test_function_arguments_and_returns {
  class Dummy final {};

  static_assert(string_view::npos != get_fully_qualified_type_name<Dummy (int)>().find("test_function_arguments_and_returns::Dummy"), "");
  static_assert(string_view::npos != get_fully_qualified_type_name<void (Dummy)>().find("test_function_arguments_and_returns::Dummy"), "");
}
