...
SymInt maybe_wrap_dim_slow_symint(
    SymInt dim,
  ...
  if (dim_post_expr == 0) {
    TORCH_CHECK_INDEX(
      ...
      dim,
      " but tensor has no dimensions");
    return c10::maybe_wrap_dim_symint(dim, /*dim_post_expr=*/1, /*wrap_scalar=*/false);
  }

  SymInt min = dim_post_expr * -1;
  SymInt max = dim_post_expr - 1;
  TORCH_CHECK_INDEX(
      min <= dim && dim <= max,
      "Dimension out of range (expected to be in range of [",
      min,
