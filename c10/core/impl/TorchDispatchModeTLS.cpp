...
const std::shared_ptr<SafePyObject> TorchDispatchModeTLS::pop_stack(
  ...
  if (is_pre_dispatch) {
    if (!torchDispatchModeState.pre_dispatch_stack_.empty()) {
      out = torchDispatchModeState.pre_dispatch_stack_.back();
      torchDispatchModeState.pre_dispatch_stack_.pop_back();
    } else {
      for (int64_t i =
    ...
    // Why do we have to do it unconditionally?
  ...
  if (is_pre_dispatch) {
    ...
    for (const auto i : c10::irange(
      ...
      if (torchDispatchModeState.pre_dispatch_infra_modes_[i] != c10::nullopt) {
        if (curr_idx == 0) {
          return torchDispatchModeState.pre_dispatch_infra_modes_[i].value();
        }
        curr_idx -= 1;
      }
    }
    // At this point, we're guaranteed that curr_idx < stack_.size()
    return torchDispatchModeState.pre_dispatch_stack_[curr_idx];
  }
  auto curr_idx = idx;
  for (const auto i :
...
int64_t TorchDispatchModeTLS::stack_len(bool is_pre_dispatch) {
  if (is_pre_dispatch) {
    auto stack_len =
        static_cast<int64_t>(torchDispatchModeState.pre_dispatch_stack_.size());
...
const c10::optional<std::shared_ptr<SafePyObject>> TorchDispatchModeTLS::
    get_mode(TorchDispatchModeKey mode_key, bool is_pre_dispatch) {
  if (is_pre_dispatch) {
...
void TorchDispatchModeTLS::set_mode(
  ...
  if (is_pre_dispatch) {
    ...
    torchDispatchModeState
        .pre_dispatch_infra_modes_[static_cast<size_t>(mode_key)] = mode;
...
const c10::optional<std::shared_ptr<SafePyObject>> TorchDispatchModeTLS::
    unset_mode(TorchDispatchModeKey mode_key, bool is_pre_dispatch) {
  if (is_pre_dispatch) {
    auto out = torchDispatchModeState
                   .pre_dispatch_infra_modes_[static_cast<size_t>(mode_key)];
    torchDispatchModeState
        .pre_dispatch_infra_modes_[static_cast<size_t>(mode_key)] =
        c10::nullopt;
