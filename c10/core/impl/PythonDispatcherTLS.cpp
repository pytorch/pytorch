...
void PythonDispatcherTLS::reset_state() {
  c10::impl::tls_set_dispatch_key_included(
      DispatchKey::PythonDispatcher, false);
  pythonDispatcherState.interpreter_ = nullptr;
  pythonDispatcherState.pre_dispatch_stack_.clear();
}
...
const std::shared_ptr<SafePyObject> PythonDispatcherTLS::pop_pre_stack() {
  ...
  if (pythonDispatcherState.pre_dispatch_stack_.size() == 0 && !pythonDispatcherState.user_activated) {
    c10::impl::tls_set_dispatch_key_included(
        DispatchKey::PythonDispatcher, false);
    pythonDispatcherState.interpreter_ = nullptr;
  }
...
void PythonDispatcherTLS::push_onto_pre_stack(std::shared_ptr<SafePyObject> mode, PyInterpreter* interpreter) {
  if (pythonDispatcherState.pre_dispatch_stack_.size() == 0 && !pythonDispatcherState.interpreter_) {
    c10::impl::tls_set_dispatch_key_included(
        DispatchKey::PythonDispatcher, true);
    pythonDispatcherState.interpreter_ = interpreter;
  }
