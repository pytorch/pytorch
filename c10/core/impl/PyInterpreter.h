...
struct C10_API PyInterpreterVTable {
  ...
  virtual bool fake_try_prim_meta(
      const c10::OperatorHandle& op,
      torch::jit::Stack* stack) const = 0;
  // whether e is a NotImplementedError raised by a Python kernel, which the
  // fake fallback treats like c10::NotImplementedError
  virtual bool fake_is_not_implemented_error(
      const std::exception_ptr& e) const = 0;
