...
void StorageImpl::set_warn_on_write(bool warn_on_write) {
...
c10::intrusive_ptr<StorageImpl> StorageImpl::copy_on_write() {
  ...
  auto* cow_ctx = data_ptr_.cast_context<impl::CopyOnWriteContext>(
      &impl::deleteCopyOnWriteContext);

  if ((data_ptr_.get_context() != data_ptr_.get() && !cow_ctx) ||
      received_cuda_) {
  ...
  if (!cow_ctx) {
    ...
    auto new_ctx =
        impl::newCopyOnWriteContext(data_ptr_.get(), data_ptr_.get_deleter());
    cow_ctx = new_ctx.get();
    auto new_void_ctx =
        static_cast<std::unique_ptr<void, DeleterFnPtr>>(std::move(new_ctx));
