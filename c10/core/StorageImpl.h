...
struct C10_API StorageImpl : public c10::intrusive_ptr_target {
 public:
  ...
  // What data()/data_ptr() do when the data pointer is invalid: nothing,
  // record an API usage event and continue, or throw.
  enum class DataPtrCheck : uint8_t { None, Log, Throw };
  ...
  // Like the above, but only records an API usage event on access. Used to
  // find accesses to storages that are about to become invalid before making
  // them errors.
  void set_data_ptr_access_log_msg_(std::optional<std::string> s) {
    immutable_data_ptr_check_ = DataPtrCheck::Log;
    get_extra_meta().custom_data_ptr_error_msg_ = std::move(s);
    refresh_has_data_ptr_check();
  }
  ...
  DataPtrCheck data_ptr_check() const {
    return immutable_data_ptr_check_;
  }

  std::optional<std::string> data_ptr_access_error_msg() const {
    return extra_meta_ ? extra_meta_->custom_data_ptr_error_msg_ : std::nullopt;
  }
