...
struct TORCH_API AutoInferenceMode {
  AutoInferenceMode(bool enabled=true) : prev_mode(InferenceMode::is_enabled()),
   prev_keyset(c10::impl::tls_local_dispatch_key_set()) {
    ...
    //
    // 1. When AutoInferenceMode is enabled, Autograd dispatch keys are excluded
    ...
    //  2. When AutoInferenceMode is disabled, InplaceOrView must be added
    //     to included set.
    //
    //     For example:
    //     torch::Tensor a;
