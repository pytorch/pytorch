...
//this function has to be called from callers performing cuda synchronizing operations,
//to raise proper error or warning
void warn_or_error_on_sync() {
  if (warning_state().get_sync_debug_mode() == SyncDebugMode::L_ERROR) {
    TORCH_CHECK(false, "called a synchronizing cuda operation");
      ...
      warning_state().get_sync_debug_mode() == SyncDebugMode::L_WARN) {
