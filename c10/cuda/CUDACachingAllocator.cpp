 ...
 public:
  ...
  Block* malloc(int device, size_t size, cudaStream_t stream)
  {
    ...
    c10::reportMemoryUsageToProfiler(
        block, block->size, c10::Device(c10::Device::Unchecked::UNCHECKED, c10::kCUDA, device));
  ...
  void free(Block* block)
  {
    ...
    c10::reportMemoryUsageToProfiler(
        block, -block->size, c10::Device(c10::Device::Unchecked::UNCHECKED, c10::kCUDA, block->device));
...
struct CudaCachingAllocator : public Allocator {
  DataPtr allocate(size_t size) const override {
    ...
    if (forceUncachedAllocator()) {
      ...
      return {r, r, &uncached_delete, Device(Device::Unchecked::UNCHECKED, kCUDA, device)};
    }
    if (size != 0) {
      caching_allocator.malloc(&r, device, size, cuda::getCurrentCUDAStream(device));
    }
    return {r, r, &raw_delete, Device(Device::Unchecked::UNCHECKED, kCUDA, device)};
  }
