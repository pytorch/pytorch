...
TEST(CUDAGraphMemoryBlockManagerTest, UsesOriginalConditionalRequestStream) {
  std::array<char, 3> stream_storage{};
  char graph_storage{};
  std::array<char, 3> node_storage{};
  auto parent_stream = handle<cudaStream_t>(stream_storage[0]);
  auto child_stream = handle<cudaStream_t>(stream_storage[1]);
  auto auxiliary_stream = handle<cudaStream_t>(stream_storage[2]);
  auto graph = handle<cudaGraph_t>(graph_storage);
  auto child_marker = handle<cudaGraphNode_t>(node_storage[0]);
  auto auxiliary_marker = handle<cudaGraphNode_t>(node_storage[1]);
  auto frontier = handle<cudaGraphNode_t>(node_storage[2]);
  FakeCaptureDAGQuery query;
  query.captures[child_stream] = {
      graph, 2, &child_marker, 1, cudaStreamCaptureStatusActive};
  query.captures[auxiliary_stream] = {
      graph, 2, &auxiliary_marker, 1, cudaStreamCaptureStatusActive};

  BlockManager<TestBlock> manager(query);
  TestPool pool{{1, 0}};
  TestBlock block{parent_stream, {testStream(auxiliary_stream)}, &pool};
  manager.recordAllocation(&block, {true, 2, child_stream, parent_stream});
  manager.recordStreamUse(&block, testStream(auxiliary_stream), 2);
  manager.recordFree(&block, child_stream);
  manager.deferFree(&block, kReuse);

  query.parents[frontier] = {child_marker, auxiliary_marker};
  query.captures[child_stream].terminals = &frontier;
  TestAllocatorOps ops;
  manager.reclaimBlocks(child_stream, kReuse, ops);
  ASSERT_EQ(ops.freed_blocks.size(), 1);
  EXPECT_EQ(ops.freed_blocks.front(), &block);
}

TEST(CUDAGraphMemoryBlockManagerTest, IgnoresInvalidatedFreeCaptureId) {
  char stream_storage{};
  auto stream = handle<cudaStream_t>(stream_storage);
  FakeCaptureDAGQuery query;
  query.captures[stream] = {
      nullptr, 0, nullptr, 0, cudaStreamCaptureStatusInvalidated};

  BlockManager<TestBlock> manager(query);
  TestPool pool{{1, 0}};
  TestBlock block{stream, {}, &pool};
  manager.captureBegin({1, {1, 0}, stream, std::nullopt, std::nullopt});
  manager.recordAllocation(&block, {true, 1, stream, stream});

  EXPECT_NO_THROW(manager.recordFree(&block, stream));
...
TEST(CUDAGraphMemoryBlockManagerTest, PreservesConditionalPoolWhileSuspended) {
  char parent_storage{};
  char child_storage{};
  auto parent_stream = handle<cudaStream_t>(parent_storage);
  auto child_stream = handle<cudaStream_t>(child_storage);
  FakeCaptureDAGQuery query;
  query.captures[parent_stream] = {
      nullptr, 1, nullptr, 0, cudaStreamCaptureStatusActive};
  query.captures[child_stream] = {
      nullptr, 2, nullptr, 0, cudaStreamCaptureStatusActive};
  BlockManager<TestBlock> manager(query);
  TestPool pool{{1, 0}};
  TestBlock block{parent_stream, {testStream(parent_stream)}, &pool};
  TestAllocatorOps ops;

  manager.captureBegin({1, {1, 0}, parent_stream, std::nullopt, std::nullopt});
  manager.recordAllocation(&block, {true, 1, parent_stream, parent_stream});
  manager.recordStreamUse(&block, testStream(parent_stream), 1);
  manager.recordFree(&block, parent_stream);
  manager.deferFree(&block, kReuse);
  manager.endCapturePool({1, 0}, kReuse, ops);
  EXPECT_TRUE(manager.isDeferred(&block));

  manager.captureBegin({2, {1, 0}, child_stream, 1, parent_stream});
  query.captures[child_stream].status = cudaStreamCaptureStatusNone;
  manager.endCapturePool({1, 0}, kReuse, ops);
  manager.captureEnd(2);
  EXPECT_TRUE(manager.isDeferred(&block));

  query.captures[parent_stream].status = cudaStreamCaptureStatusNone;
  manager.endCapturePool({1, 0}, kReuse, ops);
  manager.captureEnd(1);
  ASSERT_EQ(ops.freed_blocks.size(), 1);
  EXPECT_EQ(ops.freed_blocks.front(), &block);
