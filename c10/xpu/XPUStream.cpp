...
static std::vector<std::array<std::unique_ptr<sycl::queue>, kStreamsPerPool>>
    reserved_streams;
...
static inline StreamIdIndex streamIdIndex(StreamId s) {
  return static_cast<StreamIdIndex>(
      (s >> kStreamTypeBits) & ((1 << kStreamsPerPoolBits) - 1));
}

static inline StreamId makeStreamId(StreamIdType st, StreamIdIndex si) {
...
// call only once.
static void initDeviceStreamState(DeviceIndex device) {
...
static void initXPUStreamsOnce() {
...
static inline void initDeviceStreamOnce(DeviceIndex device) {
...
StreamIdIndex XPUStream::queue_index() const {
  return streamIdIndex(stream_.id());
}
