...
// boilerplate]. Abstracted from CUDAGuard.h
...
struct XPUGuard {
  ...
  XPUGuard(CUDAGuard&& other) = delete;
...
struct OptionalXPUGuard {
  ...
  explicit OptionalXPUGuard(std::optional<Device> device_opt)
      : guard_(device_opt) {}

  /// Set the current XPU device to the passed device index, if it is not
  /// nullopt
  explicit OptionalXPUGuard(std::optional<DeviceIndex> device_index_opt)
...
/// A variant of StreamGuard that is specialized for XPU.  See XPUGuard
...
/// A variant of OptionalStreamGuard that is specialized for XPU.  See
...
struct OptionalXPUStreamGuard {
  ...
  explicit OptionalXPUStreamGuard(std::optional<Stream> stream_opt)
      : guard_(stream_opt) {}
  ...
  /// Resets the currently set XPU stream to the original stream and
  /// the currently set device to the original device.  Then,
  /// set the current device to the device associated with the passed stream,
  /// and set the current stream on that device to the passed stream.
  ...
  std::optional<XPUStream> original_stream() const {
    auto r = guard_.original_stream();
    if (r.has_value()) {
      return std::make_optional(XPUStream(XPUStream::UNCHECKED, r.value()));
    } else {
      return std::nullopt;
    }
  }
  ...
  std::optional<XPUStream> current_stream() const {
...
/// A variant of MultiStreamGuard that is specialized for XPU.
struct XPUMultiStreamGuard {
  explicit XPUMultiStreamGuard(ArrayRef<XPUStream> streams)
      : guard_(unwrapStreams(streams)) {}

  /// Copy is disallowed
  XPUMultiStreamGuard(const XPUMultiStreamGuard&) = delete;
  XPUMultiStreamGuard& operator=(const XPUMultiStreamGuard&) = delete;

  // See Note [Move construction for RAII guards is tricky]
  XPUMultiStreamGuard(XPUMultiStreamGuard&& other) = delete;

  // See Note [Move assignment for RAII guards is tricky]
  XPUMultiStreamGuard& operator=(XPUMultiStreamGuard&& other) = delete;

 private:
  c10::impl::InlineMultiStreamGuard<impl::XPUGuardImpl> guard_;

  static std::vector<Stream> unwrapStreams(ArrayRef<XPUStream> xpuStreams) {
    std::vector<Stream> streams;
    streams.reserve(xpuStreams.size());
    for (const XPUStream& xpuStream : xpuStreams) {
      streams.push_back(xpuStream);
    }
    return streams;
  }
};
