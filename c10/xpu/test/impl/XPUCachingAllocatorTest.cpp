...
TEST(XPUCachingAllocatorTest, AllocateMemoryByExternalStream) {
  c10::xpu::XPUCachingAllocator::emptyCache();
  auto* allocator = c10::xpu::XPUCachingAllocator::get();

  {
    // Here ensure the validity of memory allocated even though ext_queue goes
    // out of scope.
    sycl::queue ext_queue = sycl::queue(
        c10::xpu::get_device_context(),
        c10::xpu::get_raw_device(0),
  ...
  c10::xpu::XPUStream ext_stream = c10::xpu::getCurrentXPUStream();
  auto _10mb = 10 * 1024 * 1024;
  auto buffer = allocator->allocate(_10mb);
  auto* deviceData = static_cast<int*>(buffer.get());
  ...
  // H2D
  ext_stream.queue().memcpy(deviceData, hostData, sizeof(int) * numel);
  c10::xpu::syncStreamsOnDevice();
  ...
  // D2H
  ext_stream.queue().memcpy(hostData, deviceData, sizeof(int) * numel);
  c10::xpu::syncStreamsOnDevice();
