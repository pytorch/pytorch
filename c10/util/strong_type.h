...
using WhenConstructible =
    std::enable_if_t<std::is_constructible_v<T, V...>>;
}
...
class type : public modifier<M, type<T, Tag, M...>>... {
 public:
  ...
  constexpr explicit type(std::initializer_list<U> us) noexcept(noexcept(T{us}))
      : val{us} {}
  template <
      typename... U,
      typename = std::enable_if_t<
          std::is_constructible_v<T, U&&...> && (sizeof...(U) > 0)>>
...
template <typename T>
using WhenNotStrongType =
    std::enable_if_t<!is_strong_type<std::decay_t<T>>::value>;
} // namespace impl
...
STRONG_NODISCARD constexpr auto access(T&& t) noexcept
...
class typed_equality {
 ...
 public:
  STRONG_NODISCARD
  friend STRONG_CONSTEXPR auto operator==(const T& lh, const Other& rh) noexcept(
  ...
  STRONG_NODISCARD
  friend STRONG_CONSTEXPR auto operator==(const Other& lh, const T& rh) noexcept(
...
template <typename T>
struct require_semiregular : valid_type<
...
struct regular {
  template <typename T>
  class modifier : public semiregular::modifier<T>,
                   public equality::modifier<T> {};
};

struct unique {
  template <typename T>
  class modifier : private impl::valid_type<
...
struct iostreamable {
  template <typename T>
  class modifier : public ostreamable::modifier<T>,
...
struct incrementable {
  ...
  class modifier {
   public:
    friend STRONG_CONSTEXPR T& operator++(T& t) noexcept(
...
struct bicrementable {
  template <typename T>
  class modifier : public incrementable::modifier<T>,
...
struct boolean {
  ...
  class modifier {
   public:
    explicit STRONG_CONSTEXPR operator bool() const noexcept(
        noexcept(static_cast<bool>(value_of(std::declval<const T&>())))) {
...
class difference::modifier<::strong::type<T, Tag, M...>>
 ...
 public:
  friend STRONG_CONSTEXPR type& operator+=(type& lh, const type& rh) noexcept(
  ...
  friend STRONG_CONSTEXPR type& operator*=(type& lh, const T& rh) noexcept(
  ...
  friend STRONG_CONSTEXPR T operator/(const type& lh, const type& rh) {
    return value_of(lh) / value_of(rh);
  }
  ...
  operator%(type lh, const T& rh) noexcept(noexcept(lh %= rh)) {
    lh %= rh;
    return lh;
  }
  ...
  operator%(type lh, type rh) noexcept(noexcept(value_of(lh) % value_of(rh))) {
    return value_of(lh) % value_of(rh);
  }
};
...
class affine_point<D>::modifier<::strong::type<T, Tag, M...>> {
 ...
 public:
  ...
  STRONG_NODISCARD
  friend STRONG_CONSTEXPR difference operator-(const type& lh, const type& rh) {
    return difference(value_of(lh) - value_of(rh));
  }

  friend STRONG_CONSTEXPR type& operator+=(
      type& lh,
      const difference& d) noexcept(noexcept(value_of(lh) += impl::access(d))) {
  ...
  STRONG_NODISCARD
  friend STRONG_CONSTEXPR type operator+(type lh, const difference& d) {
    return lh += d;
  }

  STRONG_NODISCARD
  friend STRONG_CONSTEXPR type operator+(const difference& d, type rh) {
    return rh += d;
  }
...
class pointer::modifier<::strong::type<T, Tag, M...>> {
 ...
 public:
  ...
  STRONG_NODISCARD friend STRONG_CONSTEXPR auto operator==(
      const type& t,
      std::nullptr_t) noexcept(noexcept(std::declval<const TT&>() == nullptr))
  ...
  STRONG_NODISCARD friend STRONG_CONSTEXPR auto operator==(
      std::nullptr_t,
      const type& t) noexcept(noexcept(nullptr == std::declval<const TT&>()))
...
struct arithmetic {
  ...
  class modifier {
   public:
    ...
    friend STRONG_CONSTEXPR T& operator+=(T& lh, const T& rh) noexcept(
        noexcept(value_of(lh) += value_of(rh))) {
      value_of(lh) += value_of(rh);
      return lh;
    }
    ...
    template <
        typename TT = T,
        typename =
            decltype(value_of(std::declval<TT>()) % value_of(std::declval<TT>()))>
    friend STRONG_CONSTEXPR T& operator%=(T& lh, const T& rh) noexcept(
        noexcept(value_of(lh) %= value_of(rh))) {
      value_of(lh) %= value_of(rh);
      return lh;
    ...
    template <
        ...
        typename =
            decltype(value_of(std::declval<TT>()) % value_of(std::declval<TT>()))>
    STRONG_NODISCARD friend STRONG_CONSTEXPR T operator%(T lh, const T& rh) {
      lh %= rh;
      return lh;
    }
  };
...
struct bitarithmetic {
  ...
  class modifier {
   public:
    ...
    template <typename C>
    friend STRONG_CONSTEXPR T& operator<<=(T& lh, C c) noexcept(
        noexcept(value_of(lh) <<= c)) {
      value_of(lh) <<= c;
      return lh;
    }
    ...
    template <typename C>
    STRONG_NODISCARD friend STRONG_CONSTEXPR T operator<<(T lh, C c) {
      lh <<= c;
      return lh;
    }
...
struct indexed<void> {
  ...
  class modifier<type<T, Tag, Ms...>> {
   ...
   public:
    template <typename I>
    STRONG_NODISCARD auto operator[](const I& i) const& noexcept(
    ...
    template <typename I>
    STRONG_NODISCARD auto operator[](const I& i) & noexcept(
    ...
    STRONG_NODISCARD auto at(
        const I& i) const& -> decltype(std::declval<C>().at(impl::access(i))) {
      auto& self = static_cast<const type&>(*this);
    ...
    STRONG_NODISCARD auto at(
        const I& i) & -> decltype(std::declval<R>().at(impl::access(i))) {
      auto& self = static_cast<type&>(*this);
      return value_of(self).at(impl::access(i));
    }
...
class indexed<I>::modifier<type<T, Tag, M...>> {
 ...
 public:
  ...
  STRONG_NODISCARD
  auto operator[](const I& i) & noexcept(
  ...
  template <typename TT = T>
  STRONG_NODISCARD auto at(const I& i)
      const& -> decltype(std::declval<const TT&>().at(impl::access(i))) {
    auto& self = static_cast<const type&>(*this);
    return value_of(self).at(impl::access(i));
  }
  ...
  template <typename TT = T>
  STRONG_NODISCARD auto at(
      const I& i) & -> decltype(std::declval<TT&>().at(impl::access(i))) {
    auto& self = static_cast<type&>(*this);
    return value_of(self).at(impl::access(i));
  }
 ...
 public:
  ...
  class modifier : public pointer::modifier<I>,
   ...
   public:
    ...
    using value_type =
        typename std::iterator_traits<underlying_type_t<I>>::value_type;
    using pointer =
        typename std::iterator_traits<underlying_type_t<I>>::value_type;
    using reference =
        typename std::iterator_traits<underlying_type_t<I>>::reference;
...
class range::modifier<type<T, Tag, M...>> {
 ...
 public:
  ...
  using const_iterator =
      ::strong::type<r_const_iterator, Tag, strong::iterator>;
...
struct converter {
  STRONG_CONSTEXPR explicit operator D() const noexcept(
      noexcept(static_cast<D>(std::declval<const underlying_type_t<T>&>()))) {
...
struct implicit_converter {
  STRONG_CONSTEXPR operator D() const noexcept(
      noexcept(static_cast<D>(std::declval<const underlying_type_t<T>&>()))) {
    auto& self = static_cast<const T&>(*this);
    return static_cast<D>(value_of(self));
  }
...
struct hash<::strong::type<T, Tag, M...>>
  ...
  decltype(auto) operator()(const ::strong::hashable::modifier<type>& t) const
      noexcept(noexcept(
          std::declval<hash<T>>()(value_of(std::declval<const type&>())))) {
...
struct formatter<
  ...
  STRONG_CONSTEXPR decltype(auto) format(
      const ::strong::formattable::modifier<type>& t,
      FormatContext&
          fc) noexcept(noexcept(std::declval<formatter<T, Char>>()
                                    .format(
