...
#pragma GCC diagnostic ignored "-Wexceptions"
    ...
    TORCH_INTERNAL_ASSERT_DEBUG_ONLY(
        ...
        weakcount() == 1 || weakcount() == 0 ||
            weakcount() == detail::kImpracticallyHugeWeakReferenceCount - 1 ||
            weakcount() == detail::kImpracticallyHugeWeakReferenceCount,
...
#pragma GCC diagnostic pop
 ...
 private:
  ...
  uint32_t refcount(std::memory_order order = std::memory_order_relaxed) const {
    return (uint32_t)combined_refcount_.load(order);
  }

  uint32_t weakcount(
      std::memory_order order = std::memory_order_relaxed) const {
    return (uint32_t)(combined_refcount_.load(order) >> 32);
  }
};
...
inline uint32_t atomic_refcount_increment(
    std::atomic<uint64_t>& combined_refcount) {
  return (uint32_t)(atomic_combined_refcount_increment(
...
inline uint32_t atomic_weakcount_decrement(
    std::atomic<uint64_t>& combined_refcount) {
  return (uint32_t)(atomic_combined_refcount_decrement(
                        combined_refcount, kWeakReferenceCountOne) >>
                    32);
}
...
//      intrusive_ptr_target.");
  ...
  void reset_() noexcept {
    if (target_ != NullType::singleton()) {
      ...
      if ((uint32_t)combined_refcount == 0) {
        bool should_delete =
 ...
 public:
  ...
  static intrusive_ptr unsafe_adapt_non_heap_allocated(
    ...
    result.target_->combined_refcount_.store(
        (uint32_t)(detail::kImpracticallyHugeReferenceCount +
                   expected_decrefs) |
    ...
    if (target_ == NullType::singleton()) {
      ...
      do {
        if ((uint32_t)refcount == 0) {
          ...
          refcount,
          refcount + detail::kReferenceCountOne,
