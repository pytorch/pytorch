...
// decrement, which is guaranteed by program order (atomic RMW at same
// address), so no fence is needed.
...
// invocation of the managed object destructor, and that allocation of the managed
// object storage happens-before deallocation of the storage.
//
// To get this ordering, all non-final decrements must synchronize-with the final
// decrement. So all non-final decrements have to store-release while the final
// decrement also has to load-acquire, either directly or with the help of fences.
// But it's easiest just to have all decrements be acq-rel. And it turns out, on
// modern architectures and chips, it's also fastest.
