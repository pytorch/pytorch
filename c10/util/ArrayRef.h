...
class ArrayRef final {
 ...
 public:
  ...
  C10_HOST_CONSTEXPR const T& front() const {
    TORCH_CHECK(!empty(), "ArrayRef: attempted to access front() of empty list");
    return Data[0];
  }

  /// back - Get the last element.
  C10_HOST_CONSTEXPR const T& back() const {
  ...
  C10_HOST_CONSTEXPR ArrayRef<T> slice(size_t N, size_t M) const {
    TORCH_CHECK(
        N + M <= size(),
  ...
  C10_HOST_CONSTEXPR const T& at(size_t Index) const {
    TORCH_CHECK(
        Index < Length,
