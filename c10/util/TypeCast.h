...
#else
      if (C10_UNLIKELY(static_cast<double>(r) >= ::std::rint(max_int_value))) {
        return max_int_value;
      }
#endif
      if constexpr (::std::is_unsigned_v<dest_t>) {
        ...
        constexpr auto min_int_value = ::std::numeric_limits<dest_t>::min();
#if defined(__CUDACC__) || defined(__HIPCC__)
