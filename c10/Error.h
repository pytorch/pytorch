...
#define C10_ASSERT(cond) \
  if (!(cond)) {             \
    C10_ERROR(#cond " ASSERT FAILED at ", __FILE__, ":", __LINE__, ", please report a bug.");   \
  }

#define C10_ASSERTM(cond, ...) \
  if (!(cond)) {             \
    C10_ERROR(c10::str(#cond, " ASSERT FAILED at ", __FILE__, ":", __LINE__, ", please report a bug. ", __VA_ARGS__));   \
  }

#define C10_CHECK(cond, ...) \
  if (!(cond)) {             \
    C10_ERROR(c10::str(__VA_ARGS__));   \
  }
