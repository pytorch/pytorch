...
# Resolve the Python env prefix that Halide's bindings get installed into
if command -v conda >/dev/null 2>&1; then
  eval "$(conda shell.bash hook)"
  conda activate py_$ANACONDA_PYTHON_VERSION
  ENV_PREFIX="${CONDA_PREFIX}"
  PYVER="${ANACONDA_PYTHON_VERSION}"
else
...
cmake --build build
test -e ${ENV_PREFIX}/lib/python3 || ln -s python${PYVER} ${ENV_PREFIX}/lib/python3
cmake --install build --prefix ${ENV_PREFIX}
chown -R jenkins ${ENV_PREFIX}
popd
