...
class CommitResolver:
    ...
    def detect_pr(self) -> int:
        """Detect PR number from ghstack commit trailer or current branch."""
        # ghstack: parse Pull-Request trailer from HEAD commit message
        pr = self._detect_pr_from_ghstack()
        if pr is not None:
            return pr

        # Normal workflow: find PR by branch name
