...
if [[ "$BUILD_ENVIRONMENT" == *asan* ]]; then
    ...
    echo "The next five invocations are expected to crash; if they don't that means ASAN/UBSAN is misconfigured"
    (cd test && ! get_exit_code python -c "import torch; torch._C._crash_if_csrc_asan(3)")
    (cd test && ! get_exit_code python -c "import torch; torch._C._crash_if_c10_asan(3)")
    (cd test && ! get_exit_code python -c "import torch; torch._C._crash_if_c10_ubsan()")
