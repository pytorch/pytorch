...
test_inductor_aoti() {
  ...
  if [[ "$BUILD_ENVIRONMENT" == *sm86* ]]; then
    BUILD_AOT_INDUCTOR_TEST=1 TORCH_CUDA_ARCH_LIST=8.6 USE_FLASH_ATTENTION=OFF python -m pip install -e . -v --no-build-isolation
    ...
    BUILD_AOT_INDUCTOR_TEST=1 python -m pip install -e . -v --no-build-isolation
