...
if [[ "${TEST_CONFIG}" == *numpy_2* ]]; then
  ...
  python tools/dynamo/gb_id_mapping.py check --files "$(git diff --name-only "$GITHUB_BASE_SHA" "$GITHUB_SHA" | grep '\.py$' | tr '\n' ' ')" --registry-path tools/dynamo/graph_break_registry.json
  if [ $? -ne 0 ]; then
