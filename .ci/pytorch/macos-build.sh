...
if [[ ${BUILD_ENVIRONMENT} == *"distributed"* ]]; then
  ...
  USE_DISTRIBUTED=0 USE_OPENMP=1 MACOSX_DEPLOYMENT_TARGET=14.0 WERROR=1 BUILD_TEST=OFF USE_PYTORCH_METAL=1 python -m build --wheel --no-isolation -C--build-option=--plat-name=macosx_14_0_arm64
fi
if which sccache > /dev/null; then
  print_sccache_stats
fi
