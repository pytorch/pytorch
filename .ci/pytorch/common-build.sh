...
if [[ "$BUILD_ENVIRONMENT" != *win-* ]]; then
    ...
    if which sccache > /dev/null; then
        ...
        if [[ -n "${SKIP_SCCACHE_INITIALIZATION:-}" ]]; then
            ...
            SCCACHE_ERROR_LOG=/var/lib/jenkins/workspace/sccache_error.log SCCACHE_IDLE_TIMEOUT=0 sccache --start-server
        else
            # increasing SCCACHE_IDLE_TIMEOUT so that extension_backend_test.cpp can build after this PR:
            # https://github.com/pytorch/pytorch/pull/16645
            SCCACHE_ERROR_LOG=/var/lib/jenkins/workspace/sccache_error.log SCCACHE_IDLE_TIMEOUT=0 RUST_LOG=sccache::server=debug sccache --start-server
        fi
