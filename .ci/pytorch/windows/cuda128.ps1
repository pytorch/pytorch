...
# Check and set CUDA_PATH_V128
if (-not $env:CUDA_PATH_V128) {
    $cudaPath = "C:\Program Files\NVIDIA GPU Computing Toolkit\CUDA\v12.8\bin\nvcc.exe"
    if (Test-Path $cudaPath) {
        $env:CUDA_PATH_V128 = "C:\Program Files\NVIDIA GPU Computing Toolkit\CUDA\v12.8"
    } else {
        Write-Error "CUDA 12.8 not found, failing"
        exit 1
    }
}
...
if (-not $env:BUILD_VISION) {
    $env:TORCH_CUDA_ARCH_LIST = "6.1;7.0;7.5;8.0;8.6;9.0;10.0;12.0"
    $env:TORCH_NVCC_FLAGS = "-Xfatbin -compress-all"
} else {
    $env:NVCC_FLAGS = "-D__CUDA_NO_HALF_OPERATORS__ --expt-relaxed-constexpr -gencode=arch=compute_50,code=sm_50 -gencode=arch=compute_60,code=sm_60 -gencode=arch=compute_70,code=sm_70 -gencode=arch=compute_75,code=sm_75 -gencode=arch=compute_80,code=compute_80 -gencode=arch=compute_86,code=compute_86 -gencode=arch=compute_90,code=compute_90 -gencode=arch=compute_100,code=compute_100 -gencode=arch=compute_120,code=compute_120"
}

# Set CUDA environment
$env:CUDA_PATH = $env:CUDA_PATH_V128
$env:PATH = "$env:CUDA_PATH_V128\bin;$env:PATH"
