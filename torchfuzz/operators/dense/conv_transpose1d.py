"""1D transposed convolution operator implementation."""
...
class ConvTranspose1dOperator(Operator):
    """Operator for 1D transposed convolution (torch.nn.functional.conv_transpose1d)."""

    def __init__(self):
        super().__init__("conv_transpose1d")

    def can_produce(self, tensor):
        """ConvTranspose1d can produce 3D tensors (batch, out_channels, length)."""
        return len(tensor.size) == 3

    def decompose(self, tensor):
        """Decompose tensor into input tensors for conv_transpose1d operation."""
        ...
        # So: in_length = out_length - kernel_size + 1
        in_length = out_length - kernel_size + 1
        in_length = max(in_length, 1)  # Ensure valid input size

        # Verify the calculation works
        calculated_out_length = in_length + kernel_size - 1
        if calculated_out_length != out_length:
            # If it doesn't work, use kernel_size=1 for guaranteed match
            kernel_size = 1
            in_length = out_length  # For kernel=1: out_length = in_length
        ...
        weight_size = (in_channels, out_channels, kernel_size)
    ...
    def codegen(self, output_name, input_names, output_tensor):
        """Generate code for conv_transpose1d operation."""
        stride = getattr(self, '_stride', 1)
        padding = getattr(self, '_padding', 0)

        return f"{output_name} = torch.nn.functional.conv_transpose1d({input_names[0]}, {input_names[1]}, stride={stride}, padding={padding})"
