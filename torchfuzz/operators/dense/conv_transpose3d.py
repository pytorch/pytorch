"""3D transposed convolution operator implementation."""
...
class ConvTranspose3dOperator(Operator):
    """Operator for 3D transposed convolution (torch.nn.functional.conv_transpose3d)."""

    def __init__(self):
        super().__init__("conv_transpose3d")

    def can_produce(self, tensor):
        """ConvTranspose3d can produce 5D tensors (batch, out_channels, depth, height, width)."""
        return len(tensor.size) == 5

    def decompose(self, tensor):
        """Decompose tensor into input tensors for conv_transpose3d operation."""
        ...
        in_depth = out_depth - kernel_size + 1
        in_height = out_height - kernel_size + 1
        in_width = out_width - kernel_size + 1
        in_depth = max(in_depth, 1)  # Ensure valid input size
        in_height = max(in_height, 1)
        in_width = max(in_width, 1)

        # Verify the calculation works
        calculated_out_depth = in_depth + kernel_size - 1
        ...
        if (calculated_out_depth != out_depth or
            ...
            kernel_size = 1
            in_depth = out_depth    # For kernel=1: out_dim = in_dim
        ...
        # Weight tensor: (in_channels, out_channels, kernel_size, kernel_size, kernel_size)
        weight_size = (in_channels, out_channels, kernel_size, kernel_size, kernel_size)
    ...
    def codegen(self, output_name, input_names, output_tensor):
        """Generate code for conv_transpose3d operation."""
        stride = getattr(self, '_stride', 1)
        padding = getattr(self, '_padding', 0)

        return f"{output_name} = torch.nn.functional.conv_transpose3d({input_names[0]}, {input_names[1]}, stride={stride}, padding={padding})"
