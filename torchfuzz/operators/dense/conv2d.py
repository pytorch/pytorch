...
class Conv2dOperator(Operator):
    """Operator for 2D convolution (torch.nn.functional.conv2d)."""

    def __init__(self):
        super().__init__("conv2d")

    def can_produce(self, tensor):
        """Conv2d can produce 4D tensors (batch, out_channels, height, width)."""
    ...
    def decompose(self, tensor):
        ...
        in_height = max(in_height, kernel_size)  # Ensure valid input size
    ...
    def codegen(self, output_name, input_names, output_tensor):
        ...
        return f"{output_name} = torch.nn.functional.conv2d({input_names[0]}, {input_names[1]}, stride={stride}, padding={padding})"
