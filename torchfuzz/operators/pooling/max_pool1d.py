"""1D max pooling operator implementation."""
...
class MaxPool1dOperator(Operator):
    """Operator for 1D max pooling (torch.nn.functional.max_pool1d)."""

    def __init__(self):
        super().__init__("max_pool1d")

    def can_produce(self, tensor):
        """MaxPool1d can produce 3D tensors (batch, channels, length)."""
        return len(tensor.size) == 3

    def decompose(self, tensor):
        """Decompose tensor into input tensors for max_pool1d operation."""
        ...
        # For max_pool: out_length = floor((in_length - kernel_size) / stride) + 1
    ...
    def codegen(self, output_name, input_names, output_tensor):
        """Generate code for max_pool1d operation."""
        kernel_size = getattr(self, '_kernel_size', 2)
        stride = getattr(self, '_stride', 2)

        return f"{output_name} = torch.nn.functional.max_pool1d({input_names[0]}, kernel_size={kernel_size}, stride={stride})"
