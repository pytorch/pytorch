"""3D average pooling operator implementation."""
...
class AvgPool3dOperator(Operator):
    """Operator for 3D average pooling (torch.nn.functional.avg_pool3d)."""

    def __init__(self):
        super().__init__("avg_pool3d")

    def can_produce(self, tensor):
        """AvgPool3d can produce 5D tensors (batch, channels, depth, height, width)."""
        return len(tensor.size) == 5

    def decompose(self, tensor):
        """Decompose tensor into input tensors for avg_pool3d operation."""
        ...
        # For avg_pool: out_dim = floor((in_dim - kernel_size) / stride) + 1
    ...
    def codegen(self, output_name, input_names, output_tensor):
        """Generate code for avg_pool3d operation."""
        kernel_size = getattr(self, '_kernel_size', 2)
        stride = getattr(self, '_stride', 2)

        return f"{output_name} = torch.nn.functional.avg_pool3d({input_names[0]}, kernel_size={kernel_size}, stride={stride})"
