"""1D adaptive max pooling operator implementation."""
...
class AdaptiveMaxPool1dOperator(Operator):
    """Operator for 1D adaptive max pooling (torch.nn.functional.adaptive_max_pool1d)."""

    def __init__(self):
        super().__init__("adaptive_max_pool1d")

    def can_produce(self, tensor):
        """AdaptiveMaxPool1d can produce 3D tensors (batch, channels, length)."""
        return len(tensor.size) == 3

    def decompose(self, tensor):
        """Decompose tensor into input tensors for adaptive_max_pool1d operation."""
    ...
    def codegen(self, output_name, input_names, output_tensor):
        """Generate code for adaptive_max_pool1d operation."""
        output_size = getattr(self, '_output_size', 1)

        return f"{output_name} = torch.nn.functional.adaptive_max_pool1d({input_names[0]}, {output_size})"
