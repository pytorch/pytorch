"""3D adaptive average pooling operator implementation."""
...
class AdaptiveAvgPool3dOperator(Operator):
    """Operator for 3D adaptive average pooling (torch.nn.functional.adaptive_avg_pool3d)."""

    def __init__(self):
        super().__init__("adaptive_avg_pool3d")

    def can_produce(self, tensor):
        """AdaptiveAvgPool3d can produce 5D tensors (batch, channels, depth, height, width)."""
        return len(tensor.size) == 5

    def decompose(self, tensor):
        """Decompose tensor into input tensors for adaptive_avg_pool3d operation."""
    ...
    def codegen(self, output_name, input_names, output_tensor):
        """Generate code for adaptive_avg_pool3d operation."""
        output_size = getattr(self, '_output_size', (1, 1, 1))

        return f"{output_name} = torch.nn.functional.adaptive_avg_pool3d({input_names[0]}, {output_size})"
