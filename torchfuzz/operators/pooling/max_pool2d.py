...
class MaxPool2dOperator(Operator):
    """Operator for 2D max pooling (torch.nn.functional.max_pool2d)."""

    def __init__(self):
        super().__init__("max_pool2d")

    def can_produce(self, tensor):
        """MaxPool2d can produce 4D tensors (batch, channels, height, width)."""
        return len(tensor.size) == 4

    def decompose(self, tensor):
        """Decompose tensor into input tensors for max_pool2d operation."""
        ...
        if calculated_out_height != out_height or calculated_out_width != out_width:
            ...
            kernel_size = 1
            stride = 1
            in_height = out_height  # For kernel=1, stride=1: out_dim = in_dim
            in_width = out_width
    ...
    def codegen(self, output_name, input_names, output_tensor):
        ...
        return f"{output_name} = torch.nn.functional.max_pool2d({input_names[0]}, kernel_size={kernel_size}, stride={stride})"
