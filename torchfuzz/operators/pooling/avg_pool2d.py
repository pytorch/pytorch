"""2D average pooling operator implementation."""
...
class AvgPool2dOperator(Operator):
    """Operator for 2D average pooling (torch.nn.functional.avg_pool2d)."""

    def __init__(self):
        super().__init__("avg_pool2d")

    def can_produce(self, tensor):
        """AvgPool2d can produce 4D tensors (batch, channels, height, width)."""
        return len(tensor.size) == 4

    def decompose(self, tensor):
        """Decompose tensor into input tensors for avg_pool2d operation."""
    ...
    def codegen(self, output_name, input_names, output_tensor):
        """Generate code for avg_pool2d operation."""
        kernel_size = getattr(self, '_kernel_size', 2)
        stride = getattr(self, '_stride', 2)

        return f"{output_name} = torch.nn.functional.avg_pool2d({input_names[0]}, kernel_size={kernel_size}, stride={stride})"
