"""2D adaptive max pooling operator implementation."""
...
class AdaptiveMaxPool2dOperator(Operator):
    """Operator for 2D adaptive max pooling (torch.nn.functional.adaptive_max_pool2d)."""

    def __init__(self):
        super().__init__("adaptive_max_pool2d")

    def can_produce(self, tensor):
        """AdaptiveMaxPool2d can produce 4D tensors (batch, channels, height, width)."""
        return len(tensor.size) == 4

    def decompose(self, tensor):
        """Decompose tensor into input tensors for adaptive_max_pool2d operation."""
    ...
    def codegen(self, output_name, input_names, output_tensor):
        """Generate code for adaptive_max_pool2d operation."""
        output_size = getattr(self, '_output_size', (1, 1))

        return f"{output_name} = torch.nn.functional.adaptive_max_pool2d({input_names[0]}, {output_size})"
