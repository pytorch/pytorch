...
class TestMulOperator:
    """Test class for MulOperator."""

    @pytest.fixture
    def mul_op(self):
        """Create a MulOperator instance."""
        return MulOperator()
    ...
    def test_can_produce_returns_true(self, mul_op, sample_tensor):
        """Test that MulOperator can always produce any tensor."""
        assert mul_op.can_produce(sample_tensor) is True

    def test_decompose_default_inputs(self, mul_op, sample_tensor):
        """Test decomposition with default number of inputs (2)."""
        inputs = mul_op.decompose(sample_tensor)
    ...
    def test_decompose_multiple_inputs(self, mul_op, sample_tensor):
        """Test decomposition with multiple inputs."""
        num_inputs = 4
        inputs = mul_op.decompose(sample_tensor, num_inputs=num_inputs)
    ...
    def test_decompose_type_promotion_float32(self, mul_op):
        """Test type promotion for float32 output."""
        tensor = Tensor((2, 2), (2, 1), "float32", "cuda", [])
        inputs = mul_op.decompose(tensor, num_inputs=2)
    ...
    def test_decompose_type_promotion_bfloat16(self, mul_op):
    ...
    def test_decompose_type_promotion_float16(self, mul_op):
    ...
    def test_codegen_two_inputs(self, mul_op, sample_tensor):
        """Test code generation with two inputs."""
        output_name = "out"
        input_names = ["a", "b"]

        code = mul_op.codegen(output_name, input_names, sample_tensor)
        expected = "out = a * b"

        assert code == expected

    def test_codegen_multiple_inputs(self, mul_op, sample_tensor):
    ...
    def test_operator_name(self, mul_op):
        """Test that operator has correct name."""
        assert mul_op.name == "mul"
