"""Median operator implementation."""
...
class MedianOperator(Operator):
    """Operator for tensor median reduction."""
    ...
    def can_produce(self, tensor):
        """
        torch.median can return either a scalar or a tuple (values, indices).
        For the dim case, it returns both values and indices as separate tensors.
    ...
    def decompose(self, tensor):
        """
        Construct an input shape that reduces to tensor.size via a median.
        ...
        if len(tensor.size) == 0:
            ...
            # Mark 'all' to emit .median() with no dim argument.
            tensor._median_dim = "all"
        else:
    ...
    def codegen(self, output_name, input_names, output_tensor):
        """Generate code for median operation."""
        md = getattr(output_tensor, "_median_dim", None)
        src = input_names[0]
        if md == "all":
            return f"{output_name} = {src}.median()"
        elif isinstance(md, tuple):
            # If you later extend to multi-dim reductions, this handles it.
            return f"{output_name}, _ = {src}.median(dim={md})"
