...
class MeanOperator(Operator):
    """Operator for tensor mean reduction."""
    ...
    def decompose(self, tensor):
        """
        Construct an input shape that reduces to tensor.size via a mean.
        ...
        if len(tensor.size) == 0:
            ...
            # Mark 'all' to emit .mean() with no dim argument.
    ...
    def codegen(self, output_name, input_names, output_tensor):
        """Generate code for mean operation."""
        md = getattr(output_tensor, "_mean_dim", None)
        src = input_names[0]
        if md == "all":
            return f"{output_name} = {src}.mean()"
        elif isinstance(md, tuple):
            # If you later extend to multi-dim reductions, this handles it.
            return f"{output_name} = {src}.mean(dim={md})"
