"""Nanvar operator implementation."""
...
class NanvarOperator(Operator):
    """Operator for tensor nanvar reduction."""

    def __init__(self):
        super().__init__("nanvar")
    ...
    def can_produce(self, tensor):
        ...
        Also, nanvar only supports floating point and complex dtypes.
        """
    ...
    def decompose(self, tensor):
        """
        Construct an input shape that reduces to tensor.size via a nanvar.
        ...
        if len(tensor.size) == 0:
            ...
            # Mark 'all' to emit .nanvar() with no dim argument.
            tensor._nanvar_dim = "all"
        else:
    ...
    def codegen(self, output_name, input_names, output_tensor):
        """Generate code for nanvar operation."""
        vd = getattr(output_tensor, "_nanvar_dim", None)
        src = input_names[0]
        if vd == "all":
            return f"{output_name} = {src}.nanvar()"
        elif isinstance(vd, tuple):
            # If you later extend to multi-dim reductions, this handles it.
            return f"{output_name} = {src}.nanvar(dim={vd})"
