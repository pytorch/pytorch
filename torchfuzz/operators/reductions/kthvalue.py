"""Kthvalue operator implementation."""
...
class KthvalueOperator(Operator):
    """Operator for tensor kthvalue reduction."""
    ...
    def can_produce(self, tensor):
        """
        torch.kthvalue returns both values and indices.
    ...
    def decompose(self, tensor):
        """
        Construct an input shape that reduces to tensor.size via a kthvalue.
        ...
        if len(tensor.size) == 0:
            ...
            # For kthvalue, we need to specify k (1-indexed)
            k = random.randint(1, min(input_shape))
            tensor._kthvalue_k = k
            tensor._kthvalue_dim = "all"
        else:
    ...
    def codegen(self, output_name, input_names, output_tensor):
        """Generate code for kthvalue operation."""
        kd = getattr(output_tensor, "_kthvalue_dim", None)
        k = getattr(output_tensor, "_kthvalue_k", 1)
        src = input_names[0]
        if kd == "all":
            return f"{output_name}, _ = {src}.kthvalue({k})"
        elif isinstance(kd, tuple):
            # If you later extend to multi-dim reductions, this handles it.
            return f"{output_name}, _ = {src}.kthvalue({k}, dim={kd})"
