"""Nansum operator implementation."""
...
class NansumOperator(Operator):
    """Operator for tensor nansum reduction."""

    def __init__(self):
        super().__init__("nansum")
    ...
    def decompose(self, tensor):
        """
        Construct an input shape that reduces to tensor.size via a nansum.
        ...
        if len(tensor.size) == 0:
            ...
            # Mark 'all' to emit .nansum() with no dim argument.
            tensor._nansum_dim = "all"
        else:
    ...
    def codegen(self, output_name, input_names, output_tensor):
        """Generate code for nansum operation."""
        sd = getattr(output_tensor, "_nansum_dim", None)
        src = input_names[0]
        if sd == "all":
            return f"{output_name} = {src}.nansum()"
        elif isinstance(sd, tuple):
            # If you later extend to multi-dim reductions, this handles it.
            return f"{output_name} = {src}.nansum(dim={sd})"
