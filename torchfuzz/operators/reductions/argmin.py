...
class ArgminOperator(Operator):
    ...
    def decompose(self, tensor):
        """
        Construct an input shape that reduces to tensor.size via an argmin.
        ...
        if len(tensor.size) == 0:
            ...
            # Mark 'all' to emit .argmin() with no dim argument.
        ...
        # Input tensor can be float32, bfloat16, or float16 (common types for argmin)
    ...
    def codegen(self, output_name, input_names, output_tensor):
        """Generate code for argmin operation."""
        ad = getattr(output_tensor, "_argmin_dim", None)
        src = input_names[0]
        if ad == "all":
            return f"{output_name} = {src}.argmin()"
        elif isinstance(ad, tuple):
            # If you later extend to multi-dim reductions, this handles it.
            return f"{output_name} = {src}.argmin(dim={ad})"
