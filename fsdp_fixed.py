    ...
    def _wait_for_post_backward(self) -> None:
        ...
        for m in self.modules():  # includes self
            if isinstance(m, FullyShardedDataParallel):
                if not self._is_root and any(p.requires_grad for p in m.parameters()):
                    managed_param_requires_grad = any(p.requires_grad for p in m.params)
                    if managed_param_requires_grad:
                        hook_called = getattr(p, '_post_backward_called', False)
                        if hook_called:
                        ...
                        # TODO - this does not seem to be correct when module unused
