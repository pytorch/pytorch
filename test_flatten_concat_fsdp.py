# python -m torch.distributed.run --standalone --nproc_per_node=4 test_flatten_concat_fsdp.py
...
if __name__ == "__main__":
    ...
    def fn_flatten_concat(arg55_1: "f32[96, 16][16, 1]cuda:0", arg56_1: "f32[96][1]cuda:0"):
        world_size = 4
        rank = 0
        group_name = '0'
        ...
        param_all_gather_outputs = [
            torch.empty(torch.Size([t.numel() * world_size]), dtype=t.dtype, device=device)
            for t in param_all_gather_inputs
        ]
        param_all_gather_outputs_shape_orig = [(t.shape[0] * world_size,) + t.shape[1:] for t in param_all_gather_inputs_orig]
        ...
        all_gather_into_tensor = torch.ops._c10d_functional.all_gather_into_tensor_out.default(all_gather_input, world_size, group_name, out=all_gather_output)
        wait_tensor = torch.ops._c10d_functional.wait_tensor.default(all_gather_into_tensor)
        all_gather_output = all_gather_output.view(world_size, -1)
        out = [t.view(world_size, -1) for t in param_all_gather_outputs]
    ...
    out_flatten_concat = fn_flatten_concat(arg55_1, arg56_1)
