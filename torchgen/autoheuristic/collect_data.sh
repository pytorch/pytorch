...
if [ "$#" -ne 3 ]; then
    echo "Usage: $0 \"<python_command>\" <comma_separated_device_numbers> <num_samples to generate>"
    echo "Example: $0 \"python run.py --a b --b c\" 1,4,5,3"
    exit 1
fi
...
create_pane() {
    ...
    tmux send-keys -t "$SESSION_NAME" "conda activate ${CONDA_ENV} && $PYTHON_COMMAND --device $device -o data_${device}.txt --num-samples ${NUM_SAMPLES_PER_GPU}" C-m
}
