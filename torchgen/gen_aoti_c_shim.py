...
def gen_declaration_and_definition(
    ...
    for ver_id, new_args in sorted(indexed_op_metadata.items(), reverse=True):
        ...
        # scatter/scatter_add/scatter_reduce into an empty self is a no-op that
        # still crashes ATen's out-variant in as_strided on the zero-storage
        # self. Short-circuit it, producing an out that mirrors self (empty, same
        # sizes/dtype/device/layout via clone) so ATen out semantics hold, and
        # only when out is a real handle. The self/out parameter names come from
        # the schema rather than being hardcoded, so a future variant that
        # renames them (self_arg unset / no single out) falls back to the normal
        # path instead of emitting an uncompilable shim.
        self_arg = schema.arguments.self_arg
        out_args = schema.arguments.out
        emit_scatter_guard = (
            schema.is_out_fn()
            and schema.name.name.base in {"scatter", "scatter_add", "scatter_reduce"}
            and self_arg is not None
            and len(out_args) == 1
        )
        self_name = self_arg.argument.name if self_arg is not None else "self"
        out_name = out_args[0].name if len(out_args) == 1 else "out"
        scatter_guard_str = (
            (
                "        {\n"
                "          at::Tensor* _self_t =\n"
                f"              torch::aot_inductor::tensor_handle_to_tensor_pointer({self_name});\n"
                "          at::Tensor* _out_t =\n"
                f"              torch::aot_inductor::tensor_handle_to_tensor_pointer({out_name});\n"
                "          if (_self_t != nullptr && _self_t->defined() &&\n"
                "              _self_t->numel() == 0 && _out_t != nullptr) {\n"
                "            if (_out_t != _self_t) {\n"
                "              *_out_t = _self_t->clone();\n"
                "            }\n"
                "            C10_LOG_EVERY_MS(WARNING, 10000)\n"
                f'                << "[AOTIScatterSkip] op=aoti_torch_{device}_{func_name}"\n'
                '                << " self.numel=0; skipped scatter (empty-self no-op).";\n'
                "            return AOTI_TORCH_SUCCESS;\n"
                "          }\n"
                "        }\n"
            )
            if emit_scatter_guard
            else ""
        )
        definition = (
            textwrap.dedent(f"""
        ...
        if scatter_guard_str:
            # Inject the guard right after the exception-macro opening brace,
            # leaving the single-dedent layout of the rest of the body untouched
            # so non-scatter shims are generated byte-identically.
            macro_open = "    AOTI_TORCH_CONVERT_EXCEPTION_TO_ERROR_CODE({\n"
            definition = definition.replace(
                macro_open, macro_open + scatter_guard_str, 1
            )
