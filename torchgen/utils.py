...
@functools.cache
def _read_template(template_filename: str) -> CodeTemplate:
    return CodeTemplate.from_file(template_filename)
...
class FileManager:
    ...
    def substitute_with_template(
        self, template_filename: str, env_callable: Callable[[], str | dict[str, Any]]
    ) -> str:
            ...
            if "generated_comment" not in env:
                ...
                env = {
                    **env,  # copy the original dict instead of mutating it
                    "generated_comment": (
                        "@" + f"generated by {generator_path} from {template_filename}"
                    ),
                }
