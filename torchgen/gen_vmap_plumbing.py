...
def gen_vmap_inplace_plumbing(native_function: NativeFunction) -> str | None:
    ...
    # NOTE: [vmap and DTensor Implicit Replication]
    return f"""\
template <typename batch_rule_t, batch_rule_t batch_rule>
{sig.decl(name=schema.name.unambiguous_name() + "_generated_plumbing")} {{
  c10::impl::ExcludeDispatchKeyGuard guard(DispatchKey::FuncTorchBatched);
  at::DTensorAllowImplicitReplication guard2;
...
def gen_vmap_plumbing_no_returns(native_function: NativeFunction) -> str:
    ...
    # See NOTE: [vmap and DTensor Implicit Replication]
    return f"""\
template <typename batch_rule_t, batch_rule_t batch_rule>
{sig.decl(name=schema.name.unambiguous_name() + "_generated_plumbing")} {{
  c10::impl::ExcludeDispatchKeyGuard guard(DispatchKey::FuncTorchBatched);
...
def gen_vmap_plumbing(native_function: NativeFunction) -> str | None:
    ...
    # NOTE: [vmap and DTensor Implicit Replication]
    # For all batching rules, we assume that new tensors generated by factory
    # functions are meant to be replicated by DTensor. This saves us
    # the trouble of rewriting all of them into e.g. using tensor.new_*.
    # This is fine because batching rules cannot produce tensors with different
    # shapes or values across ranks (vmap is pretty constrained),
    # with the exception of random operations (those have their own separate
    # handling that is not in this file, see BatchRulesRandomness.cpp).
    #
    # Why do we prefer the mode to e.g. rewriting calls to factory functions
    # into their new_* variants?
    #
    # Consider batched_tensor + arange(n). This would work via broadcasting.
    # However, if we changed this into batched_tensor + batched_tensor.new_arange(n),
    # then we would actually allocate a repeated tensor for the arange (it
    # would be a batched tensor), and we'd be using more memory than necessary,
    # unless we implement some sort of lazy batchedtensor materialization thing.
    return f"""\
template <typename batch_rule_t, batch_rule_t batch_rule>
{sig.decl(name=schema.name.unambiguous_name() + "_generated_plumbing")} {{
  c10::impl::ExcludeDispatchKeyGuard guard(DispatchKey::FuncTorchBatched);
