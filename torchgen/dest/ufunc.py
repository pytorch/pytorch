...
from torchgen.model import NativeFunctionsGroup, ScalarType, UfuncKey, DispatchKey, BaseType, BaseTy, Argument
...
def compute_ufunc_cpu_dtype_body(
    ...
    for a in g.functional.func.arguments.flat_non_out:
        ...
        scalar_bindings.append(Binding(
            name=a.name,
            nctype=NamedCType(a.name, BaseCType(scalar_t)),
            argument=a,
        ))
        if vec_loop is not None:
            vec_bindings.append(Binding(
                name=a.name,
