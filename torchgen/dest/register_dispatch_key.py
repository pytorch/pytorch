...
"""
    ...
    def gen_func_inplace_wrapper(
        ...
        out_goal_bindings = out_args_bindings[:-num_out_args] if num_out_args > 0 else out_args_bindings
        ...
        if k is SchemaKind.functional:
            ...
            for i, ret in enumerate(f.func.returns):
                ...
                allocation_logic += f"  auto {out_name} = at::empty({{0}}, {template}.options());\n"

            # Translate arguments from functional signature to 'out' signature
            # We must append the newly created 'out' tensors to the call
            translated_args = [e.expr for e in translate(sig.arguments(), out_goal_bindings)]
            out_names = [f"out{i}" if len(f.func.returns) > 1 else "out" for i in range(len(f.func.returns))]
            call_args = translated_args + out_names
            return_statement = f"return {out_names[0]};" if len(out_names) == 1 else f"return std::make_tuple({', '.join(out_names)});"
        ...
        if self.class_method_name is None:
            ...
            impl_name = f"{out_meta.cpp_namespace}::{self.class_method_name}::{out_meta.kernel}"
