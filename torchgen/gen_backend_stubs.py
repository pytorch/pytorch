        ...
        *,
        ...
        for op in backend_ops:
            ...
            if isinstance(op, dict):
                names = [k for k in op if k not in {"structured", "dispatch_meta"}]
                assert len(names) == 1, f"Expected one op name in {op.keys()}"
                _op = names[0]
                structured = op.get("structured", False)
                dispatch_meta = op.get("dispatch_meta", False)
                if dispatch_meta:
                    assert structured, (
                        f"Operator '{_op}' has 'dispatch_meta: True' but 'structured: False'. "
