...
# Default to non-interactive print mode. You can override, for example: CLAUDE_FLAGS="-p --model sonnet"
...
ensure_file_nonempty() {
  local path="$1"
  if [[ ! -f "$path" ]]; then
    echo "error: expected file not found: $path" 1>&2
    exit 1
  fi
  if [[ ! -s "$path" ]]; then
...
# 1) Ask Claude to write plan.md directly
plan_prompt=$(
  cat <<EOF
