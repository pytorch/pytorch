...
class TestOp(JitLlgaTestCase):
    ...
    def test_linear(self):
        for bias in [True, False]:
            ...
            _, graph = self.checkTrace(m, [x])
            self.assertGraphContainsExactly(graph, LLGA_FUSION_GROUP, 1)
            self.assertFused(graph, ['aten::addmm', 'aten::add', 'aten::mm'])
            _, graph = self.checkScript(m, [x])
            self.assertGraphContainsExactly(graph, LLGA_FUSION_GROUP, 1)
            self.assertFused(graph, ['aten::addmm', 'aten::add', 'aten::mm'])
    ...
    def test_add(self):
        ...
        for x, y in self._gen_binary_inputs():
            ...
            _, graph = self.checkScript(forward_add, [x, y])
            self.assertGraphContainsExactly(graph, LLGA_FUSION_GROUP, 2)
    ...
    def test_add_scalar(self):
        ...
        self.assertGraphContainsExactly(graph, LLGA_FUSION_GROUP, 2)
        _, graph = self.checkScript(add_scalar, [x])
        self.assertGraphContainsExactly(graph, LLGA_FUSION_GROUP, 2)
    ...
    def test_mul(self):
        ...
        for x, y in self._gen_binary_inputs():
            ...
            self.assertGraphContainsExactly(graph, LLGA_FUSION_GROUP, 2)
            _, graph = self.checkScript(forward_mul, [x, y])
            self.assertGraphContainsExactly(graph, LLGA_FUSION_GROUP, 2)
    ...
    def test_identity_binary(self):
        ...
        _, graph = self.checkScript(forward, [x])
        self.assertFused(graph, ['aten::add', 'aten::mul'])
    ...
    def test_matmul(self):
        ...
        self.assertGraphContainsExactly(graph, LLGA_FUSION_GROUP, 1)
        _, graph = self.checkScript(forward_matmul, [x, y])
        self.assertGraphContainsExactly(graph, LLGA_FUSION_GROUP, 1)
    ...
    @unittest.skipIf(True, 'Concat is not supported')
    def test_cat(self):
        def cat_along_dim(d):
        ...
        for d in range(0, 4):
            x = torch.rand(8, 8, 8, 8)
            _, graph = self.checkTrace(cat_along_dim(d), [x, x, x])
            self.assertGraphContainsExactly(graph, LLGA_FUSION_GROUP, 1)


class TestBackwardOp(JitLlgaTestCase):
    def test_sigmoid(self):
        ...
        m = M()
        x = torch.rand(1, 16, 14, 14, requires_grad=True)
        _, fwd_graph, bwd_graph = self.checkTrace(m, [x])
        self.assertFused(fwd_graph, ['aten::sigmoid'])
        self.assertFused(bwd_graph, ['aten::add', 'aten::mul'])

    def test_linear_bias(self):
        x = torch.rand(32, 28, requires_grad=True)
        m = torch.nn.Linear(in_features=28, out_features=64, bias=True)
        _, fwd_graph, bwd_graph = self.checkTrace(m, [x])
        self.assertFused(fwd_graph, ['aten::mm'])
        self.assertFused(bwd_graph, ['aten::mm'])
...
for model_name, enabled in [
    ...
    ['mobilenet_v2', False],  # TODO: backend fix for conv-bn-add fusion
    ['mnasnet1_0', False],    # TODO: backend fix for conv-bn-add fusion
    ['squeezenet1_0', True],
    ['vgg16', True],
    ['alexnet', True],
