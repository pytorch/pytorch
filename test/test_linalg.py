...
class TestLinalg(TestCase):
    ...
    def test_inner(self, device, dtype):
        ...
        self.assertEqual(
            a.inner(b).cpu().numpy(), np.inner(a.cpu().numpy(), b.cpu().numpy())
        )
    ...
    def test_cholesky_errors_and_warnings(self, device, dtype):
        ...
        with self.assertRaisesRegex(
            RuntimeError, r"For batch 4: U\(3,3\) is zero, singular U\."
        ):
            torch.linalg.cholesky(A)
    ...
    def test_cholesky_ex_non_pd(self, device, dtype):
        ...
        with self.assertRaisesRegex(
            RuntimeError, r"For batch 3: U\(2,2\) is zero, singular U\."
        ):
            torch.linalg.cholesky_ex(A, check_errors=True)
    ...
    def test_old_cholesky_autograd(self, device, dtype):
        ...
        def run_test(upper, dims):
            ...
            self.assertEqual(
                root.grad, root.grad.transpose(-1, -2).conj()
    ...
    def test_kron_non_contiguous(self, device, dtype):
        def run_test_transposed(a_shape, b_shape):
            ...
            # check the out= variant
            out = torch.empty(
                result.transpose(-2, -1).shape, dtype=dtype, device=device
            ).transpose(-2, -1)
        ...
        def run_test_skipped_elements(a_shape, b_shape):
            # check for transposed case
            a = torch.rand(2 * a_shape[0], *a_shape[1:], dtype=dtype, device=device)[
                ::2
            ]
            b = torch.rand(2 * b_shape[0], *b_shape[1:], dtype=dtype, device=device)[
                ::2
            ]
            self.assertFalse(a.is_contiguous())
            self.assertFalse(b.is_contiguous())
            ...
            # check the out= variant
            out = torch.empty(
                2 * result.shape[0], *result.shape[1:], dtype=dtype, device=device
            )[::2]
            self.assertFalse(out.is_contiguous())
            ans = torch.kron(a, b, out=out)
            self.assertEqual(ans, out)
        ...
        a = torch.randn(1, 2, 3, 4, dtype=dtype, device=device).contiguous(
            memory_format=torch.channels_last
        )
        b = torch.randn(1, 2, 3, 4, dtype=dtype, device=device).contiguous(
            memory_format=torch.channels_last
        )
        c = torch.kron(a, b)
    ...
    def test_norm_dtype(self, device):
        def run_test_case(input_size, ord, keepdim, from_dtype, to_dtype):
            ...
            self.assertEqual(
                result.to(compare_dtype), result_out.to(compare_dtype), msg=msg
            )
            ...
            if from_dtype.is_complex:
                result_convert_first = torch.linalg.norm(
                    input.to(to_dtype), ord, keepdim=keepdim
                )
                self.assertEqual(
                    result_with_dtype.to(compare_dtype),
                    result_convert_first.to(compare_dtype),
                    msg=msg,
                )
            else:
                self.assertEqual(
                    result.to(compare_dtype),
                    result_with_dtype.to(compare_dtype),
                    msg=msg,
                )
            ...
            for input_size, ord_settings in test_cases:
                for ord in ord_settings:
                    for dtype, out_dtype in dtype_pairs:
                        ...
                        with self.assertRaisesRegex(
                            RuntimeError, r"provided dtype must match dtype of result"
                        ):
                            torch.linalg.norm(
                                input, ord=ord, keepdim=keepdim, dtype=dtype, out=result
                            )
        ...
        def run_test_case(input, ord, dim, keepdim, norm_dtype):
            ...
            if error_msg is None:
                ...
                if norm_dtype is not None:
                    result_convert_before = torch.linalg.vector_norm(
                    ...
                    result_out = torch.empty(
                        (0), dtype=result_dtype.dtype, device=device
                    )
                    torch.linalg.vector_norm(
                        input, ord, dim=dim, keepdim=keepdim, out=result_out
                    )
    ...
    def test_vector_norm_out_dtype_error(self, device, dtype):
        ...
        for norm_dtype, out_dtype in product(dtypes, dtypes):
            ...
            if (
                dtype.is_complex
                and norm_dtype is not None
                and not norm_dtype.is_complex
            ):
                with self.assertRaisesRegex(
                    RuntimeError,
                    r"linalg.vector_norm expected complex 'dtype'",
                    msg=msg,
                ):
                ...
                with self.assertRaisesRegex(
                    RuntimeError,
                    r"linalg.vector_norm expected out tensor dtype",
                    msg=msg,
                ):
                    torch.linalg.vector_norm(input, dtype=norm_dtype, out=result)
            else:
    ...
    def test_norm_fused_type_promotion(self, device, dtype):
        ...
        for f, kwargs, fn_name in zip(
            (torch.norm, torch.linalg.vector_norm),
            ({"p": 2}, {}),
    ...
    def test_cond_errors_and_warnings(self, device, dtype):
        ...
        for p in norm_types:
            with self.assertRaisesRegex(
                RuntimeError, r"supports matrices or batches of matrices"
            ):
        ...
        for p in norm_types:
            with self.assertRaisesRegex(
                RuntimeError, r"supports square matrices or batches of square matrices"
            ):
                torch.linalg.cond(a, p)
        ...
        for p in ["wrong_norm", 5]:
            with self.assertRaisesRegex(
                RuntimeError, f"linalg_cond got an invalid norm type: {p}"
            ):
                torch.linalg.cond(a, p)
    ...
    def test_norm_errors(self, device, dtype):
        ...
        error_test_cases = [
            ...
            (
                ...
                None,
                RuntimeError,
                r'order "fro" can only be used if either len\(dim\) == 2',
            ),
            (
                (S,),
                ["nuc"],
                None,
                RuntimeError,
                r'order "nuc" can only be used if either len\(dim\) == 2',
            ),
            (
                (S, S),
                [3.5],
                None,
                RuntimeError,
                r"Order 3.5 not supported for matrix norm",
            ),
            ...
            (
                ...
                (0, 0),
                RuntimeError,
                r"Expected dims to be different",
            ),
            ...
            (
                ...
                (0, 1, 2),
                RuntimeError,
                r"'dim' must specify 1 or 2 dimensions",
            ),
            (
                (S, S, S),
                [1],
    ...
    def test_matrix_norm(self, device, dtype):
        ...
        with self.assertRaisesRegex(
            RuntimeError, r"linalg.matrix_norm\(\):.*must be a matrix.*"
        ):
    ...
    def test_old_eig_reuse(self, device, dtype):
        ...
        Xhat = np.matmul(
            np.matmul(v.cpu(), torch.diag(e.select(1, 0)).cpu()), v.t().cpu()
        )
    ...
    def test_old_eig_non_contiguous(self, device, dtype):
        ...
        torch.eig(X, True, out=(e, v))
        Xhat = np.matmul(
            np.matmul(v.cpu(), torch.diag(e.cpu().select(1, 0))), v.t().cpu()
        )
        if dtype is torch.float:
            atol = 1e-7
            rtol = 1e-5
            ...
            if compute_uv:
                if some:
                    ...
                    self.assertEqual(
                        ...
                        atol=1e-8,
                        rtol=0,
                        msg="Incorrect reconstruction using U @ diag(S) @ V.T",
                    )
                else:
            ...
            if x.numel() > 0:
                ...
                if compute_uv:
                    if some:
                        x_recon = torch.matmul(
                            resu,
                            torch.matmul(ress.diag_embed(), resv.transpose(-2, -1)),
                        )
                        self.assertEqual(
                            x,
                            x_recon,
    ...
    def test_cholesky_solve_autograd(self, device, dtype):
        ...
        for (a_size, b_size), upper in itertools.product(
            [
                ((3, 3), (3, 4)),
                ((3, 3), (3, 2)),
                ((2, 3, 3), (2, 3, 4)),
    ...
    def test_inverse(self, device, dtype):
        ...
        def run_test(torch_inverse, matrix, batches, n):
            ...
            matrix_inverse_out_t = matrix_inverse_out.transpose(-2, -1).clone(
    ...
    def test_inverse_errors(self, device, dtype):
        ...
        def run_test_singular_input(batch_dim, n):
            ...
            x[n, -1, -1] = 0
            with self.assertRaisesRegex(
                RuntimeError, rf"For batch {n}: U\(3,3\) is zero"
            ):
                torch.inverse(x)
    ...
    def test_pinv_errors_and_warnings(self, device, dtype):
        ...
        if torch.cuda.is_available():
            ...
            with self.assertRaisesRegex(
                RuntimeError, "Expected rcond and input to be on the same device"
            ):
    ...
    def solve_test_helper(self, A_dims, b_dims, device, dtype):
        ...
        b = torch.randn(*b_dims, dtype=dtype, device=device)
        A = random_fullrank_matrix_distinct_singular_value(*A_dims, dtype=dtype).to(
            device
        )
        return b, A
    ...
    def test_solve_batched_non_contiguous(self, device, dtype):
        ...
        A = (
            random_fullrank_matrix_distinct_singular_value(2, 2, dtype=dtype)
            .to(device)
            .permute(1, 0, 2)
        )
    ...
    def test_tensorinv_singular_input(self, device, dtype):
        def check_singular_input(a_shape, ind):
            ...
            with self.assertRaisesRegex(
                RuntimeError,
                "Failed to invert the input tensor, because it is singular",
            ):
                torch.linalg.tensorinv(a, ind=ind)
    ...
    def test_old_matrix_rank(self, device, dtype):
        ...
        self.assertEqual(
            torch.matrix_rank(a, 0.01), np.linalg.matrix_rank(a.cpu().numpy(), 0.01)
        )

        aaT = torch.mm(a, a.conj().t())
        self.assertEqual(
            torch.matrix_rank(aaT), np.linalg.matrix_rank(aaT.cpu().numpy())
        )
        self.assertEqual(
            torch.matrix_rank(aaT, 0.01), np.linalg.matrix_rank(aaT.cpu().numpy(), 0.01)
        )

        if np.lib.NumpyVersion(np.__version__) >= "1.14.0":
            self.assertEqual(
                torch.matrix_rank(aaT, True),
                np.linalg.matrix_rank(aaT.cpu().numpy(), True),
            )
            self.assertEqual(
                torch.matrix_rank(aaT, 0.01, True),
    ...
    def test_chain_matmul(self, device, dtype):
        ...
        with self.assertRaisesRegex(
            ...
            torch.chain_matmul(
                make_tensor(1, device, dtype), make_tensor(1, device, dtype)
            )
    ...
    def test_multi_dot(self, device, dtype):
        def check(*shapes, noncontiguous=False):
            tensors = [
                make_tensor(shape, device, dtype, noncontiguous=noncontiguous)
                for shape in shapes
            ]
    ...
    def test_multi_dot_errors(self, device, dtype):
        ...
        check(
            [a, make_tensor((2, 2, 2), device, dtype), a], None, "tensor 1 must be 2D"
        )

        check(
            [a, make_tensor(2, device, torch.double)],
            None,
    ...
    def test_linalg_qr_autograd_errors(self, device, dtype):
        ...
        b = torch.sum(r)
        with self.assertRaisesRegex(
            RuntimeError, "The derivative of qr is not implemented when mode='r'"
        ):
            b.backward()
        #
    ...
    def test_qr_error_cases(self, device, dtype):
        ...
        with self.assertRaisesRegex(
            RuntimeError,
            "qr input should have at least 2 dimensions, but has 1 dimensions instead",
        ):
            torch.linalg.qr(t1)
    ...
    def test_einsum(self, device, dtype):
        ...
        self._check_einsum(
            "ngrg...->nrg...", make_tensor((2, 1, 3, 1, 4), device, dtype)
        )
        ...
        self._check_einsum(
            "i...j, ij... -> ...ij", C, make_tensor((2, 5, 2, 3), device, dtype)
        )
    ...
    def test_einsum_random(self, device, dtype):
        ...
        def test(
            ...
            min_ops=1,
            max_ops=3,  # min and max number of operands per test
            min_dims=1,
    ...
    def test_einsum_corner_cases(self, device):
        def check(equation, *operands, expected_output):
            tensors = [
                ...
                else make_tensor(operand, device, torch.float32)
    ...
    def test_einsum_error_cases(self, device):
        ...
        check(
            "a, ba",
            [x, y],
            regex=r"operands do not broadcast with remapped shapes \[original->remapped\]: "
            r"\[2\]->\[1, 2\] \[2, 3\]->\[2, 3\]",
        )
    ...
    def test_triangular_solve(self, device, dtype):
        ...
        for (k, n), (upper, unitriangular, transpose) in itertools.product(
            zip(ks, ns), itertools.product([True, False], repeat=3)
        ):
        ...
        for m in range(1, 8):
            for p in range(1, 8):
                for o in range(1, 5):
                    ...
                    x = torch.arange(m, device=device)
                    y = (
                        torch.arange(o * p * m, device=device)
                        .reshape(o, p, m)
                        .transpose(-1, -2)
                    )
                    self.check_single_matmul(x, y, (o, n, p))
                    ...
                    y = torch.arange(o * m * 2 * p, device=device).reshape(o, m, 2 * p)[
                        :, :, ::2
                    ]
                    self.check_single_matmul(x, y, (o, n, p))
                    ...
                    for r in range(1, 5):
                        ...
                        x = torch.arange(m)
                        y = torch.arange(r * o * m * p, device=device).reshape(
                            r, o, m, p
                        )
                        ...
                        x = torch.arange(m)
                        y = (
                            torch.arange(r * o * p * m, device=device)
                            .reshape(r, o, p, m)
                            .transpose(-1, -2)
                        )
                        self.check_single_matmul(x, y, (r, o, n, p))
                        ...
                        x = torch.arange(2 * m, device=device)[::2]
                        y = torch.arange(r * o * m * 2 * p, device=device).reshape(
                            r, o, m, 2 * p
                        )[:, :, :, ::2]
                        self.check_single_matmul(x, y, (r, o, n, p))
            ...
            for m in range(1, 5):
                for p in range(1, 5):
                    for o in range(1, 3):
                        ...
                        x = (
                            torch.arange(m * n, device=device)
                            .reshape(m, n)
                            .transpose(-1, -2)
                        )
                        y = (
                        ...
                        x = torch.arange(n * 2 * m, device=device).reshape(n, 2 * m)[
                            :, ::2
                        ]
                        y = torch.arange(o * m * 2 * p, device=device).reshape(
                            o, m, 2 * p
                        )[:, :, ::2]
                        self.check_single_matmul(x, y, (o, n, p))
                        ...
                        for r in range(1, 2):
                            ...
                            x = torch.arange(n * 2 * m, device=device).reshape(
                                n, 2 * m
                            )[:, ::2]
                            y = torch.arange(r * o * m * 2 * p, device=device).reshape(
                                r, o, m, 2 * p
    ...
    def test_cross_errors(self, device):
        self.assertRaisesRegex(
            ...
            lambda: torch.cross(
                torch.rand(100, 3, device=device), torch.rand(100, 3, 10, device=device)
            ),
        )
        self.assertRaisesRegex(
            RuntimeError,
            ...
            lambda: torch.cross(
                torch.rand(5, 3, device=device), torch.rand(3, 5, device=device)
            ),
        )
        self.assertRaisesRegex(
            RuntimeError,
            ...
            lambda: torch.cross(
                torch.rand(5, 4, device=device), torch.rand(5, 4, device=device)
            ),
        )
        self.assertRaisesRegex(
            RuntimeError,
            ...
            lambda: torch.cross(
                torch.rand(5, 4, 3, device=device),
                torch.rand(5, 4, 3, device=device),
                dim=0,
            ),
        )
        self.assertRaisesRegex(
            ...
            lambda: torch.cross(
                torch.rand(5, 3, 4, device=device),
                torch.rand(5, 3, 4, device=device),
                dim=-1,
            ),
        )
    ...
    @dtypesIfCUDA(
        ...
        *torch.testing.get_all_fp_dtypes(
            include_half=not CUDA9, include_bfloat16=(CUDA11OrLater and SM53OrLater)
        ),
    )
    ...
    @dtypes(
        *(torch.testing.get_all_complex_dtypes() + torch.testing.get_all_fp_dtypes())
    )
    ...
    def test_lu(self, device, dtype):
        ...
        def run_test(device, pivot):
            def run_subtest(
                ...
                if a is None:
                    a = random_matrix(
                        rows, columns, *batches, **dict(singular=singular, dtype=dtype)
                    ).to(device)
                ...
                if self.device_type == "cuda":
                    ...
                    self.assertEqual(
                        np.matmul(P_nopiv_, np.matmul(L_nopiv_, U_nopiv_)), a
                    )
            ...
            for ms, batch in itertools.product(
...
""".format(
    ...
    @dtypesIfCUDA(
        ...
        *torch.testing.get_all_fp_dtypes(
            include_bfloat16=(TEST_WITH_ROCM or (CUDA11OrLater and SM53OrLater)),
    ...
    def test_addmv(self, device, dtype):
        ...
        ms = [
            ...
            .to(dtype)
            .expand(10, 100),  # to reduce errors for low precision
            # 1d
            ...
            torch.randint(3, (10, 1), dtype=torch.float, device=device)
            .to(dtype)
            .expand(10, 100),
            # 2d
    ...
    @dtypes(torch.float, torch.double)
    @dtypesIfCUDA(
        *([torch.float, torch.double] + torch.testing.get_all_complex_dtypes())
    )
    ...
    def test_solve_methods_arg_device(self, device):
        for b_device, A_device in itertools.product(["cpu", device], repeat=2):
            ...
            generic_backend_dispatch_err_str = (
                "Expected b and A to be on the same device"
            )
            specific_backend_dispatch_err_str = (
            ...
            with self.assertRaisesRegex(
                RuntimeError, specific_backend_dispatch_err_str
            ):
                torch.triangular_solve(b, A)
    ...
    def test_matrix_power_negative(self, device, dtype):
        ...
        def check(*size):
            t = random_fullrank_matrix_distinct_singular_value(
    ...
    def test_matrix_exp_boundary_cases(self, device, dtype):
        ...
        with self.assertRaisesRegex(
            RuntimeError, "expected a tensor of floating or complex types"
        ):
        ...
        with self.assertRaisesRegex(
            RuntimeError, "expected a tensor of squared matrices"
        ):
    ...
    def test_matrix_exp_analytic(self, device, dtype):
        ...
        self.assertTrue(
            (x.matrix_exp() == torch.eye(20, 20, dtype=dtype, device=device))
            .all()
            .item()
        )
    ...
    def test_slogdet_errors_and_warnings(self, device, dtype):
        ...
        with self.assertRaisesRegex(
            RuntimeError, r"of float, double, cfloat or cdouble types"
        ):
            torch.linalg.slogdet(a)
    ...
    def test_det_logdet_slogdet_batched(self, device, dtype):
        ...
        def run_test(matsize, batchdims, mat_chars):
            ...
            for fn in [torch.det, torch.logdet, torch.slogdet, torch.linalg.slogdet]:
                ...
                for full_idx in itertools.product(
    ...
    def test_cholesky_inverse_errors_and_warnings(self, device, dtype):
        ...
        if self.device_type == "cpu":
            with self.assertRaisesRegex(
                RuntimeError, r"cholesky_inverse: U\(2,2\) is zero, singular U\."
            ):
                torch.cholesky_inverse(a)
    ...
    def test_symeig(self, device, dtype):
        ...
        def run_test(dims, eigenvectors, upper):
            ...
            if eigenvectors:
                ...
                x_recon = np.matmul(
                    np.matmul(outv_, torch.diag_embed(oute.to(dtype)).cpu().numpy()),
                ...
                self.assertEqual(
                    ...
                    atol=1e-8,
                    rtol=0,
                    msg="Incorrect reconstruction using V @ diag(e) @ V.T",
                )
            else:
                ...
                self.assertEqual(
                    ...
                    outv,
                    msg="Eigenvector matrix not empty",
                )

            rese, resv = x.symeig(eigenvectors=eigenvectors, upper=upper)
            self.assertEqual(
                rese, oute, msg="outputs of symeig and symeig with out don't match"
            )
            self.assertEqual(
                resv, outv, msg="outputs of symeig and symeig with out don't match"
            )
            ...
            if eigenvectors:
                ...
                x_recon = np.matmul(
                    np.matmul(resv_, torch.diag_embed(rese.to(dtype)).cpu().numpy()),
        ...
        for batch_dims, eigenvectors, upper in itertools.product(
    ...
    def test_lstsq(self, device, dtype):
        def _test_underdetermined(a, b, expectedNorm):
            ...
            self.assertEqual(
                (torch.mm(a, res1) - b).norm(), expectedNorm, atol=1e-8, rtol=0
            )
            ...
            self.assertEqual(
                (torch.mm(a_copy, b) - b_copy).norm(), expectedNorm, atol=1e-8, rtol=0
            )
