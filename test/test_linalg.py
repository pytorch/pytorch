...
from torch.testing._internal.common_device_type import \
    (instantiate_device_type_tests, dtypes, has_cusolver, has_hipsolver,
     onlyCPU, skipCUDAIf, skipCUDAIfNoCusolver, skipCPUIfNoLapack, precisionOverride,
     skipCUDAIfRocm, onlyNativeDeviceTypes, dtypesIfCUDA,
     onlyCUDA, skipMeta, skipCUDAIfNotRocm, skipCUDAIfRocmVersionLessThan,
...
from torch.testing._internal.common_cuda import SM53OrLater, SM80OrLater, SM90OrLater, tf32_on_and_off, \
...
class TestLinalg(TestCase):
    ...
    def test_eig_check_cusolver(self, device, dtype):
        # For CUDA inputs only matrices of size larger than 2048x2048 actually call cusolver library
        shape = (2049, 2049)
        a = make_tensor(shape, dtype=dtype, device=device)
    ...
    @unittest.skipIf(IS_FBCODE or IS_SANDCASTLE, "Test fails for float64 on GPU (P100, V100) on Meta infra")
    @onlyCUDA
    @skipCUDAIfNoCusolver  # cusolver needed for the PLU decomposition
    @dtypes(*floating_and_complex_types())
    @precisionOverride({torch.float32: 1e-2, torch.complex64: 1e-2,
                        torch.float64: 1e-8, torch.complex128: 1e-8})
    def test_linalg_solve_triangular_large(self, device, dtype):
        # Exercises cusolver and cublas
        cusolver = (9, 513, 1)
        iterative_cublas = (2, 64, 1)

        gen_inputs = self._gen_shape_inputs_linalg_triangular_solve
        for shape in (cusolver, iterative_cublas):
