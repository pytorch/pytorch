...
class TestLinalg(TestCase):
    ...
    def test_lobpcg_torchscript(self, device, dtype):
        ...
        eq_err = torch.linalg.vector_norm((mm(A1, V1) - V1 * E1), 2) / E1.max()
    ...
    def test_lobpcg_scipy(self, device, dtype):
        ...
        eq_err = torch.linalg.vector_norm((mm(A1, V1) - mm(B1, V1) * E1), 2) / E1.max()
...
-(input size: {:4}, eigenpairs:{:2}, units: ms per call)-
        ...
        eq_err_general = torch.linalg.vector_norm((mm(A1, V1) - mm(B1, V1) * E1), 2) / E1.max()
