...
class TestFxSplitNodeFinder(TestCase):
    ...
    def _testTrackerMode(self, mode):
        ...
        Args:
            mode: the mode to be tested. 0: no local dump, 1: dump all events to file, 2: dump specific nodes in recursive manner, 3: dump all nodes with more than 1 event in recursive manner.
        """
        ...
        if mode == 0:
            ...
            self._assert_events_file(events_file)
            self._validate_file_content(nodes_file, ["|-sup_f_2: init_acc|callable_and_operator_supported #"]),
        elif mode == 3:
            self._assert_events_file(events_file)
            self._validate_file_content(nodes_file, ["|-sup_f_1: init_acc|callable_and_operator_supported #",
            "|-sup_f_1: acc_del|non_tensor_output_with_cpu_user add",
            "| |-add: init_cpu|operator_support #"])
