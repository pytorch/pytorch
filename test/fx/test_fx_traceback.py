...
class TestFXTracebackThreadLocalState(TestCase):
    def test_caller_trace_avoids_native_tls_lookup(self):
        with fx_traceback.preserve_node_meta():
            with (
                patch.object(
                    torch._C,
                    "_is_key_in_tls",
                    side_effect=AssertionError("native TLS lookup"),
                ),
                patch.object(
                    torch._C,
                    "_get_obj_in_tls",
                ...
                torch.fx.symbolic_trace(lambda x: x + 1)
    ...
    def test_preserve_node_contexts_restore_current_thread(self):
        ...
        try:
            ...
            self.assertFalse(bool(fx_traceback.should_preserve_node_meta))
            ...
            with fx_traceback.preserve_node_meta():
                self.assertTrue(fx_traceback.has_preserved_node_meta())
                self.assertTrue(bool(fx_traceback.should_preserve_node_meta))
