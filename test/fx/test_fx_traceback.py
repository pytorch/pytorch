...
(name=add, pass_name=test_pass, action=create, graph_id={graph_id})
    ...
    def test_graph_provenance(self):
        ...
        node_name_to_from_node = {node.name: node.meta["from_node"] for node in decomposed_ep.graph.nodes if node.op not in {"placeholder", "output"}}
        same_ancestor_nodes = {"permute": "addmm", "addmm": "permute", "permute_1": "addmm_1", "addmm_1": "permute_1"}

        for node_name_1 in node_name_to_from_node:
            for node_name_2 in node_name_to_from_node:
                if node_name_2 in {node_name_1, same_ancestor_nodes[node_name_1] if node_name_1 in same_ancestor_nodes else None}:
                    self.assertTrue(node_name_to_from_node[node_name_1] == node_name_to_from_node[node_name_2])
                else:
                    self.assertTrue(node_name_to_from_node[node_name_1] != node_name_to_from_node[node_name_2])
