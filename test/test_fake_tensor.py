...
[torch.storage.TypedStorage(dtype=torch.float32, device=meta) of size 3]""",
    ...
    def test_convert_fake_module_without_swap_tensors(self):
        old_swap_tensors = torch.__future__.get_swap_module_params_on_conversion()
        try:
            for swap_tensors in (False, True):
                torch.__future__.set_swap_module_params_on_conversion(swap_tensors)
                with FakeTensorMode():
            ...
            torch.__future__.set_swap_module_params_on_conversion(old_swap_tensors)
        ...
        try:
            for swap_tensors in (False, True):
                ...
                with FakeTensorMode() as mode:
                    ...
                    with self.assertRaisesRegex(
                        AssertionError, "Please convert all Tensors"
                    ):
