...
class FakeTensorDispatchCache(TestCase):
    ...
    @unittest.skipIf(not RUN_CUDA, "requires cuda")
    def test_offload_tensor(self):
        from torch.nested._internal.wrappers import make_offload_tensor
        from torch.nested._internal.offload_tensor import request_offload_all, OffloadTensor, offload_tensor_registry, OffloadTensorRegistry

        with offload_tensor_registry(OffloadTensorRegistry()):
