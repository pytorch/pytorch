...
class FakeTensorTest(TestCase):
    ...
    def test_mkldnn_to_dense(self):
        ...
        with FakeTensorMode() as fake_mode:
            ...
            flattened = torch.flatten(mkldnn, 0)
            self.assertTrue(flattened.is_mkldnn)
...
class FakeTensorViewCopy(TestCase):
    ...
    def test_mkldnn_reshape_and_to_dense_match_eager(self):
        ...
        with FakeTensorMode() as fake_mode:
            ...
            reshaped = fake_x.reshape(fake_x.size(0), -1)
            aliased = torch.ops.aten.alias.default(reshaped)
            self.assertTrue(aliased.is_mkldnn)
            with self.assertRaisesRegex(RuntimeError, "Change to use reshape"):
                aliased.view(aliased.size(0), -1)

            self.assertEqual(torch.ops.prim.device.default(fake_x), torch.device("cpu"))
            self.assertFalse(
                _should_propagate_mkldnn(torch.ops.prim.device.default, (fake_x,))
            )
            ...
            for to_dense in (
                ...
                with self.assertRaisesRegex(
                    RuntimeError, "mkldnn tensor only can be converted"
                ):
                    to_dense()
