...
class TestQuantizedOps(TestCase):
    ...
    def test_qtopk(self, X, k, dim, largest, sorted):
        X, (scale, zero_point, torch_type) = X
        qX = torch.quantize_linear(torch.from_numpy(X), scale, zero_point, torch_type)
        assume(dim < X.ndim)
        assume(k < X.shape[dim])

        unquantized_out = torch.topk(qX.dequantize(), k, dim=dim, largest=largest, sorted=sorted)

        values = torch.quantize_linear(torch.from_numpy(X), scale, zero_point, torch_type)
        ...
        torch.topk(qX, k, out=(values, indices), dim=dim, largest=largest, sorted=sorted)

        torch.testing.assert_allclose(values.dequantize(), unquantized_out[0])
        torch.testing.assert_allclose(indices, unquantized_out[1])
