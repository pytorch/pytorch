...
def foo(x):
    ...
    def test_file_line_error(self):
        ...
        with self.assertRaisesRegex(RuntimeError, "test_jit_script.py\", line {}".format(lineno + 1)):
    ...
    def test_file_line_error_class_defn(self):
        ...
        _, lineno = inspect.getsourcelines(FooBar)
        with self.assertRaisesRegex(RuntimeError, "test_jit_script.py\", line {}".format(lineno + 2)):
    ...
    def test_file_line_graph(self):
        ...
        _, lineno = inspect.getsourcelines(foobar)
        fc = FileCheck().check('test_jit_script.py:{}:19'.format(lineno + 1))
...
def foo(xyz):
    ...
    def test_file_line_trace(self):
        ...
        _, lineno = inspect.getsourcelines(foobar)
        fc = FileCheck().check('test_jit_script.py:{}:0'.format(lineno + 1))
        fc.run(scripted.graph)
        fc.run(str(scripted.graph))
    ...
    def test_serialized_source_ranges(self):
        ...
        with self.assertRaisesRegex(RuntimeError, 'test_jit_script.py\", line {}'.format(lineno + 3)):
    ...
    def test_serialized_source_ranges_graph(self):
        ...
        fc = FileCheck().check('test_jit_script.py:{}'.format(lineno + 3))
        fc.run(loaded.graph)
    ...
    def test_serialized_source_ranges2(self):
        ...
        with self.assertRaisesRegex(torch.jit.Error, 'test_jit_script.py\", line {}'.format(lineno + 3)):
            ft = FooTest2()
            loaded = self.getExportImportCopy(ft)
            loaded()
...
dedent """
    ...
    def test_mypy_type_ignore(self):
        @torch.jit.script
        def foo(x):  # type: ignore[no-redef]
            return x
