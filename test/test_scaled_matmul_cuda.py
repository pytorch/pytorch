...
class TestFP8MatmulCUDA(TestCase):
    ...
    def test_mxfp8_nvfp4_scaled_grouped_mm_2d_2d(self, G, M, N, K, format, use_out, device):
        ...
        if use_out:
            kwargs["out"] = torch.empty((G, M, N), dtype=torch.bfloat16, device=device)
        ...
        y_bf16 = grouped_mm(xh, wh.t(), offs=input_group_end_offsets, out_dtype=torch.bfloat16)
    ...
    def test_mxfp8_scaled_grouped_mm_2d_3d(self, G, M, N, K, format, use_out, device):
        ...
        if use_out:
            kwargs["out"] = torch.empty((total_M, N), dtype=torch.bfloat16, device=device)
    ...
    def test_scaled_addmm_scalar_semantics(self, device, inplace):
        ...
        self.assertEqual(op(input.clone(), *args, alpha=0, beta=0.5), input * 0.5)
    ...
    def test_scaled_addmm_contraction_dim(self, device, fake, inplace, contraction_dim, supported):
    ...
    def test_scaled_addmm_validation(self, device):
        ...
        aligned_16 = input.new_empty(input.numel() + alignment_offset)[alignment_offset:].view_as(input)
        ...
        self.assert_scaled_addmm_inplace(aligned_16, scaled_mm(*args, output_dtype=input.dtype), args, beta=0)
        ...
        if not (IS_SM90 and _get_torch_cuda_version() >= (12, 9)):
            ...
            with self.assertRaisesRegex(NotImplementedError, "row-wise CUTLASS fallback"):
                scaled_addmm(
                    input,
                    mat1,
                    mat2,
    ...
    def test_scaled_addmm_cublas_recipes(self, device, output_dtype, recipe_a, recipe_b):
        ...
        expected = (beta * input.float() + alpha * mat1.float() @ mat2.float()).to(output_dtype)
        actual = scaled_addmm(
            input,
            mat1,
            mat2,
    ...
    def test_scaled_addmm_mxfp8(self, device, output_dtype):
        ...
        scale_a = to_blocked(torch.ones(m, k // 32, device=device, dtype=torch.float8_e8m0fnu))
        scale_b = to_blocked(torch.ones(n, k // 32, device=device, dtype=torch.float8_e8m0fnu))

        args = (mat1, mat2, scale_a, ScalingType.BlockWise1x32, scale_b, ScalingType.BlockWise1x32)
        kwargs = {"swizzle_a": SwizzleType.SWIZZLE_32_4_4, "swizzle_b": SwizzleType.SWIZZLE_32_4_4}
        actual = scaled_addmm(input, *args, **kwargs)
        reference = (input.float() + mat1_ref.float() @ mat2_ref.float().t()).to(output_dtype)
    ...
    def test_scaled_addmm_nvfp4(self, device, two_level, output_dtype):
        ...
        scale_a = to_blocked(torch.ones(m, k // 16, device=device, dtype=torch.float8_e4m3fn))
        scale_b = to_blocked(torch.ones(n, k // 16, device=device, dtype=torch.float8_e4m3fn))
        swizzle = SwizzleType.SWIZZLE_32_4_4
        gemm_scale = 1.0
        if two_level:
        ...
        expected = (beta * input.float() + alpha * gemm_scale * product).to(output_dtype)
        actual = scaled_addmm(input, *args, beta=beta, alpha=alpha, **kwargs)
        self.assertEqual(input, input_before)
        self.assertEqual(actual, expected, atol=5e-2, rtol=5e-2)
        self.assert_scaled_addmm_inplace(input.clone(), actual, args, beta=beta, alpha=alpha, **kwargs)
        ...
        if two_level and output_dtype == torch.bfloat16:
            ...
            self.assert_scaled_addmm_cudagraph(input, actual, args, beta=beta, alpha=alpha, **kwargs)

            def fn(input):
                return scaled_addmm_(input, *args, beta=beta, alpha=alpha, **kwargs)
    ...
    def test_scaled_addmm_fullgraph(self, device):
        ...
        def fn(input, mat1, mat2, scale_a, scale_b):
            return scaled_addmm(input, *tensorwise_scaled_mm_args(mat1, mat2, scale_a, scale_b))
        ...
        def fn_(input, mat1, mat2, scale_a, scale_b):
            return scaled_addmm_(input, *tensorwise_scaled_mm_args(mat1, mat2, scale_a, scale_b))
        ...
        self.assertEqual(functionalized_input, expected_inplace, atol=5e-2, rtol=5e-2)

        inplace = input.clone()
        actual_inplace, source_codes = run_and_get_code(torch.compile(fn_, fullgraph=True), inplace, *operands)
    ...
    def test_scaled_mm_row_wise_fp32_out_with_bias_errors(self, wrap_v2, device):
        ...
        with self.assertRaisesRegex((ValueError, RuntimeError), "Bias is not supported when out_dtype is set to Float32"):
            scaled_mm_wrap(
                x_fp8,
                y_fp8,
                scale_a=x_scales.reciprocal(),
    ...
    def test_scaled_mm_deepseek_error_messages(self, output_dtype, lhs_block, rhs_block, M, N, K, device):
        torch.manual_seed(42)
