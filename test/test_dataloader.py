...
"""])
    ...
    def test_iterable_style_dataset(self):
        ...
        for prefetch_factor in [2, 3, 4]:
            ...
            dataloader = self._get_data_loader(dataset, num_workers=num_workers, batch_size=7,
                                               prefetch_factor=prefetch_factor)
            dataloader_iter = iter(dataloader)
    ...
    def test_buffer_shuffle_dataset(self):
        ...
        for num_workers in [0, 1]:
            ...
            for buffer_size in buffer_sizes:
                fetched = sorted(
                    list(self._get_data_loader(BufferedShuffleDataset(dataset, buffer_size), num_workers=num_workers)))
                self.assertEqual(len(fetched), len(expected))
                for e, d in zip(expected, fetched):
                    self.assertIsInstance(d, torch.Tensor)
                    self.assertEqual(e, d)
                ...
                random.seed(123)
                fetched_seed1 = list(
                    self._get_data_loader(BufferedShuffleDataset(dataset, buffer_size), num_workers=num_workers,
    ...
    def test_shuffle_reproducibility(self):
        for fn in (
                lambda: DataLoader(self.dataset, shuffle=True, num_workers=0,
                                   generator=torch.Generator().manual_seed(42)),
                lambda: DataLoader(self.dataset, shuffle=True, num_workers=2,
                                   generator=torch.Generator().manual_seed(42)),
        ):
    ...
    def test_duplicating_data_with_drop_last(self):
        ...
        for i in range(num_processes):
            ...
            d_loader = self._get_data_loader(data_set, batch_size=int(num_batches / num_processes), drop_last=True,
                                             sampler=s)
...
class TestPartialBalancedBatchSampler(TestCase):
    def test_first_batch_balanced(self):
        tmp_label = []
        x = [[1, 1], [2, 1], [3, 0], [4, 0], [5, 2], [6, 2]]
        dataset = CustomDataset(self, x)
        b_balanced = torch.utils.data.PartialBalancedBatchSampler(dataset, balanced_classes=2, num_classes=3)
        train_loader = torch.utils.data.DataLoader(dataset, sampler=b_balanced, batch_size=6)

        for data, label in train_loader:
            tmp_label.append(label.data.numpy())
            counting = Counter(tmp_label[0])
            self.assertEqual(counting[0], 2)
            self.assertEqual(counting[1], 2)
            self.assertEqual(counting[2], 2)

    def test_all_batch_partial_balanced_batch(self):
        x = [[1, 1], [2, 1], [3, 0], [4, 0], [5, 2], [6, 2], [7, 0], [8, 2], [9, 2]]
        dataset = CustomDataset(self, x)
        b_balanced = torch.utils.data.PartialBalancedBatchSampler(dataset, balanced_classes=2, num_classes=3)
        train_loader = torch.utils.data.DataLoader(dataset, sampler=b_balanced, batch_size=6)

        for idx, (data, label) in enumerate(train_loader):
            tmp_label = [label.data.numpy()]
            counting = Counter(tmp_label[0])
            if idx == 0:
                self.assertEqual(counting[0], 2)
                self.assertEqual(counting[1], 2)
