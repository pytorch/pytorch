...
"""])
    ...
    def test_default_convert_mapping_keep_type(self):
        ...
        converted = collate.default_convert(data)
    ...
    def test_default_collate_dtype(self):
        arr = [1, 2, -1]
        collated = collate.default_collate(arr)
    ...
    def test_default_collate_bad_numpy_types(self):
        ...
        arr = np.array([[['a', 'b', 'c']]])
        self.assertRaises(TypeError, lambda: collate.default_collate(arr))
    ...
    def test_default_collate_numpy_memmap(self):
        ...
        with tempfile.TemporaryFile() as f:
            ...
            tensor = collate.default_collate(list(arr_new))

        self.assertTrue((tensor == tensor.new_tensor([[0, 1], [2, 3], [4, 5], [6, 7]])).all().item())

    def test_default_collate_bad_sequence_type(self):
        batch = [['X'], ['X', 'X']]
        self.assertRaises(RuntimeError, lambda: collate.default_collate(batch))
        self.assertRaises(RuntimeError, lambda: collate.default_collate(batch[::-1]))
    ...
    def test_default_collate_shared_tensor(self):
        ...
        self.assertEqual(collate.default_collate([t_in]).is_shared(), False)
        self.assertEqual(collate.default_collate([n_in]).is_shared(), False)
        ...
        try:
            ...
            self.assertEqual(collate.default_collate([t_in]).is_shared(), True)
            self.assertEqual(collate.default_collate([n_in]).is_shared(), True)
        finally:
