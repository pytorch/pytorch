...
class TestNestedTensorAutograd(TestCase):
    ...
    def test_nested_tensor_dropout(self):
        p = 0.2
        x = torch.nested_tensor([torch.randn((2, 5)), torch.randn((3, 4))]).requires_grad_(True)
        y = torch.nn.functional.dropout(x, p)
        y.backward(y.clone())
        actual = x.grad.to_padded_tensor(0.0)
        expect = y.to_padded_tensor(0.0).detach_() / (1.0 - p)
        self.assertEqual(actual, expect)
