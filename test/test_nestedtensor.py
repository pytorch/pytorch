...
from torch.nested._internal.nested_tensor import buffer_from_jagged, jagged_from_list
...
class TestNestedTensorSubclass(TestCase):
    ...
    def test_tensor_attributes(self, device):
        ...
        singleton_int = torch.nested._internal.nested_tensor.get_tensor_id(
            _offsets, coeff=1
        )
    ...
    def test_unary_pointwise(self, device):
        ...
        c = torch.randn(4, 3, requires_grad=True, dtype=torch.float64, device=device)
        weight = torch.randn(
            3, 4, requires_grad=True, dtype=torch.float64, device=device
        )
