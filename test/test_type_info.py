...
class TestDTypeInfo(TestCase):
    ...
    def test_finfo(self):
        ...
        for dtype in [torch.float16, torch.float32, torch.float64, torch.complex64, torch.complex128]:
            ...
            rtol = 1e-5 if dtype is torch.float16 else None
            self.assertEqual(xinfo.bits, xninfo.bits)
            self.assertEqual(xinfo.max, xninfo.max, rtol=rtol)
            self.assertEqual(xinfo.min, xninfo.min, rtol=rtol)
            self.assertEqual(xinfo.eps, xninfo.eps)
            self.assertEqual(xinfo.tiny, xninfo.tiny, rtol=rtol)
        ...
        self.assertEqual(xinfo.max, 3.38953e+38, rtol=1e-5)
        self.assertEqual(xinfo.min, -3.38953e+38, rtol=1e-5)
        self.assertEqual(xinfo.eps, 0.0078125)
        self.assertEqual(xinfo.tiny, 1.17549e-38, rtol=1e-5)
