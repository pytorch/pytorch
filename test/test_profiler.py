...
from torch.profiler import (kineto_available, profile, record_function,
                            supported_activities, DeviceType, ProfilerAction,
                            ProfilerActivity, ExecutionGraphObserver, _utils)
...
class TestProfilerCUDA(TestCase):
    ...
    @skipCUDAVersionIn([(11, 5)]
                       )  # https://github.com/pytorch/pytorch/issues/69023
...
class TestExecutionGraph(TestCase):
    ...
    def payload(self, use_cuda=False):
        ...
        with record_function("## TEST 1 ##", "1, 2, 3"):
            rf_handle = _record_function_with_args_enter(
                "## TEST 2 ##", 1, False, 2.5, [u, u], (u, u), "hello", u)
            x = torch.randn(10, 10, requires_grad=True)
            if use_cuda:
                x = x.cuda()
            y = torch.randn(10, 10, requires_grad=True)
...
class TestProfiler(TestCase):
    ...
    def test_kineto_profiler_api(self):
        ...
        test_schedule = torch.profiler.schedule(skip_first=2,
                                                wait=1,
                                                warmup=1,
                                                active=2,
                                                repeat=2)
        ...
        for step in range(len(test_schedule_expected_outputs)):
            self.assertEqual(test_schedule(step),
                             test_schedule_expected_outputs[step])
    ...
    def test_export_stacks(self):
        ...
        with TemporaryFileName(mode="w+") as fname:
            ...
            for line in lines:
                ...
                try:
                    assert int(
                        line.split(" ")[-1]) > 0, "Invalid stacks record"
                    is_int = True
                except ValueError:
                    pass
    ...
    @unittest.skip(
        "Disable forward->backward link to workaround profiler crash")
