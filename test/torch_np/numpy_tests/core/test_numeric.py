...
class TestNonarrayArgs(TestCase):
    ...
    def test_dunder_round_accuracy(self):
        ...
        assert_array_max_ulp(round(f, -73), 5.0 * 10**73)  # noqa: F821
        assert_(isinstance(round(f, ndigits=-73), np.float64))
        assert_array_max_ulp(round(f, ndigits=-73), 5.0 * 10**73)  # noqa: F821

        i = np.int64(501)
        assert_(isinstance(round(i, -2), np.int64))
        assert_array_max_ulp(round(i, -2), 500)  # noqa: F821
        assert_(isinstance(round(i, ndigits=-2), np.int64))
        assert_array_max_ulp(round(i, ndigits=-2), 500)  # noqa: F821
