...
class f(torch.nn.Module):
    def forward(self, a_1: f32[s0, s1], b_1: f32[s2, s1]):
        ...
        new_empty: f32[s0 + s2, 2*s1] = torch.ops.aten.new_empty.default(a_1, [add, add_1], device = device(type='cpu'), dtype = torch.float32, layout = torch.strided, pin_memory = False);  a_1 = add = add_1 = None
