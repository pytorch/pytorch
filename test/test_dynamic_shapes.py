...
class f(torch.nn.Module):
    ...
    def test_statically_known_false(self):
        ...
        # Statically known true
        self.assertTrue(statically_known_false(True))
        self.assertTrue(statically_known_false(s2 == s2))
        self.assertTrue(statically_known_false(s2 * s3 > s3))
        self.assertTrue(statically_known_false(s3 * s4 > s4))
        self.assertTrue(statically_known_false((s3 + s3) % 2 == 0))

        # Statically known false
        self.assertFalse(statically_known_false(False))
        self.assertFalse(statically_known_false(s3 * s4 <= s4))
        self.assertFalse(statically_known_false((s3 + s3) % 2 == 1))

        # True for hints, but not known statically
        self.assertTrue(statically_known_false(s2 + s2 == s4))
        self.assertTrue(statically_known_false(s4 % s2 == 0))
        self.assertTrue(statically_known_false(s2 != s3))
        self.assertTrue(statically_known_false(s3 * s4 > s2))

        # False for hints, but not known statically
        self.assertTrue(statically_known_false(s2 == s3))
        self.assertTrue(statically_known_false(s2 > s3))
        self.assertTrue(statically_known_false(s3 + s3 == s4))
