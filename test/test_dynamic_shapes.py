...
expected_failure_sym_magic_methods = {
    ('floordiv', 'int', 'SymFloatNode'),  # unsupported operand type(s) for //: 'int' and 'torch._C.SymFloatNode'
    ('mod', 'int', 'SymFloatNode'),  # unsupported operand type(s) for %: 'int' and 'torch._C.SymFloatNode'
    ('pow', 'float', 'SymIntNode'),  # unsupported operand type(s) for ** or pow(): 'float' and 'torch._C.SymIntNode'
    ('pow', 'SymIntNode', 'int'),  # unsupported operand type(s) for ** or pow(): 'torch._C.SymIntNode' and 'int'
    ('pow', 'int', 'SymIntNode'),  # unsupported operand type(s) for ** or pow(): 'int' and 'torch._C.SymIntNode'
    ('pow', 'SymIntNode', 'SymIntNode'),  # unsupported operand type(s) for ** or pow(): 'torch._C.SymIntNode' and 'torc
}
    ...
    def _do_test(self, fn, inp1, inp2, shape_env, is_unary_fn):
        ...
        def get_sym_inp(inp):
            if isinstance(inp, int):
                ...
                return torch.SymFloatNode.new_symfloat(seed_symfloat.wrap(inp))
