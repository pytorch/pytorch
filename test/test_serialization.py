...
class SerializationMixin(object):
    ...
    def test_save_different_dtype_unallocated(self):
        ...
        for device, dtype in product(devices, [k for k in all_types_and_complex_and(torch.half,
                                                                                    torch.bfloat16, torch.bool)]):
            a = torch.tensor([], dtype=dtype, device=device)

            for other_dtype in [k for k in all_types_and_complex_and(torch.half, torch.bfloat16, torch.bool)]:
