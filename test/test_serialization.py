...
dill = import_dill()
HAS_DILL_AT_LEAST_0_3_1 = dill is not None and check_module_version_greater_or_equal(
    dill, (0, 3, 1)
)
...
class Int4Tensor(torch.Tensor):
    ...
    def __new__(cls, elem, **kwargs):
        ...
        kwargs["requires_grad"] = False
        return torch.Tensor._make_wrapper_subclass(
            cls, up_size(elem.shape), dtype=torch.int4, **kwargs
        )
...
class SerializationMixin:
    def _test_serialization_data(self):
        ...
        t1 = torch.FloatTensor().set_(
            a[0].reshape(-1)[1:4].clone().storage(), 0, (3,), (1,)
        )
        t2 = torch.FloatTensor().set_(
            a[0].reshape(-1)[1:4].clone().storage(), 0, (3,), (1,)
        )
    ...
    def test_serialization_dill_version_not_supported(self):
        ...
        with tempfile.NamedTemporaryFile() as f:
            ...
            with self.assertRaisesRegex(ValueError, "supports dill >="):
                ...
                x2 = torch.load(
                    f, pickle_module=dill, encoding="utf-8", weights_only=False
                )
        ...
        class TensorSerializationSpoofer:
            ...
            def __reduce_ex__(self, proto):
                ...
                return (
                    ...
                    (
                        self.tensor.layout,
                        (invalid_indices, self.tensor._values(), self.tensor.size()),
                    ),
                )
            ...
            class LegacyCtorSerializationSpoofer:
                ...
                def __reduce_ex__(self, proto):
                    ...
                    return (
                        ...
                        (indices, self.tensor._values(), self.tensor.size()),
                    )

            with tempfile.NamedTemporaryFile() as f:
                sd = {
                    "spoofed_legacy_ctor": LegacyCtorSerializationSpoofer(x_legacy_ctor)
                }
                torch.save(sd, f)
    ...
    def _test_serialization_sparse_compressed_invalid(
        self, conversion, get_compressed_indices, get_plain_indices
    ):
        x = torch.zeros(3, 3)
        x[1][1] = 1
        x = conversion(x)
    ...
    def test_serialization_map_location(self):
        ...
        if torch.cuda.is_available():
            check_map_locations(
                gpu_0_map_locations, torch.float, torch.device("cuda", 0)
            )
            check_map_locations(
                gpu_last_map_locations,
                torch.float,
        ...
        if torch.xpu.is_available():
            check_map_locations(
                xpu_0_map_locations, torch.float, torch.device("xpu", 0)
            )
            check_map_locations(
                xpu_last_map_locations,
                torch.float,
    ...
    @unittest.skipIf(
        torch.cuda.is_available(), "Testing torch.load on CPU-only machine"
    )
    ...
    def test_serialization_filelike_stress(self):
        ...
        self._test_serialization_filelike(
            a, lambda x: FilelikeMock(x, has_readinto=False), "read() stress test"
        )
        self._test_serialization_filelike(
            a, lambda x: FilelikeMock(x, has_readinto=True), "readinto() stress test"
        )
    ...
    def test_load_unicode_error_msg(self):
        ...
        self.assertRaises(
            UnicodeDecodeError,
            lambda: torch.load(path, encoding="ascii", weights_only=False),
        )
    ...
    def test_save_different_dtype_unallocated(self):
        ...
        for device, dtype in product(
            devices, all_types_and_complex_and(torch.half, torch.bfloat16, torch.bool)
        ):
            ...
            for other_dtype in all_types_and_complex_and(
                ...
                s = torch.TypedStorage(
                    wrap_storage=a.storage().untyped(), dtype=other_dtype
                )
                save_load_check(a, s)
    ...
    def test_save_different_dtype_error(self):
        ...
        for device in devices:
            ...
            s_bytes = torch.TypedStorage(
                wrap_storage=a.storage().untyped(), dtype=torch.uint8
            )
    ...
    def test_skip_data_load(self):
        ...
        sd_zeroed = {
            ...
            "tt": TwoTensor(
                torch.zeros(2, device=t_device), torch.zeros(2, device=t_device)
            ),
        }
...
class TestBothSerialization(TestCase):
    ...
    def test_serialization_new_format_old_format_compat(self, device, weights_only):
        ...
        with (
            AlwaysWarnTypedStorageRemoval(True),
            warnings.catch_warnings(record=True) as w,
        ):
            with (
                tempfile.NamedTemporaryFile() as f_new,
                tempfile.NamedTemporaryFile() as f_old,
            ):
                test(f_new, f_old)
            self.assertTrue(
                len(w) == 0, msg=f"Expected no warnings but got {[str(x) for x in w]}"
            )
...
class TestSerialization(TestCase, SerializationMixin):
    ...
    def test_serialization_4gb_file(self):
        ...
        big_model = torch.nn.ModuleList(
            [
                torch.nn.Linear(1, int(1024 * 1024 * 1024) + 12, bias=False),
    ...
    def test_lr_scheduler_serialization(self):
        ...
        if "anneal_func" in lr_scheduler_state:
            self.assertFalse(
                hasattr(lr_scheduler_state["anneal_func"], "__self__")
            )  # check method is not bound
        else:
    ...
    def test_weights_only_assert(self):
        ...
        with BytesIOContext() as f:
            ...
            with self.assertRaisesRegex(
                pickle.UnpicklingError, "Unsupported global: GLOBAL print"
            ):
                torch.load(f, weights_only=True)
    ...
    def test_weights_only_safe_globals_build_with_slots(self, slots):
        obj_cls = (
            ...
            if slots == "all"
            else ClassThatUsesBuildInstructionSomeSlots
        )
    ...
    def test_weights_only_safe_globals_blocklist(self):
        ...
        error_msg = (
            f"unsupported GLOBAL {module}.execv whose module {module} is blocked"
        )
        with BytesIOContext() as f:
    ...
    def test_serialization_load_bom_data(self):
        ...
        #
        data_le_no_bom = (
            b"PK\x03\x04\x00\x00\x08\x08\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00"
        ...
        try:
            ...
            lstm_le_no_bom_little.load_state_dict(
                torch.load(buf_le_no_bom), strict=True
            )
            lstm_be_no_bom_little.load_state_dict(
                torch.load(buf_be_no_bom), strict=True
            )
        finally:
            set_default_load_endianness(current_load_endian)
        ...
        self.assertNotEqual(
            lstm_be_no_bom_little.state_dict(), lstm_be_bom.state_dict()
        )
    ...
    def test_serialization_warning_s390x(self):
        ...
        try:
            ...
            with self.assertWarnsRegex(
                UserWarning,
                "The default load endianness for checkpoints "
    ...
    def test_serialization_mmap_loading_options(self, weights_only, path_type):
        ...
        self.assertEqual(
            model_mmap_state_dict(input), model_non_mmap_state_dict(input.clone())
        )
    ...
    def test_serialization_dtype(self, dtype, weights_only):
        ...
        with tempfile.NamedTemporaryFile() as f:
            ...
            self.assertEqual(
                y["x"][:2].to(dtype=check_dtype), torch.tensor([val2, val1])
            )
    ...
    def test_filewriter_metadata_writing(self, filename):
        ...
        file_creation_func = (
            TemporaryFileName
            if filename
            else functools.partial(tempfile.NamedTemporaryFile, delete=False)
        )
    ...
    def test_skip_data_serialization(self, materialize_fake):
        ...
        if not materialize_fake:
            sd["st"] = st
            sd_expected["st"] = torch.sparse_coo_tensor(
                torch.zeros(2, 3), torch.zeros(3), (2, 4)
            )
        ...
        with BytesIOContext() as f:
            ...
            self.assertFalse(
                getattr(
                    torch.serialization._serialization_tls,
                    "materialize_fake_tensors",
                    False,
                )
            )
            self.assertFalse(
                getattr(torch.serialization._serialization_tls, "skip_data", False)
            )
    ...
    def test_skip_data_serialization_preserves_views(self, materialize_fake):
        ...
        with BytesIOContext() as f:
            ...
            sd_loaded = torch.load(f, weights_only=True)
            self.assertTrue(
                id(sd_loaded["t_view"].untyped_storage())
                == id(sd_loaded["t"].untyped_storage())
            )
            self.assertTrue(
                id(sd_loaded["t_slice"].untyped_storage())
                == id(sd_loaded["t"].untyped_storage())
            )
    ...
    def test_skip_data_serialization_error_cases(self):
        ...
        with self.assertRaisesRegex(
            RuntimeError,
            "Cannot serialize nested tensor under skip_data context manager",
        ):
            _save_load(nt)

        with self.assertWarnsRegex(
            UserWarning, "meta device under skip_data context manager is a no-op"
        ):
            _save_load(t)
    ...
    def test_weights_only_env_variables(self, force_weights_only):
        env_var = (
            ...
            else "TORCH_FORCE_NO_WEIGHTS_ONLY_LOAD"
        )
        args = (
    ...
    def test_crc32_options(self, compute_crc32, filename):
        ...
        file_creation_func = (
            TemporaryFileName if filename else tempfile.NamedTemporaryFile
        )
        sd = torch.nn.Linear(3, 5).state_dict()
    ...
    def test_get_unsafe_globals_in_checkpoint(self):
        ...
        with BytesIOContext() as f:
            ...
            try:
                ...
                unsafe_all_globals = (
                    torch.serialization.get_unsafe_globals_in_checkpoint(f)
                )
    ...
    @parametrize(
        "dtype", all_types_and_complex_and(torch.half, torch.bfloat16, torch.bool)
    )
    @parametrize("weights_only", [True, False])
    ...
    @unittest.skipIf(
        ...
        or torch.accelerator.current_accelerator().type == "mps",
    ...
    def test_use_pinned_memory_for_d2h(self):
        ...
        with patch("torch._C.PyTorchFileWriter.write_record", patched_write_record):
            with tempfile.NamedTemporaryFile() as f:
                with self.assertRaisesRegex(
                    RuntimeError, "Expected storage to be in pinned memory"
                ):
                    torch.save(sd, f)
    ...
    def test_has_format_version(self):
        ...
        with tempfile.NamedTemporaryFile() as f:
            ...
            f.seek(0)
            with torch.serialization._open_file_like(f, "rb") as opened_file:
                with torch.serialization._open_zipfile_reader(
                    opened_file
                ) as opened_zipfile:
    ...
    def test_storage_alignment(self):
        ...
        with tempfile.NamedTemporaryFile() as f:
            ...
            self.assertEqual(
                sd_fake["weight"].untyped_storage()._checkpoint_offset, 832
            )
        ...
        with tempfile.NamedTemporaryFile() as f:
            try:
                ...
                with FakeTensorMode():
                    sd_fake = torch.load(f)
                self.assertEqual(
                    sd_fake["weight"].untyped_storage()._checkpoint_offset, 20480
                )
                self.assertEqual(
                    sd_fake["bias"].untyped_storage()._checkpoint_offset, 24576
                )
                f.seek(0)
                sd_loaded = torch.load(f)
                self.assertEqual(sd_loaded, sd)
    ...
    def test_mmap_load_offset_calculation(self, path_type):
        ...
        try:
            ...
            with torch.device("meta"):
                model_mmap_state_dict = torch.nn.Sequential(
                    *[torch.nn.Linear(4, 4) for _ in range(20)]
                )
                model_non_mmap_state_dict = torch.nn.Sequential(
                    *[torch.nn.Linear(4, 4) for _ in range(20)]
                )
            ...
            inp = torch.randn(4, 4)
            self.assertEqual(
                model_mmap_state_dict(inp), model_non_mmap_state_dict(inp.clone())
            )
        finally:
            serialization_config.load.calculate_storage_offsets = (
                calculate_offsets_before
            )
    ...
    def test_safe_globals_for_weights_only(self):
        ...
        with tempfile.NamedTemporaryFile() as f:
            ...
            try:
                ...
                self.assertEqual(
                    set(torch.serialization.get_safe_globals()), expected_safe_globals
                )
                sd = torch.load(f, weights_only=True)
