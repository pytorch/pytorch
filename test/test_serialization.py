...
class SerializationMixin(object):
    ...
    def _test_serialization_assert(self, b, c):
        ...
        self.assertTrue(isinstance(c[4]._storage, torch.ByteStorage))
        c[0].fill_(10)
        ...
        views = c[7]
        self.assertEqual(views[0]._storage._cdata, views[1]._storage._cdata)
        self.assertEqual(views[0], views[2])
        self.assertNotEqual(views[0]._storage._cdata, views[2]._storage._cdata)

        rootview = c[8]
        self.assertEqual(rootview._storage.data_ptr(), c[0].data_ptr())
    ...
    def test_serialization_storage_slice(self):
        ...
        self.assertEqual(torch.tensor(s1._storage, dtype=s1.dtype)[0], 0)
        self.assertEqual(s2.dtype, torch.float32)
        self.assertEqual(torch.tensor(s2._storage, dtype=s1.dtype)[0], 0)
        self.assertEqual(s1._storage.data_ptr() + 4, s2._storage.data_ptr())
