...
from torch.testing._internal.common_device_type import largeTensorTest, onlyNativeDeviceTypes, dtypes, \
    ...
    TEST_WITH_ROCM, expectedFailureCUDA, onlyCUDAAndPRIVATEUSE1, dtypesIfCUDAAndPRIVATEUSE1
...
class TestPoolingNNDeviceType(NNTestCase):
    ...
    @dtypesIfCUDAAndPRIVATEUSE1(torch.float32, torch.float64, torch.bfloat16, torch.float16)
...
'''
    ...
    @gcIfJetson
    @dtypes(torch.float, torch.double)
    @dtypesIfCUDAAndPRIVATEUSE1(torch.half, torch.float, torch.double)
    ...
    @dtypesIfCUDAAndPRIVATEUSE1(*floating_types_and(torch.half, torch.bfloat16))
    @dtypes(torch.float)
