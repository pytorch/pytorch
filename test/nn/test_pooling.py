...
"""
    ...
    def test_max_pool_indices_corner_cases(self, device, dtype):
        ...
        if dtype is torch.float16:
            ...
            x[-1] = 1
            check_indices(x.reshape(1, 1, -1), ([1],), [[[N - 1]]], torch.nn.AdaptiveMaxPool1d)
            check_indices(x.reshape(1, 1, 1, -1), ([1, 1],), [[[[N - 1]]]], torch.nn.AdaptiveMaxPool2d)
        ...
        if dtype is torch.float32:
            ...
            check_indices(x.reshape(1, 1, -1), ([1],), [[[N - 1]]], torch.nn.AdaptiveMaxPool1d)
            check_indices(x.reshape(1, 1, 1, -1), ([1, 1],), [[[[N - 1]]]], torch.nn.AdaptiveMaxPool2d)
            check_indices(x.reshape(1, 1, 2, 1, N // 2), ([1, 1, 1],), [[[[[N - 1]]]]], torch.nn.AdaptiveMaxPool3d)
