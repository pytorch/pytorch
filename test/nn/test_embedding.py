...
class TestEmbeddingNNDeviceType(NNTestCase):
    ...
    def test_embedding_backward_atomic_accumulate_kernel(self, device, dtype):
        ...
        for i in range(num_embeddings):
            if i in used_indices_set:
                ...
                self.assertTrue(
                    embedding.weight.grad[i].abs().sum() > 0,
        ...
        for i in range(total_indices):
            idx = indices[i].item()
            expected_grad[idx] += grad_output[i]
        ...
        if dtype in [torch.half, torch.bfloat16]:
            ...
            atol = 5e-5
            rtol = 5e-5
        self.assertEqual(embedding.weight.grad, expected_grad, atol=atol, rtol=rtol)
    ...
    def test_embedding_backward_atomic_accumulate_kernel_with_padding_idx(
        ...
        self.assertTrue(
            embedding.weight.grad[padding_idx].abs().sum() == 0,
        ...
        for idx in [10, 20, 30]:
            self.assertTrue(
                embedding.weight.grad[idx].abs().sum() > 0,
