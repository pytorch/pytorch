...
class TestPrefixCopy(TestCase):
    ...
    def test_split_copy(self, device, dtype, num_chunks, num_prefixes, inference):
        with torch.inference_mode(inference):
            ...
            result = torch.ops.aten.split_with_sizes_copy.prefix_out(
                source, splits, prefixes, num_chunks, out=outputs
            )
    ...
    @parametrize("transform", ["eager", "functionalize", "compile"])
    def test_split_copy_mixed_dtype(self, device, transform):
        num_chunks = 4
        ...
        def copy(destinations, packed_source):
            torch.ops.aten.split_with_sizes_copy.prefix_out(
                packed_source, splits, [1, 2, 6], num_chunks, out=destinations
            )
            return tuple(destinations)

        if transform == "functionalize":
            copy = torch.func.functionalize(copy)
        elif transform == "compile":
            copy = torch.compile(copy, fullgraph=True, backend="aot_eager")
        result = copy(outputs, source)

        self.assertEqual(outputs, expected, atol=0, rtol=0)
        self.assertEqual(result, tuple(expected), atol=0, rtol=0)
    ...
    def test_split_copy_empty(self, device, all_empty):
        ...
        torch.ops.aten.split_with_sizes_copy.prefix_out(
            source, [0, source.numel() // 4], [128, 1], 4, out=outputs
        )
        self.assertEqual(outputs[1], source)
        torch.ops.aten.split_with_sizes_copy.prefix_out(source[:0], [], [], 4, out=[])
    ...
    def test_chunk_cat(
        ...
        result = torch.ops.aten._chunk_cat.prefix_out(
            tensors, [0, 1, 2], num_chunks, out=output
        )
    ...
    @parametrize("inference", [False, True])
    def test_chunk_cat_empty_prefix(self, device, num_leading_dims, inference):
        with torch.inference_mode(inference):
            ...
            torch.ops.aten._chunk_cat.prefix_out(
                [tensor[:0], tensor], [num_leading_dims] * 2, 4, out=output
            )
            self.assertEqual(output, expected, atol=0, rtol=0)
    ...
    def test_chunk_cat_invalid(self, device, invalid, match):
        ...
        if invalid == "chunks":
            ...
            output = output.t()
        with self.assertRaisesRegex(RuntimeError, match):
            torch.ops.aten._chunk_cat.prefix_out(tensors, dims, num_chunks, out=output)
    ...
    def test_view_flags(self, device, operation, view, flagged_output):
        ...
        if operation == "split" and flagged_output:
            with self.assertRaisesRegex(RuntimeError, "mutable TensorLists"):
                torch.ops.aten.split_with_sizes_copy.prefix_out(
                    packed, [tensor.numel() // 4], [2], 4, out=[output]
                )
            return
        if operation == "split":
            torch.ops.aten.split_with_sizes_copy.prefix_out(
            ...
            torch.ops.aten._chunk_cat.prefix_out([tensor], [1], 4, out=output)
        self.assertEqual(output, expected, atol=0, rtol=0)
    ...
    def test_functionalize(self, device, operation):
        ...
        def copy(destination):
            if operation == "split":
                torch.ops.aten.split_with_sizes_copy.prefix_out(
                    packed, [tensor.numel() // 4], [2], 4, out=[destination]
                )
            else:
                torch.ops.aten._chunk_cat.prefix_out([tensor], [1], 4, out=destination)
            return destination
    ...
    @parametrize(
        "operation,empty", [("split", False), ("split", True), ("chunk", False)]
    )
    @parametrize("requires_grad_arg", ["input", "output"])
    def test_out_autograd(self, device, operation, empty, requires_grad_arg):
        tensor = make_tensor(
            (2, 0 if empty else 8, 3), device=device, dtype=torch.float32
        )
        packed = torch.stack([t.flatten() for t in torch.chunk(tensor, 4, dim=1)])
        source = packed if operation == "split" else tensor
        expected = tensor if operation == "split" else packed
        source.requires_grad_(requires_grad_arg == "input")
        output = torch.empty_like(expected, requires_grad=requires_grad_arg == "output")

        def copy():
            if operation == "split":
                torch.ops.aten.split_with_sizes_copy.prefix_out(
                    source, [source.numel() // 4], [2], 4, out=[output]
                )
            else:
                torch.ops.aten._chunk_cat.prefix_out([source], [1], 4, out=output)

        with self.assertRaisesRegex(RuntimeError, "automatic differentiation"):
            copy()
        with torch.no_grad():
            copy()
        self.assertEqual(output, expected, atol=0, rtol=0)
    ...
    def test_inference_output_outside_inference_mode(self, device, operation, empty):
        tensor = make_tensor(
            (2, 0 if empty else 8, 3), device=device, dtype=torch.float32
        )
        packed = torch.stack([t.flatten() for t in torch.chunk(tensor, 4, dim=1)])
        with torch.inference_mode():
            output = torch.empty_like(tensor if operation == "split" else packed)
        with self.assertRaisesRegex(
    ...
    def test_fake_tensor(self, device):
        with FakeTensorMode():
            tensor = torch.empty((2, 8, 3), device=device)
            packed = torch.empty((4, 12), device=device)
            result = torch.ops.aten.split_with_sizes_copy.prefix_out(
                packed, [12], [2], 4, out=[tensor]
            )
            self.assertIsNone(result)
            result = torch.ops.aten._chunk_cat.prefix_out([tensor], [1], 4, out=packed)
            self.assertIs(result, packed)
            self.assertEqual(tensor.shape, (2, 8, 3))
            self.assertEqual(packed.shape, (4, 12))
    ...
    def test_device_and_stream(self, devices):
        ...
        with torch.cuda.stream(stream):
            ...
            with torch.cuda.device(devices[1]):
                torch.ops.aten._chunk_cat.prefix_out([tensor], [1], 4, out=packed)
                torch.ops.aten.split_with_sizes_copy.prefix_out(
                    packed, [12], [2], 4, out=[output]
                )
                self.assertEqual(
    ...
    def test_cuda_graph(self, device):
        ...
        torch.ops.aten._chunk_cat.prefix_out(tensors, [0, 1], num_chunks, out=packed)
        torch.ops.aten.split_with_sizes_copy.prefix_out(
            packed, splits, [1, 128], num_chunks, out=outputs
        )
        graph = torch.cuda.CUDAGraph()
        with torch.cuda.graph(graph):
            torch.ops.aten._chunk_cat.prefix_out(
                tensors, [0, 1], num_chunks, out=packed
            )
            torch.ops.aten.split_with_sizes_copy.prefix_out(
                packed, splits, [1, 128], num_chunks, out=outputs
            )
