...
class TestForeach(TestCase):
    ...
    def test_addcmul(self, device, dtype):
        if dtype in [torch.half, torch.bool] and self.device_type == 'cpu':
            with self.assertRaisesRegex(RuntimeError, "not implemented for"):
                self._test_pointwise_op(device, dtype, torch._foreach_addcmul, torch._foreach_addcmul_, torch.addcmul)
            return

        if dtype in [torch.bfloat16] and self.device_type == 'cpu':
            with self.assertRaisesRegex(RuntimeError, "not implemented for"):
                self._test_pointwise_op(device, dtype, torch._foreach_addcmul, torch._foreach_addcmul_, torch.addcmul)
            return

        torch_op = torch.addcmul
        foreach_op = torch._foreach_addcmul
        foreach_op_ = torch._foreach_addcmul_
        ...
        for N in N_values:
            ...
            for vals in [values[0], values]:
                ...
                if not isinstance(vals, list):
                    expected = [torch_op(tensors[i],tensors1[i], tensors2[i], value=values[0]) for i in range(N)]
                else:
                    expected = [torch_op(tensors[i], tensors1[i], tensors2[i], value=values[i]) for i in range(N)]
