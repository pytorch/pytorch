...
class TestForeach(TestCase):
    ...
    bin_ops_sl = [
        (torch._foreach_add_sl, torch._foreach_add_sl_, torch.add),
        (torch._foreach_sub_sl, torch._foreach_sub_sl_, torch.sub),
        (torch._foreach_mul_sl, torch._foreach_mul_sl_, torch.mul),
        (torch._foreach_div_sl, torch._foreach_div_sl_, torch.div),
    ]
    ...
    # Pointwise ops
    #
    @dtypes(*torch.testing.get_all_dtypes(include_bfloat16=False, include_bool=False, include_complex=True))
    ...
    def test_int_scalar(self, device, dtype):
        for N in N_values:
            for foreach_bin_op, foreach_bin_op_, torch_bin_op in self.bin_ops:
                ...
                # TODO[type promotion]
                # In case of CUDA, passed scalar will be handled as a float in foraech APIs, so there wont be a runtime error.
    ...
    def test_int_scalarlist(self, device, dtype):
        for N in N_values:
            for foreach_bin_op, foreach_bin_op_, torch_bin_op in self.bin_ops_sl:
                tensors = self._get_test_data(device, dtype, N)
                ...
                if dtype == torch.bool:
                    ...
                    with self.assertRaisesRegex(RuntimeError, "result type Long can't be cast to the desired output type"):
                        foreach_bin_op_(tensors, scalars)
                    return
                ...
                # test in-place
                if foreach_bin_op_ == torch._foreach_div_sl_ and dtype in torch.testing.integral_types() and self.device_type == "cpu":
                    with self.assertRaisesRegex(RuntimeError, "can't be cast to the desired output type"):
        ...
        for N in N_values:
            for foreach_bin_op, foreach_bin_op_, torch_bin_op in self.bin_ops:
                ...
                if foreach_bin_op == torch._foreach_sub and self.device_type == "cpu":
                    ...
                    if dtype in torch.testing.integral_types():
                        if foreach_bin_op == torch._foreach_div:
    ...
    @skipCUDAIfRocm
    @dtypes(*torch.testing.get_all_dtypes())
    def atest_bool_scalarlist(self, device, dtype):
        for N in N_values:
            for foreach_bin_op, foreach_bin_op_, torch_bin_op in self.bin_ops_sl:
                tensors = self._get_test_data(device, dtype, N)
                ...
                if dtype == torch.bool:
                    if self.device_type == 'cuda':
                        ...
                        if foreach_bin_op == torch._foreach_sub_sl:
    ...
    def test_add_list(self, device, dtype):
        for dtype2 in torch.testing.get_all_dtypes():
            for N in N_values:
                tensors1 = self._get_test_data(device, dtype, N)
                tensors2 = self._get_test_data(device, dtype2, N)
    ...
    def test_sub_list(self, device, dtype):
        for dtype2 in torch.testing.get_all_dtypes():
            for N in N_values:
                tensors1 = self._get_test_data(device, dtype, N)
                tensors2 = self._get_test_data(device, dtype2, N)
                if dtype == torch.bool or dtype2 == torch.bool:
                    # TODO: add alpha case
                    with self.assertRaisesRegex(RuntimeError, "Subtraction, the `-` operator,"):
    ...
    def test_add_list_different_sizes(self, device, dtype):
        for dtype2 in torch.testing.get_all_dtypes():
            ...
            tensors2 = [torch.ones(10 + n, 10 + n, device=device, dtype=dtype2) for n in range(10)]
...
1. Refactor
    - reuse methods
    - bin ops into one list 
    - use a tuple instead of forloop  @dtypes(*tuple(itertools.combinations_with_replacement(torch.testing.get_all_dtypes(), 2)))
2. add/sub have alphas. are those being tested? Lists and single cases
3. what to do in case of inplace ops? 
4. int16 + int16 -> should be checked in ForeachUtils.h


All test must use ALL types
Enable complex
Everywhere where we throw an exception, have a torch throw too

!!!! ADD BOOL SCALAR LIST TEST
'''