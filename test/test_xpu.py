...
"""
    ...
    def test_stream_compatibility(self):
        ...
        self.assertEqual(
            torch.accelerator.current_stream().stream_handle, s1.sycl_queue
        )
        self.assertEqual(
            torch.accelerator.current_stream().stream_handle, s1.stream_handle
        )
        torch.accelerator.set_stream(s2)
        self.assertEqual(torch.accelerator.current_stream().stream_id, s2.stream_id)
        self.assertEqual(
            torch.accelerator.current_stream().stream_handle, s2.sycl_queue
        )
        self.assertEqual(
            torch.accelerator.current_stream().stream_handle, s2.stream_handle
        )
