...
class TestFP8Matmul(TestCase):
    ...
    def test_blockwise_mxfloat8_error_messages(self, device) -> None:
        ...
        with self.assertRaisesRegex(
            ...
            incorrect_size_a = torch.ones(
                expected_a_size - 1, device=device, dtype=torch.float8_e8m0fnu
            )
            correct_size_b = torch.ones(
                expected_b_size, device=device, dtype=torch.float8_e8m0fnu
            )
            torch._scaled_mm(
                x_fp8,
                y_fp8,
        ...
        with self.assertRaisesRegex(
            ...
            correct_size_a = torch.ones(
                expected_a_size, device=device, dtype=torch.float8_e8m0fnu
            )
            incorrect_size_b = torch.ones(
                expected_b_size + 1, device=device, dtype=torch.float8_e8m0fnu
            )
            torch._scaled_mm(
                x_fp8,
                y_fp8,
        ...
        with self.assertRaisesRegex(
            ...
            non_contiguous_a = torch.ones(
                expected_a_size * 2, device=device, dtype=torch.float8_e8m0fnu
            )[::2]
            contiguous_b = torch.ones(
                expected_b_size, device=device, dtype=torch.float8_e8m0fnu
            )
            torch._scaled_mm(
                x_fp8,
    ...
    def test_grouped_gemm_2d_3d(self, fast_accum, strided):
        ...
        scale_a = torch.arange(n_groups * m, device="xpu", dtype=torch.float32)
        scale_b = torch.ones(n_groups * n, device="xpu", dtype=torch.float32).view(n_groups, n)
    ...
    def test_grouped_gemm_3d_3d(self, fast_accum, strided):
        ...
        self.assertTrue(a.is_contiguous() is not strided)
        self.assertTrue(b.is_contiguous() is not strided)
        scale_a = torch.ones(n_groups * m, device="xpu", dtype=torch.float32).view(n_groups, m)
        scale_b = torch.ones(n_groups * n, device="xpu", dtype=torch.float32).view(n_groups, n)
    ...
    def test_grouped_gemm_3d_2d(self, fast_accum, strided):
        ...
        self.assertTrue(a.is_contiguous() is not strided)
        self.assertTrue(b.is_contiguous() is not strided)
        scale_a = torch.arange(n_groups * m, device="xpu", dtype=torch.float32).view(n_groups, m)
        scale_b = torch.arange(n_groups * n, device="xpu", dtype=torch.float32)
