...
Optimization 1 — flat-cat path:
  When foreach=None (auto) and device is CPU and ALL tensors have numel ≤ 512,
  ...
  Note: _group_tensors_by_device_and_dtype groups by (device, dtype) not by
  size, so a group may contain mixed-size tensors; all must be checked.
...
def test_mixed_numel_group_uses_foreach_not_flat():
    """If a dtype group contains both small and large tensors, flat path must not fire.

    _group_tensors_by_device_and_dtype groups by (device, dtype), not by size.
    A single group can contain tensors of any numel. Checking only device_tensors[0]
    would incorrectly apply the flat-cat path to a huge concatenation.
    The fix: check ALL tensors with all(t.numel() <= 512 ...).
    """
    # Mix of 512 and 1024 — all float32 on CPU → same group; flat path must not fire.
    small = _make_tensors(50, 512)
    large = _make_tensors(50, 1024)
    mixed = small + large
    result = _get_total_norm(mixed, norm_type=2.0, foreach=None)
    expected = _reference_norm(mixed, 2.0)
    assert torch.allclose(result, expected, atol=1e-5, rtol=1e-5)
