...
graph(%Ra, %Rb):
  return (%Ra)""",
    ...
    def test_shape_analysis_broadcast(self):
        ...
        FileCheck().check(
            "Double(4, 3, 8, 5, strides=[120, 40, 5, 1], device=cpu)"
        ).run(str(graph))
        ...
        with enable_profiling_mode_for_profiling_tests():
            ...
            for training in (True, False):
                ...
                for requires_grad in (True, False):
                    ...
                    if requires_grad:
                        FileCheck().check("aten::bernoulli_").run(
    ...
    def test_profiling_merge(self):
        ...
        with enable_profiling_mode_for_profiling_tests():
            with num_profiled_runs(2):
                ...
                FileCheck().check(
                    "profiled_type=Double(*, 2, strides=[2, 1], requires_grad=0, device=cpu"
                ).run(graph_str)
                FileCheck().check_not(
                    "profiled_type=Double(1, 2, strides=[2, 1], requires_grad=0, device=cpu"
                ).run(graph_str)
...
def foo(x):
    ...
    @unittest.skipIf(
        IS_WINDOWS and sys.version_info >= (3, 8), "TODO: need to fix the test case"
    )
    ...
    def test_interpreter_fuzz(self):
        ...
        def gen_code():
            ...
            for i in range(50):
                ...
                if "VAR" in template:
                    src_lines.append(
                        "  v{} = {}".format(n_variables, select_expr_or_var())
                    )
                    n_variables += 1
                else:
                    exprs.append(
            ...
            src_lines.append(
                "  return ({})\n".format(
                    "".join("v{},".format(i) for i in range(n_variables))
                )
            )
            return "\n".join(src_lines)
    ...
    def test_file_line_error(self):
        ...
        with self.assertRaisesRegex(
            RuntimeError, 'test_jit.py", line {}'.format(lineno + 1)
        ):
    ...
    def test_file_line_error_class_defn(self):
        ...
        _, lineno = inspect.getsourcelines(FooBar)
        with self.assertRaisesRegex(
            RuntimeError, 'test_jit.py", line {}'.format(lineno + 2)
        ):
...
def foo(xyz):
    ...
    def test_serialized_source_ranges(self):
        ...
        with self.assertRaisesRegex(
            RuntimeError, 'test_jit.py", line {}'.format(lineno + 3)
        ):
    ...
    def test_serialized_source_ranges2(self):
        ...
        with self.assertRaisesRegex(
            torch.jit.Error, 'test_jit.py", line {}'.format(lineno + 3)
        ):
            ft = FooTest2()
            loaded = self.getExportImportCopy(ft)
            loaded()
...
a")
    ...
    def test_unbind(self):
        with enable_profiling_mode_for_profiling_tests():
            ...
            with disable_autodiff_subgraph_inlining():
                ...
                self.assertAutodiffNode(
                    func.graph_for(x, y), True, ["aten::unbind"], []
                )
    ...
    def test_meshgrid(self):
        with enable_profiling_mode_for_profiling_tests():
            ...
            with disable_autodiff_subgraph_inlining():
                ...
                if GRAPH_EXECUTOR != ProfilingMode.SIMPLE:
                    self.assertAutodiffNode(
                        func.graph_for(inputs), True, ["aten::meshgrid"], []
                    )
                ...
                if res_python != res_script:
                    ...
                    if type(res_python) == type(res_script):
                        ...
                        if (
                            ...
                            continue
                    msg = "Failed on {func_name} with inputs {a} {b}. Python: {res_python}, Script: {res_script}".format(
                        func_name=func_name,
                        a=a,
                        b=b,
    ...
    def _test_tensor_number_math(self, device="cpu"):
        ...
        def test(op, tensor, const, swap_args, template=template):
            ...
            execWrapper(code, globals(), scope)
            cu = torch.jit.CompilationUnit(code)
            message = "with code `{} {} {}` and t={}".format(
                args[0], op, args[1], tensor
            )
            res1 = cu.func(tensor)
    ...
    def test_tensor_as_tensor_shape_prop(self):
        ...
        for op in ops:
            for inp, expect in zip(inputs, expected_shape):
                ...
                FileCheck().check(expect).check(
                    "aten::{tensor_op}".format(tensor_op=op)
                ).run(cu.func.graph)
...
dedent """
    ...
    def test_is_isnot(self):
        ...
        def test(op, args):
            ...
            self.assertEqual(
                cu.func(),
                scope["func"](),
                msg="Failed with op: {}, lhs: {}, rhs: {}".format(op, args[0], args[1]),
            )
    ...
    def test_isinstance(self):
        ...
        def test(inp, typ, type_hint):
            ...
            cu = torch.jit.CompilationUnit(code)
            self.assertEqual(
                cu.func(inp), scope["func"](inp), msg="Failed with typ: {}".format(typ)
            )
                ...
                for tensor_type in dtypes:
                    ...
                    if not tensor_type.is_floating_point or (
                        ...
                        if op in ["mean", "softmax", "log_softmax"]:
                            continue
                    return_line = "torch.tensor({}, dtype={}).{}({}dtype={})".format(
                        tensor_data, tensor_type, op, str_args, dtype
                    )
    ...
    def test_if_different_type(self):
        with self.assertRaisesRegex(
            RuntimeError,
            "Type mismatch: c0 is set to type int "
        ...
        with self.assertRaisesRegex(
            RuntimeError, "Variable 'c0' previously has type float"
        ):
    ...
    def test_import_constants_not_specialized(self):
        ...
        FileCheck().check(
            "Double(1, 3, strides=[3, 1], requires_grad=0, device=cpu)"
    ...
    def test_index_select_shape_prop(self):
        ...
        FileCheck().check(
            "Double(2, 4, strides=[4, 1], requires_grad=0, device=cpu)"
        ).run(str(foo.graph))
    ...
    def test_torch_ignore_conversion_to_none(self):
        ...
        with self.assertRaisesRegexWithHighlight(
            RuntimeError, "expected value of type Tensor", "self.ignored"
        ):
    ...
    def test_multiline_annot_ast_py3_fn(self):
        ...
        for pair in self.type_input_return_pairs():
            ...
            self.assertEqual(
                str(returns[0].type), "Tuple[{}, {}]".format(pair[1], pair[1])
            )
    ...
    @unittest.skipIf(
        sys.version_info < (3, 7, 0), "defaults keyword added in Python 3.8"
    )
...
def add_nn_functional_test(
    ...
    def do_test(self, name=name, args=args, test_name=test_name, check_ad=check_ad):
        ...
        if not no_grad:
            output_variable = getattr(F, name)(
                self_variable, *args_variable, **kwargs_variable
            )
        ...
        should_autodiff_node, autodiff_nodes, fusible_nodes = normalize_check_ad(
            check_ad, name
        )
