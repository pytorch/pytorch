...
void get_autograd_operator_from_registry_and_execute() {
  ...
  auto& op = ops.front();
  AT_ASSERT(op->schema().name() == "custom::op_with_autograd");
  ...
  AT_ASSERT(0 == stack.size());

  AT_ASSERT(output.allclose(x + 2*y + x*y));
  auto go = torch::ones({}, torch::requires_grad());
  output.sum().backward(go, false, true);

  AT_ASSERT(torch::allclose(x.grad(), y + torch::ones({5,5})));
  AT_ASSERT(torch::allclose(y.grad(), x + torch::ones({5,5})*2));
}
