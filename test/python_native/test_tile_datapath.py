...
class TestTileDatapath(TestCase):
    ...
    def test_rejects_a_partial_warp(self):
        # Lane shuffles require threads_per_row=1 or whole warps.
        with self.assertRaises(ValueError):
            self._tm(N=512, itemsize=4, threads_per_row=48, loads=1)

    def test_exact_is_derived_and_overridable(self):
        # Exact tiles need no load predication; batched tiles retain bounds.
        covering = self._tm(
            N=256, itemsize=4, threads_per_row=32, loads=2
        )  # 4 * 2 * 32 == 256
        self.assertTrue(covering.exact)
        ragged = self._tm(N=252, itemsize=4, threads_per_row=32, loads=2)
        self.assertFalse(ragged.exact)
        self.assertFalse(
            self._tm(N=256, itemsize=4, threads_per_row=32, loads=2, exact=False).exact,
    ...
    def test_vec_override_is_what_an_order_needs(self):
        ...
        derived = self._tm(N=252, itemsize=4, threads_per_row=32, loads=2)
        self.assertEqual(derived.vec, math.gcd(252, 4))
        fixed = self._tm(
            N=252, itemsize=4, threads_per_row=32, loads=2, vec=4, exact=False
        )
    ...
    def test_strides_are_the_only_difference_between_the_two_orders(self):
        ...
        kw = dict(N=1024, itemsize=4, threads_per_row=128, loads=2)
    ...
    def test_align_bytes_tracks_the_load_width(self):
        ...
        wide = self._tm(N=1024, itemsize=4, threads_per_row=32, loads=1)
        self.assertTrue(wide.wide_ok)
        self.assertEqual(wide.align_bytes(4), wide.vec * 4)

        padded = self._tm(
            N=253, itemsize=4, threads_per_row=32, loads=2, vec=4, exact=False
        )
