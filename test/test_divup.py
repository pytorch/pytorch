...
from torch.testing._internal.common_utils import make_tensor, run_tests, TestCase
...
class TestDivUp(TestCase):
    ...
    @onlyCPU
    def test_divup_basic(self, device):
        """Test basic divup operation."""
        a = make_tensor(
            (10, 10), dtype=torch.float16, device=device, low=None, high=None
        )
        b = (
            make_tensor(
                (10, 10), dtype=torch.float16, device=device, low=None, high=None
    ...
    def test_divup_edge_cases(self, device, dtype):
        ...
        expected = torch.tensor(
            [float("inf"), -float("inf"), 1.0], dtype=dtype, device=device
        )
        self.assertEqual(result, expected)
