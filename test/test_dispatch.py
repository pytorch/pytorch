...
class TestDispatch(TestCase):
    ...
    def run_ops(self, name, ops, ctor_order=None, dtor_order=None,
        ...
        def check_invariants(actual_provenance):
            ...
            expected_state, expected_table, expected_provenance = results.setdefault(
                ...
                (actual_state, actual_table, actual_provenance)
            )
    ...
    def commute(self, name, ops, ctor_order=None, expect_raises=False):
        ...
        # Return the "full" state and computed dispatch table after
        ...
        return results[frozenset(range(len(ops)))][:-1]

    def test_def(self):
        state, _ = self.commute("foo", [
...
catchall: default_def_name_t_t :: (Tensor _0) -> (Tensor _0) [ boxed unboxed ]
''')
        # computed dispatch table is too big to use assertExpectedInline,
        # so we only assert on a few entries we're interested in.
        self.assertTrue('CPU: impl_t_t [kernel]' in table)
        self.assertTrue('CUDA: default_def_name_t_t [catch all]' in table)
        self.assertTrue('XLA: impl_t_t [kernel]' in table)
        self.assertTrue('AutogradOther: impl_t_t [autograd kernel]' in table)
        self.assertTrue('AutogradCPU: impl_t_t [kernel]' in table)
        self.assertTrue('AutogradCUDA: impl_t_t [autograd kernel]' in table)
        self.assertTrue('AutogradXLA: impl_t_t [autograd kernel]' in table)
...
''')
    ...
    def test_multiple_def_schema_mismatch(self):
        ...
        self.assertExpectedInline(
            self.commute("foo", ops, ctor_order=(0, 1), expect_raises=True)[0],
        ...
        self.assertExpectedInline(
            self.commute("foo", ops, ctor_order=(1, 0), expect_raises=True)[0],
    ...
    def test_multiple_def_alias_defaulting(self):
        ...
        state, _ = self.commute("foo", ops, ctor_order=(0, 1))
        self.assertExpectedInline(
            state,
            '''\
name: test::foo
...
'''
    ...
    def test_overwrite_catchall(self):
        ...
        # Not commutative
        self.assertExpectedInline(
            self.commute("foo", ops, ctor_order=(0, 1))[0],
            '''\
name: test::foo
schema: (none)
