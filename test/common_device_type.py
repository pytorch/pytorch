...
from common_device_type_base import DeviceTypeTestBase
from common_utils import TEST_WITH_ROCM, TEST_MKL, \
...
#
#   (1) Create a file in this directory called "device_type_<name>.py"
#   (2) Create a new "TestBase" extending DeviceTypeTestBase in that file.
#       See CPUTestBase and CUDATestBase below for examples.
#   (3) Implement a function get_test_base() in your file that returns
#       this test base.
...
# Finds other device types
torch_test_dir = os.path.dirname(os.path.abspath(__file__))
torch_test_files = [file for file in os.listdir(torch_test_dir) if os.path.isfile(os.path.join(torch_test_dir, file))]
regex = re.compile('^device_type_(.+).py$')
device_type_files = []
for test_file in torch_test_files:
    result = regex.match(test_file)
    if result:
        device_type_name = result.group(1)
        module_name = result.group(0).split('.')[0]
        device_type_module = __import__(module_name, globals(), locals(), [], 0)
        device_type_test_bases.append(device_type_module.get_test_base())
...
# precisions (or are working with multiple dtypes) in your test then they should
# be specified explicitly and computed using self.precision (e.g.
