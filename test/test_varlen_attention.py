...
VarlenShape = namedtuple('VarlenShape', ['batch_size', 'max_seq_len', 'embed_dim', 'num_heads'])
...
def create_variable_length_batch(shape: VarlenShape, device: torch.device, dtype: torch.dtype):
    seq_lengths = []
    ...
    x_padded = torch.zeros(shape.batch_size, max_len, shape.embed_dim, device=device, dtype=dtype)
...
class TestVarlenAttention(NNTestCase):
    ...
    def test_basic_functionality(self, device, dtype):
        ...
        attention_block = AttentionBlock(shape.embed_dim, shape.num_heads, device, dtype)

        total_tokens = shape.batch_size * shape.max_seq_len
        x_packed = torch.randn(total_tokens, shape.embed_dim, device=device, dtype=dtype)
        cu_seq = torch.tensor([0, shape.max_seq_len, total_tokens], device=device, dtype=torch.int32)

        output = attention_block.forward_varlen(x_packed, cu_seq, shape.max_seq_len, is_causal=False)
    ...
    def test_varlen_vs_sdpa(self, device, dtype, is_causal):
        ...
        shape = VarlenShape(batch_size=8, max_seq_len=2048, embed_dim=1024, num_heads=16)
        ...
        sdpa_output = attention_block.forward_sdpa(variable_length_batch_data['x_padded'], is_causal=is_causal)
