...
class TestGuardSerialization(TestGuardSerializationBase):
    ...
    def test_reducer_handles_an_empty_cell_reached_directly(self):
        ...
        empty = [
            cell
            for cell in EMPTY_CELL_WRAPPED.__closure__
            if not self._cell_has_contents(cell)
        ]
    ...
    def test_reduce_handles_an_empty_closure_cell(self):
        ...
        empty = [
            cell for cell in wrapped.__closure__ if not self._cell_has_contents(cell)
        ]
        self.assertEqual(len(empty), 1)
        cell = wrapped.__closure__[0]
        buf = io.BytesIO()
        GuardsStatePickler({id(wrapped): wrapped, id(cell): cell}, {}, {}, buf).dump(
            {"fn": wrapped}
        )
        self.assertGreater(len(buf.getvalue()), 0)
    ...
    def test_fqn_mismatched_function_keeps_a_shared_closure_cell_shared(self):
        ...
        buf = io.BytesIO()
        pickler = GuardsStatePickler({id(a): a, id(b): b, id(cell): cell}, {}, {}, buf)
    ...
    def test_fqn_mismatched_function_preserves_guarded_globals(self):
        ...
        try:
            ...
            loaded = torch._dynamo.package.load_guard_manager(
                guards_state, self._cached_f_code, globals()
            )
    ...
    def test_tensor_match_does_not_serialize_fake_mode_caches(self):
        def f(x):
            return x + 1

        self._test_serialization("TENSOR_MATCH", f, torch.randn(2))
        self.assertNotIn(b"WeakIdKeyDictionary", self._cached_guards_state)
        self.assertNotIn(b"WeakValueDictionary", self._cached_guards_state)
