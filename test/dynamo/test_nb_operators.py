...
class TestNbSub(torch._dynamo.test_case.TestCase):
    ...
    def test_isub_tensor_and_tensor(self):
        def fn(x, y):
            x -= y
            return x
    ...
    def test_isub_tensor_preserves_identity(self):
        def fn():
            d = torch.ones(2, 2)
            d_copy = d
            d -= torch.ones(2, 2)
    ...
    def test_sub_tensor_and_scalar(self):
        def fn(x):
            return x - 3, 3 - x, x - 2.5, 2.5 - x

        x = torch.randn(4)
        opt_fn = torch.compile(fn, backend="eager", fullgraph=True)
        self.assertEqual(opt_fn(x), fn(x))

    def test_sub_tensor_and_symnode(self):
        def fn(x):
            s = x.shape[0]
            return x - s, s - x
    ...
    def test_isub_tensor_and_scalar(self):
        def fn(x):
            x -= 3
            return x
    ...
    def test_sub_tensor_broadcast(self):
        def fn(x, y):
            return x - y
