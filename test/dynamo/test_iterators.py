...
class TestIterators(torch._dynamo.test_case.TestCase):
    ...
    def test_list_reverseiterator_not_a_list_iterator(self):
        ...
        self.assertFalse(
            issubclass(ListReverseIteratorVariable, ListIteratorVariable)
        )
    ...
    def test_list_reverseiterator_arg_guards(self):
        ...
        # First call
        self.assertEqual(
            opt_fn(reversed([1, 2, 3]), torch.tensor(10)), torch.tensor(16)
        )
        # Same length, different values
        self.assertEqual(
            opt_fn(reversed([4, 5, 6]), torch.tensor(10)), torch.tensor(25)
        )
        # Different length (triggers recompile)
        self.assertEqual(
            opt_fn(reversed([1, 2]), torch.tensor(10)), torch.tensor(13)
        )
