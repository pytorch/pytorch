...
class DecoratorTests(torch._dynamo.test_case.TestCase):
    ...
    def test_nested_disable_decorator(self):
        ...
        @torch.compile(backend=cnts, error_on_graph_break=True)
        def fn3(x):
            return fn2(x)
    ...
    def test_error_on_graph_break_nested(self):
        ...
        @torch.compile(backend=cnts, error_on_graph_break=False)
        def f7(x):
            x = x + 1
            return inner_f7(x)
    ...
    def test_nested_compile_error_on_graph_break(self):
        ...
        @torch.compile(backend="eager", error_on_graph_break=False)
        def f1(x):
            return inner_f1(x)
