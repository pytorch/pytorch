...
class AOTAutogradCacheTests(InductorTestCase):
    ...
    def test_compiled_autograd_bypass(self):
        ...
        self.assertEqual(
            ...
            1,  # CA graph is not serializable yet
    ...
    @parametrize("dtype", (torch.float32, torch.bfloat16))
    @parametrize("dynamic", (False, True))
    def test_compiled_autograd_cache_hot_load(self, device, dtype, dynamic):
        """
        Verify that we can populate and hot load functions from the cache.
        There is currently still a bypass from compiled autograd.
        """
        if device == GPU_TYPE and not HAS_GPU:
            raise unittest.SkipTest(f"requires {GPU_TYPE}")
        if device == "cuda" and dtype == torch.bfloat16 and not SM80OrLater:
