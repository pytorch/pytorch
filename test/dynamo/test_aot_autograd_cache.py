...
class AOTAutogradCachePicklerTests(torch._dynamo.test_case.TestCase):
    ...
    def test_sdpa_backend_state(self):
        ...
        try:
            default_value = torch.backends.cuda.preferred_rocm_fa_library("default")
            default_library = self.gen_cache_key(fn, config)
            aotriton_value = torch.backends.cuda.preferred_rocm_fa_library("aotriton")
            aotriton_library = self.gen_cache_key(fn, config)
        finally:
            torch.backends.cuda.preferred_rocm_fa_library(old_library)
        # On ROCm the getter normalizes Default to AOTriton, so the two settings
        # collapse to one value; the key must differ iff the value differs.
        if default_value != aotriton_value:
            self.assertNotEqual(default_library, aotriton_library)
        else:
            self.assertEqual(default_library, aotriton_library)
