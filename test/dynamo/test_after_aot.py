...
class TestAfterAotDeviceDecoupling(torch._dynamo.test_case.TestCase):
    ...
    def test_minifier_env_variables_npu_mapping(self):
        with patch.object(after_aot, "_infer_repro_device_type", return_value="npu"):
            with patch.object(
                torch,
                "npu",
                SimpleNamespace(device_count=lambda: 8),
                create=True,
            ):
                self.assertEqual(
                    after_aot._minifier_env_variables([]),
            ...
            with patch.object(
                torch,
                "npu",
                SimpleNamespace(device_count=lambda: 1),
                create=True,
            ):
                self.assertEqual(
                    after_aot._minifier_env_variables([]),
            ...
            # missing / failing device module degrades to device 0
            with patch.object(torch, "npu", None, create=True):
                self.assertEqual(
                    after_aot._minifier_env_variables([]),
                    {"ASCEND_RT_VISIBLE_DEVICES": "0"},
