...
class TestShapeVarConstruction(TestCase):
    ...
    def test_with_optimization_hint(self):
        s = VAR("seq", max=2048, optimization_hint=512)
    ...
    def test_repr(self):
        s = VAR("batch", max=64, optimization_hint=32)
        self.assertEqual(
            repr(s),
...
class TestTensorSpecConstruction(TestCase):
    ...
    def test_list_construction(self):
        ts = T([VAR("batch"), None, 10])
        self.assertIsInstance(ts[0], VAR)
...
class TestParamsSpecConstruction(TestCase):
    ...
    def test_shapes_spec_repr(self):
        sv = VAR("batch")
        ss = ShapesSpec(params=PARAMS({"x": T([sv, STATIC])}))
        self.assertEqual(
            repr(ss),
            """\
shapes_spec:
  params:
    ...
    def test_shapes_spec_repr_with_assumptions(self):
        ...
        B = VAR("b")
        ss = ShapesSpec(
            params={"x": T([A, STATIC]), "y": T([B, STATIC])},
...
class TestShapeVarCompile(TestCase):
    ...
    def test_unbacked_graph_has_unbacked_symbol(self):
        ...
        fn = torch.compile(
            lambda x: x.sum(0),
            backend=backend,
            shapes_spec={"x": T([VAR("batch"), STATIC])},
        )
        for n in [4, 8, 16, 32]:
            fn(torch.randn(n, 3))
    ...
    def test_params_spec_shorthand(self):
        ...
        fn = torch.compile(
            lambda x: x.sum(0),
            backend=backend,
            shapes_spec=PARAMS({"x": T([VAR("batch"), STATIC])}),
        )
        for n in [4, 8, 16]:
            fn(torch.randn(n, 3))
    ...
    def test_min_max_bypasses_dde_on_branching(self):
        ...
        compiled = torch.compile(
            ...
            backend="eager",
            fullgraph=True,
            shapes_spec={"x": T([VAR("batch", min=10, max=100), STATIC])},
        )
    ...
    def test_tensor_dim_optimization_hint_in_shape_env(self):
        ...
        fn = torch.compile(
            lambda x: x.sum(0),
            backend=backend,
            shapes_spec={"x": T([VAR("batch", optimization_hint=32), STATIC])},
        )
        fn(torch.randn(8, 3))
        shape = _tensor_placeholder_shape(backend.graphs[0])
        sym = shape[0]
...
class TestDerivedDimSpec(TestCase):
    ...
    def test_derived_scalar_arg(self):
        ...
        compiled = torch.compile(
            ...
            backend="eager",
            fullgraph=True,
            shapes_spec={"x": T([B, STATIC]), "n": B * 2},
        )
...
class TestObjectSpec(TestCase):
    ...
    def test_recursive_nesting(self):
        ...
        inner_spec = T([VAR("h"), STATIC])
        inner = OBJ({"weight": inner_spec})
...
object_spec:
  .weight: None""",
    ...
    def test_repr_with_tensorspec(self):
        ...
        os = OBJ({"weight": T([VAR("batch"), STATIC])})
        self.assertEqual(
            repr(os),
            """\
object_spec:
...
class TestObjectSpecCompile(TestCase):
    ...
    def test_attr_tensor_dim_dynamic(self):
        ...
        compiled = torch.compile(
            fn,
            backend=backend,
            shapes_spec={"obj": OBJ({"w": T([VAR("h"), STATIC])})},
        )
    ...
    def test_nn_module_parameter_dim_dynamic(self):
        ...
        compiled = torch.compile(
            m,
            backend=backend,
            shapes_spec={"self": OBJ({"weight": T([VAR("h"), STATIC])})},
        )
...
class TestDictSpecCompile(TestCase):
    ...
    def test_dict_spec_value_marked_dynamic(self):
        ...
        compiled = torch.compile(
            fn,
            backend=backend,
            shapes_spec={"cfg": DICT({"x": T([VAR("h"), STATIC])})},
        )
    ...
    def test_dict_spec_int_key_marked_dynamic(self):
        ...
        compiled = torch.compile(
            fn,
            backend=backend,
            shapes_spec={"cfg": DICT({0: T([VAR("h"), STATIC])})},
        )
    ...
    def test_nested_dict_of_list_of_tensors(self):
        ...
        compiled = torch.compile(
            ...
            backend=cnts,
            fullgraph=True,
            shapes_spec={"cfg": DICT({"xs": L([T([B, 3]), T([B, 3])])})},
        )
    ...
    def test_walk_terminal_container_raises(self):
        ...
        compiled = torch.compile(
            fn,
            backend="eager",
            shapes_spec={"x": DICT({"k": T([VAR("h"), STATIC])})},
        )
...
class TestWalkSpecRaises(TestCase):
    ...
    def test_object_spec_with_dict_input_raises(self):
        ...
        compiled = torch.compile(
            fn,
            backend="eager",
            shapes_spec={"cfg": OBJ({"x": T([VAR("h"), STATIC])})},
        )
        with self.assertRaises(torch._dynamo.exc.InternalTorchDynamoError) as ctx:
            compiled({"x": torch.randn(4, 3)})
        self.assertEqual(
    ...
    def test_dict_spec_with_object_input_raises(self):
        ...
        compiled = torch.compile(
            fn,
            backend="eager",
            shapes_spec={"obj": DICT({"x": T([VAR("h"), STATIC])})},
        )
        with self.assertRaises(torch._dynamo.exc.InternalTorchDynamoError) as ctx:
            compiled(Container(torch.randn(4, 3)))
        self.assertEqual(
    ...
    def test_seq_spec_with_dict_input_raises(self):
        ...
        compiled = torch.compile(
            fn,
            backend="eager",
            shapes_spec={"cfg": L([T([VAR("h"), STATIC])])},
        )
        with self.assertRaises(torch._dynamo.exc.InternalTorchDynamoError) as ctx:
            compiled({"x": torch.randn(4, 3)})
        self.assertEqual(
    ...
    def test_leaf_spec_with_container_input_raises(self):
        ...
        compiled = torch.compile(
            fn,
            backend="eager",
            shapes_spec={"cfg": T([VAR("h"), STATIC])},
        )
        with self.assertRaises(torch._dynamo.exc.InternalTorchDynamoError) as ctx:
            compiled({"x": torch.randn(4, 3)})
        self.assertEqual(
