...
"""
    ...
    def test_itertools_accumulate_tensors_user_defined(self):
        ...
        for udo_fn in [udo_fn_0, udo_fn_1]:
            counters.clear()
            torch._dynamo.reset()
    ...
    def test_itertools_accumulate_user_defined_closure_mutation(self):
        seen = []
    ...
    def test_itertools_accumulate_assume_constant_result_mutation(self):
        seen = []
    ...
    def test_itertools_accumulate_user_defined_local_mutation(self):
        def fn(a, b):
            seen = []
        ...
        eager = fn(torch.tensor([1]), torch.tensor([2]))
        compiled_fn = torch.compile(fn, backend="eager", fullgraph=True)
        compiled = compiled_fn(torch.tensor([1]), torch.tensor([2]))
        self.assertEqual(list(eager), list(compiled))

    def test_itertools_accumulate_user_defined_escaping_local_mutation(self):
        def fn(a, b):
            seen = []
            ...
            return itertools.accumulate([a, b], udo_fn), seen

        eager, seen = fn(torch.tensor([1]), torch.tensor([2]))
        self.assertEqual(seen, [])
        self.assertEqual(len(list(eager)), 2)
