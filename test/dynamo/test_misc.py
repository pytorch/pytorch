...
class MiscTests(torch._dynamo.test_case.TestCase):
    ...
    def test_py311_jump_offset(self):
        ...
        def create_test_code(jump_opname, target_idx):
            ...
            insts = [
                new_inst("RESUME", 0),
                new_inst("JUMP_FORWARD", target=targets[1]),
                targets[0],
                new_inst("LOAD_CONST", 1),
                new_inst("STORE_NAME", argval="test_local"),
                new_inst("LOAD_CONST", 1),
                new_inst("RETURN_VALUE"),
                targets[1],
                new_inst(jump_opname, target=targets[target_idx]),
        ...
        test_args = (
            ...
            ("JUMP_FORWARD", 2, True, 3),
            ("JUMP_BACKWARD", 0, False, 1),
            ("JUMP_BACKWARD", 2, True, 3),
        )
        ...
        for test in test_args:
            ...
            # exec doesn't propagate code's return, so look into code's locals
            locals_dict = {}
            exec(code, globals(), locals_dict)
            self.assertEqual(locals_dict["test_local"], test[3])
