...
def forward(self, L_x_ : torch.Tensor):
    ...
    def test_string_format_splitlines_crlf_empty_field_graph_breaks(self):
        def known_empty_field(x):
            return "a\r{}\n{}".format("", x.size(0)).splitlines(True)

        def maybe_empty_field(x):
            return "a\r{!s:.0}\n{}".format(x.size(0), x.size(1)).splitlines(True)

        x = torch.ones(2, 3)
        self.assertEqual(known_empty_field(x), ["a\r\n", "2"])
        self.assertEqual(maybe_empty_field(x), ["a\r\n", "3"])

        for fn in (known_empty_field, maybe_empty_field):
            opt = torch.compile(fn, backend="eager", fullgraph=True, dynamic=True)
            with self.assertRaisesRegex(
                Unsupported,
                "formatted field may be empty between CRLF splitlines boundary",
            ):
                opt(x)
