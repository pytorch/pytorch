...
def forward(self, s0 : torch.SymInt, s1 : torch.SymInt, L_x_ : torch.Tensor):
    ...
    def test_incompatible_configs(self):
        with torch._dynamo.config.patch(suppress_errors=False, fail_on_recompile_limit_hit=False):
        ...
        with torch._dynamo.config.patch(suppress_errors=True, fail_on_recompile_limit_hit=True):
