...
class ReproTests(torch._dynamo.test_case.TestCase):
    ...
    def _assert_named_out_resize_break(self, fn):
        ...
        # caller silently read back the original instead of getting a break.
        torch._dynamo.reset()
        self.assertEqual(torch.compile(fn, backend="eager")(), fn())
        torch._dynamo.reset()
        with self.assertRaisesRegex(
            torch._dynamo.exc.Unsupported,
            "Shape mismatch with schema-named out= tensor variant",
        ):
    ...
    @parametrize(
        "op",
        (
            subtest(torch.ops.aten.aminmax, name="packet"),
            subtest(torch.ops.aten.aminmax.out, name="overload"),
        ),
    )
    def test_out_shape_change_named_out_kwargs(self, op):
        # aten.aminmax names its out arguments min/max
        def fn():
            mn = torch.empty(0)
            mx = torch.empty(0)
            op(torch.zeros(4, 5), min=mn, max=mx)
            return (tuple(mn.size()), tuple(mx.size()))

        self._assert_named_out_resize_break(fn)

    def test_out_shape_change_numbered_out_kwargs(self):
        # autogenerated multi-output variants spell them out0/out1/...
        def fn():
            outs = [torch.empty(0) for _ in range(3)]
            x = torch.zeros(1, 2, 3)
            ...
            return tuple(tuple(o.size()) for o in outs)

        self._assert_named_out_resize_break(fn)

    def test_out_shape_change_out_and_named_out_kwargs(self):
        # aten::max_pool2d_with_indices.out declares `out` alongside `indices`;
        # `out` is already the right size and only `indices` is resized, so
        # keying off `out` alone misses it
        def fn():
            out = torch.empty(1, 1, 2, 2)
            indices = torch.empty(0, dtype=torch.long)
            x = torch.arange(16.0).reshape(1, 1, 4, 4)
            torch.ops.aten.max_pool2d_with_indices(x, [2, 2], out=out, indices=indices)
            return (tuple(out.size()), tuple(indices.size()))

        self._assert_named_out_resize_break(fn)

    def test_out_presized_schema_named_out_kwargs_no_graph_break(self):
        # The resize check must not turn into "always break on a named out arg":
        # correctly-sized out arguments still compile in a single graph, and the
        # op still writes through to them.
        def presized():
            # nan sentinel rather than raw `empty`: an uninitialized read is
            # frequently 0.0, which would let a dropped mutation pass by luck
            mn = torch.full((), float("nan"))
            mx = torch.full((), float("nan"))
            torch.ops.aten.aminmax(
                torch.arange(1.0, 21.0).reshape(4, 5), min=mn, max=mx
            )
            return (mn.clone(), mx.clone())

        # canonicalize_output_graph_node_order (forced on by test_case.TestCase)
        # keys its reorder barrier on a literal "out" kwarg, so it floats a
        # named-out call past the clones that read what it writes. That is a
        # separate pre-existing bug in torch/fx/passes/canonicalize.py, fixed
        # outside this stack; pin the ordering here so this test checks the
        # mutation rather than that bug.
        torch._dynamo.reset()
        with torch._dynamo.config.patch(canonicalize_output_graph_node_order=False):
            self.assertEqual(
                torch.compile(presized, backend="eager", fullgraph=True)(),
                presized(),
            )
        self.assertEqual(presized(), (torch.tensor(1.0), torch.tensor(20.0)))

    def test_out_shape_change_single_out_arg_keeps_single_tensor_path(self):
        # A schema whose only out argument is spelled "out" must keep taking the
        # single-tensor path, which reports its own gb_type rather than the
        # schema-named one added alongside it.
        def fn():
            out = torch.empty(0)
            torch.ops.aten.add(torch.ones(3), torch.ones(3), out=out)
        ...
        torch._dynamo.reset()
        with self.assertRaisesRegex(
            torch._dynamo.exc.Unsupported, "Shape mismatch with out= tensor variant"
        ):
            torch.compile(fn, backend="eager", fullgraph=True)()

    def test_out_kwarg_unbacked_shape_with_schema_named_out_args(self):
        # A literal `out=` on a schema that also names other out arguments must
        # keep taking the single-tensor path. That path answers the layout
        # question with false_if_dde=True; the list path raises on an unbacked
        # size, so rerouting `out=` there turns this into a hard error.
        def fn(t):
            n = t.nonzero().size(0)
            inp = torch.arange(n * 3, dtype=torch.float).reshape(n, 3)
            out = torch.empty(n, 3)
            save_mean = torch.empty(3)
            save_invstd = torch.empty(3)
    ...
    def test_out_kwarg_non_contiguous_with_schema_named_out_args(self):
        # The complement of test_out_schema_named_out_kwargs_non_contiguous:
        # exempting schema-named args from the layout check must not exempt the
        # literal `out=` sitting next to them.
        def fn(x, out, indices):
            torch.ops.aten.max_pool2d_with_indices(x, [2, 2], out=out, indices=indices)
            return out.clone()

        x = torch.arange(16.0).reshape(1, 1, 4, 4)
        out = torch.zeros(8)[::2].view(1, 1, 2, 2)
        indices = torch.zeros(1, 1, 2, 2, dtype=torch.long)
        torch._dynamo.reset()
        self.assertEqual(
            torch.compile(fn, backend="eager")(x, out, indices), fn(x, out, indices)
        )
        torch._dynamo.reset()
        with self.assertRaisesRegex(
            torch._dynamo.exc.Unsupported,
            ...
            torch.compile(fn, backend="eager", fullgraph=True)(x, out, indices)

    # see test_out_presized_schema_named_out_kwargs_no_graph_break: canonicalization
    # reorders a named-out call past its readers, which is fixed separately
    @torch._dynamo.config.patch(canonicalize_output_graph_node_order=False)
    def test_out_schema_named_out_kwargs_non_contiguous(self):
        # Schema-named out args get the resize check, not the layout check:
        # extending the latter would newly reject calls that compile correctly
        # today. `is_out` is also true of kwarg-only mutable args on ops that
        # are not out variants at all, which must not be layout-restricted.
        def fn(x):
            mn = torch.empty(10)[::2]
            mx = torch.empty(10)[::2]
            torch.ops.aten.aminmax(x, dim=0, min=mn, max=mx)
            return mn.clone(), mx.clone()

        x = torch.randn(4, 5)
        self.assertEqual(
            torch.compile(fn, backend="aot_eager", fullgraph=True)(x), fn(x)
        )

    def test_out_schema_named_out_kwargs_unbacked_shape(self):
        # An out tensor with an unbacked size is contiguous, but proving it
        # requires a data-dependent guard, so the layout check on this path must
        # not demand one -- otherwise routing named out args through it turns
        # working code into a hard error under fullgraph.
        def fn(t):
            n = t.nonzero().size(0)
            x = torch.arange(n, dtype=torch.float)
            values = torch.empty(n)
            indices = torch.empty(n, dtype=torch.long)
            torch.ops.aten.sort(x, values=values, indices=indices)
            return values.sum()

        t = torch.tensor([1.0, 0.0, 2.0, 3.0])
        self.assertEqual(torch.compile(fn, backend="eager", fullgraph=True)(t), fn(t))
