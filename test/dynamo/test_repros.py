    ...
    def test_lru_cache_warning_issued_during_tracing(self, device):
        ...
        try:
            with (
                ...
                self.assertLogs("torch._dynamo", level="DEBUG") as log_ctx,
            ):
        ...
        for w in caught_warnings:
            if "call to a lru_cache wrapped function at:" in str(w.message):
                self.fail("lru_cache warning was incorrectly emitted via warnings.warn")

        for record in log_ctx.records:
            if "call to a lru_cache wrapped function at:" in record.getMessage():
                self.fail("lru_cache warning was incorrectly logged by dynamo logger")
