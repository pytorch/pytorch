...
def forward(self, s0 : torch.SymInt, s1 : torch.SymInt, L_x_ : torch.Tensor):
    ...
    def test_flip_bad_accuracy(self):
        ...
        for dtype in [
            ...
            x = torch.arange(9, device=device(type="cuda"), dtype=dtype)
            fails_for_backend_dtype = []
            for backend in [
                "eager",
                "aot_eager",
                ...
                try:
                    self.assertEqual(
                        torch.flip(torch.cumsum(torch.flip(x, [0]), 0), [0]).to(
                            torch.float
                        ),
                        out[0].to(torch.float),
                    )
                except AssertionError:
                    fails_for_backend_dtype.append(backend)
            fails_for_backend[dtype] = fails_for_backend_dtype
