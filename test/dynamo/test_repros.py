...
def forward(self, L_x_ : torch.Tensor, s77 : torch.SymInt, s27 : torch.SymInt):
    ...
    def test_pad_packed_sequence_fake_tensor_invalid_data(self):
        from torch._subclasses.fake_tensor import FakeTensorMode
        from torch.fx.experimental.symbolic_shapes import ShapeEnv

        with FakeTensorMode(shape_env=ShapeEnv()):
            data = torch.empty(())
            batch_sizes = torch.empty(5, dtype=torch.int64)
            with self.assertRaisesRegex(RuntimeError, "data.*at least 1D"):
                torch.ops.aten._pad_packed_sequence.default(
                    data, batch_sizes, False, 0.0, 5
                )
    ...
    @torch._dynamo.config.patch(capture_dynamic_output_shape_ops=True)
    @parametrize("backend", ["aot_eager", "inductor"])
    @parametrize("feature_size", [0, 2])
    def test_pad_packed_sequence_backward(self, backend, feature_size):
        batch_sizes = torch.tensor([3, 3, 2, 1, 1])
        data = torch.randn(10, feature_size)
        ...
        expected_data = data.detach().clone().requires_grad_()
        actual_data = data.detach().clone().requires_grad_()
        expected_output, expected_lengths = fn(expected_data, batch_sizes)
        expected_output.sum().backward()

        compiled_fn = torch.compile(fn, backend=backend, fullgraph=True, dynamic=True)
        actual_output, actual_lengths = compiled_fn(actual_data, batch_sizes)
