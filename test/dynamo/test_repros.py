...
class ReproTests(torch._dynamo.test_case.TestCase):
    ...
    def test_as_strided_on_view_input_storage_bounds_inductor(self):
        ...
        base = torch.arange(20)
        with fresh_cache():
            actual = torch.compile(foo, backend="inductor")(base[:10])
    ...
    # https://github.com/pytorch/pytorch/issues/140171
    def test_as_strided_scatter_dynamic_intermediate_storage_inductor(self):
        def foo(base, src):
            start = base.shape[0] - 14
            x = base[start:]
            return torch.as_strided_scatter(
                x, src, (5,), (1,), storage_offset=x.storage_offset()
            )

        src = torch.arange(5)
        cnt = CompileCounterWithBackend("inductor")
        with fresh_cache():
            opt_foo = torch.compile(foo, backend=cnt, dynamic=True)
            for storage_size in (15, 20):
                with self.subTest(storage_size=storage_size):
                    base = torch.arange(storage_size)
                    expected = foo(base, src)
                    actual = opt_foo(base, src)

                    self.assertEqual(actual, expected)
                    self.assertEqual(actual.storage_offset(), expected.storage_offset())
                    self.assertEqual(
                        actual.untyped_storage().size(),
                        expected.untyped_storage().size(),
                    )
                    self.assertEqual(
                        actual.as_strided((storage_size,), (1,), 0),
                        expected.as_strided((storage_size,), (1,), 0),
                    )

        self.assertEqual(cnt.frame_count, 1)

    # https://github.com/pytorch/pytorch/issues/140171
    @parametrize("op", ["as_strided", "diagonal", "slice"])
    @parametrize("dynamic", [False, True])
    def test_scatter_storage_query_in_graph_inductor(self, op, dynamic):
        def foo(x, src):
            if op == "as_strided":
                y = torch.as_strided_scatter(x, src, (5,), (1,), 5)
            elif op == "diagonal":
                y = torch.diagonal_scatter(x, src)
            else:
                y = torch.slice_scatter(x, src, 0, 5, 10)
            return y.untyped_storage().size(), y.storage_offset()

        if op == "diagonal":
            x = torch.arange(36).reshape(6, 6)[1:4, 2:5]
            src = torch.arange(3)
        else:
            x = torch.arange(20)[5:15]
            src = torch.arange(5)

        expected = foo(x, src)
        with fresh_cache():
            actual = torch.compile(
                foo, backend="inductor", dynamic=dynamic, fullgraph=True
            )(x, src)
    ...
    # https://github.com/pytorch/pytorch/issues/140171
    @parametrize("op", ["as_strided", "diagonal", "slice"])
    def test_scatter_storage_query_dynamic_reuse_inductor(self, op):
        def foo(base, src):
            if op == "diagonal":
                x = base[-3:, 2:5]
                y = torch.diagonal_scatter(x, src)
        ...
        cnt = CompileCounterWithBackend("inductor")
        with fresh_cache():
            opt_foo = torch.compile(foo, backend=cnt, dynamic=True, fullgraph=True)
            for size in (15, 20):
                with self.subTest(size=size):
                    if op == "diagonal":
                        base = torch.arange(size * 6).reshape(size, 6)
                        src = torch.arange(3)
                    else:
                        base = torch.arange(size)
                        src = torch.arange(5)

                    self.assertEqual(opt_foo(base, src), foo(base, src))
    ...
    # https://github.com/pytorch/pytorch/issues/140171
    def test_slice_scatter_dense_input_storage_observation_dynamic_inductor(self):
        def foo(x, src):
            y = torch.slice_scatter(x, src, 0, 5, 10)
            return y.as_strided((x.shape[0],), (1,), 0)
