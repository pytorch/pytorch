...
def forward(self, s77 : torch.SymInt, s27 : torch.SymInt, L_x_ : torch.Tensor):
    ...
    def test_tracable_parameter_context_force_restores_after_nested_disable(self):
        self.assertTrue(can_convert_to_tracable_parameter())
        self.assertFalse(force_tracable_parameter())

        with allow_convert_to_tracable_parameter():
            self.assertTrue(can_convert_to_tracable_parameter())
            self.assertTrue(force_tracable_parameter())

            with do_not_convert_to_tracable_parameter():
                self.assertFalse(can_convert_to_tracable_parameter())
                self.assertFalse(force_tracable_parameter())
