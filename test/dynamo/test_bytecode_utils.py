...
class BytecodeHookTests(torch._dynamo.test_case.TestCase):
    ...
    def test_runtime_error_in_generated_bytecode_is_internal(self):
        def fn(x):
            return x.sum()
        ...
        def hook(code, out_code):
            ...
            def transform(instructions, _):
                for i, inst in enumerate(instructions):
                    if inst.opname == "RETURN_VALUE":
                        instructions[i:i] = [
                            bytecode_transformation.create_instruction(
                                "LOAD_FAST", argval="x"
                            ),
                            bytecode_transformation.create_instruction(
                                "LOAD_CONST", argval=100
                            ),
                            bytecode_transformation.create_binary_subscr(),
                        ]
                        break
                else:
                    self.fail("RETURN_VALUE not found in generated bytecode")

            new_code, _ = bytecode_transformation.transform_code_object(
                out_code, transform
            )
            return new_code
        ...
        try:
            ...
            with self.assertRaisesRegex(
                ...
                opt_fn(torch.ones(10))
        finally:
            handle.remove()

    def test_runtime_error_in_generated_graph_break_bytecode_is_not_internal(self):
        def fn(x):
            torch._dynamo.graph_break()
            return x.sum()

        def hook(code, out_code):
            if not code.co_name.startswith("torch_dynamo_resume_in_fn"):
                return None
        ...
        try:
            opt_fn = torch.compile(fn, backend="eager")
            with self.assertRaisesRegex(IndexError, "out of bounds") as cm:
                opt_fn(torch.ones(10))
            self.assertNotIsInstance(
                cm.exception, torch._dynamo.exc.InternalTorchDynamoError
            )
        finally:
            handle.remove()

    def test_runtime_error_in_user_backend_callable_is_not_internal(self):
        def fn(x):
            return x + 1

        def backend(gm, example_inputs):
            def compiled_fn(*args):
                raise RuntimeError("user backend runtime error")

            return compiled_fn

        opt_fn = torch.compile(fn, backend=backend, fullgraph=True)
        with self.assertRaisesRegex(RuntimeError, "user backend runtime error") as cm:
            opt_fn(torch.ones(2))
        self.assertNotIsInstance(
            cm.exception, torch._dynamo.exc.InternalTorchDynamoError
        )

    def test_runtime_error_in_generated_graph_is_not_internal(self):
        def fn(x):
        ...
        with self.assertRaisesRegex(
            ...
            opt_fn(torch.tensor([[1.0, 2.0], [2.0, 1.0]]))
        self.assertNotIsInstance(
            cm.exception, torch._dynamo.exc.InternalTorchDynamoError
        )

    def test_runtime_error_in_replayed_side_effect_is_not_internal(self):
        def fn(x, err):
            err.__cause__ = 2
            return x + 1

        opt_fn = torch.compile(fn, backend="eager", fullgraph=True)
        with self.assertRaisesRegex(TypeError, "exception cause must be") as cm:
            opt_fn(torch.ones(2), TypeError("user error"))
        self.assertNotIsInstance(
            cm.exception, torch._dynamo.exc.InternalTorchDynamoError
        )
