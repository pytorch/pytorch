...
"""
    ...
    def _get_guard_failure_on_overlapping_view_inputs(
        self, f, argsfn1, argsfn2, *, dynamic=True
    ):
        ...
        opt_f = torch._dynamo.optimize(
            "aot_eager", dynamic=dynamic, guard_fail_fn=guard_fail_fn
        )(f)
    ...
    def test_mutated_input_overlaps_unmutated_input_recompile(self):
        ...
        guard_failure = self._get_guard_failure_on_overlapping_view_inputs(
            f, non_overlapping_args, overlapping_args, dynamic=False
        )
        self.assertExpectedInline(
            guard_failure,
    ...
    def test_input_storage_overlap_guard_compile_order(self):
        ...
        for backend in ("aot_eager", "inductor"):
            with fresh_cache():
                ...
                if backend == "inductor":
                    torch._dynamo.reset()
                    cached_fn = torch.compile(fn, backend=backend)
                    self.assertEqual(cached_fn(*non_alias_args()), torch.tensor([-9.0]))
                    self.assertEqual(cached_fn(*alias_args()), torch.tensor([0.0]))
    ...
    def test_input_storage_overlap_guard_preserves_groups(self):
        ...
        guard_failure = self._get_guard_failure_on_overlapping_view_inputs(
            fn, args_ab_cd, args_ac_bd, dynamic=False
        )
        self.assertExpectedInline(
            guard_failure,
            """0/0: check_storage_aliasing([[a, b], [c, d]])""",
        )
        ...
        torch._dynamo.reset()
        compiler = Compiler()
        torch.compile(fn, backend=compiler, dynamic=False)(*args_ab_cd(None))
        aliasing_guards = [
            guard for guard in compiler.guards if isinstance(guard, StorageAliasing)
        ]
        overlap_guards = [
            guard for guard in compiler.guards if isinstance(guard, StorageOverlap)
        ]
        ...
        self.assertEqual(
            [len(guard.overlapping_sources) for guard in overlap_guards], [2, 2]
        )

    def test_input_storage_partition_guard_preserves_false_sharing(self):
        def fn(a, b, c):
            a.add_(1)
            return b.clone(), c.clone()
        ...
        def split_storage(_):
            base = torch.zeros(4)
            return base[0:2], base[1:3], torch.zeros(2)

        guard_failure = self._get_guard_failure_on_overlapping_view_inputs(
            fn, one_storage, split_storage, dynamic=False
        )
        self.assertExpectedInline(
            guard_failure,
            """0/0: check_storage_aliasing([[a, b, c]])""",
        )

    def test_duplicate_input_overlap_guard_uses_deduped_sources(self):
        def fn(a, a_dupe, b, b_dupe):
            a.add_(1)
            b.add_(2)
            return a_dupe.clone(), b_dupe.clone()

        def disjoint_duplicates(base):
            a = base[:4]
            b = base[4:8]
            return a, a, b, b

        def overlapping_duplicates(base):
            a = base[:4]
            b = base[2:6]
            return a, a, b, b

        guard_failure = self._get_guard_failure_on_overlapping_view_inputs(
            fn, disjoint_duplicates, overlapping_duplicates, dynamic=False
        )
        self.assertExpectedInline(
            guard_failure,
            """0/0: check_overlapping(overlapping=[], non_overlapping=[a, b])""",
    ...
    def _test_no_storage_overlap_guards(self, f, argsfn):
        ...
        # argsfn function. Meanwhile, keep track of the AOTAutograd guards to make sure
        # no storage-relationship guard was added.
        ...
        class Compiler:
            ...
            def __call__(self, *args, **kwargs):
                ...
                # AOTAutograd storage guards are only added later.
                self.guards = TracingContext.get().guards_context.aotautograd_guards
                return self.counter(*args, **kwargs)
        ...
        # Check that none of the AOTAutograd guards track storage relationships.
        for g in compiler.guards:
            self.assertNotIsInstance(g, (StorageAliasing, StorageOverlap))
    ...
    def test_storage_overlap_guards_no_aliasing_with_mutation(self):
        ...
        aliasing_guards = [g for g in compiler.guards if isinstance(g, StorageAliasing)]
        self.assertEqual(len(overlap_guards), 0)
        self.assertEqual(len(aliasing_guards), 1)
        self.assertEqual(
            [len(group) for group in aliasing_guards[0].source_groups], [1, 1]
        )

    def test_many_unaliased_mixed_mutation_inputs_use_bounded_guards(self):
        def f(*args):
            for arg in args[::2]:
                arg.add_(1)
