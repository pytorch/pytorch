...
class OptimizedModuleTest(torch._dynamo.test_case.TestCase):
    ...
    def test_hook_dict_mutation_notification_does_not_graph_break(self):
        hooks_dict = collections.OrderedDict()

        @torch.compile(backend="eager", fullgraph=True)
        def fn(x):
            torch.utils.hooks._notify_hook_dict_mutation(hooks_dict)
            return x + 1

        x = torch.ones(3)
        self.assertEqual(fn(x), x + 1)

    def test_hook_dict_mutation_during_compile_session_invalidates(self):
        layer = torch.nn.Linear(3, 3, bias=False)
        layer.weight = torch.nn.Parameter(torch.eye(3))
        cnt = torch._dynamo.testing.CompileCounter()
        ...
        with torch.compiler._compile_session_context():
            self.assertTrue(torch.compiler.is_compiling())
            self.assertFalse(torch.compiler.is_dynamo_compiling())
            handle = layer.register_forward_hook(
                lambda _module, _args, output: output * 2
            )
