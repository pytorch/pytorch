...
class NNModuleTests(torch._dynamo.test_case.TestCase):
    ...
    def test_numpy_float_module_attr_dtype(self):
        class Mod(torch.nn.Module):
            def __init__(self) -> None:
                super().__init__()
                self.scale = np.float64(2.0)
            ...
            def forward(self, x):
                return (
                    x + (self.scale.dtype == np.dtype("float64")),
                    x + (self.scale.astype(np.float32).dtype == np.dtype("float32")),
                )

        mod = Mod()
        x = torch.randn(4)
        self.assertEqual(torch.compile(mod, backend="eager", fullgraph=True)(x), mod(x))
