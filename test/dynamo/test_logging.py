...
TRACE FX call mul from test_logging.py:N in fn (LoggingTests.test_trace_call_prefix.fn)
    ...
    def test_guards_sloc(self, records):
        ...
        self.assertExpectedInline(
            ...
            """\
+- LAMBDA_GUARD: L['x'].size()[0] == L['z'].size()[0] + L['y'].size()[0]  # return x + torch.cat([y, z])  # #:# in # #:# in #
+- LAMBDA_GUARD: Eq(Mod(L['z'].size()[0] + L['y'].size()[0], 3), 0)  # if x.size(0) % 3 == 0:  # #:# in # #:# in #
+- LAMBDA_GUARD: 2 <= L['z'].size()[0]  # return x + torch.cat([y, z])  # #:# in # (user code shown is first use of this value--the guard itself is not due user code but due to 0/1 specialization in the framework; to avoid specialization try torch._dynamo.mark_unbacked(tensor, dim))
