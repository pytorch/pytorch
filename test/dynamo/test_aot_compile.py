...
from user code:
   File "test_aot_compile.py", line N, in foo
    ...
    def test_no_match_message_when_a_guard_answers_inconsistently(self):
        ...
        self.assertIn(
            "  [1] <guards did not accept this call in dispatch and accepted it here: a guard that does not answer consistently>",
            message,
        )
    ...
    def test_no_match_message_after_a_raise_counts_no_second_rejection(self):
        ...
        # never took; the raise's text reaches the report only in the caveat
        # below, the fix-or-drop advice keeping just the fact of it. The one
        ...
        message = str(ctx.exception)
        # One raise and one rejection: the stub was evaluated exactly twice, and
        # the whole line counts no rejection dispatch did not get.
        self.assertEqual(stub.checks, 2)
        accepted = "  [0] <guards did not accept this call in dispatch and accepted it here: a guard that does not answer consistently>"
        self.assertIn(accepted, message.splitlines())
        self.assertEqual(str(ctx.exception.__cause__), "the first pass is unhappy")
        self.assertEqual(message.count("the first pass is unhappy"), 1)
        self.assertIn("[0]'s guard check raised while checking this call", message)
        self.assertIn("Add a ModelInput", message)
    ...
    def test_no_match_message_hint_sees_through_a_wrapped_rebound_forward(
        ...
        try:
            ...
            self.assertIn(
                ...
                "wrappers torch.compile and torch._dynamo.disable return and through "
                "any functools.wraps'd torch._dynamo.external_utils function to the "
                "function they wrap, since that is the one the load resolved",
                message,
            )
    ...
    def test_aot_compile_module_two_raisers_in_the_report_and_then_the_warning(self):
        ...
        # part: the chain carries the FIRST index that raised, whose line as an
        # opt-out quotes no exception text, while the advice names the first
        # ENABLED one -- chaining raised[raiser] instead would delete [0]'s raise.
        ...
        results[0].disable_guard_check()
        results[1]._artifacts.guard_manager = Raises("checked\x0ctree\x1eis unhappy")
        with self.assertRaises(RuntimeError) as ctx:
            model(x)
        lines = str(ctx.exception).splitlines()
        ...
        self.assertIn("[1]'s guard check raised RuntimeError: checked", warned)
    ...
    def test_aot_compile_module_serves_a_later_match_after_an_earlier_raise(self):
        ...
        # either -- a throw out of C++ skips _reset_relational_guard_state(), and
        # that residue stays whichever way dispatch answers -- so the raise is
        # served over, and warned about, rather than becoming a second failure
        # mode.
        self._hide_leaked_dynamo_globals()
        mod = GlobalConfigModule()
        model = torch.compile(
    ...
    def test_aot_compile_module_veto_and_report_survive_without_the_dict_leaf(self):
        ...
        class Rejects:
            ...
            # Unreachable while the opt-out is reported before the re-check, which
            # the entry-line count below pins; defined per NeverReChecked's note.
            def check_verbose(self, f_locals):
                return types.SimpleNamespace(
                    result=False, verbose_code_parts=["stub guard rejected"]
                )
    ...
    def test_no_match_message_quotes_the_last_raise_at_one_index(self):
        ...
        # `raised[i]` is overwritten in place, so the entry line and the chain
        # read the same exception, and the report's own re-check line quotes the
        # later raise for an entry that answered and then raised as well -- one
    ...
    def test_aot_compiled_model_keeps_its_warning_dedup_out_of_the_constructor(self):
        # init=False: a caller's set would share one dedup across every model.
        init_fields = [f.name for f in dataclasses.fields(AOTCompiledModel) if f.init]
        self.assertNotIn("_warned", init_fields)
    ...
    def test_aot_compile_module_warns_again_about_a_replaced_index(self):
        ...
        # public, so after a pop the artifact at a warned-about index is a
        # different one whose raise the first warning never described. The
        # warning compares the results to the ones it last logged about and
        # starts over when they differ. The one-result twin below is the
        # load-bearing case, not this test: both replacements reuse the key
        # (0, "RuntimeError"), but a one-result model never re-decides the
        # binding verdict, so a reset tied to it, or gated on two or more
        # results, passes here and fails only there (measured).
        self._hide_leaked_dynamo_globals()
        x = torch.randn(3, 3)
        inputs = [ModelInput(args=(x,), kwargs={}, contexts=[]) for _ in range(3)]
        model = torch.compile(ScaleModule(), fullgraph=True, backend="eager")
        ...
        results = model.forward.compiled_results
        results[0]._artifacts.guard_manager = Raises("zero is unhappy")
        results[1]._artifacts.guard_manager = Raises("one is unhappy")
        logger = "torch._dynamo.aot_compile"
        with self.assertLogs(logger, level="WARNING") as logs:
            self.assertEqual(model(x), x * 2)
        self.assertEqual(len(logs.output), 2)
    ...
    def test_aot_compile_module_interrupt_out_of_a_later_tree_propagates(self):
        ...
        # assertNoLogs stays outermost: its __exit__ checks nothing while an
        # exception is passing through it, and this call always raises, so
    ...
    def test_aot_compile_module_later_result_that_cannot_bind_raises_type_error(self):
        ...
        # it (same nesting, for the same reason). Both premises are pinned first:
        # the results cannot share a binding, so [1] binds inside the second
        # accepts(), and [0]'s tree does raise.
        self._hide_leaked_dynamo_globals()
        mod = ScaleModule()
        x = torch.randn(3, 3)
        model = torch.compile(mod, fullgraph=True, backend="eager")
        ...
        mod.forward = types.MethodType(needs_z, mod)
        model = torch.compile(mod, fullgraph=True, backend="eager")
        model._aot_compile([ModelInput(args=(x, x), kwargs={}, contexts=[])])
        combined = AOTCompiledModel(mod, doubled + model.forward.compiled_results)
    ...
    def test_aot_compile_module_deserialize_refuses_a_targetless_dynamo_wrapper(
        ...
        # skips functools.wraps: a plain function owning external_utils' dict
        # with no __wrapped__ to follow to the forward it wraps. The hop stops on
        # it and the namespace test refuses it as it refuses any external_utils
        # function bound as forward, with the reason it gave before the hop
        # existed, and external_utils' namespace gains nothing.
        mod = GlobalConfigModule()
        ...
        self._assert_forward_refused(
            ...
            "wrap_dunder_call_ctx_manager.<locals>.inner) resolves to "
            "wrap_dunder_call_ctx_manager.<locals>.inner, whose globals are "
    ...
    def test_aot_compile_module_fallback_names_a_compiled_partial_forward(self):
        ...
        # not a function), so the hop follows __wrapped__ onto the partial and
        # get_traced_fn fails THERE. Falling back is right (before the hop this
        # shape resolved eval_frame's dict and the torch-namespace rule refused
        # it); the reason has to say what the wrapper reached, not tell the user
        # to make model.forward a plain function -- the compile_wrapper already
        # is one -- and eval_frame's namespace gains nothing.
        mod = GlobalConfigModule()
    ...
    def test_aot_compile_module_deserialize_unwraps_a_disabled_forward(self, recursive):
        ...
        # so recursive_False is served by either mechanism and only
        # recursive_True catches a load that drops innermost_fn; the
        # innermost_fn assertion records which mechanism reaches the bound
        # method first, not which one the load needed.
        x = torch.randn(4, 8)
        data = self._two_input_global_guard_artifact(x)
        mod = GlobalConfigModule()
        expected = {}
    ...
    def test_load_seeds_a_rebound_nn_module_alias_through_the_torch_package(self):
        ...
        # Cache entries for the alias and the fallback root; the module dict's
        # __import_* bindings are _hide_leaked_dynamo_globals' cleanup.
        self.addCleanup(_import_source_cache.pop, name, None)
        self.addCleanup(_import_source_cache.pop, "torch", None)
