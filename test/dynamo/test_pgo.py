...
class PgoTest(torch._dynamo.test_case.TestCase):
    ...
    def test_sticky_pgo_read_write(self):
        ...
        with mock_cache.PatchCaches():
            ...
            self.reset()
            cnts.clear()
            with torch.compiler.config.patch(job_id="b", sticky_pgo_write="sticky_0"):
                f(t(2, 2), t(2, 2))
                f(t(2, 4), t(2, 2))
                self.assertEqual(cnts.frame_count, 2)
            ...
            self.reset()
            cnts.clear()
            with torch.compiler.config.patch(job_id="c", sticky_pgo_read="sticky_0"):
                f(t(2, 2), t(2, 2))
                f(t(2, 4), t(2, 2))
                self.assertEqual(cnts.frame_count, 1)
            ...
            self.reset()
            cnts.clear()
            with torch.compiler.config.patch(job_id="d", sticky_pgo_write="sticky_1"):
                f(t(2, 2), t(2, 2))
                f(t(2, 2), t(2, 4))
                f(t(2, 2), t(4, 4))
                self.assertEqual(cnts.frame_count, 3)
            ...
            self.reset()
            cnts.clear()
            with torch.compiler.config.patch(sticky_pgo_read="sticky_1"):
                f(t(2, 2), t(2, 2))
                f(t(2, 4), t(4, 2))
                f(t(4, 2), t(2, 4))
                self.assertEqual(cnts.frame_count, 1)
