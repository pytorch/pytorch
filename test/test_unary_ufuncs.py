...
from torch.testing._internal.common_methods_invocations import unary_ufuncs, _NOTHING
...
def generate_tensors_from_vals(vals, device, dtype, domain):
    ...
    assert _large_size[1] > (
        _medium_length + offset
    )  # large tensor should be large enough
    ...
    large_tensor = make_tensor(
        _large_size, device=device, dtype=dtype, low=domain[0], high=domain[1]
    )

    # Inserts the vals at an odd place
    large_tensor[57][offset : offset + len(vals)] = torch.tensor(
    ...
    empty_tensors = (
        torch.empty(size, device=device, dtype=dtype) for size in empty_sizes
    )
...
#   argument.
def generate_numeric_tensors(device, dtype, *, domain=(None, None)):
    # Special-cases bool
    if dtype is torch.bool:
        tensors = (
            ...
            make_tensor(
                (_medium_length,), device=device, dtype=dtype, low=None, high=None
            ),
...
def generate_numeric_tensors_hard(device, dtype, *, domain=(None, None)):
...
def generate_numeric_tensors_extremal(device, dtype, *, domain=(None, None)):
...
class TestUnaryUfuncs(TestCase):
    ...
    def test_reference_numerics_normal(self, device, dtype, op):
        tensors = generate_numeric_tensors(device, dtype, domain=op.domain)
        self._test_reference_numerics(dtype, op, tensors)

    @suppress_warnings
    @ops(
        reference_filtered_ops,
        allowed_dtypes=floating_and_complex_types_and(
            torch.bfloat16,
            torch.half,
            torch.int8,
            torch.int16,
    ...
    def test_reference_numerics_hard(self, device, dtype, op):
        ...
        tensors = generate_numeric_tensors_hard(device, dtype, domain=op.domain)
        self._test_reference_numerics(dtype, op, tensors)
    ...
    def test_reference_numerics_extremal(self, device, dtype, op):
        handles_extremals = (
            op.handles_complex_extremals
            if dtype in (torch.cfloat, torch.cdouble)
            else op.handles_extremals
        )
        if not handles_extremals:
            raise self.skipTest("This op does not handle extremal values")

        tensors = generate_numeric_tensors_extremal(device, dtype, domain=op.domain)
    ...
    def test_non_contig(self, device, dtype, op):
        ...
        for shape in shapes:
            contig = make_tensor(
                shape, device, dtype, low=op.domain[0], high=op.domain[1]
            )
    ...
    def test_non_contig_index(self, device, dtype, op):
        contig = make_tensor(
            (2, 2, 1, 2), device, dtype, low=op.domain[0], high=op.domain[1]
        )
        non_contig = contig[:, 1, ...]
    ...
    def test_contig_size1(self, device, dtype, op):
        contig = make_tensor(
            (5, 100), device, dtype, low=op.domain[0], high=op.domain[1]
        )
        contig = contig[:1, :50]
    ...
    def test_out_arg_all_dtypes(self, device, dtype, op):
        ...
        input = make_tensor(
            (64, 64), dtype=dtype, device=device, low=op.domain[0], high=op.domain[1]
        )
        torch_kwargs, _ = op.sample_kwargs(device, dtype, input)
    ...
    def test_frexp_out(self, device, dtype):
        ...
        outputs = (
            ...
            (
                torch.empty_like(input).transpose(0, 1),
                make_tensor((50, 50), device, torch.int, noncontiguous=True),
            ),
        )
    ...
    def test_isposinf_isneginf_complex(self, device, dtype):
        ...
        for torch_op in torch_ops:
            with self.assertRaisesRegex(
                RuntimeError, "does not support complex inputs"
            ):
                torch_op(t)
            with self.assertRaisesRegex(
                RuntimeError, "does not support complex inputs"
    ...
    def test_nonzero_noncontiguous(self, device, dtype):
        x = make_tensor(
            (10, 10, 10), dtype=dtype, device=device, low=1, noncontiguous=False
        )
    ...
    @dtypes(
        *(
            torch.testing.get_all_fp_dtypes(include_half=False)
            + torch.testing.get_all_complex_dtypes()
        )
    )
    @dtypesIfCUDA(
        *(
            torch.testing.get_all_fp_dtypes(include_half=True)
            + torch.testing.get_all_complex_dtypes()
        )
    )
