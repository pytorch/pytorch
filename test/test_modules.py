...
class TestModule(TestCase):
    ...
    def _test_forward(self, device, dtype, module_info, module_input):
        ...
        with freeze_rng_state():
            ...
            m = module_cls(*construct_args, **construct_kwargs)
            m.to(device).to(dtype)

            # === Do forward pass. ===
            forward_args, forward_kwargs = module_input.forward_input.compute()
            outputs = m(*forward_args, **forward_kwargs)

            # === Compare outputs to a reference if one is specified. ===
            # TODO: Handle precision
            reference_fn = module_input.reference_fn
            if reference_fn is not None:
                ref_outputs = reference_fn(m, *forward_args, **forward_kwargs, **construct_kwargs)
    ...
    def _test_pickle(self, device, dtype, module_info, module_input):
        ...
        with freeze_rng_state():
            ...
            # === Do forward pass. ===
            args, kwargs = module_input.forward_input.compute()
    ...
    def test_check_inplace(self, device, dtype, module_info, module_input):
