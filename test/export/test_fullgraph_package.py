...
class TestFullgraphPackage(TestCase):
    ...
    def test_basic_function(self):
        ...
        with torch.compiler.fullgraph_package(path=self.path()):
            f(torch.randn(3), torch.randn(3))

        self.assertTrue(os.path.exists(self.path()))

        inputs = torch.randn(3), torch.randn(3)
        with torch.compiler.fullgraph_package(mode="load", path=self.path()):
            results = f(*inputs)
    ...
    def test_basic_function_dynamo(self):
        ...
        inputs = torch.randn(3), torch.randn(3)
        with torch.compiler.fullgraph_package(mode="load", path=self.path(), frontend="dynamo"):
            results = f(*inputs)
    ...
    def test_multiple_compilations(self):
        ...
        with self.assertRaisesRegex(RuntimeError, "Multiple compilations to the same model name.*are not supported"):
            with torch.compiler.fullgraph_package(path=self.path()):
