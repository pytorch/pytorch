...
def _check_graph_nodes(test, gm1, gm2, _check_meta=True):
    ...
    for node1, node2 in zip(gm1.graph.nodes, gm2.graph.nodes):
        test.assertEqual(node1.op, node2.op)
        if node1.op == "call_function":
            ...
            if val1 is None or val2 is None:
                ...
                for v1, v2 in zip(pytree.tree_leaves(val1), pytree.tree_leaves(val2)):
                    if isinstance(v1, FakeTensor):
                        test.assertEqual(v1.shape, v2.shape)
                        test.assertEqual(v1.dtype, v2.dtype)
            else:
                # For expressions like 's0 < 10' can only compare through string
                test.assertEqual(str(val1), str(val2))
            ...
            if node1.target == torch.ops.higher_order.cond:
                ...
                _check_graph_nodes(test, true_graph1, true_graph2)

                false_graph1 = getattr(gm1, node1.args[2].target)
                false_graph2 = getattr(gm2, node2.args[2].target)
                _check_graph_nodes(test, false_graph1, false_graph2)
                ...
                _check_graph_nodes(test, map_graph1, map_graph2, False)
...
class TestDeserialize(TestCase):
    def check_graph(self, fn, inputs, dynamic_shapes=None, _check_meta=True) -> None:
        ...
        self.assertTrue(torch._dynamo.utils.same(orig_outputs, loaded_outputs))
        _check_graph_nodes(self, ep.graph_module, deserialized_ep.graph_module, _check_meta)
