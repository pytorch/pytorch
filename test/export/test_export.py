...
graph():
    ...
    def test_export_max_onnx_reported(self):
        ...
        ep = export(model, (x,y), dynamic_shapes=({0:DYN, 1:DYN},{0:DYN, 1:DYN}), strict=False)
        from torch.export import _trace 
        eeee = torch.export._trace._export(model, (x, y), dynamic_shapes=({0:DYN, 1:DYN},{0:DYN, 1:DYN}), strict=False, pre_dispatch=False)
        print("GRAPH", eeee.module().graph)
        self.assertTrue(torch.allclose(ep.module()(x, y), model(x, y)))
        x2 = torch.arange(4).reshape((2,2))
        y2 = torch.arange(9).reshape((3,3))
        model(x2, y2)
        def mine(gm, example_inputs):
            print("GRAOH COMPILE", gm.graph)
            return gm.forward
        opt_m = torch.compile(model, fullgraph=True, dynamic=True, backend="inductor")
        opt_m(x, y)
        opt_m(x2, y2)
...
def forward(self, p_linear_weight, p_linear_bias, b_buffer, x):
    ...
    def test_solver_unsupported_sympy_function(self):
        ...
        dynamic_shapes = {"x": {2: torch.export.Dim.AUTO, 3: torch.export.Dim.AUTO}, "y": {2: torch.export.Dim.AUTO, 3: torch.export.Dim.AUTO}}
