...
def forward(self, x):
    ...
    return pytree.tree_unflatten((getitem,), self._out_spec)""",
        ...
        mod.train()
        gm_train = torch.export._trace._export(mod, (inp,), pre_dispatch=True).module()
        self.assertExpectedInline(
...
def forward(self, x):
    ...
    def test_train_eval_on_exported_preautograd_module(self):
        ...
        graph_module = torch.export._trace._export(Foo(), (torch.ones(7, 5),), pre_dispatch=True).module()
        with self.assertRaisesRegex(
...
graph():
    ...
    def test_export_then_compile_tensor_ctor(self):
        ...
        exported_model = torch.export._trace._export(m, (tensor_cpu, mask_cpu), pre_dispatch=True).module()
