...
def forward(self, x):
    cos = torch.ops.aten.cos.default(x)
    auto_functionalized = torch.ops.higher_order.auto_functionalized(torch.ops.testlib.foo.default, x = x, z = cos, _x_aliases = [], _z_aliases = [], _all_aliased = []);  x = cos = None
...
def forward(self, x):
    ...
    auto_functionalized = torch.ops.higher_order.auto_functionalized(torch.ops.testlib.foo.default, x = cos, z = cos_1, _x_aliases = [], _z_aliases = [], _all_aliased = []);  cos = cos_1 = None
    getitem_3 = auto_functionalized[3];  auto_functionalized = None
