...
""",  # noqa: B950
    ...
    def test_cond_branch_tensor_constant_run_decompositions(self):
        ...
        ep = torch.export.export(M(), (torch.empty(()),), strict=False)
        ...
        for strict in (False, True):
            with self.subTest(strict=strict):
                used_ep = torch.export.export(
                    UsedConstant(), (torch.ones(()),), strict=strict
                )
                assert_no_branch_buffer_leak(used_ep, expected_constants)
                used_decomp_ep = used_ep.run_decompositions()
                self.assertEqual(
                    used_decomp_ep.module()(torch.ones(())), torch.tensor(3.0)
                )
                self.assertEqual(
                    used_decomp_ep.module()(torch.zeros(())), torch.tensor(4.0)
                )
        ...
        no_user_ep = torch.export.export(NoUserInputWithBuffer(), (), strict=False)
        no_user_constants = {
            "true_graph_0__tensor_constant0",
            "false_graph_0__tensor_constant0",
        }
        ...
        nested_ep = torch.export.export(NestedCond(), (torch.ones(()),), strict=False)
