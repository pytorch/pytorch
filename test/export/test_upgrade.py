...
def div__Scalar_mode_0_3(self: torch.Tensor, other: Any,  *, rounding_mode: Optional[str]=None) -> torch.Tensor:
    ...
    def test_div_upgrader_pass_return_new_op_after_retrace(self):
        ...
        # div.Tensor_mode is now decomposed into trunc + div.Tensor
        ...
        # furthermore, trunc gets decomposed to where + ceil + floor
        decomposed_op_count = count_op(upgraded_ep.graph, "aten::trunc")
        self.assertEqual(decomposed_op_count, 0)

        decomposed_op_count = count_op(upgraded_ep.graph, "aten::where.self")
        self.assertEqual(decomposed_op_count, 1)

        decomposed_op_count = count_op(upgraded_ep.graph, "aten::ceil")
        self.assertEqual(decomposed_op_count, 1)

        decomposed_op_count = count_op(upgraded_ep.graph, "aten::floor")
        self.assertEqual(decomposed_op_count, 1)
