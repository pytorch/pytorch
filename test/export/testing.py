...
def _check_meta(test: Any, gm1: torch.fx.GraphModule, gm2: torch.fx.GraphModule):
    def _filter_meta(meta: dict[str, Any]) -> dict[str, Any]:
        return {k: v for k, v in meta.items() if k in NODE_META_CHECK_LIST}

    test.assertEqual(len(gm1.graph.nodes), len(gm2.graph.nodes))
    for n1, n2 in zip(gm1.graph.nodes, gm2.graph.nodes):
        test.assertEqual(_filter_meta(n1.meta), _filter_meta(n2.meta))
...
def _check_ep(test: Any, ep1: ExportedProgram, ep2: ExportedProgram, args: tuple[Any]):
    ...
    _check_meta(test, gm1, gm2)
    _check_meta(test, ep1.graph_module, ep2.graph_module)
