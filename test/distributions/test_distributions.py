...
from typing import Any, NamedTuple, Optional, Protocol, TYPE_CHECKING, TypeVar, Union
...
if TYPE_CHECKING:
    ...
    # scipy does not export rv_generic publicly, so we approximate it with union.
    rv_generic: TypeAlias = Union[rv_continuous, rv_discrete]  # noqa: PYI042
...
class TestDistributions(DistributionsTestCase):
    ...
    def test_argmax_relaxed_categorical(self) -> None:
        ...
        class ScipyCategorical:
            def __init__(self, dist: "rv_generic") -> None:
                self.dist = dist
    ...
    @unittest.skipIf(not TEST_NUMPY, "Numpy not found")
    def test_mixture_same_family_normal_log_prob(self) -> None:
...
class TestAgainstScipy(DistributionsTestCase):
    def setUp(self) -> None:
        ...
        self.distribution_pairs: list[tuple[Distribution, rv_generic]] = [
...
class TestJit(DistributionsTestCase):
    ...
    def test_entropy(self) -> None:
        for Dist, keys, values, sample in self._examples():
            # FIXME: ExponentialFamily.entropy not compatible with JIT-tracing
            xfail = [Poisson]
            if Dist in xfail:
                continue

            def f(*args: Any) -> Tensor:
                param = dict(zip(keys, args))
                dist = Dist(**param)
                return dist.entropy()
