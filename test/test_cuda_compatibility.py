...
@unittest.mock.patch("torch.cuda.get_device_name", return_value="NVIDIA MOCK DEVICE")
@unittest.mock.patch("torch.cuda.device_count", return_value=1)
@unittest.mock.patch("torch.version.cuda", "12.6")
class TestCheckCapability(TestCase):
    @unittest.mock.patch("torch.version.cuda", None)
    ...
    @unittest.mock.patch(
        "torch.cuda.get_arch_list", return_value=["sm_70", "sm_80", "sm_90"]
    )
    @unittest.mock.patch("torch.cuda.get_device_capability", return_value=(8, 0))
    def test_compatible_device_no_warning(self, *args):
        with warnings.catch_warnings():
            warnings.simplefilter("error")
            torch.cuda._check_capability()

    @unittest.mock.patch("torch.cuda.get_arch_list", return_value=["sm_80"])
    @unittest.mock.patch("torch.cuda.get_device_capability", return_value=(7, 5))
    ...
    @unittest.mock.patch("torch.cuda.get_arch_list", return_value=["sm_80"])
    @unittest.mock.patch("torch.cuda.get_device_capability", return_value=(8, 7))
    ...
    @unittest.mock.patch("torch.cuda.get_arch_list", return_value=["sm_80", "sm_90"])
    def test_multiple_devices_mixed_compatibility(self, *args):
        caps = [(8, 0), (7, 5), (8, 6)]
        with unittest.mock.patch("torch.cuda.device_count", return_value=len(caps)):
            with unittest.mock.patch(
                "torch.cuda.get_device_capability", side_effect=caps
            ):
                with warnings.catch_warnings(record=True) as w:
                    warnings.simplefilter("always")
                    torch.cuda._check_capability()
    ...
    @unittest.mock.patch("torch.cuda.get_arch_list", return_value=["sm_60"])
    @unittest.mock.patch("torch.cuda.get_device_capability", return_value=(7, 0))
    ...
    @unittest.mock.patch("torch.cuda.get_arch_list", return_value=["sm_80"])
    @unittest.mock.patch("torch.cuda.get_device_capability", return_value=(5, 3))
