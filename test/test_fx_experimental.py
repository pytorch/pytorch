...
from torch.fx.experimental.symbolic_shapes import ShapeEnv, DimConstraints
...
terrible spacing
    ...
    def test_prettify_results(self):
        import inspect
        from torch._dynamo.source import LocalSource

        symbol_to_source: dict[sympy.Symbol, list] = {}
        var_to_val: dict[sympy.Symbol, sympy.Integer] = {}
        marked_dynamic: set[sympy.Symbol] = set()
        ...
        # Empty results
        result = dim_constraints.prettify_results(sig, {}, None, {})
        self.assertEqual(result, "")
        
        # Forced specializations
        s0 = sympy.Symbol("s0", integer=True, positive=True)
        source0 = LocalSource("input.size()[0]")
        symbol_to_source_forced = {s0: [source0]}
        var_to_val_forced = {s0: sympy.Integer(4)}
        source_name_to_debug_name_forced = {"input.size()[0]": "s0"}

        dim_constraints_forced = DimConstraints(
            symbol_to_source_forced,
            var_to_val_forced,
            set(),
            source_name_to_debug_name_forced,
        )

        result = dim_constraints_forced.prettify_results(
            sig, {"s0": torch.export.Dim("s0")}, None, {"s0 = input.size()[0]": "4"}
        )
        self.assertIn("Specializations unexpectedly required", result)
        self.assertIn("s0 = input.size()[0]", result)
        
        # Min/max constraints
        s1 = sympy.Symbol("s1", integer=True, positive=True)
        source = LocalSource("x.size()[0]")
        symbol_to_source2 = {s1: [source]}
        var_to_val2 = {s1: sympy.Integer(5)}
        source_name_to_debug_name2 = {"x.size()[0]": "dx"}

        dim_constraints2 = DimConstraints(
            symbol_to_source2,
            var_to_val2,
            set(),
            source_name_to_debug_name2,
        )
        # Manually add dynamic results that would be generated by solve()
        dim_constraints2._dynamic_results.add("x.size()[0] >= 3")
        dim_constraints2._dynamic_results.add("x.size()[0] <= 10")

        result = dim_constraints2.prettify_results(sig, {"dx": torch.export.Dim("dx")}, None, {})
        ...
        source2 = LocalSource("y.size()[0]")
        symbol_to_source3 = {s2: [source2]}
        var_to_val3 = {s2: sympy.Integer(5)}
        source_name_to_debug_name3 = {"y.size()[0]": "dy"}

        dim_constraints3 = DimConstraints(
            symbol_to_source3,
            var_to_val3,
            set(),
            source_name_to_debug_name3,
        )

        # Add a static result (equality/specialization)
        dim_constraints3._static_results.add("y.size()[0] == 5")

        result = dim_constraints3.prettify_results(sig, {"dy": torch.export.Dim("dy")}, None, {})
