...
TEST(AotiTorchStableIValue, TestStableIValueUse) {
  ...
  ASSERT_NE(a, nullptr);
  // Assign to verify that we can actually write to the full u64 size, this will show up in valgrind
  // if we were to allocate less than the required size.
...
TEST(AotiTorch, TestStableIValueLargeAmount) {
  // Next, do a bunch of allocations and frees, such that if the delete isn't working but the allocation is we see a
  // clear memory leak in valgrind.
  StableIValue* v = nullptr;
  const size_t total_allocation_bytes = 10'000'000;
  const size_t number_of_allocations = total_allocation_bytes / sizeof(StableIValue);
