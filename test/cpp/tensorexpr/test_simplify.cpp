...
void testHashLargeExpression() {
  ...
  ExternalTensor d(BufHandle("D", {1}, kInt));
  ExternalTensor e(BufHandle("E", {1}, kInt));
...
void testSimplifyConstantCond() {
...

  {
    // If the condition is constant true then take the true_value.
    // 1 ? A[0] = 1 : B[0] = 1 => A[0] = 1
    ExternalTensor a(BufHandle("A", {1}, kInt));
    ExternalTensor b(BufHandle("B", {1}, kInt));
...
void testSimplifyEliminateZeroLengthFor() {
...

  {
    ...
    ExternalTensor a(BufHandle("A", {4}, kInt));
    ExternalTensor c(BufHandle("C", {4}, kInt));
    auto mask = IntImm::make(1);
    VarHandle i("i", kInt);
    auto body =
...
void testSimplifyForCleansUp() {
  ...
  {
    ExternalTensor a("a", kFloat, {1, 12, 1});
    VarHandle x("x", kInt);
    Tensor* b = Compute(
        "x",
        {{1, "i"}, {12, "m"}, {1, "n"}},
...
void testSimplifyFuseConditions() {
  KernelScope kernel_scope;
  ExternalTensor a(BufHandle("A", {2}, kInt));
  ExternalTensor b(BufHandle("B", {2}, kInt));
  auto mask = IntImm::make(1);
  VarHandle i("i", kInt);
  VarHandle j("j", kInt);
...
void testSimplifyBroadcastTermExpander() {
  ...
  ExternalTensor buf(BufHandle("buf", {num_lanes}, kInt));
