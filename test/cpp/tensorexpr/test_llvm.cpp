...
void testLLVMLetTest01() {
  ...
  ExternalTensor a(BufHandle("A", {1}, kFloat));
...
void testLLVMLetTestMultitype() {
  ...
  ExternalTensor a(BufHandle("A", {1}, kDouble));
...
void testLLVMBufferTest() {
  KernelScope kernel_scope;
  ExternalTensor a(BufHandle("A", {32}, kFloat));
...
void testLLVMBlockTest() {
  KernelScope kernel_scope;
  ExternalTensor a(BufHandle("A", {32}, kInt));
...
void testLLVMIfThenElseTest() {
  ...
  ExternalTensor a(BufHandle("A", {1}, kInt));
  ExternalTensor b(BufHandle("B", {1}, kInt));
  ExternalTensor c(BufHandle("C", {1}, kInt));
  std::vector<int32_t> a_buffer = {42};
  std::vector<int32_t> b_buffer = {-11};
...
#define FLOAT_INTRINSICS_TEST(Name, Lanes)                       \
  void testLLVMVecFloat_##Name##Lane##Lanes##Test() {            \
    KernelScope kernel_scope;                                    \
    ExternalTensor a(BufHandle("A", {1}, kFloat));               \
    ExternalTensor b(BufHandle("B", {1}, kFloat));               \
    float val = 0.5f;                                            \
...
#define DOUBLE_INTRINSICS_TEST(Name, Lanes)                      \
  void testLLVMVecDouble_##Name##Lane##Lanes##Test() {           \
    KernelScope kernel_scope;                                    \
    ExternalTensor a(BufHandle("A", {1}, kDouble));              \
    ExternalTensor b(BufHandle("B", {1}, kDouble));              \
    float val = 0.5f;                                            \
...
void testLLVMCompareSelectByteGT() {
  KernelScope kernel_scope;
  constexpr int N = 1024;
  ExternalTensor a(BufHandle("A", {N}, kByte));
  ExternalTensor b(BufHandle("B", {N}, kByte));
  ExternalTensor c(BufHandle("C", {N}, kInt));
...
void testLLVMStoreFloat() {
  KernelScope kernel_scope;
  ExternalTensor result(BufHandle("result", {1}, kFloat));
...
void testLLVMSimpleMath01() {
  ...
  ExternalTensor f_buf(BufHandle(tensor->func_var()));
