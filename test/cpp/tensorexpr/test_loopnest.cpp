...
void testExprSplitWithMask01() {
  ...
  const int M = 26;
  const int N = 5;
  ExternalTensor a_buf("a", kFloat, {M, N});
  ExternalTensor b_buf("b", kFloat, {M, N});
  Tensor* tensor = Compute(
...
void testExprSplitWithMaskRepeatedNoMask() {
  ...
  ExternalTensor b_buf("b", kFloat, {M});
...
void testScheduleBroadcastAddBuffer() {
  ...
  const int K = 6;
  ExternalTensor a_buf("a", kFloat, {M, N});
  ExternalTensor b_buf("b", kFloat, {N, K});
  Tensor* c = Compute(
      "broadcast_add",
      {{M, "m"}, {N, "n"}, {K, "k"}},
...
void testScheduleInlineSimple() {
  ...
  ExternalTensor a_buf("a", kFloat, {M, N});
  ExternalTensor b_buf("b", kFloat, {N, K});
  ExternalTensor c_buf("c", kFloat, {M, N});
  ExternalTensor d_buf("d", kFloat, {M, K});
...
void testScheduleFuserThreeArg() {
  ...
  ExternalTensor a(BufHandle("A", {ExprHandle(kTotalSize)}, kFloat));
  ExternalTensor b(BufHandle("B", {ExprHandle(kTotalSize)}, kFloat));
  ExternalTensor c(BufHandle("C", {ExprHandle(kTotalSize)}, kFloat));
  ExternalTensor d(BufHandle("D", {ExprHandle(kTotalSize)}, kFloat));
...
void testLoopNestReorderExtraStatements() {
  ...
  ExternalTensor extra(BufHandle("res", {6, 3}, kFloat));
...
void LoopNestReorderTestHelper(
  ...
  ExternalTensor extra(BufHandle("extra", {5}, kInt));
...
void testDetectInlineRankMismatch() {
  ...
  ASSERT_THROWS_WITH(
      ...
      "ExternalTensor indexed access is inconsistent with its rank");
}
