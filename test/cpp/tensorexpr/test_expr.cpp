...
void testExprLetStmtTest01() {
  KernelScope kernel_scope;
  ExternalTensor a_buf("a", kFloat, {1});
...
void testExprCompareSelectDtypes() {
  ...
  ExternalTensor a(BufHandle("A", {N}, kInt));
  ExternalTensor b(BufHandle("B", {N}, kInt));
  ExternalTensor c(BufHandle("C", {N}, kFloat));
  std::vector<int> a_buffer(N, 1);
  std::vector<int> b_buffer(N, 1);
...
void testExprIntrinsicsDtypes() {
  KernelScope kernel_scope;
  constexpr int N = 256;
  ExternalTensor a(BufHandle("A", {N}, kDouble));
  ExternalTensor b(BufHandle("B", {N}, kDouble));
...
void testExprDynamicShapeAdd() {
  ...
  auto testWithSize = [](int32_t size) {
    ...
    ExternalTensor b(BufHandle("b", {n}, kFloat));
    ExternalTensor c(BufHandle("c", {n}, kFloat));
    VarHandle i("i", kInt);
