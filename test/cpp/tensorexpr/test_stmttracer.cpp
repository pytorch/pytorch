...
TEST(StmtTracer, BasicTest) {
  ...
  for (auto& acc : accesses) {
    ...
    auto trace = std::get<2>(acc).getTraceString();

    // check the stmtTrace of aInit
    if (store == aInit) {
      if (trace == "0:0:") {
        pass += 1;
      }
    ...
    // check the stmtTrace of aReduce
    if (store == aReduce) {
      if (trace == "0:1:0:") {
        pass += 1;
      }
...
TEST(StmtTracer, CondTest) {
  ...
  //   for (int i = 0; i < 32; i++) {
  //     if (i<10 ? 1 : 0) {
  //       a[i] = i + i; // StmtTrace: 0:0
  //       b[i] = i * i; // StmtTrace: 0:0
  //     }
  //   }
  // }
  ...
  // check the stmtTrace of aStore
  auto acc_a = BufAccesses::find(stmt, a.node()).at(0);
  auto trace_a = std::get<2>(acc_a).getTraceString();
  ASSERT_TRUE(trace_a == "0:0:");

  // check the stmtTrace of bStore
  auto acc_b = BufAccesses::find(stmt, b.node()).at(0);
  auto trace_b = std::get<2>(acc_b).getTraceString();
  ASSERT_TRUE(trace_b == "0:0:");
}
