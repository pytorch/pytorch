...
void testRecordFunction() {
  ...
  { // START: global test
    ...
    addGlobalCallback(RecordFunctionCallback(
        [test_val, test_str, &ids](const RecordFunction& fn) {
        ...
        [test_val, test_str](const RecordFunction&, ObserverContext* ctx_ptr) {
  ...
  clearCallbacks();
  }  // END: thread local / global context check callbacks
