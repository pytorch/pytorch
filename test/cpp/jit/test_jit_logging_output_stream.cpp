...
TEST(JitLoggingOutputStreamTest, CheckDefaultStream) {
  ...
  ASSERT_EQ(::torch::jit::get_jit_logging_output_stream().rdbuf(), static_cast<std::ostringstream&>(std::cerr).rdbuf());
}
...
TEST(JitLoggingOutputStreamTest, CheckSetLoggingOutputStream) {
  ...
  ASSERT_EQ(::torch::jit::get_jit_logging_output_stream().rdbuf(), stream.rdbuf());
}
...
TEST(JitLoggingOutputStreamTest, CheckSetLoggingOutputStreamStdout) {
  ...
  ASSERT_EQ(::torch::jit::get_jit_logging_output_stream().rdbuf(), stdout.rdbuf());
  ASSERT_EQ(&::torch::jit::get_jit_logging_output_stream(), &std::cout);
  ASSERT_EQ(::torch::jit::get_jit_logging_output_stream().rdbuf(), static_cast<std::ostringstream&>(std::cout).rdbuf());
}
