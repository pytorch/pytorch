...
std::vector<std::string> values_to_value_ids(
...
graph(%x.1 : Tensor,
  ...
  %6 : Tensor = aten::add(%x.1, %y.1, %3)
  %7 : Tensor = prim::If(%z.1)
    block0():
      %8 : Tensor = aten::mul(%x.1, %4)
      -> (%8)
    block1():
      %9 : Tensor = aten::mul(%y.1, %5)
      %10 : Tensor = aten::add(%9, %4, %3)
      -> (%10)
  %11 : Tensor = aten::mul(%7, %6)
  return (%11))IR",
      &*graph);
  ...
  std::vector<std::string> expected_liveness_if{
      "x.1", "y.1", "z.1", "3", "4", "5", "6"};
  std::vector<std::string> expected_first_node_liveness{"x.1", "y.1", "z.1"};

  auto liveness = BuildLivenessSets(graph);
...
graph(%n.1 : int,
  ...
  %5 : int, %6 : int, %7 : Tensor = prim::Loop(%n.1, %2, %3, %3, %x.1)
    block0(%i.1 : int, %sum.7 : int, %sum2.5 : int, %c.5 : Tensor):
      %12 : int = aten::add(%sum.7, %i.1)
      %13 : int = aten::add(%i.1, %4)
      %14 : int = aten::add(%sum2.5, %13)
      %15 : int = aten::add(%14, %i.1)
      %16 : Tensor = aten::mul(%c.5, %i.1)
      -> (%2, %12, %15, %16)
  %17 : int = aten::add(%5, %6)
  %18 : (int, Tensor, Tensor) = prim::TupleConstruct(%17, %7, %x.1)
  return (%18)
)IR",
      &*graph);

  std::vector<std::string> expected_for{"n.1", "x.1", "2", "3", "4"};
  std::vector<std::string> expected_first_node_liveness{"n.1", "x.1"};
