...
struct ForEachAddFuser {
  ForEachAddFuser(std::shared_ptr<Graph> graph)
  ...
  public:
    ...
    void createFusionGroups(Block * b) {
      // iterate over Nodes in graph, trying horizontall merge add nodes with the same scalar
      ...
      for (auto it = b->nodes().begin(); it != b->nodes().end();) {
        ...
        if (!node->matches("aten::add.Scalar(Tensor self, Scalar other, Scalar alpha=1) -> Tensor",
        ...
        if (curr_fusion_graph && scalar_arg == curr_scalar_arg && alias_db_->moveAfterTopologicallyValid(node, curr_fusion_graph)) {
          SubgraphUtils::mergeNodeIntoSubgraphAndUpdateAliasing(node, curr_fusion_graph, *alias_db_);
          continue;
        }
    ...
    void guardFusionGroups() {
      for (Node * n: fusion_groups_) {
        ...
        // to do this (cuda-only) optimization, we need to guard that the inputs are cuda
        // If the guards fail, then we re-profile the fused graph nodes
        // NB: if we wanted to guard on complete shapes/strides/etc, we would return
        // profiled_type
        // To guard without strides, we remove strides from the input:
        // profiled_type->withStrides({});
        // To guard on number of dimensions, we would remove complete sizes
...
graph(%x.1 : Tensor,
  ...
  for (size_t i = 0 ; i < 3; ++i) {
    ASSERT_TRUE(at::allclose(tup_output->elements().at(i).toTensor(), at::add(inps[i], scalar_input)));
  }
  ASSERT_TRUE(at::allclose(tup_output->elements().at(3).toTensor(), at::add(inps[0], inps[1])));
