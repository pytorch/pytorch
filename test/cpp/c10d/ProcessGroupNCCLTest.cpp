...
class NCCLTest : public NCCLTestBase {
 ...
 protected:
  ...
  at::Tensor to_sparse_row_indices_format(at::Tensor& tensor) {
      ...
      auto [row_indices, _] = at::_unique(tensor.nonzero().select(/*dim=*/1, /*index=*/0));
      at::Tensor sparse_values = tensor.index_select(/*dim=*/0, row_indices);  // get the values at the non-zero indices
      return at::sparse_coo_tensor(row_indices.unsqueeze(0), sparse_values, tensor.sizes()).to(tensor.device());
  }
