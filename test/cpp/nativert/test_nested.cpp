...
#include <torch/nativert/detail/NestedStructure.h>
...
TEST(NestedStructureTest, Unflatten) {
...
[
  ...
  const auto spec = nestedSpecLoads(jsonSpec, valuePtrs);
  ...
  auto nested = nestedUnflatten(flats, spec);
  EXPECT_TRUE(nested.isList());
  EXPECT_EQ(nested.toListRef().size(), 5);

  EXPECT_TRUE(nested.toListRef().at(0).isTuple());
  EXPECT_EQ(
      nested.toListRef().at(0).toTupleRef().elements()[0], c10::IValue(0));
  EXPECT_EQ(
      nested.toListRef().at(0).toTupleRef().elements()[1], c10::IValue(1));

  EXPECT_TRUE(nested.toListRef().at(1).isInt());
  EXPECT_EQ(nested.toListRef().at(1), c10::IValue(2));

  EXPECT_TRUE(nested.toListRef().at(2).isGenericDict());
  EXPECT_EQ(nested.toListRef().at(2).toGenericDict().at("4"), c10::IValue(7));
  EXPECT_EQ(nested.toListRef().at(2).toGenericDict().at("5"), c10::IValue(8));
  EXPECT_EQ(nested.toListRef().at(2).toGenericDict().at("6"), c10::IValue(9));

  EXPECT_TRUE(nested.toListRef().at(3).isList());
  EXPECT_EQ(nested.toListRef().at(3).toListRef().at(0), c10::IValue(10));

  EXPECT_TRUE(nested.toListRef().at(4).isGenericDict());
  EXPECT_EQ(nested.toListRef().at(4).toGenericDict().at("11"), c10::IValue(12));

  const auto flattened = nestedFlatten(nested, spec);
...
TEST(NestedStructureTest, NoVersion) {
  auto jsonSpec = R"(
  {
    "type": "builtins.list",
    "context": "null",
  ...
  EXPECT_THROW({ nestedSpecLoads(jsonSpec, valuePtrs); }, std::exception);
}

TEST(NestedStructureTest, NoField) {
  auto jsonSpec = R"(
[
  1,
  {
...
TEST(NestedStructureTest, NoContext) {
  auto jsonSpec = R"(
[
  1,
  {
  ...
  auto [graph, valuePtrs] = makeValues(3);
  auto spec = nestedSpecLoads(jsonSpec, valuePtrs);
  ...
  ASSERT_DEATH({ nestedUnflatten(flats, spec); }, "Check failed");
}

TEST(NestedStructureTest, TooManyContext) {
  auto jsonSpec = R"(
[
  1,
  {
...
TEST(NestedStructureTest, DoubleRegister) {
  EXPECT_THROW(
      { registerPytreeNode("builtins.dict", NodeDef{}); }, std::exception);
}

TEST(NestedStructureTest, NotEnoughUnflatten) {
...
TEST(NestedStructureTest, TooManyUnflatten) {
  // Original data: [(0, 1), 2, {"4": 7, "5": 8, "6": 9}]
  auto jsonSpec = R"(
[
  1,
...
TEST(NestedStructureTest, Flatten) {
  // Original data: [(0, 1), 2, {"4": 7, "5": 8, "6": 9}, (10,), {"11": 12}]
  auto jsonSpec = R"(
[
  1,
  ...
  auto flats = nestedFlatten(c10::IValue{list}, spec);
...
TEST(NestedStructureTest, IValueApplyFromArgs) {
...
return(%o1)
    ...
    const auto spec = nestedSpecLoads(jsonSpec, userInputs);
...
TEST(NestedStructureTest, UnmatchedFlattenType) {
  // Original data: [(0, 1), 2, {"4": 7, "5": 8, "6": 9}]
  auto jsonSpec = R"(
[
  1,
  ...
  dict.insert("6", c10::IValue(9));
  EXPECT_THROW(
      { nestedFlatten(c10::IValue{std::move(dict)}, spec); }, std::exception);
}

TEST(NestedStructureTest, UnmatchedDictFlatten) {
  // Original data: [(0, 1), 2, {"4": 7, "5": 8, "6": 9}]
  auto jsonSpec = R"(
[
  1,
  ...
  list.push_back(std::move(dict));
  ASSERT_DEATH(
      { nestedFlatten(c10::IValue{std::move(list)}, spec); }, "Check failed");
}

TEST(NestedStructureTest, DictFlattenTest) {
  auto jsonSpec = R"(
[
  1,
  {
  ...
  c10::List<c10::IValue> list(c10::AnyType::get());
  list.push_back(std::move(dict));
  nestedFlatten(c10::IValue{std::move(list)}, spec);
}

TEST(NestedStructureTest, UnmatchedTupleFlatten) {
  // Original data: [(0, 1), 2, {"4": 7, "5": 8, "6": 9}]
  auto jsonSpec = R"(
[
  1,
...
TEST(NestedStructureTest, ToAtenType) {
