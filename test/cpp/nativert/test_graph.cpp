...
return(%o2, %bar)
  ...
  graph->addValue(v0, Type::None, fooNode);
  auto v1 = graph->getUniqueValueName();
  graph->addValue(v1, Type::None, fooNode);
...
TEST(NodeTest, GetInputAndAttribute) {
  auto graph = Graph::createGraph();
  auto input1 = graph->addInput("input1", Type::Tensor);
  auto input2 = graph->addInput("input2", Type::Tensor);
  auto input3 = graph->addInput("input3", Type::Tensor);
...
TEST(GraphTest, RemoveValue) {
...

  {
    ...
    EXPECT_DEATH(
        { graph->removeValue(val_o1); }, "Cannot erase a value with users");
  }
