...
TEST_F(SequentialTest, AccessWithAt) {
  ...
  for (size_t i = 0; i < modules.size(); ++i) {
    ASSERT_TRUE(&sequential->at<M>(i) == modules[i].get());
  }
...
TEST_F(SequentialTest, AccessWithPtr) {
  ...
  // returns the correct module for a given index
  for (size_t i = 0; i < modules.size(); ++i) {
    ASSERT_TRUE(sequential->ptr(i).get() == modules[i].get());
    ASSERT_TRUE(sequential[i].get() == modules[i].get());
    ASSERT_TRUE(sequential->ptr<M>(i).get() == modules[i].get());
  }
...
TEST_F(SequentialTest, CallingForwardChainsCorrectly) {
  ...
  ASSERT_TRUE(sequential->forward<int>(1) == 4);
}
...
TEST_F(SequentialTest, CallingForwardWithTheWrongReturnTypeThrows) {
  ...
  ASSERT_TRUE(sequential->forward<int>() == 5);
  ASSERT_THROWS_WITH(
...
TEST_F(SequentialTest, ForwardReturnsTheLastValue) {
  ...
  ASSERT_TRUE(y.size(0) == 1000);
  ASSERT_TRUE(y.size(1) == 100);
}
...
TEST_F(SequentialTest, IsCloneable) {
  ...
  ASSERT_TRUE(sequential->size() == clone->size());

  for (size_t i = 0; i < sequential->size(); ++i) {
    // The modules should be the same kind (type).
    ASSERT_TRUE(sequential[i]->name() == clone[i]->name());
    // But not pointer-equal (distinct objects).
    ASSERT_TRUE(sequential[i] != clone[i]);
  }
  ...
  for (auto& param : params1) {
    ...
    ASSERT_TRUE(param->device() == params2[param.key].device());
...
TEST_F(SequentialTest, RegistersElementsAsSubmodules) {
  ...
  ASSERT_TRUE(modules.size() == sequential->children().size());
...
TEST_F(SequentialTest, CloneToDevice_CUDA) {
  ...
  for (const auto& p : clone->parameters()) {
    ASSERT_TRUE(p->device() == device);
  }
  for (const auto& b : clone->buffers()) {
    ASSERT_TRUE(b->device() == device);
  }
}
