...
//         return std::to_string(std::accumulate(input.begin(), input.end(),
//         0));
//       }));
...
//       torch::kFloat32).allclose(dataset.get(2)));
// }
//
...
//   ASSERT_TRUE(first.target.allclose(1 + torch::eye(4).slice(/*dim=*/0, 0,
//   2)));
//
//   Example<> second = d.get_batch({2, 3});
//   ASSERT_TRUE(second.data.allclose(torch::eye(4).slice(/*dim=*/0, 2, 4)));
//   ASSERT_TRUE(second.target.allclose(1 + torch::eye(4).slice(/*dim=*/0, 2,
//   4)));
// }
//
...
//     return {torch::tensor(static_cast<double>(index)),
//     std::to_string(index)};
//   }
//
//   torch::optional<size_t> size() const override {
...
//   std::vector<Example<torch::Tensor, std::string>> batch = d.get_batch({1,
//   2});
//
...
//   DummyDataset dataset;
//   auto data_loader =
//       torch::data::make_data_loader(dataset,
//       DataLoaderOptions(1).workers(2));
//   auto i = data_loader->begin();
//   ASSERT_THROWS_WITH(
//       data_loader->begin(),
...
//   start); ASSERT_LT(duration.count(), 1);
// }
//
// //
...
// // `enforce_ordering` option of the dataloader works correctly. The reason
// this
...
// // thread (for outside consumption) is not deterministic. Imagine the sampler
// is
// // a SequentialSampler with indices 0, 1, 2, 3. With batch size 1, each index
// // will be a single "job". Inside the dataloader, worker threads block until
// a
// // job is available. It is not deterministic which worker thread wakes up
// first
// // to dequeue a particular batch. Further, some worker threads may take
// longer
...
// // the main thread. In that case, the dataloader iterator would return the
// datum
// // at index 2 first, and afterwards the datum from whatever thread finishes
// // next. As such, the user may see data from indices 2, 0, 3, 1. On another
// run
// // of the same dataloader on the same data, threads may be scheduled
// differently
// // and return in order 0, 2, 3, 1. To force this ordering to
// deterministically
// // be 0, 1, 2, 3, the `enforce_ordering` flag can be set to true. In that
// case,
// // the dataloader will use a *sequencer* internally which keeps track of
// which
// // datum is expected next, and buffers any other results until that next
// // expected value arrives. For example, workers 1, 2, 3 may finish before
// worker
// // 0. If `enforce_ordering` is true, the sequencer will internally buffer the
// // results from 1, 2, 3 until worker 0 finishes. Only then does the
// dataloader
// // return the datum from worker 0 to the user (and then datum 1 the next
// time,
// // then 2 and so on).
// //
// // The way the test works is that we start
// // `kNumberOfWorkers` workers in the dataloader, which each get an index from
// a
// // `SequentialSampler` in the range `0...kNumberOfWorkers-1`. Each worker
// thread
// // has a copy of the dataset, and thus `get_batch()` is called on the
// // thread-local copy in each worker. We want to simulate out-of-order
// completion
// // of these threads. For this, we first set a barrier in the `get_batch()`
// // method to make sure every worker has some index to fetch assigned.
// Further,
...
// // which we want the worker threads to return. For this, an iterator into
// this
// // order is maintained. When the derferenced iterator (the current order
// index)
...
//     static auto order_iterator =
//     kOrderInWhichWorkersReturnTheirBatch.begin(); static
//     std::condition_variable cv; static std::mutex mutex;
//
...
//     ASSERT_LT(t.item<int64_t>(),
//     kNumberOfExamplesAfterWhichTheDatasetExhausts);
//   }
// }
