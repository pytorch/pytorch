...
TEST(CustomAutogradTest, DeepReentrant) {
  ...
  auto v = torch::tensor({8193}, torch::dtype(torch::kDouble).requires_grad(true));
...
TEST(CustomAutogradTest, ReentrantPriority) {
  ...
  auto a = MyFunction::apply(torch::tensor({6}, torch::dtype(torch::kDouble).requires_grad(true)));
  auto b = Reenter::apply(torch::tensor({9}, torch::dtype(torch::kDouble).requires_grad(true)));
  auto v = a*b;
  v.backward();
