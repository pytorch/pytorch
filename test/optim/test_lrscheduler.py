...
class TestLRScheduler(TestCase):
    ...
    def test_raise_error_when_last_epoch_is_greater_than_0_and_initial_lr_is_not_specified(self):
        optimizer = SGD([Parameter(torch.randn(2, 2, requires_grad=True))], 0.1)
        with self.assertRaisesRegex(KeyError, r"param \'initial_lr\' is not specified in param_groups\[0\] when resuming scheduler with last_epoch >= 0"):
