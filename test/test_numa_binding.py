...
class NumaBindingTest(TestCase):
    def setUp(self) -> None:
        ...
        self._context_managers_to_apply_to_all_tests = [
            ...
            patch("tempfile.mkstemp", self._mock_mkstemp),
        ]
    ...
    def test_callable_entrypoint_basic(self) -> None:
        ...
        executable_lines = executable_contents.split("\n")
        self.assertEqual(executable_lines[0], "#!/bin/bash")
        self.assertEqual(executable_lines[1], 'rm -- "$0"')
        self.assertEqual(
            executable_lines[2], f'numactl --cpunodebind=5 {sys.executable} "$@"'
        )
