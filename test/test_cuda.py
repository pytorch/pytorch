...
"""])
    ...
    # Despite the large number of RNN cases it tries, the test takes < 5 seconds on a Titan V (similar to V100).
    @unittest.skipIf(not TEST_CUDNN, 'CUDNN not available')
    def test_autocast_rnn(self):
        with torch.backends.cudnn.flags(enabled=True, deterministic=True):
            ...
            # TODO BEFORE MERGE:  fix bias = False backward
            for cls in ("RNN", "GRU", "LSTM"):
                for (num_layers, bias, input_layout, bidirectional, input_dtype, hidden_dtype, weight_dtype) in \
                        product((1, 2), (True,), input_layouts, (True, False), dtypes, dtypes, dtypes):
