...
from torch.sparse.semi_structured import to_semi_structured_sparse, gen_semi_structured_sparse_mask
...
class TestSemiStructuredSparse(TestCase):
    ...
    def test_mm_sparse_first_NT_T(self, dtype, device):
        ...
        if dtype is torch.int8:
            dense_result = torch.mm(A.cpu(),
                                    B.T.cpu()).to(dtype).to(device)
        else:
