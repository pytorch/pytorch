...
class TestStatefulPRNG(TestCase):
    ...
    def test_matches_stateless_f64_uniform(self, device):
        """Float64 draws 2 values per call, so the stream joins at even sizes."""
        g = random.StatefulPRNG(42, device=device)
        first = g.uniform(16, dtype=torch.float64)
        second = g.uniform(34, dtype=torch.float64)
    ...
    def test_matches_stateless_f64_normal(self, device):
        """Float64 draws 2 values per call, so the stream joins at even sizes."""
        g = random.StatefulPRNG(42, device=device)
        first = g.normal(16, dtype=torch.float64)
        second = g.normal(34, dtype=torch.float64)
    ...
    def test_partial_call_is_skipped_not_replayed(self, device, dtype):
        # A size that is not a whole number of philox calls rounds the offset
        # up: the tail of the final call is discarded, never reused.
        g = random.StatefulPRNG(42, device=device)
        first = g.uniform(5, dtype=dtype)
        second = g.uniform(5, dtype=dtype)
        self.assertEqual(len(set(torch.cat([first, second]).tolist())), 10)

        k = random.key(42, device=device)
        elems_per_call = 2 if dtype == torch.float64 else 4
        skipped = -(-5 // elems_per_call) * elems_per_call
        full = random.uniform(k, (skipped + 5,), dtype=dtype)
        self.assertEqual(second, full[skipped:])
