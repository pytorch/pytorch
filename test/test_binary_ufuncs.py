...
class TestBinaryUfuncs(TestCase):
    ...
    def test_ldexp_pow2_float16_cases(self, device, dtype):
        ...
        for case in test_cases:
            ...
            self.assertTrue(
                torch.allclose(result_func, expected, atol=1e-8)
                or (torch.isinf(expected) and torch.isinf(result_func)),
            ...
            a_clone.ldexp_(b)
            self.assertTrue(
                torch.allclose(a_clone, expected, atol=1e-8)
                or (torch.isinf(expected) and torch.isinf(a_clone)),
            ...
            self.assertTrue(
                torch.allclose(out_tensor, expected, atol=1e-8)
                or (torch.isinf(expected) and torch.isinf(out_tensor)),
