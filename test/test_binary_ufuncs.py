...
class TestBinaryUfuncs(TestCase):
    ...
    def test_complex_scalar_pow_tensor(self, device, dtype):
        ...
        second_exp = make_tensor(
            (100,), device, dtype, low=-2, high=2, noncontiguous=True
        )
    ...
    @dtypes(
        *product(torch.testing.get_all_complex_dtypes(), torch.testing.get_all_dtypes())
    )
    ...
    def test_sub(self, device, dtype):
        ...
        if dtype == torch.bool:
            ...
            self.assertEqual(
                m1 - m2, torch.tensor([1.11, 2.11], dtype=dtype), atol=0.01, rtol=0
            )
        else:
    ...
    @dtypes(
        *torch.testing.get_all_dtypes(
            include_bfloat16=False, include_bool=False, include_complex=False
        )
    )
    ...
    @dtypes(
        *tuple(
            itertools.combinations_with_replacement(torch.testing.get_all_dtypes(), 2)
        )
    )
    ...
    @dtypes(
        *product(
            torch.testing.get_all_dtypes(
                include_complex=False, include_half=False, include_bfloat16=False
            ),
            torch.testing.get_all_dtypes(
                include_complex=False, include_half=False, include_bfloat16=False
            ),
