...
class TestSparseCSR(TestCase):
    ...
    def test_sparse_csr_constructor_shape_inference(self, device, dtype):
        ...
        self.assertEqual(
            torch.tensor(crow_indices, dtype=torch.int64), sparse.crow_indices()
        )
    ...
    def test_sparse_csr_constructor(self, device, dtype):
        ...
        for index_dtype in [torch.int32, torch.int64]:
            sparse = torch.sparse_csr_tensor(
                torch.tensor(crow_indices, dtype=index_dtype),
            ...
            self.assertEqual(
                torch.tensor(crow_indices, dtype=index_dtype), sparse.crow_indices()
            )
            self.assertEqual(
                torch.tensor(col_indices, dtype=index_dtype), sparse.col_indices()
            )
    ...
    def test_sparse_csr_constructor_from_lists(self, device, dtype):
        # without size
        sparse = torch.sparse_csr_tensor(
            [0, 2, 4], [0, 1, 0, 1], [1, 2, 3, 4], dtype=dtype, device=device
        )
        ...
        self.assertEqual(
            torch.tensor([1, 2, 3, 4], dtype=dtype, device=device), sparse.values()
        )
    ...
    def test_factory_type_invariants_check(self, device):
        with self.assertRaisesRegex(
            RuntimeError, "both crow_indices and col_indices should have the same type."
        ):
            torch.sparse_csr_tensor(
        ...
        with self.assertRaisesRegex(
            RuntimeError, r"\"csr_construct_check\" not implemented for 'Short'"
        ):
            torch.sparse_csr_tensor(
    ...
    def test_factory_layout_invariants_check(self, device):
        with self.assertRaisesRegex(
            ...
            values = torch.tensor([1.0], device=device).expand(
                4,
            )
            torch.sparse_csr_tensor(
        ...
        with self.assertRaisesRegex(
            ...
            col_indices = torch.tensor([0], device=device).expand(
                4,
            )
            torch.sparse_csr_tensor(
                torch.tensor([0, 2, 4]), col_indices, torch.tensor([1, 2, 3, 4])
            )

        with self.assertRaisesRegex(
            RuntimeError, "expected crow_indices to be a strided and contiguous tensor"
        ):
    ...
    def test_factory_shape_invariants_check(self, device):
        ...
        with self.assertRaisesRegex(
            RuntimeError, r"size of a CSR tensor must be of length 2, but got: 3"
        ):
            torch.sparse_csr_tensor(
                torch.tensor(crow_indices),
                torch.tensor(col_indices),
        ...
        with self.assertRaisesRegex(
            ...
            torch.sparse_csr_tensor(
                torch.tensor(crow_indices).repeat(2, 1),
                torch.tensor(col_indices),
                torch.tensor(values),
                size,
                device=device,
        ...
        with self.assertRaisesRegex(
            RuntimeError, r"values must have dim\=1 but got values\.dim\(\)\=2"
        ):
            torch.sparse_csr_tensor(
                torch.tensor(crow_indices),
                torch.tensor(col_indices),
        ...
        with self.assertRaisesRegex(
            RuntimeError, r"crow_indices\.numel\(\) must be size\(0\) \+ 1, but got: 3"
        ):
            torch.sparse_csr_tensor(
                torch.tensor(crow_indices),
                torch.tensor(col_indices),
    ...
    def test_factory_indices_invariants_check(self, device):
        ...
        size = (2, 10)
        with self.assertRaisesRegex(
            RuntimeError, "0th value of crow_indices must be 0."
        ):
            torch.sparse_csr_tensor(
                torch.tensor([-1, 0, 4]),
                torch.tensor(col_indices),
        ...
        with self.assertRaisesRegex(
            RuntimeError, r"col_indices\.min\(\) should be greater or equal to zero"
        ):
            torch.sparse_csr_tensor(
                torch.tensor(crow_indices),
        ...
        with self.assertRaisesRegex(
            RuntimeError, r"size\(1\) should be greater than col_indices\.max\(\)"
        ):
            torch.sparse_csr_tensor(
                torch.tensor(crow_indices),
    ...
    def test_factory_device_type_inference(self, device, dtype):
        ...
        for (
            ...
            values_device,
            device,
        ) in itertools.product(cpu_cuda, cpu_cuda, cpu_cuda, cpu_cuda_none):
            for index_dtype in [torch.int32, torch.int64]:
                crow_indices = torch.tensor(
                    [0, 2, 4], dtype=index_dtype, device=crow_indices_device
                )
                col_indices = torch.tensor(
                    [0, 1, 0, 1], dtype=index_dtype, device=col_indices_device
                )
                ...
                if device is None and (
                    ...
                    t = torch.sparse_csr_tensor(
                        crow_indices, col_indices, values, size=(2, 10), device=device
                    )
    ...
    def test_sparse_csr_to_dense(self, device, dtype):
        ...
        csr = torch.sparse_csr_tensor(
            crow_indices, col_indices, values, dtype=dtype, device=device
        )
        ...
        def test_shape(di, dj, dk, nnz):
            for index_dtype in [torch.int32, torch.int64]:
                x = self.genSparseCSRTensor(
                    (di, dj), nnz, device=device, dtype=dtype, index_dtype=index_dtype
                )
    ...
    def test_sparse_mm(self, device, dtype):
        def test_shape(d1, d2, d3, nnz, transposed):
            ...
            S = self.genSparseCSRTensor(
                (d1, d2), nnz, device=device, dtype=dtype, index_dtype=torch.int32
            )
            S_dense = S.to_dense()
    ...
    def test_sparse_addmm(self, device, dtype):
        def test_shape(m, n, p, nnz, broadcast, alpha_beta=None):
            ...
            S = self.genSparseCSRTensor(
                [n, m], nnz, dtype=dtype, device=device, index_dtype=torch.int32
            )
            S_dense = S.to_dense()
