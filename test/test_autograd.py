...
class TestAutograd(TestCase):
    ...
    def _is_inference_tensor(self, tensor):
        try:
            err_msg = "Inference tensor do not track version counter"
            with self.assertRaisesRegex(RuntimeError, err_msg):
                tensor._version
            return True
    ...
    def test_set_inference_mode(self):
        ...
        with torch.set_inference_mode(True):
            self.assertTrue(torch.is_inference_mode_enabled())
            with torch.set_inference_mode(False):
                self.assertFalse(torch.is_inference_mode_enabled())
            self.assertTrue(torch.is_inference_mode_enabled())
        self.assertFalse(torch.is_inference_mode_enabled())
    ...
    def test_inference_mode_decorator(self):
        ...
        for requires_grad in (True, False):
            c = torch.ones(1, 2, 3).requires_grad_(requires_grad)
            d = func(c)
            self.assertTrue(self._is_inference_tensor(d))
            # TODO: is this expected?
            # Normal tensor functional op in inference mode produces
            # a inference mode tensor that retains its requires grad
            self.assertEqual(d.requires_grad, requires_grad)
    ...
    def test_inference_mode_tensor_creation(self):
        with torch.inference_mode():
            ...
            tmp = torch.ones(1, 2, 3).requires_grad_(True)
    ...
    def test_inference_mode_existing_autograd_session(self):
        s = torch.ones(1, 2, 3).requires_grad_(True)
        a = s.clone()
    ...
    def test_normal_tensor_inplace_output_in_inference_mode(self):
        for requires_grad in (True, False):
            s = torch.ones(1, 2, 3).requires_grad_(requires_grad)
            a = s.clone()
    ...
    def test_normal_tensor_view_op_in_inference_mode(self):
        for requires_grad in (True, False):
            s = torch.ones(1, 2, 3).requires_grad_(requires_grad)
            a = s.clone()
            ...
            with torch.inference_mode():
                ...
                # view -> view -> inplace
                # TODO: INTERNAL ASSERT FAILED at "../torch/csrc/autograd/variable.cpp":654
                # tmp.add_(2)
                self.assertFalse(self._is_inference_tensor(tmp))
                self.assertEqual(tmp.requires_grad, requires_grad)
                # TODO: FAILS
    ...
    def test_mix_inference_and_normal_tensor_functional_op(self):
        for requires_grad in (True, False):
            ...
            if requires_grad:
                ...
                # inference tnesor in TensorList input
                inputs = [s, c]
                with self.assertRaisesRegex(RuntimeError, err_msg):
                    torch.stack(inputs)
