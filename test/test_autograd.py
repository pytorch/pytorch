...
class TestAutograd(TestCase):
    ...
    def test_function(self):
        class MyFunction(Function):
            ...
            def backward(ctx, grad_output):
                ...
                self.assertInstance(var1, torch.Tensor)
                self.assertInstance(var2, torch.Tensor)
                self.assertInstance(grad_output, torch.Tensor)
    ...
    def test_hooks(self):
        ...
        def bw_hook(inc, grad):
            self.assertInstance(grad, torch.Tensor)
            counter[0] += inc
    ...
    def test_functional_blas(self):
        def compare(fn, *args):
            unpacked_args = tuple(arg.data if isinstance(arg, torch.Tensor) else arg
                                  for arg in args)
            ...
            if not isinstance(unpacked_result, torch.Tensor):
        ...
        test_blas(torch.mm, torch.randn(2, 10),
                  torch.randn(10, 4))
        test_blas_add(torch.addmm, torch.randn(2, 4),
                      torch.randn(2, 10), torch.randn(10, 4))
        test_blas(torch.bmm, torch.randn(4, 2, 10),
                  torch.randn(4, 10, 4))
        test_blas_add(torch.addbmm, torch.randn(2, 4),
                      torch.randn(4, 2, 10), torch.randn(4, 10, 4))
        test_blas_add(torch.baddbmm, torch.randn(4, 2, 4),
                      torch.randn(4, 2, 10), torch.randn(4, 10, 4))
        test_blas(torch.mv, torch.randn(2, 10),
                  torch.randn(10))
        test_blas_add(torch.addmv, torch.randn(2),
                      torch.randn(2, 10), torch.randn(10))
        test_blas(torch.ger, torch.randn(5),
                  torch.randn(6))
        test_blas_add(torch.addr, torch.randn(5, 6),
                      torch.randn(5), torch.randn(6))
        test_blas(torch.matmul, torch.randn(6), torch.randn(6))
        test_blas(torch.matmul, torch.randn(10, 4), torch.randn(4))
        test_blas(torch.matmul, torch.randn(5), torch.randn(5, 6))
        test_blas(torch.matmul, torch.randn(2, 10), torch.randn(10, 4))
        test_blas(torch.matmul, torch.randn(5, 2, 10), torch.randn(5, 10, 4))
        test_blas(torch.matmul, torch.randn(3, 5, 2, 10), torch.randn(3, 5, 10, 4))
        test_blas(torch.matmul, torch.randn(3, 5, 2, 10), torch.randn(10))
        test_blas(torch.matmul, torch.randn(10), torch.randn(3, 5, 10, 4))
...
def create_input(call_args, requires_grad=True, non_contiguous=False):
    ...
    def map_arg(arg):
        ...
        if isinstance(arg, torch.Size) or isinstance(arg, dont_convert):
            ...
            return var
        elif isinstance(arg, tuple) and not isinstance(arg[0]):
            assert not isinstance(arg[0], torch.Tensor)
