...
class TestAutogradFunctional(TestCase):
    ...
    def test_vjp_err_check(self):
        ...
        with self.assertRaisesRegex(RuntimeError, "Entry 0 in v has invalid size: should be torch.Size"):
            res = autogradF.vjp(foo, inp, torch.ones(2))

        res = autogradF.vjp(foo, inp, torch.ones(3))[1]
    ...
    def test_jvp_err_check(self):
        ...
        with self.assertRaisesRegex(RuntimeError, "v is a tuple of invalid length: should be 1 but got 2."):
            res = autogradF.jvp(foo, inp, (torch.ones_like(inp), torch.ones_like(inp)))

        with self.assertRaisesRegex(RuntimeError, "Entry 0 in v has invalid size: should be torch.Size"):
            res = autogradF.jvp(foo, inp, torch.ones(2))

        res = autogradF.jvp(foo, inp, torch.ones(inp.size()))[1]
