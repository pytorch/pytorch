...
SLOW_TESTS = [
    ...
    "test_jit_profiling",
    "test_torch",
    "test_binary_ufuncs" "test_numpy_interop",
    "test_reductions",
    "test_shape_ops",
    "test_sort_and_select",
    "test_testing",
...
if dist.is_available():
    ...
    if not TEST_WITH_ROCM and dist.is_mpi_available():
        DISTRIBUTED_TESTS_CONFIG["mpi"] = {"WORLD_SIZE": "3", "TEST_REPORT_SOURCE_OVERRIDE": "dist-mpi"}
...
SIGNALS_TO_NAMES_DICT = {getattr(signal, n): n for n in dir(signal) if n.startswith("SIG") and "_" not in n}
...
def _test_cpp_extensions_aot(test_module, test_directory, options, use_ninja):
    ...
    if sys.platform != "win32":
        return_code = shell(cmd, cwd=os.path.join(cpp_extensions_test_dir, "no_python_abi_suffix_test"), env=shell_env)
        if return_code != 0:
            return return_code
...
def test_cpp_extensions_aot_ninja(test_module, test_directory, options):
    return _test_cpp_extensions_aot("test_cpp_extensions_aot", test_directory, options, use_ninja=True)


def test_cpp_extensions_aot_no_ninja(test_module, test_directory, options):
    return _test_cpp_extensions_aot("test_cpp_extensions_aot", test_directory, options, use_ninja=False)
...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Run the PyTorch unit test suite", epilog="where TESTS is any of: {}".format(", ".join(TESTS))
    )
    parser.add_argument(
        "-v", "--verbose", action="count", default=0, help="print verbose information and test-by-test results"
    )
    parser.add_argument(
        "-v", "--verbose", action="store_true", help="print verbose information and test-by-test results"
    )
    ...
    parser.add_argument(
        "-c", "--coverage", action="store_true", help="enable coverage", default=PYTORCH_COLLECT_COVERAGE
    )
    parser.add_argument(
        "-i",
        "--include",
    ...
    parser.add_argument(
        "-f", "--first", choices=TESTS, metavar="TESTS", help="select the test to start from (excludes previous tests)"
    )
    parser.add_argument(
        "-l", "--last", choices=TESTS, metavar="TESTS", help="select the last test to run (excludes following tests)"
    )
    parser.add_argument(
        "--bring-to-front",
        nargs="+",
    ...
    parser.add_argument(
        "--exclude-jit-executor", action="store_true", help="exclude tests that are run for a specific jit config"
    )
...
def get_selected_tests(options):
    ...
    if options.bring_to_front:
        ...
        selected_tests = options.bring_to_front + list(filter(lambda name: name not in to_front, selected_tests))
