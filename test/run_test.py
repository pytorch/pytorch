...
def get_test_times_json() -> Optional[Dict[str, float]]:
    ...
    if test_config not in test_file_times:
        print("::warning:: Gathered no stats from artifacts.")
        return None
    else:
...
def run_test(
    ...
    log_fd, log_path = tempfile.mkstemp(dir=REPO_ROOT / "test" / "test-reports",
                                        prefix=f"{test_module}_".replace("\\", "-").replace("/", "-"))
    os.close(log_fd)
...
def get_selected_tests(options, test_times: Dict[str, float]) -> List[TestJob]:
    ...
    assert min(which_shard, num_shards) > 0, "Shards must be positive numbers"
    ...
    print_to_stderr("expected times: ")
    for i, _ in shards:
...
def main():
    ...
    test_times = get_test_times_json()
    if test_times is None:
        test_times = dict()
        print("no test times found from artifacts")
    else:
