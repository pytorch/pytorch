...
class TestQuantizedTensor(TestCase):
    ...
    @given(dtype=st.sampled_from([torch.qint8, torch.quint8, torch.qint32]))
    def test_qtensor_float_assignment(self, dtype):
        return
        # Scalar Tensor
        # item
        scale = 1.0
    ...
    @given(dtype=st.sampled_from([torch.qint8, torch.quint8, torch.qint32]))
    def test_qtensor_per_channel_permute(self, dtype):
        return
    ...
    def test_qtensor_copy(self, device, dtype):
        ...
        q = torch._empty_affine_quantized([numel], scale=scale, zero_point=zero_point, device=device, dtype=dtype)
        q2 = torch._empty_affine_quantized([numel], scale=scale, zero_point=zero_point, device=device, dtype=dtype)
        q.copy_(q2)
    ...
    @no_deadline
    @given(device=st.sampled_from(['cpu', 'cuda'] if torch.cuda.is_available() else ['cpu']))
    def test_qtensor_clone(self, device):
        return
        numel = 10
        scale = 0.5
        zero_point = 10
