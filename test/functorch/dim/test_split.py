...
class TestSplitDevice(TestCase):
    """Device-generic tests for split behavior across accelerators."""

    def test_device_handling(self, device):
        """Test split behavior preserves device placement."""
        ...
        for i, part in enumerate(result):
            ordered = part.order(x, d1 if i == 0 else d2, z)
            self.assertEqual(ordered.device.type, torch.device(device).type)
            self.assertEqual(ordered.shape[0], 3)
            self.assertEqual(ordered.shape[2], 5)


instantiate_device_type_tests(
    TestSplitDevice, globals(), allow_mps=True, allow_xpu=True
)
