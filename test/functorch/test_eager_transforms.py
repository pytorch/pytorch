...
class TestCompileTransforms(TestCase):
    ...
    # Triton only supports GPU with SM70 or later.
    @expectedFailureIf(IS_WINDOWS or (TEST_CUDA and not SM70OrLater))
    def test_errors_functorch_compile(self, device):
        ...
        with self.assertRaisesRegex(RuntimeError, "torch.func.grad transform is currently not supported"):
            torch.compile(f)(x)
        ...
        with self.assertRaisesRegex(RuntimeError, "torch.func.vmap transform is currently not supported"):
            torch.compile(f)(x)

        # vjp
        def f(x):
            o, vjp_fn = torch.func.vjp(torch.sum, x)
            return vjp_fn(torch.randn_like(o))

        with self.assertRaisesRegex(RuntimeError, "torch.func.vjp transform is currently not supported"):
            torch.compile(f)(x)
        ...
        with self.assertRaisesRegex(RuntimeError, "torch.func.jvp transform is currently not supported"):
            torch.compile(f)(x)
        ...
        with self.assertRaisesRegex(RuntimeError, "torch.func.jacrev transform is currently not supported"):
            torch.compile(f)(x)
        ...
        with self.assertRaisesRegex(RuntimeError, "torch.func.jacfwd transform is currently not supported"):
            torch.compile(f)(x)
