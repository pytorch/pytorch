...
class _AOTAutogradHardwareTestBase(AOTTestCase):
    run_autograd = TestAOTAutograd.run_autograd
    verify_aot_autograd = TestAOTAutograd.verify_aot_autograd
    _compile_autocast = TestAOTAutograd._compile_autocast
    _make_model_and_input = TestAOTAutograd._make_model_and_input
    _make_refcount_probe = TestAOTAutograd._make_refcount_probe
    _assert_no_extra_refs = TestAOTAutograd._assert_no_extra_refs


class _AOTAutogradDynamoHardwareMixin:
    _is_dynamo_test = True

    assertExpectedInline = TestAOTAutogradWithDynamo.assertExpectedInline
    make_compiler = TestAOTAutogradWithDynamo.make_compiler
    run_autograd = TestAOTAutogradWithDynamo.run_autograd


class _AOTAutogradCacheHardwareMixin:
    _is_cache_test = True

    make_compiler = TestAOTAutogradWithCache.make_compiler
...
class TestAOTAutogradHardware(_AOTAutogradHardwareTestBase):
...
class TestAOTAutogradWithDynamoHardware(
    _AOTAutogradDynamoHardwareMixin, TestAOTAutogradHardware
):
    hw_classification = HardwareClassification.CUDA

    test_mem_leak_from_save_for_bw = (
        TestAOTAutogradHardware.test_mem_leak_from_save_for_bw
    )
    test_save_input_view_for_bw_does_not_leak_memory = (
        TestAOTAutogradHardware.test_save_input_view_for_bw_does_not_leak_memory
    )
    test_synthetic_base_base_attribute_is_none = (
        TestAOTAutogradHardware.test_synthetic_base_base_attribute_is_none
    )
    test_autocast_disable_guard = TestAOTAutogradHardware.test_autocast_disable_guard
    test_nonidempotent_amp = TestAOTAutogradHardware.test_nonidempotent_amp
    test_batch_norm_amp = TestAOTAutogradHardware.test_batch_norm_amp
    test_saved_tensors_hooks_mutations_raise = (
        TestAOTAutogradHardware.test_saved_tensors_hooks_mutations_raise
    )
    test_tangent_freed_compiled_model_and_loss = (
        TestAOTAutogradHardware.test_tangent_freed_compiled_model_and_loss
    )
    test_tangent_freed_compiled_model_eager_loss = (
        TestAOTAutogradHardware.test_tangent_freed_compiled_model_eager_loss
    )
    test_tangent_user_provided_via_pop = (
        TestAOTAutogradHardware.test_tangent_user_provided_via_pop
    )


class TestAOTAutogradWithCacheHardware(
    _AOTAutogradCacheHardwareMixin, TestAOTAutogradWithDynamoHardware
):
    hw_classification = HardwareClassification.CUDA

    test_mem_leak_from_save_for_bw = (
        TestAOTAutogradWithDynamoHardware.test_mem_leak_from_save_for_bw
    )
    test_save_input_view_for_bw_does_not_leak_memory = TestAOTAutogradWithDynamoHardware.test_save_input_view_for_bw_does_not_leak_memory
    test_synthetic_base_base_attribute_is_none = (
        TestAOTAutogradWithDynamoHardware.test_synthetic_base_base_attribute_is_none
    )
    test_autocast_disable_guard = (
        TestAOTAutogradWithDynamoHardware.test_autocast_disable_guard
    )
    test_nonidempotent_amp = TestAOTAutogradWithDynamoHardware.test_nonidempotent_amp
    test_batch_norm_amp = TestAOTAutogradWithDynamoHardware.test_batch_norm_amp
    test_saved_tensors_hooks_mutations_raise = (
        TestAOTAutogradWithDynamoHardware.test_saved_tensors_hooks_mutations_raise
    )
    test_tangent_freed_compiled_model_and_loss = (
        TestAOTAutogradWithDynamoHardware.test_tangent_freed_compiled_model_and_loss
    )
    test_tangent_freed_compiled_model_eager_loss = (
        TestAOTAutogradWithDynamoHardware.test_tangent_freed_compiled_model_eager_loss
    )
    test_tangent_user_provided_via_pop = (
        TestAOTAutogradWithDynamoHardware.test_tangent_user_provided_via_pop
    )


class _PartitioningHardwareTestBase(AOTTestCase):
    _make_effectful_op = TestPartitioning._make_effectful_op
    _make_wrapper_via_post_compile = TestPartitioning._make_wrapper_via_post_compile
    _make_rng_wrapper_via_post_compile = (
        TestPartitioning._make_rng_wrapper_via_post_compile
    )
    _build_control_deps_graph = TestPartitioning._build_control_deps_graph
    _extract_and_check = TestPartitioning._extract_and_check
    _build_backward_extract_graph = TestPartitioning._build_backward_extract_graph


class TestPartitioningHardware(_PartitioningHardwareTestBase):
    hw_classification = HardwareClassification.CUDA
...
class _AOTModuleSimplifiedHardwareTestBase(AOTTestCase):
    _test_pack_hooks = TestAOTModuleSimplified._test_pack_hooks


class TestAOTModuleSimplifiedHardware(_AOTModuleSimplifiedHardwareTestBase):
    hw_classification = HardwareClassification.CUDA
...
instantiate_device_type_tests(TestAOTAutogradHardware, globals(), only_for=("cuda",))


instantiate_device_type_tests(
    TestAOTAutogradWithDynamoHardware, globals(), only_for=("cuda",)
)


instantiate_device_type_tests(
    TestAOTAutogradWithCacheHardware, globals(), only_for=("cuda",)
)


instantiate_device_type_tests(TestPartitioningHardware, globals(), only_for=("cuda",))


instantiate_device_type_tests(
    TestAOTModuleSimplifiedHardware, globals(), only_for=("cuda",)
)
