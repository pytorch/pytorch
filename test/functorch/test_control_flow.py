...
class TestControlFlowTraced(TestCase):
    ...
    def test_cond_bool_constant_outputs_rejected(self):
        def f(pred, x):
            return torch.cond(pred, lambda y: True, lambda y: True, (x,))
    ...
    def test_switch_bool_constant_outputs_rejected(self):
        def f(index, x):
            return switch(index, [lambda y: True, lambda y: True], (x,))
        ...
        with self.assertRaisesRegex(
            ...
            torch.compile(f, backend="eager", fullgraph=True)(
                torch.tensor(0), torch.ones(4)
            )

    def test_cond_mismatched_constant_tensor_output_types_rejected(self):
        def f(pred, x):
            return torch.cond(
                pred,
                lambda y: (y + 1, 1),
        ...
        with self.assertRaisesRegex(
            ...
            "branches must return the same type for corresponding output leaves",
        ):
            torch.compile(f, backend="eager", fullgraph=True)(
                torch.tensor(True), torch.ones(4)
            )
...
def forward(self, L_pred_ : torch.Tensor, L_x_ : torch.Tensor):
    ...
    def test_cond_traced_unsupported_constant_outputs_cudagraphs(self):
        ...
        with self.assertRaisesRegex(
            ValueError,
            r"constant branch outputs must be int \(but not bool\) or None",
        ):
            with torch.cuda.graph(g), CUDAGraphCaptureControlFlowOpDispatchMode():
