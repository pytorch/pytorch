...
""",
    ...
    def test_scan_unroll_graph(self):
        ...
        def f_internal_full(init, xs):
            flat_combine_fn = functools.partial(
                wrap_combine_fn_flat,
                combine_fn=combine_fn,
                spec_init=pytree.tree_structure(init),
                spec_xs=pytree.tree_structure(xs),
                num_init_leaves=1,
                num_inp_leaves=1,
            )
            return scan_op(flat_combine_fn, [init], [xs], (), _FULL_UNROLL)

        expected = f_internal_full(init, xs)
        actual = torch.compile(f_internal_full, backend="eager", fullgraph=True)(
            init, xs
        )
...
def forward(self, L_init_ : torch.Tensor, L_xs_ : torch.Tensor):
    ...
    @parametrize("compile_mode", ["none", "eager"])
    @parametrize("unroll", [2, True])
    def test_scan_xs_none_length_unroll(self, compile_mode, unroll):
        def body(c, x):
            return torch.sin(c), torch.sin(c).clone()
        ...
        result = scan_fct(body, init, None, length=3, unroll=unroll)
        result_exp = _fake_scan(body, init, None, length=3, unroll=unroll)
        self.assertEqual(result, result_exp)
        self.check_autograd(result, result_exp, (init,))
