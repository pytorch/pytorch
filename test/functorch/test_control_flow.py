...
def forward(self, arg0_1, arg1_1, arg2_1, arg3_1, arg4_1):
    ...
    def test_cond_runtime_asserts_preserve_condition_provenance(self):
        def branch(x):
            torch._check(x.shape[0] * 2 == 4)
            torch._check(x.shape[1] + 1 == 4)
            return x + 1

        def f(pred, x):
            return cond(pred, branch, branch, [x])

        gm = make_fx(f, tracing_mode="symbolic")(torch.tensor(True), torch.ones(2, 3))
        assertions = [
            node
            for node in gm.true_graph_0.graph.nodes
            if node.target is torch.ops.aten._assert_scalar.default
        ]
        self.assertEqual(len(assertions), 2)
        self.assertIsNot(assertions[0].args[0], assertions[1].args[0])

        valid = torch.ones(2, 3)
        self.assertEqual(gm(torch.tensor(True), valid), valid + 1)
        with self.assertRaisesRegex(RuntimeError, "Runtime assertion failed"):
            gm(torch.tensor(True), torch.ones(2, 4))
        with self.assertRaisesRegex(RuntimeError, "Runtime assertion failed"):
            gm(torch.tensor(True), torch.ones(3, 3))
