...
class TestFX(JitTestCase):
    ...
    @unittest.skipIf(
        ...
        "`__future__` feature " "`annotations` is not defined in Python <3.7",
    )
    ...
    def test_annotations_empty_tuple(self):
        ...
        FileCheck().check("_Tuple[()]").check("typing_Tuple[str,typing_Tuple[()]]").run(
            traced.code
        )
...
class TestFXAPIBackwardCompatibility(JitTestCase):
    ...
    def _annotation_type_to_stable_str(self, t, sig_str):
        ...
        contained_type_annots = [
            self._annotation_type_to_stable_str(ct, sig_str) for ct in contained
        ]
        contained_type_str = (
