...
class TestFX(JitTestCase):
    ...
    def test_replace_uses_with_dataclass_args_and_dce(self):
        ...
        @dataclasses.dataclass(frozen=True)
        class DataclassEnum(enum.Enum):
            value: int

            ITEM = 1

        class EnumModule(torch.nn.Module):
            def forward(self, x):
                return (x, DataclassEnum.ITEM)
        ...
        mapped_enum = torch.fx.node._map_aggregate_with_dataclasses(
            DataclassEnum.ITEM, lambda x: ("leaf", x)
        )
        self.assertEqual(mapped_enum, ("leaf", DataclassEnum.ITEM))
        self.assertFalse(torch.fx.node._is_dataclass_instance(DataclassEnum.ITEM))
        enum_gm = torch.fx.symbolic_trace(EnumModule())
        self.assertEqual(enum_gm(torch.tensor(1))[1], DataclassEnum.ITEM)
    ...
    @skipIfTorchDynamo("Dynamo wraps class access in this artificial FX test")
