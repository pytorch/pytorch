...
class TestFX(JitTestCase):
    ...
    def test_node_normalize_arguments_use_kwargs(self):
        ...
        def get_linear_node(nodes):
            result = None
            for node in nodes:
                if node.target == torch.nn.functional.linear or node.target == 'linear':
                    result = node
                    break
        ...
        node = get_linear_node(trace.nodes)
        ...
        node = get_linear_node(symbolic_traced.graph.nodes)
