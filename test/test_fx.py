...
class TestFX(JitTestCase):
    ...
    def test_subgraph_creation(self):
        ...
        def split_module(
            ...
            split_callback: Callable[[torch.fx.Node, int, int], int],
        ):
            partitions: Dict[str, Partition] = {}
            orig_nodes: Dict[str, torch.fx.Node] = {}

            def record_cross_partition_use(def_node : torch.fx.Node, use_node : Optional[torch.fx.Node]):
            ...
            for node in m.graph.nodes:
                ...
                if node.op in ["placeholder", "get_attr"]:
                    continue
                partition_name = str(split_callback(node, partition_counter, NPARTITIONS))
                partition_counter += 1
        ...
        def mod_partition(node: torch.fx.Node, partition_counter, n_partitions):
            partition = partition_counter % n_partitions
            return partition
