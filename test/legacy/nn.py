...
class SimpleTestCase(TestCaseBase):
    def __init__(self, *args, check_inplace=False, jacobian_parameters=True, **kwargs):
    ...
    def __call__(self, test_case):
        ...
        # TODO: test IO
        module.training()
        test_case.check_jacobian(module, input, self.jacobian_parameters)
        module.evaluate()
        test_case.check_jacobian(module, input, self.jacobian_parameters)
...
simple_tests = [
    ...
    # SimpleTestCase(nn.MixtureTable,
                    # tuple(),
                    # input_size=[(4,), (4, 5, 3)],
                    # reference_fn=lambda _,i: sum(t * i for t, i in zip(*i))),
    # TODO: FIX THIS
    # SimpleTestCase(nn.LookupTable,
                    # (4, 10),
                    # input=torch.randperm(4)+1,
    ...
    CriterionTestCase(nn.ClassSimplexCriterion,
                        ...
                        target=torch.rand(5).mul(30).ceil().long(),
                        desc='margin'),
]
