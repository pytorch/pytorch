...
class TestPrecompile(TestCase):
    ...
    def test_make_fx_requires_one_example_input(self, num_examples):
        x = torch.randn(4)
        with self.assertRaisesRegex(AssertionError, "exactly one example input tuple"):
            torch.compiler.precompile(
                lambda t: t + 1,
    ...
    def test_capture_leaves_the_accelerator_rng_state_alone(self):
        ...
        torch.compiler.precompile(
            lambda a: torch.rand_like(a),
            example_inputs=[(torch.empty(4, device="cuda"),)],
            backend="eager",
        )
    ...
    def test_concurrent_captures_are_serialized(self):
        ...
        def worker(i):
            ...
            results[i] = torch.compiler.precompile(
                lambda a: a + 1, example_inputs=[(torch.ones(2),)], backend="eager"
            )
    ...
    def test_nested_capture_does_not_deadlock(self):
        ...
        def outer(a):
            torch.compiler.precompile(
                inner, example_inputs=[(torch.ones(2),)], backend="eager"
            )
            return a + 1

        code, _cache = torch.compiler.precompile(
            outer, example_inputs=[(torch.ones(2),)], backend="eager"
        )
        self.assertIn("def forward", code)
    ...
    def test_generated_source_says_it_may_be_edited(self):
        ...
        for backend in ("eager", "inductor"):
            code, _cache = torch.compiler.precompile(
                lambda a: a + 1, example_inputs=[(torch.ones(2),)], backend=backend
            )
    ...
    def test_load_from_a_code_string_gives_triton_a_file(self):
        ...
        code, cache = torch.compiler.precompile(
            lambda a: (a * 2).relu(), example_inputs=[(torch.ones(64, device="cuda"),)]
        )
    ...
    def test_export_and_direct_precompile_captures_are_serialized(self, device):
        ...
        def precompile_worker():
            try:
                barrier.wait()
                torch.compiler.precompile(
                    lambda inp: inp + 2, example_inputs=[(x,)], backend="eager"
                )
            except BaseException as e:
                errors.append(e)
