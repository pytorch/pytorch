...
class CodegenInductorTest(InductorTestCase):
    def run_and_compare(
        ...
        *args,
        ...
        for ref, actual in zip(ref_tensors, actual_tensors):
            self.assertTrue(torch.allclose(ref, actual, atol=1e-4))
    ...
    def test_cse_make_block_ptr_softmax(self):
        def func(a, b):
            return torch.softmax(a * b, dim=0)
        ...
        self.count_code("tl.make_block_ptr(in_ptr", code, 2 + 4)
        self.count_code("tl.load(block_ptr", code, 2)
