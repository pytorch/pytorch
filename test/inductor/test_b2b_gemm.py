...
class B2BGEMMTest(TestCase):
    ...
    def test_b2b_gemm_supported_block_size(self):
        """sizes match the supported block sizes"""
        ...
        self.assertTrue(torch.allclose(f(A, B, C), res, atol=0.5, rtol=0.01))
        self.assertTrue("B2B_GEMM_TRITON_ENTRANCE" in code)

    @torch._inductor.config.patch(b2b_gemm_pass=True)
    def test_b2b_gemm_unsupported_block_size(self):
        """sizes don't match the supported block sizes"""
