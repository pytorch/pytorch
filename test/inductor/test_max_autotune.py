...
class TestMaxAutotuneSubproc(TestCase):
    ...
    def test_async_autotuner_retries_all_choices_after_cudagraph_failure(self):
        from concurrent.futures import Future

        inputs_key = "inputs"

        def choice_for(name):
            choice = mock.Mock(hash_key=lambda: name)
            choice.bmreq.force_eager_benchmark = False
            choice.bmreq.module_path = f"/{name}.py"
            return choice

        choices = [choice_for("first"), choice_for("capture_failure")]
        ...
        initial_futures = {
            AsyncAutotuner.get_choice_hash(choices[0], inputs_key): completed(1.0),
            AsyncAutotuner.get_choice_hash(choices[1], inputs_key): completed(
                CUDAGraphBenchmarkError("capture failed")
            ),
        }
        original_futures = dict(initial_futures)
        pool = mock.Mock()
        pool.submit.side_effect = lambda _fn, request: completed(
            10.0 if request.force_eager_benchmark else 1.0
        )

        with (
            mock.patch.object(AsyncAutotuner, "choice_hash_to_future", initial_futures),
            mock.patch.object(AutotuneProcessPool, "get_instance", return_value=pool),
        ):
            self.assertEqual(
                AsyncAutotuner.get_results(choices, inputs_key),
                {choices[0]: 10.0, choices[1]: 10.0},
            )
            self.assertEqual(
                AsyncAutotuner.choice_hash_to_future,
                original_futures,
            )
    ...
    def test_async_autotuner_recovers_broken_submission_during_eager_retry(self):
        from concurrent.futures import Future
        ...
        broken_pool = mock.Mock()
        broken_pool.submit.side_effect = BrokenProcessPool("worker exited")
        replacement_pool = mock.Mock()
        replacement_pool.submit.side_effect = [completed(10.0), completed(11.0)]

        with (
            mock.patch.object(AsyncAutotuner, "choice_hash_to_future", initial_futures),
            mock.patch.object(
                AsyncAutotuner, "invalidated_choice_hashes", OrderedSet()
            ),
            mock.patch.object(
                AutotuneProcessPool,
                "get_instance",
                side_effect=[broken_pool, replacement_pool, replacement_pool],
            ),
            mock.patch.object(AutotuneProcessPool, "shutdown_instance") as shutdown,
        ):
            self.assertEqual(
                AsyncAutotuner.get_results(choices, inputs_key),
                {choices[0]: 10.0, choices[1]: 11.0},
            )

        shutdown.assert_called_once_with(terminate_workers=True)
        broken_pool.submit.assert_called_once_with(
            run_autotune_in_subprocess, choices[0].bmreq
        )
        self.assertEqual(replacement_pool.submit.call_count, 2)
        self.assertFalse(choices[0].bmreq.force_eager_benchmark)
        self.assertFalse(choices[1].bmreq.force_eager_benchmark)

    def test_async_autotuner_retires_pool_after_sticky_cuda_failure(self):
        from concurrent.futures import Future

        inputs_key = "inputs"
        failed_choice = mock.Mock(hash_key=lambda: "failed")
        failed_choice.bmreq.module_path = "/failed.py"
        valid_choice = mock.Mock(hash_key=lambda: "valid")
        valid_choice.bmreq.module_path = "/valid.py"
        ...
        futures = {
            AsyncAutotuner.get_choice_hash(failed_choice, inputs_key): completed(
                StickyCUDABenchmarkError("cudaErrorIllegalAddress")
            ),
        }
        poisoned_future = mock.Mock()
        poisoned_future.result.side_effect = AssertionError(
            "the poisoned worker queue must not be drained"
        )
        futures[AsyncAutotuner.get_choice_hash(valid_choice, inputs_key)] = (
            poisoned_future
        )
        pool = mock.Mock()
        pool.supports_sticky_cuda_recovery = True
        pool.submit.return_value = completed(2.0)

        with (
            mock.patch.object(AsyncAutotuner, "choice_hash_to_future", futures),
            mock.patch.object(AutotuneProcessPool, "get_instance", return_value=pool),
            mock.patch.object(AutotuneProcessPool, "shutdown_instance") as shutdown,
        ):
            self.assertEqual(
                AsyncAutotuner.get_results([failed_choice, valid_choice], inputs_key),
                {failed_choice: float("inf"), valid_choice: 2.0},
            )

        shutdown.assert_called_once_with(terminate_workers=True)
        poisoned_future.result.assert_not_called()
        pool.submit.assert_called_once_with(
            run_autotune_in_subprocess, valid_choice.bmreq
        )

    def test_async_autotuner_resubmits_other_groups_after_pool_retirement(self):
        from concurrent.futures import Future
        ...
        first_key = "first_inputs"
        later_key = "later_inputs"
        failed_choice = mock.Mock(hash_key=lambda: "failed")
        failed_choice.bmreq.module_path = "/failed.py"
        later_choice = mock.Mock(hash_key=lambda: "later")
        later_choice.bmreq.module_path = "/later.py"
        poisoned_future = mock.Mock()
        poisoned_future.result.side_effect = AssertionError(
            "future from the retired pool must not be consumed"
        )
        futures = {
            AsyncAutotuner.get_choice_hash(failed_choice, first_key): completed(
                StickyCUDABenchmarkError("cudaErrorIllegalAddress")
            ),
            AsyncAutotuner.get_choice_hash(later_choice, later_key): poisoned_future,
        }
        pool = mock.Mock()
        pool.supports_sticky_cuda_recovery = True
        pool.submit.return_value = completed(3.0)
        ...
        with (
            ...
            self.assertEqual(
                AsyncAutotuner.get_results([failed_choice], first_key),
                {failed_choice: float("inf")},
            )
            self.assertEqual(
                AsyncAutotuner.get_results([later_choice], later_key),
                {later_choice: 3.0},
            )
        ...
        poisoned_future.result.assert_not_called()
        pool.submit.assert_called_once_with(
            run_autotune_in_subprocess, later_choice.bmreq
        )

    def test_async_autotuner_does_not_blame_choice_skipped_on_poisoned_worker(self):
        from concurrent.futures import Future
        ...
        failed_key = "failed_inputs"
        innocent_key = "innocent_inputs"
        failed_choice = mock.Mock(hash_key=lambda: "failed")
        failed_choice.bmreq.module_path = "/failed.py"
        innocent_choice = mock.Mock(hash_key=lambda: "innocent")
        innocent_choice.bmreq.module_path = "/innocent.py"
        futures = {
            AsyncAutotuner.get_choice_hash(failed_choice, failed_key): completed(
                StickyCUDABenchmarkError("cudaErrorIllegalAddress")
            ),
            AsyncAutotuner.get_choice_hash(innocent_choice, innocent_key): completed(
                PoisonedAutotuneWorkerError("autotune worker CUDA context is poisoned")
            ),
        }
        pool = mock.Mock()
        pool.supports_sticky_cuda_recovery = True
        pool.submit.side_effect = [
            completed(3.0),
            completed(StickyCUDABenchmarkError("cudaErrorIllegalAddress")),
        ]
        ...
        with (
            ...
            self.assertEqual(
                AsyncAutotuner.get_results([innocent_choice], innocent_key),
                {innocent_choice: 3.0},
            )
            self.assertEqual(
                AsyncAutotuner.get_results([failed_choice], failed_key),
                {failed_choice: float("inf")},
            )

        self.assertEqual(shutdown.call_count, 2)

    def test_async_autotuner_resubmits_cancelled_future_from_retired_pool(self):
        from concurrent.futures import Future

        inputs_key = "inputs"
        choice = mock.Mock(hash_key=lambda: "cancelled")
        choice.bmreq.module_path = "/cancelled.py"
        choice_hash = AsyncAutotuner.get_choice_hash(choice, inputs_key)
        cancelled_future = Future()
        cancelled_future.cancel()
        replacement_future = Future()
        replacement_future.set_result(4.0)
        pool = mock.Mock()
        pool.submit.return_value = replacement_future

        with (
            mock.patch.object(
                AsyncAutotuner,
                "choice_hash_to_future",
                {choice_hash: cancelled_future},
            ),
            mock.patch.object(
                AsyncAutotuner,
                "invalidated_choice_hashes",
                OrderedSet([choice_hash]),
            ),
            mock.patch.object(AutotuneProcessPool, "get_instance", return_value=pool),
        ):
            self.assertEqual(
                AsyncAutotuner.get_results([choice], inputs_key),
                {choice: 4.0},
            )

        pool.submit.assert_called_once_with(run_autotune_in_subprocess, choice.bmreq)

    def test_async_autotuner_recovers_broken_pool_during_submission(self):
        from concurrent.futures import Future
        from concurrent.futures.process import BrokenProcessPool

        inputs_key = "inputs"
        choice = mock.Mock(hash_key=lambda: "innocent")
        choice.bmreq.module_path = "/innocent.py"
        stale_hash = "stale"
        stale_future = mock.Mock()
        replacement_future = Future()
        replacement_future.set_result(2.0)

        broken_pool = mock.Mock()
        broken_pool.submit.side_effect = BrokenProcessPool("worker exited")
        replacement_pool = mock.Mock()
        replacement_pool.submit.return_value = replacement_future

        with (
            mock.patch.object(
                AsyncAutotuner,
                "choice_hash_to_future",
                {stale_hash: stale_future},
            ),
            mock.patch.object(
                AsyncAutotuner, "invalidated_choice_hashes", OrderedSet()
            ),
            mock.patch.object(
                AutotuneProcessPool,
                "get_instance",
                side_effect=[broken_pool, replacement_pool],
            ),
            mock.patch.object(AutotuneProcessPool, "shutdown_instance") as shutdown,
        ):
            self.assertIs(
                AsyncAutotuner._submit_choice(choice, inputs_key),
                replacement_future,
            )
            self.assertIn(stale_hash, AsyncAutotuner.invalidated_choice_hashes)

        shutdown.assert_called_once_with(terminate_workers=True)
        broken_pool.submit.assert_called_once_with(
            run_autotune_in_subprocess, choice.bmreq
        )
        replacement_pool.submit.assert_called_once_with(
            run_autotune_in_subprocess, choice.bmreq
        )

    def test_async_autotuner_serializes_concurrent_submission_recovery(self):
        from concurrent.futures import Future
        from concurrent.futures.process import BrokenProcessPool

        inputs_key = "inputs"
        first_choice = mock.Mock(hash_key=lambda: "first")
        first_choice.bmreq.module_path = "/first.py"
        second_choice = mock.Mock(hash_key=lambda: "second")
        second_choice.bmreq.module_path = "/second.py"

        broken_submit_entered = threading.Event()
        allow_broken_submit = threading.Event()
        ...
        def broken_submit(*args):
            broken_submit_entered.set()
            self.assertTrue(allow_broken_submit.wait(timeout=5))
            raise BrokenProcessPool("worker exited")

        broken_pool.submit.side_effect = broken_submit
        replacement_pool = mock.Mock()

        def replacement_submit(_fn, request):
            replacement_requests.append(request)
            future = Future()
            future.set_result(float(len(replacement_requests)))
            return future

        replacement_pool.submit.side_effect = replacement_submit
        results = {}
        errors = []

        def submit(name, choice, *, started=None, finished=None):
            if started is not None:
                started.set()
            try:
                results[name] = AsyncAutotuner._submit_choice(choice, inputs_key)
        ...
        with (
            mock.patch.object(AsyncAutotuner, "choice_hash_to_future", {}),
            mock.patch.object(
                AsyncAutotuner, "invalidated_choice_hashes", OrderedSet()
            ),
            mock.patch.object(
            ...
            first_thread = threading.Thread(target=submit, args=("first", first_choice))
            first_thread.start()
            self.assertTrue(broken_submit_entered.wait(timeout=5))

            second_thread = threading.Thread(
                target=submit,
                args=("second", second_choice),
                kwargs={
                    "started": second_submit_started,
                    "finished": second_submit_finished,
                },
            )
            second_thread.start()
            self.assertTrue(second_submit_started.wait(timeout=5))
            self.assertFalse(second_submit_finished.is_set())

            allow_broken_submit.set()
        ...
        self.assertEqual(
            replacement_requests,
            [first_choice.bmreq, second_choice.bmreq],
        )
        self.assertEqual(results["first"].result(), 1.0)
        self.assertEqual(results["second"].result(), 2.0)
        shutdown.assert_called_once_with(terminate_workers=True)

    def test_async_autotuner_does_not_blame_first_waiter_for_broken_pool(self):
        from concurrent.futures import Future
        from concurrent.futures.process import BrokenProcessPool
        ...
        failed_future.result.side_effect = AssertionError(
            "the unrelated failed Future must not be consumed"
        )
        innocent_future = Future()
        innocent_future.set_exception(BrokenProcessPool("worker exited"))
        futures = {
            AsyncAutotuner.get_choice_hash(failed_choice, failed_key): failed_future,
            AsyncAutotuner.get_choice_hash(
                innocent_choice, innocent_key
            ): innocent_future,
        }
        pool = mock.Mock()
        pool.supports_sticky_cuda_recovery = True
        isolated_results = ({innocent_choice: 2.0}, OrderedSet())
        ...
        with (
            ...
            mock.patch.object(
                AsyncAutotuner,
                "_benchmark_choices_in_isolated_processes",
                return_value=isolated_results,
            ) as isolate,
        ):
            self.assertEqual(
                AsyncAutotuner.get_results([innocent_choice], innocent_key),
        ...
        shutdown.assert_called_once_with(terminate_workers=True)
        failed_future.result.assert_not_called()
        isolate.assert_called_once_with([innocent_choice], innocent_key)
        pool.submit.assert_not_called()

    def test_async_autotuner_concurrent_collectors_recheck_invalidation(self):
        from concurrent.futures import Future
        from concurrent.futures.process import BrokenProcessPool

        first_key = "first_inputs"
        second_key = "second_inputs"
        first_choice = mock.Mock(hash_key=lambda: "first")
        first_choice.bmreq.module_path = "/first.py"
        second_choice = mock.Mock(hash_key=lambda: "second")
        second_choice.bmreq.module_path = "/second.py"
        ...
        futures = {
            AsyncAutotuner.get_choice_hash(first_choice, first_key): BrokenFuture(),
            AsyncAutotuner.get_choice_hash(second_choice, second_key): BrokenFuture(),
        }
        replacement_future = Future()
        replacement_future.set_result(2.0)
        pool = mock.Mock()
        ...
        both_checked_invalidation = threading.Event()
        get_instance_lock = threading.Lock()

        def get_instance():
            nonlocal get_instance_calls
            with get_instance_lock:
                get_instance_calls += 1
                if get_instance_calls == 2:
                    both_checked_invalidation.set()
            # Before the fix, both collectors could reach this point after the
            # first non-atomic invalidation check. With the lock held across the
            # decision, the first collector times out and retires the pool while
            # the second waits, then the second resubmits its invalidated Future.
            both_checked_invalidation.wait(timeout=0.2)
            return pool

        results = {}
        errors = []

        def collect(name, choice, key):
            try:
                results[name] = AsyncAutotuner.get_results([choice], key)
            except Exception as error:
                errors.append(error)

        def isolate(choices, _key):
            return {choices[0]: 1.0}, OrderedSet()
        ...
        with (
            ...
            mock.patch.object(
                AutotuneProcessPool, "get_instance", side_effect=get_instance
            ),
            mock.patch.object(AutotuneProcessPool, "shutdown_instance"),
            mock.patch.object(
                AsyncAutotuner,
                "_benchmark_choices_in_isolated_processes",
                side_effect=isolate,
            ...
            first = threading.Thread(
                target=collect, args=("first", first_choice, first_key)
            )
            second = threading.Thread(
                target=collect, args=("second", second_choice, second_key)
            )
            first.start()
            second.start()
        ...
        self.assertEqual(errors, [])
        self.assertEqual(set(results["first"]), {first_choice})
        self.assertEqual(set(results["second"]), {second_choice})
        self.assertEqual(
            sorted([*results["first"].values(), *results["second"].values()]),
            [1.0, 2.0],
        )

    def test_async_autotuner_isolated_process_attributes_hard_failure(self):
        from concurrent.futures import Future
        from concurrent.futures.process import BrokenProcessPool
        ...
        failed_future.set_exception(BrokenProcessPool("worker exited"))
        valid_future = Future()
        valid_future.set_result(2.0)

        failed_pool = mock.Mock()
        failed_pool.submit.return_value = failed_future
        valid_pool = mock.Mock()
        valid_pool.submit.return_value = valid_future
        ...
        with (
            ...
            mock.patch(
                "torch._inductor.autotune_process.AutotuneProcessPool",
                side_effect=[failed_pool, valid_pool],
            ),
        ):
            timings, sticky_choices = (
                AsyncAutotuner._benchmark_choices_in_isolated_processes(
                    [failed_choice, valid_choice], inputs_key
                )
            )

            self.assertEqual(timings, {valid_choice: 2.0})
            self.assertEqual(sticky_choices, OrderedSet([failed_choice]))
            self.assertEqual(
                AsyncAutotuner.choice_hash_to_future[
                    AsyncAutotuner.get_choice_hash(failed_choice, inputs_key)
                ].result(),
                float("inf"),
            )
            self.assertEqual(
                AsyncAutotuner.choice_hash_to_future[
                    AsyncAutotuner.get_choice_hash(valid_choice, inputs_key)
                ].result(),
                2.0,
            )

        failed_pool._shutdown.assert_called_once_with(terminate_workers=True)
        valid_pool._shutdown.assert_called_once_with(terminate_workers=True)

    def test_async_autotuner_retires_pool_after_sticky_eager_retry(self):
        from concurrent.futures import Future
        ...
        with (
            ...
            self.assertEqual(
                AsyncAutotuner.get_results(choices, inputs_key),
                {choices[0]: float("inf"), choices[1]: 10.0},
            )
    ...
    def test_async_autotuner_propagates_sticky_failure_from_thread_pool(self):
        from concurrent.futures import Future
        ...
        future = Future()
        failure = StickyCUDABenchmarkError("cudaErrorIllegalAddress")
        future.set_result(failure)
        futures = {
            AsyncAutotuner.get_choice_hash(failed_choice, inputs_key): future,
        }
        pool = mock.Mock()
        pool.supports_sticky_cuda_recovery = False

        with (
            mock.patch.object(AsyncAutotuner, "choice_hash_to_future", futures),
            mock.patch.object(AutotuneProcessPool, "get_instance", return_value=pool),
            mock.patch.object(AutotuneProcessPool, "shutdown_instance") as shutdown,
            self.assertRaisesRegex(StickyCUDABenchmarkError, "cudaErrorIllegalAddress"),
        ):
            AsyncAutotuner.get_results([failed_choice], inputs_key)

        shutdown.assert_called_once_with(terminate_workers=True)
        pool.submit.assert_not_called()

    def test_autotune_process_pool_terminates_poisoned_process_workers(self):
        class FakeProcessPool:
            def __init__(self):
                self.shutdown = mock.Mock()

        process_pool = FakeProcessPool()
        autotune_pool = AutotuneProcessPool.__new__(AutotuneProcessPool)
        autotune_pool._pool = process_pool
        autotune_pool._timer = None

        with (
            mock.patch(
                "torch._inductor.autotune_process.ProcessPoolExecutor",
                FakeProcessPool,
            ),
            mock.patch(
                "torch._inductor.autotune_process._terminate_process_pool"
            ) as terminate,
        ):
            autotune_pool._shutdown(terminate_workers=True)

        terminate.assert_called_once_with(process_pool)
        process_pool.shutdown.assert_not_called()
        self.assertIsNone(autotune_pool._pool)

    def test_autotune_process_pool_initializes_process_workers(self):
        from torch._inductor.autotune_process import _init_autotune_subprocess

        autotune_pool = AutotuneProcessPool.__new__(AutotuneProcessPool)
        process_pool = mock.Mock()
        mp_context = mock.Mock()

        with (
            mock.patch(
                "torch._inductor.utils.should_use_thread_workers",
                return_value=False,
            ),
            mock.patch(
                "torch._inductor.autotune_process.mp.get_context",
                return_value=mp_context,
            ),
            mock.patch(
                "torch._inductor.autotune_process.ProcessPoolExecutor",
                return_value=process_pool,
            ) as process_pool_constructor,
            mock.patch("torch._inductor.autotune_process.atexit.register"),
        ):
            self.assertIs(autotune_pool._init_pool(), process_pool)

        process_pool_constructor.assert_called_once_with(
            max_workers=1,
    ...
    def test_pipelined_worker_reports_sticky_cuda_failure(self):
        for message in (
            ...
            "the MPS client has been terminated",
            ...
            with (
                ...
                mock.patch(
                    "torch._inductor.autotune_process._autotune_worker_cuda_poisoned",
                    False,
                ),
            ):
                request = mock.Mock()
                request.benchmark.side_effect = RuntimeError(message)

                result = run_autotune_in_subprocess(request)

                self.assertIsInstance(result, StickyCUDABenchmarkError)
                self.assertIn(message, str(result))

    def test_pipelined_worker_skips_queued_work_after_sticky_cuda_failure(self):
        failed_request = mock.Mock()
        failed_request.benchmark.side_effect = RuntimeError("cudaErrorLaunchFailure")
        queued_request = mock.Mock()

        with mock.patch(
            "torch._inductor.autotune_process._autotune_worker_cuda_poisoned",
            False,
        ):
            first_result = run_autotune_in_subprocess(failed_request)
            queued_result = run_autotune_in_subprocess(queued_request)

        self.assertIsInstance(first_result, StickyCUDABenchmarkError)
        self.assertIsInstance(queued_result, PoisonedAutotuneWorkerError)
        self.assertIn("context is poisoned", str(queued_result))
        queued_request.benchmark.assert_not_called()

    def test_tuning_thread_pool_propagates_sticky_cuda_failure(self):
        pool = TuningThreadPool.__new__(TuningThreadPool)
        pool.device_queue = queue.Queue()
        pool.device_queue.put(None)
        pool.device_locks = {None: threading.Lock()}
        pool._poison_lock = threading.Lock()
        pool._cuda_poisoned = False
        choice = mock.Mock()
        choice.bmreq.benchmark.side_effect = RuntimeError("cudaErrorIllegalAddress")

        with self.assertRaisesRegex(RuntimeError, "cudaErrorIllegalAddress"):
            pool.target(choice)

        self.assertEqual(pool.device_queue.get_nowait(), None)

    def test_tuning_thread_pool_skips_queued_work_after_sticky_cuda_failure(self):
        pool = TuningThreadPool.__new__(TuningThreadPool)
        pool.device_queue = queue.Queue()
        pool.device_queue.put(None)
        pool.device_locks = {None: threading.Lock()}
        pool._poison_lock = threading.Lock()
        pool._cuda_poisoned = False
        failed_choice = mock.Mock()
        failed_choice.bmreq.benchmark.side_effect = RuntimeError(
            "cudaErrorIllegalAddress"
        )
        queued_choice = mock.Mock()

        with self.assertRaisesRegex(RuntimeError, "cudaErrorIllegalAddress"):
            pool.target(failed_choice)
        with self.assertRaisesRegex(PoisonedAutotuneWorkerError, "context is poisoned"):
            pool.target(queued_choice)

        queued_choice.bmreq.benchmark.assert_not_called()
        self.assertEqual(pool.device_queue.get_nowait(), None)

    def test_benchmark_choices_propagates_sticky_cuda_failure(self):
        choices = [mock.Mock(), mock.Mock()]
        autotune_args = mock.Mock()
        ...
        with mock.patch.object(
            ...
            with self.assertRaisesRegex(RuntimeError, "cudaErrorIllegalAddress"):
                AlgorithmSelectorCache.benchmark_choices(choices, autotune_args)

        benchmark_choice.assert_called_once_with(choices[0], autotune_args)
...
class TestMaxAutotuneAsyncPipelined(TestMaxAutotune, TestEpilogueFusionStaticAnalysis):
    ...
    def tearDown(self):
        ...
        AsyncAutotuner.invalidated_choice_hashes.clear()
