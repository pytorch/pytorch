...
class TestMaxAutotune(TestCase):
    ...
    def test_persistent_tma_block_local_reduction_cases(self, case):
        if case == "non_power_of_two_block":
            block = 96
        elif case == "unsplit":
            block = 64
        else:
...
class TestEpilogueFusionStaticAnalysis(TestCase):
    ...
    def test_template_local_reduction_rejects_nondivisible_tile(self):
        from torch._inductor.codegen.triton import TritonScheduling

        self.assertFalse(
            TritonScheduling._template_local_reduction_tile_is_compatible(
                (192, 128), (128, 128)
            )
        )
