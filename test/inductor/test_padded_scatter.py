...
class PaddedScatterTest(TestCase):
    ...
    @inductor_config.patch("triton.nested_reduction", False)
    def test_dynamic_quantization_fusion(self):
        ...
        output = torch.empty((256 * 4,), dtype=torch.uint8, device=GPU_TYPE)
        torch._dynamo.mark_dynamic(example, 0, min=1, max=192)
        torch._dynamo.mark_dynamic(output, 0, min=128 * 4, max=256 * 4)
        compiled = torch.compile(f, fullgraph=True)
        with (
            fresh_inductor_cache(),
            V.set_choices_handler(_PersistentReductionChoices()),
        ):
            actual, code = run_and_get_code(compiled, example, output)

        expected = f(example, torch.empty_like(output))
        ...
        FileCheck().check_count(
            ...
            1,
            exactly=True,
        ).check_count("for padded_scatter_offset_", 1, exactly=True).run(code[0])

        for rows in (37, 95, 96, 97, 192):
            x = torch.randn(rows, cols * group, device=GPU_TYPE)
            padded_rows = (rows + 95) // 96 * 128
            actual_output = torch.empty(
                (padded_rows * 4,), dtype=torch.uint8, device=GPU_TYPE
            )
            expected = f(x, torch.empty_like(actual_output))
            actual = compiled(x, actual_output)
            self.assertEqual(expected[0], actual[0], atol=1.0e-3, rtol=1.0e-3)
            self.assertEqual(expected[1], actual[1], atol=0, rtol=0)
    ...
    @inductor_config.patch(
        {"loop_ordering_after_fusion": True, "triton.nested_reduction": True}
    )
    def test_dynamic_nested_reduction_fusion(self):
        ...
        example = torch.randn(101, cols * group, device=GPU_TYPE)
        torch._dynamo.mark_dynamic(example, 0, min=1, max=1024)
        compiled = torch.compile(f, fullgraph=True)
        with (
            fresh_inductor_cache(),
            V.set_choices_handler(_PersistentReductionChoices()),
        ...
        self.assertEqual(metrics.generated_kernel_count, 1)
        self.assertEqual(len(code), 1)
        FileCheck().check("nested_R0_LOCAL_REDUCTION_SIZE").check_count(
            "if tl.program_id(1) == 0 and tl.program_id(2) == 0:",
            1,
            exactly=True,
        ).check_count("for padded_scatter_offset_", 1, exactly=True).run(code[0])
        ...
        with torch._dynamo.config.patch(error_on_recompile=True):
            for rows in (37, 95, 96, 97, 192, 1024):
                x = torch.randn(rows, cols * group, device=GPU_TYPE)
                expected = f(x)
                actual = compiled(x)
                self.assertEqual(expected[0], actual[0], atol=1.0e-3, rtol=1.0e-3)
