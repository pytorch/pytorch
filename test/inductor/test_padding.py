...
class PaddingTest(TestCaseBase):
    ...
    def test_outter_dynamic_shape_padding(self, shape, alignment_bytes, pad_output):
        """
        ...
        with config.patch(config_patches):
            ...
            _, code = run_and_get_code(compiled, *input_tensors)

        s = input_tensors[0].stride()
        pattern = rf"buf0 = empty_strided_{GPU_TYPE}\(\(s\d+, {shape[1]}\), \({s[0]}, {s[1]}\), torch\.float32\)"
        self.assertTrue(re.search(pattern, code[0]))
