...
def gen_transformer_inputs(vocab_size, bs, seq_length, device):
    def geninp():
        return torch.randint(
            0, vocab_size, (bs, seq_length), device=device, dtype=torch.int64, requires_grad=False
        )
        ...
        class Model(nn.Module):
            def __init__(self, device) -> None:
                ...
                for i in range(len(layer_sizes) - 1):
                    mod_list.append(nn.Linear(layer_sizes[i], layer_sizes[i + 1], device=device))
...
class PaddingTest(TestCaseBase):
    ...
    def test_conv(self, device):
        ...
        if DO_PERF_TEST:
            ...
            self.do_profiling(device, lambda: fun(x2, weight), lambda: fun(x1, weight))
    ...
    def test_cat(self, device):
        ...
        self.do_profiling(device, lambda: f(x), lambda: opt_f(x), "Eager Cat", "Compiled Cat")
    ...
    def test_pad_outputs(
        ...
        """
        func = torch.add
        inputs = tuple(torch.randn(*shape, device=device, dtype=dtype) for input_idx in range(2))
    ...
    def test_noop_concat_output_padding(self, device, shape, alignment_bytes, pad_output):
        """
        ...
        output_line = f"buf12 = empty_strided_{dev_type}({output_shape}, {output_stride}, torch.float32)"
    ...
    def test_outer_dynamic_shape_padding(self, device, shape, alignment_bytes, enable_pad):
        """
        ...
        num_inputs = 2
        input_tensors = [
            torch.randn(shape, device=device, dtype=torch.float32) for _ in range(num_inputs)
        ]
