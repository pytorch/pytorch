...
set_caching_module_enabled = lambda enabled: patch.object(config, "IS_CACHING_MODULE_ENABLED", lambda: enabled)
set_deterministic_caching_enabled = lambda enabled: patch.object(config, "IS_DETERMINISTIC_CACHING_ENABLED", lambda: enabled)
set_strictly_pre_populated_determinism = lambda enabled: patch.object(config, "STRICTLY_PRE_POPULATED_DETERMINISM", enabled)
set_strictly_cached_determinism = lambda enabled: patch.object(config, "STRICTLY_CACHED_DETERMINISM", enabled)
set_local_determinism = lambda enabled: patch.object(config, "LOCAL_DETERMINISM", enabled)
set_global_determinism = lambda enabled: patch.object(config, "GLOBAL_DETERMINISM", enabled)
...
def patch_on_disk_cache_base_dir(fn):
    ...
    def wrapper(self, *args, **kwargs):
        ...
        with patch.object(impls._OnDiskCacheImpl, "_base_dir", default_base_dir / f"{self.sub_dir()}/rng-{self.random_string[4:]}"):
...
def patch_deterministic_cache_intf_no_dump_on_exit(fn):
    ...
    def wrapper(*args, **kwargs):
        ...
        def patched_init(intf: intfs._DeterministicCacheIntf, *args: Any, **kwargs: dict[str, Any]) -> None:
...
class ExceptionsTest(TestCase):
    ...
    def test_exception_other(self) -> None:
        ...
        self.assertTrue(
            issubclass(
                exceptions.DeterministicCachingRequiresStrongConsistencyError, exceptions.UserError
            )
        )
...
class InterfacesTest(TestMixin, TestCase):
    ...
    def assert_cached_call_is(
        self, fn, params, result, intf, *args, **kwargs
    ) -> None:
        self.assertTrue(((get := intf.get(fn, params, *args, **kwargs)) is not None) and (get.value == result))
    ...
    def test_defaults(self, intf_typename: str) -> None:
        ...
        args, kwargs = (1, 2, 3,), {"bar": "bar"}
        params = (args, kwargs)
        self.assertEqual(foo(*args, **kwargs), params)
        start_t: float = time()
        self.assertEqual(foo(*args, **kwargs), params)
        self.assertTrue((time() - start_t) < sleep_t)
        self.assert_cached_call_is(foo, params, params, intf, ischema=context._DEFAULT_ISOLATION_SCHEMA)
    ...
    def test_strictly_pre_populated_determinism(self) -> None:
        intf: intfs._DeterministicCacheIntf = self.intf_from_typename("_DeterministicCacheIntf")
        ...
        self.assertTrue(((get := intf.get(foo, params)) is not None) and (get.value == result))
        ...
        with (
            ...
            with self.assertRaises(exceptions.StrictDeterministicCachingKeyNotFoundError):
                foo()
    ...
    def test_caching_module_disabled(self) -> None:
        intf: intfs._DeterministicCacheIntf = intfs._DeterministicCacheIntf()
