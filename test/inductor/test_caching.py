...
class ExceptionsTest(TestCase):
    ...
    def test_exception_is_CacheError(self, exception_typename: str) -> None:
        ...
        """
        self.assertTrue(issubclass(getattr(exceptions, exception_typename), exceptions.CacheError))
    ...
    def test_exception_other(self) -> None:
        ...
        self.assertTrue(issubclass(exceptions.FileLockTimeoutError, exceptions.SystemError))
        ...
        self.assertTrue(issubclass(exceptions.KeyPicklingError, exceptions.KeyEncodingError))
        self.assertTrue(issubclass(exceptions.ValueEncodingError, exceptions.UserError))
        self.assertTrue(issubclass(exceptions.ValuePicklingError, exceptions.ValueEncodingError))
        self.assertTrue(issubclass(exceptions.ValueDecodingError, exceptions.UserError))
        self.assertTrue(issubclass(exceptions.ValueUnPicklingError, exceptions.ValueDecodingError))
...
class LocksTest(TestMixin, TestCase):
    ...
    def test_acquire_with_timeout(
        ...
        lock_or_flock: Union[Lock, FileLock] = Lock() if lock_typename == "Lock" else FileLock(str(flock_fpath))
        lock_exception_type: type = exceptions.LockTimeoutError if lock_typename == "Lock" else exceptions.FileLockTimeoutError
            ...
            if release == "unlocked":
                ...
                if lock_timeout == "BLOCKING":
                    with self.assertRaises(TimeoutError):
                        lock_or_flock_future.result(timeout=locks._BLOCKING_WITH_TIMEOUT)
...
class UtilsTest(TestMixin, TestCase):
    def test_lru_cache(self) -> None:
        ...
        self.assertEqual(foo(0.0), ((0.0,), {},))
        self.assertEqual(foo("foo"), (("foo",), {},))
        self.assertEqual(foo("foo", bar="bar"), (("foo",), {"bar": "bar"},))
