...
class TestSelectAlgorithmCuda(BaseTestSelectAlgorithm):
    ...
    def test_int8_woq_mm_cuda(self, dtype, batch_size, mid_dim, in_features, out_features):
        """Test CUDA int8pack_mm with woq optimization pattern"""
    ...
    @unittest.skipIf(not TEST_CUDA, "CUDA not available")
    def test_int8_woq_mm_concat_cuda(self, dtype, batch_size, mid_dim, in_features, out_features):
        """Test CUDA concat linear int8pack_mm with woq optimization pattern"""
        def _convert_weight_to_int8pack(w):
            # Move to CPU for quantization calculation, then back to original device
            device = w.device
            w_cpu = w.cpu()
