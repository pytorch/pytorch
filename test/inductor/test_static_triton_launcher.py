...
class TestStaticTritonLauncher(TestCase):
    ...
    @skipIfRocm
    @skipIfXpu
    @unittest.skipUnless(SM90OrLater, "sm90+ static launch coverage")
    def test_static_launch_sm90(self):
        """Static launch of a Triton kernel, gated on sm90+.

        Mirrors test_basic. The gate means it never runs unless
        .ci/pytorch/test.sh selects this file for the ciflow/h100 job, which is
        the situation #191133 hit.
        """

        @triton.jit
        def sm90_add_kernel(in_ptr, out_ptr, n, BLOCK: tl.constexpr):
        ...
        compiled_kernel = sm90_add_kernel[(1,)](src, dst, n, BLOCK=n)
