...
if RUN_GPU:
    ...
    tmp1 = tl.load(tl.make_block_ptr(in_ptr1, shape=[262144, 512], strides=[1, 262144], block_shape=[XBLOCK, R0_BLOCK], order=[0, 1], offsets=[xoffset, roffset]), boundary_check=[1], padding_option='zero', eviction_policy='evict_first')
        ...
        def test_split_reduction_comment_graph_fragment(self):
            def check_fragment_references_are_defined(graph_fragments):
                for fragment in graph_fragments:
                    declared_names = set()
                    referenced_names = set()
                    for line in fragment.splitlines():
                        lhs_match = re.match(r"#   (?P<lhs>.*?) = ", line)
                        if lhs_match:
                        ...
                        if "args = " in line or line.startswith("#   return "):
                            referenced_names.update(
                                re.findall(r"%([A-Za-z_][A-Za-z0-9_]*)\b", line)
                            )

                    self.assertTrue(
                        referenced_names <= declared_names,
                        (
                            f"undefined references: "
                            f"{sorted(referenced_names - declared_names)}\n"
                            f"{fragment}"
                        ),
                    )
            ...
            def check_metadata(code, reduction_kind):
                ...
                check_fragment_references_are_defined(graph_fragments)
            ...
            for mix_order_reduction in (False, True):
                with self.subTest(
                    mix_order_reduction=mix_order_reduction,
                    reduction_kind="var_mean",
                ):

                    @torch.compile(backend="inductor")
                    def fn(x):
                        return torch.var_mean(x, dim=[0], keepdim=True, correction=0)
                    ...
                    graph_fragments = [
                        ...
                        if "Graph fragment" in block
                        and "torch.ops.aten.var_mean.correction" in block
                    ]
                    self.assertEqual(len(graph_fragments), 2, code)
                    check_fragment_references_are_defined(graph_fragments)
