...
def forward(self, arg0_1: "Sym(s77)", arg1_1: "Sym(s27)", arg2_1: "Sym(s53)", arg3_1: "u8[s77, s27, s53][s27*s53, s53, 1]{str(x.device)}"):
    ...
    def test_autotune_large_outer_reduction_plans(self):
        ...
        def with_epilogue(x, y, stats):
            reduced = (x.float() * y.float() * torch.rsqrt(stats + 1e-5)).sum(dim=0)
            return reduced.to(torch.bfloat16).float()

        epilogue_expected = with_epilogue(x, y, stats)
        epilogue_actual, epilogue_source_codes = run_and_get_code(
            torch.compile(with_epilogue, fullgraph=True), x, y, stats
        )
        self.assertEqual(epilogue_expected, epilogue_actual, atol=0.125, rtol=0.01)
        epilogue_code = epilogue_source_codes[0]
        self.assertEqual(epilogue_code.count("async_compile.multi_kernel_plan("), 1)
        # Keep the unsupported epilogue outside the two complete reduction
        # plans instead of silently committing to the structural sequence.
        self.assertEqual(epilogue_code.count("async_compile.triton("), 4)
