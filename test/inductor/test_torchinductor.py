...
def forward(self, arg0_1: "Sym(s77)", arg1_1: "Sym(s27)", arg2_1: "Sym(s53)", arg3_1: "f32[s77, s27, s53][s27*s53, s53, 1]{str(x.device)}"):
    ...
    def test_fmod_remainder_scalar_type_promotion_bf16_fp16(self, dtype):
        ...
        compiled_fn = torch.compile(fn, fullgraph=True)
        x = torch.tensor([1.7], device=self.device, dtype=dtype)
        expected = fn(x)
        ...
        if self.device == "mps":
            ...
            for opmath_result, rounded_result in zip(opmath_expected, rounded_expected):
                self.assertNotEqual(opmath_result.item(), rounded_result.item())
            self.assertNotEqual(
                scalar_first_opmath_expected.item(),
                torch.remainder(rounded_scalar, x).item(),
            )
