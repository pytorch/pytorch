...
def forward(self, arg0_1: "Sym(s77)", arg1_1: "Sym(s27)", arg2_1: "Sym(s53)", arg3_1: "f32[s77, s27, s53][s27*s53, s53, 1]{str(x.device)}"):
    ...
    def test_mixed_dtype_xlogy_noncontiguous(self):
        if self.device != "cuda":
            raise unittest.SkipTest("CUDA-specific TensorIterator stride behavior")

        def fn(a, b):
            return torch.xlogy(a, b)

        a = torch.rand(4, 6, device=self.device, dtype=torch.float32).t()
        b = torch.rand(6, 4, device=self.device, dtype=torch.float64)
    ...
    def test_mixed_dtype_where_noncontiguous_stride(self):
        if self.device != "cuda":
            raise unittest.SkipTest("CUDA-specific TensorIterator stride behavior")

        shape = (2, 3)
        pred = torch.empty_strided(
            shape, (0, 1), device=self.device, dtype=torch.bool
        ).fill_(True)
        a = torch.empty_strided(
            shape, (0, 1), device=self.device, dtype=torch.float32
        ).fill_(1)
        b = torch.empty_strided(
            shape, (1, 2), device=self.device, dtype=torch.float64
        ).fill_(2)

        ref = torch.where(pred, a, b)
        actual = torch.compile(torch.where, backend="inductor", fullgraph=True)(
            pred, a, b
        )
        torch.cuda.synchronize()
    ...
    def test_mixed_dtype_zero_size_broadcast_stride(self):
        if self.device != "cuda":
            raise unittest.SkipTest("CUDA-specific TensorIterator stride behavior")

        a = torch.empty_strided((1,), (1,), device=self.device, dtype=torch.float32)
        b = torch.empty_strided((0, 3), (3, 3), device=self.device, dtype=torch.float64)

        ref = torch.add(a, b)
        actual = torch.compile(torch.add, backend="inductor", fullgraph=True)(a, b)
        torch.cuda.synchronize()
