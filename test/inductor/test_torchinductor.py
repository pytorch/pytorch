...
class CommonTemplate:
    ...
    @skip_if_gpu_halide
    @torch._inductor.config.patch({"joint_graph_constant_folding": True})
    def test_constant_fold_expanded_nonzero_training(self):
        def fn(x):
            bias = torch.full((1, x.shape[1]), 2.0, dtype=x.dtype, device=x.device)
            y = x + bias.expand(x.shape)
            return (y.sin() * x).sum()
        ...
        actual = torch.compile(fn, backend="inductor")(actual_x)
        actual.backward()
