...
def create_new_common_inductor_test_class(subclass_testcase: bool = False):
    """
    ...
    allows unique `CommonTemplate` classes to be returned so that each call to
    `instantiate_device_type_tests` operates on a generic test class, allowing each
    device to customise the instantiation of the template.
    """
    bases = (object,)
    if subclass_testcase:
        bases = (TestCase,) + bases

    class CommonTemplate(*bases):
        def common(self, *args, **kwargs):
            device_type = get_device_from_test_args_kwargs(self, *args, **kwargs)
            if device_type == "cpu":
                return check_model(self, *args, **kwargs)
            return check_model_device(self, *args, device=device_type, **kwargs)
        ...
        def test_aoti_eager_support_out(self):
            ...
            with _scoped_library("aten", "IMPL") as torch_compile_op_lib_impl:
                ref_out_tensor = torch.randn(
                    128, dtype=torch.float, device=device
                ).fill_(-1)
                ref_tensor = torch.clamp(
                    max=max_tensor, min=min_tensor, input=inp_tensor, out=ref_out_tensor
                )

                ref_out_tensor1 = torch.randn(
                    128, dtype=torch.float, device=device
                ).fill_(-1)
        ...
        def test_add_complex3(self):
            ...
            x = torch.randn(
                41, dtype=torch.complex64, device=get_device_from_test_args_kwargs(self)
            )
            y = x.clone()
        ...
        def test_randn_generator(self):
            ...
            with self.assertRaisesRegex(torch._dynamo.exc.Unsupported, "Generator"):
                self.common(
                    fn,
                    (
                        torch.linspace(-10, 10, 41),
                        torch.Generator(get_device_from_test_args_kwargs(self)),
                    ),
                )
        ...
        def test_horizonal_fusion1(self):
            ...
            self.common(
                fn,
                (torch.randn(8, 16, 16), torch.randn(8, 16, 16), torch.randn(1, 16, 1)),
            )
        ...
        def test_horizonal_fusion2(self):
            ...
            self.common(
                fn, (torch.randn(8, 16, 8), torch.randn(8, 16), torch.randn(16, 8))
            )
        ...
        def test_scheduler_vertical_fusion1(self):
            ...
            assertGeneratedKernelCountEqual(
                self,
                1 if not is_cpp_backend(get_device_from_test_args_kwargs(self)) else 2,
            )
        ...
        def test_index_propagation_nested_indirect_indexing(self):
            ...
            example_inputs = (
                ...
                repeats := torch.tensor(
                    [5, 10, 15], device=get_device_from_test_args_kwargs(self)
                ),
            )
        ...
        def test_neg_index(self):
            def test(
                ...
                elif get_device_from_test_args_kwargs(
                    self
                ) == "cpu" and not is_triton_cpu_backend(
                    get_device_from_test_args_kwargs(self)
                ):
            ...
            b = torch.zeros(
                256, dtype=torch.long, device=get_device_from_test_args_kwargs(self)
            )
        ...
        def test__unsafe_masked_index(self):
            ...
            self.common(
                ...
                (
                    ...
                    [
                        torch.tensor(
                            [3, 9, 2], device=get_device_from_test_args_kwargs(self)
                        )
                    ],
                ),
            )
        ...
        def test_sum3(self):
            ...
            self.common(
                fn, (torch.randn(10, 10), torch.randn(1, 10)), atol=1e-5, rtol=2e-3
            )
        ...
        def test_reduction4(self):
            ...
            for i in inputs:
                self.common(
                    fn,
                    (i,),
                    check_lowp=not is_halide_backend(
                        get_device_from_test_args_kwargs(self)
                    ),
                )
        ...
        def test_multilayer_prime_size(self):
            ...
            if get_device_from_test_args_kwargs(self) == "mps" and MACOS_VERSION < 13.3:
                raise unittest.SkipTest(
                    "Fails with internal compiler error on MacOS-13"
                )
        ...
        def test_multilayer_var_lowp(self):
            ...
            rtol = None
            if (
                get_device_from_test_args_kwargs(self) == "cpu"
                and os.getenv("ATEN_CPU_CAPABILITY") == "default"
            ):
                atol = 1e-3
                rtol = 1e-3
        ...
        def test_split_cumsum(self):
            ...
            for dtype in get_all_dtypes(
                ...
                self.common(
                    fn, (inp.view(10, -1),), rtol=1e-4, atol=1e-5, check_lowp=False
                )
        ...
        def test_custom_scan_op_multi_input(self):
            ...
            def argmax_combine(a, b):
                ...
                mask = (a_value > b_value) | (
                    (a_value == b_value) & (a_index > b_index)
                )
                return (
            ...
            idx = (
                torch.arange(100, device=get_device_from_test_args_kwargs(self))
                .view(100, 1)
                .expand(100, 100)
            )
        ...
        def test_embedding_bag_byte_unpack(self):
            if get_device_from_test_args_kwargs(self) != "cpu":
                raise unittest.SkipTest(
                    f"No {GPU_TYPE} implementation (it returns empty)"
                )
        ...
        def test_expanded_reduction(self):
            ...
            self.common(
                fn,
                (torch.randn(2, 197, 256), torch.randn(2, 1, 256)),
                atol=atol,
                rtol=rtol,
            )
        ...
        def test_min_max_reduction(self):
            ...
            for dtype in dtypes:
                self.common(
                    fn, (torch.randn(8, 8).to(dtype), torch.randn(8, 8).to(dtype))
                )
        ...
        def test_cumsum_no_mask(self):
            ...
            self.common(
                ...
                check_lowp=not (
                    TEST_WITH_ROCM
                    or is_halide_backend(get_device_from_test_args_kwargs(self))
                ),
            )
        ...
        def test_clamp_type_promotion(self):
            ...
            def fn(a):
                b = torch.tensor(
                    1.0, dtype=tgt_dtype, device=get_device_from_test_args_kwargs(self)
                )
        ...
        def test_arange1(self):
            def fn(x):
                rng1 = torch.arange(8 * 8, dtype=torch.float32, device=x.device).view(
                    8, 8
                )
        ...
        def test_builtins_round(self):
            ...
            x = torch.zeros(
                5, dtype=torch.int, device=get_device_from_test_args_kwargs(self)
            )
            with torch.no_grad():
                for i in range(1, 6):
        ...
        def test_large_tensor_reduction(self):
            ...
            t = torch.ones(
                2**32, dtype=torch.int8, device=get_device_from_test_args_kwargs(self)
            )
            t[-1] = 2
            ...
            expect = torch.tensor(
                2, dtype=torch.int8, device=get_device_from_test_args_kwargs(self)
            )
        ...
        def test_large_broadcast_reduction(self):
            ...
            expect = torch.tensor(
                4, dtype=torch.int8, device=get_device_from_test_args_kwargs(self)
            )
            self.assertEqual(actual, expect)
        ...
        def test_mixed_mm3(self):
            ...
            self.common(
                ...
                check_lowp=get_device_from_test_args_kwargs(self) != "mps"
        ...
        def test_mm_mixed_dtype(self):
            ...
            t1 = torch.arange(
                6, dtype=torch.float, device=get_device_from_test_args_kwargs(self)
            ).view(2, 3)
            t2 = torch.arange(
                9, dtype=torch.int64, device=get_device_from_test_args_kwargs(self)
            ).view(3, 3)
        ...
        def test_linear_mixed_dtype(self):
            ...
            t = torch.arange(27, device=get_device_from_test_args_kwargs(self)).view(
                3, 3, 3
            )
        ...
        def test_linear_dynamic_maxautotune(self):
            if get_device_from_test_args_kwargs(self) == "cpu":
                raise unittest.SkipTest(
                    "using triton backend only is not supported on CPU"
                )
        ...
        def test_scalar_output(self):
            ...
            arg0_1 = rand_strided(
                (32128, 768), (768, 1), device="cpu", dtype=torch.float32
            )
        ...
        def test_conv_inference_heuristics(self):
            ...
            input_tensor = torch.randn(1, in_channels, 10, 10).to(
                get_device_from_test_args_kwargs(self)
            )
        ...
        def test_device_assert(self):
            ...
            y = torch.ones(
                1, dtype=dtype, device=get_device_from_test_args_kwargs(self)
            )
        ...
        def test_to_device(self):
            def fn(a):
                if a.device.type == "cpu":
                    ...
                    return aten._to_copy(
                        a, device=torch.device("cpu"), dtype=6, layout=0
                    )
        ...
        def test_adaptive_avg_pool2d_low_prec(self):
            ...
            for dtype in [torch.half, torch.bfloat16]:
                ...
                if not self.is_dtype_supported(dtype):
                    continue
                x = torch.randn(
                    4, 3, 7, 7, device=get_device_from_test_args_kwargs(self)
                ).to(dtype=dtype)
        ...
        def test_buffer_copied_in_graph(self):
            ...
            inp_ref = torch.ones(
                1, requires_grad=True, device=get_device_from_test_args_kwargs(self)
            )
            inp_test = torch.ones(
                1, requires_grad=True, device=get_device_from_test_args_kwargs(self)
            )
        ...
        def test_buffer_copied_in_graph_with_different_shapes(self):
            ...
            inp_ref = torch.ones(
                2, 4, requires_grad=True, device=get_device_from_test_args_kwargs(self)
            )
            inp_test = torch.ones(
                2, 4, requires_grad=True, device=get_device_from_test_args_kwargs(self)
            )
        ...
        def test_avg_pool2d5(self):
            def fn(x):
                return aten.avg_pool2d(
                    x, [3, 3], [2, 2], [1, 1], count_include_pad=False
                )
        ...
        def test_alexnet_prefix(self):
            ...
            self.common(
                ...
                (
                    ...
                    rand_strided(
                        (64, 3, 11, 11), (363, 121, 11, 1), torch.float32, "cpu"
                    ),
                    rand_strided(
        ...
        def test_elu(self):
            def fn(x):
                return aten.elu(
                    x, 1.6732632423543772, 1.0507009873554805
                ) + 2, aten.elu(x + 1, 2, 3, 4)
        ...
        def test_randn_with_dtype_and_device(self):
            if get_device_from_test_args_kwargs(self) == GPU_TYPE:
                raise unittest.SkipTest(
                    "only support cpu randn_with_dtype_and_device test"
                )
        ...
        def test_transposed_propagates(self):
            ...
            a = torch.randn(
                1, 4, 4, 4, device=get_device_from_test_args_kwargs(self)
            ).permute(0, 2, 3, 1)
            b = torch.randn(
                4, 4, 4, device=get_device_from_test_args_kwargs(self)
            ).permute(1, 2, 0)
            c = fn(a, b)
        ...
        def test_matmul_layer_norm(self):
            ...
            layer_norm = torch.nn.LayerNorm(
                hidden_size, device=get_device_from_test_args_kwargs(self)
            )
        ...
        def test_view_as_complex(self):
            ...
            inp = torch.empty_strided(
                (128, 64, 12, 32, 2), (1, 98304, 8192, 256, 128)
            ).to(get_device_from_test_args_kwargs(self))
            mod = Repro()
        ...
        def test_polar(self):
            ...
            self.common(
                fn,
                (*inp,),
                reference_in_float=get_device_from_test_args_kwargs(self) != "mps",
            )
        ...
        def test_fusing_write_into_disjoint_read(self):
            ...
            if get_device_from_test_args_kwargs(self) != "cpu":
                ...
                a = torch.rand(
                    (1, 1000000), device=get_device_from_test_args_kwargs(self)
                )
                self.common(f, (a,))
        ...
        def test_masked_fill_promotion(self):
            ...
            for inp in (
                ...
                torch.randint(
                    16, (16, 16), device=get_device_from_test_args_kwargs(self)
                ),
            ):
                inputs = (
                    torch.randint(
        ...
        def test_masked_scatter(self):
            ...
            mask = make_tensor(
                10, 10, dtype=torch.bool, device=get_device_from_test_args_kwargs(self)
            )
            source = make_tensor(
                mask.count_nonzero(),
                dtype=torch.float32,
        ...
        def test_remove_no_ops(self):
            ...
            inps = [
                torch.rand([256, 256], device=get_device_from_test_args_kwargs(self))
                for _ in range(2)
            ]
    ...
    def forward(self, arg0_1: "Sym(s77)", arg1_1: "Sym(s27)", arg2_1: "Sym(s53)", arg3_1: "f32[s77, s27, s53][s27*s53, s53, 1]{str(x.device)}"):
        ...
        def test_deterministic_codegen(self):
            if (
                "cpu" in str(get_device_from_test_args_kwargs(self))
        ...
        def test_full_boolean(self):
            def fn(n):
                x = torch.full(
                    (1,), n >= 1024, device=get_device_from_test_args_kwargs(self)
                )
                return x, x + 1
        ...
        def test_output_strides(self):
            ...
            inp = torch.rand(
                [4, 4, 4, 4], device=get_device_from_test_args_kwargs(self)
            )
        ...
        def test_float_index_expression(self):
            ...
            for code in source_codes:
                self.assertIsNone(
                    ...
                    msg="Found bad index_expr in code:\n" + code,
                )
        ...
        def test_constant_pad_1d(self):
            ...
            self.common(
                fn, (torch.randint(0, 999, size=[2, 16, 31], dtype=torch.float32),)
            )
        ...
        def test_constant_pad_fill_dtype(self):
            ...
            self.common(
                fn,
                (
                    torch.randint(2, (4,), dtype=torch.bool),
        ...
        def test_l1_loss(self):
            def fn(a, b):
                return torch.nn.functional.l1_loss(a, b), torch.nn.functional.mse_loss(
                    a, b
                )
        ...
        def test_inplace_add(self):
            ...
            inputs = (
                rand_strided(
                    (4, 4), (4, 1), device=get_device_from_test_args_kwargs(self)
                ),
                rand_strided(
                    (4, 4), (4, 1), device=get_device_from_test_args_kwargs(self)
                ),
        ...
        def test_kernel_names(self):
            ...
            inputs = (
                rand_strided((8,), (1,), device=get_device_from_test_args_kwargs(self)),
            )
        ...
        def test_strided_inputs(self):
            ...
            inputs = (
                rand_strided(
                    (8, 16), (32, 2), device=get_device_from_test_args_kwargs(self)
                ),
                rand_strided(
                    (8, 16), (16, 1), device=get_device_from_test_args_kwargs(self)
                ),
            )
        ...
        def test_input_mutation2(self):
            ...
            arg1 = (
                torch.randn([1, 64], device=get_device_from_test_args_kwargs(self))
                .requires_grad_(True)
                .add(1)
            )
            arg2 = arg1.clone()
        ...
        def test_input_mutation5(self):
            ...
            a = torch.zeros(
                (), dtype=torch.int64, device=get_device_from_test_args_kwargs(self)
            )
            a_expect = a.clone()
            expect = fn(a_expect)
        ...
        def test_tensor_index_slice(self):
            def fn(a):
                ...
                xx = torch.tensor(
                    [1, 2], device=get_device_from_test_args_kwargs(self)
                ).view(1, 2)
                yy = torch.tensor(
                    [1, 2, 3], device=get_device_from_test_args_kwargs(self)
                ).view(3, 1)
                return [
                    a[x, y],
                    a[:, x, y],
            ...
            a = torch.arange(
                3 * 4 * 5 * 6 * 7, device=get_device_from_test_args_kwargs(self)
            ).view(3, 4, 5, 6, 7)
            refs = fn(a)
        ...
        def test_nll_loss_backward(self):
            def fn(a, b, c):
                return aten.nll_loss_backward(
                    ...
                    1,
                    -100,
                    torch.tensor(1.0, device=get_device_from_test_args_kwargs(self)),
                )
        ...
        def test_isinf(self):
            ...
            for dtype in [torch.float32, torch.float64, torch.half, torch.bfloat16]:
                ...
                with ctx:
                    self.common(
                        fn, [torch.tensor(values, dtype=dtype)], check_lowp=False
                    )
        ...
        def test_inplace_activations(self):
            def fn(x):
                ...
                e = aten.log1p(x + 1)
                f = aten.masked_fill_(
                    x + 1, torch.zeros_like(x, dtype=torch.bool), 99.0
                )
        ...
        def test_dropout(self):
            ...
            x = torch.ones(
                1000, device=get_device_from_test_args_kwargs(self), dtype=torch.float32
            )
            result1 = fn1(x)
        ...
        def test_rand_like_deterministic(self):
            ...
            x = torch.ones(
                1024, device=get_device_from_test_args_kwargs(self), dtype=torch.float32
            )
        ...
        def test_philox_rand(self):
            ...
            self.assertEqual(
                getattr(
                    torch, get_device_from_test_args_kwargs(self)
        ...
        def test_softmax_backward_data(self):
            def fn(a, b):
                return aten._softmax_backward_data(
                    a, b, dim=1, input_dtype=torch.float32
                )
        ...
        def test_randint(self):
            ...
            a0, b0, c0 = fn(
                torch.zeros([40, 40], device=get_device_from_test_args_kwargs(self))
            )
        ...
        def test_randint_distribution(self):
            ...
            def fn(n_argsmax, size):
                return torch.randint(
                    n_max, (size,), device=get_device_from_test_args_kwargs(self)
                )
        ...
        def test_check_stack_no_cycles(self):
            ...
            r = fn(
                torch.randn(
                    2, device=get_device_from_test_args_kwargs(self), requires_grad=True
                )
            )
        ...
        def test_like_rands2(self):
            ...
            x = torch.ones(
                10, device=get_device_from_test_args_kwargs(self), dtype=torch.float32
            )
            a0 = fn(x).clone()
            a1 = fn(x).clone()
            self.assertFalse(torch.allclose(a0, a1))
        ...
        def test_dropout2(self):
            ...
            ones = torch.ones(
                n, device=get_device_from_test_args_kwargs(self), dtype=torch.float32
            )
        ...
        def test_dropout3(self):
            ...
            result, (fw_code, bw_code) = run_fw_bw_and_get_code(
                lambda: run(
                    torch.randn([8, 32], device=get_device_from_test_args_kwargs(self))
                )
            )
        ...
        def test_randint_kernel_count(self):
            ...
            @torch._dynamo.optimize_assert("inductor")
            def fn1():
                random_tensor1 = torch.randint(
                    10, [32], device=get_device_from_test_args_kwargs(self)
                )
                random_tensor2 = torch.randint(
                    10, [32], device=get_device_from_test_args_kwargs(self)
                )
                random_tensor3 = torch.randint(
                    10, [32], device=get_device_from_test_args_kwargs(self)
                )
        ...
        def test_tmp_not_defined_issue1(self, use_block_ptr):
            def forward(
                ...
                add_tensor_3 = torch.ops.aten.add.Tensor(
                    broadcast_in_dim_default_2, 1e-05
                )
        ...
        def test_tmp_not_defined_issue3(self):
            ...
            def forward(
                ...
                permute: "f32[6, 1001]" = torch.ops.aten.permute.default(
                    primals_1, [1, 0]
                )
                ...
                unsqueeze: "f32[6144, 1, 64]" = torch.ops.aten.unsqueeze.default(
                    getitem, 1
                )
            ...
            kwargs = aot_graph_input_parser(
                forward, device=get_device_from_test_args_kwargs(self)
            )
        ...
        def test_zero_dim_reductions(self):
            for kd in [True, False]:
                ...
                pass_ops = [
                    lambda *x: fn(*x)
                    for fn in [aten.sum, aten.prod, aten.any, aten.all]
                ]
                for po in pass_ops:
        ...
        def test_inductor_layout_optimization_input_mutations(self):
            ...
            mod = nn.Conv2d(3, 128, 1, stride=1, bias=False).to(
                get_device_from_test_args_kwargs(self)
            )
            ...
            x_ref = torch.rand(
                2, 3, 128, 128, device=get_device_from_test_args_kwargs(self)
            )
        ...
        def test_inductor_multiple_specializations(self):
            ...
            dynamic_specialized_a = torch.randn(
                m, k, device=GPU_TYPE, dtype=torch.bfloat16
            )
        ...
        def test_insignificant_strides(self):
            ...
            x = torch.arange(
                8, device=get_device_from_test_args_kwargs(self), dtype=torch.float32
            )
            out = f(x)
        ...
        def test_where_broadcast(self):
            ...
            class Repro(torch.nn.Module):
                def __init__(self) -> None:
                    super().__init__()
                    self._tensor_constant0 = nn.Buffer(
                ...
                def forward(self, arg0_1, arg1_1):
                    ...
                    bitwise_not = torch.ops.aten.bitwise_not.default(
                    ...
                    where = torch.ops.aten.where.self(
                        bitwise_not, lift_fresh_copy, arg0_1
                    )
        ...
        def test_sdpa_unaligned_mask_freezing(self):
            class Mod(torch.nn.Module):
                ...
                def forward(
                    ...
                    arg0_1 = arg1_1 = arg2_1 = expand = None
                    getitem: "f32[8, 8, 16, 16]" = (
                        _scaled_dot_product_efficient_attention[0]
                    )
                    _scaled_dot_product_efficient_attention = None
                    return (getitem,)
        ...
        def test_getitem(self):
            ...
            x = [
                torch.rand(
                    [1, 256, 100, 152], device=get_device_from_test_args_kwargs(self)
                ),
                torch.rand(
                    [1, 256, 50, 76], device=get_device_from_test_args_kwargs(self)
                ),
                torch.rand(
                    [1, 256, 25, 38], device=get_device_from_test_args_kwargs(self)
                ),
            ]
            opt_fn = torch.compile(fn, backend="inductor")
            same(fn(x), opt_fn(x))
        ...
        def test_AllenaiLongformerBase_repro(self):
            def fn(query, scores, window_overlap):
                ...
                input_tensor[
                    :, :window_overlap, :, : window_overlap + 1
                ] = torch.full_like(beginning_input, -float("inf"))
                return input_tensor
        ...
        def _check_resize_common(
            ...
            self.assertTrue(
                same(correct_values, actual_values, equal_nan=deterministic)
            )
        ...
        def test_scaled_dot_product_attention(self):
            if (
                get_device_from_test_args_kwargs(self) == "cuda"
                and not PLATFORM_SUPPORTS_FLASH_ATTENTION
            ):
        ...
        def test_searchsorted(self):
            ...
            shapes = (
                ...
                (
                    (3, 5),
                    (3, 7),
                ),  # prime dimensioned sequence, to flush out indexing bugs
            )
            booleans = (False, True)
        ...
        def test_bucketize_int(
            ...
            offsets = (torch.arange(10, dtype=torch.int32) ** 2 - 512).to(
                dtype_boundaries
            )
            ...
            for out_int32 in [True, False]:
                for right in [True, False]:
                    self.common(
                        fn, (input, offsets, out_int32, right), check_lowp=False
                    )
        ...
        def test_mutable_custom_op_fixed_layout(self):
            with torch.library._scoped_library("mylib", "DEF") as lib:
                ...
                compiled_inductor_f = torch.compile(
        ...
        def test_custom_op_fixed_layout_channels_last(self):
            ...
            input_t = torch.randn(
                [1, 320, 128, 128], dtype=torch.float32, device=GPU_TYPE
            )
        ...
        def test_custom_op_default_layout_constraint(self):
            with torch.library._scoped_library("mylib", "DEF") as lib:
                ...
                def f(x):
                    full_default_3 = torch.full(
                        [3, 3], 7.0, device=get_device_from_test_args_kwargs(self)
                    )
                    chunk_cat_default_1 = torch.ops.mylib.copy_.default(
                        full_default_3, x
                    )
        ...
        def test_buffer_use_after_remove(self):
            ...
            def run(fn):
                coord = torch.ones(
                    (2, 3), device=get_device_from_test_args_kwargs(self)
                )
                rot = nn.Parameter(
                    torch.ones((2, 3), device=get_device_from_test_args_kwargs(self))
                )
                trans = nn.Parameter(
                    torch.ones((2, 3), device=get_device_from_test_args_kwargs(self))
                )
        ...
        def test_float16_to_int16(self):
            ...
            x = torch.ones(
                4, dtype=torch.float16, device=get_device_from_test_args_kwargs(self)
            )
            ref = fn(x)
        ...
        def test_bfloat16_to_int16(self):
            ...
            if not self.is_dtype_supported(torch.bfloat16):
                raise unittest.SkipTest(
                    "bfloat16 is not supported on {get_device_from_test_args_kwargs(self)}"
                )
            a = torch.ones(
                4, dtype=torch.bfloat16, device=get_device_from_test_args_kwargs(self)
            )
            b = torch.ones(
                4, dtype=torch.bfloat16, device=get_device_from_test_args_kwargs(self)
            )
        ...
        def test_float32_to_int32(self):
            ...
            a = 0.5 * torch.ones(
                4, dtype=torch.float32, device=get_device_from_test_args_kwargs(self)
            )
            b = 0.5 * torch.ones(
                4, dtype=torch.float32, device=get_device_from_test_args_kwargs(self)
            )
        ...
        def test_pointwise(self, name, op):
            ...
            if name in {"gammainc", "gammaincc"}:
                args = (
                    torch.randn(
                        8, 8, dtype=dtype, device=get_device_from_test_args_kwargs(self)
                    ),
                    torch.empty(
                        8, 8, dtype=dtype, device=get_device_from_test_args_kwargs(self)
                    ).uniform_(1, 2),
        ...
        def test_large_grid(self):
            ...
            compiled_fn = torch.compile(fn)
            actual = compiled_fn(
                torch.ones(s0, s1, device=get_device_from_test_args_kwargs(self))
            )
        ...
        def test_weight_norm_bwd(self):
            ...
            def f(m, x):
                with torch.amp.autocast(
                    device_type=get_device_from_test_args_kwargs(self), dtype=torch.half
                ):
                    loss = m(x).sum()
                    loss.backward()
                return loss
        ...
        def test_size_asserts_for_multi_output_fallback(self):
            ...
            if is_dynamic_shape_enabled():
                FileCheck().check(
                    "assert_size_stride(buf1, (s77, s27), (s27, 1)"
                ).check("assert_size_stride(buf2, (s77, s27), (s27, 1)").run(code)
            else:
        ...
        def test_graph_partition_pad_dynamic(self):
            ...
            x = torch.randn(
                2, 24, 110, 110, device=get_device_from_test_args_kwargs(self)
            )
        ...
        def test_split_reduction_with_int64_size(self):
            ...
            t = torch.rand(
                size, dtype=torch.float, device=get_device_from_test_args_kwargs(self)
            )
            op = torch.mean
            expected = op(t)
...
CommonTemplate = instantiate_parametrized_tests(create_new_common_inductor_test_class())
TestTorchInductor = make_test_cls_with_patches(
    create_new_common_inductor_test_class(subclass_testcase=True),
    "TestTorchInductor",
    "",
