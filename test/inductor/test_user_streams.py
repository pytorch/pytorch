...
class TestUserStreamCompile(InductorTestCase):
    ...
    def test_codegen_structure_single_stream(self):
        ...
        graph = normalize_gm(counter.graphs[0].print_readable(print_output=False))
        (
            FileCheck()
            .check('a: "f32[1024]" = l_x_ * 2')
            .check('b: "f32[1024]" = l_x_ * 3')
            .check("synchronize_stream = torch.ops.streams.synchronize_stream(1)")
            .check('add: "f32[1024]" = a + b')
            .run(graph)
        )
        ...
        (
            ...
            .check("stream1 = get_external_object_by_index(1)")
            .check("with stream1:")
