...
class TestCase(InductorTestCase):
    ...
    @config.patch("triton.use_block_ptr", True)
    def test_low_precision_reduction(self, device, input_shape, reduction_func, input_dtype):
        @torch.compile
        def func(a, b, c, d):
    ...
    @parametrize("input_dtype", [torch.float16, torch.bfloat16])
    @config.patch("triton.use_block_ptr", True)
    def test_dtype_aware_codegen(self, device, op_name: str, load_upcast_to_fp32, input_dtype):
        """
...
instantiate_device_type_tests(
    TestCase, globals(), except_for="cpu", allow_xpu=True
)
