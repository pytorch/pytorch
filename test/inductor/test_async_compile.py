...
class TestAsyncCompile(TestCase):
    ...
    @requires_gpu()
    @requires_triton()
    def test_triton_generated_source_with_stale_device_properties_runs(self):
        if torch.version.hip is not None or not torch.cuda.is_available():
            self.skipTest("CUDA-only stale architecture metadata coverage")

        device_index = torch.cuda.current_device()
        device = torch.device("cuda", device_index)
        runtime_props = DeviceProperties.create(device)
        runtime_cc = runtime_props.cc
        if not isinstance(runtime_cc, int):
            self.fail(f"Expected CUDA compute capability, got {runtime_cc!r}")
        stale_cc = 80 if runtime_cc != 80 else 90
        stale_major = stale_cc // 10
        source = textwrap.dedent(
            f"""
            import triton
            import triton.language as tl
            from torch._inductor.runtime import triton_heuristics
            from torch._inductor.runtime.hints import AttrsDescriptorWrapper, DeviceProperties


            @triton_heuristics.fixed_config(
                config={{"XBLOCK": 16, "num_warps": 4, "num_stages": 1}},
                filename=__file__,
                triton_meta={{
                    "signature": {{
                        "in_ptr0": "*fp32",
                ...
                inductor_meta={{
                    "grid_type": "Grid1D",
                    "kernel_name": "stale_device_kernel",
                }},
            )
            @triton.jit
            def stale_device_kernel(in_ptr0, out_ptr0, xnumel, XBLOCK: tl.constexpr):
                xoffset = tl.program_id(0) * XBLOCK
                xindex = xoffset + tl.arange(0, XBLOCK)
                xmask = xindex < xnumel
                tmp0 = tl.load(in_ptr0 + xindex, xmask, other=0.0)
                tl.store(out_ptr0 + xindex, tmp0 + 1.0, xmask)
            """
        )

        CompiledTritonKernels.cache_clear()
        with config.patch(compile_threads=1), fresh_cache():
            kernel = AsyncCompile().triton(
                "stale_device_kernel", source, device_str=str(device)
            )

            self.assertEqual(kernel.device_props.cc, runtime_cc)
            self.assertNotEqual(kernel.device_props.cc, stale_cc)

            x = torch.arange(16, device=device, dtype=torch.float32)
            out = torch.empty_like(x)
            stream = torch._C._cuda_getCurrentRawStream(device_index)
            kernel.run(x, out, x.numel(), stream=stream)
