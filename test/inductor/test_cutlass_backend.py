...
def _get_cutlass_dir(device) -> str:
    return config.xpu.cutlass_dir if device == "xpu" else config.cutlass.cutlass_dir
...
use_evt_config = config.patch(
    {
        ...
        "cutlass.cutlass_tma_only": (acc := torch.accelerator.current_accelerator())
        is not None
        and acc.type == "cuda",  # Only CUDA requires TMA for EVT.
...
class TestCutlassBackendAccelerator(TestCase):
    ...
    def test_sm103_cutlass_ops_skip_int8_umma(self, device):
        from torch.utils import _pytree as pytree
    ...
    @mock.patch.dict(os.environ, {"PATH": _get_path_without_sccache()})
    def test_get_max_alignment(self, device):
        l4 = FixedLayout(
    ...
    def test_standalone_runner(self, device):
        ...
        with config.patch(
            ...
            with CUTLASSCompileSourceCapturingContext(device) as ctx:
            ...
            suffix = ".cpp" if device == "xpu" else ".cu"
            ...
            command = cutlass_standalone_runner_compile_command(
                device, Path(cu_file.name), Path(exe_file.name)
            )
    ...
    def test_config_number_post_filtering(self, device) -> None:
        ...
        expected_count = 2 if device == "xpu" else 1
        self.assertTrue(
