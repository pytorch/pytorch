...
class MinifierTestsCPU(MinifierTestsBase):
...
@torch.compile()
def inner(x):
    return x - torch.tensor(655, dtype=torch.half, device='{device_type}') * 100

inner(torch.tensor(655 * 100, dtype=torch.half, device='{device_type}'))
"""
