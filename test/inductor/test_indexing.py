...
class ExprPrinterTests(InductorTestCase):
    def test_print_pow(self):
        ...
        cpu_cases = common_cases + [
            (
                sympy.Pow(s1 + s2, 2),
                lambda c, L: "static_cast<int32_t>((bar + foo)*(bar + foo))",
            )
        ]
    ...
    def test_print_floor(self):
        for integer in [True, False]:
            ...
            if integer:
                ...
                self.assertEqual(
                    cexpr(expr), "static_cast<int32_t>(std::floor((1.0/2.0)*s1))"
                )
            else:
    ...
    def test_print_ceil(self):
        for integer in [True, False]:
            ...
            if integer:
                ...
                self.assertExpectedInline(
                    cexpr(expr), """static_cast<int32_t>(std::ceil((1.0/2.0)*s1))"""
                )
            else:
                self.assertExpectedInline(pexpr(expr), """math.ceil((1/2)*s1)""")
    ...
    def test_print_floor_div(self):
        ...
        self.assertEqual(cexpr(expr), "(-1)*s1")
    ...
    def test_print_Min_Max(self):
        ...
        for f, s, cmp in cases:
            ...
            self.assertEqual(cexpr(expr), f"std::{s}(-2, x)")
            ...
            self.assertEqual(cexpr(expr), f"std::{s}({{x, 2*x, 3*x}})")
