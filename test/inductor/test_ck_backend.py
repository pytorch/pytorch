...
class TestCKBackend(TestCase):
    ...
    @parametrize(
        ...
        (
            ...
            subtest(("CK", torch.bfloat16), name="standalone_ck"),
...
class TestCKHeaderDiagnostic(TestCase):
    """The missing-header warning.

    ROCm stopped installing the CK headers system-wide, and ``ck/config.h`` is
    generated by CK's CMake so it ships in neither ROCm nor the ck4inductor
    wheel. When it is absent every CK choice fails to compile and is silently
    discarded, which reaches the user as an autotune with no CK candidates
    rather than as a missing header. These tests pin the diagnostic that
    explains it.

    No GPU or ROCm install required: the include roots are temporary
    directories, so the answer depends only on what the test puts in them.
    """

    def setUp(self):
        super().setUp()
        utils._warn_missing_ck_header.cache_clear()
    ...
    @contextlib.contextmanager
    def _roots(self, *present):
        """A CK dir containing exactly ``present``, made the only include root.

        Both the environment and the config are redirected. ck4inductor's
        check_headers() resolves its roots from TORCHINDUCTOR_CK_DIR / ROCM_HOME
        / ROCM_PATH and never reads config.rocm, so patching config alone would
        leave the installed wheel's own headers on the search path and the test
        would be answering about those rather than about the stubs written here.
        """
        with tempfile.TemporaryDirectory() as ck_dir:
            for header in present:
                path = os.path.join(ck_dir, "include", header)
                os.makedirs(os.path.dirname(path), exist_ok=True)
                with open(path, "w") as f:
                    f.write("/* test stub */\n")
            env = {
                "TORCHINDUCTOR_CK_DIR": ck_dir,
                "ROCM_HOME": ck_dir,
                "ROCM_PATH": ck_dir,
            }
            with (
                unittest.mock.patch.dict(os.environ, env),
                config.patch({"rocm.ck_dir": ck_dir, "rocm.rocm_home": ck_dir}),
            ):
                yield

    def test_resolved_header_is_silent(self):
        with self._roots("ck/ck.hpp"):
            with self.assertNoLogs(utils.log, level="WARNING"):
                utils._warn_missing_ck_headers("ck/ck.hpp")

    def test_missing_header_warns_and_names_the_roots(self):
        with self._roots("ck/ck.hpp"):
            with self.assertLogs(utils.log, level="WARNING") as captured:
                utils._warn_missing_ck_headers("ck/config.h")
        # The roots are in the message because "not found" is only actionable
        # once you know where it was looked for.
        self.assertIn("ck/config.h", captured.output[0])
        self.assertIn("include", captured.output[0])

    def test_warns_once_per_configuration(self):
        with self._roots():
            with self.assertLogs(utils.log, level="WARNING") as captured:
                for _ in range(5):
                    utils._warn_missing_ck_headers("ck/config.h")
    ...
    def test_cache_key_includes_the_ck_dir(self):
        # The verdict depends on where we looked, not just what we looked for:
        # a second configuration must be re-evaluated rather than inherit the
        # first one's answer. Long-lived compile workers and the test suite
        # itself both change ck_dir mid-process.
        with self._roots():
            with self.assertLogs(utils.log, level="WARNING"):
                utils._warn_missing_ck_headers("ck/config.h")
        with self._roots():
            with self.assertLogs(utils.log, level="WARNING"):
                utils._warn_missing_ck_headers("ck/config.h")

    def test_falls_back_when_ck4inductor_is_too_old(self):
        # ck4inductor without check_headers() must degrade to searching the
        # compiler's own include roots, not raise out of choice generation.
        real_import = builtins.__import__

        def without_check_headers(
            name, globals=None, locals=None, fromlist=(), level=0
        ):
            if name == "ck4inductor" and "check_headers" in (fromlist or ()):
                raise ImportError("no check_headers in this ck4inductor")
            return real_import(name, globals, locals, fromlist, level)

        with self._roots("ck/ck.hpp"):
            with patch.object(builtins, "__import__", without_check_headers):
                with self.assertNoLogs(utils.log, level="WARNING"):
                    utils._warn_missing_ck_headers("ck/ck.hpp")
                utils._warn_missing_ck_header.cache_clear()
                with self.assertLogs(utils.log, level="WARNING"):
