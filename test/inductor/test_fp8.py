...
class TestFP8Lowering(TestCase):
    ...
    def test_scaled_mm_scale_result(self, device, has_bias):
        ...
        if not has_bias:
            self.assertRegex(
                code,
                r"extern_kernels\._scaled_mm\((?:arg\d+_\d+, ){4}None, "
                r"arg\d+_\d+, out_dtype=",
            )
    ...
    @unittest.skipIf(
        ...
        "Need CUDA + Triton and a large GPU to run max-autotune",
    )
    @skipIfRocm(msg="FP8 scaled_mm tensorwise eager path is not supported by hipBLAS")
    @onlyCUDA
    @parametrize("has_bias", (False, True))
    @parametrize("scale_result_shape", ((), (1,), (1, 1), (1, 1, 1)))
    def test_scaled_mm_scale_result_triton_template(
        self, device, has_bias, scale_result_shape
    ):
        dtype_float8 = torch.float8_e4m3fn
        M, K, N = 16, 32, 16
        ...
        bias = (
            torch.full((N,), 14.0, device=device, dtype=torch.bfloat16)
            if has_bias
            else None
        )
        scale_result = torch.full(scale_result_shape, 2.0, device=device)
        ...
        expected = fn(a, b, scale_a, scale_b, bias, scale_result)
        expected_value = 32.0 if has_bias else 4.0
        self.assertEqual(
            expected.float(), torch.full_like(expected.float(), expected_value)
        )
    ...
    @unittest.skipIf(not PLATFORM_SUPPORTS_FP8, f8_msg)
    @onlyOn(["xpu"])
    def test_scaled_mm_scale_result_xpu_triton_template(self, device):
        dtype_float8 = torch.float8_e4m3fn
        M, K, N = 16, 32, 16
        a = torch.ones(M, K, device=device, dtype=torch.bfloat16).to(dtype_float8)
        b = torch.ones(N, K, device=device, dtype=torch.bfloat16).to(dtype_float8).t()
        ...
        expected = fn(a, b, scale_a, scale_b, scale_result)
        ...
        self.assertEqual(expected, unscaled)
    ...
    @skipIfRocm(msg="FP8 scaled_mm tensorwise eager path is not supported by hipBLAS")
    @onlyCUDA
    def test_scaled_mm_scale_result_high_precision_triton_template(self, device):
        dtype_float8 = torch.float8_e4m3fn
        M, K, N = 16, 32, 16
        a = torch.ones(M, K, device=device, dtype=torch.bfloat16).to(dtype_float8)
        b = torch.ones(N, K, device=device, dtype=torch.bfloat16).to(dtype_float8).t()
    ...
    @onlyCUDA
    @parametrize("has_bias", (False, True))
    def test_functional_scaled_mm_fullgraph(self, device, has_bias):
        M, N, K = 128, 128, 128
        ...
        bias = torch.randn(N, device=device, dtype=torch.bfloat16) if has_bias else None

        def fn(x_fp8, w_fp8_t, x_scale, w_scale, bias):
            return scaled_mm(
                x_fp8,
                w_fp8_t,
                x_scale,
        ...
        expected = fn(x_fp8, w_fp8.t(), x_scale, w_scale, bias)
        with config.patch(
            {
                "max_autotune_gemm_backends": "TRITON",
                "max_autotune": True,
