...
if TEST_WITH_ROCM:
    ...
    if torch.cuda.is_available():
        ...
        if prop in MI300_ARCH:
            test_failures.update(
                {
                    "test_aoti_eager_cache_hit_dynamic_shapes_cuda": TestFailure(
                        "cuda"
                    ),
                    "test_aoti_eager_support_str_dynamic_shapes_cuda": TestFailure(
                        "cuda"
                    ),
                    "test_aoti_eager_support_out_dynamic_shapes_cuda": TestFailure(
                        "cuda"
                    ),
                    "test_aoti_eager_dtype_device_layout_dynamic_shapes_cuda": TestFailure(
                        "cuda"
                    ),
                }
            )
