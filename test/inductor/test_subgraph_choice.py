...
class TestFlexAttention(TestCase):
    ...
    def test_subgraph_decompose_k(self):
        ...
        a = torch.randn(
            (32, 4096), dtype=torch.float16, device=torch.device(f"{GPU_TYPE}:0")
        )
        b = torch.randn(
            (4096, 32), dtype=torch.float16, device=torch.device(f"{GPU_TYPE}:0")
        )
        gm = make_fx(lambda a, b: a @ b)(
            a, b
        ...
        with V.set_graph_handler(graph), V.set_fake_mode(fake_tensor_mode), tracing(
            TracingContext(fake_tensor_mode)
            ...
            out = torch.zeros(
                (32, 32), dtype=torch.float16, device=torch.device(f"{GPU_TYPE}:0")
            )
