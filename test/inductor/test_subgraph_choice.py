...
from torch._inductor.select_algorithm import autotune_select_algorithm, AlgorithmSelectorCache
...
class TestFlexAttention(TestCase):
    ...
    def test_subgraph_decompose_k(self):
        ...
        def _(a, b):
            ...
            choices = [choices[1]]
            return autotune_select_algorithm("test_subgraph_choice", choices, [a, b], layout)

        a_in = torch.randn(mat1_shape, dtype=torch.float16, device=torch.device(f"{GPU_TYPE}:0"))
        b_in = torch.randn(mat2_shape, dtype=torch.float16, device=torch.device(f"{GPU_TYPE}:0"))
