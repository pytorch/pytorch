...
class TestSymmMemOpsRegistration(TestCase):
    ...
    def setUp(self):
        super().setUp()
        # C++ TORCH_LIBRARY_FRAGMENT registrations may not have been synced
        # to the Python registry yet, so reload them.
    ...
    def test_symm_mem_correct_args(self):
        """Test that each symm_mem op registers only the args that need symmetric memory."""
        try:
            import torch.distributed._symmetric_memory  # noqa: F401
        except ImportError:
            self.skipTest("torch.distributed._symmetric_memory not available")

        # Only the tensor passed to rendezvous() in the implementation requires
        # symmetric memory. For most ops this is "input"; for all_gather it's "out".
        expected_args = {
            "symm_mem::multimem_all_reduce_": {"input"},
            "symm_mem::multimem_one_shot_all_reduce": {"input"},
            "symm_mem::multimem_one_shot_all_reduce_out": {"input"},
            "symm_mem::multimem_one_shot_reduce_out": {"input"},
            "symm_mem::multimem_all_gather_out": {"out"},
            "symm_mem::one_shot_all_reduce": {"input"},
            "symm_mem::one_shot_all_reduce_out": {"input"},
            "symm_mem::one_shot_all_reduce_copy": {"symm_buffer"},
            "symm_mem::one_shot_all_reduce_copy_out": {"symm_buffer"},
        ...
        for qualname, args in expected_args.items():
            entry = singleton.find(qualname)
            self.assertTrue(
                entry.symm_mem_args.is_registered(),
                f"{qualname} is not registered",
            )
            self.assertEqual(
                set(entry.symm_mem_args.get()),
                args,
                f"{qualname}: expected {args}, got {set(entry.symm_mem_args.get())}",
            )

    def test_fallback_kernel_uses_registry(self):
        """Test that FallbackKernel checks the symm_mem registry."""
        try:
            import torch.distributed._symmetric_memory  # noqa: F401
        except ImportError:
            self.skipTest("torch.distributed._symmetric_memory not available")
