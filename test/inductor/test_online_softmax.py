...
class _FakeNode:
    def __init__(self, device_type):
        self.meta = {"val": _FakeVal(device_type)}


class _FakeMatch:
    def __init__(self, device_type):
        self.kwargs = {"x": _FakeNode(device_type)}


class TestPrepareSoftmaxExtraCheck(TestCase):
    """
    Zero-regression coverage for the migration of prepare_softmax_extra_check
    from `device_type in ["cuda", "xpu"]` to a BackendFeature.ONLINE_SOFTMAX
    membership check. Pure gating logic -- no device is required.
    """

    def check(self, device_type):
        from torch._inductor.fx_passes.post_grad import prepare_softmax_extra_check

        return prepare_softmax_extra_check(_FakeMatch(device_type))

    def test_enabled_backends(self):
        # The exact set the old hardcoded list allowed.
        with inductor_config.patch(online_softmax=True, cuda_backend="triton"):
            self.assertTrue(self.check("cuda"))
        with inductor_config.patch(online_softmax=True, xpu_backend="triton"):
            self.assertTrue(self.check("xpu"))

    def test_cpu_is_rejected(self):
        with inductor_config.patch(online_softmax=True):
            self.assertFalse(self.check("cpu"))

    def test_gpu_without_online_softmax_is_rejected(self):
        # mtia/mps advertise BackendFeature.GPU but not ONLINE_SOFTMAX. They must
        # also short-circuit *before* `getattr(config, f"{device_type}_backend")`,
        # since no mtia_backend/mps_backend config option exists -- reaching it
        # would raise AttributeError.
        self.assertFalse(hasattr(inductor_config, "mtia_backend"))
        self.assertFalse(hasattr(inductor_config, "mps_backend"))
        with inductor_config.patch(online_softmax=True):
            self.assertFalse(self.check("mtia"))
            self.assertFalse(self.check("mps"))

    def test_config_switch_still_gates(self):
        with inductor_config.patch(online_softmax=False, cuda_backend="triton"):
            self.assertFalse(self.check("cuda"))

    def test_non_triton_inductor_backend_still_gates(self):
        # The per-inductor-backend secondary switch is retained by design.
        with inductor_config.patch(online_softmax=True, cuda_backend="halide"):
            self.assertFalse(self.check("cuda"))
