...
def get_softmax_wrapper(V=50304, use_log_softmax=False, device=None):
    N = 32 * 1024
...
class TestOnlineSoftmax(TestCase):
    ...
    def test_codegen_online_softmax(self, device, use_log_softmax, V):
        wrapper_code = get_softmax_wrapper(use_log_softmax=use_log_softmax, V=V, device=device)

        self.assertEqual(wrapper_code.count("for r0_offset in"), 2)

    @torch._dynamo.config.patch(capture_scalar_outputs=True)
    @parametrize("use_log_softmax", [False, True])
    def test_codegen_online_softmax_unbacked_non_reduction_dim(self, device, use_log_softmax):
        def f(x, n):
            x = x[: n.item(), :]
            if use_log_softmax:
                return torch.log_softmax(x, dim=-1)
    ...
    def test_codegen_softmax_persistent_reduction(self, device):
        ...
        """
        wrapper_code = get_softmax_wrapper(1024, device=device)
...
class TestOnlineSoftmaxCpu(TestCase):
    hw_classification = HardwareClassification.CPU

    def test_no_online_softmax_for_cpu(self):
        code = get_softmax_wrapper(V=2048, device="cpu")
