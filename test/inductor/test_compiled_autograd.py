...
class TestCompiledAutogradCuda(TestCase):
    ...
    def test_cudagraphs_cpu_division(self):
        ...
        model = torch.nn.Linear(10, 10, device="cuda", dtype=torch.float16)
        inputs = torch.randn(10, 10, device="cuda", dtype=torch.float16)
        out = model(inputs)
...
class TestCompiledAutogradAccel(TestCase):
...
instantiate_device_type_tests(TestCompiledAutogradAccel, globals(), allow_xpu=True)
instantiate_parametrized_tests(TestCompiledAutogradCuda)
