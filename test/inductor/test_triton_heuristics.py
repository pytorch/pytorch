...
class TestRecheckAutotuneCache(TestCase):
    ...
    @skipUnless(HAS_GPU_AND_TRITON, "requires gpu and triton")
    def test_recheck_propagates_found_by_coordesc(self):
        """
        When the cached best config matches an existing compile result,
        found_by_coordesc=True must be set on the compile result's config
        so that run() doesn't re-trigger coordinate descent.
        """
        ...
        # This is the key assertion: the flag must be on the compile result's
        # config object, not just on the cached config
        self.assertTrue(autotuner.compile_results[0].config.found_by_coordesc)
