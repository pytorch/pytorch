...
{{GEMM_DEFINE}}
extern "C" void cpp_micro_gemm(const {{input_type}}* X, const {{input_type}}* W, float* Y) {
    {{GEMM_CALL}}
}
"""

        # gemm template for reference
        TEST_REF_TEMPLATE = r"""
    ...
    for (int64_t m = 0; m < M; ++m) {
        for (int64_t n = 0; n < N; ++n) {
            ...
            for (int64_t k = 0; k < K; ++k) {
                if (trans_b) {
                    result += (float)A[m * lda + k] * (float)B[k + n * ldb];
                } else {
                    result += (float)A[m * lda + k] * (float)B[k * ldb + n];
                }
            }
            C[m * ldc + n] = result;
        }
...
extern "C" void cpp_ref_gemm(const {{input_type}}* X, const {{input_type}}* W, float* Y) {
    {{REF_CALL}}
}
"""

        def ref_gemm_codegen_call(
            A: torch.Tensor,
            B: torch.Tensor,
            C: torch.Tensor,
            accum: bool,
            ...
            ldc = C.stride()[0]
            res = IndentedBuffer()
            res.writeline(f"kernel_micro_gemm<{value_to_cpp(accum, 'bool')}>(")
            with res.indent():
                with res.indent():
                    ...
                    res.writeline(f"{ldb},")
                    res.writeline(f"{ldc},")
                    res.writeline(f"{value_to_cpp(trans_b, 'bool')}")
                res.writeline(");")
            return res.getvalue()

        def micro_gemm_codegen_call(
            A: torch.Tensor,
            B: torch.Tensor,
            C: torch.Tensor,
            accum: bool,
        ...
        def micro_gemm_define(A, B, C, trans_b):
        ...
        for trans_b in [True, False]:
            for accum in [True, False]:
                for shape in shapes:
                    ...
                    K = shape[2]
                    A = torch.rand(M, K)
                    B = torch.rand(N, K) if trans_b else torch.rand(K, N)
                    A = A.to(dtype=dtype)
                    B = B.to(dtype=dtype)
                    C = torch.rand(M, N)
                    C_ref = C.clone()
                    # generate the call of gemm
                    micro_gemm_call = micro_gemm_codegen_call(A, B, C, accum, trans_b)
                    # generate the definition of gemm
                    micro_gemm = micro_gemm_define(A, B, C, trans_b)

                    TEST_GEMM_TEMPLATE = TEST_GEMM_TEMPLATE.replace(
                        "{{input_type}}", dtype_cpp[dtype]
                    )
                    GEMM_SOURCE_CODE = TEST_GEMM_TEMPLATE.replace(
                        "{{GEMM_DEFINE}}", micro_gemm
                    )
                    GEMM_SOURCE_CODE = GEMM_SOURCE_CODE.replace(
                        "{{GEMM_CALL}}", micro_gemm_call
                    )

                    DLL = CppCodeCache.load(GEMM_SOURCE_CODE, device_type="cpu")
                    run_method = DLL.cpp_micro_gemm
                    inputs = []
                    inputs.append(A)
                    inputs.append(B)
                    inputs.append(C)
                    args = [tensor.data_ptr() for tensor in list(inputs)]
                    run_method.argtypes = [ctypes.c_ulonglong] * (len(args))
                    run_method(*args)

                    ref_gemm_call = ref_gemm_codegen_call(A, B, C_ref, accum, trans_b)
                    TEST_REF_TEMPLATE = TEST_REF_TEMPLATE.replace(
                        "{{input_type}}", dtype_cpp[dtype]
                    )
                    REF_GEMM_SOURCE_CODE = TEST_REF_TEMPLATE.replace(
                        "{{REF_CALL}}", ref_gemm_call
                    )
                    DLL = CppCodeCache.load(REF_GEMM_SOURCE_CODE, device_type="cpu")
                    run_method = DLL.cpp_ref_gemm
                    inputs = []
                    inputs.append(A)
                    inputs.append(B)
                    inputs.append(C_ref)
                    ...
                    run_method.argtypes = [ctypes.c_ulonglong] * (len(args))
                    run_method(*args)
                    self.assertEqual(C, C_ref, atol=1e-3, rtol=1e-3)
