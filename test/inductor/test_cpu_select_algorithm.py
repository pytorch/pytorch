...
class TestSelectAlgorithm(BaseTestSelectAlgorithm):
    ...
    @inductor_config.patch({"freezing": True})
    @inductor_config.patch(
        {"cpp.gemm_thread_factors": "4,2,7", "cpp.gemm_cache_blocking": "1,2,1024"}
    )
    @patches
    @torch.no_grad
    @requires_mkl
    @set_num_threads(56)
    def test_linear_thread_factors_k_slicing_n_cache_blocks(self):
        class M(torch.nn.Module):
            def __init__(self):
                super().__init__()
        ...
        # Forces Nc_blocks > 1 while k-slicing is active.
        dtype = torch.float
        counters.clear()
