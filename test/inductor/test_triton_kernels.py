...
class CustomOpTests(torch._inductor.test_case.TestCase):
    ...
    def test_autotune_no_pre_or_post_hook(self):
        ...
        @triton.autotune(configs = get_okay_config(), key=['n_elements'], pre_hook=lambda x: None, post_hook=lambda x: None)
        @triton.jit
        def add_kernel_autotuner_config(x_ptr, y_ptr, output_ptr, n_elements, BLOCK_SIZE: tl.constexpr):
            pid = tl.program_id(axis=0)
