...
class KernelTests(torch._inductor.test_case.TestCase):
    ...
    def test_triton_kernel_namedtuple_arg(self, backend):
        # Both backends must rebuild the NamedTuple before Triton launches it.
        import triton
    ...
    def test_triton_kernel_namedtuple_nested_constexpr(self, backend):
        # All three ways to create a constexpr leaf must survive both backends.
        import triton
        import triton.language as tl
    ...
    def test_triton_kernel_aggregate_view_mutation(self):
        # Clone only the written leaf and replay its view update to the base.
        import triton
        import triton.language as tl
        ...
        with inductor_config.patch(post_grad_custom_post_pass=graph_pass):
            base_result, view_result = torch.compile(fn, fullgraph=True)(
                base, read_only
            )
    ...
    def test_triton_kernel_aggregate_multiple_mutations(self):
        # Both written leaves must be cloned; the nested read-only leaf must not.
        import triton
        import triton.language as tl
