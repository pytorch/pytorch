...
class TransformerSnippet(nn.Module):
    ...
    def example_inputs(self, device):
        return (torch.randn(2, 64, device=device), torch.randn(2, 64, device=device))
...
def make_cpp_wrapper_test(orig_test, **extra_args):
    ...
    def fn(self, device):
        ...
        return orig_test(self, device, **extra_args)
...
class MultiKernelTest(TestCase):
    ...
    def setUpClass(cls):
        super().setUpClass()
        cls._config_stack = contextlib.ExitStack()
        cls._config_stack.enter_context(
            config.patch(
                {
    ...
    def test_layernorm(self, device):
        ln = nn.LayerNorm(1024, device=device)
        x = torch.rand(2, 1024, device=device)
        ref = ln(x)
    ...
    def test_transformer_snippet_with_fallback_random(self, device):
        ...
        with config.patch("fallback_random", True):
            self.test_transformer_snippet(device)
    ...
    def test_batchnorm_training(self, device):
        ...
        """
        bn = nn.BatchNorm2d(3, device=device)
