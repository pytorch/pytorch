...
class TestFlexFlashAuxVecSelection(InductorTestCase):
    ...
    def _eval_packed_mask_intervals(self, intervals, q_idx, kv_idx):
        ...
        for interval in intervals:
            lower = eval_index(interval.lower)
            upper = eval_index(interval.upper)
...
class TestFlexFlash(InductorTestCase):
    ...
    @torch._inductor.config.patch(force_disable_caches=True)
    def test_flash_attention_sm100_packed_mask_respects_forced_vec_size(self):
        seq_len = 128
        q, k, v = create_test_tensors(
            batch_size=1,
            num_heads=1,
        ...
        expected = fn(q, k, v)
        torch._dynamo.reset()
        with force_flex_flash_mask_mod_vec_size(16):
            actual, code = run_and_get_code(
