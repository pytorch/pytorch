...
class CommonTemplate:
    ...
    # Based on test_torchinductor_opinfo:test_comprehensive_diag_embed_cpu_float64
    # A scalar generated by tl.full([1], 0.0, tl.float64) is attempted to be reshaped
    # into the shape of the output block ptr, but scalars cannot be reshaped to a
    # different size
    @parametrize("dtype", [torch.float32, torch.float64])
    def test_store_constant(self, dtype):
        def foo(x):
            return torch.diag_embed(x, offset=2)

        x = torch.zeros((5, 5), dtype=dtype)
        run_and_compare(
            self,
            foo,
            x,
            ...
            expected_num_block_pointers=3,
            config_patches={"triton.use_block_ptr": True},
        )
