...
def forward(self, arg0_1, arg1_1, arg2_1, arg3_1, arg4_1):
    ...
    @skip_on_mps
    @skip_on_xpu
    def test_shared_memory_kernel_options_error(self, device):
        ...
        def raise_out_of_resources(self):
            raise OutOfResources(266240, 232448, "shared memory")

        with (
            mock.patch.object(
                TritonTemplateCaller, "precompile", raise_out_of_resources
            ),
            self.assertRaisesRegex(
                (InductorError, RuntimeError),
                "requires too much shared memory.*fwd_BLOCK_M.*fwd_num_stages",
            ),
        ):
            torch.compile(flex_attention)(
                query,
    ...
    @skip_on_mps
    @skip_on_xpu
    def test_backward_shared_memory_kernel_options_error(self, device):
        from triton.runtime.autotuner import OutOfResources
        ...
        def fail_backward_precompile(self):
            if "backward" in self.name:
                raise OutOfResources(266240, 232448, "shared memory")
            return original_precompile(self)

        with mock.patch.object(
            TritonTemplateCaller, "precompile", fail_backward_precompile
        ):
            out = torch.compile(flex_attention)(
                query,
                key,
            ...
            with self.assertRaisesRegex(
                (InductorError, RuntimeError),
                "FlexAttention backward.*requires too much shared memory.*"
                "bwd_BLOCK_M1.*bwd_num_stages",
            ):
                out.mean().backward()
