...
class TestSingleProc(DynamoDistributedSingleProcTestCase):
    ...
    def test_graph_split_ctx_manager_nested(self):
        ...
        def get_model_multi_output(device, ctx_manager_1, ctx_manager_2, bsz=20, hidden_feat=1000):
            m = ToyModelMultiOutput(ctx_manager_1, ctx_manager_2, hidden_feat=hidden_feat).to(device)
            m.apply(init_weights)
            inputs = torch.rand(bsz, hidden_feat).to(device)
            outputs = m(inputs)
            return m, inputs, outputs
                ...
                with ambient_grad():
                    for ctx_manager_1, output_test_1 in ctx_managers:
                        for ctx_manager_2, output_test_2 in ctx_managers:
                            m, inputs, correct_outputs = get_model_multi_output(
                                self.device, ctx_manager_1, ctx_manager_2, hidden_feat=1000
                            )
                            ddp_m = DDP(m, device_ids=self.device_ids, bucket_cap_mb=bucket_cap_mb)
