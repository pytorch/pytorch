...
class TestCollectivesWithBaseClassDevice(MultiThreadedTestCase):
    ...
    def test_collectives_issued_from_side_stream(self, device):
        ...
        if self.rank == 0:
            get_cycles_per_ms(device)
        dist.barrier()
        delay_cycles = int(200 * get_cycles_per_ms(device))
        ...
        with device_module.stream(side_stream):
            if self.rank != 0:
                ...
                device_sleep(device, delay_cycles)
        ...
        expected_reduce_scatter = torch.full(
            (8,), float(self.world_size * (self.rank + 1)), device=device
        )
