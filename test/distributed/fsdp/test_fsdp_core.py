...
class TestParityWithDDP(FSDPTest):
    ...
    @unittest.skipIf(TEST_HPU, "HPU doesn't has HW sleep API support (like CUDA), skipping")
    @skip_if_lt_x_gpu(2)
    @parametrize(params, configs, subtest_name)
...
class TestHooks(FSDPTest):
    ...
    def test_pre_backward_hook_registration(self, device, cuda_first: bool):
        ...
        self._test_pre_backward_hook_registration(device, fsdp_model)
    @skip_if_lt_x_gpu(2)
    def test_pre_backward_hook_registration_after_state_dict(self, device):
        ...
        self._test_pre_backward_hook_registration(device, fsdp_model)
    def _test_pre_backward_hook_registration(self, device, model):
        optim = torch.optim.SGD(model.parameters(), lr=0.01, momentum=0.9)
        optim.zero_grad()
        # Inputs always cuda, as computation happens on CUDA device only
        input = model.module.get_input(device if TEST_HPU else self.device_type)
        output = model(*input)
    ...
    def test_register_functions_called(self, device, cuda_first: bool, mixed_precision: bool):
        ...
        input = fsdp_model.module.get_input(device)
...
class TestAutograd(FSDPTest):
    ...
    def _test_unshard_params_as_tensors(
        ...
        device_id = device if TEST_HPU else self.device_type
        ...
        NUM_LINEARS = 5
        model = nn.Sequential(
            *[nn.Linear(3, 3, device=device_id) for _ in range(NUM_LINEARS)]
        )
        fsdp_model = FSDP(model, **fsdp_kwargs)
        self.assertEqual(len(list(FSDP.fsdp_modules(fsdp_model))), NUM_LINEARS + 1)
        for _ in range(3):
            inp = torch.randn((2, 3), device=device_id)
