...
class TestClipGradNorm(FSDPTestContinuous):
    ...
    def test_non_root(self, device):
        ...
        model = Model().to(torch.device(device).type)
        model.lin2 = FSDP(model.lin2)
        fsdp_model = FSDP(model)
        inp = torch.randn((2, 5), device=torch.device(device).type)
    ...
    def _test_ddp_parity(
        ...
        ddp_model = DDP(local_model, device_ids=[torch.device(device).type])
        fsdp_kwargs = {
        ...
        LARGE_FACTOR = 100
        inp = ddp_model.module.get_input(torch.device(device).type)
    ...
    def _test_low_precision_grads(
        ...
        inp = fsdp_model.module.get_input(torch.device(device).type)
        out = fsdp_model(*inp)
        out.sum().backward()
        for param in fsdp_model.parameters():
    ...
    def _test_no_gradients(self, device, use_orig_params: bool):
        ...
        inp = torch.randn(32, 24, device=torch.device(device).type)
        fsdp_module(inp)
        ...
        expected = torch.tensor(0.0, device=torch.device(device).type)
