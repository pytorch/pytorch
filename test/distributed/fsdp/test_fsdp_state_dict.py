...
class TestFSDPStateDict(FSDPTest):
    ...
    def _test_state_dict_save_load_flow(
        self, device_type, state_dict_type, move_to_cpu
    ):
    ...
    def test_fsdp_state_dict_keys(self, device, state_dict_type):
        device_type = torch.device(device).type
        model = self._initialize_model(device_type, True)
        state_dict = self._state_dict(model, state_dict_type)
    ...
    def test_state_dict_load_into_local_module(
        ...
        if not fsdp_root:
            ...
            blank_local_model = self._initialize_model(
                device_type, wrap_fsdp=False, wrap_ddp=False, register_buffers=True
            )
    ...
    def test_state_dict_type(self, device):
        ...
        with enable_wrap(wrapper_cls=FSDP):
            fsdp = wrap(module, device_id=device_type)
...
class TestFSDPStateDict4GPUs(FSDPTest):
    ...
    def test_local_state_dict_reshard(self, device):
        ...
        batch = torch.randn(4, 4, device=device_type)
        output = model(batch)
        loss = output.sum()
        loss.backward()
        optim.step()
...
instantiate_device_type_tests(
    TestFSDPStateDict, globals(), except_for=("cpu",), allow_xpu=True
)
instantiate_device_type_tests(
    TestFSDPStateDict4GPUs, globals(), except_for=("cpu",), allow_xpu=True
)
