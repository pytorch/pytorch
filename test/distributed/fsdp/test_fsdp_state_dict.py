...
class TestFSDPStateDict(FSDPTest):
    ...
    def test_save_and_load_after_forward_state_dict(
        ...
        for _ in range(6):
            inp = torch.randn(
                1, 10, device=torch.accelerator.current_device_index()
            )
            output = model(*inp)
            loss = output.sum()
    ...
    def _initialize_model(
        ...
        model = Model(wrap_fsdp, register_buffers=register_buffers).to(
            device=device_type
        )
        if wrap_fsdp:
            model = FSDP(model)
    ...
    def test_state_dict_with_ignored_modules(
        ...
        # the model
        nonwrapped_model = Model(wrap_fsdp=False, register_buffers=True).to(
            device=device_type
        )
        ...
        with FSDP.state_dict_type(model, StateDictType.LOCAL_STATE_DICT):
            ...
            with torch.no_grad():
                with FSDP.summon_full_params(model):
                    ...
                    model.my_parameter.copy_(
                        torch.full((1,), 1.75).to(device=device_type)
                    )
    ...
    def test_sharded_load_multi_backend_pg(self):
        ...
        for load_cpu in [True, False]:
            with self.subTest(load_cpu=load_cpu):
                ...
                pg = (
                    dist.new_group(backend=f"cpu:gloo,{device_type}:{backend}")
                )
                fsdp_model = TransformerWithSharedParams.init(
                    pg,
                    FSDPInitMode.RECURSIVE,
...
class TestFSDPStateDict4GPUs(FSDPTest):
    ...
    def test_local_state_dict_reshard(self):
        ...
        batch = torch.randn(
            4, 4, device=torch.accelerator.current_device_index()
        )
        output = model(batch)
        loss = output.sum()
        loss.backward()
