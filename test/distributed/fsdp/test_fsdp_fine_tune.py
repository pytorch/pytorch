...
class TestFSDPFineTune(FSDPTest):
    ...
    def _test_backward_reshard_hooks(
        ...
        with mock.patch(
            ...
            for step_idx in range(num_steps):
                ...
                inp = torch.randn(
                    (8, 5), device=device_id, requires_grad=inp_requires_grad
                )
    ...
    def _test_hooks_multi_traversal(
        ...
        losses = []
        for _ in range(6):
            inp = torch.randn((8, 5), device=device_id, requires_grad=inp_requires_grad)
    ...
    def _test_parity_with_ddp(
        ...
        losses = []
        for _ in range(6):
            inp = torch.randn((8, 5), device=device_id)
            for seq, optim in ((fsdp_seq, fsdp_optim), (ddp_seq, ddp_optim)):
                loss = seq(inp).sum()
                losses.append(loss)
                loss.backward()
    ...
    def _test_parity_with_non_frozen_fsdp(
        ...
        if TEST_CUDA:
            device_id = self.device_type
        torch.manual_seed(42)
        model = ModelUnusedInput(freeze=True).to(device_id)
        torch.manual_seed(42)
        ref_model = ModelUnusedInput(freeze=False).to(device_id)
        if TEST_CUDA:
            fsdp_kwargs = {
        ...
        losses = []
        for idx in range(6):
            frozen_input = torch.randn((4, 4), device=device_id, requires_grad=False)
            learnable_input = torch.randn((4, 4), device=device_id, requires_grad=True)
