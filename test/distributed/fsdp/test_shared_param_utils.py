...
class TestSharedParamUtils(TestCase):
    def test_get_shared_param_info_to_lca(self):
        ...
        # (1)
        query1 = _get_lca_query(root.inner1.child1, root.inner2.child2)
        spi1 = SharedParamInfo(*query1, root.inner1.child1.p)
        lca1 = root
        # (2)
        query2 = _get_lca_query(root.inner1, root.inner2.child1)
        spi2 = SharedParamInfo(*query2, root.inner1.p)
        lca2 = root
        # (3)
        query3 = _get_lca_query(root.inner3.child1, root.inner3.child2)
        spi3 = SharedParamInfo(*query3, root.inner3.child1.p)
        lca3 = root.inner3
        # (4, 5, 6)
        query4 = _get_lca_query(root.inner2, root.leaf)
        spi4 = SharedParamInfo(*query4, root.inner2.p)
        query5 = _get_lca_query(root.inner3, root.leaf)
        spi5 = SharedParamInfo(*query5, root.inner2.p)
        query6 = _get_lca_query(root.inner2, root.inner3)
        spi6 = SharedParamInfo(*query6, root.inner2.p)
        lca4 = lca5 = lca6 = root
        # (7)
        query7 = _get_lca_query(root.inner4.child1, root.inner4)
