...
class TestFSDPMiscMultiThread(FSDPTestMultiThread):
    ...
    def test_no_params(self):
        ...
        no_params = nn.ReLU().to(device=device_type)
        expected_device_str = f"hip:{self.rank}" if TEST_WITH_ROCM else f"cuda:{self.rank}"
        expected_default_str = "hip:0" if TEST_WITH_ROCM else "cuda:0"
        context = (
            (
                self.assertRaisesRegex(
                    ValueError, f"Inconsistent.*{expected_device_str} vs {expected_default_str}"
                )
            )
            if self.rank != 0
            else nullcontext()
...
class TestFSDPMiscWorldSize1(FSDPTestMultiThread):
    ...
    def test_training_device_mismatch_errors(self):
        ...
        device_name = "hip:0" if TEST_WITH_ROCM else "cuda:0"
        with self.assertRaisesRegex(
            RuntimeError,
            f"An FSDP-managed module unexpectedly has parameters on cpu. Make "
            f"sure to move the module to {device_name} before training.",
        ):
            fsdp_model(inp)
        ...
        with self.assertRaisesRegex(
            ...
            f"parameters on {device_name}. Make sure to not move the module from CPU "
