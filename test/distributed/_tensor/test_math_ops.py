...
class DistMathOpsTest(DTensorTestBase):
    ...
    def test_layer_norm_fwd(self):
        ...
        shard_dims = [-1, 0, 1, 2]
        test_config_list = list(itertools.product(shard_dims, norm_shape_idx_list, (False, True)))
        ...
        for shard_dim, norm_idx, elementwise_affine in test_config_list:
            ...
            layer_norm = torch.nn.LayerNorm(normalized_shape, elementwise_affine=elementwise_affine)
    ...
    def test_layer_norm_bwd(self):
        ...
        # NLP example from pytorch docs
        # https://pytorch.org/docs/stable/generated/torch.nn.LayerNorm.html
        batch, sentence_length, embedding_dim = 4, 4, 4
        ...
        # normalized shape is a torch.Size object
        for shard_dim, norm_idx, elementwise_affine in test_config_list:
            x = torch.rand(batch, sentence_length, embedding_dim, device=self.device_type, requires_grad=True)
            normalized_shape = x.shape[norm_idx:]
            layer_norm = torch.nn.LayerNorm(
                normalized_shape, elementwise_affine=elementwise_affine, device=self.device_type
            )
            layer_norm_local = layer_norm
            ...
            layer_norm_dist = distribute_module(
                copy.deepcopy(layer_norm).to(self.device_type), device_mesh, _replicate_fn
            )

            if elementwise_affine:
                self.assertEqual(layer_norm_local.weight, layer_norm_dist.weight.full_tensor())
                self.assertEqual(layer_norm_local.bias, layer_norm_dist.bias.full_tensor())
            ...
            if (self.rank == 0 and False):
                print(f"shard dim = {shard_dim}, norm_shape = {normalized_shape}")
                print(f"y_local = {y_local}")
                print(f"y_dist = {y_dist.full_tensor()}")
            self.assertEqual(y_local, y_dist.full_tensor())
