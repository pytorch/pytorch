    ...
    def test_simple_model_sac_estimation(self):
        ...
        with FakeTensorMode():
            ...
            with sac_estimator(estimate_mode_type="operator-level-cost-model"):
                loss = model(x).sum()
            loss.backward()
            self.assertEqual(sac_estimator.sac_mod_stats["Foo"].view_like_ops, [0, 7])
            self.assertEqual(sac_estimator.sac_mod_stats["Foo"].rand_ops, [10])
            self.assertEqual(
                sac_estimator.sac_mod_stats["Foo"].inplace_ops,
                [(2, 1), (4, 1), (6, 1), (10, 9), (11, 9), (12, 8)],
            )
            self.assertEqual(
