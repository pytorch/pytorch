    ...
    def compare_objects(cuda_obj, cpu_obj, path=""):
        ...
        if isinstance(cuda_obj, torch.Tensor) and isinstance(cpu_obj, torch.Tensor):
            ...
            if cuda_obj.device.type != "cuda":
                return False, f"Expected CUDA tensor, got {cuda_obj.device.type} tensor at {path}"
            if cpu_obj.device.type != "cpu":
                return False, f"Expected CPU tensor, got {cpu_obj.device.type} tensor at {path}"
            
            # Check tensor properties
            if cuda_obj.dtype != cpu_obj.dtype:
                return False, f"Dtype mismatch at {path}: {cuda_obj.dtype} vs {cpu_obj.dtype}"
            if cuda_obj.shape != cpu_obj.shape:
                return False, f"Shape mismatch at {path}: {cuda_obj.shape} vs {cpu_obj.shape}"
            if cuda_obj.stride() != cpu_obj.stride():
                return False, f"Stride mismatch at {path}: {cuda_obj.stride()} vs {cpu_obj.stride()}"
            if cuda_obj.storage_offset() != cpu_obj.storage_offset():
                return False, f"Storage offset mismatch at {path}: {cuda_obj.storage_offset()} vs {cpu_obj.storage_offset()}"
            
            # Check tensor values
            if not torch.allclose(cuda_obj.cpu(), cpu_obj, rtol=rtol, atol=atol):
            ...
            if cuda_storage_ptr in cuda_storage_ptrs:
                ...
                if cpu_storage_ptr != cuda_storage_ptrs[cuda_storage_ptr]:
                    return False, f"Storage sharing mismatch: CUDA tensors share storage but CPU tensors don't at {path}"
            else:
        ...
        # If objects are dictionaries, compare them recursively
            ...
            if cuda_obj.keys() != cpu_obj.keys():
                return False, f"Dictionary keys mismatch at {path}: {cuda_obj.keys()} vs {cpu_obj.keys()}"
            
            for key in cuda_obj:
                result, error = compare_objects(cuda_obj[key], cpu_obj[key], f"{path}.{key}" if path else key)
                if not result:
                    return False, error
        ...
        elif isinstance(cuda_obj, (list, tuple, set)) and isinstance(cpu_obj, (list, tuple, set)):
            if len(cuda_obj) != len(cpu_obj):
                return False, f"Collection length mismatch at {path}: {len(cuda_obj)} vs {len(cpu_obj)}"
            if type(cuda_obj) != type(cpu_obj):
                return False, f"Collection type mismatch at {path}: {type(cuda_obj)} vs {type(cpu_obj)}"
        ...
        # If objects are custom classes, compare their attributes
            ...
            if type(cuda_obj) != type(cpu_obj):
                return False, f"Object type mismatch at {path}: {type(cuda_obj)} vs {type(cpu_obj)}"
            
            result, error = compare_objects(cuda_obj.__dict__, cpu_obj.__dict__, f"{path}.__dict__")
            if not result:
                return False, error
        ...
        # For other types, use direct equality comparison
            ...
            if type(cuda_obj) != type(cpu_obj):
                return False, f"Type mismatch at {path}: {type(cuda_obj)} vs {type(cpu_obj)}"
            if cuda_obj != cpu_obj:
...
class TestStateDictStager(unittest.TestCase):
    ...
    def test_views(self):
        ...
        for pin_memory, share_memory in test_configs:
            with self.subTest(pin_memory=pin_memory, share_memory=share_memory):
                ...
                assert(state_dict["tensor1"].storage().data_ptr() == state_dict["tensor2"].storage().data_ptr())
                ...
                num_bytes = sum(storage.nbytes() for storage in stager._cached_storage_mapping.values())
                
                # Validate tensor count and bytes
                expected_storage_cnt = 2
                assert num_storages == expected_storage_cnt, f"Expected {expected_storage_cnt} storages, got {num_storages}"
                
                # Calculate expected bytes
                # Note: Only unique storages are counted in the byte count
                expected_bytes = (
                    tensor1.numel() * tensor1.element_size() +  # tensor1 and tensor2 share storage
                    tensor3.numel() * tensor3.element_size()    # tensor3 and its narrow view share storage
                )
                assert num_bytes == expected_bytes, f"Expected {expected_bytes} bytes, got {num_bytes}"
                ...
                assert(cpu_state_dict["tensor1"].storage().data_ptr() == cpu_state_dict["tensor2"].storage().data_ptr())
                ...
                assert(recursive["tensor3"].storage().data_ptr() == recursive["type"].tensor1.storage().data_ptr())
    ...
    def test_caching(self):
        ...
        for pin_memory, share_memory in test_configs:
            with self.subTest(pin_memory=pin_memory, share_memory=share_memory):
                ...
                assert result, f"First state dict is not equivalent to original: {error}"
                ...
                assert result, f"Second state dict is not equivalent to modified original: {error}"
                
                # Verify that the number of cached storages hasn't changed
                assert num_storages1 == num_storages2, f"Storage count changed: {num_storages1} vs {num_storages2}"
                
                # Verify that the tensors in the second state dict have the same storage pointers as the first
                assert cpu_state_dict1["tensor1"].storage().data_ptr() == cpu_state_dict2["tensor1"].storage().data_ptr(), "Storage pointers should match for tensor1"
                assert cpu_state_dict1["tensor2"].storage().data_ptr() == cpu_state_dict2["tensor2"].storage().data_ptr(), "Storage pointers should match for tensor2"
                assert cpu_state_dict1["recursive"]["tensor3"].storage().data_ptr() == cpu_state_dict2["recursive"]["tensor3"].storage().data_ptr(), "Storage pointers should match for tensor3"
                ...
                assert torch.all(cpu_state_dict3["tensor1"] == 42.0), "Updated values should be reflected in the cached state dict"
                assert torch.all(cpu_state_dict3["tensor2"] == 42.0), "Updated values should be reflected in the cached state dict"
    ...
    def test_tensor_attrs(self):
        ...
        assert hasattr(cpu_state_dict["tensor1"], 'a'), "Tensor attribute 'a' was not preserved"
        assert cpu_state_dict["tensor1"].a == 42, "Tensor attribute 'a' has incorrect value"
        assert hasattr(cpu_state_dict["tensor1"], 'b'), "Tensor attribute 'b' was not preserved"
        assert cpu_state_dict["tensor1"].b == 43, "Tensor attribute 'b' has incorrect value"
        assert hasattr(cpu_state_dict["recursive"]["tensor3"], 'c'), "Tensor attribute 'c' was not preserved"
        assert cpu_state_dict["recursive"]["tensor3"].c == 44, "Tensor attribute 'c' has incorrect value"
    ...
    def test_different_dtypes(self):
        ...
        for dtype_name, original_tensor in tensors.items():
            ...
            self.assertEqual(cpu_tensor.device.type, "cpu", f"Tensor {dtype_name} should be on CPU")
            self.assertEqual(cpu_tensor.dtype, original_tensor.dtype, f"Tensor {dtype_name} has incorrect dtype")
            self.assertTrue(torch.allclose(cpu_tensor, original_tensor.cpu()), f"Tensor {dtype_name} has incorrect values")
    ...
    def test_empty_tensors(self):
        ...
        tensors = {
            ...
            "empty_2d": torch.tensor([], dtype=torch.float32).reshape(0, 0).cuda(),
            "empty_3d": torch.tensor([], dtype=torch.float32).reshape(0, 0, 0).cuda(),
        ...
        for tensor_name, original_tensor in tensors.items():
            ...
            self.assertEqual(cpu_tensor.device.type, "cpu", f"Tensor {tensor_name} should be on CPU")
            self.assertEqual(cpu_tensor.shape, original_tensor.shape, f"Tensor {tensor_name} has incorrect shape")
            self.assertEqual(cpu_tensor.dtype, original_tensor.dtype, f"Tensor {tensor_name} has incorrect dtype")
    ...
    def test_complex_storage_sharing(self):
        ...
        self.assertEqual(cpu_state_dict["view1"].storage().data_ptr(), storage_ptr, "view1 should share storage with base")
        self.assertEqual(cpu_state_dict["view2"].storage().data_ptr(), storage_ptr, "view2 should share storage with base")
        self.assertEqual(cpu_state_dict["slice1"].storage().data_ptr(), storage_ptr, "slice1 should share storage with base")
        self.assertEqual(cpu_state_dict["slice2"].storage().data_ptr(), storage_ptr, "slice2 should share storage with base")
        self.assertEqual(cpu_state_dict["slice3"].storage().data_ptr(), storage_ptr, "slice3 should share storage with base")
        
        # Verify that modifying the base tensor affects all views and slices
        cpu_state_dict["base"].fill_(42.0)
        self.assertTrue(torch.all(cpu_state_dict["view1"] == 42.0), "view1 should reflect changes to base")
        self.assertTrue(torch.all(cpu_state_dict["view2"] == 42.0), "view2 should reflect changes to base")
        self.assertTrue(torch.all(cpu_state_dict["slice1"] == 42.0), "slice1 should reflect changes to base")
        self.assertTrue(torch.all(cpu_state_dict["slice2"] == 42.0), "slice2 should reflect changes to base")
        self.assertTrue(torch.all(cpu_state_dict["slice3"] == 42.0), "slice3 should reflect changes to base")
    ...
    def test_cpu_storage_independence(self):
        ...
        self.assertNotEqual(tensor1.storage().data_ptr(), cpu_tensor1.storage().data_ptr(), 
                           "CPU tensor should have a different storage pointer than the original tensor")
        self.assertNotEqual(tensor2.storage().data_ptr(), cpu_tensor2.storage().data_ptr(),
                           "CPU tensor should have a different storage pointer than the original tensor")
        

        self.assertTrue(torch.allclose(tensor1, cpu_tensor1), "CPU tensor should have the same values as the original tensor")
        self.assertTrue(torch.allclose(tensor2, cpu_tensor2), "CPU tensor should have the same values as the original tensor")
        ...
        self.assertTrue(torch.allclose(cloned_orginial1, cpu_tensor1), "CPU tensor should have the same values as the original tensor")
        self.assertTrue(torch.allclose(cloned_orginia2, cpu_tensor2), "CPU tensor should have the same values as the original tensor")
    ...
    def test_tensor_pinned_and_shared(self):
        ...
        for pin_memory, share_memory in test_configs:
            with self.subTest(pin_memory=pin_memory, share_memory=share_memory):
                ...
                self.assertEqual(cpu_tensor1.device.type, "cpu", "Staged tensor should be on CPU")
                self.assertEqual(cpu_tensor2.device.type, "cpu", "Staged tensor should be on CPU")
                
                # Verify tensor values
                self.assertTrue(torch.allclose(cpu_tensor1, tensor1.cpu()), 
                               "CPU tensor should have the same values as the original tensor")
                self.assertTrue(torch.allclose(cpu_tensor2, tensor2.cpu()), 
                               "CPU tensor should have the same values as the original tensor")
                
                # Verify pinned memory status
                self.assertEqual(cpu_tensor1.is_pinned(), pin_memory, 
                               f"Tensor pinned status should be {pin_memory}")
                self.assertEqual(cpu_tensor2.is_pinned(), pin_memory, 
                               f"Tensor pinned status should be {pin_memory}")
                
                # Verify shared memory status
                self.assertEqual(cpu_tensor1.is_shared(), share_memory, 
                               f"Tensor shared status should be {share_memory}")
                self.assertEqual(cpu_tensor2.is_shared(), share_memory, 
                               f"Tensor shared status should be {share_memory}")
                
                # Verify storage sharing is consistent with tensor sharing
                if share_memory:
                    # When share_memory is True, the storage should also be shared
                    self.assertTrue(cpu_tensor1.storage().is_shared(), 
                                  "When share_memory=True, tensor storage should be shared")
                    self.assertTrue(cpu_tensor2.storage().is_shared(), 
                                  "When share_memory=True, tensor storage should be shared")
                else:
                    # When share_memory is False, the storage should not be shared
                    self.assertFalse(cpu_tensor1.storage().is_shared(), 
                                   "When share_memory=False, tensor storage should not be shared")
                    self.assertFalse(cpu_tensor2.storage().is_shared(), 
                                   "When share_memory=False, tensor storage should not be shared")
...
class TestDTensorStateDictStager(DTensorTestBase):
    ...
    def test_dtensor(self):
        ...
        print (f"cpu dtensor.tolocal  {cpu_state_dict['dtensor'].to_local()}")
        print (f"cuda dtensor.tolocal  {dtensor.to_local().cpu()}")
        # Verify the original DTensor has the expected values
        self.assertTrue(torch.allclose(dtensor.to_local(), (tensor + 1) * 2))
        self.assertTrue(torch.allclose(cpu_state_dict["dtensor"].to_local(), dtensor.to_local().cpu()))
