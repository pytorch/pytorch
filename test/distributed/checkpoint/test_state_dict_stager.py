...
from torch.distributed.checkpoint._state_dict_stager import (
    staging_context,
    StateDictStager,
)
...
class TestStateDictStager(unittest.TestCase):
    ...
    def test_views(self):
        ...
        for pin_memory, share_memory in test_configs:
            with self.subTest(pin_memory=pin_memory, share_memory=share_memory):
                ...
                # Use the staging context to set this instance as the active stager
                with staging_context(stager):
                    # Let Python's deepcopy do the heavy lifting
    ...
    def test_caching(self):
        ...
        for pin_memory, share_memory in test_configs:
            with self.subTest(pin_memory=pin_memory, share_memory=share_memory):
                ...
                # First call to deepcopy with staging context
                with staging_context(stager):
                    cpu_state_dict1 = copy.deepcopy(state_dict)
                
                # Get the number of cached storages after first deepcopy
                ...
                # Second call to deepcopy with staging context
                with staging_context(stager):
                    cpu_state_dict2 = copy.deepcopy(state_dict)
                
                # Get the number of cached storages after second deepcopy
                ...
                # Third call to deepcopy with staging context
                with staging_context(stager):
                    cpu_state_dict3 = copy.deepcopy(state_dict)
    ...
    def test_tensor_attrs(self):
        """
        Test that tensor attributes are preserved during deepcopy with StateDictStager.
        """
        tensor1 = torch.randn(4,4).cuda()
        tensor2 = tensor1.view(16)
        tensor3 = torch.randn(4,4).cuda()
