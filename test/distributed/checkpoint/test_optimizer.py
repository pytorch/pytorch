...
class TestOptimizerDeviceDefaults(TestCase):
        ...
        with mock.patch.object(
            ...
            with mock.patch.object(torch.xpu, "is_available", return_value=True):
                with mock.patch.object(torch.xpu, "device_count", return_value=2):
                    self.assertEqual(dcp_optimizer._infer_default_device_type(), "xpu")
    ...
    def test_alloc_tensor_default_follows_accelerator(self):
        props = TensorProperties(dtype=torch.float32)
        with mock.patch.object(
            torch.accelerator,
            "current_accelerator",
            ...
            with mock.patch.object(torch.xpu, "current_device", return_value=1):
                with mock.patch.object(
                    torch, "empty", return_value=torch.empty(0)
                ) as mock_empty:
                    _alloc_tensor(props, (2, 3))
        self.assertEqual(mock_empty.call_args.kwargs["device"], torch.device("xpu", 1))
        self.assertEqual(mock_empty.call_args.kwargs["size"], (2, 3))
