...
def _test_consolidate_to_one_file(test_case) -> None:
    import safetensors

    global_tensor = torch.arange(16, dtype=torch.float).view(4, 4)
    mesh_shape = (test_case.world_size,)
    mesh_1d = init_device_mesh(test_case.device_type, mesh_shape)
    ...
    local_tensor = global_tensor[start_row:end_row].clone().to(test_case.device_type)
...
def _test_1d_to_1d_reshard_placement_change(test_case) -> None:
    for original_placement, new_placement in ONE_D_TO_ONE_D_PLACEMENTS:
        global_tensor = torch.arange(16, dtype=torch.float).view(4, 4)
        mesh_shape = (test_case.world_size,)
        device_mesh = init_device_mesh(test_case.device_type, mesh_shape)
...
class TestDTensorReshardPlacementChange(_CPUHFSafetensorsTestBase):
    ...
    def test_1d_to_1d_reshard_placement_change(self) -> None:
        _test_1d_to_1d_reshard_placement_change(self)
