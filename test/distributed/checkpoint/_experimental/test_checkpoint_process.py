...
def subprocess_init_fn(name, parent_pid):
...
def failing_subprocess_init_fn(name, parent_pid):
...
def timedout_subprocess_init_fn(**kwargs):
    # Acknowledge parameters to avoid unused variable warnings
    _ = kwargs
    time.sleep(3)  # Simulate a long initialization


def ckpt_writer_init_fn(**kwargs):
...
def failing_ckpt_writer_init_fn(**kwargs):
    """Initialize function that raises an exception."""
    # Acknowledge parameters to avoid unused variable warnings
    _ = kwargs
    raise RuntimeError("CheckpointWriter initialization failed")

def shared_tensor_verifier_init_fn(**kwargs):
    ...
    class SharedTensorVerifier(CheckpointWriter):
        ...
        def write(self, state_dict, _, **_):
            ...
            if "shared_tensor" in state_dict:
                ...
                assert shared_tensor.is_shared(), "Shared tensor should be in shared memory in subprocess"
            ...
            if "regular_tensor" in state_dict:
                ...
                assert state_dict["regular_tensor"].is_shared(), "Regular tensor should also be in shared memory in subprocess"
...
class TestCheckpointProcessConfig(unittest.TestCase):
    ...
    def test_custom_options(self):
        ...
        options = CheckpointProcessConfig(
            subprocess_init_timeout_secs=10,
            subprocess_shutdown_timeout_secs=30
        )
...
class TestCheckpointProcess(unittest.TestCase):
    ...
    def _create_checkpoint_process(self, 
        ...
        return CheckpointProcess(
            ...
            subprocess_init_args=subprocess_init_args_override or (
                "test-checkpointer",
                os.getpid(),
            ),
    ...
    def test_checkpoint_process_initialization(self):
    ...
    def test_checkpoint_write_sync_state_dict(self):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            expected_file = os.path.join(checkpoint_path, f"checkpoint_{self.rank_info.global_rank}.pt")
    ...
    def test_checkpoint_write_future_state_dict(self):
    ...
    def test_checkpoint_write_with_kwargs(self):
    ...
    def test_subprocess_initialization_timeout(self):
    ...
    def test_subprocess_initialization_failure(self):
    ...
    def test_shared_memory_tensor_ipc(self):
        ...
        self.assertTrue(shared_tensor.is_shared(), "Shared tensor should be in shared memory")
        self.assertFalse(regular_tensor.is_shared(), "Regular tensor should not be in shared memory")
        ...
        try:
            result = future.result()  # This will raise an exception if the subprocess assertions fail
        ...
        self.assertEqual(shared_tensor_data_ptr, shared_tensor.data_ptr(), "Shared tensor should still be in same shared memory")
        self.assertTrue(shared_tensor.is_shared(), "Shared tensor should still be in shared memory")
        ...
        self.assertAlmostEqual(shared_tensor[0][0], 42.0, places=6,
