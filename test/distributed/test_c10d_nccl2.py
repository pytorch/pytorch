...
class ProcessGroupNCCL2ObservabilityTest(MultiProcContinuousTest):
    ...
    @requires_nccl()
    @skip_if_lt_x_gpu(2)
    def test_profiler_batch_isend_irecv_annotations(self) -> None:
        def run_batch() -> None:
            send = torch.full(
                (1024,), self.rank, device=self.device, dtype=torch.float32
            )
            recv = torch.empty_like(send)
            ops = [
                dist.P2POp(dist.isend, send, (self.rank + 1) % self.world_size),
                dist.P2POp(dist.irecv, recv, (self.rank - 1) % self.world_size),
            ]
            for work in dist.batch_isend_irecv(ops):
                work.wait()

        # Initialize the communicator outside the profiled region.
        run_batch()
        torch.cuda.synchronize()
        ...
        batch_annotations = [
            ...
            and ev.get("args", {}).get("Collective name") == "batch_isend_irecv"
        ]
        self.assertEqual(len(batch_annotations), 1)
        args = batch_annotations[0]["args"]
        self.assertEqual(args["In msg nelems"], 1024)
        self.assertEqual(args["Out msg nelems"], 1024)
        self.assertEqual(args["dtype"], "Float")
        self.assertTrue(args["Is asynchronized op"])
        ...
        self.assertGreater(
            len(batch_kernels),
            0,
            "Expected a batch P2P GPU kernel with NCCL annotations",
        )
