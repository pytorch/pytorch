...
class NVSHMEMSymmetricMemoryTest(MultiProcContinuousTest):
    ...
    def test_cuda_graph_collective(self) -> None:
        ...
        # Allocation and rendezvous are host-side collectives (nvshmem_malloc,
        # team split, peer pointer upload), so they cannot be captured; the
        # buffers and their rendezvous must be established up front.
        inp = symm_mem.empty(numel, dtype=dtype, device=self.device)
