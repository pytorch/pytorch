...
def chunk_cat_mixed_dtype(tensors, num_chunks, out_dtype):
    expected = torch._chunk_cat([t.to(out_dtype) for t in tensors], 0, num_chunks)
    out = torch.empty_like(expected)
    torch.ops.fsdp.chunk_cat_mixed_dtype(tensors, num_chunks, out=out)
    return out, expected
...
class TestChunkCatMixedDtype(TestCase):
    ...
    def test_numerics(self, device, input_dtypes, out_dtype, num_chunks, noncontiguous):
        ...
        out, expected = chunk_cat_mixed_dtype(tensors, num_chunks, out_dtype)
    ...
    def test_version_counter(self, device):
        ...
        torch.ops.fsdp.chunk_cat_mixed_dtype(tensors, 2, out=out)
    ...
    def test_invalid_inputs(self, device):
        ...
        with self.assertRaisesRegex(RuntimeError, "positive num_chunks"):
            torch.ops.fsdp.chunk_cat_mixed_dtype([valid], 0, out=out)
        with self.assertRaisesRegex(RuntimeError, "non-empty tensor"):
            torch.ops.fsdp.chunk_cat_mixed_dtype(
                [valid, torch.empty(0, device=device)], 2, out=out
            )
    ...
    def test_fake_tensor(self, device):
        with FakeTensorMode():
            ...
            torch.ops.fsdp.chunk_cat_mixed_dtype(tensors, 4, out=out)
