...
class TestHSDPTraining(FSDPTest):
    @property
    def world_size(self) -> int:
        return min(8, torch.get_device_module(device_type).device_count())

    @skip_if_lt_x_gpu(4)
    def test_hsdp_train_parity_multi_group(self):
        """
        ...
        self.run_subtests(
            ...
            self._test_hsdp_train_parity_multi_group,
        )

    @skip_if_lt_x_gpu(4)
    def test_hsdp_train_parity_cpu_offload_eager(self):
        """
        Tests train parity against DDP when using multiple parameter groups for
    ...
    @skip_if_lt_x_gpu(4)
    def test_hsdp_train_parity_multi_group_unshard_async_op(self):
        """
        Tests train parity against DDP when using multiple parameter groups for
    ...
    def _test_hsdp_train_parity_multi_group(
        self,
        ...
        replicate(ref_model.to(device_type))
