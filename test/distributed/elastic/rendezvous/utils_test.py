        ...
        for config_str in config_strs:
            with self.subTest(config_str=config_str):
                with self.assertRaisesRegex(
                    ValueError,
                    r"^The rendezvous config string must be in format "
    ...
    def test_parse_rendezvous_endpoint_raises_error_if_hostname_is_invalid(self) -> None:
        ...
        for endpoint in endpoints:
            with self.subTest(endpoint=endpoint):
                with self.assertRaisesRegex(
                    ValueError,
                    r"^The hostname of the rendezvous endpoint must be a dot-separated list of "
                    r"labels, an IPv4 address, or an IPv6 address.$",
                ):
    ...
    def test_parse_rendezvous_endpoint_raises_error_if_port_is_invalid(self) -> None:
        ...
        for endpoint in endpoints:
            with self.subTest(endpoint=endpoint):
                with self.assertRaisesRegex(
                    ValueError,
                    r"^The port number of the rendezvous endpoint must be an integer between 0 and "
                    r"65536.$",
                ):
                    _parse_rendezvous_endpoint(endpoint, default_port=123)
