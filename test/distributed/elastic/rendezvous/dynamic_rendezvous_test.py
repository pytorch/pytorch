...
class RendezvousStateTest(TestCase):
    def test_encoded_size_is_within_expected_limit(self) -> None:
        ...
        state.round = 1
        state.complete = True
        state.deadline = datetime.now(datetime.timezone.utc)
        state.closed = True
        ...
        for num_nodes, max_byte_size in expected_max_sizes:
            with self.subTest(num_nodes=num_nodes, max_byte_size=max_byte_size):
                for i in range(num_nodes):
                    ...
                    state.last_heartbeats[node_running] = datetime.now(datetime.timezone.utc)
                    state.last_heartbeats[node_waiting] = datetime.now(datetime.timezone.utc)
...
class DynamicRendezvousHandlerTest(TestCase):
    ...
    def _assert_keep_alive_swallows_rendezvous_errors(self) -> None:
        last_heartbeat_time = datetime.now(datetime.timezone.utc) - (self._keep_alive_interval * 2)
