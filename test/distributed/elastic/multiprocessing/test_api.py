...
class SignalHandlingTest(unittest.TestCase):
    def setUp(self):
        ...
        self.original_signals_env = os.environ.get("TORCHELASTIC_SIGNALS_TO_HANDLE", None)
    ...
    def test_start_registers_default_signals(self, mock_logger, mock_signal, mock_threading):
        """Test that the start method registers the default signals."""
        # Setup
        mock_threading.current_thread.return_value = mock_threading.main_thread.return_value
    ...
    @patch("torch.distributed.elastic.multiprocessing.api.signal")
    @patch("torch.distributed.elastic.multiprocessing.api.logger")
    def test_start_registers_custom_signals(self, mock_logger, mock_signal, mock_threading):
    ...
    @patch("torch.distributed.elastic.multiprocessing.api.signal")
    @patch("torch.distributed.elastic.multiprocessing.api.logger")
    def test_start_handles_invalid_signals(self, mock_logger, mock_signal, mock_threading):
    ...
    @patch("torch.distributed.elastic.multiprocessing.api.signal")
    @patch("torch.distributed.elastic.multiprocessing.api.logger")
    def test_start_handles_windows_signals(self, mock_logger, mock_signal, mock_threading):
        ...
        with patch("torch.distributed.elastic.multiprocessing.api.IS_WINDOWS", True):
            ...
            sighup_logged = any("SIGHUP" in call and "Windows" in call for call in info_calls)
            sigusr1_logged = any("SIGUSR1" in call and "Windows" in call for call in info_calls)
            
            self.assertTrue(sighup_logged, f"Expected SIGHUP Windows message in info calls: {info_calls}")
    ...
    @patch("torch.distributed.elastic.multiprocessing.api.signal")
    @patch("torch.distributed.elastic.multiprocessing.api.logger")
    def test_start_supports_sigusr1_and_sigusr2(self, mock_logger, mock_signal, mock_threading):
        ...
        registered_signals = [call[0][0] for call in signal_calls]  # Extract the signal from each call
        
        # Verify both SIGUSR1 and SIGUSR2 were registered
        self.assertIn(signal.SIGUSR1, registered_signals, "SIGUSR1 should be registered")
        ...
        info_calls = [str(call) for call in mock_logger.info.call_args_list]
        sigusr1_logged = any("SIGUSR1" in call and "Registered signal handler" in call for call in info_calls)
        sigusr2_logged = any("SIGUSR2" in call and "Registered signal handler" in call for call in info_calls)
        
        self.assertTrue(sigusr1_logged, f"Expected SIGUSR1 registration message in info calls: {info_calls}")
        self.assertTrue(sigusr2_logged, f"Expected SIGUSR2 registration message in info calls: {info_calls}")
