...
if not (TEST_WITH_DEV_DBG_ASAN or IS_WINDOWS or IS_MACOS):
    ...
    class StartProcessesListAsBinaryTest(_StartProcessesTest):
        ...
        def test_binary_duplicate_log_filters(self):
            ...
            self.assert_not_in_file(["[rank0]:helloB stdout from 0"], pc.filtered_stdout)
        ...
        def test_function_duplicate_log_filters(self):
            for start_method in self._start_methods:
                with self.subTest(start_method=start_method):
                    ...
                    self.assertFalse(result.is_failed())
                    self.assert_in_file(["[trainer0]:helloA stdout from 0"], pc.filtered_stdout)
                    self.assert_not_in_file(["[trainer0]:helloB stdout from 0"], pc.filtered_stdout)
                    self.assert_in_file(["[trainer1]:worldA stderr from 1"], pc.filtered_stderr)
                    self.assert_in_file(["[trainer1]:worldB stderr from 1"], pc.filtered_stderr)
                    for tail_log in pc._tail_logs:
                        self.assertTrue(tail_log.stopped())
