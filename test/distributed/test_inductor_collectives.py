...
class TestCollectivesInductor(DynamoDistributedSingleProcTestCase):
    ...
    def test_inductor_steal_buffer(self):
        ...
        (
            FileCheck()
            .check("buf2 = empty_strided")
            .check(".run(arg1_1, buf2")
            .check("torch.ops._c10d_functional.all_reduce_.default(buf2")
            .check("torch.ops._c10d_functional.wait_tensor.default(buf2")
            .check("buf7 = empty_strided")
            .check(".run(buf7, 16")
            .check("return (buf2, buf7")
            .run(code)
        )
        out = compiled(inputs, **self.get_world_trs())
        correct = func(inputs, **self.get_world_trs())
    ...
    def test_inductor_doesnt_mutate_shared(self):
        ...
        (
            ...
            .check("buf2 = empty_strided")
            .check("buf7 = empty_strided")
            .check(".run(arg1_1, buf2, buf7, 16")
            .check("torch.ops._c10d_functional.all_reduce_.default(buf2")
            .check("torch.ops._c10d_functional.wait_tensor.default(buf2")
            .check("buf8 = empty_strided")
            .check(".run(buf8, 16")
            .check("return (buf2, buf7, buf8")
            .run(code)
        )
        out = compiled(inputs, **self.get_world_trs())
        correct = func(inputs, **self.get_world_trs())
    ...
    def test_inductor_reduce_scatter_coalesced(self):
        ...
        (
            ...
            .check(".run(arg1_1, buf0, buf10, 16")
            .check(
                "buf1 = torch.ops._c10d_functional.reduce_scatter_tensor_coalesced.default([buf0, arg1_1]"
            )
            .check("buf2 = buf1[0]")
            .check("buf3 = buf1[1]")
            .check("torch.ops._c10d_functional.wait_tensor.default(buf2")
