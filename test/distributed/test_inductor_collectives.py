...
class TestBucketingTrace(torch._dynamo.test_case.TestCase):
    ...
    def test_all_gather_bucket_trace_accepts_hinted_unbacked_chunk_numel(self):
        ...
        self.assertFalse(
            any(
                node.target == torch.ops.aten.split_with_sizes_copy.out
                for node in gm.graph.nodes
            )
        )
    ...
    def test_all_gather_bucket_trace_uses_fused_unpack_for_static_shapes(self):
        ...
        self.assertEqual(
            targets.count(torch.ops.aten.split_with_sizes_copy.out),
            1,
        )
    ...
    def test_match_dim0_padded_cat(self):
        ...
        match = _match_dim0_padded_cat(cat, 4)

        self.assertIsNotNone(match)
        src, packing_nodes = match
        self.assertEqual(src.op, "placeholder")
        ...
        split.args = (*split.args, -2)
        self.assertIsNotNone(_match_dim0_padded_cat(cat, 4))

        cat.args = (cat.args[0], -4)
        self.assertIsNone(_match_dim0_padded_cat(cat, 4))
        cat.args = (cat.args[0], -2)
        ...
        with gm.graph.inserting_before(reduce_scatter):
            source_fanout = gm.graph.call_function(
                torch.ops.aten.neg.default,
                args=(src,),
            )
        self.assertIsNone(_match_dim0_padded_cat(cat, 4))
        gm.graph.erase_node(source_fanout)
    ...
    def test_match_dim0_padded_cat_rejects_symbolic_shapes(self):
        def padded_input(value):
            chunks, _ = Shard(0)._split_tensor(
                value,
                4,
        ...
        with fake_mode:
            symbolic_size = fake_mode.shape_env.create_unbacked_symint()
            value = torch.empty(symbolic_size, 4)

        gm = make_fx(padded_input)(value)
        cat = next(
            node
    ...
    def test_reduce_scatter_bucket_fuses_dim0_padded_cat(self):
        def padded_input(value):
            chunks, _ = Shard(0)._split_tensor(
                value,
                4,
        ...
        with fake_mode:
            ...
            y = torch.empty(7, 2)
            z = torch.empty(8, 3)
        gm = make_fx(lambda x, y, z: (padded_input(x), padded_input(y), z))(x, y, z)
        graph = gm.graph
        ...
        self.assertEqual(
            ...
            [torch.Size((2, 4)), torch.Size((2, 2)), torch.Size((2, 3))],
        )
...
class TestBucketingTraceAccelerator(torch._dynamo.test_case.TestCase):
    ...
    def test_fused_all_gather_unpack_handles_mixed_dtypes(self, device):
        group_size = 2
        ...
        unpack = functools.partial(
            _unpack_bucketed_all_gather_output,
            ins_sizes=[torch.Size((3,)), torch.Size((2, 2)), torch.Size((0,))],
            ins_split_sizes=[6, 16, 0],
            out_dtypes=[torch.bfloat16, torch.float32, torch.bfloat16],
            group_size=group_size,
            bucket_dtype=torch.uint8,
        )
        gm = make_fx(unpack)(rank_major_bytes)
        compiled = compile_fx_inner(gm, [rank_major_bytes])

        FileCheck().check_count(
            "torch.ops.aten.split_with_sizes_copy.out(", count=1, exactly=True
        ).run(compiled.source_code)
        actual = compiled([rank_major_bytes])
    ...
    @onlyAccelerator
    @requires_triton_backend
    def test_all_gather_unpack_handles_all_empty_outputs(self, device):
        actual = _unpack_bucketed_all_gather_output(
            torch.empty(2, 0, device=device, dtype=torch.uint8),
    ...
    @onlyAccelerator
    @requires_triton_backend
    def test_pre_bucket_all_gather_foreach_handles_different_lengths(self, device):
        inputs = [
            torch.arange(3, device=device, dtype=torch.bfloat16),
            torch.arange(8, device=device, dtype=torch.bfloat16).reshape(2, 4).T,
...
instantiate_device_type_tests(TestBucketingTraceAccelerator, globals(), allow_xpu=True)
...
class _CollectivesMultiProcBase(DynamoDistributedMultiProcTestCase):
    """
...
class TestCollectivesMultiProcGeneric(_CollectivesMultiProcBase):
    hw_classification = HardwareClassification.GENERIC
...
@requires_accelerator_dist_backend(["nccl", "xccl"])
class TestCollectivesMultiProcAccelerator(_CollectivesMultiProcBase):
    hw_classification = HardwareClassification.ACCELERATOR
...
@requires_accelerator_dist_backend(["nccl", "xccl"])
class TestCollectivesMultiProcCUDA(_CollectivesMultiProcBase):
...
instantiate_parametrized_tests(TestCollectivesMultiProcGeneric)
instantiate_parametrized_tests(TestCollectivesMultiProcCUDA)
instantiate_parametrized_tests(TestCollectivesMultiProcAccelerator)


class _CollectivesInductorBase(DynamoDistributedSingleProcTestCase):
    """
...
class TestCollectivesInductorGeneric(_CollectivesInductorBase):
    hw_classification = HardwareClassification.GENERIC
...
@requires_accelerator_dist_backend(["nccl", "xccl"])
@unittest.skipIf(not torch.accelerator.is_available(), "No accelerator is available")
class TestCollectivesInductorAccelerator(_CollectivesInductorBase):
    hw_classification = HardwareClassification.ACCELERATOR
...
@requires_accelerator_dist_backend(["nccl", "xccl"])
@unittest.skipIf(not torch.accelerator.is_available(), "No accelerator is available")
class TestCollectivesInductorCUDA(_CollectivesInductorBase):
    hw_classification = HardwareClassification.CUDA
...
instantiate_parametrized_tests(TestCollectivesInductorGeneric)
instantiate_parametrized_tests(TestCollectivesInductorCUDA)
instantiate_parametrized_tests(TestCollectivesInductorAccelerator)


class _SyncDecisionCrossRanksBase(MultiProcessTestCase):
    def setUp(self) -> None:
        super().setUp()
...
@requires_accelerator_dist_backend(["nccl", "xccl"])
class TestSyncDecisionCrossRanksAccelerator(_SyncDecisionCrossRanksBase):
    hw_classification = HardwareClassification.ACCELERATOR
...
@requires_accelerator_dist_backend(["nccl", "xccl"])
class TestSyncDecisionCrossRanksCUDA(_SyncDecisionCrossRanksBase):
    hw_classification = HardwareClassification.CUDA
