...
class ProcessGroupGlooTestBase(MultiProcessTestCase):
    lazy_init = False
...
class ProcessGroupGlooTestCUDA(ProcessGroupGlooTestBase):
    ...
    @skip_if_lt_x_gpu(2)
    @requires_gloo()
    def test_broadcast_basics_cuda(self, device):
        self._test_broadcast_basics(lambda t: t.clone().to(device))

    @skip_if_lt_x_gpu(2)
    @requires_gloo()
    @skipIfRocm
    def test_broadcast_stress_cuda(self, device):
        inputs = [
            torch.tensor([i * self.world_size + self.rank], device=device)
            for i in range(1000)
        ]
        self._test_broadcast_stress(inputs)
    ...
    @requires_gloo()
    @skipIfRocm
    def test_allreduce_stress_cuda(self, device):
        inputs = [torch.tensor([i + self.rank], device=device) for i in range(1000)]
        self._test_allreduce_stress(inputs)

    @skip_if_lt_x_gpu(1)
    @requires_gloo()
    def test_allreduce_coalesced_checks_cuda(self, device):
        store = c10d.FileStore(self.file_name, self.world_size)
        pg = self._create_process_group_gloo(
            store, self.rank, self.world_size, self.opts()
        )
        ...
        with self.assertRaisesRegex(RuntimeError, "unsupported device type"):
            opts = c10d.AllreduceCoalescedOptions()
            pg.allreduce_coalesced([t1.to(device), t1.to(device)], opts)

    @skip_if_lt_x_gpu(2)
    @requires_gloo()
    def test_sparse_allreduce_basics_cuda(self, device):
        self._test_sparse_allreduce_basics(lambda t: t.clone().to(device))

    @skip_if_lt_x_gpu(2)
    @requires_gloo()
    def test_sparse_allreduce_cuda_dispatched(self, device):
        store = c10d.FileStore(self.file_name, self.world_size)
        dist.init_process_group(
        ...
        for inputs, outputs in tests:
            tensors = inputs[-1].clone().to(device)
    ...
    @skip_if_lt_x_gpu(2)
    @requires_gloo()
    def test_scatter_basics_cuda(self, device):
        self._test_scatter_basics(lambda t: t.clone().to(device))
    ...
    @requires_gloo()
    @skipIfRocm
    def test_scatter_stress_cuda(self, device):
        inputs = [
            [torch.tensor([i + self.rank]) for _ in range(self.world_size)]
            for i in range(1000)
        ]
        self._test_scatter_stress(inputs, lambda t: t.clone().to(device))

    @skip_if_lt_x_gpu(2)
    @requires_gloo()
    def test_gather_basics_cuda(self, device):
        self._test_gather_basics(lambda t: t.clone().to(device))

    @skip_if_lt_x_gpu(2)
    @skipIfRocm
    @requires_gloo()
    def test_gather_stress_cuda(self, device):
        inputs = [torch.tensor([i + self.rank], device=device) for i in range(1000)]
        self._test_gather_stress(inputs, lambda t: t.clone().to(device))

    @skip_if_lt_x_gpu(2)
    @requires_gloo()
    def test_allgather_basics_cuda(self, device):
        self._test_allgather_basics(lambda t: t.clone().to(device))

    @skip_if_lt_x_gpu(2)
    @requires_gloo()
    @skipIfRocm
    def test_allgather_stress_cuda(self, device):
        inputs = [torch.tensor([i + self.rank], device=device) for i in range(1000)]
        self._test_allgather_stress(inputs, lambda t: t.clone().to(device))

    @skip_if_lt_x_gpu(2)
    @requires_gloo()
    def test_reduce_basics_cuda(self, device):
        self._test_reduce_basics(lambda t: t.clone().to(device))

    @skip_if_lt_x_gpu(2)
    @requires_gloo()
    @skipIfRocm
    def test_reduce_stress_cuda(self, device):
        inputs = [torch.tensor([i + self.rank], device=device) for i in range(1000)]
        self._test_reduce_stress(inputs)
    ...
    @requires_gloo()
    @skipIfRocm
    def test_block_current_stream_cuda(self, device):
        store = c10d.FileStore(self.file_name, self.world_size)
        pg = self._create_process_group_gloo(
            store, self.rank, self.world_size, self.opts()
        )
    ...
    @skip_if_lt_x_gpu(2)
    @requires_gloo()
    def test_alltoall_basics_cuda(self, device):
        self._test_alltoall_basics(lambda t: t.clone().to(device))

    @skip_if_lt_x_gpu(2)
    @requires_gloo()
    @skipIfRocm
    def test_alltoall_stress_cuda(self, device):
        inputs = [
            [torch.tensor([i * self.world_size + j]) for j in range(self.world_size)]
            for i in range(1000)
        ]
        self._test_alltoall_stress(inputs, lambda t: t.clone().to(device))

    @skip_if_lt_x_gpu(2)
    @requires_gloo()
    def test_alltoall_data_routing_cuda(self, device):
        self._test_alltoall_data_routing(lambda t: t.clone().to(device))

    @skip_if_lt_x_gpu(2)
    @requires_gloo()
    def test_alltoall_multidim_cuda(self, device):
        self._test_alltoall_multidim(lambda t: t.clone().to(device))


class DistributedDataParallelTestBase(
...
class DistributedDataParallelTest(DistributedDataParallelTestBase):
    hw_classification = HardwareClassification.GENERIC
...
class DistributedDataParallelTestCUDA(DistributedDataParallelTestBase):
    hw_classification = HardwareClassification.CUDA

    @requires_gloo()
    @skip_if_lt_x_gpu(2)
    def test_gloo_backend_1gpu_module_device_ids_integer_list(self, device):
        int_devices = gpus_for_rank(self.world_size)[self.rank][:1]
        devices = [torch.device(device + ":" + str(i)) for i in int_devices]
        self._test_gloo_backend(devices, int_devices)

    @requires_gloo()
    @skip_if_lt_x_gpu(2)
    def test_gloo_backend_1gpu_module_device_ids_torch_device_list(self, device):
        int_devices = gpus_for_rank(self.world_size)[self.rank][:1]
        devices = [torch.device(device + ":" + str(i)) for i in int_devices]
    ...
    @requires_gloo()
    @skip_if_lt_x_gpu(4)
    def test_gloo_backend_2gpu_module(self, device):
    ...
    @requires_gloo()
    @skip_if_lt_x_gpu(8)
    def test_gloo_backend_4gpu_module(self, device):
    ...
    @requires_gloo()
    @skip_if_lt_x_gpu(2)
    def test_ignored_sharded_tensor(self, device):
    ...
    def test_save_load_checkpoint(self, device):
        ...
        dist.barrier()
        map_location = {f"{device}:0": f"{device}:{self.rank:d}"}
    ...
    @requires_gloo()
    @skip_if_lt_x_gpu(2)
    def test_ddp_comm_hook_future_passing_gpu_gloo(self, device):
        """
...
@skip_if_win32()
class ProcessGroupGlooLazyInitTestCUDA(ProcessGroupGlooTestCUDA):
    hw_classification = HardwareClassification.CUDA
...
class ProcessGroupGlooFRTestCUDA(ProcessGroupGlooTestCUDA):
    hw_classification = HardwareClassification.CUDA
...
class CommTestBase(test_c10d_common.AbstractCommTest, MultiProcessTestCase):
    @property
    def device(self):
        return "cpu"
...
class CommTestCUDA(CommTestBase):
    hw_classification = HardwareClassification.CUDA

    @requires_gloo()
    @skip_if_lt_x_gpu(2)
    def test_broadcast_coalesced_gloo_cuda(self, device):
        store = c10d.FileStore(self.file_name, self.world_size)
        c10d.init_process_group(
            backend="gloo", store=store, rank=self.rank, world_size=self.world_size
        )
        process_group = c10d.distributed_c10d._get_default_group()
        device = torch.device(f"{device}:{self.rank:d}")
        backend = process_group._get_backend(device)
        backend.create_device(interface=LOOPBACK)
        ranks = list(range(self.world_size))
        for root_rank in ranks:
    ...
    @skip_if_lt_x_gpu(2)
    @requires_gloo()
    def test_gloo_warn_not_in_group(self, device):
        self._test_warn_not_in_group(backend="gloo")

    @skip_if_lt_x_gpu(2)
    @requires_gloo()
    def test_gloo_rank_membership(self, device):
    ...
    @skip_if_lt_x_gpu(1)
    @requires_gloo()
    def test_split_group_keeps_gloo_options(self, device):
        ...
        c10d.init_process_group(
            ...
            rank=self.rank,
            world_size=self.world_size,
            device_id=torch.device(device, self.rank % torch.cuda.device_count()),
        )
        ranks = list(range(self.world_size))
...
instantiate_device_type_tests(ProcessGroupGlooTestCUDA, globals(), only_for="cuda")
instantiate_device_type_tests(
    DistributedDataParallelTestCUDA, globals(), only_for="cuda"
)
instantiate_device_type_tests(
    ProcessGroupGlooLazyInitTestCUDA, globals(), only_for="cuda"
)
instantiate_device_type_tests(ProcessGroupGlooFRTestCUDA, globals(), only_for="cuda")
instantiate_device_type_tests(CommTestCUDA, globals(), only_for="cuda")
