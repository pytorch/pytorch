...
BACKEND = torch.distributed.Backend.NCCL if torch.cuda.is_available() else torch.distributed.Backend.GLOO  # type: ignore
...
class TestZeroRedundancyOptimizer(common_distributed.MultiProcessTestCase):
    ...
    @property
    def device(self):
        return torch.device(self.rank) if BACKEND == torch.distributed.Backend.NCCL else torch.device("cpu")
    ...
    def dist_init(self, rank, world_size=-1):
        store = torch.distributed.FileStore(self.file_name, self.world_size if world_size < 1 else world_size)
        return torch.distributed.init_process_group(backend=BACKEND, store=store, rank=rank, world_size=self.world_size)
...
class TestZeroRedundancyOptimizerSingleRank(TestZeroRedundancyOptimizer):
    def test_state_dict(self):
        ...
        o = ZeroRedundancyOptimizer([x], optim=torch.optim.SGD, lr=0.1, momentum=0.9)
        x.backward()
        o.step()
        ...
        o = ZeroRedundancyOptimizer([x], optim=torch.optim.SGD, lr=0.01)
    ...
    def test_local_state_dict(self):
        ...
        x = torch.tensor([1.0], device=DEVICE, requires_grad=True)
        o = ZeroRedundancyOptimizer([x], optim=torch.optim.SGD, lr=0.1)
    ...
    def test_zero_grad(self):
        ...
        o = ZeroRedundancyOptimizer(m.parameters(), optim=torch.optim.SGD, lr=0.1)
        y = m(x)
        y.backward(x)
...
class TestZeroRedundancyOptimizerDistributed(TestZeroRedundancyOptimizer):
    ...
    def test_step(self):
        ...
        if self.rank > 1 or (BACKEND == torch.distributed.Backend.NCCL and torch.cuda.device_count() < 2):
            return
    ...
    def test_sharding(self):
        ...
        o = ZeroRedundancyOptimizer(params, optim=torch.optim.SGD, lr=0.1)
    ...
    def test_collect_shards(self):
        ...
        optimizer = ZeroRedundancyOptimizer(model.parameters(), optim=torch.optim.SGD, lr=0.1, momentum=0.99)
        ...
        optimizer_state_dict = _broadcast_object(
            optimizer_state_dict, src_rank=RECIPIENT_RANK, group=torch.distributed.group.WORLD, dist_device=self.device
        )
    ...
    def test_multiple_groups(self):
        ...
        torch.distributed.init_process_group(backend="gloo", store=store, rank=self.rank, world_size=self.world_size)
        ...
        def check(optimizer):
            ...
            for _ in range(epochs):
                ...
                def closure():
                    ...
                    loss.backward()
                    torch.distributed.all_reduce(loss, group=process_group)  # Not strictly needed for the test below
                ...
                for pg in optimizer.param_groups:
                    for p in pg["params"]:
                        ...
                        torch.distributed.gather(p, receptacle, dst=0, group=process_group)
                        if self.rank == 0:
        ...
        if self.rank in sub_group_ranks:
            ...
            # With SGD, Momentum is required to get a state to shard
            optimizer = ZeroRedundancyOptimizer(
                model.parameters(), optim=torch.optim.SGD, lr=0.1, momentum=0.99, group=process_group, bucket_cap_kb=0
            )
            check(optimizer)
