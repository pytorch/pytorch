...
class TestZeroRedundancyOptimizerDistributedBase(TestZeroRedundancyOptimizer):
    @property
    def world_size(self):
    ...
    def context(self, device_type=None):
        dev = device_type if device_type is not None else self.device
        if dev != "cpu":
            return torch.get_device_module(dev).device(self.rank)
        else:
            return nullcontext()
    ...
    def _test_zero_join(self, device):
        ...
        ddp_model = (
            DDP(model, device_ids=[rank]) if device != "cpu" else DDP(model)
        )
...
class TestZeroRedundancyOptimizerDistributed(TestZeroRedundancyOptimizerDistributedBase):
    hw_classification = HardwareClassification.GENERIC
...
class TestZeroRedundancyOptimizerDistributedDevice(TestZeroRedundancyOptimizerDistributedBase):
    ...
    def test_step(self, device):
        ...
        self._skip_if_insufficient_devices(device_type)
        self.create_pg(device_type)
        LR = 0.01

        with self.context(device_type):
            x = torch.tensor([float(self.rank + 1)], device=device_type)
            m = torch.nn.Linear(1, 1)
            m.weight.data = torch.tensor([[1.0]])
            m.bias.data = torch.tensor([2.0])
            m = m.to(device_type)
            m_zero = copy.deepcopy(m).to(device_type)
    ...
    def test_step_with_closure(self, device):
        ...
        with self.context(device_type):
            for bucket_view in [False, True]:
                ...
                x = torch.tensor([float(x_val)], device=device_type)
                m = torch.nn.Linear(1, 1)
    ...
    def test_lr_scheduler(self, device):
        ...
        self._skip_if_insufficient_devices(device_type)
        self.create_pg(device_type)
        x = torch.tensor([1.0], device=device_type, requires_grad=True)
        x2 = torch.tensor([1.0], device=device_type, requires_grad=True)
    ...
    def test_multiple_param_groups(self, device):
        ...
        model1 = model1.to(device_type)
        model2 = model2.to(device_type)
        model3 = model3.to(device_type)
        inputs = [
            torch.randn(BATCH_SIZE, INPUT_DIM).to(device_type) for _ in range(NUM_ITERS)
        ]
    ...
    def test_collect_shards(self, device):
        ...
        REFERENCE_RANK = 0
        target = torch.rand((BATCH_SIZE, OUTPUT_DIM), device=device_type)
        inputs = torch.rand((BATCH_SIZE, INPUT_DIM), device=device_type)
        model = torch.nn.Sequential(
            torch.nn.Linear(INPUT_DIM, HIDDEN_DIM),
            torch.nn.Linear(HIDDEN_DIM, OUTPUT_DIM),
        ).to(device_type)
        loss_fn = torch.nn.L1Loss()
        loss_fn.to(device_type)
        optimizer = ZeroRedundancyOptimizer(
            model.parameters(),
            optimizer_class=SGD,
            lr=LR,
    ...
    def test_local_optimizer_parity(
        ...
        with self.context(device_type):
            ...
            model.test_buffer = torch.nn.Buffer(
                torch.ones((1), device=device_type) * self.rank,
            )
            ...
            sharded_ddp_model = DDP(
                module=model,
                device_ids=[self.rank] if device_type != "cpu" else None,
                broadcast_buffers=True,
                find_unused_parameters=True,
            )
            local_model = copy.deepcopy(model).to(device_type)
            ...
            def check_step():
                input_tensor = torch.rand((BATCH_SIZE, INPUT_DIM)).to(device_type)
    ...
    def test_zero_join_gpu(self, device):
        ...
        device_type = torch.device(device).type
        self._skip_if_insufficient_devices(device_type)
        self._test_zero_join(device_type)
...
instantiate_device_type_tests(TestZeroRedundancyOptimizerDistributedDevice, globals(), except_for="cpu")
