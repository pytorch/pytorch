...
class TestZeroRedundancyOptimizerDistributed(TestZeroRedundancyOptimizer):
    ...
    def test_repeatability(self):
        """Check that saving/loading and iterating produces the same results as only interating."""
        ...
        with torch.cuda.device(self.rank):
            ...
            def check_optimizer_equivalence(optimizer: Type[torch.optim.Optimizer]):
                ...
                def step():
                    ...
                    return sharded_optimizer.step(closure=closure_sharded)
                ...
                # - load the saved state, should rewind everything
                sharded_optimizer.load_state_dict(sharded_optimizer_state_dict)

                # - re-step() a couple of times, check that we get the exact same loss
                rewind_loss = None
                for i in range(5):
                    rewind_loss = step()

                assert torch.allclose(
                    rewind_loss, ref_loss
                ), f"Rewind and reference losses differ: {rewind_loss}, {ref_loss}"
