...
class StageTest(MultiProcContinuousTest):
    ...
    def test_recv_buffer_hooks_do_not_accumulate(self):
        ...
        #
        # Backward hooks on stage-boundary activations must not accumulate
        # across training steps. Before the fix, requires_grad_() was in-place
        # and returned self, aliasing the persistent recv buffer directly as
        # the stage activation. Hooks registered in step N persisted into
        # step N+1 causing fired counts to grow: 2, 4, 6, 8, 10.
        # detach() gives a fresh tensor identity each step (zero-copy,
        # preserves pinned/custom allocator properties) so hooks are
