...
class TestMiscCollectiveUtils(TestCase):
    def test_device_to_backend_mapping(self, device) -> None:
        ...
        if "cuda" in device:
            ...
            if dist.get_default_backend_for_device(device) != BACKEND:
                raise AssertionError(
                    f"Expected {BACKEND}, got {dist.get_default_backend_for_device(device)}"
                )
        else:
...
devices = ["cpu", "cuda", "mps", "xpu", device_type]
