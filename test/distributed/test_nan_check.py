...
class TestNanCheckDevice(DeviceTypeTestBase):
    ...
    def test_no_nan_float64_accelerator(self, device):
        self._check_for_nan(
            torch.randn(100, dtype=torch.float64, device=device)
        )
        torch.accelerator.synchronize(device)

    def test_no_nan_float16_accelerator(self, device):
        self._check_for_nan(
            torch.randn(100, dtype=torch.float16, device=device)
        )
        torch.accelerator.synchronize(device)

    def test_no_nan_bfloat16_accelerator(self, device):
        self._check_for_nan(
            torch.randn(100, dtype=torch.bfloat16, device=device)
        )
        torch.accelerator.synchronize(device)
...
instantiate_device_type_tests(TestNanCheckDevice, globals(), except_for="cpu")
