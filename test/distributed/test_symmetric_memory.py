...
# When `nullcontext` is used, all the fused ops will be dispatched to the regular
# symmetric memory implementation. When `_test_mode` is used, the calls will be
# dispatched to the fallback implementation, which will not utilize any symmetric
# memory but the regular collective APIs + the pytorch computation kernels.
