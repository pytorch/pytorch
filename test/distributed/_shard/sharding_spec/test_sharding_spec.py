...
class TestShardingSpec(TestCase):
    ...
    def test_enumerable_sharding_spec(self):
        ...
        with self.assertRaisesRegex(ValueError, "shard_offsets should be >=0"):
            ShardMetadata(
                shard_offsets=[-1, 0], shard_lengths=[1, 1], placement="cuda:0"
            )

        with self.assertRaisesRegex(ValueError, "shard_lengths should be >= 0"):
            ShardMetadata(
                shard_offsets=[0, 0], shard_lengths=[-1, 1], placement="cuda:0"
            )
        ...
        with self.assertRaisesRegex(ValueError, "Found inconsistent ranks for shards"):
            EnumerableShardingSpec(
                [
                    ShardMetadata(
                        shard_offsets=[0, 0], shard_lengths=[1, 1], placement="cpu"
                    ),
                    ShardMetadata(
                        shard_offsets=[0, 0, 0],
                        shard_lengths=[1, 1, 1],
        ...
        with self.assertRaisesRegex(ValueError, "Shards.*overlap"):
            EnumerableShardingSpec(
                [
                    ShardMetadata(
                        shard_offsets=[0, 0], shard_lengths=[3, 3], placement="cpu"
                    ),
                    ShardMetadata(
                        shard_offsets=[2, 0], shard_lengths=[3, 3], placement="cpu"
                    ),
                ]
            )
