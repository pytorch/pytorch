...
class TestCreateTensorFromParams(TestCase):
    ...
    def test_empty(self):
        ...
        local_device = torch.device(device_type, 0)
...
class TestShardedTensorChunked(ShardedTensorTestBase):
    ...
    def test_complete_world_size(self):
        for dim in [0, -2]:
            ...
            self.assertEqual(torch.device(self.device_type, self.rank), local_shard.device)
            if self.rank == 3:
            ...
            for rank, shard_metadata in enumerate(shards_metadata):
                ...
                self.assertEqual(
                    f"rank:{rank}/{self.device_type}:{rank}", str(shard_metadata.placement)
                )
            ...
            for rpc_rank, shards in remote_shards.items():
                ...
                for remote_shard in shards:
                    ...
                    self.assertEqual(
                        f"rank:{rpc_rank}/{self.device_type}:{rpc_rank}",
    ...
    def test_gather_even(self) -> None:
        ...
        if self.rank == dst:
            full_tensor = torch.zeros(
                h,
                w,
                device=torch.device(self.device_type, dst),
            )
    ...
    def test_create_sharded_tensor_with_rand(self):
        ...
        expected_h = 2
        expected_device = torch.device(self.device_type, self.rank)
        dtype = torch.double
    ...
    def test_partial_world_size(self):
        ...
        for shard_rank, shard_metadata in enumerate(shards_metadata):
            ...
            self.assertEqual(
                f"rank:{shard_rank + 2}/{self.device_type}:{shard_rank + 2}",
        ...
        for rpc_rank, shards in remote_shards.items():
            ...
            for remote_shard in shards:
                ...
                shard = remote_shard.to_here()
                self.assertEqual(
                    f"rank:{rpc_rank}/{self.device_type}:{rpc_rank}", str(shard.metadata.placement)
                )
    ...
    def test_multiple_local_shards(self):
        ...
        for local_shard in local_shards:
            self.assertEqual(
                torch.device(self.device_type, self.rank), local_shard.tensor.device
            )
        ...
        for shard_idx, shard_metadata in enumerate(shards_metadata):
            ...
            self.assertEqual(
                f"rank:{shard_idx % 4}/{self.device_type}:{shard_idx % 4}",
                str(shard_metadata.placement),
            )
    ...
    def test_invalid_sharding(self):
        ...
        with self.assertRaisesRegex(
            ...
            spec = ChunkShardingSpec(dim="H", placements=[f"rank:1/{self.device_type}:1"])
            sharded_tensor.empty(spec, 10, 20)

        for dim in [2, 3, 4, -3, -4, -5]:
            spec = ChunkShardingSpec(dim=dim, placements=[f"rank:1/{self.device_type}:1"])
            with self.assertRaisesRegex(ValueError, "Invalid sharding dim"):
                sharded_tensor.empty(spec, 10, 20)

        spec = ChunkShardingSpec(dim=0, placements=[f"rank:5/{self.device_type}:1"])
        with self.assertRaisesRegex(
            ValueError, "Global rank 5 does not exist in input process group"
        ):
            sharded_tensor.empty(spec, 10, 20)

        spec = ChunkShardingSpec(dim=0, placements=[f"rank:0/{self.device_type}:1"])
        ...
        spec = ChunkShardingSpec(dim=0, placements=[f"worker0/{self.device_type}:1"])
        with self.assertRaisesRegex(
        ...
        self.init_rpc()
        spec = ChunkShardingSpec(dim=0, placements=[f"workerfoo/{self.device_type}:1"])
    ...
    def test_invalid_pg_rpc_ranks(self):
        ...
        spec = ChunkShardingSpec(dim=0, placements=[f"rank:1/{self.device_type}:1"])
        with self.assertRaisesRegex(
    ...
    def test_insufficient_sharding_dims(self):
        ...
        for shard_rank, shard_metadata in enumerate(shards_metadata):
            ...
            self.assertEqual(
                f"rank:{shard_rank}/{self.device_type}:{shard_rank}", str(shard_metadata.placement)
            )
            if shard_rank <= 1:
                self.assertEqual([1, 20], shard_metadata.shard_sizes)
            else:
    ...
    def test_state_dict_new_group(self):
        spec = ChunkShardingSpec(
            dim=0,
            placements=[
                f"rank:2/{self.device_type}:0",
                f"rank:3/{self.device_type}:1",
                f"rank:2/{self.device_type}:2",
                f"rank:3/{self.device_type}:3",
            ],
        )
    ...
    def test_load_state_dict_errors(self):
        ...
        # Use dynamic backend instead of hardcoded "nccl" to support
        # out-of-tree backends (e.g., hccl for NPU).
...
class TestShardedTensorEnumerable(ShardedTensorTestBase):
    ...
    def test_grid_sharding(self):
        ...
        self.assertEqual(torch.device(self.device_type, self.rank), local_shard.tensor.device)
        ...
        self.assertEqual(
            f"rank:{self.rank}/{self.device_type}:{self.rank}", str(local_shard.metadata.placement)
        )
        ...
        for rank, shard_metadata in enumerate(shards_metadata):
            ...
            self.assertEqual(f"rank:{rank}/{self.device_type}:{rank}", str(shard_metadata.placement))
    ...
    def test_gather_even(self) -> None:
        ...
        dst = 0
        if self.rank == dst:
            full_tensor = torch.zeros(h, w, device=torch.device(self.device_type, dst))
        st.gather(dst, full_tensor)
    ...
    def test_sharded_tensor_to_cuda(self):
        ...
        new_st_cuda = st_cuda.to(self.device_type)
        ...
        new_st_gpu = st_cpu.to(self.device_type)
        # check the spec is still ChunkShardingSpec
        ...
        for i, remote_device_after in enumerate(spec_after_move.placements):
            ...
            self.assertEqual(str(remote_device_after.device().type), self.device_type)

        # ensure metadata also get changed to GPU
        metas = new_st_gpu.metadata().shards_metadata
        for meta in metas:
            self.assertEqual(str(meta.placement.device().type), self.device_type)
    ...
    def test_sharded_tensor_to_test(self):
        ...
        st_self = st.to(dtype=st.dtype, device=self.device_type)
        ...
        st_cuda = st_cpu.to(device=torch.device(self.device_type))
        self.assertEqual(st_cuda.local_tensor().device.type, self.device_type)
        # non-kwarg device to
        st_cuda = st_cpu.to(torch.device(self.device_type))
        self.assertEqual(st_cuda.local_tensor().device.type, self.device_type)
        ...
        self.assertEqual(st_cpu.local_tensor().device.type, "cpu")
        st_cuda = st_cpu.to(self.device_type)
        self.assertEqual(st_cuda.local_tensor().device.type, self.device_type)
        ...
        # test tensor to
        cuda_tensor = torch.randn(3, 4, dtype=torch.float16, device=self.device_type)
        st_cuda = st.to(cuda_tensor)
        self.assertFalse(st_cuda is st)
        self.assertEqual(st_cuda.dtype, torch.float16)

        cuda_tensor = torch.randn(3, 4, dtype=torch.float16, device=f"{self.device_type}:2")
        st_cuda = st.to(cuda_tensor)
        self.assertEqual(st_cuda.dtype, torch.float16)
        ...
        st_cuda_32 = st_cpu_16.to(self.device_type, torch.float32)
        self.assertEqual(st_cuda_32.dtype, torch.float32)
        self.assertEqual(st_cuda_32.local_tensor().device.type, self.device_type)
    ...
    def test_sharded_tensor_device(self):
        ...
        # local shards(no movements)
        st = sharded_tensor.zeros(spec, h, w)
        current_device = torch.device(self.device_type, self.rank)
    ...
    def test_partial_world_size(self):
        ...
        if self.rank <= 1:
            ...
            self.assertEqual((5, 5), local_shard.metadata.shard_sizes)
            self.assertEqual(
                f"rank:{self.rank}/{self.device_type}:{self.rank}",
    ...
    def test_new_group(self):
        ...
        for rank, shard_metadata in enumerate(shards_metadata):
            ...
            self.assertEqual((5, 5), shard_metadata.shard_sizes)
            self.assertEqual(
                f"rank:{rank * 2 + 1}/{self.device_type}:{rank * 2 + 1}",
                str(shard_metadata.placement),
            )
    ...
    def test_with_rpc_names(self):
        spec = EnumerableShardingSpec(
            [
                ShardMetadata(
                    shard_offsets=[0, 0],
                    shard_sizes=[5, 5],
                    placement=f"worker0/{self.device_type}:0",
                ),
                ShardMetadata(
                    shard_offsets=[0, 5],
                    shard_sizes=[5, 5],
                    placement=f"worker1/{self.device_type}:1",
                ),
                ShardMetadata(
                    shard_offsets=[5, 0],
                    shard_sizes=[5, 5],
                    placement=f"worker2/{self.device_type}:2",
                ),
                ShardMetadata(
                    shard_offsets=[5, 5],
                    shard_sizes=[5, 5],
                    placement=f"worker3/{self.device_type}:3",
                ),
            ]
        )
        ...
        self.assertEqual((5, 5), local_shard.metadata.shard_sizes)
        self.assertEqual(
            f"worker{self.rank}/{self.device_type}:{self.rank}", str(local_shard.metadata.placement)
        )
        ...
        for rank, shard_metadata in enumerate(shards_metadata):
            ...
            self.assertEqual((5, 5), shard_metadata.shard_sizes)
            self.assertEqual(f"worker{rank}/{self.device_type}:{rank}", str(shard_metadata.placement))
...
class TestShardedTensorFromLocalTensor(ShardedTensorTestBase):
    def _generate_st_from_chunk_local_tensor(self, st_size, sharding_spec):
        ...
        for shard_metadata in tensor_meta.shards_metadata:
            ...
            if rank == self.rank:
                local_tensor = torch.rand(shard_metadata.shard_sizes).to(device)
    ...
    def test_init_from_local_tensor_errors(self):
        ...
        st_size = [24, 12]
        local_tensor = torch.rand(*st_size).to(self.device_type)
...
class TestShardedTensorFromLocalShards(ShardedTensorTestBase):
    ...
    def test_local_shards(self):
        ...
        local_shard_metadata = ShardMetadata(
            ...
            shard_sizes=[5, 5],
            placement=f"rank:{self.rank}/{self.device_type}:{self.rank}",
        )

        local_tensor = torch.randn(5, 5, device=f"{self.device_type}:{self.rank}")
    ...
    def test_init_from_local_shards(self):
        ...
        local_shards = [
            sharded_tensor.Shard(
                torch.randn(5, 5, device=f"{self.device_type}:{self.rank}"), local_shard_metadata
            )
        ]
    ...
    def test_recalc_for_metadata(self):
        ...
        for shard_size in shard_sizes:
            ...
            local_shards = [
                sharded_tensor.Shard(
                    torch.randn(shard_size, shard_size, device=f"{self.device_type}:{self.rank}"),
    ...
    def test_init_from_local_shards_with_different_glb_size(self):
        ...
        wrong_offset_local_shards = [
            sharded_tensor.Shard(
                torch.randn(5, 5, device=f"{self.device_type}:{self.rank}"),
    ...
    def test_st_base_init_from_local_shards_and_global_metadata(self):
        ...
        for rank in range(world_size):
            ...
            shards.append(
                sharded_tensor.Shard(
                    torch.randn(5, 5, device=f"{self.device_type}:{rank}"), local_shard_metadata
                )
            )
        ...
        self.assertEqual(torch.device(self.device_type, 0), local_shard.tensor.device)
        self.assertEqual((5, 5), local_shard.tensor.size())
        ...
        self.assertEqual((5, 5), local_shard.metadata.shard_sizes)
        self.assertEqual(f"rank:0/{self.device_type}:0", str(local_shard.metadata.placement))
            ...
            if r == self.rank:
                ...
                shards_metadata.append(
                    ShardMetadata(
                        shard_offsets=[0, 0],
                        shard_sizes=[0, 0],
                        placement=f"rank:{r}/{self.device_type}:{r}",
                    )
                )

        local_shards = [
            sharded_tensor.Shard(
                torch.randn(0, 0, device=f"{self.device_type}:{self.rank}"), local_shard_metadata
            )
        ]
    ...
    def test_init_from_local_shards_new_group(self):
        ...
        if self.rank != 0:
            ...
            for rank, shard_metadata in enumerate(shards_metadata):
                ...
                self.assertEqual((5, 5), shard_metadata.shard_sizes)
                self.assertEqual(
                    f"rank:{rank + 1}/{self.device_type}:{rank + 1}", str(shard_metadata.placement)
                )
    ...
    def test_init_from_local_shards_invalid_local_shards(self):
        ...
        sparse_tensor = torch.sparse_coo_tensor(
            indices, values, (5, 5), device=f"{self.device_type}:{self.rank}"
        )
        ...
        wrong_memory_format_shards = [
            sharded_tensor.Shard(
                torch.randn(5, 5, device=f"{self.device_type}:{self.rank}").t(), local_shard_metadata
            )
        ]
        with self.assertRaisesRegex(
            ValueError,
        ...
        with self.assertRaisesRegex(ValueError, "Shard tensor size does not match"):
            sharded_tensor.Shard(
                torch.randn(2, 3, device=f"{self.device_type}:{self.rank}"), local_shard_metadata
            )
    ...
    def test_init_from_local_shards_invalid_property_cross_ranks(self):
        ...
        wrong_dtype_shards = [
            sharded_tensor.Shard(
                torch.ones(5, 5, device=f"{self.device_type}:{self.rank}"), local_shard_metadata
            )
        ]
        with self.assertRaisesRegex(
            ValueError,
        ...
        wrong_dtype_shards = [
            sharded_tensor.Shard(
                torch.ones(5, 5, device=f"{self.device_type}:{self.rank}", dtype=tensor_dtype),
                local_shard_metadata,
            )
        ]
        with self.assertRaisesRegex(
        ...
        wrong_requires_grad_shards = [
            sharded_tensor.Shard(
                torch.randn(
                    5, 5, device=f"{self.device_type}:{self.rank}", requires_grad=tensor_requires_grad
                ),
                local_shard_metadata,
            )
        ]
    ...
    def test_init_from_local_shards_invalid_shards_overlap(self):
        ...
        local_shards = [
            sharded_tensor.Shard(
                torch.randn(local_shard_size, device=f"{self.device_type}:{self.rank}"),
                local_shard_metadata,
            )
        ]
    ...
    def test_init_from_local_shards_and_global_metadata_invalid_shards(self):
        ...
        wrong_dtype_shards = [
            sharded_tensor.Shard(
                torch.ones(5, 5, device=f"{self.device_type}:{self.rank}", dtype=torch.int),
                local_shard_metadata,
            )
        ]
        with self.assertRaisesRegex(
        ...
        wrong_requires_grad_shards = [
            sharded_tensor.Shard(
                torch.randn(5, 5, device=f"{self.device_type}:{self.rank}", requires_grad=True),
                local_shard_metadata,
            )
        ]
        with self.assertRaisesRegex(
...
class TestShardedTensorCustomOps(ShardedTensorTestBase):
    ...
    def test_custom_op_override(self):
        t = torch.rand(10, 10).to(self.device_type)
        ...
        m = torch.nn.Linear(32, 16).to(self.device_type)
        shard_parameter(m, "weight", spec)

        result = m(torch.rand(15, 32).to(self.device_type))
