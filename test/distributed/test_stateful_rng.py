...
class TestCheckpointableTensorRNG(TestCase):
    ...
    def test_shard_metadata_lowers_to_flat_indices(self):
        ...
        noncontiguous = torch.empty(2, 3).t()
        self._set_shard_metadata(
            noncontiguous,
            (3, 2),
            ((0, 0),),
        ...
        layout = _derive_checkpointable_rng_layout(
            cast(CheckpointableTensor, noncontiguous)
        )
        self.assertFalse(layout.is_direct)

    def test_invalid_shard_metadata(self):
        cases = (
            (
                "global shards",
                ((0,), (1,)),
        ...
        for error, global_offsets, local_offsets, local_sizes in cases:
    ...
    def test_symbolic_total_numel_meta_and_fake(self):
        ...
        for device in ("meta", "cuda"):
            with self.subTest(device=device):
                ...
                block_count = shape_env.create_unbacked_symint()
                context = (
                ...
                with context:
                    result = torch.empty(block_count, device=device)
                ...
                self.assertIsNone(block_count.node.maybe_as_int())
