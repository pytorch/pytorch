...
class DeviceMeshTestGlooBackend(DTensorTestBase):
    ...
    def test_device_mesh_reuse_default_group(self, device):
        ...
        if self.device_type == "cpu":
            ...
            if curr_acc is not None and curr_acc.type == "cuda":
                self.assertEqual(
                    get_world_size(mesh_group), get_world_size(default_group)
                )
            else:
    ...
    @with_comms
    def test_device_mesh_reuse_default_group_for_custom_order_by_default(self, device):
        ...
        if (
            ...
            and torch.get_device_module(self.device_type).is_available()
        ):
    ...
    @with_comms
    def test_device_mesh_skips_default_group_reuse_when_opted_in(self, device):
...
class DeviceMeshTest(DTensorTestBase):
    ...
    def test_2d_mesh_eager_init_subgroup(self, device):
        ...
        device_backend = default_pg._get_backend(torch.device(self.device_type))
        if not getattr(device_backend, "supports_splitting", False):
            self.skipTest(
                f"backend {self.backend} does not support split_group, "
                "eager-init subgroup creation is not applicable"
            )
        mesh_shape = (2, self.world_size // 2)
        mesh_2d = init_device_mesh(self.device_type, mesh_shape)
    ...
    def test_fake_pg_device_mesh_accelerator_on_cpu(self, device):
        ...
        backend = device_mesh.get_all_groups()[0]._get_backend(
...
class DeviceMeshTestOnCUDA(DTensorTestBase):
...
class DeviceMeshTestNDim(DTensorTestBase):
    ...
    def test_device_mesh_preserve_rank_order(self, device):
...
instantiate_device_type_tests(DeviceMeshTestOnCUDA, globals(), only_for="cuda")
instantiate_device_type_tests(DeviceMeshTestNDim, globals(), allow_xpu=True)
instantiate_device_type_tests(InitDeviceMeshTest, globals(), allow_xpu=True)
