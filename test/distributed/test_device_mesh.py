...
class DeviceMeshTestGlooBackend(DTensorTestBase):
    ...
    def test_device_mesh_reuse_default_group_for_custom_order_by_default(self):
        # Without opting in via _preserve_rank_order, a mesh dim whose size
        ...
        # for existing callers whose fallthrough path is split_group, which
        # (unlike new_group) preserves caller-provided order unconditionally.
    ...
    def test_device_mesh_skips_default_group_reuse_when_opted_in(self):
        # With _preserve_rank_order=True, a permuted full-world mesh dim must
        # get a dedicated group instead of silently reusing default_group.
        reversed_ranks = list(reversed(range(self.world_size)))
        mesh = DeviceMesh(
            self.device_type,
            torch.tensor(reversed_ranks),
            _preserve_rank_order=True,
        )
        mesh_group = mesh.get_group()
        default_group = _get_default_group()
...
class DeviceMeshTestNDim(DTensorTestBase):
    ...
    def test_device_mesh_preserve_rank_order(self):
        ...
        ordered_mesh = DeviceMesh(
            self.device_type, mesh_tensor, _preserve_rank_order=True
        )
        ...
        for ranks in dim_ranks:
            if self.rank in ranks:
                # With _preserve_rank_order=True, subgroup ranks follow the mesh tensor order.
