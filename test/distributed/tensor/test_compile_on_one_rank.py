...
class TestCompileOnOneRankDeviceAsParameter(TestCase):
    ...
    accelerator (indexed or unindexed) traces with its device= fed by a single in-graph
    current_device() node, instead of baking the concrete device. At runtime the device
    follows each rank's current accelerator device (not any input), so one compiled
    artifact runs on each rank without --virtual-local-rank.
...
class TestCompileOnOneRankCUDA(TestCase):
    ...
    @skip_if_lt_x_gpu(2)
    @compiler_config.patch(compile_on_one_rank=True)
    def test_inductor_code_identical_across_devices(self, device):
        ...
        devices = [primary_device] + [
            ...
            for candidate in type(self).get_all_devices()
            if torch.device(candidate) != primary_device
        ]
        if len(devices) < 2:
            self.skipTest(f"requires >= 2 {primary_device.type} devices")
