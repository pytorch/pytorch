...
class TestViewOps(DTensorTestBase):
    ...
    def test_dtensor_unflatten_2d(self):
        ...
        # Following the original test pattern where shard_dim1 = shard_dim0 + 1
        # (adjacent sharded dimensions).
        # Requires at least 2 dims in the flatten range for two shard dims,
        # AND at least 1 dim outside the flatten range (view ops fail on 1D input).
        for tensor_ndim in [3, 4, 5]:
            for flatten_start in range(tensor_ndim - 2):  # Room for 2 dims + 1 outside
                for flatten_end in range(
                    flatten_start + 2, tensor_ndim
                ):  # Keep 1 dim outside
                    # shard_dim0 must allow shard_dim1 = shard_dim0 + 1 within flatten range
                    for shard_dim0 in range(flatten_start + 1, flatten_end - 1):
                        shard_dim1 = shard_dim0 + 1  # Adjacent shard dims
                        self._test_dtensor_unflatten_2d_ss(
                            tensor_ndim,
                            flatten_start,
                            flatten_end,
                            shard_dim0,
        ...
        # Test (Replicate, Shard/_StridedShard) pattern
        # Note: shard_dim > flatten_start is required because plain Shard(0) on a
        # 2D mesh doesn't propagate correctly through unflatten. We need _StridedShard
        # with an explicit split_factor to indicate which output dim should be sharded.
        for tensor_ndim in [2, 3, 4]:
            for flatten_start in range(tensor_ndim):
                for flatten_end in range(flatten_start + 2, tensor_ndim + 1):
                    # Only test shard_dim > flatten_start to use _StridedShard
                    for shard_dim in range(flatten_start + 1, flatten_end):
                        for (
                            tensor_dims_unflatten,
                            local_tensor_dims_unflatten,
                            tensor_dims_flatten,
                        ) in self.generate_tensor_dims_2d(
                            tensor_ndim,
                            flatten_start,
                            flatten_end,
                            shard_dim,
                            ...
                            ctx = contextlib.nullcontext()
                            is_uneven = (
                                tensor_dims_unflatten[shard_dim] % mesh.size(1) != 0
                            )
                            is_last_dim = shard_dim == flatten_end - 1
                            if is_uneven and not is_last_dim:
                                ctx = self.assertRaisesRegex(
                            ...
                            with ctx:
                                self._test_dtensor_unflatten_2d_rs(
                                    tensor_dims_unflatten,
                                    local_tensor_dims_unflatten,
                                    tensor_dims_flatten,
                                    flatten_start,
    ...
    def _test_dtensor_unflatten_2d_rs(
        self,
        tensor_dims_unflatten,
        local_tensor_dims_unflatten,
        tensor_dims_flatten,
        ...
        # shard_dim > flatten_start, so split_factor > 1
        split_factor = math.prod(local_tensor_dims_unflatten[flatten_start:shard_dim])
    ...
    def _test_dtensor_unflatten_2d_ss(
        self,
        tensor_ndim,
        flatten_start,
        flatten_end,
        ...
        This follows the original test pattern where:
        - shard_dim0's size = 2 * mesh.size(0) (sharded by mesh dim 0)
        - shard_dim1's size = mesh.size(1) (sharded by mesh dim 1)
        """
        # Build tensor dimensions following the original test pattern
        # Use sizes that make split_factor computation work correctly
        tensor_dims_unflatten = [12] * tensor_ndim  # Default size for all dims
        tensor_dims_unflatten[shard_dim0] = 2 * mesh.size(0)  # = 6
        tensor_dims_unflatten[shard_dim1] = mesh.size(
            1
        )  # = 2 (matches mesh dim 1 size)
        # For dims outside flatten range, use a small value
        for i in range(flatten_end, tensor_ndim):
            tensor_dims_unflatten[i] = 3
        ...
        # For mesh dim 0: use global dims (no prior shardings)
        split_factor0 = math.prod(tensor_dims_unflatten[flatten_start:shard_dim0])

        # For mesh dim 1: use dims with only mesh dim 0's sharding applied
        dims_for_sf1 = list(tensor_dims_unflatten[flatten_start:shard_dim1])
        if shard_dim0 >= flatten_start and shard_dim0 < shard_dim1:
            # shard_dim0 is in the range, use local size for that dim
            idx = shard_dim0 - flatten_start
            dims_for_sf1[idx] = dims_for_sf1[idx] // mesh.size(0)
        split_factor1 = math.prod(dims_for_sf1)

        # Both should be > 1 since shard_dim > flatten_start
        assert split_factor0 > 1, f"Expected split_factor0 > 1, got {split_factor0}"
        assert split_factor1 > 1, f"Expected split_factor1 > 1, got {split_factor1}"
