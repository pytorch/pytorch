...
class TestViewOps(DTensorTestBase):
    ...
    def test_view_groups_unbacked_sharding_propagation(self):
        ...
        # Split(InputDim(0), (2, 3), 0) -> out dim 0 maps to in dim 0
        # 2 % 2 == 0, so shardable on mesh dim 0; 2 % 3 != 0, not on mesh dim 1
        self.assertEqual(out_plc, [Shard(0), Replicate()])

        # Shard on concrete dim that gets flattened (leftmost): should propagate
        u3 = fresh_sym()
        from_shape = (u3, 3, 4)
        to_shape = (u3, 12)
        rule = view_groups(from_shape, to_shape)
        ...
        # dim 1 is leftmost in the flatten, should propagate to out dim 1
        self.assertEqual(out_plc, [Replicate(), Shard(1)])
