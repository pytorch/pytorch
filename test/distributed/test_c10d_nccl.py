...
class CommTest(test_c10d_common.AbstractCommTest, MultiProcessTestCase):
    ...
    @skip_if_lt_x_gpu(2)
    @skip_if_rocm_ver_atleast_multiprocess([7, 14])
    def test_profiler_nccl_batch_isend_irecv_annotations(self):
        store = c10d.FileStore(self.file_name, self.world_size)
        c10d.init_process_group(
            backend=NCCL_BACKEND,
            store=store,
        ...
        def run_batch():
            send = torch.full(
                (1024,), self.rank, device=device, dtype=torch.float32
            )
            recv = torch.empty_like(send)
            ops = [
                dist.P2POp(
                    dist.isend, send, (self.rank + 1) % self.world_size
                ),
                dist.P2POp(
                    dist.irecv, recv, (self.rank - 1) % self.world_size
                ),
            ]
        ...
        batch_annotations = [
            ...
            and ev.get("args", {}).get("Collective name")
            == "batch_isend_irecv"
        ]
