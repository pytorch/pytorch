...
# For ROCm systems, RCCL provides NCCL-compatible API through the same interface
# Only skip if neither NCCL/RCCL are available  
if not c10d.is_available():
    print("c10d not available, skipping tests", file=sys.stderr)
    sys.exit(0)

if not c10d.is_nccl_available():
    print("c10d NCCL/RCCL backend not available, skipping tests", file=sys.stderr)
    sys.exit(0)
...
def _get_device_type():
    """Returns the appropriate device type string for the current environment."""
    return "hip" if TEST_WITH_ROCM else "cuda"


def _get_torch_device(rank: int):
    """Returns a torch.device for the given rank using the appropriate device type."""
    device_type = _get_device_type()
    device_count = torch.cuda.device_count()  # Works for both CUDA and ROCm
    return torch.device(device_type, rank % device_count)
...
class ProcessGroupNCCLGroupTest(MultiProcessTestCase):
    ...
    def test_close_pg(self, eager_init: bool):
        ...
        store = c10d.FileStore(self.file_name, self.world_size)
        device = _get_torch_device(self.rank)
        c10d.init_process_group(
            "nccl",
            world_size=self.world_size,
            rank=self.rank,
