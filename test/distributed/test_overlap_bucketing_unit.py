...
class TestOverlapPreservingBucketing(InductorTestCase):
    ...
    def test_can_bucket_with_convert_dtype(self):
        """
        Test that all_gathers with convert_element_type operations CAN bucket together
        and that ALL convert_element_type ops are completely removed.

        Graph structure:
        ag1_start (with convert) -> ag2_start (with convert) -> mm1 (hides ag1) -> mm2 (hides ag2) -> ag1_wait -> ag2_wait

        The convert_element_type ops are NOT hiding nodes - matmuls provide the hiding.
        After bucketing:
        - ALL convert_element_type ops are removed (original and no new ones created)
        - _pre_bucket_all_gather handles dtype conversion as part of the copy operation
        - Destination slices are viewed as output dtypes, copy handles conversion
        """
        ...
        def func(a, b):
            ...
            # Convert dtype before all_gather using prims.convert_element_type
            # a is float32 -> bfloat16
            a_converted = torch._prims.convert_element_type(a, torch.bfloat16)
            # b is float16 -> bfloat16
            b_converted = torch._prims.convert_element_type(b, torch.bfloat16)

            # Start both collectives with converted inputs
            ag1 = torch.ops._c10d_functional.all_gather_into_tensor(
                a_converted, group_size, group_name
            )
            ag2 = torch.ops._c10d_functional.all_gather_into_tensor(
                b_converted, group_size, group_name
            )

            # Independent compute that hides both collectives (using original dtypes)
            mm1 = torch.mm(a, a)
            mm2 = torch.mm(b.float(), b.float())
        ...
        with FakeTensorMode():
            a = torch.ones(4, 4, device=self.device, dtype=torch.float32)
            b = torch.ones(4, 4, device=self.device, dtype=torch.float16)
        ...
        # Verify that convert_element_type ops exist before bucketing
        convert_dtype_ops_before = traced.graph.find_nodes(
            op="call_function",
            target=torch.ops.prims.convert_element_type.default,
        )
        self.assertEqual(len(convert_dtype_ops_before), 2)

        # Manually annotate hiding relationships - matmuls hide the collectives
        hiding_annotations = {
            ag1: mm1,  # mm1 hides ag1
            ag2: mm2,  # mm2 hides ag2
        }
        ...
        # Verify: should have 1 bucketed collective
        FileCheck().check_count("all_gather_into_tensor_out", 1, exactly=False).run(
            graph_str
        )

        # Verify: should have _pre_bucket_all_gather which handles the bucketing
        FileCheck().check_count("_pre_bucket_all_gather", 1, exactly=False).run(
            graph_str
        )

        # Verify: ALL convert_element_type ops should be completely removed
        # _pre_bucket_all_gather handles dtype conversion via copy, not explicit convert ops
        convert_dtype_ops_after = traced.graph.find_nodes(
            op="call_function",
            target=torch.ops.prims.convert_element_type.default,
        )
        self.assertEqual(
            len(convert_dtype_ops_after),
            0,
            f"Expected all convert_element_type ops to be removed after bucketing. "
            f"Found {len(convert_dtype_ops_after)} ops remaining",
        )
    ...
    def test_can_bucket_with_convert_dtype_as_hiding_nodes(self):
        ...
        def func(a, b):
            ...
            # First all_gather without conversion
            ag1 = torch.ops._c10d_functional.all_gather_into_tensor(
                a, group_size, group_name
            )

            # Convert that will hide ag1
            convert1 = torch._prims.convert_element_type(a, torch.bfloat16)

            # Wait for ag1
            ag1_out = torch.ops._c10d_functional.wait_tensor(ag1)

            # Second all_gather without conversion
            ag2 = torch.ops._c10d_functional.all_gather_into_tensor(
                b, group_size, group_name
            )

            # Convert that will hide ag2
            convert2 = torch._prims.convert_element_type(b, torch.bfloat16)

            # Wait for ag2
            ag2_out = torch.ops._c10d_functional.wait_tensor(ag2)

            return ag1_out.sum() + ag2_out.sum() + convert1.sum() + convert2.sum()
        ...
        convert_nodes = traced.graph.find_nodes(
            op="call_function",
            target=torch.ops.prims.convert_element_type.default,
        )
        convert1 = convert_nodes[0]
        convert2 = convert_nodes[1]

        # Manually annotate hiding relationships - convert ops hide the collectives
        hiding_annotations = {
            ag1: convert1,  # convert1 hides ag1
            ag2: convert2,  # convert2 hides ag2
        }
        ...
        # Verify: convert_element_type ops should still exist (they're not inputs to all_gather)
        convert_dtype_ops_after = traced.graph.find_nodes(
            op="call_function",
            target=torch.ops.prims.convert_element_type.default,
        )
        self.assertEqual(
            len(convert_dtype_ops_after),
            2,
            f"Expected convert_element_type ops to remain when they are hiding nodes. "
            f"Found {len(convert_dtype_ops_after)} ops",
        )
