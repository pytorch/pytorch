...
class TestXNNPACKOps(TestCase):
    @given(data_shape=hu.array_shapes(2, 4, 2, 64),
           weight_output_dim=st.integers(2, 64),
           use_bias=st.booleans())
    def test_linear(self, data_shape, weight_output_dim, use_bias):
...
class TestXNNPACKSerDes(TestCase):
    ...
    def test_combined_model(self,
        ...
        scripted_m_prepacked = torch.jit.script(MPrePacked(conv_weight, conv_bias,
            linear_weight, linear_bias, strides, paddings, dilations, groups))
