...
class TestTypePromotion(TestCase):
    ...
    def test_cat_different_dtypes(self, device):
        dtypes = [k for k in all_types_and_complex_and(torch.half, torch.bool)]
    ...
    def test_cat_out_different_dtypes(self, device):
        dtypes = [k for k in all_types_and_complex_and(torch.half)]
    ...
    def test_atan2_type_promotion(self, device, dtypes):
        ...
        def is_int(dtype):
            return dtype in [k for k in integral_types_and(torch.bool)]

        def is_float(dtype):
            return dtype in [k for k in floating_types_and(torch.half)]
