...
class TestTypePromotion(TestCase):
        ...
        for dt1, dt2 in itertools.product(dtypes1, dtypes2):
            for op, non_contiguous in itertools.product(ops, [True, False]):
                ...
                self.assertEqual(
                    result.dtype,
                    expected.dtype,
                    msg="{} with {}, {}".format(op.__name__, dt1, dt2),
                )
                self.assertEqual(
    ...
    def test_create_bool_tensors(self, device):
        ...
        self.assertEqualIgnoreType(
            torch.arange(False, True, 0.5, device=device), expected
        )
    ...
    @dtypes(
        *itertools.product(
            torch.testing.get_all_dtypes(), torch.testing.get_all_dtypes()
        )
    )
    ...
    def test_sparse_add(self, device):
        self._run_all_tests_for_sparse_op(
            "add", device, dtypes=torch.testing.get_all_math_dtypes(device)
        )

    @onlyOnCPUAndCUDA
    def test_sparse_mul(self, device):
        self._run_all_tests_for_sparse_op(
            "mul", device, dtypes=torch.testing.get_all_math_dtypes(device)
        )
    ...
    def test_sparse_sub(self, device):
        self._run_all_tests_for_sparse_op(
            "sub", device, dtypes=torch.testing.get_all_math_dtypes(device)
        )
    ...
    @dtypes(
        *list(
            itertools.product(
                torch_to_numpy_dtype_dict.keys(), torch_to_numpy_dtype_dict.keys()
            )
        )
    )
    ...
    def test_unary_op_out_casting(self, device, dtypes):
        ...
        for op in ops:
            if dtypes[0] is not dtypes[1]:
                ...
                with self.assertRaises(RuntimeError):
                    op(t, out=out)
            elif op in float_only_ops and (
                not dtypes[0].is_floating_point and not dtypes[0].is_complex
            ):
                with self.assertRaises(RuntimeError):
                    op(t, out=out)
            else:
    ...
    @dtypes(
        *itertools.product(
            torch.testing.get_all_dtypes(
                include_half=False, include_bfloat16=False, include_complex=False
            ),
            torch.testing.get_all_dtypes(
                include_half=False, include_bfloat16=False, include_complex=False
            ),
    ...
    def test_atan2_type_promotion(self, device, dtypes):
        ...
        def is_float(dtype):
            return dtype in torch.testing.get_all_fp_dtypes(
                include_half=True, include_bfloat16=False
            )
