...
from torch.testing._internal.common_dtype import (
    all_types_and_complex_and, all_types_and, get_all_math_dtypes, integral_types_and
)
...
class TestTypePromotion(TestCase):
    ...
    @dtypesIfCUDA(*itertools.product(all_types_and(torch.half, torch.bool), repeat=2))
    @dtypes(*itertools.product(all_types_and(torch.bool), repeat=2))
    def test_atan2_type_promotion(self, device, dtypes):
        ...
        def get_binary_float_result_type(x, y):
            dtype1 = x.dtype
            dtype2 = y.dtype
            if is_int(dtype1) and is_int(dtype2):
    ...
    def test_clamp_type_promotion(self, device, dtypes):
        ...
        for min_v, max_v in itertools.product(mins, maxs):
            ...
            def expected_type(inp, max, min):
                exp_type = torch.result_type(inp, max)
                inp_new = torch.empty_like(inp, dtype=exp_type)
                return torch.result_type(inp_new, min)
