...
def run_ort(ort_sess, input):
    ...
    ort_inputs = dict(
        (ort_sess.get_inputs()[i].name, input) for i, input in enumerate(input)
    )
...
class _TestONNXRuntime:
    ...
    def run_test(
        ...
        if self.is_script_test_enabled and not isinstance(
    ...
    def test_input_names_with_optional_args(self):
        ...
        # With optional arguments dictionary
        self.run_test(
            MixedModel(), (x, {"y": y, "z": None}), input_names=["input_x", "input_y"]
        )
        self.run_test(
            MixedModel(), (x, {"y": None, "z": z}), input_names=["input_x", "input_z"]
        )
    ...
    def test_input_as_output(self):
        ...
        y = torch.randn(3, 4)
        self.run_test(
            Model(), (x, y), input_names=["x", "y"], output_names=["x_out", "y_out"]
        )
    ...
    def test_script_custom_class_error(self):
        ...
        class MyModule(torch.nn.Module):
            ...
            def forward(
                self, box_regression: torch.Tensor, proposals: List[torch.Tensor]
            ):
