...
class TestQuantizeFx(QuantizationTestCase):
    ...
    def test_fold_weight_preserves_external_weight_users(self):
        with override_quantized_engine('fbgemm'):
            ...
            # The failure being fixed is independent of keep_original_weights,
            # so exercise both the default and keep-original-weights paths.
            for keep_original_weights in (False, True):
                with self.subTest(keep_original_weights=keep_original_weights):
                    ...
                    prepared = prepare_fx(
                        model, qconfig_mapping, example_inputs=inputs
                    )
                    converted = convert_fx(
                        prepared, keep_original_weights=keep_original_weights
                    )
                    ...
                    # The external weight.dtype user keeps the weight attribute
                    # alive regardless of keep_original_weights.
