...
class TestQuantizedLinear(TestCase):
    ...
    def test_qlinear(self, batch_size, input_channels, output_channels, use_bias,
        ...
        test_cases = itertools.product(dtypes, post_ops)
        for dtype, post_op in test_cases:
            ...
            # Quantized Linear operator with prepacked weight
            Y_q = qlinear(X_q, W_prepack, Y_scale, Y_zp) if post_op != 'leaky_relu' \
                else qlinear(X_q, W_prepack, negative_slope, Y_scale, Y_zp)
            if not use_channelwise and post_op != 'leaky_relu':
