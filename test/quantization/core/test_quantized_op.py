...
class TestQuantizedLinear(TestCase):
    ...
    @given(batch_size=st.integers(1, 4),
           ...
           use_channelwise=st.booleans())
    @skipIfNoONEDNN
    def test_qlinear_leaky_relu(self, batch_size, input_channels, output_channels, use_bias,
                                use_multi_dim_input, use_channelwise):
        ...
        with override_quantized_engine('onednn'):
            ...
            if use_multi_dim_input:
                X_q = X_q.view(3, int(batch_size / 3), input_channels)
            # Quantized Linear operator with prepacked weight
            Y_q = qlinear(X_q, W_prepack, negative_slope, Y_scale, Y_zp)
            # Test both per-tensor and per-channel quantization
            # Reference quantized result from PyTorch Linear operator
            W_fp32 = W_q.dequantize().to(dtype=torch.float)
            X_fp32 = X_q.dequantize().to(dtype=torch.float)
