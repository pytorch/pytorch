...
class TestQuantizedTensor(TestCase):
    ...
    def test_per_tensor_qtensor_to_memory_format(self):
        ...
        qx_nhwc_using_contiguous = qx.contiguous(memory_format=torch.channels_last)
        self.assertNotEqual(
            qx_nhwc_using_to.stride(), qx_nhwc_using_contiguous.stride()
        )
    ...
    def test_per_channel_qtensor_to_memory_format(self):
        ...
        for axis in range(x.ndim):
            ...
            qx = torch.quantize_per_channel(
                x, scales=scales, zero_points=zero_points, dtype=dtype, axis=axis
            )
            qx_nhwc_using_to = qx.to(memory_format=torch.channels_last)
            self.assertEqual(qx_nhwc_using_to.stride(), x_nhwc.stride())
    ...
    def _test_qtensor_sub_byte(
        self, rows, cols, dtype, elements_per_byte, expected_packed_vals
    ):
        ...
        self.assertEqual(
            qr.storage().size(),
            rows * math.ceil(cols / elements_per_byte),
        ...
        for num, expected in zip(int_repr, expected_packed_vals):
            self.assertEqual(
                num,
                expected,
                f"with dtype={dtype}, elements_per_byte={elements_per_byte}, rows={rows}, cols={cols}",
            )

        # Test tensor creation
        q = torch._empty_affine_quantized(
            [num_elements], scale=scale, zero_point=zero_point, dtype=dtype
        )
        self.assertEqual(
            q.storage().size(),
            math.ceil(num_elements / elements_per_byte),
            f"with {dtype}, {elements_per_byte}",
        )
    ...
    def test_qtensor_channel_float_assignment(self):
        ...
        for axis in range(t1.ndim):
            ...
            for dtype in [torch.qint8, torch.quint8, torch.qint32]:
                qt1 = torch.quantize_per_channel(
                    t1,
                ...
                qt2 = torch.quantize_per_channel(
                    t2,
                    scales=torch.tensor(scales),
                    zero_points=torch.tensor(zero_points),
                    dtype=dtype,
        ...
        for device in get_supported_device_types():
            ...
            for memory_format in [torch.contiguous_format, torch.channels_last]:
                ...
                for dtype in [torch.qint8, torch.quint8, torch.qint32]:
                    ...
                    rqr = qr.dequantize()
                    self.assertTrue(
                        np.allclose(r.cpu().numpy(), rqr.cpu().numpy(), atol=2 / scale)
                    )
    ...
    def _test_per_channel_qtensor_creation(self, device):
        ...
        for zero_points in [zero_points_float, zero_points_int]:
            int_tensor = torch.randint(
                0, 100, size=(numel,), dtype=torch.uint8, device=device
            )
    ...
    def test_qtensor_creation(self):
        ...
        for device in get_supported_device_types():
            ...
            int_tensor = torch.randint(
                0, 100, size=(10,), device=device, dtype=torch.uint8
            )
    ...
    def _test_quantize_per_channel(self, r, scales, zero_points, axis, float_params):
                ...
                for j in range(res.size()[1]):
                    for k in range(res.size()[2]):
                        if float_params:
                            ...
                            res[i][j][k] = np.clip(
                                np.round(data[i][j][k] * inv_scale + zero_points[j]),
                                quant_min,
                                quant_max,
                            )
                        else:
                            res[i][j][k] = np.clip(
                                np.round(data[i][j][k] / scales[j]) + zero_points[j],
                                quant_min,
                                quant_max,
                            )
            res = res.view(*dims)
        ...
        for memory_format in [torch.contiguous_format, contig_format]:
            ...
            qr = torch.quantize_per_channel(
                r_contig, scales, zero_points, axis, torch.quint8
            )
            rqr = qr.dequantize()
    ...
    def test_quantize_per_channel_float_qparams(self):
        ...
        zero_points = torch.tensor([0.1, 0.2, 1.0], dtype=torch.float)
    ...
    def test_quantize_per_channel_sub_byte(self):
        ...
        def _quantize_per_channel_sub_byte_ref(
            data, scales, zero_points, axis, bit_width
        ):
            dims = data.size()
            data = data.view(-1, dims[axis], np.prod(dims[axis + 1 :]))
            ...
            for i in range(data.size()[0]):
                for j in range(data.size()[1]):
                    for k in range(data.size()[2]):
                        inv_scale = 1.0 / scales[j]
                        index = (
                            i * data.size()[1] * data.size()[2] + j * data.size()[2] + k
                        )
                        qvalue = np.clip(
                            np.round(data[i][j][k] * inv_scale + zero_points[j]),
                            quant_min,
        ...
        self.assertTrue(
            np.allclose(
                r.numpy(), dequant_tensor.numpy(), atol=1 / np.min(scales.numpy())
            )
        )
        ...
        zero_points = torch.tensor([0.1, 0.2], dtype=torch.float)
        qr = torch.quantize_per_channel(
            r, scales, zero_points, axis=1, dtype=torch.quint4x2
        )
    ...
    def test_qtensor_permute(self):
        ...
        for device in get_supported_device_types():
            ...
            for dtype in [torch.qint8, torch.quint8, torch.qint32]:
                ...
                self.assertNotEqual(
                    qlast.stride(), sorted(qlast.stride(), reverse=True)
                )
    ...
    def test_qtensor_per_channel_permute(self):
        for device in get_supported_device_types():
            ...
            zero_points = torch.round(torch.rand(10, device=device) * 2 - 1).to(
                torch.long
            )
            ...
            self.assertEqual(qr.int_repr(), qlast.int_repr())
            self.assertEqual(
                scales.to(dtype=torch.float64), qlast.q_per_channel_scales()
            )
        ...
        for dtype in [torch.qint8, torch.quint8, torch.qint32]:
            ...
            with tempfile.NamedTemporaryFile() as f:
                ...
                for weights_only in [True, False]:
                    ...
                    self.assertEqual(
                        qr2.storage().data_ptr(), qrv2.storage().data_ptr()
                    )
    ...
    def test_qtensor_copy(self):
        ...
        for dtype in [torch.qint8, torch.quint8, torch.qint32]:
            for device in get_supported_device_types():
                ...
                q = torch._empty_affine_quantized(
                    [numel], scale=scale, zero_point=zero_point, dtype=dtype
                )
            ...
            q = torch._empty_affine_quantized(
                [numel], scale=scale, zero_point=zero_point, dtype=dtype, device=device
            )
            q.copy_(r)
            qr = torch.quantize_per_tensor(
                r, scale=scale, zero_point=zero_point, dtype=dtype
            )
    ...
    def test_clone(self):
        ...
        options = itertools.product(
            get_supported_device_types(), [torch.qint8, torch.quint8, torch.qint32]
        )

        for device, dtype in options:
            per_tensor_quantized = torch._empty_affine_quantized(
                [numel], scale=scale, zero_point=zero_point, device=device, dtype=dtype
            )
    ...
    def test_qtensor_fill_per_tensor(self):
        ...
        for qtype, val2fill, device in itertools.product(qtypes, vals2fill, devices):
            ...
            q_filled = torch._empty_affine_quantized(
                [numel], scale=scale, zero_point=zero_point, device=device, dtype=qtype
            )
            q_filled.fill_(val2fill)
            # reference tensor for comparing q_filled
            q_ref = torch.quantize_per_tensor(ones * val2fill, scale, zero_point, qtype)
    ...
    def test_qtensor_fill_per_tensor_nhwc(self):
        ...
        devices = get_supported_device_types()
        for qtype, val2fill, memory_format, device in itertools.product(
            qtypes, vals2fill, memory_formats, devices
        ):
            q_filled = torch._empty_affine_quantized(
                dims,
                scale=scale,
    ...
    def test_qtensor_fill_per_channel(self):
        ...
        for qtype, val2fill, device in itertools.product(qtypes, vals2fill, devices):
            ...
            q_filled.fill_(val2fill)
            # reference tensor for comparing q_filled
            q_ref = torch.quantize_per_channel(
                ones * val2fill,
                scales=scales,
                zero_points=zero_points,
                axis=axis,
    ...
    def _test_qtensor_masked_fill(self, device):
        ...
        for qtype, fill_with in itertools.product(types, fills):
            ...
            int_repr = torch.quantize_per_tensor(
                ones * fill_with, scale, zero_point, qtype
            )
        ...
        qx = torch.quantize_per_tensor(
            x, scale=scale, zero_point=zero_point, dtype=qtype
        )
        for qtype, fill_with in itertools.product(types, fills):
    ...
    def _test_qtensor_index_put(self, device):
        ...
        values_list = [
            ...
            torch.rand(len(indices[0]), device=device) * 1000,
        ]
        scale = 0.5
        zero_point = 10
        types = [torch.qint8, torch.quint8, torch.qint32]
        for qtype, values in itertools.product(types, values_list):
            ...
            qx_ref = torch.quantize_per_tensor(
                x_ref, scale=scale, zero_point=zero_point, dtype=qtype
            )
        ...
        with DeterministicGuard(True):
            ...
            for qtype in types:
                for i in range(3):
                    ...
                    qx_ref = torch.quantize_per_tensor(
                        output_ref, scale=scale, zero_point=zero_point, dtype=qtype
                    )
    ...
    def _test_qtensor_index_select(self, device):
        for quant_type in [torch.quint8, torch.qint8]:
            ...
            x_selected_quantized = torch.quantize_per_tensor(
                x_selected, scale, zp, quant_type
            )
    ...
    def test_qtensor_resize(self):
        for device in get_supported_device_types():
            ...
            q1 = torch._make_per_tensor_quantized_tensor(
                q1_int, scale=scale, zero_point=zero_point
            )
            q2 = q1.resize(*sizes2)
            q3 = q2.resize(*sizes3)
            q4 = q3.resize(*sizes4)
    ...
    def test_qtensor_unsqueeze(self):
        for device in get_supported_device_types():
            ...
            qx = torch.quantize_per_channel(
                x, scales=scales, zero_points=zero_points, dtype=torch.quint8, axis=0
            )
            qy = qx.unsqueeze(0)
            ...
            qx = torch.quantize_per_channel(
                x, scales=scales, zero_points=zero_points, dtype=torch.quint8, axis=2
            )
            qz = qx.squeeze()
    ...
    @given(
        X=hu.tensor(
            shapes=hu.array_shapes(min_dims=2, max_dims=4, min_side=1, max_side=10),
            qparams=hu.qparams(),
        ),
    ...
    def test_choose_qparams(self, X, reduce_range):
        ...
        X_scale, X_zp = _calculate_dynamic_qparams(
            X, torch.quint8, reduce_range=reduce_range
        )
    ...
    def test_choose_qparams_optimized(self):
        for bit_width in [4, 2]:
            ...
            y = torch.choose_qparams_optimized(
                x, numel=64, n_bins=200, ratio=0.16, bit_width=bit_width
            )
    ...
    def test_decomposed_quantize_per_channel(self):
        ...
        quantized_decomposed_X = torch.ops.quantized_decomposed.quantize_per_channel(
