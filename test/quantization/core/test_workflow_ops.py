...
def _fake_quantize_learnable_per_channel_affine_grad_reference(
    ...
    per_channel_zero_point = (
        (per_channel_zero_point.detach() + 0.5).clamp(quant_min, quant_max)
    ).type(torch.int32)
    ...
    for i, X_i in enumerate(torch.unbind(X, dim=axis), 0):
        ...
        indicate_middle_scale_i = (
            torch.ones(indicate_small_scale_i.shape).to(device)
            - indicate_small_scale_i
            - indicate_big_scale_i
        )

        indicate_saturate_zp_i = (
            (Xq_i < quant_min).float() + (Xq_i > quant_max).float()
        ).to(device)
        indicate_unsaturate_zp_i = (
            torch.ones(indicate_saturate_zp_i.shape).to(device) - indicate_saturate_zp_i
        )
        ...
        grad_scale_i = (
            indicate_small_scale_i * grad_small_scale_i
            + indicate_middle_scale_i * grad_middle_scale_i
            + indicate_big_scale_i * grad_big_scale_i
        )
        grad_zp_i = (
            indicate_saturate_zp_i * grad_saturate_zp_i
...
class TestFakeQuantizeOps(TestCase):
    ...
    def test_backward_per_tensor(self, device, X):
        ...
        np.testing.assert_allclose(
            dX.cpu(), X.grad.cpu().detach().numpy(), rtol=tolerance, atol=tolerance
        )
    ...
    def _test_forward_per_tensor_cachemask_impl(self, device):
        ...
        Xs = (
            ...
            torch.randn(4, 16, device=device)[:, ::2],
        )
        tensor_qparam = (True, False)
        for float_type, torch_type, X, tensor_qparams in itertools.product(
            float_types, torch_types, Xs, tensor_qparam
        ):
    ...
    def _test_backward_per_tensor_cachemask_impl(self, device):
        ...
        for float_type, torch_type, tensor_qparam in itertools.product(
            float_types, torch_types, tensor_qparams
        ):
    ...
    def _test_learnable_forward_per_tensor(
        self, X, device, scale_base, zero_point_base
    ):
    ...
    def test_fq_module_per_tensor(self, device, X):
        ...
        Y = _fake_quantize_per_tensor_affine_reference(
            X, fq_module.scale, fq_module.zero_point, quant_min, quant_max
        )
        np.testing.assert_allclose(
            Y.cpu().detach().numpy(),
        ...
        Y_prime.backward(dout)
        dX = _fake_quantize_per_tensor_affine_grad_reference(
            dout, X, fq_module.scale, fq_module.zero_point, quant_min, quant_max
        )
        np.testing.assert_allclose(
            dX.cpu().numpy(),
        ...
        for FakeQuantizeClass in [FakeQuantize, _LearnableFakeQuantize]:
            ...
            for weights_only in [True, False]:
                ...
                for key in state_dict:
                    self.assertEqual(
                        state_dict[key], loaded_fq_module.state_dict()[key]
                    )
    ...
    def fake_quant_scriptable(self):
        ...
        for FakeQuantizeClass in [FakeQuantize, _LearnableFakeQuantize]:
            ...
            scripted_module(X)
            self.assertEqual(
                fq_module.calculate_qparams(), scripted_module.calculate_qparams()
            )
    ...
    def _test_forward_per_channel_cachemask_impl(self, device):
        ...
        for torch_type, float_type, zero_point_type in itertools.product(
            torch_types, float_types, zero_point_types
        ):
            X = torch.randn(1, 2, 4, 4, dtype=float_type).to(device)
            # pick the scale + zp so that some values get clipped
            axis = 1
            obs = torch.ao.quantization.PerChannelMinMaxObserver(axis, torch_type).to(
                device
            )
            obs(X * 0.75)
            scale, zero_point = obs.calculate_qparams()
    ...
    def test_fake_quant_per_channel_qparam_range(self, X):
        ...
        for device in ["cpu", "cuda"] if torch.cuda.is_available() else ["cpu"]:
            ...
            zero_point = torch.full(zero_point.shape, -1 - quant_min).to(
            ...
            for zero_point_dtype in [torch.float32, torch.float16]:
                ...
                np.testing.assert_allclose(
                    Y.cpu().numpy(), Y_ref.cpu().numpy(), rtol=tolerance, atol=tolerance
                )

    def _test_learnable_forward_per_channel(
        self, X_base, device, scale_base, zero_point_base, axis
    ):
        ...
        for n_bits in (4, 8):
            ...
            Y = _fake_quantize_per_channel_affine_reference(
                X_curr,
                scale_curr,
                zero_point_curr.round().clamp(quant_min, quant_max),
                axis,
                quant_min,
                quant_max,
            ).to(device)
    ...
    def _test_learnable_backward_per_channel(
        self, X_base, device, scale_base, zero_point_base, axis
    ):
                    ...
                    Y = (
                        torch.dequantize(
                            torch.quantize_per_tensor(
                                X.to("cpu").to(torch.float), scale, zero, torch_type
                            )
                        )
                        .to(device)
                        .to(float_type)
...
class TestFusedObsFakeQuant(TestCase):
    ...
    @settings(deadline=None)
    def test_fused_obs_fake_quant_moving_avg_per_channel(
        self, device, symmetric_quant
    ) -> None:
        """
        Tests the case where we call the fused_obs_fake_quant op multiple times
        and update the running_min and max of the activation tensors.
