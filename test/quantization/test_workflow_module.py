...
class TestFakeQuantize(TestCase):
    ...
    def test_learnable_py_module_backward_per_tensor(self, device, X):
        ...
        X = torch.normal(mean=0, std=1, size=(1, 5)).clamp(-1e3, 1e3).to(device)
        scale = torch.normal(mean=0, std=1, size=(1,)).clamp(1e-4, 100).to(device)
        zero_point = torch.normal(mean=0, std=128, size=(1,)).to(device)
