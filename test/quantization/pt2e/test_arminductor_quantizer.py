...
from torch.ao.quantization.quantizer.arm_inductor_quantizer import (
...
class TestQuantizePT2EArmInductor(ArmInductorQuantTestCase):
    ...
    def cat(self):
        """
        Test pattern of conv2d with unary post ops (such as relu, hardtanh, hardswish, relu6) with ArmInductorQuantizer.
        """
        unary_map = {
    ...
    def test_conv2d_binary_unary(self):
        """
        Test pattern of conv2d with binary + unary post ops (such as add + relu) with ArmInductorQuantizer.
    ...
    def test_conv2d_serials_binary_unary(self):
        """
        Test pattern of 2 following up conv2d add relu with ArmInductorQuantizer.
        """
        with override_quantized_engine("arm"), torch.no_grad():
    ...
    def _test_linear_binary_unary_helper(self, is_qat=False, is_dynamic=False):
        """
        Test pattern of linear with binary + unary post ops (such as add + relu) with ArmInductorQuantizer.
        Currently, only add as binary post op and relu as unary post op are supported.
        """
        linear_pos_list = [NodePosType.left, NodePosType.right, NodePosType.both]
        # TODO test for inplace add after refactoring of export_for_training
    ...
    def test_linear_binary_unary_serials(self):
        """
        Test pattern of 2 following up linear add relu with ArmInductorQuantizer.
        """
        is_qat_list = [False, True]
        is_dynamic_list = [False, True]
        ...
        with override_quantized_engine("arm"), torch.no_grad():
            for use_bias in [True, False]:
                ...
                quantizer.set_module_type_qconfig(
                    torch.nn.Linear, armiq.get_arm_inductor_linear_dynamic_fp16_config()
                )
                node_occurrence = {
    ...
    def test_qat_conv2d_unary(self):
        """
        Test QAT pattern of conv2d_bn with unary post ops (such as relu, sigmoid) with ArmInductorQuantizer.
    ...
    def test_qat_conv2d_binary_unary(self):
        """
        Test QAT pattern of conv2d_bn with binary + unary post ops (such as add + relu) with ArmInductorQuantizer.
        Currently, only add as binary post op and relu as unary post op are supported.
        """
        example_inputs = (torch.randn(2, 3, 6, 6),)
        quantizer = ArmInductorQuantizer().set_global(
    ...
    def test_filter_maxpool2d_recipe(self):
        """
        Test removing maxpool2d from default recipe of ArmInductorQuantizer.
        """
        with override_quantized_engine("arm"), torch.no_grad():
            m = TestHelperModules.Conv2dUnaryModule(torch.nn.ReLU(inplace=False)).eval()
            example_inputs = (torch.randn(2, 3, 16, 16),)
