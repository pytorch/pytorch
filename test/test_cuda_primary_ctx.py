...
def get_is_primary_context_created(device):
    ...
    global _caffe2_nvtrc
    if _caffe2_nvtrc is None:
        path = glob.glob('{}/lib/libcaffe2_nvtrc.*'.format(os.path.dirname(torch.__file__)))[0]
        _caffe2_nvtrc = ctypes.cdll.LoadLibrary(path)
    result = _caffe2_nvtrc.cuDevicePrimaryCtxGetState(ctypes.c_int(device), flags, active)
