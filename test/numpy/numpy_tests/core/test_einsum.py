...
class TestEinsum:
    def test_einsum_errors(self):
        for do_opt in [True, False]:
            ...
            assert_raises((NotImplementedError, ValueError), np.einsum, "", 0, order='W',
                          optimize=do_opt)
            ...
            assert_raises((RuntimeError, ValueError), np.einsum, "ij->jij", [[0, 0], [0, 0]],
                          optimize=do_opt)

            # dimensions much match when being collapsed
            assert_raises((RuntimeError, ValueError), np.einsum, "ii",
                          np.arange(6).reshape(2, 3), optimize=do_opt)
            assert_raises((RuntimeError, ValueError), np.einsum, "ii->i",
                          np.arange(6).reshape(2, 3), optimize=do_opt)

            # broadcasting to new dimensions must be enabled explicitly
            assert_raises((RuntimeError, ValueError), np.einsum, "i", np.arange(6).reshape(2, 3),
                          optimize=do_opt)
            assert_raises((RuntimeError, ValueError), np.einsum, "i->i", [[0, 1], [0, 1]],
            ...
            assert_raises((NotImplementedError, ValueError), np.einsum, "i->i", np.arange(6).reshape(-1, 1),
