...
class TestCustomOpAPI(TestCase):
    ...
    @requires_compile
    @parametrize("use_auto_functionalized_v2", (True, False))
    def test_custom_op_inplace_tag_set_data_compile(self, use_auto_functionalized_v2):
        name = "v2" if use_auto_functionalized_v2 else "old"

        @torch.library.custom_op(
            f"_torch_testing::inplace_tag_set_data_compile_{name}",
            mutates_args={"param"},
            tags=torch.Tag.inplace,
        )
        def set_data(param: Tensor, new_data: Tensor) -> Tensor:
            param.set_(new_data)
            return param
