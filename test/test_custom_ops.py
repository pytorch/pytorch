...
class TestCustomOpAPI(TestCase):
    ...
    def test_register_autograd_with_torch_func_grad(self):
        ...
        def setup_context(ctx, inputs, output):
            setup_context_modes.append(
                (torch.is_grad_enabled(), torch._C._is_fwd_grad_enabled())
            )
            (x,) = inputs
            ctx.save_for_backward(x)
        ...
        self.assertTrue(setup_context_modes)
        self.assertTrue(all(mode == (False, False) for mode in setup_context_modes))

    @skipIfTorchDynamo("Expected to fail due to no FakeTensor support; not a bug")
    def test_register_autograd_with_torch_func_grad_vjp(self):
        @torch.library.custom_op(
            "_torch_testing::scale_for_torch_func_grad_vjp", mutates_args=()
        )
        def scale(x: Tensor, w: Tensor) -> Tensor:
            return x * w
        ...
        def primal_loss(fn, xx, ww):
            primal, _ = torch.func.vjp(lambda z: fn(z, ww).sum(), xx)
            return primal

        grad = torch.func.grad(primal_loss, argnums=1)(scale, x.clone(), w)
        expected = torch.func.grad(primal_loss, argnums=1)(torch.mul, x.clone(), w)
        ...
        def parameter_loss(fn, ww):
            primal, _ = torch.func.vjp(lambda xx: fn(xx, ww).sum(), x)
            return (primal - target).pow(2)

        grad = torch.func.grad(parameter_loss, argnums=1)(scale, w.clone())
        expected = torch.func.grad(parameter_loss, argnums=1)(torch.mul, w.clone())
        self.assertEqual(grad, expected)
