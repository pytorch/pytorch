...
module_tests = [
    ...
    dict(
        ...
        input_size=(4, 10),
        desc='no_bias',
        reference_fn=lambda i, p, m: torch.mm(i, p[0].t())
    ),
    dict(
    ...
    dict(
        ...
        input_size=(3, 2, 5),
        reference_fn=lambda i, p, m: i.clamp(-1, 1),
    ),
    dict(
        module_name='Sigmoid',
        input_size=(2, 3, 4, 5)
    ...
    dict(
        ...
        constructor_args=(1,),
        input_size=(10, 20),
        reference_fn=lambda i, p, m: torch.exp(i).div(torch.exp(i).sum(1, True).expand(10, 20)),
    ),
    dict(
        module_name='Softmax2d',
        input_size=(1, 3, 10, 20),
        reference_fn=lambda i, p, m: torch.exp(i).div(torch.exp(i).sum(1, False)),
    ),
    dict(
        module_name='LogSoftmax',
        constructor_args=(1,),
        input_size=(10, 20),
        reference_fn=lambda i, p, m: torch.exp(i).div_(torch.exp(i).sum(1, True).expand(10, 20)).log_(),
    ),
    dict(
        module_name='LogSoftmax',
        constructor_args=(1,),
        input_size=(1, 3, 10, 20),
        reference_fn=lambda i, p, m: torch.exp(i).div_(torch.exp(i).sum(1, False)).log_(),
        desc='multiparam',
    ),
    dict(
        module_name='ELU',
        constructor_args=(2.,),
        input_size=(3, 2, 5),
        reference_fn=lambda x, p, m: torch.where(x >= 0, x, 2 * (x.exp() - 1)),
    ),
    ...
    dict(
        ...
        input_size=(2, 3, 4),
        reference_fn=lambda i, p, m: i.sigmoid().log(),
    ),
    dict(
        module_name='Softplus',
        input_size=(10, 20),
        reference_fn=lambda i, p, m: torch.log(1 + torch.exp(i)),
    ),
    dict(
        module_name='Softplus',
        constructor_args=(2,),
        input_size=(10, 20),
        reference_fn=lambda i, p, m: 1. / 2. * torch.log(1 + torch.exp(2 * i)),
        desc='beta',
    ),
    dict(
        module_name='Softplus',
        ...
        input_size=(10, 20),
        reference_fn=(lambda i, p, m: ((i * 2) > -100).type_as(i) * i +
    ...
    dict(
        module_name='PReLU',
        input_size=(2, 3, 4),
        reference_fn=lambda i, p, m: torch.clamp(i, min=0) + torch.clamp(i, max=0) * p[0][0],
        desc='1d',
    ),
    dict(
        module_name='PReLU',
    ...
    dict(
        ...
        input_size=(3, 2, 5),
        reference_fn=lambda i, p, m: i.div(1 + torch.abs(i)),
    ),
    dict(
        module_name='Softmin',
        constructor_args=(1,),
...
def bce_with_logistic_legacy_enum_test():
    ...
    return dict(
        ...
        input_fn=lambda: torch.rand(15, 10).clamp_(2.8e-2, 1 - 2.8e-2),
        reference_fn=lambda i, p, m: -(t * sigmoid(i).log() + (1 - t) * (1 - sigmoid(i)).log()),
        check_gradgrad=False,
        pickle=False,
    )
...
def l1loss_no_reduce_test():
    ...
    return dict(
        ...
        reference_fn=lambda i, p, m: (i - t.type_as(i)).abs(),
        pickle=False)
...
def mseloss_no_reduce_test():
    ...
    return dict(
        ...
        reference_fn=lambda i, p, m: (i - target).pow(2),
        pickle=False)
...
new_module_tests = [
    ...
    dict(
        ...
        constructor_args=(2.,),
        check_inplace=True,
        reference_fn=lambda x, p, m: torch.where(x >= 0, x, 2. * ((.5 * x).exp() - 1)),
    ),
    dict(
        module_name='CELU',
        input_size=(),
    ...
    dict(
        ...
        constructor_args=(0,),
        input_size=(),
        reference_fn=lambda i, p, m: torch.exp(i).div(torch.exp(i).sum(0, True)),
        desc='scalar',
    ),
    dict(
        module_name='LogSoftmax',
        constructor_args=(0,),
        input_size=(),
        reference_fn=lambda i, p, m: torch.exp(i).div_(torch.exp(i).sum(0, False)).log_(),
    ...
    dict(
        ...
        reference_fn=lambda i, p, m: padding1d_circular(i, (1, 2)),
    ...
    dict(
        ...
        input_fn=lambda: torch.arange(6, out=torch.DoubleTensor()).reshape([1, 2, 3]),
        reference_fn=lambda i, p, m: padding1d_circular(i, (3, 1)),
        skip_double=TEST_WITH_ROCM,
        pickle=False,
    ),
    dict(
        ...
        input_fn=lambda: torch.arange(6, out=torch.DoubleTensor()).reshape([1, 2, 3]),
        reference_fn=lambda i, p, m: padding1d_circular(i, (3, 3)),
        skip_double=TEST_WITH_ROCM,
        pickle=False,
    ),
    dict(
        ...
        reference_fn=lambda i, p, m: padding2d_circular(i, (1, 2, 2, 1)),
        skip_double=TEST_WITH_ROCM,
        pickle=False,
    ),
    dict(
        ...
        input_fn=lambda: torch.arange(6, out=torch.DoubleTensor()).reshape([1, 1, 2, 3]),
        reference_fn=lambda i, p, m: padding2d_circular(i, (2, 3, 2, 2)),
        skip_double=TEST_WITH_ROCM,
        pickle=False,
    ),
    dict(
        ...
        reference_fn=lambda i, p, m: padding2d_circular(i, (3, 3, 3, 1)),
        skip_double=TEST_WITH_ROCM,
        pickle=False,
    ),
    dict(
        ...
        reference_fn=lambda i, p, m: padding3d_circular(i, (1, 2, 2, 1, 1, 2)),
        skip_double=TEST_WITH_ROCM,
        pickle=False,
    ),
    dict(
        ...
        input_fn=lambda: torch.arange(12, out=torch.DoubleTensor()).reshape([1, 1, 2, 2, 3]),
        reference_fn=lambda i, p, m: padding3d_circular(i, (3, 2, 2, 1, 1, 2)),
        skip_double=TEST_WITH_ROCM,
        pickle=False,
    ),
    dict(
        ...
        input_fn=lambda: torch.arange(12, out=torch.DoubleTensor()).reshape([1, 1, 2, 2, 3]),
        reference_fn=lambda i, p, m: padding3d_circular(i, (3, 3, 2, 1, 2, 2)),
        skip_double=TEST_WITH_ROCM,
        pickle=False,
    ),
