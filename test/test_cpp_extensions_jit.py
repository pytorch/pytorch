...
class TestFindRocmHome(common.TestCase):
    ...
    def test_warns_when_unexpanded_devel_wheel_present(self):
        with tempfile.TemporaryDirectory() as tmp:
            core = Path(tmp) / "_rocm_sdk_core"
            unexpanded = Path(tmp) / "rocm_sdk_devel"
            core.mkdir()
            unexpanded.mkdir()
            (core / "__init__.py").write_text("")
            (unexpanded / "__init__.py").write_text("")
            with mock.patch.object(
                torch.utils.cpp_extension.logger, "warning"
            ) as warn:
                home = self._find_home(
                    {
                        "_rocm_sdk_core": str(core / "__init__.py"),
                        "rocm_sdk_devel": str(unexpanded / "__init__.py"),
                    }
                )
        self.assertEqual(home, str(core.resolve()))
        self.assertTrue(
            any("rocm-sdk init" in str(call) for call in warn.call_args_list),
            warn.call_args_list,
        )

    def test_no_unexpanded_warning_when_devel_package_exists(self):
        with tempfile.TemporaryDirectory() as tmp:
            devel = Path(tmp) / "_rocm_sdk_devel"
            unexpanded = Path(tmp) / "rocm_sdk_devel"
            devel.mkdir()
