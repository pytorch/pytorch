...
class TestReductions(TestCase):
    ...
    def test_mode(self, device):
        SIZE = 10
        x = (
            torch.arange(1.0, SIZE * SIZE + 1, device=device)
            .clone()
            .resize_(SIZE, SIZE)
        )
        x[:2] = 1
    ...
    def test_mode_large(self, device):
        ...
        def testset_for_shape(shape, i):
            ...
            self._test_mode_intervals(
                shape, [(0, i), (i + 1, d - i - 1), (d - i, d)], device
            )
    ...
    @dtypes(
        *(
            set(torch.testing.get_all_dtypes(include_bool=False, include_complex=False))
            - {torch.uint8}
        )
    )
    ...
    def test_histogram(self, device, dtype):
        ...
        for contig, bins_contig, bin_ct, weighted, density, shape in product(
            ...
            values = make_tensor(
                shape, device, dtype, low=-9, high=9, noncontiguous=not contig
            )
            weights = (
                make_tensor(
                    shape, device, dtype, low=0, high=9, noncontiguous=not contig
                )
                if weighted
                else None
            )
            ...
            if not bins_contig:
                ...
                bin_edges_noncontig = make_tensor(
                    bin_ct + 1, device, dtype, noncontiguous=not bins_contig
                )
            ...
            values = make_tensor(
                shape, device, dtype, low=elt, high=elt, noncontiguous=not contig
            )
            self._test_histogram_numpy(values, bin_ct, bin_range, weights, density)
            self._test_histogram_numpy(values, bin_edges, None, weights, density)
            ...
            weights = (
                make_tensor(
                    bin_ct + 1, device, dtype, low=0, high=9, noncontiguous=not contig
                )
                if weighted
                else None
            )
    ...
    def test_histogram_error_handling(self, device, dtype):
        with self.assertRaisesRegex(
            RuntimeError, '"histogram_cpu" not implemented for'
        ):
        ...
        with self.assertRaisesRegex(
            RuntimeError, "input tensor and bins tensor should have the same dtype"
        ):
        ...
        with self.assertRaisesRegex(
            RuntimeError, "bins tensor should have dimension 1"
        ):
        ...
        with self.assertRaisesRegex(
            RuntimeError, "input tensor and weight tensor should have the same shape"
        ):
    ...
    def test_tensor_reduce_ops_empty(self, device):
        ...
        for name, fn, return_value, np_function in test_functions:
            ...
            check_func = (
                ...
                if math.isnan(return_value) or math.isinf(return_value)
