...
from torch.testing._internal.common_device_type import (
    ...
    dtypesIfCPU, dtypesIfMPS, dtypesIfXPU, onlyNativeDeviceTypes, onlyAccelerator,
    largeTensorTest, ops, precisionOverride)
...
class TestReductions(TestCase):
    ...
    def test_dim_reduction_lastdim(self, dtype):
        x = torch.randn(3, 5, 40, device="cpu", dtype=dtype)
        x = x[:, :, 0:40:2]
        x2 = x.contiguous()
    ...
    def test_mean_int_with_optdtype(self):
        a = make_tensor((3, 4, 5), dtype=torch.int64, device="cpu")
    ...
    def test_mean_out_is_alias_of_return(self, dtype, device):
        ...
        target = torch.tensor([[[2.0, 2.0, 2.0, 2.0]]], dtype=dtype, device="cpu")
    ...
    @parametrize("dtype", [torch.bool, torch.double])
    def test_sum_all(self, dtype) -> None:
        def check_sum_all(tensor: torch.Tensor) -> None:
            pylist = tensor.reshape(-1).tolist()
            self.assertEqual(tensor.sum(), sum(pylist))

        if dtype != torch.bool:
            check_sum_all(torch.tensor([1, 2, 3, 4, 5], dtype=dtype, device="cpu"))
            check_sum_all(torch.randn(200000, dtype=dtype, device="cpu"))
            check_sum_all(torch.randn(2000, 2, dtype=dtype, device="cpu")[:, 0])
        else:
            check_sum_all(torch.tensor([True, False, True], dtype=torch.bool, device="cpu"))

    @parametrize("dtype", [torch.double])
    def test_sum_out(self, dtype: torch.dtype) -> None:
        x = torch.rand(100, 100, dtype=dtype, device="cpu")
        res1 = torch.sum(x, 1)
        res2 = torch.tensor((), dtype=dtype, device="cpu")
        torch.sum(x, 1, out=res2)
        self.assertEqual(res1, res2)
        x = torch.rand(100, 100, 100, dtype=dtype, device="cpu")
        res1 = x.sum(2).sum(1)
        res2 = torch.tensor((), dtype=dtype, device="cpu")
    ...
    def test_max_mixed_devices(self):
    ...
    def test_min_mixed_devices(self):
        a = torch.randn(10, device="cpu")
        if torch.accelerator.is_available():
            values = torch.randn(10).to(device_type)
            indices = torch.LongTensor().to(device_type)
    ...
    @parametrize("dtype", [*complex_types()])
    def test_nansum_complex(self, dtype):
        x = torch.randn((3, 3, 3), device="cpu", dtype=dtype)
    ...
    def test_nansum_int_out_dtype_matches_inductor(self, dtype):
    ...
    @parametrize("dtype", [*integral_types_and(torch.bool)])
    def test_nanmean_integral_types(self, dtype):
    ...
    def test_histc_lowp(self, dtype):
        actual = torch.histc(
            torch.tensor([1, 2, 1], dtype=dtype, device="cpu"), bins=4, min=0, max=3)
        self.assertEqual(
            torch.tensor([0, 2, 1, 0], dtype=dtype, device="cpu"),
            actual)
        self.assertEqual(actual.dtype, dtype)

    @parametrize("dtype", [torch.float, torch.double])
    def test_histc_value_corner_cases(self, dtype):
        min_val = torch.finfo(dtype).min
        actual = torch.histc(
            torch.tensor([min_val, min_val, min_val], dtype=dtype, device="cpu"),
            bins=4)
        self.assertEqual(3.0, actual.sum())
        max_val = torch.finfo(dtype).max
        actual = torch.histc(
            torch.tensor([max_val, max_val, max_val], dtype=dtype, device="cpu"),
            bins=4)
        self.assertEqual(3.0, actual.sum())
    ...
    def test_histogram(self, dtype):
        ...
        for contig, bins_contig, bin_ct, weighted, density, shape in \
            ...
            values = make_tensor(shape, dtype=dtype, device="cpu", low=-9, high=9, noncontiguous=not contig)
            weights = make_tensor(shape, dtype=dtype, device="cpu", low=0, high=9, noncontiguous=not contig) if weighted else None
            ...
            bin_edges = make_tensor(bin_ct + 1, dtype=dtype, device="cpu", low=-9, high=9).msort()
            if not bins_contig:
                # Necessary because msort always produces contiguous output
                bin_edges_noncontig = make_tensor(bin_ct + 1, dtype=dtype, device="cpu", noncontiguous=not bins_contig)
            ...
            values = make_tensor(shape, dtype=dtype, device="cpu", low=elt, high=elt, noncontiguous=not contig)
            self._test_histogram_numpy(values, bin_ct, bin_range, weights, density)
            self._test_histogram_numpy(values, bin_edges, None, weights, density)

            # Tests with input equal to bin_edges
            weights = (
                make_tensor(bin_ct + 1, dtype=dtype, device="cpu", low=0, high=9, noncontiguous=not contig)
                if weighted
                else None
            )
        ...
        for bin_ct, shape in product(range(1, 10), shapes):
            values = make_tensor(shape, dtype=dtype, device="cpu", low=-9, high=9)
    ...
    @parametrize("dtype", [torch.float32])
    def test_histogramdd(self, dtype):
        shapes = (
            (1, 5),
            (3, 5),
            (1, 5, 1),
        ...
        for contig, bins_contig, weighted, density, shape in \
            ...
            values = make_tensor(shape, dtype=dtype, device="cpu", low=-9, high=9, noncontiguous=not contig)
            weights = (
                make_tensor(shape[:-1], dtype=dtype, device="cpu", low=0, high=9, noncontiguous=not contig)
                if weighted
                else None
            )
            ...
            # Tests with caller-specified bin edges
            bin_edges = [make_tensor(ct + 1, dtype=dtype, device="cpu", low=-9, high=9).msort() for ct in bin_ct]
            if not bins_contig:
                # Necessary because msort always produces contiguous output
                bin_edges_noncontig = [
                    make_tensor(ct + 1, dtype=dtype, device="cpu", noncontiguous=not bins_contig)
                    for ct in bin_ct
                ]
                for dim in range(D):
    ...
    @parametrize("dtype", [torch.float32])
    def test_histogram_error_handling(self, dtype):
        ...
        with self.assertRaisesRegex(RuntimeError, 'input tensor and bins tensors should have the same dtype'):
            values = make_tensor((), dtype=dtype, device="cpu")
        ...
        with self.assertRaisesRegex(RuntimeError, 'input tensor and hist tensor should have the same dtype'):
            ...
            bin_edges = make_tensor((), dtype=dtype, device="cpu")
        ...
        with self.assertRaisesRegex(RuntimeError, 'input tensor and bin_edges tensor should have the same dtype'):
            values = make_tensor((), dtype=dtype, device="cpu")
            hist = make_tensor((), dtype=dtype, device="cpu")
...
as the input tensor excluding its innermost dimension'):
            values = make_tensor((2, 2), dtype=dtype, device="cpu")
            weight = make_tensor((1), dtype=dtype, device="cpu")
            torch.histogram(values, 1, weight=weight)
        ...
        with self.assertRaisesRegex(RuntimeError, r'range \[nan, nan\] is not finite'):
            values = torch.tensor([float("nan")], device="cpu", dtype=dtype)
