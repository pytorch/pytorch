...
class TestDistributions(TestCase):
    ...
    def test_beta_underflow(self):
        ...
        for dtype in [torch.float, torch.double]:
            ...
            frac_zeros = float((beta_samples < 0.1).sum(dtype=torch.int)) / num_samples
            frac_ones = float((beta_samples > 0.9).sum(dtype=torch.int)) / num_samples
    ...
    def test_beta_underflow_gpu(self):
        ...
        beta_samples = Beta(conc, conc).sample([num_samples])
        self.assertEqual((beta_samples == 0).sum(dtype=torch.int), 0)
        self.assertEqual((beta_samples == 1).sum(dtype=torch.int), 0)
        # assert support is concentrated around 0 and 1
        frac_zeros = float((beta_samples < 0.1).sum(dtype=torch.int)) / num_samples
        frac_ones = float((beta_samples > 0.9).sum(dtype=torch.int)) / num_samples
