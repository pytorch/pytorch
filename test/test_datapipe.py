...
class TestFunctionalIterDataPipe(TestCase):
    ...
    def test_fork_iterdatapipe(self):
        ...
        # Functional Test: three child DataPipes yield shallow copies with copy equals shallow
        dp1, dp2, dp3 = input_dp.map(_to_list).fork(num_instances=3, copy="shallow")
        for n1, n2, n3 in zip(dp1, dp2, dp3):
        ...
        # Functional Test: three child DataPipes yield deep copies with copy equals deep
        dp1, dp2, dp3 = input_dp.map(_to_list).map(_to_list).fork(num_instances=3, copy="deep")
        for n1, n2, n3 in zip(dp1, dp2, dp3):
            self.assertIsNot(n1[0], n2[0])
            self.assertIsNot(n1[0], n3[0])
            self.assertIsNot(n2[0], n3[0])
            self.assertEqual(n1, n2)
            self.assertEqual(n1, n3)
            self.assertEqual(n2, n3)
