...
class TestFunctionalIterDataPipe(TestCase):
    ...
    def test_map_tuple_list_with_col_datapipe(self):
        ...
        # Replacing with one input column and multiple specified output columns
        _helper(lambda data: (-data[1], data[1], data[1]), fn_1n, 1, [0, 2])
        #  Unmatched output columns with fn outputs
        with self.assertRaises(TypeError):
            _helper(None, fn_1n, 1, [0, 1, 2])
        ...
        # Appending the expanded output at the end
        _helper(lambda data: (*data, -data[1], data[1]), fn_1n, 1, [-1, -1])
        _helper(lambda data: (*data, -data[1], -data[2], data[1] + data[2]), fn_nn, [1, 2], [-1, -1, -1])

        # Replacing and appending
        _helper(lambda data: (-data[1], *data[1:], data[1]), fn_1n, 1, [0, -1])
        _helper(lambda data: (data[0], data[1] + data[2], -data[1], -data[2]), fn_nn, [1, 2], [2, -1, 1])
    ...
    def test_map_dict_with_col_datapipe(self):
        ...
        # Replacing with one input column and multiple specified output columns
        _helper(lambda data: _dict_update(data, {"x": -data["y"], "z": data["y"]}), fn_1n, "y", ["x", "z"])
        #  Unmatched output columns with fn outputs
        with self.assertRaises(TypeError):
