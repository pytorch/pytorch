...
class MathOpsModule(torch.nn.Module):
    def __init__(self):
        super(MathOpsModule, self).__init__()
    ...
    def pointwise_ops(self):
        ...
        return [
            ...
            torch.bitwise_and(
                torch.tensor([True, True, False]), torch.tensor([False, True, False])
            ),
    ...
    def spectral_ops(self):
        ...
        return [
            torch.stft(a, 2),
            torch.istft(a, 2),
            torch.bartlett_window(4),
            torch.blackman_window(4),
            torch.hamming_window(4),
            torch.hann_window(4),
            torch.kaiser_window(4),
        ]
