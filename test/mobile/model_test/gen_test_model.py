...
def getModuleFromName(model_name):
    if model_name not in all_modules:
        print("cannot find module for " + model_name)
        return
...
The "checked-in" version is for back compatibility test (a model generated today should
run on master branch in the next 6 months).
- use 'python gen_test_model.py android-test' to generate on-the-fly test models for android
