...
class TensorWeakKeyDictionary(weakref.WeakKeyDictionary):
...
class MetaConverter:
    def __init__(self):
        ...
        self.tensor_memo = TensorWeakKeyDictionary()
        self.hit = 0
        self.miss = 0
        self.collected = 0
    ...
    def meta_storage(self, s):
        ...
        if s._cdata not in self.storage_memo:
            self.storage_memo[s._cdata] = torch.empty(
                s.size(), dtype=s.dtype, device="meta"
            )
            ...
            with torch.inference_mode(t.is_inference()):
                if t._is_view():
                    ...
                    def is_c_of_r(complex_dtype, real_dtype):
                        return (
                            is_complex_dtype(complex_dtype)
                            and corresponding_real_dtype(complex_dtype) == real_dtype
                        )
