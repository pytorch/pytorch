...
class TestMatmulCuda(InductorTestCase):
    ...
    def test_addmm_baddmm_dtype_overload(self, input_dtype, M, N, K, batch_size, broadcast_self, high_precision_self, backend):
        ...
        device = "cuda"
        dtype = input_dtype
        mm = torch.addmm if batch_size is None else torch.baddbmm
        batch_size = list() if batch_size is None else (batch_size,)
        ...
        with blas_library_context(backend):
            ...
            for output_dtype in output_dtypes:
                ...
                if input_dtype == torch.bfloat16 and not PLATFORM_SUPPORTS_BF16:
                    ...
                    with self.assertRaises(RuntimeError):
                        mm(c, a, b, out_dtype=output_dtype)
                else:
                    if c.dtype != output_dtype and high_precision_self:
                        c = c.to(output_dtype)

                    out = mm(c, a, b, out_dtype=output_dtype)
                    if output_dtype == torch.float32:
                        baseline = mm(c_fp32, a_fp32, b_fp32)
                    else:
                        baseline = mm(c, a, b)

                    # test out variant
                    out_ten = torch.full_like(out, float("nan"))
                    mm(c, a, b, out_dtype=output_dtype, out=out_ten)
