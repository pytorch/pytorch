...
class TestNN(NNTestCase):
    ...
    def test_noisylinear(self):
        ...
        noisy.reset_noise()
        second_output = noisy(input_tensor)
        self.assertNotEqual(first_output, second_output)
        noisy.eval()
        linear = nn.modules.linear.Linear(10,8)
        linear.weight.data = noisy.weight_mu.data
        linear.bias.data = noisy.bias_mu.data
        self.assertEqual(noisy(input_tensor), linear(input_tensor))
