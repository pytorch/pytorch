...
class TestAutogradFallback(TestCase):
    ...
    def test_no_autograd_kernel(self, mode):
        with autograd_fallback_mode(mode):
            ...
            out = op(a, b, 1).sum()
            if mode == 'warn':
                ctx = self.assertWarnsRegex(UserWarning, "_test_autograd_fallback::foo")
            else:
                ctx = self.assertRaisesRegex(RuntimeError, "does not require grad")
            with ctx:
                out.backward()
            self.assertIsNone(b.grad)

    def _check_ctx(self, mode):
        if mode == "warn":
