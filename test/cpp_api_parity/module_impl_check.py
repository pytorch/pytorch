...
def write_test_to_test_class(
    ...
    test_instance = test_instance_class(**test_params_dict)
    unit_test_name = f"test_torch_nn_{test_instance.get_name()[5:]}"
    ...
    unit_test_class.module_test_param_factories[unit_test_name] = make_test_params
    ...
    def test_fn(self, device):
        ...
        test_forward_backward(
            ...
            test_params=self.__class__.module_test_params_map[unit_test_name],
        )
...
def prepare_test_params(unit_test_class, device):
    ...
    for make_test_params in unit_test_class.module_test_param_factories.values():
...
def build_cpp_tests(unit_test_class, print_cpp_source=False):
    ...
    cpp_module = compile_cpp_code_inline(
        name=f"module_impl_check_{unit_test_class.__name__.lower()}",
