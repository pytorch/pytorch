...
class TestTensorCreation(TestCase):
    ...
    def test_roll(self, device):
        ...
        self.assertEqual(
            single_roll,
            expected,
            msg="{} did not equal expected result".format(single_roll),
        )
        ...
        self.assertEqual(
            roll_backwards,
            expected,
            msg="{} did not equal expected result".format(roll_backwards),
        )
        ...
        self.assertEqual(
            expected,
            rolled,
            msg="{} did not equal expected result: {}".format(rolled, expected),
        )
        ...
        self.assertEqual(
            data,
            loop_rolled,
            msg="{} did not equal the original: {}".format(loop_rolled, data),
        )
        ...
        self.assertEqual(
            expected,
            rolled,
            msg="non contiguous tensor rolled to {} instead of {} ".format(
                rolled, expected
            ),
        )
        ...
        self.assertEqual(
            double_rolled,
            expected,
            msg="should be able to roll over two dimensions, got {}".format(
                double_rolled
            ),
        )
            ...
            for upper in [True, False]:
                ...
                if 0 not in shape:
                    for s in range(-len(shape), -1):
                        ...
                        self.assertEqual(
                            torch_tri_func(x_nc, diagonal), exp_nc, atol=0, rtol=0
                        )
                        x_nc_is_contiguous = x_nc.is_contiguous()
                        if upper:
                            self.assertEqual(
                                x_nc.triu_(diagonal), exp_nc, atol=0, rtol=0
                            )
                        else:
                            self.assertEqual(
                                x_nc.tril_(diagonal), exp_nc, atol=0, rtol=0
                            )
                    ...
                    self.assertEqual(
                        output, expected.expand(expanded_size), atol=0, rtol=0
                    )
                    if x.size(0) != 1:
                        self.assertTrue(
                            0 in x_expanded.stride(),
                    ...
                    if upper:
                        self.assertEqual(
                            output, x_expanded.triu_(diagonal), atol=0, rtol=0
                        )
                    else:
                        self.assertEqual(
                            output, x_expanded.tril_(diagonal), atol=0, rtol=0
                        )
    ...
    def test_torch_complex_floating_dtype_error(self, device, dtype):
        for op in (torch.complex, torch.polar):
            ...
            error = (
                r"Expected both inputs to be Float or Double tensors but "
    ...
    def test_torch_complex_same_dtype_error(self, device, dtype):
        ...
        for op in (torch.complex, torch.polar):
            ...
            error = (
                "Expected object of scalar type {} but got scalar type "
                "{} for second argument".format(
                    dtype_name(dtype), dtype_name(other_dtype)
                )
            )
            with self.assertRaisesRegex(RuntimeError, error):
                op(a, b)
    ...
    @unittest.skipIf(
        IS_MACOS,
        "Test is broken on MacOS, see https://github.com/pytorch/pytorch/issues/38752",
    )
    @unittest.skipIf(
        IS_PPC,
        "Test is borken on PowerPC, see https://github.com/pytorch/pytorch/issues/39671",
    )
    ...
    def test_zeros_dtype_out_match(self, device):
        ...
        self.assertRaises(
            RuntimeError,
            lambda: torch.zeros((2, 3), device=device, dtype=torch.float32, out=d),
        )
    ...
    def test_arange(self, device):
        ...
        res2 = torch.tensor(((0.0, 0.0, 1.0), (0.0, 2.0, 3.0)))
        self.assertEqual(x, res2, atol=1e-16, rtol=0)

        # Check negative
        res1 = torch.tensor((1.0, 0.0))
        res2 = torch.tensor([])
    ...
    @dtypes(
        *torch.testing.get_all_dtypes(
            include_bool=False, include_half=False, include_complex=False
        )
    )
    @dtypesIfCUDA(
        *(
            ...
            if TEST_WITH_ROCM
            else torch.testing.get_all_dtypes(
                include_bool=False, include_half=True, include_complex=False
            )
        )
    )
