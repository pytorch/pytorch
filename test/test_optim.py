...
class TestOptim(TestCase):
    ...
    def _test_state_dict(self, weight, bias, input, constructor):
        ...
        self.assertEqual(optimizer.state_dict()['param_groups'][-1], optimizer_c.state_dict()['param_groups'][-1])
    ...
    def _test_basic_cases(
        self, constructor, scheduler_constructors=None, ignore_multidevice=False, constructor_accepts_maximize=False
    ):
        if scheduler_constructors is None:
            scheduler_constructors = []
    ...
    def test_sgd(self):
        for optimizer in [optim.SGD, optim_mt.SGD]:
            ...
            self._test_basic_cases(
                lambda weight, bias, maximize: optimizer(
                    self._build_params_dict_single(weight, bias, lr=1e-2), lr=1e-3, maximize=maximize
                ),
                constructor_accepts_maximize=True,
            )
            self._test_basic_cases(
            ...
            self._test_basic_cases(
                lambda weight, bias, maximize: optimizer(
                    [weight, bias], lr=1e-3, momentum=1, weight_decay=1, maximize=maximize
                ),
                constructor_accepts_maximize=True,
            )
            self._test_basic_cases(
                lambda weight, bias, maximize: optimizer(
                    [weight, bias], nesterov=True, lr=1e-3, momentum=1, weight_decay=1, maximize=maximize
                ),
                constructor_accepts_maximize=True,
            )
    ...
    def test_sgd_sparse(self):
        for optimizer in [optim.SGD, optim_mt.SGD]:
            self._test_rosenbrock_sparse(lambda params: optimizer(params, lr=5e-3))
            self._test_rosenbrock_sparse(
                lambda params: optimizer(params, lr=0.005), [lambda opt: StepLR(opt, gamma=0.99999, step_size=300)]
            )

    def test_sgd_complex(self):
        for optimizer in [optim.SGD, optim_mt.SGD]:
            self._test_complex_optimizer(lambda param: optimizer([param], lr=0.001))
            self._test_complex_optimizer(lambda param: optimizer([param], lr=0.001, momentum=1))
            self._test_complex_optimizer(lambda param: optimizer([param], lr=0.001, momentum=1, weight_decay=1))
    ...
    def test_multi_tensor_optimizers(self):
        ...
        optimizer_pairs_with_flags = [
            ...
            ((optim.NAdam, optim._multi_tensor.NAdam), dict(weight_decay=0.0, momentum_decay=6e-3)),
            ((optim.NAdam, optim._multi_tensor.NAdam), dict(weight_decay=1.0, momentum_decay=6e-3)),
            ((optim.NAdam, optim._multi_tensor.NAdam), dict(weight_decay=0.0, momentum_decay=4e-3)),
        ...
        for optimizers, params in optimizer_pairs_with_flags:
            ...
            for opt in optimizers:
                ...
                weight2 = torch.tensor(
                    [[-0.0508, -0.3941, -0.2843]], dtype=torch.float64, device=device, requires_grad=True
                )
                ...
                model = torch.nn.Sequential(
                    torch.nn.Linear(2, 3), torch.nn.Sigmoid(), torch.nn.Linear(3, 1), torch.nn.Sigmoid()
                )
    ...
    def test_adam(self):
        for optimizer in [optim.Adam, optim_mt.Adam]:
            ...
            self._test_basic_cases(
                lambda weight, bias: optimizer(self._build_params_dict(weight, bias, lr=1e-2), lr=1e-3, amsgrad=True)
            )
            self._test_basic_cases(
                lambda weight, bias: optimizer(self._build_params_dict(weight, bias, lr=1e-2), lr=1e-3),
            ...
            self._test_basic_cases(
                ...
                [lambda opt: ConstantLR(opt, factor=0.4, total_iters=4), lambda opt: ExponentialLR(opt, gamma=0.9)],
            )
            self._test_basic_cases(
                lambda weight, bias: optimizer([weight, bias], lr=1e-3, amsgrad=True),
    ...
    def test_sparse_adam(self):
        self._test_rosenbrock_sparse(lambda params: optim.SparseAdam(params, lr=4e-2), [], True)
        with self.assertRaisesRegex(ValueError, "Invalid beta parameter at index 0: 1.0"):
    ...
    def test_adadelta(self):
        for optimizer in [optim.Adadelta, optim_mt.Adadelta]:
            self._test_basic_cases(lambda weight, bias: optimizer([weight, bias]))
            self._test_basic_cases(lambda weight, bias: optimizer(self._build_params_dict(weight, bias, rho=0.95)))
            self._test_basic_cases(
                lambda weight, bias: optimizer(self._build_params_dict(weight, bias, rho=0.95)),
                [lambda opt: StepLR(opt, gamma=0.9, step_size=10), lambda opt: ReduceLROnPlateau(opt)],
            )
            self._test_basic_cases(lambda weight, bias: optimizer([weight, bias], weight_decay=1))
    ...
    def test_adadelta_complex(self):
        for optimizer in [optim.Adadelta]:
            ...
            self._test_complex_optimizer(lambda weight: optimizer([weight], rho=0.95, weight_decay=1))
    ...
    def test_adagrad(self):
        for optimizer in [optim.Adagrad, optim_mt.Adagrad]:
            ...
            self._test_basic_cases(
                lambda weight, bias: optimizer([weight, bias], lr=1e-1, initial_accumulator_value=0.1)
            )
            self._test_basic_cases(
    ...
    def test_adagrad_sparse(self):
        for optimizer in [optim.Adagrad, optim_mt.Adagrad]:
            self._test_rosenbrock_sparse(lambda params: optimizer(params, lr=1e-1))
            self._test_rosenbrock_sparse(
    ...
    def test_adagrad_complex(self):
        for optimizer in [optim.Adagrad, optim_mt.Adagrad]:
            self._test_complex_optimizer(lambda param: optimizer([param], lr=1e-1))
            self._test_complex_optimizer(lambda param: optimizer([param], lr=1e-1, initial_accumulator_value=0.1))
    ...
    def test_adamax(self):
        for optimizer in [optim.Adamax, optim_mt.Adamax]:
            ...
            self._test_basic_cases(lambda weight, bias: optimizer([weight, bias], lr=1e-1, weight_decay=1))
    ...
    def test_rmsprop(self):
        for optimizer in [optim.RMSprop, optim_mt.RMSprop]:
            self._test_basic_cases(lambda weight, bias: optimizer([weight, bias], lr=1e-2))
            self._test_basic_cases(
                lambda weight, bias: optimizer(self._build_params_dict(weight, bias, lr=1e-3), lr=1e-2)
            )
            self._test_basic_cases(
                lambda weight, bias: optimizer(self._build_params_dict(weight, bias, lr=1e-3), lr=1e-2, centered=True)
            )
            self._test_basic_cases(
                lambda weight, bias: optimizer(
                    self._build_params_dict(weight, bias, lr=1e-3), lr=1e-2, centered=True, momentum=0.1
                )
            )
            self._test_basic_cases(
                lambda weight, bias: optimizer(self._build_params_dict(weight, bias, lr=1e-3), lr=1e-2, momentum=0.1)
            )
            self._test_basic_cases(
                lambda weight, bias: optimizer(
                    self._build_params_dict(weight, bias, lr=1e-3), lr=1e-2, momentum=0.1, weight_decay=1
                )
            )
    ...
    def test_asgd(self):
        for optimizer in [optim.ASGD, optim_mt.ASGD]:
            self._test_basic_cases(lambda weight, bias: optimizer([weight, bias], lr=1e-3, t0=100))
            self._test_basic_cases(
                lambda weight, bias: optimizer(self._build_params_dict(weight, bias, lr=1e-2), lr=1e-3, t0=100)
            )
            self._test_basic_cases(
                lambda weight, bias: optimizer(self._build_params_dict(weight, bias, lr=1e-3), lr=1e-2, weight_decay=1)
            )
    ...
    def test_lbfgs(self):
        self._test_basic_cases(lambda weight, bias: optim.LBFGS([weight, bias]), ignore_multidevice=True)
        self._test_basic_cases(
            lambda weight, bias: optim.LBFGS([weight, bias], line_search_fn="strong_wolfe"), ignore_multidevice=True
        )
...
class TestLRScheduler(TestCase):
    ...
    def setUp(self):
        ...
        self.opt = SGD(
            [{'params': self.net.conv1.parameters()}, {'params': self.net.conv2.parameters(), 'lr': 0.5}], lr=0.05
        )
    ...
    def test_reduce_lr_on_plateau1(self):
        ...
        scheduler = ReduceLROnPlateau(
            self.opt, threshold_mode='abs', mode='min', threshold=0.01, patience=5, cooldown=5
        )
    ...
    def test_reduce_lr_on_plateau8(self):
        ...
        metrics = [1.5 * (1.005 ** i) for i in range(20)]
        scheduler = ReduceLROnPlateau(
            self.opt, mode='max', threshold_mode='rel', min_lr=[0.4, 0.3], threshold=0.1, patience=5, cooldown=5
        )
        self._test_reduce_lr_on_plateau(scheduler, targets, metrics, epochs)
    ...
    def test_compound_reduce_lr_on_plateau2(self):
        ...
        schedulers = [None] * 2
        schedulers[0] = ReduceLROnPlateau(
            self.opt, patience=5, cooldown=0, threshold_mode='abs', mode='min', threshold=0.1
        )
    ...
    def test_cycle_lr_triangular2_mode_one_lr(self):
        lr_target = [1, 2, 3, 4, 5, 4, 3, 2, 1, 1.5, 2.0, 2.5, 3.0, 2.5, 2.0, 1.5, 1, 1.25, 1.50, 1.75, 2.00, 1.75]
        momentum_target = [
            5.0,
            4.0,
            3.0,
    ...
    def test_cycle_lr_triangular2_mode(self):
        lr_target_1 = [1, 2, 3, 4, 5, 4, 3, 2, 1, 1.5, 2.0, 2.5, 3.0, 2.5, 2.0, 1.5, 1, 1.25, 1.50, 1.75, 2.00, 1.75]
    ...
    def test_onecycle_lr_cosine_annealing(self):
        ...
        momentum_targets = [momentum_target, momentum_target]
        scheduler = OneCycleLR(
            self.opt, max_lr=25, final_div_factor=2, base_momentum=1, max_momentum=22, total_steps=10
        )
        self._test_cycle_lr(scheduler, lr_targets, momentum_targets, 10)
    ...
    def test_swalr_cosine_anneal_after_multiplicative(self):
        ...
        targets = [
            lrs + [lrs[-1] * anneal_coef(t) + swa_lr * (1 - anneal_coef(t)) for t in range(swa_epochs)]
    ...
    def test_step_lr_state_dict(self):
        self._check_scheduler_state_dict(
            lambda: StepLR(self.opt, gamma=0.1, step_size=3), lambda: StepLR(self.opt, gamma=0.01 / 2, step_size=1)
        )
    ...
    def test_exp_step_lr_state_dict(self):
        self._check_scheduler_state_dict(
            lambda: ExponentialLR(self.opt, gamma=0.1), lambda: ExponentialLR(self.opt, gamma=0.01)
        )
    ...
    def test_swa_lr_state_dict(self):
        self._check_scheduler_state_dict(
            ...
            lambda: SWALR(self.opt, anneal_epochs=10, anneal_strategy="linear", swa_lr=5.0),
        )
        ...
        for epoch in range(epochs):
            ...
            for t, r in zip(target, result):
                self.assertEqual(
                    target,
                    result,
                    msg='LR is wrong in epoch {}: expected {}, got {}'.format(epoch, t, r),
                    atol=1e-5,
                    rtol=0,
                )
        ...
        for index, epoch in enumerate(torch.arange(0, epochs, 0.1)):
            ...
            for param_group, target in zip(self.opt.param_groups, targets):
                self.assertEqual(
                    target[index],
                    param_group['lr'],
                    msg='LR is wrong in epoch {}: expected {}, got {}'.format(epoch, target[index], param_group['lr']),
                    atol=1e-5,
                    rtol=0,
                )
                ...
                if 'momentum' in self.opt.param_groups[0].keys():
                    print(
                        'batch{}:\tlr={},momentum={}'.format(
                            batch_num, self.opt.param_groups[0]['lr'], self.opt.param_groups[0]['momentum']
                        )
                    )
                elif use_beta1 and 'betas' in self.opt.param_groups[0].keys():
                    print(
                        'batch{}:\tlr={},beta1={}'.format(
                            batch_num, self.opt.param_groups[0]['lr'], self.opt.param_groups[0]['betas'][0]
                        )
                    )
                else:
                ...
                if use_beta1 and 'betas' in param_group.keys():
                    self.assertEqual(
                        ...
                        msg='Beta1 is wrong in batch_num {}: expected {}, got {}'.format(
                            batch_num, momentum_target[batch_num], param_group['betas'][0]
                        ),
                        atol=1e-5,
                        rtol=0,
                    )
...
class TestSWAUtils(TestCase):
        ...
        for i in range(n_updates):
            ...
            for p, p_avg in zip(dnn.parameters(), averaged_params):
                ...
                if i == 0:
                    ...
                    updated_averaged_params.append((p_avg * alpha + p * (1 - alpha)).clone())
            averaged_dnn.update_parameters(dnn)
    ...
    def test_averaged_model_exponential_use_state_dict(self):
        ...
        averaged_params = [
            torch.zeros_like(param) for param in dnn.state_dict().values() if param.size() != torch.Size([])
        ]
        n_updates = 10
        for i in range(n_updates):
            updated_averaged_params = []
