...
class TestOptim(TestCase):
    ...
    def test_multi_tensor_optimizers(self):
        ...
        optimizer_pairs_with_flags = [
            ...
            (
                (optim.NAdam, optim._multi_tensor.NAdam),
                dict(weight_decay=0.0, momentum_decay=6e-3),
            ),
            (
                (optim.NAdam, optim._multi_tensor.NAdam),
                dict(weight_decay=1.0, momentum_decay=6e-3),
            ),
            (
                (optim.NAdam, optim._multi_tensor.NAdam),
                dict(weight_decay=0.0, momentum_decay=4e-3),
            ),
            (
                (optim.NAdam, optim._multi_tensor.NAdam),
            ...
            (
                ...
                dict(weight_decay=1, momentum=1, centered=True),
            ),
            (
                (optim.RMSprop, optim._multi_tensor.RMSprop),
                dict(weight_decay=1, momentum=0, centered=True),
            ),
            (
                (optim.RMSprop, optim._multi_tensor.RMSprop),
                dict(weight_decay=1, momentum=1, centered=False),
            ),
            (
                (optim.RMSprop, optim._multi_tensor.RMSprop),
                dict(weight_decay=0, momentum=1, centered=False),
            ),
            (
        ...
        for optimizers, params in optimizer_pairs_with_flags:
            ...
            for opt in optimizers:
                ...
                bias2 = torch.tensor(
                    [-0.0711], dtype=torch.float64, device=device, requires_grad=True
                )
                input = torch.tensor(
    ...
    def test_adam(self):
        for optimizer in [optim.Adam, optim_mt.Adam]:
            ...
            self._test_basic_cases(
                lambda weight, bias: optimizer(
                    [weight, bias], lr=1e-3, weight_decay=0.1
                )
            )
            self._test_basic_cases(
                lambda weight, bias: optimizer(
    ...
    def test_adamw(self):
        for optimizer in [optim.AdamW, optim_mt.AdamW]:
            ...
            self._test_basic_cases(
                lambda weight, bias: optimizer(
                    [weight, bias], lr=1e-3, weight_decay=1, amsgrad=True
                )
            )
            with self.assertRaisesRegex(ValueError, "Invalid weight_decay value: -1"):
                optimizer(None, lr=1e-2, weight_decay=-1)
    ...
    def test_nadam(self):
        for optimizer in [optim.NAdam, optim_mt.NAdam]:
            ...
            self._test_basic_cases(
                lambda weight, bias: optimizer(
                    [weight, bias], lr=1e-3, weight_decay=0.1, momentum_decay=6e-3
                )
            )
            self._test_basic_cases(
                lambda weight, bias: optimizer(
            ...
            with self.assertRaisesRegex(
                ...
                optimizer(None, lr=1e-2, betas=(1.0, 0.0))
            with self.assertRaisesRegex(
                ValueError, "Invalid momentum_decay value: -0.2"
            ):
