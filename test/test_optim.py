...
class TestParamGroupsForMuon(TestCase):
    ...
    def test_shared_parameter_excluded_by_any_name(self):
        model = torch.nn.Module()
        model.embed = torch.nn.Embedding(10, 4)
        model.lm_head = torch.nn.Linear(4, 10, bias=False)
        model.lm_head.weight = model.embed.weight

        muon_params, other_params = torch.optim.param_groups_for_muon(
            model, exclude_name_patterns=("lm_head",)
        )

        self.assertEqual({id(p) for p in muon_params}, set())
        self.assertEqual({id(p) for p in other_params}, {id(model.embed.weight)})
...
class TestMuon(TestCase):
    ...
    def test_muon_accepts_conv2d_weight(self, device):
        torch.manual_seed(0)
        conv = torch.nn.Conv2d(3, 8, 3, bias=False).to(device)
