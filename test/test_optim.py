...
class TestOptim(TestCase):
    ...
    def _call_differentiable_optimizer(self, opt, model, step, states, epoch):
        ...
        if opt == differentiable.adamw:
            differentiable.adamw(model, step, states["exp_avgs"], states["exp_avg_sqs"], states["max_exp_avg_sqs"],
                                 [epoch + 1], amsgrad=False, beta1=0.99, beta2=0.99, lr=0.5, weight_decay=0., eps=0.001)
        if opt == differentiable.adadelta:
            differentiable.adadelta(model, step, states["square_avg"], states["acc_delta"], lr=0.5, weight_decay=0.1,
                                    rho=0.6, eps=0.001)
    ...
    def test_differentiable_adamw(self):
        model = [torch.randn(1, 2, dtype=torch.double, requires_grad=True)]
        self._inner_loop_differentiable_optimizers(differentiable.adamw, model)
    ...
    def test_differentiable_adadelta(self):
        model = [torch.randn(1, 2, dtype=torch.double, requires_grad=True)]
        self._inner_loop_differentiable_optimizers(differentiable.adadelta, model)
    ...
    def test_differentiable_adagrad(self):
        model = [torch.randn(1, 2, dtype=torch.double, requires_grad=True)]
        self._inner_loop_differentiable_optimizers(differentiable.adagrad, model)
