...
import { buildCallstackTree } from '../utils/dataProcessor';
import './IcicleModal.css';
...
function IcicleModal({ events, onClose }) {
  ...
  useEffect(() => {
    ...
    // Start from root's children to skip displaying the root node
    const rootNode = focusedNode || tree;
    const startNodes = focusedNode ? [focusedNode] : tree.children;
    ...
    // Calculate total count for the starting level
    const totalCount = startNodes.reduce((sum, node) => sum + node.count, 0);

    function drawNode(node, x, nodeWidth, depth, isRoot = false) {
    ...
    // Draw starting from actual callstack entries (skip root)
    if (focusedNode) {
      drawNode(focusedNode, 0, width, 0);
    } else {
      let x = 0;
      startNodes.forEach(node => {
        const nodeWidth = (node.count / totalCount) * width;
        drawNode(node, x, nodeWidth, 0);
        x += nodeWidth;
      });
    }
  ...
  const handleCanvasClick = (e) => {
    ...
    if (focusedNode) {
      clickedNode = findNodeAt(focusedNode, 0, width, 0, x, y);
    } else {
      let nodeX = 0;
      for (const node of startNodes) {
        const nodeWidth = (node.count / totalCount) * width;
        clickedNode = findNodeAt(node, nodeX, nodeWidth, 0, x, y);
