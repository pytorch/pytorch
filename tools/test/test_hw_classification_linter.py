...
class TestHwClassificationLinter(unittest.TestCase):
    ...
    # ==================================================================
    # GENERIC / SPECIFIC (CPU, CUDA, MPS, XPU)
    # ==================================================================

    def test_valid_non_accelerator_classification(self) -> None:
        for classification in (GENERIC, CPU, CUDA, MPS, XPU):
            src = f"""\
    ...
    def test_non_accelerator_classification_with_device_param(self) -> None:
        for param in ("device", "devices"):
            for classification in (GENERIC, CPU, CUDA, MPS, XPU):
                ...
                msgs = self._run(src)
                self.assertEqual(
                    len(msgs), 1, f"failed for {classification} with {param}"
                )
                self.assertEqual(
                    msgs[0],
                    create_error_msg(
                        msgs[0].path,
                        4,
                        f"{classification} test method 'TestFoo.test_x' must not accept a 'device' or 'devices' parameter.",
                    ),
                )

    def test_non_accelerator_classification_with_instantiated(self) -> None:
        for classification in (GENERIC, CPU, CUDA, MPS, XPU):
            src = f"""\
            ...
            self.assertEqual(
                ...
                create_error_msg(
                    msgs[0].path,
                    3,
                    f"{classification} class 'TestFoo' must not be instantiated via 'instantiate_device_type_tests'.",
                ),
            )
