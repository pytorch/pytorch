...
# unequal shards.
IS_ROCM = os.path.exists("/opt/rocm") or (
    torch is not None and torch.version.hip is not None
)
...
# Detect ROCm either via a system install (/opt/rocm) or a ROCm wheel
# (torch.version.hip set, e.g. TheRock preview wheels which have no /opt/rocm).
# Both must reach the rocminfo-based NUM_PROCS clamp below; otherwise NUM_PROCS
# stays >GPU-count and maybe_set_hip_visible_devies() in run_test.py assigns
# workers to nonexistent device indices -> torch.cuda.device_count()==0.
