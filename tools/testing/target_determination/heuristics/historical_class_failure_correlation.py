...
from tools.stats.export_test_times import TEST_CLASS_RATINGS_FILE
...
def _get_file_rating_tests() -> List[str]:
    path = REPO_ROOT / TEST_CLASS_RATINGS_FILE
    if not os.path.exists(path):
        print(f"could not find path {path}")
        return []
    with open(path) as f:
        test_class_ratings = cast(Dict[str, Dict[str, float]], json.load(f))
    try:
