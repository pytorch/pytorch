...
This module generates type stubs for torch, torch.Tensor, and native
bindings for torch.nn, torch.fft, torch.linalg, and torch.special.
...
def get_py_torch_functions(
    ...
    as functions in "torch" (or a native Python module) or methods on Tensor.
    """
    ...
    def should_bind_function(python_func: PythonSignatureNativeFunctionPair) -> bool:
        return (
            ...
            and python_func.function.python_module == python_module
...
def generate_type_hints(
    sig_group: PythonSignatureGroup, *, use_sequence: bool = False
) -> list[str]:
    type_hints: list[str] = []
    ...
    if sig_group.signature.deprecated and sig_group.outplace is not None:
        type_hint = sig_group.signature.signature_str_pyi(
            skip_outputs=True, use_sequence=use_sequence
        )
    ...
    type_hint = sig_group.signature.signature_str_pyi(
        skip_outputs=sig_group.outplace is None, use_sequence=use_sequence
    )
    type_hints.append(type_hint)
...
def gen_pyi(
    ...
    torch_structseq_names = set(structseqs)

    # These modules expose native bindings through type-preserving _add_docstr
    # calls. Generate the bindings, leaving the Python modules (and aliases) as
    # the source of truth for their public API.
    for module in ("fft", "linalg", "special"):
        ...
        hints = []
        for _, overloads in sorted(module_hints.items()):
            hints.extend(
                ("@overload\n" + hint) if len(overloads) > 1 else hint
                for hint in overloads
            )
        fm.write_with_template(
            f"torch/_C/_{module}.pyi",
            "torch/_C/_native_functions.pyi.in",
            lambda: {"function_hints": hints},
        )
    ...
    all_symbols = sorted(hinted_function_names.union(torch_structseq_names))
    all_directive = [
        "__all__ = [",
