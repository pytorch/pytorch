...
def construct_version_maps(upgrader_bytecode_function_to_index_map: Dict) -> Dict:
    ...
    for op_name in version_map:
        ...
        for upgrader_entry in version_map[op_name]:
            ...
            if upgrader_entry.upgrader_name == "full_names_0_4" or upgrader_entry.upgrader_name == "full_out_0_4":
                continue
...
def write_cpp(cpp_path, upgrader_dict):
    ...
    with open(
        os.path.join("/tmp", UPGRADER_MOBILE_FILE_NAME), "wb"
    ) as out_file:
        final_output = "".join(body_parts)
        out_file.write(upgrader_file_content.encode("utf-8"))
