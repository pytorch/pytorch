...
def parse_native_yaml(path: str) -> Tuple[List[NativeFunction], Dict[DispatchKey, BackendIndex]]:
...
@with_native_function_and_indices
def compute_declaration_yaml(f: NativeFunction, backend_indices: Dict[DispatchKey, BackendIndex]) -> object:
    ...
    return OrderedDict([
        ...
        ('with_gil', False),
        ('deprecated', False),
        ('has_math_kernel', backend_indices[DispatchKey.CompositeImplicitAutograd].has_backend(f)),
    ])
...
def compute_registration_declarations(f: NativeFunction, backend_indices: Dict[DispatchKey, BackendIndex]) -> str:
    ...
    comment_data : Dict[str, str] = {
        ...
        'dispatch': str({k for k, v in backend_indices.items() if v.has_backend(f)} != {DispatchKey.CompositeImplicitAutograd}),
        'default': str(any(is_generic_dispatch_key(k) for k, v in backend_indices.items() if v.has_backend(f)) or \
                       has_autogenerated_composite_kernel(f))
    }
...
def get_grouped_native_functions(
    ...
    def flatten_pre_group(d: Dict[SchemaKind, NativeFunction]) -> Sequence[Union[NativeFunction, NativeFunctionsGroup]]:
        r = NativeFunctionsGroup.from_dict(d, backend_indices)
        if r is None:
...
#include <ATen/hip/HIPContext.h>'''
        ...
        fm.write_with_template(f'Register{dispatch_key}.cpp', 'RegisterDispatchKey.cpp', lambda: {
            ...
            'dispatch_namespaced_definitions': list(concatMap(
                dest.RegisterDispatchKey(
                    backend_indices[dispatch_key], Target.NAMESPACED_DEFINITION, selector, rocm=options.rocm),
                grouped_native_functions
            )),
            'dispatch_anonymous_definitions': list(concatMap(
                dest.RegisterDispatchKey(
                    backend_indices[dispatch_key], Target.ANONYMOUS_DEFINITION, selector, rocm=options.rocm),
                grouped_native_functions
            )),
            'dispatch_registrations': list(concatMap(
                dest.RegisterDispatchKey(backend_indices[dispatch_key], Target.REGISTRATION, selector, rocm=options.rocm),
                grouped_native_functions
            )),
        })
        ...
        if dispatch_key in functions_keys:
            fm.write_with_template(f'{dispatch_key}Functions.h', 'DispatchKeyFunctions.h', lambda: {
                ...
                'dispatch_namespaced_declarations': list(concatMap(
                    dest.RegisterDispatchKey(
                        backend_indices[dispatch_key], Target.NAMESPACED_DECLARATION, selector, rocm=options.rocm),
                    grouped_native_functions
                )),
            })
    ...
    cpu_fm.write('Declarations.yaml', lambda: format_yaml([compute_declaration_yaml(f, backend_indices) for f in native_functions]))
