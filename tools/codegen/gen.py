...
@with_native_function
def compute_out_convention_refactor_native_functions(g: NativeFunction) -> List[str]:
    ...
    for arg in args:
        ...
        if isinstance(arg.argument, Argument) and 'Tensor?' in str(arg.argument.type):
            has_optional_tensor = True
            break
    if not has_optional_tensor:
        return []
...
def compute_out_convention_refactor(g: Union[NativeFunctionsGroup, NativeFunction]) -> List[str]:
    if isinstance(g, NativeFunctionsGroup):
        if g.structured:
            ...
            return list(concatMap(compute_out_convention_refactor_native_functions, g.functions()))
    else:
        return compute_out_convention_refactor_native_functions(g)
