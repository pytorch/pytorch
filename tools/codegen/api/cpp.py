...
from tools.codegen.api.types import (ArgName, BaseCType, Binding,
                                     ConstRefCType, NamedCType, CType, MutRefCType, ArrayCType, ListCType, VectorCType, ArrayRefCType,
                                     OptionalCType, TupleCType, SpecialArgName, boolT, scalarT, tensorListT, dimnameListT,
                                     tensorT, voidT, BaseTypeToCppMapping, intArrayRefT, tensorOptionsT)
    ...
    for i, r in enumerate(f.func.returns):
        ...
        # result1, etc (zero-indexed)
            ...
            if override_name is not None:
                name = override_name if len(f.func.returns) == 1 else f'{override_name}[{i}]'
            else:
