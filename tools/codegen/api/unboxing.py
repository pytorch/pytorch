...
from tools.codegen.api.types import BaseCType, tensorOptionsT, CppSignature
from tools.codegen.model import Argument, Type, BaseType, OptionalType, ListType, BaseTy, \
    TensorOptionsArguments, NativeFunction, Variant
...
def convert_arguments(args: List[Argument],
                      tensor_option_arg: Optional[TensorOptionsArguments]) -> Dict[str, ArgumentCppCode]:
...
def argumenttype_ivalue_convert(t: Type, ival: str, arg_name: str) -> ArgumentCppCode:
    if isinstance(t, BaseType):
        ctype = argumenttype_type(t=t, mutable=True, binds=arg_name).cpp_type(strip_ref=True)
