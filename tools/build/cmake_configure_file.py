...
def main(argv: List[str]) -> None:
    ...
    subprocess.run(
        " ".join(cmake_configure_file_cmd(template_path, definitions, output_path)),
        check=True,
        shell=True,
    )


def cmake_configure_file_cmd(
    template_path: str, definitions: List[str], output_path: pathlib.Path
) -> list[str]:
