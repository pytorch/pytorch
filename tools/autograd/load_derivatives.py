...
def remove_list_type(nctypes: List[NamedCType]) -> Tuple[List[NamedCType], List[bool]]:
    ...
    for nctype in nctypes:
        ...
        if cpp_type in (
            str(tensorListT),  # type generated by torchgen foreach logic
            "::std::vector<at::Tensor>",  # type parsed & generated from derivatives.yaml
        ):
            cpp_type = tensorT
            needs_change = True
        if cpp_type == str(
...
def create_derivative(
    ...
    formula, saved_outputs = saved_variables(
        formula, named_returns, var_names, ret_needs_indexing if is_foreach_op else []
    )
