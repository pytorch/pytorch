...
Tensor cummax_backward(const Tensor &out, const Tensor &indices, const Tensor &grad, const Tensor &input, int64_t dim) {
  if (input.dim() == 0) {
    return input;
  }
  return grad * at::scatter(out, dim, indices, at::ones(out.sizes()));
}
