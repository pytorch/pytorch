...
def print_tensor(debugger, command, result, internal_dict):
    """
    ...
    with DisableBreakpoints():
        ...
        assert len(command_str) == 1, "print_tensor only accepts the tensor name as argument"
...
def __lldb_init_module(debugger, internal_dict):
    debugger.HandleCommand('command script add -f pytorch_lldb.print_tensor print_tensor')
