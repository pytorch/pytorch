...
def test_fuzzing_tensors() -> None:
    ...
    for i in range(total_tests):
        try:
            ...
            print(
                f"    is_contiguous: {is_contig}, is_non_overlapping_and_dense: {is_dense}"
            )
    ...
    print(
        f"Contiguous: {contiguous_count} ({contiguous_count / total_tests * 100:.1f}%)"
    )
    print(
        f"Non-contiguous but dense: {non_contiguous_dense_count} ({non_contiguous_dense_count / total_tests * 100:.1f}%)"
    )
    print(
...
def test_fuzz_spec():
    ...
    from ops_fuzzer import fuzz_spec, ScalarSpec, TensorSpec
    ...
    for i in range(5):
        ...
        if isinstance(spec, TensorSpec):
            print(
                f"   Type: TensorSpec, Size: {spec.size}, Stride: {spec.stride}, Dtype: {spec.dtype}"
            )
...
def test_fuzz_op():
    ...
    from ops_fuzzer import fuzz_op, ScalarSpec, TensorSpec
    ...
    op_name, arg_specs = fuzz_op(scalar_spec, depth=2, stack_size=5)
    ...
    op_name, arg_specs = fuzz_op(tensor_spec_1d, depth=2, stack_size=5)
    print(f"   Operation: {op_name}")
    print(f"   Argument specs: {arg_specs}")
    ...
    op_name, arg_specs = fuzz_op(tensor_spec_2d, depth=2, stack_size=5)
    print(f"   Operation: {op_name}")
    print(f"   Argument specs: {arg_specs}")
    ...
    op_name, arg_specs = fuzz_op(tensor_spec_custom, depth=2, stack_size=5)
    print(f"   Operation: {op_name}")
    print(f"   Argument specs: {arg_specs}")
...
def test_fuzz_spec_with_fuzz_op():
    ...
    for i in range(3):
        ...
        # Use it with fuzz_op
        op_name, arg_specs = fuzz_op(random_spec, depth=2, stack_size=5)
        print(f"   Operation: {op_name}")
