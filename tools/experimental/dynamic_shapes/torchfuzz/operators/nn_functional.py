...
class EmbeddingOperator(Operator):
    ...
    def can_produce(self, output_spec: Spec) -> bool:
        ...
        return output_spec.dtype in [torch.float32, torch.float64, torch.float16, torch.bfloat16]
    ...
    def fuzz_inputs_specs(self, output_spec: Spec) -> list[Spec]:
        ...
        # Generate reasonable vocab size (between 100 and 10000)
    ...
    def codegen(
        ...
        return f"{output_name} = torch.nn.functional.embedding({input_name}, {weight_name})"
...
class LinearOperator(Operator):
    ...
    def codegen(
        ...
        if len(input_names) == 2:
            ...
            raise ValueError("Linear requires 2 or 3 inputs: input, weight, and optional bias")
...
class LayerNormOperator(Operator):
    ...
    def codegen(
        ...
        if len(input_names) < 1 or len(input_names) > 3:
            raise ValueError("LayerNorm requires 1-3 inputs: input, optional weight, optional bias")
