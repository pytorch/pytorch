...
def write_shim_function_dynamic_list(
    functions: dict[str, tuple[int, int, int]],
    output_file: Path | str | None = None,
) -> None:
    """
    Write the `--dynamic-list=` file consumed by `//caffe2:stable_abi_shim_for_dlopen`:
    keeps shim symbols from `--gc-sections` and exports them to `.dynsym` for
    dlopen-loaded extensions. Sibling to `write_shim_function_versions` — same main()
    keeps .txt and .syms in lockstep.
    """
    if output_file is None:
        repo_root = Path(__file__).resolve().parents[3]
        output_file = repo_root / "torch/csrc/stable/c/libtorch_stable_exports.syms"
    output_file = Path(output_file)

    # Require `_` to skip PascalCase typedefs (ParallelFunc, StableListHandle, ...).
    body = "".join(f"    {n};\n" for n in sorted(n for n in functions if "_" in n))
    output_file.write_text(
        "# Auto-generated by stable_shim_usage_linter alongside shim_function_versions.txt.\n"
        "# Consumed via --dynamic-list= on //caffe2:stable_abi_shim_for_dlopen. DO NOT EDIT MANUALLY.\n"
        f"{{\n{body}}};\n"
    )
...
if __name__ == "__main__":
    ...
    write_shim_function_dynamic_list(shim_functions)
