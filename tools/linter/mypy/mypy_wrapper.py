...
def read_config(config_path: Path) -> Set[str]:
    ...
    return set(mypy.config_parser.split_and_match_files(config["mypy"]["files"],))
...
def run(*, args: List[str], files: List[str],) -> Tuple[int, List[str], List[str]]:
    """
    ...
    return (
        ...
        max([exit_code for _, _, exit_code in mypy_results], default=0,),
        list(
