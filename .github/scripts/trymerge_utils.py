...
from github_queries import (
    GH_GET_COMMIT_CHECKSUITES,
    GH_GET_COMMIT_NEXT_CHECK_RUNS,
    GH_GET_COMMIT_NEXT_CHECKSUITES,
...
def fetch_json_dict(
    ...
    return cast(
        Dict[str, Any], _fetch_url(url, headers=headers, data=data, reader=json.load)
    )
...
def gh_post_pr_comment(
    org: str, project: str, pr_num: int, comment: str, dry_run: bool = False
) -> List[Dict[str, Any]]:
    return _gh_post_comment(
        f"https://api.github.com/repos/{org}/{project}/issues/{pr_num}/comments",
        comment,
        dry_run,
    )
...
def gh_post_commit_comment(
    org: str, project: str, sha: str, comment: str, dry_run: bool = False
) -> List[Dict[str, Any]]:
    return _gh_post_comment(
        f"https://api.github.com/repos/{org}/{project}/commits/{sha}/comments",
        comment,
        dry_run,
    )
...
def gh_add_labels(
    org: str, project: str, pr_num: int, labels: Union[str, List[str]]
) -> None:
    fetch_json(
        ...
        for edge_idx, edge in enumerate(edges):
            ...
            if workflow_run is not None:
                ...
                conclusions[workflow_name] = WorkflowCheckState(
                    name=workflow_name, status=workflow_conclusion, url=node["url"]
                )
            ...
            if workflow_run is not None and has_failing_check:
                workflow_name = workflow_run["workflow"]["name"]
                conclusions[workflow_name] = WorkflowCheckState(
                    name=workflow_name, status="FAILURE", url=node["url"]
                )
...
class GitHubPR:
    ...
    def gen_commit_message(self, filter_ghstack: bool = False) -> str:
        ...
        msg += (
            self.get_body()
            if not filter_ghstack
            else re.sub(RE_GHSTACK_DESC, "", self.get_body())
        )
    ...
    def merge_changes(
        ...
        if not self.is_ghstack_pr():
            ...
            self.merge_ghstack_into(
                repo, force, comment_id=comment_id, land_check_commit=land_check_commit
            )
    ...
    for rule in rules:
        ...
        if len(non_matching_files) > 0:
            ...
            if num_matching_files > reject_reason_score:
                ...
                reject_reason = (
                    f"{num_matching_files} files matched rule {rule_name}, but there are still non-matching files: "
        ...
        if len(approvers_intersection) == 0 and len(rule_approvers_set) > 0:
            if reject_reason_score < 10000:
                reject_reason_score = 10000
                reject_reason = (
                    f"Matched rule {rule_name}, but PR #{pr.pr_num} was not reviewed yet by any of: "
        ...
        required_checks = filter(
            lambda x: force is False or "CLA Check" in x, mandatory_checks
        )
...
def get_land_checkrun_conclusions(
    org: str, project: str, commit: str
) -> Dict[str, WorkflowCheckState]:
    def get_commit_next_check_runs(
        edges: List[Dict[str, Dict[str, Any]]], edge_idx: int, checkruns: Any
    ) -> Any:
        rc = gh_graphql(
            GH_GET_COMMIT_NEXT_CHECK_RUNS,
        ...
        return rc["data"]["repository"]["object"]["checkSuites"]["nodes"][-1][
            "checkRuns"
        ]
    ...
    return add_workflow_conclusions(
        checksuites, get_commit_next_check_runs, get_commit_next_checksuites
    )
...
def get_combined_checks_from_pr_and_land_validation(
    pr: GitHubPR, land_check_commit: Optional[str]
) -> Dict[str, WorkflowCheckState]:
    """
    ...
    land_validation_checks = (
        get_land_checkrun_conclusions(pr.org, pr.project, land_check_commit)
    ...
    merged_checks = {
        **pr_checks,
        **land_validation_checks,
    }  # explanation: https://stackoverflow.com/a/26853961/21539
...
def filter_failed_checks(
    ...
    return filter_checks_with_lambda(
        checks, lambda x: x in ["FAILURE", "STARTUP_FAILURE"]
    )
...
def validate_revert(
    ...
    find_matching_merge_rule(
        pr, repo, force=True, skip_internal_checks=skip_internal_checks
    )
...
def validate_land_time_checks(org: str, project: str, commit: str) -> None:
    ...
    if len(failed_checks) > 0:
        raise RuntimeError(
            f"Failed to merge; some land checks failed: {checks_to_str(failed_checks)}"
        )
...
def categorize_checks(
    check_runs: Dict[str, WorkflowCheckState], required_checks: Iterable[str]
    ...
    for checkname in required_checks:
        ...
        elif str(check_runs[checkname].status).upper() not in [
            "SUCCESS",
            "SKIPPED",
            "NEUTRAL",
        ]:
...
def merge(
    ...
    explainer = TryMergeExplainer(
        force, on_green, land_checks, pr.get_labels(), pr.pr_num, org, project
    )
    ...
    if land_checks:
        land_check_commit = pr.create_land_time_check_branch(
            repo, "viable/strict", force=force, comment_id=comment_id
        )

    gh_post_pr_comment(
        org, project, pr.pr_num, explainer.get_merge_message(land_check_commit)
    )
    ...
    while elapsed_time < timeout_minutes * 60:
        ...
        try:
            ...
            checks = get_combined_checks_from_pr_and_land_validation(
                pr, land_check_commit
            )
            ...
            if len(startup_failures) > 0:
                raise RuntimeError(
                    f"{len(failing)} STARTUP failures reported, please check workflows syntax! "
