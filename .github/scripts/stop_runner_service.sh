...
if [[ -n "${RUNNER_SERVICE}" ]]; then
  ...
  # Stop it to prevent the runner from receiving new jobs. Run this as one command as
  # svc.sh stop would terminate the connection to the runner
  sudo ./svc.sh stop && sudo ./svc.sh uninstall && sudo shutdown -P now
