...
def main() -> None:
    ...
    with open(f"{args.pin_folder}/{args.repo_name}.txt", "r+") as f:
        ...
        subprocess.run(
            f"git checkout {old_hash}".split(), cwd=args.repo_name, check=True
        )
        f.seek(0)
        f.truncate()
        f.write(f"{hash}\n")
    if is_newer_hash(hash, old_hash, args.repo_name):
        ...
        subprocess.run(
            f"git add {args.pin_folder}/{args.repo_name}.txt".split(), check=True
        )
        subprocess.run(
        ...
        subprocess.run(
            f"git push --set-upstream origin {branch_name} -f".split(), check=True
        )
