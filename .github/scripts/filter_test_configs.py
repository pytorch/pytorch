...
def get_pr_info(pr_number: int) -> Dict[str, Any]:
    ...
    if not json_response:
        warnings.warn(f"Failed to get the labels for #{pr_number}", stacklevel=TO_BE_DETERMINED)
        return {}
...
def process_jobs(
    ...
    try:
        ...
        warnings.warn(f"Invalid job name {job_name}, returning", stacklevel=TO_BE_DETERMINED)
        return test_matrix
...
def download_json(url: str, headers: Dict[str, str], num_retries: int = 3) -> Any:
    for _ in range(num_retries):
        try:
            ...
            warnings.warn(f"Could not download {url}: {e}", stacklevel=TO_BE_DETERMINED)

    warnings.warn(
        f"All {num_retries} retries exhausted, downloading {url} failed", stacklevel=TO_BE_DETERMINED
    )
    return {}
...
def get_reenabled_issues(pr_body: str = "") -> List[str]:
    ...
    try:
        ...
        warnings.warn(f"failed to get commit messages: {e}", stacklevel=TO_BE_DETERMINED)
        commit_messages = ""
...
def main() -> None:
    ...
    if test_matrix is None:
        warnings.warn(
            f"Invalid test matrix input '{args.test_matrix}', exiting", stacklevel=TO_BE_DETERMINED
        )
