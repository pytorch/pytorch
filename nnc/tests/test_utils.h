...
class Value {
 public:
  Value() : dtype_(kInt32) { i32_values.push_back(0); }
  Value(int v) : dtype_(kInt32) { i32_values.push_back(v); }
  Value(float v) : dtype_(kFloat32) { f32_values.push_back(v); }
  Value(const std::vector<int>& v) : dtype_(Dtype(kInt32, v.size())), i32_values(v) {}
  Value(const std::vector<float>& v) : dtype_(Dtype(kFloat32, v.size())), f32_values(v) {}
...
class SimpleIREvaluator : public IRVisitor {
 public:
  ...
  void visit(const IntImm* v) override { value_ = Value(v->value()); }
  void visit(const FloatImm* v) override { value_ = Value(v->value()); }
  ...
  void visit(const For* v) override {
    ...
    CHECK(iter == eval_context_.end()) << "var in For must not exist in eval context";
  ...
  void SetBufferMapping(const BufferMapping& buffer_mapping) { buffer_mapping_ = buffer_mapping; }
