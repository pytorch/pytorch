...
* Q: What is cherry-pick ?
  * A: A cherry pick is a process of propagating commits from the main into the release branch. Those commits are typically small fixes or documentation updates to ensure that the team has sufficient time to complete a thorough round of testing on a stable code base. To nominate fix for cherry-pick, separate pull request must be created against respective release branch and then mentioned in Release Tracker issue following the template from the issue description. That is  comment nominating cherry-pick for inclusion in release,  should include committed PR against main branch, cherry-pick PR and acceptance criteria. For example see [Release tracker for 2.0.0 release](https://github.com/pytorch/pytorch/issues/94937).
