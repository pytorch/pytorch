...
class UnaryFuzzer(ExtendedFuzzer):
    def __init__(self, seed, dtype, cuda):
        super(UnaryFuzzer, self).__init__(
            parameters=[
                # Dimensionality of x. (e.g. 1D, 2D, or 3D.)
                FuzzedParameter("dim", distribution={1: 0.3, 2: 0.4, 3: 0.3}, strict=True),

                # Shapes for `x`. (Values may be discarded if dim < 3)
                FuzzedParameterGroup(
                    names=["k0", "k1", "k2"],
                    minval=16, maxval=16 * 1024 ** 2,
                    distribution="loguniform",
                    constraint=self._limit_alloc_size,
                ),

                FuzzedParameterGroup(
                    names=["x_step_0", "x_step_1", "x_step_2"],
            ...
            tensors=[
                FuzzedTensor(
                    ...
                    dim_parameter="dim",
                    tensor_constructor=self._x_constructor,
                ),
            ],
            seed=seed,
            dtype=dtype,
