...
# Regex for identifying unimplemented ops for the given device.
#
# Example:
#   operator '<op-name>' is not implemented for <device>. Please file a feature request
_UNIMPLEMENTED_ERROR_TPL = re.compile(
    r"operator '.*' is not implemented for [a-zA-Z0-9]*\. Please file a feature request"
)
...
def _move_to_device(op, device):
    """Recursively convert all tensors and modules in op to the specified device."""
    import torch.utils._pytree as pytree

    device_str = str(device)

    # Calling nn.Module.to() for registered submodules/parameters/buffers
    op.to(device_str)

    def mapfn(tensor):
        dev_tensor = tensor.to(device_str).detach()
        dev_tensor.requires_grad_(tensor.requires_grad)
        return dev_tensor

    # Moving op.inputs (generated by op.init() call) to device.
    op.inputs = pytree.tree_map_only(torch.Tensor, mapfn, op.inputs)


def _device_is_available(device_type):
    if device_type == "mps":
        return torch.backend.mps.is_available()
...
def _get_module_name(
    ...
    """Retrieves the module name of `bench_op`.

    This is needed to filter operators correctly before expensive device
    ...
    # will return the correct module name (either set via `op.set_module_name()`
    # in `init()` or fallback to the class name).

    op = bench_op()
    init_dict = {}

    for attr in configs[0]:
        if "tags" not in attr:
            init_dict.update(attr)

    # Force initialization on CPU.
    # Avoid device overhead.
    if "device" in init_dict:
        init_dict["device"] = "cpu"

    # Needed for benchmarks that rely on auto_set() function.
...
def _is_native_device(device_type: str) -> bool:
...
def _build_test(
    ...
    OperatorTestCase,
    run_backward,
    op_name_function=None,
    *,
    force_device=None,
    force_init_on_cpu=False,
):
    """Generate PyTorch/Caffe2 tests of operators with different inputs.

    Args:
        configs: a dictionary that has the input shapes
        bench_op: the operator class to instantiate
        OperatorTestCase: a named tuple to save the metadata of an test
        run_backward: a bool parameter indicating backward path
        op_name_function: a dictionary includes operator name and function
        force_device: force all tests to run on this device
        force_init_on_cpu: force all inputs to be initialized on CPU, and then
    ...
    for config in configs:
        ...
        for attr in config:
            ...
            if "device" in attr:
                if force_device:
                    attr["device"] = force_device

                if not _device_is_available(attr["device"]):
                    keep_config = False
                    break
        ...
        # Deduplicate configurations to avoid executing identical configs when forcing devices
        config_key = tuple(sorted((k, str(v)) for k, v in test_attrs.items()))
        ...
        # Store device on op instance
        target_device = _extract_device(test_attrs)
        op.device = target_device
        ...
        # Move to the target_device, if initialized on CPU.
        if force_init_on_cpu:
            _move_to_device(op, target_device)
        ...
        for i in range(op._num_inputs_require_grads):
            ...
            if force_init_on_cpu:
                _move_to_device(new_op, target_device)
...
def _is_unimplemented_error(e: Exception, device_type: str) -> bool:
    if isinstance(e, NotImplementedError):
        return True

    if isinstance(e, RuntimeError):
        match = _UNIMPLEMENTED_ERROR_TPL.match(str(e))
...
class BenchmarkRunner:
    ...
    def __init__(self, args):
        ...
        self.operators = (
            benchmark_utils.process_arg_list(args.operators) if args.operators else None
        )
    ...
    def _foreach_supported_test_metainfo(self):
        for test_metainfo in BENCHMARK_TESTER:
            name = _get_module_name(*test_metainfo)
            if not (
                self._check_keep_list(name, self.operators)
                and self._check_skip(name, SKIP_OP_LISTS)
                and self._check_operator_first_char(name, self.operator_range)
            ):
                continue
            yield name, test_metainfo

    def _foreach_supported_test(self):
        for _, test_metainfo in self._foreach_supported_test_metainfo():
            yield from _build_test(
                *test_metainfo,
                force_device=self.args.force_device,
                force_init_on_cpu=self.args.force_init_on_cpu,
            )
    ...
    def _launch_forward(self, test_case, iters, print_per_iter):
        ...
        device_sync = test_case.op_bench.device.type != "cpu"
        ...
        if not device_sync:
            forward_time = timeit.timeit(
                functools.partial(func, iters, print_per_iter, device_sync), number=1
            )
            return forward_time
        timer = Timer(
            stmt="func(iters, print_per_iter, device_sync)",
            globals={
                "func": func,
                "iters": iters,
    ...
    def _launch_backward(self, test_case, iters, print_per_iter=False):
        ...
        """
        device_sync = test_case.op_bench.device.type != "cpu"
        test_case.run_forward(num_runs=1, print_per_iter=False, device_sync=device_sync)
        test_case._output_mean()

        timer = Timer(
            stmt="test_case.run_backward(iters, print_per_iter, device_sync)",
            globals={
                "test_case": test_case,
                "iters": iters,
                "print_per_iter": print_per_iter,
    ...
    def _keep_test(self, test_case):
        ...
        return (
            ...
            and self._check_keep_list(test_case.op_bench.module_name(), self.operators)
    ...
    def _run_impl(self):
        ...
        for test in self._foreach_supported_test():
            ...
            print(
                f"# Benchmarking {test_case.framework}:"
                f" {test_case.op_bench.module_name()}"
            )
            ...
            try:
                ...
                # Log unimplemented errors, but don't stop the execution.
                if _is_unimplemented_error(e, test_case.op_bench.device.type):
                    width = 80

                    header = f"=== FAILED: {test_case.op_bench.module_name()}"
                    print(f"\n{header:=<{width}}", flush=True)

                    print(f"Test Config: {op_test_config}", flush=True)
            ...
            # Clean up test case to avoid OOM.
            del test_case
            del test
            gc.collect()
    ...
    def run(self):
        self._print_header()
        if self.args.list_tests:
            for _, test_case in self._foreach_supported_test():
                print(f"# {test_case.test_config.test_name}")
        elif self.args.list_ops:
            for op_name, _ in self._foreach_supported_test_metainfo():
                print(f"# {op_name}")
        else:
            self._run_impl()
