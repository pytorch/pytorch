  ...
  python benchmark.py softmax_forward softmax_backward  # Run multiple benchmarks
    ...
    if args.custom_compile_options:
        ...
        try:
            ...
            raise RuntimeError(f"Invalid json string for --custom-compile-options: {args.custom_compile_options}")

        if not args.custom_compile_options:
            raise RuntimeError(f"Found no options for --custom-compile-options")
