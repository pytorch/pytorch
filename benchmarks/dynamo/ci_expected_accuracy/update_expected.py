...
# graph_breaks is always present; the coverage metrics
# (calls_captured/unique_graphs/fallbacks_to_eager) appear only once the
# benchmark run's get_dynamo_stats writes them, and check_graph_breaks.py skips
# any metric absent from the baseline, so older CSVs stay valid.
METRIC_COLUMNS = [
    "graph_breaks",
    "calls_captured",
    "unique_graphs",
