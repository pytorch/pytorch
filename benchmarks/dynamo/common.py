...
from torch.dynamo.profiler import Profiler
from torch.dynamo.profiler import fx_insert_profiling
from torch.dynamo.testing import dummy_fx_compile
from torch.dynamo.testing import format_speedup
...
class Stats:
    ...
    def reset_counters(cls):
        for k, v in torch.dynamo.utils.counters.items():
            cls.totals[k].update(v)
        ok = torch.dynamo.utils.counters["frames"]["ok"]
        total = torch.dynamo.utils.counters["frames"]["total"]
...
def coverage_experiment(args, model_iter_fn, model, example_inputs):
    ...
    """
    profiler = Profiler()
    frozen_model_iter_fn = torch.dynamo.run(model_iter_fn)
    with profiler.prof:
...
def recompile_profiler_experiment(args, model_iter_fn, model, example_inputs):
    prof = torch.dynamo.utils.CompileProfiler()
    opt_model_iter_fn = torch.dynamo.optimize(prof, nopython=args.nopython)(
        model_iter_fn
    )
...
def randomize_input(inputs):
    if isinstance(inputs, (list, tuple)):
        ...
        if inputs.dtype in (torch.float32, torch.float64):
            torch.dynamo.utils.counters["randomize_input"]["times"] += 1
...
def speedup_experiment(args, model_iter_fn, model, example_inputs, **kwargs):
    ...
    if args.export_profiler_trace:
        ...
        name = os.path.join(torch.dynamo.config.base_dir, name)
    ...
    headers, data = torch.dynamo.utils.compile_times(repr="csv", aggregate=True)
    assert (
    ...
    def run_performance_test(
        ...
        with self.pick_grad(name, self.args.training):
            ...
            if experiment.func is coverage_experiment:
                ok, total = Stats.reset_counters()
                results = []
                # run with torch.dynamo few times to populate the cache
                for _ in range(3):
...
def parse_args():
    ...
    parser.add_argument(
        ...
        action="store_true",
        help="Fail a benchmark if torch.dynamo triggers an internal assertion",
    )
    parser.add_argument(
...
def main(runner, original_dir=None):
    ...
    if args.verbose:
        torch.dynamo.config.log_level = logging.DEBUG

    if args.quiet:
        torch.dynamo.config.log_level = logging.ERROR

    torch.dynamo.config.raise_on_assertion_error = args.raise_on_assertion_error
    torch.dynamo.config.raise_on_backend_error = args.raise_on_backend_error
    ...
    if args.overhead:
        optimize_ctx = torch.dynamo.optimize(dummy_fx_compile, nopython=args.nopython)
        experiment = speedup_experiment
        output_filename = "overheads.csv"
    elif args.cold_start:
        optimize_ctx = torch.dynamo.optimize("aot_nvfuser", nopython=args.nopython)
        ...
        torch.inductor.config.debug = args.verbose
        if args.threads:
            torch.inductor.config.cpp.threads = args.threads

        if args.inductor_dynamic:
            torch.inductor.config.triton.cudagraphs = False
            torch.inductor.config.dynamic_shapes = True
        else:
            torch.inductor.config.dynamic_shapes = False
            if args.export_profiler_trace:
                print("Profiling requested, setting cudagraphs to False")
                torch.inductor.config.triton.cudagraphs = False

        optimize_ctx = torch.dynamo.optimize("inductor", nopython=args.nopython)
        experiment = speedup_experiment
        output_filename = "inductor.csv"
    elif args.speedup_ltc:
        optimize_ctx = torch.dynamo.optimize(
        ...
        optimize_ctx = torch.dynamo.optimize(backends.ts, nopython=args.nopython)
        experiment = speedup_experiment
        ...
        optimize_ctx = torch.dynamo.optimize("prims_nvfuser", nopython=args.nopython)
        experiment = speedup_experiment
        ...
        optimize_ctx = torch.dynamo.optimize(args.backend, nopython=args.nopython)
        experiment = speedup_experiment
        if args.accuracy:
        ...
        optimize_ctx = torch.dynamo.optimize(conv_args_analysis, nopython=args.nopython)
        output_filename = "log_conv_args.csv"
    elif args.recompile_profiler:
        output_filename = "recompile_profiler_log.csv"
        experiment = recompile_profiler_experiment
    else:
        optimize_ctx = torch.dynamo.optimize(fx_insert_profiling, nopython=args.nopython)
    ...
    if output_filename:
        output_filename = os.path.join(torch.dynamo.config.base_dir, output_filename)
