...
def coverage_experiment(args, model_iter_fn, optimize_ctx, model, example_inputs):
    """
...
def speedup_experiment_fx2trt(args, model_iter_fn, optimize_ctx, model, example_inputs):
    """
    Measure speedups over eager using the trt inference backend. TRT backend is based fx graph
    generated by torch._dynamo.
    Writes to ./speedups_fx2trt.csv
    """
    return speedup_experiment(args, model_iter_fn, optimize_ctx, model, example_inputs)


# TODO: CompilerProfiler is deprecated, remove this
def recompile_profiler_experiment(args, model_iter_fn, optimize_ctx, model, example_inputs):
...
def speedup_experiment(args, model_iter_fn, model, optimize_ctx, example_inputs, **kwargs):
    """
...
def speedup_experiment_ds(args, model_iter_fn, optimize_ctx, model, example_inputs):
    """
...
def overhead_experiment(*args, model_iter_fn, optimize_ctx):
    """
    ...
    """
    return speedup_experiment(*args, model_iter_fn, optimize_ctx)
...
def null_experiment(args, model_iter_fn, optimize_ctx, model, example_inputs):
    """
...
def run(runner, args, original_dir=None):
    ...
    if (
        args.devices == ["cuda"] and not is_hpu()
    ...
    experiment = functools.partial(experiment, args, runner.model_iter_fn, optimize_ctx)
