...
def compare_op():
    for benchmarking_space in ["SAME"]:
        for op in [aten.mm.default]:
            for dtype in [torch.float16]:
                print(f"START OF {benchmarking_space}", flush=True)
                ...
                with open(f"new_old_config_compare_{op}_{dtype}.csv", 'w', newline='') as file:
                    ...
                    # for i, (args, kwargs) in enumerate(loader.get_inputs_for_operator(op, dtype=dtype, device="cuda")):
                        ...
                        print("benchmarking_space", benchmarking_space, flush=True)
                        with open(f"{benchmarking_space}_{op}_{dtype}_benchmark_results.csv", "a") as file2:
