...
//!  This utility helps create and manage FusionExecutorCaches and tries to use the caching
//! mechanism in NVFuser to avoid re-compilation.
//!
//!  There are two macros in this utility: NVFUSER_BENCHMARK_DEFINE, and NVFUSER_BENCHMARK_RUN,
//! and user needs to supply two functions SETUP_FUSION and RUN_FUSION, with following signatures:
//!
//!  SETUP_FUSION(Fusion* , args...);
//!  RUN_FUSION(benchmark::State&, FusionExecutorCache* , args...);
//!
//!  where args... are additional arguments, and they need to be the same for SETUP_FUSION and
//!  RUN_FUSION.
//!
//!  SETUP_FUSION is called once in each definition of benchmark to build the fusionIR graph
//!
//!  RUN_FUSION is just like the normal benchmark instance, except that a FusionExecutorCache
//!   will be provided for scheduling, running and timing the fusion runs. It is called
//!   once in each benchmark instance. For example:
...
//!
//!  each NVFUSER_BENCHMARK_DEFINE registers a benchmark with a single FusionExecutorCache,
//!  i.e. a single fusion graph, and multiple benchmark data points can be registered like:
//!
//!  NVFUSER_BENCHMARK_RUN(my_benchmark)
//!    ->Ranges({{1,2}});
//!
...
//!    ->Ranges({{3,4}});
//!
//!  All datapoints will use the same FusionExecutorCache so recompilation is avoided as much as possible.

#define NVFUSER_BENCHMARK_DEFINE(BENCHMARK_NAME, SETUP_FUSION, RUN_FUSION, ...)                           \
  class BENCHMARK_NAME##___GRAPH : public BenchmarkGraph {                                                \
    public:                                                                                               \
      std::string graphName () {return NVFUSER_TO_STRING(BENCHMARK_NAME##___GRAPH);}                      \
      SetupFusionFunction setupFusion (){                                                                 \
        return [](Fusion* fusion){                                                                        \
          SETUP_FUSION(fusion,__VA_ARGS__);                                                               \
        };                                                                                                \
      }                                                                                                   \
    };                                                                                                    \
  BENCHMARK_DEFINE_F(BENCHMARK_NAME##___GRAPH, BENCHMARK_NAME)(benchmark::State& benchmark_state) {       \
    RUN_FUSION(benchmark_state, BENCHMARK_NAME##___GRAPH::getExecutorCache(), __VA_ARGS__);               \
  }

#define NVFUSER_BENCHMARK_RUN(BENCHMARK_NAME) BENCHMARK_REGISTER_F(BENCHMARK_NAME##___GRAPH, BENCHMARK_NAME)