...
static void setup_vit_base_patch16_224_norm_inner2(Fusion* fusion, void* null) {
  FusionGuard fg(fusion);
  ...
  auto t2 = mul(t1, d13);
  auto t3 = max(t2, {3});
  auto t4 = broadcast(t3, {false, false, false, true});
  auto t5 = sub(t2, t4);
  auto t6 = exp(t5);
  auto t7 = sum(t2, {3});
  auto t8 = broadcast(t7, {false, false, false, true});
  auto t9 = reciprocal(t8);
  auto t10 = mul(t6, t9);
  auto t11 = randlike(t10);
  auto d59 = sub(IrBuilder::create<Double>(1), IrBuilder::create<Double>(0));
  ;
  auto t12 = lt(t10, d59);
  auto t13 = castOp(DataType::Float, t12);
  auto t14 = mul(t10, t13);
  auto b61 = eq(d59, IrBuilder::create<Double>(0));
  auto d62 = castOp(DataType::Float, b61);
  auto d63 = add(d62, d59);
  auto d65 = div(IrBuilder::create<Double>(1), d63);
  auto t15 = mul(t14, d65);
  auto t16 = castOp(DataType::Half, t15);
  auto t17 = castOp(DataType::Half, t10);
  auto t18 = castOp(DataType::Half, t2);

  fusion->addOutput(t16);
  fusion->addOutput(t12);
  fusion->addOutput(t17);
  fusion->addOutput(t18);
}

static void NvFuserScheduler_TIMM_vit_base_patch16_224_norm_inner2(
    benchmark::State& benchmark_state,
    FusionExecutorCache* fusion_executor_cache,
    void* null) {
  std::vector<int64_t> input_shape{
  ...
  // Full tensors - halfx4, bool - t12, t4, t0, t19, t14
  benchmark_state.SetBytesProcessed(
      int64_t(benchmark_state.iterations()) * t0.numel() * (4 * 2 + 1));
}

NVFUSER_BENCHMARK_DEFINE(
    NvFuserScheduler_TIMM_NCHW_vit_base_patch16_224_norm_inner2,
    setup_vit_base_patch16_224_norm_inner2,
    NvFuserScheduler_TIMM_vit_base_patch16_224_norm_inner2,
    nullptr);
