...
from torch.profiler import _cupti_monitor as cupti_monitor
...
def make_workload(args):
    if args.workload == "mixed":
        graph = build_mixed_graph(args.mixed_groups, args.tensor_size, args.sleep_cycles)
    else:
...
def run_profiled_step(
    ...
    samples: int,
    *,
    sync_before_final_step: bool = False,
):
    active_times = []
    warmup_times = []
    exit_times = []
...
def main():
    ...
    parser.add_argument("--workload", choices=["mixed", "multistream_mixed"], default="multistream_mixed")
    ...
    parser.add_argument("--sync-before-final-step", action="store_true")
    ...
    if args.mode == "baseline":
        ...
        result["profiled"] = run_profiled_step(
            ...
            args.mode,
            args.samples,
            sync_before_final_step=args.sync_before_final_step,
        )
