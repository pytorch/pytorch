...
if __name__ == '__main__':
    for workload_name in ["loop", "parallel"]:
        print("Payload: {}; {} iterations, N = {}\n".format(workload_name, INTERNAL_ITER, N))
        for params in itertools.product(TENSOR_SIZES, [False, True]):
            ...
            print("Profiling {}, tensor size {}x{}".format(
                "enabled " if profiling_enabled else "disabled", profiling_tensor_size, profiling_tensor_size))
            ...
            if workload_name == "loop":
                ...
                traced_parallel_workload = torch.jit.trace(parallel_workload, x)
            ...
            print("\tavg. time: {:.3f} ms, stddev: {:.3f} ms ;  time per iteration: {:.3f} ms\n".format(avg_time, stddev_time, avg_time/INTERNAL_ITER))
