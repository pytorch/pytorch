...
def benchmark_compilation(kernel_source, kernel_name, use_pch=False, iterations=10):
    """Benchmark NVRTC compilation time."""
...
def main():
    ...
    parser.add_argument("--iterations", type=int, default=10,
                        help="Number of compilation iterations")
    ...
    # Test kernel 2: Complex kernel with multiple CUB includes
    kernel2_source = """
    #include <cub/block/block_reduce.cuh>
    ...
    __global__ void complex_cub_kernel(const float* input, float* output, int n) {
        ...
        float warp_max = WarpReduce(temp_storage.warp[warp_id]).Reduce(val, cub::Max());
        
        if (threadIdx.x == 0) {
            output[blockIdx.x] = sum + scan_result + warp_max;
        }
    }
    """
    ...
    # Without PCH
    print("Compiling without PCH...", end="", flush=True)
    times_no_pch = benchmark_compilation(kernel1_source, "block_reduce_kernel", 
                                        use_pch=False, iterations=args.iterations)
    print(" done")
    
    if times_no_pch:
        avg_no_pch = mean(times_no_pch) * 1000  # Convert to ms
        std_no_pch = stdev(times_no_pch) * 1000 if len(times_no_pch) > 1 else 0
        print(f"  Average: {avg_no_pch:.2f} ms (±{std_no_pch:.2f} ms)")
    
    # With automatic PCH (if supported)
    if pch_supported:
        print("Compiling with automatic PCH...", end="", flush=True)
        times_with_pch = benchmark_compilation(kernel1_source, "block_reduce_kernel", 
                                              use_pch=True,
    ...
    print()
    print("Benchmark 2: Complex Kernel with Multiple CUB Headers")
    print("-"*40)
    ...
    if pch_supported:
        print("Compiling with automatic PCH...", end="", flush=True)
        times_with_pch = benchmark_compilation(kernel2_source, "complex_cub_kernel", 
                                              use_pch=True,
                                              iterations=args.iterations)
        print(" done")
