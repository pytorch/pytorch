...
#define INIT_DISPATCH_PTR(OP)    \
  do {                           \
    int i;                       \
    for (i = 0; i < sizeof(THZVector_(OP ## _DISPATCHTABLE)) / sizeof(FunctionDescription); ++i) { \
      THZVector_(OP ## _DISPATCHPTR) = THZVector_(OP ## _DISPATCHTABLE)[i].function;                     \
      if (THZVector_(OP ## _DISPATCHTABLE)[i].supportedSimdExt & hostSimdExts) {                       \
        break;                                                                                     \
      }                                                                                            \
    }                                                                                              \
  } while(0)
