...
void fractional_max_pool3d_out_cpu_template(
  ...
  TORCH_CHECK(ndims == 4 || ndims == 5,
              "fractional_max_pool3d_out(): Expected non-empty 4D or 5D tensor with optional 0-dim batch size, but got: ",
              input_.sizes());
  for (int64_t i = 1; i < ndims; ++i) {
    TORCH_CHECK(input_.size(i) > 0,
                "fractional_max_pool3d_out(): Expected input to have non-zero dimensions for non-batch dimensions, but got",
