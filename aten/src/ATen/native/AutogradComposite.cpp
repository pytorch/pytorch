...
/// This function can be used to unpack a given dual Tensor to get its primal
/// and tangent. The returned primal is a view of the dual and the tangent is
/// returned as is. This function is backward differentiable.
std::tuple<at::Tensor, at::Tensor> _unpack_dual(
...
Tensor _new_zeros_with_same_feature_meta(
  ...
  if (self_num_batch_dims == 0) {
    ...
    return new_tensor.as_strided(
        other_sizes, other_strides, other_storage_offset);
  }
  ...
  //     Also see the test test_inplace_on_view_not_same_layout, for when we
  //     reach this case.
  constexpr int64_t kSmallBufferSizeHint = 8;

  auto out_sizes = c10::SmallBuffer<int64_t, kSmallBufferSizeHint>(
      other.dim() + self_num_batch_dims);
  std::copy(
      self_sizes.begin(),
      self_sizes.begin() + self_num_batch_dims,
      out_sizes.begin());
  std::copy(
      other_sizes.begin(),
      other_sizes.end(),
  ...
  auto out_strides = c10::SmallBuffer<int64_t, kSmallBufferSizeHint>(
      other.dim() + self_num_batch_dims);
...
bool _has_same_storage_numel(const at::Tensor& base, const at::Tensor& other) {
  return base.storage().nbytes() / base.itemsize() ==
      other.storage().nbytes() / other.itemsize();
}
