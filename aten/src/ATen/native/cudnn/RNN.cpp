...
Tensor _cudnn_rnn_flatten_weight(
    const ITensorList& weight_list, int64_t weight_stride0,
    int64_t input_size,
...
std::tuple<Tensor, Tensor, Tensor, Tensor, Tensor> _cudnn_rnn(
    ...
    const ITensorList& weight_list, int64_t weight_stride0, const c10::optional<Tensor>& weight_buf_r_opt, const Tensor& hx, const c10::optional<Tensor>& cx_opt,
    int64_t fn_mode, int64_t fn_hidden_size, int64_t fn_proj_size,
...
std::tuple<Tensor, Tensor, Tensor, std::vector<Tensor>> _cudnn_rnn_backward(
    const Tensor& input, const ITensorList& weight_list, int64_t weight_stride0, const Tensor& weight_buf, const Tensor& hx, const c10::optional<Tensor>& cx_opt,
...
Tensor _cudnn_rnn_flatten_weight(
  ...
  return std::get<0>(copy_weights_to_flat_buf_views(
      ...
      batch_first,
      fn_bidirectional,
      /*flat_buf_datatype=*/getCudnnDataType(weight_list[0]),
      /*flat_buf_options=*/weight_list[0].options(),
...
std::tuple<Tensor, Tensor, Tensor, Tensor, Tensor> _cudnn_rnn(
  ...
  check_attributes(input_r, weight_list, {hx, cx}, /*check_dtype=*/true);
  auto input = input_r;
...
std::vector<Tensor> _cudnn_rnn_backward_weight(
    ...
    const Tensor& input_r, const ITensorList& weight_list, int64_t weight_stride0,
...
std::tuple<Tensor, Tensor, Tensor, std::vector<Tensor>> _cudnn_rnn_backward(
  ...
  if (!grad_output_r.defined() && !grad_hy_r.defined() && !grad_cy_r.defined()) {
    return std::tuple<Tensor, Tensor, Tensor, std::vector<Tensor>>(Tensor(), Tensor(), Tensor(), std::vector<Tensor>(weight_list.size()));
  }
  ...
  if (output_mask[3]) {
    dw = at::native::_cudnn_rnn_backward_weight(input, weight_list, weight_stride0, weight_buf, hx, cx, output, mode, hidden_size, proj_size, num_layers, batch_first, dropout, train, bidirectional, batch_sizes, dropout_state, reserve);
  }
...
Tensor try_get_weight_buf(
      const Tensor& input, ITensorList parameters, bool has_biases,
...
std::pair<Tensor, hidden_type> _cudnn_impl(
      ...
      ITensorList params, bool has_biases, cudnnRNNMode_t mode,
