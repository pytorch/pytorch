...
cudnn_frontend::Tensor getTensorDescriptor(
  ...
  return getTensorDescriptorWithTypeVirtual(
      t, id, alignment, getCudnnDataType(t), false);
}
...
struct BenchmarkCache {
  ...
  cudnn_frontend::ExecutionPlan* find(const KeyType& key) {
    ...
    // TODO: probably want ExecutionPlan copy constructor or better way to
    // return
    return &(it->second);
  }
...
BenchmarkCache<cudnn_frontend::ExecutionPlan, CacheKeyFused>
    benchmark_cache_fused;

} // namespace

// NB: This (and the fused version) can't be a constructor, because then
// CacheKey would not be a POD anymore.
void setCacheKey(
    CacheKey& key,
...
auto build_opgraph(
    ...
    const IntArrayRef dilation) {
  auto op =
      cudnn_frontend::OperationBuilder(desc)
          ...
          .setcDesc(getConvDescriptor(
              key.params.dataType, padding, stride, dilation, x.scalar_type()))
          .build();
...
auto build_opgraph_fused(
  ...
  auto conv_op = cudnn_frontend::OperationBuilder(
                     ...
                     .setyDesc(getTensorDescriptorWithTypeVirtual(
                         y, 'C', key.y_alignment, precision, true))
                     .setwDesc(getTensorDescriptor(w, 'w', key.w_alignment))
                     .setAlpha(alpha1)
                     .setcDesc(convDesc)
                     .build();
  auto add_op = cudnn_frontend::OperationBuilder(
                    ...
                    .setyDesc(getTensorDescriptorWithTypeVirtual(
                        y, 'A', key.y_alignment, precision, true))
                    .setpwDesc(addDesc)
                    .setAlpha(alpha1)
                    .setAlpha2(alpha2)
                    .build();
  auto add_bias_op = cudnn_frontend::OperationBuilder(
                         CUDNN_BACKEND_OPERATION_POINTWISE_DESCRIPTOR)
                         .setxDesc(add_op.getOutputTensor())
                         .setbDesc(getTensorDescriptor(b, 'b', key.b_alignment))
                         // another virtual output (of add bias)
                         .setyDesc(getTensorDescriptorWithTypeVirtual(
                             y, 'B', key.y_alignment, precision, true))
...
auto get_generator_sources(
  ...
  auto heurgen_method = [/*&desc,*/ &x, deterministic, allow_tf32, heur_mode](
  ...
  auto fallback_method = [&desc, &x, deterministic, allow_tf32](
                             cudnn_frontend::OperationGraph& opGraph)
      -> cudnn_frontend::EngineConfigList {
...
auto get_plans_from_find(
  ...
  auto sources = get_generator_sources(
      desc, x, deterministic, allow_tf32, CUDNN_HEUR_MODE_INSTANT);
  cudnn_frontend::EngineConfigGenerator generator(
  ...
  auto plans = cudnn_frontend::time_sorted_plan<
      ...
      handle, std::move(valid_plans), variantPack);
...
auto get_configs_from_heuristics(
  ...
  auto heuristic_mode = at::native::cudnnv8_use_heur_mode_b()
      ...
      : CUDNN_HEUR_MODE_INSTANT;
  auto sources =
      get_generator_sources(desc, x, deterministic, allow_tf32, heuristic_mode);
