...
Tensor cudnn_grid_sampler_forward(const Tensor& input_t, const Tensor& grid_t) {
  TORCH_CHECK(false, "cudnn_grid_sampler_forward: ATen not compiled with cuDNN support");
}
...
std::tuple<Tensor, Tensor> cudnn_grid_sampler_backward(
  ...
  TORCH_CHECK(false, "cudnn_grid_sampler_backward: ATen not compiled with cuDNN support");
}
