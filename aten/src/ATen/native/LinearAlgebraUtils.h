...
static inline c10::MaybeOwned<Tensor> expect_resolved_conj(
    const Tensor& tensor) {
  if (tensor.is_conj()) {
 ...
 */
static inline c10::MaybeOwned<Tensor> borrow_else_clone(
    const bool cond,
    const Tensor& borrow,
    const Tensor& clone,
    const bool contig) {
 ...
 *  broadcasted shape.
 */
static inline Tensor copyBatchedColumnMajor(
    const Tensor& src,
    int64_t nrows = -1,
  ...
  const auto copy_strides =
      batched_matrix_contiguous_strides(copy_sizes, /*f-contig*/ true);
...
// cholesky, symeig, eig)
static inline void checkIsMatrix(
    const Tensor& A,
...
static inline void checkInputsSolver(
    const Tensor& A,
    const Tensor& B,
    const bool left,
    const char* const f_name) {
  ...
  TORCH_CHECK(
      left ? A.size(-2) == B.size(-2) : A.size(-1) == B.size(-1),
      f_name,
      ": Incompatible shapes of A and B for the equation ",
      left ? "AX = B" : "XA = B",
      " (",
      A.size(-2),
      "x",
...
static inline bool is_row_or_column_contiguous(const Tensor& t) {
  // This could be made more general, similar to how it's checked in matmul,
  // which would allow to ellide the copy with strides such as (6, 12, 1, 3) or
  // (3, 1, 9), but this is quite tricky. We choose to be conservative for
  // simplicity
  return t.is_contiguous() || t.transpose(-2, -1).is_contiguous();
}

static inline TransposeType to_transpose_type(
    const bool contig,
    const bool conj) {
  if (conj) {
    if (contig) {
...
// This function is designed to be used with linear algebra methods that
// minimize L(ax - b) = 0, where L is generally the identity map (`solve`, for
// example) or the L2 norm (`lstsq`). It is expected that `a` and `b` are
// contiguous tensors of column-major matrices (so that a.view({-1, a.size(-2),
// a.size(-1)}) succeeds, same for `b`), with the following additional
// properties:
//
// 1. a.dim() == b.dim()
// 2. a.shape[:-2] broadcasts over b.shape[:-2]
// 3. a.size(i) <= b.size(i) for i=0,..., a.dim() - 3 (only for batch
// dimensions)
//
// MAGMA/LAPACK modify tensor `a` in-place, and the main goal of this method
// is to be memory efficient, which means that if there exists an index i such
// that a.shape[i] < b.shape[i], 0 <= i <= a.dim() - 3, then instead of
// materializing copies of `a` in the broadcasted shape, we keep a buffer copy
// of `a` along with flags that check whether specific batch dimension indices
// for `a` were already accessed. If they were, we copy the data from the buffer
...
// a_working_ptr and b_working_ptr can directly be passed to LAPACK/MAGMA
// routines, and a_linear_batch_idx is an index in the 3d representation which
// corresponds to the memory a_working_ptr points to, in other words:
// a_working_ptr == a.view({-1, a.size(-2), a.size(-1)}.select(0,
// a_linear_batch_idx).data_ptr<scalar_t>(); a_linear_batch_idx is useful to
// store metadata related to `a`, such as, for example, its rank or singular
// values (see linalg_lstsq).
template <typename scalar_t, typename func_t>
void batch_iterator_with_broadcasting(
    const Tensor& a,
    const Tensor& b,
    const func_t& f) {
  ...
  if (a_broadcasts_over_b) {
    a_buffer = at::empty_strided(a.sizes(), a.strides(), a.options()).copy_(a);
    ...
    check_if_copy_needed_for_a = [&](int64_t a_curr_linear_batch_idx) {
      auto* a_was_accessed_flag =
          a_was_accessed.select(0, a_curr_linear_batch_idx).data_ptr<bool>();
  ...
  auto loop = [&](char** data, const int64_t* strides, int64_t nelems) {
    ...
    for (const auto elem : c10::irange(nelems)) {
      ...
      auto a_curr_linear_batch_idx =
          *reinterpret_cast<int64_t*>(a_batch_idx_ptr);

      check_if_copy_needed_for_a(a_curr_linear_batch_idx);

      auto* a_working_ptr =
          a_3d.select(0, a_curr_linear_batch_idx).data_ptr<scalar_t>();
      auto* b_working_ptr =
          b_3d.select(0, b_curr_linear_batch_idx).data_ptr<scalar_t>();
...
static inline void checkFloatingOrComplex(
    const Tensor& t,
    const char* const f_name) {
  TORCH_CHECK(
      (at::isFloatingType(t.scalar_type()) ||
       at::isComplexType(t.scalar_type())),
      f_name,
 ...
 * the latter.
 */
static inline void singleCheckErrors(
    int64_t info,
    const c10::string_view name,
    int64_t batch_id = -1) {
  ...
  if (info < 0) {
    // Reference LAPACK 3.10+ changed `info` behavior for inputs with non-finite
    // values Previously, it would return `info` > 0, but now it returns `info`
    // = -4 OpenBLAS 0.3.15+ uses the Reference LAPACK 3.10+. MKL 2022.0+ uses
    // the Reference LAPACK 3.10+. Older version of MKL and OpenBLAS follow the
    // old behavior (return `info` > 0). Here we check for the case where `info`
    // is -4 and raise an error
    ...
    if (name.find("inv") != name.npos) {
      ...
      TORCH_CHECK_LINALG(
          ...
          info,
          " is zero, the inversion could not be completed because the input matrix is singular.");
      ...
      TORCH_CHECK_LINALG(
        ...
        name.find("eig") != name.npos || name.find("syevd") != name.npos) {
      TORCH_CHECK_LINALG(
          false,
          name,
          batch_string,
          ": The algorithm failed to converge because the input matrix is ill-conditioned or has too many repeated eigenvalues (error code: ",
          info,
          ").");
      ...
      TORCH_CHECK_LINALG(
          ...
          name,
          batch_string,
          ": The least squares solution could not be computed because the input matrix does not have full rank (error code: ",
          info,
          ").");
      ...
      TORCH_CHECK(
          ...
          ",",
          info,
          "] is zero and using it on lu_solve would result in a division by zero. "
...
static inline void batchCheckErrors(
    const std::vector<int64_t>& infos,
    const c10::string_view name) {
...
_linalg_broadcast_batch_dims(const Tensor& arg1, const Tensor& arg2) {
  ...
  return std::make_tuple(
      std::move(arg1_expand_size), std::move(arg2_expand_size));
}

static inline std::tuple<Tensor, Tensor> _linalg_broadcast_batch_dims(
    const Tensor& arg1,
    const Tensor& arg2,
    const char* name) {
  ...
  std::tie(arg1_expand_size, arg2_expand_size) =
      at::native::_linalg_broadcast_batch_dims(arg1, arg2);

  auto arg1_broadcasted =
      arg1_expand_size == arg1.sizes() ? arg1 : arg1.expand(arg1_expand_size);
  auto arg2_broadcasted =
      arg2_expand_size == arg2.sizes() ? arg2 : arg2.expand(arg2_expand_size);
  return std::make_tuple(arg1_broadcasted, arg2_broadcasted);
}

static inline std::vector<int64_t> broadcast_batch_size(
    const Tensor& t1,
    const Tensor& t2,
    int64_t n_batch_dims) {
...
// parse the "mode" param in linalg_qr: return a tuple of bools (compute_q,
// reduced)
...
// Creates a dimension permutation array that can be given to `at::permute()`,
// which will shift the two specified dimensions to the end of a tensor, without
// changing the order of the other dimensions. `dim1` will be placed at the very
// end, and `dim0` will be placed just to the left of it.
//
// For instance, given a 4-D tensor, dimensions 1 and 3 can be shifted to the
// end by calling `create_dim_backshift_permutation(1, 3, 4)`. The resulting
// vector will be `vec(0, 2, 1, 3)`.
static inline std::vector<int64_t> create_dim_backshift_permutation(
    int64_t dim0,
    int64_t dim1,
    int64_t ndim) {
  TORCH_CHECK(
      (dim0 != dim1) && (dim0 < ndim) && (dim0 >= 0) && (dim1 < ndim) &&
          (dim1 >= 0),
...
// Creates a dimension permutation array that can be given to `at::permute()`,
// which will reverse a given permutation. The reverse permutation array is
// created by swapping the indices and their associated values from the given
// permutation array.
static inline std::vector<int64_t> create_reverse_permutation(
...
// See
// https://github.com/Reference-LAPACK/lapack/blob/122506cd8b6ce050a200920c3d4c0b153b150fd8/SRC/cgesdd.f#L186
static inline int64_t computeLRWorkDim(const char jobz, int64_t m, int64_t n) {
  auto mn = std::min(m, n);
  auto mx = std::max(m, n);
  if (jobz == 'N') {
#ifdef __APPLE__
    // According to `vecLib.framework/Headers/clapack.h` Accelerate.framework is
    // based on LAPACK 3.2.1
    return 7 * mn;
#else
...
static inline void checkUplo(const c10::string_view uplo) {
  // To use std::toupper safely with plain chars (or signed chars), the argument
  // should first be converted to unsigned char
  char uplo_uppercase =
      static_cast<char>(std::toupper(static_cast<unsigned char>(uplo[0])));
  TORCH_CHECK(
      uplo.size() == 1 && (uplo_uppercase == 'U' || uplo_uppercase == 'L'),
      "Expected UPLO argument to be 'L' or 'U', but got ",
      uplo);
}

static inline void checkSameDevice(
...
// (either floating or complex type input), so we can check whether input's
// dtype can be casted to result's dtype. According to
...
static inline void checkLinalgCompatibleDtype(
    const std::string& fn_name,
    Tensor result,
    Tensor input,
    const std::string& result_name = "result") {
...
// Alternatively, we can check whether the specific expected output type
// (result_type) can be safely casted to out tensor dtype (out_type)
static inline void checkLinalgCompatibleDtype(
    const std::string& fn_name,
    ScalarType out_type,
    ScalarType result_type,
    const std::string& out_name = "result") {
...
static inline void checkNotComplexTolerance(
    const Tensor& tol,
    const c10::string_view f_name,
    const c10::string_view tol_name) {
  TORCH_CHECK(
      !at::isComplexType(tol.scalar_type()),
      f_name,
      ": ",
      tol_name,
      " tensor of complex type is not supported. Got ",
      tol.scalar_type());
}
...
/*
  ...
  * 2-dimensional (2D) tensor or batch of 2D tensors (matrix case).
  The original torch.solve supported only the matrix case, while NumPy works for
  both cases. For the batched input we need to be able to distinguish them. Let
  input.shape = (batch_dimensions, m, n), then 'other' is of vector type if
  other.shape == (batch_dimensions, m). This rule is compatible with NumPy, see
  https://github.com/numpy/numpy/blob/v1.20.0/numpy/linalg/linalg.py#L384-L389
*/
static inline bool linalg_solve_is_vector_rhs(
    const Tensor& input,
    const Tensor& other) {
  auto expected_batched_rhs_shape =
      IntArrayRef(input.sizes().data(), input.dim() - 1); // input.shape[:-1]
  bool vector_case = other.dim() == 1 ||
...
/*
  Computes linear indices for a tensor with original_shape to access its
  elements like it was a materialized broadcast tensor.
*/
static inline Tensor get_linear_indices(
    int64_t numel,
  ...
  return at::arange(numel, options)
      .view(original_shape)
