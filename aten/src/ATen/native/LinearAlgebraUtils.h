...
_compute_geometry_for_Q(const Tensor& input, bool reduced) {
...
static inline std::tuple<Tensor, Tensor, Tensor> _create_U_S_VT(
  ...
  Tensor U_empty =
      at::empty_strided(sizes, strides, input.options().device(usvt_device));
  U_empty.zero_();
  ...
  Tensor S_empty =
      at::empty(sizes, input.options().dtype(dtype).device(usvt_device));
