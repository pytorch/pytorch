...
inline const std::complex<T>* tensor_cdata(const Tensor& t) {
  return reinterpret_cast<const std::complex<T>*>(
      t.const_data_ptr<c10::complex<T>>());
}
...
Tensor _fft_c2r_mkl(
  ...
  if (self.scalar_type() == kComplexFloat) {
    pocketfft::c2r(
        shape_from_tensor(out),
        stride_from_tensor(self),
        stride_from_tensor(out),
        axes,
        false,
        tensor_cdata<float>(self),
        out.data_ptr<float>(),
        compute_fct<float>(out, dim, normalization));
  } else {
    pocketfft::c2r(
        shape_from_tensor(out),
        stride_from_tensor(self),
        ...
        out.data_ptr<double>(),
        compute_fct<double>(out, dim, normalization));
  }
  return out;
}
...
Tensor _fft_r2c_mkl(
  ...
  if (self.scalar_type() == kFloat) {
    pocketfft::r2c(
        shape_from_tensor(self),
        stride_from_tensor(self),
        stride_from_tensor(out),
        axes,
        true,
        self.const_data_ptr<float>(),
        tensor_cdata<float>(out),
        compute_fct<float>(self, dim, normalization));
  } else {
    pocketfft::r2c(
        shape_from_tensor(self),
        stride_from_tensor(self),
        ...
        tensor_cdata<double>(out),
        compute_fct<double>(self, dim, normalization));
  }
...
Tensor _fft_c2c_mkl(
  ...
  // MKL has a known issue with energy normalization for exactly 2048x2048 2D
  // FFTs on Intel CPUs. Ensuring contiguous memory layout can help mitigate the
  // issue.
  if (dim.size() == 2) {
    ...
    if (is_2048x2048) {
      input = self.is_contiguous() ? self : self.contiguous();
    }
  }
  ...
  return _exec_fft(
      out, input, input.sizes(), sorted_dims, normalization, forward);
}
