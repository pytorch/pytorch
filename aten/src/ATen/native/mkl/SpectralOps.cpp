...
static inline void _fft_fill_with_hermitian_symmetry_slice(Tensor& output,
  ...
  while (num > 0) {
    ...
    for (int64_t d = signal_ndim - 1; d >= 0; d--) {
      ...
      if (d > 0) {
        // this is tricky
        //   1. need to carry over if this dim idx becomes 0
        //   2. if nonbatch dim, to get next from_data offset
        //      i.  if this dim idx becomes 1, will need to add (size - 1) * stride
        //      ii. otherwise, will need to subtract stride
        if (slice_indices[d] == 0) {
          from_data -= output.stride(d);
        } else if (slice_indices[d] == 1) {
          from_data += (output.size(d) - 1) * output.stride(d);
          break;
        } else {
          from_data -= output.stride(d);
          break;
...
static inline void _fft_fill_with_hermitian_symmetry(Tensor& input,
                      int64_t signal_ndim, int64_t size_last_dim,
  ...
  if (num > 500) {
    ...
    #pragma omp parallel
    {
      ...
      AT_DISPATCH_FLOATING_TYPES(input.type(), "_fft_fill_with_hermitian_symmetry", [&] {
        _fft_fill_with_hermitian_symmetry_slice<scalar_t>(input, signal_ndim, size_last_dim,
...
Tensor _fft_mkl(const Tensor& self, int64_t signal_ndim,
  ...
  DftiDescriptor descriptor(prec, signal_type, signal_ndim, mkl_signal_sizes.data());
  // out of place FFT
  ...
  if (!complex_input && complex_output && !onesided) {
    ...
    _fft_fill_with_hermitian_symmetry(output, signal_ndim, size_last_signal_dim, start_slice);
  }
  return output;
}
