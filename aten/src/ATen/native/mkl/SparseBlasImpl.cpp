...
void addmv_out_sparse_csr(
  ...
  if (mat.layout() == kSparseCsr) {
    ...
    AT_ERROR(
        "MKL only supports CSR and BSR layout, but got ",
        mat.layout(),
        " instead.");
  }
        ...
        X.scalar_type(), "triangular_solve_out_sparse_csr_impl_mkl", [&] {
          ...
          if (B.size(-1) == 1) {
            ...
            TORCH_INTERNAL_ASSERT_DEBUG_ONLY(
                !(is_X_row_major ^ is_B_row_major));

            auto order = is_X_row_major ? SPARSE_LAYOUT_ROW_MAJOR
                                        : SPARSE_LAYOUT_COLUMN_MAJOR;
            auto nrhs = mkl_int_cast(B.size(-1), "nrhs");
            auto ldx =
                is_X_row_major ? X_strides[ndim - 2] : X_strides[ndim - 1];
            auto ldb =
                is_B_row_major ? B_strides[ndim - 2] : B_strides[ndim - 1];
