...
kernel void lgamma(device T0 *input [[buffer(0)]],
...
template<typename T0, typename T1>
kernel void digamma (device T0 *input [[buffer(0)]],
                    device T1 *output [[buffer(1)]],
                    uint id [[thread_position_in_grid]])
{
    float x = input[id];
    if (x < 0.0f) {
        if (x == trunc(x)) {
            ...
            float r = fract(x);
            output[id] = static_cast<T1>(calc_digamma_positive_domain(1.0f - x) - PI / tan(PI * r));
        }
    } else if (x == 0.0f) {
        // As per C++ standard for gamma related functions and SciPy,
...
template<typename T0, typename T1>
kernel void trigamma(device T0 *input [[buffer(0)]],
                     device T1 *output [[buffer(1)]],
                     uint id [[thread_position_in_grid]])
{
    float x = input[id];
...
template<typename T0, typename T1>
kernel void polygamma(device T0 *input [[buffer(0)]],
                     device T1 *output [[buffer(1)]],
...
template [[host_name("lgamma_" #DTYPE0 "_" #DTYPE1)]] kernel void \
      lgamma (device DTYPE0 *input [[buffer(0)]],                 \
               device DTYPE1 *output [[buffer(1)]],                \
               uint id [[thread_position_in_grid]]);               \
template [[host_name("digamma_" #DTYPE0 "_" #DTYPE1)]] kernel void \
      digamma (device DTYPE0 *input [[buffer(0)]],                 \
               device DTYPE1 *output [[buffer(1)]],                \
               uint id [[thread_position_in_grid]]);               \
template [[host_name("trigamma_" #DTYPE0 "_" #DTYPE1)]] kernel void \
      trigamma (device DTYPE0 *input [[buffer(0)]],                \
               device DTYPE1 *output [[buffer(1)]],                \
               uint id [[thread_position_in_grid]]);               \
