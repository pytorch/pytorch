// Gemm.metal - binder + explicit-instantiation site for the GEMM kernels.
// Compiled twice (metal3.1 -> kernels_basic, metal4.0 -> kernels_40); matmul2d
// families sit behind #if __METAL_VERSION__ >= 400. Host names encode the
// dispatch key.
...
// name:
// gemm_simd_{dt}_{BM}_{BN}_{BK}_{WM}_{WN}_ta{0|1}_tb{0|1}_{none|ab}_{b0|b1}
// ---------------------------------------------------------------------------
#define MB_SIMD(DT, BM, BN, BK, WM, WN, TAN, TAV, TBN, TBV, EN, EV, BTN, BTV) \
  template                                                                    \
      [[host_name("gemm_simd_" #DT "_" #BM "_" #BN "_" #BK "_" #WM "_" #WN    \
...
#define MB_SIMD_EB(DT, BM, BN, BK, WM, WN, TAN, TAV, TBN, TBV)               \
  ...
  MB_SIMD(                                                                   \
      DT, BM, BN, BK, WM, WN, TAN, TAV, TBN, TBV, ab, AlphaBeta, b0, false)  \
...
// ---------------------------------------------------------------------------
// int_gemm (Metal 3 + 4): register-tiled integer GEMM (no float-only matrix
// unit). name:
// gemm_int_{dt}_{BM}_{BN}_{BK}_{TX}_{TY}_ta{0|1}_tb{0|1}_{none|ab}_{b0|b1}
// ---------------------------------------------------------------------------
#define MB_INT(DT, BM, BN, BK, TX, TY, TAN, TAV, TBN, TBV, EN, EV, BTN, BTV)   \
  template [[host_name("gemm_int_" #DT "_" #BM "_" #BN "_" #BK "_" #TX "_" #TY \
                       "_ta" #TAN "_tb" #TBN "_" #EN "_" #BTN)]]               \
  kernel void                                                                  \
...
// ---------------------------------------------------------------------------
// complex deinterleave / interleave (Metal 3 + 4) for the decomposed complex
// GEMM. names: complex_split_{c2}, complex_combine_{c2}  (c2 = float2 | half2)
// ---------------------------------------------------------------------------
#define MB_CPLX(C2, R)                                               \
...
// ---------------------------------------------------------------------------
// GEMV (Metal 3 + 4): rank-1 (M==1 / N==1) bandwidth-bound kernels. VEC=1 is
// the universal, always-aligned config (still far better than a ~97%-masked
// GEMM tile). names: gemv_t_{dt}_{NW}_{VEC}_{none|ab},
// gemv_nt_{dt}_{NW}_{VEC}_{red|nr}_{none|ab}
// ---------------------------------------------------------------------------
...
// half/bf16: the full _gemv_pick VEC/NWARPS family (VEC scales with N for
// cache- line coverage). gemv_nt adds VEC 2/4 (each row read a cache line at a
// time).
#define MB_GEMV_LP(DT)                                                   \
  MB_GEMV_T_E(DT, 16, 1)                                                 \
  MB_GEMV_T_E(DT, 32, 1)                                                 \
  MB_GEMV_T_E(DT, 32, 2) MB_GEMV_T_E(DT, 16, 2) MB_GEMV_T_E(DT, 16, 4)   \
      MB_GEMV_T_E(DT, 8, 4) MB_GEMV_T_E(DT, 8, 8) MB_GEMV_T_E(DT, 16, 8) \
...
// alignment at dispatch (clamp halves VEC, keeps NWARPS), so the full VEC
// ladder must exist. gemv_t NW=8; gemv_nt NW=4 (NW=8 for int32); ACC_T =
// opmath_t (int / long).
#define MB_GEMV_INT(DT)                                                    \
  MB_GEMV_T_E(DT, 8, 1)                                                    \
  MB_GEMV_T_E(DT, 8, 2)                                                    \
  MB_GEMV_T_E(DT, 8, 4) MB_GEMV_T_E(DT, 8, 8) MB_GEMV_NT_E(DT, 4, 1)       \
      MB_GEMV_NT_E(DT, 4, 2) MB_GEMV_NT_E(DT, 4, 4) MB_GEMV_NT_E(DT, 4, 8) \
          MB_GEMV_NT_E(DT, 8, 1) MB_GEMV_NT_E(DT, 8, 2) MB_GEMV_NT_E(DT, 8, 4)
...
//   cgemv_t_{c2}_{NW}_{none|ab}, cgemv_nt_{c2}_{NW}_{none|ab}  (c2 =
//   float2|half2)
// ---------------------------------------------------------------------------
...
#define MB_MPP(                                                            \
    DT, BM, BN, NSG, TAN, TAV, TBN, TBV, RXN, RXV, EN, EV, BTN, BTV)       \
...
#define MB_MPP_EB(DT, BM, BN, NSG, TAN, TAV, TBN, TBV, RXN, RXV)               \
  ...
  MB_MPP(                                                                      \
      DT, BM, BN, NSG, TAN, TAV, TBN, TBV, RXN, RXV, ab, AlphaBeta, b0, false) \
...
// transposed fp32 on the tensor unit instead of falling back to the simd
// kernel.
...
#define MB_MPP_UNTRANS_TILES(MAC, DT)                        \
  MAC(DT, 128, 32, 2)                                        \
  MAC(DT, 256, 32, 4)                                        \
  MAC(DT, 32, 128, 2) MAC(DT, 32, 256, 4) MAC(DT, 64, 32, 2) \
      MAC(DT, 128, 32, 4) MAC(DT, 192, 32, 2) MAC(DT, 128, 64, 4)
...
#define MB_SPLITK(DT, BM, BN, NSG)                                    \
  ...
  kernel void splitk_gemm<DT, BM, BN, NSG, true>(                     \
      device DT*, device DT*, device float*, constant SplitKDims&, uint3);
#define MB_SPLITK_REDUCE(DT)                                                  \
  template [[host_name("splitk_reduce_" #DT)]] kernel void splitk_reduce<DT>( \
      device const float*, device DT*, constant SplitKReduceDims&, uint);
#define MB_SPLITK_ALL(DT)   \
  MB_SPLITK(DT, 128, 32, 2) \
  MB_SPLITK(DT, 64, 64, 2) MB_SPLITK(DT, 32, 64, 2) MB_SPLITK_REDUCE(DT)

MB_SPLITK_ALL(half)
MB_SPLITK_ALL(bfloat)

// ---------------------------------------------------------------------------
// 1x1-conv (Metal 4 only): very-thin-N GEMM via convolution2d, low precision
// only. KCONST must be compile-time, so common K values are precompiled (else
// -> mpp). name: conv1x1_gemm_{dt}_{BMW}_{BNO}_{NSG}_{KCONST}  (BNO = N
// channels: 32 | 64)
// ---------------------------------------------------------------------------
#define MB_CONV(DT, BMW, BNO, NSG, KC)                                      \
  template                                                                  \
      [[host_name("conv1x1_gemm_" #DT "_" #BMW "_" #BNO "_" #NSG "_" #KC)]] \
...
#define MB_CONV_TILES(DT, KC) \
  MB_CONV_BNO(DT, 64, 2, KC)  \
  MB_CONV_BNO(DT, 64, 4, KC)  \
  MB_CONV_BNO(DT, 128, 2, KC) MB_CONV_BNO(DT, 128, 4, KC)
#define MB_CONV_ALL(DT)  \
  MB_CONV_TILES(DT, 512) \
  MB_CONV_TILES(DT, 1024) MB_CONV_TILES(DT, 2048) MB_CONV_TILES(DT, 4096)

MB_CONV_ALL(half)
MB_CONV_ALL(bfloat)

// ---------------------------------------------------------------------------
// gemv_bt (Metal 4 only): thin-M batched GEMV (M in 2..16), routed only on the
// NAX matrix unit (kernels_40). The host requests a (MROWS, VEC, NCOLS) from
// this set. names: gemv_bt_{dt}_{MR}_{VEC}_{none|ab},
// gemv_bt_t_{dt}_{MR}_{VEC}_{NC}_{none|ab}
// ---------------------------------------------------------------------------
...
#define MB_GEMV_BT_T_E(DT, MR, VEC, NC)     \
  MB_GEMV_BT_T(DT, MR, VEC, NC, none, None) \
  MB_GEMV_BT_T(DT, MR, VEC, NC, ab, AlphaBeta)

// Row-major B (NCOLS=1): (MROWS, VEC) with MROWS*VEC <= 32.
#define MB_GEMV_BT_RM_ALL(DT)                                              \
  MB_GEMV_BT_E(DT, 2, 1)                                                   \
  MB_GEMV_BT_E(DT, 2, 2)                                                   \
  MB_GEMV_BT_E(DT, 2, 4) MB_GEMV_BT_E(DT, 4, 1) MB_GEMV_BT_E(DT, 4, 2)     \
      MB_GEMV_BT_E(DT, 4, 4) MB_GEMV_BT_E(DT, 8, 1) MB_GEMV_BT_E(DT, 8, 2) \
          MB_GEMV_BT_E(DT, 8, 4) MB_GEMV_BT_E(DT, 16, 1)                   \
              MB_GEMV_BT_E(DT, 16, 2)

// Column-major B: (MROWS, VEC, NCOLS). NCOLS>1 only for MROWS>=8 (the M>=6
// reuse threshold) with MROWS*NCOLS <= 48 registers.
#define MB_GEMV_BT_TB_ALL(DT)                                              \
  MB_GEMV_BT_T_E(DT, 2, 2, 1)                                              \
  MB_GEMV_BT_T_E(DT, 2, 4, 1)                                              \
  MB_GEMV_BT_T_E(DT, 2, 8, 1) MB_GEMV_BT_T_E(DT, 4, 2, 1) MB_GEMV_BT_T_E(  \
      DT, 4, 4, 1) MB_GEMV_BT_T_E(DT, 4, 8, 1) MB_GEMV_BT_T_E(DT, 8, 2, 1) \
      MB_GEMV_BT_T_E(DT, 8, 4, 1) MB_GEMV_BT_T_E(DT, 8, 8, 1)              \
          MB_GEMV_BT_T_E(DT, 8, 2, 4) MB_GEMV_BT_T_E(DT, 8, 4, 4)          \
              MB_GEMV_BT_T_E(DT, 8, 8, 4) MB_GEMV_BT_T_E(DT, 16, 2, 2)     \
                  MB_GEMV_BT_T_E(DT, 16, 4, 2) MB_GEMV_BT_T_E(DT, 16, 8, 2)
