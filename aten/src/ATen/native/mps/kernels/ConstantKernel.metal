...
// Byte types use the vec4 variant under the same naming convention.
#define REGISTER_FILL_BYTE_OP(T)                              \
  template [[host_name("fill_scalar_dense_" #T)]] kernel void \
  fill_scalar_dense_vec4<T>(device T*, constant T&, constant uint&, uint)

REGISTER_FILL_BYTE_OP(char);
