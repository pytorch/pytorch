...
// ===========================================================================
// Uses stochastic pooling where pooling regions are determined by random
// samples, allowing for non-integer downsampling ratios.
...
kernel void fractional_max_pool2d_forward(
    ...
    constant FractionalMaxPool2dParams<int32_t>& params [[buffer(4)]],
  ...
  // Get random samples for this batch/plane (2 samples per plane)
  ...
  // Input offset for this batch/plane
  int inputOffset =
...
kernel void fractional_max_pool2d_backward(
    ...
    constant T* grad_output [[buffer(1)]],
    constant int64_t* indices [[buffer(2)]],
    constant FractionalMaxPool2dBackwardParams<int32_t>& params [[buffer(3)]],
    uint3 tid [[thread_position_in_grid]]) {
  int outputPoint = tid.x;
  int plane = tid.y;
  int batch = tid.z;
...
template <typename T>
kernel void fractional_max_pool3d_forward(
    constant T* input [[buffer(0)]],
    device T* output [[buffer(1)]],
    device int64_t* indices [[buffer(2)]],
    constant T* randomSamples [[buffer(3)]],
    constant FractionalMaxPool3dParams<int32_t>& params [[buffer(4)]],
    uint3 tid [[thread_position_in_grid]]) {
  using accT = float;
  ...
  int outputT = params.outputT;
  int outputH = params.outputH;
  int outputW = params.outputW;
  int outputSize = outputT * outputH * outputW;

  if (outputPoint >= outputSize || plane >= params.numPlanes ||
      batch >= params.numBatch) {
    return;
  }

  // Convert linear index to 3D coordinates
  int outputTIdx = outputPoint / (outputH * outputW);
  int outputHIdx = (outputPoint / outputW) % outputH;
  int outputWIdx = outputPoint % outputW;

  // Get random samples for this batch/plane (3 samples per plane)
  int sampleIdx = batch * params.numPlanes * 3 + plane * 3;
  accT sampleT = static_cast<accT>(randomSamples[sampleIdx]);
  accT sampleH = static_cast<accT>(randomSamples[sampleIdx + 1]);
  accT sampleW = static_cast<accT>(randomSamples[sampleIdx + 2]);

  // Calculate pooling window start positions
  int poolT = get_fractional_interval<T, accT>(
      sampleT, outputTIdx, params.inputT, outputT, params.poolSizeT);
  int poolH = get_fractional_interval<T, accT>(
      sampleH, outputHIdx, params.inputH, outputH, params.poolSizeH);
  int poolW = get_fractional_interval<T, accT>(
      sampleW, outputWIdx, params.inputW, outputW, params.poolSizeW);
  ...
  // Input offset for this batch/plane
  int inputOffset =
      batch * params.numPlanes * params.inputT * params.inputH * params.inputW +
      plane * params.inputT * params.inputH * params.inputW;

  int inputHW = params.inputH * params.inputW;

  // Find max value in the pooling window
  T maxVal = input[inputOffset + poolT * inputHW + poolH * params.inputW + poolW];
  int64_t maxIndex = poolT * inputHW + poolH * params.inputW + poolW;

  for (int t = poolT; t < poolT + params.poolSizeT; ++t) {
    for (int h = poolH; h < poolH + params.poolSizeH; ++h) {
      for (int w = poolW; w < poolW + params.poolSizeW; ++w) {
        T val = input[inputOffset + t * inputHW + h * params.inputW + w];
        if (val > maxVal || isnan(val)) {
          maxVal = val;
          maxIndex = t * inputHW + h * params.inputW + w;
        }
      }
    }
  }
  ...
  // Write output
  int outputHW = outputH * outputW;
  int outputOffset =
      batch * params.numPlanes * outputSize + plane * outputSize;
  output[outputOffset + outputTIdx * outputHW + outputHIdx * outputW + outputWIdx] = maxVal;
  indices[outputOffset + outputTIdx * outputHW + outputHIdx * outputW + outputWIdx] = maxIndex;
}
...
kernel void fractional_max_pool3d_backward(
    ...
    constant T* grad_output [[buffer(1)]],
    constant int64_t* indices [[buffer(2)]],
    constant FractionalMaxPool3dBackwardParams<int32_t>& params [[buffer(3)]],
    uint3 tid [[thread_position_in_grid]]) {
  int outputPoint = tid.x;
  int plane = tid.y;
  int batch = tid.z;
  ...
  int outputOffset =
      batch * params.numPlanes * outputSize + plane * outputSize;
  int inputSize = params.inputT * params.inputH * params.inputW;
  int inputOffset =
      batch * params.numPlanes * inputSize + plane * inputSize;

  int64_t index = indices[outputOffset + outputTIdx * outputHW + outputHIdx * outputW + outputWIdx];
  T gradVal = grad_output[outputOffset + outputTIdx * outputHW + outputHIdx * outputW + outputWIdx];

  atomic_add_float(&grad_input[inputOffset + index], gradVal);
}

// Template instantiations for fractional max pooling
#define REGISTER_FRACTIONAL_MAX_POOL2D_OP(DTYPE)                             \
...
#define REGISTER_FRACTIONAL_MAX_POOL2D_BACKWARD_OP(DTYPE)                    \
...
#define REGISTER_FRACTIONAL_MAX_POOL3D_OP(DTYPE)                             \
  template [[host_name("fractional_max_pool3d_forward_" #DTYPE)]]            \
  kernel void fractional_max_pool3d_forward<DTYPE>(                          \
      constant DTYPE* input [[buffer(0)]],                                   \
      device DTYPE* output [[buffer(1)]],                                    \
      device int64_t* indices [[buffer(2)]],                                 \
      constant DTYPE* randomSamples [[buffer(3)]],                           \
...
#define REGISTER_FRACTIONAL_MAX_POOL3D_BACKWARD_OP(DTYPE)                    \
  template [[host_name("fractional_max_pool3d_backward_" #DTYPE)]]           \
  kernel void fractional_max_pool3d_backward<DTYPE>(                         \
      device AtomicType_t<DTYPE>* grad_input [[buffer(0)]],                  \
      constant DTYPE* grad_output [[buffer(1)]],                             \
      constant int64_t* indices [[buffer(2)]],                               \
      constant FractionalMaxPool3dBackwardParams<int32_t>& params [[buffer(3)]], \
...
REGISTER_FRACTIONAL_MAX_POOL2D_OP(float);
REGISTER_FRACTIONAL_MAX_POOL2D_OP(half);
REGISTER_FRACTIONAL_MAX_POOL2D_OP(bfloat);

REGISTER_FRACTIONAL_MAX_POOL2D_BACKWARD_OP(float);
REGISTER_FRACTIONAL_MAX_POOL2D_BACKWARD_OP(half);
REGISTER_FRACTIONAL_MAX_POOL2D_BACKWARD_OP(bfloat);

REGISTER_FRACTIONAL_MAX_POOL3D_OP(float);
REGISTER_FRACTIONAL_MAX_POOL3D_OP(half);
REGISTER_FRACTIONAL_MAX_POOL3D_OP(bfloat);

REGISTER_FRACTIONAL_MAX_POOL3D_BACKWARD_OP(float);
REGISTER_FRACTIONAL_MAX_POOL3D_BACKWARD_OP(half);
REGISTER_FRACTIONAL_MAX_POOL3D_BACKWARD_OP(bfloat);
