...
// Metal's frexp() computes in float, and Apple GPUs flush float32 subnormals to
// zero; bfloat16 shares float32's exponent range, so its subnormals are lost
// the same way. Decompose the bit pattern instead -- integer ops are not
// subject to flush-to-zero. MANT and EXP are the field widths of T, U its
// unsigned twin.
template <typename T, typename U, int MANT, int EXP>
inline T frexp_bits(const T x, thread int& exponent) {
  ...
  if (exp_field == 0) {
    ...
    // the implicit-bit position leaves g in the significand field.
    // frac is nonzero here: the exp_field == 0 && frac == 0 case returned
    // above.
    const int lead = 31 - int(clz(uint(frac)));
    frac = U((uint(frac) << (MANT - lead)) & uint(mant_mask));
    exponent = lead + 2 - bias - MANT;
  } else {
    // Normal: value = 1.f * 2^(exp_field - bias) = (1.f / 2) * 2^(exp_field -
    // bias + 1).
...
inline float frexp_(const float x, thread int& e) {
  return frexp_bits<float, uint, 23, 8>(x, e);
}
inline half frexp_(const half x, thread int& e) {
  return frexp_bits<half, ushort, 10, 5>(x, e);
}
inline bfloat frexp_(const bfloat x, thread int& e) {
  return frexp_bits<bfloat, ushort, 7, 8>(x, e);
}
...
kernel void frexp_strided(
    ...
    uint index [[thread_position_in_grid]]) {
  int pos[max_ndim];
  pos_from_thread_index(static_cast<int>(index), pos, sizes, ndim);
  int exp = 0;
  const auto val =
      val_at_offs<T>(input, offset_from_coord(pos, input_strides, ndim));
  ref_at_offs<T>(mantissa, offset_from_coord(pos, mantissa_strides, ndim)) =
      frexp_(val, exp);
