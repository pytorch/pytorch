// Kth value metal kernels, most of the code here is reusing the
// load/sort/scatter bodies from the sort kernels (SortMerge.h and SortRadix.h)
// and only customize the writeback Instead of emitting all sort_size elements
// per row, they emit one element per row at sorted rank `kth`. Output is
// [n_rows], i.e. one scalar per row
#pragma once
...
kernel void mb_merge_final_kth(
...
#define INSTANTIATE_KTH_SORT_BLOCK(T, TPTG, TN)                          \
  template [[host_name("sort_block_kth_" #T "_tptg" #TPTG)]] kernel void \
  sort_block_kth<T, TPTG, TN>(                                           \
      const device T*,                                                   \
      device T*,                                                         \
      device long*,                                                      \
      constant int&,                                                     \
...
#define INSTANTIATE_KTH_MB_MERGE_FINAL(T, TPTG, TN)                     \
  ...
  kernel void mb_merge_final_kth<T, uint, TPTG, TN>(                    \
      const device T*,                                                  \
      const device uint*,                                               \
      device T*,                                                        \
      device long*,                                                     \
  ...
  kernel void mb_merge_final_kth<T, ushort, TPTG, TN>(                  \
      const device T*,                                                  \
      const device ushort*,                                             \
      device T*,                                                        \
      device long*,                                                     \
...
#define _SCATTER_KTH_SIG(T, InIdxT)                                       \
  const device T*, const device InIdxT*, device T*, device long*,         \
      const device uint*, constant int3&, constant bool2&, constant int&, \
      uint3, uint3

#define INSTANTIATE_KTH_RADIX_SCATTER(T, InIdxT, RTPTG, EPT, RBITS, NAME) \
  template [[host_name("radix_scatter_final_kth_" NAME)]]                 \
  kernel void radix_scatter_kth<T, InIdxT, RTPTG, EPT, RBITS, false>(     \
      _SCATTER_KTH_SIG(T, InIdxT));                                       \
  template [[host_name("radix_scatter_fused_final_kth_" NAME)]]           \
  kernel void radix_scatter_kth<T, InIdxT, RTPTG, EPT, RBITS, true>(      \
      _SCATTER_KTH_SIG(T, InIdxT));

#define INSTANTIATE_KTH_RADIX(T, RTPTG, EPT, RBITS)    \
  INSTANTIATE_KTH_RADIX_SCATTER(                       \
