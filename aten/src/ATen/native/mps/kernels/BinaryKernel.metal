...
// Opt-in inner_strided registrations for the arithmetic core -- the ops whose
// call sites actually see broadcast/strided layouts (bias/scale patterns).
// Blanket registration across every binary op measured +11 MB of metallib;
// exec_binary_kernel probes hasFunction() and falls back to the generic
// strided kernel for ops not listed here.
#define REGISTER_FLOAT_BINARY_INNER_STRIDED_OP(NAME)    \
  REGISTER_BINARY_INNER_STRIDED_OP(NAME, float, float); \
  REGISTER_BINARY_INNER_STRIDED_OP(NAME, half, half);   \
  REGISTER_BINARY_INNER_STRIDED_OP(NAME, bfloat, bfloat)

#define REGISTER_OPMATH_FLOAT_BINARY_INNER_STRIDED_OP(NAME)    \
  REGISTER_OPMATH_BINARY_INNER_STRIDED_OP(NAME, float, float); \
  REGISTER_OPMATH_BINARY_INNER_STRIDED_OP(NAME, half, half);   \
  REGISTER_OPMATH_BINARY_INNER_STRIDED_OP(NAME, bfloat, bfloat)

#define REGISTER_INTEGER_BINARY_INNER_STRIDED_OP(NAME)  \
  REGISTER_BINARY_INNER_STRIDED_OP(NAME, long, long);   \
  REGISTER_BINARY_INNER_STRIDED_OP(NAME, int, int);     \
  REGISTER_BINARY_INNER_STRIDED_OP(NAME, short, short); \
  REGISTER_BINARY_INNER_STRIDED_OP(NAME, uchar, uchar); \
  REGISTER_BINARY_INNER_STRIDED_OP(NAME, char, char);   \
  REGISTER_BINARY_INNER_STRIDED_OP(NAME, bool, bool)

#define REGISTER_INT2FLOAT_BINARY_INNER_STRIDED_OP(NAME) \
  REGISTER_BINARY_INNER_STRIDED_OP(NAME, long, float);   \
  REGISTER_BINARY_INNER_STRIDED_OP(NAME, int, float);    \
  REGISTER_BINARY_INNER_STRIDED_OP(NAME, short, float);  \
  REGISTER_BINARY_INNER_STRIDED_OP(NAME, uchar, float);  \
  REGISTER_BINARY_INNER_STRIDED_OP(NAME, char, float);   \
  REGISTER_BINARY_INNER_STRIDED_OP(NAME, bool, float)

REGISTER_FLOAT_BINARY_INNER_STRIDED_OP(add);
REGISTER_INTEGER_BINARY_INNER_STRIDED_OP(add);
REGISTER_FLOAT_BINARY_INNER_STRIDED_OP(sub);
REGISTER_INTEGER_BINARY_INNER_STRIDED_OP(sub);
REGISTER_OPMATH_FLOAT_BINARY_INNER_STRIDED_OP(mul);
REGISTER_INTEGER_BINARY_INNER_STRIDED_OP(mul);
REGISTER_OPMATH_FLOAT_BINARY_INNER_STRIDED_OP(div_true);
REGISTER_INT2FLOAT_BINARY_INNER_STRIDED_OP(div_true);
