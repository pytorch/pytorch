...
kernel void segment_reduce_forward(
    ...
    constant SegmentReduceParams<T>& params [[buffer(4)]],
    uint idx [[thread_position_in_grid]]) {
  if ((index_t)idx >= (index_t)params.outer_offset *
          (index_t)params.segment_count * (index_t)params.inner_offset) {
    return;
  }
  ...
  index_t offset_idx =
      outer_idx * (index_t)(params.segment_count + 1) + dim_idx;
  index_t offset_start = lengths_cumsum_data[offset_idx];
  index_t offset_end = lengths_cumsum_data[offset_idx + 1];

  T accumulator = params.is_initial_set ? params.initial_value
                                        : get_initial_value<T>(reduction);

  for (index_t i = offset_start; i < offset_end; i++) {
    index_t data_index = outer_idx * (index_t)params.data_size_axis *
            (index_t)params.inner_offset +
        i * (index_t)params.inner_offset + lane_id;

    T val = values_data[data_index];
    if constexpr (
        reduction == ReductionType::SUM || reduction == ReductionType::MEAN) {
      accumulator += val;
    } else if constexpr (reduction == ReductionType::MAX) {
      accumulator = (isnan(val) || val > accumulator) ? val : accumulator;
    } else if constexpr (reduction == ReductionType::MIN) {
      accumulator = (isnan(val) || val < accumulator) ? val : accumulator;
    } else {
      accumulator *= val;
    }
  ...
  if constexpr (reduction == ReductionType::MEAN) {
...
kernel void segment_reduce_backward(
    ...
    constant SegmentReduceParams<T>& params [[buffer(6)]],
    uint idx [[thread_position_in_grid]]) {
  if ((index_t)idx >= (index_t)params.outer_offset *
          (index_t)params.segment_count * (index_t)params.inner_offset) {
    return;
  ...
  index_t data_base_idx = outer_idx * (index_t)params.data_size_axis *
          (index_t)params.inner_offset +
      lane_id;
  ...
  if constexpr (
    ...
    for (index_t i = offset_start; i < offset_end; i++) {
      ...
      reduction == ReductionType::MIN || reduction == ReductionType::MAX) {
  ...
  } else if constexpr (reduction == ReductionType::PROD) {
    for (index_t i = offset_start; i < offset_end; i++) {
      index_t data_index = data_base_idx + i * (index_t)params.inner_offset;
      T val = values_data[data_index];
      if (isnan(val) || val == T(0)) {
        T exclusive_prod = params.initial_value;
        for (index_t k = offset_start; k < offset_end; k++) {
          if (k != i) {
            index_t k_data_index =
                data_base_idx + k * (index_t)params.inner_offset;
...
#define REGISTER_SEGMENT_REDUCE(T, IDX_T, LENGTHS_T, REDUCTION)               \
  template [[host_name("segment_reduce_forward_" #T "_" #IDX_T "_" #LENGTHS_T \
                       "_" #REDUCTION)]]                                      \
  kernel void                                                                 \
  segment_reduce_forward<T, IDX_T, LENGTHS_T, ReductionType::REDUCTION>(      \
      device T*,                                                              \
      constant T*,                                                            \
      constant IDX_T*,                                                        \
      constant LENGTHS_T*,                                                    \
      constant SegmentReduceParams<T>&,                                       \
      uint);

#define REGISTER_SEGMENT_REDUCE_BACKWARD(T, IDX_T, LENGTHS_T, REDUCTION)       \
  template [[host_name("segment_reduce_backward_" #T "_" #IDX_T "_" #LENGTHS_T \
                       "_" #REDUCTION)]]                                       \
  kernel void                                                                  \
  segment_reduce_backward<T, IDX_T, LENGTHS_T, ReductionType::REDUCTION>(      \
      device T*,                                                               \
      constant T*,                                                             \
      constant T*,                                                             \
      constant T*,                                                             \
