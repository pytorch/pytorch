...
inline void safe_add_3d(
    device atomic<float>* grad_input,
    long z,
    long y,
    long x,
    long inp_D,
  ...
  if (within_bounds_3d(z, y, x, inp_D, inp_H, inp_W)) {
    const auto grad_input_offset = base_offset + z * grad_input_strides[2] +
        y * grad_input_strides[3] + x * grad_input_strides[4];
    atomic_fetch_add_explicit(
        &grad_input[grad_input_offset], weight, memory_order_relaxed);
  }
}
...
kernel void grid_sampler_3d(
    ...
    constant ulong* input_sizes
    [[buffer(6)]], // [N, C, D, H, W] - all 5 dimensions
    constant ulong* output_sizes
    [[buffer(7)]], // [N, C, D_out, H_out, W_out] - all 5 dimensions
    constant ulong* input_strides
    [[buffer(8)]], // all 5 input strides [N, C, D, H, W]
    constant ulong* output_strides
    [[buffer(9)]], // all 5 output strides [N, C, D, H, W]
    constant ulong* grid_strides
    [[buffer(10)]], // all 5 grid strides [N, D, H, W, coord]
  ...
  // Early bounds check
  if (N >= input_sizes[0] || C >= input_sizes[1] || out_d >= output_sizes[2] ||
  ...
  const auto output_offset = N * output_strides[0] + C * output_strides[1] +
      out_d * output_strides[2] + out_h * output_strides[3] +
  ...
  if (interpolation_mode == 0) { // trilinear interpolation
    ...
    const array<T, 2> x_weights = {T(1.0) - x_diff, x_diff};
    const array<T, 2> y_weights = {T(1.0) - y_diff, y_diff};
    const array<T, 2> z_weights = {T(1.0) - z_diff, z_diff};

    // Trilinear weights for 8 corners
    const array<T, 8> weights = {
      ...
      for (int corner = 0; corner < 8; corner++) {
        if (within_bounds_3d(
          ...
          const auto input_offset = N * input_strides[0] +
              C * input_strides[1] + corners_z[corner] * input_strides[2] +
    ...
    if (padding_mode == 0) { // zeros padding
      if (within_bounds_3d(
              iz_nearest, iy_nearest, ix_nearest, in_D, in_H, in_W)) {
        const auto input_offset = N * input_strides[0] + C * input_strides[1] +
            iz_nearest * input_strides[2] + iy_nearest * input_strides[3] +
...
kernel void grid_sampler_3d_vectorized(
  ...
  constexpr int ELEMS_PER_THREAD =
      4; // Process 4 elements per thread for better bandwidth
    ...
    if (interpolation_mode == 0) { // trilinear interpolation
      ...
      for (int z_idx = 0; z_idx < 2; z_idx++) {
        for (int y_idx = 0; y_idx < 2; y_idx++) {
          for (int x_idx = 0; x_idx < 2; x_idx++) {
            ...
            const T weight =
                x_weights[x_idx] * y_weights[y_idx] * z_weights[z_idx];

            if (padding_mode == 0) { // zeros padding
              if (within_bounds_3d(
                      sample_z, sample_y, sample_x, in_D, in_H, in_W)) {
                const auto input_offset = N * input_strides[0] +
                    C * input_strides[1] + sample_z * input_strides[2] +
                    sample_y * input_strides[3] + sample_x * input_strides[4];
      ...
      if (padding_mode == 0) { // zeros padding
        if (within_bounds_3d(
                iz_nearest, iy_nearest, ix_nearest, in_D, in_H, in_W)) {
          const auto input_offset = N * input_strides[0] +
              C * input_strides[1] + iz_nearest * input_strides[2] +
              iy_nearest * input_strides[3] + ix_nearest * input_strides[4];
          outputData[output_offset] = inputData[input_offset];
        } else {
          outputData[output_offset] = T(0);
        }
...
// Helper function to unnormalize coordinates from [-1, 1] to [0, size-1] or
// [-0.5, size-0.5] and compute the gradient multiplier
template <typename T>
T grid_sampler_unnormalize_set_grad(
    T coord,
    ulong size,
    bool align_corners,
    thread T* grad_in) {
...
kernel void grid_sampler_3d_backward_input(
  ...
  float ix = grid_sampler_compute_source_index_set_grad(
      grid_x, inp_W, padding_mode, align_corners, &gix_mult);
  float iy = grid_sampler_compute_source_index_set_grad(
      grid_y, inp_H, padding_mode, align_corners, &giy_mult);
  float iz = grid_sampler_compute_source_index_set_grad(
      grid_z, inp_D, padding_mode, align_corners, &giz_mult);
...
kernel void grid_sampler_3d_backward_grid(
  ...
  if (interpolation_mode == 0) { // trilinear interpolation
    ...
    // Process all channels and accumulate gradients following CUDA
    // implementation
    for (ulong c = 0; c < C; c++) {
      const auto grad_out_offset = N * grad_output_strides[0] +
          c * grad_output_strides[1] + out_d * grad_output_strides[2] +
    ...
    // Write gradients to grad_grid, multiplied by the coordinate gradient
    // multipliers
    grad_grid[grad_grid_base_offset] = gix_mult * gix;
    grad_grid[grad_grid_base_offset + grad_grid_strides[4]] = giy_mult * giy;
    grad_grid[grad_grid_base_offset + 2 * grad_grid_strides[4]] =
        giz_mult * giz;

  } else if (interpolation_mode == 1) { // nearest neighbor
    // For nearest neighbor, gradients w.r.t. grid are zero (following CUDA
    // implementation)
...
#define INSTANTIATE_GRID_SAMPLER_3D(DTYPE)                      \
  template [[host_name("grid_sampler_3d_" #DTYPE)]] kernel void \
  grid_sampler_3d<DTYPE>(                                       \
...
#define INSTANTIATE_GRID_SAMPLER_3D_VECTORIZED(DTYPE)                      \
  template [[host_name("grid_sampler_3d_vectorized_" #DTYPE)]] kernel void \
  grid_sampler_3d_vectorized<DTYPE>(                                       \
      constant DTYPE * inputData [[buffer(0)]],                            \
      device DTYPE * outputData [[buffer(1)]],                             \
      constant DTYPE * gridData [[buffer(2)]],                             \
      constant int& interpolation_mode [[buffer(3)]],                      \
