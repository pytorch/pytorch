...
Tensor segment_offsets(const Tensor& data, const Tensor& metadata, int64_t axis) {
  ...
  TORCH_CHECK(data.device() == metadata.device(), "segment_reduce(): data and lengths/offsets must be on the same device");
...
SegmentReduceParams segment_params(
    const Tensor& data,
    const Tensor& offsets,
    int64_t axis,
    const std::optional<Scalar>& initial) {
  SegmentReduceParams p{1, 1, static_cast<uint64_t>(offsets.size(axis) - 1),
                        static_cast<uint64_t>(data.size(axis)), 0.0f, initial.has_value()};
...
Tensor segment_reduce_mps(
  ...
  const auto name = fmt::format("segment_{}_{}_{}_{}", parallel ? "parallel" : "serial",
                                scalarToMetalTypeString(data), scalarToMetalTypeString(offsets),
...
template <bool IsOffsets>
Tensor segment_reduce_backward_mps(
    const Tensor& grad,
    const Tensor& output,
    const Tensor& data,
    ReductionType reduction,
  ...
  const auto name = fmt::format("segment_backward_{}_{}_{}", scalarToMetalTypeString(data),
                                scalarToMetalTypeString(offsets), reduction_name(reduction));
  auto* stream = getCurrentMPSStream();
  dispatch_sync_with_rethrow(stream->queue(), ^() {
    @autoreleasepool {
      auto encoder = stream->commandEncoder();
      ...
      for (uint64_t base = 0; base < static_cast<uint64_t>(output.numel());) {
        const auto count = std::min<uint64_t>(output.numel() - base, std::numeric_limits<uint32_t>::max());
        mtl_setArgs(encoder, grad, output, data, grad_input, offsets, valid, p, base);
        mtl_dispatch1DJob(encoder, pipeline, count);
        base += count;
      }
    }
