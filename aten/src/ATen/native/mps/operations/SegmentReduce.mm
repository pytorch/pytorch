...
static Tensor _segment_reduce_forward_mps_impl(ReductionType reduction,
  ...
  int64_t outer_offset = c10::multiply_integers(data.sizes().begin(), data.sizes().begin() + axis);
  int64_t inner_offset = c10::multiply_integers(data.sizes().begin() + axis + 1, data.sizes().end());
  ...
  dispatch_sync_with_rethrow(mpsStream->queue(), ^() {
    @autoreleasepool {
      ...
      AT_DISPATCH_FLOATING_TYPES_AND2(kHalf, kBFloat16, data.scalar_type(), "segment_reduce_mps", [&] {
        SegmentReduceParams<scalar_t> params;
        ...
        params.initial_value = initial.has_value() ? initial.value().to<scalar_t>() : scalar_t(0);
        mtl_setArgs(computeEncoder, output, data.contiguous(), lengths_cumsum, lengths_2d, params);
        int64_t total_threads = outer_offset * segment_count * inner_offset;
        mtl_dispatch1DJob(computeEncoder, pso, total_threads);
      });
  ...
  dispatch_sync_with_rethrow(mpsStream->queue(), ^() {
    @autoreleasepool {
      ...
      AT_DISPATCH_FLOATING_TYPES_AND2(kHalf, kBFloat16, data.scalar_type(), "segment_reduce_backward_mps", [&] {
        ...
        params.initial_value = initial.has_value() ? initial.value().to<scalar_t>() : scalar_t(1);
        mtl_setArgs(computeEncoder, grad_input, grad, output, data.contiguous(), lengths_cumsum, lengths_2d, params);
        int64_t total_threads = outer_offset * segment_count * inner_offset;
        mtl_dispatch1DJob(computeEncoder, pso, total_threads);
      });
      getMPSProfiler().endProfileKernel(pso);
