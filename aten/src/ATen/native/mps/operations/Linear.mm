...
std::tuple<Tensor, Tensor, Tensor> mps_linear_backward(
  ...
  if (output_mask[0]) {
    grad_input = mps_linear_backward_input(input.sizes(), grad_output, weight);
  }
  if (output_mask[1] || output_mask[2]) {
    std::tie(grad_weight, grad_bias) = mps_linear_backward_weights(grad_output, input, weight, output_mask[2]);
  }
