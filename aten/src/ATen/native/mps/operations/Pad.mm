...
static Tensor& pad_out_template(Tensor& output,
  ...
  if ((ndims == 3 || ndims == 4) && mode == MPSGraphPaddingModeClampToEdge) {
  ...
  do {
    ...
    if (sliced_dim >= 0) {
      int64_t sub_batch_size = is_backward_pass ? std::min<int64_t>(remaining_batch_size, max_sub_batch_size) : remaining_batch_size;
    ...
    @autoreleasepool {
      string key = op_name + getTensorsStringKey({sub_batch_input, sub_batch_grad_output, sub_batch_output}) + ":[" + getArrayRefString(padding) +
          "]:" + std::to_string(constantValue) + std::to_string(sub_batch_start);
      ...
      auto cachedGraph = LookUpOrCreateCachedGraph<CachedGraph>(key, [&](auto mpsGraph, auto newCachedGraph) {
        ...
        if (!is_backward_pass) {
          ...
          newCachedGraph->gradOutputTensor_ = mpsGraphRankedPlaceHolder(mpsGraph, dataType, getMPSShape(sub_batch_grad_output));
      ...
      Placeholder inputPlaceholder = Placeholder(cachedGraph->inputTensor_, sub_batch_input, getMPSShape(sub_batch_input), true, dataType);
      Placeholder outputPlaceholder = Placeholder(cachedGraph->gradInputTensor_, sub_batch_output, getMPSShape(sub_batch_output), true, dataType);
      Placeholder gradOutputPlaceholder = !is_backward_pass
          ? Placeholder()
          : Placeholder(cachedGraph->gradOutputTensor_, sub_batch_grad_output, getMPSShape(sub_batch_grad_output), true, dataType);
