...
(const Tensor& grad_output, const Tensor& output, const Tensor& grad_input) {
  ...
  @autoreleasepool {
    string key =
        "tanh_backward_out_mps:" + getMPSTypeString(grad_output.scalar_type());
    CachedGraph* cachedGraph = static_cast<CachedGraph*>(cache_->LookUp(key));
    if (!cachedGraph) {
      MPSCachedGraph* tmpCachedGraph =
          cache_->CreateCachedGraph(key, ^MPSCachedGraph*() {
 ...
 const Tensor& result) {
  ...
  @autoreleasepool {
    string key = "threshold_out_mps" + getTensorsStringKey({self}) + ":" +
        to_string(threshold.to<double>()) + ":" + to_string(value.to<double>());
      ...
      MPSCachedGraph* tmpCachedGraph =
          cache_->CreateCachedGraph(key, ^MPSCachedGraph*() {
            ...
            @autoreleasepool {
              ...
              MPSGraphTensor* firstHalfOutputTensor =
                  [mpsGraph multiplicationWithPrimaryTensor:sigmoidOutputTensor
                                            secondaryTensor:gradOutputTensor
                                                       name:nil];
      ...
      MPSCachedGraph* tmpCachedGraph =
          cache_->CreateCachedGraph(key, ^MPSCachedGraph*() {
            ...
            @autoreleasepool {
              ...
              MPSGraphTensor* softplusTensor =
                  [mpsGraph multiplicationWithPrimaryTensor:logTensor
...
(const Tensor& grad_output,
 ...
 const Tensor& grad_input) {
  ...
  @autoreleasepool {
    string key =
        "softplus_backward_out_mps:" + getTensorsStringKey({grad_output, self});
