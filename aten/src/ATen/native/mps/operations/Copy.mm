...
static at::Tensor& copy_kernel_mps(at::Tensor& dst_, const at::Tensor& src_, bool non_blocking) {
  ...
  id<MTLBuffer> sourceBuffer = ConstMTLBufferStorage(src).mtl_buffer_unsafe();
  ...
  if (sameDataType) {
    ...
    if (dst_.is_complex() && !supportsComplex()) {
      ...
      auto maybeCastedSourceBuffer = ConstMTLBufferStorage(maybeCastedSource).mtl_buffer_unsafe();
