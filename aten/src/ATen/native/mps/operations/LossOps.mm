...
Tensor& l1_loss_backward_out_mps(
    const Tensor& grad_output,
    const Tensor& input,
    const Tensor& target,
    int64_t reduction,
    Tensor& grad_input)
{
    ...
    @autoreleasepool {
        ...
        string key = "l1_loss_backward_out_mps:" + reductionToString(reduction) + ":" +
                                            [ns_shape_key UTF8String] + ":" +
                                            getMPSTypeString(input.scalar_type()) + ":" +
                                            getMPSTypeString(target.scalar_type());
        CachedGraph* cachedGraph = static_cast<CachedGraph *>(cache_->LookUp(key));
        if(!cachedGraph) {
            MPSCachedGraph *tmpCachedGraph = cache_->CreateCachedGraph(key, ^ MPSCachedGraph * () {
                ...
                @autoreleasepool {
                    ...
                    MPSGraph* mpsGraph = make_mps_graph();
                    newCachedGraph = new CachedGraph(mpsGraph);
                    MPSGraphTensor* gradOutputTensor = mpsGraphRankedPlaceHolder(mpsGraph, getMPSDataType(grad_output.scalar_type()), getMPSShape(grad_output));
                    MPSGraphTensor* inputTensor = mpsGraphRankedPlaceHolder(mpsGraph, getMPSDataType(input.scalar_type()), input_shape);
                    ...
                    MPSGraphTensor* normTensor = [mpsGraph selectWithPredicateTensor:isMeanReductionTensor
                                                                ...
                                                                falsePredicateTensor: gradOutputTensor
                                                                                name:nil];
                    MPSGraphTensor* outputTensor = [mpsGraph subtractionWithPrimaryTensor:inputTensor
                                                                          secondaryTensor:targetTensor
                                                                                     name:nil];
                    outputTensor = [mpsGraph signWithTensor:outputTensor
                                                       name:nil];
