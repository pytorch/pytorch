...
Tensor& eye_out_mps(int64_t n, int64_t m, Tensor& result) {
  ...
  // If the operator to be added is unary or binary, instead of creating a new CachedGraph struct yourself,
  // please consider using `MPSUnaryCachedGraph` or `MPSBinaryCachedGraph` and their corresponding Grad versions in `OperationUtils.h`.
