...
API_AVAILABLE(ios(11.0), macos(10.13))
static Tensor symmetric_pad2d(const Tensor& input, IntArrayRef padding) {
  TORCH_CHECK(input.is_metal());
  ...
  id<MTLComputeCommandEncoder> encoder =
      [commandBuffer.buffer computeCommandEncoder];
  id<MTLComputePipelineState> state = [[MetalContext sharedInstance]
      specializedPipelineState:"symmetric_pad2d"
                     Constants:@[
                       @(Y.height),
                       @(Y.width),
                       @(Y.featureChannels),
...
TORCH_LIBRARY_IMPL(aten, Metal, m) {
  ...
  m.impl(TORCH_SELECTIVE_NAME("aten::symmetric_pad2d"), TORCH_FN(symmetric_pad2d));
}
