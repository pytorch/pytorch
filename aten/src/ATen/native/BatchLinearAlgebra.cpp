...
//  auto pivots_tensor = at::arange(1, k + 1, self.options().dtype(at::kInt)).expand(req_size).contiguous();

  auto pivots_tensor = at::empty(req_size, self.options().dtype(at::kInt)).expand(req_size).contiguous();
  req_size.pop_back();
