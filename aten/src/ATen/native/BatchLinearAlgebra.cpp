...
static void apply_solve(Tensor& b, Tensor& A, std::vector<int64_t>& infos) {
  ...
  for (int64_t i = 0; i < batch_size; i++) {
    ...
    lapackSolve<scalar_t>(
        n, nrhs, A_working_ptr, n, ipiv_data, b_working_ptr, n, &info);
    infos[i] = info;
    if (info != 0) {
      return;
    }
...
std::tuple<Tensor, Tensor> solve(const Tensor& self, const Tensor& A) {
  ...
  std::tie(self_broadcasted, A_broadcasted) =
      _linear_solve_broadcast_args(self, A, "solve");
...
static void apply_inverse(Tensor& self, std::vector<int64_t>& infos) {
  ...
  for (int64_t i = 0; i < batch_size; i++) {
    ...
    lapackGetri<scalar_t>(
        n, self_working_ptr, n, ipiv_data, work_data, lwork, &info);
    infos[i] = info;
    if (info != 0) {
      return;
    }
...
Tensor cholesky_solve(const Tensor& self, const Tensor& A, bool upper) {
  ...
  Tensor self_broadcasted, A_broadcasted;
  std::tie(self_broadcasted, A_broadcasted) =
      _linear_solve_broadcast_args(self, A, "cholesky_solve");
...
static void apply_lu(Tensor& self, Tensor& pivots, Tensor& infos) {
  ...
  for (int64_t i = 0; i < batch_size; i++) {
    ...
    lapackLu<scalar_t>(
        n, n, self_working_ptr, n, pivots_working_ptr, infos_working_ptr);
  }
#endif
}

std::tuple<Tensor, Tensor, Tensor> _lu_with_info_cpu(
    const Tensor& self,
    bool pivot,
    bool check_errors) {
...
static void apply_triangular_solve(
  ...
  for (int64_t i = 0; i < batch_size; i++) {
    ...
    lapackTriangularSolve<scalar_t>(
        uplo, trans, diag, n, nrhs, A_working_ptr, n, b_working_ptr, n, &info);
  }
#endif
}

std::tuple<Tensor, Tensor> _triangular_solve_helper_cpu(
    const Tensor& self,
    const Tensor& A,
    bool upper,
    bool transpose,
...
std::tuple<Tensor, Tensor> triangular_solve(
  ...
  Tensor self_broadcasted, A_broadcasted;
  std::tie(self_broadcasted, A_broadcasted) =
      _linear_solve_broadcast_args(self, A, "triangular_solve");
...
static void apply_orgqr(
  ...
  lapackOrgqr<scalar_t>(
      m, n_columns, k, self_data, m, tau_data, &wkopt, lwork, &info);
  lwork = static_cast<int>(wkopt);
  Tensor work = at::empty({lwork}, self.options());
...
std::tuple<Tensor, Tensor> _qr_helper_cpu(const Tensor& self, bool some) {
  ...
  std::tie(q_sizes, q_strides, n_columns_q) =
      _compute_geometry_for_Q(self, some);
...
static void apply_symeig(
  ...
  int lwork = -1;
  scalar_t wkopt;
  lapackSymeig<scalar_t>(
      jobz, uplo, n, self_data, n, eigvals_data, &wkopt, lwork, &info);
  lwork = static_cast<int>(wkopt);
  Tensor work = at::empty({lwork}, self.options());
...
Tensor lu_solve(
  ...
  TORCH_CHECK(
      ...
      "b should have 2 or 3 dimensions, but has ",
      self.dim(),
      " dimensions instead");
  TORCH_CHECK(
      LU_data.dim() == 3,
      "LU_data should have 3 dimensions, but has ",
      LU_data.dim(),
      " dimensions instead");
  TORCH_CHECK(
      self.size(0) == LU_data.size(0),
      "b and LU_data should have the same number of batches");
  TORCH_CHECK(
      LU_pivots.size(1) == LU_data.size(2),
      "Number of pivots per batch should be same as the dimension of the matrix");
  TORCH_CHECK(
      LU_pivots.size(0) == LU_data.size(0),
  ...
  if (self.dim() == 2) {
    TORCH_WARN(
        "Passing RHS tensor with number of dimensions = 2 is deprecated, "
