...
TORCH_META_FUNC(fractional_max_pool2d) (
  ...
  TORCH_CHECK(ndims == 3 || ndims == 4,
              "fractional_max_pool2d(): Expected non-empty 3D or 4D tensor with optional 0-dim batch size, but got: ",
              input.sizes());
  for (int64_t i = 1; i < ndims; ++i) {
    TORCH_CHECK(input.size(i) > 0,
                "fractional_max_pool2d(): Expected input to have non-zero dimensions for non-batch dimensions, but got",
