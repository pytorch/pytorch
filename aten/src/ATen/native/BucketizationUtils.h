...
// original values given by raw_*. If an original value is not contiguous, will
// make a contiguous copy to the corresponding trimmed_* value. Additionally, if
// the dtypes of the boundary and input tensor do not match, will change them to
// be a common super type so comparisons are done between the same types. For
// any trimmed_* tensor, if its outgoing value matches what it was incoming
// (typically null), then the corresponding raw_* version should be used since
// it was already contiguous of the right type.
inline void searchsorted_maybe_trim_input_tensors(
  ...
  if (raw_input.dtype() != raw_boundaries.dtype()) {
    ...
    if (common_stype != raw_input.scalar_type()) {
      trimmed_input = in_is_contiguous ? raw_input.to(common_stype)
                                       : trimmed_input.to(common_stype);
    }
    if (common_stype != raw_boundaries.scalar_type()) {
      trimmed_boundaries = bd_is_contiguous
          ? raw_boundaries.to(common_stype)
          : trimmed_boundaries.to(common_stype);
    }
  }
}
...
inline bool searchsorted_dims_matched_before_last_dim(
...
inline Tensor searchsorted_scalar_tensor(
    const Scalar& scalar,
    const c10::Device& device) {
  auto tensor = c10::scalar_to_tensor(scalar, device);
  // This is to adopt the scalar promotion rules defined in
  // native/TypeProperties.h So we have the same type promotion rules as binary
  // operations.
...
inline void searchsorted_pre_check(
  ...
  if (side_opt) {
    ...
    TORCH_CHECK(
        !right || side == "right",
        "torch.searchsorted(): side and right can't be set to opposites, got side "
        "of ",
        side,
  ...
  TORCH_CHECK(
      ...
      "should have same device type, but got boundaries tensor device type ",
      boundaries.device(),
      " and input value ",
      "tensor device type ",
      input.device());
  ...
  if (sorter.defined()) {
    TORCH_CHECK(
        ...
        "have same device type, but got sorter tensor device type ",
        sorter.device(),
    ...
    TORCH_CHECK(
        ...
        "size, but got boundary tensor ",
        boundaries.sizes(),
        "and got sorter tensor ",
        sorter.sizes());

    TORCH_CHECK(
        sorter.scalar_type() == ScalarType::Long,
        "torch.searchsorted(): sorter must be a tensor of long ",
        "dtype but got dtype ",
        sorter.scalar_type());
  }
  ...
  TORCH_CHECK(
      boundaries.dim() != 0,
      "torch.searchsorted(): boundaries tensor should have positive dimension, but ",
      "got 0 dimension");

  TORCH_CHECK(
      boundaries.dim() == 1 ||
          searchsorted_dims_matched_before_last_dim(boundaries, input),
      "torch.searchsorted(): boundaries tensor should be 1 dimension or the first N-1 dimensions of boundaries tensor ",
      "and input value tensor must match, but we got boundaries tensor ",
      boundaries.sizes(),
      " and input value tensor ",
      input.sizes());
