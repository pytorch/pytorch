...
inline void searchsorted_maybe_trim_input_tensors(
  ...
  if (!in_is_contiguous) {
    TORCH_WARN_ONCE(
        "input value tensor is non-contiguous, this will lower the performance due to extra data copy "
...
inline void searchsorted_pre_check(
  ...
  TORCH_CHECK(
      ...
      "boundaries and input value tensors should have same device type, ",
      "but we got boundaries tensor device type ",
      boundaries.device(),
  ...
  TORCH_CHECK(
      boundaries.dim() != 0,
      "boundaries tensor should have positive dimension, but got 0 dimension");
  ...
  TORCH_CHECK(
      ...
      "but we got output tensor's dtype ",
      output_dtype,
  ...
  if (out_int32) {
    TORCH_CHECK(
        ...
        "the size of boundaries' last dimension should be less than ",
        INT_MAX,
        ", but we got ",
