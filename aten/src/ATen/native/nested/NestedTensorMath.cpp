...
get_elementwise_nested_tensor_impl(
  ...
  Tensor broadcasted_self, broadcasted_other;
  if (self.is_nested() && !(other.is_nested())) {
    auto broadcasted_tensors = _broadcast_nested_dense(self, other);
    broadcasted_self = std::get<0>(broadcasted_tensors);
    broadcasted_other = std::get<1>(broadcasted_tensors);
  } else if (!(self.is_nested()) && other.is_nested()) {
    auto broadcasted_tensors = _broadcast_nested_dense(other, self);
    broadcasted_self = std::get<1>(broadcasted_tensors);
    broadcasted_other = std::get<0>(broadcasted_tensors);
  } else if (!(self.is_nested()) || !(other.is_nested())) {
    TORCH_CHECK(
        false,
        "Expected either self or other to be nested, but got a non-nested self and non-nested other");
  } else {
    broadcasted_self = self;
    broadcasted_other = other;
  }

  auto self_ptr = get_nested_tensor_impl(self);
  auto other_ptr = get_nested_tensor_impl(other);

  // FIXME: this check going to fail, we need an if statement to fix this
  TORCH_CHECK(
