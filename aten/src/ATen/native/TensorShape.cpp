...
static inline void check_cat_shape_except_dim(
  ...
  TORCH_CHECK(
      ...
      "torch.cat(): Tensors must have same number of dimensions: got ",
      first_dims,
      " and ",
      second_dims);
  for (int64_t dim = 0; dim < first_dims; dim++) {
    ...
    TORCH_CHECK(
        ...
        "torch.cat(): Sizes of tensors must match except in dimension ",
        dimension,
        ". Got ",
        first_dim_size,
        " and ",
...
Tensor& _cat_out_cpu(TensorList tensors, int64_t dim, Tensor& result) {
  ...
  // not empty
  allContiguous =
      allContiguous && result.is_contiguous(first_tensor_mem_format);
  ...
  int64_t offset = 0;
  if (reuse_iterator && result.is_contiguous(first_tensor_mem_format) &&
      no_type_promotion) {
...
std::vector<Tensor> tensor_split(
  ...
  if (split_dim == 0) {
    ...
    std::vector<int64_t> indices(
        indices_data, indices_data + tensor_indices_or_sections.numel());
...
Tensor index_select_sparse(
  ...
  if (ndim == 0) {
    TORCH_CHECK_INDEX(
        false, "index_select() cannot be applied to a 0-dim tensor.");
  }
  if (!(index.dim() == 1 && index.dtype() == at::kLong)) {
    TORCH_CHECK_INDEX(
        false, "index_select() argument index must be 1-D long-tensor.");
  }
  dim = maybe_wrap_dim(dim, ndim);
  auto size = self.size(dim);
...
bool inline can_use_native_serial_stack(
    Tensor& result,
    TensorList tensors,
    int64_t dim) {
  ...
  if (dim >= firstTensor.dim())
    return false;
  // Native stack doesn't apply any tensor is skipped.
  if (should_skip(firstTensor))
    return false;
  // there should be no type promotion
  if (result.dtype() != firstTensor.dtype())
    return false;
  ...
  for (size_t i = 1; i < tensors.size(); i++) {
    auto const& tensor = tensors[i];
    TORCH_CHECK(
        tensors[i].sizes() == firstTensor.sizes(),
        "stack expects each tensor to be equal size, but got ",
        first_tensor_shape,
        " at entry 0 and ",
        tensor.sizes(),
    ...
    // there should be no type promotion
    if (!tensor.is_contiguous(first_tensor_mem_format) ||
        tensor.strides() != firstTensor.strides() || tensor.dtype() != dtype) {
      return false;
    }
  }
