...
Tensor _shape_as_tensor(const Tensor& self) {
  auto options =
      TensorOptions(at::kLong).is_variable(self.options().is_variable());
...
Tensor as_strided_qtensorimpl(
  ...
  auto result = detail::make_tensor<QTensorImpl>(
      Storage(self.storage()), tid, get_qtensorimpl(self)->quantizer());
  setStrided(result, size, stride, storage_offset);
  return result;
}
...
Tensor reshape(const Tensor& self, IntArrayRef proposed_shape) {
  ...
  if (auto stride =
          THTensor_compute_stride(self.sizes(), self.strides(), shape)) {
...
Tensor alias(const Tensor& self) {
  ...
  if (self.is_quantized()) {
    ...
    auto impl = c10::make_intrusive<TensorImpl>(
        Storage(self.storage()), self.type_id());
    impl->set_storage_offset(self.storage_offset());
    impl->set_sizes_and_strides(self.sizes(), self.strides());
    self_ = Tensor(std::move(impl));
  }
