...
Tensor sparse_broadcast_to(const Tensor& self, IntArrayRef size) {
  ...
  return at::sparse_coo_tensor(new_indices, new_values, size, is_coalesced);
}
...
Tensor narrow_copy_sparse(const Tensor& self, int64_t dim, int64_t start, int64_t length) {
  ...
  return at::sparse_coo_tensor(new_indices, new_values, new_sizes, self.is_coalesced());
}
...
Tensor permute_sparse_coo(const Tensor& self, IntArrayRef dims) {
  ...
  return _sparse_coo_tensor_with_dims_and_tensors(
      sparse_ndim, dense_ndim, new_sizes, new_indices, new_values, is_coalesced, self.options());
}
