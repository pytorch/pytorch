...
Tensor _reshape_from_tensor(const Tensor& self, const Tensor& shape_tensor) {
  ...
  return self.reshape(IntListRef(shape));
}
...
static bool sizes_match_except(IntListRef s1, IntListRef s2, int64_t dim_except /* should already be wrapped */) {
  if (s1.size() != s2.size()) {
    return false;
  }
...
static Tensor cat_sparse(TensorList tensors, int64_t dim) {
  ...
  IntListRef sizes = tensors[0].sizes();
...
Tensor expand(const Tensor& self, IntListRef size, bool implicit) {
  // [expand implicit]
...
Tensor sum_to_size(const Tensor& self, IntListRef size) {
...
Tensor as_strided(const Tensor& self, IntListRef size, IntListRef stride, optional<int64_t> storage_offset_) {
...
Tensor &as_strided_(Tensor& self, IntListRef size, IntListRef stride, optional<int64_t> storage_offset_) {
  auto storage_offset = storage_offset_.value_or(self.storage_offset());
...
Tensor permute(const Tensor& self, IntListRef dims) {
  auto nDims = self.dim();
...
Tensor repeat(const Tensor& self, IntListRef repeats) {
...
Tensor reshape(const Tensor& self, IntListRef proposed_shape) {
  if (self.is_sparse()) {
...
std::vector<Tensor> split_with_sizes(const Tensor& self, IntListRef split_sizes, int64_t dim) {
  AT_CHECK(self.dim() != 0, "split expects at least a 1-dimensional tensor");
  int64_t dim_size = self.size(dim);
...
Tensor _unsafe_view(const Tensor& self, IntListRef size) {
