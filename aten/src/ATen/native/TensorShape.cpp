...
Tensor & cat_out(Tensor & result, TensorList tensors, int64_t dim, bool pad, Scalar pad_value) {
  check_cat_no_zero_dim(tensors);
  dim = legacy_cat_wrap_dim(dim, tensors);
  return at::_cat_out(result, tensors, dim, pad, pad_value);
}

Tensor cat(TensorList tensors, int64_t dim, bool pad, Scalar pad_value) {
  check_cat_no_zero_dim(tensors);
  dim = legacy_cat_wrap_dim(dim, tensors);
