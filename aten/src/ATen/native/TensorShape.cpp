...
(const ITensorListRef& tensors,
 ...
 const Tensor& result) {
  ...
  bool use_fast_kernel = all_contiguous && result.is_contiguous() &&\
    (dim == 0 || dim == 1) && all_same_dtype && serial_dtype;
  if (use_fast_kernel) {
    cat_fast_stub(kCPU, result, materialized, dim);
    return;
  }
