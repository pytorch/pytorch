...
struct DimCounter {
  DimCounter(IntListRef shape, Range range);
...
struct CAFFE2_API TensorIterator {
  ...
  IntListRef shape() const { return shape_; }
  int64_t numel() const;
  ...
  IntListRef strides(int arg) const { return operands_[arg].stride_bytes; }
  ...
  void select_all_keeping_dim(int start_dim, IntListRef starts);
  /// Replaces the data pointer and strides for the operand at index `arg`
  void replace_operand(int arg, void* data, IntListRef stride);
  ...
  DimVector invert_perm(IntListRef input) const;
  ...
  PtrVector get_data_ptrs(ArrayRef<char*> base, IntListRef counter) const;
...
protected:
  ...
  void permute_dimensions(IntListRef perm);
  void compute_types();
