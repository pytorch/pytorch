...
bool copy_transpose_valid(const Tensor& self, const Tensor& src) {
  ...
  return self.is_contiguous() && src.numel() != 0 && src.dim() == 2 &&
      ...
      self.sizes().equals(src.sizes()) && self.is_neg() == src.is_neg() &&
      self.is_conj() == src.is_conj() && self.numel() >= MIN_SZ;
}
...
void copy_same_type_transpose_(Tensor& self, const Tensor& src) {
  ...
  AT_DISPATCH_ALL_TYPES_AND_COMPLEX_AND4(
      kHalf, kBool, kBFloat16, kComplexHalf, self.scalar_type(), "copy_", [&] {
...
bool is_supported_device(Device device) {
  ...
  return device_type == kCPU || device_type == kCUDA || device_type == kHIP ||
      device_type == kVulkan || device_type == kMetal || device_type == kMPS;
}
...
// 3. dtype conversion between FP32->FP16 and FP16->FP32.
  ...
  if (self.device().type() == at::kVulkan ||
...
Tensor copy(const Tensor& self, const Tensor& src, bool non_blocking) {
  // copy() is the "functional" form of copy_(). It exists so we can properly
  // functionalize copy_(), but: (1) It isn't exposed to the frontend (no python
  // bindings) (2) It isn't exposed to the backend (it's a composite, that
  // decomposes into to() and expand_as() calls. Note: This implementation
  // doesn't currently preserve the strides of `self`. That might be fine for
  // functorch (which already doesn't preserve strides in vmap), but it's worth
  // looking into whether or not this implementation will be problematic for
  // LazyTensor/XLA.
