...
static Tensor convolution_transpose_same(
  ...
  TORCH_CHECK(dilation.size() == dim || dilation.size() == 1U,
              "dilation cannot broadcast to ", dim, " dimensions");
  TORCH_CHECK(output_padding.size() == dim || output_padding.size() == 1U,
              "output_padding cannot broadcast to ", dim, " dimensions");

  // TODO: Need to check if this restriction is necessary.
  // This restriction is here because it exists for traditional convolutions.
  for (auto i: c10::irange(stride.size())) {
    TORCH_CHECK(stride[i] == 1, "padding='same' is not supported for strided convolutions");
  }

  // TODO: Need to check if `output_padding` makes sense in transposed convolution with padding = "same"
  for (auto i: c10::irange(output_padding.size())) {
    TORCH_CHECK(output_padding[i] == 0, "padding='same' only supports output_padding=0");
  }
