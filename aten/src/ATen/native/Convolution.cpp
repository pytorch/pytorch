...
auto ConvParams::use_cpu_depthwise3x3_winograd(
  ...
  return (input.ndimension() == 4) && (input.size(1) == groups) &&
      ...
      (weight.size(1) == 1) && (weight.size(2) == 3) && (weight.size(3) == 3) &&
      (input.device().is_cpu()) && (input.scalar_type() == at::kFloat) &&
      input.is_contiguous() && (weight.device().is_cpu()) &&
...
auto ConvParams::needs_64bit_indexing_no_split(
    const at::Tensor& input,
    const at::Tensor& weight) const -> bool {
  ...
  int64_t outsize = 1;
  if (transposed) {
    std::vector<int64_t> o = conv_input_size(
        input.sizes(),
        weight.sizes(),
        padding,
        output_padding,
    ...
    std::vector<int64_t> o = conv_output_size(
...
auto ConvParams::use_cudnn(const at::Tensor& input, const at::Tensor& weight)
    const -> bool {
// Note [Mobile check segfaults]
// cudnn and miopen are guaranteed not to be on mobile, and T102591915 /
// T110194934 suggest that maybe the compiledWithCuDNN() check sometimes
// segfaults (though I can't imagine how)
  ...
  if (cudnn_conv_suggest_memory_format(input, weight) ==
...
auto ConvParams::use_mps(const at::Tensor& input, const at::Tensor& weight)
    const -> bool {
...
auto ConvParams::use_miopen(
    const at::Tensor& input,
    const at::Tensor& weight,
    bool bias_defined) const -> bool {
  if (needs_64bit_indexing_no_split(input, weight)) {
    return false;
  }
  return ((input.scalar_type() == at::kFloat) ||
          (input.scalar_type() == at::kHalf) ||
          (input.scalar_type() == at::kBFloat16)) &&
      detail::getCUDAHooks().compiledWithMIOpen() && input.is_cuda() &&
      input.dim() <= MIOPEN_DIM_MAX &&
      !(groups > 1 && is_dilated()) // MIOpen currently does not support
                                    // dilation with groups of size > 1
      && !(input.scalar_type() == at::kBFloat16 &&
           bias_defined) // MIOpen currently doesn't support bias with bfloat16
      && cudnn_enabled;
}

auto ConvParams::use_mkldnn(const at::Tensor& input, const at::Tensor& weight)
    const -> bool {
#if AT_MKLDNN_ENABLED()
  ...
  return (input.is_mkldnn()) || // input is mkldnn Tensor
      (input.device().is_cpu() &&
       ...
       (groups > 1 || (weight.size(-1) > 3 && weight.size(-2) > 3) ||
        input.size(0) > 1 ||
...
auto ConvParams::use_nnpack(const at::Tensor& input, const at::Tensor& weight)
    const -> bool {
#if AT_NNPACK_ENABLED()
  return at::_nnpack_available() && input.device().is_cpu() &&
      input.scalar_type() == kFloat && // only on CPU Float Tensors
      !is_dilated() && // or dilation
      !transposed && // or transposed tensors
      input.ndimension() == 4 && // must be in NCHW format
      weight.ndimension() == 4 && (weight.size(2) < 17) &&
      (weight.size(3) < 17) // NNPACK only supports kernels up to 16x16
#if !defined(C10_MOBILE)
      && input.size(0) >=
      16 // ensure large enough batch size to ensure perf, tuneable
#endif
      ;
#endif
  return false;
...
auto ConvParams::is_depthwise(const at::Tensor& input, const at::Tensor& weight)
    const -> bool {
  return input.is_cuda() && !transposed &&
...
bool check_cudnn_depthwise_workload_with_filter(
    const at::Tensor& input,
    int stride,
    const at::Tensor& weight) {
  // 1D conv
  ...
  if (filter != 1 && filter != 3 && filter != 5)
    return false;
  // we don't enforce square input but only check width to reduce heuristic
  // space
  if (input.size(3) < 7)
    return false; // min width 7
  int w = input.size(3);
  ...
  if (bs == 1) {
    ...
    if (filter == 1 && bs <= 16 && ch >= 128 && w <= 7)
      return true;
    if (filter == 3 || filter == 5) {
      if ((ch >= 512) || (ch >= 256 && w >= 28))
        return true;
    }
  }
  return false;
    ...
    if (cudnn_version >= 8200) {
      bool kernel_cond =
          (use_cudnn(input, weight) &&
           ...
           weight.scalar_type() == kHalf && is_depthwise(input, weight) &&
           input.ndimension() == 4 && // TODO: 5-D contiguous depthwise is not
                                      // supported yet, need benchmarks
           !is_dilated() && // no dilation supported
           (stride[0] == stride[1] || input.size(2) == 1) && // square or 1d
           input.size(1) >= 32); // min 32 channels supported)
      if (kernel_cond) {
        return check_cudnn_depthwise_workload_with_filter(
            input, stride[1], weight);
      }
    }
    // keep (7600 <= cudnn < 8200) code unchanged
    bool kernel_cond =
        (cudnn_version >= 7600 && use_cudnn(input, weight) &&
         input.scalar_type() == kHalf && // only for FP16
         weight.scalar_type() == kHalf && is_depthwise(input, weight) &&
         input.ndimension() == 4 && // TODO: 5-D contiguous depthwise is not
                                    // supported yet, need benchmarks
...
// Given an input tensor and an expected number of spatial dimensions, checks
// that the input is a valid shape and returns the batched form of the input.
//
// Args:
//     input (Tensor): Input tensor
//     num_spatial_dims (int): Number of spatial dimensions expected for the
//     input func_name (string): Function name to produce a nice error message
//     for invalid input
//
// Returns a std::tuple containing:
//     batched_input (Tensor): Input with a batch dimension
//     is_batched (bool): Indicates whether the original input was already
//     batched
...
static void check_input_same_type_as_parameters(
  ...
  if (backend == ConvBackend::Mkldnn) {
    TORCH_CHECK(
        input.options().type_equal(weight.options()) ||
            (input.is_mkldnn() && weight.device().is_cpu() &&
    ...
    TORCH_CHECK(
        !bias.defined() || (input.options().type_equal(bias.options())) ||
            (input.is_mkldnn() && bias.device().is_cpu() &&
...
at::Tensor complex_convolution(
  ...
  // conv(W, x, b) = conv(Wr, xr, br) - conv(Wi, xi, 0) + i(conv(Wi, xr, bi) +
  // conv(Wr, xi, 0)) where W, x and b are all complex inputs. With Gauss Trick:
  // a = conv(Wr, xr, br),
  // b = conv(Wi, xi, 0),
...
at::Tensor complex_convolution_mode(
  ...
  Tensor a, b, c;
  if (!bias.defined()) {
    a = at::_convolution_mode(
        i_r, w_r, bias, stride, padding, dilation, groups);
    b = at::_convolution_mode(
        i_i, w_i, bias, stride, padding, dilation, groups);
    c = at::_convolution_mode(
        i_r + i_i, w_r + w_i, bias, stride, padding, dilation, groups);
  } else {
    Tensor b_r, b_i;
    std::tie(b_r, b_i) = complex_to_real(bias.resolve_conj());
    a = at::_convolution_mode(i_r, w_r, b_r, stride, padding, dilation, groups);
    b = at::_convolution_mode(
        i_i, w_i, Tensor(), stride, padding, dilation, groups);
    c = at::_convolution_mode(
        i_r + i_i, w_r + w_i, b_r + b_i, stride, padding, dilation, groups);
  }
...
at::Tensor conv1d(
  ...
  std::tie(input, is_batched) =
      batchify(input_, /*num_spatial_dims=*/1, "conv1d");
  Tensor output;
  if (at::isComplexType(input_.scalar_type())) {
    output = complex_convolution(
        input, weight, bias, stride, padding, dilation, {0}, groups);
  } else {
...
at::Tensor conv2d(
  ...
  if (at::isComplexType(input_.scalar_type())) {
    output = complex_convolution(
        input, weight, bias, stride, padding, dilation, {{0, 0}}, groups);
  } else {
    output = at::convolution(
        input,
        weight,
...
at::Tensor conv3d(
  ...
  bool is_batched;
  std::tie(input, is_batched) =
      batchify(input_, /*num_spatial_dims=*/3, "conv3d");
  Tensor output;
  if (at::isComplexType(input_.scalar_type())) {
    output = complex_convolution(
        input, weight, bias, stride, padding, dilation, {{0, 0, 0}}, groups);
  } else {
    output = at::convolution(
        input,
        weight,
...
static Tensor convolution_same(
  ...
  TORCH_WARN_ONCE(
      "Using padding='same' with even kernel lengths and odd dilation may"
      " require a zero-padded copy of the input be created");
  SmallVector<int64_t, kDimVectorStaticSize * 2> pad_nd(
...
at::Tensor conv1d(
  ...
  Tensor output;
  if (at::isComplexType(input_.scalar_type())) {
    output = complex_convolution_mode(
        input, weight, bias, stride, std::move(padding), dilation, groups);
  } else {
    output = at::_convolution_mode(
        input, weight, bias, stride, std::move(padding), dilation, groups);
  }
  return is_batched ? output : output.squeeze(0);
}
...
at::Tensor conv_transpose1d(
  ...
  bool is_batched;
  std::tie(input, is_batched) =
      batchify(input_, /*num_spatial_dims=*/1, "conv_transpose1d");
...
at::Tensor conv_transpose2d(
  ...
  bool is_batched;
  std::tie(input, is_batched) =
      batchify(input_, /*num_spatial_dims=*/2, "conv_transpose2d");
  auto output = at::convolution(
      input,
      weight,
      bias,
...
at::Tensor conv_transpose3d(
  ...
  bool is_batched;
  std::tie(input, is_batched) =
      batchify(input_, /*num_spatial_dims=*/3, "conv_transpose3d");
  auto output = at::convolution(
      input,
      weight,
      bias,
...
at::Tensor convolution(
  ...
  bool deterministic =
      ctx.deterministicCuDNN() || ctx.deterministicAlgorithms();
...
ConvBackend select_conv_backend(
  ...
  auto bias_sizes_opt =
      bias.defined() ? c10::optional<IntArrayRef>(bias.sizes()) : c10::nullopt;
  bool need_backward = GradMode::is_enabled() &&
      (input.requires_grad() || weight.requires_grad() ||
       (bias.defined() && bias.requires_grad()));
  return select_conv_backend(
      input, weight, bias_sizes_opt, need_backward, params);
}
...
ConvBackend select_conv_backend(
  ...
  if (input.size(0) == 0 || input.size(1) == 0) {
    ...
    TORCH_CHECK(
        false,
        "Only zero batch or zero channel inputs are supported, but got input shape: ",
        input.sizes());
  }
  ...
  if (params.is_depthwise(input, weight)) {
    ...
    return ConvBackend::Mkldnn;
      ...
      !need_backward && params.use_xnnpack(input, weight, bias_sizes_opt)) {
    ...
    // 3x3 depthwith convolutions implementation is inference only
      ...
      !need_backward && params.use_cpu_depthwise3x3_winograd(input, weight)) {
    return ConvBackend::Winograd3x3Depthwise;
  } else if (
      !params.transposed && (input.ndimension() == 5) &&
      (input.device().is_cpu()) && !params.is_dilated()) {
    ...
    if (params.transposed) {
      ...
      if (input.ndimension() == 4) {
        if (params.is_dilated()) {
          ...
          input.ndimension() == 5 && (input.is_cuda() || params.is_dilated())) {
  ...
  switch (backend) {
    case ConvBackend::NnpackSpatial:
      ...
      TORCH_INTERNAL_ASSERT(
          false,
          "NnpackSpatial backend was selected in PyTorch compiled without nnpack support");
#endif
    case ConvBackend::Slow2d:
      return at::thnn_conv2d(
          input, weight, kernel_size, bias, params.stride, params.padding);
  ...
  switch (backend) {
    ...
    case ConvBackend::MiopenTranspose:
      if (detail::getCUDAHooks().compiledWithMIOpen() &&
        ...
        backend_memory_format = (k == 5)
            ? at::MemoryFormat::Contiguous /*at::MemoryFormat::ChannelsLast3d*/
...
at::Tensor _convolution(
    const Tensor& input_r,
    const Tensor& weight_r,
    const c10::optional<Tensor>& bias_r_opt,
    IntArrayRef stride_,
    IntArrayRef padding_,
    IntArrayRef dilation_,
    bool transposed_,
  ...
  bool need_backward = GradMode::is_enabled() &&
      (input.requires_grad() || weight.requires_grad() ||
       (bias.defined() && bias.requires_grad()));
  ConvBackend backend =
      select_conv_backend(input, weight, bias_sizes_opt, need_backward, params);
  at::MemoryFormat backend_memory_format =
      determine_backend_memory_format(input, weight, backend);
  ...
  switch (backend) {
    ...
    case ConvBackend::Mkldnn:
      ...
      TORCH_INTERNAL_ASSERT(
          false,
          "Mkldnn backend was selected in PyTorch compiled without mkldnn support");
#endif
      break;
    case ConvBackend::MkldnnEmpty:
#if AT_MKLDNN_ENABLED()
      output = empty_mkldnn(
          calc_output_size(input, weight, params),
    ...
    case ConvBackend::SlowTranspose3d:
      ...
      if (params.groups == 1) {
        output =
            _convolution_nogroup_backend(input, weight, bias, backend, params);
      } else {
        ...
        for (const auto g : c10::irange(params.groups)) {
          ...
          outputs[g] = _convolution_nogroup_backend(
              input_g, weight_g, bias_g, backend, params);
        }
...
#else
      TORCH_INTERNAL_ASSERT(
          false, "MPS backend was selected in PyTorch without support");
#endif
      break;
...
std::tuple<Tensor, Tensor, Tensor> convolution_backward_overrideable(
  ...
  AT_ERROR(
      "You are likely triggering this with tensor backend other than CPU/CUDA/MKLDNN, if this is intended, please use TORCH_LIBRARY_IMPL to override this function ");
...
std::tuple<Tensor, Tensor, Tensor> _convolution_double_backward(
    const c10::optional<Tensor>& ggI_opt,
    const c10::optional<Tensor>& ggW_r_opt,
    const c10::optional<Tensor>& ggb_opt,
    const Tensor& gO_r,
    const Tensor& weight_r,
    const Tensor& input,
    IntArrayRef stride_,
    ...
    std::array<bool, 3> output_mask) {
  // See [Note: hacky wrapper removal for optional tensor]
  c10::MaybeOwned<Tensor> ggI_maybe_owned =
      at::borrow_from_optional_tensor(ggI_opt);
  ...
  if (!params.transposed && input.dim() > 4) {
    // Avoid undefined behavior when num channels == 0; params are unused for
    // that case.
      ...
      if (params.transposed) {
        ...
        for (const auto i : c10::irange(kernel_size.size())) {
          ...
          auto expected_input_shape =
              (kernel_size[i] - 1) * gi_conv_params.dilation[i] -
  ...
  switch (backend) {
    case ConvBackend::Slow2d:
      return at::_slow_conv2d_backward(
          ...
          params.padding,
          output_mask);
    // NB: nnpack backward does not support strided convolutions; use slow impl
    // instead
    case ConvBackend::NnpackSpatial:
    case ConvBackend::SlowDilated2d:
...
// Backward pass for convolution. Computes gradients for input, weight, and bias
// depending on the output_mask setting. This function supports 1D, 2D, or 3D
// spatial convolution and currently requires a single batch dimension to be
// present.
//
// Args:
//   grad_output_: tensor of shape (N, C_out, L_out), (N, C_out, H_out, W_out),
//   or (N, C_out, D_out, H_out, W_out) input_: tensor of shape (N, C_in, L_in),
//   (N, C_in, H_in, W_in), or (N, C_in, D_in, H_in, W_in) weight_: tensor of
//   shape (C_out, C_in // groups, *kernel_size); dimension of kernel_size must
//   match the number
...
//   stride: single value or an array with dimension matching the number of
//   input spatial dimensions padding: single value or an array with dimension
//   matching the number of input spatial dimensions dilation: single value or
//   an array with dimension matching the number of input spatial dimensions
//   transposed: boolean indicating whether the convolution is transposed
//   output_padding: single value or dimension == number of input spatial
//   dimensions; only supported when
//       transposed is true
//   groups: number of groups for grouped convolution
//   output_mask: 3-dim boolean array specifying which gradients to compute in
...
std::tuple<Tensor, Tensor, Tensor> convolution_backward(
  ...
  if (!params.transposed) {
    for (auto pad : params.output_padding) {
      TORCH_CHECK(
          pad == 0,
          "output_padding is not supported for non-transposed convolutions; got: ",
          params.output_padding);
    }
  }
  ...
  // Select appropriate backend to use.
  ConvBackend backend = select_conv_backend(
      input, weight, bias_sizes_opt, /*need_backward=*/true, params);
  at::MemoryFormat backend_memory_format =
      determine_backend_memory_format(input, weight, backend);
  ...
  switch (backend) {
    case ConvBackend::CudaDepthwise2d: {
      std::array<bool, 2> input_weight_output_mask = {
    ...
    case ConvBackend::Cudnn: {
      ...
      std::tie(backend_grad_input, backend_grad_weight) =
          cudnn_convolution_backward_stub(
              input.device().type(),
              // Only make input contiguous when it is necessary for the
    ...
    case ConvBackend::MpsTranspose: {
      ...
      std::array<bool, 2> input_weight_output_mask = {
          output_mask[0], output_mask[1]};
      std::tie(backend_grad_input, backend_grad_weight) =
          at::mps_convolution_transpose_backward(
              // Only make input contiguous when it is necessary for the
              // backwards computation
              output_mask[1] ? input.contiguous(backend_memory_format) : input,
              grad_output,
    ...
    case ConvBackend::MkldnnEmpty:
      ...
      if (output_mask[0]) {
        if (input.is_mkldnn()) {
          backend_grad_input = empty_mkldnn(
              input.sizes(),
              optTypeMetaToScalarType(input.options().dtype_opt()),
              input.options().layout_opt(),
              input.options().device_opt(),
    ...
    case ConvBackend::Mkldnn:
      ...
      break;
    case ConvBackend::Overrideable:
      // Only reach here when input is backend with out-of-source
      // implementation.
      std::tie(backend_grad_input, backend_grad_weight, backend_grad_bias) =
          at::convolution_backward_overrideable(
              grad_output,
              input,
              weight,
              params.stride,
    ...
    case ConvBackend::SlowTranspose3d: {
      ...
      if (params.groups == 1) {
        ...
        for (int g = 0; g < params.groups; ++g) {
          ...
          auto weight_g = subtensor(weight, 0, params.groups, g);
          std::tie(
              backend_grad_inputs[g],
              backend_grad_weights[g],
              backend_grad_biases[g]) =
              _convolution_backward_nogroup_backend(
                  grad_output_g,
                  input_g,
                  weight_g,
    ...
    case ConvBackend::Winograd3x3Depthwise:
      TORCH_CHECK(
          false, "Backward is not supported for depthwise 3x3 winograd");
      break;
    case ConvBackend::Xnnpack2d:
