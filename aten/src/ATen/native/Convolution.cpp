...
struct ConvParams {
  ...
  bool use_cudnn_depthwise(const at::Tensor& input, const at::Tensor& weight) const  {
    ...
    if (detail::getCUDAHooks().supportsDepthwiseConvolutionWithCuDNN()) {
      ...
      if (kernel_cond) {
        at::CuDNNForce force_cudnn = at::globalContext().userForceCuDNN();
        if (force_cudnn == at::CuDNNForce::OFF) {
          return false;
        } else if (force_cudnn == at::CuDNNForce::ON) {
          return true;
        } else {
          // default: force_cudnn == at::CuDNNForce::HEURISTIC
