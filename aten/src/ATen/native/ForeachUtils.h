...
void check_foreach_api_restrictions(TensorList tensors1, TensorList tensors2) {
  TORCH_CHECK(
      tensors1.size() > 0, "Tensor list must have at least one tensor.");
  TORCH_CHECK(
      tensors2.size() > 0, "Tensor list must have at least one tensor.");
  TORCH_CHECK(
  ...
  for (const auto i : c10::irange(tensors1.size())) {
    TORCH_CHECK(
        tensors1[i].dtype() == expected_dtype,
        "All tensors in the tensor list must have the same dtype.");
    TORCH_CHECK(
        tensors2[i].dtype() == expected_dtype,
        "All tensors in the tensor list must have the same dtype.");
    TORCH_CHECK(
        tensors1[i].sizes() == tensors2[i].sizes(),
        "Corresponding tensors in lists must have the same size, got ",
        tensors1[i].sizes(),
        " and ",
        tensors2[i].sizes());
  }
...
void check_foreach_api_restrictions(
  ...
  TORCH_CHECK(
      tensors2.size() > 0, "Tensor list must have at least one tensor.");
  TORCH_CHECK(
      tensors3.size() > 0, "Tensor list must have at least one tensor.");
  TORCH_CHECK(
      tensors1.size() == tensors2.size(),
      "Tensor lists must have the same number of tensors, got ",
      tensors1.size(),
  ...
  for (const auto i : c10::irange(tensors1.size())) {
    ...
    TORCH_CHECK(
        ...
        tensors2[i].sizes());
    TORCH_CHECK(
        tensors1[i].sizes() == tensors3[i].sizes(),
        "Corresponding tensors in lists must have the same size, got ",
        tensors1[i].sizes(),
        " and ",
        tensors3[i].sizes());
...
bool check_fast_path_restrictions(
  ...
  // For all j, tensorList[j][0] have the same shape and dtype. (this was a
  // precondition checked by `check_foreach_api_restrictions`). This means we
  // only need to check if {tensorList[0][0], tensorList[0][1],
  // tensorList[0][2], ...} do type promotion with scalarLIst.
  for (int i = 0; i < tensorLists[0].size(); i++) {
    ...
    if (scalarList.size() == 1) {
      ...
      // Complex scalar list is not supported due to the limit for kernel launch
      // argument (4KB)
