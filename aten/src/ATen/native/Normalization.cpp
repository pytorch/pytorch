...
/// directory.
template <typename scalar_t>
void batch_norm_cpu_inference_contiguous(
    Tensor& output,
    const Tensor& input,
  ...
  const scalar_t* weight_data =
      weight.defined() ? weight.data<scalar_t>() : nullptr;
...
std::tuple<Tensor, Tensor, Tensor> batch_norm_cpu_transform_input_template(
  ...
  parallel_for(0, n_input, 1, [&](int64_t b_begin, int64_t b_end) {
    for (int64_t f = b_begin; f < b_end; ++f) {
      ...
      // compute output
      scalar_t w =
          weight.defined() ? weight.data<scalar_t>()[f * weight.stride(0)] : 1;
      scalar_t b =
          bias.defined() ? bias.data<scalar_t>()[f * bias.stride(0)] : 0;

      CPU_tensor_apply2<scalar_t, scalar_t>(
          out, in, [&](scalar_t& o, const scalar_t& i) {
            o = ((i - mean) * invstd) * w + b;
          });
    }
  });
...
std::tuple<Tensor, Tensor> batch_norm_cpu_update_stats_template(
  ...
  parallel_for(0, n_input, 1, [&](int64_t b_begin, int64_t b_end) {
    for (int64_t f = b_begin; f < b_end; ++f) {
      ...
      accscalar_t sum = 0;
      CPU_tensor_apply1<scalar_t>(in, [&](const scalar_t& i) { sum += i; });
      scalar_t mean = sum / n;
      save_mean_a[f] = mean;
      ...
      CPU_tensor_apply1<scalar_t>(
          in, [&](const scalar_t& i) { var_sum += (i - mean) * (i - mean); });
...
std::tuple<Tensor, Tensor, Tensor> batch_norm_backward_cpu_template(
  ...
  parallel_for(0, n_input, 1, [&](int64_t b_begin, int64_t b_end) {
    for (int64_t f = b_begin; f < b_end; ++f) {
      ...
      accscalar_t sum = 0;
      CPU_tensor_apply1<scalar_t>(
          grad_out, [&](const scalar_t& g) { sum += g; });

      // dot product of the Q(X) and gradOuput
      accscalar_t dotp = 0;
      CPU_tensor_apply2<scalar_t, scalar_t>(
          in, grad_out, [&](const scalar_t& i, const scalar_t& go) {
            dotp += (i - mean) * go;
          });
      ...
      if (grad_input_mask[0]) {
        ...
        if (train) {
          ...
          CPU_tensor_apply2<scalar_t, scalar_t>(
              grad_in, in, [&](scalar_t& gi, const scalar_t& i) {
                gi = (i - mean) * k;
              });

          accscalar_t grad_mean = sum / n;
          CPU_tensor_apply2<scalar_t, scalar_t>(
              grad_in, grad_out, [&](scalar_t& gi, const scalar_t& go) {
...
std::tuple<Tensor, Tensor, Tensor> _batch_norm_impl_index_backward(
    ...
    const Tensor& running_var /* optional */,
    const Tensor& save_mean /* optional */,
    const Tensor& save_var_transform /* optional */,
    bool train,
    double epsilon,
