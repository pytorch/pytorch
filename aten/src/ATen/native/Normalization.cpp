...
/// directory.
template <typename scalar_t>
void batch_norm_cpu_inference_channels_last(
    Tensor& output,
    const Tensor& input,
    const Tensor& weight /* optional */,
    const Tensor& bias /* optional */,
  ...
  // memory-limited.
  // Keep the loop structure simple to make sure compiler vectorization kicks
  // in.
  if (n_channel != 1) {
    for (int64_t n = 0; n < n_batch; ++n) {
      for (int64_t i = 0; i < image_size; ++i) {
        for (int64_t c = 0; c < n_channel; ++c) {
          ...
          output_data[offset] =
              input_data[offset] * alpha_data[c] + beta_data[c];
        }
      }
    }
  } else {
...
std::tuple<Tensor, Tensor, Tensor> batch_norm_cpu_transform_input_template(
  ...
  if (!train && input.is_contiguous() &&
    ...
    batch_norm_cpu_inference_contiguous_stub(
        kCPU, output, input, weight, bias, running_mean, running_var, eps);
  ...
  cpu_kernel(
      ...
      [=](scalar_t input,
          ...
          scalar_t invstd,
          scalar_t weight,
          scalar_t bias) { return ((input - mean) * invstd) * weight + bias; });
  return std::make_tuple(output, save_mean, save_invstd);
}
...
std::tuple<Tensor, Tensor> batch_norm_cpu_update_stats_template(
  ...
  parallel_for(0, n_input, 1, [&](int64_t b_begin, int64_t b_end) {
    for (int64_t f = b_begin; f < b_end; ++f) {
      ...
      auto iter = TensorIteratorConfig().add_input(in).build();
...
std::tuple<Tensor, Tensor, Tensor> batch_norm_backward_cpu_template(
  ...
  parallel_for(0, n_input, 1, [&](int64_t b_begin, int64_t b_end) {
    for (int64_t f = b_begin; f < b_end; ++f) {
      ...
      accscalar_t dotp = 0;
      auto iter =
          TensorIteratorConfig().add_input(in).add_input(grad_out).build();
...
// _batch_norm_impl_index(_backward) are used in the JIT be able to keep the
// run-time selection of backends, while enabling it to keep the information
// about the used backend, so that it can use its corresponding backward
// implementation.
// XXX: The indices of backends need to be kept synchronized between this
// function and its _backward.
std::tuple<Tensor, Tensor, Tensor, Tensor, int64_t> _batch_norm_impl_index(
  ...
  if (use_cudnn) {
    ...
    std::tie(output, save_mean, save_var, reserve) = at::cudnn_batch_norm(
        input_c, weight_c, bias_c, rmean_c, rvar_c, training, momentum, eps);
  ...
  bool use_miopen =
      (input.is_cuda() && input.dim() <= MIOPEN_DIM_MAX &&
       input.scalar_type() != at::kDouble &&
       input.scalar_type() != at::kBFloat16 &&
       (weight.scalar_type() != at::kHalf) && weight.defined() &&
       bias.defined() &&
       ((running_mean.defined() && running_var.defined()) ||
        (!running_mean.defined() && !running_var.defined() && training)) &&
       detail::getCUDAHooks().compiledWithMIOpen() && cudnn_enabled);
...
std::tuple<Tensor, Tensor, Tensor> _batch_norm_impl_index_backward(
    ...
    const c10::optional<Tensor>& save_var_transform_opt /* optional */,
    bool train,
    double epsilon,
  ...
  const Tensor& save_var_transform =
      c10::value_or_else(save_var_transform_opt, [] { return Tensor(); });
  ...
  if (impl_index == 0) {
    ...
    return at::miopen_batch_norm_backward(
        input,
        grad_output,
        weight,
        running_mean,
  ...
  TORCH_INTERNAL_ASSERT(
      false,
      "Unsupported impl_index in _batch_norm_impl_index_backward: ",
      impl_index);
}
...
std::tuple<Tensor, Tensor, Tensor> batch_norm_backward_cpu(
  ...
  return AT_DISPATCH_FLOATING_TYPES(
      self.scalar_type(), "batch_norm_backward_cpu", [&] {
        return batch_norm_backward_cpu_template<scalar_t>(
            grad_out,
            self,
            weight,
            running_mean,
