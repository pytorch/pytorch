...
// For some ambiguous cases, it is possible a channels last contiguous Tensor
// has
...
std::tuple<Tensor, Tensor, Tensor> batch_norm_cpu(
  ...
  return AT_DISPATCH_FLOATING_TYPES_AND(
      ScalarType::BFloat16, self.scalar_type(), "batch_norm", [&] {
        if (mixed_type) {
          ...
          if (!train) {
            ...
            return batch_norm_cpu_transform_input_template<BFloat16, float>(
                self,
                weight,
                bias,
                save_mean,
            ...
            auto save_stats =
                batch_norm_cpu_update_stats_template<BFloat16, float, InvStd>(
...
std::tuple<Tensor, Tensor, Tensor> batch_norm_backward_cpu(
  ...
  return AT_DISPATCH_FLOATING_TYPES_AND(
      ScalarType::BFloat16, self.scalar_type(), "batch_norm_backward_cpu", [&] {
        if (mixed_type) {
          check_mixed_data_type(
              self, weight, running_mean, running_var, save_mean, save_invstd);
          return batch_norm_backward_cpu_template<BFloat16, float>(
              grad_out,
              self,
              weight,
              running_mean,
