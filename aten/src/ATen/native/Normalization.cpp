...
DEFINE_DISPATCH(batch_norm_cpu_inference_stub);
...
std::tuple<Tensor,Tensor,Tensor> batch_norm_cpu_transform_input_template(
  ...
  bool all_contiguous = (input.is_contiguous() || input.is_contiguous(at::MemoryFormat::ChannelsLast))
  ...
  if (!train && all_contiguous) {
    batch_norm_cpu_inference_stub(kCPU, output, input, weight,
