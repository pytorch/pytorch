...
at::Tensor miopen_convolution_backward_bias(const at::Tensor& grad_output) {
  AT_ERROR(
...
struct ConvolutionParams {
  ...
  int64_t groups;
  bool deterministic;
  int device_id; // This is needed to distinguish between miopen handles of
                 // multiple gpus.
...
struct algorithm_search<miopenConvFwdAlgorithm_t> {
  ...
  static BenchmarkCache<size_t>& wsscache() {
    return fwd_wssizes;
  }
  ...
  static miopenConvSolution_t getSolution(
      const ConvolutionArgs& args,
      bool force_default) {
    ...
    if (max_solution_count > AT_MIOPEN_MAX_SOLUTIONS) {
      AT_ERROR(
          "miopenConvFwdAlgorithm_t getSolution max_solution_count > AT_MIOPEN_MAX_SOLUTIONS");
    }
    ...
    if (force_default) {
      ...
      // default algo was not found, select first algo without workspace
      // requirement
      for (size_t i = 0; i < solution_count; ++i) {
...
struct algorithm_search<miopenConvBwdDataAlgorithm_t> {
  ...
  static miopenConvSolution_t getSolution(
    ...
    if (max_solution_count > AT_MIOPEN_MAX_SOLUTIONS) {
      AT_ERROR(
          "miopenConvBwdDataAlgorithm_t getSolution max_solution_count > AT_MIOPEN_MAX_SOLUTIONS");
    }
...
struct algorithm_search<miopenConvBwdWeightsAlgorithm_t> {
  ...
  static miopenConvSolution_t getSolution(
    ...
    if (max_solution_count > AT_MIOPEN_MAX_SOLUTIONS) {
      AT_ERROR(
          "miopenConvBwdWeightsAlgorithm_t getSolution max_solution_count > AT_MIOPEN_MAX_SOLUTIONS");
    }
...
// In-place!
void miopen_convolution_add_bias_(
    CheckedFrom c,
  ...
  MIOPEN_CHECK(miopenConvolutionForwardBias(handle, &one, bdesc.desc(),
  bias->data_ptr(), &zero, odesc.desc(), output->data_ptr()));
  */
}
...
void raw_miopen_convolution_forward_out(
  ...
  if (benchmark) {
    ...
    Workspace workspace =
        chooseSolution<miopenConvFwdAlgorithm_t>(args, &solution_id);
...
Tensor miopen_convolution_backward_bias(const Tensor& grad_output_t) {
  ...
  /* MIOpen does not support NHWC bias. Activate once support is added.
    auto grad_bias_t = at::empty( { grad_output->size(output_channels_dim) },
    grad_output->options());
    ...
    MIOPEN_CHECK(miopenConvolutionBackwardBias(handle, &one, odesc.desc(),
    grad_output->data_ptr(), &zero, bdesc.desc(), grad_bias->data_ptr()));
    return *grad_bias;
  */
}
...
void raw_miopen_convolution_backward_weight_out(
  ...
  if (benchmark) {
    ...
    uint64_t solution_id;
    Workspace workspace =
        chooseSolution<miopenConvBwdWeightsAlgorithm_t>(args, &solution_id);
...
Tensor miopen_depthwise_convolution_backward_weight(
  ...
  if (miopen_conv_use_channels_last(*input, *grad_output)) {
    memory_format = (input->ndimension() == 5)
        ? /*at::MemoryFormat::ChannelsLast3d*/ at::MemoryFormat::Contiguous
        : at::MemoryFormat::ChannelsLast;
  }
...
void raw_miopen_convolution_backward_input_out(
  ...
  if (benchmark) {
    ...
    uint64_t solution_id;
    Workspace workspace =
        chooseSolution<miopenConvBwdDataAlgorithm_t>(args, &solution_id);
...
// see NOTE [ Backward vs transpose convolutions ] in
// src/Aten/native/cudnn/Conv.cpp
...
miopen_depthwise_convolution_backward(
  ...
  Tensor grad_input, grad_weight, grad_bias;
  if (output_mask[0]) {
    grad_input = miopen_depthwise_convolution_backward_input(
        input.sizes(),
        grad_output,
        weight,
        padding,
  ...
  if (output_mask[1]) {
    grad_weight = miopen_depthwise_convolution_backward_weight(
        weight.sizes(),
        grad_output,
        input,
        padding,
