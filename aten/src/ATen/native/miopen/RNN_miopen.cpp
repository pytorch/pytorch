...
std::vector<TensorDescriptor> rnn_descriptor_sequence(
  ...
  for (auto batch_size : batch_sizes) {
    ...
    descriptors[i].set(
        getMiopenDataType(tensor), batch_tensor_size, tensor.strides(), 3);
    i++;
  }
...
struct RNNDescriptors {
  ...
  std::vector<miopenTensorDescriptor_t> get_descs(
...
Tensor permute_wei_for_miopen(Tensor wei, int64_t mode) {
  ...
  if (mode == 2) { // LSTM
    ...
    permuted_wei =
        at::cat({sliced_tensor[1], sliced_tensor[0], sliced_tensor[2]});
  }
  return permuted_wei;
}
...
int64_t get_num_weights(
  ...
  TORCH_CHECK(
      weight_size % element_size == 0,
      "miopenGetRNNParamsSize returned nonsensical weight_size.");
...
std::pair<std::vector<Tensor>, size_t> get_parameters(
    miopenHandle_t handle,
    ...
    for (const auto linear_id : c10::irange(num_linear_layers)) {
      ...
      if (linear_id == 0 || linear_id == num_linear_layers / 2) {
        std::initializer_list<int64_t> size = {
            static_cast<int64_t>(param_size * num_linear_layers / 2), 1L};
    ...
    if (bias_mode == miopenRNNwithBias) {
      for (const auto linear_id : c10::irange(num_linear_layers)) {
        ...
        MIOPEN_CHECK(miopenGetRNNLayerBiasSize(
            handle, rnn_desc.desc(), layer, linear_id, &bias_size));
        bias_size /= elem_size;

        if (linear_id == 0 || linear_id == num_linear_layers / 2) {
          std::initializer_list<int64_t> size = {
              static_cast<int64_t>(bias_size * num_linear_layers / 2), 1L};
          Tensor param = at::empty({0}, weight_buf.options())
                             .set_(weight_buf.storage(), offset, size);
          params.emplace_back(std::move(param));
          layer_params_count++;
...
std::tuple<Tensor, Tensor, Tensor, Tensor, Tensor> miopen_rnn(
  ...
  miopenRNNBiasMode_t bias_mode =
      (weight_stride0 == 4) ? miopenRNNwithBias : miopenRNNNoBias;
  fn.rnn.set(
      fn_mode,
      fn_hidden_size,
      fn_num_layers,
  ...
  if (fn.rnn.rnn_mode != miopenLSTM) {
    TORCH_CHECK(
        !cx.defined(), "miopen_rnn: illegal defined cx for non-LSTM RNN.");
  }
  ...
  TORCH_CHECK(
      ...
      "miopen_rnn : cx is not contiguous.");
  ...
  if (fn_mode < 2)
    ...
    _copyParams_and_permute(
        MatrixRef<Tensor>{weight, static_cast<size_t>(weight_stride0)},
        MatrixRef<Tensor>{params, params_stride0},
        fn_mode);

  TORCH_CHECK(
      !cx.defined() || cx.sizes().equals(hidden_size),
      "Expected cell size ",
  ...
  if (fn_train) { // Train.
    ...
    MIOPEN_CHECK(miopenGetRNNTrainingReserveSize(
        handle,
        descs.rnn_desc.desc(),
        fn.tensors.seq_length,
        x_descs_arr.data(),
    ...
    MIOPEN_CHECK(miopenRNNForwardTraining(
        handle,
        descs.rnn_desc.desc(),
        fn.tensors.seq_length,
        x_descs_arr.data(),
...
std::tuple<Tensor, Tensor, Tensor, Tensor> miopen_rnn_backward_input(
  ...
  TORCH_CHECK(
      fn_train, "miopen RNN backward can only be called in training mode");
  ...
  TORCH_CHECK(
      ...
      dhy.sizes());
  TORCH_CHECK(
      !dcy.defined() || dcy.sizes().equals(hidden_size),
...
std::tuple<Tensor, Tensor, Tensor, std::vector<Tensor>> miopen_rnn_backward(
  ...
  std::tie(dx, dhx, dcx, ws) = at::native::miopen_rnn_backward_input(
      input,
      weight_buf,
      hx,
      cx,
  ...
  if (output_mask[3]) {
    dw = at::native::miopen_rnn_backward_weight(
        input,
        weight,
        weight_stride0,
        weight_buf,
...
std::pair<Tensor, hidden_type> _miopen_impl(
  ...
  return {
      std::get<0>(miopen_output),
      pack_hidden<hidden_type>(
          std::get<1>(miopen_output), std::get<2>(miopen_output))};
}
...
#define ONE_HIDDEN_RNN(NAME, MODE)                            \
  void NAME##_miopen(                                         \
      ...
      bool bidirectional,                                     \
      bool batch_first) {                                     \
    std::tie(output, hy) = _miopen_impl(                      \
        input,                                                \
        hx,                                                   \
        params,                                               \
        has_biases,                                           \
