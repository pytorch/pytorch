...
static Tensor compute_columns3d(
  ...
  if ((kernel_depth == 1) && (kernel_height == 1) && (kernel_width == 1) &&
    ...
    AT_DISPATCH_ALL_TYPES_AND(
        kBFloat16, input.scalar_type(), "compute_columns3d", [&] {
...
void slow_conv3d_backward_update_grad_input_frame(
    ...
    int64_t pad_width,
    int64_t groups) {
  // Compute fgrad_input = weight.T * grad_output.reshape({grad_output.shape(0),
  // -1}) Note gemm expects fortran order, so all 3 matrices are transposed.
  // Swapping argument order cancels this, since C == AB <=> T(C) == T(B)T(A)
  const int64_t m =
      ...
      kBFloat16, input.scalar_type(), "slow_conv3d_cpu_grad_input", [&] {
        at::parallel_for(
            0, batch_size, CONV3D_GRAIN_SALT, [&](int64_t start, int64_t end) {
              ...
              auto grad_output_a =
                  grad_output_contiguous.accessor<scalar_t, 5>();
...
void slow_conv3d_backward_weight_frame(
    ...
    TensorAccessor<scalar_t, 2> finput,
    int64_t groups) {
  // Compute grad_weight += grad_output.reshape({grad_output.shape(0), -1}) *
  // finput.T Note gemm expects fortran order, so all 3 matrices are transposed.
  // Swapping argument order cancels this, since C == AB <=> T(C) == T(B)T(A)
  ...
  at::native::cpublas::gemm_batched_with_stride(
      ...
      grad_output.data(),
      ldb,
      grad_output.stride(0) * n,
      static_cast<scalar_t>(1),
      grad_weight.data(),
      ldc,
      grad_weight.stride(0) * n);
}
...
static void slow_conv3d_backward_parameters_out_cpu_template(
  ...
  const int64_t batch_size = input.size(0);
  Tensor finput =
      compute_columns3d(input, stride, padding, kernel_size, groups);
...
Tensor& slow_conv3d_forward_out_cpu(
    const Tensor& self,
    const Tensor& weight,
    IntArrayRef kernel_size,
  ...
  AT_DISPATCH_ALL_TYPES_AND(
      kBFloat16, input.scalar_type(), "slow_conv3d_cpu", [&] {
        auto input_a = input.accessor<scalar_t, 5>();
...
std::tuple<Tensor&, Tensor&, Tensor&> slow_conv3d_backward_out_cpu(
  ...
  if (grad_weight.defined()) {
    ...
    slow_conv3d_backward_parameters_out_cpu_template(
        grad_weight, self, grad_output, kernel_size, stride, padding, groups);
  }
