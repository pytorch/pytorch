...
TORCH_META_FUNC(threshold)
(const Tensor& self, const Scalar& threshold, const Scalar& value) {
...
// computes `result = self <= threshold ? value : other`
// other is `self` in threshold() and `grad` in threshold_backward()
TORCH_META_FUNC(threshold_backward)
(const Tensor& grad, const Tensor& self, const Scalar& threshold) {
...
TORCH_META_FUNC(silu_backward)(const Tensor& grad_output, const Tensor& input) {
...
TORCH_META_FUNC(leaky_relu)(const Tensor& self, const Scalar& negval) {
  build_unary_op(maybe_get_output(), self);
}
...
TORCH_META_FUNC(hardsigmoid_backward)
...
TORCH_META_FUNC(softshrink)(const Tensor& self, const Scalar& lambd) {
...
TORCH_IMPL_FUNC(silu_out)(const Tensor& self, const Tensor& result) {
...
TORCH_IMPL_FUNC(mish_out)(const Tensor& self, const Tensor& result) {
...
TORCH_IMPL_FUNC(hardsigmoid_out)(const Tensor& self, const Tensor& result) {
  ...
  return (input.is_mkldnn()) || // input is mkldnn Tensor
      (input.device().is_cpu() &&
       (((input.scalar_type() == kBFloat16) && mkldnn_bf16_device_check()) ||
        (input.scalar_type() ==
         kFloat))); // input is dense layout and bfloat16/float32
#endif
  return false;
}
...
Tensor math_mish_backward(const Tensor& grad_output, const Tensor& input) {
  auto input_tanh_softplus = at::tanh(at::softplus(input));
  auto input_sigmoid = at::sigmoid(input);
  return grad_output *
      (input_tanh_softplus +
       (input * input_sigmoid *
        (1 - input_tanh_softplus * input_tanh_softplus)));
}
...
TORCH_IMPL_FUNC(threshold_backward_out)
(const Tensor& grad,
 const Tensor& self,
 const Scalar& threshold,
...
Tensor prelu_cpu(const Tensor& self, const Tensor& weight_) {
  ...
  // Helper to convert 1d tensors or scalar tensor to an nd tensor that
  // broadcasts with input All elements go into the channel dimension
  ...
  if (self.scalar_type() == ScalarType::BFloat16) {
    auto w_bf16 = at::empty(
        weight_.sizes(), weight_.options().dtype(ScalarType::BFloat16));
...
std::tuple<Tensor, Tensor> prelu_backward_cpu(
  ...
  Tensor weight_grad_collector =
      at::empty_like(self, at::MemoryFormat::Contiguous);
  ...
  const int64_t ndim = self.dim();
  // Helper to convert 1d tensor or scalar tensor to an nd tensor that
  // broadcasts with input All elements go into the channel dimension
  DimVector sizes(ndim, 1), strides(ndim, 0);
  auto as_nd = [&](const Tensor& t) {
    TORCH_INTERNAL_ASSERT(t.defined() && (t.dim() == 1 || t.dim() == 0));
  ...
  if (weight_num == 1) {
    ...
    if (input_ndim > 2) {
      for (int64_t i = 2; i < input_ndim; i++)
...
Tensor log_sigmoid_backward_cuda(
    const Tensor& grad_output,
    const Tensor& input,
    const Tensor& buffer) {
...
Tensor& log_sigmoid_backward_cuda_out(
    const Tensor& grad_output,
    const Tensor& input,
    const Tensor& buffer,
    Tensor& grad_input) {
...
Tensor& log_sigmoid_backward_cpu_out(
    const Tensor& grad_output,
    const Tensor& input,
    const Tensor& buffer,
    Tensor& grad_input) {
