...
DimVector TensorIterator::invert_perm(IntListRef input) const {
...
SmallVector<char*, 4> TensorIterator::get_data_ptrs(ArrayRef<char*> base, IntListRef counter) const {
...
void TensorIterator::permute_dimensions(IntListRef perm) {
  AT_ASSERT(perm.size() == ndim());

  auto reorder = [perm](IntListRef data) {
...
void TensorIterator::replace_operand(int arg, void* data, IntListRef stride) {
...
void TensorIterator::select_all_keeping_dim(int start_dim, IntListRef indices) {
...
static DimVector compute_stride(const Tensor& tensor, IntListRef shape) {
  int ndim = shape.size();
...
DimCounter::DimCounter(IntListRef shape, Range range)
  : shape(shape)
  , range(range)
  , values(shape.size(), 0)
  , offset(range.begin) {
