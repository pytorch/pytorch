...
TORCH_META_FUNC(upsample_nearest_exact2d) (
  const Tensor& input, IntArrayRef output_size, c10::optional<double> scales_h, c10::optional<double> scales_w
) {
  auto full_output_size = native::upsample_2d_common_check(input.sizes(), output_size);
...
TORCH_META_FUNC(upsample_nearest_exact2d_backward) (
  const Tensor& grad_output,
  IntArrayRef output_size,
  IntArrayRef input_size,
  c10::optional<double> scales_h,
...
TORCH_IMPL_FUNC(upsample_nearest_exact2d_out_cpu) (
    const Tensor& input,
    IntArrayRef output_size,
    c10::optional<double> scales_h,
    c10::optional<double> scales_w,
  ...
  upsample_nearest_exact2d_kernel(kCPU, output, input, scales_h, scales_w);
}
...
TORCH_IMPL_FUNC(upsample_nearest_exact2d_backward_out_cpu) (
    const Tensor& grad_output,
    IntArrayRef output_size,
    IntArrayRef input_size,
    c10::optional<double> scales_h,
    ...
    const Tensor& grad_input) {
  grad_input.zero_();
  upsample_nearest_exact2d_backward_kernel(kCPU, grad_input, grad_output, scales_h, scales_w);
}
...
Tensor upsample_nearest_exact2d(
    const Tensor& input,
    c10::optional<IntArrayRef> output_size,
    c10::optional<ArrayRef<double>> scale_factors) {
  auto osize = compute_output_size(input.sizes(), output_size, scale_factors);
  auto scale_h = get_scale_value(scale_factors, 0);
  auto scale_w = get_scale_value(scale_factors, 1);
  return at::upsample_nearest_exact2d(input, osize, scale_h, scale_w);
}
...
Tensor upsample_nearest_exact2d_backward(
    const Tensor& grad_output,
    c10::optional<IntArrayRef> output_size,
    IntArrayRef input_size,
    c10::optional<ArrayRef<double>> scale_factors) {
  ...
  auto scale_h = get_scale_value(scale_factors, 0);
  auto scale_w = get_scale_value(scale_factors, 1);
  return at::upsample_nearest_exact2d_backward(grad_output, osize, input_size, scale_h, scale_w);
}

DEFINE_DISPATCH(upsample_nearest2d_kernel);
DEFINE_DISPATCH(upsample_nearest_exact2d_kernel);
DEFINE_DISPATCH(upsample_nearest2d_backward_kernel);
DEFINE_DISPATCH(upsample_nearest_exact2d_backward_kernel);
