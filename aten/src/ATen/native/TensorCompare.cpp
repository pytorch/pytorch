...
Tensor where(const Tensor& condition, const Tensor& self, const Tensor& other) {
  ...
  if (condition.scalar_type() == ScalarType::Byte) {
    TORCH_WARN_ONCE("where received a uint8 condition tensor. This behavior is deprecated and ",
              "will be removed in a future version of PyTorch. Use a boolean condition instead.");
  } else {
    TORCH_CHECK(condition.scalar_type() == ScalarType::Bool, "where expected condition to be a ",
              "boolean tensor, but got a tensor with dtype ", condition.scalar_type());
  }
