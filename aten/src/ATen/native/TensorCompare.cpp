...
Tensor isclose(
  ...
  if (isFloatingType(self.scalar_type()) &&
      isFloatingType(other.scalar_type())) {
    // Handle +/-inf
...
std::tuple<Tensor&, Tensor&> mode_out(
  ...
  TORCH_CHECK(
      ...
      "mode only supports CPU AND CUDA backend, got: ",
      toString(self.type().backend()));
...
std::tuple<Tensor&, Tensor&> _max_out_cpu(
    Tensor& max,
    Tensor& max_indices,
    const Tensor& self,
    int64_t dim,
    bool keepdim) {
  if (self.is_contiguous() && max.is_contiguous() &&
      max_indices.is_contiguous()) {
...
std::tuple<Tensor&, Tensor&> max_out(
  ...
  TORCH_CHECK(
      self.type().backend() == Backend::CPU ||
          self.type().backend() == Backend::CUDA,
      "max only supports CPU AND CUDA backend, got: ",
      toString(self.type().backend()));
  dim = maybe_wrap_dim(dim, self.dim());
...
std::tuple<Tensor&, Tensor&> _min_out_cpu(
    Tensor& min,
    Tensor& min_indices,
    const Tensor& self,
    int64_t dim,
    bool keepdim) {
  if (self.is_contiguous() && min.is_contiguous() &&
      min_indices.is_contiguous()) {
...
std::tuple<Tensor&, Tensor&> min_out(
  ...
  TORCH_CHECK(
      self.type().backend() == Backend::CPU ||
          self.type().backend() == Backend::CUDA,
      "min only supports CPU AND CUDA backend, got: ",
      toString(self.type().backend()));
  dim = maybe_wrap_dim(dim, self.dim());
