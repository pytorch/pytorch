...
DEFINE_DISPATCH(
    _aminmax_stub); // NOLINT(cppcoreguidelines-avoid-non-const-global-variables)
DEFINE_DISPATCH(
...
DEFINE_DISPATCH(
    clamp_min_stub); // NOLINT(cppcoreguidelines-avoid-non-const-global-variables)
DEFINE_DISPATCH(
    clamp_max_stub); // NOLINT(cppcoreguidelines-avoid-non-const-global-variables)
DEFINE_DISPATCH(
...
Tensor isinf(const Tensor& self) {
  ...
  return AT_DISPATCH_FLOATING_TYPES_AND2(
      kBFloat16, kHalf, self.scalar_type(), "isinf", [&]() {
...
Tensor isfinite(const Tensor& self) {
  ...
  return AT_DISPATCH_FLOATING_TYPES_AND2(
      kHalf, kBFloat16, self.scalar_type(), "isfinite", [&]() {
        return (self == self) *
...
// DO NOT USE THIS -- it's just an implementation detail of wrapped_scalar
// tensor below.
...
// TLDR: Don't call `wrapped_scalar_tensor_default_dtype` -- this function is
// only necessary to support the partial type-promotion that torch.where
// supports.  Once torch.where fully supports type promotion, we won't need this
// function.
//
// Longer explanation:
// `wrapped_scalar_tensor_default_dtype` is a bit of a hack because torch.where
// doesn't support type promotion, but does support `torch.where(tensor,
// scalar1, scalar2)` with default scalar types.  The trickiness is we usually
// convert double scalars to doubles, and `set_wrapped_number` defines type
// promotion priority as being below tensor types rather than as the default
// dtype (perhaps we should?).  This wouldn't matter if we just supported type
...
Tensor where(const Tensor& condition, const Scalar& self, const Tensor& other) {
  return at::where(
      condition, wrapped_scalar_tensor(self, other.device()), other);
}

Tensor where(const Tensor& condition, const Tensor& self, const Scalar& other) {
  return at::where(
      condition, self, wrapped_scalar_tensor(other, self.device()));
}
...
std::tuple<Tensor&, Tensor&> mode_out(
  ...
  TORCH_CHECK(
      ...
      "mode only supports CPU AND CUDA device type, got: ",
      self.device().type());
  TORCH_CHECK(
...
static std::tuple<Tensor&, Tensor&> max_out_impl(
    Tensor& max,
    Tensor& max_indices,
    const Tensor& self,
    int64_t dim,
  ...
  TORCH_CHECK(
      self.device().is_cpu() || self.is_cuda(),
      "max only supports CPU AND CUDA device type, got: ",
      self.device().type());
  TORCH_CHECK(
      self.layout() == Layout::Strided,
      "max only supports strided layout, got: ",
      self.layout());
  TORCH_CHECK(
      self.device() == max.device(),
      "expected device ",
      self.device(),
      " but got ",
      max.device(),
      " for max values output");
  TORCH_CHECK(
      self.device() == max_indices.device(),
      "expected device ",
      self.device(),
      " but got ",
      max_indices.device(),
...
static std::tuple<Tensor&, Tensor&> _aminmax_out_impl(
    Tensor& min,
    Tensor& max,
    const Tensor& self,
    int64_t dim,
  ...
  TORCH_CHECK(
      self.device().is_cpu() || self.is_cuda(),
      "min_max_val only supports CPU AND CUDA device type, got: ",
      self.device().type());
  TORCH_CHECK(
      self.layout() == Layout::Strided,
      "min_max only supports strided layout, got: ",
      self.layout());
  TORCH_CHECK(
      self.device() == min.device(),
      "expected device ",
      self.device(),
      " but got ",
      min.device(),
      " for min values output");
  TORCH_CHECK(
      self.device() == max.device(),
      "expected device ",
      self.device(),
...
std::tuple<Tensor, Tensor> _aminmax(
  ...
  TORCH_CHECK(
      !self.is_quantized(),
      "min is not yet implemented for quantized tensors.");
...
static std::tuple<Tensor&, Tensor&> min_out_impl(
    Tensor& min,
    Tensor& min_indices,
    const Tensor& self,
    int64_t dim,
  ...
  TORCH_CHECK(
      self.device().is_cpu() || self.is_cuda(),
      "min only supports CPU AND CUDA device type, got: ",
      self.device().type());
  TORCH_CHECK(
      self.layout() == Layout::Strided,
      "min only supports strided layout, got: ",
      self.layout());
  TORCH_CHECK(
      self.device() == min.device(),
      "expected device ",
      ...
      " for min values output");
  TORCH_CHECK(
      self.device() == min_indices.device(),
      "expected device ",
      self.device(),
      " but got ",
      min_indices.device(),
