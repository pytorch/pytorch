...
static inline Tensor diff_helper(const Tensor& self, int64_t n, int64_t dim) {
  ...
  if (self.dtype() == at::kBool) {
    return at::logical_xor(
        at::narrow(self, dim, 1, out_len), at::narrow(self, dim, 0, out_len));
  }
...
static inline Tensor& diff_out_helper(
  ...
  auto out_len = self.size(dim) - 1;
  if (self.dtype() == at::kBool) {
    return at::logical_xor_out(
        result,
        at::narrow(self, dim, 1, out_len),
        at::narrow(self, dim, 0, out_len));
  }
  return at::sub_out(
      result,
      at::narrow(self, dim, 1, out_len),
...
Tensor& all_out(const Tensor& self, int64_t dim, bool keepdim, Tensor& result) {
  ...
  if (_dimreduce_return_trivial(result, self, 1, dim, keepdim)) {
    ...
    auto iter = make_reduction(
        "all", result, self, dim, keepdim, /*out_dtype=*/out_dtype);
    return _all(result, iter);
  }
}
...
Tensor& any_out(const Tensor& self, int64_t dim, bool keepdim, Tensor& result) {
  ...
  TORCH_CHECK(
      result.scalar_type() == ScalarType::Bool ||
          result.scalar_type() == ScalarType::Byte,
      "any only supports bool tensor for result, got: ",
      result.scalar_type());
  ...
  if (_dimreduce_return_trivial(result, self, 0, dim, keepdim)) {
    ...
    auto iter = make_reduction(
        "any", result, self, dim, keepdim, /*out_dtype=*/out_dtype);
    return _any(result, iter);
  }
}
...
Tensor& amin_out(
  ...
  auto iter =
      make_reduction("amin", result, self, dim, keepdim, self.scalar_type());
  if (iter.numel() != 0) {
...
Tensor& amax_out(
  ...
  auto iter =
      make_reduction("amax", result, self, dim, keepdim, self.scalar_type());
  if (iter.numel() != 0) {
...
Tensor& argmax_out(
  ...
  if (dim) {
    ...
    TORCH_CHECK_INDEX(
        self.numel() != 0,
        "argmax_out(): Expected reduction dim to be specified for input.numel() == 0.");
    in = self.reshape({-1});
    keepdim = false;
  }
...
Tensor& argmin_out(
  ...
  if (dim) {
    ...
    TORCH_CHECK_INDEX(
        self.numel() != 0,
        "argmin_out(): Expected reduction dim to be specified for input.numel() == 0.");
    in = self.reshape({-1});
    keepdim = false;
  }
  auto itr = make_reduction(
...
std::tuple<Tensor, Tensor> var_mean(
  ...
  return at::var_mean(
      self,
      /*dim=*/c10::optional<IntArrayRef>(dim),
