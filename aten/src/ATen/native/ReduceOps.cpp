...
std::tuple<Tensor&, Tensor&> cummax_out(Tensor& out, Tensor& indices, const Tensor& self, int64_t dim, c10::optional<ScalarType> dtype) {
      ...
      ".");
  {
    ...
    for(int64_t i=1; i<self.size(dim); i++) {
      //compare self[i] and out_[i-1] values and update out_ and indices_
      ...
      auto max_indices_at_i = at::max(indices_.narrow(dim, i-1, 1), (i * (std::get<1>(res_at_i)).unsqueeze(dim)).toType(at::kFloat));
