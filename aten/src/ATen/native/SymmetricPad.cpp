...
TORCH_META_FUNC(symmetric_pad1d)(const Tensor& input, IntArrayRef padding) {
  int64_t dim_plane = 0;
  int64_t dim_w = 1;
  int64_t nbatch = 1;
  ...
  TORCH_CHECK(
      ...
      "should be less than or equal to the corresponding input dimension, but got: padding (",
      pad_l,
      ", ",
      pad_r,
      ") at dimension ",
...
TORCH_META_FUNC(symmetric_pad1d_backward)(const Tensor& grad_output,
    const Tensor& input,
    IntArrayRef padding) {
  int64_t dim_w = 1;
  if (input.ndimension() == 3) {
...
TORCH_META_FUNC(symmetric_pad3d)(const Tensor& input, IntArrayRef padding) {
...
TORCH_META_FUNC(symmetric_pad3d_backward)(
...
void symmetric_pad2d_out_template(
  ...
  TORCH_CHECK(pad_l <= input_w && pad_r <= input_w,
    "Argument #4: Padding size should be less than or equal to the corresponding "
    "input dimension, but got: padding (", pad_l, ", ", pad_r,
    ") at dimension ", dim_w, " of input ", input.sizes());

  TORCH_CHECK(pad_t <= input_h && pad_b <= input_h,
    "Argument #6: Padding size should be less than or equal to the corresponding "
  ...
  symmetric_pad2d_kernel(kCPU, output, input, padding);
}
...
void symmetric_pad2d_backward_out_template(
  ...
  symmetric_pad2d_backward_kernel(kCPU, grad_input, grad_output, padding);
}

} // namespace

Tensor& symmetric_pad1d_out_quantized_cpu(const Tensor& input, IntArrayRef padding,
    Tensor& output) {
  ...
  symmetric_pad1d_kernel(kCPU, output, input, padding);
  return output;
}

TORCH_IMPL_FUNC(symmetric_pad1d_out_cpu)
(const Tensor& input, IntArrayRef padding, const Tensor& output) {
  symmetric_pad1d_kernel(kCPU, output, input, padding);
}

TORCH_IMPL_FUNC(symmetric_pad1d_backward_out_cpu)(const Tensor& grad_output,
    const Tensor& input,
    IntArrayRef padding,
  ...
  grad_input.zero_();
  symmetric_pad1d_backward_kernel(kCPU, grad_input, grad_output, padding);
}

Tensor& symmetric_pad2d_out_cpu(const Tensor& input, IntArrayRef padding,
    Tensor& output) {
  symmetric_pad2d_out_template(output, input, padding);
  return output;
}

Tensor symmetric_pad2d_cpu(const Tensor& input, IntArrayRef padding) {
  Tensor output = at::empty({0}, input.options());
  symmetric_pad2d_out_template(output, input, padding);
  return output;
}

Tensor symmetric_pad2d_quantized_cpu(const Tensor& input, IntArrayRef padding) {
  TORCH_CHECK(input.qscheme() == kPerTensorAffine, "Only per tensor quantization is supported");
...
Tensor& symmetric_pad2d_backward_out_cpu(const Tensor& grad_output,
    const Tensor& input,
    IntArrayRef padding,
    Tensor& grad_input) {
...
Tensor symmetric_pad2d_backward_cpu(
    const Tensor& grad_output,
    const Tensor& input,
    IntArrayRef padding) {
...
TORCH_IMPL_FUNC(symmetric_pad3d_out_cpu)
(const Tensor& input, IntArrayRef padding, const Tensor& output) {
  ...
  symmetric_pad3d_kernel(kCPU, output, input, padding);
}

TORCH_IMPL_FUNC(symmetric_pad3d_backward_out_cpu)(const Tensor& grad_output,
    const Tensor& input,
    IntArrayRef padding,
    const Tensor& grad_input) {
  if (grad_output.numel() == 0) {
  ...
  grad_input.zero_();
  symmetric_pad3d_backward_kernel(kCPU, grad_input, grad_output, padding);
}

DEFINE_DISPATCH(symmetric_pad1d_kernel);
DEFINE_DISPATCH(symmetric_pad1d_backward_kernel);
DEFINE_DISPATCH(symmetric_pad2d_kernel);
DEFINE_DISPATCH(symmetric_pad2d_backward_kernel);
DEFINE_DISPATCH(symmetric_pad3d_kernel);
DEFINE_DISPATCH(symmetric_pad3d_backward_kernel);
