...
Tensor empty_cpu(IntListRef size, const TensorOptions& options) {
...
Tensor empty_strided_cpu(IntListRef size, IntListRef stride, const TensorOptions& options) {
  auto t = at::native::empty_cpu({0}, options);
  at::native::resize_impl_cpu_(t.unsafeGetTensorImpl(), size, stride);
  return t;
}

Tensor& empty_out(Tensor& result, IntListRef size) {
  if (result.is_sparse()) {
...
Tensor full(IntListRef size, Scalar fill_value, const TensorOptions& options) {
...
Tensor& full_out(Tensor& result, IntListRef size, Scalar fill_value) {
  if (result.is_sparse()) {
    AT_ERROR("full(...) is not implemented for sparse layout");
  }
  result.resize_(size);
...
Tensor ones(IntListRef size, const TensorOptions& options) {
  return native::full(size, /*fill_value=*/1, options);
}

Tensor& ones_out(Tensor& result, IntListRef size) {
...
Tensor rand(IntListRef size, const TensorOptions& options) {
  return native::rand(size, nullptr, options);
}

Tensor rand(IntListRef size, Generator* generator, const TensorOptions& options) {
  auto result = at::empty(size, options);
  return result.uniform_(0, 1, generator);
}

Tensor& rand_out(Tensor& result, IntListRef size) {
  return native::rand_out(result, size, nullptr);
}

Tensor& rand_out(Tensor& result, IntListRef size, Generator* generator) {
  result.resize_(size);
  return result.uniform_(0, 1, generator);
}
...
Tensor randint(int64_t high, IntListRef size, const TensorOptions& options) {
...
Tensor& randint_out(Tensor& result, int64_t high, IntListRef size) {
...
Tensor& randint_out(Tensor& result, int64_t low, int64_t high, IntListRef size) {
...
Tensor randn(IntListRef size, const TensorOptions& options) {
  return native::randn(size, nullptr, options);
}

Tensor randn(IntListRef size, Generator* generator, const TensorOptions& options) {
  auto result = at::empty(size, options);
  return result.normal_(0, 1, generator);
}

Tensor& randn_out(Tensor& result, IntListRef size) {
  return native::randn_out(result, size, nullptr);
}

Tensor& randn_out(Tensor& result, IntListRef size, Generator* generator) {
  result.resize_(size);
  return result.normal_(0, 1, generator);
}
...
Tensor zeros(IntListRef size, const TensorOptions& options) {
  auto result = at::empty(size, options);
  return result.zero_();
}

Tensor& zeros_out(Tensor& result, IntListRef size) {
  if (result.is_sparse()) {
    result.sparse_resize_and_clear_(size, size.size(), 0);
    return result;
  } else {
