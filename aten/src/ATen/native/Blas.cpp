...
(const Tensor& self,
 ...
 const Scalar& alpha) {
  ...
  TORCH_CHECK(
      result.dim() == 1 && result.sizes()[0] == mat.sizes()[0],
      "output of addmv operation should be 1D with ",
      "size equal to mat.size(0), yet got output size ",
      result.sizes(),
      " and mat.size(0) ",
      mat.size(0));
}
 ...
 const Tensor& result) {
  ...
  if (mat.numel() == 0) {
    ...
    if (!result.is_same(*self_) &&
        betaval != 0.0) { // if beta is 0, result contents is ignored
      // NOLINTNEXTLINE(cppcoreguidelines-pro-type-const-cast)
    ...
    if (result.numel() != 0) {
      ...
      AT_DISPATCH_ALL_TYPES_AND_COMPLEX_AND(
          kBFloat16, mat.scalar_type(), "addmv_impl_cpu", [&] {
...
Tensor& mv_out(const Tensor& self, const Tensor& vec, Tensor& result) {
  ...
  // here we use result as self argument for addmv, and result is user supplied
  // and can be wrong size it's not a hard error, because we allow resizing
  // result, but it becomes a hard error in addmv, because addmv expects self to
  // satisfy proper conditions to avoid this, supply correctly sized self, its
  // contents doesn't matter because beta is 0
  if (result.dim() > 1 ||
