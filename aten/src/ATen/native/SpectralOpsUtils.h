...
// Validates inputs of the low-level _fft_c2r operator. Without these checks,
// passing a `last_dim_size` inconsistent with the input shape can cause a
// heap-buffer overflow in the underlying FFT library. The accepted shapes
// mirror the invariant in _exec_fft (mkl/SpectralOps.cpp): input may be the
// canonical onesided size (last_dim_size / 2 + 1) or >= last_dim_size, in
// which case only the first last_dim_size / 2 + 1 entries are read.
inline void check_fft_c2r_input(int64_t last_dim_size,
  ...
  TORCH_CHECK(input_last_dim_size == expected ||
              ...
              expected, " (= last_dim_size / 2 + 1) or >= ", last_dim_size,
