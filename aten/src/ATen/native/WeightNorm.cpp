...
// Staying faithful to the Python for now for clarity, look for optimizations
// later (e.g., single return statement for RVO)
Tensor norm_except_dim(const Tensor& v, int64_t pow, int64_t dim) {
  // I assume tensor.contiguous(), view(), norm(), etc. here will dispatch
  // through VariableType.
  if (dim == -1) {
    return v.norm(pow);
  } else if (dim == 0) {
...
std::tuple<Tensor, Tensor> weight_norm_cpu(
  ...
  const auto dtype = g.scalar_type() == at::ScalarType::BFloat16
      ...
      : g.scalar_type();
  auto norm =
      at::empty_strided(g.sizes(), g.strides(), g.options().dtype(dtype));
...
std::tuple<Tensor, Tensor> weight_norm_backward_cpu(
  ...
  weight_norm_backward_stub(
      kCPU, grad_v, grad_g, grad_w, saved_v, saved_g, saved_norm, dim);

  return std::tuple<Tensor, Tensor>{grad_v, grad_g};
}

Tensor _weight_norm(const Tensor& v_in, const Tensor& g_in, int64_t dim) {
  TORCH_CHECK(
  ...
  if (can_use_fused) {
    // weight_norm does not have a derivative defined for it, so this will route
    // back through VariableType.cpp, and construct a WeightNormFusedBackward
    // object in the autograd graph.
...
// Differentiable backward path, an alternative to weight_norm_backward, to be
// used when backward is itself creating a graph. The GradMode::is_enabled()
// check must be performed within Functions.cpp; that's why we define a separate
