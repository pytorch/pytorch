...
sycl::event woq_matmul_int4(
    Tensor& result, // dst, [M, N]   ao: [M, N1]
    const Tensor& mat1_, // src, [M, K]  ao: [M, K1]
...
//   }
  ...
  auto scale_user_dt = get_onednn_dtype(scale_); // bf16
  //   auto zp_user_dt = dnnl::memory::data_type::s4; // int32, representing 8xint4
  auto zp_user_dt = get_onednn_dtype(zp_); // bf16
