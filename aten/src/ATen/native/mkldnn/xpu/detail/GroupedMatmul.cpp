...
sycl::event scaled_grouped_matmul(
  ...
  TORCH_CHECK(
      (a_is_2d && !b_is_2d) || (a_is_2d && b_is_2d),
      "Currently only 2d x 3d and 2d x 2d grouped matmul with offsets is supported");
  TORCH_CHECK(
      !(a_is_2d && b_is_2d && scale_a.has_value()),
      "Currently scaling for 2d x 2d grouped matmul only supports fp16/bf16/fp32 input");
