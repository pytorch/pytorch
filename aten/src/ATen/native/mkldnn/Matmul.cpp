...
void mkldnn_matmul(
    ...
    float alpha) {
  TORCH_CHECK(
      (mat1.dim() == 2 && mat2.dim() == 2) || // aten::addmm
  ...
  TORCH_CHECK(
      ...
      "mkldnn_matmul:  only enabled for bf16 path");
  TORCH_CHECK(
  ...
  // to address their differences, we use mkldnn post ops to perform a fused
  ...
  // Mkldnn only optimized for contiguous or transposed (transpose last 2 dim if
  // 3-D tensor) format now Will remove this "contiguous" after mkldnn have
  // fully supported
  Tensor mat1_ = is_mkldnn_optimized_format(mat1_unsqueezed)
      ? mat1_unsqueezed
      : mat1_unsqueezed.contiguous();
  Tensor mat2_ = is_mkldnn_optimized_format(mat2_unsqueezed)
      ? mat2_unsqueezed
  ...
  if (y.get_data_handle() != result.data_ptr()) {
    ...
    // if given output format is not expected, ideep will re-init an output
    // buffer under this case, we need copy the re-inited buffer back to given
    // buffer
...
inline bool checksize(const Tensor& mat1, const Tensor& mat2) {
  ...
  // only m * n * b * k(if exist) are large enough we can get benefit from
  // mkldnn optimized gemm kernel
...
bool use_mkldnn_bf16_matmul(
  ...
  return (
      ...
      mat1.scalar_type() == kBFloat16 && mat2.scalar_type() == kBFloat16 &&
      (!result.defined() || result.scalar_type() == kBFloat16) &&
      mat1.numel() != 0 && mat2.numel() != 0 && mkldnn_bf16_device_check() &&
      checksize(mat1, mat2));
}
