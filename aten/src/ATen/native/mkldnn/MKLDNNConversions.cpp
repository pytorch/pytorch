...
Tensor mkldnn_to_dense(
    const Tensor& mkldnn_tensor,
  ...
  TORCH_CHECK(
      data_type == ScalarType::Float || data_type == ScalarType::BFloat16,
...
Tensor dense_to_mkldnn(
    const Tensor& cpu_tensor,
    c10::optional<ScalarType> dtype) {
  TORCH_CHECK(
      cpu_tensor.device().is_cpu(), "dense_to_mkldnn expects CPU tensor input");
  TORCH_CHECK(
  ...
  // NOTE: forbid direct convert from non-contiguous (or channels last) to
  // `ideep::tensor`.
