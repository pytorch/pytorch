...
std::tuple<Tensor, Tensor, Tensor, Tensor, Tensor, Tensor, Tensor> mkldnn_rnn_layer_backward(
  ...
  new_grad_hy = grad_output[2].add(grad_hy);
  d1 = grad_cy.add(new_grad_hy * o2 * (1 - cy2.tanh() * cy2.tanh()));
  dgp = d1 * i2;
  dip = d1 * g2;
  dprev_c = d1 * f2;
  dfp = d1 * cy1;
  dop = new_grad_hy * cy2.tanh();
  do_ = dop * o2 * (1 - o2);
  dg = dgp * (1 - g2 * g2);
  df = dfp * f2 * (1 - f2);
  di = dip * i2 * (1 - i2);
  ...
  dWx_ = at::matmul(da.transpose(0, 1), input_[2]);
  dWx = dWx_;
  dWh_ = at::matmul(da.transpose(0, 1), hy1);
  dWh = dWh_;
  layer_dx[2] = dx;
