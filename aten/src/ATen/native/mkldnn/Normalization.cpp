...
std::tuple<Tensor, Tensor, Tensor> mkldnn_batch_norm(
    ...
    double eps) {
  TORCH_CHECK(
      false, "mkldnn_batch_norm: ATen not compiled with MKLDNN support");
}
...
std::tuple<Tensor, Tensor, Tensor> mkldnn_batch_norm_backward(
  ...
  TORCH_CHECK(
      false,
      "mkldnn_batch_norm_backward: ATen not compiled with MKLDNN support");
}
...
std::tuple<Tensor, Tensor, Tensor> mkldnn_layer_norm_last_index_weight_bias_f32(
  ...
  TORCH_CHECK(
      false,
      "mkldnn_layer_norm_last_index_weight_bias_f32: ATen not compiled with MKLDNN support");
}
...
std::tuple<Tensor, Tensor, Tensor> mkldnn_layer_norm_last_index_weight_bias_f32(
  ...
  TORCH_INTERNAL_ASSERT(
      ...
      "only accept shapes with the last dimension");
