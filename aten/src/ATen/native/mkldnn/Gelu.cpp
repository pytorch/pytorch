...
Tensor mkldnn_gelu_backward(
    const Tensor& grad_output,
    const Tensor& input,
    c10::string_view approximate) {
  TORCH_CHECK(
      false, "mkldnn_gelu_backward: ATen not compiled with MKLDNN support");
}
...
Tensor mkldnn_gelu(const Tensor& input, c10::string_view approximate) {
  ...
  TORCH_CHECK(
      get_gelutype_enum(approximate) == GeluType::None,
...
Tensor mkldnn_gelu_backward(
  ...
  ideep::eltwise_backward::compute(
      x, grady, gradx, ideep::algorithm::eltwise_gelu_erf, /*alpha*/ 0.0);
  return new_with_itensor_mkldnn(
      std::move(gradx),
