...
Tensor mkldnn_linear_backward_input(
  ...
  TORCH_CHECK(
      false,
      "mkldnn_linear_backward_input: ATen not compiled with MKLDNN support");
}
...
std::tuple<Tensor, Tensor> mkldnn_linear_backward_weights(
  ...
  TORCH_CHECK(
      false,
      "mkldnn_linear_backward_weights: ATen not compiled with MKLDNN support");
}
...
std::tuple<Tensor, Tensor, Tensor> mkldnn_linear_backward(
  ...
  TORCH_CHECK(
      false, "mkldnn_linear_backward: ATen not compiled with MKLDNN support");
}
...
Tensor mkldnn_linear(
  ...
  TORCH_CHECK(
      ...
      self.dim());
  TORCH_CHECK(
      self.is_mkldnn(), "mkldnn_linear: input needs to be mkldnn layout");
  ...
  const Tensor weight = (weight_t.is_mkldnn() || weight_t.is_contiguous())
      ? weight_t
      : weight_t.contiguous();
...
Tensor mkldnn_linear_backward_input(
  ...
  ideep::inner_product_backward_data::compute(
      grady,
      w,
      {input_reshaped_size.begin(), input_reshaped_size.end()},
      gradx);
...
std::tuple<Tensor, Tensor> mkldnn_linear_backward_weights(
    ...
    bool bias_defined) {
  TORCH_CHECK(
      grad_output.is_mkldnn() && input.is_mkldnn(),
  ...
  auto grad_output_reshaped = grad_output.dim() > 2
      ? grad_output.reshape({-1, grad_output.size(grad_output.dim() - 1)})
      : grad_output;
  auto input_reshaped = input.dim() > 2
      ? input.reshape({-1, input.size(input.dim() - 1)})
      : input;
...
std::tuple<Tensor, Tensor, Tensor> mkldnn_linear_backward(
  ...
  if (output_mask[0]) {
    grad_input =
        at::mkldnn_linear_backward_input(input.sizes(), grad_output, weight);
  }
  if (output_mask[1] || output_mask[2]) {
    std::tie(grad_weight, grad_bias) = at::mkldnn_linear_backward_weights(
