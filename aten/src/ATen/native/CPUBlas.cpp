...
extern "C" void sbgemm_(
    ...
    int* k,
    float* alpha,
    const decltype(c10::impl::ScalarTypeToCPPType<at::kBFloat16>::t)* a,
    int* lda,
    const decltype(c10::impl::ScalarTypeToCPPType<at::kBFloat16>::t)* b,
    int* ldb,
    float* beta,
    float* c,
    int* ldc);
...
void gemm(
    ...
    decltype(c10::impl::ScalarTypeToCPPType<at::kBFloat16>::t)* c,
    int64_t ldc) {
  internal::normalize_last_dims(transa, transb, m, n, k, &lda, &ldb, &ldc);
  ...
  if (use_blas_gemm(transa, transb, m, n, k, lda, ldb, ldc)) {
    int m_ = m, n_ = n, k_ = k, lda_ = lda, ldb_ = ldb, ldc_ = ldc;
    char transa_ = to_blas(transa), transb_ = to_blas(transb);
    // alpha and beta and C matrix in OpenBLAS sbgemm are of type "float" so we
    // have to convert, copy and copy back.
...
void gemm_batched(
  ...
  c10::guts::if_constexpr<
      AT_MKL_ENABLED() && is_blas_library_type<scalar_t>::value>(
      [&](auto _) {
        internal::normalize_last_dims(
