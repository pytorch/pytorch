...
fbgemm::matrix_op_t to_fbgemm(TransposeType trans) {
  switch (trans) {
    case TransposeType::Transpose:
      return fbgemm::matrix_op_t::Transpose;
    case TransposeType::NoTranspose:
      return fbgemm::matrix_op_t::NoTranspose;
    case TransposeType::ConjTranspose:
      TORCH_INTERNAL_ASSERT(
          false, "ConjTranspose type is not supported in fbgemm");
  }
...
void gemm(
  ...
  if (use_blas_gemm(transa, transb, m, n, k, lda, ldb, ldc)) {
    ...
    float alpha_ = alpha, beta_ = beta;
    int c_size = n_ * ldc_;
    // C matrix in OpenBLAS sbgemm are of type "float" so we have to convert,
    // copy and copy back.
...
#else
  const bool bf16_gemv_trans_would_be_faster =
...
void gemm(
  ...
  const bool fp16_gemv_trans_would_be_faster =
      cpuinfo_initialize() && cpuinfo_has_x86_f16c();
...
void gemm(
  ...
  if (use_blas_gemm(transa, transb, m, n, k, lda, ldb, ldc)) {
    int m_ = m, n_ = n, k_ = k, lda_ = lda, ldb_ = ldb, ldc_ = ldc;
    mkl_gemm_bf16bf16f32(
        transa, transb, m_, n_, k_, alpha, a, lda_, b, ldb_, beta, c, ldc_);
    return;
  }
#endif
  ...
  for (const auto j : c10::irange(n)) {
    for (const auto i : c10::irange(m)) {
      ...
      if (beta == 0.f) {
        ...
        c[offset] =
            beta * c[offset] + c10::convert<float>(float16_c[j * m + i]);
      }
    }
  }
}
...
template <typename scalar_t>
using is_blas_library_type = std::integral_constant<
    bool,
    std::is_same_v<scalar_t, double> || std::is_same_v<scalar_t, float> ||
...
#endif
  axpy_stub(kCPU, at::kDouble, n, a, x, incx, y, incy);
}
...
#endif
  axpy_stub(kCPU, at::kFloat, n, a, x, incx, y, incy);
}
...
#endif
  axpy_stub(kCPU, at::kComplexDouble, n, a, x, incx, y, incy);
}
...
#endif
  axpy_stub(kCPU, at::kComplexFloat, n, a, x, incx, y, incy);
}
...
#endif
  copy_stub(kCPU, at::kDouble, n, x, incx, y, incy);
}
...
#endif
  copy_stub(kCPU, at::kFloat, n, x, incx, y, incy);
}
...
#endif
  copy_stub(kCPU, at::kComplexDouble, n, x, incx, y, incy);
}
...
#endif
  copy_stub(kCPU, at::kComplexFloat, n, x, incx, y, incy);
}
...
inline dnnl::memory::data_type get_dnnl_dtype(ScalarType dtype) {
  if (dtype == ScalarType::Float) {
    ...
    TORCH_CHECK(
        false, "get_dnnl_dtype expects float/bfloat16/half/int8 tensor input");
  }
}
...
template <>
std::size_t UnsafeUkernelKeyHasher<BrgemmKey>::operator()(
    const BrgemmKey& key) const {
...
template <>
std::size_t UnsafeUkernelKeyHasher<PackKey>::operator()(
    const PackKey& key) const {
...
struct KernelCache {
  using kstore_t = std::unordered_map<
      key_t,
      std::shared_ptr<value_t>,
      UnsafeUkernelKeyHasher<key_t>>;
...
struct GemmHelper {
  GemmHelper(
    ...
    // Prepare default vector of pairs of tensors A and B offsets for each
    // batch.
...
struct Brgemm : public KernelCache<BrgemmKey, GemmHelper> {
  ...
  static inline bool device_check(ScalarType dtype) {
    ...
    if (dtype == ScalarType::Half) {
      ...
      static bool fp32_support =
          dnnl::get_effective_cpu_isa() >= dnnl::cpu_isa::avx2;
      return fp32_support;
    } else if (dtype == ScalarType::BFloat16) {
      static bool bf16_support =
...
struct Pack : public KernelCache<PackKey, pack_t> {
  ...
  static inline bool could_pack(ScalarType dtype) {
    ...
    if (dtype == ScalarType::Half) {
      static bool fp16_pack =
          dnnl::get_effective_cpu_isa() >= dnnl::cpu_isa::avx512_core_amx_fp16;
      return fp16_pack;
    } else if (dtype == ScalarType::BFloat16) {
      static bool bf16_pack =
          dnnl::get_effective_cpu_isa() >= dnnl::cpu_isa::avx512_core_amx;
      return bf16_pack;
...
void brgemm(
    ...
    float* C,
    bool is_vnni) {
  TORCH_CHECK(!is_vnni, "Float Brgemm does not support vnni layout.");
...
#else
  TORCH_CHECK(
      false, "pack is only supported on X64 with oneDNN ukernel enabled");
#endif
}
