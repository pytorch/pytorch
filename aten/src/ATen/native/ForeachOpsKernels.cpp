...
void foreach_tensor_##OP##_scalar_kernel_slow_(const ITensorList& tensors, const Scalar& scalar) {        \
  ...
  for (auto& t: tensors) {                                                                                \
    t.OP##_(scalar);                                                                                      \
                                                                                                          ...
                                                                                                          \
std::vector<Tensor> foreach_tensor_##OP##_scalar_kernel_slow(const ITensorList& tensors, const Scalar& scalar) { \
  check_foreach_api_restrictions(tensors);                                                                \
                                                                                                          \
...
void foreach_tensor_##OP##_scalarlist_kernel_slow_(const ITensorList& tensors, at::ArrayRef<Scalar> scalars) {          \
  ...
  for (const auto i : c10::irange(tensors.size())) {                                                                    \
      tensors[i].OP##_(scalars[i]);                                                                                     \
                                                                                                                        ...
                                                                                                                        \
std::vector<Tensor> foreach_tensor_##OP##_scalarlist_kernel_slow(const ITensorList& tensors, at::ArrayRef<Scalar> scalars) { \
  check_foreach_api_restrictions(tensors, scalars);                                                                     \
  std::vector<Tensor> result;                                                                                           \
  result.reserve(tensors.size());                                                                                       \
  for (const auto i : c10::irange(tensors.size())) {                                                                    \
...
std::vector<Tensor> foreach_tensor_##OP##_list_kernel_slow(const ITensorList& tensors1, const ITensorList& tensors2) { \
  ...
  return result;                                                                                          \
                                                                                                          ...
                                                                                                          \
void foreach_tensor_##OP##_list_kernel_slow_(const ITensorList& tensors1, const ITensorList& tensors2) {  \
  check_foreach_api_restrictions(tensors1, tensors2);                                                     \
                                                                                                          \
  for (const auto i : c10::irange(tensors1.size())) {                                                     \
...
std::vector<Tensor> foreach_tensor_##OP##_list_kernel_slow(const ITensorList& tensors1, const ITensorList& tensors2, const Scalar& alpha) { \
  check_foreach_api_restrictions(tensors1, tensors2);                                                                   \
                                                                                                                        \
  std::vector<Tensor> result;                                                                                           \
  result.reserve(tensors1.size());                                                                                      \
  ...
  return result;                                                                                                        \
                                                                                                                        ...
                                                                                                                        \
void foreach_tensor_##OP##_list_kernel_slow_(const ITensorList& tensors1, const ITensorList& tensors2, const Scalar& alpha) {                  \
  check_foreach_api_restrictions(tensors1, tensors2);                                                                   \
                                                                                                                        \
  for (const auto i : c10::irange(tensors1.size())) {                                                                   \
...
std::vector<Tensor> foreach_tensor_##OP##_slow(const ITensorList& tensors) { \
  check_foreach_api_restrictions(tensors);                                   \
                                                                             \
  std::vector<Tensor> result;                                                \
  result.reserve(tensors.size());                                            \
  ...
  return result;                                                             \
                                                                             ...
                                                                             \
void foreach_tensor_##OP##_slow_(const ITensorList& tensors) {               \
  check_foreach_api_restrictions(tensors);                                   \
                                                                             \
  for (auto& t : tensors) {                                                  \
    t.OP##_();                                                               \
...
std::vector<Tensor> foreach_tensor_##OP##_scalar_slow(const ITensorList& input, const ITensorList& tensors1, const ITensorList& tensors2, const Scalar& scalar) { \
  ...
  return result;                                                                                                                     \
                                                                                                                                     ...
                                                                                                                                     \
void foreach_tensor_##OP##_scalar_slow_(const ITensorList& input, const ITensorList& tensors1, const ITensorList& tensors2, const Scalar& scalar) { \
  check_foreach_api_restrictions(input, tensors1, tensors2);                                                                         \
                                                                                                                                     \
  for(const auto i : c10::irange(input.size())) {                                                                                    \
...
std::vector<Tensor> foreach_tensor_##OP##_scalarlist_slow(const ITensorList& input, const ITensorList& tensors1, const ITensorList& tensors2, at::ArrayRef<Scalar> scalars) { \
  ...
  return result;                                                                                                                                        \
                                                                                                                                                        ...
                                                                                                                                                        \
void foreach_tensor_##OP##_scalarlist_slow_(const ITensorList& input, const ITensorList& tensors1, const ITensorList& tensors2, at::ArrayRef<Scalar> scalars) { \
  check_foreach_api_restrictions(input, tensors1, tensors2, scalars);                                                                                   \
                                                                                                                                                        \
  for(const auto i : c10::irange(input.size())) {                                                                                                       \
...
std::vector<Tensor> foreach_tensor_##NAME##_slow(const ITensorList& tensors1, const ITensorList& tensors2) { \
  check_foreach_api_restrictions(tensors1, tensors2);                                        \
                                                                                             \
  std::vector<Tensor> result;                                                                \
  result.reserve(tensors1.size());                                                           \
...
void foreach_tensor_zero_slow_(const ITensorList& tensors) {
  check_foreach_api_restrictions(tensors);
...
std::vector<Tensor> foreach_tensor_norm_slow(const ITensorList& tensors, const Scalar& ord) {
  check_foreach_api_restrictions(tensors);
  std::vector<Tensor> result;
  for (const auto& t : tensors) {
