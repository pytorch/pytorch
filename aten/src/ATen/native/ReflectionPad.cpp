...
void reflection_pad1d_out_template(
    Tensor& output, const Tensor& input_, IntListRef padding) {
  int64_t dim_plane = 0;
  int64_t dim_w = 1;
  int64_t nbatch = 1;
...
void reflection_pad2d_backward_out_template(
    ...
    const Tensor &input, IntListRef padding) {
  int dim_w = 2;
  int dim_h = 1;
  int dim_plane = 0;
  int64_t nbatch = 1;
...
Tensor& reflection_pad1d_out_cpu(
    Tensor& output, const Tensor& input, IntListRef padding) {
  reflection_pad1d_out_template(output, input, padding);
  return output;
}

Tensor reflection_pad1d_cpu(const Tensor& input, IntListRef padding) {
...
Tensor reflection_pad2d_cpu(const Tensor& input, IntListRef padding) {
  auto output = at::empty({0}, input.options());
  reflection_pad2d_out_template(output, input, padding);
  return output;
}
