          ...
          for (const auto h : c10::irange(out_H)) {
            for (const auto w : c10::irange(out_W)) {
              ...
              if (interpolation_mode == GridSamplerInterpolation::Bilinear) {
                ...
                scalar_t tx = ix - ix_nw;
                scalar_t ty = iy - iy_nw;
                scalar_t tz = iz - iz_nw;
                ...
                get_cubic_upsample_coefficients<scalar_t>(z_coeffs, tz);

                scalar_t *out_ptr_NCDHW = out_ptr + n * out_sN + d * out_sD + h * out_sH + w * out_sW;
                const scalar_t *inp_ptr_NC = inp_ptr_N;
                for (int64_t c = 0; c < C; ++c, out_ptr_NCDHW += out_sC, inp_ptr_NC += inp_sC) {
                  scalar_t coefficients_z[4];
                  for (const auto k : c10::irange(4)) {
                    scalar_t coefficients_y[4];
                    ...
                    coefficients_z[k] = cubic_interp1d<scalar_t>(coefficients_y[0], coefficients_y[1], coefficients_y[2], coefficients_y[3], ty);
                  }
                  *out_ptr_NCDHW = cubic_interp1d<scalar_t>(coefficients_z[0], coefficients_z[1], coefficients_z[2], coefficients_z[3], tz);
                }
              }
            }
          }
          ...
          for (const auto h : c10::irange(out_H)) {
            for (int64_t w = 0; w < out_W; ++w, gGrid_ptr_NDHW += gGrid_sW /* grad_grid is contiguous */ ) {
              ...
              if (interpolation_mode == GridSamplerInterpolation::Bilinear) {
                ...
                ix = grid_sampler_unnormalize_set_grad(ix, inp_W, align_corners, &gix_mult);
                iy = grid_sampler_unnormalize_set_grad(iy, inp_H, align_corners, &giy_mult);
                iz = grid_sampler_unnormalize_set_grad(iz, inp_D, align_corners, &giz_mult);
                ...
                const scalar_t *gOut_ptr_NCDHW = gOut_ptr + n * gOut_sN + d * gOut_sD + h * gOut_sH + w * gOut_sW;
                const scalar_t *inp_ptr_NC = inp_ptr_N;
                scalar_t *gInp_ptr_NC = input_requires_grad ? (gInp_ptr + n * gInp_sN) : nullptr;

                for (int64_t c = 0; c < C; ++c, gOut_ptr_NCDHW += gOut_sC, inp_ptr_NC += inp_sC) {
                  ...
                  for (const auto k : c10::irange(4)) {
                    for (const auto j : c10::irange(4)) {
                      for (const auto i : c10::irange(4)) {
                        const scalar_t val = get_value_bounded_3d<scalar_t>(
                            inp_ptr_NC,
                            ix_nw - 1 + i,
                            iy_nw - 1 + j,
                            iz_nw - 1 + k,
