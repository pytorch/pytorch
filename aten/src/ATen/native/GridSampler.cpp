...
Tensor grid_sampler_3d_cpu_impl(
    const Tensor& input,
    const Tensor& grid,
  ...
  check_grid_sampler_3d(input, grid, static_cast<int64_t>(interpolation_mode));
      ...
      for (const auto d : c10::irange(out_D)) {
        for (const auto h : c10::irange(out_H)) {
          for (const auto w : c10::irange(out_W)) {
            ...
            scalar_t* grid_ptr_NDHW =
                grid_ptr_N + d * grid_sD + h * grid_sH + w * grid_sW;
            ...
            if (interpolation_mode == GridSamplerInterpolation::Bilinear) {
              ...
              scalar_t* out_ptr_NCDHW =
                  out_ptr + n * out_sN + d * out_sD + h * out_sH + w * out_sW;
              scalar_t* inp_ptr_NC = inp_ptr_N;
              for (int64_t c = 0; c < C;
                   ++c, out_ptr_NCDHW += out_sC, inp_ptr_NC += inp_sC) {
                //   (c, iz_tnw, iy_tnw, ix_tnw) * tnw + (c, iz_tne, iy_tne,
                //   ix_tne) * tne
                // + (c, iz_tsw, iy_tsw, ix_tsw) * tsw + (c, iz_tse, iy_tse,
                // ix_tse) * tse
                // + (c, iz_bnw, iy_bnw, ix_bnw) * bnw + (c, iz_bne, iy_bne,
                // ix_bne) * bne
                // + (c, iz_bsw, iy_bsw, ix_bsw) * bsw + (c, iz_bse, iy_bse,
                // ix_bse) * bse
                *out_ptr_NCDHW = static_cast<scalar_t>(0);
                if (within_bounds_3d(
                        iz_tnw, iy_tnw, ix_tnw, inp_D, inp_H, inp_W)) {
                  *out_ptr_NCDHW += inp_ptr_NC
                                        [iz_tnw * inp_sD + iy_tnw * inp_sH +
                                         ix_tnw * inp_sW] *
                      tnw;
                }
                if (within_bounds_3d(
                        iz_tne, iy_tne, ix_tne, inp_D, inp_H, inp_W)) {
                  *out_ptr_NCDHW += inp_ptr_NC
                                        [iz_tne * inp_sD + iy_tne * inp_sH +
                                         ix_tne * inp_sW] *
                      tne;
                }
                if (within_bounds_3d(
                        iz_tsw, iy_tsw, ix_tsw, inp_D, inp_H, inp_W)) {
                  *out_ptr_NCDHW += inp_ptr_NC
                                        [iz_tsw * inp_sD + iy_tsw * inp_sH +
                                         ix_tsw * inp_sW] *
                      tsw;
                }
                if (within_bounds_3d(
                        iz_tse, iy_tse, ix_tse, inp_D, inp_H, inp_W)) {
                  *out_ptr_NCDHW += inp_ptr_NC
                                        [iz_tse * inp_sD + iy_tse * inp_sH +
                                         ix_tse * inp_sW] *
                      tse;
                }
                if (within_bounds_3d(
                        iz_bnw, iy_bnw, ix_bnw, inp_D, inp_H, inp_W)) {
                  *out_ptr_NCDHW += inp_ptr_NC
                                        [iz_bnw * inp_sD + iy_bnw * inp_sH +
                                         ix_bnw * inp_sW] *
                      bnw;
                }
                if (within_bounds_3d(
                        iz_bne, iy_bne, ix_bne, inp_D, inp_H, inp_W)) {
                  *out_ptr_NCDHW += inp_ptr_NC
                                        [iz_bne * inp_sD + iy_bne * inp_sH +
                                         ix_bne * inp_sW] *
                      bne;
                }
                if (within_bounds_3d(
                        iz_bsw, iy_bsw, ix_bsw, inp_D, inp_H, inp_W)) {
                  *out_ptr_NCDHW += inp_ptr_NC
                                        [iz_bsw * inp_sD + iy_bsw * inp_sH +
                                         ix_bsw * inp_sW] *
                      bsw;
                }
                if (within_bounds_3d(
                        iz_bse, iy_bse, ix_bse, inp_D, inp_H, inp_W)) {
                  *out_ptr_NCDHW += inp_ptr_NC
                                        [iz_bse * inp_sD + iy_bse * inp_sH +
                                         ix_bse * inp_sW] *
                      bse;
                }
              }
            } else if (
                interpolation_mode == GridSamplerInterpolation::Nearest) {
...
template <typename scalar_t>
std::tuple<Tensor, Tensor> grid_sampler_3d_backward_cpu_impl(
  ...
  at::parallel_for(0, N, 0, [&](int64_t start, int64_t end) {
    for (const auto n : c10::irange(start, end)) {
      ...
      for (const auto d : c10::irange(out_D)) {
        for (const auto h : c10::irange(out_H)) {
          for (int64_t w = 0; w < out_W;
               ++w, gGrid_ptr_NDHW += gGrid_sW /* grad_grid is contiguous */) {
            // get the corresponding input x, y, z co-ordinates from grid
            scalar_t* grid_ptr_NDHW =
                grid_ptr_N + d * grid_sD + h * grid_sH + w * grid_sW;
            scalar_t ix = *grid_ptr_NDHW;
            ...
            if (interpolation_mode == GridSamplerInterpolation::Bilinear) {
              ...
              scalar_t gix = static_cast<scalar_t>(0),
                       giy = static_cast<scalar_t>(0),
                       giz = static_cast<scalar_t>(0);
              scalar_t* gOut_ptr_NCDHW = gOut_ptr + n * gOut_sN + d * gOut_sD +
                  h * gOut_sH + w * gOut_sW;
              scalar_t* inp_ptr_NC = inp_ptr_N;
...
Tensor _grid_sampler_2d_cpu_fallback(
    const Tensor& input,
    const Tensor& grid,
    int64_t interpolation_mode_,
    int64_t padding_mode_,
      ...
      for (const auto h : c10::irange(out_H)) {
        for (const auto w : c10::irange(out_W)) {
          ...
          if (interpolation_mode == GridSamplerInterpolation::Bilinear) {
            ...
            // depends on the padding, which would cause calculation to be
            // wrong, for example x = -0.1 -> ix = 0 for zero padding, but in
            // bicubic ix = floor(x) = -1 There would be more problem in
            // reflection padding, since the -1 and +1 direction is not fixed in
            // boundary condition
...
std::tuple<Tensor, Tensor> _grid_sampler_2d_cpu_fallback_backward(
    const Tensor& grad_output,
    const Tensor& input,
    const Tensor& grid,
    int64_t interpolation_mode_,
  ...
  at::parallel_for(0, N, 0, [&](int64_t start, int64_t end) {
    for (const auto n : c10::irange(start, end)) {
      ...
      for (const auto h : c10::irange(out_H)) {
        for (int64_t w = 0; w < out_W;
             ++w, gGrid_ptr_NHW += gGrid_sW /* grad_grid is contiguous */) {
          ...
          if (interpolation_mode == GridSamplerInterpolation::Bilinear) {
            ...
            scalar_t* gOut_ptr_NCHW =
                gOut_ptr + n * gOut_sN + h * gOut_sH + w * gOut_sW;
            scalar_t* gInp_ptr_NC = gInp_ptr + n * gInp_sN;
            scalar_t* inp_ptr_NC = inp_ptr_N;
            // calculate bilinear weighted pixel value and set output pixel
            for (int64_t c = 0; c < C; ++c,
            ...
            scalar_t* gInp_ptr_NC = gInp_ptr + n * gInp_sN;
            for (int64_t c = 0; c < C;
                 ++c, gOut_ptr_NCHW += gOut_sC, gInp_ptr_NC += gInp_sC) {
              // calculate and set grad_input
              safe_add_2d(
                  gInp_ptr_NC,
                  iy_nearest,
...
Tensor grid_sampler_2d_cpu(
  ...
  if (input.scalar_type() != kDouble) {
    TORCH_CHECK(
        ...
        "grid_sampler_2d_cpu not implemented for ",
        input.scalar_type());
...
Tensor grid_sampler_3d_cpu(
  ...
  return AT_DISPATCH_FLOATING_TYPES(
      input.scalar_type(), "grid_sampler3d_cpu", [&] {
...
std::tuple<Tensor, Tensor> grid_sampler_2d_backward_cpu(
  ...
  if (input.scalar_type() != kDouble) {
    TORCH_CHECK(
        input.scalar_type() == kFloat,
        "grid_sampler_2d_backward_cpu not implemented for ",
        input.scalar_type());
...
std::tuple<Tensor, Tensor> grid_sampler_3d_backward_cpu(
  ...
  return AT_DISPATCH_FLOATING_TYPES(
      input.scalar_type(), "grid_sampler_3d_backward_cpu", [&] {
