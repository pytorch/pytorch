...
static void checkLongTensor(const Tensor& tensor) {
  TORCH_CHECK(
      tensor.dim() == 1 && tensor.device().type() == at::kCPU &&
...
std::tuple<Tensor, Tensor> _pack_padded_sequence(
    const Tensor& _input,
    const Tensor& _lengths,
    bool batch_first) {
  ...
  std::vector<int64_t> step_shape; // == [-1, *input.shape[2:]]
  {
    ...
    step_shape.insert(
        step_shape.end(), s_input_sizes.begin(), s_input_sizes.end());
  }

  // To understand what's going on in this loop imagine that the input is a
  ...
  // step, and wait for the moments where the length increases. In this example,
  // that will happen at the first, second and fourth steps. Then, we slice out
  // the whole block of the input that corresponds to this length, and hasn't
  // been sliced yet (the steps at which each element is sliced are annotated in
  // the array above).  You can think of this as if we were scanning the
  // sequences from the shortest one, and every time we realize there's more
  // elements below in our column, we lower the counter (prev_l), and append the
  // new block to the output.
  int64_t prev_l = 0;
  for (const auto i : c10::irange(batch_size)) {
    int64_t l = lengths[batch_size - 1 - i];
    if (l > prev_l) {
      auto current_batch_size = batch_size - i;
      steps.push_back(input.slice(0, prev_l, l)
...
// `grad` could be on arbitrary device and of arbitrary dtype, but
// `_batch_sizes` is guaranteed to be a CPU int64 tensor. See NOTE [ device and
// dtype of a PackedSequence ]
Tensor _pack_padded_sequence_backward_symint(
    const Tensor& grad,
  ...
  for (const auto i : c10::irange(max_seq_len)) {
    grad_input[i]
        .slice(0, 0, batch_sizes[i])
        .copy_(grad.slice(0, offset, offset + batch_sizes[i]));
...
std::tuple<Tensor, Tensor> _pad_packed_sequence(
  ...
  std::vector<int64_t>
      output_size; // == [max_seq_length, max_batch_size, *var_data.size()[1:]]
  {
    ...
    output_size.insert(
        output_size.end(), s_data_size.begin(), s_data_size.end());
  }
  ...
  std::vector<int64_t> tmp_view_size =
      std::move(output_size); // == [-1, -1, *var_data.size()[1:]]
  ...
  for (int64_t i = 0; i <= max_real_seq_length; ++i) {
    ...
    if (batch_size != prev_batch_size) {
      ...
      // output[prev_i:i, :prev_batch_size] = tmp.view(i - prev_i,
      // prev_batch_size, *input.shape[2:])
      auto tmp = data.slice(0, data_offset, data_offset + l);
      tmp_view_size[0] = i - prev_i;
      tmp_view_size[1] = prev_batch_size;
      output.slice(0, prev_i, i)
          .slice(1, 0, prev_batch_size)
          .copy_(tmp.view(tmp_view_size));
      data_offset += l;
      prev_i = i;
    }
...
Tensor pad_sequence(
    ...
    const std::string_view padding_side) {
