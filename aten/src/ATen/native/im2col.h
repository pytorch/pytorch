...
static void col2im_slice(
    ...
    T* data_im,
    bool is_channels_last = false) {
  if (!is_channels_last && kernel_h == 2 && kernel_w == 2 &&
    ...
    for (const auto c : c10::irange(0, channels)) {
      for (const auto h : c10::irange(0, output_height)) {
        const T* src = data_col + c * 4 * col_size + h * output_width;
        T* dst = data_im + c * height * width + h * 2 * width;
        for (const auto w : c10::irange(0, output_width)) {
  ...
  if (!is_channels_last && dilation_h == 1 && dilation_w == 1 &&
    ...
    if (kernel_h == 1 && kernel_w == 1) {
      for (const auto i : c10::irange(0, channels * height * width)) {
        data_im[i] = T(0) + data_col[i];
      }
      return;
    }
    for (const auto c_col : c10::irange(0, channels * kernel_h * kernel_w)) {
  ...
  if (is_channels_last) {
    for (const auto h_col : c10::irange(0, height_col)) {
      for (const auto w_col : c10::irange(0, width_col)) {
        for (const auto h_offset : c10::irange(0, kernel_h)) {
          int64_t h_im = h_col * stride_h - pad_h + h_offset * dilation_h;
          for (const auto w_offset : c10::irange(0, kernel_w)) {
            int64_t w_im = w_col * stride_w - pad_w + w_offset * dilation_w;
    ...
    for (const auto c_col : c10::irange(0, channels_col)) {
      ...
      for (const auto h_col : c10::irange(0, height_col)) {
        ...
        if (h_im >= 0 && h_im < height) {
          ...
          for (const auto w : c10::irange(0, count)) {
