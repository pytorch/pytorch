...
inline void checkInBoundsForStorage(
  ...
  // It's ok to always evaluate to False for this early return for SymInts
  // because (1) maybe_convert_symint below only installs guard for int64_t case
  ...
  TORCH_MAYBE_SYM_CHECK(
      sym_eq(storage_size_bytes, 0) ||
          sym_le(storage_size_plus_offset_bytes, new_storage_size_bytes),
      "setStorage: sizes ",
      size,
      ", strides ",
      stride,
...
inline void checkSetStorage(
    ...
    ArrayRef<T> size,
    ArrayRef<T> stride,
    bool check_offset_in_bounds = true) {
