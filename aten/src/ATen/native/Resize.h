...
namespace {
inline int64_t _getStorageEndOffset(
    int64_t storage_offset,
    const caffe2::TypeMeta& data_type,
    int64_t storage_size_bytes) {
#ifndef C10_MOBILE
  uint64_t storage_offset_bytes;
  bool overflowed = c10::mul_overflows(
      storage_offset, data_type.itemsize(), &storage_offset_bytes);
  overflowed |= c10::add_overflows(
      storage_size_bytes, storage_offset_bytes, &storage_offset_bytes);
  overflowed |= storage_offset_bytes > at::detail::storage_max();
  TORCH_CHECK(!overflowed, "storage offset and size overflowed");
  return static_cast<int64_t>(storage_offset_bytes);
#else
  return storage_offset * data_type.itemsize() + storage_size_bytes;
#endif
}

inline c10::SymInt _getStorageEndOffset(
    c10::SymInt storage_offset,
    const caffe2::TypeMeta& data_type,
    c10::SymInt storage_size_bytes) {
  return storage_offset * data_type.itemsize() + storage_size_bytes;
}
} // namespace
...
inline void checkInBoundsForStorage(
  ...
  T storage_end_bytes =
      _getStorageEndOffset(storage_offset, data_type, storage_size_bytes);
  ...
  TORCH_CHECK(
      storage_end_bytes <= new_storage_size_bytes,
      "setStorage: sizes ",
      size,
      ", strides ",
      stride,
