...
inline void checkInBoundsForStorage(
  ...
  int64_t storage_offset_bytes_temp;
  bool storage_offset_bytes_overflowed = c10::mul_overflows(
      storage_offset, data_type.itemsize(), &storage_offset_bytes_temp);
  storage_offset_bytes_overflowed |=
      c10::overflows<T>(storage_offset_bytes_temp);
  TORCH_CHECK(!storage_offset_bytes_overflowed, "storage offset is too large");
  T storage_offset_bytes = static_cast<T>(storage_offset_bytes_temp);
  if (storage_size_bytes == 0) {
    // NB: (a tensor with arbitrary 0 dims)'s storage can have any numel.
    return;
  }
  int64_t storage_size_plus_offset_bytes_temp;
  bool storage_size_plus_offset_bytes_overflowed = c10::add_overflows(
      storage_size_bytes,
      storage_offset_bytes,
      &storage_size_plus_offset_bytes_temp);
  storage_size_plus_offset_bytes_overflowed |=
      c10::overflows<T>(storage_size_plus_offset_bytes_temp);
  TORCH_CHECK(
      !storage_size_plus_offset_bytes_overflowed,
      "storage size plus offset is too large");
  T storage_size_plus_offset_bytes =
      static_cast<T>(storage_size_plus_offset_bytes_temp);
