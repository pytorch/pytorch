    ...
    if (v_input.has_image() &&
      ...
      if (unpacked_.bias && unpacked_.bias->defined()) {
        ...
        context->dispatch(
            ...
            VK_KERNEL(addmm),
            {
              static_cast<uint32_t>(div_up(unpacked_.weight.sizes()[Layout::Parameter::width], INT64_C(2))),
              static_cast<uint32_t>(div_up(v_input.sizes()[Layout::Parameter::height], INT64_C(2))),
              1,
            },
            {8, 8, 1},
