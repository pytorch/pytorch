...
Tensor permute_4d(
  ...
  api::Command::Buffer& command_buffer = command_pool.stream();
  {
    api::OpProfiler profiler(
        command_buffer, context->querypool(), "aten::permute (permute_4d)");
    ...
    if C10_LIKELY (v_output.has_image() && v_self.has_image()) {
      auto src_image = v_self.image(command_buffer, vTensor::Stage::Compute);
      ...
      context->dispatch(
          ...
          // to avoid the nondeterministic order of GPU shader operations
          // between texels
          v_output.extents(),
