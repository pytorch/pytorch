...
//    close.  This shared memory is more often than not located in system
//    memory, making for slow GPU read and write access over the PCI-e bus on
//    discrete. Maintaining two separate copies on the other hand, requires
//    synchronization to guarantee coherence.  This is not an issue on UMA and
//    this implementation accounts for that optimization.
//
...
// data model fundamentally depends on limiting what the user can achieve
// through the interface to guarantee performance and coherence.
//
...
class vTensor final {
  ...
  */
  ...
  class Future final {
    ...
    using is_convertible = std::enable_if_t<std::is_convertible<
        Access::Pointer<T, A>,
   ...
   public:
    ...
    typedef Memory::Handle<Access::Pointer<Type, kAccess>> Payload;
  ...
  /*
    ...
    such scenarios, which is then explicitly waited on as part of
    Future::wait(). Consequently, for optimal performance, put as much time and
    distance between the place where this function is called, and the location
 ...
 private:
  class View final {
   ...
   private:
    ...
    friend std::ostream& operator<<(std::ostream&, const View::State::Bundle&);
#endif /* VULKAN_TENSOR_DEBUG */
  };
...
inline vTensor::Future<Type, kAccess>::Future(const vTensor* const tensor)
    : tensor_(tensor) {
  TORCH_INTERNAL_ASSERT_DEBUG_ONLY(tensor_, "Invalid Vulkan tensor!");
}

template <typename Type, vTensor::Access::Flags kAccess>
inline vTensor::Future<Type, kAccess>::Future(Future&& future)
    : tensor_(std::move(future.tensor_)) {
  future.tensor_ = nullptr;
}

template <typename Type, vTensor::Access::Flags kAccess>
inline vTensor::Future<Type, kAccess>& vTensor::Future<Type, kAccess>::
...
inline vTensor::Future<Type, kAccess>::Future(Future<Type_, kAccess_>&& future)
    : tensor_(std::move(future.tensor_)) {
  future.tensor_ = nullptr;
}
...
template <typename Type_, vTensor::Access::Flags kAccess_, typename>
inline vTensor::Future<Type, kAccess>& vTensor::Future<Type, kAccess>::
operator=(Future<Type_, kAccess_>&& future) & {
  tensor_ = std::move(future.tensor_);
  future.tensor_ = nullptr;
  return *this;
}
...
template <typename Type, vTensor::Access::Flags kAccess>
inline typename vTensor::Future<Type, kAccess>::Payload vTensor::
    Future<Type, kAccess>::wait() const& {
  TORCH_CHECK(
      tensor_,
...
template <typename Type>
inline vTensor::Future<Type, vTensor::Access::Read> vTensor::host(
...
inline void vTensor::View::State::set_clean(const Component::Flags components) {
  dirty_ &= ~components;
}

inline void vTensor::View::State::set_dirty(const Component::Flags components) {
  dirty_ |= components;
}
