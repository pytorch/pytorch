...
vTensor pack_weights(const Tensor& weight_arg, const Conv2dMethod conv_method) {
  ...
  if (conv_method == Conv2dDepthwise) {
    return pack_weights_dw(context, command_buffer, weight);
  }

  if (conv_method == Conv2dWinograd_2_3) {
    return pack_weights_2d_winograd_2_3(context, command_buffer, weight);
  }

  return pack_weights_2d(context, command_buffer, weight);
}
...
void conv2d_sliding_window(
  ...
  bool valid = C10_LIKELY(
      v_output.has_image() && v_input.has_image() &&
...
void conv2d_winograd_2_3(
  ...
  api::Command::Buffer& command_buffer = command_pool.stream();
  {
    api::OpProfiler profiler(
        command_buffer,
        context->querypool(),
        "prepacked::conv2d_clamp_run (conv2d_winograd_2_3)");
    ...
    }
    {
      ...
      context->dispatch(
          ...
          packed_v_weight.image(command_buffer, vTensor::Stage::Compute),
          packed_v_bias.buffer(command_buffer, vTensor::Stage::Compute),
          context->resource().pool.uniform(block).object);
    }
  }
  command_pool.submit(context->gpu().queue, command_buffer);
