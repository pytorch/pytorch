...
inline bool is_winograd_n_3(
  ...
  return (3 == filter[Layout::Filter::height]) &&
      (3 == filter[Layout::Filter::width]) && all_lessthan(stride, 2) &&
...
Conv2dMethod determine_method(
  ...
  if (Experimentation::kUseWinogradConvs &&
      is_winograd_n_3(filter, stride, dilation))
...
vTensor pack_weights_dw(
  ...
  for (int64_t src_oc = 0; src_oc < src_filter[Layout::Filter::output];
       ++src_oc) {
    /* Source */
    ...
    for (int64_t src_ih = 0; src_ih < src_filter[Layout::Filter::height];
         ++src_ih) {
      memcpy(
      ...
      for (uint32_t sy = 0; sy < H; ++sy) {
        for (uint32_t sz = 0; sz < D; ++sz) {
          for (uint32_t szvi = 0; szvi < 4; ++szvi) {
            dst_weight_ptr[W * sy + sx + (4 * sz + szvi) * W * H] =
                image.get(sx, sy, sz, szvi);
          }
        }
      }
    }
...
vTensor pack_weights_2d_winograd_2_3(
  ...
  TORCH_CHECK(
      src_filter[Layout::Filter::width] == 3 &&
          src_filter[Layout::Filter::height] == 3,
  ...
  for (int64_t src_oc = 0; src_oc < src_oc_sz; ++src_oc) {
    ...
    for (int64_t src_ic = 0; src_ic < src_ic_sz; ++src_ic) {
      ...
      // const float* const src_k_ptr = src_weight_ptr + src_oc * src_block_sz +
      // src_ic * 9;
      ...
      dst_k
          [(4 * dst_oh + 0) * dst_kw_sz + 0 * dst_ow_sz + 4 * dst_ow + dst_iw] =
              m00;
      dst_k
          [(4 * dst_oh + 0) * dst_kw_sz + 4 * dst_ow_sz + 4 * dst_ow + dst_iw] =
              (m00 + m01 + m02) / 2.f;
      dst_k
          [(4 * dst_oh + 0) * dst_kw_sz + 8 * dst_ow_sz + 4 * dst_ow + dst_iw] =
              (m00 - m01 + m02) / 2.f;
      dst_k
          [(4 * dst_oh + 0) * dst_kw_sz + 12 * dst_ow_sz + 4 * dst_ow +
           dst_iw] = m02;
      dst_k
          [(4 * dst_oh + 1) * dst_kw_sz + 0 * dst_ow_sz + 4 * dst_ow + dst_iw] =
              m10;
      dst_k
          [(4 * dst_oh + 1) * dst_kw_sz + 4 * dst_ow_sz + 4 * dst_ow + dst_iw] =
              (m10 + m11 + m12) / 2.f;
      dst_k
          [(4 * dst_oh + 1) * dst_kw_sz + 8 * dst_ow_sz + 4 * dst_ow + dst_iw] =
              (m10 - m11 + m12) / 2.f;
      dst_k
          [(4 * dst_oh + 1) * dst_kw_sz + 12 * dst_ow_sz + 4 * dst_ow +
           dst_iw] = m12;
      dst_k
          [(4 * dst_oh + 2) * dst_kw_sz + 0 * dst_ow_sz + 4 * dst_ow + dst_iw] =
              m20;
      dst_k
          [(4 * dst_oh + 2) * dst_kw_sz + 4 * dst_ow_sz + 4 * dst_ow + dst_iw] =
              (m20 + m21 + m22) / 2.f;
      dst_k
          [(4 * dst_oh + 2) * dst_kw_sz + 8 * dst_ow_sz + 4 * dst_ow + dst_iw] =
              (m20 - m21 + m22) / 2.f;
      dst_k
          [(4 * dst_oh + 2) * dst_kw_sz + 12 * dst_ow_sz + 4 * dst_ow +
           dst_iw] = m22;
      dst_k
          [(4 * dst_oh + 3) * dst_kw_sz + 0 * dst_ow_sz + 4 * dst_ow + dst_iw] =
              m30;
      dst_k
          [(4 * dst_oh + 3) * dst_kw_sz + 4 * dst_ow_sz + 4 * dst_ow + dst_iw] =
              (m30 + m31 + m32) / 2.f;
      dst_k
          [(4 * dst_oh + 3) * dst_kw_sz + 8 * dst_ow_sz + 4 * dst_ow + dst_iw] =
              (m30 - m31 + m32) / 2.f;
      dst_k
          [(4 * dst_oh + 3) * dst_kw_sz + 12 * dst_ow_sz + 4 * dst_ow +
           dst_iw] = m32;
    }
  }
...
vTensor pack_weights(
  ...
  if (conv_method == Conv2dDepthwise) {
    return pack_weights_dw(context, command_buffer, pool, weight);
  }

  if (conv_method == Conv2dOld) {
    return pack_weights_2d_old(context, command_buffer, pool, weight);
  }

  if (conv_method == Conv2dWinograd_2_3) {
    return pack_weights_2d_winograd_2_3(context, command_buffer, pool, weight);
  }

  return pack_weights_2d(context, command_buffer, pool, weight);
}
...
vTensor pack_biases(
...

  {
    ...
    Future v_bias_future =
        v_bias.host<void, vTensor::Access::Write>(command_buffer);
...
void conv2d_dw(
  ...
  bool valid = C10_LIKELY(
      v_output.has_image() && v_input.has_image() && v_weight.has_image());
  ...
  api::Command::Buffer& command_buffer = command_pool.stream();
  {
    ...
    context->dispatch(
        ...
        // Read-only access is implied on const tensors and triggers an async
        // synchronization if necessary.
        v_bias.buffer(command_buffer, vTensor::Stage::Compute),
...
void conv2d_winograd_2_3(
  ...
  const int64_t out_h_units =
      div_up(v_output.sizes()[Layout::Activation4D::height], INT64_C(2));
  const int64_t out_w_units =
      div_up(v_output.sizes()[Layout::Activation4D::width], INT64_C(2));
  ...
  }
  {
    ...
    context->dispatch(
        ...
        v_output.image(
            command_buffer, vTensor::Stage::Compute, vTensor::Access::Write),
        v_input_winograd.image(command_buffer, vTensor::Stage::Compute),
        v_weight.image(command_buffer, vTensor::Stage::Compute),
        v_bias.buffer(command_buffer, vTensor::Stage::Compute),
        context->resource().pool.uniform(block).object);
  }
