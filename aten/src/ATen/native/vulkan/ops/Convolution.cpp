...
void conv2d_sliding_window_q(
  ...
  const double scale_in = v_input.get_scale();
  const int64_t zero_point_in = v_input.get_zero_point();
  const struct Block final {
    uvec3 extents;
    int32_t ic4;
    ivec4 kernel;
    ...
    safe_downcast<float>(scale_in),
    safe_downcast<int32_t>(zero_point),
    safe_downcast<int32_t>(zero_point_in),
    {
      safe_downcast<int32_t>(unpacked_filter[Layout::Filter::width]),
      safe_downcast<int32_t>(unpacked_filter[Layout::Filter::height]),
    },
