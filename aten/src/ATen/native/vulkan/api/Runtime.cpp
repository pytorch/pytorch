...
VkInstance create_instance(const Runtime::Type type) {
  ...
  if (Runtime::Type::Debug == type) {
    ...
    VK_CHECK(vkEnumerateInstanceLayerProperties(
        &instance_layers_count, instance_layer_properties.data()));
    ...
    for (const auto& requested_instance_extension :
         requested_instance_extensions) {
      for (const auto& extension : instance_extension_properties) {
        if (strcmp(requested_instance_extension, extension.extensionName) ==
            0) {
...
std::vector<VkPhysicalDevice> acquire_physical_devices(
  ...
  TORCH_CHECK(
      device_count > 0, "Vulkan: Could not find a device with Vulkan support!");
...
uint32_t query_compute_queue_family_index(
    const VkPhysicalDevice physical_device) {
  TORCH_INTERNAL_ASSERT_DEBUG_ONLY(
      physical_device, "Invalid Vulkan physical device!");
...
Runtime::Debug::Debug(const VkInstance instance) : instance_(instance) {
  TORCH_INTERNAL_ASSERT_DEBUG_ONLY(instance, "Invalid Vulkan instance!");
}
...
void Runtime::Debug::operator()(
  ...
  if (debug_report_callback) {
    ...
    vkDestroyDebugReportCallbackEXT(instance_, debug_report_callback, nullptr);
  }
}
...
Adapter Runtime::select(const Selector& selector) {
  ...
  TORCH_CHECK(
      false, "Vulkan: no adapter was selected as part of device enumeration!");
}
...
Runtime* runtime() {
  ...
  TORCH_INTERNAL_ASSERT_DEBUG_ONLY(runtime, "Invalid Vulkan runtime!");
