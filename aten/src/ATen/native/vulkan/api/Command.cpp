...
VkCommandPool create_command_pool(
  ...
  TORCH_CHECK(command_pool, "Invalid Vulkan command pool!");
...
void allocate_command_buffers(
  ...
  TORCH_INTERNAL_ASSERT_DEBUG_ONLY(
      command_pool, "Invalid Vulkan command pool!");

  TORCH_INTERNAL_ASSERT_DEBUG_ONLY(
      command_buffers && (count > 0u), "Invalid usage!");
  ...
  VK_CHECK(vkAllocateCommandBuffers(
      device, &command_buffer_allocate_info, command_buffers));
}
...
void Command::Buffer::barrier(const Pipeline::Barrier& barrier) {
  ...
  barriers_.buffers.insert(
      barriers_.buffers.end(), barrier.buffers.begin(), barrier.buffers.end());

  barriers_.images.insert(
      barriers_.images.end(), barrier.images.begin(), barrier.images.end());
}
...
void Command::Buffer::bind(const Pipeline::Object& pipeline) {
  ...
  TORCH_INTERNAL_ASSERT_DEBUG_ONLY(pipeline, "Invalid Vulkan pipeline!");

  if (pipeline.handle != bound_.pipeline.handle) {
    vkCmdBindPipeline(
        command_buffer_, VK_PIPELINE_BIND_POINT_COMPUTE, pipeline.handle);
...
void Command::Buffer::bind(const Descriptor::Set& set) {
  ...
  TORCH_INTERNAL_ASSERT_DEBUG_ONLY(
      descriptor_set, "Invalid Vulkan descriptor set!");
...
void Command::Buffer::copy(
  ...
  TORCH_INTERNAL_ASSERT_DEBUG_ONLY(source, "Invalid Vulkan source buffer!");

  TORCH_INTERNAL_ASSERT_DEBUG_ONLY(
      destination, "Invalid Vulkan destination buffer!");
  ...
  vkCmdCopyBuffer(
      command_buffer_, source.handle, destination.handle, 1u, &buffer_copy);
}

void Command::Buffer::dispatch(const Shader::WorkGroup& global_work_group) {
  TORCH_INTERNAL_ASSERT_DEBUG_ONLY(
      command_buffer_,
      "This command buffer is in an invalid state! "
      "Potential reason: This command buffer is moved from.");
...
Command::Pool::Pool(const GPU& gpu)
  ...
  TORCH_INTERNAL_ASSERT_DEBUG_ONLY(
      command_pool_, "Invalid Vulkan command pool!");
...
Command::Buffer Command::Pool::allocate() {
  ...
  if (buffer_.pool.size() == buffer_.in_use) {
    buffer_.pool.resize(buffer_.pool.size() + Configuration::kQuantum);
...
void Command::Pool::submit(
  ...
  TORCH_INTERNAL_ASSERT_DEBUG_ONLY(queue, "Invalid Vulkan queue!");
  ...
  for (const Buffer& buffer : buffers) {
    ...
    TORCH_INTERNAL_ASSERT_DEBUG_ONLY(
        command_buffer, "Invalid Vulkan command buffer!");
  ...
  if (!command_buffers.empty()) {
    ...
    {
      // vkQueueSubmit is not thread-safe, only one thread can push the commands
      // at a time. (See
      // https://vkguide.dev/docs/chapter-1/vulkan_command_flow/#vulkan-command-execution)
      // The number of available queues depends on GPU. It could be 1 and we
      // cannot assume we can create multiple queues. Thus, we need to avoid
      // calling vkQueueSubmit from multiple threads at the same time. When
      // running Vulkan backend in different threads without any locking
      // mechanism, vkQueueSubmit will get the
      // VK_ERROR_INITIALIZATION_FAILED(-3) error.
