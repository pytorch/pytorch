...
bool QueryPool::enable() {
  ...
  TORCH_CHECK(
      is_timestamps_supported_,
      "The device doesn't support for timestamps on all graphics and compute queues.");
  querypool_ = create_query_pool(device_, Configuration::kMaxQueryCount);
  return is_enabled();
}

std::vector<QueryPool::PerfInfo> QueryPool::disable(
...
int QueryPool::begin(
  ...
  TORCH_CHECK(
      newQueryIndex < Configuration::kMaxQueryCount,
      "The query index cannot exceed Configuration::kMaxQueryCount.");
  ...
  vkCmdWriteTimestamp(
      ...
      querypool_,
      newQueryIndex * Configuration::kTimestampsPerQuery);
...
void QueryPool::end(
  ...
  vkCmdWriteTimestamp(
      ...
      VK_PIPELINE_STAGE_BOTTOM_OF_PIPE_BIT,
      querypool_,
      static_cast<uint32_t>(queryIndex) * Configuration::kTimestampsPerQuery +
          1u);
}

std::vector<QueryPool::PerfInfo> QueryPool::result(
    const bool waitfor_allqueries) const {
  if (VK_NULL_HANDLE == querypool_) {
    return std::vector<QueryPool::PerfInfo>{};
  }

  std::vector<QueryPool::PerfInfo> perfInfo;
  const VkQueryResultFlags flags = waitfor_allqueries
      ? (VK_QUERY_RESULT_64_BIT | VK_QUERY_RESULT_WAIT_BIT)
      : VK_QUERY_RESULT_64_BIT;
  std::array<uint64_t, 2> counter_data{};
  for (uint32_t queryIndex = 0u; queryIndex < query_names_.size();
       ++queryIndex) {
    const auto& query_name = query_names_[queryIndex];

    // Grab the gpu timings (nanoseconds)
    auto ret = vkGetQueryPoolResults(
        device_,
        querypool_,
        queryIndex * Configuration::kTimestampsPerQuery,
        Configuration::kTimestampsPerQuery,
        sizeof(uint64_t) * counter_data.size(),
        counter_data.data(),
        sizeof(uint64_t),
        flags);
    if (ret != VK_SUCCESS) {
      std::stringstream msg;
      msg << "vkGetQueryPoolResults() for \"" << query_name << "\""
          << " returned an error code " << ret << ".";
      TORCH_WARN(msg.str());
      continue;
    }
    ...
    int64_t gpu_time_us = static_cast<int64_t>(
        ...
        1'000.f); // convert ns to us

    perfInfo.emplace_back(QueryPool::PerfInfo{
        query_name,
        static_cast<int64_t>(
            static_cast<double>(counter_data[0]) * timestamp_period_us_ /
            1'000.f),
        static_cast<int64_t>(
            static_cast<double>(counter_data[1]) * timestamp_period_us_ /
            1'000.f),
        gpu_time_us});
  }
  return perfInfo;
