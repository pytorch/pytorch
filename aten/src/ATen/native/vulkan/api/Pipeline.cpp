...
Pipeline::Layout::Factory::Factory(const GPU& gpu) : device_(gpu.device) {
  TORCH_INTERNAL_ASSERT_DEBUG_ONLY(device_, "Invalid Vulkan device!");
}

typename Pipeline::Layout::Factory::Handle Pipeline::Layout::Factory::
operator()(const Descriptor& descriptor) const {
  ...
  TORCH_CHECK(pipeline_layout, "Invalid Vulkan pipeline layout!");
...
VkPipelineCache create_pipeline_cache(const VkDevice device) {
  ...
  TORCH_CHECK(pipeline_cache, "Invalid Vulkan pipeline cache!");
...
Pipeline::Factory::Factory(const GPU& gpu)
  ...
  TORCH_INTERNAL_ASSERT_DEBUG_ONLY(
      pipeline_cache_, "Invalid Vulkan pipeline cache!");
}

typename Pipeline::Factory::Handle Pipeline::Factory::operator()(
    const Descriptor& descriptor) const {
  TORCH_INTERNAL_ASSERT_DEBUG_ONLY(
      descriptor.pipeline_layout, "Invalid Vulkan pipeline layout!");

  TORCH_INTERNAL_ASSERT_DEBUG_ONLY(
      descriptor.shader_module, "Invalid Vulkan shader module!");
  ...
  TORCH_CHECK(pipeline, "Invalid Vulkan pipeline!");
...
Pipeline::Cache::Cache(Factory factory) : cache_(std::move(factory)) {}
