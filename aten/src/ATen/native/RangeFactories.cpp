...
Tensor& logspace_cpu_out(
  ...
  if (steps == 0) {
    ...
    AT_DISPATCH_COMPLEX_TYPES(r.scalar_type(), "logspace_cpu", [&]() {
      ...
      at::parallel_for(
          0, steps, internal::GRAIN_SIZE, [&](int64_t p_begin, int64_t p_end) {
            ...
            for (int64_t i = p_begin; i < p_end;
                 ++i, is += 1) { // std::complex does not support ++operator
              if (i < halfway) {
                data_ptr[i] = std::pow(scalar_base, scalar_start + step * is);
              } else {
                data_ptr[i] = std::pow(
                    scalar_base,
                    scalar_end - (step * static_cast<scalar_t>(steps - i - 1)));
              }
            }
          });
    });
    ...
    AT_DISPATCH_ALL_TYPES_AND(
        kBFloat16, r.scalar_type(), "logspace_cpu", [&]() {
          double scalar_base =
              static_cast<double>(base); // will be autopromoted anyway
          scalar_t scalar_start = start.to<scalar_t>();
          scalar_t scalar_end = end.to<scalar_t>();
          scalar_t* data_ptr = r.data_ptr<scalar_t>();
          double step =
          ...
          at::parallel_for(
              ...
              [&](int64_t p_begin, int64_t p_end) {
                for (int64_t i = p_begin; i < p_end; i++) {
                  if (i < halfway) {
                    data_ptr[i] =
                        std::pow(scalar_base, scalar_start + step * i);
                  } else {
                    data_ptr[i] = std::pow(
                        scalar_base, scalar_end - step * (steps - i - 1));
                  }
                }
              });
        });
...
Tensor& range_cpu_out(
  ...
  AT_DISPATCH_ALL_TYPES(result.scalar_type(), "range_cpu", [&]() {
    ...
    at::parallel_for(
        0, size, internal::GRAIN_SIZE, [&](int64_t p_begin, int64_t p_end) {
          scalar_t is = p_begin;
...
Tensor& arange_cpu_out(
  ...
  AT_DISPATCH_ALL_TYPES(result.scalar_type(), "arange_cpu", [&]() {
    ...
    // The problem with using accscalar_t is that accscalar_t might be float32
    // on gpu for a float32 scalar_t, but double on cpu for the same, and the
    // effective output size starts differing on CPU vs GPU because of precision
    // issues, which we dont want. the corner-case we do want to take into
    // account is int64_t, which has higher precision than double
    double size_d;
    // NOLINTNEXTLINE(bugprone-branch-clone)
    if (std::is_same<scalar_t, int64_t>::value) {
      size_d = std::ceil(
          static_cast<double>(end.to<accscalar_t>() - start.to<accscalar_t>()) /
          step.to<accscalar_t>());
    } else {
      size_d = std::ceil(
