...
int64_t _default_cross_dim(
    ...
    IntArrayRef sizes) {
  // If dimension is not given, it defaults to the first dimension found with
  // the size 3. Note that this behaviour might be unexpected.
  // _default_cross_dim is called internally inside the cross implementation to
  // calculate the dim and finally cross delegates to the linalg_cross
  // implementation with this dim
...
(const Tensor& input,
 ...
 const Tensor& out) {
  ...
  cross_stub(
      input.device().type(), out, input_broadcasted, other_broadcasted, dim);
}
