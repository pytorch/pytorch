...
static void aminmax_kernel(
  ...
  TORCH_CHECK(
      min_result.scalar_type() == self.scalar_type() &&
          max_result.scalar_type() == self.scalar_type(),
      "Expect min and max dtype ",
      self.scalar_type(),
      " but got ",
      min_result.scalar_type(),
      " and ",
      max_result.scalar_type());

  AT_DISPATCH_ALL_TYPES_AND(
      ScalarType::Bool, self.scalar_type(), "aminmax_cpu", [&] {
...
static void where_kernel_impl(TensorIterator& iter) {
  AT_DISPATCH_ALL_TYPES_AND_COMPLEX_AND4(
      kComplexHalf, kHalf, kBFloat16, kBool, iter.dtype(), "where_cpu", [&] {
        cpu_kernel(
            iter,
...
// Default brute force implementation of isin(). Used when the number of test
// elements is small. Iterates through each element and checks it against each
// test element.
  ...
  AT_DISPATCH_ALL_TYPES_AND(kBFloat16, iter.common_dtype(), "clamp_cpu", [&]() {
    cpu_kernel_vec(
        ...
        [](Vectorized<scalar_t> a,
           Vectorized<scalar_t> min,
           Vectorized<scalar_t> max) {
          return vec::minimum(vec::maximum(a, min), max);
        });
  });
}

static void clamp_scalar_kernel_impl(
    TensorIteratorBase& iter,
    const Scalar& min_,
    const Scalar& max_) {
  AT_DISPATCH_ALL_TYPES_AND(
      kBFloat16, iter.common_dtype(), "clamp_scalar_cpu", [&]() {
...
static void clamp_max_scalar_kernel_impl(
    TensorIteratorBase& iter,
    Scalar max_) {
  AT_DISPATCH_ALL_TYPES_AND(
      kBFloat16, iter.common_dtype(), "clamp_max_scalar_cpu", [&]() {
        const auto max = max_.to<scalar_t>();
        const Vectorized<scalar_t> max_vec(max);
        cpu_kernel_vec(
            iter,
            [=](scalar_t a) -> scalar_t { return std::min(a, max); },
            [=](Vectorized<scalar_t> a) { return vec::clamp_max(a, max_vec); });
      });
}

static void clamp_min_scalar_kernel_impl(
    TensorIteratorBase& iter,
    Scalar min_) {
  AT_DISPATCH_ALL_TYPES_AND(
      kBFloat16, iter.common_dtype(), "clamp_min_cpu", [&]() {
        const auto min = min_.to<scalar_t>();
        const Vectorized<scalar_t> min_vec(min);
        cpu_kernel_vec(
            iter,
            [=](scalar_t a) -> scalar_t { return std::max(a, min); },
            [=](Vectorized<scalar_t> a) { return vec::clamp_min(a, min_vec); });
      });
}
