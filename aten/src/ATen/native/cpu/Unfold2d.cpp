...
static void unfolded2d_acc_channels_last(
  ...
  for (int64_t y = 0; y < output_height; y++) {
    for (int64_t x = 0; x < output_width; x++) {
      scalar_t* src = finput_data + y * output_width * kH * kW * n_input_plane +
        ...
        for (int64_t kh = 0; kh < kH; kh++) {
          for (int64_t kw = 0; kw < kW; kw++) {
            ...
            if (iy < 0 || iy >= input_height || ix < 0 || ix >= input_width) {
              ...
              scalar_t* dst_slice =
                  dst + iy * input_width * n_input_plane + ix * n_input_plane;
              scalar_t* src_slice =
                  src + kh * kW * n_input_plane + kw * n_input_plane;
              cadd(dst_slice, dst_slice, src_slice, n_input_plane);
            }
          }
        }
      } else {
...
static void unfolded2d_copy_channels_last(
    ...
    int64_t output_width) {
  at::parallel_for(
      0, output_height * output_width, 0, [&](int64_t start, int64_t end) {
        int64_t y = 0;
        int64_t x = 0;
        ...
        for (const auto k : c10::irange(start, end)) {
          ...
          scalar_t* dst = finput_data +
              y * output_width * kH * kW * n_input_plane +
              x * kH * kW * n_input_plane;
          ...
          if (padW > 0 || padH > 0) {
            for (int64_t kh = 0; kh < kH; kh++) {
              for (int64_t kw = 0; kw < kW; kw++) {
                int64_t iy = y * dH - padH + kh;
                int64_t ix = x * dW - padW + kw;
                if (iy < 0 || iy >= input_height || ix < 0 ||
                    ix >= input_width) {
                  memset(
                      dst + kh * kW * n_input_plane + kw * n_input_plane,
                      0,
                      sizeof(scalar_t) * n_input_plane);
                } else {
                  memcpy(
                      dst + kh * kW * n_input_plane + kw * n_input_plane,
                      src + iy * input_width * n_input_plane +
                          ix * n_input_plane,
                      sizeof(scalar_t) * n_input_plane);
                }
              }
...
void unfolded2d_copy_kernel(
  ...
  if (is_channels_last) {
    AT_DISPATCH_ALL_TYPES_AND(
        at::ScalarType::BFloat16, dtype, "unfolded2d_copy_channels_last", [&] {
