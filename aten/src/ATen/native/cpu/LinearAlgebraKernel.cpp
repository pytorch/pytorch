...
void addr_kernel(
  ...
  AT_DISPATCH_ALL_TYPES_AND_COMPLEX_AND2(
      kBFloat16, kHalf, iter.dtype(), "addr_cpu", [&]() {
        ...
        if (beta_val == zero_val) {
          cpu_kernel_vec(
              ...
              [=](Vec self_vec, Vec vec1_vec, Vec vec2_vec)
          ...
          cpu_kernel_vec(
              ...
              [=](Vec self_vec, Vec vec1_vec, Vec vec2_vec)
                  __ubsan_ignore_undefined__ {
                    return beta_vec * self_vec +
                        alpha_vec * vec1_vec * vec2_vec;
                  });
        }
      });
}
...
void linalg_vector_norm_kernel_cpu_impl(TensorIterator& iter, Scalar ord) {
  ...
  acc_t init_val = (ord_val == -INFINITY)
  ...
  if (ord_val == 0) {
    ...
    binary_kernel_reduce(
        iter, NormOps<scalar_t, acc_t>{static_cast<acc_t>(ord_val)}, init_val);
  }
...
static void linalg_vector_norm_kernel_cpu(TensorIterator& iter, Scalar ord) {
  AT_DISPATCH_FLOATING_AND_COMPLEX_TYPES_AND2(
      kHalf, kBFloat16, iter.input_dtype(), "linalg_vector_norm_cpu", [&] {
