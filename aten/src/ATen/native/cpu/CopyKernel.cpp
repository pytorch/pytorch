...
void direct_copy_kernel(TensorIteratorBase& iter) {
  ...
  if (isQIntType(dtype)) {
    ...
    cpu_kernel(iter, [=](c10::complex<at::Half> a) -> c10::complex<at::Half> {
      return a;
    });
  } else {
...
void copy_same_dtype(
    ...
    bool requires_neg) {
  if (requires_neg) {
    // This case should never actually happen since currently there's no way to
    // get a complex tensor with negative bit.
    if (requires_conj) {
