...
static void binary_cross_entropy_backward_cpu_kernel(TensorIterator& iter, Scalar value) {
  ScalarType dtype = iter.dtype(0);
  AT_DISPATCH_FLOATING_TYPES_AND_HALF(dtype, "binary_cross_entropy_backward_cpu_out", [&] {
    scalar_t scalar_val = value.to<scalar_t>();
    auto scalar_vec = Vec256<scalar_t>(scalar_val);
    scalar_t one_val = (scalar_t)(1);
    auto one_vec = Vec256<scalar_t>(one_val);
    scalar_t eps_val = (scalar_t)(1e-12);
    auto eps_vec = Vec256<scalar_t>(eps_val);
    cpu_kernel_vec(
        iter,
        [=](scalar_t self_val, scalar_t t1_val, scalar_t t2_val) -> scalar_t {
          return scalar_val * (self_val - t1_val) / ((one_val - self_val + eps_val) *(self_val + eps_val)) * t2_val;
        },
        [=](Vec256<scalar_t> self_vec,
            Vec256<scalar_t> t1_vec,
            Vec256<scalar_t> t2_vec) {
          return scalar_vec * (self_vec - t1_vec) / ((one_vec - self_vec + eps_vec) *(self_vec + eps_vec)) * t2_vec;
    });
  });
}
...
REGISTER_DISPATCH(binary_cross_entropy_backward_stub, &binary_cross_entropy_backward_cpu_kernel);
