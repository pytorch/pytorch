  ...
  AT_DISPATCH_ALL_TYPES(dtype, "smooth_l1_backward_cpu_out", [&] {
    ...
    cpu_kernel_vec(
        ...
        [norm_val_vec, beta_val_vec, neg_1_vec, zero_vec, pos_1_vec](
            Vec256<scalar_t> input,
            Vec256<scalar_t> target,
            Vec256<scalar_t> grad_output) -> Vec256<scalar_t> {
          // using two blendv calls to simulate the 3 cases
          // 1        if  x >= beta
          // -1       if x <= -beta
          // x / beta if |x| < beta
          const auto x = input - target;
          const auto pos_or_neg_1_vec =
              Vec256<scalar_t>::blendv(neg_1_vec, pos_1_vec, x > zero_vec);
