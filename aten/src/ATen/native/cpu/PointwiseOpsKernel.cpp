...
static void addcmul_cpu_kernel(TensorIterator& iter, const Scalar& value) {
  ...
  AT_DISPATCH_ALL_TYPES_AND_COMPLEX(dtype, "addcmul_cpu_out", [&] {
    ...
    auto scalar_vec = Vectorize<scalar_t>(scalar_val);
    cpu_kernel_vec(
        iter,
        ...
        [=](Vectorize<scalar_t> self_vec,
            Vectorize<scalar_t> t1_vec,
            Vectorize<scalar_t> t2_vec) {
...
static void smooth_l1_backward_cpu_kernel(TensorIterator& iter, const Scalar& norm, double beta) {
  ...
  AT_DISPATCH_ALL_TYPES(dtype, "smooth_l1_backward_cpu_out", [&] {
    ...
    auto norm_val_vec = Vectorize<scalar_t>(norm_val);
    auto beta_val_vec = Vectorize<scalar_t>(beta_val);
    const auto neg_1_vec = Vectorize<scalar_t>(-1);
    const auto zero_vec = Vectorize<scalar_t>(0);
    const auto pos_1_vec = Vectorize<scalar_t>(1);
    cpu_kernel_vec(iter,
      ...
      [norm_val_vec, beta_val_vec, neg_1_vec, zero_vec, pos_1_vec](
         Vectorize<scalar_t> input, Vectorize<scalar_t> target, Vectorize<scalar_t> grad_output) -> Vectorize<scalar_t> {
        // using two blendv calls to simulate the 3 cases
        // 1        if  x >= beta
        // -1       if x <= -beta
        // x / beta if |x| < beta
        const auto x = input - target;
        const auto pos_or_neg_1_vec = Vectorize<scalar_t>::blendv(
            neg_1_vec, pos_1_vec, x > zero_vec);
        const auto x_abs = x.abs();
        const auto output = Vectorize<scalar_t>::blendv(
...
static void huber_backward_cpu_kernel(TensorIterator& iter, const Scalar& norm, double delta) {
  ...
  AT_DISPATCH_FLOATING_TYPES_AND2(kBFloat16, kHalf, dtype, "huber_backward_cpu_out", [&] {
    ...
    auto norm_val_vec = Vectorize<scalar_t>(norm_val);
    auto delta_val_vec = Vectorize<scalar_t>(delta_val);
    const auto neg_1_vec = Vectorize<scalar_t>(-1);
    const auto zero_vec = Vectorize<scalar_t>(0);
    const auto pos_1_vec = Vectorize<scalar_t>(1);
    cpu_kernel_vec(iter,
