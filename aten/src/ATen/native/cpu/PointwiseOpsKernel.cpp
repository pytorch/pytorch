...
static void smooth_l1_backward_cpu_kernel(
    TensorIterator& iter,
    const Scalar& norm,
    double beta) {
  ScalarType dtype = iter.dtype(0);
  if (dtype == kBFloat16) {
    ...
    cpu_kernel_vec(
        ...
        [norm_val_vec, beta_val_vec, neg_1_vec, zero_vec, pos_1_vec](
            Vectorized<BFloat16> input,
            Vectorized<BFloat16> target,
            Vectorized<BFloat16> grad_output) -> Vectorized<BFloat16> {
          // using two blendv calls to simulate the 3 cases
          // 1        if  x >= beta
          // -1       if x <= -beta
          // x / beta if |x| < beta
          Vectorized<float> input0, input1, target0, target1, grad_output0,
              grad_output1;
          std::tie(input0, input1) = convert_bfloat16_float(input);
          std::tie(target0, target1) = convert_bfloat16_float(target);
          std::tie(grad_output0, grad_output1) =
              convert_bfloat16_float(grad_output);
          auto x = input0 - target0;
          auto pos_or_neg_1_vec =
              Vectorized<float>::blendv(neg_1_vec, pos_1_vec, x > zero_vec);
          auto x_abs = x.abs();
    ...
    AT_DISPATCH_ALL_TYPES(dtype, "smooth_l1_backward_cpu_out", [&] {
      ...
      cpu_kernel_vec(
          ...
          [norm_val_vec, beta_val_vec, neg_1_vec, zero_vec, pos_1_vec](
              Vectorized<scalar_t> input,
              Vectorized<scalar_t> target,
              Vectorized<scalar_t> grad_output) -> Vectorized<scalar_t> {
            // using two blendv calls to simulate the 3 cases
            // 1        if  x >= beta
            // -1       if x <= -beta
            // x / beta if |x| < beta
...
static void huber_backward_cpu_kernel(
    TensorIterator& iter,
    const Scalar& norm,
    double delta) {
  ScalarType dtype = iter.dtype(0);
  AT_DISPATCH_FLOATING_TYPES_AND2(
      kBFloat16, kHalf, dtype, "huber_backward_cpu_out", [&] {
        auto norm_val = norm.to<scalar_t>();
