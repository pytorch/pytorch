...
static void exponential_kernel_default(Tensor& self, double lambda, c10::optional<Generator> gen) {
  CPUGeneratorImpl* generator = get_generator_or_default<CPUGeneratorImpl>(gen, detail::getDefaultCPUGenerator());
...
void exponential_kernel(Tensor& self, double lambda, c10::optional<Generator> gen) {
  exponential_kernel_default(self, lambda, gen);
}
#else
void exponential_kernel(Tensor &self, double lambda, c10::optional<Generator> gen) {
        ...
        if (len > 0) {
          ...
          if (std::is_same<scalar_t, double>::value) {
            ...
            vdRngExponential(VSL_RNG_METHOD_EXPONENTIAL_ICDF, stream, len,
              (double *)(sample_ptr + begin), 0, lambda <= 1e-15 ? 0. : 1./lambda);
            vslDeleteStream(&stream);
          } else {
            vslNewStream(&stream, VSL_BRNG_MCG31, seed);
            vslSkipAheadStream(stream, begin);
            vsRngExponential(VSL_RNG_METHOD_EXPONENTIAL_ICDF, stream, len,
              (float *) (sample_ptr + begin), 0, lambda <= 1e-15 ? 0. : 1./lambda);
            vslDeleteStream(&stream);
          }
