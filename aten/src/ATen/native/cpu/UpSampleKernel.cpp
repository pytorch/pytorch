...
inline VecType<scalar_t> interpolate(
    const scalar_t* t,
    accscalar_t w,
    Args... args) {
  return VecType<scalar_t>::loadu(t) * VecType<scalar_t>(w) +
        ...
        [&] {
          ...
          for (const auto i : c10::irange(output_size)) {
            const scalar_t real_input_index =
                area_pixel_compute_source_index<scalar_t>(
...
struct HelperInterpLinear : public HelperInterpBase {
  ...
  static inline std::vector<Tensor> compute_indices_weights_aa(
    ...
    AT_DISPATCH_FLOATING_TYPES(scalar_type, "compute_indices_weights_aa", [&] {
      scalar_t scale = area_pixel_compute_scale<scalar_t>(
          input_size, output_size, align_corners, opt_scale);
      ...
      indices_weights =
          HelperInterpLinear::_compute_indices_weights_aa<scalar_t>(
              input_size,
              output_size,
              stride,
              ndims,
    ...
    AT_DISPATCH_FLOATING_TYPES_AND(
        ...
        [&] {
          ...
          for (const auto i : c10::irange(output_size)) {
            ...
            input_index = static_cast<int64_t>(floorf(real_input_index));
            get_cubic_upsample_coefficients<scalar_t>(
...
#undef a
  ...
  static inline std::vector<Tensor> compute_indices_weights_aa(
    ...
    AT_DISPATCH_FLOATING_TYPES(scalar_type, "compute_indices_weights_aa", [&] {
      ...
      indices_weights =
          HelperInterpCubic::_compute_indices_weights_aa<scalar_t>(
              input_size,
              output_size,
              stride,
              ndims,
...
// Generic separable upsampling interpolation kernels for N-d case with
// anti-aliasing
template <int out_ndims, typename scale_type, class F>
void _separable_upsample_generic_Nd_kernel_impl_single_dim(
  ...
  indices_weights.emplace_back(F::compute_indices_weights_aa(
      input_scalar_type,
...
void separable_upsample_generic_Nd_kernel_impl(
  ...
  for (const auto i : c10::irange(out_ndims - 1)) {
    ...
    _separable_upsample_generic_Nd_kernel_impl_single_dim<
        out_ndims,
        scale_t,
        F>(temp_output, temp_input, interp_dim, align_corners, scales);
    temp_input = temp_output;
  }
  _separable_upsample_generic_Nd_kernel_impl_single_dim<out_ndims, scale_t, F>(
      output, temp_input, 2, align_corners, scales);
}
...
void upsample_nearest2d_kernel_impl(
  ...
  if (input.is_contiguous(at::MemoryFormat::ChannelsLast)) {
    AT_DISPATCH_FLOATING_TYPES_AND2(
        ...
        [&] {
          cpu_upsample_nearest_channels_last<scalar_t, scale_t, nearest_idx>(
    ...
    AT_DISPATCH_FLOATING_TYPES_AND(
        ...
        [&] {
          cpu_upsample_nearest_channels_last<
              scalar_t,
              scale_t,
              nearest_exact_idx>(output, input, {scales_h, scales_w});
        });
  } else {
    ...
    AT_DISPATCH_FLOATING_TYPES_AND(
        ...
        [&] {
          cpu_upsample_nearest_channels_last<
              scalar_t,
              scale_t,
              nearest_exact_idx>(output, input, {scales_d, scales_h, scales_w});
        });
  } else {
...
REGISTER_DISPATCH(
    ...
    &upsample_bilinear2d_aa_kernel_impl);
REGISTER_DISPATCH(
    _upsample_bilinear2d_aa_backward_kernel,
    &upsample_bilinear2d_aa_backward_kernel_impl);
REGISTER_DISPATCH(
...
REGISTER_DISPATCH(
    ...
    &upsample_bicubic2d_aa_kernel_impl);
REGISTER_DISPATCH(
    _upsample_bicubic2d_aa_backward_kernel,
    &upsample_bicubic2d_aa_backward_kernel_impl);
} // namespace native
} // namespace at
