...
#define USE_VECTORIZED_KERNEL_COND (((input.is_contiguous(at::MemoryFormat::ChannelsLast)) && (input.size(-3) > 3)) || ((output.size(-2) + output.size(-1)) <= 128))
...
void upsample_bilinear2d_kernel_impl(
  ...
  // See note above about USE_VECTORIZED_KERNEL_COND. The extra cond is present
  ...
  if ((USE_VECTORIZED_KERNEL_COND) || (at::get_num_threads() == 1 && input.size(-3) == 3)) {
