...
template <typename scalar_t>
struct LoadImpl<Vectorize<scalar_t>> {
  static Vectorize<scalar_t> load(const char * C10_RESTRICT data, int64_t stride, int64_t index) {
    auto *ptr = data + index * stride;
    return Vectorize<scalar_t>::loadu(ptr);
  }
};
    ...
    [&] {
      ...
      iter.parallel_reduce(
        [&](char** data, const int64_t* strides, int64_t size0, int64_t size1) {
          ...
          if (in_strides[0] == sizeof(scalar_t) && size0 >= Vectorize<scalar_t>::size()) {
            // Contiguous inner reduction
            vectorized_inner_sum<scalar_t>(data, in_strides[1], out_stride, size0, size1);
          } else if (in_strides[1] == sizeof(scalar_t) && size1 >= Vectorize<scalar_t>::size()) {
