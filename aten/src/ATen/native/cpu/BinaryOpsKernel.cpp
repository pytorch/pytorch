...
void add_kernel(TensorIterator& iter, Scalar alpha_scalar) {
  if (iter.dtype() == ScalarType::Bool) {
    ...
    AT_DISPATCH_ALL_TYPES_AND(kHalf, iter.dtype(), "add_cpu", [&]() {
...
void mul_kernel(TensorIterator& iter) {
  if (iter.dtype() == ScalarType::Bool) {
    ...
    AT_DISPATCH_ALL_TYPES_AND(kHalf, iter.dtype(), "mul_cpu", [&]() {
      cpu_kernel_vec(iter,
...
void div_kernel(TensorIterator& iter) {
  if (isIntegralType(iter.dtype())) {
    ...
    AT_DISPATCH_FLOATING_TYPES_AND_HALF(iter.dtype(), "div_cpu", [&]() {
      cpu_kernel_vec(iter,
