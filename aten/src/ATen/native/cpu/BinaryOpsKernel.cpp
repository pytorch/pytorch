...
void smooth_l1_kernel(TensorIterator& iter, Scalar beta) {
  AT_DISPATCH_FLOATING_TYPES_AND2(
        kBFloat16, kHalf, iter.dtype(), "smooth_l1_cpu", [&]() {
        using Vec = Vec256<scalar_t>;
        const Vec beta_vec(static_cast<scalar_t>(beta));
        const Vec point_five_vec(static_cast<scalar_t>(0.5));
        cpu_kernel_vec(
            iter,
            [&beta](scalar_t a, scalar_t b) -> scalar_t {
              auto z = std::abs(a - b);
              return z < static_cast<scalar_t>(beta)
                  ? static_cast<scalar_t>(0.5) * z * z / static_cast<scalar_t>(beta)
                  : z - static_cast<scalar_t>(0.5) * static_cast<scalar_t>(beta);;
            },
