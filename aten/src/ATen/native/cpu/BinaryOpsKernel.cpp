...
void smooth_l1_kernel(TensorIterator& iter) {
  AT_DISPATCH_FLOATING_TYPES_AND(kBFloat16, iter.dtype(), "smooth_l1_cpu", [&]() {
    ...
    cpu_kernel_vec(iter,
      [](scalar_t a, scalar_t b) -> scalar_t {
        auto z = std::abs(a - b);
        return z < static_cast<scalar_t>(1) ? static_cast<scalar_t>(0.5) * z * z : z - static_cast<scalar_t>(0.5);
      },
      [&one_vec, &point_five_vec](Vec a, Vec b) {
        auto z = (a - b).abs();
        return Vec::blendv(point_five_vec * z * z, z - point_five_vec, z < one_vec);
      });
  });
}
