...
void eq_kernel(TensorIteratorBase& iter) {
  ...
  if (iter.dtype() == ScalarType::Bool) {
    ...
    AT_DISPATCH_ALL_TYPES_AND_COMPLEX_AND3(
        kComplexHalf, kBFloat16, kHalf, iter.common_dtype(), "eq_cpu", [&]() {
          cpu_kernel_vec(
              iter,
...
void ne_kernel(TensorIteratorBase& iter) {
  ...
  if (iter.dtype() == ScalarType::Bool) {
    ...
    AT_DISPATCH_ALL_TYPES_AND_COMPLEX_AND3(
        kComplexHalf, kBFloat16, kHalf, iter.common_dtype(), "ne_cpu", [&]() {
          cpu_kernel_vec(
              iter,
              [](scalar_t a, scalar_t b) -> scalar_t {
                return static_cast<scalar_t>(a != b);
    ...
    cpu_kernel_vec(
        ...
        [=](BFloat16 a, BFloat16 b) -> BFloat16 {
          ...
          if (std::isinf(a0) && a0 == b0) {
            ...
            return m0 +
                std::log(
                       static_cast<float>(1.0) + std::exp(-std::abs(a0 - b0)));
          }
        },
        [=](Vectorized<BFloat16> a, Vectorized<BFloat16> b) {
          Vectorized<float> a0, a1, b0, b1;
          ...
          if (std::isinf(a0) && a0 == b0) {
            ...
            return m0 +
                std::log2(
                       ...
                       std::pow(static_cast<float>(2), -std::abs(a0 - b0)));
          }
        },
        [=](Vectorized<BFloat16> a, Vectorized<BFloat16> b) {
          Vectorized<float> a0, a1, b0, b1;
          ...
          [=](scalar_t a, scalar_t b) -> scalar_t {
            if (std::isinf(a) && a == b) {
              ...
              return m +
                  std::log2(
                         static_cast<scalar_t>(1.0) +
                         std::pow(static_cast<scalar_t>(2), -std::abs(a - b)));
            }
          },
          [=](Vectorized<scalar_t> a, Vectorized<scalar_t> b) {
            Vectorized<scalar_t> inf(std::numeric_limits<scalar_t>::infinity());
