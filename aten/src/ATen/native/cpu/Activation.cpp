  ...
  if (input.scalar_type() == kBFloat16) {
    ...
    parallel_for(0, input.numel(), 1, [&](int64_t begin, int64_t end) {
      ...
      for (; d < size - (size % Vec::size()); d += Vec::size()) {
        ...
        Vectorized<float> min_vec =
            minimum(data_vec0, Vectorized<float>(float(0)));
        ...
        convert_float_bfloat16(buffer_vec0, buffer_vec1)
            .store(buffer_data + begin + d);
        convert_float_bfloat16(output_vec0, output_vec1)
            .store(output_data + begin + d);
      }
      if (size - d > 0) {
        ...
        Vectorized<float> output_vec1 = min_vec - buffer_vec1.log1p();
        convert_float_bfloat16(buffer_vec0, buffer_vec1)
            .store(buffer_data + begin + d, size - d);
        convert_float_bfloat16(output_vec0, output_vec1)
            .store(output_data + begin + d, size - d);
      }
    });
  } else {
...
static void log_sigmoid_cpu_kernel(
  ...
  AT_DISPATCH_FLOATING_TYPES_AND(
      kBFloat16, input.scalar_type(), "log_sigmoid_cpu", [&] {
  ...
  if (iter.dtype() == kBFloat16) {
    ...
    cpu_kernel_vec(
        ...
        [=](Vec a, Vec b, Vec c) -> Vec {
          ...
          auto max_deriv_vec =
              Vectorized<float>::blendv(zero_vec, one_vec, mask);
          auto sign_vec =
              Vectorized<float>::blendv(one_vec.neg(), one_vec, mask);
    ...
    cpu_kernel_vec(
        ...
        [negcoef, negiptcoef, poscoef](BFloat16 a) -> BFloat16 {
          return float(a) <= float(0)
              ? (std::exp(float(a) * negiptcoef) - float(1)) * negcoef
              : float(a) * poscoef;
        },
        [&negcoef_vec, &negiptcoef_vec, &poscoef_vec, &one_vec, &zero_vec](
          ...
          if (!cmp0.zero_mask() &&
              !cmp1.zero_mask()) { // only a * poscoef (which is very quick)
                                   // needs to be computed
            return convert_float_bfloat16(a0 * poscoef_vec, a1 * poscoef_vec);
          } else {
            auto res0 = Vectorized<float>::blendv(
                ((a0 * negiptcoef_vec).exp() - one_vec) * negcoef_vec,
                a0 * poscoef_vec,
                cmp0);
            auto res1 = Vectorized<float>::blendv(
                ((a1 * negiptcoef_vec).exp() - one_vec) * negcoef_vec,
                a1 * poscoef_vec,
                cmp1);
...
void elu_backward_kernel(
  ...
  if (it.common_dtype() == kBFloat16) {
    ...
    cpu_kernel_vec(
        it,
        [negcoef, negiptcoef, poscoef, is_result](
            BFloat16 a, BFloat16 b) -> BFloat16 {
          if (is_result) {
            return float(b) <= float(0)
                ? float(a) * negiptcoef * (float(b) + negcoef)
                : float(a) * poscoef;
          } else {
            return float(b) <= float(0) ? float(a) * negiptcoef * negcoef *
                    std::exp(float(b) * negiptcoef)
                                        : float(a) * poscoef;
          }
        },
        [&negcoef_vec, &negiptcoef_vec, &poscoef_vec, &zero_vec, is_result](
            Vectorized<BFloat16> a,
            Vectorized<BFloat16> b) -> Vectorized<BFloat16> {
          Vectorized<float> a0, a1;
          std::tie(a0, a1) = convert_bfloat16_float(a);
          ...
          if (is_result) {
            if (!cmp0.zero_mask() &&
              ...
              auto res0 = Vectorized<float>::blendv(
                  a0 * negiptcoef_vec * (b0 + negcoef_vec),
                  a0 * poscoef_vec,
                  cmp0);
              auto res1 = Vectorized<float>::blendv(
                  a1 * negiptcoef_vec * (b1 + negcoef_vec),
                  a1 * poscoef_vec,
                  cmp1);
              return convert_float_bfloat16(res0, res1);
            }
            ...
            auto res0 = Vectorized<float>::blendv(
                a0 * negiptcoef_vec * negcoef_vec * (b0 * negiptcoef_vec).exp(),
                a0 * poscoef_vec,
                cmp0);
            auto res1 = Vectorized<float>::blendv(
                a1 * negiptcoef_vec * negcoef_vec * (b1 * negiptcoef_vec).exp(),
                a1 * poscoef_vec,
                cmp1);
            return convert_float_bfloat16(res0, res1);
          }
      ...
      cpu_kernel_vec(
          ...
          [negcoef, negiptcoef, poscoef, is_result](
            ...
            if (is_result) {
              ...
              return b <= scalar_t(0)
                  ? a * negiptcoef * negcoef * std::exp(b * negiptcoef)
                  : a * poscoef;
            }
          },
          [&negcoef_vec, &negiptcoef_vec, &poscoef_vec, &zero_vec, is_result](
            ...
            if (is_result) {
              if (!cmp.zero_mask()) { // only a * poscoef (which is very quick)
                ...
                return Vec::blendv(
                    a * negiptcoef_vec * (b + negcoef_vec),
                    a * poscoef_vec,
                    cmp);
              }
            } else {
              return Vec::blendv(
                  a * negiptcoef_vec * negcoef_vec * (b * negiptcoef_vec).exp(),
                  a * poscoef_vec,
                  cmp);
            }
          });
  ...
  if (iter.dtype() == kBFloat16) {
    ...
    cpu_kernel_vec(
        ...
        [&](vec::Vectorized<BFloat16> self_val) -> vec::Vectorized<BFloat16> {
          ...
          self_val0 =
              minimum(maximum(self_val0 + kThreeVec, kZeroVec), kSixVec) /
              kSixVec;
          self_val1 =
              minimum(maximum(self_val1 + kThreeVec, kZeroVec), kSixVec) /
              kSixVec;
    ...
    AT_DISPATCH_FLOATING_TYPES(iter.dtype(), "hardsigmoid_cpu", [&] {
      ...
      cpu_kernel_vec(
          ...
          [&](Vec self_val) {
            return vec::minimum(
                       vec::maximum(self_val + kThreeVec, kZeroVec), kSixVec) /
                kSixVec;
          });
    });
  }
...
void hardsigmoid_backward_kernel(TensorIteratorBase& iter) {
  if (iter.dtype() == kBFloat16) {
    ...
    cpu_kernel_vec(
        ...
        [=](Vectorized<BFloat16> grad_val,
            Vectorized<BFloat16> self_val) -> Vectorized<BFloat16> {
          ...
          self_val0 =
              Vec::blendv(kZeroVec, grad_val0 * kOneSixthVec, gradNonZeroMask);
          gradNonZeroMask = (self_val1 > neg_three) & (self_val1 < three);
          self_val1 =
              Vec::blendv(kZeroVec, grad_val1 * kOneSixthVec, gradNonZeroMask);
          return convert_float_bfloat16(self_val0, self_val1);
        });
  } else {
    AT_DISPATCH_FLOATING_TYPES(iter.dtype(), "hardsigmoid_backward", [&] {
      ...
      cpu_kernel_vec(
          ...
          [=](Vec grad_val, Vec self_val) {
            ...
            return Vec::blendv(
                kZeroVec, grad_val * kOneSixthVec, gradNonZeroMask);
          });
    });
  }
}
...
void hardshrink_kernel(TensorIteratorBase& iter, const Scalar& lambd) {
  AT_DISPATCH_FLOATING_TYPES_AND(
      kBFloat16, iter.dtype(), "hardshrink_cpu", [&] {
...
void softshrink_kernel(TensorIteratorBase& iter, const Scalar& lambd) {
  if (iter.dtype() == kBFloat16) {
    ...
    cpu_kernel_vec(
        iter,
        [=](BFloat16 a) -> BFloat16 {
          return float(a) > lambd_val
              ? a - lambd_val
              : (float(a) < -lambd_val ? a + lambd_val : float(0));
        },
        [=](Vectorized<BFloat16> self_val) {
          Vectorized<float> self_val0, self_val1;
          Vectorized<BFloat16> self_val_t0, self_val_t1;
          std::tie(self_val0, self_val1) = convert_bfloat16_float(self_val);
          self_val_t0 = convert_float_bfloat16(
              (self_val0 > lambdVec) & (self_val0 - lambdVec),
              (self_val1 > lambdVec) & (self_val1 - lambdVec));
          self_val_t1 = convert_float_bfloat16(
              (self_val0 < -lambd_val) & (self_val0 + lambdVec),
              (self_val1 < -lambd_val) & (self_val1 + lambdVec));
...
void shrink_backward_kernel(TensorIteratorBase& iter, const Scalar& lambd) {
  AT_DISPATCH_FLOATING_TYPES_AND(
      kBFloat16, iter.dtype(), "shrink_backward_cpu", [&] {
        auto lambd_val = lambd.to<scalar_t>();
        cpu_kernel_vec(
            iter,
            [=](scalar_t grad_val, scalar_t self_val) {
...
void hardswish_kernel(TensorIterator& iter) {
  if (iter.dtype() == kBFloat16) {
    ...
    cpu_kernel_vec(
        iter,
        [&](BFloat16 x) -> BFloat16 {
          return float(x) * std::min(std::max(float(x) + three, zero), six) /
              six;
        },
        [&](vec::Vectorized<BFloat16> x_vec) {
          ...
          x_vec0 = x_vec0 *
              minimum(maximum(x_vec0 + kThreeVec, kZeroVec), kSixVec) / kSixVec;
          x_vec1 = x_vec1 *
              minimum(maximum(x_vec1 + kThreeVec, kZeroVec), kSixVec) / kSixVec;
    ...
    AT_DISPATCH_FLOATING_TYPES(iter.dtype(), "hardswish_cpu", [&]() {
      ...
      cpu_kernel_vec(
          ...
          [&](Vec x_vec) {
            return x_vec *
                vec::minimum(
                       vec::maximum(x_vec + kThreeVec, kZeroVec), kSixVec) /
                kSixVec;
          });
    });
  }
...
void hardswish_backward_kernel(TensorIterator& iter) {
  if (iter.dtype() == kBFloat16) {
    ...
    cpu_kernel_vec(
        ...
        [&](vec::Vectorized<BFloat16> grad_val,
            vec::Vectorized<BFloat16> self_val) {
  ...
  if (iter.common_dtype() == kBFloat16) {
    ...
    cpu_kernel_vec(
        ...
        [&](Vectorized<BFloat16> a) -> Vectorized<BFloat16> {
          ...
          std::tie(a0, a1) = convert_bfloat16_float(a);
          auto res0 = a0 *
              (Vectorized<float>::blendv(negval_v, one_vec, a0 > zero_vec));
          auto res1 = a1 *
              (Vectorized<float>::blendv(negval_v, one_vec, a1 > zero_vec));
          return convert_float_bfloat16(res0, res1);
        });
  } else {
...
static void leaky_relu_backward_kernel(
    TensorIteratorBase& iter,
  ...
  if (iter.dtype() == kBFloat16) {
    ...
    cpu_kernel_vec(
        ...
        [beta, threshold](BFloat16 a) -> BFloat16 {
          return (float(a) * beta) > threshold
              ? a
              : static_cast<BFloat16>(
                    (std::log1p(std::exp(float(a) * beta))) / beta);
        },
        [beta_vec,
         threshold_vec](Vectorized<BFloat16> a) -> Vectorized<BFloat16> {
          Vectorized<float> a0, a1;
          std::tie(a0, a1) = convert_bfloat16_float(a);
          a0 = Vec::blendv(
              (a0 * beta_vec).exp().log1p() / beta_vec,
              a0,
              (a0 * beta_vec) > threshold_vec);
          a1 = Vec::blendv(
              (a1 * beta_vec).exp().log1p() / beta_vec,
              a1,
              (a1 * beta_vec) > threshold_vec);
          return convert_float_bfloat16(a0, a1);
        });
  } else {
  ...
  if (iter.dtype() == kBFloat16) {
    ...
    cpu_kernel_vec(
        ...
        [beta, threshold](BFloat16 a, BFloat16 b) -> BFloat16 {
          ...
          return (float(b) * beta) > threshold
              ? a
              : static_cast<BFloat16>(float(a) * z / (z + float(1.)));
        },
        [beta_vec, one_vec, threshold_vec](
            Vectorized<BFloat16> a,
            Vectorized<BFloat16> b) -> Vectorized<BFloat16> {
          Vectorized<float> a0, a1, b0, b1;
          std::tie(a0, a1) = convert_bfloat16_float(a);
          ...
          a0 = Vec::blendv(
              a0 * z / (z + one_vec), a0, (b0 * beta_vec) > threshold_vec);
          z = (b1 * beta_vec).exp();
          a1 = Vec::blendv(
              a1 * z / (z + one_vec), a1, (b1 * beta_vec) > threshold_vec);
          return convert_float_bfloat16(a0, a1);
        });
  } else {
    AT_DISPATCH_FLOATING_TYPES(iter.dtype(), "softplus_backward_cpu", [&]() {
      ...
      cpu_kernel_vec(
          ...
          [beta_vec, one_vec, threshold_vec](Vec a, Vec b) -> Vec {
            ...
            return Vec::blendv(
                a * z / (z + one_vec), a, (b * beta_vec) > threshold_vec);
          });
    });
  }
}
  ...
  if (iter.dtype() == kBFloat16) {
    ...
    cpu_kernel_vec(
        ...
        [float_one_val](BFloat16 a, BFloat16 b) -> BFloat16 {
          return float(a) *
              (float_one_val / (float_one_val + std::exp(-float(b))));
        },
        [float_one_vec](Vectorized<BFloat16> a, Vectorized<BFloat16> b)
            -> Vectorized<BFloat16> {
          Vectorized<float> a0, a1, b0, b1;
          std::tie(a0, a1) = convert_bfloat16_float(a);
          std::tie(b0, b1) = convert_bfloat16_float(b);
  ...
  if (iter.dtype() == kBFloat16) {
    ...
    cpu_kernel_vec(
        ...
        [float_one_vec](
            ...
            Vectorized<BFloat16> c) -> Vectorized<BFloat16> {
          Vectorized<float> a0, a1, b0, b1, c0, c1;
          std::tie(a0, a1) = convert_bfloat16_float(a);
          std::tie(b0, b1) = convert_bfloat16_float(b);
          std::tie(c0, c1) = convert_bfloat16_float(c);
...
void silu_kernel(TensorIteratorBase& iter) {
  AT_DISPATCH_FLOATING_AND_COMPLEX_TYPES_AND1(
      kBFloat16, iter.dtype(), "silu_cpu", [&]() {
        ...
        cpu_kernel_vec(
            iter,
            [](scalar_t x) { return x / (scalar_t(1) + std::exp(-x)); },
...
void mish_kernel(TensorIteratorBase& iter) {
  AT_DISPATCH_FLOATING_TYPES(iter.dtype(), "mish_cpu", [&]() {
    ...
    cpu_kernel_vec(
        ...
        [](Vec x_vec) -> Vec { return x_vec * x_vec.exp().log1p().tanh(); });
  });
}
  ...
  AT_DISPATCH_FLOATING_TYPES(iter.dtype(), "mish_backward_cpu", [&]() {
    ...
    cpu_kernel_vec(
        ...
        [](scalar_t dy, scalar_t x) -> scalar_t {
          const scalar_t sigmoid = scalar_t(1) / (scalar_t(1) + std::exp(-x));
          const scalar_t tanh_softplus = std::tanh(std::log1p(std::exp(x)));
          return dy *
              (tanh_softplus +
               x * sigmoid * (scalar_t(1) - tanh_softplus * tanh_softplus));
        },
        [kOneVec](Vec dy_vec, Vec x_vec) -> Vec {
          ...
          return dy_vec *
              (tanh_softplus +
               x_vec * sigmoid * (kOneVec - tanh_softplus * tanh_softplus));
        });
  });
}
  ...
  if (iter.common_dtype() == kBFloat16) {
    ...
    cpu_kernel_vec(
        ...
        [=](BFloat16 input, BFloat16 weight) -> BFloat16 {
          return (float(input) > float(0)) ? float(input)
                                           : float(weight) * float(input);
        },
        [=](Vectorized<BFloat16> input,
            Vectorized<BFloat16> weight) -> Vectorized<BFloat16> {
          ...
          auto res0 = input0 *
              (Vectorized<float>::blendv(weight0, one_vec, input0 > zero_vec));
          auto res1 = input1 *
              (Vectorized<float>::blendv(weight1, one_vec, input1 > zero_vec));
          return convert_float_bfloat16(res0, res1);
        });
  } else {
...
void prelu_backward_cpu_kernel(TensorIterator& iter) {
  if (iter.common_dtype() == kBFloat16) {
    cpu_kernel_multiple_outputs(
        ...
        [=](BFloat16 input,
            BFloat16 grad_out,
            BFloat16 weight) -> std::tuple<BFloat16, BFloat16> {
          float input_grad = (float(input) > float(0))
              ? float(grad_out)
              : float(weight) * float(grad_out);
          float weight_grad_collector = (float(input) > float(0))
              ? float(0)
              : float(input) * float(grad_out);
          return std::tuple<BFloat16, BFloat16>(
              input_grad, weight_grad_collector);
        });
  } else {
    AT_DISPATCH_FLOATING_TYPES(iter.dtype(), "prelu_backward_cpu", [&]() {
      cpu_kernel_multiple_outputs(
          ...
          [=](scalar_t input,
              scalar_t grad_out,
              scalar_t weight) -> std::tuple<scalar_t, scalar_t> {
            scalar_t input_grad =
                (input > scalar_t(0)) ? grad_out : weight * grad_out;
            scalar_t weight_grad_collector =
                (input > scalar_t(0)) ? scalar_t(0) : input * grad_out;
