...
inline void reduce_all_impl_vec(
  ...
  scalar_t result = at::parallel_reduce(
      0,
      input_numel,
      internal::GRAIN_SIZE,
      ident_v,
...
static void min_all_kernel_impl(Tensor& result, const Tensor& input) {
  if (input.scalar_type() == ScalarType::Bool) {
    TensorIterator iter = TensorIteratorConfig().add_input(input).build();
    bool result_data = true;
    cpu_serial_kernel(
        iter, [&](const bool a) -> void { result_data = result_data && a; });
    ...
    AT_DISPATCH_ALL_TYPES_AND2(
        kHalf, kBFloat16, input.scalar_type(), "min_all", [&] {
          using Vec = Vectorized<vec_scalar_t<scalar_t>>;
...
static void max_all_kernel_impl(Tensor& result, const Tensor& input) {
  if (input.scalar_type() == ScalarType::Bool) {
    ...
    AT_DISPATCH_ALL_TYPES_AND2(
        kHalf, kBFloat16, input.scalar_type(), "max_all", [&] {
          using Vec = Vectorized<vec_scalar_t<scalar_t>>;
          reduce_all_impl_vec<scalar_t>(
              result,
              input,
...
inline void reduce_all_impl_two_outputs(
  ...
  const int64_t input_numel = input.numel();
  auto input_data = input.data_ptr<scalar_t>();
  scalar_t_pair result = at::parallel_reduce(
      0,
      input_numel,
      internal::GRAIN_SIZE,
      ident_v,
      [&](int64_t start,
          int64_t end,
          const scalar_t_pair& ident) -> scalar_t_pair {
...
template <
    typename scalar_t,
    typename func_t,
    typename vec_func_t1,
    typename vec_func_t2>
inline void reduce_all_impl_vec_two_outputs(
  ...
  auto input_data = input.data_ptr<scalar_t>();
  // NOTE: parallel_reduce not support bool type
  std::pair<scalar_t, scalar_t> result = at::parallel_reduce(
      0,
      input_numel,
      internal::GRAIN_SIZE,
      ident_v,
...
static void aminmax_allreduce_kernel(
  ...
  if (input.scalar_type() == ScalarType::Bool) {
    ...
    reduce_all_impl_two_outputs<int64_t>(
        min_result,
        max_result,
        input,
    ...
    AT_DISPATCH_ALL_TYPES_AND(
        kBFloat16, input.scalar_type(), "aminmax_cpu", [&] {
          using Vec = Vectorized<vec_scalar_t<scalar_t>>;
          using scalar_t_pair = std::pair<scalar_t, scalar_t>;
