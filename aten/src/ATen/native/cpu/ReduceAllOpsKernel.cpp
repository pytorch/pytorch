...
static void min_and_max_all_kernel_impl(Tensor& min_result, Tensor& max_result,
    const Tensor& input) {
  if (input.scalar_type() == ScalarType::Bool) {
    TensorIterator iter = TensorIteratorConfig()
      .add_input(input)
    ...
    AT_DISPATCH_ALL_TYPES_AND_COMPLEX(input.scalar_type(), "min_and_max_all", [&] {
      using Vec = vec256::Vec256<scalar_t>;
      using scalar_t_pair = std::pair<scalar_t, scalar_t>;
...
REGISTER_DISPATCH(min_and_max_all_stub, &min_and_max_all_kernel_impl);
