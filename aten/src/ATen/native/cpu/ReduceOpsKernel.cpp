...
static inline void cpu_cum_base_kernel(
  ...
  int64_t grain_size =
      internal::GRAIN_SIZE / std::max(int64_t{1}, self.size(dim));
...
static void cumsum_cpu_kernel(
  ...
  AT_DISPATCH_ALL_TYPES_AND_COMPLEX_AND2(
      kBFloat16, kHalf, self.scalar_type(), "cumsum_out_cpu", [&] {
...
static void cumprod_cpu_kernel(
  ...
  AT_DISPATCH_ALL_TYPES_AND_COMPLEX_AND2(
      kBFloat16, kHalf, self.scalar_type(), "cumprod_out_cpu", [&] {
        cpu_cum_base_kernel<scalar_t>(
            result,
            self,
            wrap_dim,
...
static void logcumsumexp_cpu_kernel(
  ...
  AT_DISPATCH_FLOATING_AND_COMPLEX_TYPES_AND2(
      kBFloat16, kHalf, self.scalar_type(), "logcumsumexp_out_cpu", [&] {
        cpu_cum_base_kernel<scalar_t>(
            result,
            self,
            wrap_dim,
...
static void prod_kernel_impl(TensorIterator& iter) {
  ...
  if (iter.dtype() == ScalarType::Bool) {
    ...
    AT_DISPATCH_ALL_TYPES_AND_COMPLEX_AND2(
        kBFloat16, kHalf, iter.dtype(), "prod_out_cpu", [&] {
          binary_kernel_reduce_vec(
              iter,
              [=](scalar_t a, scalar_t b) -> scalar_t { return a * b; },
...
static void norm_kernel_tensor_iterator_impl(
  ...
  if (val == 2.0 && is_reduce_lastdim(iter) &&
    ...
    AT_DISPATCH_FLOATING_AND_COMPLEX_TYPES_AND3(
        kHalf, kBFloat16, kComplexHalf, iter.input_dtype(), "norm_cpu", [&] {
  ...
  if (iter.dtype() == ScalarType::Byte) {
    ...
    binary_kernel_reduce_vec(
        ...
        [=](Vectorized<uint8_t> a, Vectorized<uint8_t> b) {
          ...
          // the desired result
          return (a != Vectorized<uint8_t>(0)).neg() &
              (b != Vectorized<uint8_t>(0)).neg();
        },
        /*ident=*/true);
  } else {
    binary_kernel_reduce_vec(
