...
static void cumsum_cpu_kernel(
  ...
  AT_DISPATCH_ALL_TYPES_AND_COMPLEX_AND(
      kBFloat16, self.scalar_type(), "cumsum_out_cpu", [&] {
...
static void cumprod_cpu_kernel(
  ...
  AT_DISPATCH_ALL_TYPES_AND_COMPLEX_AND(
      kBFloat16, self.scalar_type(), "cumprod_out_cpu", [&] {
        cpu_cum_base_kernel<scalar_t>(
            result,
            self,
            wrap_dim,
...
static void logcumsumexp_cpu_kernel(
    Tensor& result,
    const Tensor& self,
    int64_t dim) {
  auto wrap_dim = maybe_wrap_dim(dim, self.dim());
  ...
  AT_DISPATCH_FLOATING_TYPES_AND(
      kBFloat16, self.scalar_type(), "logcumsumexp_out_cpu", [&] {
        cpu_cum_base_kernel<scalar_t>(
            result,
            self,
            wrap_dim,
            [&](scalar_t* result_data,
              ...
              for (const auto i : c10::irange(self_dim_size)) {
                ...
                auto log_add_exp = [](accscalar_t x,
                                      accscalar_t y) -> accscalar_t {
                  accscalar_t min = std::isnan(y)
                      ? y
                      : std::min(x, y); // std::min returns first arg if one of
                                        // the args is nan
                  accscalar_t max = std::isnan(y)
                      ? y
                      : std::max(x, y); // std::max returns first arg if one of
                                        // the args is nan
                ...
                result_data[i * result_dim_stride] =
                    static_cast<scalar_t>(cum_number);
              }
            },
...
static void std_var_kernel_impl(
  ...
  AT_DISPATCH_FLOATING_TYPES_AND2(
      kHalf, kBFloat16, iter.dtype(), "std_cpu", [&] {
        binary_kernel_reduce(
            iter,
            WelfordOps<
                scalar_t,
...
static void norm_kernel_tensor_iterator_impl(
  ...
  bool use_fast_path = is_reduce_lastdim(iter) &&
  ...
  if (val == 0) {
    AT_DISPATCH_FLOATING_AND_COMPLEX_TYPES_AND2(
        kHalf, kBFloat16, iter.input_dtype(), "norm_cpu", [&] {
          ...
          binary_kernel_reduce(iter, NormZeroOps<scalar_t, acc_t>(), acc_t(0));
        });
  } else if (val == 1) {
    AT_DISPATCH_FLOATING_AND_COMPLEX_TYPES_AND2(
        kHalf, kBFloat16, iter.input_dtype(), "norm_cpu", [&] {
          using acc_t = typename scalar_value_type<scalar_t>::type;
          binary_kernel_reduce(iter, NormOneOps<scalar_t, acc_t>(), acc_t(0));
        });
  } else if (val == 2) {
    if (use_fast_path) {
      AT_DISPATCH_FLOATING_TYPES_AND(
          kBFloat16, iter.input_dtype(), "norm_cpu", [&] {
            ...
            binary_kernel_reduce_lastdim(
                iter,
                [](char* result_data_bytes,
                   char* self_data_bytes,
                   int64_t size) {
    ...
    AT_DISPATCH_FLOATING_AND_COMPLEX_TYPES_AND2(
        kHalf, kBFloat16, iter.input_dtype(), "norm_cpu", [&] {
          using acc_t = typename scalar_value_type<scalar_t>::type;
          binary_kernel_reduce(iter, NormTwoOps<scalar_t, acc_t>(), acc_t(0));
        });
  } else if (val == INFINITY) {
    AT_DISPATCH_FLOATING_AND_COMPLEX_TYPES_AND2(
        kHalf, kBFloat16, iter.input_dtype(), "norm_cpu", [&] {
          using acc_t = typename scalar_value_type<scalar_t>::type;
          binary_kernel_reduce(iter, AbsMaxOps<scalar_t, acc_t>(), acc_t(0));
        });
  } else if (val == -INFINITY) {
    AT_DISPATCH_FLOATING_AND_COMPLEX_TYPES_AND2(
        kHalf, kBFloat16, iter.input_dtype(), "norm_cpu", [&] {
    ...
    AT_DISPATCH_FLOATING_AND_COMPLEX_TYPES_AND2(
        kHalf, kBFloat16, iter.input_dtype(), "norm_cpu", [&] {
          using acc_t = typename scalar_value_type<scalar_t>::type;
          binary_kernel_reduce(
              iter, NormOps<scalar_t, acc_t>{acc_t(val)}, acc_t(0));
        });
  }
...
static void and_kernel_impl(TensorIterator& iter) {
  if (iter.dtype() == ScalarType::Byte) {
    ...
    binary_kernel_reduce_vec(
        ...
        [=](Vectorized<uint8_t> a, Vectorized<uint8_t> b) { return a & b; },
#else
        [=](Vectorized<uint8_t> a, Vectorized<uint8_t> b) {
          Vectorized<uint8_t> c = Vectorized<uint8_t>();

          for (decltype(c.size()) i = 0; i != Vectorized<uint8_t>::size();
               i++) {
            c[i] = (a[i] && b[i]) ? 1 : 0;
          }
          return c;
...
static void min_values_kernel_impl(TensorIterator& iter) {
  ...
  AT_DISPATCH_ALL_TYPES_AND3(
      kBFloat16, kHalf, kBool, iter.dtype(), "min_values_cpu", [&iter] {
        binary_kernel_reduce_vec(
            iter,
...
static void max_values_kernel_impl(TensorIterator& iter) {
  AT_DISPATCH_ALL_TYPES_AND3(
      kBFloat16, kHalf, kBool, iter.dtype(), "max_values_cpu", [&iter] {
        binary_kernel_reduce_vec(
            iter,
...
static void argmax_kernel_impl(TensorIterator& iter) {
  AT_DISPATCH_ALL_TYPES_AND2(
      kHalf, kBFloat16, iter.dtype(1), "argmax_cpu", [&] {
...
static void argmin_kernel_impl(TensorIterator& iter) {
  AT_DISPATCH_ALL_TYPES_AND2(
      kHalf, kBFloat16, iter.dtype(1), "argmin_cpu", [&] {
        if (is_reduce_lastdim(iter)) {
          using arg_t = std::pair<scalar_t, int64_t>;
