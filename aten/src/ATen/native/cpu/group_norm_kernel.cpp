      ...
      if (!gamma_data && !beta_data) {
        ...
        for (const auto j : c10::irange(D)) {
          ...
          const opmath_t scale =
              rstd_val * (gamma_data ? opmath_t(gamma_data[c]) : opmath_t(1));
          const opmath_t bias = -scale * mean_val +
              (beta_data ? opmath_t(beta_data[c]) : opmath_t(0));
...
template <typename T>
std::enable_if_t<std::is_same_v<T, at::opmath_type<T>>, std::tuple<T, T>>
ColumnwiseMoments(const T* X_data, int64_t HxW, int64_t C, int64_t D) {
  ...
  T mean_val =
      vec::vec_reduce_all([](Vec& x, Vec& y) { return x + y; }, acc0_vec);
  T rstd_val =
      vec::vec_reduce_all([](Vec& x, Vec& y) { return x + y; }, acc1_vec);
...
template <typename T>
std::enable_if_t<
    !std::is_same_v<T, at::opmath_type<T>>,
...
ColumnwiseMoments(const T* X_data, int64_t HxW, int64_t C, int64_t D) {
  ...
  opmath_t mean_val =
      vec::vec_reduce_all([](fVec& x, fVec& y) { return x + y; }, acc0_fvec);
  opmath_t rstd_val =
      vec::vec_reduce_all([](fVec& x, fVec& y) { return x + y; }, acc1_fvec);
  return std::tuple<opmath_t, opmath_t>(mean_val, rstd_val);
}

template <typename T, typename opmath_t>
inline std::enable_if_t<std::is_same_v<T, opmath_t>, void> CalcMeanVar(
    const T* X_ptr,
    opmath_t* mean_ptr,
    opmath_t* rstd_ptr,
    int64_t C) {
...
template <typename T, typename opmath_t>
inline std::enable_if_t<std::is_same_v<T, opmath_t>, void> ApplyScaleBias(
    T* Y_ptr,
    const T* X_ptr,
...
void GroupNormKernelImplChannelsLastInternal(
  ...
  if (HxW < feature_map_threshold) {
    ...
    Tensor buffer = at::zeros(
        {num_threads, N, 2 * C},
        X.options().dtype(c10::CppTypeToScalarType<opmath_t>::value));
    opmath_t* buffer_data = buffer.data_ptr<opmath_t>();
    Tensor tmp_buffer = at::empty(
        {N, 2 * G},
        X.options().dtype(c10::CppTypeToScalarType<opmath_t>::value));
    ...
    // configs, e.g. say input shape is {1, 32, h, w} and G = 8, this will give
    // D = 4 which is unable to take full SIMD length.
    //
...
template <typename T, typename opmath_t>
std::enable_if_t<std::is_same_v<T, opmath_t>, void> ComputeInternalGradients(
    int64_t N,
    int64_t C,
    int64_t HxW,
    const T* dY,
...
std::enable_if_t<!std::is_same_v<T, opmath_t>, void> ComputeInternalGradients(
  ...
  at::parallel_for(0, N * C, 1, [=](int64_t start, int64_t end) {
    ...
    for (const auto i : c10::irange(start, end)) {
      ...
      opmath_t ds_val =
          std::accumulate(ds_arr.cbegin(), ds_arr.cend(), opmath_t(0));
      opmath_t db_val =
          std::accumulate(db_arr.cbegin(), db_arr.cend(), opmath_t(0));
...
inline std::enable_if_t<std::is_same_v<PT, opmath_t>, void> CalcDsDb(
  ...
  at::parallel_for(0, N * G, 1, [=](int64_t start, int64_t end) {
    ...
    for (const auto i : c10::irange(start, end)) {
      ...
      for (const auto j : c10::irange(d, D)) {
        const opmath_t gamma_v =
            gamma ? opmath_t(gamma[g * D + j]) : opmath_t(1);
        ds_val += ds_ptr[j] * gamma_v;
        db_val += db_ptr[j] * gamma_v;
      }
      const opmath_t c2 = (db_val * opmath_t(mean[i]) - ds_val) *
          opmath_t(rstd[i]) * opmath_t(rstd[i]) * opmath_t(rstd[i]) * s;
      const opmath_t c3 =
          -c2 * opmath_t(mean[i]) - db_val * opmath_t(rstd[i]) * s;
      ...
      for (const auto j : c10::irange(D)) {
        ...
        const opmath_t c1 =
            opmath_t(rstd[i]) * (gamma ? opmath_t(gamma[c]) : opmath_t(1));
        for (const auto k : c10::irange(HxW)) {
...
template <typename PT, typename opmath_t>
std::enable_if_t<std::is_same_v<PT, opmath_t>, void> GammaBackward(
    int64_t N,
    int64_t C,
    int64_t group,
    const PT* mean,
...
std::enable_if_t<!std::is_same_v<PT, opmath_t>, void> GammaBackward(
  ...
  for (const auto g : c10::irange(G)) {
    ...
    if (D - i > 0) {
      ...
      convert_from_float<PT>(acc0_vec, acc1_vec)
...
template <typename PT, typename opmath_t>
std::enable_if_t<std::is_same_v<PT, opmath_t>, void> BetaBackward(
    int64_t N,
    int64_t C,
    const opmath_t* db,
    PT* dbeta) {
...
void GroupNormBackwardKernelImplInternal(
  ...
  using opmath_t = at::opmath_type<T>;
  Tensor ds = at::empty(
      {N, C}, X.options().dtype(c10::CppTypeToScalarType<opmath_t>::value));
  Tensor db = at::empty(
      {N, C}, X.options().dtype(c10::CppTypeToScalarType<opmath_t>::value));
  opmath_t* ds_data = ds.data_ptr<opmath_t>();
  opmath_t* db_data = db.data_ptr<opmath_t>();
  ComputeInternalGradients<T, opmath_t>(
...
inline std::enable_if_t<
    std::is_same_v<T, at::opmath_type<T>>,
    std::tuple<vec::Vectorized<T>, vec::Vectorized<T>>>
...
ApplyInputGradientsChannelsLastColMov(
  ...
  for (; d < D / K * K; d += K) {
    ...
    for (const auto m : c10::irange(HxW)) {
      ...
      auto dx_vec =
          c1 * dy_vec + vec::Vectorized<T>(c2) * x_vec + vec::Vectorized<T>(c3);
      dx_vec.store(dX_ptr + d);
    }
  }
  if (D - d > 0) {
    auto c1 = vec::Vectorized<T>(*rstd) *
        (gamma ? vec::Vectorized<T>::loadu(gamma + d, D - d)
               : vec::Vectorized<T>(1));
    for (const auto m : c10::irange(HxW)) {
      const T* X_ptr = X_data + m * C;
      const T* dY_ptr = dY_data + m * C;
...
ApplyInputGradientsChannelsLastColMov(
  ...
  for (; d < D / K * K; d += K) {
    auto [c1_0, c1_1] =
        gamma ? load_util(gamma + d, K) : std::make_tuple(fVec(1), fVec(1));
  ...
  if (D - d > 0) {
    auto [c1_0, c1_1] =
        gamma ? load_util(gamma + d, D - d) : std::make_tuple(fVec(1), fVec(1));
    c1_0 = c1_0 * fVec(opmath_t(*rstd));
    c1_1 = c1_1 * fVec(opmath_t(*rstd));
    for (const auto m : c10::irange(HxW)) {
      const T* X_ptr = X_data + m * C;
...
inline std::
  ...
  for (; d < inner_size; d += K) {
    ...
    ds_gamma += vec::vec_reduce_all(
        [](Vec& x, Vec& y) { return x + y; }, acc0_vec * gamma_vec);
    db_gamma += vec::vec_reduce_all(
        [](Vec& x, Vec& y) { return x + y; }, acc1_vec * gamma_vec);
  }
  if (D - d > 0) {
    Vec acc0_vec{0}, acc1_vec{0};
    for (const auto m : c10::irange(HxW)) {
  ...
  if (HxW < feature_map_threshold) {
    ...
    at::parallel_for(0, N * G, 1, [=](int64_t begin, int64_t end) {
      ...
      for (const auto i : c10::irange(begin, end)) {
        ...
        auto [ds_gamma, db_gamma] =
            CalcInternalGradientsChannelsLast<T, PT, opmath_t>(
        ...
        if (dX_data) {
          ...
          const opmath_t c2 = (db_gamma * opmath_t(mean_data[i]) - ds_gamma) *
              opmath_t(rstd_data[i]) * opmath_t(rstd_data[i]) *
              opmath_t(rstd_data[i]) * s;
          const opmath_t c3 = -c2 * opmath_t(mean_data[i]) -
              db_gamma * opmath_t(rstd_data[i]) * s;
          ApplyInputGradientsChannelsLastColMov<T, PT, opmath_t>(
    ...
    at::parallel_for(0, N * HxW, 1, [&](int64_t begin, int64_t end) {
      ...
      for (const auto i : c10::irange(begin, end)) {
        ...
        DsDbRowwiseMomentsChannelsLast<T, opmath_t>(
      ...
      for (const auto g : c10::irange(G)) {
        ...
        for (const auto d : c10::irange(D)) {
          ...
          for (const auto t : c10::irange(num_threads)) {
            opmath_t* buffer_ptr = buffer_data + t * N * 2 * C + n * 2 * C;
            opmath_t gamma_val =
                gamma_data ? opmath_t(gamma_data[g * D + d]) : opmath_t(1);
        ...
        for (const auto i : c10::irange(begin, end)) {
          for (const auto g : c10::irange(G)) {
            ...
            const opmath_t c2 = (db_val * opmath_t(*mean_ptr) - ds_val) *
                opmath_t(*rstd_ptr) * opmath_t(*rstd_ptr) *
                opmath_t(*rstd_ptr) * s;
            const opmath_t c3 =
                -c2 * opmath_t(*mean_ptr) - db_val * opmath_t(*rstd_ptr) * s;
            ApplyInputGradientsChannelsLastRowMov<T, PT, opmath_t>(
                dY_ptr, X_ptr, dX_ptr, rstd_ptr, gamma_ptr, c2, c3, HxW, C, D);
          }
