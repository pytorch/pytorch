...
#pragma unroll
  ...
  inline void backward(
      TensorAccessor<scalar_t, 3>* gInp_slice_ptr,
      TensorAccessor<scalar_t, 3>& gGrid_slice,
      const TensorAccessor<scalar_t, 3>& gOut_slice,
      const TensorAccessor<scalar_t, 3>& inp_slice,
      int64_t offset,
...
#endif
    for (int64_t c = 0; c < C;
         ++c, out_ptr += out_sC, inp_slice_ptr += inp_sC) {
...
struct ApplyGridSample<
  ...
  // NOLINTNEXTLINE(modernize-avoid-c-arrays,cppcoreguidelines-avoid-c-arrays)
  inline void get_cubic_coefficients_grad(Vec (&coeffs)[4], const Vec& tx)
      const {
    Vec x;
  ...
  inline Vec get_value_bounded(const scalar_t* data, const Vec& x, const Vec& y)
      const {
    ...
    auto mask_x =
        must_in_bound ? iVec(-1) : (ix > iVec(-1)) & (ix < iVec(inp_W));
    auto mask_y =
        must_in_bound ? iVec(-1) : (iy > iVec(-1)) & (iy < iVec(inp_H));
...
#pragma unroll
    ...
    for (const auto c : c10::irange(C)) {
      ...
      for (const auto i : c10::irange(4)) {
        interp_x[i] = coeff_x[0] *
                get_value_bounded(
                          inp_slice_C_ptr, ix - Vec(1), iy + Vec(-1 + i)) +
            coeff_x[1] *
                get_value_bounded(
                    inp_slice_C_ptr, ix + Vec(0), iy + Vec(-1 + i)) +
            coeff_x[2] *
                get_value_bounded(
                    inp_slice_C_ptr, ix + Vec(1), iy + Vec(-1 + i)) +
            coeff_x[3] *
                get_value_bounded(
                    inp_slice_C_ptr, ix + Vec(2), iy + Vec(-1 + i));
      }
      ...
      for (const auto i : c10::irange(4)) {
        for (const auto j : c10::irange(4)) {
          ...
          if (input_requires_grad) {
            auto gInp_slice_C_ptr = (*gInp_slice_ptr)[c].data();
            add_value_bounded(
                gInp_slice_C_ptr, len, xx, yy, gOut * coeff_x[i] * coeff_y[j]);
          }
...
#pragma unroll
      ...
      for (int64_t w = 0; w < out_W; w += step) {
        ...
        apply_fn(
            vec::gather<sizeof(scalar_t)>(grid_ptr_x, i_offsets),
...
void grid_sampler_2d_backward_cpu_kernel_impl(
  ...
  // If `input` gradient is not required, we skip computing it -- not needing to
  // create the tensor to hold the gradient can markedly increase performance.
  // (`grid` gradient is always computed.)
...
#define GINP_SLICE_PTR_true      \
  auto gInp_slice = gInp_acc[n]; \
  auto gInp_slice_ptr = &gInp_slice;
#define GINP_SLICE_PTR_false \
  TensorAccessor<scalar_t, 3>* gInp_slice_ptr = nullptr;
      ...
      for (const auto n : c10::irange(begin, end)) {                          \
        ...
        grid_sample_2d_grid_slice_iterator(                                   \
            ...
            [&](const Vectorized<scalar_t>& grid_x,                           \
                ...
                int64_t spatial_offset,                                       \
                int64_t len) {                                                \
              grid_sample.backward<input_requires_grad>(                      \
                  gInp_slice_ptr,                                             \
                  gGrid_slice,                                                \
                  gOut_slice,                                                 \
                  inp_slice,                                                  \
...
#define HANDLE_INTERP(interp, align_corners, input_requires_grad) \
  case interp: {                                                  \
    switch (static_cast<GridSamplerPadding>(padding_mode)) {      \
      ...
      HANDLE_CASE(                                                \
          interp,                                                 \
          GridSamplerPadding::Border,                             \
          align_corners,                                          \
          input_requires_grad);                                   \
      HANDLE_CASE(                                                \
          interp,                                                 \
          GridSamplerPadding::Reflection,                         \
          align_corners,                                          \
          input_requires_grad);                                   \
    }                                                             \
    return;                                                       \
