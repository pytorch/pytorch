...
static void arange_kernel(
    TensorIterator& iter,
    const Scalar& scalar_start,
    const Scalar& scalar_steps,
    const Scalar& scalar_step) {
  AT_DISPATCH_ALL_TYPES_AND(kBFloat16, iter.dtype(), "arange_cpu", [&]() {
    ...
    at::parallel_for(
        0, steps, internal::GRAIN_SIZE, [&](int64_t p_begin, int64_t p_end) {
          int64_t idx(p_begin);
...
static void linspace_kernel(
    TensorIterator& iter,
    const Scalar& scalar_start,
    const Scalar& scalar_end,
    int64_t steps) {
  AT_DISPATCH_ALL_TYPES_AND_COMPLEX_AND(
      kBFloat16, iter.dtype(), "linspace_cpu", [&]() {
        // step should be of double type for all integral types
        using step_t = std::
            conditional_t<std::is_integral<scalar_t>::value, double, scalar_t>;
        const scalar_t start = scalar_start.to<scalar_t>();
        const scalar_t end = scalar_end.to<scalar_t>();
        // Cast `end` and `start` to `step_t`, since range can be larger than
        // scalar_t for integral types
        const step_t step =
            (static_cast<step_t>(end) - static_cast<step_t>(start)) /
            (steps - 1);
        ...
        at::parallel_for(
            ...
            [&](int64_t p_begin, int64_t p_end) {
              ...
              cpu_serial_kernel_vec(
                  ...
                  [start, end, step, halfway, steps, &idx]()
                      -> Vectorized<scalar_t> {
                    Vectorized<scalar_t> res;
                    if (idx < halfway) {
