...
void random_full_64_bits_range_kernel(TensorIteratorBase& iter, RNG generator) {
  AT_DISPATCH_ALL_TYPES_AND(
      at::ScalarType::BFloat16,
      iter.dtype(),
      "random_full_64_bits_range_kernel_cpu",
      [&] {
        std::lock_guard<std::mutex> lock(generator->mutex_);
        if (std::is_same<scalar_t, int64_t>::value ||
          ...
          TORCH_CHECK(
              false,
              "random_full_64_bits_range_kernel_cpu handles only int64, double, float and bfloat16");
        }
      });
}
...
#ifdef CPU_CAPABILITY_AVX2
static void normal_fill_16_AVX2(
    float* data,
    const __m256* two_pi,
    const __m256* one,
  ...
  const __m256 radius =
      _mm256_sqrt_ps(_mm256_mul_ps(*minus_two, log256_ps(u1)));
...
void normal_kernel(Tensor& self, double mean, double std, RNG generator) {
  auto size = self.numel();
  if (self.scalar_type() == ScalarType::Float && size >= 16 &&
      self.is_contiguous()) {
#ifdef CPU_CAPABILITY_AVX2
    normal_fill_AVX2(
        self, static_cast<float>(mean), static_cast<float>(std), generator);
#else
    normal_fill(
        self, static_cast<float>(mean), static_cast<float>(std), generator);
#endif
    ...
    AT_DISPATCH_FLOATING_TYPES_AND2(
        kHalf, kBFloat16, self.scalar_type(), "normal_kernel_cpu", [&] {
...
void uniform_kernel(
    ...
    RNG generator) {
  AT_DISPATCH_FLOATING_TYPES_AND2(
      kHalf, kBFloat16, iter.dtype(), "uniform_kernel_cpu", [&]() {
        std::lock_guard<std::mutex> lock(generator->mutex_);
...
void cauchy_kernel(
    ...
    RNG generator) {
  AT_DISPATCH_FLOATING_TYPES_AND2(
      kHalf, kBFloat16, iter.dtype(), "cauchy_cpu", [&]() {
        std::lock_guard<std::mutex> lock(generator->mutex_);
      ...
      [&] {
        ...
        if (p_.scalar_type() == kDouble) {
          ...
          AT_DISPATCH_FLOATING_TYPES(
              p_.scalar_type(), "bernoulli_tensor_cpu_p_", [&] {
                using p_t = scalar_t;
...
void bernoulli_kernel(Tensor& self, double p, RNG generator) {
  AT_DISPATCH_ALL_TYPES_AND(
      at::ScalarType::Bool, self.scalar_type(), "bernoulli_scalar_cpu_", [&] {
        // See Note [Acquire lock when using random generators]
        std::lock_guard<std::mutex> lock(generator->mutex_);
        auto iter = TensorIterator::nullary_op(self);
