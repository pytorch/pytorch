...
#include <ATen/native/special/ln_gamma.h>
...
void ln_gamma_cpu_kernel(TensorIteratorBase &iterator) {
  AT_DISPATCH_FLOATING_AND_COMPLEX_TYPES(iterator.common_dtype(), "ln_gamma_cpu_kernel", [&]() {
    cpu_kernel(iterator, [](scalar_t z) {
      return at::native::special::ln_gamma(z);
    });
  });
} // void ln_gamma_cpu_kernel(TensorIteratorBase &iterator)
