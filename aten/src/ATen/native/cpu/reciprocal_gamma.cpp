...
#include <ATen/native/special/reciprocal_gamma.h>
...
void reciprocal_gamma_cpu_kernel(TensorIteratorBase &iterator) {
  AT_DISPATCH_FLOATING_AND_COMPLEX_TYPES(iterator.common_dtype(), "reciprocal_gamma_cpu_kernel", [&]() {
    cpu_kernel(iterator, [](scalar_t z) {
      return at::native::special::reciprocal_gamma(z);
    });
  });
} // void reciprocal_gamma_cpu_kernel(TensorIteratorBase &iterator)
