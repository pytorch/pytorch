...
namespace at::native {
// NOCOMMIT: try moving everything into the .cpp file to establish whether it's the Extract Function refactor or something about the includes
...
template <typename scalar_t>
std::pair<int64_t,int64_t> vec_logsoftmax_chunk_size_and_num_chunks(int64_t grain_size, int64_t inner_size, int64_t dim_size) {
...
// NOCOMMIT: end stuff moved from header
namespace {
template <typename scalar_t>
...
_vec_logsoftmax(
  ...
  const auto [CHUNK_SIZE_binding, num_chunks_binding] = vec_logsoftmax_chunk_size_and_num_chunks<scalar_t>(
      at::internal::GRAIN_SIZE, inner_size, dim_size);
