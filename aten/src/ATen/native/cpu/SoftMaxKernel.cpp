...
inline void _vec_softmax_lastdim(
  ...
  int64_t grain_size =
      std::max(internal::GRAIN_SIZE / (16 * dim_size), (int64_t)1);
  parallel_for(0, outer_size, grain_size, [&](int64_t begin, int64_t end) {
...
inline void _vec_softmax_lastdim<BFloat16>(
  ...
  parallel_for(0, outer_size, grain_size, [&](int64_t begin, int64_t end) {
    ...
    for (const auto i : c10::irange(begin, end)) {
      ...
      float max_val = vec::vec_reduce_all(
          [](fVec& x, fVec& y) { return vec::maximum(x, y); }, max_fvec);
      for (; d0 < dim_size; d0++) {
      ...
      float sum_val =
          vec::vec_reduce_all([](fVec& x, fVec& y) { return x + y; }, sum_fvec);
      for (; d1 < dim_size; d1++) {
      ...
      for (; d2 < dim_size - (dim_size % bVec::size()); d2 += bVec::size()) {
        ...
        fVec out_fvec1 =
            fVec::loadu(buffer_data + d2 + fVec::size()) * fVec(sum_val);
...
inline void _vec_softmax(
  ...
  int vectorized_step =
      Vec_bf16().size(); // Currently, we only support BFloat16 in this special
                         // implementation
  parallel_for(
      0, outer_size * inner_size, grain_size, [&](int64_t begin, int64_t end) {
        int64_t idx = begin;
        std::unique_ptr<float[]> temp_vec_input(
        ...
        while (idx < end) {
          ...
          if (((inner_idx + vectorized_step) <= inner_size) &&
            ...
            std::tuple<vec::Vectorized<float>, vec::Vectorized<float>>
                convert_result = convert_bfloat16_float(max_vec_bf16);
            ...
            std::get<1>(convert_result)
                .store(temp_vec_input_data + vectorized_step);
            for (const auto d : c10::irange(1, dim_size)) {
              Vec_bf16 input_vec_bf16 =
                  Vec_bf16::loadu(input_data + d * dim_stride);
              convert_result = convert_bfloat16_float(input_vec_bf16);
              max_vec_o1 =
                  vec::maximum(max_vec_o1, std::get<0>(convert_result));
              max_vec_o2 =
                  vec::maximum(max_vec_o2, std::get<1>(convert_result));
              std::get<0>(convert_result)
                  .store(temp_vec_input_data + d * vectorized_step * 2);
              std::get<1>(convert_result)
                  .store(
                      temp_vec_input_data + d * vectorized_step * 2 +
                      vectorized_step);
            }
            // Step2: Calculate sum
            ...
            for (const auto d : c10::irange(dim_size)) {
              Vec output_vec_o1 =
                  Vec::loadu(temp_vec_input_data + d * vectorized_step * 2);
              Vec output_vec_o2 = Vec::loadu(
                  temp_vec_input_data + d * vectorized_step * 2 +
                  vectorized_step);
            ...
            for (const auto d : c10::irange(dim_size)) {
              Vec output_vec_o1 =
                  Vec::loadu(temp_vec_output_data + d * vectorized_step * 2);
              Vec output_vec_o2 = Vec::loadu(
                  temp_vec_output_data + d * vectorized_step * 2 +
                  vectorized_step);
              output_vec_o1 = output_vec_o1 / sum_vec_o1;
              output_vec_o2 = output_vec_o2 / sum_vec_o2;
              Vec_bf16 output_vec_bf16 =
                  convert_float_bfloat16(output_vec_o1, output_vec_o2);
            ...
            // Case 1: For the idx at the end of total chunk for each thread,
            // there are not enough numbers for parallization. Case 2: For the
            // idx at the end of each inner_size inside thread, there are not
            // enough numbers for parallization.
            ...
            for (const auto i : c10::irange(tail_number)) {
              ...
              for (const auto d : c10::irange(1, dim_size)) {
                max_input =
                    std::max(max_input, float(input_data[d * dim_stride]));
              }
              ...
              // Step3: Unify
              for (const auto d : c10::irange(dim_size)) {
                output_data[d * dim_stride] = c10::BFloat16(
                    float(output_data[d * dim_stride]) / sum_data);
              }
            }
            idx += tail_number;
          }
...
//
// Parallel on {outer_size, num_chunks} and do vertical reduction on each block
// of {dim_size, CHUNK_SIZE}, block size (128KB) selected to be L2 hit.
//
template <typename scalar_t>
inline void _vec_logsoftmax(
  ...
  at::parallel_for(
      0, outer_size * num_chunks, grain_size, [&](int64_t begin, int64_t end) {
        // thread local temp buffer which holds vertical reduction result: max
        // and sum.
        ...
        for (int64_t i = begin; i < end; i++) {
          ...
          for (int64_t dim_idx = 0; dim_idx < dim_size; dim_idx++) {
            scalar_t* input_ptr = input_data_base +
                outer_idx * dim_size * inner_size + dim_idx * inner_size +
                inner_idx_begin;
          ...
          // apply log
          vec::map(
              [](Vec x) { return x.log(); }, tmp_sum_data, tmp_sum_data, size);

          // compute x - max - sum
          for (int64_t dim_idx = 0; dim_idx < dim_size; dim_idx++) {
            int64_t offset = outer_idx * dim_size * inner_size +
            ...
            for (; d3 < size; d3++) {
              output_ptr[d3] =
                  input_ptr[d3] - input_max_data[d3] - tmp_sum_data[d3];
            }
          }
        }
      });
...
inline void _vec_logsoftmax<BFloat16>(
  ...
  at::parallel_for(
      0, outer_size * num_chunks, grain_size, [&](int64_t begin, int64_t end) {
        ...
        // thread local buffer that holds input data in float32 to save next 2
        // dtype conversion
        ...
        for (int64_t i = begin; i < end; i++) {
          ...
          // compute max
          for (int64_t dim_idx = 0; dim_idx < dim_size; dim_idx++) {
            BFloat16* input_ptr = input_data_base +
                outer_idx * dim_size * inner_size + dim_idx * inner_size +
                inner_idx_begin;
            ...
            for (; d1 < size - (size % bVec::size()); d1 += bVec::size()) {
              ...
              max_fvec0 =
                  fVec::blendv(max_fvec0, data_fvec0, data_fvec0 > max_fvec0);
              max_fvec1 =
                  fVec::blendv(max_fvec1, data_fvec1, data_fvec1 > max_fvec1);
          ...
          for (int64_t dim_idx = 0; dim_idx < dim_size; dim_idx++) {
            ...
            for (; d2 < size - (size % bVec::size()); d2 += bVec::size()) {
              ...
              fVec data_fvec1 =
                  fVec::loadu(input_buffer_ptr + d2 + fVec::size());
          ...
          // apply log
          vec::map(
              [](fVec x) { return x.log(); }, tmp_sum_data, tmp_sum_data, size);

          // compute x - max - sum
          for (int64_t dim_idx = 0; dim_idx < dim_size; dim_idx++) {
            float* input_buffer_ptr = input_buffer_data + dim_idx * CHUNK_SIZE;
            BFloat16* output_ptr = output_data_base +
                outer_idx * dim_size * inner_size + dim_idx * inner_size +
                inner_idx_begin;
            ...
            for (; d3 < size - (size % bVec::size()); d3 += bVec::size()) {
              ...
              fVec data_fvec1 =
                  fVec::loadu(input_buffer_ptr + d3 + fVec::size());
            ...
            for (; d3 < size; d3++) {
              output_ptr[d3] = BFloat16(
                  input_buffer_ptr[d3] - input_max_data[d3] - tmp_sum_data[d3]);
            }
          }
        }
      });
...
static void softmax_kernel_impl(
    const Tensor& result,
    const Tensor& self,
    int64_t dim) {
  AT_DISPATCH_FLOATING_TYPES_AND(
      at::ScalarType::BFloat16, self.scalar_type(), "softmax_kernel_impl", [&] {
...
static void log_softmax_kernel_impl(
    const Tensor& result,
    const Tensor& self,
    int64_t dim) {
  AT_DISPATCH_FLOATING_TYPES_AND(
