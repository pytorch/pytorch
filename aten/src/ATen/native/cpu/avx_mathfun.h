...
#define _PI64_CONST512(Name, Val)                                                               \
  static const ALIGN64_BEG int _PI64_512_##Name[16] = { Val, Val, Val, Val, Val, Val, Val, Val, \
                                                        Val, Val, Val, Val, Val, Val, Val, Val }
...
_PI64_CONST512(0, 0);
_PI64_CONST512(1, 1);
_PI64_CONST512(inv1, ~1);
_PI64_CONST512(2, 2);
_PI64_CONST512(4, 4);
_PI64_CONST512(0x7f, 0x7f);
...
inline v16sf log512_ps(v16sf x) {
  ...
  imm0 = _mm512_sub_epi32(imm0, *(v16si*)_PI64_512_0x7f);
...
inline v16sf exp512_ps(v16sf x) {
  ...
  imm0 = _mm512_add_epi32(imm0, *(v16si*)_PI64_512_0x7f);
...
/* evaluation of 8 sines at onces using AVX512 intrisics
...
inline v16sf sin512_ps(v16sf x) { // any x
  ...
  imm2 = _mm512_add_epi32(imm2, *(v16si*)_PI64_512_1);
  imm2 = _mm512_and_si512(imm2, *(v16si*)_PI64_512_inv1);
  y = _mm512_cvtepi32_ps(imm2);

  /* get the swap sign flag */
  imm0 = _mm512_and_si512(imm2, *(v16si*)_PI64_512_4);
  ...
  /* get the polynom selection mask
     ...
     Both branches will be computed.
  */
  imm2 = _mm512_and_si512(imm2, *(v16si*)_PI64_512_2);
  __mmask16 imm2_mask = _mm512_cmpeq_epi32_mask(imm2,*(v16si*)_PI64_512_0); 
  v16si all_one = _mm512_set1_epi32(0xffffffff);
  v16si all_zero = _mm512_set1_epi32(0x00000000);
...
inline v16sf cos512_ps(v16sf x) { // any x
  ...
  imm2 = _mm512_and_si512(imm2, *(v16si*)_PI64_512_inv1);
  y = _mm512_cvtepi32_ps(imm2);
  imm2 = _mm512_sub_epi32(imm2, *(v16si*)_PI64_512_2);

  /* get the swap sign flag */
  imm0 =  _mm512_andnot_si512(imm2, *(v16si*)_PI64_512_4);
  imm0 = _mm512_slli_epi32(imm0, 29);
  /* get the polynom selection mask */
  imm2 = _mm512_and_si512(imm2, *(v16si*)_PI64_512_2);
  __mmask16 imm2_mask = _mm512_cmpeq_epi32_mask(imm2, *(v16si*)_PI64_512_0);
  const v16si all_one = _mm512_set1_epi32(0xffffffff);
...
inline void sincos512_ps(v16sf x, v16sf *s, v16sf *c) {
  ...
  imm4 = _mm512_sub_epi32(imm4, *(v16si*)_PI64_512_2);
  imm4 =  _mm512_andnot_si512(imm4, *(v16si*)_PI64_512_4);
