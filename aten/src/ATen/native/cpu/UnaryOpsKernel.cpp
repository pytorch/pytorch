...
static void cauchy_kernel(TensorIterator& iter, double median, double sigma, GeneratorHolder gen) {
...
#if !AT_MKL_ENABLED()
void bernoulli_mkl_kernel(Tensor &output, const double p, GeneratorHolder gen) {
...
#else
void bernoulli_mkl_kernel(Tensor &self, const double p, GeneratorHolder gen) {
  CPUGenerator* generator = get_generator_or_default<CPUGenerator>(gen, detail::getDefaultCPUGenerator());
  int64_t seed;
  {
...
static void exponential_kernel(TensorIterator& iter, double lambda, GeneratorHolder gen) {
...
static void geometric_kernel(TensorIterator& iter, double p, GeneratorHolder gen) {
...
static void log_normal_kernel(TensorIterator& iter, double mean, double std, GeneratorHolder gen) {
...
void normal_fill_AVX2(Tensor& self, const float mean, const float std, GeneratorHolder gen) {
...
template <typename scalar_t>
void normal_fill(Tensor& self, const scalar_t mean, const scalar_t std, GeneratorHolder gen) {
...
void normal_kernel(Tensor& self, double mean, double std, GeneratorHolder gen) {
  if(self.is_complex()) {
...
static void random_from_to_kernel(TensorIterator& iter, uint64_t range, int64_t base, GeneratorHolder gen) {
  CPUGenerator* generator = get_generator_or_default<CPUGenerator>(gen, detail::getDefaultCPUGenerator());
  templates::cpu::random_from_to_kernel(iter, range, base, generator);
}

static void random_kernel(TensorIterator& iter, GeneratorHolder gen) {
  CPUGenerator* generator = get_generator_or_default<CPUGenerator>(gen, detail::getDefaultCPUGenerator());
...
static void random_full_64_bits_range_kernel(TensorIterator& iter, GeneratorHolder gen) {
  CPUGenerator* generator = get_generator_or_default<CPUGenerator>(gen, detail::getDefaultCPUGenerator());
