    ...
    cpu_kernel_vec(
        ...
        [=](BFloat16 a) -> BFloat16 {
          ...
          return static_cast<float>(1) /
              (static_cast<float>(1) + std::exp((-a0)));
        },
        [=](Vectorized<BFloat16> a) {
          Vectorized<float> a0, a1;
          std::tie(a0, a1) = convert_bfloat16_float(a);
          a0 = (Vectorized<float>(static_cast<float>(1)) + a0.neg().exp())
                   .reciprocal();
          a1 = (Vectorized<float>(static_cast<float>(1)) + a1.neg().exp())
                   .reciprocal();
          return convert_float_bfloat16(a0, a1);
        });
  } else {
    AT_DISPATCH_FLOATING_AND_COMPLEX_TYPES(
        iter.common_dtype(), "sigmoid_cpu", [&]() {
          cpu_kernel_vec(
              iter,
              [=](scalar_t a) -> scalar_t {
                return (
...
static void abs_kernel(TensorIteratorBase& iter) {
  ...
  if (dtype == kComplexHalf) {
    ...
    cpu_kernel(
        iter, [=](scalar_t a) -> scalar_t { return abs_impl(opmath_t{a}); });
  } else {
    AT_DISPATCH_ALL_TYPES_AND_COMPLEX_AND2(
        kBFloat16, kHalf, iter.dtype(), "abs_cpu", [&]() {
          cpu_kernel_vec(
              iter,
...
static void angle_kernel(TensorIteratorBase& iter) {
  AT_DISPATCH_FLOATING_AND_COMPLEX_TYPES_AND2(
      kBFloat16, kHalf, iter.common_dtype(), "angle_cpu", [&]() {
        cpu_kernel_vec(
            iter,
...
static void bitwise_not_kernel(TensorIteratorBase& iter) {
  if (iter.dtype() == ScalarType::Bool) {
    // Boolean type does not work with ~ (bitwise NOT) in C++. bitwise_not wraps
...
void logical_not_kernel(TensorIteratorBase& iter) {
  // NOTE: this implementation differs from the CUDA implementation which only
  // does single dispatch (to avoid expensive compilation) because CPU kernels
  // don't handle dynamic_casting (see needs_dynamic_casting).
  AT_DISPATCH_ALL_TYPES_AND_COMPLEX_AND3(
      kBool, kHalf, kBFloat16, iter.dtype(1), "logical_not_cpu", [&]() {
        using self_t = scalar_t;
        AT_DISPATCH_ALL_TYPES_AND_COMPLEX_AND3(
            kBool, kHalf, kBFloat16, iter.dtype(0), "logical_not_cpu", [&]() {
              cpu_kernel(iter, [](self_t a) -> scalar_t {
...
void reciprocal_kernel(TensorIteratorBase& iter) {
  AT_DISPATCH_FLOATING_AND_COMPLEX_TYPES_AND2(
      kBFloat16, kHalf, iter.common_dtype(), "reciprocal_cpu", [&]() {
        cpu_kernel_vec(
            iter,
...
void sign_kernel(TensorIteratorBase& iter) {
  if (iter.dtype() == ScalarType::Bool) {
    ...
    AT_DISPATCH_ALL_TYPES_AND2(
        kBFloat16, ScalarType::Half, iter.dtype(), "sign_cpu", [&]() {
          ...
          cpu_kernel_vec(
              ...
              [=](Vectorized<scalar_t> self_vec) {
                ...
                auto left = Vectorized<scalar_t>::blendv(
                    zero_vec, one_vec, zero_vec < self_vec);
                auto right = Vectorized<scalar_t>::blendv(
...
static void signbit_kernel(TensorIteratorBase& iter) {
  ...
  if (at::isIntegralType(iter.input_dtype(), /*includeBool=*/false)) {
    ...
    AT_DISPATCH_FLOATING_TYPES_AND2(
        kBFloat16, ScalarType::Half, iter.input_dtype(), "signbit_cpu", [&]() {
          using opmath_t = at::opmath_type<scalar_t>;
...
static void acosh_kernel(TensorIteratorBase& iter) {
  AT_DISPATCH_FLOATING_AND_COMPLEX_TYPES_AND1(
      kBFloat16, iter.dtype(), "acosh_cpu", [&]() {
        cpu_kernel(iter, [=](scalar_t a) -> scalar_t { return std::acosh(a); });
      });
}

static void asinh_kernel(TensorIteratorBase& iter) {
  AT_DISPATCH_FLOATING_AND_COMPLEX_TYPES_AND1(
      kBFloat16, iter.dtype(), "asinh_cpu", [&]() {
        cpu_kernel(iter, [=](scalar_t a) -> scalar_t { return std::asinh(a); });
      });
}

static void atanh_kernel(TensorIteratorBase& iter) {
  AT_DISPATCH_FLOATING_AND_COMPLEX_TYPES_AND1(
      kBFloat16, iter.dtype(), "atanh_cpu", [&]() {
        cpu_kernel(iter, [=](scalar_t a) -> scalar_t { return std::atanh(a); });
      });
}

static void digamma_kernel(TensorIteratorBase& iter) {
  AT_DISPATCH_FLOATING_TYPES_AND(
      kBFloat16, iter.common_dtype(), "digamma", [&]() {
        cpu_kernel(
...
static void trigamma_kernel(TensorIteratorBase& iter) {
  AT_DISPATCH_FLOATING_TYPES_AND(kBFloat16, iter.dtype(), "trigamma", [&]() {
    cpu_kernel(iter, [=](scalar_t a) -> scalar_t { return trigamma(a); });
  });
}
...
static void exp2_kernel(TensorIteratorBase& iter) {
  ...
  AT_DISPATCH_FLOATING_TYPES_AND2(
      kBFloat16, kHalf, iter.dtype(), "exp2", [&]() {
        cpu_kernel(iter, [=](scalar_t a) -> scalar_t { return std::exp2(a); });
      });
}
...
static void nan_to_num_kernel(
  ...
  AT_DISPATCH_FLOATING_TYPES_AND2(
      kBFloat16, kHalf, iter.dtype(), "nan_to_num", [&]() {
...
static void kaiser_window_kernel(
    TensorIteratorBase& iter,
    int64_t window_length,
    double beta) {
  AT_DISPATCH_FLOATING_TYPES_AND(
      kBFloat16, iter.dtype(), "kaiser_window_cpu", [&]() {
        ...
        cpu_kernel(iter, [=](scalar_t a) {
          return calc_i0(
              ...
              calc_i0(static_cast<scalar_t>(beta));
        });
      });
}
...
void rsqrt_kernel(TensorIteratorBase& iter) {
  AT_DISPATCH_FLOATING_AND_COMPLEX_TYPES_AND1(
      kBFloat16, iter.common_dtype(), "rsqrt_cpu", [&] {
        cpu_kernel_vec(
            iter,
            [=](scalar_t a) __ubsan_ignore_float_divide_by_zero__ -> scalar_t {
...
static void erfcx_kernel(TensorIteratorBase& iter) {
  AT_DISPATCH_FLOATING_TYPES(iter.common_dtype(), "erfcx_cpu", [&]() {
    cpu_kernel(iter, [](scalar_t a) -> scalar_t { return calc_erfcx(a); });
  });
}
...
static void bessel_j0_kernel(TensorIteratorBase& iterator) {
  ...
  AT_DISPATCH_FLOATING_TYPES(iterator.common_dtype(), "bessel_j0_cpu", [&]() {
    cpu_kernel(iterator, [](scalar_t x) { return bessel_j0_forward(x); });
  });
...
static void bessel_j1_kernel(TensorIteratorBase& iterator) {
  ...
  AT_DISPATCH_FLOATING_TYPES(iterator.common_dtype(), "bessel_j1_cpu", [&]() {
    cpu_kernel(iterator, [](scalar_t x) { return bessel_j1_forward(x); });
  });
...
static void bessel_y0_kernel(TensorIteratorBase& iterator) {
  ...
  AT_DISPATCH_FLOATING_TYPES(iterator.common_dtype(), "bessel_y0_cpu", [&]() {
    cpu_kernel(iterator, [](scalar_t x) { return bessel_y0_forward(x); });
  });
...
static void bessel_y1_kernel(TensorIteratorBase& iterator) {
  ...
  AT_DISPATCH_FLOATING_TYPES(iterator.common_dtype(), "bessel_y1_cpu", [&]() {
    cpu_kernel(iterator, [](scalar_t x) { return bessel_y1_forward(x); });
  });
...
static void modified_bessel_i0_kernel(TensorIteratorBase& iterator) {
  ...
  AT_DISPATCH_FLOATING_TYPES(
      iterator.common_dtype(), "modified_bessel_i0_cpu", [&]() {
        cpu_kernel(
            iterator, [](scalar_t x) { return modified_bessel_i0_forward(x); });
      });
...
static void modified_bessel_i1_kernel(TensorIteratorBase& iterator) {
  ...
  AT_DISPATCH_FLOATING_TYPES(
      iterator.common_dtype(), "modified_bessel_i1_cpu", [&]() {
        cpu_kernel(
            iterator, [](scalar_t x) { return modified_bessel_i1_forward(x); });
      });
...
static void modified_bessel_k0_kernel(TensorIteratorBase& iterator) {
  ...
  AT_DISPATCH_FLOATING_TYPES(
      iterator.common_dtype(), "modified_bessel_k0_cpu", [&]() {
        cpu_kernel(
            iterator, [](scalar_t x) { return modified_bessel_k0_forward(x); });
      });
...
static void modified_bessel_k1_kernel(TensorIteratorBase& iterator) {
  ...
  AT_DISPATCH_FLOATING_TYPES(
      iterator.common_dtype(), "modified_bessel_k1_cpu", [&]() {
        cpu_kernel(
            iterator, [](scalar_t x) { return modified_bessel_k1_forward(x); });
      });
...
#define IMPLEMENT_FLOAT_KERNEL(op)                               \
  ...
  void op##_kernel(TensorIteratorBase& iter) {                   \
    TORCH_INTERNAL_ASSERT(iter.ntensors() == 2);                 \
    AT_DISPATCH_FLOATING_TYPES_AND(                              \
        kBFloat16, iter.dtype(), #op "_vml_cpu", [&]() {         \
          iter.serial_for_each(                                  \
              IMPLEMENT_ITERATOR_LAMBDA(op), {0, iter.numel()}); \
        });                                                      \
    iter.cast_outputs();                                         \
  }                                                              \
  }                                                              \
...
REGISTER_DISPATCH(
    ...
    &CPU_CAPABILITY::modified_bessel_i0_kernel);
REGISTER_DISPATCH(
    special_modified_bessel_i1_stub,
    &CPU_CAPABILITY::modified_bessel_i1_kernel);
REGISTER_DISPATCH(
    special_modified_bessel_k0_stub,
    &CPU_CAPABILITY::modified_bessel_k0_kernel);
REGISTER_DISPATCH(
