...
static void ellpk_kernel(TensorIteratorBase& iter){
  AT_DISPATCH_FLOATING_TYPES(iter.common_dtype(), "ellpk_cpu", [&]() {
    cpu_kernel(
      iter,
      [](scalar_t a) -> scalar_t { return ellpk(a); });
  });
}
...
REGISTER_DISPATCH(special_ellpk_stub, &CPU_CAPABILITY::ellpk_kernel);
