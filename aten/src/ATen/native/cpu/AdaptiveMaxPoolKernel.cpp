...
void cpu_adaptive_max_pool(
  ...
  at::parallel_for(0, channels, 0, [&](int64_t begin, int64_t end) {
    for (int64_t c = begin; c < end; c++) {
      ...
      for (int64_t i = 0; i < output_depth * output_height * output_width; i++) {
        PreCalc pc = pre_calc[i];

        // compute local max
        int64_t maxindex = pc.id0 * input_height * input_width + pc.ih0 * input_width + pc.iw0;
...
void cpu_adaptive_max_pool_channels_last(
  ...
  if (is_3d) {
    TORCH_CHECK(ndim == 5, "AdaptiveMaxPool3d with channels last format supports tensors with 5 dims");
  } else {
    TORCH_CHECK(ndim == 4, "AdaptiveMaxPool2d with channels last format supports tensors with 4 dims");
  }
...
void cpu_adaptive_max_pool_backward_channels_last(
  ...
  if (is_3d) {
    TORCH_CHECK(ndim == 5, "AdaptiveMaxPool3d backward with channels last format supports tensors with 5 dims.");
  } else {
    TORCH_CHECK(ndim == 4, "AdaptiveMaxPool2d backward with channels last format supports tensors with 4 dims.");
  }
  auto memory_format = is_3d ? at::MemoryFormat::ChannelsLast3d
                             : at::MemoryFormat::ChannelsLast;
  ...
  switch (input.suggest_memory_format()) {
    case at::MemoryFormat::Contiguous: {
      AT_DISPATCH_FLOATING_TYPES(input.scalar_type(), "adaptive_max_pool2d", [&] {
        cpu_adaptive_max_pool<scalar_t, /* is_3d */ false>(output, indices, input, output_size);
      });
      break;
    }
    case at::MemoryFormat::ChannelsLast: {
      AT_DISPATCH_FLOATING_TYPES(input.scalar_type(), "adaptive_max_pool2d_channels_last", [&]{
        cpu_adaptive_max_pool_channels_last<scalar_t, /* is_3d */ false>(output, indices, input, output_size);
      });
      break;
    }
    default:
  ...
  switch (grad_input.suggest_memory_format()) {
    case at::MemoryFormat::Contiguous: {
      AT_DISPATCH_FLOATING_TYPES(grad_output.scalar_type(), "adaptive_max_pool2d_backward", [&] {
        cpu_adaptive_max_pool_backward<scalar_t, /* is_3d */ false>(grad_input, grad_output, indices);
      });
      break;
    }
    case at::MemoryFormat::ChannelsLast: {
      AT_DISPATCH_FLOATING_TYPES(grad_output.scalar_type(), "adaptive_max_pool2d_backward_channels_last", [&]{
        cpu_adaptive_max_pool_backward_channels_last<scalar_t, /* is_3d */ false>(grad_input, grad_output, indices);
      });
      break;
    }
    default:
...
void adaptive_max_pool3d_kernel_impl(
  ...
  switch (input.suggest_memory_format()) {
    case at::MemoryFormat::Contiguous: {
      AT_DISPATCH_FLOATING_TYPES(input.scalar_type(), "adaptive_max_pool3d", [&] {
        cpu_adaptive_max_pool<scalar_t, /* is_3d */ true>(output, indices, input, output_size);
      });
      break;
    }
    case at::MemoryFormat::ChannelsLast3d: {
      AT_DISPATCH_FLOATING_TYPES(input.scalar_type(), "adaptive_max_pool3d_channels_last", [&]{
        cpu_adaptive_max_pool_channels_last<scalar_t, /* is_3d */ true>(output, indices, input, output_size);
      });
      break;
    }
    default:
...
void adaptive_max_pool3d_backward_kernel_impl(
  ...
  switch (grad_input.suggest_memory_format()) {
    case at::MemoryFormat::Contiguous: {
      AT_DISPATCH_FLOATING_TYPES(grad_output.scalar_type(), "adaptive_max_pool3d_backward", [&] {
        cpu_adaptive_max_pool_backward<scalar_t, /* is_3d */ true>(grad_input, grad_output, indices);
      });
      break;
    }
    case at::MemoryFormat::ChannelsLast3d: {
      AT_DISPATCH_FLOATING_TYPES(grad_output.scalar_type(), "adaptive_max_pool3d_backward_channels_last", [&]{
        cpu_adaptive_max_pool_backward_channels_last<scalar_t, /* is_3d */ true>(grad_input, grad_output, indices);
      });
      break;
    }
    default:
