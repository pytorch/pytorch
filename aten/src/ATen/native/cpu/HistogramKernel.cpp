 ...
 *
 *     - BINARY_SEARCH: Handles torch.histogram's general case by by searching
 * over the elements of bin_edges. Implemented using std::upper_bound.
 *
 * See discussion at
...
void histogramdd_cpu_contiguous(
    Tensor& hist,
  ...
  const auto accessor_wt = weight.has_value()
      ? c10::optional<TensorAccessor<input_t, 1>>(
            weight.value().accessor<input_t, 1>())
      : c10::optional<TensorAccessor<input_t, 1>>();
  ...
  std::copy(
      hist_sizes.begin(), hist_sizes.end(), thread_hist_sizes.begin() + 1);
    ...
    for (const auto i : c10::irange(start, end)) {
      ...
      for (const auto dim : c10::irange(D)) {
        ...
        if (algorithm == BINARY_SEARCH) {
          ...
          pos = std::upper_bound(
                    bin_seq[dim], bin_seq[dim] + num_bin_edges[dim], elt) -
              bin_seq[dim] - 1;
        } else if (
          ...
          pos = static_cast<int64_t>(
              (elt - leftmost_edge[dim]) /
              (rightmost_edge[dim] - leftmost_edge[dim]) *
              (num_bin_edges[dim] - 1));

          /* Ensures consistency with bin_edges by checking the bins to the left
           * and right of the selected position. Necessary for cases in which an
           * element very close to a bin edge may be misclassified by simple
           * division.
           */
          if (algorithm == LINEAR_INTERPOLATION_WITH_LOCAL_SEARCH) {
            ...
            pos = std::upper_bound(
                      bin_seq[dim] + pos_min, bin_seq[dim] + pos_max, elt) -
                bin_seq[dim] - 1;
          }
        } else {
      ...
      if (!skip_elt) {
        ...
        input_t wt = accessor_wt.has_value() ? accessor_wt.value()[i]
                                             : static_cast<input_t>(1);
 ...
 * Initializes hist to 0, calls into the main algorithm, and normalizes output
 * if necessary.
 */
template <BIN_SELECTION_ALGORITHM bin_algorithm>
void histogramdd_out_cpu_template(
    const Tensor& self,
    const c10::optional<Tensor>& weight,
    bool density,
    Tensor& hist,
  ...
  AT_DISPATCH_FLOATING_TYPES_AND(
      kBFloat16, self.scalar_type(), "histogram_cpu", [&]() {
...
/* The general implementation of the histogram kernel. Maps each element of the
 * input tensor to its corresponding bin by performing a binary search over the
 * elements of bin_edges.
 *
 * Refer to histogramdd_out_cpu_template for more details.
 */
static void histogramdd_kernel_impl(
    const Tensor& self,
    const c10::optional<Tensor>& weight,
    bool density,
    Tensor& hist,
    const TensorList& bin_edges) {
  histogramdd_out_cpu_template<BINARY_SEARCH>(
      self, weight, density, hist, bin_edges);
}

/* A faster version of the histogram kernel for cases in which bin_edges are
 * known to form a linear progression.
 *
 * Refer to histogramdd_out_cpu_template for more details.
 */
static void histogramdd_linear_kernel_impl(
    const Tensor& self,
    const c10::optional<Tensor>& weight,
    bool density,
    Tensor& hist,
    ...
    bool local_search) {
  if (local_search) {
    // histogramdd codepath: both hist and bin_edges are eventually returned as
    // output, so we'll keep them consistent
