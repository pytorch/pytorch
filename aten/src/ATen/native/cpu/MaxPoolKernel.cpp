...
void cpu_max_pool_channels_last(
  ...
  TORCH_CHECK(
      input_height <=
      std::ceil(
          (double)std::numeric_limits<integer_t>::max() / (double)input_width));
  ...
  switch (input.suggest_memory_format()) {
    ...
    case at::MemoryFormat::ChannelsLast: {
      AT_DISPATCH_FLOATING_TYPES(
          input.scalar_type(), "max_pool2d_channels_last", [&] {
...
void max_pool2d_backward_kernel_impl(
  ...
  switch (grad_output.suggest_memory_format()) {
    case at::MemoryFormat::Contiguous: {
      AT_DISPATCH_FLOATING_TYPES(
          grad_output.scalar_type(), "max_pool2d_backward", [&] {
    ...
    case at::MemoryFormat::ChannelsLast: {
      AT_DISPATCH_FLOATING_TYPES(
          grad_output.scalar_type(), "max_pool2d_backward_channels_last", [&] {
