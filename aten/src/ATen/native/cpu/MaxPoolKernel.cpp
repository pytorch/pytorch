...
void cpu_max_pool_channels_last(
  ...
  if (is_3d) {
    ...
    TORCH_CHECK(ndim == 4, "MaxPool2d with channels last format supports tensors with 4 dims");
  }
  ...
  switch (input.suggest_memory_format()) {
    case at::MemoryFormat::Contiguous: {
      AT_DISPATCH_FLOATING_TYPES(input.scalar_type(), "max_pool2d", [&] {
        cpu_max_pool<scalar_t, /* is_3d */ false>(output, indices, input,
            kW, kH, /* kD */ 1, dW, dH, /* dD */ 1, padW, padH, /* padD */ 0, dilationW, dilationH, /* dilationD */ 1);
      });
      break;
    }
    case at::MemoryFormat::ChannelsLast: {
      AT_DISPATCH_FLOATING_TYPES(input.scalar_type(), "max_pool2d_channels_last", [&] {
        cpu_max_pool_channels_last<scalar_t, /* is_3d */ false>(output, indices, input,
            kW, kH, /* kD */ 1, dW, dH, /* dD */ 1,  padW, padH, /* padD */ 0,  dilationW, dilationH, /* dilationD */ 1);
      });
      break;
    }
  ...
  switch (grad_output.suggest_memory_format()) {
    case at::MemoryFormat::Contiguous: {
      AT_DISPATCH_FLOATING_TYPES(grad_output.scalar_type(), "max_pool2d_backward", [&] {
        cpu_max_pool_backward<scalar_t, /* is_3d*/ false>(grad_input, grad_output, indices);
      });
      break;
    }
    case at::MemoryFormat::ChannelsLast: {
      AT_DISPATCH_FLOATING_TYPES(grad_output.scalar_type(), "max_pool2d_backward_channels_last", [&] {
        cpu_max_pool_backward_channels_last<scalar_t, /* is_3d */ false>(grad_input, grad_output, indices);
      });
      break;
    }
    default:
...
void max_pool3d_kernel_impl(
  ...
  switch (input.suggest_memory_format()) {
    case at::MemoryFormat::Contiguous: {
      AT_DISPATCH_FLOATING_TYPES(input.scalar_type(), "max_pool3d", [&] {
        cpu_max_pool<scalar_t, /* is_3d */ true>(output, indices, input,
            kW, kH, kD, dW, dH, dD, padW, padH, padD, dilationW, dilationH, dilationD);
      });
      break;
    }
    case at::MemoryFormat::ChannelsLast3d: {
      AT_DISPATCH_FLOATING_TYPES(input.scalar_type(), "max_pool3d_channels_last", [&] {
        cpu_max_pool_channels_last<scalar_t, /* is_3d */ true>(output, indices, input,
            kW, kH, kD, dW, dH, dD, padW, padH, padD, dilationW, dilationH, dilationD);
      });
      break;
    }
...
void max_pool3d_backward_kernel_impl(
  ...
  switch (grad_output.suggest_memory_format()) {
    case at::MemoryFormat::Contiguous: {
      AT_DISPATCH_FLOATING_TYPES(grad_output.scalar_type(), "max_pool3d_backward", [&] {
        cpu_max_pool_backward<scalar_t, /* is_3d */ true>(grad_input, grad_output, indices);
      });
      break;
    }
    case at::MemoryFormat::ChannelsLast3d: {
      AT_DISPATCH_FLOATING_TYPES(grad_output.scalar_type(), "max_pool3d_backward_channels_last", [&] {
