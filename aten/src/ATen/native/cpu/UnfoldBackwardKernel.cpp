...
//
// case1. step >= size.
// In this case the iteration takes over grad_in and performs the following
// copy: grad_out[..., i_out_dim,...] = grad_in[..., i_in_dim,...,
// i_in_last_dim], where i_out_dim = i_in_dim * step + i_in_last_dim.
//
// case2. step < size.
...
// intersect with i_out_dim, these are precisely [left_idx_fold,
// right_idx_fold], and then the corresponding value of
// grad_in[...,i_in_dim,...,i_in_last_dim] gets added up to
// grad_out[...,i_out_dim,...].
  ...
  auto loop = [&](char** data, const int64_t* strides, int64_t nelems) {
    ...
    if (is_step_ge_size) {
      ...
      for (const auto elem : c10::irange(nelems)) {
        ...
        auto* RESTRICT grad_out_data =
            reinterpret_cast<scalar_t*>(grad_out_ptr);
