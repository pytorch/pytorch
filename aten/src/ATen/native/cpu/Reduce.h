...
static inline bool is_contiguous_reduction(const int64_t* strides) {
  return strides[0] == 0 && strides[1] == sizeof(typename traits::arg2_t);
}
...
static inline void vectorized_reduction(
    char** data,
    int64_t n,
    int64_t stride,
    func_t op,
  ...
  for (const auto i : c10::irange(1, n)) {
    ...
    acc[0] =
        vop(acc[0], Vec::loadu(ptr + (0 * Vec::size() * sizeof(scalar_t))));
    acc[1] =
        vop(acc[1], Vec::loadu(ptr + (1 * Vec::size() * sizeof(scalar_t))));
    acc[2] =
        vop(acc[2], Vec::loadu(ptr + (2 * Vec::size() * sizeof(scalar_t))));
    acc[3] =
        vop(acc[3], Vec::loadu(ptr + (3 * Vec::size() * sizeof(scalar_t))));
  }
  if (reduce) {
...
template <typename F>
static inline void UNARY_OUTER_LOOP(
    char* data[2],
...
// computes the reduction out = op(out, in)
template <typename func_t, typename vec_func_t>
static inline void vectorized_outer_reduction(
    char** data,
...
static void set_result(
  ...
  // static_assert(std::is_same<res_t, typename traits::arg2_t>::value, "data
  // types must match");
...
static void set_results(
  ...
  std::size_t result_size =
      for_each_in_tuple<traits>(result, iter, num_outputs);
  AT_ASSERT((size_t)num_outputs == result_size);
}

template <typename T, typename... Args>
struct all_same : guts::conjunction<std::is_same<T, Args>...> {};
...
// and satisfy the following. reduce: (acc_t, data_t, index_t) -> acc_t adds one
// data point to the accumulated value. combine: (acc_t, acc_t) -> acc_t
// combines two accumulated values into one. project: acc_t -> out_t finishes
// the reduction, getting the required output.
//
...
void binary_kernel_reduce(TensorIteratorBase& iter, ops_t ops, init_t init) {
  ...
  iter.foreach_reduced_elt([&ops, &init, num_outputs](
                               TensorIteratorBase& sub_iter) {
    auto reduction_body = [&ops, &sub_iter, num_outputs](
                              acc_t acc, int64_t begin, int64_t end) -> acc_t {
      int ntensors = sub_iter.ntensors();
      sub_iter.serial_for_each(
          [&acc, &ops, num_outputs, ntensors, begin](
...
void binary_kernel_reduce_lastdim(
  ...
  int64_t grain_size =
      std::max((int64_t)1, at::internal::GRAIN_SIZE / dim_size);
