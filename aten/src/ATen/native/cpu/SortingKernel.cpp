...
void _dim_apply(
  ...
  AT_DISPATCH_V2(
      ...
      AT_WRAP([&] {
        ...
        int64_t grain_size =
            internal::GRAIN_SIZE / std::max(int64_t{1}, dim_size);
...
#ifdef USE_FBGEMM
static bool can_use_radix_sort(
    const TensorBase& values,
  ...
  if (!at::isIntegralType(values.scalar_type(), /*includeBool=*/false))
    return false;
  // performance improvements are visible for bigger tensor sizes, when
  // radix_sort is accelerated with OpenMP
  if (values.numel() < at::internal::GRAIN_SIZE ||
      !fbgemm::is_radix_sort_accelerated_with_openmp())
    return false;
...
static void parallel_sort1d_kernel(
  ...
  AT_DISPATCH_INTEGRAL_TYPES(
      values.scalar_type(), "parallel_sort1d_kernel", [&] {
        ...
        if (!sorted_in_place) {
          ...
          at::parallel_for(
              ...
              [&](int64_t begin, int64_t end) {
                ...
                vec::map(
                    [](vec::Vectorized<scalar_t> x)
                        -> vec::Vectorized<scalar_t> { return x; },
                    keys + begin,
                    sorted_keys + begin,
                    job_size);
                vec::map(
                    [](vec::Vectorized<int64_t> x) -> vec::Vectorized<int64_t> {
                      return x;
                    },
                    vals + begin,
                    sorted_vals + begin,
...
template <
    typename scalar_t,
    typename value_accessor_t,
    typename indices_accessor_t>
static inline void sort_kernel_impl(
    const value_accessor_t& value_accessor,
    const indices_accessor_t& indices_accessor,
    int64_t dim_size,
    bool descending,
    bool stable) {
  auto composite_accessor =
      CompositeRandomAccessorCPU<value_accessor_t, indices_accessor_t>(
          value_accessor, indices_accessor);
  if (descending) {
    if (stable) {
      std::stable_sort(
          composite_accessor,
          composite_accessor + dim_size,
