...
using forward_fn_structured = void (*)(const Tensor&, const Tensor&);
using backward_fn = void(*)(Tensor &, const Tensor &, const Tensor&);
using backward_fn_structured = void (*)(const Tensor&, const Tensor&, const Tensor&);

DECLARE_DISPATCH(forward_fn, softmax_lastdim_kernel);
DECLARE_DISPATCH(forward_fn_structured, log_softmax_lastdim_kernel);
DECLARE_DISPATCH(backward_fn, softmax_backward_lastdim_kernel);
DECLARE_DISPATCH(backward_fn_structured, log_softmax_backward_lastdim_kernel);
