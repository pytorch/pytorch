  ...
  for (int64_t i = 0; i < n_dist; i++) {
    ...
    if (!lost_precision) {
      ...
      for (int64_t j = 0; j < n_categories; j++) {
        ...
        accscalar_t h = static_cast<accscalar_t>(std::floor(val));
        accscalar_t l = static_cast<accscalar_t>(val) - h;
        sum_high += h;
        sum_low += l;
        if (sum_low >= 1) {
          sum_low--;
      ...
      for (int64_t j = 0; j < n_sample; j++) {
        ...
        int right_pointer = n_categories;
        int mid_pointer;
        accscalar_t cum_prob_high;
        accscalar_t cum_prob_low;
        int sample_idx;
        ...
        while(right_pointer - left_pointer > 0) {
          ...
          accscalar_t diff_high = cum_prob_high - uniform_sample * sum_high;
          accscalar_t diff_low = cum_prob_low - uniform_sample * sum_low;
          if (diff_high < -1 * diff_low) {
            left_pointer = mid_pointer + 1;
          }
          else {
            right_pointer = mid_pointer;
        ...
        if (!with_replacement && j < n_sample - 1) {
          ...
          accscalar_t new_val_high = 0;
          accscalar_t new_val_low = 0;
