...
TORCH_LIBRARY(quantized, m) {
  ...
  m.def(TORCH_SELECTIVE_SCHEMA("quantized::linear_with_mkldnn_weight_bias(Tensor qx, float x_scale, int x_zero_point, Tensor qw, Tensor inv_w_scale, Tensor w_zero_point, Tensor? bias, str post_op_name, float[] post_op_args, float output_scale, int output_zero_point) -> Tensor"));
  ...
  m.def(TORCH_SELECTIVE_SCHEMA("quantized::linear_prepack_mkldnn(Tensor weight, Tensor w_scales, int[] x_shape, float x_scale, int x_zp, Tensor? bias) -> (Tensor, Tensor)"));
