...
TORCH_LIBRARY(quantized, m) {
  ...
  m.def(TORCH_SELECTIVE_SCHEMA("quantized::conv_binary.tensor(Tensor qx, Tensor input_scale, Tensor input_zero_point, Tensor qaccum, Tensor accum_scale, Tensor accum_zp, Tensor qw, Tensor weight_scale, Tensor weight_zero_point, int w_axis, Tensor? bias, int[] stride, int[] padding, int[] dilation, int groups, Tensor output_scale, Tensor output_zero_point, str binary_post_op) -> Tensor"));
