...
TORCH_LIBRARY(quantized, m) {
  ...
  m.def(TORCH_SELECTIVE_SCHEMA("quantized::fused_fake_quant_embedding(Tensor weight, Tensor indices, int quant_min=0, int quant_max=255, int padding_idx=-1, bool scale_grad_by_freq=False, bool sparse=False) -> Tensor"));
  m.def(TORCH_SELECTIVE_SCHEMA("quantized::fused_fake_quant_embedding_bag(Tensor weight, Tensor indices, Tensor offsets, int quant_min=0, int quant_max=255, bool scale_grad_by_freq=False, int mode=0, bool sparse=False, Tensor? per_sample_weights=None, bool include_last_offset=False, int padding_idx=-1) -> (Tensor, Tensor, Tensor, Tensor)"));
