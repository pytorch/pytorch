...
Tensor add(Tensor qa, Tensor qb, double output_scale, int64_t output_zero_point) {
  ...
  auto run = [&](cudnn_frontend::ManagedOpaqueDescriptor plan_desc) {
    ...
    data_ptrs.reserve(8);
    uids.reserve(8);
    data_ptrs = {qb.mutable_data_ptr<int8_t>(), rhs_multiplier_tensor.mutable_data_ptr(), add_mutable_output.data_ptr(),
                 qa.mutable_data_ptr<int8_t>(), add_output.mutable_data_ptr(), requantize_multiplier_tensor.mutable_data_ptr(),
