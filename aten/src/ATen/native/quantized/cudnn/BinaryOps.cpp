...
struct AddParams {
  ...
  uint8_t input_dim; // we currently assume both inputs are given as the same
                     // size (i.e., no broadcasting)
...
// TODO: this is also in BinaryOps.cpp and some other cpp files in
// quantized/cpu/. I think we should move everything into a utilities file in
// quantized/ directory later.
...
// output) quantized add We implement relu ( (a_int8 + b_int8 * (
// b_scale/a_scale) ) ) * ( a_scale / out_scale ) which requires 4 cudnn ops (2
// multiplication, 1 addition, and 1 relu ops) Multiplication ops: rhs_mult_op,
// requant_op Addition op: add_op Relu op: relu_op
...
Tensor add(
  ...
  // TODO: add shape checking when broadcasted add is supported. For now we
  // assume the input tensors are the same shape
  TORCH_CHECK(
      qa.sizes() == qb.sizes(),
      "Quantized cudnn add currently expects both input tensors to be the same shape");

  check_inputs(qa, qb);

  // cudnn expects tensors to be at least 3D. So we will prepend dummy
  // dimensions if the input tensors are not at least 3D
  ...
  auto memory_format = qa.dim() == 4 ? at::MemoryFormat::ChannelsLast
                                     : at::MemoryFormat::Contiguous;
  at::Tensor add_output = at::empty(
      qa.sizes(), at::device(at::kCUDA).dtype(at::kFloat), memory_format);
  ...
  at::Tensor requantize_multiplier_tensor =
      cudnn_utils::getRequantMultiplierTensor(
          requantize_multiplier, quantized_output.dim());
  at::Tensor rhs_multiplier_tensor = at::empty(
      quantized_output.sizes(),
  ...
  auto run = [&](cudnn_frontend::ManagedOpaqueDescriptor plan_desc) {
    ...
    data_ptrs = {
        qb.data_ptr<int8_t>(),
        rhs_multiplier_tensor.data_ptr(),
        add_output.data_ptr(),
        qa.data_ptr<int8_t>(),
        add_output.data_ptr(),
    ...
    uids = {'b', 'm', 'c', 'a', 'p', 'r', 'q'};
    if (kReluFused) {
      data_ptrs.emplace_back(add_output.data_ptr()), uids.emplace_back('f');
    }
    ...
    AT_CUDNN_CHECK(cudnnBackendExecute(
        handle, plan_desc->get_backend_descriptor(), variant_pack_desc));
  };
  ...
  auto rhs_mult_op = cudnn_frontend::OperationBuilder(
                         ...
                         .setxDesc(cudnn_utils::getTensorDescriptor(
                             ...
                             CUDNN_DATA_INT8,
                             'b',
                             key.input_b_alignment))
                         .setbDesc(cudnn_utils::getTensorDescriptor(
                             rhs_multiplier_tensor,
                             'm',
                             cudnn_utils::getAlignment(rhs_multiplier_tensor)))
                         .setyDesc(cudnn_utils::getTensorDescriptor(
                             add_output, 'c', key.output_alignment))
  ...
  auto add_op = cudnn_frontend::OperationBuilder(
                    ...
                    .setbDesc(cudnn_utils::getTensorDescriptor(
                        ...
                        CUDNN_DATA_INT8,
                        'a',
                        key.input_a_alignment))
                    .setyDesc(cudnn_utils::getTensorDescriptor(
                        add_output, 'p', key.output_alignment))
                    .setpwDesc(cudnn_utils::getPointWiseAddDescriptor(
                        at::native::getCudnnDataType(add_output)))
                    .build();
  ...
  if (kReluFused) {
    ...
    relu_op.emplace(cudnn_frontend::OperationBuilder(
                        CUDNN_BACKEND_OPERATION_POINTWISE_DESCRIPTOR)
                        .setxDesc(add_op.getOutputTensor())
                        .setyDesc(cudnn_utils::getTensorDescriptor(
                            add_output, 'f', key.output_alignment))
                        .setpwDesc(cudnn_utils::getPointWiseReluDescriptor(
                            at::native::getCudnnDataType(add_output)))
                        .build());
  }
  ...
  auto requant_op =
      cudnn_frontend::OperationBuilder(
          CUDNN_BACKEND_OPERATION_POINTWISE_DESCRIPTOR)
          .setxDesc(
              kReluFused ? relu_op.value().getOutputTensor()
                         : add_op.getOutputTensor())
          .setbDesc(cudnn_utils::getTensorDescriptor(
          ...
          .setyDesc(cudnn_utils::getTensorDescriptor(
              quantized_output.sizes(),
              quantized_output.strides(),
              CUDNN_DATA_INT8,
              'q',
              cudnn_utils::getAlignment(quantized_output)))
          .setpwDesc(cudnn_utils::getPointWiseMulDescriptor(
  ...
  cudnn_utils::filterEngineConfigs(
      engine_configs, filtered_configs, deterministic, allow_tf32, at::kChar);
  cudnn_utils::filterEngineConfigs(
      fallback_list, filtered_configs, deterministic, allow_tf32, at::kChar);
  ...
  TORCH_CHECK(
      false,
      "Unable to find an engine to execute this computation in Quantized Add Cudnn");
}

TORCH_LIBRARY_IMPL(quantized, QuantizedCUDA, m) {
  m.impl(
      TORCH_SELECTIVE_NAME("quantized::add"),
      TORCH_FN(add</*ReLUFused=*/false>));
  m.impl(
      TORCH_SELECTIVE_NAME("quantized::add_relu"),
      TORCH_FN(add</*ReLUFused=*/true>));
}
