...
#else // USE_CUDA
 TORCH_FAIL("at::native::adaptive_avg_pool2d_quantized_cuda: ATen not compiled with USE_CUDA support");
...
Tensor quantized_max_pool2d_cudnn(
 ...
 TORCH_FAIL("at::native::quantized_max_pool2d_cudnn: ATen not compiled with cuDNN support");
  return Tensor{}; // never reached, placates the compiler
#endif // AT_CUDNN_ENABLED()
#else // USE_CUDA
 TORCH_FAIL("at::native::quantized_max_pool2d_cudnn: ATen not compiled with USE_CUDA support");
  return Tensor{}; // never reached, placates the compiler
#endif
}
