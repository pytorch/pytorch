...
void PackedConvWeightCudnn<kSpatialDim>::apply_impl_helper(const at::Tensor& quantized_output, const at::Tensor& input, double output_scale) {
  ...
  auto run = [&](const cudnn_frontend::ExecutionPlan& plan_desc) {
    ...
    data_ptrs = {input.mutable_data_ptr<int8_t>(), maybe_padded_weight_.mutable_data_ptr<int8_t>(),
                 const_cast<void*>(requantize_multiplier_tensor.const_data_ptr()), quantized_output.mutable_data_ptr<int8_t>()};
    uids = {'x', 'w', 's', 'r'};
    if (bias_.has_value()) {
      void* bias_ptr = broadcasted_bias.value().mutable_data_ptr();
      data_ptrs.insert(data_ptrs.end(), {bias_ptr, const_cast<void*>(bias_multiplier_tensor.value().const_data_ptr()), bias_ptr});
