...
void PackedConvWeightCudnn<kSpatialDim>::apply_impl_helper(
  ...
  at::Tensor requantize_multiplier_tensor =
      cudnn_utils::getRequantMultiplierTensor(
          requantize_multiplier, kSpatialDim + 2);
  ...
  if (bias_.has_value()) {
    ...
    // second dimension of quantized_output. we need to add trailing dimensions
    // in order to properly broadcast bias, otherwise broadcast_to will fail.
    // the number of trailling dimensions is quantized_output.dim() - 2, so the
    // new size of the broadcast_bias becomes quantized_output.dim() - 2 + 1.
    // nothing needs to be done for the leading dimensions
    ...
    broadcasted_bias.value() =
        broadcasted_bias.value().broadcast_to(quantized_output.sizes());
    broadcasted_bias.value() =
        broadcasted_bias.value().to(c10::MemoryFormat::ChannelsLast);
    bias_multiplier_tensor = at::empty(
  ...
  auto run = [&](const cudnn_frontend::ExecutionPlan& plan_desc) {
    ...
    data_ptrs = {
        ...
        maybe_padded_weight_.data_ptr<int8_t>(),
    ...
    AT_CUDNN_CHECK(cudnnBackendExecute(
        handle, plan_desc.get_raw_desc(), variant_pack_desc));
  };
  ...
  auto conv_op =
      cudnn_frontend::OperationBuilder(
          ...
          .setyDesc(cudnn_utils::getTensorDescriptor(
              ...
              CUDNN_DATA_FLOAT,
              'y',
              key.output_alignment,
              true))
          .setwDesc(cudnn_utils::getTensorDescriptor(
              maybe_padded_weight_.sizes(),
              maybe_padded_weight_.strides(),
              CUDNN_DATA_INT8,
              'w',
              key.weight_alignment))
          .setcDesc(getConvDescriptor(
              key.params.dataType, padding_vec, stride_vec, dilation_vec))
          .build();
    ...
    bias_mult_op.emplace(
        cudnn_frontend::OperationBuilder(
            ...
            .setpwDesc(cudnn_utils::getPointWiseMulDescriptor(
                at::native::getCudnnDataType(bias_multiplier_tensor.value())))
            .build());

    // computes (act_int8 * w_int8 + [bias_fp32/(act_scale * w_scale)])
    // where the 1st and 2nd summands is output of conv_op and broadcasted_bias,
    // resp. output is a fp32 tensor we use inplace operation here where the
    // output is assigned to the input
    sum_conv_bias_op.emplace(
        cudnn_frontend::OperationBuilder(
            CUDNN_BACKEND_OPERATION_POINTWISE_DESCRIPTOR)
            ...
            .setpwDesc(cudnn_utils::getPointWiseAddDescriptor(
                at::native::getCudnnDataType(broadcasted_bias.value())))
            .build());
  }
  ...
  std::shared_ptr<cudnn_frontend::OpaqueBackendPointer> tensor2requant_ptr =
      bias_.has_value() ? sum_conv_bias_op.value().getOutputTensor()
                        : conv_op.getOutputTensor();
  if (kReluFused) {
    ...
    relu_op.emplace(
        cudnn_frontend::OperationBuilder(
            ...
            .setyDesc(cudnn_utils::getTensorDescriptor(
                ...
                true))
            .setpwDesc(
                cudnn_utils::getPointWiseReluDescriptor(CUDNN_DATA_FLOAT))
            .build());
  }

  // relu_op computes relu(act_int8 * w_int8 + [bias_fp32/(act_scale *
  ...
  TORCH_CHECK(
      false,
      "Unable to find an engine to execute this computation in Quantized Conv2D Cudnn");
}
...
out_int8 = (act_fp32 * w_fp32 + [bias_fp32]) / out_scale + out_zero_point
              = (act_int8 - act_zero_point) * act_scale * (w_int8 -
w_zero_point) * w_scale / out_scale + out_zero_point + [bias_fp32 / out_scale]
             = (act_int8 * w_int8 - act_int8 * w_zero_point - act_zero_point *
w_int8 + act_zero_point * w_zero_point) * act_scale * w_scale / out_scale +
out_zero_point + [bias_fp32 / out_scale] = (if both act and weight are
symmetrically quantized, int8, then act_zero_point = w_zero_point = 0) =
(act_int8 * w_int8 + [bias_fp32/(act_scale * w_scale)]) * act_scale * w_scale /
out_scale = (act_int8 * w_int8 + [bias_fp32/(act_scale * w_scale)]) / (out_scale
/ (act_scale * w_scale)) = requantize((act_int8 * w_int8 + [bias_fp32/(act_scale
* w_scale)]), out_scale / (act_scale * w_scale))
*/
template <int kSpatialDim>
template <bool kReluFused>
at::Tensor PackedConvWeightCudnn<kSpatialDim>::apply_impl(
  ...
  const auto num_output_channels =
      maybe_padded_weight_.size(0); // output channels
  std::vector<int64_t> kernel_size = {
      maybe_padded_weight_.size(2), maybe_padded_weight_.size(3)};
  at::SmallVector<int64_t, kSpatialDim + 2> output_shape =
      MakeConvOutputShape<kSpatialDim>(
          batch_size,
          num_output_channels,
          {H, W},
          kernel_size,
  ...
  // cudnn v8.4.0 expects conv2d's int8 activation tensor's input channels to be
  // a multiple of 4. if it is not we need to explicitly pad it to a multiple of
  // 4 ourselves as cudnn does not currently support padding.
  // TODO: when and if cudnn enables padding in their operators, we can remove
  // padding on our end; currently, limit padding support to groups=1 (ungrouped
  // conv)
  // TODO: implement this for groups > 1; should be straightforward since we're
  // only padding a single dimension
  auto act_maybe_padded = act;
  if (num_input_channels % 4 != 0) {
    int8_t num_slices =
        4 - num_input_channels % 4; // number of slices we need to pad
    act_maybe_padded =
...
class QConv1dInt8 final {
 public:
  static Tensor run(
      ...
      int64_t output_zero_point) {
    at::Tensor output;
    // we currently use conv2d kernel for conv1d by making the input and weight
    // tensors 4D rather than 3D. we add a dummy width dimension of size 1 N, C,
    // L -> N, C, 1, L
    act = act.unsqueeze(-2);
    if (kReluFused) {
 ...
 public:
  static at::Tensor run(
    ...
    TORCH_CHECK(
        ...
        "Error in quantized cudnn conv2d operator: "
        "Expected kSpatialDim == 1 || kSpatialDim == 2; received kSpatialDim=",
        kSpatialDim);
...
TORCH_LIBRARY_IMPL(quantized, QuantizedCUDA, m) {
  // the cpu conv1d doesn't use the quantized::conv1d*.new variant for packed
  // weights. instead it just uses quantized::conv1d for packed weights (see
  // quantized/library.cpp). this is inconsistent with what has been done for
  // conv2d where new variants use packed weights, and old variant does not. we
  // adopt this inconsistency for now to be consistent with QuantizedCPU's
  // conv1d and will eventually deprecate the old variants
