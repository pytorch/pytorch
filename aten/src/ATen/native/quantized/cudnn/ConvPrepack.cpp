...
c10::intrusive_ptr<ConvPackedParamsBase<kSpatialDim>> PackedConvWeightCudnn<
  ...
  TORCH_CHECK(
      groups == 1,
      "Quantized cudnn conv2d is currenty limited to groups = 1; received groups =",
      groups);
  TORCH_CHECK(
  ...
  TORCH_CHECK(
      ...
      "D convolution.");
  TORCH_CHECK(
      !transpose, "cudNN quantized conv prepack expects transpose = false")
  ...
  if (bias.has_value()) {
    ...
    // TODO: we create a broadcasted_bias tensor later so I think we don't need
    // to make this contiguous here. we will revisit this when nvidia adds
    // proper support for broadcasting bias_contig = bias->contiguous();
  }
  ...
  auto ret_ptr = c10::make_intrusive<PackedConvWeightCudnn<kSpatialDim>>(
      weight.to(
          c10::MemoryFormat::ChannelsLast), // TODO: this assumes 2D I think.
...
template c10::intrusive_ptr<ConvPackedParamsBase<2>> PackedConvWeightCudnn<2>::
    prepack(
        at::Tensor weight,
...
TORCH_LIBRARY_IMPL(quantized, QuantizedCUDA, m) {
  m.impl(
      ...
      TORCH_FN(QConv1dPackWeightInt8Cudnn::run_conv));
  m.impl(
      TORCH_SELECTIVE_NAME("quantized::conv2d_prepack"),
      TORCH_FN(QConvPackWeightInt8Cudnn<2>::run_conv));
}
