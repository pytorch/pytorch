...
void PackedLinearWeightCudnn::apply_impl_helper(
  ...
  if (bias_.has_value()) {
    ...
    bias_multiplier_tensor = at::empty(
        quantized_output.sizes(), at::device(at::kCUDA).dtype(at::kFloat));
  ...
  auto run = [&](const cudnn_frontend::ExecutionPlan& plan_desc) {
    ...
    data_ptrs = {
        ...
        weight_transposed.data_ptr<int8_t>(),
  ...
  auto linear_op =
      cudnn_frontend::OperationBuilder(
          ...
          .setaMatDesc(cudnn_utils::getTensorDescriptor(
              ...
              'x',
              key.input_alignment))
          .setbMatDesc(cudnn_utils::getTensorDescriptor(
              weight_transposed.sizes(),
              weight_transposed.strides(),
              CUDNN_DATA_INT8,
              'w',
  ...
  TORCH_CHECK(
      false,
      "Unable to find an engine to execute this computation Quantized Linear Cudnn");
}
