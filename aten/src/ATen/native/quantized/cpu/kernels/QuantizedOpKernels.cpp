        ...
        [&](qVec value_qx) -> qVec {
          ...
          for (int idx = 0; idx < dx_vec.size(); ++idx) {
            ...
            if (cmp_to_threshold.zero_mask()) {
              // blend
              dx_vec[idx] =
                  Vec::blendv(value_vec, dx_vec[idx], cmp_to_threshold);
            }
          }
          // quantize
          return qVec::quantize(
      ...
      for (int64_t oh = 0; oh < osizeH; oh++) {
        ...
        for (int64_t ow = 0; ow < osizeW; ow++) {
          auto* o_p = reinterpret_cast<typename scalar_t::underlying*>(
              ...
              od * osizeH * osizeW * sizeC + oh * osizeW * sizeC + ow * sizeC);
...
void qadaptive_avg_pool2d_nhwc_kernel(
    ...
    int64_t istrideH,
    int64_t istrideW) {
  _qadaptive_avg_pool_kernel(
      "adaptive_avg_pool2d_nhwc",
      qx,
      qy,
      b,
      sizeC,
...
void fake_quant_per_channel_cachemask_cpu(
  ...
  AT_DISPATCH_FLOATING_TYPES_AND_HALF(
      iter.dtype(), "fake_quantize_channel_cachemask_cpu_type_handling", [&] {
        ...
        cpu_kernel(
            iter_mask,
            [=](scalar_t self, float scale, int64_t zero_point) -> bool {
              float inv_scale = 1.0f / scale;
              const auto qval = static_cast<int64_t>(
        ...
        cpu_kernel(
            iter,
            [=](scalar_t self, float scale, int64_t zero_point) -> scalar_t {
              float inv_scale = 1.0f / scale;
              // NOLINTNEXTLINE(bugprone-narrowing-conversions,cppcoreguidelines-narrowing-conversions)
              return (std::fmin(
                          std::fmax(
          ...
          for (int64_t i = start; i < end; ++i) {
            ...
            if (affine_per_channel) {
              ...
              for (int64_t chIdx = 0; chIdx < channels_per_group; chIdx++) {
                ...
                for (int64_t vecIdx = 0; vecIdx < kNumIntVecInChannel;
                     vecIdx++) {
                  ...
                  // NOLINTNEXTLINE(clang-diagnostic-sign-compare)
                  for (int dqXVecIdx = 0; dqXVecIdx < dqXVec.size();
                       dqXVecIdx++) {
                    dqXVec[dqXVecIdx] =
