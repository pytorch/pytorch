  ...
  AT_DISPATCH_QINT_TYPES(out.scalar_type(), "qmul", [&]() {
    ...
    cpu_kernel_vec(
        ...
        [&](scalar_t a, scalar_t b) -> scalar_t {
          ...
          scalar_t res =
              at::requantize_val<scalar_t>(multiplier, zero_point, c);
          if (ReLUFused) {
        ...
        [&](Vec a, Vec b) -> Vec {
          Vec::int_vec_return_type a_sub_zp =
              a.subtract(Vec(static_cast<scalar_t>(self_zero_point)));
          Vec::int_vec_return_type b_sub_zp =
              b.subtract(Vec(static_cast<scalar_t>(other_zero_point)));
          Vec::int_vec_return_type c;
          for (int i = 0; i < Vec::int_num_vecs(); ++i) {
            c[i] = a_sub_zp[i] * b_sub_zp[i];
          }
          Vec rv = Vec::requantize(c, multiplier, zero_point);
          if (ReLUFused) {
