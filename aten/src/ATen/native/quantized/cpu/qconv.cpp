...
at::Tensor PackedConvWeight<kSpatialDim>::apply_impl(
  ...
  if (transpose()) {
    ...
    if (pack_w->getPackedWForDirectconv().get() &&
      ...
      pack_w->getPackedWForDirectconv()
          .get()
          ->col_offsets_with_zero_pt_s8acc32_DirectConvT(
              conv_p, w_zp.data(), col_offsets, M);
    }
  } else {
...
at::Tensor PackedConvWeightsQnnp<kSpatialDim>::apply_impl_xnnp(
  ...
  TORCH_CHECK(
      kSpatialDim == 2,
      func_name,
      ": xnnpack does not currently support 3d convolution.");
   ...
   * need to fix when support is added. ** - zero_points for int8_t: symmetric
  ...
  if ((std::is_same<underlying_t, c10::quint8>::value)) {
    TORCH_CHECK(
        !per_channel(),
        func_name,
        ": xnnpack does not currently have per_channel support with activation dtype of c10::quint8.");
  }
  ...
  if (!xnnp_convolution_op ||
    ...
    if (!per_channel()) {
      ...
      weight_tensor = at::native::empty_per_channel_affine_quantized(
          ...
          w_scales,
          at::zeros(
              w_scales.sizes(), at::kInt), /* see comment above about w_zp */
  ...
  if (act_nhwc.numel() > 0) {
    TORCH_CHECK(
        ...
        ": ",
        kSpatialDim,
        "d (xnnpack): each dimension of output tensor should be greater than 0.")
  }
  ...
  status = at::native::xnnp_utils::xnnp_setup_convolution2d_nhwc(
      ...
      W,
      reinterpret_cast<const underlying_t*>(
          act_nhwc.template data_ptr<scalar_t>()),
...
#ifdef USE_XNNPACK
  if (can_use_xnnp(
          input.scalar_type(), kSpatialDim, per_channel(), transpose())) {
...
at::Tensor PackedConvWeightsOnednn<kSpatialDim>::apply_impl(
  ...
  ConvDimChecks<kSpatialDim>(
      ...
      transpose());
  TORCH_CHECK(
      act.scalar_type() == c10::ScalarType::QUInt8,
      func_name,
      " (ONEDNN): data type of input should be QUint8.");
  ...
  if (transpose()) {
    ...
    TORCH_CHECK(
        C == groups() * weights.get_dim(1), // weight: [o, i, ...]
        func_name,
        " (ONEDNN): input channel number should be ",
        groups() * weights.get_dim(1),
        ", but got ",
        C);
    auto output_shape = MakeDeConvOutputShape<kSpatialDim>(
        ...
        kSpatialDim == 2 ? std::vector<int64_t>{H, W}
                         : std::vector<int64_t>{D, H, W},
        kSpatialDim == 2 ? std::vector<int64_t>{KH, KW}
                         : std::vector<int64_t>{KD, KH, KW},
        stride(),
        padding(),
        output_padding(),
        dilation());
    ...
    output_sizes = at::native::conv_output_size(
        input_size,
        kernel_size,
        padding().vec(),
        stride().vec(),
  ...
  ideep::dims dst_dims =
      ideep::dims({output_sizes.cbegin(), output_sizes.cend()});
  ...
  const ideep::scale_t& src_scales = ideep::scale_t(
      1, 1.0 / act.q_scale()); // Scales of ONEDNN and PyTorch are reciprocal
  const ideep::scale_t& weights_scales = weights.get_scale();
  const ideep::scale_t& dst_scales = ideep::scale_t(
      weights_scales.size(),
      1.0 / output_scale); // Scales of ONEDNN and PyTorch are reciprocal
  const ideep::zero_point_t src_zero_points =
      ideep::zero_point_t(1, act.q_zero_point());
  const ideep::zero_point_t dst_zero_points =
      ideep::zero_point_t(1, output_zero_point);
  ideep::attr_t op_attr =
      kReluFused ? ideep::attr_t::fuse_relu() : ideep::attr_t();
  op_attr.set_zero_points(
      DNNL_ARG_SRC,
      ideep::utils::tensor_zp_mask(1),
      {DNNL_RUNTIME_S32_VAL}); // runtime src zero point
  if (with_bias) {
