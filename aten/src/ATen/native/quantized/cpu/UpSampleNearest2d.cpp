...
static void upsample_nearest2d_out_frame(
  ...
  int64_t grain_size =
      internal::GRAIN_SIZE / std::max(int64_t{1}, output_width);
  at::parallel_for(
      0, channels * output_height, grain_size, [&](int64_t begin, int64_t end) {
        ...
        for (const auto i : c10::irange(begin, end)) {
          ...
          const auto* pos1 =
              &i_p[nc * input_height * input_width + h1 * input_width];
...
Tensor upsample_nearest2d_quantized_cpu(
  ...
  return _upsample_nearest2d_quantized_cpu<
      nearest_neighbor_compute_source_index>(input, osize, scale_h, scale_w);
}
...
Tensor _upsample_nearest_exact2d_quantized_cpu(
  ...
  auto scale_h = get_scale_value(scale_factors, 0);
  auto scale_w = get_scale_value(scale_factors, 1);
  return _upsample_nearest_exact2d_quantized_cpu(
      input, osize, scale_h, scale_w);
}
