...
TEST(Q8AVGPOOL_UP16x9__VSX, kc_eq_16_fulltile) {
  TEST_REQUIRES_VSX;
  auto tester = AvgPoolMicrokernelTester().kr(16).mr(9).kc(16);
  for (size_t kh = 1; kh <= tester.mr(); kh++) {
    for (size_t kw = 1; kw <= tester.mr(); kw++) {
      if (kh * kw == tester.mr()) {
        tester.kh(kh).kw(kw).test(pytorch_q8avgpool_ukernel_up16x9__vsx);
      }
    }
  }
}

TEST(Q8AVGPOOL_UP16x9__VSX, kc_eq_16_subtile) {
  TEST_REQUIRES_VSX;
  auto tester = AvgPoolMicrokernelTester().kr(16).mr(9).kc(16);
  for (size_t ks = 2; ks < tester.mr(); ks++) {
    for (size_t kh = 1; kh <= ks; kh++) {
...
TEST(Q8AVGPOOL_UP16x9__VSX, kc_div_16_fulltile) {
  TEST_REQUIRES_VSX;
  auto tester = AvgPoolMicrokernelTester().kr(16).mr(9);
  for (size_t kh = 1; kh <= tester.mr(); kh++) {
    for (size_t kw = 1; kw <= tester.mr(); kw++) {
      if (kh * kw == tester.mr()) {
...
TEST(Q8AVGPOOL_UP16x9__VSX, kc_div_16_subtile) {
  TEST_REQUIRES_VSX;
  auto tester = AvgPoolMicrokernelTester().kr(16).mr(9).iterations(3);
  for (size_t ks = 2; ks < tester.mr(); ks++) {
    for (size_t kh = 1; kh <= ks; kh++) {
      for (size_t kw = 1; kw <= ks; kw++) {
        if (kh * kw == ks) {
...
TEST(Q8AVGPOOL_UP16x9__VSX, kc_div_16_fulltile_with_x_stride) {
  TEST_REQUIRES_VSX;
  auto tester = AvgPoolMicrokernelTester().kr(16).mr(9).iterations(3);
  for (size_t kh = 1; kh <= tester.mr(); kh++) {
    for (size_t kw = 1; kw <= tester.mr(); kw++) {
...
TEST(Q8AVGPOOL_UP16x9__VSX, kc_gt_16_fulltile) {
  TEST_REQUIRES_VSX;
  auto tester = AvgPoolMicrokernelTester().kr(16).mr(9);
  for (size_t kh = 1; kh <= tester.mr(); kh++) {
    for (size_t kw = 1; kw <= tester.mr(); kw++) {
...
TEST(Q8AVGPOOL_UP16x9__VSX, kc_gt_16_subtile) {
  TEST_REQUIRES_VSX;
  auto tester = AvgPoolMicrokernelTester().kr(16).mr(9).iterations(3);
  for (size_t ks = 2; ks < tester.mr(); ks++) {
    for (size_t kh = 1; kh <= ks; kh++) {
...
TEST(Q8AVGPOOL_UP16x9__VSX, kc_gt_16_fulltile_with_x_stride) {
  TEST_REQUIRES_VSX;
  auto tester = AvgPoolMicrokernelTester().kr(16).mr(9).iterations(3);
  for (size_t kh = 1; kh <= tester.mr(); kh++) {
    for (size_t kw = 1; kw <= tester.mr(); kw++) {
...
TEST(Q8AVGPOOL_UP16x9__VSX, kc_div_16_with_x_scale) {
  TEST_REQUIRES_VSX;
  for (size_t n = 1; n <= 5; n += 2) {
    for (size_t kc = 16; kc < 128; kc += 24) {
      for (float xScale = 0.01f; xScale < 100.0f; xScale *= 3.14159265f) {
...
TEST(Q8AVGPOOL_UP16x9__VSX, kc_div_16_with_x_zero_point) {
  TEST_REQUIRES_VSX;
  for (size_t n = 1; n <= 5; n += 2) {
    for (size_t kc = 16; kc < 128; kc += 24) {
      for (int32_t xZeroPoint = 0; xZeroPoint <= 255; xZeroPoint += 51) {
...
TEST(Q8AVGPOOL_UP16x9__VSX, kc_div_16_with_y_scale) {
  TEST_REQUIRES_VSX;
  for (size_t n = 1; n <= 5; n += 2) {
    for (size_t kc = 16; kc < 128; kc += 24) {
      for (float yScale = 0.01f; yScale < 100.0f; yScale *= 3.14159265f) {
...
TEST(Q8AVGPOOL_UP16x9__VSX, kc_div_16_with_y_zero_point) {
  TEST_REQUIRES_VSX;
  for (size_t n = 1; n <= 5; n += 2) {
    for (size_t kc = 16; kc < 128; kc += 24) {
      for (int32_t yZeroPoint = 0; yZeroPoint <= 255; yZeroPoint += 51) {
...
TEST(Q8AVGPOOL_UP16x9__VSX, kc_div_16_with_y_max) {
  TEST_REQUIRES_VSX;
  for (size_t n = 1; n <= 5; n += 2) {
    for (size_t kc = 16; kc < 128; kc += 24) {
      AvgPoolMicrokernelTester()
...
TEST(Q8AVGPOOL_UP16x9__VSX, kc_div_16_with_y_min) {
  TEST_REQUIRES_VSX;
  for (size_t n = 1; n <= 5; n += 2) {
    for (size_t kc = 16; kc < 128; kc += 24) {
      AvgPoolMicrokernelTester()
...
TEST(Q8AVGPOOL_UP16x9__VSX, small_n) {
  TEST_REQUIRES_VSX;
  for (size_t n = 2; n < 5; n++) {
    for (size_t ks : std::vector<size_t>{{2, 3}}) {
      for (size_t kc = 16; kc < 35; kc += 5) {
        AvgPoolMicrokernelTester().kr(16).mr(9).n(n).kh(ks).kw(ks).kc(kc).test(
            pytorch_q8avgpool_ukernel_up16x9__vsx);
      }
    }
  }
...
TEST(Q8AVGPOOL_UP16x9__VSX, small_n_with_x_stride) {
  TEST_REQUIRES_VSX;
  for (size_t n = 2; n < 5; n++) {
    for (size_t ks : std::vector<size_t>{{2, 3}}) {
      for (size_t kc = 16; kc < 35; kc += 5) {
...
TEST(Q8AVGPOOL_UP16x9__VSX, small_n_with_y_stride) {
  TEST_REQUIRES_VSX;
  for (size_t n = 2; n < 5; n++) {
    for (size_t ks : std::vector<size_t>{{2, 3}}) {
      for (size_t kc = 16; kc < 35; kc += 5) {
...
TEST(Q8AVGPOOL_UP16x9__VSX, small_n_with_s) {
  TEST_REQUIRES_VSX;
  for (size_t n = 2; n < 5; n++) {
    for (size_t ks : std::vector<size_t>{{2, 3}}) {
      for (size_t kc = 16; kc < 35; kc += 5) {
