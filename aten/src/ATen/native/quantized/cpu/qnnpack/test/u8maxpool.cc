...
TEST(U8MAXPOOL_SUB16__VSX, kc_lt_16_mx1_pool) {
  TEST_REQUIRES_VSX;
  for (size_t kc = 1; kc < 16; kc++) {
    for (size_t ks = 2; ks < 16; ks++) {
      MaxPoolMicrokernelTester().kr(16).kh(ks).kw(1).kc(kc).test(
          pytorch_u8maxpool_ukernel_sub16__vsx);
    }
  }
}

TEST(U8MAXPOOL_SUB16__VSX, kc_lt_16_mx1_pool_with_qmin) {
  TEST_REQUIRES_VSX;
  for (size_t kc = 1; kc < 16; kc++) {
    for (size_t ks = 2; ks < 16; ks++) {
      MaxPoolMicrokernelTester().kr(16).kh(ks).kw(1).kc(kc).qmin(192).test(
...
TEST(U8MAXPOOL_SUB16__VSX, kc_lt_16_mx1_pool_with_qmax) {
  TEST_REQUIRES_VSX;
  for (size_t kc = 1; kc < 16; kc++) {
    for (size_t ks = 2; ks < 16; ks++) {
      MaxPoolMicrokernelTester().kr(16).kh(ks).kw(1).kc(kc).qmax(192).test(
...
TEST(U8MAXPOOL_SUB16__VSX, kc_lt_16_1xm_pool) {
  TEST_REQUIRES_VSX;
  for (size_t kc = 1; kc < 16; kc++) {
    for (size_t ks = 2; ks < 16; ks++) {
      MaxPoolMicrokernelTester().kr(16).kh(1).kw(ks).kc(kc).test(
...
TEST(U8MAXPOOL_SUB16__VSX, kc_lt_16_1xm_pool_with_qmin) {
  TEST_REQUIRES_VSX;
  for (size_t kc = 1; kc < 16; kc++) {
    for (size_t ks = 2; ks < 16; ks++) {
      MaxPoolMicrokernelTester().kr(16).kh(1).kw(ks).kc(kc).qmin(192).test(
...
TEST(U8MAXPOOL_SUB16__VSX, kc_lt_16_1xm_pool_with_qmax) {
  TEST_REQUIRES_VSX;
  for (size_t kc = 1; kc < 16; kc++) {
    for (size_t ks = 2; ks < 16; ks++) {
      MaxPoolMicrokernelTester().kr(16).kh(1).kw(ks).kc(kc).qmax(192).test(
...
TEST(U8MAXPOOL_SUB16__VSX, kc_lt_16_small_n) {
  TEST_REQUIRES_VSX;
  for (size_t n = 2; n < 5; n++) {
    for (size_t ks : std::vector<size_t>{{2, 3, 5}}) {
      for (size_t kc = 1; kc < 16; kc++) {
        MaxPoolMicrokernelTester()
            ...
            .kc(kc)
            .iterations(3)
            .test(pytorch_u8maxpool_ukernel_sub16__vsx);
      }
    }
  }
}

TEST(U8MAXPOOL_SUB16__VSX, kc_lt_16_small_n_with_x_stride) {
  TEST_REQUIRES_VSX;
  for (size_t n = 2; n < 5; n++) {
    for (size_t ks : std::vector<size_t>{{2, 3, 5}}) {
      for (size_t kc = 1; kc < 16; kc++) {
...
TEST(U8MAXPOOL_SUB16__VSX, kc_lt_16_small_n_with_s) {
  TEST_REQUIRES_VSX;
  for (size_t n = 2; n < 5; n++) {
    for (size_t ks : std::vector<size_t>{{2, 3, 5}}) {
      for (size_t s = 2; s <= 5; s++) {
...
TEST(U8MAXPOOL_SUB16__VSX, kc_lt_16_small_n_with_qmin) {
  TEST_REQUIRES_VSX;
  for (size_t n = 2; n < 5; n++) {
    for (size_t ks : std::vector<size_t>{{2, 3, 5}}) {
      for (size_t kc = 1; kc < 16; kc++) {
...
TEST(U8MAXPOOL_SUB16__VSX, kc_lt_16_small_n_with_qmax) {
  TEST_REQUIRES_VSX;
  for (size_t n = 2; n < 5; n++) {
    for (size_t ks : std::vector<size_t>{{2, 3, 5}}) {
      for (size_t kc = 1; kc < 16; kc++) {
...
TEST(U8MAXPOOL_16x9P8Q__VSX, kc_eq_16_unipass_fulltile) {
  TEST_REQUIRES_VSX;
  auto tester = MaxPoolMicrokernelTester().kr(16).mr(9).kc(16);
  for (size_t kh = 1; kh <= tester.mr(); kh++) {
    for (size_t kw = 1; kw <= tester.mr(); kw++) {
      if (kh * kw == tester.mr()) {
        tester.kh(kh).kw(kw).test(pytorch_u8maxpool_ukernel_16x9p8q__vsx);
      }
    }
  }
}

TEST(U8MAXPOOL_16x9P8Q__VSX, kc_eq_16_unipass_fulltile_with_qmin) {
  TEST_REQUIRES_VSX;
  auto tester = MaxPoolMicrokernelTester().kr(16).mr(9).kc(16);
  for (size_t kh = 1; kh <= tester.mr(); kh++) {
    for (size_t kw = 1; kw <= tester.mr(); kw++) {
      if (kh * kw == tester.mr()) {
        tester.kh(kh).kw(kw).qmin(192).test(
            pytorch_u8maxpool_ukernel_16x9p8q__vsx);
      }
    }
  }
}

TEST(U8MAXPOOL_16x9P8Q__VSX, kc_eq_16_unipass_fulltile_with_qmax) {
  TEST_REQUIRES_VSX;
  auto tester = MaxPoolMicrokernelTester().kr(16).mr(9).kc(16);
  for (size_t kh = 1; kh <= tester.mr(); kh++) {
    for (size_t kw = 1; kw <= tester.mr(); kw++) {
      if (kh * kw == tester.mr()) {
        tester.kh(kh).kw(kw).qmax(192).test(
            pytorch_u8maxpool_ukernel_16x9p8q__vsx);
      }
    }
  }
...
TEST(U8MAXPOOL_16x9P8Q__VSX, kc_eq_16_unipass_subtile) {
  TEST_REQUIRES_VSX;
  auto tester = MaxPoolMicrokernelTester().kr(16).mr(9).kc(16);
  for (size_t ks = 2; ks < tester.mr(); ks++) {
    tester.kh(ks).kw(1).test(pytorch_u8maxpool_ukernel_16x9p8q__vsx);
    tester.kh(1).kw(ks).test(pytorch_u8maxpool_ukernel_16x9p8q__vsx);
  }
}

TEST(U8MAXPOOL_16x9P8Q__VSX, kc_div_16_unipass_fulltile) {
  TEST_REQUIRES_VSX;
  auto tester = MaxPoolMicrokernelTester().kr(16).mr(9);
  for (size_t kh = 1; kh <= tester.mr(); kh++) {
    for (size_t kw = 1; kw <= tester.mr(); kw++) {
...
TEST(U8MAXPOOL_16x9P8Q__VSX, kc_div_16_unipass_fulltile_with_qmin) {
  TEST_REQUIRES_VSX;
  auto tester = MaxPoolMicrokernelTester().kr(16).mr(9);
  for (size_t kh = 1; kh <= tester.mr(); kh++) {
    for (size_t kw = 1; kw <= tester.mr(); kw++) {
...
TEST(U8MAXPOOL_16x9P8Q__VSX, kc_div_16_unipass_fulltile_with_qmax) {
  TEST_REQUIRES_VSX;
  auto tester = MaxPoolMicrokernelTester().kr(16).mr(9);
  for (size_t kh = 1; kh <= tester.mr(); kh++) {
    for (size_t kw = 1; kw <= tester.mr(); kw++) {
...
TEST(U8MAXPOOL_16x9P8Q__VSX, kc_div_16_unipass_fulltile_with_x_stride) {
  TEST_REQUIRES_VSX;
  auto tester = MaxPoolMicrokernelTester().kr(16).mr(9).iterations(3);
  for (size_t kh = 1; kh <= tester.mr(); kh++) {
    for (size_t kw = 1; kw <= tester.mr(); kw++) {
...
TEST(U8MAXPOOL_16x9P8Q__VSX, kc_div_16_unipass_subtile) {
  TEST_REQUIRES_VSX;
  auto tester = MaxPoolMicrokernelTester().kr(16).mr(9).iterations(3);
  for (size_t ks = 2; ks < tester.mr(); ks++) {
    for (size_t kc = 16; kc < 256; kc += 48) {
      tester.kh(ks).kw(1).kc(kc).test(pytorch_u8maxpool_ukernel_16x9p8q__vsx);
      tester.kh(1).kw(ks).kc(kc).test(pytorch_u8maxpool_ukernel_16x9p8q__vsx);
    }
  }
}

TEST(U8MAXPOOL_16x9P8Q__VSX, kc_gt_16_unipass_fulltile) {
  TEST_REQUIRES_VSX;
  auto tester = MaxPoolMicrokernelTester().kr(16).mr(9);
  for (size_t kh = 1; kh <= tester.mr(); kh++) {
    for (size_t kw = 1; kw <= tester.mr(); kw++) {
...
TEST(U8MAXPOOL_16x9P8Q__VSX, kc_gt_16_unipass_fulltile_with_qmin) {
  TEST_REQUIRES_VSX;
  auto tester = MaxPoolMicrokernelTester().kr(16).mr(9);
  for (size_t kh = 1; kh <= tester.mr(); kh++) {
    for (size_t kw = 1; kw <= tester.mr(); kw++) {
...
TEST(U8MAXPOOL_16x9P8Q__VSX, kc_gt_16_unipass_fulltile_with_qmax) {
  TEST_REQUIRES_VSX;
  auto tester = MaxPoolMicrokernelTester().kr(16).mr(9);
  for (size_t kh = 1; kh <= tester.mr(); kh++) {
    for (size_t kw = 1; kw <= tester.mr(); kw++) {
...
TEST(U8MAXPOOL_16x9P8Q__VSX, kc_gt_16_unipass_fulltile_with_x_stride) {
  TEST_REQUIRES_VSX;
  auto tester = MaxPoolMicrokernelTester().kr(16).mr(9).iterations(3);
  for (size_t kh = 1; kh <= tester.mr(); kh++) {
    for (size_t kw = 1; kw <= tester.mr(); kw++) {
...
TEST(U8MAXPOOL_16x9P8Q__VSX, kc_gt_16_unipass_subtile) {
  TEST_REQUIRES_VSX;
  auto tester = MaxPoolMicrokernelTester().kr(16).mr(9).iterations(3);
  for (size_t ks = 2; ks < tester.mr(); ks++) {
    for (size_t kc = 17; kc < 32; kc++) {
...
TEST(U8MAXPOOL_16x9P8Q__VSX, kc_eq_16_twopass_fulltile) {
  TEST_REQUIRES_VSX;
  auto tester = MaxPoolMicrokernelTester().kr(16).mr(9).qr(8).kc(16);
  for (size_t kh = 1; kh <= tester.mr() + tester.qr(); kh++) {
    for (size_t kw = 1; kw <= tester.mr() + tester.qr(); kw++) {
...
TEST(U8MAXPOOL_16x9P8Q__VSX, kc_eq_16_twopass_fulltile_with_qmin) {
  TEST_REQUIRES_VSX;
  auto tester = MaxPoolMicrokernelTester().kr(16).mr(9).qr(8).kc(16);
  for (size_t kh = 1; kh <= tester.mr() + tester.qr(); kh++) {
    for (size_t kw = 1; kw <= tester.mr() + tester.qr(); kw++) {
...
TEST(U8MAXPOOL_16x9P8Q__VSX, kc_eq_16_twopass_fulltile_with_qmax) {
  TEST_REQUIRES_VSX;
  auto tester = MaxPoolMicrokernelTester().kr(16).mr(9).qr(8).kc(16);
  for (size_t kh = 1; kh <= tester.mr() + tester.qr(); kh++) {
    for (size_t kw = 1; kw <= tester.mr() + tester.qr(); kw++) {
...
TEST(U8MAXPOOL_16x9P8Q__VSX, kc_eq_16_twopass_subtile) {
  TEST_REQUIRES_VSX;
  auto tester = MaxPoolMicrokernelTester().kr(16).mr(9).qr(8).kc(16);
  for (size_t ks = tester.mr() + 1; ks < tester.mr() + tester.qr(); ks++) {
    tester.kh(ks).kw(1).test(pytorch_u8maxpool_ukernel_16x9p8q__vsx);
...
TEST(U8MAXPOOL_16x9P8Q__VSX, kc_div_16_twopass_fulltile) {
  TEST_REQUIRES_VSX;
  auto tester = MaxPoolMicrokernelTester().kr(16).mr(9).qr(8);
  for (size_t kh = 1; kh <= tester.mr() + tester.qr(); kh++) {
    for (size_t kw = 1; kw <= tester.mr() + tester.qr(); kw++) {
...
TEST(U8MAXPOOL_16x9P8Q__VSX, kc_div_16_twopass_fulltile_with_qmin) {
  TEST_REQUIRES_VSX;
  auto tester = MaxPoolMicrokernelTester().kr(16).mr(9).qr(8);
  for (size_t kh = 1; kh <= tester.mr() + tester.qr(); kh++) {
    for (size_t kw = 1; kw <= tester.mr() + tester.qr(); kw++) {
...
TEST(U8MAXPOOL_16x9P8Q__VSX, kc_div_16_twopass_fulltile_with_qmax) {
  TEST_REQUIRES_VSX;
  auto tester = MaxPoolMicrokernelTester().kr(16).mr(9).qr(8);
  for (size_t kh = 1; kh <= tester.mr() + tester.qr(); kh++) {
    for (size_t kw = 1; kw <= tester.mr() + tester.qr(); kw++) {
...
TEST(U8MAXPOOL_16x9P8Q__VSX, kc_div_16_twopass_fulltile_with_x_stride) {
  TEST_REQUIRES_VSX;
  auto tester = MaxPoolMicrokernelTester().kr(16).mr(9).qr(8).iterations(3);
  for (size_t kh = 1; kh <= tester.mr() + tester.qr(); kh++) {
    for (size_t kw = 1; kw <= tester.mr() + tester.qr(); kw++) {
...
TEST(U8MAXPOOL_16x9P8Q__VSX, kc_div_16_twopass_subtile) {
  TEST_REQUIRES_VSX;
  auto tester = MaxPoolMicrokernelTester().kr(16).mr(9).qr(8).iterations(3);
  for (size_t ks = tester.mr() + 1; ks < tester.mr() + tester.qr(); ks++) {
    for (size_t kc = 16; kc < 256; kc += 48) {
...
TEST(U8MAXPOOL_16x9P8Q__VSX, kc_gt_16_twopass_fulltile) {
  TEST_REQUIRES_VSX;
  auto tester = MaxPoolMicrokernelTester().kr(16).mr(9).qr(8);
  for (size_t kh = 1; kh <= tester.mr() + tester.qr(); kh++) {
    for (size_t kw = 1; kw <= tester.mr() + tester.qr(); kw++) {
...
TEST(U8MAXPOOL_16x9P8Q__VSX, kc_gt_16_twopass_fulltile_with_qmin) {
  TEST_REQUIRES_VSX;
  auto tester = MaxPoolMicrokernelTester().kr(16).mr(9).qr(8);
  for (size_t kh = 1; kh <= tester.mr() + tester.qr(); kh++) {
    for (size_t kw = 1; kw <= tester.mr() + tester.qr(); kw++) {
...
TEST(U8MAXPOOL_16x9P8Q__VSX, kc_gt_16_twopass_fulltile_with_qmax) {
  TEST_REQUIRES_VSX;
  auto tester = MaxPoolMicrokernelTester().kr(16).mr(9).qr(8);
  for (size_t kh = 1; kh <= tester.mr() + tester.qr(); kh++) {
    for (size_t kw = 1; kw <= tester.mr() + tester.qr(); kw++) {
...
TEST(U8MAXPOOL_16x9P8Q__VSX, kc_gt_16_twopass_fulltile_with_x_stride) {
  TEST_REQUIRES_VSX;
  auto tester = MaxPoolMicrokernelTester().kr(16).mr(9).qr(8).iterations(3);
  for (size_t kh = 1; kh <= tester.mr() + tester.qr(); kh++) {
    for (size_t kw = 1; kw <= tester.mr() + tester.qr(); kw++) {
...
TEST(U8MAXPOOL_16x9P8Q__VSX, kc_gt_16_twopass_subtile) {
  TEST_REQUIRES_VSX;
  auto tester = MaxPoolMicrokernelTester().kr(16).mr(9).qr(8).iterations(3);
  for (size_t ks = tester.mr() + 1; ks < tester.mr() + tester.qr(); ks++) {
    for (size_t kc = 17; kc < 32; kc++) {
...
TEST(U8MAXPOOL_16x9P8Q__VSX, kc_eq_16_multipass) {
  TEST_REQUIRES_VSX;
  auto tester = MaxPoolMicrokernelTester().kr(16).mr(9).qr(8).kc(16);
  for (size_t ks = tester.mr() + tester.qr() + 1;
       ks < tester.mr() + 3 * tester.qr();
...
TEST(U8MAXPOOL_16x9P8Q__VSX, kc_eq_16_multipass_with_qmin) {
  TEST_REQUIRES_VSX;
  auto tester = MaxPoolMicrokernelTester().kr(16).mr(9).qr(8).kc(16);
  for (size_t ks = tester.mr() + tester.qr() + 1;
       ks < tester.mr() + 3 * tester.qr();
       ks += 3) {
    tester.kh(ks).kw(1).qmin(192).test(pytorch_u8maxpool_ukernel_16x9p8q__vsx);
    tester.kh(1).kw(ks).qmin(192).test(pytorch_u8maxpool_ukernel_16x9p8q__vsx);
  }
}

TEST(U8MAXPOOL_16x9P8Q__VSX, kc_eq_16_multipass_with_qmax) {
  TEST_REQUIRES_VSX;
  auto tester = MaxPoolMicrokernelTester().kr(16).mr(9).qr(8).kc(16);
  for (size_t ks = tester.mr() + tester.qr() + 1;
       ks < tester.mr() + 3 * tester.qr();
       ks += 3) {
    tester.kh(ks).kw(1).qmax(192).test(pytorch_u8maxpool_ukernel_16x9p8q__vsx);
    tester.kh(1).kw(ks).qmax(192).test(pytorch_u8maxpool_ukernel_16x9p8q__vsx);
  }
}

TEST(U8MAXPOOL_16x9P8Q__VSX, kc_div_16_multipass) {
  TEST_REQUIRES_VSX;
  auto tester = MaxPoolMicrokernelTester().kr(16).mr(9).qr(8);
  for (size_t ks = tester.mr() + tester.qr() + 1;
       ks < tester.mr() + 3 * tester.qr();
...
TEST(U8MAXPOOL_16x9P8Q__VSX, kc_div_16_multipass_with_qmin) {
  TEST_REQUIRES_VSX;
  auto tester = MaxPoolMicrokernelTester().kr(16).mr(9).qr(8);
  for (size_t ks = tester.mr() + tester.qr() + 1;
       ks < tester.mr() + 3 * tester.qr();
...
TEST(U8MAXPOOL_16x9P8Q__VSX, kc_div_16_multipass_with_qmax) {
  TEST_REQUIRES_VSX;
  auto tester = MaxPoolMicrokernelTester().kr(16).mr(9).qr(8);
  for (size_t ks = tester.mr() + tester.qr() + 1;
       ks < tester.mr() + 3 * tester.qr();
...
TEST(U8MAXPOOL_16x9P8Q__VSX, kc_div_16_multipass_with_x_stride) {
  TEST_REQUIRES_VSX;
  auto tester = MaxPoolMicrokernelTester().kr(16).mr(9).qr(8).iterations(3);
  for (size_t ks = tester.mr() + tester.qr() + 1;
       ks < tester.mr() + 3 * tester.qr();
...
TEST(U8MAXPOOL_16x9P8Q__VSX, kc_gt_16_multipass) {
  TEST_REQUIRES_VSX;
  auto tester = MaxPoolMicrokernelTester().kr(16).mr(9).qr(8).iterations(3);
  for (size_t ks = tester.mr() + tester.qr() + 1;
       ks < tester.mr() + 3 * tester.qr();
...
TEST(U8MAXPOOL_16x9P8Q__VSX, kc_gt_16_multipass_with_qmin) {
  TEST_REQUIRES_VSX;
  auto tester = MaxPoolMicrokernelTester().kr(16).mr(9).qr(8).iterations(3);
  for (size_t ks = tester.mr() + tester.qr() + 1;
       ks < tester.mr() + 3 * tester.qr();
...
TEST(U8MAXPOOL_16x9P8Q__VSX, kc_gt_16_multipass_with_qmax) {
  TEST_REQUIRES_VSX;
  auto tester = MaxPoolMicrokernelTester().kr(16).mr(9).qr(8).iterations(3);
  for (size_t ks = tester.mr() + tester.qr() + 1;
       ks < tester.mr() + 3 * tester.qr();
...
TEST(U8MAXPOOL_16x9P8Q__VSX, kc_gt_16_multipass_with_x_stride) {
  TEST_REQUIRES_VSX;
  auto tester = MaxPoolMicrokernelTester().kr(16).mr(9).qr(8).iterations(3);
  for (size_t ks = tester.mr() + tester.qr() + 1;
       ks < tester.mr() + 3 * tester.qr();
...
TEST(U8MAXPOOL_16x9P8Q__VSX, small_n) {
  TEST_REQUIRES_VSX;
  for (size_t n = 2; n < 5; n++) {
    for (size_t ks : std::vector<size_t>{{2, 3, 5, 10}}) {
      for (size_t kc = 16; kc < 51; kc += 5) {
        MaxPoolMicrokernelTester()
            ...
            .kc(kc)
            .iterations(3)
            .test(pytorch_u8maxpool_ukernel_16x9p8q__vsx);
      }
    }
  }
}

TEST(U8MAXPOOL_16x9P8Q__VSX, small_n_with_x_stride) {
  TEST_REQUIRES_VSX;
  for (size_t n = 2; n < 5; n++) {
    for (size_t ks : std::vector<size_t>{{2, 3, 5, 10}}) {
      for (size_t kc = 16; kc < 51; kc += 5) {
...
TEST(U8MAXPOOL_16x9P8Q__VSX, small_n_with_y_stride) {
  TEST_REQUIRES_VSX;
  for (size_t n = 2; n < 5; n++) {
    for (size_t ks : std::vector<size_t>{{2, 3, 5, 10}}) {
      for (size_t kc = 16; kc < 51; kc += 5) {
...
TEST(U8MAXPOOL_16x9P8Q__VSX, small_n_with_s) {
  TEST_REQUIRES_VSX;
  for (size_t n = 2; n < 5; n++) {
    for (size_t ks : std::vector<size_t>{{2, 3, 5}}) {
      for (size_t kc = 16; kc < 51; kc += 5) {
