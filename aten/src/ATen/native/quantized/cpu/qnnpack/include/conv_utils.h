...
struct conv_param_t {
  ...
  const std::array<uint32_t, 4>
      padding; // input padding top, left, bottom, right
  ...
  enum pytorch_qnnp_ukernel_type
      ukernel_type; // kernel type based on input params
   ...
   */
  conv_param_t(
      const std::array<uint32_t, 2> kernel_,
    ...
    if (stride_dims[1] > kernel_height) {
      pytorch_qnnp_log_info(
          "inefficiency in %s with %" PRIu32 "x%" PRIu32 " kernel and %" PRIu32
          "x%" PRIu32
          " subsampling: "
    ...
    if (transpose) {
      ...
      const bool any_padding =
          (input_padding_left | input_padding_top | input_padding_right |
      ...
      if ((kernel_size == 9 || kernel_size == 25) &&
        ...
        ukernel_type = pytorch_qnnp_ukernel_type_dwconv;
          ...
          kernel_size == 1 && stride_dims[1] == 1 && stride_dims[0] == 1 &&
          !any_padding) {
  ...
  std::array<size_t, 2> compute_output_dims(
    ...
    output_dims[0] = compute_output_dimension(
        input_dims[0], // width
        padding[1] + padding[3],
        adjustment_dims[0],
        kernel_dims[0],
        ...
        stride_dims[0],
        transpose);
    output_dims[1] = compute_output_dimension(
        input_dims[1], // height
        padding[0] + padding[2],
        adjustment_dims[1],
        kernel_dims[1],
