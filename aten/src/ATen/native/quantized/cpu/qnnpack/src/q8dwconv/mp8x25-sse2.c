...
void pytorch_q8dwconv_ukernel_mp8x25__sse2(
  ...
  const __m128i vkernel_zero_point =
      _mm_set1_epi16(quantization_params->sse2.kernel_zero_points[0]);
  ...
  do {
    ...
    }
    {
      ...
      for (; c >= 8; c -= 8) {
        ...
        vacc_lo =
            _mm_cvtps_epi32(_mm_mul_ps(_mm_cvtepi32_ps(vacc_lo), vmultiplier));
        vacc_hi =
            _mm_cvtps_epi32(_mm_mul_ps(_mm_cvtepi32_ps(vacc_hi), vmultiplier));
