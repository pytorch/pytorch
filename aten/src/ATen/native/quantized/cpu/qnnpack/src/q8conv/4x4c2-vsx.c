...
void pytorch_q8conv_ukernel_4x4c2__vsx(
  ...
  do {
    ...
    size_t k = kc;
    for (; k >= 16; k -= 16) {
      /* Load 4 vectors with 16 input channels each, subtract the input zero
       * point and convert the 4x int8 vectors into 8x int16 vectors */
      ...
      /* Load filters, subtract the weight zero point, and accumulate the
       * product of the first 8 input channels */
      ...
      /* Load filters, subtract the weight zero point, and accumulate the
       * product of the last 8 input channels */
  ...
  vout = vec_min(vout, vec_splats(quantization_params->vsx.output_max));
  vout = vec_max(vout, vec_splats(quantization_params->vsx.output_min));
