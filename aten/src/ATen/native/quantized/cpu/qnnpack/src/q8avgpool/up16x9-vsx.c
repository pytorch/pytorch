...
void pytorch_q8avgpool_ukernel_up16x9__vsx(
  ...
  do {
    ...
    /* Iterate over 16 channels, computing a window for each channel. For
     * windows smaller than 9, zeros are loaded instead to accumulate them into
     * the accumulators */
    size_t k = kc;
    while (k >= 16) {
      ...
      /* Pack the accumulators into a uint8 vector, check ranges (min/max), and
       * store the result */
