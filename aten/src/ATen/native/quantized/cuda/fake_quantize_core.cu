...
void fake_quantize_tensor_cachemask_kernel_cuda(
  ...
  AT_DISPATCH_FLOATING_TYPES_AND_HALF(
      input.scalar_type(), "fake_quantize_tensor_cachemask_kernel_types", [&] {
        gpu_kernel_multiple_outputs(
            iter,
            [=] GPU_LAMBDA(
                scalar_t input_val) -> thrust::tuple<scalar_t, bool> {
...
void _fake_quantize_grad_learnable_tensor_kernel_cuda(
  ...
  gpu_kernel_multiple_outputs(
      iter,
      [=] GPU_LAMBDA(
          float XInput, float dYInput) -> thrust::tuple<float, float, float> {
        ...
        float xfq = static_cast<float>(
            (std::max(std::min(xq, quant_max), quant_min) - zero_point) *
            scale);
...
void fake_quant_per_channel_cachemask_cuda(
  ...
  AT_DISPATCH_FLOATING_TYPES_AND_HALF(
      iter.dtype(),
      "fake_quantize_channel_cachemask_cuda_mask_type_handling",
      [&] {
        // write mask
        gpu_kernel(
            iter_mask,
        ...
        gpu_kernel(
            iter,
            [=] GPU_LAMBDA(scalar_t input_val, float scale, int64_t zero_point)
                -> scalar_t {
              float inv_scale = 1.0f / scale;
              return (fminf(
                          quant_max,
...
void _fake_quantize_grad_learnable_channel_kernel_cuda(
    TensorIterator& iter,
    int64_t quant_min,
    int64_t quant_max,
    float grad_factor) {
  gpu_kernel_multiple_outputs(
      ...
      [=] GPU_LAMBDA(
          ...
          float dy_input,
          float scale_input,
          float zero_point_input) -> thrust::tuple<float, float, float> {
        ...
        float xfqi = static_cast<float>(
            (std::max(std::min(xqi, quant_max), quant_min) - zero_point_input) *
            scale_input);
