...
Args:
  ...
  compute_mask: if True, a mask is computed for the backward pass

Returns:
  Quantized tensor (double dtype).
  Mask (bool dtype).
*/
std::tuple<Tensor, Tensor> fake_quantize_per_tensor_affine(
    const Tensor& self,
    double scale,
    int64_t zero_point,
    int64_t quant_min,
  ...
  fake_quant_tensor_stub(
      self.device().type(), Y, mask, self, scale, zero_point, quant_min,
      quant_max, compute_mask);
...
Tensor fake_quantize_per_tensor_affine_backward(
  ...
  TORCH_CHECK(compute_mask, "compute_mask must be True to use fake_quantize_per_tensor_affine_backward");
  if (dY.numel() <= 0) {
    return dY;
  }
...
Tensor _fake_quantize_learnable_per_tensor_affine(
  ...
  return std::get<0>(native::fake_quantize_per_tensor_affine(
    self, scale_val, zero_point_val, quant_min, quant_max, compute_mask));
}
