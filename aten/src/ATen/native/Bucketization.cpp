 ...
 *   sorted_sequence - N*D or 1D (apply to all values) tensor containing sorted
 * sequences in last dimension values          - N*D tensor or a Scalar (when
 * sorted_sequence is 1D) containing the search values right           -
 * corresponding to lower bound if False and upper bound if True out_int32 - the
 * output tensor is int64_t type if False and int(32bit normally) type if True.
 *
...
// the end of boundary std::lower_bound can not be used here since its
// customized comparator need strict weak ordering
template <typename input_t>
const input_t* cus_lower_bound(
    const input_t* start,
    const input_t* end,
    input_t val) {
  while (start < end) {
