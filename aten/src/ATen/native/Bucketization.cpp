...
/* Implement a numpy like searchsorted and a TF like bucketize function running
 * on cpu
 *
 * - torch.searchsorted(sorted_sequence, values, right=False, side='left',
 * out_int32=False, sorter=None) sorted_sequence - N*D or 1D (apply to all
 * values) tensor containing sorted sequences in last dimension values - N*D
 * tensor or a Scalar (when sorted_sequence is 1D) containing the search values
 *   right           - corresponding to lower bound if False and upper bound if
 * True side            - (preferred to right) corresponding to lower bound if
 * 'left' and upper bound if 'right' out_int32       - the output tensor is
 * int64_t type if False and int(32bit normally) type if True. sorter          -
 * if provided, sorted_sequence may not be sorted and the sorted order is given
 * by this tensor
 *
 ...
 *   out_int32  - the output tensor is int64_t type if False and int(32bit
...
void searchsorted_cpu_contiguous(
    Tensor& result,
    const Tensor& input,
  ...
  at::parallel_for(
      0, numel_in, SEARCHSORTED_GRAIN_SIZE, [&](int64_t start, int64_t end) {
        for (const auto i : c10::irange(start, end)) {
          ...
          int64_t pos = !right
              ? cus_lower_bound(
                    start_bd, end_bd, data_in[i], data_bd, data_st) -
                  start_bd
              : cus_upper_bound(
                    start_bd, end_bd, data_in[i], data_bd, data_st) -
                  start_bd;
...
Tensor searchsorted_cpu(
  ...
  at::native::searchsorted_out_cpu(
