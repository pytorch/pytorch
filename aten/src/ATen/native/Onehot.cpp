...
Tensor one_hot(const Tensor& self, int64_t num_classes) {
  TORCH_CHECK(
      self.dtype() == kLong, "one_hot is only applicable to index tensor.");
  ...
  // using meta bit test to catch Fake Tensor as well until __torch__function
  // defined
  ...
  if (self.device().type() != at::kCUDA) {
    ...
    TORCH_CHECK(
        self.min().item().toLong() >= 0, "Class values must be non-negative.");
  }
  if (num_classes == -1) {
