...
using _upsampling_nearest_exact1d =
    void (*)(const Tensor& output, const Tensor& input, scale_t scales_w);
using upsampling_nearest2d = void (*)(
    ...
    scale_t scales_h,
    scale_t scales_w);
using _upsampling_nearest_exact2d = void (*)(
    const Tensor& output,
    const Tensor& input,
    scale_t scales_h,
    scale_t scales_w);
using upsampling_nearest3d = void (*)(
    ...
    scale_t scales_h,
    scale_t scales_w);
using _upsampling_nearest_exact3d = void (*)(
    const Tensor& output,
    const Tensor& input,
    scale_t scales_d,
    scale_t scales_h,
...
using upsampling_bilinear2d = void (*)(
    ...
    scale_t scales_h,
    scale_t scales_w);
using _upsampling_bilinear2d_aa = void (*)(
    const Tensor& output,
    const Tensor& input,
    bool align_corners,
    scale_t scales_h,
...
using upsampling_bicubic2d = void (*)(
    ...
    scale_t scales_h,
    scale_t scales_w);
using _upsampling_bicubic2d_aa = void (*)(
    const Tensor& output,
    const Tensor& input,
    bool align_corners,
    scale_t scales_h,
...
static C10_UNUSED std::array<int64_t, 4> upsample_2d_common_check(
    IntArrayRef input_size,
    IntArrayRef output_size) {
  TORCH_CHECK(
      output_size.size() == 2,
...
static C10_UNUSED std::array<int64_t, 5> upsample_3d_common_check(
    IntArrayRef input_size,
    IntArrayRef output_size) {
  TORCH_CHECK(
      output_size.size() == 3,
...
static inline int64_t nearest_neighbor_exact_compute_source_index(
  ...
  const int64_t src_index = std::min(
      static_cast<int64_t>(floorf((dst_index + 0.5) * scale)), input_size - 1);
  return src_index;
}
...
static inline int64_t nearest_idx(
  ...
  if (output_size == input_size) {
    ...
    return nearest_neighbor_compute_source_index(
...
static inline int64_t nearest_exact_idx(
    ...
    c10::optional<double> scales) {
  float scale = compute_scales_value<float>(scales, input_size, output_size);
  return nearest_neighbor_exact_compute_source_index(
      scale, output_index, input_size);
}

// Define a typedef to dispatch to nearest_idx or nearest_exact_idx
typedef int64_t (
    *nearest_idx_fn_t)(int64_t, int64_t, int64_t, c10::optional<double>);
