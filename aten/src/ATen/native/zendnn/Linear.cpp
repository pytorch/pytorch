...
inline void zendnn_linear_impl(const at::Tensor &input,
  ...
  TORCH_CHECK(weight_tensor.check(), "tensor creation of ",
                 weight_tensor.get_name(), " failed.");
  tensor_t input_tensor = tensor_t();
  set_zendnn_tensor_attributes(input, input_tensor, "matmul_input", datatype);
  input_tensor.create();
  TORCH_CHECK(input_tensor.check(), "tensor creation of ", input_tensor.get_name(), " failed.");
  ...
  bias_tensor.create();
  TORCH_CHECK(bias_tensor.check(), "tensor creation of ",
                 bias_tensor.get_name(), " failed.");
    set_matmul_context_attributes(matmul_context, weight_tensor, post_op_ids,
                                num_post_ops, bias_tensor);
  }
  else {
    set_matmul_context_attributes(matmul_context, weight_tensor, post_op_ids,
                                num_post_ops);
  ...
  auto matmul_operator = matmul_operator_t()
                             ...
                             .create();
  TORCH_CHECK(matmul_operator.check(), "operator ",
                 matmul_operator.get_name(), " creation failed.");
  // output tensor
  tensor_t output_tensor = tensor_t();
  set_zendnn_tensor_attributes(result, output_tensor, "matmul_output", datatype);
  output_tensor.create();
  TORCH_CHECK(output_tensor.check(), "tensor creation of ",
                 output_tensor.get_name(), " failed.");
...
at::Tensor zendnn_linear(const at::Tensor &input, const at::Tensor &weight,
const std::optional<at::Tensor> &bias, std::string zendnn_op_name) {
// Reshape input tensor to 2D view, handling both contiguous and non-contiguous cases
  auto input_2d_view =
      input.is_contiguous()
...
TORCH_LIBRARY_FRAGMENT(aten, m) {
  m.def(TORCH_SELECTIVE_SCHEMA("zendnn_linear(Tensor input, Tensor weight, Tensor? bias= None, str "
        "zendnn_op_name='zendnn_linear') "
        "-> Tensor"));
}
TORCH_LIBRARY_IMPL(aten, CPU, m) {
  m.impl("aten::zendnn_linear",  TORCH_FN(zendnn_linear));
}
} // namespace at::native
