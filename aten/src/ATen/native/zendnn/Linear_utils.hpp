...
get_matmul_output_sizes(const at::Tensor &tensor1, const at::Tensor &tensor2) {
  ...
  output_size.push_back(tensor2_last_dim_size);
  return output_size;
}
...
check_valid_sizes_for_matmul(const at::Tensor &mat1, const at::Tensor &mat2,
  ...
  TORCH_CHECK(mat1_sizes[mat1_dim - 1] == mat2_sizes[mat1_dim - 2],
                 "Tensor shapes incompatible for matrix multiplication");
  TORCH_CHECK(result.dim() == mat1_dim,
                 "unsupported dims for mat1, mat2 and "
                 "result buffer");
  TORCH_CHECK(result.sizes() ==
  ...
  if (post_op_buffers.size() != 0) {
    ...
    TORCH_CHECK(are_postops_dim_compatible,
                   "unsupported dims for mat1, mat2 and "
                   "post op buffers");
    TORCH_CHECK(are_postops_shape_compatible,
                   "unsupported shapes for mat1, mat2 and "
                   "post op buffers");
  } else {
...
check_valid_dtypes_for_matmul(const at::Tensor &mat1, const at::Tensor &mat2,
  ...
  TORCH_CHECK(are_params_fp32 ^ are_params_bf16,
...
void set_zendnn_tensor_attributes(
    const at::Tensor &at_tensor, tensor_t &zendnn_tensor,
...
void set_matmul_context_attributes(
    matmul_context_t &matmul_context, tensor_t &weights,
    const std::vector<int64_t> &post_op_ids, int &num_post_ops,
