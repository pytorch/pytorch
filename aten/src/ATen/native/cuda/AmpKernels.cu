...
namespace {
// Thin wrapper around
// https://docs.nvidia.com/cuda/cuda-math-api/group__CUDA__MATH__SINGLE.html#group__CUDA__MATH__SINGLE_1g57a3c8313f570282a1a7bcc78743b08e,
...
// - A bare call to "isfinite(val)" inside at::native causes nvcc to prefer the
// unrelated
...
// - Unfortunately, the Cuda math library documentation doesn't say how (or if)
// you can provide a full namespace path
//   to ensure that its version of a particular function is invoked.  It only
//   shows bare (not-namespaced) calls to its routines inside kernel or device
//   functions.
// - "std::isfinite(val)" in the gpu_kernel lambda causes an "unspecified launch
// failure" at runtime with cuda 9 on Windows.
//
// isfinite_ensure_cuda_math, declared at file scope outside the at::native
// region, uses isfinite as math library docs suggest and allows disambiguated
// usage in the lambda within the at::native region. GPU_LAMBDA is defined as
// __host__ __device__ (see Loops.cuh), so I need the __host__ keyword or else
// nvcc complains that "calling a __device__
// function("isfinite_ensure_cuda_math") from a __host__ __device__
// function("operator()") is not allowed."
static __host__ __device__ __forceinline__ int isfinite_ensure_cuda_math(
    float val) {
  return isfinite(val);
}
} // namespace
...
void _amp_non_finite_check_and_unscale_cuda_(
    Tensor& scaled_grad,
    Tensor& found_inf,
    const Tensor& inv_scale) {
  // The only way we reach this function is through
  // _amp_foreach_non_finite_check_and_unscale_cuda_, so no input checks.

  // It's not obvious gpu_kernel always guards onto its argument.  Guarding here
  // just in case.
  ...
  AT_DISPATCH_FLOATING_TYPES_AND_HALF(
      ...
      [&iter, &found_inf, &inv_scale] {
        ...
        gpu_kernel(
            iter,
            [found_inf_ptr,
             inv_scale_ptr] GPU_LAMBDA(scalar_t val_in) -> scalar_t {
...
void _amp_foreach_non_finite_check_and_unscale_cuda_(
  ...
  if (can_use_fast_route(scaled_grads)) {
    ...
    TORCH_CHECK(
        scaled_grads[0].is_cuda(), "scaled_grads must be CUDA tensors.");
    ...
    for (const Tensor& t : scaled_grads) {
      ...
      if (!t.is_non_overlapping_and_dense()) {
        ...
        _amp_non_finite_check_and_unscale_cuda_(
      ...
      [&tensor_lists, &found_inf, &inv_scale] {
        ...
        multi_tensor_apply<1>(
            ...
            UnaryOpFunctor<
                ...
                /* res_arg_index */ 0>(),
            [found_inf_ptr,
             inv_scale_ptr] GPU_LAMBDA(opmath_t val) -> opmath_t {
...
// amp_update_scale_cuda_kernel is launched with a single thread to compute the
// new scale. The scale factor is maintained and updated on the GPU to avoid
// synchronization.
__global__ void amp_update_scale_cuda_kernel(
    float* current_scale,
    int* growth_tracker,
    float* found_inf,
...
// recent consecutive unskipped steps. found_inf:  A one-element cuda float
// tensor. If > 0, indicates that infs/nans were found by the relevant
//             prior _amp_non_finite_check_and_unscale_cuda call, and 0 if no
...
Tensor& _amp_update_scale_cuda_(
  ...
  TORCH_CHECK(
      growth_tracker.is_cuda(), "growth_tracker must be a CUDA tensor.");
