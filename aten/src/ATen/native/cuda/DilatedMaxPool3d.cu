...
__global__ static void max_pool3d_with_indices_single_out_frame(
  ...
  int64_t slice =
      (blockIdx.z + offsetZ) / output.size(1); // output slice/feature
...
void max_pool3d_with_indices_out_frame(
  ...
  while (totalZ > 0) {
    dim3 grid(
        cuda::ATenCeilDiv(owidth, static_cast<int>(block.x)),
...
__global__ static void max_pool3d_with_indices_backward_single_out_frame(
  ...
  int slice =
      (blockIdx.z + offsetZ) / gradOutput.size(1); // output slice/feature

  if (oRow < gradOutput.size(2) && oColumn < gradOutput.size(3)) {
    int maxIndex = indices[slice][oFrame][oRow][oColumn];
    if (maxIndex != -1) {
      gpuAtomicAdd(
          &gradInputData[slice * itime * iheight * iwidth + maxIndex],
...
void max_pool3d_with_indices_out_cuda_template(
  ...
  checkAllSameGPU(
      "max_pool3d_with_indices_out_cuda", {output_arg, indices_arg, input_arg});
  ...
  if (input.ndimension() == 5) {
    ...
    work_indices =
        work_indices.reshape({nbatch * nslices, otime, oheight, owidth});
  }
...
void max_pool3d_with_indices_backward_out_cuda_template(
  ...
  checkAllSameGPU(
      "max_pool3d_with_indices_backward_out_cuda",
