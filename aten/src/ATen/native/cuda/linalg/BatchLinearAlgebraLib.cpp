...
// Some cuBLAS and cuSOLVER batched routines require input to be a device array
...
// rocBLAS does not implement BLAS-like extensions of cuBLAS, they're in
// rocSOLVER rocSOLVER is currently not used in ATen, therefore we raise an
// error in this case
#ifndef CUDART_VERSION
  TORCH_CHECK(
      false, "geqrf: Batched version is supported only with cuBLAS backend.")
#else
  ...
  // cuBLAS batched geqrf requires input to be the device array of pointers to
  ...
  auto input_ptr_array_data =
      reinterpret_cast<scalar_t**>(input_ptr_array.data_ptr());
  auto tau_ptr_array_data =
      reinterpret_cast<scalar_t**>(tau_ptr_array.data_ptr());
...
void geqrf_batched_cublas(const Tensor& input, const Tensor& tau) {
  AT_DISPATCH_FLOATING_AND_COMPLEX_TYPES(
      input.scalar_type(), "geqrf_batched_cuda", [&] {
        apply_geqrf_batched<scalar_t>(input, tau);
      });
}

template <typename scalar_t>
static void apply_lu_factor_batched_cublas(
    const Tensor& A,
    const Tensor& pivots,
    const Tensor& infos,
    bool get_pivots) {
  ...
  TORCH_CHECK(
      false,
      "linalg.lu_factor: cuBLAS backend for linalg.lu_factor is not available.")
#else
  ...
  at::cuda::blas::getrfBatched(
      n, a_ptr_array_data, lda, pivots_data, infos_data, batch_size);
#endif
}
...
void lu_factor_batched_cublas(
    ...
    bool get_pivots) {
  AT_DISPATCH_FLOATING_AND_COMPLEX_TYPES(
      A.scalar_type(), "lu_factor_cublas", [&] {
        apply_lu_factor_batched_cublas<scalar_t>(A, pivots, infos, get_pivots);
      });
}

template <typename scalar_t>
static void apply_lu_solve_batched_cublas(
    const Tensor& LU,
    const Tensor& pivots,
    const Tensor& B,
  ...
  TORCH_CHECK(
      false,
      "linalg.lu_solve: cuBLAS backend for linalg.lu_solve is not available.")
#else
  TORCH_INTERNAL_ASSERT(
      batchCount(LU) == batchCount(B),
  ...
  auto lu_ptr_array_data =
      reinterpret_cast<scalar_t**>(lu_ptr_array.data_ptr());
...
void lu_solve_batched_cublas(
    ...
    TransposeType trans) {
  AT_DISPATCH_FLOATING_AND_COMPLEX_TYPES(
      LU.scalar_type(), "lu_solve_cublas", [&] {
...
void apply_ldl_solve_cusolver(
    ...
    bool upper) {
#if !(                                                      \
    defined(CUDART_VERSION) && defined(CUSOLVER_VERSION) && \
...
// error in this case
  ...
  TORCH_CHECK(
      false,
      "torch.linalg.lstsq: Batched version is supported only with cuBLAS backend.")
#else
  ...
  // Matrices Aarray[i] should not overlap; otherwise, undefined behavior is
  // expected. explicitly broadcast the batch dimensions of A
  IntArrayRef A_batch_sizes(A.sizes().data(), A.dim() - 2);
  IntArrayRef B_batch_sizes(B.sizes().data(), B.dim() - 2);
  std::vector<int64_t> expand_batch_portion =
      at::infer_size(A_batch_sizes, B_batch_sizes);
  expand_batch_portion.insert(
      expand_batch_portion.end(), {A.size(-2), A.size(-1)});
  Tensor A_expanded = A.expand({expand_batch_portion});
  Tensor A_broadcasted = cloneBatchedColumnMajor(A_expanded);

  // cuBLAS batched gels requires input to be the device array of pointers to
  // device single matrices
...
void gels_batched_cublas(const Tensor& a, Tensor& b, Tensor& infos) {
  AT_DISPATCH_FLOATING_AND_COMPLEX_TYPES(
      a.scalar_type(), "gels_batched_cublas", [&] {
...
template <typename scalar_t>
inline static void _apply_single_inverse_helper(
    scalar_t* self_ptr,
    scalar_t* self_inv_ptr,
    int* ipiv_ptr,
    int* info_getrf_ptr,
    int* info_getrs_ptr,
    int n,
  ...
  at::cuda::solver::getrf<scalar_t>(
      handle, n, n, self_ptr, lda, ipiv_ptr, info_getrf_ptr);
...
template <typename scalar_t>
static void apply_batched_inverse_lib(
    Tensor& self,
    Tensor& self_inv,
    Tensor& infos_getrf,
    Tensor& infos_getrs) {
  ...
  // Heuristic: For small batch size or large matrix size, we use for-loop to
  // iterate over the batches instead of
  ...
  if (batch_size <= 8 || /* batch_size > 8 && */ n >= 512) {
    for (int64_t i = 0; i < batch_size; i++) {
      ...
      _apply_single_inverse_helper<scalar_t>(
          ...
          pivot,
          infos_getrf_working_ptr,
          infos_getrs_working_ptr,
          n,
          lda);
    }
  } else {
    ...
    Tensor self_array = at::arange(
        ...
        reinterpret_cast<int64_t>(
            &self_data[(batch_size - 1) * self_mat_stride]) +
            1,
        static_cast<int64_t>(self_mat_stride * sizeof(scalar_t)),
        self.options().dtype(at::kLong));
    Tensor self_inv_array = at::arange(
        reinterpret_cast<int64_t>(self_inv_data),
        reinterpret_cast<int64_t>(
            &self_inv_data[(batch_size - 1) * self_inv_mat_stride]) +
            1,
        static_cast<int64_t>(self_inv_mat_stride * sizeof(scalar_t)),
        self.options().dtype(at::kLong));

    auto dataPtr = allocator.allocate(sizeof(int) * batch_size * lda);
    int* ipiv_array = reinterpret_cast<int*>(dataPtr.get());

    at::cuda::blas::getrfBatched<scalar_t>(
        n,
        reinterpret_cast<scalar_t**>(self_array.data_ptr()),
        lda,
        ipiv_array,
        infos_getrf_data,
        batch_size);

    at::cuda::blas::getriBatched<scalar_t>(
        n,
        reinterpret_cast<scalar_t**>(self_array.data_ptr()),
        lda,
        ipiv_array,
        reinterpret_cast<scalar_t**>(self_inv_array.data_ptr()),
        lda,
        infos_getrs_data,
        batch_size);
  }
...
template <typename scalar_t>
static void apply_single_inverse_lib(
    const Tensor& self,
    Tensor& self_inv,
    Tensor& infos_getrf,
    Tensor& infos_getrs) {
  ...
  _apply_single_inverse_helper<scalar_t>(
      ...
      self_inv.data_ptr<scalar_t>(),
      ipiv.data_ptr<int>(),
      infos_getrf.data_ptr<int>(),
      infos_getrs.data_ptr<int>(),
      n,
      lda);
}

// This is a type dispatching helper function for 'apply_batched_inverse_lib'
// and 'apply_single_inverse_lib'
Tensor& _linalg_inv_out_helper_cuda_lib(
  ...
  if (result.dim() > 2) {
    ...
    AT_DISPATCH_FLOATING_AND_COMPLEX_TYPES(
        result.scalar_type(), "linalg_inv_out_cuda", [&] {
          apply_single_inverse_lib<scalar_t>(
              input_working_copy, result, infos_getrf, infos_getrs);
        });
  }
...
inline static void apply_svd_cusolver_gesvd(
  ...
  TORCH_INTERNAL_ASSERT(
      m >= n, "cusolver gesvd only supports matrix with sizes m >= n");
  ...
  TORCH_INTERNAL_ASSERT(
      lwork >= 0,
      "gesvd_buffersize failed to get needed buffer size, got lwork = ",
      lwork);

  auto& allocator = *::c10::cuda::CUDACachingAllocator::get();
  const auto dataPtr_work = allocator.allocate(sizeof(scalar_t) * lwork);
  const auto dataPtr_rwork =
      allocator.allocate(sizeof(value_t) * std::min(m, n));

  // nb. We can do this .view() because V is a batch of F-contig matrices
  const auto V_view = compute_uv ? V.view({-1, n, V.size(-1)}) : Tensor{};
  // V is F-contig. Since this function computes Vh, we need an auxiliary
  // F-conj-transposed matrix to hold Vh
  const auto Vh_workspace = compute_uv
      ? at::empty(
            {n, full_matrices ? n : k},
            A.options().memory_format(at::MemoryFormat::Contiguous))
            .conj()
      : Tensor{};
  const auto Vh_ptr = compute_uv ? Vh_workspace.data_ptr<scalar_t>() : nullptr;

  const auto U_stride = compute_uv ? matrixStride(U) : 0;
  const auto U_ptr = compute_uv ? U.data_ptr<scalar_t>() : nullptr;

  int batchsize = calculate_all_batches
      ? cuda_int_cast(batchCount(A), "batch size")
      : batches.size();
...
inline static void svd_cusolver_gesvd(
    const Tensor& A,
    const Tensor& U,
    const Tensor& S,
    const Tensor& V,
  ...
  // gesvd just knows how to handle m >= n, so in the other case we need to
  // transpose A
  const auto not_A_H = A.size(-2) >= A.size(-1);
  AT_DISPATCH_FLOATING_AND_COMPLEX_TYPES(
      A.scalar_type(), "svd_cuda_gesvd", [&] {
        apply_svd_cusolver_gesvd<scalar_t>(
...
template <typename scalar_t>
inline static void apply_svd_cusolver_gesvdj(
    const Tensor& A,
    const Tensor& U,
    const Tensor& S,
    const Tensor& V,
  ...
  auto& allocator = *::c10::cuda::CUDACachingAllocator::get();
  auto dataPtr_U = !compute_uv ? allocator.allocate(sizeof(scalar_t) * m * k)
                               : c10::DataPtr{};
  auto dataPtr_V = !compute_uv ? allocator.allocate(sizeof(scalar_t) * n * k)
                               : c10::DataPtr{};

  auto A_data = A.data_ptr<scalar_t>();
  auto U_data = compute_uv ? U.data_ptr<scalar_t>()
                           : reinterpret_cast<scalar_t*>(dataPtr_U.get());
  auto S_data = S.data_ptr<value_t>();
  auto V_data = compute_uv ? V.data_ptr<scalar_t>()
                           : reinterpret_cast<scalar_t*>(dataPtr_V.get());
  auto A_stride = matrixStride(A);
  ...
  // gesvdj_params controls the numerical accuracy of cusolver gesvdj iterations
  // on GPU
  gesvdjInfo_t gesvdj_params;
  TORCH_CUSOLVER_CHECK(cusolverDnCreateGesvdjInfo(&gesvdj_params));

  // Todo: expose the following two parameters to users
  TORCH_CUSOLVER_CHECK(cusolverDnXgesvdjSetTolerance(
      gesvdj_params, std::numeric_limits<scalar_t>::epsilon()));
  TORCH_CUSOLVER_CHECK(
      cusolverDnXgesvdjSetMaxSweeps(gesvdj_params, cusolver_gesvdj_max_sweeps));
  ...
  TORCH_INTERNAL_ASSERT(
      lwork >= 0,
      "gesvdj_buffersize failed to get needed buffer size, got lwork = ",
      lwork);
  ...
  for (int i = 0; i < batchsize; i++) {
    ...
    // // Note: this will introduce a device-host sync and may negatively affect
    // the performance double residual = 0;
    // TORCH_CUSOLVER_CHECK(cusolverDnXgesvdjGetResidual(handle, gesvdj_params,
    // &residual)); printf("gesvdj residual = %.6e\n", residual);
  }
...
inline static void svd_cusolver_gesvdj(
    const Tensor& A,
    const Tensor& U,
    const Tensor& S,
    const Tensor& V,
    ...
    bool compute_uv) {
  AT_DISPATCH_FLOATING_AND_COMPLEX_TYPES(
      A.scalar_type(), "svd_cuda_gesvdj", [&] {
        apply_svd_cusolver_gesvdj<scalar_t>(
            A, U, S, V, infos, full_matrices, compute_uv);
      });
}

// call cusolver gesvdj batched function to calculate svd
template <typename scalar_t>
inline static void apply_svd_cusolver_gesvdjBatched(
    const Tensor& A,
    const Tensor& U,
    const Tensor& S,
    const Tensor& V,
  ...
  // Need to pass allocated memory to the function, otherwise it fails
  auto& allocator = *::c10::cuda::CUDACachingAllocator::get();
  auto dataPtr_U = !compute_uv
      ? allocator.allocate(sizeof(scalar_t) * batchsize * m * k)
      : c10::DataPtr{};
  auto dataPtr_V = !compute_uv
      ? allocator.allocate(sizeof(scalar_t) * batchsize * n * k)
      : c10::DataPtr{};
  ...
  TORCH_INTERNAL_ASSERT(
      m <= 32 && n <= 32,
      "gesvdjBatched requires both matrix dimensions not greater than 32, but got "
      "m = ",
      m,
      " n = ",
      n);
...
inline static void svd_cusolver_gesvdjBatched(
    const Tensor& A,
    const Tensor& U,
    const Tensor& S,
    const Tensor& V,
    ...
    bool compute_uv) {
  AT_DISPATCH_FLOATING_AND_COMPLEX_TYPES(
      A.scalar_type(), "svd_cuda_gesvdjBatched", [&] {
        apply_svd_cusolver_gesvdjBatched<scalar_t>(
            A, U, S, V, infos, compute_uv);
      });
}
...
inline static void apply_svd_cusolver_gesvdaStridedBatched(
  ...
  int n = cuda_int_cast(A.size(-1), "n");
  TORCH_INTERNAL_ASSERT(
      m >= n, "cusolver gesvdaStridedBatched requires m >= n");
  int batchsize = cuda_int_cast(batchCount(A), "batch size");
  ...
  auto U_stride = compute_uv ? matrixStride(U)
                             : ldu * rank; // The strides for "empty matrices"
                                           // are needed to satisfy cusolver.
  auto V_stride = compute_uv ? matrixStride(V) : ldv * rank;

  // Need to pass allocated memory to the function, otherwise it fails
  auto& allocator = *::c10::cuda::CUDACachingAllocator::get();
  auto dataPtr_U = !compute_uv
      ? allocator.allocate(sizeof(scalar_t) * batchsize * m * n)
      : c10::DataPtr{};
  auto dataPtr_V = !compute_uv
      ? allocator.allocate(sizeof(scalar_t) * batchsize * n * n)
      : c10::DataPtr{};
  ...
  TORCH_INTERNAL_ASSERT(
      lwork >= 0,
      "gesvdaStridedBatched_buffersize failed to get needed buffer size, got lwork = ",
      lwork);
  auto workspace = allocator.allocate(sizeof(scalar_t) * lwork);

  // The residual Frobenius norm is always returned in double.
  // cuSOLVER remark: if the user is confident on the accuracy of singular
  // values and singular vectors,
  //   for example, certain conditions hold (required singular value is far from
  //   zero), then the performance can be improved by passing a null pointer to
  //   h_RnrmF, i.e. no computation of residual norm.
  // Comment: calculation of Frobenius norm is expensive and doesn't affect
  ...
  at::cuda::solver::gesvdaStridedBatched<scalar_t>(
      ...
      lwork,
      infos.data_ptr<int>(),
      nullptr, // cuSOLVER h_RnrmF is not calculated:
...
inline static void svd_cusolver_gesvdaStridedBatched(
    ...
    bool compute_uv) {
  // We need to pass a copy of A, as it will be overwritten
  // gesvdaStridedBatched just knows how to handle m >= n, so in the other case
  // we need to transpose A
  const auto not_A_H = A.size(-2) >= A.size(-1);
  AT_DISPATCH_FLOATING_AND_COMPLEX_TYPES(
      A.scalar_type(), "svd_cuda_gesvdaStridedBatched", [&] {
...
// If not converged, return a vector that contains indices of the non-converging
// batches. If the returned vector is empty, all the matrices are converged.
// This function will cause a device-host sync.
std::vector<int64_t> _check_gesvdj_convergence(
    const Tensor& infos,
    int64_t non_converging_info) {
  ...
  for (int64_t i = 0; i < infos.numel(); i++) {
    ...
    // From cusolver doc, if info < 0, the i-th function call parameter is
    // wrong, which means pytorch implementation of cusolver is wrong.
    TORCH_INTERNAL_ASSERT_DEBUG_ONLY(info_for_batch_i >= 0);

    // In our use case, gesvdj, gesvdjBatched, and gesvdaStridedBatched have the
    // same notations for `info`.
    if (info_for_batch_i == non_converging_info)
      res.push_back(i);

    // However, it is not the same for gesvd, though we don't use this function
    // to check gesvd convergence either. If it's implemented some day in the
    // future, this needs to be handled carefully.
  }
...
// format the non-converging batches string as either (no leading or trailing
// whitespaces) batches 2, 3, 5  // or batches 2, 3, 5, 7, 11 and other 65535
// batches
std::string _format_non_converging_batches(
    const std::vector<int64_t>& batches) {
  std::stringstream ss;
...
void svd_cusolver(
    ...
    const Tensor& V,
    const Tensor& info) {
  // Here U and V are F-contig whenever they are defined (i.e. whenever
  // compute_uv=true)
  ...
  static const char* check_svd_doc =
  ...
  if (driver_v == "gesvd") {
    ...
    if (m <= 32 && n <= 32 && batch_size > 1 && (full_matrices || m == n)) {
      svd_cusolver_gesvdjBatched(
          cloneBatchedColumnMajor(A), U, S, V, info, compute_uv);
    } else {
      // gesvdj driver may be numerically unstable for large sized matrix
      svd_cusolver_gesvdj(
          cloneBatchedColumnMajor(A), U, S, V, info, full_matrices, compute_uv);
    }
  } else if (driver_v == "gesvda") {
    // cuSOLVER: gesvdaStridedBatched is preferred for "tall skinny" (m > n)
    // matrices We do a transpose here to make it also work for (m < n)
    // matrices.
    svd_cusolver_gesvdaStridedBatched(
        A, U, S, V, info, full_matrices, compute_uv);
  } else {
    TORCH_CHECK(
        false,
  ...
  if (driver_v != "gesvd") {
    ...
    // Todo: implement the svd_ex variant to not check result convergence, thus
    // removing the device-host sync
    const auto svd_non_converging_batches =
        _check_gesvdj_convergence(info, k + 1);

    if (!svd_non_converging_batches.empty()) {
      TORCH_WARN_ONCE(
          "torch.linalg.svd: During SVD computation with the selected cusolver driver, ",
      ...
      // We'll do the fallback if user doesn't specify a driver and the default
      // heuristic doesn't converge well. However, if user manually chooses a
      // driver, should we just do a warning or a hard crash?
      if (!driver.has_value()) {
        svd_cusolver_gesvd(
            A,
            U,
            ...
            compute_uv,
            false,
            svd_non_converging_batches);
      }
    }
  }

  // `info` will be checked later at `TORCH_IMPL_FUNC(_linalg_svd_out)`
  // function.
}

// Implementation of Cholesky decomposition using looped cusolverDn<T>potrf or
// cusolverDnXpotrf (64-bit)
template <typename scalar_t>
inline static void apply_cholesky_cusolver_potrf_looped(
  ...
  at::cuda::solver::xpotrf_buffersize(
      handle,
      params,
      uplo,
      n,
  ...
  // allocate workspace storage
  auto& device_allocator = *at::cuda::getCUDADeviceAllocator();
  auto workdata_device =
      device_allocator.allocate(worksize_device * batch_size);
  void* workdata_device_ptr = workdata_device.get();
  ...
  for (int64_t i = 0; i < batch_size; i++) {
    at::cuda::solver::xpotrf(
        ...
        lda,
        datatype,
        (char*)workdata_device_ptr + i * worksize_device,
        worksize_device,
        (char*)workdata_host_ptr + i * worksize_host,
        worksize_host,
        infos_ptr + i);
  }
...
// cusolverDn<T>potrfBatched Warning: cusolverDn<T>potrfBatched doesn't work
// quite well when matrix size or batch size is zero. If you write your own C++
// extension and use this function, make sure you do a zero numel check for the
// input.
template <typename scalar_t>
inline static void apply_cholesky_cusolver_potrfBatched(
    const Tensor& self_working_copy,
    bool upper,
    const Tensor& infos) {
  auto handle = at::cuda::getCurrentCUDASolverDnHandle();
  ...
  const int batch_size =
      cuda_int_cast(batchCount(self_working_copy), "batch_size");
...
// This code path is only dispatched to if MAGMA is not linked in the pytorch
// build. cusolverDn<t>potrsBatched only supports nrhs == 1
template <typename scalar_t>
inline static void apply_cholesky_cusolver_potrsBatched(
    Tensor& self_working_copy,
    const Tensor& A_column_major_copy,
    bool upper,
    Tensor& infos) {
...
Tensor _cholesky_solve_helper_cuda_cusolver(
  ...
  if (batch_size > 1 && nrhs == 1) {
    AT_DISPATCH_FLOATING_AND_COMPLEX_TYPES(
        self.scalar_type(), "cholesky_cuda_potrs_batched", [&] {
          apply_cholesky_cusolver_potrsBatched<scalar_t>(
              self_working_copy, A_column_major_copy, upper, infos);
        });
  } else {
    AT_DISPATCH_FLOATING_AND_COMPLEX_TYPES(
        self.scalar_type(), "cholesky_cuda_potrs", [&] {
          apply_cholesky_cusolver_potrs<scalar_t>(
              self_working_copy, A_column_major_copy, upper, infos);
        });
  }

  // info from potrs and potrsBatched only report if the i-th parameter is
  // wrong, not about the matrix singularity, etc. So we don't need to check it
  // all the time.
...
void _cholesky_inverse_cusolver_potrs_based(
    Tensor& result,
    Tensor& infos,
    bool upper) {
  ...
  result.diagonal(/*offset=*/0, /*dim1=*/-2, /*dim2=*/-1).fill_(1);
  AT_DISPATCH_FLOATING_AND_COMPLEX_TYPES(
      result.scalar_type(), "cholesky_cuda_potri", [&] {
        apply_cholesky_cusolver_potrs<scalar_t>(
...
static void apply_geqrf(const Tensor& A, const Tensor& tau) {
  ...
  cusolverDnParams_t params =
      NULL; // use default algorithm (currently it's the only option)
...
/*
  ...
  * `transpose` - bool, determining whether to transpose (or conjugate
  ...
  For further details, please see the cuSOLVER documentation for ORMQR and
  UNMQR.
*/
template <typename scalar_t>
static void apply_ormqr(
    const Tensor& input,
    const Tensor& tau,
    const Tensor& other,
    bool left,
  ...
  auto trans = transpose ? (input.is_complex() ? CUBLAS_OP_C : CUBLAS_OP_T)
                         : CUBLAS_OP_N;
...
static void apply_syevj(
  ...
  // cuSOLVER documentations says: "15 sweeps are good enough to converge to
  // machine accuracy" LAPACK has SVD routine based on similar Jacobi algorithm
  // (gesvj) and there a maximum of 30 iterations is set Let's use the default
  // values for now
...
static void linalg_eigh_cusolver_syevd(
    ...
    bool compute_eigenvectors) {
  AT_DISPATCH_FLOATING_AND_COMPLEX_TYPES(
      eigenvectors.scalar_type(), "linalg_eigh_cuda", [&] {
        apply_syevd<scalar_t>(
...
static void linalg_eigh_cusolver_syevj(
  ...
  AT_DISPATCH_FLOATING_AND_COMPLEX_TYPES(
      eigenvectors.scalar_type(), "linalg_eigh_cuda", [&] {
        apply_syevj<scalar_t>(
            eigenvalues, eigenvectors, infos, upper, compute_eigenvectors);
      });
}

static void linalg_eigh_cusolver_syevj_batched(
    const Tensor& eigenvalues,
    const Tensor& eigenvectors,
    const Tensor& infos,
    bool upper,
...
void linalg_eigh_cusolver(
    ...
    bool upper,
    bool compute_eigenvectors) {
  if (use_cusolver_syevj_batched_ && batchCount(eigenvectors) > 1 &&
    ...
    linalg_eigh_cusolver_syevj_batched(
      ...
      eigenvectors.scalar_type() == at::kFloat && eigenvectors.size(-1) >= 32 &&
      eigenvectors.size(-1) <= 512) {
    // syevj is better than syevd for float32 dtype and matrix sizes 32x32 -
    // 512x512 See
...
void lu_factor_looped_cusolver(
  ...
  // Necessary because cuSOLVER uses nan for outputs that correspond to 0 in
  // MAGMA for non-pivoted LU.
...
void lu_solve_looped_cusolver(
    ...
    TransposeType transpose) {
  AT_DISPATCH_FLOATING_AND_COMPLEX_TYPES(
      LU.scalar_type(), "lu_solve_cusolver", [&] {
        const auto trans = to_cublas(transpose);
        ...
        int leading_dimension =
            cuda_int_cast(std::max<int>(1, n), "leading_dimension");
