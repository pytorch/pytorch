...
#define CUDASOLVER_GETRS_ARGTYPES(Dtype)                                     \
  cusolverDnHandle_t handle, int n, int nrhs, Dtype *dA, int lda, int *ipiv, \
      Dtype *ret, int ldb, int *info, cublasOperation_t trans
...
void gesvd_buffersize(CUDASOLVER_GESVD_BUFFERSIZE_ARGTYPES()) {
  TORCH_CHECK(
      false,
      "at::cuda::solver::gesvd_buffersize: not implemented for ",
      typeid(Dtype).name());
}
template <>
...
#define CUDASOLVER_GESVD_ARGTYPES(Dtype, Vtype)                          \
  cusolverDnHandle_t handle, signed char jobu, signed char jobvt, int m, \
      int n, Dtype *A, int lda, Vtype *S, Dtype *U, int ldu, Dtype *VT,  \
      int ldvt, Dtype *work, int lwork, Vtype *rwork, int *info

template <class Dtype, class Vtype>
void gesvd(CUDASOLVER_GESVD_ARGTYPES(Dtype, Vtype)) {
  TORCH_CHECK(
      false,
      "at::cuda::solver::gesvd: not implemented for ",
      typeid(Dtype).name());
}
template <>
...
void gesvdj_buffersize(CUDASOLVER_GESVDJ_BUFFERSIZE_ARGTYPES(Dtype, Vtype)) {
  TORCH_INTERNAL_ASSERT(
      false,
      "at::cuda::solver::gesvdj_buffersize: not implemented for ",
      typeid(Dtype).name());
}
template <>
void gesvdj_buffersize<float>(
    CUDASOLVER_GESVDJ_BUFFERSIZE_ARGTYPES(float, float));
template <>
void gesvdj_buffersize<double>(
    CUDASOLVER_GESVDJ_BUFFERSIZE_ARGTYPES(double, double));
template <>
void gesvdj_buffersize<c10::complex<float>>(
    CUDASOLVER_GESVDJ_BUFFERSIZE_ARGTYPES(c10::complex<float>, float));
template <>
void gesvdj_buffersize<c10::complex<double>>(
    CUDASOLVER_GESVDJ_BUFFERSIZE_ARGTYPES(c10::complex<double>, double));

#define CUDASOLVER_GESVDJ_ARGTYPES(Dtype, Vtype)                             \
  cusolverDnHandle_t handle, cusolverEigMode_t jobz, int econ, int m, int n, \
      Dtype *A, int lda, Vtype *S, Dtype *U, int ldu, Dtype *V, int ldv,     \
      Dtype *work, int lwork, int *info, gesvdjInfo_t params
...
#define CUDASOLVER_GESVDA_STRIDED_BATCHED_BUFFERSIZE_ARGTYPES(Dtype, Vtype)  \
  cusolverDnHandle_t handle, cusolverEigMode_t jobz, int rank, int m, int n, \
      Dtype *A, int lda, long long int strideA, Vtype *S,                    \
      long long int strideS, Dtype *U, int ldu, long long int strideU,       \
      Dtype *V, int ldv, long long int strideV, int *lwork, int batchSize

template <class Dtype, class Vtype>
void gesvdaStridedBatched_buffersize(
    CUDASOLVER_GESVDA_STRIDED_BATCHED_BUFFERSIZE_ARGTYPES(Dtype, Vtype)) {
  TORCH_INTERNAL_ASSERT(
      false,
      "at::cuda::solver::gesvdaStridedBatched_buffersize: not implemented for ",
      typeid(Dtype).name());
}
template <>
void gesvdaStridedBatched_buffersize<float>(
    CUDASOLVER_GESVDA_STRIDED_BATCHED_BUFFERSIZE_ARGTYPES(float, float));
template <>
void gesvdaStridedBatched_buffersize<double>(
    CUDASOLVER_GESVDA_STRIDED_BATCHED_BUFFERSIZE_ARGTYPES(double, double));
template <>
void gesvdaStridedBatched_buffersize<c10::complex<float>>(
    CUDASOLVER_GESVDA_STRIDED_BATCHED_BUFFERSIZE_ARGTYPES(
        c10::complex<float>,
        float));
template <>
...
#define CUDASOLVER_GESVDA_STRIDED_BATCHED_ARGTYPES(Dtype, Vtype)             \
  cusolverDnHandle_t handle, cusolverEigMode_t jobz, int rank, int m, int n, \
      Dtype *A, int lda, long long int strideA, Vtype *S,                    \
      long long int strideS, Dtype *U, int ldu, long long int strideU,       \
      Dtype *V, int ldv, long long int strideV, Dtype *work, int lwork,      \
      int *info, double *h_R_nrmF, int batchSize
// h_R_nrmF is always double, regardless of input Dtype.

template <class Dtype, class Vtype>
void gesvdaStridedBatched(
    CUDASOLVER_GESVDA_STRIDED_BATCHED_ARGTYPES(Dtype, Vtype)) {
  TORCH_INTERNAL_ASSERT(
      false,
      "at::cuda::solver::gesvdaStridedBatched: not implemented for ",
      typeid(Dtype).name());
}
template <>
void gesvdaStridedBatched<float>(
    CUDASOLVER_GESVDA_STRIDED_BATCHED_ARGTYPES(float, float));
template <>
void gesvdaStridedBatched<double>(
    CUDASOLVER_GESVDA_STRIDED_BATCHED_ARGTYPES(double, double));
template <>
void gesvdaStridedBatched<c10::complex<float>>(
    CUDASOLVER_GESVDA_STRIDED_BATCHED_ARGTYPES(c10::complex<float>, float));
template <>
void gesvdaStridedBatched<c10::complex<double>>(
    CUDASOLVER_GESVDA_STRIDED_BATCHED_ARGTYPES(c10::complex<double>, double));
