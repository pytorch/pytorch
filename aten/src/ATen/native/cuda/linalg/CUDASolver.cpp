...
void getrs<double>(
  ...
  TORCH_CUSOLVER_CHECK(
      cusolverDnDgetrs(handle, trans, n, nrhs, dA, lda, ipiv, ret, ldb, info));
}
...
void getrs<float>(
    ...
    cublasOperation_t trans) {
  TORCH_CUSOLVER_CHECK(
      cusolverDnSgetrs(handle, trans, n, nrhs, dA, lda, ipiv, ret, ldb, info));
}
...
void gesvd_buffersize<c10::complex<float>>(
    CUDASOLVER_GESVD_BUFFERSIZE_ARGTYPES()) {
...
void gesvd<c10::complex<float>>(
    CUDASOLVER_GESVD_ARGTYPES(c10::complex<float>, float)) {
...
void gesvd<c10::complex<double>>(
    CUDASOLVER_GESVD_ARGTYPES(c10::complex<double>, double)) {
...
void gesvdj_buffersize<float>(
    ...
    int* lwork,
    gesvdjInfo_t params) {
  TORCH_CUSOLVER_CHECK(cusolverDnSgesvdj_bufferSize(
...
void gesvdj_buffersize<double>(
    ...
    int* lwork,
    gesvdjInfo_t params) {
  TORCH_CUSOLVER_CHECK(cusolverDnDgesvdj_bufferSize(
      handle, jobz, econ, m, n, A, lda, S, U, ldu, V, ldv, lwork, params));
}
...
void gesvdjBatched<float>(
    ...
    int batchSize) {
  int lwork;
  TORCH_CUSOLVER_CHECK(cusolverDnSgesvdjBatched_bufferSize(
      handle,
      jobz,
      m,
      n,
...
void gesvdjBatched<double>(
    ...
    int batchSize) {
  int lwork;
  TORCH_CUSOLVER_CHECK(cusolverDnDgesvdjBatched_bufferSize(
      handle,
      jobz,
      m,
      n,
...
void potrf<float>(
    ...
    int* info) {
  TORCH_CUSOLVER_CHECK(
      cusolverDnSpotrf(handle, uplo, n, A, lda, work, lwork, info));
}
...
void potrf<double>(
    ...
    int* info) {
  TORCH_CUSOLVER_CHECK(
      cusolverDnDpotrf(handle, uplo, n, A, lda, work, lwork, info));
}
...
void potrfBatched<float>(
    ...
    int batchSize) {
  TORCH_CUSOLVER_CHECK(
      cusolverDnSpotrfBatched(handle, uplo, n, A, lda, info, batchSize));
}
...
void potrfBatched<double>(
    ...
    int batchSize) {
  TORCH_CUSOLVER_CHECK(
      cusolverDnDpotrfBatched(handle, uplo, n, A, lda, info, batchSize));
}
...
void potrfBatched<c10::complex<float>>(
  ...
  TORCH_CUSOLVER_CHECK(cusolverDnCpotrfBatched(
      handle, uplo, n, reinterpret_cast<cuComplex**>(A), lda, info, batchSize));
}
...
void potrsBatched<float>(
    ...
    int* info,
    int batchSize) {
  TORCH_CUSOLVER_CHECK(cusolverDnSpotrsBatched(
...
void potrsBatched<double>(
    ...
    int* info,
    int batchSize) {
  TORCH_CUSOLVER_CHECK(cusolverDnDpotrsBatched(
      handle, uplo, n, nrhs, Aarray, lda, Barray, ldb, info, batchSize));
}
...
void ormqr_bufferSize<c10::complex<double>>(
  ...
  TORCH_CUSOLVER_CHECK(cusolverDnZunmqr_bufferSize(
      ...
      lda,
      reinterpret_cast<const cuDoubleComplex*>(tau),
      reinterpret_cast<const cuDoubleComplex*>(C),
      ldc,
      lwork));
}
...
void xpotrs(
    ...
    int64_t ldb,
    int* info) {
  TORCH_CUSOLVER_CHECK(cusolverDnXpotrs(
      handle,
      params,
      uplo,
      n,
