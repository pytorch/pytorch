...
#define AT_MAGMA_VERSION \
  MAGMA_VERSION_MAJOR * 100 + MAGMA_VERSION_MINOR * 10 + MAGMA_VERSION_MICRO

// Check that MAGMA never releases MAGMA_VERSION_MINOR >= 10 or
// MAGMA_VERSION_MICRO >= 10
#if MAGMA_VERSION_MINOR >= 10 || MAGMA_VERSION_MICRO >= 10
#error \
    "MAGMA release minor or micro version >= 10, please correct AT_MAGMA_VERSION"
#endif
...
void magmaSvd<double>(
  ...
  MagmaStreamSyncGuard guard;
  magma_dgesdd(
      jobz, m, n, A, lda, s, U, ldu, Vh, ldvh, work, lwork, iwork, info);
  AT_CUDA_CHECK(cudaGetLastError());
}
...
void magmaSvd<c10::complex<float>, float>(
  ...
  magma_cgesdd(
      ...
      ldu,
      reinterpret_cast<magmaFloatComplex*>(Vh),
      ldvh,
      reinterpret_cast<magmaFloatComplex*>(work),
      lwork,
      rwork,
...
void magmaSvd<c10::complex<double>, double>(
  ...
  magma_zgesdd(
      ...
      ldu,
      reinterpret_cast<magmaDoubleComplex*>(Vh),
      ldvh,
      reinterpret_cast<magmaDoubleComplex*>(work),
      lwork,
      rwork,
...
void ldl_factor_kernel(
  ...
  switch (preferred_backend) {
    case at::LinalgBackend::Cusolver:
      return ldl_factor_cusolver(LD, pivots, info, upper, hermitian);
...
void ldl_solve_kernel(
    ...
    bool upper,
    bool hermitian) {
  // TODO: It should be possible to add the MAGMA backend for this function when
  // using MAGMA 2.6.0
...
static void apply_single_inverse(
  ...
  magmaLu<scalar_t>(
      n, n, self_data, lda, ipiv.data_ptr<magma_int_t>(), &info_lu_cpu);
  magmaGetri<scalar_t>(
      n,
      self_data,
      lda,
...
*/
template <typename scalar_t>
static void apply_lu_factor_looped_magma(
    const Tensor& input,
    const Tensor& pivots,
    const Tensor& infos,
    bool compute_pivots) {
...
*/
template <typename scalar_t>
static void apply_lu_factor_batched_magma(
    const Tensor& input,
    const Tensor& pivots,
    const Tensor& infos,
    bool compute_pivots) {
  ...
  const bool magma_batched_buggy =
      version < std::make_tuple<magma_int_t, magma_int_t, magma_int_t>(2, 5, 2);
  TORCH_CHECK(
      !magma_batched_buggy,
      "linalg.lu_factor has buggs on MAGMA < 2.5.2. Please update your MAGMA version to a newer one.");
  ...
  if (compute_pivots) {
    ...
    auto pivots_stride = pivots.size(-1);
    // fill pivots with ones to avoid memory access violations inside magma
    // kernels magmaLuBatched might not set the values for it see
    // https://github.com/pytorch/pytorch/pull/53064
    pivots.fill_(1);
  ...
  // this explicit sync prevents garbage results from the subsequent
  // magmaLuSolveBatched call from a different queue
  magma_queue_sync(magma_queue.get_queue());
#endif
}

static void lu_factor_looped_magma(
    const Tensor& input,
    const Tensor& pivots,
    const Tensor& infos,
    bool compute_pivots) {
  AT_DISPATCH_FLOATING_AND_COMPLEX_TYPES(
      input.scalar_type(), "lu_factor_magma_looped", [&] {
        apply_lu_factor_looped_magma<scalar_t>(
            input, pivots, infos, compute_pivots);
      });
}

static void lu_factor_batched_magma(
    const Tensor& input,
    const Tensor& pivots,
    const Tensor& infos,
    bool compute_pivots) {
  AT_DISPATCH_FLOATING_AND_COMPLEX_TYPES(
      input.scalar_type(), "lu_factor_magma_batched", [&] {
        apply_lu_factor_batched_magma<scalar_t>(
            input, pivots, infos, compute_pivots);
      });
}
...
static void lu_factor(
  ...
  const auto lu_factor_magma = [batch_size](
                                   const Tensor& input,
                                   const Tensor& pivots,
                                   const Tensor& infos,
                                   const bool compute_pivots) {
    if (batch_size == 1) {
...
#ifdef USE_CUSOLVER
  const auto lu_factor_cusolver = [batch_size, m, n](
                                      const Tensor& input,
                                      const Tensor& pivots,
                                      const Tensor& infos,
                                      bool compute_pivots) {
  ...
  if (preferred_backend == at::LinalgBackend::Cusolver) {
    ...
    // otherwise, lu_factor just works for square matrices, for non-square
    // matrices magma batched is the fastest otherwise (i.e. for square
    // matrices), we choose between cusolver and magma using a heuristic
    if (m == n &&
        (batch_size == 1 || m <= 16 || (m <= 128 && batch_size <= 16))) {
      lu_factor_cusolver(input, pivots, infos, compute_pivots);
    } else {
      lu_factor_batched_magma(input, pivots, infos, compute_pivots);
    }
...
template <typename scalar_t>
static void apply_triangular_solve_batched_magma(
    const Tensor& A,
    const Tensor& b,
    bool left,
    bool upper,
...
void triangular_solve_batched_magma(
  ...
  AT_DISPATCH_FLOATING_AND_COMPLEX_TYPES(
      A.scalar_type(), "triangular_solve_cuda", [&] {
        apply_triangular_solve_batched_magma<scalar_t>(
...
Tensor& orgqr_kernel_impl(Tensor& result, const Tensor& tau) {
  ...
  // (tau.size(-1) <= 32) using MAGMA, however it fails on Windows because of
  // some illegal memory reads inside MAGMA. See discussions in
...
void geqrf_kernel(const Tensor& input, const Tensor& tau) {
  ...
  switch (preferred_backend) {
      ...
      // It may be the case that geqrf followed by orgqr is wrong for the magma
      // backend geqrf_magma currently uses geqrf2_gpu
      //
      // We require to perform ?geqrf_gpu again due to this bug in MAGMA:
      // - ?geqrf_gpu allows fast computation of Q via ?orgqr_gpu, but doesn't
      // give R properly.
...
static void apply_svd_magma(
...
#if !AT_MAGMA_ENABLED()
  AT_ERROR(
      "linalg.svd: MAGMA library not found in "
      "compilation. Please rebuild with MAGMA.");
#else
  using value_t = typename c10::scalar_value_type<scalar_t>::type;
  ...
  const auto jobz =
      compute_uv ? (full_matrices ? MagmaAllVec : MagmaSomeVec) : MagmaNoVec;
  ...
  const auto ldu = compute_uv ? magma_int_cast(U.strides().end()[-1], "ldu")
                              : magma_int_t{1};
  const auto ldvh = compute_uv ? magma_int_cast(Vh.strides().end()[-1], "ldvh")
                               : magma_int_t{1};
  ...
  if (A.is_complex()) {
    auto lrwork =
        computeLRWorkDim(compute_uv ? (full_matrices ? 'A' : 'S') : 'N', m, n);
    storage_rwork = pin_memory<value_t>(lrwork);
    rwork = static_cast<value_t*>(storage_rwork.data());
  }
  ...
  magma_int_t lwork = -1;
  {
    scalar_t wkopt = 1; // MAGMA might not set the value for the optimal
                        // workspace therefore use 1 as the default value
    magmaSvd<scalar_t, value_t>(
        ...
        lda,
        S_data,
        compute_uv ? U_data : nullptr,
        ldu,
        compute_uv ? Vh_data : nullptr,
        ldvh,
        &wkopt,
        lwork,
        rwork,
  ...
  for (int64_t i = 0; i < batchsize; i++) {
    ...
    magmaSvd<scalar_t, value_t>(
        ...
        lda,
        S_data + i * S_stride,
        compute_uv ? U_data + i * U_stride : nullptr,
        ldu,
        compute_uv ? Vh_data + i * Vh_stride : nullptr,
        ldvh,
        work,
        lwork,
        rwork,
...
void svd_magma(
  ...
  // We copy it into CPU with the correct strides and in pinned_memory as MAGMA
  // moves things between CPU and GPU
  const auto A_ = A.mT()
                      .to(A.options()
                              .device(kCPU)
  ...
  AT_DISPATCH_FLOATING_AND_COMPLEX_TYPES(A.scalar_type(), "svd_cuda", [&] {
    apply_svd_magma<scalar_t>(
        A_, full_matrices, compute_uv, U_, S_, Vh_, info_);
  });
...
void svd_kernel(
  ...
  if (at::globalContext().linalgPreferredBackend() ==
      at::LinalgBackend::Magma) {
...
#else
  TORCH_INTERNAL_ASSERT(
      batchCount(B) == batchCount(LU),
      "batch_size of LU and B must be the same");
  TORCH_INTERNAL_ASSERT(
      batchCount(LU) == batchCount(pivots.unsqueeze(-1)),
      "batch_size of LU and pivots must be the same");
  auto trans = to_magma(transpose);
  auto b_data = B.data_ptr<scalar_t>();
  auto lu_data = LU.data_ptr<scalar_t>();
...
static void lu_solve_batched_magma(
    const Tensor& LU,
    const Tensor& pivots,
    const Tensor& B,
    TransposeType trans) {
  ...
  AT_DISPATCH_FLOATING_AND_COMPLEX_TYPES(
      LU.scalar_type(), "lu_solve_batched_magma", [&] {
        apply_lu_solve_batched_magma<scalar_t>(LU, pivots, B, trans);
      });
}

static void lu_solve_looped_magma(
    const Tensor& LU,
    const Tensor& pivots,
    const Tensor& B,
    TransposeType trans) {
  AT_DISPATCH_FLOATING_AND_COMPLEX_TYPES(
      LU.scalar_type(), "lu_solve_looped_magma", [&] {
...
c10::MaybeOwned<Tensor> maybe_expand_pivots(
    const Tensor& B,
    const Tensor& pivots) {
  ...
  if (batchCount(B) != batchCount(pivots.unsqueeze(-1))) {
    ...
    return c10::MaybeOwned<Tensor>::owned(
        pivots.expand(expand_shape).contiguous());
  } else {
...
static void lu_solve_kernel(
  ...
  auto m = LU.size(-2);
  auto b2 = B.size(-1);
  // magma implementation of LU solve cannot handle a b tensor with last dim >
  // 1024 See
  // https://bitbucket.org/icl/magma/issues/19/dgesv_batched-dgetrs_batched-fails-for
  bool over_batched_magma_dim_limit = b2 > 1024;
  // heuristics determined from tests dicussed in
  // https://github.com/pytorch/pytorch/pull/72935

  // Computes X = U^{-1}L^{-1}P^T B via triangular solves
  // Helps mitigating the bugs in magma
  auto lu_solve_triangular = [m](const Tensor& LU,
                                 const Tensor& pivots,
                                 const Tensor& B,
                                 const TransposeType trans) {
    ...
    // Here we transform it to a vector representing a permutation, i.e. a
    // (batch of) vectors st. P(i) = j
    auto perm = at::arange(m, pivots_->options().dtype(kLong))
                    .expand(pivots_->sizes())
                    .contiguous();
    auto iter = TensorIteratorConfig()
                    ...
                    .declare_static_shape(
                        pivots_->sizes(), /*squash_dim=*/pivots_->dim() - 1)
                    .add_output(perm)
                    .add_input(*pivots_)
                    .build();
    ...
    if (trans == TransposeType::NoTranspose) {
      ...
      // B1 = P^T @ B  (must be done out-of-place as B is both source and
      // target)
      ...
      // B = U^{-1} @ B
      at::linalg_solve_triangular_out(
          const_cast<Tensor&>(B), *LU_, B, /*upper=*/true);
    } else {
      auto LU_H = LU_->mH();
      // B = U^{-H} @ B
      at::linalg_solve_triangular_out(
          const_cast<Tensor&>(B), LU_H, B, /*upper=*/false);
      // B = L^{-H} @ B
      at::linalg_solve_triangular_out(
          const_cast<Tensor&>(B),
          LU_H,
...
#ifdef CUDART_VERSION
  auto lu_solve_batched_cublas_fn = [](const Tensor& LU,
                                       const Tensor& pivots,
                                       const Tensor& B,
                                       TransposeType trans) {
    auto LU_ = maybe_expand_lu(B, LU);
  ...
  auto lu_solve_batched_magma_fn = [](const Tensor& LU,
                                      const Tensor& pivots,
                                      const Tensor& B,
                                      TransposeType trans) {
    auto LU_ = maybe_expand_lu(B, LU);
  ...
  // MAGMA is faster for large matrices, but it is buggy for trans !=
  // NoTranspose or for large batches. LU solve is fast in some cases when
  // adjoint=True
#ifdef CUDART_VERSION
#ifdef USE_CUSOLVER
  if (batch_size <= 2 && m >= 64) {
  ...
  if (trans != TransposeType::NoTranspose && m <= 2 && batch_size >= 128) {
    lu_solve_triangular(LU, pivots, B, trans);
      ...
      !over_batched_magma_dim_limit && trans == TransposeType::NoTranspose &&
      m >= 256 && batch_size >= 128) {
    lu_solve_batched_magma_fn(LU, pivots, B, trans);
  }
#endif
  else {
...
#else
  // If it's not buggy and it's faster than solve triangular (large matrix and
  // large batch regime) we use batched_magma, otherwise, we resort to two
  // triangular solves For trans != TransposeType::NoTranspose
  // lu_solve_triangular is faster anyway
  if (!over_batched_magma_dim_limit && trans == TransposeType::NoTranspose &&
      m >= 256 && batch_size >= 128) {
    lu_solve_batched_magma_fn(LU, pivots, B, trans);
  } else {
...
std::tuple<Tensor, Tensor> legacy_lstsq_cuda(const Tensor& B, const Tensor& A) {
  ...
  TORCH_CHECK(
      ...
      B.scalar_type());
  TORCH_CHECK(
      A.numel() > 0 && A.dim() == 2, "A should be (non-empty) 2 dimensional");
  TORCH_CHECK(
      B.numel() > 0 && B.dim() == 2, "B should be (non-empty) 2 dimensional");
