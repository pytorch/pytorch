...
__device__ void binary_op_update(
    const scalar_t lhs,
    scalar_t& rhs,
    const idx_t lhs_idx,
    idx_t& rhs_idx,
    BinaryOperation binary_op) {
  if (!THCNumerics<scalar_t>::isnan(rhs) &&
      (THCNumerics<scalar_t>::isnan(lhs) || !binary_op(rhs, lhs))) {
    rhs = lhs;
    rhs_idx = lhs_idx;
  }
}
 ...
 *
 * The outer dimensions of the tensor are considered as a single dimension, i.e.
 * the tensor is considered as having 'num_rows' rows of size 'row_size'. Each
 * thread block processes one or more sets of contiguous rows (processing
 * multiple rows per thread block is quicker than processing a single row,
 * especially for short rows).
 */
template <
    typename scalar_t,
    int num_threads_x,
    ...
    for (int block_col = 0; block_col < row_size;
      ...
      if (row < num_rows) {
        if (col1 < row_size) {
          ...
          // No need to set the index here as the value in init will never be
          // selected
        }
 ...
 *
 * The dimensions to the outside and inside of the specified dimension are
 * considered as flattened. Thread blocks with the same blockIdx.y process an
 * "outer row" (i.e. an element of the flattened outer dimensions, which
 * contains several "inner rows"). Each thread processes a single inner row at a
 * time.
 */
template <typename scalar_t, class BinaryFunction>
__global__ void tensor_kernel_scan_outer_dim_with_indices(
    scalar_t* self_,
    scalar_t* values_,
    int64_t* indices_,
  ...
  for (uint32_t orow = blockIdx.x; orow < num_orows; orow += gridDim.x) {
    for (uint32_t irow = blockIdx.y * blockDim.x + threadIdx.x;
      ...
      for (auto col = decltype(row_size){0}; col < row_size; ++col) {
        if (THCNumerics<scalar_t>::isnan(*self) ||
            (!THCNumerics<scalar_t>::isnan(out) && binary_op(*self, out))) {
          out = *self;
          out_idx = col;
        }
        *values = out;
...
__host__ void scan_outer_dim_with_indices(
  ...
  const int64_t num_orows =
      c10::multiply_integers(sizes.begin(), sizes.begin() + dim);

  // Treat all inner dimensions (i.e. dim > dimension) as one.
  const int64_t num_irows =
      c10::multiply_integers(sizes.begin() + dim + 1, sizes.end());
  // for performance reasons, cuda kernels use uint32_t for loops over irows,
  // orows and row, make sure that input is not bigger than supported by uint32_t
  ...
  dim3 grid(
      ...
      std::min(maxGridDim, ceil_div(num_irows, int64_t{threads.x})));
  tensor_kernel_scan_outer_dim_with_indices<scalar_t>
...
__host__ void scan_innermost_dim_with_indices(
  ...
  dim3 grid(std::min(
      ...
      ceil_div(num_rows, int(threads.y))));

  tensor_kernel_scan_innermost_dim_with_indices<scalar_t, 16, 32>
      <<<grid, threads, 0, at::cuda::getCurrentCUDAStream()>>>(
          self.data_ptr<scalar_t>(),
          values.data_ptr<scalar_t>(),
          indices.data_ptr<int64_t>(),
...
void scan_dim_with_indices(
  ...
  if (dim == ndim - 1) {
    scan_innermost_dim_with_indices<scalar_t>(
        self_, values_, indices_, init, binary_op);
  } else {
    scan_outer_dim_with_indices<scalar_t>(
        self_, values_, indices_, dim, init, binary_op);
  }
  if (copy_values) {
...
void cummax_helper_cuda(
  ...
  AT_DISPATCH_ALL_TYPES_AND2(
      ...
      [&]() {
        ...
        scan_dim_with_indices<scalar_t>(
...
__global__ void tensor_kernel_scan_outer_dim(
    scalar_t* tgt_,
    scalar_t* src_,
    const uint32_t num_orows,
    const uint32_t num_irows,
...
template <
    ...
    int num_threads_y,
    class BinaryFunction>
__device__ void tensor_kernel_scan_innermost_dim_impl(
    T* row_buf,
    T* tgt_,
    T* src_,
...
__host__ void scan_outer_dim(
  ...
  tensor_kernel_scan_outer_dim<scalar_t>
      <<<grid, threads, 0, at::cuda::getCurrentCUDAStream()>>>(
...
void scan_innermost_dim(
  ...
  check_fits_in_unsigned(
      num_rows, "Number of rows (self.numel()/self.size(self.dim()-1))");
  check_fits_in_unsigned(row_size, "row_size");

  tensor_kernel_scan_innermost_dim<scalar_t, 16, 32>
      <<<grid, threads, 0, at::cuda::getCurrentCUDAStream()>>>(
          result.data_ptr<scalar_t>(),
          self.data_ptr<scalar_t>(),
          num_rows,
...
__global__ void transform_vals(
    scalar_t* a,
    scalar_t* b,
    scalar_t* out,
    func_t binary_op) {
...
void scan_cub(
  ...
  // even though cub is supposed to support tensors with int_max elements, in
  // reality it doesn't, so split at int_max/2
...
Tensor& _logcumsumexp_out_cuda(
  ...
  AT_DISPATCH_FLOATING_TYPES_AND(
      at::ScalarType::Half, self.scalar_type(), "logcumsumexp_cuda", [&]() {
        using accscalar_t = acc_type<scalar_t, true>;
        scalar_t init = -std::numeric_limits<scalar_t>::infinity();
        auto log_add_exp = [] C10_HOST_DEVICE(
                               const scalar_t x, const scalar_t y) -> scalar_t {
          scalar_t min = at::_isnan(y)
              ? y
              : std::min<scalar_t>(
                    x,
                    y); // std::min returns first arg if one of the args is nan
          scalar_t max = at::_isnan(y)
              ? y
              : std::max<scalar_t>(
                    x,
                    y); // std::max returns first arg if one of the args is nan
...
Tensor& _cumsum_out_cuda(const Tensor& self, int64_t dim, Tensor& result) {
  ...
  AT_DISPATCH_ALL_TYPES_AND_COMPLEX_AND(
      at::ScalarType::Half, self.scalar_type(), "cumsum_cuda", [&]() {
        scalar_t init = 0;
        scan_dim<scalar_t>(self, result, wrap_dim, init, std::plus<scalar_t>());
      });
...
Tensor& _cumprod_out_cuda(const Tensor& self, int64_t dim, Tensor& result) {
  ...
  AT_DISPATCH_ALL_TYPES_AND_COMPLEX_AND(
      at::ScalarType::Half, self.scalar_type(), "cumprod_cuda", [&]() {
        scalar_t init = 1;
        scan_dim<scalar_t>(
            self, result, wrap_dim, init, std::multiplies<scalar_t>());
      });
