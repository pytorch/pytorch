...
void nonzero_cuda_out_impl(const Tensor& self, Tensor& out) {
  ...
  cub::TransformInputIterator<bool, NonZeroOp<scalar_t>, scalar_t*> itr(
      self_.data_ptr<scalar_t>(), NonZeroOp<scalar_t>());
  cub::DeviceReduce::Sum(
      nullptr, temp_storage_bytes, itr, (int*)num_nonzeros.get(), N, stream);
  ...
  // we are producing output with size {num_nonzeros, ndim} and strides
  // {num_nonzeros, 1} (that is, transposed ndim x num_nonzeros output) we are
  // able to directly use passed output with this size and strides, and we can
  // also (per contract) resize passed output with incorrect sizes anyway we
  // want. However, out with correct sizes and incorrect strides will have to be
  ...
  at::Tensor out_temp = need_to_copy
      ? at::native::empty_cuda(
            {self.dim(), num_nonzeros_h},
            optTypeMetaToScalarType(out.options().dtype_opt()),
            out.options().layout_opt(),
            out.options().device_opt(),
            out.options().pinned_memory_opt())
  ...
  if (self.dim() > 0) {
    ...
    if (num_nonzeros_h > 0 && self.dim() > 1) {
      ...
      write_indices<<<nblocks, nthreads, 0, stream>>>(
          out_temp.data_ptr<int64_t>(), dims, self.dim(), num_nonzeros_h);
...
Tensor nonzero_cuda(const Tensor& self) {
  Tensor out = at::native::empty_cuda(
      {0},
      kLong,
