...
// Temporary wrapper for DeviceSelect::If from https://github.com/NVIDIA/cccl/pull/1379
// until CCCL https://github.com/NVIDIA/cccl/issues/1422 is resolved
  ...
  size_t temp_storage_bytes=0;
    ...
    if (self.dim() > 0) {
        ...
        if (num_nonzeros_h > 0 && self.dim() > 1){
            ...
            write_indices<<<nblocks, nthreads, 0, stream>>>(out_temp.mutable_data_ptr<int64_t>(), dims, self.dim(), num_nonzeros_h);
...
Tensor& nonzero_out_cuda(const Tensor& self, Tensor& out) {
    TORCH_CHECK(self.numel() < std::numeric_limits<int64_t>::max(), "nonzero is not supported for tensors with more than INT64_MAX elements.");
    TORCH_CHECK(out.dtype() == at::kLong, "Expected object of scalar type ", at::kLong, " as out, but got ", out.dtype());
    TORCH_CHECK(self.device() == out.device(), "Expected self and out to be on the same device, but got out on ", out.device(), " and self on ", self.device());
    TORCH_CHECK(self.dim() <= MAX_DIMS, "nonzero is not supported for tensor with more than ", MAX_DIMS, " dimensions");
    AT_DISPATCH_ALL_TYPES_AND_COMPLEX_AND4(at::ScalarType::ComplexHalf, at::ScalarType::Bool, at::ScalarType::BFloat16, at::ScalarType::Half, self.scalar_type(), "nonzero_cuda", [&] {nonzero_cuda_out_impl<scalar_t>(self, out);});
    return out;
}
