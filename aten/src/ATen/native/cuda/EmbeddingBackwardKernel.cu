...
__global__ void compute_grad_weight_atomic_accumulate(
  ...
  using accscalar_t = acc_type<scalar_t, true>;
  const int64_t gid = static_cast<int64_t>(blockIdx.x) * blockDim.x + threadIdx.x;
  const int64_t partial_id = gid / stride_warped;
  const int64_t startFeature = gid % stride_warped;
...
Tensor embedding_backward_cuda_kernel(
  ...
  AT_DISPATCH_INDEX_TYPES(orig_indices.scalar_type(), "embedding_backward_cuda_kernel", [&] () {
    ...
    const int64_t sum_scatter_grid = ceil_div(max_segments * stride_warped, block);
    const int64_t num_sms = at::cuda::getCurrentDeviceProperties()->multiProcessorCount;
    const int64_t min_blocks_for_good_occupancy = num_sms * MAX_ATOMIC_ACCUM_BLOCKS_PER_SM;
    ...
    AT_DISPATCH_FLOATING_TYPES_AND2(at::ScalarType::Half, at::ScalarType::BFloat16,
      grad.scalar_type(), "embedding_bag_backward_cuda_compute_grad_weight", [&] {
        ...
        if (use_fused_kernel) {
          ...
          // For half/bfloat16 types, use acc_type (float) intermediate buffer for atomic adds
          ...
          if (grad.dtype() == at::kHalf || grad.dtype() == at::kBFloat16) {
            auto grad_weight_acc = at::zeros({num_weights, stride}, grad.options().dtype(at::kFloat));
