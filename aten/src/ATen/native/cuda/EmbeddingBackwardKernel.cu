...
__global__ void compute_grad_weight_bags(
  ...
  for (int idx = idx_begin; idx < idx_end; ++idx) {
    ...
    acc_type<scalar_t, true> gradient =
        gradOutput[gradOutputRow + startFeature];
    if (mode_mean) {
...
Tensor embedding_backward_cuda_kernel(
  ...
  // Compute the number of segments and their start position so that we do not
  // have to spawn a warp per index. In this context, a segment is a number of
  // rows that should be summarized. Unit: index in `sorted_indices` and
  // `orig_indices`
  AT_DISPATCH_INDEX_TYPES(
      orig_indices.scalar_type(), "embedding_backward_cuda_kernel", [&]() {
        ...
        int64_t num_of_segments;
        {
          ...
          auto dummy_dev =
              thrust::device_ptr<index_t>(dummy.data_ptr<index_t>());
        ...
        // Compute the number partial-segments per segment (some
        // partial-segments may not be the full `NROWS_PER_THREAD` number of
        // rows)
        auto partials_per_segment =
            at::empty({num_of_segments}, orig_indices.options());
        {
          krn_partials_per_segment<<<
              ceil_div(num_of_segments, 32),
              32,
              0,
              stream>>>(
        ...
        // In order to compute `partial_segment_offset`, which is the start
        // index of each partial-segment in `sorted_indices`, we need to compute
        // the start position of each _segment_ in `partial_segment_offset`.
        // Unit: index in `partial_segment_offset`
        auto partials_per_segment_offset =
            at::empty({num_of_segments}, orig_indices.options());
        thrust::exclusive_scan(
            policy,
            thrust::device_ptr<index_t>(
                partials_per_segment.data_ptr<index_t>()),
            thrust::device_ptr<index_t>(
                partials_per_segment.data_ptr<index_t>() + num_of_segments),
            thrust::device_ptr<index_t>(
        ...
        const int num_of_partial_segments =
            partials_per_segment[num_of_segments - 1].item<index_t>() +
        ...
        const int stride_warped =
            ceil_div(stride, C10_WARP_SIZE) * C10_WARP_SIZE;
        const int block = std::min(stride_warped, MAX_BLOCK_SIZE);
        const int grid =
            ceil_div(num_of_partial_segments * stride_warped, block);
        ...
        AT_DISPATCH_FLOATING_TYPES_AND2(
            ...
            [&] {
              ...
              // into `grad_weight`.
              const int grid2 =
                  ceil_div(num_of_segments * stride_warped, block);
