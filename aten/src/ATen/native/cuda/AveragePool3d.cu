...
__global__ void avg_pool3d_cuda_update_output(
  ...
  if (oRow < output.size(2) && oCol < output.size(3)) {
    ...
    output[slice][oFrame][oRow][oCol] =
        ScalarConvert<accscalar_t, scalar_t>::to(sum / divide_factor);
  }
}
...
__global__ void avg_pool3d_single_backward_out_frame_stride1(
  ...
  if (iRow < gradInput.size(2) && iCol < gradInput.size(3)) {
    ...
    gradInput[slice][iFrame][iRow][iCol] =
        ScalarConvert<accscalar_t, scalar_t>::to(sum * normFactor);
  }
}
...
__global__ void avg_pool3d_cuda_update_grad_input_atomic(
  ...
  int oRow = blockIdx.y * blockDim.y + threadIdx.y;
  int oFrame =
      (blockIdx.z + offsetZ) % gradOutput.size(1); // gradOutput frame/time
  int slice =
      (blockIdx.z + offsetZ) / gradOutput.size(1); // gradOutput slice/feature
  ...
  if (oRow < gradOutput.size(2) && oCol < gradOutput.size(3)) {
    ...
    scalar_t val = ScalarConvert<accscalar_t, scalar_t>::to(
        ScalarConvert<scalar_t, accscalar_t>::to(
            gradOutput[slice][oFrame][oRow][oCol]) /
        divide_factor);
...
#define LAUNCH_UPDATE_OUTPUT_KERNEL_WIDTH(KW)                   \
  case KW:                                                      \
...
void avg_pool3d_out_cuda_template(
  ...
  AT_DISPATCH_FLOATING_TYPES_AND2(
      kHalf, kBFloat16, input.scalar_type(), "avg_pool3d_out_cuda", [&] {
        ...
        while (totalZ > 0) {
          dim3 grid(
              cuda::ATenCeilDiv(owidth, static_cast<int64_t>(block.x)),
  ...
  if (dT == 1 && dH == 1 && dW == 1 && padT == 0 && padH == 0 && padW == 0) {
    AT_DISPATCH_FLOATING_TYPES_AND2(
        ...
        [&] {
          ...
          while (totalZ > 0) {
            dim3 grid(
                cuda::ATenCeilDiv(iwidth, static_cast<int64_t>(block.x)),
