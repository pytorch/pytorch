...
struct ForwardResult {
  Tensor loss;
  std::optional<SavedForBackward> saved;
};
...
Tensor linear_cross_entropy_cuda(
  ...
  const int64_t vocab_chunk_size = vocab_chunk_size_opt.value_or(lce_cuda::kDefaultVocabChunkSize);
  const int64_t batch_chunk_size = batch_chunk_size_opt.value_or(lce_cuda::kDefaultBatchChunkSize);
  const lce_cuda::ChunkingStrategy strategy = lce_cuda::select_chunking_strategy(chunking_strategy);

  auto result = lce_cuda::forward_impl(
      input,
      linear_weight,
      target,
      linear_bias_opt,
...
std::tuple<Tensor, Tensor, std::optional<Tensor>> linear_cross_entropy_backward_cuda(
  ...
  lce_cuda::ChunkingStrategy resolved_strategy = lce_cuda::select_chunking_strategy(chunking_strategy);

  if (resolved_strategy == lce_cuda::ChunkingStrategy::VOCAB_CHUNKING) {
      return lce_cuda::backward_vocabulary_chunking(
          input,
          linear_weight,
          target,
          linear_bias_opt,
          ...
          ignore_index,
          label_smoothing,
          vocab_chunk_size_opt.value_or(lce_cuda::kDefaultVocabChunkSize),
          Tensor(),
          /*has_saved=*/false);
  }

  const int64_t default_chunk = resolved_strategy == lce_cuda::ChunkingStrategy::BATCH_CHUNKING
      ? batch_chunk_size_opt.value_or(lce_cuda::kDefaultBatchChunkSize)
      : std::max<int64_t>(flattened_batch, 1);
  return lce_cuda::backward_batch_chunking(
      input,
      linear_weight,
      target,
      linear_bias_opt,
...
std::tuple<Tensor, Tensor> _linear_cross_entropy_vocab_chunking_cuda(
    const Tensor& input,
    const Tensor& linear_weight,
    const Tensor& target,
    const std::optional<Tensor>& linear_bias_opt,
    ...
    double label_smoothing,
    int64_t chunk_size) {
  auto forward = lce_cuda::forward_vocab_chunking(
      input,
      linear_weight,
      target,
      linear_bias_opt,
...
std::tuple<Tensor, Tensor, std::optional<Tensor>> _linear_cross_entropy_vocab_chunking_backward_cuda(
    const Tensor& grad_output,
    const Tensor& saved_logsumexp,
    const Tensor& input,
    const Tensor& linear_weight,
...
std::tuple<Tensor, Tensor, Tensor, Tensor, Tensor, Tensor> _linear_cross_entropy_batch_chunking_cuda(
    const Tensor& input,
    const Tensor& linear_weight,
    const Tensor& target,
    const std::optional<Tensor>& linear_bias_opt,
    ...
    double label_smoothing,
    int64_t chunk_size) {
  auto forward = lce_cuda::forward_batch_chunking(
      input,
      linear_weight,
      target,
      linear_bias_opt,
...
std::tuple<Tensor, Tensor, std::optional<Tensor>> _linear_cross_entropy_batch_chunking_backward_cuda(
    const Tensor& grad_output,
  ...
  auto grad_output_tensor = lce_cuda::cast_grad_output(grad_output, input);
  ...
  std::optional<Tensor> grad_bias_opt;
  if (lce_cuda::has_defined_tensor(linear_bias_opt)) {
    if (!grad_bias_flag) {
      auto fallback = lce_cuda::backward_batch_chunking(
          input,
          linear_weight,
          target,
          linear_bias_opt,
