...
void gpu_kernel(TensorIteratorBase& iter, const func_t& f) {
  for (int arg = 0; arg < iter.ntensors(); arg++) {
    TORCH_INTERNAL_ASSERT(
        ...
        "argument ",
        arg,
        ": expected a CUDA device but found ",
        iter.device(arg));
  }
...
template <typename... T>
struct is_tuple<thrust::tuple<T...>> : std::true_type {};
...
static inline void launch_unrolled_kernel_for_multi_outputs(
  ...
  unrolled_elementwise_kernel_for_multi_outputs<num_outputs, func_t, array_t>
      <<<grid, num_threads, 0, stream>>>(N, f, data, ic, oc);
...
void gpu_kernel_multiple_outputs_impl(
  ...
  static_assert(
      is_tuple<output_t>::value, "f's return type must be `thrust::tuple`");
