...
#define FOREACH_MAX_MIN_OP(NAME, OP)                                                                      \
std::vector<Tensor> foreach_tensor_##NAME##_cuda(TensorList tensors1, TensorList tensors2) {              \
    ...
    AT_DISPATCH_ALL_TYPES_AND(kHalf, tensors1[0].scalar_type(), "foreach_pointwise_op_cuda", [&]() {      \
        using opmath_t = get_opmath_t<scalar_t>::opmath_t;                                                \
        auto op = []  GPU_LAMBDA (opmath_t a, opmath_t b) -> opmath_t { return OP(a, b); };               \
        multi_tensor_apply<3>(tensor_lists,                                                               \
...
FOREACH_MAX_MIN_OP(max, std::max)
FOREACH_MAX_MIN_OP(min, std::min)
