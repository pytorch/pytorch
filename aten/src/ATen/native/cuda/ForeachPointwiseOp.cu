...
std::vector<Tensor> foreach_pointwise_op(
  ...
  AT_DISPATCH_ALL_TYPES_AND(
      kHalf, input[0].scalar_type(), "foreach_pointwise_op_cuda", [&]() {
...
void foreach_pointwise_op_(
  ...
  AT_DISPATCH_ALL_TYPES_AND(
      kHalf, input[0].scalar_type(), "foreach_pointwise_op__cuda", [&]() {
        using opmath_t = get_opmath_t<scalar_t>::opmath_t;
...
#define FOREACH_MAXIMUM_MINIMUM_OP(NAME, OP)                               \
  std::vector<Tensor> foreach_tensor_##NAME##_cuda(                        \
    ...
    AT_DISPATCH_ALL_TYPES_AND(                                             \
        ...
        "foreach_maximum_minimum_op_cuda",                                 \
        [&]() {                                                            \
          using opmath_t = get_opmath_t<scalar_t>::opmath_t;               \
