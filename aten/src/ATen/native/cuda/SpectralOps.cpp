...
static inline CuFFTParamsLRUCache& cufft_get_plan_cache(int64_t device_index) {
...
static const Tensor& _exec_fft(
  ...
  CuFFTParamsLRUCache& plan_cache =
      cufft_get_plan_cache(input.device().index());
  ...
  auto workspace = at::empty(
      {config->workspace_size()}, at::device(at::kCUDA).dtype(at::kByte));
...
Tensor& _fft_r2c_cufft_out(
    const Tensor& self,
    IntArrayRef dim,
    int64_t normalization,
    bool onesided,
    Tensor& out) {
  auto result = _fft_r2c_cufft(
...
Tensor& _fft_c2r_cufft_out(
    const Tensor& self,
    IntArrayRef dim,
    int64_t normalization,
    int64_t lastdim,
    Tensor& out) {
  auto result = _fft_c2r_cufft(
...
Tensor& _fft_c2c_cufft_out(
    const Tensor& self,
    IntArrayRef dim,
    int64_t normalization,
    bool forward,
    Tensor& out) {
  auto result = _fft_c2c_cufft(
