...
bool maybe_tiled_transpose_copy(TensorIterator& iter) {
  ...
  const int vec = es < 4 ? 4 / es : 1;
  const bool vectorized = es <= 4 && w % (4 * vec) == 0 && h % (4 * vec) == 0 &&
      src_pitch % (4 * vec) == 0 && dst_pitch % (4 * vec) == 0 &&
      reinterpret_cast<uintptr_t>(sp) % 16 == 0 &&
      reinterpret_cast<uintptr_t>(dp) % 16 == 0;
  // Narrow vectorized copies benefit from tiling at smaller sizes.
  const int64_t min_bytes = vectorized && es < 4
  ...
  const int access_size = vectorized ? 4 : 1;
  ...
  if (vectorized) {
    if (es == 1) {
      launch_tiled_transpose<uint32_t, 4, 4>(needs_stride, grid, block, stream,
          sp, dp, w / vec, h / vec, src_pitch / vec, dst_pitch / vec);
    } else if (es == 2) {
      launch_tiled_transpose<uint32_t, 2, 4>(needs_stride, grid, block, stream,
          sp, dp, w / vec, h / vec, src_pitch / vec, dst_pitch / vec);
    } else {
      launch_tiled_transpose<uint32_t, 1, 4>(needs_stride, grid, block, stream,
