...
__global__ void kernel_pointwise_flip_apply2(
  ...
  for (IndexType linear_index = blockIdx.x * blockDim.x + threadIdx.x;
       linear_index < N;
       linear_index += gridDim.x * blockDim.x) {
    IndexType dst_offset = 0;
    if (flip_dim == 0) {
      // flip 1st dim
      dst_offset = (in_tensor_info.sizes[0] - 1 -
                    linear_index / in_tensor_info.strides[0]) *
              in_tensor_info.strides[0] +
          linear_index % in_tensor_info.strides[0];
    } else {
      // flip last dim
      IndexType i = total_dims - 1;
      dst_offset =
          linear_index / in_tensor_info.strides[0] * in_tensor_info.strides[0] +
          (in_tensor_info.sizes[i] - 1 -
           linear_index % in_tensor_info.strides[0]);
    }
...
Tensor flip_cuda(const Tensor& self, IntArrayRef dims) {
  auto in_tensor = self;
  const int64_t flip_dims_size = dims.size(), total_dims = in_tensor.dim(),
                N = in_tensor.numel();
  ...
  // use kernel_pointwise_flip_apply2 only when to-flip dim is the 1st or last
  // dim, where collapseDims can reduce the amount of work
  if (flip_dims_size == 1 && in_tensor.is_contiguous() &&
      (flip_dims[0] == 0 || flip_dims[0] == total_dims - 1)) {
    AT_DISPATCH_ALL_TYPES_AND_COMPLEX_AND3(
        kHalf, kBool, kBFloat16, in_tensor.scalar_type(), "flip_cuda", [&] {
          auto in_tensor_info =
              cuda::detail::getTensorInfo<scalar_t, int64_t>(in_tensor);
          auto out_tensor_info =
              cuda::detail::getTensorInfo<scalar_t, int64_t>(out_tensor);
  ...
  auto flip_dims_t = at::from_blob(
      flip_dims.data(),
      {static_cast<int64_t>(flip_dims.size())},
  ...
  // stride_contiguous is the stride of non-contiguous tensor after calling
  // contiguous(), it is used to compute indices for each element in
  ...
  for (int64_t i = total_dims - 1; i >= 0; i--) {
    if (i == total_dims - 1) {
      ...
      stride_contiguous_d[i] =
          std::max<int64_t>(shape[i + 1], 1) * stride_contiguous_d[i + 1];
    }
  }
...
Tensor roll_cuda(const Tensor& self, IntArrayRef shifts, IntArrayRef dims) {
  ...
  dim3 dim_grid;
  TORCH_CHECK(
      cuda::getApplyGrid(N, dim_grid, in_tensor.get_device()),
