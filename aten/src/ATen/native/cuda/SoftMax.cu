...
struct LogSoftMaxBackwardEpilogue {
  ...
  __device__ __forceinline__ T operator()(OutT gradOutput, OutT output) const {
    return static_cast<T>(
        gradOutput - std::exp(static_cast<AccumT>(output)) * sum);
  }
...
struct SoftMaxBackwardEpilogue {
  __device__ __forceinline__ SoftMaxBackwardEpilogue(AccumT sum) : sum(sum) {}
...
// The 2d grid is used to parallelize inner dimension over y axis and outer over
// x.
...
void SpatialSoftMax_getLaunchSizes(
  ...
  cudaOccupancyMaxActiveBlocksPerMultiprocessor(
      &max_blocks, k, block_threads, smem_size);
  ...
  grid = SpatialSoftMax_getGridSize(
      block, max_active_blocks, outer_size, dim_size, inner_size);
}
...
inline dim3 SoftMax_getBlockSize(int ILP, uint64_t dim_size) {
  ...
  // be accessed in a vectorized way against wanting a larger block size to get
  // better utilisation. In general with ILP you can have (ILP-1)/ILP of the
  // buffer accessed vectorised, at the risk of having a very small block size.
  // We choose to keep >= 1/2 of the buffer vectorised while allowing a larger
  // block size.
  if (ILP > 1) {
    max_block_size /= 2;
  }
    ...
    for (uint32_t inner_index = blockIdx.y * blockDim.y + threadIdx.y;
      ...
      if (blockDim.x > 1) {
        ...
        for (uint32_t d = threadIdx.x; d < dim_size; d += blockDim.x) {
          const accscalar_t value =
              static_cast<accscalar_t>(input[data_offset + d * dim_stride]);
  ...
  for (uint32_t outer_index = blockIdx.x; outer_index < outer_size;
    ...
    for (uint32_t inner_index = blockIdx.y * blockDim.y + threadIdx.y;
      ...
      if (blockDim.x > 1) {
        ...
        for (uint32_t d = threadIdx.x; d < dim_size; d += blockDim.x) {
          gradInput[data_offset + d * dim_stride] = epilogue(
              gradOutput[data_offset + d * dim_stride],
              output[data_offset + d * dim_stride]);
        }
      } else {
        accscalar_t sum = 0;
...
__device__ __forceinline__ AccumT blockReduce(
    AccumT* smem,
    AccumT val,
...
template <
    ...
    typename T,
    typename AccumT>
__device__ __forceinline__ AccumT ilpReduce(
    int shift,
    T* data,
    int size,
...
/**
 * This will apply the Epilogue with vectorized reads & writes when input &
...
/**
 * This will apply the Epilogue with non-vectrorized reads & writes for the
 * general case
 */
template <
    int ILP,
...
#pragma unroll
    for (int j = 0; j < ILP; ++j) {
      gradInput[offset + j * blockDim.x] =
          epilogue(tmpGradOutput[j], tmpOutput[j]);
    }
  }
...
template <
    ...
    template <typename, typename, typename>
    class Epilogue>
__global__ void cunn_SoftMaxForward(
    outscalar_t* output,
    scalar_t* input,
    int classes) {
  extern __shared__ unsigned char smem[];
  ...
  const int output_shift =
      ((uint64_t)output) % ALIGN_BYTES / sizeof(outscalar_t);
  ...
  accscalar_t threadExp = ilpReduce<SumExpFloat, ILP, scalar_t, accscalar_t>(
      ...
      input,
      classes,
      SumExpFloat<scalar_t, accscalar_t>(max_k),
  ...
  if (shift == output_shift) {
    WriteFpropResultsVectorized<
        ...
        accscalar_t,
        outscalar_t,
        Epilogue>(classes, shift, input, output, epilogue);
  } else {
    WriteFpropResults<ILP, scalar_t, accscalar_t, outscalar_t, Epilogue>(
...
template <
    ...
    template <typename, typename, typename>
    class Epilogue>
__global__ void cunn_SoftMaxBackward(
    scalar_t* gradInput,
    outscalar_t* output,
    outscalar_t* gradOutput,
    int classes) {
  ...
  const int output_shift =
      ((uint64_t)output) % ALIGN_BYTES / sizeof(outscalar_t);
  const int grad_output_shift =
      ((uint64_t)gradOutput) % ALIGN_BYTES / sizeof(outscalar_t);

  accscalar_t threadSum = ilpReduce<AddFloat, ILP, outscalar_t, accscalar_t>(
      grad_output_shift,
      gradOutput,
      classes,
      AddFloat<outscalar_t, accscalar_t>(),
      accscalar_t(0));
  ...
  if (shift == output_shift && shift == grad_output_shift) {
    WriteBpropResultsVectorized<
        ...
        accscalar_t,
        outscalar_t,
        Epilogue>(classes, shift, gradInput, output, gradOutput, epilogue);
  } else {
    WriteBpropResults<ILP, scalar_t, accscalar_t, outscalar_t, Epilogue>(
        classes, gradInput, output, gradOutput, epilogue);
  }
}
...
Tensor host_softmax(
  ...
  Tensor output = half_to_float
      ? at::empty_like(
            input,
            input.options().dtype(ScalarType::Float),
            LEGACY_CONTIGUOUS_MEMORY_FORMAT)
      : at::empty_like(input, LEGACY_CONTIGUOUS_MEMORY_FORMAT);
  static_assert(
      std::is_same<acc_type<at::Half, true>, float>::value,
  ...
  if (input.numel() > 0) {
    ...
    if (inner_size == 1) {
      ...
      // This kernel runs in a 2D grid, where each application along y dimension
      // has a fixed outer_size, and runs in parallel over inner_size. Dimension
      // x is parallel over outer_size. Reductions over dim are done in a
          ...
          [&] {
            ...
            if (!half_to_float) {
              ...
              cunn_SpatialSoftMaxForward<
                  ...
                  accscalar_t,
                  scalar_t,
                  Epilogue><<<grid, block, smem_size, stream>>>(
                  output.data_ptr<scalar_t>(),
                  input.data_ptr<scalar_t>(),
                  outer_size,
                  dim_size,
...
Tensor host_softmax_backward(
  ...
  Tensor gI = half_to_float
      ? at::empty_like(
            grad_,
            grad_.options().dtype(ScalarType::Half),
            LEGACY_CONTIGUOUS_MEMORY_FORMAT)
...
Tensor log_softmax_cuda(
    const Tensor& input,
    const int64_t dim,
    const bool half_to_float) {
  return host_softmax<LogSoftMaxForwardEpilogue, true>(
      input, dim, half_to_float);
}

Tensor log_softmax_backward_cuda(
    const Tensor& grad,
    const Tensor& output,
    int64_t dim,
    const Tensor& input) {
  ...
  return host_softmax_backward<LogSoftMaxBackwardEpilogue, true>(
...
Tensor softmax_backward_cuda(
  ...
  return host_softmax_backward<SoftMaxBackwardEpilogue, false>(
