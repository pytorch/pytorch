...
cunn_SoftMaxBackwardSmem(scalar_t *gradInput, const outscalar_t *output, const outscalar_t *gradOutput, int64_t classes)
{
  ...
  // Download inputs to shared memory while doing the first step
  // in sum calculation
  for (int32_t offset = threadIdx.x; offset * ILP < classes;
       offset += blockDim.x) {
...
#pragma unroll
  ...
  accscalar_t sum_k = blockReduceWarp<Add, accscalar_t>(
      smem_reduction_cache, threadSum, Add<accscalar_t>(), accscalar_t(0));
          ...
          [&] {
            ...
            if (!half_to_float) {
              ...
              if (dim_size <= 2048 && dim_size * sizeof(scalar_t) <= 8192) {
                ...
                size_t smem_reduction_sz =
                    block.x / C10_WARP_SIZE * sizeof(accscalar_t);
                ...
                can_use_smem &=
                    !(reinterpret_cast<uintptr_t>(input_ptr) % ALIGN_BYTES);
                can_use_smem &=
                    (!(reinterpret_cast<uintptr_t>(output_ptr) % ALIGN_BYTES));
                can_use_smem &= !(dim_size % ILP);

                int32_t potential_reg_cnt =
                    potential_register_count(dim_size, block.x);
                if (potential_reg_cnt < 10) {
                  TORCH_INTERNAL_ASSERT(
                      potential_reg_cnt > 0,
                      "potential_reg_cnt for softmax with register should be greater than 0.");
                  switch (potential_reg_cnt) {
                    // TODO(Wenqin): try to investigate why we couldn't use
                    // macro for below code, because it seems on MSVS, it seems
                    // the macro way didn't expand correct.
                    case 1:
                      cunn_SoftMaxForwardReg<
                          ...
                          Epilogue,
                          int64_t,
                          1><<<grid, block, smem_reduction_sz, stream>>>(
                    ...
                    case 2:
                      cunn_SoftMaxForwardReg<
                          ...
                          Epilogue,
                          int64_t,
                          2><<<grid, block, smem_reduction_sz, stream>>>(
                          output_ptr, input_ptr, dim_size);
                      break;
                    case 3:
                      cunn_SoftMaxForwardReg<
                          ...
                          Epilogue,
                          int64_t,
                          3><<<grid, block, smem_reduction_sz, stream>>>(
                          output_ptr, input_ptr, dim_size);
                      break;
                    case 4:
                      cunn_SoftMaxForwardReg<
                          ...
                          Epilogue,
                          int64_t,
                          4><<<grid, block, smem_reduction_sz, stream>>>(
                          output_ptr, input_ptr, dim_size);
                      break;
                    case 5:
                      cunn_SoftMaxForwardReg<
                          ...
                          Epilogue,
                          int64_t,
                          5><<<grid, block, smem_reduction_sz, stream>>>(
                          output_ptr, input_ptr, dim_size);
                      break;
                    case 6:
                      cunn_SoftMaxForwardReg<
                          ...
                          Epilogue,
                          int64_t,
                          6><<<grid, block, smem_reduction_sz, stream>>>(
                          output_ptr, input_ptr, dim_size);
                      break;
                    case 7:
                      cunn_SoftMaxForwardReg<
                          ...
                          Epilogue,
                          int64_t,
                          7><<<grid, block, smem_reduction_sz, stream>>>(
                          output_ptr, input_ptr, dim_size);
                      break;
                    case 8:
                      cunn_SoftMaxForwardReg<
                          ...
                          Epilogue,
                          int64_t,
                          8><<<grid, block, smem_reduction_sz, stream>>>(
                          output_ptr, input_ptr, dim_size);
                      break;
                    case 9:
                      cunn_SoftMaxForwardReg<
                          ...
                          Epilogue,
                          int64_t,
                          9><<<grid, block, smem_reduction_sz, stream>>>(
                          output_ptr, input_ptr, dim_size);
                      break;
                  }
                } else if (can_use_smem) {
                  size_t smem_sz =
                      dim_size * sizeof(scalar_t) + smem_reduction_sz;
                  cunn_SoftMaxForwardSmem<
                      ILP,
                      scalar_t,
                      accscalar_t,
                      scalar_t,
                      Epilogue><<<grid, block, smem_sz, stream>>>(
                      output_ptr, input_ptr, dim_size);
                } else {
                  cunn_SoftMaxForward<
                      ILP,
                      ...
                      accscalar_t,
                      scalar_t,
                      Epilogue><<<grid, block, smem_reduction_sz, stream>>>(
                      output_ptr, input_ptr, dim_size);
                }
                ...
                [&] {
                  if (!half_to_float) {
                    ...
                    cunn_SpatialSoftMaxForward<
                        ...
                        index_t,
                        Epilogue><<<grid, block, smem_size, stream>>>(
                        output.mutable_data_ptr<accscalar_t>(),
                        input.const_data_ptr<scalar_t>(),
                        outer_size,
                        dim_size,
                        inner_size);
            ...
            if (dim_size <= 1024 && dim_size * sizeof(scalar_t) <= 4096) {
              ...
              can_use_smem &=
                  !(reinterpret_cast<const uintptr_t>(
                    ...
                    ALIGN_BYTES);
              can_use_smem &= (!(
                  reinterpret_cast<uintptr_t>(gI.const_data_ptr<scalar_t>()) %
                  ALIGN_BYTES));
              can_use_smem &=
              ...
              if (can_use_smem) {
                size_t smem_sz =
                    dim_size * sizeof(scalar_t) + smem_reduction_sz;
                cunn_SoftMaxBackwardSmem<
                    ILP,
                    scalar_t,
                    accscalar_t,
                    scalar_t,
                    Epilogue><<<grid, block, smem_sz, stream>>>(
                    gI.mutable_data_ptr<scalar_t>(),
                    output.const_data_ptr<scalar_t>(),
                    grad.const_data_ptr<scalar_t>(),
                    dim_size);
              } else {
            ...
            if (dim_size <= 1024 && dim_size * sizeof(scalar_t) <= 4096) {
              ...
              if (can_use_smem) {
                ...
                cunn_SoftMaxBackward<
                    ...
                    <<<grid, block, block.x * sizeof(accscalar_t), stream>>>(
                        gI.mutable_data_ptr<scalar_t>(),
                        output.const_data_ptr<accscalar_t>(),
                        grad.const_data_ptr<accscalar_t>(),
                        dim_size);
              }
              C10_CUDA_KERNEL_LAUNCH_CHECK();
            }
