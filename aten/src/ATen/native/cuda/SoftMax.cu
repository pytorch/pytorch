...
Tensor host_softmax(const Tensor & input_, const int64_t dim_, const bool half_to_float){
  ...
  Tensor output = half_to_float ? at::empty_like(input, input.options().dtype(ScalarType::Float), at::MemoryFormat::Contiguous) : at::empty_like(input, at::MemoryFormat::Contiguous);
...
Tensor host_softmax_backward(const Tensor &grad_, const Tensor &output_, int64_t dim_, bool half_to_float){
  ...
  Tensor gI = half_to_float ? at::empty_like(grad_, grad_.options().dtype(ScalarType::Half), at::MemoryFormat::Contiguous) : at::empty_like(grad_, at::MemoryFormat::Contiguous);
  if (grad_.numel() == 0) {
    return gI;
  }
