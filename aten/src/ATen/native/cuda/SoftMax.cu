...
Tensor masked_softmax_cuda(const Tensor& input_, const Tensor& mask_, const c10::optional<int64_t> dim_, const c10::optional<int64_t> mask_type_) {
  ...
  TORCH_CHECK(mask_type_, "expected `mask_type` when `mask` supplied");

  // If input is [B, H, T, T] and mask is [B, T]
  // we have special fast kernel
  bool is_BxT_mask = (mask_type_ == 2) && (input_.dim() == 4 && mask_.dim() == 2 && input_.size(0) == mask_.size(0) && input_.size(2) == mask_.size(1) && input_.size(3) == mask_.size(1));
  ...
  bool is_TxT_mask = (mask_type_ == 1) && input_.dim() == 4 && mask_.dim() == 2 && input_.size(3) == mask_.size(1) && input_.size(2) == mask_.size(0) && mask_.size(0) == mask_.size(1);
