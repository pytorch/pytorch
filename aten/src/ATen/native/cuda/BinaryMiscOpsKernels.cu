...
void smooth_l1_kernel_cuda(TensorIteratorBase& iter, double beta) {
  AT_DISPATCH_FLOATING_TYPES_AND2(
      ...
      [&iter, beta]() {
        ...
        gpu_kernel(
            iter, [beta_val] GPU_LAMBDA(scalar_t a, scalar_t b) -> scalar_t {
              auto z = ::abs(a - b);
...
void huber_kernel_cuda(TensorIterator& iter, double delta) {
  AT_DISPATCH_FLOATING_TYPES_AND2(
      kBFloat16, kHalf, iter.dtype(), "huber_cuda", [&iter, delta] {
        scalar_t delta_val(delta);
        gpu_kernel(
            iter, [delta_val] GPU_LAMBDA(scalar_t a, scalar_t b) -> scalar_t {
              auto z = ::abs(a - b);
              return z < delta_val
                  ? scalar_t(0.5) * z * z
  ...
  AT_DISPATCH_FLOATING_TYPES_AND2(
      ...
      [&]() {
        gpu_kernel_with_scalars(
            iter, [] GPU_LAMBDA(scalar_t x, scalar_t y) -> scalar_t {
              if (at::_isnan(y)) {
                return NAN;
              }
              if (x == 0) {
...
void betainc_kernel_cuda(TensorIteratorBase& iter) {
  AT_DISPATCH_FLOATING_TYPES_AND2(
      ...
      [&]() {
        gpu_kernel_with_scalars(
            iter,
            [] GPU_LAMBDA(scalar_t x, scalar_t a, scalar_t b) -> scalar_t {
