...
__global__ void embedding_backward_feature_kernel(
  ...
  int* indices_batch =
      (int*)(buf + sizeof(accscalar_t) * C10_WARP_SIZE * blockDim.y);

  const int s = (int)
      stride; // OK to make int, we don't expect 2 billion+ embedding row size
  ...
  for (int batch_start = 0; batch_start < n;
    ...
    // Loop over the batch of <= 1024 loaded indices in chunks of blockDim.y =
    // 32
    for (int chunk_start = batch_start; chunk_start < batch_end;
         chunk_start += blockDim.y) {
      // This does double duty:  it makes sure indices_batch is ready, and it
      // makes sure match-group leaders are done with their accumulates before
      // other warps start loading again.
      __syncthreads();
      ...
      int dst_row = indices_batch[src_row - batch_start]; // This warp's target
                                                          // row in grad_weight

      // All warps load their smem segments with incoming grad data
      if (src_row < n && f < s && dst_row != padding_idx)
        my_s[threadIdx.x] =
            static_cast<accscalar_t>(grad[src_row * stride + f]);

      __syncthreads();

      // To ensure determinism, we can't just have each warp add its grad data
      // to its dst_row. We need to check if any other warps pulled grad data
      // targeting dst_row. If so, we elect the first warp in each matching
      // group as the leader. Each leader warp serializes the accumulates
      // targeting dst_row in shared memory, then finishes by adding the
      // accumulated buffer to dst_row in grad_weight.
...
#endif
            my_s[threadIdx.x] +=
                smem[threadIdx.x + C10_WARP_SIZE * first_remaining_peer];
            matchmask ^= (1 << first_remaining_peer);
          }
          if (f < s)
...
#pragma unroll
      for (int ii = 0; ii < SZ; ii++) {
        ...
        if (feature_dim < stride) {
          ...
          weight[ii] =
              static_cast<accscalar_t>(grad_weight[weight_row + feature_dim]);
        }
      }
...
/* Calculate norms of the rows of weight_ptr given by idx_ptr and capture them
 * in norms */
template <typename scalar_t, typename accscalar_t, typename index_t>
__global__ void renorm_kernel(
  ...
  // Some casting hacks since dynamic shared memory and templates don't work
  // together:
...
Tensor embedding_dense_backward_cuda(
  ...
  if (num_indices <= 3072 && !scale_grad_by_freq) {
    ...
    auto grad_weight =
        at::zeros({num_weights, grad_.size(-1)}, grad_.options());
    ...
    AT_DISPATCH_FLOATING_TYPES_AND2(
        ...
        [&] {
          ...
          AT_DISPATCH_INDEX_TYPES(
              indices.scalar_type(), "embedding_dense_backward_cuda", [&]() {
                embedding_backward_feature_kernel<
                    ...
                    <<<grid,
                       block,
                       sizeof(accscalar_t) * C10_WARP_SIZE * BLOCKDIMY +
                           sizeof(int) * C10_WARP_SIZE * BLOCKDIMY,
                       stream>>>(
  ...
  return embedding_backward_cuda_kernel(
      grad, orig_indices, sorted_indices, count, num_weights, padding_idx);
}
...
Tensor& embedding_renorm_cuda_(
  ...
  AT_DISPATCH_INDEX_TYPES(
      indices.scalar_type(), "embedding_renorm_cuda_", [&]() {
        using device_ptr = thrust::device_ptr<index_t>;
        ...
        auto end = thrust::unique_copy(
            policy, indices_data, indices_data + num_indices, unique_data);
