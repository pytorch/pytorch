...
void logical_not_kernel_cuda(TensorIteratorBase& iter) {
  ...
  AT_DISPATCH_ALL_TYPES_AND_COMPLEX_AND3(
      kBool, kHalf, kBFloat16, iter.dtype(0), "logical_not_cuda", [&]() {});

  AT_DISPATCH_ALL_TYPES_AND_COMPLEX_AND3(
      kBool, kHalf, kBFloat16, iter.dtype(1), "logical_not_cuda", [&]() {
...
template <typename T>
__host__ __device__ static inline c10::complex<T> sgn_wrapper(
    c10::complex<T> z) {
...
void sgn_kernel_cuda(TensorIteratorBase& iter) {
  AT_DISPATCH_COMPLEX_TYPES(iter.dtype(), "sgn_cuda", [&]() {
    gpu_kernel(
        iter, [] GPU_LAMBDA(scalar_t a) -> scalar_t { return sgn_wrapper(a); });
  });
}
