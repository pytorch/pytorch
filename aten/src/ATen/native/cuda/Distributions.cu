 ...
 * ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
 * For CUDA < 10, curandStatePhilox4_32_10_t engine achieves poor performance
 * (60% SOL bandwidth) when called to generate one random number at a time. This
 * is because the line unsigned ret = (&state->output.x)[state->STATE++]; in
 *            QUALIFIERS unsigned int curand(curandStatePhilox4_32_10_t *state)
 * in curand_kernel.h dynamically indexes into state.output, preventing the
 * compiler from ever storing state.output in registers.
 *
 * CUDA 10 fixed this problem. However, for backwards compatibility, in the
 * following kernels we are using curand distributions that utilize curand4
 * call. curand4 call doesn't have the register spilling problem.
 */
...
struct curand_uniform_wrapper {
  curandStatePhilox4_32_10_t& state;
  __device__ curand_uniform_wrapper(curandStatePhilox4_32_10_t& state)
      : state(state) {}
...
template <typename scalar_t>
void dirichlet_scalar_cuda_kernel(at::Tensor& ret, const at::Tensor& gamma) {
  ...
  AT_DISPATCH_FLOATING_TYPES_AND2(
      ...
      [&] {
        ...
        gpu_kernel(iter, [] GPU_LAMBDA(scalar_t self_val, scalar_t output_val) {
          return standard_gamma_grad_one<scalar_t, accscalar_t>(
              self_val, output_val);
        });
      });
  return ret;
