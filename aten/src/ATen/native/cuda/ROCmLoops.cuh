...
// std::complex and thrust::complex don't work with some !needs_dynamic_casting
// optimizations. They always currently map to !needs_dynamic_casting even
// though we sometimes rely on the ability to reinterpret_cast between these
// representations. In order to separate these concerns, we have a check for
// non-c10 complex separately.
...
C10_HOST_DEVICE typename traits::result_type invoke_impl(
  ...
  return f(*(typename traits::template arg<
...
__device__ inline constexpr decltype(auto) invoke_with_array_impl(
    func_t f,
    array_t t,
    std::index_sequence<I...>) {
  return f(t[I]...);
...
__device__ inline constexpr decltype(auto) invoke_with_array(
    func_t f,
    array_t a) {
...
template <typename func_t>
using type =
    typename arg_type_helper<func_t, function_traits<func_t>::arity>::type;
...
struct has_same_arg_types {
  ...
  static constexpr bool value =
      std::is_same<
          ...
          typename traits::template arg<remaining - 1>::type>::value &&
      has_same_arg_types<func_t, remaining - 1>::value;
};
...
__global__ void elementwise_kernel(int N, func_t f, array_t data) {
  // Assumption:
  // 1. all arguments of `f` have the same type, which could be different from
  // the return type of `f`
  // 2. all tensors are contiguous, that is: stride == sizeof(type) for all
  // tensors
  ...
  // We need to create array to hold all the arguments, for nullary `f`, this
  // means array of size 0. Unfortunately the compiler don't allow us to create
  // array of 0 size, so for this case, we create an array of size 1 and just
  // don't use it.
...
static void launch_kernel(int64_t N, const func_t& f, array_t data) {
  ...
  auto stream = at::cuda::getCurrentCUDAStream();
  elementwise_kernel<func_t, array_t>
  ...
  if (iter.is_trivial_1d()) {
    ...
    if (needs_dynamic_casting<func_t>::check(iter) || non_c10_complex) {
      legacy::launch_kernel<launch_size_1d, 1>(numel, [=] GPU_LAMBDA(int idx) {
        ...
        arg0_t result = legacy::invoke(
            f, &data.data[1], &strides.data[1], &dtypes.data[1], idx);
        c10::cast_and_store<arg0_t>(dtypes[0], out, result);
      });
    } else if (
        iter.has_contiguous_first_dim() &&
        modern::detail::has_same_arg_types<func_t>::value) {
