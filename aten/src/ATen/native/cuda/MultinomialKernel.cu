...
void renormRows(Tensor& t) {
  ...
  AT_DISPATCH_FLOATING_TYPES_AND_HALF(t.scalar_type(), "renormRows_cuda", [&] {
    renormRowsL1<scalar_t>
        <<<grid,
           block,
           block.x * sizeof(scalar_t),
           at::cuda::getCurrentCUDAStream()>>>(
            t.data_ptr<scalar_t>(), rows, cols);
...
__global__ void sampleMultinomialWithReplacement(
  ...
  for (int64_t curDist = blockIdx.y; curDist < distributions;
       curDist += gridDim.y) {
  ...
  for (int64_t curDist = blockIdx.x; curDist < distributions;
    ...
    if (threadIdx.x == 0) {
      if (found) {
        ...
        // errors, our cumulative sum does not add up to 1, but and our uniform
        // sample is greater than this value. In this case we likely have
        // unitialized memory in dest[curDist]. So basically we will loop
        // through the distribution and pick the largest index where the
        // distribution is non-zero. This is obviously terribly inefficient, but
        // due to the rarity in which this occurs, this should not be an issue.
...
void multinomial_with_replacement_kernel_impl(
  ...
  AT_DISPATCH_FLOATING_TYPES_AND_HALF(
      self_v.scalar_type(), "multinomial_kernel_cuda", [&] {
        ...
        int requiredThreads =
            std::min(maxThreads, requiredWarps * C10_WARP_SIZE);
        ...
        if (n_sample == 1 && maxShared >= requiredShared) {
          ...
          Tensor sampled = native::empty_cuda(
              {numDist, n_sample},
              optTypeMetaToScalarType(self_v.options().dtype_opt()),
              self_v.options().layout_opt(),
              self_v.options().device_opt(),
          ...
          int grid_y = std::min<int>(
              numDist, at::cuda::getCurrentDeviceProperties()->maxGridSize[1]);
          dim3 grid((n_sample - 1) / block.x + 1, grid_y);
          {
            // See Note [Acquire lock when using random generators]
            std::lock_guard<std::mutex> lock(gen->mutex_);

            // each thread generates a single sample for (numdist/numblocks.y)
            // distributions, however, since we have to use curand_uniform4 (See
            // Note [Register spilling in curand call for CUDA < 10]), offset is
            // 4 times that.
