...
static at::cuda::detail::TensorInfo<scalar, IndexType> getCastTensorInfo(const at::TensorBase& t) {
  // Use data_ptr() to get void*, bypassing the ScalarType check
  scalar* data = reinterpret_cast<scalar *>(t.data_ptr());

  IndexType sz[MAX_TENSORINFO_DIMS];
  IndexType st[MAX_TENSORINFO_DIMS];
  for (int i = 0; i < t.dim(); ++i) {
    sz[i] = static_cast<IndexType>(t.size(i));
    st[i] = static_cast<IndexType>(t.stride(i));
  }

  return at::cuda::detail::TensorInfo<scalar, IndexType>(data, t.dim(), sz, st);
}
  ...
  AT_DISPATCH_ALL_TYPES_AND3(at::ScalarType::Half, at::ScalarType::BFloat16, at::ScalarType::Bool, key.scalar_type(), "sortKeyValueInplace", [&]  {
    ...
    if (at::cuda::detail::canUse32BitIndexMath(key)) {
      ...
      at::cuda::detail::TensorInfo<idx_scalar_t, unsigned int> valueInfo =
        getCastTensorInfo<idx_scalar_t, unsigned int>(value);
      ...
      at::cuda::detail::TensorInfo<idx_scalar_t, uint64_t> valueInfo =
        getCastTensorInfo<idx_scalar_t, uint64_t>(value);
...
#undef HANDLE_SORT_CASE
    ...
    #define INSTANTIATE_WARP_SORT(TYPE) \
      template void sortCommon<TYPE>(WarpMergeSort<128, 32>, const TensorBase&, const TensorBase&, int, bool); \
      template void sortCommon<TYPE>(WarpMergeSort<128, 64>, const TensorBase&, const TensorBase&, int, bool);
  #else
    #define INSTANTIATE_WARP_SORT(TYPE) \
      template void sortCommon<TYPE>(WarpMergeSort<128, C10_WARP_SIZE>, const TensorBase&, const TensorBase&, int, bool);
  #endif
#else
  #define INSTANTIATE_WARP_SORT(TYPE)
#endif
