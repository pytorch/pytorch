...
#if defined(BUILD_LAZY_CUDA_LINALG)
namespace {
cuda::detail::LinalgDispatch disp = {
    _symeig_helper_cuda,
...
std::tuple<Tensor, Tensor> lazy_eig_kernel(
    const Tensor& self,
    bool& eigenvectors) {
  loadLazyTorchLinalgLibrary();
...
Tensor& _linalg_inv_out_helper_cuda(
  ...
  getTorchLinalgLibrary();
  TORCH_CHECK(
      disp.inv_out_helper != _linalg_inv_out_helper_cuda,
      "Can't find _linalg_inv_out_helper_cuda");
...
std::tuple<Tensor, Tensor> legacy_lstsq_cuda(const Tensor& B, const Tensor& A) {
  getTorchLinalgLibrary();
  TORCH_CHECK(
      disp.legacy_lstsq != legacy_lstsq_cuda, "Can't find legacy_lstsq_cuda");
...
std::tuple<Tensor, Tensor> _symeig_helper_cuda(
  ...
  getTorchLinalgLibrary();
  TORCH_CHECK(
      disp.symeig_helper != _symeig_helper_cuda,
      "Can't find _symeig_helper_cuda");
...
std::tuple<Tensor&, Tensor&> legacy_lstsq_out_cuda(
  ...
  TORCH_CHECK(
      B.scalar_type() == dtype,
      "exepected A and B dtypes to match but found ",
      A.scalar_type(),
      " and ",
      B.scalar_type());
  TORCH_CHECK(
      A_out.scalar_type() == dtype,
      "A_out to have scalar type ",
      dtype,
      " but found",
      A_out.scalar_type());
  TORCH_CHECK(
      B_out.scalar_type() == dtype,
      "A_out to have scalar type ",
      dtype,
      " but found",
      B_out.scalar_type());
