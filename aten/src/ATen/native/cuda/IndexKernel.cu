...
void index_kernel_impl(
  ...
  gpu_index_kernel(
      ...
      index_size,
      index_stride,
      [] C10_DEVICE(char* out_data, char* in_data, int64_t offset) {
...
static Tensor& masked_select_out_cuda_impl(
    Tensor& result,
    const Tensor& self,
    const Tensor& mask) {
  NoNamesGuard guard;
  ...
  at::native::index_out(
      ...
      *std::get<1>(mask_self_expanded),
      c10::List<c10::optional<at::Tensor>>(
          {*std::get<0>(std::move(mask_self_expanded))}));
...
void take_kernel(TensorIterator& iter, const Tensor& input) {
  AT_DISPATCH_ALL_TYPES_AND_COMPLEX_AND3(
      at::ScalarType::Half,
      at::ScalarType::Bool,
      at::ScalarType::BFloat16,
...
void masked_scatter_cuda_impl(
  ...
  TORCH_CHECK(
      totalElements <= srcSize,
      "source nElements must be == mask `1` elements");
  ...
  AT_DISPATCH_ALL_TYPES_AND_COMPLEX_AND3(
      ...
      [&]() {
        ...
        gpu_kernel(
            iter,
            [=] GPU_LAMBDA(
                scalar_t a, mask_t mask, int64_t maskPrefixSum) -> scalar_t {
              if (mask) {
