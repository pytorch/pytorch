...
void gpu_index_kernel(TensorIterator& iter, IntListRef index_size, IntListRef index_stride, const func_t& f) {
...
void index_kernel_impl(TensorIterator& iter, IntListRef index_size, IntListRef index_stride) {
...
template <typename scalar_t>
void index_put_kernel_impl(TensorIterator& iter, IntListRef index_size, IntListRef index_stride) {
  gpu_index_kernel(iter, index_size, index_stride, []C10_DEVICE(char* out_data, char* in_data, int64_t offset) {
    *(scalar_t*)(out_data + offset) = *(scalar_t*)in_data;
  });
}

static void index_kernel(TensorIterator& iter, IntListRef index_size, IntListRef index_stride) {
...
static void index_put_kernel(TensorIterator& iter, IntListRef index_size, IntListRef index_stride, bool accumulate) {
