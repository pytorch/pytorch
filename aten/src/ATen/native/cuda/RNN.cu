...
void getLaunchConfig(dim3* block, dim3* grid, int64_t numel) {
  ...
  *block = cuda::getApplyBlock();
  TORCH_INTERNAL_ASSERT(
      cuda::getApplyGrid(numel, *grid, curDevice),
...
__global__ void lstm_cell_backward(
  ...
  for (index_type linearIndex = blockIdx.x * blockDim.x + threadIdx.x;
    ...
    accscalar_t go =
        has_gradoutput ? H2F(DEVICE_LINEAR_GET(gradoutput, linearIndex)) : 0.f;
...
std::tuple<Tensor, Tensor, Tensor> _thnn_fused_lstm_cell_cuda(
  ...
  AT_DISPATCH_FLOATING_TYPES_AND_HALF(
      input_gates.scalar_type(), "_thnn_fused_lstm_cell_cuda", [&] {
...
_thnn_fused_lstm_cell_backward_cuda(
  ...
  AT_DISPATCH_FLOATING_TYPES_AND_HALF(
      workspace.scalar_type(), "_thnn_fused_lstm_cell_cuda_backward", [&] {
        if (canUse32BitIndexMath(
                workspace)) { // See Note [64-bit index math check elision]
...
std::tuple<Tensor, Tensor> _thnn_fused_gru_cell_cuda(
  ...
  AT_DISPATCH_FLOATING_TYPES_AND_HALF(
      input_gates.scalar_type(), "_thnn_fused_gru_cell_cuda", [&] {
        if (canUse32BitIndexMath(
                workspace)) { // See Note [64-bit index math check elision]
...
std::tuple<Tensor, Tensor, Tensor, Tensor, Tensor>
_thnn_fused_gru_cell_backward_cuda(
    const Tensor& grad_hy,
    const Tensor& workspace,
    bool has_bias) {
  ...
  AT_DISPATCH_FLOATING_TYPES_AND_HALF(
      grad_hy.scalar_type(), "_thnn_fused_gru_cell_cuda_backward", [&] {
        if (canUse32BitIndexMath(
                workspace)) { // See Note [64-bit index math check elision]
