...
// 0).
//
// To overcome this pitfall we first carefully choose the number of bits in
// these keys, so that the probability of having duplicate keys is under a
// threshold. Let q be the threshold probability for having non-duplicate keys,
// then it can be proved that[1] the number of bits required is: ceil(log2(n -
// (6 n^2 + 1) / (12 log(q))))
//
// Then after sort, we lauch a separate kernel that additionally shuffles any
// islands of values whose keys matched. The algorithm of this kernel is as
// follows: Each thread reads its key and the keys of its neighbors to tell if
// it's part of an island. For each island, the first thread in the island sees
// a key match at index i+1 but not index i-1. This thread considers itself the
// "island leader". The island leader then reads more indices to the right to
// figure out how big the island is. Most likely, the island will be very small,
// just a few values. The island leader then rolls that many RNG, uses them to
// additionally shuffle values within the island using serial Fisher-Yates, and
// writes them out.
//
// Reference
// [1] https://osf.io/af2hy/

Tensor& randperm_out_cuda(
    int64_t n,
  ...
  // tensor is contiguous; otherwise it points to a new tensor.
  Tensor shuffled;
  void* shuffled_data;
  ...
  if (n == 0) {
    ...
    auto keys = at::empty(result.sizes(), opt.dtype(kShort))
                    .random_(
