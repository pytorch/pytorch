...
// SFINAE doesn't work well with NVCC under Windows for math functions like pow
// and sqrt. So we need to define the functions with the explicit function
// signatures. As for pow, the following signatures are defined as the device
// function:
//   pow(float, int)
//   pow(double, int)
//   pow(float, float)
...
//   sqrt(float)
//   sqrt(double)
// As for inverse sqrt, we must define it explicitly in MSVC, otherwise the
// static cast will be applied to the result of the inline function, and thus
// the result is incorrect.
//   e.g. if we use 1.0 / sqrt(2) for 2 ^ (-0.5) in MSVC, we get
//          int(2 ^ (-0.5)) = int(1.0 / sqrt(2)) = int(1.0 / int(1.414)) =
//          int(1.0 / 1) = 1
...
// pow for at::BFloat16
static inline __host__ __device__ at::BFloat16 pow_(
    at::BFloat16 base,
    at::BFloat16 exp) {
...
static inline __host__ __device__ typename std::enable_if<
