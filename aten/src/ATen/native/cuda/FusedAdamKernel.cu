  ...
  for (const auto & device_map : group_tensors_by_device_and_scalartype(nested_tensorlists, /* has_state_steps */ true)) {
    ...
    for (const auto & scalar_type_nested_tensors : device_map.second) {
      ...
      if (amsgrad) {
        ...
        _fused_adam_amsgrad_cuda_impl_(cur_params, cur_grads, cur_exp_avgs, cur_exp_avg_sqs, cur_max_exp_avg_sqs, cur_state_steps, lr, beta1, beta2, weight_decay, eps, maximize, cur_scale, cur_found_inf);
      } else {
        _fused_adam_cuda_impl_(cur_params, cur_grads, cur_exp_avgs, cur_exp_avg_sqs, cur_state_steps, lr, beta1, beta2, weight_decay, eps, maximize, cur_scale, cur_found_inf);
      }
