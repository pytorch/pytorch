    ...
    IntList paddingSize)
{
  ...
  AT_DISPATCH_FLOATING_TYPES_AND_HALF(
      ...
      if (numInputDims == 3) {
        ...
        auto input_ = input.reshape({1, input.size(0), input.size(1), input.size(2)});
        auto output_ = output.reshape({1, output.size(0), output.size(1), output.size(2)});
    ...
    IntList paddingSize)
{
  ...
  AT_DISPATCH_FLOATING_TYPES_AND_HALF(
      input.type(), "replication_pad2d_backward", [&] {
        ...
        if (numInputDims == 3) {
          gradInput_ = gradInput.reshape({1, gradInput.size(0),
              gradInput.size(1), gradInput.size(2)});
          gradOutput_ = gradOutput.reshape({1, gradOutput.size(0),
...
static inline void shapeAndGradOutputCheck(
    const Tensor& input,
    const Tensor& gradOutput,
    int pleft, int pright,
    int ptop, int pbottom,
    ...
    IntList paddingSize)
{
  ...
  shapeCheck(input, pleft, pright, ptop,
  ...
  AT_DISPATCH_FLOATING_TYPES_AND_HALF(
      ...
      auto input_ = input.reshape({1, input.size(0), input.size(1), input.size(2), input.size(3)});
      auto output_ = output.reshape({1, output.size(0), output.size(1), output.size(2), output.size(3)});
    ...
    IntList paddingSize)
{
  ...
  int pfront = paddingSize[4];
  int pback = paddingSize[5]; 
  shapeAndGradOutputCheck(input, gradOutput, pleft, pright, ptop,
      pbottom, pfront, pback);
  ...
  AT_DISPATCH_FLOATING_TYPES_AND_HALF(
      ...
      if (numInputDims == 4) {
      gradInput_ = gradInput.reshape({1, gradInput.size(0),
          gradInput.size(1), gradInput.size(2), gradInput.size(3)});
      gradOutput_ = gradOutput.reshape({1, gradOutput.size(0),
