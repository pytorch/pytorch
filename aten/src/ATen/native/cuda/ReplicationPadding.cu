...
__global__ void replication_pad_forward_kernel1d(
  ...
  int inputPointX = imin(imax(padL, outputPointX), input.size(2) + padL - 1) -
      oStartX + iStartX;
...
__global__ void replication_pad_backward_kernel(
  ...
  int inputPointX =
      imin(imax(padL, outputPointX), gradInput.size(2) + padL - 1) - oStartX +
      iStartX;
...
__global__ void replication_pad_forward_kernel2d(
  ...
  int inputPointX = imin(imax(padL, outputPointX), input.size(3) + padL - 1) -
      oStartX + iStartX;
  int inputPointY = imin(imax(padT, outputPointY), input.size(2) + padT - 1) -
      oStartY + iStartY;
...
__global__ void replication_pad_backward_kernel(
  ...
  int inputPointX =
      imin(imax(padL, outputPointX), gradInput.size(3) + padL - 1) - oStartX +
      iStartX;
  int inputPointY =
      imin(imax(padT, outputPointY), gradInput.size(2) + padT - 1) - oStartY +
      iStartY;
...
__global__ void replication_pad_forward_kernel3d(
  ...
  int plane = blockIdx.y + y_shift;
  int batch = blockIdx.z + z_shift;
  if (outputPointId >= (output.size(2) * output.size(3) * output.size(4))) {
    return;
  }
  ...
  int inputPointX = imin(imax(pleft, outputPointX), input.size(4) + pleft - 1) -
      oStartX + iStartX;
  int inputPointY = imin(imax(ptop, outputPointY), input.size(3) + ptop - 1) -
      oStartY + iStartY;
  int inputPointZ =
      imin(imax(pfront, outputPointZ), input.size(2) + pfront - 1) - oStartZ +
      iStartZ;
...
__global__ void replication_pad_backward_kernel(
  ...
  int outputPointY = (outputPointId / gradOutput.size(4)) % gradOutput.size(3);
  int outputPointZ = outputPointId / (gradOutput.size(3) * gradOutput.size(4));
  ...
  int inputPointX =
      imin(imax(pleft, outputPointX), gradInput.size(4) + pleft - 1) - oStartX +
      iStartX;
  int inputPointY =
      imin(imax(ptop, outputPointY), gradInput.size(3) + ptop - 1) - oStartY +
      iStartY;
  int inputPointZ =
      imin(imax(pfront, outputPointZ), gradInput.size(2) + pfront - 1) -
      oStartZ + iStartZ;

  scalar_t valueToCopy =
      gradOutput[batch][plane][outputPointZ][outputPointY][outputPointX];
  gpuAtomicAdd(
      &gradInput[batch][plane][inputPointZ][inputPointY][inputPointX],
      valueToCopy);
}
...
void replication_pad2d_backward_out_cuda_template(
  ...
  TORCH_CHECK(
      at::cuda::detail::canUse32BitIndexMath(gradOutput),
  ...
  AT_DISPATCH_FLOATING_AND_COMPLEX_TYPES_AND1(
      kHalf, input.scalar_type(), "replication_pad2d_backward_cuda", [&] {
        ...
        for (int64_t block_y = 0; block_y < size1; block_y += 65535) {
          int64_t block_y_size =
              std::min(size1 - block_y, static_cast<int64_t>(65535));
          for (int64_t block_z = 0; block_z < size0; block_z += 65535) {
            int64_t block_z_size =
                std::min(size0 - block_z, static_cast<int64_t>(65535));

            dim3 gridSize(
                THCCeilDiv(outputPlaneSize, static_cast<int64_t>(256)),
                block_y_size,
                block_z_size);
...
static inline void shapeAndGradOutputCheck3d(
  ...
  TORCH_CHECK(
      numPlanes == gradOutput.size(planeDim),
      "gradOutput width unexpected. Expected: ",
      numPlanes,
      ", Got: ",
  ...
  TORCH_CHECK(
      ...
      gradOutput.size(dimh));
  TORCH_CHECK(
      odepth == gradOutput.size(dimd),
      "gradOutput depth unexpected. Expected: ",
      odepth,
      ", Got: ",
...
void replication_pad3d_backward_out_cuda_template(
  ...
  shapeAndGradOutputCheck3d(
      input, gradOutput, pleft, pright, ptop, pbottom, pfront, pback);
  ...
  AT_DISPATCH_FLOATING_AND_COMPLEX_TYPES_AND1(
      kHalf, input.scalar_type(), "replication_pad3d_backward_cuda", [&] {
        auto gradInput_ = gradInput;
        auto gradOutput_ = gradOutput;
        if (numInputDims == 4) {
          gradInput_ = gradInput.unsqueeze(0);
        ...
        int64_t outputPlaneSize = devGradOutput.size(2) *
            devGradOutput.size(3) * devGradOutput.size(4);
        int64_t size1 = devGradOutput.size(1);
        int64_t size0 = devGradOutput.size(0);
...
(const Tensor& input, IntArrayRef paddingSize, const Tensor& output) {
  ...
  AT_DISPATCH_FLOATING_AND_COMPLEX_TYPES_AND1(
      kHalf, input.scalar_type(), "replication_pad1d_cuda", [&] {
        ...
        for (int64_t block_y = 0; block_y < size1; block_y += 65535) {
          ...
          for (int64_t block_z = 0; block_z < size0; block_z += 65535) {
            ...
            replication_pad_forward_kernel1d<<<
                gridSize,
                blockSize,
                0,
                at::cuda::getCurrentCUDAStream()>>>(
                devInput, devOutput, padL, padR, block_y, block_z);
            C10_CUDA_KERNEL_LAUNCH_CHECK();
          }
        }
      });
...
(const Tensor& gradOutput,
 ...
 const Tensor& gradInput) {
  ...
  AT_DISPATCH_FLOATING_AND_COMPLEX_TYPES_AND1(
      kHalf, input.scalar_type(), "replication_pad1d_backward_cuda", [&] {
        auto gradInput_ = gradInput;
        auto gradOutput_ = gradOutput;
        if (numInputDims == 2) {
          gradInput_ = gradInput.unsqueeze(0);
...
(const Tensor& input, IntArrayRef paddingSize, const Tensor& output) {
  ...
  AT_DISPATCH_FLOATING_AND_COMPLEX_TYPES_AND1(
      kHalf, input.scalar_type(), "replication_pad2d_cuda", [&] {
        at::Tensor input_ = input;
        at::Tensor output_ = output;
        if (input.dim() == 3) {
          input_ = input.unsqueeze(0);
        ...
        for (int64_t block_y = 0; block_y < size1; block_y += 65535) {
          ...
          for (int64_t block_z = 0; block_z < size0; block_z += 65535) {
            ...
            dim3 gridSize(
                ...
                block_z_size);
            dim3 blockSize(outputPlaneSize > 256 ? 256 : outputPlaneSize);
            replication_pad_forward_kernel2d<<<
                gridSize,
                blockSize,
                0,
                at::cuda::getCurrentCUDAStream()>>>(
...
(const Tensor& input, IntArrayRef paddingSize, const Tensor& output) {
  ...
  AT_DISPATCH_FLOATING_AND_COMPLEX_TYPES_AND1(
      kHalf, input.scalar_type(), "replication_pad3d_cuda", [&] {
        at::Tensor input_ = input;
        at::Tensor output_ = output;
        if (numInputDims == 4) {
        ...
        int64_t outputPlaneSize =
            devOutput.size(2) * devOutput.size(3) * devOutput.size(4);
        int64_t size1 = devOutput.size(1);
        int64_t size0 = devOutput.size(0);
