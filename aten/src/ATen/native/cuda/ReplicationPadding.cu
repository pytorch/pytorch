...
__global__ void replication_pad_forward_kernel(
    PackedTensorAccessor<Dtype, 3> input,
    PackedTensorAccessor<Dtype, 3> output,
    int padL, int padR) {
...
__global__ void replication_pad_backward_kernel(
    PackedTensorAccessor<Dtype, 3> gradInput,
    PackedTensorAccessor<Dtype, 3> gradOutput,
    int padL, int padR) {
    ...
    IntList paddingSize)
{
  ...
  AT_DISPATCH_FLOATING_TYPES_AND_HALF(
      ...
      replication_pad_forward_kernel <<<gridSize, blockSize, 0, at::cuda::getCurrentCUDAStream()>>>(
    ...
    IntList paddingSize)
{
  ...
  AT_DISPATCH_FLOATING_TYPES_AND_HALF(
      ...
      if (numInputDims == 2) {
      gradInput_ = gradInput.reshape({1, gradInput.size(0), gradInput.size(1)});
      gradOutput_ = gradOutput.reshape({1, gradOutput.size(0), gradOutput.size(1)});
      }
