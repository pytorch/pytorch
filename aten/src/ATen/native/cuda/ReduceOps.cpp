...
void norm_kernel_cuda(TensorIterator& iter, const Scalar& val) {
  ...
  if (val.isIntegral(false)) {
    ...
    TORCH_CHECK(
        false, "norm_kernel_cuda_impl expects norm to be integer or float");
  }
  if (iter.numel() == 0) {
...
void min_all_kernel_impl(Tensor& result, const Tensor& input) {
  ...
  auto iter =
      make_reduction("min_all", result, input, IntArrayRef{}, false, dtype);
...
void max_all_kernel_impl(Tensor& result, const Tensor& input) {
  auto dtype = input.scalar_type();
  auto iter =
      make_reduction("max_all", result, input, IntArrayRef{}, false, dtype);
  max_all_launch_kernel(iter);
}

void aminmax_allreduce_kernel_impl(
    const Tensor& input,
    Tensor& min_result,
    Tensor& max_result) {
  auto dtype = input.scalar_type();
