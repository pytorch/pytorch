...
inline c10::MemoryFormat compute_output_memory_format(
    const TensorList& inputs) {
...
Tensor& cat_out_cuda(TensorList inputs, int64_t dimension, Tensor& out) {
  ...
  bool allSameType =
      std::all_of(inputs.begin(), inputs.end(), [firstType](const Tensor& t) {
        return t.scalar_type() == firstType;
      });
  allSameType = allSameType && (out.scalar_type() == firstType);

#ifdef __HIP_PLATFORM_HCC__
  if (inputs.size() > 1 && out.dim() <= CAT_ARRAY_MAX_INPUT_DIMS &&
      at::cuda::detail::canUse32BitIndexMath(out) && allContiguous &&
    ...
    AT_DISPATCH_ALL_TYPES_AND_COMPLEX_AND3(
        ...
        "cat_cuda",
        [&]() {
          hip_parallel_cat<scalar_t>(
              out, inputs, dimension, nDims, memory_format);
        });
#else
  ...
  // to cuda kernel through constant memory. Therefore, we could pass more
  // inputs to cuda threads. For non-contiguous, we reduce the number of inputs
  // passed to cuda kernel due to the limitation of constant memory.
  if (inputs.size() > 1 && out.dim() <= CAT_ARRAY_MAX_INPUT_DIMS &&
      at::cuda::detail::canUse32BitIndexMath(out) && allContiguous &&
      all32BitIndexable && allSameType) {
    AT_DISPATCH_ALL_TYPES_AND_COMPLEX_AND3(
        ...
        "cat_cuda",
        [&]() {
          parallel_cat<scalar_t, CAT_ARRAY_BATCH_SIZE, 1>(
              out, inputs, dimension, nDims, memory_format);
        });
  } else if (
      inputs.size() > 1 && out.dim() <= CAT_ARRAY_MAX_INPUT_DIMS &&
      at::cuda::detail::canUse32BitIndexMath(out) &&
      nDims <= CAT_ARRAY_MAX_INPUT_DIMS && all32BitIndexable && allSameType &&
