...
__device__ int64_t
lower_bound(const input_t* data_ss, int64_t start, int64_t end, input_t val) {
  while (start < end) {
...
template <typename input_t>
__device__ int64_t
upper_bound(const input_t* data_ss, int64_t start, int64_t end, input_t val) {
  while (start < end) {
    int64_t mid = start + ((end - start) >> 1);
    if (!(data_ss[mid] > val)) {
      start = mid + 1;
...
__global__ void searchsorted_cuda_kernel(
  ...
  for (int64_t tid = blockIdx.x * blockDim.x + threadIdx.x; tid < numel_in;
...
template <typename input_t, typename output_t>
void searchsorted_cuda_contiguous(
    Tensor& result,
    const Tensor& input,
  ...
  dim3 grid =
      dim3(std::min(maxGrid, cuda::ATenCeilDiv<int64_t>(numel_in, block.x)));
...
void dispatch(
  ...
  if (!out_int32) {
    AT_DISPATCH_ALL_TYPES(input.scalar_type(), "searchsorted_out_cuda", [&] {
      searchsorted_cuda_contiguous<scalar_t, int64_t>(
          result, input, boundaries, right);
    });
  } else {
    AT_DISPATCH_ALL_TYPES(input.scalar_type(), "searchsorted_out_cuda", [&] {
      searchsorted_cuda_contiguous<scalar_t, int>(
          result, input, boundaries, right);
    });
  }
}
