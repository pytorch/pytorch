...
// Implement a numpy like searchsorted and a bucketize function running on cuda
// See details in ATen/nativate/Bucketization.cpp

namespace {

template<typename input_t>
__device__ int64_t lower_bound(const input_t *data_ss, int64_t start, int64_t end, input_t val, const int64_t *data_sort) {
  int64_t orig_start = start;
  while (start < end) {
    int64_t mid = start + ((end - start) >> 1);
    input_t mid_val = data_sort ? data_ss[orig_start + data_sort[mid]] : data_ss[mid];
    if (!(mid_val >= val)) {
      start = mid + 1;
    }
    else {
...
template<typename input_t>
__device__ int64_t upper_bound(const input_t *data_ss, int64_t start, int64_t end, input_t val, const int64_t *data_sort) {
  int64_t orig_start = start;
  while (start < end) {
    int64_t mid = start + ((end - start) >> 1);
    input_t mid_val = data_sort ? data_ss[orig_start + data_sort[mid]] : data_ss[mid];
...
Tensor searchsorted_cuda(
  ...
  at::native::searchsorted_out_cuda(sorted_sequence, self, out_int32, right, side, sorter, result);
  return result;
}
...
Tensor searchsorted_cuda(
  ...
  return searchsorted_cuda(sorted_sequence, scalar_tensor, out_int32, right, side, sorter);
}
