...
// We manually overload real because std::real does not work types other than
// c10::complex.
template <typename scalar_t>
...
template <typename T>
__host__ __device__ static inline c10::complex<T> real_wrapper(
    c10::complex<T> v) {
  return v.real();
}
...
// We manually overload imag because std::imag does not work types other than
// c10::complex.
template <typename scalar_t>
...
template <typename T>
__host__ __device__ static inline c10::complex<T> imag_wrapper(
    c10::complex<T> v) {
  return v.imag();
}
...
template <typename T>
__host__ __device__ static inline c10::complex<T> conj_wrapper(
    c10::complex<T> v) {
  return std::conj(v);
}
