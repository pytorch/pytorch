...
// The softmax_warp_* methods perform softmax forward and backward propagation
// on samples spanning the fast dimension. Each sample contains element_count
// scalar elements. element_count can be any integer value <= 1024. The template
// arguments have the following meaning: One "WARP" works on one "BATCH". One
// "BATCH" contains "WARP_BATCH" samples. WARP_BATCH is equal to 1 when
// element_count is large, and > 1 when element_count is small. A "WARP"
// contains "C10_WARPS_SIZE" threads, these treads are guaranteed to belong to
// the same warp. This is important because it means only __shfl_ instructions
// are required for reductions. Note that this means WARP_SIZE must be a power
// of two and <= architecture warp size. CUDA warp size is 32 for all existing
// GPU architectures, but there is no guarantee this will not change for future
// arch. ROCm warp size is 64 for all currently ROCm-supported GPU
// architectures, but this may change for future archs. is_log_softmax is a flag
// indicating whether SoftMax or LogSoftMax should be computed. The template can
// be instantiated with any floating point type for the type arguments input_t,
// output_t and acc_t. This allows SoftMax to be fused with a cast immediately
// following the SoftMax. For instance: input_t=half,  acc_t=float,
// output_t=half  => read half tensor, float accumulators, write half tensor.
// input_t=half,  acc_t=float, output_t=float => read half tensor, float
// accumulators, write float tensor. input_t_float, acc_t=float, output_t=half
// => read float tensor, float accumulators, write half tensor.
