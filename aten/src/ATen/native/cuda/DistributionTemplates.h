...
std::tuple<uint64_t, dim3, dim3> calc_execution_policy(const int64_t total_elements, const uint32_t unroll_factor) {
  ...
  grid.x = std::min(static_cast<uint32_t>(at::cuda::getCurrentDeviceProperties()->multiProcessorCount) * blocks_per_sm,
                    grid.x);
  // number of times random will be generated per thread, to offset philox counter in thc random state
  uint64_t counter_offset =
      ((numel - 1) / (block_size * grid.x * unroll_factor) + 1) * max_generator_offsets_per_curand_call;
...
__global__ void distribution_binary_elementwise_kernel(
  ...
  curandStatePhilox4_32_10_t state;
  curand_init(std::get<0>(seeds), blockIdx.x * blockDim.x + threadIdx.x, std::get<1>(seeds), &state);
...
void distribution_binary_kernel(TensorIteratorBase& iter, PhiloxCudaState philox_args, const func_t& f) {
  static_assert(std::is_same_v<typename function_traits<func_t>::template arg<0>::type, curandStatePhilox4_32_10_t&>,
                "the first argument of functor must be curandStatePhilox4_32_10_t");
  using input_t_1 = typename function_traits<func_t>::template arg<1>::type;
  using input_t_2 = typename function_traits<func_t>::template arg<2>::type;
  ...
  if (iter.is_contiguous()) {
    distribution_binary_elementwise_kernel<<<grid, num_threads(), 0, stream>>>(numel,
                                                                               f,
                                                                               philox_args,
                                                                               output_data,
                                                                               input_data_1,
                                                                               input_data_2,
                                                                               TrivialOffsetCalculator<2>(),
    ...
    distribution_binary_elementwise_kernel<<<grid, num_threads(), 0, stream>>>(numel,
                                                                               ...
                                                                               input_data_1,
                                                                               input_data_2,
                                                                               make_input_offset_calculator<2>(iter),
...
void random_from_to_kernel(TensorIteratorBase& iter, uint64_t range, int64_t base, RNG gen) {
  ...
  AT_DISPATCH_V2(iter.dtype(),
                 "random_from_to_kernel_cuda",
                 AT_WRAP([&] {
                   if ((std::is_same_v<scalar_t, int64_t> || std::is_same_v<scalar_t, double> ||
                        std::is_same_v<scalar_t, float> || std::is_same_v<scalar_t, at::BFloat16>) &&
                       range >= 1ULL << 32) {
                     // define lambda to mod with range and add base
                     auto random_func = [range, base] __device__(uint64_t rand) {
                       return transformation::uniform_int_from_to<scalar_t>(rand, range, base);
                     };
                     distribution_nullary_kernel<scalar_t, uint64_t, ulonglong2>(
                         iter,
                         gen,
                     ...
                     distribution_nullary_kernel<scalar_t, uint32_t, uint4>(
                         iter,
                         gen,
                         [] __device__(curandStatePhilox4_32_10_t * state) -> uint4 { return curand4(state); },
                         random_func);
                   }
                 }),
...
void random_full_64_bits_range_kernel(TensorIteratorBase& iter, RNG gen) {
  AT_DISPATCH_ALL_TYPES_AND(at::ScalarType::BFloat16, iter.dtype(), "random_full_64_bits_range_kernel_cuda", [&] {
    if (std::is_same_v<scalar_t, int64_t> || std::is_same_v<scalar_t, double> || std::is_same_v<scalar_t, float> ||
...
void random_kernel(TensorIteratorBase& iter, RNG gen) {
  AT_DISPATCH_ALL_TYPES_AND3(
      at::ScalarType::Half, at::ScalarType::BFloat16, at::ScalarType::Bool, iter.dtype(), "random_kernel_cuda", [&] {
        if (std::is_same_v<scalar_t, double> || std::is_same_v<scalar_t, int64_t>) {
          auto random_func = [] __device__(uint64_t rand) { return transformation::uniform_int<scalar_t>(rand); };
          distribution_nullary_kernel<scalar_t, uint64_t, ulonglong2>(
              iter,
              gen,
              [] __device__(curandStatePhilox4_32_10_t * state) -> ulonglong2 {
          ...
          auto random_func = [] __device__(uint32_t rand) { return transformation::uniform_int<scalar_t>(rand); };
          distribution_nullary_kernel<scalar_t, uint32_t, uint4>(
              iter,
              gen,
              [] __device__(curandStatePhilox4_32_10_t * state) -> uint4 { return curand4(state); },
...
void normal_kernel(const TensorBase& self, double mean_, double std_, RNG gen) {
  ...
  AT_DISPATCH_FLOATING_TYPES_AND2(
      at::ScalarType::Half, at::ScalarType::BFloat16, iter.dtype(), "normal_kernel_cuda", [&] {
...
void uniform_kernel(TensorIteratorBase& iter, double from_, double to_, RNG gen) {
  AT_DISPATCH_FLOATING_TYPES_AND2(
      at::ScalarType::Half, at::ScalarType::BFloat16, iter.dtype(), "uniform_kernel_cuda", [&] {
  ...
  AT_DISPATCH_FLOATING_TYPES_AND2(at::ScalarType::Half, at::ScalarType::BFloat16, iter.dtype(), "log_normal_cuda", [&] {
    ...
    auto log_normal_func = [mean, std] __device__(accscalar_t rand) {
      return static_cast<scalar_t>(
          transformation::log_normal<accscalar_t>(transformation::normal<accscalar_t>(rand, mean, std)));
    };
...
void exponential_kernel(TensorIteratorBase& iter, double lambda_, RNG gen) {
  TORCH_CHECK(
      ...
      iter.dtype());
  AT_DISPATCH_FLOATING_TYPES_AND2(
      at::ScalarType::Half, at::ScalarType::BFloat16, iter.dtype(), "exponential_cuda", [&] {
        using accscalar_t = at::acc_type<scalar_t, true>;
...
void bernoulli_tensor_cuda_kernel(const TensorBase& ret, const at::TensorBase& p, PhiloxCudaState philox_args) {
  auto functor = [philox_args] __device__(int n,
                                          scalar_t& v1,
                                          scalar_t& v2,
                                          scalar_t& v3,
                                          scalar_t& v4,
  ...
  at::cuda::CUDA_tensor_apply2<scalar_t,
                               const prob_t,
                               4,
                               decltype(functor),
...
void bernoulli_kernel(const TensorBase& self, const TensorBase& p_, RNG gen) {
  ...
  TORCH_CHECK(at::isFloatingType(p_.scalar_type()),
  ...
  AT_DISPATCH_ALL_TYPES_AND3(at::ScalarType::Half,
                             at::ScalarType::BFloat16,
                             at::ScalarType::Bool,
                             self.scalar_type(),
                             "bernoulli_tensor_cuda_self_",
                             [&] {
                               if (std::is_same_v<scalar_t, double>) {
