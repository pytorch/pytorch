 ...
 * kernels? Note that we need a grid-stride loop kernel because, we found by
 * testing that it achieves peak effective bandwidth.
 */
template <
    typename scalar_t,
    typename accscalar_t,
...
void distribution_binary_kernel(
  ...
  static_assert(
      std::is_same<
          ...
          curandStatePhilox4_32_10_t&>::value,
  ...
  AT_DISPATCH_ALL_TYPES_AND3(
      ...
      [&] {
        if ((std::is_same<scalar_t, int64_t>::value ||
          ...
          distribution_nullary_kernel<scalar_t, uint32_t, curand4_engine_calls>(
              iter,
              gen,
  ...
  AT_DISPATCH_ALL_TYPES_AND(
      ...
      [&] {
        if (std::is_same<scalar_t, int64_t>::value ||
          ...
          TORCH_CHECK(
              false,
              "random_full_64_bits_range_kernel_cuda handles only int64, double, float and bfloat16");
        }
      });
}
...
void uniform_and_transform(
  ...
  if (std::is_same<scalar_t, double>::value) {
    ...
    distribution_nullary_kernel<scalar_t, accscalar_t, curand4_engine_calls>(
        iter,
        gen,
        [] __device__(curandStatePhilox4_32_10_t * state) {
      ...
      [&] {
        ...
        auto normal_func = [mean, std] __device__(accscalar_t rand) {
          return static_cast<scalar_t>(
              transformation::normal<accscalar_t>(rand, mean, std));
        };
        normal_and_transform<scalar_t, accscalar_t, curand4_engine_calls>(
  ...
  AT_DISPATCH_FLOATING_TYPES_AND2(
      ...
      [&] {
        ...
        auto uniform_func = [range, from] __device__(accscalar_t rand) {
          ...
          // Note that this method is from legacy THCTensorRandom and is likely
          // to give you more 0-s, since, the probability of gettings 1-s is
          // higher than 0-s and by reversing the bounds, we are flipping the
          // probabilities of 1-s and 0-s. BEFORE TOUCHING THIS CODE READ:
        ...
        uniform_and_transform<scalar_t, accscalar_t, curand4_engine_calls>(
  ...
  AT_DISPATCH_ALL_TYPES_AND2(
      ...
      [&] {
        ...
        auto geometric_func = [p] __device__(accscalar_t rand) {
          return static_cast<scalar_t>(
              transformation::geometric<accscalar_t>(rand, p));
        };
        uniform_and_transform<scalar_t, accscalar_t, curand4_engine_calls>(
  ...
  AT_DISPATCH_FLOATING_TYPES_AND2(
      ...
      [&] {
        ...
        auto cauchy_func = [median, sigma] __device__(accscalar_t rand) {
          return static_cast<scalar_t>(
              transformation::cauchy<accscalar_t>(rand, median, sigma));
        };
        uniform_and_transform<scalar_t, accscalar_t, curand4_engine_calls>(
...
void bernoulli_kernel(const TensorBase& self, const TensorBase& p_, RNG gen) {
  ...
  AT_DISPATCH_ALL_TYPES_AND3(
      ...
      [&] {
        if (std::is_same<scalar_t, double>::value) {
          return bernoulli_tensor_cuda_kernel<double, double>(
              self, *p, rng_engine_inputs);
        } else {
          return bernoulli_tensor_cuda_kernel<scalar_t, float>(
              self, *p, rng_engine_inputs);
        }
      });
}
  ...
  AT_DISPATCH_ALL_TYPES_AND3(
      ...
      [&] {
        ...
        auto bernoulli_func = [p] __device__(accscalar_t rand) {
          return static_cast<scalar_t>(
              transformation::bernoulli<accscalar_t>(rand, p));
        };
        uniform_and_transform<scalar_t, accscalar_t, curand4_engine_calls>(
