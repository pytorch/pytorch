...
template<typename T, typename T_compute, template<class> class Op>
struct BinaryOpScalarListFunctor_ {
    __device__ void operator() (
        int chunk_size,
        TensorListScalarListMetadata<1, T>& tl) {
            ...
            T_compute y = tl.scalar_vals[tensor_loc];
...
struct BinaryOpScalarListFunctor {
    __device__ void operator() (
        int chunk_size,
        TensorListScalarListMetadata<2, T>& tl) {
            int tensor_loc = tl.block_to_tensor[blockIdx.x];
            int chunk_idx = tl.block_to_chunk[blockIdx.x];
            int n = tl.sizes[tensor_loc];
