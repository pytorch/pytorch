...
// log_probs (also calling them inputs) A few optimizations (similar to those
...
C10_LAUNCH_BOUNDS_2((std::is_same<scalar_t, float>::value ? 1024 : 896), 1)
  ...
  for (int64_t block_s = 0; block_s < 2 * max_target_length + 1;
    ...
    if (s < 2 * max_target_length + 1)
      log_alpha_data
          [la_batch_offset + /* la_input_stride * 0 */ +la_target_stride * s] =
              la;
  }
...
std::tuple<Tensor, Tensor> ctc_loss_gpu_template(
  ...
  dim3 grid(
      (2 * max_target_length + 1 + threads_target - 1) / threads_target,
  ...
  ctc_loss_log_alpha_gpu_kernel<scalar_t, target_t><<<grid, block, 0, stream>>>(
      ...
      log_probs.size(0),
      targets.data_ptr<target_t>(),
      target_lengths_t.data_ptr<int64_t>(),
      max_target_length,
...
template <typename scalar_t, typename target_t>
__global__ void C10_LAUNCH_BOUNDS_2(
...
C10_LAUNCH_BOUNDS_2((std::is_same<scalar_t, float>::value ? 1024 : 896), 1)
#endif
    ctc_loss_backward_collect_nonblank_gpu_kernel(
  ...
  for (int64_t t = 0; t < input_length; t++) {
    ...
    gpuAtomicAdd(
        &gradient_data
            [gr_batch_offset + t * gr_input_stride + gr_char_stride * target],
        -std::exp(
            log_alpha_data
...
C10_LAUNCH_BOUNDS_2((std::is_same<scalar_t, float>::value ? 1024 : 896), 1)
#endif
    ctc_loss_backward_collect_gpu_kernel(
        scalar_t* __restrict__ gradient_data,
        const scalar_t* __restrict__ grad_out_data,
        int64_t grad_out_batch_stride,
        const scalar_t* __restrict__ log_alpha_data,
...
template <typename scalar_t, ScalarType target_scalar_type>
Tensor ctc_loss_backward_gpu_template(
  ...
  if (is_large) { // large alphabet, large batch
    ...
    dim3 grid(
        ...
        (batch_size + threads_batch - 1) / threads_batch,
        1);
    ctc_loss_backward_collect_nonblank_gpu_kernel<scalar_t, target_t>
        <<<grid, block, 0, stream>>>(
...
std::tuple<Tensor, Tensor> ctc_loss_gpu(
  ...
  return AT_DISPATCH_FLOATING_TYPES(
      log_probs.scalar_type(), "ctc_loss_cuda", [&] {
        if (targets.scalar_type() == kLong) {
          return ctc_loss_gpu_template<scalar_t, kLong>(
              log_probs, targets, input_lengths, target_lengths, BLANK);
        } else {
          return ctc_loss_gpu_template<scalar_t, kInt>(
              log_probs, targets, input_lengths, target_lengths, BLANK);
        }
      });
}
...
Tensor ctc_loss_backward_gpu(
  ...
  return AT_DISPATCH_FLOATING_TYPES(
      log_probs.scalar_type(), "ctc_loss_backward_cuda", [&] {
        if (targets.scalar_type() == kLong) {
          return ctc_loss_backward_gpu_template<scalar_t, kLong>(
              grad,
              log_probs,
              targets,
              input_lengths,
          ...
          return ctc_loss_backward_gpu_template<scalar_t, kInt>(
              grad,
              log_probs,
              targets,
              input_lengths,
