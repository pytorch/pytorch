...
struct Float2 {
  accscalar_t v1, v2;
  __device__ Float2() {}
  __device__ Float2(scalar_t v1, scalar_t v2)
...
struct SumOp {
  ...
  __device__ __forceinline__ accscalar_t
  operator()(int batch, int plane, int n) {
...
struct GradOp {
  ...
  __device__ __forceinline__ Float2<scalar_t, accscalar_t> operator()(
      int batch,
      int plane,
      int n) {
...
template <typename scalar_t, typename accscalar_t>
static __device__ __forceinline__ Float2<scalar_t, accscalar_t> warpSum(
    Float2<scalar_t, accscalar_t> value) {
...
__host__ void flexible_launch_configs(
  ...
  int block_y = std::min(
      lastPow2(at::cuda::ATenCeilDiv(reduction, ELEMENTS_PER_THREAD)),
  ...
  int grid_y = std::min(
      at::cuda::ATenCeilDiv(reduction, block_y * ELEMENTS_PER_THREAD),
      MAX_H_BLOCK);
  if (coop_flag) {
...
__global__ void batch_norm_transform_input_kernel(
  ...
  if (train) {
    ...
    invstd = static_cast<stat_accscalar_t>(1) /
        device_sqrt(
                 static_cast<stat_accscalar_t>(var_or_invstd[plane]) + epsilon);
  }
...
__global__ void batch_norm_collect_statistics_kernel(
  ...
  stat_accscalar_t* shared_avg_var =
      (stat_accscalar_t*)&shared_n[C10_WARP_SIZE];
  ...
  if (tid < C10_WARP_SIZE) {
    ...
    avg =
        (tid < blockDim.x * blockDim.y / C10_WARP_SIZE ? shared_avg_var[2 * tid]
                                                       : stat_accscalar_t(0));
    var_n =
        (tid < blockDim.x * blockDim.y / C10_WARP_SIZE
...
__global__ void batch_norm_backward_kernel(
  ...
  if (train) {
    ...
    invstd = static_cast<stat_accscalar_t>(1) /
        device_sqrt(
                 static_cast<stat_accscalar_t>(running_var[plane]) + epsilon);
  }

  stat_accscalar_t weight_val = weight.size(0) > 0
      ? static_cast<stat_accscalar_t>(weight[plane])
      : stat_accscalar_t(1);
  ...
  Float2<input_scalar_t, stat_accscalar_t> res = reduce<
      Float2<input_scalar_t, stat_accscalar_t>,
      GradOp<
          input_scalar_t,
          stat_accscalar_t,
  ...
  if (grad_input.data() != NULL) {
    for (int batch = threadIdx.y; batch < grad_output.size(0);
         batch += blockDim.y) {
      for (int x = threadIdx.x; x < grad_output.size(2); x += blockDim.x) {
        ...
        if (train) {
          ...
          grad_input[batch][plane][x] =
              static_cast<input_scalar_t>((go - proj - grad_mean) * grad_scale);
        } else {
          grad_input[batch][plane][x] =
  ...
  for (int i = bid * blockDim.x + tid; i < feature_size;
    ...
    if (running_var.data() != NULL) {
      running_var[i] = static_cast<scalar_t>(
          (1 - momentum) * running_var[i] + momentum * unbiasedVar);
    }
  }
}
...
__global__ void batch_norm_backward_elemt_kernel(
  ...
  stat_accscalar_t factor_2_c = weight.size(0) > 0
      ? static_cast<stat_accscalar_t>(weight[plane])
      : stat_accscalar_t(1);
...
packed_accessor_or_dummy(const Tensor& t) {
  if (!t.defined()) {
...
std::tuple<Tensor, Tensor, Tensor> batch_norm_backward_cuda_template(
  ...
  auto input = input_reshaped.generic_packed_accessor<
      input_scalar_t,
      3,
      DefaultPtrTraits,
      index_t>();
  auto grad_output = grad_output_reshaped.generic_packed_accessor<
      input_scalar_t,
      3,
      DefaultPtrTraits,
      index_t>();
  ...
  auto running_var =
      packed_accessor_or_dummy<stat_scalar_t, 1, DefaultPtrTraits, index_t>(
          running_var_);
  auto save_mean =
      packed_accessor_or_dummy<accscalar_t, 1, DefaultPtrTraits, index_t>(
          save_mean_);
  auto save_invstd =
      packed_accessor_or_dummy<accscalar_t, 1, DefaultPtrTraits, index_t>(
          save_invstd_);
  ...
  batch_norm_backward_kernel<
      ...
      stat_scalar_t,
      accscalar_t,
      index_t><<<blocks, threads, 0, stream>>>(
      input,
      grad_output,
      grad_input,
      grad_weight,
...
void batch_norm_stats_cuda_template(
  ...
  auto input =
      input_reshaped
          .generic_packed_accessor<scalar_t, 3, RestrictPtrTraits, index_t>();
  ...
  batch_norm_collect_statistics_kernel<
      ...
      accscalar_t,
      index_t>
      <<<blocks, threads, 0, stream>>>(input, epsilon, 0.0, mean, invstd);
  C10_CUDA_KERNEL_LAUNCH_CHECK();
}

template <typename input_scalar_t, typename stat_scalar_t, typename index_t>
void batch_norm_elemt_cuda_template(
  ...
  auto input = input_reshaped.generic_packed_accessor<
      ...
      RestrictPtrTraits,
      index_t>();
  auto output = output_reshaped.generic_packed_accessor<
      input_scalar_t,
      3,
      RestrictPtrTraits,
      index_t>();
  auto weight =
      packed_accessor_or_dummy<stat_scalar_t, 1, RestrictPtrTraits, index_t>(
          weight_);
  auto bias =
      packed_accessor_or_dummy<stat_scalar_t, 1, RestrictPtrTraits, index_t>(
          bias_);
  ...
  // occupancy. Quiet likely, we could go with even more blocks than 1024. The
  // various planes are independent, so we use blocks for them.
  int tf = std::max<int>(
      getNumThreads(input.size(2) / 4),
  ...
  batch_norm_transform_input_kernel<
      ...
      stat_accscalar_t,
      true,
      index_t><<<blocks_trans, threads_trans, 0, stream>>>(
      input, output, mean, invstd, weight, bias, dummy_epsilon);
  C10_CUDA_KERNEL_LAUNCH_CHECK();
}

template <typename scalar_t, typename accscalar_t, typename index_t>
std::tuple<Tensor, Tensor> batch_norm_gather_stats_cuda_template(
    const Tensor& mean_,
    const Tensor& invstd_,
    const Tensor& running_mean_,
    const Tensor& running_var_,
  ...
  auto save_mean = save_mean_.generic_packed_accessor<
      accscalar_t,
      1,
      RestrictPtrTraits,
      index_t>();
  auto save_invstd = save_invstd_.generic_packed_accessor<
      accscalar_t,
      1,
      RestrictPtrTraits,
      index_t>();
...
batch_norm_backward_reduce_cuda_template(
  ...
  int block_y =
      std::min<int>(lastPow2(batch_size), MAX_BLOCK_SIZE / C10_WARP_SIZE);
  // We want block_x to be at least a warp width
  int block_x = std::min<int>(
      std::max<int>(getNumThreads(feature_size), C10_WARP_SIZE),
...
template <typename input_scalar_t, typename stat_scalar_t, typename index_t>
Tensor batch_norm_backward_elemt_cuda_template(
    const Tensor& grad_out_,
    const Tensor& input_,
    const Tensor& mean_,
    const Tensor& invstd_,
  ...
  auto input = input_reshaped.generic_packed_accessor<
      ...
      DefaultPtrTraits,
      index_t>();
  auto grad_input = grad_input_reshaped.generic_packed_accessor<
      input_scalar_t,
      3,
      DefaultPtrTraits,
      index_t>();
  ...
  // The kernel is pointwise, but we need to balance reading parameters
  // (save_var/mean, weight/bias) - which we only do once and have a for loop
  // afterwards - with having many threads and blocks and good occupancy. Quiet
  // likely, we could go with even more blocks than 1024. The various planes are
  // independent, so we use blocks for them.
  int tf = std::max<int>(
      getNumThreads(input.size(2) / 4),
      std::min<int>(getNumThreads(input.size(2)), 64));
  int tb = std::max<int>(64 / tf, 1);
...
// mean/biased_variance/unbiased_variance original apex name:
...
#pragma unroll
  ...
  welford_merge_block_vertical(
      count_th, mean_th, m2_th, shmem_count, shmem_mean, shmem_m2n);
...
#pragma unroll
  ...
  merge_block_vertical_backward(
      sum_dy_th, sum_dy_xmu_th, shmem_sum_dy, shmem_sum_dy_xmu);
  ...
  if (gridDim.y > 1) {
    ...
    if (is_last_block_done) {
      ...
      for (int y = threadIdx.y; y < gridDim.y; y += blockDim.y) {
        address_base = c_offset + y * stride;
        sum_dy_th +=
            (c_offset < stride ? staging_sum_dy[address_base]
                               : accscalar_t(0.0));
        sum_dy_xmu_th +=
...
void batch_norm_stats_channels_last_cuda_template(
  ...
  batch_norm_collect_statistics_channels_last_kernel<
      ...
      scalar_t,
      accscalar_t,
      ELEMENTS_PER_ITER><<<grid, block, 0, at::cuda::getCurrentCUDAStream()>>>(
...
void batch_norm_elemt_channels_last_cuda_template(
  ...
  if (input.scalar_type() == at::kHalf && weight.defined() &&
...
std::tuple<Tensor, Tensor, Tensor, Tensor>
batch_norm_backward_reduce_cuda_channels_last_template(
    ...
    AT_DISPATCH_FLOATING_TYPES_AND2(
        ...
        [&] {
          ...
          accscalar_t* staging_data_ptr =
              grid.y > 1 ? staging_data.data_ptr<accscalar_t>() : nullptr;
          int* semaphores_ptr =
              grid.y > 1 ? semaphores.data_ptr<int>() : nullptr;
