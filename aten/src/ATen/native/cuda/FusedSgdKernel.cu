  ...
  for (int ii = 0; ii < kILP; ii++) {
    ...
    if (depth > 2) {
      const auto momentum_buffer = is_first_step ?
        g : (momentum * static_cast<opmath_t>(r_args[2][ii]) + (1 - dampening) * g);
...
struct FusedSgdMathFunctor {
  static_assert(depth == 2 || depth == 3, "depth of 2 for SGD w/ momentum == 0, 3 for SGD w/ momentum != 0");
  C10_DEVICE __forceinline__ void operator()(
    ...
    const auto use_faster_load_store = (n % kILP == 0) && (chunk_size % kILP == 0) && all_aligned;
    if (use_faster_load_store) {
      for (auto i_start = threadIdx.x; i_start * kILP < n && i_start * kILP < chunk_size; i_start += blockDim.x) {
#pragma unroll
        for (auto i = 0; i < depth; i++) {
          load_store(r_args[i], args[i], 0, i_start);
        }
        sgd_math<scalar_t, depth>(r_args, weight_decay, momentum, lr, dampening, nesterov, maximize, is_first_step, grad_scale_ptr);
      ...
      for (auto i_start = 0; i_start < n && i_start < chunk_size; i_start += blockDim.x * kILP) {
