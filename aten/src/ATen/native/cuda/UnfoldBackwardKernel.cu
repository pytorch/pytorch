...
void _unfold_backward_internal_kernel(
  ...
  if (is_step_ge_size) {
    char* __restrict__ idx_last_dim_ptr =
        reinterpret_cast<char*>(iter.data_ptr(3));
    ...
    auto loop = [=] C10_DEVICE(int i) {
      ...
      auto* __restrict__ grad_out_data =
          reinterpret_cast<scalar_t*>(grad_out_ptr + offsets[0]);
      auto* __restrict__ grad_in_data =
    ...
    _launch_unfold_backward_kernel<num_threads, thread_work_size>(
        iter.numel(), loop);
  } else {
