...
void ndtr_kernel_cuda(TensorIteratorBase& iter) {
  AT_DISPATCH_FLOATING_TYPES(iter.common_dtype(), "ndtr_cuda", [&]() {
    gpu_kernel(iter, []GPU_LAMBDA(scalar_t a) -> scalar_t {
      return calc_ndtr(a);
    });
  });
...
REGISTER_DISPATCH(special_ndtr_stub, &ndtr_kernel_cuda);
