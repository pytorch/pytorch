...
bool launchTunableGemmAndBias(cublasCommonArgs &args, const Scalar& alpha, const scalar_t* bias, cuda::blas::GEMMAndBiasActivationEpilogue activation) {
  ...
  // Returns true if `gemm` handled `params` -- a tuned concrete/wildcard entry
  // was dispatched, or tuning is enabled and operator() will tune. Returns
  // false only on a pure miss with tuning disabled, so the caller falls back
  // to the non-tunable path.
  auto try_dispatch = [&](auto& gemm) -> bool {
    ...
    if (result == at::cuda::tunable::ResultEntry::Null()
      ...
      // Concrete miss + tuning disabled: match persisted wildcard entries by
      // token pattern. AOTI's cpp_wrapper emits no TunableDynamicDimsGuard, so
      // params.dynamic_dims_mask may be empty even when wildcards were
