...
Tensor& addmm_out_cuda_impl(
  ...
  bool transpose_mat1;
  bool transpose_mat2;
  c10::MaybeOwned<Tensor> mat1_ =
      prepare_matrix_for_cublas(transpose_result ? mat2 : mat1, transpose_mat1);
  c10::MaybeOwned<Tensor> mat2_ =
      prepare_matrix_for_cublas(transpose_result ? mat1 : mat2, transpose_mat2);
...
Tensor& bmm_out_cuda(
  ...
  at::native::resize_output(
      result, {batch1.sizes()[0], batch1.sizes()[1], batch2.sizes()[2]});
  Scalar beta(0.0);
  Scalar alpha(1.0);
  {
    NoNamesGuard guard;
...
Tensor bmm_cuda(const Tensor& self, const Tensor& mat2) {
  ...
  Tensor result = at::empty(
      {self.sizes()[0], self.sizes()[1], mat2.sizes()[2]}, self.options());
...
void addr_kernel_cuda(
  ...
  AT_DISPATCH_ALL_TYPES_AND_COMPLEX_AND2(
      kBFloat16, kHalf, iter.dtype(), "addr_cuda", [&] {
        auto beta_val = beta.to<scalar_t>();
        auto alpha_val = alpha.to<scalar_t>();
...
void linalg_vector_norm_kernel_cuda_impl(TensorIterator& iter, Scalar ord) {
  ...
  if (ord_val == 0) {
    ...
    gpu_reduce_kernel<scalar_t, out_t>(
        iter, NormOps<scalar_t, acc_t>{static_cast<acc_t>(ord_val)}, 0);
  }
...
static void linalg_vector_norm_kernel_cuda(TensorIterator& iter, Scalar ord) {
  if (iter.output().scalar_type() == kHalf) {
    return linalg_vector_norm_kernel_cuda_impl<at::Half, float>(iter, ord);
      ...
      iter.input_dtype() == kHalf && iter.output().scalar_type() == kFloat) {
    // type promotion that does cast and reduction in a single kernel
    return linalg_vector_norm_kernel_cuda_impl<at::Half, float, float>(
        iter, ord);
    ...
    return linalg_vector_norm_kernel_cuda_impl<at::BFloat16, float>(iter, ord);
      ...
      iter.output().scalar_type() == kFloat) {
    // type promotion that does cast and reduction in a single kernel
    return linalg_vector_norm_kernel_cuda_impl<at::BFloat16, float, float>(
        iter, ord);
  }
  AT_DISPATCH_FLOATING_AND_COMPLEX_TYPES(
      iter.input_dtype(), "linalg_vector_norm_cuda", [&] {
