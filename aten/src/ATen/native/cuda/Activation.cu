...
void prelu_cuda_kernel_share_weights(
  ...
  at::native::gpu_kernel(iter, [weight_data] GPU_LAMBDA(scalar_t input_val) {
...
Tensor prelu_cuda(const Tensor& self, const Tensor& weight_) {
  ...
  if (weight_num == 1) {
    AT_DISPATCH_FLOATING_TYPES_AND(
        at::ScalarType::Half, input.scalar_type(), "prelu_cuda", [&] {
          prelu_cuda_kernel_share_weights<scalar_t>(
              input, result, weight.data_ptr<scalar_t>());
        });
    ...
    const dim3 block = dim3(
        std::min(static_cast<int64_t>(cuda::getApplyBlock().x), input_numel));
    dim3 grid;
    int curDevice = -1;
    cudaGetDevice(&curDevice);
    cudaStream_t stream = at::cuda::getCurrentCUDAStream(curDevice);
    TORCH_CHECK(
        cuda::getApplyGrid(input_numel, grid, curDevice),
        "prelu: input too large or too many dimensions");

    AT_DISPATCH_FLOATING_TYPES_AND(
        at::ScalarType::Half, input.scalar_type(), "prelu_cuda", [&] {
          prelu_cuda_kernel_multi_weights<scalar_t><<<grid, block, 0, stream>>>(
...
void prelu_cuda_backward_kernel_share_weights(
  ...
  gpu_kernel_multiple_outputs(
      iter,
      [=] GPU_LAMBDA(scalar_t input, scalar_t grad_out)
...
__global__ void prelu_cuda_backward_kernel_multi_weights(
  ...
  weight_grad_collector[linearId] =
      (input_data_val > 0) ? scalar_t(0) : input_data_val * grad_out_data_val;
}

std::tuple<Tensor, Tensor> prelu_backward_cuda(
  ...
  if (weight_num == 1) {
    AT_DISPATCH_FLOATING_TYPES_AND(
        at::ScalarType::Half, input.scalar_type(), "prelu_backward_cuda", [&] {
    ...
    TORCH_CHECK(
        cuda::getApplyGrid(input_numel, grid, curDevice),
        "prelu_backward_cuda: input too large or too many dimensions");
  ...
  AT_DISPATCH_FLOATING_TYPES_AND2(
      ...
      [&]() {
        ...
        gpu_kernel(iter, [beta, threshold] GPU_LAMBDA(scalar_t a) -> scalar_t {
          return (a * beta) > threshold
              ? a
              : static_cast<scalar_t>(::log1p(std::exp(a * beta))) / beta;
        });
      });
}
      ...
      [&]() {
        ...
        gpu_kernel(
            ...
            [negcoef, poscoef, negiptcoef] GPU_LAMBDA(scalar_t a) -> scalar_t {
              return a > scalar_t(0)
                  ? a * poscoef
                  : (static_cast<scalar_t>(std::exp(a * negiptcoef)) -
                     scalar_t(1.)) *
                      negcoef;
            });
      });
      ...
      [&]() {
        ...
        gpu_kernel(
            ...
            [negcoef, poscoef, negiptcoef, is_result] GPU_LAMBDA(
              ...
              if (is_result) {
                return b <= scalar_t(0) ? a * negiptcoef * (b + negcoef)
                                        : a * poscoef;
              } else {
                return b <= scalar_t(0) ? a * negiptcoef * negcoef *
                        (static_cast<scalar_t>(std::exp(b * negiptcoef)))
                                        : a * poscoef;
              }
            });
      });
...
void hardswish_backward_kernel(TensorIterator& iter) {
  AT_DISPATCH_FLOATING_TYPES_AND2(
      ...
      [&]() {
        ...
        gpu_kernel(
            iter,
            [zero, three, neg_three, one_half] GPU_LAMBDA(
...
Tensor threshold_backward_cuda(
    const Tensor& grad,
    const Tensor& self,
    const Scalar& threshold) {
