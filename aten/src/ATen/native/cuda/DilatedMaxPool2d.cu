...
static __device__ inline int p_end(
    int size,
    int pad,
    int pooled_size,
    int stride) {
...
__global__ void max_pool_forward_nchw(
  ...
  CUDA_KERNEL_LOOP(index, nthreads) {
    ...
    while (wstart < 0)
      wstart += dilation_w;
    accscalar_t maxval =
        at::numeric_limits<accscalar_t>::lower_bound(); // -Infinity
    ...
    for (int h = hstart; h < hend; h += dilation_h) {
      for (int w = wstart; w < wend; w += dilation_w) {
        ...
        if ((ScalarConvert<scalar_t, accscalar_t>::to(val) > maxval) ||
...
__global__ void max_pool_forward_nhwc(
  ...
  out_cached = &out_cached
                   [(threadIdx.z * blockDim.y + threadIdx.y) * kernel_size_C *
                    blockDim.x];
  out_mask_cached = &out_mask_cached
                        [(threadIdx.z * blockDim.y + threadIdx.y) *
                         kernel_size_C * blockDim.x];
  ...
  for (int oh = ostartH; oh < oendH; oh += blockDim.z) {
    ...
    for (int ow = ostartW; ow < oendW; ow += blockDim.y) {
      ...
      for (int ih = hstart; ih < hend; ih++) {
        for (int iw = wstart; iw < wend; iw++) {
          ...
          const scalar_t* ptr_input =
              bottom_data + ih * in_stride_h + iw * in_stride_w;
          for (int c = channel_offset; c < channels;
               c += blockDim.x * kernel_stride_C) {
            scalar_t val = ptr_input[c * in_stride_c];
            if ((scalar_cast<accscalar_t>(val) > out_cached[cached_index]) ||
                THCNumerics<scalar_t>::isnan(val)) {
      ...
      scalar_t* ptr_output_data =
          top_data + (oh * pooled_width + ow) * channels;
        ...
        for (int ph = phstart; ph < phend; ++ph) {
          for (int pw = pwstart; pw < pwend; ++pw) {
            if (top_mask[ph * pooled_width + pw + offset] == h * width + w) {
              gradient += ScalarConvert<scalar_t, accscalar_t>::to(
                  top_diff[ph * pooled_width + pw + offset]);
            }
          }
        }
        bottom_diff[(n * channels + c) * height * width + index] =
            ScalarConvert<accscalar_t, scalar_t>::to(gradient);
      }
    }
  }
}
      ...
      if ((phstart + 1 != phend) || (pwstart + 1 != pwend)) {
        for (int oh = phstart; oh < phend; ++oh) {
          for (int ow = pwstart; ow < pwend; ++ow) {
            int cached_index = threadIdx.x;
            const int64_t* ptr_top_mask =
                top_mask + oh * out_stride_h + ow * out_stride_w;
            for (int c = channel_offset; c < channels;
                 c += blockDim.x * kernel_stride_C) {
              if (ptr_top_mask[c * out_stride_c] == index_shift) {
                out_cached[cached_index] += scalar_cast<accscalar_t>(
                    top_diff
        ...
        const int64_t* ptr_top_mask =
            top_mask + phstart * out_stride_h + pwstart * out_stride_w;
        scalar_t* ptr_bottom_diff = bottom_diff + index_shift * channels;
        int cached_index = threadIdx.x;
        for (int c = channel_offset; c < channels;
             c += blockDim.x * kernel_stride_C) {
          if (ptr_top_mask[c * out_stride_c] == index_shift) {
            ptr_bottom_diff[c] = scalar_cast<scalar_t>(
                top_diff
                    [phstart * out_stride_h + pwstart * out_stride_w +
                     c * out_stride_c]);
          }
          cached_index += blockDim.x;
        }
...
(const Tensor& input_,
 ...
 const Tensor& indices) {
  ...
  checkAllSameGPU(
      "max_pool2d_with_indices_out_cuda", {output_arg, indices_arg, input_arg});
        ...
        switch (memory_format) {
          case MemoryFormat::ChannelsLast: {
            ...
            int block_x = std::min<int>(
                maxThreadsDim[0],
                std::min<int>(lastPow2(nInputPlane), at::cuda::warp_size()));
            int block_y = std::min<int>(
                maxThreadsDim[1],
                std::min<int>(lastPow2(outputWidth), max_threads / block_x));
            int block_z = std::min<int>(
                maxThreadsDim[2],
                std::min<int>(
                    lastPow2(outputHeight), max_threads / block_x / block_y));
            block_x = std::min<int>(
                maxThreadsDim[0],
                std::min<int>(
                    lastPow2(nInputPlane), max_threads / block_y / block_z));
            ...
            int kernel_size_C = cuda::ATenCeilDiv(
                safe_downcast<int, int64_t>(nInputPlane),
                block_x * kernel_stride_C);

            int grid_x = nbatch * kernel_stride_C;
            int grid_y = std::min<int>(
                at::cuda::getCurrentDeviceProperties()->maxGridSize[1],
                cuda::ATenCeilDiv(
                    safe_downcast<int, int64_t>(outputWidth),
                    block_y * BLOCK_STRIDE));
            int grid_z = std::min<int>(
                at::cuda::getCurrentDeviceProperties()->maxGridSize[2],
                cuda::ATenCeilDiv(
                    safe_downcast<int, int64_t>(outputHeight),
                    block_z * BLOCK_STRIDE));
          ...
          case MemoryFormat::Contiguous: {
            ...
            max_pool_forward_nchw<scalar_t, scalar_t>
                <<<cuda::ATenCeilDiv(count, num_threads),
                   num_threads,
                   0,
...
void max_pool2d_with_indices_backward_out_cuda_template(
  ...
  checkAllSameGPU(
      "max_pool2d_with_indices_out_cuda",
      ...
      [&] {
        ...
        switch (memory_format) {
          case MemoryFormat::ChannelsLast: {
            ...
            int block_y = std::min<int>(
                maxThreadsDim[1],
                std::min<int>(lastPow2(inputWidth), max_threads / block_x));
            int block_z = std::min<int>(
                maxThreadsDim[2],
                std::min<int>(
                    lastPow2(inputHeight), max_threads / block_x / block_y));
            block_x = std::min<int>(
                maxThreadsDim[0],
                std::min<int>(
                    lastPow2(nInputPlane), max_threads / block_y / block_z));
            ...
            int grid_y = std::min<int>(
                at::cuda::getCurrentDeviceProperties()->maxGridSize[1],
                cuda::ATenCeilDiv(
                    safe_downcast<int, int64_t>(inputWidth),
                    block_y * BLOCK_STRIDE));
            int grid_z = std::min<int>(
                at::cuda::getCurrentDeviceProperties()->maxGridSize[2],
                cuda::ATenCeilDiv(
                    safe_downcast<int, int64_t>(inputHeight),
                    block_z * BLOCK_STRIDE));
            const dim3 grid(grid_x, grid_y, grid_z);
            ...
            // If it is launched on output layer, atomic_add would not provide
            // much benefit on FP16. Please check comments at
            // https://github.com/pytorch/pytorch/pull/34519.
