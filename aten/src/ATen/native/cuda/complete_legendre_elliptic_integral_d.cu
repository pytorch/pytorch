...
#include <ATen/native/special/complete_legendre_elliptic_integral_d.h>
...
const auto complete_legendre_elliptic_integral_d_string = jiterator_stringify(
  ...
  T1 complete_legendre_elliptic_integral_d(T1 k) {
    return k;
...
void special_complete_legendre_elliptic_integral_d_cuda_kernel(TensorIteratorBase &iterator) {
#if AT_USE_JITERATOR()
  AT_DISPATCH_FLOATING_AND_COMPLEX_TYPES(iterator.common_dtype(), "complete_legendre_elliptic_integral_d_cuda_kernel", [&]() {
  ...
  AT_DISPATCH_FLOATING_AND_COMPLEX_TYPES(iterator.common_dtype(), "complete_legendre_elliptic_integral_d_cuda_kernel", [&]() {
    gpu_kernel_with_scalars(iterator, []GPU_LAMBDA(scalar_t k) -> scalar_t {
      return at::native::special::complete_legendre_elliptic_integral_d(k);
    });
  });
#endif
} // void special_complete_legendre_elliptic_integral_d_cuda_kernel(TensorIteratorBase &iterator)
