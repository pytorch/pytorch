 ...
 * Description:
 *    this function adaptively average pools an input 4D tensor along dimensions
 * 2 and 3 NHWC layout for both input and output tensor 4D input, 4D output
 */
  ...
  switch (input.suggest_memory_format()) {
    case at::MemoryFormat::ChannelsLast: {
      ...
      if (!output.is_contiguous(at::MemoryFormat::ChannelsLast)) {
        ...
        output.resize_({sizeB, sizeC, osizeH, osizeW})
            .as_strided_(
                {sizeB, sizeC, osizeH, osizeW},
                {sizeC * osizeH * osizeW, 1, osizeW * sizeC, sizeC});
      }
      ...
      int block_x = std::min<int>(
          maxThreadsDim[0],
          std::min<int>(lastPow2(sizeC), at::cuda::warp_size()));
      int block_y = std::min<int>(
          maxThreadsDim[1],
          std::min<int>(lastPow2(osizeW), max_threads / block_x));
      int block_z = std::min<int>(
          maxThreadsDim[2],
          std::min<int>(lastPow2(osizeH), max_threads / block_x / block_y));
      block_x = std::min<int>(
          maxThreadsDim[0],
          std::min<int>(lastPow2(sizeC), max_threads / block_y / block_z));
      ...
      // it's OK to clip grid_y & grid_z, as we block the two dimensions in the
      // kernel;
      ...
      AT_DISPATCH_FLOATING_TYPES_AND2(
          ...
          kBFloat16,
          input_.scalar_type(),
          "adaptive_avg_pool2d_nhwc_cuda",
          [&] {
            size_t shmem_size = (kernel_size_C * block_x * block_y * block_z) *
                sizeof(scalar_t);
            AT_ASSERT(shmem_size <= sharedMemPerBlock);
            adaptive_average_pool_nhwc<int32_t>
...
void adaptive_avg_pool2d_backward_out_cuda_template(
  ...
  checkAllSameGPU(
      "cudnn_adaptive_avg_pooling2d_out",
  ...
  switch (input.suggest_memory_format()) {
    case at::MemoryFormat::ChannelsLast: {
      ...
      int block_y = std::min<int>(
          maxThreadsDim[1],
          std::min<int>(lastPow2(isizeW), max_threads / block_x));
      int block_z = std::min<int>(
          maxThreadsDim[2],
          std::min<int>(lastPow2(isizeH), max_threads / block_x / block_y));
      block_x = std::min<int>(
          maxThreadsDim[0],
          std::min<int>(lastPow2(sizeC), max_threads / block_y / block_z));
      const dim3 block(block_x, block_y, block_z);
      ...
      AT_DISPATCH_FLOATING_TYPES_AND2(
          ...
          kBFloat16,
          input.scalar_type(),
          "adaptive_avg_pool2d_backward_nhwc_cuda",
          [&] {
            size_t shmem_size = (kernel_size_C * block_x * block_y * block_z +
                                 osizeH + osizeW) *
                    sizeof(scalar_t) +
            ...
            AT_ASSERT(shmem_size <= sharedMemPerBlock);
            adaptive_average_gradinput_nhwc<int32_t>
                <<<grid, block, shmem_size, at::cuda::getCurrentCUDAStream()>>>(
...
Tensor& adaptive_avg_pool2d_out_cuda(
    ...
    const Tensor& input,
    IntArrayRef output_size) {
  adaptive_avg_pool2d_out_cuda_template(output, input, output_size);
  return output;
}
...
Tensor& adaptive_avg_pool2d_backward_out_cuda(
  ...
  globalContext().alertNotDeterministic(
      "adaptive_avg_pool2d_backward_out_cuda");
  gradInput.resize_as_(input);
  adaptive_avg_pool2d_backward_out_cuda_template(gradInput, gradOutput, input);
  return gradInput;
}
