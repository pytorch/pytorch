...
void triangular_solve_batched_magma(const Tensor& A, const Tensor& B, bool upper, bool transpose, bool conjugate_transpose, bool unitriangular) {
...
void triangular_solve_kernel(const Tensor& A, const Tensor& B, const Tensor& /*infos*/, bool upper, bool transpose, bool conjugate_transpose, bool unitriangular) {
  ...
  if (batchCount(A) <= 8 && A.size(-1) >= 64) {
    triangular_solve_cublas(A, B, upper, transpose, conjugate_transpose, unitriangular);
  } else {
#ifndef USE_MAGMA
    triangular_solve_batched_cublas(A, B, upper, transpose, conjugate_transpose, unitriangular);
#else
    // cuBLAS batched is faster than MAGMA batched up until 512x512, after that MAGMA is faster
    if (A.size(-1) <= 512) {
      triangular_solve_batched_cublas(A, B, upper, transpose, conjugate_transpose, unitriangular);
    } else {
      triangular_solve_batched_magma(A, B, upper, transpose, conjugate_transpose, unitriangular);
    }
#endif // USE_MAGMA
  }
}
