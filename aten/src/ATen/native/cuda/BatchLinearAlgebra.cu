...
void magmaSolveBatched<c10::complex<double>>(
  ...
  magma_zgesv_batched(
      ...
      ldda,
      dipiv_array,
      reinterpret_cast<magmaDoubleComplex**>(dB_array),
      lddb,
      dinfo_array,
      batch_count,
      magma_queue.get_queue());
...
void magmaSolveBatched<c10::complex<float>>(
  ...
  magma_cgesv_batched(
      ...
      ldda,
      dipiv_array,
      reinterpret_cast<magmaFloatComplex**>(dB_array),
      lddb,
      dinfo_array,
      batch_count,
      magma_queue.get_queue());
...
void magmaLuNoPiv<c10::complex<double>>(
  ...
  magma_zgetrf_nopiv_gpu(
      m, n, reinterpret_cast<magmaDoubleComplex*>(dA), ldda, info);
  AT_CUDA_CHECK(cudaGetLastError());
}
...
void magmaLuNoPiv<c10::complex<float>>(
  ...
  magma_cgetrf_nopiv_gpu(
      m, n, reinterpret_cast<magmaFloatComplex*>(dA), ldda, info);
  AT_CUDA_CHECK(cudaGetLastError());
}
...
void magmaLuNoPivBatched<double>(
  ...
  magma_dgetrf_nopiv_batched(
      m, n, dA_array, ldda, info_array, batchsize, magma_queue.get_queue());
  AT_CUDA_CHECK(cudaGetLastError());
}
...
void magmaCholesky<c10::complex<double>>(
  ...
  MagmaStreamSyncGuard guard;
  magma_zpotrf_gpu(
      uplo, n, reinterpret_cast<magmaDoubleComplex*>(dA), ldda, info);
  AT_CUDA_CHECK(cudaGetLastError());
}
...
void magmaCholesky<c10::complex<float>>(
  ...
  MagmaStreamSyncGuard guard;
  magma_cpotrf_gpu(
      uplo, n, reinterpret_cast<magmaFloatComplex*>(dA), ldda, info);
  AT_CUDA_CHECK(cudaGetLastError());
}
...
void magmaCholeskyBatched<double>(
  ...
  magma_dpotrf_batched(
      uplo, n, dA_array, ldda, info_array, batchsize, magma_queue.get_queue());
  AT_CUDA_CHECK(cudaGetLastError());
}
...
template <>
inline magma_int_t magmaGeqrfOptimalBlocksize<double>(
    magma_int_t m,
    magma_int_t n) {
...
template <>
inline magma_int_t magmaGeqrfOptimalBlocksize<float>(
    magma_int_t m,
    magma_int_t n) {
...
void magmaSyevd<double>(
  ...
  MagmaStreamSyncGuard guard;
  magma_dsyevd_gpu(
      jobz, uplo, n, dA, ldda, w, wA, ldwa, work, lwork, iwork, liwork, info);
  AT_CUDA_CHECK(cudaGetLastError());
}
...
void magmaSyevd<c10::complex<double>, double>(
  ...
  magma_zheevd_gpu(
      ...
      ldda,
      w,
      reinterpret_cast<magmaDoubleComplex*>(wA),
      ldwa,
...
void magmaSyevd<c10::complex<float>, float>(
  ...
  magma_cheevd_gpu(
      ...
      ldda,
      w,
      reinterpret_cast<magmaFloatComplex*>(wA),
      ldwa,
...
void magmaSvd<c10::complex<float>, float>(
  ...
  magma_cgesdd(
      ...
      lda,
      s,
      reinterpret_cast<magmaFloatComplex*>(U),
      ldu,
      reinterpret_cast<magmaFloatComplex*>(VT),
      ldvt,
      reinterpret_cast<magmaFloatComplex*>(work),
      lwork,
      rwork,
...
void magmaSvd<c10::complex<double>, double>(
  ...
  magma_zgesdd(
      ...
      lda,
      s,
      reinterpret_cast<magmaDoubleComplex*>(U),
      ldu,
      reinterpret_cast<magmaDoubleComplex*>(VT),
      ldvt,
      reinterpret_cast<magmaDoubleComplex*>(work),
      lwork,
      rwork,
...
void magmaGels<c10::complex<float>>(
  ...
  magma_cgels_gpu(
      ...
      reinterpret_cast<magmaFloatComplex*>(dB),
      lddb,
      reinterpret_cast<magmaFloatComplex*>(hwork),
      lwork,
      info);
  AT_CUDA_CHECK(cudaGetLastError());
}
...
void magmaGels<c10::complex<double>>(
  ...
  magma_zgels_gpu(
      ...
      reinterpret_cast<magmaDoubleComplex*>(dB),
      lddb,
      reinterpret_cast<magmaDoubleComplex*>(hwork),
      lwork,
      info);
  AT_CUDA_CHECK(cudaGetLastError());
}
...
/*
  ...
  info < 0, but small        |  For info = -i, the i-th argument had an illegal
  value info > 0                   |  Function-specific error such as singular
  matrix MAGMA_ERR_DEVICE_ALLOC     |  Could not allocate GPU device memory
  MAGMA_ERR_HOST_ALLOC       |  Could not allocate CPU host memory
  MAGMA_ERR_ILLEGAL_VALUE    |  An argument had an illegal value (deprecated;
  instead it should return -i to say the i-th argument was bad)
  MAGMA_ERR_INVALID_PTR      |  Can't free pointer
  MAGMA_ERR_NOT_IMPLEMENTED  |  Function or option not implemented
  MAGMA_ERR_NOT_SUPPORTED    |  Function or option not supported on the current
  architecture
*/
void checkMagmaInternalError(
    magma_int_t info,
    const std::string& magma_function_name) {
...
static void apply_solve(Tensor& b, Tensor& A, Tensor& infos_out) {
#ifndef USE_MAGMA
  AT_ERROR(
      "solve: MAGMA library not found in "
  ...
  if (b.dim() == 2) {
    ...
    Tensor infos =
        at::empty(infos_out.sizes(), infos_out.options().device(kCPU));
    magmaSolve<scalar_t>(
        n,
        nrhs,
        A_data,
        lda,
        ipiv.data_ptr<magma_int_t>(),
        b_data,
        lda,
        infos.data_ptr<magma_int_t>());
    ...
    // and these cover floor(batch_size / batch_limit) * batch_limit matrix
    // solves
    int64_t mini_batches = batch_size / batch_limit, mini_idx;
    for (mini_idx = 0; mini_idx < mini_batches * batch_limit;
         mini_idx += batch_limit) {
    ...
    // Compute whatever is left = batch_size - floor(batch_size / batch_limit) *
    // batch_limit which concisely is equal to batch_size % batch_limit
...
std::tuple<Tensor, Tensor> _solve_helper_cuda(
    const Tensor& self,
    const Tensor& A) {
...
Tensor& _linalg_solve_out_helper_cuda(
    Tensor& result,
    Tensor& input,
    Tensor& infos) {
  ...
  // contain data of origianl 'input' tensor (left-hand-side of the linear
  // system)
...
input. 'infos_lu' is for holding magmaLU errors, and 'infos_getri' is for
holding magmaGetri errors For more information see MAGMA's documentation for
GETRI and GETRF routines.
*/
template <typename scalar_t>
static void apply_batched_inverse(
    Tensor& self,
    Tensor& self_inv,
    Tensor& infos_lu,
    Tensor& infos_getri) {
#ifndef USE_MAGMA
  AT_ERROR(
      "inverse: MAGMA library not found in "
      "compilation. Please rebuild with MAGMA.");
#else
  ...
  // Initialize to avoid memory access violations inside magma kernels
  // (gh-51930).
  ...
  if (batch_size % batch_limit != 0) {
    magmaGetriBatched<scalar_t>(
        ...
        lda,
        &ipiv_array[mini_idx],
        &self_inv_array[mini_idx],
        lda,
        &infos_getri_data[mini_idx],
        batch_size % batch_limit,
        magma_queue);
  }
#endif
...
template <typename scalar_t>
static void apply_single_inverse(
    Tensor& self,
    Tensor& infos_lu,
    Tensor& infos_getri) {
#ifndef USE_MAGMA
  ...
  magmaLu<scalar_t>(
      ...
      lda,
      ipiv.data_ptr<magma_int_t>(),
      infos_lu.data_ptr<magma_int_t>());
  magmaGetri<scalar_t>(
      n,
      self_data,
      lda,
      ipiv.data_ptr<magma_int_t>(),
      dwork.data_ptr<scalar_t>(),
      lwork,
      infos_getri.data_ptr<magma_int_t>());
#endif
}
...
Tensor _inverse_helper_cuda_legacy(const Tensor& self) {
  ...
  if (self.dim() > 2) {
    ...
    AT_DISPATCH_FLOATING_AND_COMPLEX_TYPES(
        self.scalar_type(), "inverse_cuda", [&] {
          apply_single_inverse<scalar_t>(
              self_inv_working_copy, infos_lu, infos_getri);
        });
...
// This is a type dispatching helper function for 'apply_batched_inverse' and
// 'singleCheckErrors'
Tensor& _linalg_inv_out_helper_cuda_legacy(
    Tensor& result,
    Tensor& infos_lu,
    Tensor& infos_getri) {
  // assuming result is in column major order and contains the matrices to
  // invert
  if (result.dim() > 2) {
    auto input_working_copy = cloneBatchedColumnMajor(result);
    AT_DISPATCH_FLOATING_AND_COMPLEX_TYPES(
        result.scalar_type(), "linalg_inv_out_cuda", [&] {
          apply_batched_inverse<scalar_t>(
...
Tensor& _linalg_inv_out_helper_cuda(
    Tensor& result,
    Tensor& infos_lu,
    Tensor& infos_getri) {
...
#ifdef USE_CUSOLVER
  if ((result.dim() == 2) ||
      (/* result.dim() > 2 && */ batchCount(result) <= 2) || !use_magma_) {
    return _linalg_inv_out_helper_cuda_lib(
        result, infos_lu, infos_getri); // cusolver or cublas
  } else {
    return _linalg_inv_out_helper_cuda_legacy(
        result, infos_lu, infos_getri); // magma-cuda
  }
#else
  return _linalg_inv_out_helper_cuda_legacy(
      result, infos_lu, infos_getri); // magma-cuda
...
static void apply_cholesky_solve(
...
#ifndef USE_MAGMA
  AT_ERROR(
      "cholesky_solve: MAGMA library not found in "
      "compilation. Please rebuild with MAGMA.");
#else
  ...
  int info_tmp = 0;
  if (b.dim() == 2) {
    magmaCholeskySolve<scalar_t>(
        uplo, n, nrhs, A_data, lda, b_data, lda, &info_tmp);
    info = info_tmp;
  } else {
    auto A_mat_stride = matrixStride(A);
    auto b_mat_stride = matrixStride(b);
...
Tensor _cholesky_solve_helper_cuda_magma(
    const Tensor& self,
    const Tensor& A,
    bool upper) {
  int64_t info = 0;
  ...
  AT_DISPATCH_FLOATING_AND_COMPLEX_TYPES(
      self.scalar_type(), "cholesky_solve_cuda", [&] {
        apply_cholesky_solve<scalar_t>(
            self_working_copy, A_working_copy, upper, info);
      });
  TORCH_CHECK(info == 0, "MAGMA cholesky_solve : invalid argument: ", -info);
  return self_working_copy;
}

// Todo: cusolverDn<T>potrsBatched only supports nrhs == 1 and does not have
// good performance.
...
static void apply_cholesky(const Tensor& self, bool upper, const Tensor& info) {
  ...
  if (self.dim() == 2) {
    ...
    // For complex input the batch limit is 65535 (determined experimentally,
    // see https://github.com/pytorch/pytorch/pull/47047#discussion_r516086923
...
matrix 'input' using the Cholesky solver This is an in-place routine, content of
'input' is overwritten. 'infos' is an int Tensor containing error codes for each
matrix in the batched input. MAGMA requires 'infos' to reside in CPU memory. For
more information see MAGMA's documentation for POTRS routine.
*/
template <typename scalar_t>
static void apply_cholesky_inverse(Tensor& input, Tensor& infos, bool upper) {
  ...
  TORCH_CHECK(
      false,
      "cholesky_inverse: MAGMA library not found in compilation. Please rebuild with MAGMA.");
#else
  // magmaCholeskyInverse (magma_dpotri_gpu) is slow because internally
  // it transfers data several times between GPU and CPU and calls lapack
  // routine on CPU using magmaCholeskySolveBatched is a lot faster note that
  // magmaCholeskySolve is also slow

  // 'input' is modified in-place we need to clone it and replace with a
  // diagonal matrix for apply_cholesky_solve
  ...
  // magma's potrs_batched doesn't take matrix-wise array of ints as an 'info'
  // argument it returns a single 'magma_int_t' if info = 0 the operation is
  // successful, if info = -i, the i-th parameter had an illegal value.
  int64_t info_tmp = 0;
...
Tensor& cholesky_inverse_kernel_impl_magma(
    Tensor& result,
    Tensor& infos,
    bool upper) {
  AT_DISPATCH_FLOATING_AND_COMPLEX_TYPES(
      result.scalar_type(), "cholesky_inverse_out_cuda", [&] {
...
/*
  ...
  is overwritten. This is a "looped" variant for calling single input MAGMA
  function on batched input.
...
*/
template <typename scalar_t>
static void apply_lu_looped_magma(
    const Tensor& input,
    const Tensor& pivots,
    const Tensor& infos,
    bool compute_pivots) {
  ...
  Tensor infos_cpu =
      at::empty_like(infos, infos.options().device(kCPU).pinned_memory(true));
  ...
  if (compute_pivots) {
    Tensor pivots_cpu = at::empty_like(
        pivots, pivots.options().device(kCPU).pinned_memory(true));
    ...
    for (decltype(batch_size) i = 0; i < batch_size; i++) {
      scalar_t* input_working_ptr = &input_data[i * input_matrix_stride];
      int* infos_working_ptr = &infos_data[i];
      magmaLuNoPiv<scalar_t>(
          m, n, input_working_ptr, leading_dimension, infos_working_ptr);
    }

    // fill the pivots tensor with indices using 1-based (Fortran) indexing
    auto k = std::min(m, n);
    Tensor pivots_tmp =
        at::arange(1, k + 1, input.options().dtype(at::kInt)).expand_as(pivots);
...
/*
  Computes the LU decomposition of a m×n matrix or batch of matrices in 'input'
  tensor. This is an in-place routine, content of 'input', 'pivots', and 'infos'
  is overwritten. This is a specialized batched variant, it is expected to be
  faster than the "looped" version only for small inputs.
  ...
  * `compute_pivots` - controls whether LU is computed with or without pivoting
  ...
  For further details, please see the MAGMA documentation for
  magma_dgetrf_batched.
*/
template <typename scalar_t>
static void apply_lu_batched_magma(
    const Tensor& input,
    const Tensor& pivots,
    const Tensor& infos,
    bool compute_pivots) {
...
static void lu_magma(
    const Tensor& input,
    const Tensor& pivots,
    const Tensor& infos,
    bool compute_pivots) {
  // TODO: compare performance and use the best performing option based on
  // input's sizes
  if (input.dim() == 2) {
    AT_DISPATCH_FLOATING_AND_COMPLEX_TYPES(
        input.scalar_type(), "lu_magma", [&] {
          apply_lu_looped_magma<scalar_t>(input, pivots, infos, compute_pivots);
        });
  } else {
    AT_DISPATCH_FLOATING_AND_COMPLEX_TYPES(
        input.scalar_type(), "lu_magma", [&] {
          apply_lu_batched_magma<scalar_t>(
              input, pivots, infos, compute_pivots);
        });
  }
}
...
static void apply_triangular_solve_batched(
...
#ifndef USE_MAGMA
  AT_ERROR(
      "triangular_solve: MAGMA library not found in "
      "compilation. Please rebuild with MAGMA.");
#else
  magma_uplo_t uplo = upper ? MagmaUpper : MagmaLower;
  ...
  int64_t mini_idx; // this is outside the loop because it is used for the case
                    // batch_size % batch_limit != 0
  for (mini_idx = 0; mini_idx < mini_batches * batch_limit;
       mini_idx += batch_limit) {
    scalar_t** A_array_cur = &A_array[mini_idx];
    scalar_t** b_array_cur = &b_array[mini_idx];
...
void triangular_solve_batched_magma(
    Tensor& A,
    Tensor& B,
    Tensor& infos,
    bool upper,
  ...
  (void)infos; // unused
  AT_DISPATCH_FLOATING_AND_COMPLEX_TYPES(
      A.scalar_type(), "triangular_solve_cuda", [&] {
        apply_triangular_solve_batched<scalar_t>(
            A, B, upper, transpose, conjugate_transpose, unitriangular);
      });
}
...
void triangular_solve_kernel(
  ...
  if (batchCount(A) <= 8 && A.size(-1) >= 64) {
    triangular_solve_cublas(
        A, B, infos, upper, transpose, conjugate_transpose, unitriangular);
  } else {
#ifndef USE_MAGMA
    triangular_solve_batched_cublas(
        A, B, infos, upper, transpose, conjugate_transpose, unitriangular);
#else
    // cuBLAS batched is faster than MAGMA batched up until 512x512, after that
    // MAGMA is faster
    if (A.size(-1) <= 512) {
      triangular_solve_batched_cublas(
          A, B, infos, upper, transpose, conjugate_transpose, unitriangular);
    } else {
      triangular_solve_batched_magma(
          A, B, infos, upper, transpose, conjugate_transpose, unitriangular);
    }
#endif // USE_MAGMA
  }
...
// TODO: It is possible to implement efficient batched orgqr for small tau
// (tau.size(-1) <= 32) using MAGMA, however it fails on Windows because of some
// illegal memory reads inside MAGMA. See discussions in
// https://github.com/pytorch/pytorch/pull/51348 for comparison of
// cuSOLVER-MAGMA and Windows failure. For reference here is the MAGMA-based
// implementation:
// https://gist.github.com/IvanYashchuk/2db50002c9d3c1462ff769e6410ad983
#if defined(USE_CUSOLVER)
  return orgqr_helper_cusolver(result, tau); // cusolver
#else
  TORCH_CHECK(
      false,
      "Calling torch.orgqr on a CUDA tensor requires compiling ",
...
static void apply_geqrf(const Tensor& input, const Tensor& tau) {
  ...
  // magmaGeqrf uses a hybrid CPU-GPU algorithm to compute the elementary
  // reflectors. The driver routine geqrf2_gpu accepts a tensor on the CPU for
  // elementary reflectors.
  Tensor tau_cpu = at::empty(
      tau.sizes(), tau.options().device(at::kCPU).pinned_memory(true));
  ...
  for (int64_t i = 0; i < batch_size; i++) {
    ...
    // MAGMA's geqrf2_gpu function is used, this version has LAPACK-complaint
    // arguments.
    magmaGeqrf<scalar_t>(
        m,
        n,
...
// This is a backend library dispatching helper function for calling looped
...
// This is a backend library dispatching helper function for calling specialized
...
void geqrf_kernel(const Tensor& input, const Tensor& tau) {
  // if number of rows is smaller than 32 batched is always faster for batch
  // size > 1 for larger number of rows number of batches condition
  if (input.size(-2) <= 256 &&
      batchCount(input) >= std::max<int64_t>(2, input.size(-2) / 16)) {
...
template <typename scalar_t>
static void apply_qr(
    Tensor& Q,
    Tensor& R,
    int64_t q_size_minus_2,
    int64_t r_size_minus_1,
    int64_t n_columns,
    bool compute_q) {
#ifndef USE_MAGMA
  AT_ERROR(
      "qr: MAGMA library not found in "
      "compilation. Please rebuild with MAGMA.");
#else
  ...
  // magmaGeqrf uses a hybrid CPU-GPU algorithm to compute the elementary
  // reflectors. The driver routine magma_(d/s)geqrf2_gpu accepts a tensor on
  // the CPU for elementary reflectors.
  ...
  for (int64_t i = 0; i < batch_size; i++) {
    ...
    magmaGeqrf<scalar_t>(
        m, n, r_working_ptr, m, tau_data, work_data, &info, /*is_v2=*/true);
    checkMagmaInternalError(info, "geqrf");
  }
  if (!compute_q) {
    // this is for mode='r'
  ...
  // - ?geqrf_gpu allows fast computation of Q via ?orgqr_gpu, but doesn't give
  // R properly.
  // - ?geqrf2_gpu gives correct R, but doesn't allow computation of Q via
  // ?orgqr_gpu Refer to the below link for more details:
  ...
  for (int64_t i = 0; i < batch_size; i++) {
    ...
    magmaGeqrf<scalar_t>(
        m, n, q_working_ptr, m, tau_data, work_data, &info, /*is_v2=*/false);
    checkMagmaInternalError(info, "geqrf");

    magmaOrgqr<scalar_t>(
        m, n_columns, k, q_working_ptr, m, tau_data, work_data, nb, &info);
    checkMagmaInternalError(info, "orgqr");
  }
#endif
}

std::tuple<Tensor, Tensor> linalg_qr_helper_magma(
    const Tensor& self,
    std::string mode) {
  ...
  std::tie(q_sizes, q_strides, n_columns_q) =
      _compute_geometry_for_Q(self, reduced);
  Tensor q_working_copy, r_working_copy;

  // If there are no elements, then we simply return a pair of tensors of
  // required dimensions
  if (self.numel() == 0) {
  ...
  AT_DISPATCH_FLOATING_AND_COMPLEX_TYPES(self.scalar_type(), "qr_cuda", [&] {
    apply_qr<scalar_t>(
        q_working_copy, r_working_copy, m, n, n_columns_q, compute_q);
  });
...
std::tuple<Tensor, Tensor> _linalg_qr_helper_cuda(
    const Tensor& input,
    std::string mode) {
#if defined(USE_CUSOLVER)
  // _linalg_qr_helper_default is a generic function that is implemented using
  // geqrf_stub and orgqr_stub. It dispatches to cuSOLVER for CUDA inputs if
  // USE_CUSOLVER is defined
...
static void apply_magma_eigh(
  ...
  scalar_t* work;
  magma_int_t* iwork;
  lwork = magma_int_cast(
      std::max<int64_t>(1, real_impl<scalar_t, value_t>(wkopt)), "work_size");
...
std::tuple<Tensor, Tensor> _symeig_helper_cuda(
    const Tensor& self,
    bool eigenvectors,
    bool upper) {
  Tensor infos = at::zeros(
      {std::max<int64_t>(1, batchCount(self))},
      self.options().dtype(kInt).device(at::kCPU));

  auto eigvals_shape =
      IntArrayRef(self.sizes().data(), self.dim() - 1); // self.shape[:-1]
  ScalarType real_dtype = toValueType(self.scalar_type());

  // magmaSyevd uses a hybrid CPU-GPU algorithm to compute the eigenvalues and
  // eigenvectors. The driver routine magma_(d/s)syev_gpu accepts a tensor on
  // the CPU for eigvalenvalues. The data is later moved to the appropriate
  // device. In the case where self.numel() == 0, we just return an empty tensor
  // of dimensions on the CUDA (to avoid the unnecessary "to(at::kCUDA)")
  auto eigvals_working_copy = self.numel() == 0
      ? at::empty(eigvals_shape, self.options().dtype(real_dtype))
      : at::empty(
            eigvals_shape, self.options().dtype(real_dtype).device(at::kCPU));
  ...
  if (eigenvectors) {
    return std::tuple<Tensor, Tensor>(
        eigvals_working_copy.to(self.device()), self_working_copy);
  } else {
    return std::tuple<Tensor, Tensor>(
        eigvals_working_copy.to(self.device()), at::empty({0}, self.options()));
  }
}
...
void linalg_eigh_magma(
  ...
  // See
  // https://bitbucket.org/icl/magma/src/e6fdca447bd402693e8b0b950a898b6879bbcc41/src/zheevd_gpu.cpp?at=master#lines-258
...
static void apply_eig(
  ...
  TORCH_CHECK(
      false,
      "Calling torch.eig on a CUDA tensor requires compiling PyTorch with MAGMA. "
      "Either transfer the tensor to the CPU before calling torch.eig or recompile with MAGMA.");
#else
  TORCH_INTERNAL_ASSERT(
      self.device() == at::kCPU,
      "Internal error: apply_eig needs a CPU tensor");
  using value_t = typename c10::scalar_value_type<scalar_t>::type;
  ...
  if (n > 0) {
    ...
    magmaEig<scalar_t, value_t>(
        ...
        &info);
    magma_int_t lwork =
        static_cast<magma_int_t>(real_impl<scalar_t, value_t>(wkopt));
 ...
 *   2. return CPU tensors (because this is what magmaEig returns), which will
 * be copied to GPU memory by the caller
 */
std::tuple<Tensor, Tensor> eig_kernel_impl(
  ...
  // tensors holding the results. We use empty_strided to make them
  // column-ordered
  auto options = self.options().device(at::kCPU).memory_format(
  ...
  AT_DISPATCH_FLOATING_AND_COMPLEX_TYPES(self.scalar_type(), "eig_cuda", [&] {
    apply_eig<scalar_t>(
        self_working_copy, eigenvectors, out_eigvals, out_eigvecs, &info);
  });
...
overwritten. 'infos' is an int Tensor containing error codes for each matrix in
the batched input. For more information see MAGMA's documentation for GEEV
routine.
*/
template <typename scalar_t>
void apply_linalg_eig(
  ...
  TORCH_CHECK(
      false,
      "Calling torch.linalg.eig on a CUDA tensor requires compiling PyTorch with MAGMA. "
  ...
  magma_int_t lwork = std::max<magma_int_t>(
      1, static_cast<magma_int_t>(real_impl<scalar_t, value_t>(work_query)));
...
void linalg_eig_kernel(
  ...
  // apply_linalg_eig modifies the provided input matrix in-place, therefore we
  // need a copy MAGMA doesn't have GPU interface for the eigendecomposition and
  // it forces us to transfer 'input' to CPU
  TORCH_INTERNAL_ASSERT_DEBUG_ONLY(input.is_cuda());
  Tensor input_working_copy =
      at::empty(input.sizes(), input.options().device(kCPU));
  ...
  AT_DISPATCH_FLOATING_AND_COMPLEX_TYPES(
      input.scalar_type(), "linalg_eig_out_cuda", [&] {
...
static void apply_svd(
...
#ifndef USE_MAGMA
  AT_ERROR(
      "svd: MAGMA library not found in "
      "compilation. Please rebuild with MAGMA.");
#else
  using value_t = typename c10::scalar_value_type<scalar_t>::type;
  auto self_data = self.data_ptr<scalar_t>();
  ...
  magma_vec_t jobz = jobchar == 'A'
      ? MagmaAllVec
      : (jobchar == 'S' ? MagmaSomeVec : MagmaNoVec);
  ...
  // deallocate workspace using at::empty()
  magma_int_t lwork = -1;
  scalar_t wkopt = 1; // MAGMA might not set the value for the optimal workspace
                      // therefore use 1 as the default value
  magmaSvd<scalar_t, value_t>(
      jobz,
      m,
      n,
      self_data,
...
std::tuple<Tensor, Tensor, Tensor> _svd_helper_cuda_legacy(
    const Tensor& self,
    bool some,
    bool compute_uv) {
  ...
  S_working_copy = same_stride_to(
      S_working_copy, S_working_copy.options().device(self.device()));
...
std::tuple<Tensor, Tensor, Tensor> _svd_helper_cuda(
    const Tensor& self,
    bool some,
    bool compute_uv) {
#ifdef USE_CUSOLVER
...
/*
  ...
  This is a "looped" variant for calling single input MAGMA function on batched
  input.
...
*/
template <typename scalar_t>
static void apply_lu_solve_looped_magma(
    const Tensor& b,
    const Tensor& lu,
    const Tensor& pivots) {
#ifndef USE_MAGMA
...
/*
  ...
  X and B are n-by-nrhs matrices, A is represented using the LU factorization.
  This is an in-place routine, content of `b` is overwritten.
  This is a specialized batched variant, it is expected to be faster than the
  "looped" version only for small inputs.
...
*/
template <typename scalar_t>
static void apply_lu_solve_batched_magma(
    const Tensor& b,
    const Tensor& lu,
    const Tensor& pivots) {
#ifndef USE_MAGMA
...
static void lu_solve_magma(
    const Tensor& b,
    const Tensor& lu,
    const Tensor& pivots) {
  // TODO: compare performance and use the best performing option based on lu's
  // sizes
  if (b.dim() == 2) {
    AT_DISPATCH_FLOATING_AND_COMPLEX_TYPES(
        b.scalar_type(), "lu_solve_magma", [&] {
...
static void apply_gels(const Tensor& a, Tensor& b, Tensor& infos) {
  ...
  TORCH_CHECK(
      false,
      "torch.linalg.lstsq: MAGMA library not found in "
      "compilation. Please rebuild with MAGMA.");
#else
  ...
  batch_iterator_with_broadcasting<scalar_t>(
      a,
      b,
...
void linalg_lstsq_gels(const Tensor& A, const Tensor& B, const Tensor& infos) {
  ...
  // TODO: revisit this later to use batch_iterator_with_broadcasting in
  // triangular_solve
  ...
  if (m >= n) {
    ...
    // explicitly broadcast the batch dimensions of A
    // we do it after geqrf so that we don't do redundant computations for the
    // same input
    ...
    Tensor A_broadcasted = is_fortran_contiguous
        ? A_expanded
    ...
    // Step 2: B <- Q^H B
    ormqr_kernel(
        A_broadcasted, tau_broadcasted, B, /*left=*/true, /*transpose=*/true);
    ...
    Tensor Ah_broadcasted = is_fortran_contiguous
        ? Ah_expanded
    ...
    // triangular_solve_kernel writes its output into the first m rows of B
    // leaving the rest untouched we need to set the rest of the rows to zero so
    // that the multiplication from step 3 is correct
    ...
    // Step 3: X <- Q Z
    ormqr_kernel(
        Ah_broadcasted, tau_broadcasted, B, /*left=*/true, /*transpose=*/false);
  }
}
...
void lstsq_kernel(
  ...
  if (m < n) {
    ...
    // the batched vs looped dispatch is implemented based on the following
    // performance results
...
// if CUDART_VERSION is defined then cuBLAS is available
      ...
      // if MAGMA is called a runtime error is thrown about not finding MAGMA in
      // compilation
