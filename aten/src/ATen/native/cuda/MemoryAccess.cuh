...
struct multi_outputs_store_helper {
  ...
  C10_HOST_DEVICE static void apply(
    ...
    using T =
        typename thrust::tuple_element<current, thrust::tuple<Args...>>::type;
...
struct multi_outputs_unroll {
  // multi_outputs_unroll struct members and check_inbounds and load methods are
  // copypasted from unroll struct we don't use inheritance because of compiler
  // bug in cuda 10.2+
  data_t data;
  int remaining;
  inp_calc_t input_offset_calculator;
  ...
  __device__ multi_outputs_unroll(
      data_t data,
      int remaining,
      inp_calc_t ic,
      out_calc_t oc)
...
#pragma unroll
    for (int i = 0; i < thread_work_size; i++) {
      ...
      memory::detail::static_unroll<
          detail::multi_outputs_store_helper,
          num_outputs>::with_args(this->data, offsets, from[i]);
      thread_idx += num_threads;
    }
  }
};
