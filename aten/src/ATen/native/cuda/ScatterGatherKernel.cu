...
struct _cuda_scatter_gather_internal_kernel {
  ...
  void operator() (
    ...
    auto loop = [=]C10_DEVICE(int i) {
      ...
      if (is_scatter_like) {
        src_offset = 0;
        auto original_src_offset = offsets[1];
        ...
        for (auto d = ndim - 1; d >= 0; d--) {
          src_idx = (original_src_offset / src_strides[d]) % src_sizes[d];
          original_src_offset -= src_idx * src_strides[d];

          index_idx = (original_index_offset / index_strides[d]) % index_sizes[d];
          original_index_offset -= index_idx * index_strides[d];

          src_offset += (src_idx % index_idx) * src_strides[d];
        }
      }
      else {
