...
namespace {
Tensor& binary_cross_entropy_with_logits_out_cuda(
    const Tensor& input,
    const Tensor& target,
...
// See [Note: hacky wrapper removal for optional tensor]
  ...
  if (pos_weight_opt.has_value()) {
    auto iter = TensorIteratorConfig()
      ...
      .build();
    AT_DISPATCH_FLOATING_TYPES_AND2(at::ScalarType::Half, at::ScalarType::BFloat16, loss.scalar_type(), "binary_cross_entropy_with_logits_out_cuda", [&] {
      gpu_kernel(
          iter,
          [] GPU_LAMBDA (scalar_t input_val, scalar_t target_val, scalar_t pos_weight_val) -> scalar_t {
    ...
    auto iter = TensorIteratorConfig()
      ...
      .add_owned_input(at::squeeze(target))
      .build();
    AT_DISPATCH_FLOATING_TYPES_AND2(at::ScalarType::Half, at::ScalarType::BFloat16, loss.scalar_type(), "binary_cross_entropy_with_logits", [&] {
      gpu_kernel(
          iter,
          [] GPU_LAMBDA (scalar_t input_val, scalar_t target_val) -> scalar_t {
            auto max_val = std::max(-input_val, static_cast<scalar_t>(0));
...
Tensor binary_cross_entropy_with_logits_cuda(
    const Tensor& input,
    const Tensor& target,
    const c10::optional<Tensor>& weight_opt,
    const c10::optional<Tensor>& pos_weight_opt,
    int64_t reduction) {
  Tensor loss = at::empty_like(input);
  return binary_cross_entropy_with_logits_out_cuda(
