...
void binary_cross_entropy_backward_out_kernel(
    Tensor& grad_input,
    const Tensor& grad,
    const Tensor& input,
    const Tensor& target) {
  ...
  AT_DISPATCH_FLOATING_TYPES_AND2(
      ...
      iter.common_dtype(),
      "binary_cross_entropy_backward_out_cuda",
      [&]() {
...
Tensor kl_div_backward_cuda(
  ...
  if (!log_target) {
    ...
    AT_DISPATCH_FLOATING_TYPES_AND_HALF(
        input.scalar_type(), "kl_div_backward_cuda", [&]() {
          scalar_t inv = (reduction == at::Reduction::Mean)
              ? scalar_t(1.0 / input.numel())
              : scalar_t(1.0);
          gpu_kernel(
              iter, [inv] GPU_LAMBDA(scalar_t target_val, scalar_t grad_val) {
...
Tensor binary_cross_entropy_cuda(
    const Tensor& input,
    const Tensor& target,
...
Tensor& binary_cross_entropy_out_cuda(
    const Tensor& input,
    const Tensor& target,
    const c10::optional<Tensor>& weight_opt,
    int64_t reduction,
...
Tensor binary_cross_entropy_backward_cuda(
    const Tensor& grad,
    const Tensor& input,
    const Tensor& target,
    const c10::optional<Tensor>& weight_opt,
...
Tensor& binary_cross_entropy_backward_out_cuda(
    const Tensor& grad,
    const Tensor& input,
    const Tensor& target,
    const c10::optional<Tensor>& weight_opt,
  ...
  binary_cross_entropy_backward_out_kernel(
