...
#include <ATen/native/special/hankel_h_1.h>
...
void hankel_h_1_kernel_cuda(TensorIteratorBase& iterator) {
#if AT_USE_JITERATOR()
  AT_DISPATCH_FLOATING_TYPES(iterator.common_dtype(), "hankel_h_1_cuda", [&]() {
  ...
  AT_DISPATCH_FLOATING_TYPES(iterator.common_dtype(), "hankel_h_1_cuda", [&]() {
    gpu_kernel_with_scalars(iterator, []GPU_LAMBDA(scalar_t v, scalar_t z) -> scalar_t {
      return at::native::special::hankel_h_1(v, z);
    });
  });
#endif
} // void hankel_h_1_kernel_cuda(TensorIteratorBase& iterator)
...
REGISTER_DISPATCH(hankel_h_1_stub, &hankel_h_1_kernel_cuda);
} // namespace native
} // namespace at
