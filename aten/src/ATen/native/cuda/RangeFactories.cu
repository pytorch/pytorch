...
__global__ void elementwise_kernel_with_index(
    index_t N,
    func_t f,
...
void gpu_kernel_with_index(at::Tensor& output, func_t f) {
  ...
  if (N <= std::numeric_limits<int>::max()) {
    elementwise_kernel_with_index<int>
        <<<grid, num_threads, 0, stream>>>(N, f, output.data_ptr<scalar_t>());
    C10_CUDA_KERNEL_LAUNCH_CHECK();
  } else {
    elementwise_kernel_with_index<int64_t>
        <<<grid, num_threads, 0, stream>>>(N, f, output.data_ptr<scalar_t>());
    C10_CUDA_KERNEL_LAUNCH_CHECK();
  }
}
  ...
  if (steps == 0) {
    ...
    AT_DISPATCH_INTEGRAL_TYPES(r.scalar_type(), "linspace_cuda", [&]() {
      ...
      gpu_kernel_with_index(
          r,
          [scalar_start, scalar_end, steps, step, halfway] GPU_LAMBDA(
    ...
    AT_DISPATCH_FLOATING_AND_COMPLEX_TYPES_AND2(
        kHalf, kBFloat16, r.scalar_type(), "linspace_cuda", [&]() {
          scalar_t scalar_start = start.to<scalar_t>();
          scalar_t scalar_end = end.to<scalar_t>();
          scalar_t step =
...
Tensor& logspace_cuda_out(
  ...
  if (steps == 0) {
    ...
    AT_DISPATCH_INTEGRAL_TYPES(r.scalar_type(), "logspace_cuda", [&]() {
      ...
      gpu_kernel_with_index(
          ...
          [scalar_start,
           ...
           steps,
           step,
           halfway] GPU_LAMBDA(int64_t ind) -> scalar_t {
            if (ind < halfway) {
    ...
    AT_DISPATCH_FLOATING_AND_COMPLEX_TYPES_AND2(
        kHalf, kBFloat16, r.scalar_type(), "logspace_cuda", [&]() {
...
Tensor& range_cuda_out(
    const Scalar& start,
    const Scalar& end,
    const Scalar& step,
    Tensor& result) {
  AT_DISPATCH_ALL_TYPES_AND(
      at::ScalarType::Half, result.scalar_type(), "range_cuda", [&]() {
        ...
        gpu_kernel_with_index(
            r, [xstart, xstep] GPU_LAMBDA(int64_t ind) -> scalar_t {
