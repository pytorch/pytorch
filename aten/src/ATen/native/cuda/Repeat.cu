...
__global__ static void compute_cuda_kernel(
  ...
  CUDA_KERNEL_ASSERT_PRINTF(
    ...
    "block: [%d,%d,%d], thread: [%d,%d,%d] Invalid input! In `repeat_interleave`, the `output_size` argument (%ld) must be the same as the sum of the elements in the `repeats` tensor (%ld).\n",
    blockIdx.x,
    blockIdx.y,
    blockIdx.z,
    threadIdx.x,
