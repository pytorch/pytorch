...
// We could make it dependent on dtype, but that would lead to different
// results between float and reduced precision types
constexpr int vec_size = 4;

// aligned_vector generates vectorized load/store on CUDA;
// (copy-pasted from MemoryAccess.cuh)
...
template <typename U>
__device__ WelfordDataLN
cuWelfordOnlineSum(const U val, const WelfordDataLN& curr_sum) {
...
__device__ WelfordDataLN
cuWelfordCombine(const WelfordDataLN dataB, const WelfordDataLN dataA) {
...
template <typename T>
__device__ WelfordDataLN
compute_stats(const T* __restrict__ X, const int N, float* buf) {
...
// This implementation gets called when input buffers (dY, X, gamma and dX) are
// aligned to vec_size * sizeof(T). Compared to the unvectorized implementation,
// it is about 10% faster measuread at PT operator level, with cases seeing a 2X
// speedup (where N >> M). There are no noticeable regressions on the rest of
// the sizes.
...
__global__ void GammaBetaBackwardSimpleCUDAKernel(
  ...
  if (j < N) {
    ...
    for (int64_t i = 0; i < M; ++i) {
      const int64_t index = i * N + j;
      sum1 += (dg == nullptr ? T_ACC(0)) : static_cast<T_ACC>(dY[index]) *
...
void launch_vectorized_layer_norm_kernel(
    ...
    T_ACC* mean_data,
    T_ACC* rstd_data) {
  // constexpr int alignment = 16; //currently unused to make sure float and
  // half results are bw accurate
  ...
  vectorized_layer_norm_kernel<<<blocks, threads, nshared, stream>>>(
...
void LayerNormKernelImplInternal(
    ...
    Tensor* mean,
    Tensor* rstd) {
  // assumes input, gamma and beta are of proper shape, this was checked in
  // _check_layer_norm_inputs assumes all tensors are contiguous
  TORCH_CHECK(
      M <= at::cuda::getCurrentDeviceProperties()->maxGridSize[0],
      "M should be less than maximum CUDA grid size, \
  ...
  // check if can take fast path - all tensors are properly aligned, N is less
  // than 2^24 (to use float count), N is multiple of vec_size (so that all rows
  // are aligned if tensor is aligned)
  ...
  if ((std::is_same<T, float>::value || std::is_same<T, at::Half>::value ||
    ...
    LayerNormForwardCUDAKernel<T, T_ACC>
        <<<M, kCUDANumThreads, 0, cuda_stream>>>(
  ...
  if (i1 < i1_end) {
    ...
    for (int k = 0; k < blockDim.y; ++k) {
      ...
      if (i2 < N) {
        ...
        warp_buf1[write_idx] += curr_dout;
        warp_buf2[write_idx] +=
            curr_dout * (curr_input - curr_mean) * curr_rstd;
      }
    }
  }
}
...
__global__ void cuComputePartGradGammaBeta(
  ...
  const int numsegs_M =
      (M + blockDim.y * blockDim.y - 1) / (blockDim.y * blockDim.y);
  ...
  T_ACC* buf = reinterpret_cast<T_ACC*>(
      &shared); // buf has at least blockDim.x * blockDim.y * blockDim.y +
...
__global__ void cuComputeGradGammaBeta(
  ...
  const T_ACC* part_grad_gamma_ptr =
      part_grad_gamma + threadIdx.y * num_warp_reductions * N + i2;
  const T_ACC* part_grad_beta_ptr =
      part_grad_beta + threadIdx.y * num_warp_reductions * N + i2;
  ...
  for (int i1 = blockIdx.y; i1 < M; i1 += gridDim.y) {
    ...
    if (gamma != NULL) {
      ...
      for (int l = 0; l < N; l += numx) {
        int idx = l + thrx;
        const T_ACC gamma_idx =
            static_cast<T_ACC>((idx < N) ? gamma[idx] : T(0));
...
void LayerNormBackwardKernelImplInternal(
  ...
  TORCH_CHECK(
      ...
      "Normalized shape should have less than INT_MAX elements, \
  ...
  if (dX_data != nullptr) {
    ...
    if (M >= 32768) {
      ...
      int nshared =
          threads1.y > 1 ? threads1.y * threads1.x * sizeof(T_ACC) : 0;
      cuComputeGradInput<<<blocks1, threads1, nshared, cuda_stream>>>(
          dY_data, X_data, M, N, mean_data, rstd_data, gamma_data, dX_data);
      C10_CUDA_KERNEL_LAUNCH_CHECK();
    } else {
      const dim3 blocks(M);
  ...
  if (dgamma->defined() || dbeta->defined()) {
    ...
    if (M < 128) {
      ...
      const int nshared2_a =
          2 * sizeof(T_ACC) * threads2.y * threads2.y * (threads2.x + 1);
      ...
      Tensor part_grad_gamma =
          at::empty({part_size, N}, gamma.options().dtype(part_grad_dtype));
      ...
      if ((M % kWarpSize == 0) && (N % kWarpSize == 0)) {
        // This implementation relies on warp primitives and requires that M and
        // N divide exactly to warp size.
        dim3 threads{kWarpSize, kWarpSize};
        int blocks = (N + threads.x - 1) / threads.x;

        // If M and N divide by warp_size, we can use warp shuffles for the
        // final reduction. That requires transposing values in shared memory,
        // so we apply a padding to reduce bank conflicts.
