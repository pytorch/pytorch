...
__global__ void GammaBetaBackwardCUDAKernelTemplate(
  ...
  if (partial_reduction || (blockDim.y == 1 && gridDim.y == 1)) {
    ...
    // Do a block-wise reduction here.
...
bool partial_reduction>
void LaunchAndCheckLayerNormBackwardKernel(
  bool aligned_grid,
  dim3 blocks,
  dim3 threads,
  size_t shmem_sz,
...
int block_dim_x, int block_dim_y,
int rows_per_block_y>
void ConfigureAndLaunchLayerNormBackwardKernel(
    const T* dY_data,
    const T* X_data,
    const T_ACC* mean_data,
    const T_ACC* rstd_data,
  ...
  LaunchAndCheckLayerNormBackwardKernel<T, T_ACC, block_dim_x, block_dim_y, rows_per_block_y, false>(
...
void LaunchLayerNormBackwardKernel(
  ...
  if (M < 128) {
    ...
    LaunchAndCheckLayerNormBackwardKernel<T, T_ACC, block_dim_x, block_dim_y, rows_per_block_y, true>(
    ...
    *dbeta = dbeta_blocks.sum(0);
    ...
    // For all other cases we use a bigger tile size so each thread does more work.
    // That amortizes the cost of launching a block.
    ConfigureAndLaunchLayerNormBackwardKernel<T, T_ACC, 32, 32, 256>(dY_data, X_data, mean_data, rstd_data, M, N, dgamma, dbeta, cuda_stream);
  }
}
...
#else
    LaunchLayerNormBackwardKernel(dY_data, X_data, mean_data, rstd_data, M, N, dgamma, dbeta, cuda_stream);
#endif
  }
}
