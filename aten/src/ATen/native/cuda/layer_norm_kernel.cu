...
void rmsnorm_cuda_forward(
  ...
  AT_DISPATCH_FLOATING_TYPES_AND2(
    ...
    [&]() {
      ...
      auto* weight_ptr = weight_contiguous->data_ptr<scalar_t>();
...
std::tuple<Tensor, Tensor, Tensor> rms_norm_cuda(
  ...
  at::Tensor x_norm = at::empty_like(input);
  int64_t norm_numel = 1;
  for (auto dim : normalized_shape) {
    norm_numel *= dim;
  }
  int64_t M = input.numel() / norm_numel;
  // Create a tensor with size M with options like input.
  at::Tensor inverse_rms = at::empty({M}, input.options());
  rmsnorm_cuda_forward(input, output, x_norm, inverse_rms, weight_opt.value_or(at::Tensor()), norm_numel, eps.value_or(1e-6));
...
void rmsnorm_backward_input_kernel(
    const scalar_t*  grad,         // [M, N]
    const scalar_t* __restrict__ x_norm,         // [M, N]
    const scalar_t* __restrict__ inverse_rms,       // [M]
    scalar_t* __restrict__ grad_input,           // [M, N]
    int64_t N,
    int64_t M,
    const scalar_t* __restrict__ weight) {
  ...
  for (int sample = blockIdx.x; sample < M; sample += gridDim.x) {
    ...
    for (int j = threadIdx.x; j < N; j += blockDim.x) {
      ...
      acc_t g_val = weight != nullptr ? static_cast<acc_t>(grad_row[j] * weight[j]) : static_cast<acc_t>(grad_row[j]);
...
// Computes: grad_weight[j] = sum_{sample=0}^{M-1} grad[sample, j] * x_norm[sample, j]
template <typename scalar_t, typename acc_t>
__global__
void rmsnorm_backward_weight_kernel(
    const scalar_t* __restrict__ grad,   // [M, N]
    const scalar_t* __restrict__ x_norm,   // [M, N]
    scalar_t* __restrict__ grad_weight,    // [N]
    int64_t N,
    int64_t M) {
...
// grad_input_mask[0] indicates whether to compute grad with respect to input,
    ...
    if (grad_input_mask[0]) {
      ...
      AT_DISPATCH_FLOATING_TYPES_AND2(
        ...
        <<<blocksPerGrid, threadsPerBlock, sharedMemSize>>>(
          ...
          N,
          M,
          weight_opt.has_value() ? weight_contiguous->data_ptr<scalar_t>() : nullptr
        );
        C10_CUDA_KERNEL_LAUNCH_CHECK();
