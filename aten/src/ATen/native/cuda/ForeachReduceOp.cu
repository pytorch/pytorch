...
std::vector<Tensor> foreach_tensor_max_cuda(TensorList tensors) {
  ...
  for (const auto i : c10::irange(ntensors)) {
    vec_res.push_back(at::native::empty_cuda({}, optTypeMetaToScalarType(options.dtype_opt()), options.layout_opt(), options.device_opt(), options.pinned_memory_opt(), options.memory_format_opt()));
  }
  ...
  for (const auto& t : tensors) {
    if (t.numel() != 0) {
      ...
      result.emplace_back(at::native::empty_cuda({}, optTypeMetaToScalarType(options.dtype_opt()), options.layout_opt(), options.device_opt(), options.pinned_memory_opt(), options.memory_format_opt()));
    }
  }
  return result;
}
...
std::vector<Tensor> foreach_tensor_norm_cuda(
  ...
  for (const auto i : c10::irange(ntensors)) {
    vec_res.push_back(at::native::empty_cuda({}, optTypeMetaToScalarType(res_option.dtype_opt()), res_option.layout_opt(), res_option.device_opt(), res_option.pinned_memory_opt(), res_option.memory_format_opt()));
  }
