...
void masked_scale_kernel(
  ...
  at::native::gpu_kernel(
      iter,
      [=] GPU_LAMBDA(const scalar_t src_val, const uint8_t mask_val)
...
inline void launcher(
  ...
  AT_DISPATCH_FLOATING_TYPES_AND2(
      ...
      [&] {
        ...
        auto ret_info = cuda::detail::getTensorInfo<scalar_t, index_type>(ret);
        auto mask_info = cuda::detail::getTensorInfo<uint8_t, index_type>(mask);
...
std::tuple<Tensor, Tensor> fused_dropout_cuda(
    const Tensor& self,
    double p,
  ...
  if (nelem == 0)
    return std::tuple<Tensor, Tensor>(self, mask);
