...
void topk_out_with_sort(
  ...
  std::tie(sorted_values, sorted_indices) =
      at::cuda::sort(self, /* stable= */ false, dim, largest);
...
bool should_use_sort(const Tensor& self, int64_t dim) {
  if (disable_sort_for_topk())
    return false;
  // This heuristics is based on the experiment in
  // https://github.com/pytorch/pytorch/pull/68632
  if (self.dim() == 0)
    return false;
  if (self.dtype() == kBool)
    return false; // Bool is not support by topk
...
(const Tensor& self,
 ...
 const Tensor& values,
 const Tensor& indices) {
  TensorArg topK_arg{values, "topK", 1}, indices_arg{indices, "indices", 2},
