...
__host__ __device__ scalar_t ratevl(
    scalar_t x,
    const scalar_t num[],
    int64_t M,
...
// NOTE: this __noinline__ is important -- otherwise, observed compile times
// significantly increase.  The same kernel seems to get recompiled mulitple
// times via gpu_kernel_with_scalars, multiple dtypes, etc.
template <typename scalar_t>
...
// CUDA compilation times grow quickly.  It's perfectly acceptable to have a
// file per kernel.
