...
namespace native {
namespace {
const auto bessel_y_1_string = bessel_j_1_string + jiterator_stringify(
    template<typename T>
    T bessel_y_1_forward(T x) {
        static const T PP[] = {
        ...
        if (x <= T(5.0)) {
            ...
            return x * (yp / yq) + (T(0.636619772367581343075535053490057448) * (bessel_j_1_forward(x) * log(x) - T(1.0) / x));
        }
    ...
    } // bessel_y_1_forward(T x)
); // bessel_y_1_string

const char bessel_y_1_name[] = "bessel_y_1_forward";

void bessel_y_1_kernel_cuda(TensorIteratorBase &iterator) {
#if AT_USE_JITERATOR()
  AT_DISPATCH_FLOATING_TYPES(iterator.common_dtype(), "bessel_y_1_cuda", [&]() {
  ...
  AT_DISPATCH_FLOATING_TYPES(iterator.common_dtype(), "bessel_y_1_cuda", [&]() {
    gpu_kernel(iterator, []GPU_LAMBDA(scalar_t a) -> scalar_t {
      return bessel_y_1_forward(a);
    });
  });
#endif // AT_USE_JITERATOR()
} // void bessel_y_1_kernel_cuda(TensorIteratorBase &iterator)
} // namespace (anonymous)
REGISTER_DISPATCH(special_bessel_y_1_stub, &bessel_y_1_kernel_cuda);
} // namespace native
} // namespace at
