...
namespace native {
namespace {
const auto exp_modified_bessel_k_0_string = modified_bessel_i_0_string + jiterator_stringify(
    template<typename T>
    T exp_modified_bessel_k_0_forward(T x) {
        static const T A[] = {
        ...
        if (x <= T(2.0)) {
            ...
            return (T(0.5) * (a - p) - log(T(0.5) * x) * modified_bessel_i_0_forward(x)) * exp(x);
        }
...
const char exp_modified_bessel_k_0_name[] = "exp_modified_bessel_k_0_forward";

void exp_modified_bessel_k_0_kernel_cuda(TensorIteratorBase &iterator) {
#if AT_USE_JITERATOR()
  AT_DISPATCH_FLOATING_TYPES(iterator.common_dtype(), "exp_modified_bessel_k_0_cuda", [&]() {
  ...
  AT_DISPATCH_FLOATING_TYPES(iterator.common_dtype(), "exp_modified_bessel_k_0_cuda", [&]() {
    gpu_kernel(iterator, []GPU_LAMBDA(scalar_t a) -> scalar_t {
      return exp_modified_bessel_k_0_forward(a);
    });
  });
#endif // AT_USE_JITERATOR()
} // void exp_modified_bessel_k_0_kernel_cuda(TensorIteratorBase &iterator)
} // namespace (anonymous)
REGISTER_DISPATCH(special_exp_modified_bessel_k_0_stub, &exp_modified_bessel_k_0_kernel_cuda);
} // namespace native
} // namespace at
