...
template <
    ...
    typename DtypeB,
    typename DtypeBias>
void f8f8bf16_rowwise_kernel_foo_sm9x(
    at::Tensor XQ, // FP8
    at::Tensor WQ, // FP8
    at::Tensor x_scale,
    at::Tensor w_scale,
...
template <
    ...
    typename DtypeB,
    typename DtypeBias>
void f8f8bf16_rowwise_kernel_foo_sm10x(
    at::Tensor XQ, // FP8
    at::Tensor WQ, // FP8
    at::Tensor x_scale,
    at::Tensor w_scale,
...
template <
    ...
    typename DtypeB,
    typename DtypeBias>
void f8f8bf16_rowwise_kernel_foo_sm89(
    at::Tensor XQ, // FP8
    at::Tensor WQ, // FP8
    at::Tensor x_scale,
    at::Tensor w_scale,
...
template <typename ArchTag, typename ClusterShape, typename... Types>
...
void dispatch_fp8_rowwise_kernel_sm89(
  ...
  if (M <= 16) {
    return f8f8bf16_rowwise_kernel_foo_sm89<
        ArchTag,
...
void dispatch_fp8_rowwise_kernel_on_sm(
    ...
    std::optional<at::Tensor> bias,
    at::Tensor out) {
  if constexpr (std::is_same_v<ArchTag, cutlass::arch::Sm89>) {
    dispatch_fp8_rowwise_kernel_sm89<ArchTag, Types... >(XQ, WQ, x_scale, w_scale, bias, out);
  } else {
...
void f8f8bf16_rowwise_impl(
  ...
  dispatch_fp8_rowwise_kernel_on_bias_dtype<ArchTag>(
      XQ, WQ, x_scale, w_scale, bias, use_fast_accum, out);
