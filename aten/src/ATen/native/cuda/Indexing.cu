...
__global__ void indexing_backward_kernel(
  ...
  // If the preceding input has the same destination index as this input, then
  // the warp exits immediately. The warp also processes subsequent inputs with
  // the same value.
  //
  // Input Warp
  // 1     <warp 1>
...
#pragma unroll
          for (int ii = 0; ii < SZ; ii++) {
            ...
            if (feature_dim < stride) {
              gradient[ii] =
                  static_cast<accscalar_t>(grad_output[grad_row + feature_dim]);
              weight[ii] = static_cast<accscalar_t>(
...
void index_put_accum_kernel(
  ...
  std::tie(
      ...
      makeLinearIndex(self, indices, !unsafe);
  int64_t num_indices = linearIndex.numel();

  TORCH_CHECK(
      num_indices <= std::numeric_limits<int>::max(),
  ...
  if (num_indices > 0 && sliceSize > 0) {
    ...
    if (num_indices < 50000) {
      index_put_accum_kernel_thrust_helper(
          linearIndex, orig_indices, sorted_indices, num_indices);
    } else
#endif
    {
    ...
    TORCH_INTERNAL_ASSERT(
        linearIndex.numel() * sliceSize * nElemBefore == value.numel(),
    ...
    dim3 grid(
        THCCeilDiv(num_indices, (int64_t)indices_per_block),
        std::min<int>(
            at::cuda::getCurrentDeviceProperties()->maxGridSize[1],
            THCCeilDiv(sliceSize, (int64_t)(C10_WARP_SIZE * UNROLL))),
        std::min(
            std::max<int>(1, nElemBefore),
    ...
    AT_DISPATCH_ALL_TYPES_AND_COMPLEX_AND3(
        ...
        "indexing_backward",
        [&] {
          indexing_backward_kernel<scalar_t, UNROLL>
              <<<grid, block, 0, stream>>>(
                  sorted_indices.data_ptr<int64_t>(),
                  orig_indices.data_ptr<int64_t>(),
...
__global__ void indexAddSmallIndex(
  ...
  for (IndexType srcIndex = 0; srcIndex < indices.sizes[0]; ++srcIndex) {
    ...
    IndexType dstIndex =
        indices.data
            [cuda::detail::IndexToOffset<IndicesType, IndexType, IdxDim>::get(
                srcIndex, indices)];
    ...
    for (IndexType linearIndex = blockIdx.x * blockDim.x + threadIdx.x;
      ...
      IndexType dstOffset =
          cuda::detail::IndexToOffset<T, IndexType, DstDim>::get(
              linearIndex, dst);
      dstOffset += dstIndex * dst.strides[dstAddDim];

      IndexType srcOffset =
          cuda::detail::IndexToOffset<T, IndexType, SrcDim>::get(
              linearIndex, src);
...
bool indexShouldBeMajor(
    cuda::detail::TensorInfo<scalar_t, unsigned int>& info,
    int sliceDim) {
...
Tensor& index_add_cuda_(
  ...
  TensorArg self_arg{self, "self", 1}, index_arg{index, "index", 3},
      source_arg{source, "source", 4};
  checkAllSameGPU("index_add", {self_arg, index_arg, source_arg});

  TORCH_CHECK_INDEX(
      index.dim() <= 1, "index_add_(): Index is supposed to be a vector");
  TORCH_CHECK(
  ...
  TORCH_CHECK(
      index.numel() == (source.dim() == 0 ? 1 : source.size(dim)),
...
#define LARGE_INDEX(                                                 \
  ...
  indexAddLargeIndex<                                                \
      ...
      SOURCE_DIM,                                                    \
      IDX_DIM,                                                       \
      IDX_IS_MAJOR><<<largeIndexGrid, largeIndexBlock, 0, stream>>>( \
      selfInfo,                                                      \
      sourceInfo,                                                    \
      indexInfo,                                                     \
      selfAddDim,                                                    \
  ...
  dim3 largeIndexGrid(std::min(
      THCCeilDiv(sourceTotalSize, (ptrdiff_t)128), (ptrdiff_t)(mpc * 8)));
        ...
        [&] {
          ...
          AT_DISPATCH_INDEX_TYPES(
              index.scalar_type(), "index_add_cuda_", [&]() {
                auto sourceInfo =
                    cuda::detail::getTensorInfo<scalar_t, unsigned int>(
                        source_);
                ...
                if (numIndex <= 16) {
                  ...
                  if (selfInfo.dims == 1 && sourceInfo.dims == 1 && indContig) {
                    LARGE_INDEX(
                        scalar_t, index_t, unsigned int, 1, 1, -2, true);
                  } else if (
                      selfInfo.dims == 2 && sourceInfo.dims == 2 && indContig) {
                    if (indexIsMajor) {
                      LARGE_INDEX(
                          scalar_t, index_t, unsigned int, 2, 2, -2, true);
                    } else {
                      LARGE_INDEX(
                          scalar_t, index_t, unsigned int, 2, 2, -2, false);
                    }
                  } else if (
                      selfInfo.dims == 3 && sourceInfo.dims == 3 && indContig) {
                    if (indexIsMajor) {
                      ...
                      LARGE_INDEX(
                          scalar_t, index_t, unsigned int, 3, 3, -2, false);
                    }
                  } else {
                    LARGE_INDEX(
                        scalar_t, index_t, unsigned int, -1, -1, -1, true);
                  }
                }
              });
        });
...
cuda::detail::TensorInfo<T, IndexType> tensorInfoLegacyIfScalar(
    cuda::detail::TensorInfo<T, IndexType> ti) {
  if (ti.dims == 0) {
    ti.dims = 1;
    ti.sizes[0] = 1;
    ti.strides[0] = 1;
...
#define LARGE_INDEX(                                                          \
    TENSOR_TYPE, INDICES_TYPE, TYPE, DST_DIM, SRC_DIM, IDX_DIM, IDX_IS_MAJOR) \
  ...
  dim3 largeIndexGrid(
      std::min(THCCeilDiv(outTotalSize, (ptrdiff_t)128), (ptrdiff_t)(mpc * 8)));
  dim3 largeIndexBlock(std::min(outTotalSize, (ptrdiff_t)128));
  if (cuda::detail::canUse32BitIndexMath(out) &&
      cuda::detail::canUse32BitIndexMath(self) &&
      cuda::detail::canUse32BitIndexMath(index)) {
    auto outInfo = tensorInfoLegacyIfScalar(
        cuda::detail::getTensorInfo<scalar_t, unsigned int>(out));
    ...
    auto outInfo = tensorInfoLegacyIfScalar(
        cuda::detail::getTensorInfo<scalar_t, uint64_t>(out));
    int outSelectDim = outInfo.collapseDims(dim);
    outInfo.reduceDim(outSelectDim);
