...
__global__ void indexing_backward_kernel_rocm(
  ...
  for (int64_t z = blockIdx.z; z < outer_dim; z += gridDim.z) {
    ...
    if (idx < numel) {
      ...
      if (idx == 0 || sorted_indices[idx] != sorted_indices[idx - 1]) {
        ...
        if (!accumulate) {
          ...
          while (feature_dim < stride) {
            grad_weight[weight_row + feature_dim] =
              static_cast<scalar_t>(static_cast<opmath_t>(grad_output[grad_row + feature_dim]));
        ...
        if (num_duplicates < C10_WARP_SIZE) {
          ...
          while (feature_dim < stride) {
            ...
            grad_weight[weight_row + feature_dim] = static_cast<scalar_t>(static_cast<opmath_t>(grad_weight[weight_row + feature_dim]) + gradient);
            feature_dim += gridDim.y * blockDim.x;
          }
        } else {
    ...
    for (int subwarp = 0; subwarp < C10_WARP_SIZE; subwarp++) {
      ...
      while (feature_dim < stride) {
        ...
        if (laneIdx == 0) {
          ...
          grad_weight[new_weight_row + feature_dim] = static_cast<scalar_t>(static_cast<opmath_t>(grad_weight[new_weight_row + feature_dim]) + gradient);
        }
