...
static std::tuple<Tensor, Tensor, int64_t, int64_t, int64_t, std::vector<int64_t>> makeLinearIndex(Tensor self, IOptTensorRefList orig, bool check_range) {
...
void index_put_with_sort_kernel(Tensor & self, IOptTensorRefList indices, const Tensor & value, bool accumulate, bool unsafe) {
