...
inline static void _apply_single_inverse_helper(
  ...
  at::cuda::solver::getrs<scalar_t>(
      handle, n, n, self_ptr, lda, ipiv_ptr, self_inv_ptr, lda, info_getrs_ptr);
}
...
Tensor _inverse_helper_cuda_lib(const Tensor& self) {
  ...
  Tensor self_inv_working_copy =
      column_major_identity_matrix_like(self_working_copy);
  const int batch_size = cuda_int_cast(batchCount(self), "batchCount");

  if (self.dim() > 2 && batch_size > 1) {
    Tensor infos_getrf = at::zeros(
        {std::max<int64_t>(1, batchCount(self))}, self.options().dtype(kInt));
    Tensor infos_getrs = at::zeros(
        {std::max<int64_t>(1, batchCount(self))}, self.options().dtype(kInt));
    AT_DISPATCH_FLOATING_AND_COMPLEX_TYPES(
...
template <typename scalar_t>
inline static void _apply_svd_lib_gesvdj(
    const Tensor& self,
    Tensor& U,
    Tensor& S,
    Tensor& VT,
  ...
  for (int i = 0; i < batchsize; i++) {
    // gesvdj_params controls the numerical accuracy of cusolver gesvdj
    // iterations on GPU
...
// creates a working copy of the input, and creates V^H from the V returned by
// gesvdj
inline static void apply_svd_lib_gesvdj(
    const Tensor& self,
    Tensor& U,
    Tensor& S,
    Tensor& VT,
  ...
  AT_DISPATCH_FLOATING_AND_COMPLEX_TYPES(
      self.scalar_type(), "svd_cuda_gesvdj", [&] {
        _apply_svd_lib_gesvdj<scalar_t>(
            self_working_copy, U, S, VT, infos, compute_uv, some);
      });
}

// call cusolver gesvdj batched function to calculate svd
template <typename scalar_t>
inline static void _apply_svd_lib_gesvdjBatched(
    const Tensor& self,
    Tensor& U,
    Tensor& S,
    Tensor& VT,
...
// creates a working copy of the input, and creates V^H from the V returned by
// gesvdj
inline static void apply_svd_lib_gesvdjBatched(
    const Tensor& self,
    Tensor& U,
    Tensor& S,
    Tensor& VT,
  ...
  AT_DISPATCH_FLOATING_AND_COMPLEX_TYPES(
      self.scalar_type(), "svd_cuda_gesvdjBatched", [&] {
        _apply_svd_lib_gesvdjBatched<scalar_t>(
            self_working_copy, U, S, VT, infos, compute_uv);
      });
}
...
//     A loop of cusolverDnXpotrf is used in case MAGMA is not linked in the
//     pytorch build. We will switch to cusolverDnXpotrfBatched after the issue
//     is fixed. See https://github.com/pytorch/pytorch/issues/53879.
template <typename scalar_t>
inline static void apply_cholesky_cusolver(
...
void linalg_eigh_cusolver(
    ...
    bool upper,
    bool compute_eigenvectors) {
  // TODO: syevj_batched should be added here, but at least for CUDA 11.2 it
  // contains a bug leading to incorrect results See
  ...
  if (eigenvectors.scalar_type() == at::kFloat && eigenvectors.size(-1) >= 32 &&
      eigenvectors.size(-1) <= 512) {
    return linalg_eigh_cusolver_syevj(
        eigenvalues, eigenvectors, infos, upper, compute_eigenvectors);
  } else {
    return linalg_eigh_cusolver_syevd(
        eigenvalues, eigenvectors, infos, upper, compute_eigenvectors);
  }
}
