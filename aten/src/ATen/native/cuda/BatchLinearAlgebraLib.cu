...
template<typename scalar_t>
static void _apply_svd_lib_gesvd(Tensor& self, Tensor& U, Tensor& S, Tensor& VT, char jobchar, Tensor& infos) {
...
inline static void apply_svd_lib_gesvd(const Tensor& self, Tensor& U, Tensor& S, Tensor& VT, char jobchar, Tensor& infos) {
  const int64_t m = self.size(-2), n = self.size(-1);
  if (m >= n) {
    Tensor self_working_copy = cloneBatchedColumnMajor(self);

    AT_DISPATCH_FLOATING_AND_COMPLEX_TYPES(self.scalar_type(), "svd_cuda_gesvd", [&] {
      _apply_svd_lib_gesvd<scalar_t>(self_working_copy, U, S, VT, jobchar, infos);
    });
  } else {
    ...
    AT_DISPATCH_FLOATING_AND_COMPLEX_TYPES(self.scalar_type(), "svd_cuda_gesvd", [&] {
      _apply_svd_lib_gesvd<scalar_t>(self_working_copy, VT, S, U, jobchar, infos);
    });
