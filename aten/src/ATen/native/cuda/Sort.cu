...
void sortKeyValueInplace(Tensor& key, Tensor& value, int dim, bool dir) {
  ...
  dim3 grid;
  TORCH_INTERNAL_ASSERT(
      getGridFromTiles(keySlices, grid), "Too many slices to sort");
  ...
  do {                                                                  \
    ...
    if (dir) {                                                          \
      bitonicSortKVInPlace<                                             \
          ...
          TYPE,                                                         \
          SIZE><<<grid, block, 0, c10::cuda::getCurrentCUDAStream()>>>( \
          keyInfo,                                                      \
          keySlices,                                                    \
          (TYPE)keySliceSize,                                           \
...
std::tuple<Tensor&, Tensor&> sort_out_stable_cuda(
    const Tensor& self,
  ...
  TensorArg self_arg{self, "self", 1}, values_arg{values, "values", 2},
  ...
  AT_DISPATCH_ALL_TYPES_AND2(kBool, kHalf, self_.scalar_type(), "sort", [&] {
    ...
    while (remaining > 0) {
      ...
      auto reverse_indices = at::arange(nsort, indices.options())
                                 .view({1, nsort})
                                 .expand({nsegments, nsort})
                                 .contiguous();
...
std::tuple<Tensor&, Tensor&> sort_out_cuda(
    ...
    Tensor& values,
    Tensor& indices) {
  return sort_out_stable_cuda(
      self, /*stable=*/false, dim, descending, values, indices);
}

std::tuple<Tensor, Tensor> sort_stable_cuda(
    const Tensor& self,
    c10::optional<bool> stable,
    int64_t dim,
    bool descending) {
...
std::tuple<Tensor, Tensor> sort_cuda(
    const Tensor& self,
    int64_t dim,
    bool descending) {
  int64_t threshold;
