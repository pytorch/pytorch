  ...
  AT_DISPATCH_V2(value.scalar_type(), "sortKeyValueInplace", AT_WRAP([&] {
    if (sort_size <= 1) {
      ...
      if (at::cuda::warp_size() == 32) {
        sortCommon<scalar_t>(WarpMergeSort<128, 32>{}, key, value, dim, descending);
      }
      else {
        sortCommon<scalar_t>(WarpMergeSort<128, 64>{}, key, value, dim, descending);
      }
  #else
      sortCommon<scalar_t>(WarpMergeSort<128, C10_WARP_SIZE>{}, key, value, dim, descending);
  #endif
  #endif
    } else {
