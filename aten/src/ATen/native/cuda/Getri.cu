...
void magmaGetriBatched(
    ...
    magma_int_t** dipiv_array, scalar_t** dinvA_array, magma_int_t lddia,
...
void magmaGetriBatched<float>(
    ...
    magma_int_t** dipiv_array, float** dinvA_array, magma_int_t lddia,
    magma_int_t* info_array, magma_int_t batchCount, magma_queue_t queue) {
...
void magmaGetriBatched<double>(
    ...
    magma_int_t** dipiv_array, double** dinvA_array, magma_int_t lddia,
    magma_int_t* info_array, magma_int_t batchCount, magma_queue_t queue) {
    magma_sgetri_outofplace_batched(
        n, dA_array, ldda, dipiv_array, dinvA_array,
        lddia, info_array, batchCount, queue);
...
static void applyGetri(Tensor &self, Tensor &self_inv, std::vector<int64_t>infos) {
#ifndef USE_MAGMA
AT_ERROR("getri: MAGMA library not found in "
...
Tensor _inverse_helper_cuda(const Tensor& self) {
  ...
  AT_DISPATCH_FLOATING_TYPES(self.type(), "inverse", [&]{
    applyGetri<scalar_t>(self_working_copy, self_inv_working_copy, infos);
  });
  checkErrors(infos);
