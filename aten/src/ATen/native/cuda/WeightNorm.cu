...
// increase the amount of cache available per block, which can improve cache hit
// rate. However, this is less efficient for short rows.  256 is pretty
// versatile. May be worth implementing heuristics later.
#define BLOCK 256

// Block size for weight_norm_*_last_dim_kernel.
// This is tricker than the first_dim case because we must make blocks
// at least 16 fast elements wide to ensure fully-coalesced half-precision
// accesses. Since output-element parallelism is along the fast dimension, this
// reduces the number of blocks we can launch by 16X.
#define TILE_W 16
...
__global__ void weight_norm_fwd_first_dim_kernel(
  ...
  // Hack to get around nvcc complaining when an smem array is declared with the
  // same name but different types in different kernels (in this case different
  // instantiations) extern __shared__ accscalar_t s[]; // error: declaration is
  // incompatible with previous "s"
...
__global__ void weight_norm_bwd_first_dim_kernel(
  ...
  // Could choose to save reciprocal of norm instead I suppose, but norms is
  // probably more handy to keep around. Broadcast load; could use shared memory
  // instead.
  ...
  // Write v gradients.  We are reusing values that were loaded earlier, so
  // there is an optimization opportunity here (store values persistently).
  for (int j = tid; j < rowSize; j += stride) {
    ...
    accscalar_t grad_vj =
        g_this_row * (rnorm * grad_wj - rnorm3 * saved_vj * result);
...
__global__ void weight_norm_bwd_last_dim_kernel(
  ...
  if (fast_dim_location < fast_dim_size)
    while (slower_dims_location < slower_dims_size) {
      ...
      accscalar_t grad_vj =
          g_this_col * (rnorm * grad_wj - rnorm3 * saved_vj * result);
...
std::tuple<Tensor, Tensor> weight_norm_cuda(
  ...
  // weight_norm_fused does have a derivative defined in derivatives.yaml,
  // therefore, VariableType.cpp sends the unpacked g.data() as the argument. In
  // other words, we expect "g" is a bare Tensor here.
  ...
  if (dim == 0) {
    ...
    AT_DISPATCH_FLOATING_TYPES_AND_HALF(
        v.scalar_type(), "weight_norm_fwd_first_dim_kernel", [&] {
          using accscalar_t = acc_type<scalar_t, true>;

          weight_norm_fwd_first_dim_kernel<scalar_t, accscalar_t>
              <<<v.size(0), BLOCK, BLOCK * sizeof(accscalar_t), stream>>>(
                  w.data_ptr<scalar_t>(),
    ...
    AT_DISPATCH_FLOATING_TYPES_AND_HALF(
        v.scalar_type(), "weight_norm_fwd_last_dim_kernel", [&] {
          using accscalar_t = acc_type<scalar_t, true>;
  ...
  // The kernel execution is asynchronous, so this will only catch errors on the
  // kernel launch, not the kernel's execution.  Errors in kernel execution
  // aren't guaranteed to be caught until a later error check on a synchronizing
  // CUDA call.  Unfortunately, without manually synchronizing here, the
  // foregoing is the best we can do.
...
std::tuple<Tensor, Tensor> weight_norm_cuda_backward(
    ...
    int64_t dim) {
  // These checks should always succeed, because weight_norm_fused_backward
  // should only ever be recorded in the autograd graph via weight_norm, which
  // passes contiguous v and g.
  ...
  if (dim == 0) {
    ...
    AT_DISPATCH_FLOATING_TYPES_AND_HALF(
        saved_v.scalar_type(), "weight_norm_bwd_first_dim_kernel", [&] {
          using accscalar_t = acc_type<scalar_t, true>;

          weight_norm_bwd_first_dim_kernel<scalar_t, accscalar_t>
              <<<grad_w.size(0), BLOCK, BLOCK * sizeof(accscalar_t), stream>>>(
    ...
    // Precompute slower_dims_size and fast_dim_size because they involve
    ...
    AT_DISPATCH_FLOATING_TYPES_AND_HALF(
        saved_v.scalar_type(), "weight_norm_bwd_last_dim_kernel", [&] {
          using accscalar_t = acc_type<scalar_t, true>;
