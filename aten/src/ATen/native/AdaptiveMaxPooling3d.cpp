...
namespace at {
namespace meta {
TORCH_META_FUNC(adaptive_max_pool3d)
(const Tensor& input, IntArrayRef output_size) {
  ...
  if (ndim == 4) {
    set_output_raw_strided(
        0, {sizeD, osizeT, osizeH, osizeW}, {}, input.options());
    /* indices will contain max input locations for each output point */
    set_output_raw_strided(
        1, {sizeD, osizeT, osizeH, osizeW}, {}, input.options().dtype(kLong));
  } else {
    set_output_raw_strided(
        0, {sizeB, sizeD, osizeT, osizeH, osizeW}, {}, input.options());
    /* indices will contain max input locations for each output point */
    set_output_raw_strided(
        1,
      ...
      for (ot = 0; ot < osizeT; ot++) {
        ...
        for (oh = 0; oh < osizeH; oh++) {
          ...
          for (ow = 0; ow < osizeW; ow++) {
            ...
            int64_t* indp = ind_p + d * osizeT * osizeH * osizeW +
                ot * osizeH * osizeW + oh * osizeW + ow;

            /* compute local max: */
            int64_t it = 0, ih = 0, iw = 0;
            int64_t maxindex = (it + istartT) * isizeH * isizeW +
              ...
              for (ih = 0; ih < kH; ih++) {
                for (iw = 0; iw < kW; iw++) {
                  ...
                  if ((val > maxval) || std::isnan(val)) {
                    maxval = val;
                    maxindex = (it + istartT) * isizeH * isizeW +
                        (ih + istartH) * isizeW + (iw + istartW);
                  }
                }
              }
      ...
      for (ot = 0; ot < osizeT; ot++) {
        for (oh = 0; oh < osizeH; oh++) {
          for (ow = 0; ow < osizeW; ow++) {
            ...
            /* update gradient */
            gradInput_p_d[maxp] +=
                gradOutput_p_d[ot * osizeH * osizeW + oh * osizeW + ow];
          }
        }
      }
    }
