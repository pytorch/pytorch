...
modified_bessel(T1 n, T1 x, bool exp = false) {
  ...
  if (n < T1(0)) {
    if (std::abs(at::native::special::sin_pi(-n)) < std::numeric_limits<T1>::epsilon()) {
      ...
      return {n, x, modified_bessel(-n, x, exp).i
          + at::native::special::sin_pi(-n) * T1(2) * modified_bessel(-n, x, exp).k
              / c10::numbers::pi_v<T1>, modified_bessel(-n, x, exp).i_derivative
                  + at::native::special::sin_pi(-n) * T1(2) * modified_bessel(-n, x, exp).k_derivative
