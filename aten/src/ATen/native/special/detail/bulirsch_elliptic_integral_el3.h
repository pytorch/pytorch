...
bulirsch_elliptic_integral_el3(T1 x, T1 k_c, T1 p) {
  ...
  if (std::isnan(x) || std::isnan(k_c) || std::isnan(p)) {
    ...
    return x * carlson_elliptic_r_f(T2(0x01), T1(0x01) + k_c * k_c * (x * x), T1(0x01) + x * x) + (T1(0x01) - p) * (x * x) * carlson_elliptic_r_j(T1(0x01), T1(0x01) + k_c * k_c * (x * x), T1(0x01) + x * x, T1(0x01) + p * (x * x)) / T1(0x03);
  }
}
} // namespace detail
} // namespace special
