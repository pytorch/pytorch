...
ln_double_factorial(T1 n) {
  return (n / T1(2)) * std::log(T1(2)) + (at::native::special::cos_pi(n) - T1(1)) * std::log(c10::numbers::pi_v<T1> / T1(2)) / T1(4) + ln_gamma(T1(1) + n / T1(2));
}
...
ln_double_factorial(int n) {
  if (n < 0) {
    if (n % 2 == 1) {
      if (-n <= static_cast<int>(c10::numbers::negative_double_factorials_size<T1>())) {
        return c10::numbers::negative_double_factorials_v[-(1 + n) / 2].log_factorial;
      } else {
