...
// The purpose of `update_from` and `update_to` is to find the closest valid
// int64_t number that can be used as actual `from`. The current implementation
// of `random_` uses uint64_t arithmetics and casts the result to the target
// dtype(scalar_t). This casting can result in generating numbers that happen to
// be greater or equal to `to` value. For instance:
//
//    auto actual = torch::empty({3, 3}, torch::half);
//    actual.random_(0, 65504);
//
// If random's uint64_t arithmetics produces 65503 as a random value after
// casting to torch::half it becomes 65504 and violates the requirement that
// random value must be less than `to`. To resolve this issue `update_from` and
// `update_to` moves `from` to the left and `to` to the right to the next
// closest value that won't go outside [from, to) after casting to the target
// dtype. For `to` = 65504 it moves left for (1 << (log2(to) - 11 + 1)) = 32 and
// becomes 65472, which is previous available number for torch::half dtype.
...
// This function computes broadcasted size of mean and std, resize the output to
// the broadcasted size if it was empty [Note] The following features will be
// deprecated in version 1.6 release and function signature will be changed
// after
...
//     This function will return true in deprecated case, false in broadcastable
//     case and throw in all other cases before deprecation. This function will
//     not return and throw if mean and std are not broadcastable after
//     deprecation
static bool resize_output_for_normal(
    at::Tensor& output,
...
Tensor& normal_out_impl(
    ...
    c10::optional<Generator> gen) {
  TORCH_CHECK(
      !std.is_complex(), "normal expects standard deviation to be non-complex");
  TORCH_CHECK(
  ...
  // The previous function here was addcmul_out(output, mean_tensor, output,
  // std, 1); The third argument is not a constant reference and hence the
  // samples in output are overwritten. Consequently, the computation performed
  // is mean_tensor + mean_tensor * std instead of mean_tensor + output * std
...
Tensor& normal_out_impl(
  ...
  // The third argument is not a constant reference and hence the samples in
  // output are overwritten. Consequently, the computation performed is mean +
  // mean * std instead of mean + output * std
...
Tensor& geometric_impl_(Tensor& self, double p, c10::optional<Generator> gen) {
  TORCH_CHECK(
      0 < p && p < 1, "geometric_ expects p to be in (0, 1), but got p=", p);
  auto iter = TensorIterator::nullary_op(self);
...
Tensor& bernoulli_impl_(Tensor& self, double p, c10::optional<Generator> gen) {
  TORCH_CHECK(
      0 <= p && p <= 1, "bernoulli_ expects p to be in [0, 1], but got p=", p);
  at::assert_no_internal_overlap(self);
