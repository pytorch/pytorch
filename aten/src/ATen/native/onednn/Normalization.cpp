...
std::tuple<Tensor, Tensor, Tensor> onednn_batch_norm(
    ...
    double momentum,
    double eps) {
  TORCH_CHECK(false, "onednn_batch_norm: ATen not compiled with MKLDNN support");
}
...
std::tuple<Tensor, Tensor, Tensor> onednn_batch_norm_backward(
  ...
  TORCH_CHECK(false, "onednn_batch_norm_backward: ATen not compiled with MKLDNN support");
}
...
std::tuple<Tensor, Tensor, Tensor> _onednn_batch_norm_legit(
    ...
    double momentum,
    double eps) {
  TORCH_CHECK(false, "_onednn_batch_norm_legit: ATen not compiled with MKLDNN support");
}
...
std::tuple<Tensor, Tensor, Tensor> _onednn_batch_norm_legit_no_stats(
    ...
    double momentum,
    double eps) {
  TORCH_CHECK(false, "_onednn_batch_norm_legit_no_stats: ATen not compiled with MKLDNN support");
}
...
std::tuple<Tensor, Tensor, Tensor, Tensor> _batch_norm_with_update_onednn(
  ...
  TORCH_CHECK(false, "_batch_norm_with_update_onednn: ATen not compiled with MKLDNN support");
}
...
std::tuple<Tensor, Tensor, Tensor> _new_batch_norm_backward_onednn(
  ...
  TORCH_CHECK(false, "_new_batch_norm_backward_onednn: ATen not compiled with MKLDNN support");
}
...
std::tuple<Tensor, Tensor, Tensor> onednn_batch_norm(
  ...
  TORCH_CHECK(weight.defined() && bias.defined(),
             "onednn_batch_norm: currently mkldnn only support affine model");
  ...
  if (train) {
    ...
    TORCH_CHECK(input.dim() == 4,
        "onednn_batch_norm: currently mkldnn training only support 2d batchnorm");
    ...
    TORCH_CHECK(input.dim() == 4 || input.dim() == 5,
        "onednn_batch_norm: currently mkldnn inference only support 2d and 3d batchnorm");
    if (use_running_stat) {
      ideep::tensor m = itensor_from_tensor(running_mean);
      ideep::tensor v = itensor_from_tensor(running_var);
      ...
      TORCH_CHECK(false, "onednn_batch_norm: mkldnn inference is not keep running estimates.");
    }
    return std::make_tuple(
        new_with_itensor_mkldnn(std::move(y), optTypeMetaToScalarType(input.options().dtype_opt()),
                                input.options().device_opt()),
...
std::tuple<Tensor, Tensor, Tensor> onednn_batch_norm_backward(const Tensor& grad_output,
  ...
  TORCH_CHECK(train, "onednn_batch_norm_backward: currently mkldnn only support train model");
