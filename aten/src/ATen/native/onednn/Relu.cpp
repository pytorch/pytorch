...
Tensor onednn_relu(const Tensor& input) {
  TORCH_CHECK(false, "onednn_relu: ATen not compiled with MKLDNN support");
}

Tensor& onednn_relu_(Tensor& input) {
  TORCH_CHECK(false, "onednn_relu_: ATen not compiled with MKLDNN support");
}

Tensor onednn_relu_backward(const Tensor& grad_output, const Tensor& input, const Scalar& threshold) {
  TORCH_CHECK(false, "onednn_relu_backward: ATen not compiled with MKLDNN support");
}
