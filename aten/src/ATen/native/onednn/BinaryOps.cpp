...
Tensor& onednn_add_out(
  ...
  TORCH_CHECK(false, "onednn_add_out: ATen not compiled with MKLDNN support");
}

Tensor onednn_add(const Tensor& self, const Tensor& other, const Scalar& alpha) {
  TORCH_CHECK(false, "onednn_add: ATen not compiled with MKLDNN support");
}

Tensor& onednn_add_(Tensor& self, const Tensor& other, const Scalar& alpha) {
  TORCH_CHECK(false, "onednn_add_: ATen not compiled with MKLDNN support");
}

Tensor& onednn_mul_out(const Tensor& self, const Tensor& other, Tensor& result) {
  TORCH_CHECK(false, "onednn_mul_out: ATen not compiled with MKLDNN support");
}

Tensor onednn_mul(const Tensor& self, const Tensor& other) {
  TORCH_CHECK(false, "onednn_mul: ATen not compiled with MKLDNN support");
}

Tensor& onednn_mul_(Tensor& self, const Tensor& other) {
  TORCH_CHECK(false, "onednn_mul_: ATen not compiled with MKLDNN support");
}
...
Tensor& onednn_mul_out(const Tensor& self, const Tensor& other, Tensor& result) {
  ...
  if (other.ndimension() == 0) {
    ...
    TORCH_CHECK(self.sizes() == other.sizes(),
               "onednn_mul_out: currently mkldnn not support broadcasting");
    ideep::tensor y = itensor_from_mkldnn(other);
