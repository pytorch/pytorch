...
Tensor mkldnn_view(const Tensor& self, IntArrayRef size) {
  TORCH_CHECK(false, "onednn_reshape: ATen not compiled with MKLDNN support");
}
...
Tensor onednn_clone(const Tensor& self, std::optional<c10::MemoryFormat> optional_memory_format) {
  TORCH_CHECK(false, "onednn_clone: ATen not compiled with MKLDNN support");
}

Tensor onednn_transpose(const Tensor& self, int64_t dim0, int64_t dim1) {
  TORCH_CHECK(false, "onednn_transpose: ATen not compiled with MKLDNN support");
}

Tensor& onednn_transpose_(Tensor& self, int64_t dim0, int64_t dim1) {
  TORCH_CHECK(false, "onednn_transpose_: ATen not compiled with MKLDNN support");
}
...
Tensor& onednn_transpose_(Tensor& self, int64_t dim0, int64_t dim1) {
  TORCH_CHECK(false, "onednn_transpose_: in-place mkldnn operations are not supported yet");
}
