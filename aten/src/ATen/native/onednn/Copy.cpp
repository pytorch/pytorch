...
Tensor& copy_mkldnn_(Tensor& self, const Tensor& src, bool non_blocking) {
  TORCH_CHECK(false, "copy_mkldnn_: ATen not compiled with ONEDNN support");
}
...
Tensor& copy_mkldnn_(Tensor& self, const Tensor& src, bool non_blocking) {
  ...
  TORCH_CHECK(
      ...
      "copy_mkldnn_: between onednn layout and dense Tensors is not implemented! Found self type = ",
      self.toString(),
      " and src type = ",
      src.toString());
