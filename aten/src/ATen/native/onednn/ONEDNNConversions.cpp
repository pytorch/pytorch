...
Tensor mkldnn_to_dense(const Tensor& onednn_tensor, std::optional<ScalarType> dtype, std::optional<bool> masked_grad) {
  TORCH_CHECK(onednn_tensor.scalar_type() == ScalarType::Float ||
              ...
              onednn_tensor.scalar_type() == ScalarType::Float8_e4m3fn,
              "onednn_to_dense expects float, bfloat16, half, uint8, int8, float8_e4m3fn tensor input");
  ...
  if (onednn_tensor.scalar_type() == ScalarType::Byte ||
      onednn_tensor.scalar_type() == ScalarType::Char ||
      onednn_tensor.scalar_type() == ScalarType::Float8_e4m3fn) {
    // For uint8, int8, float8 input, we should not change the data type.
    TORCH_CHECK(onednn_tensor.scalar_type() == data_type,
            "For uint8, int8, float8 onednn_tensor input, we should not change the data type.");
  }
