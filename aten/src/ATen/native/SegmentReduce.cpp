...
Tensor _segment_reduce_lengths_cpu_kernel(
    ...
    int64_t axis,
    const c10::optional<Scalar>& initial) {
  // data and lengths should be contiguous from the call to .contiguous in
  ...
  AT_DISPATCH_INDEX_TYPES(
      lengths.scalar_type(), "_segment_reduce_lengths_cpu_kernel1", [&]() {
...
Tensor _segment_reduce_offsets_cpu_kernel(
  ...
  AT_DISPATCH_INDEX_TYPES(
      offsets.scalar_type(), "_segment_reduce_offsets_cpu_kernel1", [&]() {
      ...
      [&]() {
        ...
        for (const auto outer_idx : c10::irange(outer_offset)) {
          ...
          int64_t segment_end = is_offsets_like
              ...
              : 0;
          for (const auto dim_idx : c10::irange(segment_count)) {
            // int64_t segment_length = lengths_data[outer_idx *
            // lengths_stride_axis * segment_count + dim_idx];
            segment_start = segment_end;
            auto lengths_idx =
                outer_idx * lengths_stride_axis * lengths_size_axis + dim_idx;
            if (is_offsets_like) {
              ...
              if (reduction == SegmentReductionType::MAX ||
                ...
                for (const auto j : c10::irange(segment_start, segment_end)) {
                  ...
                  if (at::_isnan(values_data[data_index]) ||
                    ...
                    int64_t idx;
                    for (const auto k :
                         c10::irange(segment_start, segment_end)) {
                      if (k != j) {
...
Tensor _segment_reduce_cpu_offsets_backward_kernel(
  ...
  AT_DISPATCH_INDEX_TYPES(
      ...
      "_segment_reduce_cpu_offsets_backward_kernel1",
      [&] {
