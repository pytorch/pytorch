...
std::tuple<Tensor, Tensor> _rowwise_prune_helper(
  ...
  auto pruned_2d_tensor =
      at::empty({num_non_masked_rows, num_cols}, weights.options());
  auto compressed_indices_mapping =
      at::empty({mask.numel()}, compressed_indices_dtype);
  AT_DISPATCH_ALL_TYPES_AND2(
      ...
      [&]() {
        ...
        for (const auto i : c10::irange(mask.numel())) {
          if (mask_data[i]) {
            memcpy(
                pruned_2d_tensor_data + last_row_kept * num_cols,
  ...
  return std::tuple<Tensor, Tensor>(
      pruned_2d_tensor, compressed_indices_mapping);
}
...
std::tuple<Tensor, Tensor> _rowwise_prune(
    const Tensor& weights,
    const Tensor& mask,
    ScalarType compressed_indices_dtype) {
  TORCH_CHECK(weights.ndimension() == 2, "'weights' should have 2 dimensions.");
  TORCH_CHECK(
  ...
  if (compressed_indices_dtype == at::ScalarType::Int) {
    return _rowwise_prune_helper<int32_t>(
        weights, mask, compressed_indices_dtype);
  }
  return _rowwise_prune_helper<int64_t>(
      weights, mask, compressed_indices_dtype);
}
