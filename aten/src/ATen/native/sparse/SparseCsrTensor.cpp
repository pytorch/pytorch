...
void _validate_sparse_compressed_tensor_args_worker(
  ...
  TORCH_CHECK(
      ...
      "expected ",
      plain_indices_name,
      " to be a strided and contiguous tensor");

  TORCH_CHECK(
      compressed_indices.layout() == kStrided &&
          compressed_indices.is_contiguous(),
      "expected ",
      compressed_indices_name,
      " to be a strided and contiguous tensor");
  ...
  TORCH_CHECK(
      ...
      "size of a batched ",
      layout_name,
      " tensor must have length >= 2, but got: ",
      size.size());
  TORCH_CHECK(
      compressed_indices.dim() >= 1,
      compressed_indices_name,
      " must have dim >= 1 but got ",
      compressed_indices_name,
      ".dim() = ",
  ...
  int block_ndim = AT_DISPATCH_PLAIN_SPARSE_COMPRESSED_LAYOUTS(
      ...
      [&] {
        TORCH_CHECK(
            ...
            "number of dimensions of indices (=",
            compressed_indices.dim(),
            ") must be equal or less than the number of dimensions of values (=",
            values.dim(),
            ")");
        return 0;
      },
      [&] {
        TORCH_CHECK(
            ...
            "number of dimensions of indices (=",
            compressed_indices.dim(),
            ") plus two must be equal or less than the number of dimensions of values (=",
            values.dim(),
            ")");
        return 2;
      });
  ...
  TORCH_CHECK(
      static_cast<size_t>(compressed_indices.dim()) ==
          size.size() - 1 - dense_ndim,
  ...
  DimVector blocksize{
      (block_ndim == 2
           ? std::max<int64_t>(1, values.sizes()[values.dim() - dense_ndim - 2])
           : 1),
      (block_ndim == 2
           ? std::max<int64_t>(1, values.sizes()[values.dim() - dense_ndim - 1])
           : 1),
  };
  TORCH_INTERNAL_ASSERT(
      blocksize.size() == 2 && blocksize[0] > 0 && blocksize[1] > 0);
  ...
  DimVector compressed_indices_batch_size = DimVector(
      compressed_indices.sizes().slice(0, compressed_indices.dim() - 1));
  DimVector plain_indices_batch_size =
      DimVector(plain_indices.sizes().slice(0, plain_indices.dim() - 1));
  DimVector values_batch_size = DimVector(
      values.sizes().slice(0, values.dim() - 1 - block_ndim - dense_ndim));
  TORCH_CHECK(
      batch_size == compressed_indices_batch_size &&
          batch_size == plain_indices_batch_size &&
          batch_size == values_batch_size,
      "all batch dimensions of the provided size (",
      batch_size,
      "), indices (",
      compressed_indices_batch_size,
      ", ",
  ...
  for (int i = 0; i < block_ndim; i++) {
    TORCH_CHECK(
        size[batch_ndim + i] % blocksize[i] == 0,
        "tensor size(",
        batch_ndim + i,
        ") ",
        size[batch_ndim + i],
        " needs to be divisible with blocksize[",
        i,
        "] ",
        blocksize[i]);
  }
  ...
  if (block_ndim == 2) {
    TORCH_CHECK(
        compressed_indices.size(-1) ==
            (size[compressed_dim] / blocksize[compressed_dim - batch_ndim] + 1),
        compressed_indices_name,
        ".size(-1) must be equal to size[-",
        (size.size() - compressed_dim),
        "]/blocksize[",
        compressed_dim - batch_ndim,
        "] + 1 (that is ",
        size[compressed_dim] / blocksize[compressed_dim - batch_ndim] + 1,
        "), but got: ",
        compressed_indices.size(-1));
    TORCH_CHECK(
        ...
        "number of ",
        plain_indices_name,
        " elements must be the same as the number of blocks in values, but got ",
        plain_indices_name,
        ".numel() * numel_per_block: ",
        plain_indices.numel() * numel_per_block,
        ", values.numel(): ",
        values.numel(),
        ", numel_per_block: ",
        numel_per_block);
    ...
    TORCH_CHECK(
        ...
        "number of ",
        plain_indices_name,
        " elements must be the same number of elements, but got ",
        plain_indices_name,
        ".numel(): ",
        plain_indices.numel(),
        ", values.numel(): ",
  ...
  TORCH_CHECK(
      ...
      " and ",
      plain_indices_name,
      " should have the same type, bot got ",
  ...
  TORCH_CHECK(
      ...
      " and ",
      plain_indices_name,
      " must be an int32 or int64 type, but got: ",
      compressed_indices_type);

  // Indices invariants
  AT_DISPATCH_INDEX_TYPES(
      compressed_indices_type, "validate_sparse_compressed_tensor_args", [&] {
        Tensor compressed_indices_cpu = compressed_indices.to(kCPU);
        auto compressed_indices_data_ptr =
            compressed_indices_cpu.data_ptr<index_t>();
        auto batch_stride = compressed_indices_cpu.dim() >= 2
            ? compressed_indices_cpu.stride(-2)
            : 0;
        auto compressed_dims =
            (block_ndim == 0 ? size[compressed_dim]
                             : size[compressed_dim] /
                     blocksize[compressed_dim - batch_ndim]);
        for (const auto batch_id :
             c10::irange(batchCount(compressed_indices_cpu))) {
          TORCH_CHECK(
              ...
              compressed_indices_name,
              " must be 0, but it is ",
              compressed_indices_data_ptr[batch_id * batch_stride]);
          TORCH_CHECK(
              compressed_indices_data_ptr
                      [batch_id * batch_stride + compressed_indices.size(-1) -
                       1] == plain_indices.size(-1),
              "(Batch element ",
              batch_id,
              ") ",
              "last value of ",
          ...
          for (int i = 1; i <= compressed_dims; i++) {
            TORCH_CHECK(
                compressed_indices_data_ptr[batch_id * batch_stride + i - 1] <=
                    compressed_indices_data_ptr[batch_id * batch_stride + i],
                "(Batch element ",
                batch_id,
                ") ",
                "at position i = ",
        ...
        if (plain_indices.numel() > 0) {
          TORCH_CHECK(
              0 <= plain_indices.min().item<index_t>(),
              plain_indices_name,
              ".min() should be greater or equal to zero");
          TORCH_CHECK(
              size[plain_dim] > plain_indices.max().item<index_t>(),
              "size[-",
              (size.size() - plain_dim),
...
void _validate_sparse_csr_tensor_args(
    ...
    IntArrayRef size) {
  _validate_sparse_compressed_tensor_args_worker(
      crow_indices, col_indices, values, size, kSparseCsr);
}
...
void _validate_sparse_csc_tensor_args(
    ...
    IntArrayRef size) {
  _validate_sparse_compressed_tensor_args_worker(
      ccol_indices, row_indices, values, size, kSparseCsc);
}
...
void _validate_sparse_bsr_tensor_args(
    ...
    IntArrayRef size) {
  _validate_sparse_compressed_tensor_args_worker(
      crow_indices, col_indices, values, size, kSparseBsr);
}
...
void _validate_sparse_bsc_tensor_args(
    ...
    IntArrayRef size) {
  _validate_sparse_compressed_tensor_args_worker(
      ccol_indices, row_indices, values, size, kSparseBsc);
}
...
SparseCsrTensor new_compressed_tensor(const TensorOptions& options) {
  ...
  Layout layout = AT_DISPATCH_ALL_SPARSE_COMPRESSED_LAYOUTS(
      options.layout(), "new_compressed_tensor", [&] { return the_layout; });
  DispatchKey dispatch_key;

  TORCH_CHECK_NOT_IMPLEMENTED(
      options.device().type() == kCPU || options.device().type() == kCUDA,
      "Could not run 'new_compressed_tensor' from the '",
      options.device(),
      "' device.)");
...
Tensor _sparse_compressed_tensor_unsafe(
    const Tensor& compressed_indices,
    const Tensor& plain_indices,
    const Tensor& values,
    IntArrayRef size,
  ...
  if (!layout) {
    AT_ERROR(
        "sparse_compressed_tensor_unsafe expected sparse compressed tensor layout but got none");
  }
  Layout layout_ = layout.value();
  AT_DISPATCH_ALL_SPARSE_COMPRESSED_LAYOUTS(
      layout_, "sparse_compressed_tensor_unsafe", [&] {});
  TensorOptions options =
      TensorOptions().dtype(dtype).layout(layout_).device(device).pinned_memory(
          pin_memory);
...
Tensor _sparse_compressed_tensor_unsafe_template(
    const Tensor& compressed_indices,
    const Tensor& plain_indices,
    const Tensor& values,
    IntArrayRef size,
  ...
  TORCH_CHECK(
      layout_ == required_layout,
      "sparse compressed layout must be ",
      required_layout,
      " but got ",
      layout_);
  TensorOptions options =
...
#define SPARSE_COMPRESSED_TENSOR_UNSAFE(KIND, REQUIRED_LAYOUT)         \
  Tensor _sparse_##KIND##_tensor_unsafe(                               \
      const Tensor& compressed_indices,                                \
      const Tensor& plain_indices,                                     \
      const Tensor& values,                                            \
      IntArrayRef size,                                                \
      ...
      c10::optional<Device> device,                                    \
      c10::optional<bool> pin_memory) {                                \
    return _sparse_compressed_tensor_unsafe_template<REQUIRED_LAYOUT>( \
        compressed_indices,                                            \
        plain_indices,                                                 \
        values,                                                        \
        size,                                                          \
...
DimVector _estimate_sparse_compressed_tensor_size(
  ...
  int block_ndim = AT_DISPATCH_PLAIN_SPARSE_COMPRESSED_LAYOUTS(
      layout,
      "estimate_sparse_compressed_tensor_size",
      [&] { return 0; },
      [&] { return 2; });
  int dense_ndim = values.dim() - compressed_indices.dim() - block_ndim;
  DimVector blocksize{
  ...
  DimVector size = DimVector(
      IntArrayRef(plain_indices.sizes().data(), plain_indices.dim() - 1));
  int64_t compressed_dim =
      (plain_indices.size(-1) > 0 ? compressed_indices.size(-1) - 1 : 0);
  int64_t plain_dim = AT_DISPATCH_INTEGRAL_TYPES(
...
Tensor sparse_compressed_tensor(
  ...
  if (!layout) {
    AT_ERROR(
        "sparse_compressed_tensor expected sparse compressed tensor layout but got none");
  }
  Layout layout_ = layout.value();
  AT_DISPATCH_ALL_SPARSE_COMPRESSED_LAYOUTS(
      layout_, "sparse_compressed_tensor", [&] {});
  ...
  _validate_sparse_compressed_tensor_args_worker(
      compressed_indices, plain_indices, values, size, layout_);
...
Tensor sparse_compressed_tensor(
  ...
  DimVector size = _estimate_sparse_compressed_tensor_size(
      compressed_indices, plain_indices, values, layout_);
...
#define SPARSE_COMPRESSED_TENSOR(KIND, REQUIRED_LAYOUT) \
  Tensor sparse_##KIND##_tensor(                        \
    ...
    if (layout) {                                       \
      TORCH_CHECK(                                      \
          layout.value() == REQUIRED_LAYOUT,            \
          "sparse " #KIND " layout must be ",           \
          REQUIRED_LAYOUT,                              \
          " but got ",                                  \
          layout.value());                              \
    }                                                   \
    c10::optional<Layout> layout_(REQUIRED_LAYOUT);     \
    return at::native::sparse_compressed_tensor(        \
        compressed_indices,                             \
        plain_indices,                                  \
        values,                                         \
        dtype,                                          \
...
Tensor empty_sparse_compressed(
  ...
  TORCH_CHECK(
      size.size() >= 2,
      "torch.empty: Only batched sparse compressed (non-block) tensors are supported, but got size ",
      size);
  ...
  auto plain_indices_and_values_size =
      DimVector(size.slice(0, size.size() - 2));
  compressed_indices_size.push_back(
      size[compressedDimension(layout_, size)] + 1);
...
const Tensor& resize_sparse_csr_(
  ...
  TORCH_CHECK(
      size.size() >= 2,
      "torch.resize_: Only batched sparse CSR matrices are supported, but got size ",
      size);
  TORCH_CHECK(
      ...
      self.size(-1),
      " while the requested new number of columns is ",
      size[size.size() - 1],
      ".");
...
Tensor& copy_sparse_compressed_(
    Tensor& self,
    const Tensor& src,
    bool non_blocking) {
  AT_DISPATCH_ALL_SPARSE_COMPRESSED_LAYOUTS(
      self.layout(), "copy_sparse_compressed_", [&] {});
  TORCH_CHECK(
      ...
      src.layout());
  TORCH_CHECK(
      self._nnz() == src._nnz(), // actually, values copy allows different
                                 // shapes as long as operands are broadcastable
  ...
  auto src_compressed_dims =
      src.size(compressedDimension(src.layout(), src.sizes()));
  if (self_compressed_dim == src_compressed_dim) {
    TORCH_CHECK(
        self_compressed_dims == src_compressed_dims,
        "torch.copy_: expected shapes of self and src to match along dimension ",
        self_compressed_dim,
        " for ",
        self.layout(),
        " layout but the corresponding dimensions of self and src are ",
        self_compressed_dims,
        " and ",
        src_compressed_dims,
        ", respecitvely.");
  } else {
  ...
  AT_DISPATCH_PLAIN_SPARSE_COMPRESSED_LAYOUTS(
      self.layout(),
      "copy_sparse_compressed_",
      [&] {},
      [&] {
        auto self_values = self.values();
        auto src_values = src.values();
        auto self_blocksize =
            DimVector(self_values.sizes().slice(self_values.dim() - 2, 2));
        auto src_blocksize =
            DimVector(src_values.sizes().slice(src_values.dim() - 2, 2));
        TORCH_CHECK(
            self_blocksize == src_blocksize,
            "torch.copy_: copy of sparse compressed tensors having different block sizes is not supported.",
            " self and src block sizes are ",
            self_blocksize,
            " and ",
            src_blocksize,
            ", respectivly.");
...
Tensor crow_indices_sparse_csr(const Tensor& self) {
  return AT_DISPATCH_SPARSE_ROW_COMPRESSED_LAYOUTS(
      self.layout(), "crow_indices", [&] {
