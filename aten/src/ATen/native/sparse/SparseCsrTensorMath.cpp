...
SparseCsrTensor dense_to_sparse_csr(const Tensor& self) {
  return self.to_sparse().to_sparse_csr();
}

SparseCsrTensor csr_to_sparse_csr(const SparseCsrTensor& self) {
  return self;
}

SparseCsrTensor coo_to_sparse_csr(const SparseTensor& self) {
  TORCH_CHECK(
      self.dim() == 2,
