...
SparseTensor dense_to_sparse(const Tensor& self, const TensorOptions& options){
  return dense_to_sparse(self, self.dim(), options);
}

SparseTensor dense_to_sparse(const Tensor& self, int64_t sparse_dim, const TensorOptions& options) {
  if (options.layout() == kSparseGCS) {
  }
  else {
