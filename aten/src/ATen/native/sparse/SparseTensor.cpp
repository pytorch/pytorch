...
SparseTensor new_sparse(const TensorOptions& options) {
  AT_ASSERT(!options.is_variable());  // TODO: this can be removed when Variable and Tensor are merged
...
SparseTensor coalesce_sparse_cpu(const SparseTensor& self) {
  ...
  AT_ASSERT(!self.is_variable());  // TODO: this can be removed when Variable and Tensor are merged
