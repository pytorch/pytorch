...
torch_sparse_assert(const bool cond, const char* const message) {
#ifdef GPUCC
...
INVARIANT_CHECK_FUNC_API _check_first_cidx_is_zero(
  ...
  if (cdim_name == CDimName::CRow) {
    torch_sparse_assert(invariant, "`crow_indices[..., 0] == 0` is not satisfied.");
  } else {
    torch_sparse_assert(invariant, "`ccol_indices[..., 0] == 0` is not satisfied.");
  }
}
...
INVARIANT_CHECK_FUNC_API _check_last_cidx_is_nnz(
  ...
  if (cdim_name == CDimName::CRow) {
    torch_sparse_assert(invariant, "`crow_indices[..., -1] == nnz` is not satisfied.");
  } else {
    torch_sparse_assert(invariant, "`ccol_indices[..., -1] == nnz` is not satisfied.");
  }
}
...
INVARIANT_CHECK_FUNC_API _check_idx_bounds(
  ...
  if (cdim_name == CDimName::CRow) {
    torch_sparse_assert(invariant, "`0 <= col_indices < ncols` is not satisfied.");
  } else {
    torch_sparse_assert(invariant, "`0 <= row_indices < nrows` is not satisfied.");
  }
}
