...
SparseTensor _coalesce_sparse_cuda(const SparseTensor& self) {
  ...
  thrust::pair<thrust_ptr, thrust_ptr> newEnd = thrust::unique_by_key(
      policy, indicesIter, indicesIter + nnz, uniqueOffsetsIter);
  ...
  if (newValues.numel() > 0) {
    ...
    dim3 grid(
        THCCeilDiv(newNnz, (int64_t)SZ),
        THCCeilDiv(stride, (int64_t)C10_WARP_SIZE * SZ));
    ...
    AT_DISPATCH_ALL_TYPES_AND_COMPLEX_AND2(
        ...
        [&] {
          ...
          apply::coalesceValuesKernel<scalar_t, cuda_accscalar_t>
              <<<grid, block, 0, stream>>>(
  ...
  // int64_t grid = min((int64_t) 1024, THCCeilDiv((int64_t) newNnz * stride,
  // (int64_t) block.x * block.y));
...
Tensor sparse_mask_helper_cuda(
  ...
  */
  ...
  auto t_flatten_indices =
      at::sparse::flatten_indices(t_indices, full_size).contiguous();
  ...
  // Step 4: Copy the Filtered `t._values()` using the matches at
  // `t_indices_pos`
