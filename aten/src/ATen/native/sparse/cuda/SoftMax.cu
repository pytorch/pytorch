...
std::tuple<Tensor, Tensor, Tensor, Tensor> compute_pool_max(
  ...
  */
  ...
  thrust_ptr sorted_indices_thrust_ptr(sorted_indices.template mutable_data_ptr<int64_t>());
  thrust::sequence(
  ...
  auto new_end = thrust::reduce_by_key(
      ...
      thrust_ptr(pool_sizes.template mutable_data_ptr<int64_t>()),
      [offsets_ptr] __device__(int64_t x, int64_t y) {
        return offsets_ptr[x] == offsets_ptr[y];
      });
  auto new_sz = thrust::distance(
      thrust_ptr(pool_sizes.template mutable_data_ptr<int64_t>()), new_end.second);
  pool_sizes.resize_({new_sz});

  auto pool_offsets = pool_sizes.clone();
  thrust_ptr pool_offsets_thrust_ptr(
      pool_offsets.template mutable_data_ptr<int64_t>());
  ...
  if (requireMxRows) {
    ...
    auto pool_sizes_ptr = pool_sizes.template mutable_data_ptr<int64_t>();
    auto sorted_indices_ptr = sorted_indices.template mutable_data_ptr<int64_t>();
    auto pool_offsets_ptr = pool_offsets.template mutable_data_ptr<int64_t>();
