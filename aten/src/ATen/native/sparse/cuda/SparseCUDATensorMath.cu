...
// NB: Deleted spaddcmul (aka addcmul_, but not actually wired up), spaddcdiv
// (not wired at all)

template <typename scalar_t>
void s_addmm_out_sparse_dense_cuda_worker(
    int64_t nnz,
    int64_t m,
    int64_t n,
    int64_t k,
  ...
  Tensor colIndicesInt =
      at::empty({colIndices.size(0)}, indices.options().dtype(kInt));
...
Tensor& s_addmm_out_sparse_dense_cuda(
    Tensor& r_,
    const Tensor& t,
  ...
  TORCH_CHECK(
      sparse_.is_cuda(), "addmm: expected 'mat1' to be CUDA, but got CPU");
  TORCH_CHECK(
      dense.is_cuda(), "addmm: expected 'mat2' to be CUDA, but got CPU");
  ...
  AT_DISPATCH_FLOATING_AND_COMPLEX_TYPES(
      values.scalar_type(), "addmm_sparse_cuda", [&] {
        s_addmm_out_sparse_dense_cuda_worker<scalar_t>(
            nnz, m, n, k, r_, beta, t, alpha, indices, values, dense);
      });
...
Tensor& addmm_out_sparse_dense_cuda(
  ...
  return s_addmm_out_sparse_dense_cuda(
      result, *b_self, mat1, mat2, beta, alpha);
}
...
SparseTensor& hspmm_out_sparse_cuda(
    ...
    /* , const Scalar& alpha */) {
  TORCH_CHECK(
      sparse_.is_cuda(), "hspmm: expected 'self' to be CUDA, but got CPU");
  TORCH_CHECK(r_.is_cuda(), "hspmm: expected 'out' to be CUDA, but got CPU");
  TORCH_CHECK(
      dense.is_cuda(), "hspmm: expected 'mat2' to be CUDA, but got CPU");
  ...
  s_addmm_out_sparse_dense_cuda(
      values, values, newSparse, dense, 0, /*alpha*/ 1);
  get_sparse_impl(r_)->set_indices_and_values_unsafe(indices, values);

  return r_;
}

SparseTensor hspmm_sparse_cuda(
    const SparseTensor& sparse,
    const Tensor& dense) {
...
Tensor& add_out_dense_sparse_cuda(
    Tensor& r_,
    const Tensor& dense,
    const SparseTensor& sparse,
  ...
  TORCH_CHECK(
      sparse.is_cuda(),
      "add: expected 'other' to be a CUDA tensor, but got a CPU tensor");
  TORCH_CHECK(
      r_.is_cuda(),
      "add: expected 'out' to be a CUDA tensor, but got a CPU tensor");
  ...
  if (is_same_tensor(r, dense_buffer)) {
    TORCH_CHECK(
        r_.is_contiguous(),
        "add: CUDA dense-sparse addition with a non-contiguous output tensor does not work; shout if you need it (see https://github.com/pytorch/pytorch/issues/1521 )");
  } else {
    r.resize_as_(dense);
  ...
  if (sparse.is_coalesced()) {
    ...
    if (sparse.dense_dim() == 0) {
      TORCH_CHECK(
          cuda::getApplyGrid(nnz, grid, curDevice),
          "add: Argument #0: tensor too large or too many dimensions");
      ...
      TORCH_CHECK(
          cuda::getApplyGrid(nnz * block.x, grid, curDevice),
          "add: Argument #0: tensor too large or too many dimensions");
...
SparseTensor& add_out_sparse_cuda(
...
SparseTensor& mul_out_sparse_cuda(
    const SparseTensor& t_,
    const SparseTensor& src_,
    SparseTensor& r_) {
  if (src_.dim() == 0) {
  ...
  TORCH_CHECK(
      ...
      "mul: expected 'self' and 'other' to have same size, but ",
      t_.sizes(),
      " != ",
      src_.sizes());
  ...
  const dim3 block =
      dim3(std::min(static_cast<int64_t>(cuda::getApplyBlock().x), valueSize));
  dim3 grid;
  int curDevice = -1;
  cudaGetDevice(&curDevice);
  cudaStream_t stream = at::cuda::getCurrentCUDAStream(curDevice);
  TORCH_CHECK(
      cuda::getApplyGrid(valueSize, grid, curDevice),
      "mul: Argument #0: tensor too large or too many dimensions");

  Tensor resultNnz = at::empty({1}, CUDA(kLong));
  AT_DISPATCH_ALL_TYPES_AND(
      at::ScalarType::Half, commonDtype, "mul_out_sparse_cuda", [&] {
        apply::valueSparseIntersectionKernel<
        ...
        apply::indexSparseIntersectionKernel<uint64_t, scalar_t>
            <<<1, 1, 0, stream>>>(
                ...
                static_cast<uint64_t>(t_nnz),
                static_cast<uint64_t>(s_nnz),
                reinterpret_cast<uint64_t*>(resultNnz.data_ptr()));
        C10_CUDA_KERNEL_LAUNCH_CHECK();
      });
  ...
  get_sparse_impl(r_)->set_nnz_and_narrow(
      cpu_resultNnz.accessor<int64_t, 1>()[0]);
...
Tensor _sparse_sum_backward_cuda(
    const Tensor& grad_,
    const SparseTensor& input_,
    IntArrayRef dims_to_sum) {
  TORCH_CHECK(
      grad_.is_cuda(),
      "_sparse_sum_backward_cuda: expected 'grad_' to be CUDA tensor, but got CPU tensor");
  TORCH_CHECK(
      input_.is_cuda(),
      "_sparse_sum_backward_cuda: expected 'input_' to be CUDA tensor, but got CPU tensor");
  ...
  if (sum_all_sparse_dim) {
    TORCH_CHECK(
        !grad_.is_sparse(),
        "_sparse_sum_backward_cuda: expected grad Tensor to be dense since all sparse dims are summed");
    ...
    TORCH_CHECK(
        grad_.is_sparse(),
        "_sparse_sum_backward_cuda: expected grad_ Tensor to be sparse, but got dense");
    ...
    if (!sum_sparse_dim) {
      ...
      thrust::lower_bound(
          policy,
          grad_indices_iter,
          grad_indices_iter + grad_nnz,
          input_indices_iter,
          input_indices_iter + input_nnz,
          input_indices_pos_iter);

      // config to run cuda kernel
      int64_t total_threads = input_nnz;
      const dim3 block = dim3(std::min(
          static_cast<int64_t>(cuda::getApplyBlock().x), total_threads));
      dim3 grid;
      TORCH_CHECK(
          cuda::getApplyGrid(total_threads, grid, curDevice),
          "_sparse_sum_backward_cuda: input too large or too many dimensions");
      ...
      AT_DISPATCH_FLOATING_AND_COMPLEX_TYPES_AND1(
          kHalf, grad_values.scalar_type(), "_sparse_sum_backward_cuda", [&] {
...
Tensor _bmm_sparse_cuda(
  ...
  return _bmm_out_sparse_cuda(self, mat2, deterministic, result);
    ...
    (defined(_MSC_VER) && CUSPARSE_VERSION < 11000))
__global__ void search_end_matrix_indices_cuda_kernel(
  ...
  if (target_mat_num >= num_matrices)
    return;
...
void search_end_matrix_indices(
    int64_t* mat_el_end_indices,
    int64_t num_matrices,
    const Tensor& indices_1D) {
  int curDevice = -1;
  cudaGetDevice(&curDevice);
  cudaStream_t stream = at::cuda::getCurrentCUDAStream(curDevice);
  ...
  search_end_matrix_indices_cuda_kernel<<<grid_size, block_size, 0, stream>>>(
      mat_el_end_indices, num_matrices, indices_1D_ti, num_elements);
  C10_CUDA_KERNEL_LAUNCH_CHECK();
...
Tensor& _bmm_out_sparse_cuda(
  ...
  TORCH_CHECK(
    ...
    (CUDART_VERSION >= 10010) // linux cuda >= 10.1 or windows cuda >= 11.0
...

  {
    ...
    search_end_matrix_indices(
        mat_el_end_indices_device, num_matrices, indices_dim0);
  ...
  AT_DISPATCH_FLOATING_AND_COMPLEX_TYPES(
      values.scalar_type(), "bmm_sparse_cuda", [&] {
        ...
        uint32_t* row_indices_start_ptr =
            reinterpret_cast<uint32_t*>(indices_dim1.data_ptr());
        uint32_t* col_indices_start_ptr =
            reinterpret_cast<uint32_t*>(indices_dim2.data_ptr());
        scalar_t* values_start_ptr =
            reinterpret_cast<scalar_t*>(values.data_ptr());
        scalar_t* mat2_start_ptr =
            reinterpret_cast<scalar_t*>(mat2_contig.data_ptr());
        scalar_t* result_start_ptr =
            reinterpret_cast<scalar_t*>(tmp_result.data_ptr());
        for (int64_t cur_mat_num = 0; (cur_mat_num < num_matrices);
          ...
          if (mat_el_end_idx != -1) {
            ...
            uint32_t* row_indices_ptr =
                &row_indices_start_ptr[mat_el_begin_idx];
            uint32_t* col_indices_ptr =
                &col_indices_start_ptr[mat_el_begin_idx];
            ...
            TORCH_CUDASPARSE_CHECK(cusparseCreateDnMat(
                ...
                CUSPARSE_ORDER_COL));
            scalar_t* result_ptr =
                &result_start_ptr[dim_i * dim_k * cur_mat_num];
