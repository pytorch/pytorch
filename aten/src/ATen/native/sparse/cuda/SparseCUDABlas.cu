...
// Using these APIs in any other systems will result in compile-time or run-time
// failures. Their support will be extended in the next releases.
...
void Xcoo2csr(const int* coorowind, int64_t nnz, int64_t m, int* csrrowptr) {
  ...
  TORCH_CUDASPARSE_CHECK(cusparseXcoo2csr(
      handle, coorowind, i_nnz, i_m, csrrowptr, CUSPARSE_INDEX_BASE_ZERO));
}
...
void _csrmm2(
  ...
  if (csrvala == nullptr || b == nullptr || c == nullptr)
    return;
  ...
  // In order to support int64 here, index pointers csrrowptra, csrcolinda have
  // to be passed as int64_t.
  TORCH_CHECK(
      (m <= INT_MAX) && (n <= INT_MAX) && (k <= INT_MAX) && (nnz <= INT_MAX) &&
          (ldb <= INT_MAX) && (ldc <= INT_MAX),
      "At the moment, cusparseSpMM only supports m, n, k, nnz, ldb, ldc with the bound [val] <= ",
      INT_MAX,
      ".",
  ...
  TORCH_CUDASPARSE_CHECK(cusparseCreateCsr(
      ...
      ma,
      ka,
      nnz, /* rows, cols, number of non zero elements */
  ...
  TORCH_CUDASPARSE_CHECK(cusparseCreateDnMat(
      ...
      kb,
      nb,
      ldb, /* rows, cols, leading dimension */
      b, /* values */
      cusparse_value_type, /* data type of values */
  ...
  TORCH_CUDASPARSE_CHECK(cusparseCreateDnMat(
      ...
      m,
      n,
      ldc, /* rows, cols, leading dimension */
      c, /* values */
      cusparse_value_type, /* data type of values */
      CUSPARSE_ORDER_COL /* memory layout, ONLY column-major is supported now */
      ));
  ...
  TORCH_CUDASPARSE_CHECK(cusparseSpMM_bufferSize(
      ...
      CUSPARSE_CSRMM_ALG1, /* default computing algorithm for CSR sparse matrix
                              format */
      &bufferSize /* output */
      ));
...
void csrmm2(
  ...
  TORCH_INTERNAL_ASSERT(
      false,
      "cusparse csr MM only supports data type of float, double, cfloat and cdouble.");
}
...
template <>
void csrmm2<c10::complex<double>>(
    char transa,
    char transb,
    int64_t m,
    int64_t n,
...
void Scsrmm2(
  ...
  TORCH_CHECK(
      (m <= INT_MAX) && (n <= INT_MAX) && (k <= INT_MAX) && (nnz <= INT_MAX) &&
          (ldb <= INT_MAX) && (ldc <= INT_MAX),
      "cusparseScsrmm2 only supports m, n, k, nnz, ldb, ldc with the bound [val] <= ",
      INT_MAX);
  int i_m = (int)m;
  int i_n = (int)n;
  int i_k = (int)k;
  ...
  TORCH_CUDASPARSE_CHECK(cusparseScsrmm2(
      handle,
      opa,
      opb,
      i_m,
...
void Dcsrmm2(
  ...
  TORCH_CHECK(
      (m <= INT_MAX) && (n <= INT_MAX) && (k <= INT_MAX) && (nnz <= INT_MAX) &&
          (ldb <= INT_MAX) && (ldc <= INT_MAX),
      "cusparseDcsrmm2 only supports m, n, k, nnz, ldb, ldc with the bound [val] <= ",
      INT_MAX);
  int i_m = (int)m;
  int i_n = (int)n;
  int i_k = (int)k;
  ...
  cusparseMatDescr_t desc;
  cusparseCreateMatDescr(&desc);
  TORCH_CUDASPARSE_CHECK(cusparseDcsrmm2(
      handle,
      opa,
      opb,
      i_m,
...
void Ccsrmm2(
    ...
    int64_t k,
    int64_t nnz,
    const complex_target_t* alpha,
    const complex_target_t* csrvala,
    int* csrrowptra,
    int* csrcolinda,
    const complex_target_t* b,
    int64_t ldb,
    const complex_target_t* beta,
    complex_target_t* c,
    int64_t ldc) {
  adjustLd(transb, m, n, k, &ldb, &ldc);
  cusparseOperation_t opa = convertTransToCusparseOperation(transa);
  ...
  TORCH_CHECK(
      (m <= INT_MAX) && (n <= INT_MAX) && (k <= INT_MAX) && (nnz <= INT_MAX) &&
          (ldb <= INT_MAX) && (ldc <= INT_MAX),
      "cusparseCcsrmm2 only supports m, n, k, nnz, ldb, ldc with the bound [val] <= ",
      INT_MAX);
  int i_m = (int)m;
  int i_n = (int)n;
  int i_k = (int)k;
  ...
  cusparseMatDescr_t desc;
  cusparseCreateMatDescr(&desc);
  TORCH_CUDASPARSE_CHECK(cusparseCcsrmm2(
      handle,
      opa,
      opb,
      i_m,
...
void Zcsrmm2(
  ...
  TORCH_CHECK(
      (m <= INT_MAX) && (n <= INT_MAX) && (k <= INT_MAX) && (nnz <= INT_MAX) &&
          (ldb <= INT_MAX) && (ldc <= INT_MAX),
      "cusparseZcsrmm2 only supports m, n, k, nnz, ldb, ldc with the bound [val] <= ",
      INT_MAX);
  int i_m = (int)m;
  int i_n = (int)n;
  int i_k = (int)k;
  ...
  cusparseMatDescr_t desc;
  cusparseCreateMatDescr(&desc);
  TORCH_CUDASPARSE_CHECK(cusparseZcsrmm2(
      handle,
      opa,
      opb,
      i_m,
...
void Xcsrsort_bufferSizeExt(
  ...
  auto handle = at::cuda::getCurrentCUDASparseHandle();
  TORCH_CUDASPARSE_CHECK(cusparseXcsrsort_bufferSizeExt(
      handle, i_m, i_n, i_nnz, csrRowPtr, csrColInd, pBufferSizeInBytes));
}
...
void Xcsrsort(
  ...
  cusparseMatDescr_t desc;
  cusparseCreateMatDescr(&desc);
  TORCH_CUDASPARSE_CHECK(cusparseXcsrsort(
      handle, i_m, i_n, i_nnz, desc, csrRowPtr, csrColInd, P, pBuffer));
  TORCH_CUDASPARSE_CHECK(cusparseDestroyMatDescr(desc));
}

void Xcoosort_bufferSizeExt(
    int64_t m,
    int64_t n,
    int64_t nnz,
    const int* cooRows,
...
void XcoosortByRow(
  ...
  auto handle = at::cuda::getCurrentCUDASparseHandle();
  TORCH_CUDASPARSE_CHECK(cusparseXcoosortByRow(
      handle, i_m, i_n, i_nnz, cooRows, cooCols, P, pBuffer));
}
