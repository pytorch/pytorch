...
SparseTensor& erfinv_out_sparse(SparseTensor& r, const SparseTensor& t) {
  TORCH_CHECK(r.is_sparse(), "Tensor should be sparse");
  TORCH_CHECK(t.is_sparse(), "Tensor should be sparse");
  TORCH_CHECK(
      !c10::isIntegralType(r.scalar_type(), /*includeBool=*/true),
...
SparseTensor erfinv_sparse(const SparseTensor& t) {
  auto result = get_result_tensor_for_unary_op(t);
  return erfinv_out_sparse(result, t);
}

SparseTensor& erfinv_sparse_(SparseTensor& t) {
  return erfinv_out_sparse(t, t);
}
