...
SparseTensor& tan_out_sparse(SparseTensor& r, const SparseTensor& t) {
  TORCH_CHECK(r.is_sparse(), "Tensor should be sparse");
  TORCH_CHECK(t.is_sparse(), "Tensor should be sparse");
  TORCH_CHECK(
      !c10::isIntegralType(r.scalar_type(), /*includeBool=*/true),
      "tan: result type cannot be Integral, got:",
      r.scalar_type());
  ...
  if (is_same_tensor(r, t)) {
    ...
    TORCH_CHECK(
        r.is_coalesced(),
        "tan: in-place on uncoalesced tensors is not supported");
  } else {
    copy_sparse_to_sparse_(r, t.coalesce());
  }
  r._values().tan_();
...
SparseTensor tan_sparse(const SparseTensor& t) {
  auto result = get_result_tensor_for_unary_op(t);
  return tan_out_sparse(result, t);
}

SparseTensor& tan_sparse_(SparseTensor& t) {
  return tan_out_sparse(t, t);
}
