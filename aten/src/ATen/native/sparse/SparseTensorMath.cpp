...
// --------------------------------------------------------------------
// expm1(SparseTensor, Scalar)
// --------------------------------------------------------------------

// In-place expm1 on uncoalesced tensors is not supported since the operation is not a linear map.
// Values of uncoalesced tensor corresponding to the same indices are summed
// and expm1(summed_value) != expm1(v1) + expm1(v2)
...
SparseTensor& expm1_out_sparse(SparseTensor& r, const SparseTensor& t) {
  ...
  if (is_same_tensor(r, t)) {
    // don't have in-place expm1 for uncoalesced input because coalesce() is not in-place
    TORCH_CHECK(r.is_coalesced(), "expm1: in-place on uncoalesced tensors is not supported");
  }
  else {
    copy_sparse_to_sparse_(r, t.coalesce());
  }
