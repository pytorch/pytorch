...
SparseTensor& div_out_sparse_scalar(
    ...
    c10::optional<c10::string_view> rounding_mode,
    SparseTensor& r) {
  return div_out_sparse_zerodim(
      t, wrapped_scalar_tensor(value), std::move(rounding_mode), r);
}
...
SparseTensor& add_out_sparse_contiguous(
  ...
  AT_DISPATCH_ALL_TYPES_AND_COMPLEX(commonDtype, "cadd_sparse", [&] {
...
SparseTensor& add_out_sparse_non_contiguous(
  ...
  // If `t` or `src` contains non-contiguous `values`,
  // `at::native::cpublas::axpy` doesn't work and we concat the indices and
  // values tensors instead.
  AT_DISPATCH_ALL_TYPES_AND_COMPLEX(commonDtype, "add_out_sparse_cpu", [&] {
...
Tensor mul_sparse(const Tensor& self, const Tensor& other) {
  ...
  auto result_options = self.is_sparse() ? self.options().dtype(commonDtype)
                                         : other.options().dtype(commonDtype);
...
Tensor _sparse_mm(const Tensor& mat1, const Tensor& mat2) {
...
SparseTensor& _sspaddmm_out_cpu(
  ...
  AT_DISPATCH_ALL_TYPES_AND_COMPLEX(values.scalar_type(), "sspmm", [&] {
    auto values_accessor = values.accessor<scalar_t, 1>();
    scalar_t* dense_ptr = dense.data_ptr<scalar_t>();
...
Tensor _sparse_sum_backward_cpu(
  ...
  if (sum_all_sparse_dim) {
    ...
    if (sum_dense_dim) {
      ...
      AT_ASSERT(
          dense_expand_size.size() ==
          static_cast<size_t>(input_values.dim() - 1));
      ...
      const auto copy_iter =
          TensorIteratorConfig()
              ...
              .declare_static_shape(
                  grad_values_expand.sizes(), /*squash_dims=*/0)
              .build();
      const auto device_type = kCPU;

      const auto gIv_data =
          reinterpret_cast<char*>(grad_input_values.data_ptr());
      const auto gOv_data =
          reinterpret_cast<char*>(grad_values_expand.data_ptr());
      const auto gIv_stride =
          (grad_input_values.strides()[0] * grad_input_values.element_size());
      const auto gOv_stride =
          (grad_values_expand.strides()[0] * grad_values_expand.element_size());
...
Tensor& bmm_out_sparse_cpu(
  ...
  AT_DISPATCH_ALL_TYPES_AND_COMPLEX(
      values.scalar_type(), "bmm_sparse_dense", [&] {
        for (int64_t cur_mat_num = 0; (cur_mat_num < num_matrices);
             cur_mat_num++) {
