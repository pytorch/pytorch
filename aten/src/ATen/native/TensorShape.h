...
inline int64_t get_num_splits(const Tensor& self, int64_t split_size, int64_t dim, bool drop_remainder) {
  ...
  if (split_size != 0) {
    ...
    num_splits = std::max<int64_t>((dim_size + (drop_remainder ? 0 : split_size - 1)) / split_size, 1);
  }
  return num_splits;
}
