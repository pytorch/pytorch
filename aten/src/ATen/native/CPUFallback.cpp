...
void cpu_fallback(const c10::OperatorHandle& op, torch::jit::Stack* stack, bool error_on_views,
  ...
  for (const auto idx : c10::irange(static_cast<int64_t>(arguments.size()))) {
    const auto& ivalue = arguments[idx];
    if (ivalue.isTensor()) {
      tensor_args.push_back(ivalue.toTensor());
      tensor_args_indices.push_back(static_cast<int>(idx));
      ...
      tensorlist_args_indices.push_back(static_cast<int>(idx));
      ...
      optional_tensorlist_args_indices.push_back(static_cast<int>(idx));
  ...
  for (const auto i : c10::irange(tensorlist_args_indices.size())) {
    ...
    if (alias_info != nullptr && alias_info->isWrite()) {
      // NOLINTNEXTLINE(facebook-hte-LocalUncheckedArrayBounds)
      const auto& cpu_tensor_vec = tensorlist_cpu_args[i].toTensorVector();
      for (const auto idx : c10::irange(tensorlist_args[i].size())) {
        if (!cpu_tensor_vec[idx].defined()) continue;
        at::_copy_from_and_resize(cpu_tensor_vec[idx], tensorlist_args[i][idx]);
      }
    }
  }
  ...
  for (const auto i : c10::irange(optional_tensorlist_args_indices.size())) {
    ...
    if (alias_info != nullptr && alias_info->isWrite()) {
      // NOLINTNEXTLINE(facebook-hte-LocalUncheckedArrayBounds)
      const auto& cpu_tensor_list = optional_tensorlist_cpu_args[i].toOptionalTensorList();
      for (const auto idx : c10::irange(optional_tensorlist_args[i].size())) {
        if (cpu_tensor_list[idx].has_value() && cpu_tensor_list[idx].value().defined()) {
          const std::optional<at::Tensor>& optional_tensor = optional_tensorlist_args[i][idx];
          at::_copy_from_and_resize(cpu_tensor_list[idx].value(), optional_tensor.value());
        }
      }
    }
  }
      ...
      if (tgt_device) {
        if (returns[idx].isTensor() && returns[idx].toTensor().defined()) {
          (*stack)[returns_begin + idx] =
            ...
            returns[idx].isTensorList() &&
            validate_tensor_list(returns[idx].toTensorList())) {
          const auto& cpu_tensor_vec = returns[idx].toTensorList().vec();
          std::vector<at::Tensor> tensors;
          tensors.reserve(cpu_tensor_vec.size());

          for (const auto& tensor : cpu_tensor_vec) {
