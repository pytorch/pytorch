...
static bool can_use_fused_mps_cross_entropy(
  ...
  // The fused kernel packs batch_size / vocab_size as uint32; fall back to the
  // reference decomposed path for extents that would overflow it. The typed
  // constant keeps the SymInt comparison unambiguous across platforms (a bare
  // LL literal is ambiguous where int64_t is long, e.g. Linux clang/gcc).
  constexpr int64_t kMaxU32 = 0xFFFFFFFF;
  if (self.sym_size(0) > kMaxU32 || self.sym_size(1) > kMaxU32) {
    return false;
  }
