...
native::SCATTER_GATHER_OP get_operator_enum(
...
Tensor index_select_backward(
  ...
  if (isTensorSubclassLike(index)) {
    return grad.new_zeros(self_sizes, grad.options())
...
(const Tensor& self,
 ...
 const Tensor& out) {
  ...
  if (globalContext().deterministicAlgorithms() &&
      self.device().type() == DeviceType::CUDA && self.dim() == 1) {
    TORCH_CHECK(
        index.dim() == 1 && src.dim() == 1,
        "index and src should be 1D tensors when self is a 1D tensor, "
        "but their dims are ",
        index.dim(),
        " and ",
        src.dim(),
        ", respectively");
    TORCH_CHECK(
        index.numel() == src.numel(),
        "index and src should have same number of elements for 1D tensors, "
        "but got ",
        index.numel(),
        " versus ",
        src.numel());
    TORCH_CHECK(
        dim == 0, "dim should be zero for 1D self tensor, but got ", dim);
    torch::List<c10::optional<Tensor>> indices;
    indices.reserve(1);
  ...
  if (grad_numel > 0) {
    ...
    for (const auto i : c10::irange(self.ndimension())) {
      ...
      if (i == dim) {
        ...
        sparse_ind[i] =
            at::arange(grad.size(i), self.options().dtype(at::kLong))
                .unsqueeze(1)
...
Tensor& nonzero_out_cpu(const Tensor& self, Tensor& result) {
  ...
  auto iter =
      TensorIteratorConfig().add_input(self).enforce_linear_iteration().build();
