...
Tensor& index_put_(
  ...
  if (!accumulate) {
    ...
    at::globalContext().alertNotDeterministic(
        "index_put_ with accumulate=False");
  }
...
Tensor& _index_copy_impl_(
  ...
  // The only different between the following  tensor iterator and that of
  ...
  index_copy_stub(
      iter.device_type(), iter, dim, self_dim_size, self_dim_stride);
...
Tensor& index_add_cpu_(
  ...
  if (self.dim() > 1) {
    ...
    AT_DISPATCH_ALL_TYPES_AND_COMPLEX_AND3(
        ...
        self.scalar_type(),
        "index_add_",
        [&self, &source, &dim, &index_contig, &numel, &alpha] {
...
Tensor gather_backward(
  ...
  if (globalContext().deterministicAlgorithms() && index.dim() == 1 &&
      self.dim() == 1) {
    TORCH_CHECK(
        index.numel() == grad.numel(),
        "index and grad should have same number of elements, "
        "but got ",
        index.numel(),
        " versus ",
        grad.numel());
    ...
    return at::zeros(self.sizes(), grad.options())
        .index_put_(indices, grad, true);
  }
...
static Tensor& masked_select_out_impl_cpu(
  ...
  if (use_serial_kernel) {
    auto iter = TensorIteratorConfig()
                    .set_check_mem_overlap(
                        false) // result is intenionally zero-strided above
                    .check_all_same_dtype(false)
                    .resize_outputs(false)
  ...
  // peformance. std::exclusive_scan(mask_long_data, mask_long_data +
