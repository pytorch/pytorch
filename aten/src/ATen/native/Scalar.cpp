...
Scalar item(const Tensor& self) {
  ...
  if (self.is_sparse()) {
    if (self.nse(false) == 0) return Scalar(0);
    if (self.is_coalesced()) return at::_local_scalar_dense(self.values(false));
    return at::_local_scalar_dense(self.values(false).sum());
