...
(const Tensor& input,
 ...
 std::optional<int64_t> divisor_override) {
  ...
  const int64_t kH = safe_downcast<int, int64_t>(kernel_size[0]);
  const int64_t kW = kernel_size.size() == 1 ? kH : safe_downcast<int, int64_t>(kernel_size[1]);

  TORCH_CHECK(stride.empty() || stride.size() == 1 || stride.size() == 2,
    "avg_pool2d: stride must either be omitted, a single int, or a tuple of two ints");
  const int64_t dH = stride.empty() ? kH : safe_downcast<int, int64_t>(stride[0]);
  const int64_t dW = stride.empty() ? kW : stride.size() == 1 ? dH : safe_downcast<int, int64_t>(stride[1]);

  TORCH_CHECK(padding.size() == 1 || padding.size() == 2,
    "avg_pool2d: padding must either be a single int, or a tuple of two ints");
  const int64_t padH = safe_downcast<int, int64_t>(padding[0]);
  const int64_t padW = padding.size() == 1 ? padH : safe_downcast<int, int64_t>(padding[1]);
