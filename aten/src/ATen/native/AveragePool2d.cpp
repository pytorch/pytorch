...
void avg_pool2d_out_cpu_template(
  ...
  AT_DISPATCH_FLOATING_TYPES_AND(
      at::ScalarType::Long, input.scalar_type(), "avg_pool2d_out_frame", [&] {
...
static void avg_pool2d_backward_out_frame(
  ...
  at::parallel_for(0, nInputPlane, 0, [&](int64_t start, int64_t end) {
    for (auto k = start; k < end; k++) {
      ...
      int64_t p;
      for (p = 0; p < nbatch; p++) {
        const scalar_t* ptr_gradOutput = gradOutput_data +
            p * nInputPlane * outputHeight * outputWidth +
            k * outputWidth * outputHeight;
        // NOLINTNEXTLINE(cppcoreguidelines-init-variables)
        int64_t xx, yy;

        scalar_t* ptr_gi = gradInput_data +
            p * nInputPlane * inputWidth * inputHeight +
            k * inputWidth * inputHeight;
        scalar_t* ptr_gradInput = gradInput_data +
            p * nInputPlane * inputWidth * inputHeight +
            k * inputWidth * inputHeight;
