...
Tensor autocast_to_fp16(const Tensor& self, bool cuda_enabled, bool cpu_enabled) {
  if (self.dtype() == at::ScalarType::Float &&
      ((self.device().is_cuda() && cuda_enabled) ||
      (self.device().is_cpu() && cpu_enabled))
      ) {
    return to_impl(
        self, at::ScalarType::Half, c10::nullopt, c10::nullopt, c10::nullopt, false, false, c10::nullopt);
  } else {
    return self;
  }
}
...
// (this is intended to be used internally by the JIT autocast implementation)
Tensor autocast_to_fp32(const Tensor& self, bool cuda_enabled, bool cpu_enabled) {
