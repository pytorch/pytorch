...
using max_pool2d_fn = void (*)(
    ...
    int dilationW,
    int dilationH);
using max_pool2d_backward_fn = void (*)(
...
static inline void pool2d_shape_check(
  ...
  if (memory_format == at::MemoryFormat::ChannelsLast) {
    ...
    TORCH_CHECK(
        (ndim == 3 && input.size(0) != 0 && valid_dims) ||
            (ndim == 4 && valid_dims && input.size(3) != 0),
...
static inline void max_pool2d_backward_shape_check(
    const Tensor& input,
...
static inline void avg_pool2d_backward_shape_check(
    const Tensor& input,
    const Tensor& gradOutput,
    int64_t nbatch,
    int kH,
...
static inline void pool3d_shape_check(
    const Tensor& input,
    int64_t nslices,
    int kT,
    int kH,
  ...
  TORCH_CHECK(
      ...
      dW);
  TORCH_CHECK(
      dilationT > 0 && dilationW > 0 && dilationH > 0,
      "dilation should be greater than zero, but got ",
      "dilationT: ",
      dilationT,
      " dilationH: ",
  ...
  TORCH_CHECK(
      input.numel() > 0 && (ndim == 4 || ndim == 5),
      "non-empty 4D or 5D (batch mode) tensor expected for input, but got ndim: ",
      ndim);

  if (check_input_size) { // AveragePool3d
    TORCH_CHECK(
        itime >= kT && iheight >= kH && iwidth >= kW,
        "input image ",
        "(T: ",
        itime,
        " H: ",
...
static inline void max_pool3d_backward_shape_check(
    const Tensor& input,
    const Tensor& gradOutput,
    const Tensor& indices,
    int64_t nslices,
...
static inline void avg_pool3d_backward_shape_check(
    const Tensor& input,
    const Tensor& gradOutput,
    int64_t nslices,
    int kT,
