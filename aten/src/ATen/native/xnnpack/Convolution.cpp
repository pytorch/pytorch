...
bool available(
  ...
  return xnnpack::available() &&
      ...
      (weight.size(Layout::Filter::width) > 0) && (weight.device().is_cpu()) &&
      (kFloat == weight.scalar_type()) &&
      // Bias
      (bias_sizes_opt.has_value()
           ? ((1 == bias_sizes_opt->size()) &&
...
bool usable(const Tensor& input) {
  // Input
  return (4 == input.ndimension()) && (input.device().is_cpu()) &&
...
// XNNPack's deconvolution operator expects weights to be indexed in the
// following order:
//   * Groups
...
//
// (ref:
// https://github.com/google/XNNPACK/blob/ecd8311c8fd3d9ab47edbc3df5f2b5de7dabe75f/test/deconvolution-operator-tester.h#L678)
//
// This function takes in a contiguous NHWC pytorch tensor (e.g. MemoryFormat ==
// ChannelsLast) and rearranges the weights in preparation for use with xnnpack.
// By default, for pytorch, transpose conv2d weights are {input_channels,
// output_Channels_per_group, kernel_height, kernel_width}. In addition, it
// condenses the tensor from 5 to 4 dimensions as expected by the rest of the
// pytorch framework by combining the groups and input_channels dimension.
const Tensor reorder_weights_for_transpose_conv(
    const Tensor& weight_nhwc,
    int num_groups) {
  TORCH_CHECK(
      weight_nhwc.size(0) % num_groups == 0,
  ...
  int g_offset = (output_channels_per_group) * (kernel_height) *
      (kernel_width) * (input_channels_per_group);
      ...
      for (const auto w : c10::irange(kernel_width)) {
        for (const auto h : c10::irange(kernel_height)) {
          for (const auto i : c10::irange(input_channels_per_group)) {
            int in_index = (g * g_offset) + (i * i_offset) + (h * h_offset) +
                (w * w_offset) + (o * o_offset);
...
ContextConv2D create(
  ...
  TORCH_CHECK(
      available(
          ...
          (bias.has_value() && bias->defined())
              ? at::OptionalIntArrayRef(bias->sizes())
              : c10::nullopt,
          padding_expanded,
          stride_expanded,
          dilation_expanded,
  ...
  if (transposed) {
    const Tensor weight_reordered =
        reorder_weights_for_transpose_conv(weight_nhwc, groups);
    ...
    create_status = xnn_create_deconvolution2d_nhwc_f32(
        ...
        groups, // groups
        weight_reordered.size(Layout::Filter::output) /
        ...
        weight_reordered.size(Layout::Filter::input) *
...
c10::intrusive_ptr<xnnpack::Conv2dOpContext> createConv2dClampPrePackOpContext(
    Tensor weight,
...
// Op is registered to have Any argument as we plan to reuse it for prepacked
// conv2d of other backends
