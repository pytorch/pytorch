...
inline DimVector shape_from_dim_mask(
    const Tensor& self,
    DimMask mask,
    bool keepdim) {
...
static std::vector<int64_t> get_zero_numel_tensor_size(
  ...
  TORCH_INTERNAL_ASSERT(
      self.numel() == 0, fn_name, ": Expected self.numel() == 0.");
...
// Resize the result tensor and indices when result.numel() == 0 depending on
// values of dim and keepdim for returning tensors containing reduction results.
// This function should be called when you are reducing a zero-numel tensor and
// want to resize the output and return it. This function exists for resizing
// zero-numel tensors when the size of the reduction dimension is non-zero.
static C10_UNUSED void zero_numel_tensor_resize(
    Tensor& result,
...
static C10_UNUSED DimVector
get_reduction_shape(const Tensor& self, IntArrayRef dims, bool keepdim) {
...
static TensorIterator make_reduction(
  ...
  auto mask = at::native::make_dim_mask(dims, ndim);
  auto viewed_result1 =
      at::native::review_reduce_result(result1, ndim, mask, keepdim);
  auto viewed_result2 =
      at::native::review_reduce_result(result2, ndim, mask, keepdim);
  // special case for type promotion in mixed precision, improves computational
  // efficiency. We don't generalize this to common mismatched input/output
  // types to avoid cross product of templated kernel launches.
  if (self.scalar_type() == dtype1 ||
      (self.is_cuda() && self.scalar_type() == kHalf && dtype1 == kFloat)) {
    return TensorIterator::reduce_op(viewed_result1, viewed_result2, self);
  }
