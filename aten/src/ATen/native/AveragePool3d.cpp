 ...
 c10::optional<int64_t> divisor_override) {
  ...
  if (input.ndimension() == 4) {
    set_output_raw_strided(
        0, {nslices, otime, oheight, owidth}, {}, input.options());
  } else {
    set_output_raw_strided(
        0, {nbatch, nslices, otime, oheight, owidth}, {}, input.options());
  }
}
 ...
 const Tensor& output) {
  ...
  if (input.ndimension() == 4) /* non-batch mode */
  {
    AT_DISPATCH_FLOATING_TYPES_AND(
        at::ScalarType::Long, input.scalar_type(), "avg_pool3d_out_frame", [&] {
        ...
        for (i = 0; i < oheight; i++) {
          for (j = 0; j < owidth; j++) {
            ...
            for (z = tstart; z < tend; z++) {
              for (y = hstart; y < hend; y++) {
                for (x = wstart; x < wend; x++) {
                  *(ip + z * iheight * iwidth + y * iwidth + x) +=
                      val / divide_factor;
                }
              }
            }
