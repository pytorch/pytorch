...
TORCH_META_FUNC(avg_pool3d) (
  ...
  const int kH = kernel_size.size() == 1 ? kD : c10::checked_convert<int>(kernel_size[1], "int");
  const int kW = kernel_size.size() == 1 ? kD : c10::checked_convert<int>(kernel_size[2], "int");
  ...
  const int padH = padding.size() == 1 ? padD : c10::checked_convert<int>(padding[1], "int");
  const int padW = padding.size() == 1 ? padD : c10::checked_convert<int>(padding[2], "int");
...
void avg_pool3d_out_cpu_legacy(
  const Tensor& input_,
  IntArrayRef kernel_size,
  IntArrayRef stride,
  IntArrayRef padding,
...
Tensor avg_pool3d_legacy_cpu(
  ...
  Tensor output;
  if (input.ndimension() == 4) {
    output = input.new_empty({nslices, otime, oheight, owidth});
  } else {
    output = input.new_empty(
        {input.size(0), nslices, otime, oheight, owidth},
        input.options().memory_format(input.suggest_memory_format()));
  }

  avg_pool3d_out_cpu_legacy(
      input,
      kernel_size,
      stride,
      padding,
...
void avg_pool3d_backward_out_cpu_legacy(
  const Tensor& gradOutput_,
  const Tensor& input,
  IntArrayRef kernel_size,
  IntArrayRef stride,
...
Tensor avg_pool3d_backward_legacy_cpu(
  ...
  Tensor grad_input = input.new_empty(
      input.sizes(),
      input.options().memory_format(input.suggest_memory_format()));
  avg_pool3d_backward_out_cpu_legacy(
      grad_output,
      input,
      kernel_size,
      stride,
