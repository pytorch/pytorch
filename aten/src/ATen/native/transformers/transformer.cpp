...
Tensor transformer_encoder_layer_forward(
  ...
  {
    const Tensor& check_for_empty =
        src.is_nested() ? get_nested_tensor_impl(src)->get_buffer() : src;
    if (check_for_empty.numel() == 0) {
      return src.is_nested()
          ? at::detail::make_tensor<NestedTensorImpl>(
                check_for_empty,
                get_nested_tensor_impl(src)->get_nested_size_tensor())
          : src.clone();
    }
  }
...
std::tuple<Tensor, Tensor, Tensor> transformer_decoder_only_layer_forward(
  ...
  return std::make_tuple(
      std::move(x), std::move(incr_key_out), std::move(incr_value_out));
}
