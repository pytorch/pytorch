...
std::tuple<Tensor, Tensor, Tensor, Tensor, Tensor> _flash_attention_forward(
  ...
  */
  ...
  std::tie(logsumexp, philox_seed, philox_offset, debug_attn_mask) =
      fmha::mha_fwd(
          query,
          key,
          value,
...
std::tuple<at::Tensor, at::Tensor> _efficient_attention_forward(
  ...
  // !!TODO_DRISS: We are throwing this away for now and need to change how its
  // done uint64_t -> int64_t bitwise casting as PyTorch don't support uint64_t
...
// !!This function is deprecated. See FunctionsManual.cpp for the
// implementation!!
