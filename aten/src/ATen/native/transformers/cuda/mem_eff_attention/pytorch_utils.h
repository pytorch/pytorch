 ...
 * are:
 * - (n_queries, n_keys)
 * - (batch_sz * n_heads, n_queries, n_keys) # I think we can expect users to do the splitting
 * - (batch_sz, n_heads, 1, n_keys)
 *
...
inline at::Tensor get_bias_4d_view(
    ...
    int n_queries,
    int n_keys) {
    return bias.expand({batch_sz, n_heads, n_queries, n_keys});
  // TORCH_CHECK(
