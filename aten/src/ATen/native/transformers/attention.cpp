...
Tensor scaled_dot_product_attention(
  ...
  switch (backend) {
    ...
    case SDPBackend::flash_attention: {
      ...
      } else if (query_device_type == DeviceType::XPU) {
        c10::SymInt og_size = query_.sym_size(-1);
        Tensor query_padded = pad_last_dim<64, false>(query_);
        Tensor key_padded = pad_last_dim<64, false>(key);
        Tensor value_padded = pad_last_dim<64, false>(value);
        auto og_scale = sdp::calculate_scale(query_, scale);
        auto out_lse_softmax = at::_scaled_dot_product_flash_attention(
            query_padded, key_padded, value_padded, dropout_p, is_causal, false /*return_debug_mask*/, og_scale.guard_float("attention.cpp", 763));
        return post_process_flash_output(std::get<0>(out_lse_softmax), og_size);
      }
