...
Tensor affine_grid_generator_4D(const Tensor& theta, int64_t N, int64_t C, int64_t H, int64_t W) {
...
Tensor affine_grid_generator(const Tensor& theta, IntList size) {
    ...
    if (size.size() == 4) {
      ...
      return affine_grid_generator_5D(theta, size[0], size[1], size[2], size[3], size[4]);
    }
}

Tensor affine_grid_generator_4D_backward(const Tensor& grad_grid,
      int64_t N, int64_t C, int64_t H, int64_t W) {
  auto base_grid = make_base_grid_4D(grad_grid, N, C, H, W);
  IntList exp_size = {N, H, W, 2};
...
Tensor affine_grid_generator_5D_backward(const Tensor& grad_grid,
