...
Tensor affine_grid_generator(const Tensor& theta, IntListRef size) {
  AT_CHECK(
...
Tensor affine_grid_generator_4D_backward(
  ...
  AT_ASSERT(grad_grid.sizes() == IntListRef({N, H, W, 2}));
...
Tensor affine_grid_generator_5D_backward(
  ...
  AT_ASSERT(grad_grid.sizes() == IntListRef({N, D, H, W, 3}));
...
Tensor affine_grid_generator_backward(const Tensor& grad, IntListRef size) {
  AT_CHECK(
      size.size() == 4 || size.size() == 5,
      "AffineGridGenerator needs 4d (spatial) or 5d (volumetric) inputs.");
  if (size.size() == 4) {
