    ...
    IntArrayRef padding, IntArrayRef stride, IntArrayRef dilation, int64_t groups)
{
  ...
  for (auto s : stride) {
      TORCH_CHECK(s > 0, "Stride must be greater than 0, got ", s);
  }
  for (auto d : dilation) {
      TORCH_CHECK(d > 0, "Dilation must be greater than 0, got ", d);
  }
  for (auto p : padding) {
      TORCH_CHECK(p >= 0, "Padding must be non-negative, got ", p);
  }
