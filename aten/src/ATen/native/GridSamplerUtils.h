...
void check_grid_sampler_common(
  ...
  TORCH_CHECK(
      input.defined(), "grid_sampler(): expected input to not be undefined");
  TORCH_CHECK(
      grid.defined(), "grid_sampler(): expected grid to not be undefined");
  TORCH_CHECK(
...
bool cond_cudnn_grid_sampler(const TensorBase& input, const TensorBase& grid) {
  return (
      ...
      at::native::canUse32BitIndexMath(grid) && input.dim() == 4 &&
      input.size(1) <= 1024);
}
