...
// sampled at points (y_1, ..., y_n) that are separated by distance (dx_1, ...,
// dx_{n-1}), is given by the trapezoid rule:
//
...
// but with '1's padded in the beginning to match the target number of
// dimensions. For example, curr_shape = (5,5,5) and target_n_dim = 6 ==>
// (1,1,1,5,5,5) Note that no padding will be added if the current shape has the
// greater than or equal number of dimensions than the target numbers of
// dimensions.
...
Tensor trapezoid(const Tensor& y, const Tensor& x, int64_t dim) {
  ...
  TORCH_CHECK(
      y.scalar_type() != kBool && x.scalar_type() != kBool,
      "trapezoid: received a bool input for `x` or `y`, but bool is not supported")
  Tensor x_viewed;
  // Note that we explicitly choose not to broadcast 'x' to match the shape of
  // 'y' here because we want to follow NumPy's behavior of broadcasting 'dx'
  // and 'dy' together after the differences are taken.
  if (x.dim() == 1) {
    // This step takes 'x' with dimension (n,), and returns 'x_view' with
    // dimension (1,1,...,n,...,1,1) based on dim and y.dim() so that, later on,
    // 'dx' can be broadcast to match 'dy' at the correct dimensions.
    TORCH_CHECK(
        x.size(0) == y.size(dim),
    ...
    // When 'y' has more dimension than 'x', this step takes 'x' with dimension
    // (n_1, n_2, ...), and add '1's as dimensions in front to become (1, 1,
    // ..., n_1, n_2), matching the dimension of 'y'. This allows the subsequent
    // slicing operations to proceed with any 'dim' without going out of bound.
...
Tensor cumulative_trapezoid(const Tensor& y, const Tensor& x, int64_t dim) {
  ...
  TORCH_CHECK(
      y.scalar_type() != kBool && x.scalar_type() != kBool,
      "cumulative_trapezoid: received a bool input for `x` or `y`, but bool is not supported")
  Tensor x_viewed;
  if (x.dim() == 1) {
    // See trapezoid for implementation notes
    TORCH_CHECK(
        x.size(0) == y.size(dim),
        "cumulative_trapezoid: There must be one `x` value for each sample point");
    DimVector new_sizes(y.dim(), 1); // shape = [1] * y.
    new_sizes[dim] = x.size(0); // shape[axis] = d.shape[0]
    x_viewed = x.view(new_sizes);
  } else if (x.dim() < y.dim()) {
...
Tensor cumulative_trapezoid(const Tensor& y, const Scalar& dx, int64_t dim) {
  TORCH_CHECK(
      y.scalar_type() != kBool,
      "cumulative_trapezoid: received a bool input for `y`, but bool is not supported")
  TORCH_CHECK(
      !(dx.isComplex() || dx.isBoolean()),
      "cumulative_trapezoid: Currently, we only support dx as a real number.");
