...
void replication_pad2d_out_cpu_template(
    Tensor& output,
    const Tensor& input_,
    IntArrayRef paddingSize) {
  ...
  TORCH_CHECK(
      input_.numel() > 0 && (input_.dim() == 3 || input_.dim() == 4),
...
void replication_pad3d_out_cpu_template(
  ...
  shapeCheck3d(input_, pleft, pright, ptop, pbottom, pfront, pback);
...
Tensor& replication_pad1d_out_cpu(
    ...
    const Tensor& input,
    IntArrayRef paddingSize) {
  replication_pad1d_out_cpu_template(output, input, paddingSize);
  return output;
}

Tensor replication_pad1d_cpu(const Tensor& input, IntArrayRef paddingSize) {
...
Tensor& replication_pad2d_out_cpu(
    ...
    const Tensor& input,
    IntArrayRef paddingSize) {
  replication_pad2d_out_cpu_template(output, input, paddingSize);
  return output;
}

Tensor replication_pad2d_cpu(const Tensor& input, IntArrayRef paddingSize) {
  auto output = at::empty({0}, input.options());
  replication_pad2d_out_cpu_template(output, input, paddingSize);
  return output;
}
...
Tensor& replication_pad3d_out_cpu(
    ...
    const Tensor& input,
    IntArrayRef paddingSize) {
  replication_pad3d_out_cpu_template(output, input, paddingSize);
  return output;
}

Tensor replication_pad3d_cpu(const Tensor& input, IntArrayRef paddingSize) {
  auto output = at::empty({0}, input.options());
  replication_pad3d_out_cpu_template(output, input, paddingSize);
  return output;
}
