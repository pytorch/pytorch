...
(const Tensor& input, IntArrayRef paddingSize) {
  ...
  if (input.dim() == 3) {
    ...
    set_output_raw_strided(
        0, {nbatch, nslices, oheight, owidth}, {}, input.options());
  }
}
...
(const Tensor& input, IntArrayRef paddingSize) {
  ...
  if (input.dim() == 4) {
    set_output_raw_strided(
        0, {nslices, odepth, oheight, owidth}, {}, input.options());
  } else {
    set_output_raw_strided(
        0, {nbatch, nslices, odepth, oheight, owidth}, {}, input.options());
  }
}
  ...
  at::parallel_for(0, nbatch, 0, [&](int64_t start, int64_t end) {
    for (const auto p : c10::irange(start, end)) {
      ...
      replication_pad1d_out_frame(
          input_p, output_p, nslices, iwidth, owidth, pad_l);
    }
  });
}
  ...
  at::parallel_for(0, nbatch, 0, [&](int64_t start, int64_t end) {
    for (const auto p : c10::irange(start, end)) {
      ...
      replication_pad1d_backward_out_frame(
          ginput_p, goutput_p, nslices, iwidth, owidth, pad_l);
    }
  });
}
    ...
    AT_DISPATCH_FLOATING_AND_COMPLEX_TYPES(
        input.scalar_type(), "replication_pad1d_cpu", [&] {
          ...
          replication_pad1d_out_frame<scalar_t>(
              input_data, output_data, nslices, iwidth, owidth, pad_l);
        });
  } else {
    AT_DISPATCH_FLOATING_AND_COMPLEX_TYPES(
        input.scalar_type(), "replication_pad1d_cpu", [&] {
          ...
          replication_pad1d_out_batch<scalar_t>(
              input_data, output_data, nslices, iwidth, owidth, pad_l, nbatch);
        });
  }
}
    ...
    AT_DISPATCH_FLOATING_AND_COMPLEX_TYPES(
        input.scalar_type(), "replication_pad1d_backward_cpu", [&] {
          ...
          replication_pad1d_backward_out_frame<scalar_t>(
              gradInput_data, gradOutput_data, nslices, iwidth, owidth, pad_l);
        });
  } else {
    AT_DISPATCH_FLOATING_AND_COMPLEX_TYPES(
        input.scalar_type(), "replication_pad1d_backward_cpu", [&] {
