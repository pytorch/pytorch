...
#define INSTANTIATE(scalar_t)                                          \
  ...
  template bool gemv_use_fast_path<scalar_t>(                          \
      int64_t m, int64_t n, int64_t lda, int64_t incx, int64_t incy);  \
  template void gemv_fast_path<scalar_t>(                              \
      char* trans,                                                     \
      int* m,                                                          \
      int* n,                                                          \
      scalar_t* alpha,                                                 \
      ...
      scalar_t* y,                                                     \
      int* incy);                                                      \
  template void scal_fast_path<scalar_t>(                              \
...
bool gemv_use_fast_path<float>(
    ...
    int64_t incy) {
  auto intmax = std::numeric_limits<int>::max();
  return (m <= intmax) && (n <= intmax) && (lda <= intmax) && (incx > 0) &&
      (incx <= intmax) && (incy > 0) && (incy <= intmax);
}
...
void gemv(
  ...
  if (blas_impl::gemv_use_fast_path<scalar_t>(m, n, lda, incx, incy)) {
    ...
    int i_incx = (int)incx;
    int i_incy = (int)incy;
    blas_impl::gemv_fast_path<scalar_t>(
        &trans, &i_m, &i_n, &alpha, a, &i_lda, x, &i_incx, &beta, y, &i_incy);
    return;
  }
  ...
  if ((trans == 'T') || (trans == 't')) {
    ...
    if (beta != scalar_t(1) && beta != scalar_t(0))
    ...
    for (const auto j : c10::irange(n)) {
      ...
      for (const auto i : c10::irange(m)) {
        // output values are ignored if beta is 0, and set to 0, nans and infs
        // are not propagated
...
c10::complex<float> vdot_fast_path(
    int n,
    c10::complex<float>* x,
    int incx,
    c10::complex<float>* y,
...
c10::complex<double> vdot_fast_path(
    int n,
...
c10::complex<double> dot_fast_path(
    int n,
    c10::complex<double>* x,
    int incx,
    c10::complex<double>* y,
...
c10::complex<float> dot_fast_path(
    int n,
    c10::complex<float>* x,
    int incx,
    c10::complex<float>* y,
...
scalar_t dot_impl_floating(
  ...
  if ((n <= INT_MAX) && (incx <= INT_MAX) && (incy <= INT_MAX)) {
    ...
    return blas_impl::dot_naive(
        n, x, incx, y, incy, std::multiplies<scalar_t>{});
  }
#else
  {
    return blas_impl::dot_naive(
