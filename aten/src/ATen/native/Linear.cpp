...
Tensor linear(
  ...
  if (bias->defined()) {
    ...
    if (isTensorSubclassLike(*bias) || bias->_fw_grad(/*level*/ 0).defined()) {
...
Tensor& linear_out(
    ...
    Tensor& output) {
  TORCH_CHECK(
      !input.is_mkldnn(), "linear doesn't support out for MKLDNN tensors");
  // See [Note: hacky wrapper removal for optional tensor]
  auto bias = bias_opt.has_value()
      ? c10::MaybeOwned<Tensor>::borrowed(*bias_opt)
      : c10::MaybeOwned<Tensor>::owned(c10::in_place);
...
Tensor _trilinear(
    ...
    const Tensor& i2_,
    const Tensor& i3_,
    IntArrayRef expand1_,
    IntArrayRef expand2_,
    IntArrayRef expand3_,
    IntArrayRef sumdim_,
    int64_t unroll_dim) {
  ...
  // Three conditionals are necessary since this function is meant to work for
  // both forward and backward, which changes the dimensions of the inputs. Note
  // that if output has zero elems is because (at least) one of i1, i2, i3 has
  // zero elems.
...
// implements tensordot, a matrix-multiplication-like contraction, but the
// dimensions given in the two dimension lists
Tensor tensordot(
    const Tensor& input1,
    const Tensor& input2,
    IntArrayRef dims1,
