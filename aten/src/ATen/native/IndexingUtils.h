...
static C10_UNUSED std::vector<Tensor> expandTensors(
    const Tensor& self,
...
inline torch::List<c10::optional<Tensor>> toListOfOptionalTensors(
  ...
  for (const IValue& a : list) {
    result.push_back(
        a.isTensor() ? c10::optional<Tensor>(a.toTensor())
                     : c10::optional<Tensor>());
  }
  return result;
}
...
static C10_UNUSED std::tuple<Tensor, std::vector<Tensor>> transposeToFront(
    Tensor self,
    TensorList indices) {
