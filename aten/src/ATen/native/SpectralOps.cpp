...
Tensor stft(const Tensor& self, const int64_t n_fft, const optional<int64_t> hop_length_,
            const optional<int64_t> win_length_, const Tensor& window,
  ...
  std::cout<<"calling stft" << std::endl;
  // default_init hop_length and win_length
  auto hop_length = hop_length_.value_or(n_fft >> 2);
  auto win_length = win_length_.value_or(n_fft);
