...
Tensor tensor(const Type& dtype) {
  if (_type_has_native(dtype)) {
    return static_cast<const TypeInternalInterface&>(dtype).native_tensor();
  } else {
    return static_cast<const TypeInternalInterface&>(dtype).th_tensor();
  }
}

Tensor tensor(const Type& dtype, ArrayRef<int64_t> size) {
  if (_type_has_native(dtype)) {
    return static_cast<const TypeInternalInterface&>(dtype).native_tensor(size);
  } else {
    return static_cast<const TypeInternalInterface&>(dtype).th_tensor(size);
  }
}

Tensor sparse_coo_tensor(const Type& dtype, ArrayRef<int64_t> size) {
  return static_cast<const TypeInternalInterface&>(dtype.toSparse()).native_sparse_coo_tensor(size);
}

Tensor sparse_coo_tensor(const Tensor& indices, const Tensor& values) {
  return static_cast<const TypeInternalInterface&>(values.type().toSparse()).native_sparse_coo_tensor(indices, values);
}

Tensor sparse_coo_tensor(const Tensor& indices, const Tensor& values, ArrayRef<int64_t> size) {
  return static_cast<const TypeInternalInterface&>(values.type().toSparse()).native_sparse_coo_tensor(indices, values, size);
}

Tensor _sparse_coo_tensor_unsafe(const Tensor& indices, const Tensor& values, ArrayRef<int64_t> size) {
  return static_cast<const TypeInternalInterface&>(values.type().toSparse())._native_sparse_coo_tensor_unsafe(indices, values, size);
}
