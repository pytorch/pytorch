...
/// `sobolstate`.
std::tuple<Tensor, Tensor> _sobol_engine_draw(
    const Tensor& quasi,
    int64_t n,
  ...
  TORCH_CHECK(
      sobolstate.dtype() == at::kLong,
      "sobolstate needs to be of type ",
      at::kLong);
  TORCH_CHECK(
      quasi.dtype() == at::kLong, "quasi needs to be of type ", at::kLong);

  Tensor wquasi = quasi.clone();
  auto result_dtype = dtype.has_value() ? dtype.value() : at::kFloat;
  Tensor result =
      at::empty({n, dimension}, sobolstate.options().dtype(result_dtype));
  ...
  AT_DISPATCH_FLOATING_TYPES(result_dtype, "_sobol_engine_draw", [&]() -> void {
    ...
    int64_t sobolstate_row_stride = sobolstate.stride(0),
            sobolstate_col_stride = sobolstate.stride(1);
    int64_t result_row_stride = result.stride(0),
    ...
    for (int64_t i = 0; i < n; i++, num_generated++) {
      ...
      for (int64_t j = 0; j < dimension; j++) {
        wquasi_data[j * wquasi_stride] ^= sobolstate_data
            [j * sobolstate_row_stride + l * sobolstate_col_stride];
        result_data[i * result_row_stride + j * result_col_stride] =
            wquasi_data[j * wquasi_stride];
      }
    }
  });
...
/// its state variables (`sobolstate` and `quasi`). `dimension` can be
/// inferred from `sobolstate`, but is passed as an argument for the same
/// reasons specified above.
Tensor& _sobol_engine_ff_(
    Tensor& quasi,
    int64_t n,
    const Tensor& sobolstate,
    int64_t dimension,
  ...
  for (int64_t i = 0; i < n; i++, num_generated++) {
    l = rightmost_zero(num_generated);
    for (int64_t j = 0; j < dimension; j++) {
      quasi_data[j * quasi_stride] ^= sobolstate_data
          [j * sobolstate_row_stride + l * sobolstate_col_stride];
    }
  }
  return quasi;
...
/// This is an implicit function used for randomizing the state variables of
/// the. `SobolEngine`. Arguments are a randomized `sobolstate` state variables
/// and a list of random lower triangular matrices consisting of 0s and 1s.
/// `dimension` is passed explicitly again.
Tensor& _sobol_engine_scramble_(
    Tensor& sobolstate,
    const Tensor& ltm,
    int64_t dimension) {
  TORCH_CHECK(
  ...
  /// Require a dot product of every row with a specific vector of each of the
  /// matrices in `ltm`. Instead, we perform an element-wise product of all the
  /// matrices and sum over the last dimension. The required product of the
  /// m^{th} row in the d^{th} square matrix in `ltm` can be accessed using
  /// ltm_d_a[d][m] m and d are zero-indexed
...
/// This is a core function to initialize the main state variable of a
/// `SobolEngine`. `dimension` is passed explicitly as well (see why above)
Tensor& _sobol_engine_initialize_state_(Tensor& sobolstate, int64_t dimension) {
  ...
  Tensor pow2s = at::pow(
      2, at::native::arange((MAXBIT - 1), -1, -1, sobolstate.options()));
