 ...
 *
 * See the docstr for torch.histogramdd in torch/functional.py for further
 * explanation.
 *
 * - torch.histogramdd(input, bins, range=None, weight=None, density=False)
 *   input     - tensor with shape (M, N). input is interpreted as M coordinates
 * in N-dimensional space. If a tensor with more than 2 dimensions is passed,
 * all but the last dimension will be flattened. bins      - int[] of length N
 * or tensor list of length N. If int[], defines the number of equal-width bins
 *               in each dimension. If tensor list, defines the sequences of bin
 * edges, including rightmost edges, for each dimension. range     - float[] of
 * length 2 * N, optional. If specified, defines the leftmost and rightmost bin
 * edges for each dimension. weight    - tensor, optional. If provided, weight
 * should have the same shape as input excluding its last dimension. Each
 * N-dimensional value in input contributes its associated weight towards its
 * bin's result. If weight is not specified, each value has weight 1 by default.
 *   density   - bool, optional. If false (default), the result will contain the
 * total count (weight) in each bin. If True, each count (weight) is divided by
 * the total count (total weight), then divided by the volume of its associated
 * bin.
 *
 * Returns:
 *   hist      - N-dimensional tensor containing the values of the histogram.
 *   bin_edges - tensor list of length N containing the edges of the histogram
 * bins in each dimension. Bins include their left edge and exclude their right
 * edge, with the exception of the rightmost bin in each dimension which
 * includes both of its edges.
 *
 * Restrictions are defined in histogram_check_inputs() and in
...
void infer_bin_edges_from_input(
    const Tensor& input,
    const int64_t N,
...
std::pair<std::vector<double>, std::vector<double>> select_outer_bin_edges(
  ...
  if (range.has_value()) {
    ...
    AT_DISPATCH_FLOATING_TYPES(input.scalar_type(), "histogramdd", [&]() {
      infer_bin_edges_from_input<scalar_t>(
 ...
 */
std::pair<double, double> histc_select_outer_bin_edges(
    const Tensor& input,
    const Scalar& min,
    const Scalar& max) {
...
Tensor& histogramdd_out_cpu(
  ...
  histogramdd_stub(
      self.device().type(), self, weight, density, hist, bin_edges);
  return hist;
}

Tensor histogramdd_cpu(
    const Tensor& self,
    TensorList bins,
    const c10::optional<Tensor>& weight,
    bool density) {
 ...
 */
std::vector<Tensor>& histogramdd_bin_edges_out_cpu(
    const Tensor& self,
    IntArrayRef bin_ct,
    c10::optional<c10::ArrayRef<double>> range,
    const c10::optional<Tensor>& weight,
...
std::vector<Tensor> histogramdd_bin_edges_cpu(
    const Tensor& self,
    IntArrayRef bin_ct,
    c10::optional<c10::ArrayRef<double>> range,
    const c10::optional<Tensor>& weight,
    bool density) {
  std::vector<Tensor> bin_edges_out = allocate_bin_edges_tensors(self);
  return histogramdd_bin_edges_out_cpu(
...
Tensor& histogramdd_out_cpu(
  ...
  std::vector<Tensor> bins =
      histogramdd_bin_edges_cpu(self, bin_ct, range, weight, density);
  ...
  histogramdd_linear_stub(
      self.device().type(), self, weight, density, hist, bin_edges, true);
  return hist;
}
...
Tensor histogramdd_cpu(
  ...
  histogramdd_out_cpu(
      self, bin_ct, range, weight, density, hist, bin_edges_out_tl);
  return hist;
}
 ...
 * edges.
 */
std::tuple<Tensor&, Tensor&> histogram_out_cpu(
    const Tensor& self,
    const Tensor& bins,
    const c10::optional<Tensor>& weight,
    bool density,
...
std::tuple<Tensor, Tensor> histogram_cpu(
    const Tensor& self,
    const Tensor& bins,
    const c10::optional<Tensor>& weight,
    bool density) {
 ...
 */
Tensor& histogram_histc_cpu_out(
    const Tensor& self,
    int64_t bin_ct,
    const Scalar& min,
    const Scalar& max,
  ...
  histogramdd_prepare_out(
      reshaped, std::vector<int64_t>{bin_ct}, hist, bins_out);

  auto outer_bin_edges = histc_select_outer_bin_edges(self, min, max);
  linspace_out(
      outer_bin_edges.first, outer_bin_edges.second, bin_ct + 1, bin_edges);

  histogramdd_check_inputs(reshaped, bins_in, {});

  histogramdd_linear_stub(
      reshaped.device().type(),
      reshaped,
      c10::optional<Tensor>(),
      false,
      hist,
...
std::tuple<Tensor, std::vector<Tensor>> histogramdd(
    ...
    bool density) {
  auto bin_edges =
      at::_histogramdd_bin_edges(self, bins, range, weight, density);
