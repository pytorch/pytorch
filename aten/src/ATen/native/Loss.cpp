...
Tensor smooth_l1_loss(const Tensor& input, const Tensor& target, const int64_t reduction, float beta = 1.0) {
  Tensor loss;
...
Tensor& smooth_l1_loss_backward_out(Tensor& grad_input, const Tensor& grad_output, const Tensor& input, const Tensor& target, int64_t reduction, float beta = 1.0) {
  auto norm = reduction == Reduction::Mean ? 1. / input.numel() : 1.;
...
Tensor smooth_l1_loss_backward(const Tensor& grad_output, const Tensor& input, const Tensor& target, int64_t reduction, float beta = 1.0) {
  auto grad_input = at::zeros_like(input, LEGACY_CONTIGUOUS_MEMORY_FORMAT);
