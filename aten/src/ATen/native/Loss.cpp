...
(const Tensor& input,
 ...
 double beta) {
  ...
  // TODO: Reduce this extra TensorIterator construction for Reduction::Mean &
  // Sum. We do another TensorIterator construction in the IMPL for the two
  // cases.
...
Tensor kl_div_backward_cpu(
  ...
  if (!log_target) {
    ...
    AT_DISPATCH_FLOATING_TYPES(
        input.scalar_type(), "kl_div_backward_cpu", [&]() {
          cpu_serial_kernel(
              iter, [](scalar_t target_val, scalar_t grad_val) -> scalar_t {
...
Tensor& binary_cross_entropy_out_cpu(
  ...
  AT_DISPATCH_FLOATING_TYPES(loss.scalar_type(), "binary_cross_entropy", [&] {
    at::native::cpu_kernel(iter, [](scalar_t input_val, scalar_t target_val) {
      TORCH_CHECK(
      ...
      return (target_val - scalar_t(1)) *
          std::max(
                 scalar_t(std::log(scalar_t(1) - input_val)), scalar_t(-100)) -
...
Tensor& binary_cross_entropy_backward_out_cpu(
  ...
  AT_DISPATCH_FLOATING_TYPES(
      grad_input.scalar_type(), "binary_cross_entropy_backward", [&] {
...
Tensor binary_cross_entropy_with_logits(
  ...
  c10::MaybeOwned<Tensor> weight_maybe_owned =
      at::borrow_from_optional_tensor(weight_opt);
  const Tensor& weight = *weight_maybe_owned;
  c10::MaybeOwned<Tensor> pos_weight_maybe_owned =
...
Tensor poisson_nll_loss(
  ...
  if (full) {
    auto stirling_term = target * at::log(target) - target +
        0.5 * at::log(2 * c10::pi<double> * target);
...
Tensor& soft_margin_loss_backward_out(
  ...
  // inplace version of: grad_input = -norm * target * z / (1. + z) *
  // grad_output;
...
Tensor soft_margin_loss_backward(
  ...
  at::soft_margin_loss_backward_out(
...
Tensor smooth_l1_loss_backward(
    ...
    double beta) {
  auto grad_input = at::zeros_like(input, LEGACY_CONTIGUOUS_MEMORY_FORMAT);
  return at::smooth_l1_loss_backward_out(
      grad_input, grad_output, input, target, reduction, beta);
}
...
Tensor huber_loss_backward(
  ...
  return at::huber_loss_backward_out(
      grad_input, grad_output, input, target, reduction, delta);
}
