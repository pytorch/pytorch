...
// Shape inference + full input validation for `_scaled_mm_v2`. Runs in eager
// (before TORCH_IMPL_FUNC) and during `torch.compile` tracing, so the same
// errors that the C++ kernel would otherwise raise late surface here. The
// output dtype defaults to `mat_a`'s dtype when `out_dtype` is unset, matching
// the legacy functional.
