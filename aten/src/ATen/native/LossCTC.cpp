...
Tensor ctc_loss_backward_cpu_template(
  ...
  Tensor grad = at::full_like(
      log_probs, neginf); // at this point, this is log of empty sum
  ...
  Tensor log_beta =
      at::empty_like(log_alpha); // could be optimized to use only 2 rows
  auto lpp = log_probs.permute({1, 0, 2});
  auto log_probs_a_global = lpp.accessor<scalar_t, 3>();
  auto log_alpha_a_global = log_alpha.accessor<scalar_t, 3>();
  ...
  at::parallel_for(0, batch_size, 0, [&](int64_t start, int64_t end) {
    for (int64_t b = start; b < end; b++) {
      ...
      if (input_length < max_input_length) {
        grad.narrow(0, input_length, max_input_length - input_length)
            .narrow(1, b, 1)
            .zero_();
      }
    }
...
Tensor ctc_loss(
  ...
  bool use_cudnn = detail::getCUDAHooks().compiledWithCuDNN() &&
      (detail::getCUDAHooks().versionCuDNN() >= 7000) &&
      ctx.userEnabledCuDNN() && (BLANK == 0) && (targets.dim() == 1) &&
...
Tensor ctc_loss(
  ...
  Tensor tlc =
      target_lengths.toType(kLong).toBackend(Backend::CPU).contiguous();
