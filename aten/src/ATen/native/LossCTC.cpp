...
// (log_probs and log_alpha). The function returns the loss and the alphas, the
// alphas are kept for the backward step. The wrapper (ctc_loss below) hides the
// alphas from the user by only returning the loss.
...
std::tuple<Tensor, Tensor> ctc_loss_cpu_template(
  ...
  TORCH_CHECK(
      (int64_t)input_lengths.size() == batch_size,
      "input_lengths must be of size batch_size");
  TORCH_CHECK(
      (int64_t)target_lengths.size() == batch_size,
  ...
  // alpha calculation for the first row, the three equations for alpha_1 above
  // eq (6) first the default
  ...
  at::parallel_for(0, batch_size, 0, [&](int64_t start, int64_t end) {
    for (const auto b : c10::irange(start, end)) {
      ...
      if (target_length > 0)
        log_alpha_a[0][1] = log_probs_a[0][get_target_prime(
            targets_data, tg_batch_offset, tg_target_stride, 1, BLANK)];

      // now the loop over the inputs
      for (const auto t : c10::irange(1, input_length)) {
        for (const auto s : c10::irange(2 * target_length + 1)) {
          auto current_target_prime = get_target_prime(
              targets_data, tg_batch_offset, tg_target_stride, s, BLANK);
          // this loop over s could be parallel/vectorized, too, but the
          // required items are one index apart alternatively, one might
          // consider moving s to the outer loop to cache current_target_prime
          // more (but then it needs to be descending) for the cuda
          // implementation, that gave a speed boost. This is eq (6) and (7),
          // la1,2,3 are the three summands. We keep track of the maximum for
          // the logsumexp calculation.
          ...
          log_alpha_a[t][s] =
              std::log(
                  ...
                  std::exp(la3 - lamax)) +
              lamax + log_probs_a[t][current_target_prime];
        }
      }
...
// a) computing the beta analogous to the alphas in the forward (backward half
// of the forward-backward algorithm) (eq (10) and (11)) b) collecting the
// per-activation characters for all s and wrapping the gradient (eq (16), the
// collection is the sum)
template <typename scalar_t, ScalarType target_scalar_type>
Tensor ctc_loss_backward_cpu_template(
  ...
  Tensor grad = at::full_like(
      log_probs,
      neginf,
      LEGACY_CONTIGUOUS_MEMORY_FORMAT); // at this point, this is log of empty
                                        // sum
  ...
  Tensor log_beta = at::empty_like(
      log_alpha,
      LEGACY_CONTIGUOUS_MEMORY_FORMAT); // could be optimized to use only 2 rows
  auto lpp = log_probs.permute({1, 0, 2});
  auto log_probs_a_global = lpp.accessor<scalar_t, 3>();
  auto log_alpha_a_global = log_alpha.accessor<scalar_t, 3>();
  ...
  auto create_fill_iterator = [](const Tensor& tensor,
  ...
  const auto fill_log_beta_1d_iter =
      create_fill_iterator(log_beta, /*squash_dims=*/{0, 1});
  ...
  at::parallel_for(0, batch_size, 0, [&](int64_t start, int64_t end) {
    ...
    for (const auto b : c10::irange(start, end)) {
      ...
      // here we do the fill for each batch item separately, as the input
      // lengths will differ, so the t in which we start varies
      if (input_length > 0) {
        ...
        log_beta_a[input_length - 1][2 * target_length] =
            log_probs_a[input_length - 1][BLANK];
        grad_a[input_length - 1][BLANK] =
            log_alpha_a[input_length - 1][2 * target_length] +
            log_beta_a[input_length - 1][2 * target_length];
        ...
        if (target_length > 0) {
          auto current_target_prime = get_target_prime(
              ...
              2 * target_length - 1,
              BLANK);
          log_beta_a[input_length - 1][2 * target_length - 1] =
              log_probs_a[input_length - 1][current_target_prime];

          // the first two are a blank and a non-blank, so we know they are
          // different and we don't need to do log+
          grad_a[input_length - 1][current_target_prime] =
              log_alpha_a[input_length - 1][2 * target_length - 1] +
              log_beta_a[input_length - 1][2 * target_length - 1];
        }
      }
      ...
      for (int64_t t = input_length - 2; t >= 0; t--) {
        // this loop over s could be parallel/vectorized and doesn't really need
        // to be descending... alternatively, one might consider moving s to the
        // outer loop to cache current_target_prime more (but then it needs to
        // be descending) for the cuda implementation, that gave a speed boost.
        for (int64_t s = 2 * target_length; s >= 0; s--) {
          ...
          log_beta_a[t][s] = std::log(
                                 std::exp(lb1 - lbmax) + std::exp(lb2 - lbmax) +
                                 std::exp(lb3 - lbmax)) +
              lbmax + log_probs_a[t][current_target_prime];
          // one might check whether one can vectorize this better when done
          // after the t-loop... now that we have beta, we fill in the sum of
          // alpha*beta in eq (16) in contrast to the cuda implementation, we
          // only parallelize over the batch, so we don't have a concurrency
          ...
          if (lcab == neginf) {
            ...
            lcab = std::log(
                       std::exp(lcab - max) + std::exp(log_alpha_beta - max)) +
                max;
          }
        }
      }
      ...
      // now we wrap up the calculation by adding in the remaining items of eq
      // (16) this could be a great target for further vectorization. grad is
      // the output gradient, nll is the loss. Note that the likelihood -nll is
      // the Z of eq (16)
      scalar_t gr = grad_out.accessor<scalar_t, 1>()[b];
      for (const auto t :
           c10::irange(input_length)) { // or go for the full thing?
...
std::tuple<Tensor, Tensor> ctc_loss_cpu(
    const Tensor& log_probs,
    const Tensor& targets,
    IntArrayRef input_lengths,
    IntArrayRef target_lengths,
  ...
  return AT_DISPATCH_FLOATING_TYPES(
      log_probs.scalar_type(), "ctc_loss_cpu", [&] {
        if (targets.scalar_type() == kLong) {
          return ctc_loss_cpu_template<scalar_t, kLong>(
              log_probs, targets, input_lengths, target_lengths, BLANK);
        } else {
          return ctc_loss_cpu_template<scalar_t, kInt>(
              log_probs, targets, input_lengths, target_lengths, BLANK);
        }
      });
}

Tensor ctc_loss_backward_cpu(
    const Tensor& grad,
    const Tensor& log_probs,
    const Tensor& targets,
    IntArrayRef input_lengths,
    ...
    bool zero_infinity) {
  return AT_DISPATCH_FLOATING_TYPES(
      log_probs.scalar_type(), "ctc_loss_backward_cpu", [&] {
        if (targets.scalar_type() == kLong) {
          return ctc_loss_backward_cpu_template<scalar_t, kLong>(
              grad,
              log_probs,
              targets,
              input_lengths,
          ...
          return ctc_loss_backward_cpu_template<scalar_t, kInt>(
              grad,
              log_probs,
              targets,
              input_lengths,
...
// this wrapper function dispatches to the native and cudnn implementations and
// hides the alpha/grad from the user (by just returning the loss) the gradient
// is implemented for _cudnn_ctc_loss (just in derivatives.yaml) and _ctc_loss
// and this function has automatic gradients it also handles the reduction if
// desired
Tensor ctc_loss(
