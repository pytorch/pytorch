...
static std::vector<Dimname> compute_dot_product_outnames(
  ...
  int64_t index = 0;
  for (const auto j : c10::irange(tensor_names.size())) {
    if (static_cast<int64_t>(j) == tensor_dotted_dim) continue;
    outnames[index++] = tensor_names[j];
  }
  for (const auto j : c10::irange(other_names.size())) {
    if (static_cast<int64_t>(j) == other_dotted_dim) continue;
