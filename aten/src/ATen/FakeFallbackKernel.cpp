...
bool cpp_meta_supports_symint(const c10::OperatorHandle& op) {
  static const std::unordered_set<c10::OperatorHandle> allowlist = {
      ...
      c10::Dispatcher::singleton().findSchemaOrThrow("aten::stack", ""),
  };
...
void fakeFallback(
  ...
  if (can_decompose) {
    ...
    if (!has_python_cia && op.hasKernelForDispatchKey(
  ...
  try {
    // Meta dispatch falls through to CEA when there is no direct Meta kernel.
    // Call it directly so nested tensor ops can still re-enter Fake dispatch.
    if (!is_builtin(op) &&
        !op.hasKernelForDispatchKey(c10::DispatchKey::Meta)) {
      for (const auto key : {
               c10::DispatchKey::CompositeExplicitAutogradNonFunctional,
               c10::DispatchKey::CompositeExplicitAutograd}) {
        if (op.hasKernelForDispatchKey(key)) {
          op.callBoxedForDispatchKey(key, *stack);
          wrap_meta_outputs_with_default_device_logic();
          return;
        }
      }
    ...
    if (op.hasTag(at::Tag::nondeterministic_seeded)) {
      ...
      // Select Meta for this op without excluding Fake globally so tensor ops
      // inside Python Meta kernels can re-enter the fake fallback.
      run_meta();
    }
  } catch (...) {
