...
void transmute_to_fake(
    ...
    const std::shared_ptr<c10::FakeTensorMode>& mode) {
  at::set_and_normalize_fake_device(t.unsafeGetTensorImpl(), fake_device);
  if (mode) {
    t.unsafeGetTensorImpl()->set_fake_tensor_mode(mode);
  }
}

bool is_definitely_one(const c10::SymInt& value) {
  return !value.is_heap_allocated() && value.expect_int() == 1;
}

std::optional<c10::SymDimVector> infer_binary_shape_without_guards(
    c10::SymIntArrayRef a,
    c10::SymIntArrayRef b) {
  const auto ndim = std::max(a.size(), b.size());
  c10::SymDimVector shape(ndim);
  for (const auto offset : c10::irange(ndim)) {
    const auto out_dim = ndim - offset - 1;
    const auto a_dim = a.size() > offset ? a.size() - offset - 1 : 0;
    const auto b_dim = b.size() > offset ? b.size() - offset - 1 : 0;
    const auto a_size = a.size() > offset ? a[a_dim] : c10::SymInt(1);
    const auto b_size = b.size() > offset ? b[b_dim] : c10::SymInt(1);
    if (a_size.is_same(b_size) || is_definitely_one(b_size)) {
      shape[out_dim] = a_size;
    } else if (is_definitely_one(a_size)) {
      shape[out_dim] = b_size;
    } else {
      return std::nullopt;
    }
  }
...
bool try_run_symbolic_binary_fast_path(
    const c10::OperatorHandle& op,
    torch::jit::Stack* stack,
    size_t arguments_begin,
    size_t num_arguments,
    c10::Device common_device) {
  static const auto add = c10::Dispatcher::singleton().findSchemaOrThrow(
      "aten::add", "Tensor");
  static const auto sub = c10::Dispatcher::singleton().findSchemaOrThrow(
      "aten::sub", "Tensor");
  static const auto mul = c10::Dispatcher::singleton().findSchemaOrThrow(
      "aten::mul", "Tensor");
  static const auto div = c10::Dispatcher::singleton().findSchemaOrThrow(
      "aten::div", "Tensor");
  ...
  if (arguments.size() < 2 || !arguments[0].isTensor() ||
      !arguments[1].isTensor()) {
    return false;
  }
  const auto& lhs = arguments[0].toTensor();
  const auto& rhs = arguments[1].toTensor();
  if (lhs.layout() != c10::Layout::Strided ||
      rhs.layout() != c10::Layout::Strided) {
    return false;
  }

  auto shape = infer_binary_shape_without_guards(lhs.sym_sizes(), rhs.sym_sizes());
  if (!shape.has_value()) {
    shape = at::infer_size_symdimvector(lhs.sym_sizes(), rhs.sym_sizes());
  }

  std::optional<c10::MemoryFormat> memory_format;
  if (common_device.type() == c10::DeviceType::HPU ||
      (lhs.is_contiguous_or_false(c10::MemoryFormat::Contiguous) &&
       rhs.is_contiguous_or_false(c10::MemoryFormat::Contiguous))) {
    memory_format = c10::MemoryFormat::Contiguous;
  } else if (
      lhs.is_contiguous_or_false(c10::MemoryFormat::ChannelsLast) &&
      rhs.is_contiguous_or_false(c10::MemoryFormat::ChannelsLast)) {
  ...
  state = at::native::update_result_type_state(lhs, state);
  state = at::native::update_result_type_state(rhs, state);
  auto dtype = at::native::result_type(state);
  if (is_div && c10::isIntegralType(dtype, true)) {
    dtype = c10::get_default_dtype_as_scalartype();
  }
  if (is_sub) {
    at::native::sub_check(lhs, rhs);
  }
  if ((is_add || is_sub) && arguments.size() > 2) {
    at::native::alpha_check(dtype, arguments[2].toScalar());
  }

  at::Tensor out(at::detail::empty_generic_symint(
      *shape,
      c10::GetAllocator(c10::DeviceType::Meta),
      c10::DispatchKeySet(c10::DispatchKey::Meta),
      dtype,
      memory_format));
  stack->resize(arguments_begin);
  stack->emplace_back(std::move(out));
  return true;
}
...
// WARNING: do not add namespaces here for operators outside pytorch/pytorch.
// The fallback does not always work and MAY CRASH with unreadable errors.
// Mirrors FakeTensorMode.can_run_unsafe_fallback.
bool can_run_unsafe_fallback(
    const c10::OperatorHandle& op,
    const std::shared_ptr<c10::FakeTensorMode>& mode) {
  if (mode && !mode->allow_fallback_kernels_) {
    return false;
  }
...
void set_constant_on_mode(
  ...
  if (!mode || !constant)
    return;
  mode->set_constant(fake_tensor.unsafeGetTensorImpl(), std::move(constant));
}
...
void maybe_run_unsafe_fallback(
  ...
  TORCH_CHECK(
      !has_symints && can_run_unsafe_fallback(op, mode),
...
// Owns the real-op result computed before dispatch under propagate_real_tensors
// and stamps it onto the fake outputs on any normal exit of the dispatch
// (skipping exception paths, where outputs are invalid).
struct PropagateRealTensorsGuard {
  ...
  ~PropagateRealTensorsGuard() noexcept(false) {
    if (get() == nullptr || std::uncaught_exceptions() > entry_uncaught) {
      return;
    }
    (*interp)->propagate_real_tensors(*op, get(), stack);
  }

  // borrowed; nullptr if the real op did not run
  PyObject* get() const {
    return real.ptr(interp);
  }

  const c10::OperatorHandle* op;
  torch::jit::Stack* stack;
  c10::impl::PyInterpreter* interp;
  c10::SafePyObject real;
  int entry_uncaught;
};
...
void fakeFallback(
  ...
  auto* interp = c10::impl::getGlobalPyInterpreter();
  // Match Python FakeTensorMode: under propagate_real_tensors, run the real op
  // once before any fake kernel, then reuse its output for missing-fake-kernel
  // inference and for stamping reals onto the fake outputs.
  // A fake arg without a shadow real makes run_real_op bail out, so skip the
  // Python call.
  const bool do_propagate_real_tensors = mode &&
      mode->propagate_real_tensors_ && interp != nullptr &&
      std::all_of(
          flat_arg_fake_tensors.begin(),
          flat_arg_fake_tensors.end(),
          [](const at::Tensor& t) {
            return t.unsafeGetTensorImpl()->real_tensor() != nullptr;
          });
  PropagateRealTensorsGuard propagate_guard{
      ...
      do_propagate_real_tensors
          ? (*interp)->fake_run_real_op(
                op, torch::jit::last(*stack, num_arguments).vec())
          : nullptr};

  if (has_symints && mode && !device_mismatch.has_value() &&
      try_run_symbolic_binary_fast_path(
          op,
          stack,
          arguments_begin,
          num_arguments,
  ...
  // Missing-fake-kernel inference only applies to custom ops; checking for
  // torch._library.utils.is_builtin namespaces here avoids a Python call per op.
  const bool is_builtin = op_ns.has_value() &&
      (*op_ns == "aten" || *op_ns == "prim" || *op_ns == "prims");
  if (propagate_guard.get() && mode->shape_env_ && !is_builtin &&
      (*interp)->fake_infer_from_real_out(op, stack, propagate_guard.get())) {
    return;
  }
  ...
  if (mode && interp &&
    ...
    if ((*interp)->fake_try_custom_op_impl(op, stack, propagate_guard.get())) {
      return;
    }
  }
  ...
  try {
    ...
    if (meta_keyset.has(c10::DispatchKey::Meta)) {
      ...
      at::sparse_csr::CheckSparseTensorInvariants invariants_guard(false);
      run_meta();
    }
  } catch (...) {
    auto eptr = std::current_exception();

    // For NotImplementedError, raised by a C++ or Python meta kernel, try the
    // unsafe fallback. For other errors, rethrow.
    ...
    try {
      ...
      is_not_implemented =
          interp && (*interp)->fake_is_not_implemented_error(eptr);
    }
