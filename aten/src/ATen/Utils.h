...
static inline std::vector<TensorImpl*> checked_tensor_list_unwrap(ArrayRef<Tensor> tensors, const char * name, int pos, Backend backend, ScalarType scalar_type) {
  ...
  for (unsigned int i = 0; i < tensors.size(); ++i) {
    ...
    if (dispatchKeyToBackend(c10::impl::dispatchTypeId(expr.type_set())) != backend) {
      AT_ERROR("Expected object of backend ", backend, " but got backend ", dispatchKeyToBackend(c10::impl::dispatchTypeId(expr.type_set())),
