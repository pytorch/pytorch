...
int check__getDevice(uint32_t devIndex, ANeuralNetworksDevice** device) {
  ...
  // TODO: Maybe add better logging here.
  CAFFE_ENFORCE(
      ret == ANEURALNETWORKS_NO_ERROR, "_getDevice", "failed with error ", ret);
  return ret;
}
int check_Device_getName(
...
int check_Device_getFeatureLevel(
    const ANeuralNetworksDevice* device,
...
int check_Model_getSupportedOperationsForDevices(
    const ANeuralNetworksModel* model,
    const ANeuralNetworksDevice* const* devices,
    uint32_t numDevices,
    bool* supportedOps) {
  CAFFE_ENFORCE(nnapi_.Model_getSupportedOperationsForDevices);
  int ret = nnapi_.Model_getSupportedOperationsForDevices(
      model, devices, numDevices, supportedOps);
  // TODO: Maybe add better logging here.
  CAFFE_ENFORCE(
      ret == ANEURALNETWORKS_NO_ERROR,
      "Model_getSupportedOperationsForDevices",
      "failed with error ",
      ret);
  return ret;
}
int check_Compilation_createForDevices(
    ANeuralNetworksModel* model,
    const ANeuralNetworksDevice* const* devices,
    uint32_t numDevices,
    ANeuralNetworksCompilation** compilation) {
  CAFFE_ENFORCE(nnapi_.Compilation_createForDevices);
  int ret = nnapi_.Compilation_createForDevices(
      model, devices, numDevices, compilation);
  // TODO: Maybe add better logging here.
  CAFFE_ENFORCE(
      ret == ANEURALNETWORKS_NO_ERROR,
      "Compilation_createForDevices",
      "failed with error ",
      ret);
  return ret;
}
...
int check_Model_setOperandValue(
    ANeuralNetworksModel* model,
    int32_t index,
    const void* buffer,
    size_t length) {
  ...
  CAFFE_ENFORCE(
      ret == ANEURALNETWORKS_NO_ERROR,
      "Model_setOperandValue",
      "failed with error ",
      ret);
  return ret;
}
int check_Model_setOperandValueFromMemory(
    ANeuralNetworksModel* model,
    int32_t index,
  ...
  int ret = nnapi_.Model_setOperandValueFromMemory(
      model, index, memory, offset, length);
  // TODO: Maybe add better logging here.
  CAFFE_ENFORCE(
      ret == ANEURALNETWORKS_NO_ERROR,
      "Model_setOperandValueFromMemory",
      "failed with error ",
      ret);
  return ret;
}
int check_Model_addOperation(
    ANeuralNetworksModel* model,
  ...
  int ret = nnapi_.Model_addOperation(
...
int check_Model_identifyInputsAndOutputs(
    ANeuralNetworksModel* model,
    uint32_t inputCount,
    const uint32_t* inputs,
    uint32_t outputCount,
  ...
  int ret = nnapi_.Model_identifyInputsAndOutputs(
      model, inputCount, inputs, outputCount, outputs);
  // TODO: Maybe add better logging here.
  CAFFE_ENFORCE(
      ret == ANEURALNETWORKS_NO_ERROR,
      "Model_identifyInputsAndOutputs",
      "failed with error ",
      ret);
  return ret;
}
int check_Model_relaxComputationFloat32toFloat16(
    ANeuralNetworksModel* model,
    bool allow) {
  ...
  CAFFE_ENFORCE(
      ret == ANEURALNETWORKS_NO_ERROR,
      "Model_relaxComputationFloat32toFloat16",
      "failed with error ",
      ret);
  return ret;
}
int check_Compilation_create(
    ANeuralNetworksModel* model,
    ANeuralNetworksCompilation** compilation) {
...
int check_Compilation_setPreference(
    ANeuralNetworksCompilation* compilation,
    int32_t preference) {
...
int check_Execution_setInputFromMemory(
    ANeuralNetworksExecution* execution,
    int32_t index,
    const ANeuralNetworksOperandType* type,
    const ANeuralNetworksMemory* memory,
  ...
  int ret = nnapi_.Execution_setInputFromMemory(
      execution, index, type, memory, offset, length);
  // TODO: Maybe add better logging here.
  CAFFE_ENFORCE(
      ret == ANEURALNETWORKS_NO_ERROR,
      "Execution_setInputFromMemory",
      "failed with error ",
      ret);
  return ret;
}
int check_Execution_setOutput(
    ANeuralNetworksExecution* execution,
    int32_t index,
    const ANeuralNetworksOperandType* type,
    void* buffer,
...
int check_Execution_setOutputFromMemory(
    ANeuralNetworksExecution* execution,
    int32_t index,
    const ANeuralNetworksOperandType* type,
    const ANeuralNetworksMemory* memory,
  ...
  int ret = nnapi_.Execution_setOutputFromMemory(
      execution, index, type, memory, offset, length);
  // TODO: Maybe add better logging here.
  CAFFE_ENFORCE(
      ret == ANEURALNETWORKS_NO_ERROR,
      "Execution_setOutputFromMemory",
      "failed with error ",
      ret);
  return ret;
}
int check_Execution_startCompute(
    ANeuralNetworksExecution* execution,
...
int check_Event_wait(ANeuralNetworksEvent* event) {
  ...
  // TODO: Maybe add better logging here.
  CAFFE_ENFORCE(
      ret == ANEURALNETWORKS_NO_ERROR, "Event_wait", "failed with error ", ret);
  return ret;
}
...
int check_Execution_getOutputOperandRank(
    ANeuralNetworksExecution* execution,
    int32_t index,
    uint32_t* rank) {
  ...
  CAFFE_ENFORCE(
      ret == ANEURALNETWORKS_NO_ERROR,
      "Execution_getOutputOperandRank",
      "failed with error ",
      ret);
  return ret;
}
int check_Execution_getOutputOperandDimensions(
    ANeuralNetworksExecution* execution,
    int32_t index,
    uint32_t* dimensions) {
  CAFFE_ENFORCE(nnapi_.Execution_getOutputOperandDimensions);
  int ret =
      nnapi_.Execution_getOutputOperandDimensions(execution, index, dimensions);
  // TODO: Maybe add better logging here.
  CAFFE_ENFORCE(
      ret == ANEURALNETWORKS_NO_ERROR,
      "Execution_getOutputOperandDimensions",
      "failed with error ",
      ret);
  return ret;
}
void nnapi_wrapper_load(
  ...
  if (!loaded) {
    ...
    *(void**)&nnapi_._getDeviceCount =
        dlsym(handle, "ANeuralNetworks_getDeviceCount");
    ...
    *(void**)&nnapi_.Device_getName =
        dlsym(handle, "ANeuralNetworksDevice_getName");
    check_nnapi_.Device_getName = check_Device_getName;
    *(void**)&nnapi_.Device_getVersion =
        dlsym(handle, "ANeuralNetworksDevice_getVersion");
    check_nnapi_.Device_getVersion = check_Device_getVersion;
    *(void**)&nnapi_.Device_getFeatureLevel =
        dlsym(handle, "ANeuralNetworksDevice_getFeatureLevel");
    check_nnapi_.Device_getFeatureLevel = check_Device_getFeatureLevel;
    *(void**)&nnapi_.Model_getSupportedOperationsForDevices =
        dlsym(handle, "ANeuralNetworksModel_getSupportedOperationsForDevices");
    check_nnapi_.Model_getSupportedOperationsForDevices =
        check_Model_getSupportedOperationsForDevices;
    *(void**)&nnapi_.Compilation_createForDevices =
        dlsym(handle, "ANeuralNetworksCompilation_createForDevices");
    check_nnapi_.Compilation_createForDevices =
        check_Compilation_createForDevices;
    *(void**)&nnapi_.Execution_compute =
        dlsym(handle, "ANeuralNetworksExecution_compute");
    check_nnapi_.Execution_compute = check_Execution_compute;
    *(void**)&nnapi_.Memory_createFromFd =
        dlsym(handle, "ANeuralNetworksMemory_createFromFd");
    ...
    *(void**)&nnapi_.Model_create =
        dlsym(handle, "ANeuralNetworksModel_create");
    ...
    *(void**)&nnapi_.Model_finish =
        dlsym(handle, "ANeuralNetworksModel_finish");
    check_nnapi_.Model_finish = check_Model_finish;
    *(void**)&nnapi_.Model_addOperand =
        dlsym(handle, "ANeuralNetworksModel_addOperand");
    check_nnapi_.Model_addOperand = check_Model_addOperand;
    *(void**)&nnapi_.Model_setOperandValue =
        dlsym(handle, "ANeuralNetworksModel_setOperandValue");
    check_nnapi_.Model_setOperandValue = check_Model_setOperandValue;
    *(void**)&nnapi_.Model_setOperandValueFromMemory =
        dlsym(handle, "ANeuralNetworksModel_setOperandValueFromMemory");
    check_nnapi_.Model_setOperandValueFromMemory =
        check_Model_setOperandValueFromMemory;
    *(void**)&nnapi_.Model_addOperation =
        dlsym(handle, "ANeuralNetworksModel_addOperation");
    check_nnapi_.Model_addOperation = check_Model_addOperation;
    *(void**)&nnapi_.Model_identifyInputsAndOutputs =
        dlsym(handle, "ANeuralNetworksModel_identifyInputsAndOutputs");
    check_nnapi_.Model_identifyInputsAndOutputs =
        check_Model_identifyInputsAndOutputs;
    *(void**)&nnapi_.Model_relaxComputationFloat32toFloat16 =
        dlsym(handle, "ANeuralNetworksModel_relaxComputationFloat32toFloat16");
    check_nnapi_.Model_relaxComputationFloat32toFloat16 =
        check_Model_relaxComputationFloat32toFloat16;
    *(void**)&nnapi_.Compilation_create =
        dlsym(handle, "ANeuralNetworksCompilation_create");
    check_nnapi_.Compilation_create = check_Compilation_create;
    *(void**)&nnapi_.Compilation_free =
        dlsym(handle, "ANeuralNetworksCompilation_free");
    check_nnapi_.Compilation_free = check_Compilation_free;
    *(void**)&nnapi_.Compilation_setPreference =
        dlsym(handle, "ANeuralNetworksCompilation_setPreference");
    check_nnapi_.Compilation_setPreference = check_Compilation_setPreference;
    *(void**)&nnapi_.Compilation_finish =
        dlsym(handle, "ANeuralNetworksCompilation_finish");
    check_nnapi_.Compilation_finish = check_Compilation_finish;
    *(void**)&nnapi_.Execution_create =
        dlsym(handle, "ANeuralNetworksExecution_create");
    check_nnapi_.Execution_create = check_Execution_create;
    *(void**)&nnapi_.Execution_free =
        dlsym(handle, "ANeuralNetworksExecution_free");
    check_nnapi_.Execution_free = check_Execution_free;
    *(void**)&nnapi_.Execution_setInput =
        dlsym(handle, "ANeuralNetworksExecution_setInput");
    check_nnapi_.Execution_setInput = check_Execution_setInput;
    *(void**)&nnapi_.Execution_setInputFromMemory =
        dlsym(handle, "ANeuralNetworksExecution_setInputFromMemory");
    check_nnapi_.Execution_setInputFromMemory =
        check_Execution_setInputFromMemory;
    *(void**)&nnapi_.Execution_setOutput =
        dlsym(handle, "ANeuralNetworksExecution_setOutput");
    check_nnapi_.Execution_setOutput = check_Execution_setOutput;
    *(void**)&nnapi_.Execution_setOutputFromMemory =
        dlsym(handle, "ANeuralNetworksExecution_setOutputFromMemory");
    check_nnapi_.Execution_setOutputFromMemory =
        check_Execution_setOutputFromMemory;
    *(void**)&nnapi_.Execution_startCompute =
        dlsym(handle, "ANeuralNetworksExecution_startCompute");
    ...
    *(void**)&nnapi_.Execution_getOutputOperandRank =
        dlsym(handle, "ANeuralNetworksExecution_getOutputOperandRank");
    check_nnapi_.Execution_getOutputOperandRank =
        check_Execution_getOutputOperandRank;
    *(void**)&nnapi_.Execution_getOutputOperandDimensions =
        dlsym(handle, "ANeuralNetworksExecution_getOutputOperandDimensions");
    check_nnapi_.Execution_getOutputOperandDimensions =
        check_Execution_getOutputOperandDimensions;
    loaded = 1;
  }
  *nnapi = &nnapi_;
