...
int load_nnapi_model(
  ...
  CAFFE_ENFORCE(
      model_length >= required_size,
      "Model is too small.  Size = ",
      model_length);
  ...
  CAFFE_ENFORCE(
      ...
      "Model is too small.  Size = ",
      model_length);
  const SerializedOperation* operations =
      (const SerializedOperation*)next_pointer;
  next_pointer = (uint8_t*)serialized_model + required_size;
  CAFFE_ENFORCE(next_pointer <= end_of_buf);
  ...
  for (const auto i : c10::irange(ser_model->operation_count)) {
    required_size +=
        4 * (operations[i].input_count + operations[i].output_count);
  }
  ...
  for (const auto i : c10::irange(ser_model->operand_count)) {
    ...
    operand.dimensions =
        operands[i].dimension_count ? (const uint32_t*)next_pointer : NULL;
  ...
  for (const auto i : c10::irange(ser_model->value_count)) {
    ...
    switch ((SourceType)values[i].source_type) {
      ...
      case SOURCE_NUMBERED_BUFFER: {
        ...
        CAFFE_ENFORCE(
            buffer_offset + operand_length >=
            buffer_offset); // No integer overflow
        CAFFE_ENFORCE(
            buffer_offset + operand_length <=
            (uint32_t)buffer_sizes[buffer_number]); // No buffer overflow
    ...
    int result = nnapi->Model_setOperandValue(
        model, values[i].index, value_pointer, value_length);
    NNAPI_CHECK(result);
  }
  ...
  *out_output_count = ser_model->output_count;
  ...
  // TODO: Maybe eliminate required_size and just rely on next_pointer for
  // bounds checking.
  CAFFE_ENFORCE(next_pointer <= end_of_buf);
  CAFFE_ENFORCE(
      next_pointer == (const uint8_t*)serialized_model + required_size);
  // NOLINTNEXTLINE(modernize-use-nullptr)
