...
template <typename T>
using can_box = guts::disjunction<
...
template <typename T>
using can_unbox = guts::conjunction<
    guts::disjunction<
        has_ivalue_to<T>,
        // void returns are ok
        std::is_same<void, T>>,
...
//
// Note: all signatures matching this pattern are are assumed to be for such
// ops. Because of this, the generated BoxedKernelWrapper specializations simply
...
//
// Note: all signatures matching this pattern are are assumed to be for such
// ops. This assumption permits the generated BoxedKernelWrapper specializations
// to simply return out arguments.
//
...
// Note: all signatures matching this pattern are are assumed to be for such
// ops. This assumption permits the generated BoxedKernelWrapper specializations
// to simply return the out arguments.
//
template <class Result, class... Args>
struct BoxedKernelWrapper<
    Result(Args...),
