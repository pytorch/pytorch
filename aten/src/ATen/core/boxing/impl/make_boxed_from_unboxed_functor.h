...
struct assert_is_valid_input_type {
  assert_is_valid_input_type() {
    guts::if_constexpr<
        ...
        /* else */
        [] {
          /* otherwise this must be an instance of a valid custom class, since
             it can only have been created via IValue(x), which ensures this. */
        });
  }
};
...
struct assert_is_valid_output_type {
  assert_is_valid_output_type() {
    guts::if_constexpr<
        ...
        /* else */
        [] {
          /* otherwise T is verified to be a registered custom class in the
            IValue constructor, so no benefit in double-checking here */
        });
  }
};
...
struct ivalue_to_arg<optional<ArrayRef<T>>, AllowDeprecatedTypes> final {
  // If an argument is optional<ArrayRef<T>>, convert the IValue to an
  // optional<std::vector<T>> and pass that to the operator. OptionalArray<T> is
  // basically a optional<std::vector<T>> but impliticly convertible to
  // optional<ArrayRef<T>>.
...
struct return_to_ivalue<
    T,
    AllowDeprecatedTypes,
    std::enable_if_t<!std::is_same<at::Tensor&, T>::value>>
    final {
...
struct make_boxed_from_unboxed_functor final {
  ...
  static void call(
    ...
    guts::if_constexpr<has_outputs>(
      ...
      // is only required because some kernels still return `Tensor&`.
#ifdef __cpp_if_constexpr
...
#else
          using ReturnType_ = std::decay_t<typename decltype(
              delay_check)::template type_identity<ReturnType>>;
          ReturnType_ output = call_functor_with_args_from_stack<
              KernelFunctor,
              AllowDeprecatedTypes>(
              functor, dispatchKeySet, delay_check(stack));
#endif
