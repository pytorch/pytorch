...
inline KernelFunction KernelFunction::makeFromBoxedKernel(BoxedKernel boxed_fn) {
  return KernelFunction(boxed_fn, nullptr);
}
...
inline KernelFunction KernelFunction::makeFromUnboxedFunction(FuncPtr func_ptr) {
    ...
    static_assert(!std::is_same<typename FuncPtr::FuncType, BoxedKernel>::value, "Tried to call KernelFunction::makeFromUnboxedFunction with a boxed function pointer. Please use KernelFunction::makeFromBoxedFunction instead.");
