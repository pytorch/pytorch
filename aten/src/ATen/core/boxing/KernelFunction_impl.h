...
inline KernelFunction::KernelFunction()
    : functor_(), boxed_kernel_func_(nullptr), unboxed_kernel_func_(nullptr) {}
...
inline KernelFunction KernelFunction::makeFromBoxedFunctor(
  ...
  static_assert(
      std::is_base_of<OperatorKernel, KernelFunctor>::value,
      "Tried to call KernelFunction::makeFromBoxedFunctor<KernelFunctor>, but the functor doesn't inherit from c10::OperatorKernel. Please have the functor inherit from it.");
  return KernelFunction(
      std::move(kernelFunctor),
...
inline KernelFunction KernelFunction::makeFromUnboxedFunction(
  ...
  static_assert(
      !std::is_same<typename FuncPtr::FuncType, BoxedKernelFunction>::value,
