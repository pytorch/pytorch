...
#ifdef C10_MOBILE
  // Please read the comment in `VariableFallbackKernel.cpp` about the
...
class TORCH_API TensorBase {
 ...
 public:
  ...
  c10::SymInt sym_stride(int64_t dim) const {
    ...
    // false is passed to maybe_wrap_dim so behavior is identical to array
    // access (but with wrapping)
  ...
  // Ban assignment to rvalues, since at::Tensor (weirdly) performs a deep copy
  // here
  ...
  // Like is_contiguous, but more dynamic shape-friendly. May return a symbolic
  // representation of contiguity instead of SymTrue SymFalse, when results are
  // data-dependent.
  ...
  // but for now we are inclined to keep it consistent with existing THP
  // behavior
  ...
  // NOTE: Conjugate bit is supposed to be a read-only field. Only change this,
  // if you are sure that's what you want. Changing this might lead to incorrect
  // behavior since conjugation is a lazy operation and we rely on this bit to
  // determine if a conjugation needs to be materialized.
  ...
  // NOTE: Negative bit is supposed to be a read-only field. Only change this,
  // if you are sure that's what you want. Changing this might lead to incorrect
  // behavior since we rely on this bit to determine if a negation needs to be
  // materialized.
  ...
  /// \fn void backward(const Tensor & gradient={}, std::optional<bool>
  /// retain_graph=std::nullopt, bool create_graph=false,
  ...
  using hook_return_void_t = std::enable_if_t<
      std::is_void_v<typename std::invoke_result_t<T&, TensorBase>>,
      unsigned>;
  template <typename T>
...
template <typename T>
auto TensorBase::register_hook(T&& hook) const
    -> TensorBase::hook_return_void_t<T> {
  // Return the grad argument in case of a hook with void return type to have an
  // std::function with Tensor return type
  static_assert(
      std::is_same_v<decltype(hook(TensorBase())), void>,
...
template <typename T>
auto TensorBase::register_hook(T&& hook) const
    -> TensorBase::hook_return_var_t<T> {
...
inline c10::MaybeOwned<TensorBase> TensorBase::expect_contiguous(
