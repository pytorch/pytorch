...
inline void check_and_update_common_device(
  ...
  if (!common_device.has_value()) {
    ...
    is_cpu_zero_dim = cpu_zero_dim(tensor);
    return;
  }
  bool tensor_is_cpu_zero_dim = cpu_zero_dim(tensor);
  if (C10_LIKELY(common_device == tensor.device())) {
    if (is_cpu_zero_dim) {
      is_cpu_zero_dim = tensor_is_cpu_zero_dim;
    }
    return;
  }
  if (tensor_is_cpu_zero_dim) {
    return;
  }
  if (is_cpu_zero_dim) {
    common_device = tensor.device();
...
inline void check_and_update_common_device(
  ...
  if (tensor.has_value()) {
    check_and_update_common_device(
        common_device, is_cpu_zero_dim, tensor.value(), methodName, argName);
  }
}
...
inline void check_and_update_common_device(
  ...
  for (const auto& tensor : tensors) {
    check_and_update_common_device(
        common_device, is_cpu_zero_dim, tensor, methodName, argName);
  }
}
