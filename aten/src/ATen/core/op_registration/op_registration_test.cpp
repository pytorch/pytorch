...
TEST(
  ...
  expectThrows<c10::Error>(
      [&] { callOp(*op, dummyTensor(c10::DispatchKey::XLA)); }, "CPU");
  expectThrows<c10::Error>(
      [&] { callOp(*op, dummyTensor(c10::DispatchKey::XLA)); }, "CUDA");
}
...
TEST(OperatorRegistrationTest, AutogradXLAOverridesAutogradKernel) {
  ...
  called_nonautograd = called_autograd = false;
  op->typed<void(Tensor)>().call(
      dummyTensor(DispatchKey::XLA, /*requires_grad=*/true));
  EXPECT_TRUE(called_nonautograd);
  EXPECT_FALSE(called_autograd);
...
TEST(
  ...
  }
  {
    ...
    // When there's direct registration to XLA backend, AutogradXLA doesn't pick
    // up catchAll kernel in precompute but just keep fallthrough kernel from
    // backend fallback. Thus it falls through AutogradXLA and reaches the
    // kernel at XLA key.
    called_nonautograd = called_autograd = false;
    op->typed<void(Tensor)>().call(
        dummyTensor(DispatchKey::XLA, /*requires_grad=*/true));
...
TEST(
  ...
  auto m = MAKE_TORCH_LIBRARY(test);
  m.def(
      "fn", torch::dispatch(c10::DispatchKey::SparseCPU, [&](const Tensor& x) {
        sparsecpu_called = true;
        return x;
      }));
  m.impl(
      "fn", c10::DispatchKey::CompositeImplicitAutograd, [&](const Tensor& x) {
...

  {
    expectThrows<c10::Error>(
        [&] {
          callOp(
              *op,
              dummyTensor(c10::DispatchKey::SparseCPU, /*requires_grad=*/true));
        },
...
TEST(NewOperatorRegistrationTest, dispatchMultipleTensors) {
  ...
  {
    // TODO(#43908): currently this will fallthrough AutogradPrivateUse1 then
    // call catchall kernel at AutogradCPU, while backend extenders are indeed
    // expecting to call PrivateUse1 kernel. This confusing behavior is caused
    // by we registering fallthrough as backend fallback for Autograd keys. Note
    // users could always work around this by registering the same kernel to
