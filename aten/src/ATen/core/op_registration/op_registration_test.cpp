...
//     c10::RegisterOperators::options().kernel<MockKernel>(c10::DispatchKey::CPU,
//     &called));
...
//   kernels for dispatch keys CPU. An operator can only have either a catch-all
//   kernel or kernels with dispatch keys. The operator schema is
//   _test::dummy");
// }
//
// TEST(OperatorRegistrationTest,
...
TEST(
  ...
  auto registrar = c10::RegisterOperators().op(
      "_test::dummy(Tensor dummy) -> ()",
      c10::RegisterOperators::options().kernel<MockKernel>(
          c10::DispatchKey::CPU, &called_kernel_cpu));
  ...
  callOpUnboxedWithPrecomputedDispatchKeySet<void, Tensor>(
      *op,
      c10::DispatchKeySet(c10::DispatchKey::CPU),
      dummyTensor(c10::DispatchKey::CUDA));
...
std::string expectedMessageForBackend(DispatchKey key) {
  ...
  return "Could not run '_test::dummy' with arguments from the '" + key_str +
      "' backend";
}
...
TEST(
  ...
  // Test for out of tree lazy backends- ::Lazy key is now registered to TS
  // backend in tree
  for (c10::DispatchKey key : {c10::DispatchKey::XLA}) {
    std::string expectMessage = expectedMessageForBackend(key);
    expectThrows<c10::Error>(
        [&] { callOp(*op, dummyTensor(key)); }, expectMessage.c_str());

    // also assert that the error message contains the available tensor type
    // ids, but don't assert their order
    expectThrows<c10::Error>([&] { callOp(*op, dummyTensor(key)); }, "CPU");
    expectThrows<c10::Error>([&] { callOp(*op, dummyTensor(key)); }, "CUDA");
  }
}
...
TEST(
  ...
  auto registrar1 = c10::RegisterOperators().op(
      ...
      c10::RegisterOperators::options().kernel(
          c10::DispatchKey::CUDA, [](Tensor, std::string) { called = true; }));
  auto op = Dispatcher::singleton().findSchema({"_test::dummy", ""});
  ASSERT_TRUE(op.has_value());
...
TEST(
  ...
  auto registrar1 = c10::RegisterOperators().op(
      "_test::dummy(Tensor dummy, str input) -> ()",
      c10::RegisterOperators::options().kernel(
          c10::DispatchKey::CPU, [](Tensor, std::string) { called = true; }));
  auto op = Dispatcher::singleton().findSchema({"_test::dummy", ""});
  ASSERT_TRUE(op.has_value());
...
TEST(
  ...
  auto registrar = c10::RegisterOperators().op(
      "_test::dummy(Tensor dummy) -> ()",
      c10::RegisterOperators::options()
          .kernel<decltype(nonautograd_kernel), nonautograd_kernel>(
              DispatchKey::CPU)
          .kernel<decltype(autograd_kernel), &autograd_kernel>(
              DispatchKey::Autograd));
...
TEST(
  ...
  // catchAll now maps to CompositeImplicitAutograd which has higher precedence
  // than Autograd
  called_nonautograd = called_autograd = false;
  op->typed<void(Tensor)>().call(
      dummyTensor(DispatchKey::CPU, /*requires_grad=*/true));
...
TEST(OperatorRegistrationTest, AutogradBackendOverridesAutogradKernel) {
  ...
  called_nonautograd = called_autograd = false;
  op->typed<void(Tensor)>().call(
      dummyTensor(DispatchKey::CUDA, /*requires_grad=*/true));
  EXPECT_TRUE(called_autograd);
  EXPECT_FALSE(called_nonautograd);
}
...
void LazyBackendsAutogradOverridesAutogradKernel(DispatchKey key) {
  auto registrar = c10::RegisterOperators().op(
      ...
      c10::RegisterOperators::options()
          .kernel<decltype(nonautograd_kernel), &nonautograd_kernel>(
              c10::getAutogradKeyFromBackend(toBackendComponent(key)))
          .kernel<decltype(autograd_kernel), &autograd_kernel>(
              DispatchKey::Autograd));
  ...
  expectThrows<c10::Error>(
      [&] { callOp(*op, dummyTensor(key)); }, expectedMessage.c_str());
...
// since it is now registered to TS backend in-tree and thus behaves
// differently, does not throw the expected 'could not run..' messages
...
void whenRegisterWithLazyBackendsAndCatchAll_AutogradLazyBackendsIsNotFilled(
    DispatchKey key) {
  {
    auto registrar = c10::RegisterOperators().op(
        "_test::dummy(Tensor dummy) -> ()",
  ...
  }
  {
    ...
    // When there's direct registration to XLA / Lazy backend, Autograd{XLA,
    // Lazy} doesn't pick up catchAll kernel in precompute but just keep
    // fallthrough kernel from backend fallback. Thus it falls through
    // Autograd{XLA, Lazy} and reaches the kernel at XLA / Lazy key.
    called_nonautograd = called_autograd = false;
    op->typed<void(Tensor)>().call(dummyTensor(key, /*requires_grad=*/true));
    EXPECT_FALSE(called_nonautograd);
    EXPECT_TRUE(called_autograd);
...
TEST(
  ...
  whenRegisterWithLazyBackendsAndCatchAll_AutogradLazyBackendsIsNotFilled(
      DispatchKey::XLA);
}
...
TEST(
  ...
  auto registrar = c10::RegisterOperators().op(
      "_test::dummy",
      c10::RegisterOperators::options().kernel(
          DispatchKey::CPU, [](int64_t) {}));
  expectThrows<c10::Error>(
      [] {
        c10::Dispatcher::singleton()
            .findSchemaOrThrow("_test::dummy", "")
...
TEST(
  ...
  auto registrar = c10::RegisterOperators().op(
      "_test::dummy",
      c10::RegisterOperators::options().catchAllKernel([](int64_t) {}));
  expectThrows<c10::Error>(
      [] {
        c10::Dispatcher::singleton()
            .findSchemaOrThrow("_test::dummy", "")
...
TEST(OperatorRegistrationTest, testAvailableArgTypes) {
  ...
  testArgTypes<Tensor>::test(
      dummyTensor(c10::DispatchKey::CPU),
      [](const Tensor& v) {
        EXPECT_EQ(c10::DispatchKey::CPU, extractDispatchKey(v));
      },
      dummyTensor(c10::DispatchKey::CUDA),
      [](const IValue& v) {
        EXPECT_EQ(c10::DispatchKey::CUDA, extractDispatchKey(v.toTensor()));
      },
      "(Tensor a) -> Tensor");
  ...
  testArgTypes<c10::optional<Tensor>>::test(
      c10::optional<Tensor>(dummyTensor(c10::DispatchKey::CPU)),
      [](const c10::optional<Tensor>& v) {
        EXPECT_EQ(c10::DispatchKey::CPU, extractDispatchKey(v.value()));
      },
      c10::optional<Tensor>(dummyTensor(c10::DispatchKey::CUDA)),
      [](const IValue& v) {
        EXPECT_EQ(c10::DispatchKey::CUDA, extractDispatchKey(v.toTensor()));
      },
      "(Tensor? a) -> Tensor?");
  ...
  testArgTypes<c10::ArrayRef<double>, c10::List<double>>::test(
      c10::ArrayRef<double>(),
      [](c10::ArrayRef<double> v) { EXPECT_EQ(0, v.size()); },
      c10::List<double>(),
      [](const IValue& v) { EXPECT_EQ(0, v.to<c10::List<double>>().size()); },
      "(float[] a) -> float[]");
  testArgTypes<c10::ArrayRef<int64_t>, c10::List<int64_t>>::test(
      c10::ArrayRef<int64_t>(),
      [](c10::ArrayRef<int64_t> v) { EXPECT_EQ(0, v.size()); },
      c10::List<int64_t>(),
      [](const IValue& v) { EXPECT_EQ(0, v.to<c10::List<int64_t>>().size()); },
      "(int[] a) -> int[]");
  testArgTypes<c10::ArrayRef<std::string>, c10::List<std::string>>::test(
      c10::ArrayRef<std::string>(),
      [](c10::ArrayRef<std::string> v) { EXPECT_EQ(0, v.size()); },
      c10::List<std::string>(),
      [](const IValue& v) { EXPECT_EQ(0, v.toListRef().size()); },
      "(str[] a) -> str[]");
  ...
  // list types (with non-empty list)
  testArgTypes<c10::ArrayRef<double>, c10::List<double>>::test(
      c10::ArrayRef<double>({1.5, 2.5}),
      [](c10::ArrayRef<double> v) {
        expectListEquals({1.5, 2.5}, v);
      },
      c10::List<double>({3.5, 4.5}),
      [](const IValue& v) {
      ...
      "(float[] a) -> float[]");
  testArgTypes<c10::ArrayRef<int64_t>, c10::List<int64_t>>::test(
      c10::ArrayRef<int64_t>({1, 2}),
      [](c10::ArrayRef<int64_t> v) {
        expectListEquals({1, 2}, v);
      },
      c10::List<int64_t>({3, 4}),
      [](const IValue& v) {
      ...
      "(int[] a) -> int[]");
  testArgTypes<c10::ArrayRef<std::string>, c10::List<std::string>>::test(
      c10::ArrayRef<std::string>({"first", "second"}),
      [](c10::ArrayRef<std::string> v) {
        expectListEquals({"first", "second"}, v);
      },
      c10::List<std::string>({"first", "second"}),
      [](const IValue& v) {
  ...
  testArgTypes<std::array<bool, 0>>::test(
      ...
      std::array<bool, 0>(),
      [](const IValue& v) {
        EXPECT_EQ(0, (v.to<std::array<bool, 0>>().size()));
      },
      "(bool[0] a) -> bool[0]");
  testArgTypes<std::array<std::string, 0>>::test(
      std::array<std::string, 0>(),
      [](std::array<std::string, 0> v) { EXPECT_EQ(0, v.size()); },
      std::array<std::string, 0>(),
      [](const IValue& v) { EXPECT_EQ(0, v.toListRef().size()); },
      "(str[0] a) -> str[0]");
  ...
  testArgTypes<std::array<double, 2>>::test(
      ...
      [](const IValue& v) {
        expectListEquals({3.5, 4.5}, v.to<std::array<double, 2>>());
      },
      "(float[2] a) -> float[2]");
  testArgTypes<std::array<int64_t, 2>>::test(
      ...
      std::array<int64_t, 2>({3, 4}),
      [](const IValue& v) {
        expectListEquals({3, 4}, v.to<std::array<int64_t, 2>>());
      },
      "(int[2] a) -> int[2]");
  testArgTypes<std::array<bool, 2>>::test(
      ...
      std::array<bool, 2>({true, false}),
      [](const IValue& v) {
        expectListEquals({true, false}, v.to<std::array<bool, 2>>());
      },
      "(bool[2] a) -> bool[2]");
...
TEST(NewOperatorRegistrationTest, testBasics) {
  ...
  m.def(
      "dummy4", [](const Tensor& self, const Tensor& other) { return other; });
  m.impl(
      "dummy", c10::DeviceType::CPU, [](const Tensor& self) { return self; });
  m.impl(
      "dummy", c10::DeviceType::XLA, [](const Tensor& self) { return self; });
  m.impl(
      "dummy", c10::DeviceType::Lazy, [](const Tensor& self) { return self; });
  // Internal API
  m.impl(
      "dummy2", c10::DispatchKey::CPU, [](const Tensor& self) { return self; });
  m.impl(
      "dummy2", c10::DispatchKey::XLA, [](const Tensor& self) { return self; });
  m.impl("dummy2", c10::DispatchKey::Lazy, [](const Tensor& self) {
    return self;
  });
...
TEST(NewOperatorRegistrationTest, schema) {
  ...
  m.def(torch::schema(
      "def3(Tensor self) -> Tensor", c10::AliasAnalysisKind::PURE_FUNCTION));
...
TEST(
  ...
  called = false;
  auto stack = callOp(*op, dummyTensor(c10::DispatchKey::CPU), "hello ");
  // CatchAll now maps to CompositeImplicitAutograd and has higher precedence
  // than backend fallback.
  EXPECT_TRUE(called);
}
...
TEST(NewOperatorRegistrationTest, dispatchWithCompositeImplicitAutogradKernel) {
  ...
  m.def(
      "fn",
      torch::dispatch(
          c10::DispatchKey::CompositeImplicitAutograd, [&](const Tensor& x) {
            math_called = true;
            return x;
          }));
...

  {
    math_called = false;
    callOp(
        *op, dummyTensor(c10::DispatchKey::SparseCPU, /*requires_grad=*/true));
    ASSERT_TRUE(math_called);
  }
}
...
TEST(
  ...
  m.impl("fn", c10::DispatchKey::Autograd, [&](const Tensor& x) {
...
TEST(
  ...
  // catchAll now maps to CompositeImplicitAutograd, which means we have two
  // registrations to CompositeImplicitAutograd key. The last registration is
  // used.
  {
...
TEST(
  ...
  m.impl("fn", c10::DispatchKey::AutogradCPU, [&](const Tensor& x) {
    autograd_called = true;
    return x;
  });
...
TEST(NewOperatorRegistrationTest, dispatchWithCompositeExplicitAutogradKernel) {
  ...
  m.def(
      "fn",
      torch::dispatch(
          c10::DispatchKey::CompositeExplicitAutograd, [&](const Tensor& x) {
            called = true;
            return x;
          }));
...
TEST(
  ...
  m.impl(
      "fn", c10::DispatchKey::CompositeImplicitAutograd, [&](const Tensor& x) {
        math_called = true;
        return x;
      });
...
TEST(NewOperatorRegistrationTest, dispatch) {
  ...
  bool autograd_called = false;
  auto m = MAKE_TORCH_LIBRARY(test);
  m.def("fn_cpu", torch::dispatch(c10::DispatchKey::CPU, [&](const Tensor& x) {
          cpu_called = true;
          return x;
        }));
  m.def("fn_cuda", torch::dispatch(c10::kCUDA, [&](const Tensor& x) {
          cuda_called = true;
          return x;
        }));
  m.def("fn_autograd", torch::dispatch(c10::kAutograd, [&](const Tensor& x) {
          autograd_called = true;
          return x;
        }));
...
TEST(NewOperatorRegistrationTest, dispatchAutogradPrecedence) {
  bool cpu_called = false;
  auto m = MAKE_TORCH_LIBRARY(test);
  m.def("fn", torch::dispatch(c10::DispatchKey::CPU, [&](const Tensor& x) {
          cpu_called = true;
          return x;
        }));
  ...
  bool autograd_called = false;
  m.impl("fn", c10::kAutograd, [&](const Tensor& x) {
    autograd_called = true;
    return x;
  });
...
TEST(
    NewOperatorRegistrationTest,
    throwsWhenRegisterToBackendMapsToAutogradOther) {
  // NOLINTNEXTLINE(cppcoreguidelines-init-variables)
  bool fpga_called, math_called = false;
  auto m = MAKE_TORCH_LIBRARY(test);
  m.def("fn", torch::dispatch(c10::DispatchKey::FPGA, [&](const Tensor& x) {
          fpga_called = true;
          return x;
        }));
  m.impl(
...

  {
    expectThrows<c10::Error>(
        [&] {
          callOp(
              *op, dummyTensor(c10::DispatchKey::FPGA, /*requires_grad=*/true));
        },
...
TEST(NewOperatorRegistrationTest, dispatchMultipleTensors) {
  ...
  bool catchall_called = false;
  // Similar to in-tree AutogradCPU/AutogradCUDA etc, out-of-tree backends
  // usually register a fallthrough kernel for AutogradPrivateUse1.
  ...
  m.def(
      "fn",
      torch::dispatch(
          c10::DispatchKey::PrivateUse1, [&](const Tensor& x, const Tensor& y) {
            privateuse1_called = true;
            return x;
          }));
  m.impl("fn", [&](const Tensor& x, const Tensor& y) {
    catchall_called = true;
    return x;
  });

  {
    ...
    callOp(
        *op,
        dummyTensor(c10::DispatchKey::PrivateUse1),
        dummyTensor(c10::DispatchKey::CPU));
...
TEST(
    NewOperatorRegistrationTest,
    registerCompositeImplicitAutogradWithCPUKernel_andCallAutogradOtherKernel_callsComposite) {
  bool math_called = false;
  bool cpu_called = false;
  auto m = MAKE_TORCH_LIBRARY(test);
...
TEST(NewOperatorRegistrationTest, dispatchMultiple) {
  ...
  m.impl("fn", c10::kCUDA, [&](const Tensor& x) {
    cuda_called = true;
    return x;
  });
  m.impl("fn", c10::kAutograd, [&](const Tensor& x) {
...
TEST(NewOperatorRegistrationTest, BackendSelectRedispatchesToCPU) {
  ...
  auto m = MAKE_TORCH_LIBRARY(test);
  auto after_backend_select = c10::DispatchKeySet(
      c10::DispatchKeySet::FULL_AFTER, c10::DispatchKey::BackendSelect);
  m.def("fn(Tensor self) -> Tensor");
  m.impl("fn", c10::kCPU, [&](const Tensor& x) {
    cpu_called = true;
    return x;
  });
  m.impl(
      ...
      [&](c10::DispatchKeySet ks, const Tensor& x) {
        backend_generic_called = true;
        auto op = c10::Dispatcher::singleton()
                      .findSchema({"test::fn", ""})
                      .value()
                      .typed<Tensor(const Tensor&)>();
        return c10::Dispatcher::singleton().redispatch<Tensor, const Tensor&>(
            op, ks & after_backend_select, x);
      });
...
TEST(NewOperatorRegistrationTest, CppFunction) {
  ...
  m.def(
      "fn5(Tensor x) -> Tensor",
      CppFunction::makeFromUnboxedFunction(dummy_fn));
  m.def(
      "fn6(Tensor x) -> Tensor",
      CppFunction::makeFromBoxedFunction<&backend_fallback_kernel>());
}
...
TEST(NewOperatorRegistrationTest, testImplNoDefGetsCaught) {
  ...
  std::string error_str =
      "Discovered operators that have been registered through the dispatcher"
...
// autograd kernel that redispatches. Explicitly takes in and updates the
// DispatchKeySet
void autograd_kernel_redispatching_with_DispatchKeySet(
    c10::DispatchKeySet ks,
    Tensor a) {
  called_kernel_autograd = true;
  auto op = Dispatcher::singleton().findSchema({"test::fn", ""});
  auto updatedDispatchKeySet = ks &
      c10::DispatchKeySet(c10::DispatchKeySet::FULL_AFTER,
                          c10::DispatchKey::AutogradOther);
  callOpUnboxedWithPrecomputedDispatchKeySet<void, Tensor>(
      *op, updatedDispatchKeySet, a);
}

// autograd kernel that redispatches. Does not take in a DispatchKeySet
void autograd_kernel_redispatching_without_DispatchKeySet(
    c10::DispatchKeySet ks,
    Tensor a) {
  called_kernel_autograd = true;
  auto op = Dispatcher::singleton().findSchema({"test::fn", ""});
...
// tracing kernel that redispatches. Explicitly takes in and updates the
// DispatchKeySet
void tracing_kernel_redispatching_with_DispatchKeySet(
    c10::DispatchKeySet ks,
    Tensor a) {
...
TEST(
  ...
  m.impl(
      ...
      autograd_kernel_redispatching_with_DispatchKeySet);
  m.impl(
      "fn",
      c10::DispatchKey::Tracer,
      tracing_kernel_redispatching_with_DispatchKeySet);
...
TEST(
  ...
  m.def("fn(Tensor dummy) -> ()");
  m.impl("fn", c10::DispatchKey::CPU, cpu_kernel);
  // the tracing kernel takes in a DispatchKeySet, but the autograd kernel does
  // not the dispatcher should handle correctly plumbing its DispatchKeySet to
  // tracing and not autograd.
  m.impl(
      "fn",
      c10::DispatchKey::AutogradCPU,
      autograd_kernel_redispatching_without_DispatchKeySet);
  m.impl(
      "fn",
      c10::DispatchKey::Tracer,
      tracing_kernel_redispatching_with_DispatchKeySet);
...
TEST(OperatorRegistrationTest, getRegistrationsForDispatchKey) {
  ...
  auto all_ops =
      Dispatcher::singleton().getRegistrationsForDispatchKey(c10::nullopt);
  // should return every registered op with a cpu kernel
  auto cpu_ops = Dispatcher::singleton().getRegistrationsForDispatchKey(
      c10::DispatchKey::CPU);
  ASSERT_TRUE(all_ops.size() > 0);
  ASSERT_TRUE(cpu_ops.size() > 0);

  auto cmp_lambda = [](const c10::OperatorName a,
                       const c10::OperatorName& b) -> bool {
