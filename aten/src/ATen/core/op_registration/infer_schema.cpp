...
c10::optional<std::string> findSchemaDifferences(const FunctionSchema& lhs, const FunctionSchema& rhs) {
  if (lhs.arguments().size() != rhs.arguments().size()) {
    return "The number of arguments is different. " + std::to_string(lhs.arguments().size()) +
             " vs " + std::to_string(rhs.arguments().size()) + ".";
  }
  if (lhs.returns().size() != rhs.returns().size()) {
    return "The number of returns is different. " + std::to_string(lhs.returns().size()) +
             " vs " + std::to_string(rhs.returns().size());
  }
  ...
  for (const auto i : c10::irange(lhs.arguments().size())) {
    ...
    if (leftType.get() != rightType.get() && *leftType != *rightType) {
      return "Type mismatch in argument " + std::to_string(i+1) + ": " + lhs.arguments()[i].type()->str() +
  ...
  for (const auto i : c10::irange(lhs.returns().size())) {
    ...
    if (leftType.get() != rightType.get() && *leftType != *rightType) {
      return "Type mismatch in return " + std::to_string(i+1) + ": " + lhs.returns()[i].type()->str() +
