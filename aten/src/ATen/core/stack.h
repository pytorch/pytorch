...
// element i
static inline c10::IValue& peek(Stack& stack, size_t i, size_t N) {
...
static inline at::ArrayRef<c10::IValue> peekSlice(
    const Stack& stack,
    size_t i,
    size_t len,
    size_t N) {
  return at::ArrayRef<c10::IValue>(stack).slice(stack.size() - N + i, len);
}
static inline at::ArrayRef<c10::IValue> last(const Stack& stack, size_t N) {
  return peekSlice(stack, 0, N, N);
}
static inline void drop(Stack& stack, size_t n) {
  stack.erase(stack.end() - n, stack.end());
}
static inline c10::IValue pop(Stack& stack) {
