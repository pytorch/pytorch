...
struct DictImpl final : public c10::intrusive_ptr_target {
  ...
  explicit DictImpl(dict_map_type dict_, DictElementTypes elementTypes_)
      : dict(std::move(dict_)), elementTypes(std::move(elementTypes_)) {}
  dict_map_type dict;
...
class DictEntryRef final {
 public:
  explicit DictEntryRef(Iterator iterator) : iterator_(std::move(iterator)) {}
  ...
  void setValue(Value_&& value) const {
    static_assert(
        std::is_constructible<Value, Value_>::value,
        "Wrong type for the value argument of setValue()");
    iterator_->second = Value(std::forward<Value_>(value));
  }

 private:
  // allow copying and moving, but only our friends (i.e. the Dict class) can do
  // it. Copying/moving this reference wrapper would be too ambiguous to allow
...
template <class Key, class Value, class Iterator>
class DictIterator final : public std::iterator<
                               std::forward_iterator_tag,
                               DictEntryRef<Key, Value, Iterator>> {
 public:
  explicit DictIterator() = default;
  ~DictIterator() = default;

  DictIterator(const DictIterator& rhs) : entryRef_(rhs.entryRef_) {}
  DictIterator(DictIterator&& rhs) noexcept
  ...
  friend typename std::iterator<
      std::random_access_iterator_tag,
      DictEntryRef<Key, Value, Iterator>>::difference_type
  operator-(const DictIterator& lhs, const DictIterator& rhs) {
 ...
 private:
  ...
  friend class DictIterator<
      Key,
      Value,
      typename c10::detail::DictImpl::dict_map_type::iterator>;
  friend class Dict<Key, Value>;
};

template <class Key, class Value>
Dict<Key, Value> toTypedDict(Dict<IValue, IValue> dict);
template <class Key, class Value>
Dict<IValue, IValue> toGenericDict(Dict<Key, Value> dict);
} // namespace impl
...
class Dict final {
 private:
  static_assert(
      (std::is_same<IValue, Key>::value &&
       std::is_same<IValue, Value>::value) ||
          guts::typelist::contains<impl::valid_dict_key_types, Key>::value,
      "Invalid Key type for Dict. We only support int64_t, double, bool, and string.");

  // impl_ stores the underlying map as a
  // ska_ordered::order_preserving_flat_hash_map. We intentionally don't offer
  // conversion from/to order_preserving_flat_hash_map, return references to it
  // or something like that, because such operations would get expensive if we
  // switch out the actual map implementation. This is an intrusive_ptr because
  // Dict is a pointer type. Invariant: This will never be a nullptr, there will
  // always be a valid DictImpl.
  c10::intrusive_ptr<detail::DictImpl> impl_;

  explicit Dict(c10::intrusive_ptr<detail::DictImpl>&& impl);
  friend struct IValue;
  template <class K, class V>
  friend Dict<K, V> impl::toTypedDict(Dict<IValue, IValue>);
  template <class K, class V>
  friend Dict<IValue, IValue> impl::toGenericDict(Dict<K, V>);
 ...
 public:
  ...
  using iterator = impl::DictIterator<
      Key,
      Value,
      typename detail::DictImpl::dict_map_type::iterator>;
   ...
   * contain an element with an equivalent key. May invalidate any references,
  ...
  /**
   * If an element with the given key already exists, it is overwritten with the
   * given value. Otherwise, a new element with the given key and value are
   * inserted. May invalidate any references, pointers, or iterators referring
   ...
   * contained elements. The iterator iter must be valid and dereferenceable.
   ...
   * contained elements.
   *
   * @return The number of elements removed. This is either '1' if an element
   * with the key existed, or '0' if it didn't.
   */
   ...
   * for equality: two dicts with the same identity (e.g. same pointer)
