...
//
// What exactly does AutoDispatchBelowAutograd do?  The short answer is, it
// causes dispatches on ATen functions to go to the non-variable implementation,
...
//
// The solution to the above problem is to add `at::AutoDispatchBelowAutograd`,
// which when enabled will cause `legacyTensorType()` and `getType()` to always
// return non-Variable type, even if the tensor being called on is a variable.
 ...
 * In the past AutoDispatchBelowAutograd(or its old version
 * AutoNonVariableTypeMode) was used in the user code for inference-only
 * workload, this was under risk of producing wrong results silently in some
 ...
 * error saying "one of the variables needed for gradient computation has be
 * modified."
 */
