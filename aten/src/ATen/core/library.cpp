...
#define ERROR_CONTEXT                                                        \
  "(Error occurred while processing ", toString(kind_), " block at ", file_, \
      ":", line_, ")"
...
void initialize_torch_libraries() {
  const std::lock_guard<std::mutex> lock(
      detail::torch_library_initializer_mutex);
...
Library::Library(
    ...
    : kind_(kind),
      ...
      dispatch_key_(
          k.value_or(CatchAll) == CatchAll ? std::optional<c10::DispatchKey>()
                                           : k),
      file_(file),
      line_(line) {
  switch (kind_) {
    case DEF:
      ...
      registrars_.emplace_back(c10::Dispatcher::singleton().registerLibrary(
    ...
    case FRAGMENT:
      TORCH_CHECK(
          ...
          ": cannot define ",
          toString(kind_),
          " with the wildcard namespace _ "
          "(every ",
          toString(kind_),
          " defines operators for a distinct namespace!) "
          "Did you mean to use TORCH_LIBRARY_IMPL instead?  " ERROR_CONTEXT);
...
Library& Library::_def(
    c10::FunctionSchema&& schema,
    c10::OperatorName* out_name,
    const std::vector<at::Tag>& tags) & {
  TORCH_CHECK(
      kind_ == DEF || kind_ == FRAGMENT,
      DEF_PRELUDE,
      "Cannot define an operator inside of a ",
      toString(kind_),
      " block.  "
  ...
  if (ns_opt.has_value()) {
    ...
    TORCH_CHECK(
        *ns_opt == *ns_,
        "Explicitly provided namespace (",
        *ns_opt,
        ") in schema string "
        "does not match namespace of enclosing ",
        toString(kind_),
        " block (",
        *ns_,
        ").  "
...
// Since Python isn't available at all we can noop registerPythonModule
  ...
  if (python_module_.has_value()) {
    registrars_.emplace_back(c10::Dispatcher::singleton().registerPythonModule(
        schema.operator_name(), python_module_->first, python_module_->second));
  }
#endif
  registrars_.emplace_back(c10::Dispatcher::singleton().registerDef(
      std::move(schema), debugString(file_, line_), tags));
  return *this;
}
#undef DEF_PRELUDE
...
Library& Library::_def(
  ...
  c10::FunctionSchema schema = [&] {
    if (std::holds_alternative<c10::FunctionSchema>(name_or_schema)) {
      ...
      c10::OperatorName name =
          std::get<c10::OperatorName>(std::move(name_or_schema));
      TORCH_CHECK(
          f.schema_,
          "def(\"",
          name,
          ...
          ERROR_CONTEXT);
      c10::FunctionSchema s = f.schema_->cloneWithName(
  ...
  auto dispatch_key =
      f.dispatch_key_.has_value() ? f.dispatch_key_ : dispatch_key_;
  registrars_.emplace_back(c10::Dispatcher::singleton().registerImpl(
      std::move(name),
      dispatch_key,
      std::move(f.func_),
      f.cpp_signature_,
...
at::OperatorName Library::_parseNameForLib(const char* name_str) const {
  ...
  if (ns_opt.has_value()) {
    ...
    TORCH_CHECK(
        ...
        ns_,
        ").  "
        "Move this definition to the ",
        toString(kind_),
        " block corresponding to this namespace "
        "(and consider deleting the namespace from your schema string.)  ",
        ERROR_CONTEXT);
  } else {
    // NOLINTNEXTLINE(bugprone-unchecked-optional-access)
...
Library& Library::_impl(const char* name_str, CppFunction&& f) & {
  ...
  // See Note [Redundancy in registration code is OK]
  TORCH_CHECK(
      !(f.dispatch_key_.has_value() && dispatch_key_.has_value() &&
        *f.dispatch_key_ != *dispatch_key_),
      IMPL_PRELUDE,
      "Explicitly provided dispatch key (",
      *f.dispatch_key_,
      ") is inconsistent "
      "with the dispatch key of the enclosing ",
      toString(kind_),
      " block (",
      *dispatch_key_,
      ").  "
      "Please declare a separate ",
      toString(kind_),
      " block for this dispatch key and "
      "move your impl() there.  " ERROR_CONTEXT);
  auto dispatch_key =
      f.dispatch_key_.has_value() ? f.dispatch_key_ : dispatch_key_;
  registrars_.emplace_back(c10::Dispatcher::singleton().registerImpl(
      std::move(name),
...
Library& Library::_fallback(CppFunction&& f) & {
  TORCH_CHECK(
      kind_ == IMPL,
      "fallback(...): Cannot define an operator inside of a ",
      toString(kind_),
      " block.  "
  ...
  TORCH_CHECK(
      ...
      "(you specified ",
      *ns_,
      ") are not supported.  If you intended to apply ",
      "this fallback function globally, please define a separate block:\n\n",
      "    TORCH_LIBRARY_IMPL(_, ",
      *dispatch_key,
      ", m) { m.fallback(...); }\n\n",
      ERROR_CONTEXT);
  // Note if dispatch_key is DispatchKey::Undefined, it'll be ignored here since
  // Undefined isn't a runtime key, you shouldn't register anything to it at
  // all.
  for (auto k : c10::getRuntimeDispatchKeySet(*dispatch_key)) {
    // mobile doesn't use all dispatch keys, so skip any fallback registrations
    // for the unused keys.
    auto idx = getDispatchTableIndexForDispatchKey(k);
    if (idx < 0)
      continue;
    registrars_.emplace_back(c10::Dispatcher::singleton().registerFallback(
        k, f.func_, debugString(f.debug_, file_, line_)));
  }
  return *this;
}
