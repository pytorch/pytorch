...
Library::Library(
    ...
    : kind_(kind),
      ...
      dispatch_key_(
          (!k.has_value() || *k == c10::DispatchKey::CatchAll) ? c10::nullopt
                                                               : k),
      file_(file),
      line_(line) {
  switch (kind_) {
...
Library& Library::_def(
    c10::either<c10::OperatorName, c10::FunctionSchema>&& name_or_schema,
    CppFunction&& f) & {
  c10::FunctionSchema schema = [&] {
...
Library& Library::_fallback(CppFunction&& f) & {
  ...
  for (auto k : c10::getRuntimeDispatchKeySet(*dispatch_key)) {
    ...
    registrars_.emplace_back(c10::Dispatcher::singleton().registerFallback(
        k, std::move(f.func_), debugString(std::move(f.debug_), file_, line_)));
  }
  return *this;
}
