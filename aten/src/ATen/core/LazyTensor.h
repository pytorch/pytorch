...
struct CAFFE2_API LazyTensorImpl : public TensorImpl {
  ...
  inline void resolve(c10::IValue val) {
    TORCH_INTERNAL_ASSERT(!isResolved(), "Already resolved.");
    schema_ = c10::nullopt;
    inps_ = { val };
  }

  inline c10::IValue value() const {
    TORCH_INTERNAL_ASSERT(isResolved());
    return inps_[0];
  }

  inline const std::vector<c10::IValue>& inputs() const {
    TORCH_INTERNAL_ASSERT(!isResolved());
    return inps_;
  }

  inline c10::FunctionSchema schema() const {
    TORCH_INTERNAL_ASSERT(!isResolved());
  ...
  inline const std::vector<LazyTensorImpl*>& siblings() const {
    return siblings_;
  }

  inline std::vector<LazyTensorImpl*>& siblings() {
    return siblings_;
  }

  inline void setSiblings(std::vector<LazyTensorImpl*> sibs) {
    siblings_ = sibs;
  }

  inline size_t outputIndex() const {
    return index_;
  }

  Tensor to_eager();

  static std::function<Tensor(LazyTensorImpl*)>& getResolver();
 ...
 private:
  ...
  size_t index_ = 0;
  std::vector<LazyTensorImpl*> siblings_ = {};
