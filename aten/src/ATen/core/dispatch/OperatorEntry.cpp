...
bool OperatorEntry::hasKernelForAnyDispatchKey(DispatchKeySet ks) const {
  ...
  for (auto& kv : kernels_) {
    ...
    if (!isAliasDispatchKey(kv.first) && ks.has(kv.first))
      return true;
  }
  return false;
}
...
const AnnotatedKernel* OperatorEntry::getKernelForDispatchKey(
    DispatchKey dispatch_key) const {
...
std::pair<const AnnotatedKernel&, const char*> OperatorEntry::
  ...
  //    (2.1) Use kernel from
  //    DispatchKey::CompositeExplicitAutogradNonFunctional if available.
  //          This is used to register a kernel that works for all backends in
  //          inference, except "functional" backends like LazyTensor/XLA. But
  //          it requires separate registration for Autograd keys to support
  //          training.
  //    (2.2) Use kernel from DispatchKey::CompositeExplicitAutograd if
  //    available.
  ...
  //          to support training.
  //    (2.3) Use kernel from DispatchKey::CompositeImplicitAutograd if
  //    available.
  ...
  //          For AutogradOther, we eagerly return
  //          ambiguousAutogradOtherKernel() if there's registration to any of
  //          its backends and ask backend extender to request a decicated
  //          Autograd key for the backend. See Note [Ambiguity in AutogradOther
  //          kernel] for more details. A CompositeExplicitAutograd kernel
  //          prevents CompositeImplicitAutograd kernel being used for Autograd
  //          keys, but it doesn't cause confusion for AutogradOther. It's
  //          pretty straightforward to use Autograd (if available) in this
  //          case.
  ...
  //   CompositExplicitAutogradNonFunctional > CompositeExplicitAutograd >
  //   CompositeImplicitAutograd > Autograd
  // Note [CompositeExplicitAutograd and CompositeImplicitAutograd]
  ...
  //      ambiguousAutogradOtherKernel() if there's registration to any of its
  ...
  // 2.4. For autograd backend keys, use kernel from DispatchKey::Autograd if
  // available
...
void OperatorEntry::updateDispatchTable_(
  ...
  // don't have the dense key set. E.g. DispatchKey::Meta, DispatchKey::ORT.
...
void OperatorEntry::updateDispatchTableFull_(
  ...
  // CompositeExplicitAutograd, or CompositeImplicitAutograd alias key so that
  // we don't break the support. Ideally isIncludedInAlias(Undefined,
...
void OperatorEntry::checkInvariants() const {
  ...
  for (auto k : DispatchKeySet(DispatchKeySet::FULL)) {
    auto expected_k =
        computeDispatchTableEntry(c10::Dispatcher::singleton(), k);
    auto idx = getDispatchTableIndexForDispatchKey(k);
    if (C10_UNLIKELY(idx == -1)) {
      continue;
    }
    TORCH_INTERNAL_ASSERT(
        expected_k._equalsBoxedAndUnboxed(dispatchTable_[idx]),
        "Canonical state\n~~~~~~~~~~~\n",
        dumpState(),
        "\n\n"
        "Computed table:\n~~~~~~~~~~~\n",
...
void OperatorEntry::reportSignatureError(
    const CppSignature call_signature) const {
  TORCH_CHECK(
      false,
...
std::string OperatorEntry::dumpComputedTable() const {
  ...
  // Need to handle Undefined separately, because its a runtime key that can't
  // be represented in a DispatchKeySet.
...
std::string OperatorEntry::dumpState() const {
  ...
  auto print_kernel = [&](const char* k_desc,
                          const AnnotatedKernelContainer& jts,
