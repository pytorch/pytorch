...
void checkSchema(
    ...
    const std::string& from_def_debug,
    const FunctionSchema& inferred,
    const std::string& inferred_debug) {
  c10::optional<std::string> schema_difference =
      findSchemaDifferences(from_def, inferred);
...
const AnnotatedKernel OperatorEntry::ambiguousAutogradOtherKernel_ =
    AnnotatedKernel(
        c10::KernelFunction::makeAmbiguousAutogradOther(),
        nullptr,
        "ambiguous_autogradother");

void OperatorEntry::registerSchema(
  ...
  for (auto i = kernels_.begin(); i != kernels_.end(); ++i) {
    for (auto j = i->second.begin(); j != i->second.end(); ++j) {
      if (j->inferred_function_schema != nullptr) {
        checkSchema(
            name_, schema, debug, *j->inferred_function_schema, j->debug);
      }
    }
  }
...
std::list<AnnotatedKernel>::iterator OperatorEntry::registerKernel(
  ...
  if (cpp_signature.has_value()) {
    if (cpp_signature_.has_value()) {
      TORCH_CHECK(
          *cpp_signature == cpp_signature_->signature,
          "\nMismatch in kernel C++ signatures\n",
          "  operator: ",
          (this->schema_.has_value() ? toString(this->schema_->schema)
                                     : toString(name_)),
          "\n",
          "    ",
          ...
          "\n",
          "    dispatch key: ",
          toString(cpp_signature_->dispatch_key),
          "\n",
          "    ",
  ...
  auto& k = dispatch_key.has_value()
      ? kernels_[*dispatch_key]
      : kernels_[DispatchKey::CompositeImplicitAutograd];
  ...
  k.emplace_front(
      std::move(kernel), std::move(inferred_function_schema), std::move(debug));
...
void OperatorEntry::updateFallback(
    const c10::Dispatcher& dispatcher,
    DispatchKey dispatch_key) {
  updateDispatchTable_(dispatcher, dispatch_key);
}

const KernelFunction& OperatorEntry::computeDispatchTableEntry(
    const c10::Dispatcher& dispatcher,
    DispatchKey dispatch_key) const {
  return computeDispatchTableEntryWithDebug(dispatcher, dispatch_key)
      .first.kernel;
}

bool OperatorEntry::hasKernelForAnyDispatchKey(DispatchKeySet ks) const {
  TORCH_INTERNAL_ASSERT(
      kernels_.find(DispatchKey::Undefined) == kernels_.end());
...
c10::optional<const AnnotatedKernel*> OperatorEntry::getKernelForDispatchKey(
    DispatchKey dispatch_key) const {
...
std::pair<const AnnotatedKernel&, const char*> OperatorEntry::
  ...
  //    (2.1) Use kernel from DispatchKey::CompositeExplicitAutograd if
  //    available.
  //          This is used to register a kernel that works for all backend in
  //          inference. But it requires separate registration for Autograd keys
  //          to support training.
  //    (2.2) Use kernel from DispatchKey::CompositeImplicitAutograd if
  //    available.
  //          For autograd keys, we only use kernel from
  //          CompositeImplicitAutograd when there's no direct registration to
  //          its corresponding backend key or CompositeExplicitAutograd. See
  //          Note [CompositeExplicitAutograd and CompositeImplicitAutograd].
  //          For AutogradOther, we eagerly return ambiguousAutogradOtherKernel_
  //          if there's registration to any of its backends and ask backend
  //          extender to request a decicated Autograd key for the backend. See
  //          Note [Ambiguity in AutogradOther kernel] for more details. A
  //          CompositeExplicitAutograd kernel prevents
  //          CompositeImplicitAutograd kernel being used for Autograd keys, but
  //          it doesn't cause confusion for AutogradOther. It's pretty
  //          straightforward to use Autograd (if available) in this case.
  //    (2.3) Use kernel from DispatchKey::Autograd if available
  //    The implementation of (2.2) relies on the invariant that for a given
  //    backend, `computeDispatchTableEntryWithDebug()` will be called for that
  //    backend's autograd key after the backend key. See Note [Refresh Runtime
  //    Autograd entries in dispatchTable_]
  ...
  // Note [CompositeExplicitAutograd and CompositeImplicitAutograd]
  //   When there're registrations to both CompositeExplicitAutograd &
  //   CompositeImplicitAutograd & Autograd, from (2.2) we know
  //   CompositeExplicitAutograd and Autograd kernels will be picked up and
  //   CompositeImplicitAutograd is overriden. This is fine and in practice
  //   CompositeExplicitAutograd and CompositeImplicitAutograd shouldn't
  //   co-exist for an op.
  // TODO: Update alias key precedence after we add new alias keys
  // AutogradDispatchCPUOrCUDA .
  ...
  //     See Note [Undefined in dispatchTable_] for the special handling for
  //     Undefined.
  if (dispatch_key == DispatchKey::Undefined ||
      isIncludedInAlias(dispatch_key, DispatchKey::CompositeExplicitAutograd)) {
    if (auto default_backend_registration =
            getKernelForDispatchKey(DispatchKey::CompositeExplicitAutograd)) {
      return {*default_backend_registration.value(), "default backend kernel"};
    }
  }

  // Note when there's direct registration to CompositeExplicitAutograd, this
  // code path will only be hit by non backend keys (e.g AutogradXXX, Batched
  // etc) due to (2.1).
  bool has_backend_kernel = hasKernelForAnyDispatchKey(
      getBackendKeySetFromAutograd(dispatch_key)
          .add(DispatchKey::CompositeExplicitAutograd));

  // 2.2. Use CompositeImplicitAutograd kernel if available. For autograd keys,
  // we only use kernel from CompositeImplicitAutograd
  //      when there's no direct registration to its corresponding backend key
  //      or CompositeExplicitAutograd. For AutogradOther, we return
  //      ambiguousAutogradOtherKernel_ if there's registration to any of its
  //      backends. See Note [Undefined in dispatchTable_] for the special
  //      handling for Undefined.
  if (dispatch_key == DispatchKey::Undefined ||
      isIncludedInAlias(dispatch_key, DispatchKey::CompositeImplicitAutograd)) {
    if (auto math_registration =
            getKernelForDispatchKey(DispatchKey::CompositeImplicitAutograd)) {
  ...
  // 2.3. For autograd backend keys, use kernel from DispatchKey::Autograd if
  // available
  if (isIncludedInAlias(dispatch_key, DispatchKey::Autograd)) {
    if (auto autograd_registration =
            getKernelForDispatchKey(DispatchKey::Autograd)) {
  ...
  if (dispatcher.backendFallbackKernels_[dispatch_ix].kernel.isValid()) {
    return {
        dispatcher.backendFallbackKernels_[dispatch_ix], "backend fallback"};
  }
...
// This function should be considered a private helper for
...
void OperatorEntry::updateDispatchTable_(
  ...
  // Registration to CompositeExplicitAutograd and CompositeImplicitAutograd
  // should be populated to Undefined. We cannot do this above since Undefined
  // cannot be represented in DispatchKeySet.
...
// invariants.
//
void OperatorEntry::updateDispatchTableFull_(
    const c10::Dispatcher& dispatcher) {
  // Note [Undefined in dispatchTable_]
  // DispatchKey Undefined is used in runtime:
  // (1) it gives people place to specify functionality that should run when
  // there are no dispatch keys,
  //     e.g., an op without Tensor inputs or empty TensorList arguments
  // (2) it would let us remove the explicit error checking code in the dispatch
  // hotpath, and so when
  //     no dispatch keys are available we just slide into the undefined handler
  //     which would then raise the error message.
  // In the old world of catchAll, the only way to "register" a kernel to
  // Undefined is by registering it to catchAll. After catchAllKernel_ is
  // removed, Undefined now can get a kernel from either
  // CompositeExplicitAutograd or CompositeImplicitAutograd alias key so that we
  // don't break the support. Ideally isIncludedInAlias(Undefined,
  // CompositeImplicitAutograd) should return true, it returns false because
  // Undefined cannot be represented in a DispatchKeySet.
  for (uint8_t iter = 0;
...
void OperatorEntry::checkInvariants() const {
  if (schema_) {
    TORCH_INTERNAL_ASSERT(
        schema_->schema.operator_name() == name_, dumpState());
    dispatchKeyExtractor().checkInvariants(schema_->schema);
  }
  TORCH_INTERNAL_ASSERT(
      kernels_.find(DispatchKey::Undefined) == kernels_.end(), dumpState());
  ...
  for (uint8_t iter = 0;
       iter != static_cast<uint8_t>(DispatchKey::NumDispatchKeys);
       ++iter) {
    auto expected_k = computeDispatchTableEntry(
        c10::Dispatcher::singleton(), static_cast<DispatchKey>(iter));
    TORCH_INTERNAL_ASSERT(
...
std::string OperatorEntry::dumpComputedTable() const {
  ...
  for (uint8_t i = 0; i < static_cast<uint8_t>(DispatchKey::NumDispatchKeys);
       i++) {
...
// *non-derived* information including kernels registered to alias dispatch
// keys; i.e., what the source of truth says about the operator.  This dumping
// function is appropriate for expect tests. This WON'T report backend
// fallbacks.
std::string OperatorEntry::dumpState() const {
  ...
  auto print_kernel = [&](const char* k_desc,
                          const std::list<AnnotatedKernel>& jts,
                          bool is_alias_key = false) {
    int64_t i = 0;
    for (const auto& jt : jts) {
      oss << k_desc << (is_alias_key ? "[alias]" : "")
          << (i > 0 ? " (inactive)" : "") << ": " << jt.debug << " :: "
          << (jt.inferred_function_schema
                  ? toString(*jt.inferred_function_schema)
                  : "(none)")
          << " [ " << jt.kernel.dumpState() << "]\n";
      i++;
    }
  ...
  for (uint8_t i = 0; i <= static_cast<uint8_t>(DispatchKey::EndOfAliasKeys);
       i++) {
    auto k = static_cast<DispatchKey>(i);
