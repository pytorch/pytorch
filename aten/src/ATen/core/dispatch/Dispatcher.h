...
private:
  ...
  template<class Return, class... Args>
  Return doCallUnboxed(const DispatchTable& dispatchTable, Args... args) const;
  template<class Return, class... Args>
  Return doCallUnboxedOnly(const DispatchTable& dispatchTable, Args... args) const;
...
inline Return Dispatcher::callUnboxed(const OperatorHandle& op, Args... args) const {
  ...
  return doCallUnboxed<Return, Args...>(op.operatorIterator_->op.dispatch_table(), std::forward<Args>(args)...);
}

template<class Return, class... Args>
inline Return Dispatcher::doCallUnboxed(const DispatchTable& dispatchTable, Args... args) const {
  detail::unused_arg_(args...);  // workaround for a false-positive warning about unused parameters in gcc 5
...
inline Return Dispatcher::callUnboxedOnly(const OperatorHandle& op, Args... args) const {
  ...
  // TODO This should be a nested lambda instead of a separate function call, but that triggers an internal
  // compiler error on GCC5. Change this once we don't need gcc 5 anymore.
  return doCallUnboxedOnly<Return, Args...>(op.operatorIterator_->op.dispatch_table(), std::forward<Args>(args)...);
}

template<class Return, class... Args>
inline Return Dispatcher::doCallUnboxedOnly(const DispatchTable& dispatchTable, Args... args) const {
  detail::unused_arg_(args...);  // workaround for a false-positive warning about unused parameters in gcc 5
