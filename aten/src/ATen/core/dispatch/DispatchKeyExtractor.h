...
private:
  static c10::utils::bitset makeBitsetForDispatchArgs(const FunctionSchema& schema) {
    ...
    for (size_t index = 0; index < schema.arguments().size(); ++index) {
      if (schema.arguments()[index].type()->isSubtypeOf(TensorType::get()) || schema.arguments()[index].type()->isSubtypeOf(ListType::ofTensors()) || schema.arguments()[index].type()->isSubtypeOf(OptionalType::ofTensor())) {
