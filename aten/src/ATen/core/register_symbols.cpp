...
#define SYMBOL_ENTRY(n, s) {#s, n::s, namespaces::n},

    FORALL_NS_SYMBOLS(SYMBOL_ENTRY)
#undef SYMBOL_ENTRY
};

constexpr c10::string_view kNamespaces[] = {
#define NAMESPACE_STR(_, s) #s,
FORALL_NS_NAMESPACES(NAMESPACE_STR)
#undef NAMESPACE_STR
};

std::string qual_name_for_entry(const Entry& entry) {
  const char *const sep = "::";
  const auto namespace_len = kNamespaces[entry.ns_sym].size();
  ...
  s.append(kNamespaces[entry.ns_sym].data(), namespace_len);
