...
Dict<Key, Value>::Dict()
    : Dict(make_intrusive<detail::DictImpl>(
          ...
          detail::DictImpl::DictElementTypes{
              ...
              getTypePtr<Value>()})) {
  static_assert(
      !std::is_same<Key, IValue>::value,
      "This constructor is not valid for Dict<IValue, _>. Please use c10::impl::GenericDict(keyType, valueType) instead, or if you absolutely have to, use c10::impl::GenericDict(c10::impl::deprecatedUntypedDict()).");
  static_assert(
      !std::is_same<Value, IValue>::value,
      "This constructor is not valid for Dict<_, IValue>. Please use c10::impl::GenericDict(keyType, valueType) instead, or if you absolutely have to, use c10::impl::GenericDict(c10::impl::deprecatedUntypedDict()).");
}
...
Dict<Key, Value>::Dict(Dict&& rhs) noexcept : impl_(std::move(rhs.impl_)) {
  rhs.impl_ = make_intrusive<detail::DictImpl>(
      detail::DictImpl::dict_map_type(), impl_->elementTypes);
}
...
std::pair<typename Dict<Key, Value>::iterator, bool> Dict<Key, Value>::insert(
    ...
    Value_&& value) const {
  static_assert(
      std::is_constructible<Key, Key_>::value,
