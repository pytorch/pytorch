...
struct TORCH_API UnionType : public Type {
  ...
  bool can_hold_type(TypePtr& tptr) const {
    return std::any_of(types_.begin(), types_.end(), [&](TypePtr union_tptr){
      return tptr->isSubtypeOf(union_tptr);
    });
  }

  bool can_hold_type(const TypePtr& tptr) const {
    return std::any_of(types_.begin(), types_.end(), [&](TypePtr union_tptr){
      return tptr->isSubtypeOf(union_tptr);
    });
  }
