...
// Note to person merging utils, feel free to delete this and replace with the
// caffe2::dynamic_cast_if_rtti version.
//
...
struct AT_API Type : std::enable_shared_from_this<Type> {
private:
  TypeKind kind_;
  template<bool is_singleton, typename T>
...
public:
  ...
  std::shared_ptr<T> cast() {
    auto r = detail::dynamic_pointer_cast_if_rtti<T>(shared_from_this());
    if (!r || T::Kind == kind()) {
      return r;
    } else {
  ...
  std::shared_ptr<const T> cast() const {
    auto r = detail::dynamic_pointer_cast_if_rtti<const T>(shared_from_this());
    if (!r || T::Kind == kind()) {
      return r;
    } else {
      return cloneType<T::is_singleton, T>{}(r);
...
struct AT_API DynamicType : public Type {
...
struct AT_API TensorType : public Type {
...
// This node represents a single Tensor value with a specific size
struct AT_API CompleteTensorType : public TensorType {
  static constexpr bool is_singleton = false;
  friend struct Type;
  template<typename ... T>
...
struct AT_API ListType : public Type {
...
struct AT_API TupleType : public Type {
  static constexpr bool is_singleton = false;
  friend struct Type;
...
struct AT_API NumberType : public Type {
  static constexpr bool is_singleton = true;
  template<typename ... T>
...
struct AT_API FloatType : public Type {
  static constexpr bool is_singleton = true;
  template<typename ... T>
...
struct AT_API IntType : public Type {
  static constexpr bool is_singleton = true;
  template<typename ... T>
...
struct AT_API StringType : public Type {
  static constexpr bool is_singleton = true;
  template<typename ... T>
...
AT_API at::optional<TypePtr> unifyTypes(const TypePtr& t1, const TypePtr& t2);
...
AT_API TypePtr inferTypeFrom(const IValue& value);
