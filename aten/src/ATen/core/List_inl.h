...
List<T>::List()
    : List(make_intrusive<c10::detail::ListImpl>(
          ...
          getTypePtr<T>())) {
  static_assert(
      !std::is_same<T, IValue>::value,
      "This constructor is not valid for List<IValue>. Please use c10::impl::GenericList(elementType) instead.");
}
...
List<T>::List(ArrayRef<T> values)
  ...
  static_assert(
      !std::is_same<T, IValue>::value,
      "This constructor is not valid for List<IValue>. Please use c10::impl::GenericList(elementType).");
...
List<T>::List(TypePtr elementType)
  ...
  static_assert(
      std::is_same<T, IValue>::value ||
          std::is_same<T, c10::intrusive_ptr<ivalue::Future>>::value,
...
List<T> toTypedList(impl::GenericList list) {
  // If there's other instances of the list (i.e. list.use_count() > 1), then we
  // have to be invariant because upcasting would allow people to add types into
  // the new list that would break the old list. However, if there aren't any
  // other instances of this list (i.e. list.use_count() == 1), then we can
  // allow upcasting. This can be a perf improvement since we can cast List<T>
  // to List<optional<T>> without having to copy it. This is also used to
  // provide backwards compatibility with some old models that serialized the
  // index arguments to aten::index, aten::index_put, aten::index_put_ and
  // aten::index_put_impl_ as List<Tensor> before we changed that argument to be
  // List<optional<Tensor>>. When deserializing, we have list.use_count() == 1
  // and can deserialize the List<Tensor> directly as List<optional<Tensor>>.
  TORCH_CHECK(
      *list.impl_->elementType == *getTypePtr<T>() ||
          (list.use_count() == 1 &&
           list.impl_->elementType->isSubtypeOf(getTypePtr<T>())),
      "Tried to cast a List<",
      toString(list.impl_->elementType),
      "> to a List<",
      toString(getTypePtr<T>()),
      ">. Types mismatch.");
...
List<T>::List(List&& rhs) noexcept : impl_(std::move(rhs.impl_)) {
  rhs.impl_ = make_intrusive<c10::detail::ListImpl>(
      std::vector<IValue>{}, impl_->elementType);
}
...
template <class T, class Iterator>
ListElementReference<T, Iterator>& ListElementReference<T, Iterator>::operator=(
    T&& new_value) && {
...
template <class T, class Iterator>
ListElementReference<T, Iterator>& ListElementReference<T, Iterator>::operator=(
    ListElementReference<T, Iterator>&& rhs) && {
...
inline bool operator==(
    const T& lhs,
    const ListElementReference<T, Iterator>& rhs) {
  return rhs == lhs;
}
...
template <>
inline typename ListElementConstReferenceTraits<
    c10::optional<std::string>>::const_reference
...
template <class T>
typename List<T>::internal_const_reference_type List<T>::operator[](
    size_type pos) const {
...
typename List<T>::iterator List<T>::insert(iterator pos, const T& value) const {
  return iterator{impl_->list.insert(
      pos.iterator_, c10::detail::ListElementFrom<T>::from(value))};
}

template <class T>
typename List<T>::iterator List<T>::insert(iterator pos, T&& value) const {
  return iterator{impl_->list.insert(
      pos.iterator_, c10::detail::ListElementFrom<T>::from(std::move(value)))};
}

template <class T>
template <class... Args>
typename List<T>::iterator List<T>::emplace(iterator pos, Args&&... value)
    const {
  // TODO Use list_element_from?
  return iterator{
      impl_->list.emplace(pos.iterator_, std::forward<Args>(value)...)};
}
...
void List<T>::push_back(T&& value) const {
  impl_->list.push_back(
      c10::detail::ListElementFrom<T>::from(std::move(value)));
}
...
void List<T>::append(List<T> b) const {
  if (b.use_count() == 1) {
    impl_->list.insert(
        impl_->list.end(),
        make_move_iterator(b.impl_->list.begin()),
        make_move_iterator(b.impl_->list.end()));
  } else {
    impl_->list.insert(
        impl_->list.end(), b.impl_->list.begin(), b.impl_->list.end());
  }
}
