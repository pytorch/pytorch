...
class FileManager(object):
    ...
    def write(self, filename, s, env=None):
        path = '{}/{}'.format(options.output_dir, filename)
        if isinstance(s, CodeTemplate):
            assert env is not None
            env['generated_comment'] = GENERATED_COMMENT.substitute(filename=filename)
            s = s.substitute(env)
        self._write_if_changed(path, s)
        if path not in self.filenames:
            self.undeclared_files.append(path)
        else:
...
GENERATED_COMMENT = CodeTemplate(
    "@" + "generated from aten/src/ATen/templates/${filename}")
