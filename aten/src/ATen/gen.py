...
def generate_outputs():
    ...
    # Note [Named tensor build codegen]
    # We filter out named-tensor only declarations if BUILD_NAMEDTENSOR=0.
    #
    # Here are some things to remember when working with BUILD_NAMEDTENSOR=1.
    # (1) When working on named tensor code, one should remember NOT to check
    # in {Type|Tensor|Tensormethods}.h
    # (2) The autogenerated {Type|Tensor|TensorMethods}.h get checked into the
    # codebase and need to be consistent no matter what BUILD_NAMEDTENSOR is.
    # The autogenerated files that are checked in come from a
    # BUILD_NAMEDTENSOR=0 build.
    # (3) When BUILD_NAMEDTENSOR=1, GEN_TO_SOURCE=1 gets set automatically.
    #
    # (1), (2), and (3) are a pretty hacky solution. Here is some context for
    # why we do it.
    # We could make it so that the generated code is consistent no matter
    # what BUILD_NAMEDTENSOR is. However, because the project is experimental,
    # we will most likely be modifying the generated {Type|Tensor|TensorMethods}.h
    # a lot. This can and will lead to merge conflicts because it is bad practice
    # to check in autogenerated code (see #24931). Therefore we can either
    # resolve #24921, or do this workaround.
