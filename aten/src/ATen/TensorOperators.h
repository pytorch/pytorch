...
_(+,x.add(y), y.add(x)) \
_(*,x.mul(y), y.mul(x)) \
_(-,x.sub(y), ::at::empty_like(y, LEGACY_CONTIGUOUS_MEMORY_FORMAT).fill_(x).sub_(y)) \
_(/,x.div(y), ::at::empty_like(y, LEGACY_CONTIGUOUS_MEMORY_FORMAT).fill_(x).div_(y)) \
_(%,x.remainder(y), ::at::empty_like(y, LEGACY_CONTIGUOUS_MEMORY_FORMAT).fill_(x).remainder_(y)) \
_(<,x.lt(y), y.gt(x)) \
_(<=,x.le(y), y.ge(x)) \
_(>,x.gt(y),y.lt(x)) \
_(>=,x.ge(y), y.le(x)) \
