...
public:
  ...
  void raw_resize_(int64_t sparse_dim, int64_t dense_dim, IntList size) {
    AT_CHECK(allow_tensor_metadata_change(), "raw_resize_ " + FLAGS_err_msg_for_changing_metadata_on_derived_tensor);
    sizes_ = size.vec();
  ...
  void resize_(int64_t sparse_dim, int64_t dense_dim, IntList size) {
    AT_CHECK(allow_tensor_metadata_change(), "resize_ " + FLAGS_err_msg_for_changing_metadata_on_derived_tensor);
...
2. If you need to resize this tensor, you have the following options:\n\
  ...
  void resize_and_clear_(int64_t sparse_dim, int64_t dense_dim, IntList size) {
    AT_CHECK(allow_tensor_metadata_change(), "resize_and_clear_ " + FLAGS_err_msg_for_changing_metadata_on_derived_tensor);
    AT_CHECK(sparse_dim + dense_dim == size.size(), "number of dimensions must be sparse_dim (", sparse_dim, ") + dense_dim (", dense_dim, "), but got ", size.size());
  ...
  void set_coalesced(bool coalesced) {
    AT_CHECK(allow_tensor_metadata_change(), "set_coalesced " + FLAGS_err_msg_for_changing_metadata_on_derived_tensor);
    coalesced_ = coalesced;
  }

  // NOTE: this function is only used internally and not exposed to Python frontend
  void set_nnz_and_narrow(int64_t new_nnz) {
    AT_CHECK(allow_tensor_metadata_change(), "set_nnz_and_narrow " + FLAGS_err_msg_for_changing_metadata_on_derived_tensor);
