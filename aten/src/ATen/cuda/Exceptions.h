...
#define AT_CUDNN_CHECK_WITH_SHAPES(EXPR, ...) \
  AT_CUDNN_CHECK(EXPR, "\n", ##__VA_ARGS__)
...
// When cuda < 11.5, cusolver raises CUSOLVER_STATUS_EXECUTION_FAILED when input
// contains nan. When cuda >= 11.5, cusolver normally finishes execution and
// sets info array indicating convergence issue.
#define TORCH_CUSOLVER_CHECK(EXPR)                                             \
  do {                                                                         \
    cusolverStatus_t __err = EXPR;                                             \
    if ((CUDA_VERSION < 11500 && __err == CUSOLVER_STATUS_EXECUTION_FAILED) || \
        (CUDA_VERSION >= 11500 && __err == CUSOLVER_STATUS_INVALID_VALUE)) {   \
      TORCH_CHECK_LINALG(                                                      \
          false,                                                               \
          "cusolver error: ",                                                  \
