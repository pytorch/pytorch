...
ResultEntry TuningResultsManager::Lookup(const std::string& op_signature, const std::string& params_signature) {
  ...
  // Exact-match-only Lookup: callers that want a wildcard key (e.g. operator()
  // in TunableOp.h, or try_dispatch in Blas.cpp) must pass the already-formed
  // wildcard signature. This neither expands wildcards in the request nor
  // matches a wildcard request against concrete candidates; wildcard fallback
  // lives in LookupWildcardFallback below.
  TUNABLE_LOG3(
