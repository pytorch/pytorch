...
#define CUDASOLVER_GETRF_ARGTYPES(Dtype)                                   \
  cusolverDnHandle_t handle, int m, int n, Dtype *dA, int ldda, int *ipiv, \
      int *info
...
void getrf(CUDASOLVER_GETRF_ARGTYPES(Dtype)) {
  TORCH_CHECK(
      false,
      "at::cuda::solver::getrf: not implemented for ",
      typeid(Dtype).name());
}
template <>
...
void getrf<c10::complex<double>>(
    CUDASOLVER_GETRF_ARGTYPES(c10::complex<double>));
template <>
void getrf<c10::complex<float>>(CUDASOLVER_GETRF_ARGTYPES(c10::complex<float>));

#define CUDASOLVER_GETRS_ARGTYPES(Dtype)                                     \
  cusolverDnHandle_t handle, int n, int nrhs, Dtype *dA, int lda, int *ipiv, \
      Dtype *ret, int ldb, int *info

template <class Dtype>
void getrs(CUDASOLVER_GETRS_ARGTYPES(Dtype)) {
  TORCH_CHECK(
      false,
      "at::cuda::solver::getrs: not implemented for ",
      typeid(Dtype).name());
}
template <>
...
void getrs<c10::complex<double>>(
    CUDASOLVER_GETRS_ARGTYPES(c10::complex<double>));
template <>
...
#define CUDASOLVER_GESVDJ_ARGTYPES(Dtype, Vtype)                             \
  cusolverDnHandle_t handle, cusolverEigMode_t jobz, int econ, int m, int n, \
      Dtype *A, int lda, Vtype *S, Dtype *U, int ldu, Dtype *V, int ldv,     \
...
void gesvdj(CUDASOLVER_GESVDJ_ARGTYPES(Dtype, Vtype)) {
  TORCH_INTERNAL_ASSERT(
      false,
      "at::cuda::solver::gesvdj: not implemented for ",
      typeid(Dtype).name());
}
template <>
...
void gesvdj<c10::complex<float>>(
    CUDASOLVER_GESVDJ_ARGTYPES(c10::complex<float>, float));
template <>
void gesvdj<c10::complex<double>>(
    CUDASOLVER_GESVDJ_ARGTYPES(c10::complex<double>, double));

#define CUDASOLVER_GESVDJ_BATCHED_ARGTYPES(Dtype, Vtype)                     \
  cusolverDnHandle_t handle, cusolverEigMode_t jobz, int m, int n, Dtype *A, \
      int lda, Vtype *S, Dtype *U, int ldu, Dtype *V, int ldv, int *info,    \
...
void gesvdjBatched<c10::complex<float>>(
    CUDASOLVER_GESVDJ_BATCHED_ARGTYPES(c10::complex<float>, float));
template <>
void gesvdjBatched<c10::complex<double>>(
    CUDASOLVER_GESVDJ_BATCHED_ARGTYPES(c10::complex<double>, double));
...
void potrf(CUDASOLVER_POTRF_ARGTYPES(Dtype)) {
  TORCH_INTERNAL_ASSERT(
      false,
      "at::cuda::solver::potrf: not implemented for ",
      typeid(Dtype).name());
}
template <>
...
void potrf<c10::complex<double>>(
    CUDASOLVER_POTRF_ARGTYPES(c10::complex<double>));
...
void potrf_buffersize(CUDASOLVER_POTRF_BUFFERSIZE_ARGTYPES(Dtype)) {
  TORCH_INTERNAL_ASSERT(
      false,
      "at::cuda::solver::potrf_buffersize: not implemented for ",
      typeid(Dtype).name());
}
template <>
...
void potrf_buffersize<c10::complex<float>>(
    CUDASOLVER_POTRF_BUFFERSIZE_ARGTYPES(c10::complex<float>));
template <>
void potrf_buffersize<c10::complex<double>>(
    CUDASOLVER_POTRF_BUFFERSIZE_ARGTYPES(c10::complex<double>));
...
void potrfBatched(CUDASOLVER_POTRF_BATCHED_ARGTYPES(Dtype)) {
  TORCH_INTERNAL_ASSERT(
      false,
      "at::cuda::solver::potrfBatched: not implemented for ",
      typeid(Dtype).name());
}
template <>
...
void potrfBatched<c10::complex<float>>(
    CUDASOLVER_POTRF_BATCHED_ARGTYPES(c10::complex<float>));
template <>
void potrfBatched<c10::complex<double>>(
    CUDASOLVER_POTRF_BATCHED_ARGTYPES(c10::complex<double>));
...
#define CUDASOLVER_POTRS_ARGTYPES(Dtype)                             \
  cusolverDnHandle_t handle, cublasFillMode_t uplo, int n, int nrhs, \
      const Dtype *A, int lda, Dtype *B, int ldb, int *devInfo

template <class Dtype>
void potrs(CUDASOLVER_POTRS_ARGTYPES(Dtype)) {
  TORCH_INTERNAL_ASSERT(
      false,
      "at::cuda::solver::potrs: not implemented for ",
      typeid(Dtype).name());
}
template <>
...
void potrs<c10::complex<double>>(
    CUDASOLVER_POTRS_ARGTYPES(c10::complex<double>));
...
void potrsBatched(CUDASOLVER_POTRS_BATCHED_ARGTYPES(Dtype)) {
  TORCH_INTERNAL_ASSERT(
      false,
      "at::cuda::solver::potrsBatched: not implemented for ",
      typeid(Dtype).name());
}
template <>
...
void potrsBatched<c10::complex<float>>(
    CUDASOLVER_POTRS_BATCHED_ARGTYPES(c10::complex<float>));
template <>
void potrsBatched<c10::complex<double>>(
    CUDASOLVER_POTRS_BATCHED_ARGTYPES(c10::complex<double>));
...
void orgqr_buffersize<c10::complex<float>>(
    CUDASOLVER_ORGQR_BUFFERSIZE_ARGTYPES(c10::complex<float>));
template <>
void orgqr_buffersize<c10::complex<double>>(
    CUDASOLVER_ORGQR_BUFFERSIZE_ARGTYPES(c10::complex<double>));
...
void orgqr<c10::complex<double>>(
    CUDASOLVER_ORGQR_ARGTYPES(c10::complex<double>));
...
#define CUDASOLVER_ORMQR_ARGTYPES(Dtype)                                     \
  cusolverDnHandle_t handle, cublasSideMode_t side, cublasOperation_t trans, \
      int m, int n, int k, const Dtype *A, int lda, const Dtype *tau,        \
      Dtype *C, int ldc, Dtype *work, int lwork, int *devInfo
...
cudaDataType get_cusolver_datatype() {
  TORCH_CHECK(
      false, "cusolver doesn't support data type ", typeid(Dtype).name());
}
template <>
cudaDataType get_cusolver_datatype<float>();
template <>
cudaDataType get_cusolver_datatype<double>();
template <>
cudaDataType get_cusolver_datatype<c10::complex<float>>();
template <>
cudaDataType get_cusolver_datatype<c10::complex<double>>();
