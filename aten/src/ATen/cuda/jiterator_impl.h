...
#define DEFINE_CASE(ctype, scalartype) \
  case ScalarType::scalartype:         \
    return memory::can_vectorize_up_to<ctype>(pointer);
...
int jitted_can_vectorize_up_to(const TensorIteratorBase& iter) {
  ...
  // Deals with output
  int result =
      can_vectorize_up_to(result_dtype, static_cast<char*>(iter.data_ptr(0)));
  ...
  for (auto i = 1; i < iter.ntensors(); ++i) {
    result = std::min<int>(
        result,
        can_vectorize_up_to(
            common_dtype, static_cast<char*>(iter.data_ptr(i))));
  }
...
static std::unique_ptr<OffsetCalculator<N>> make_unique_offset_calculator(
  ...
  return std::make_unique<OffsetCalculator<N>>(
...
#define DEFINE_CASE(index) std::unique_ptr<OffsetCalculator<index>>,
  using OffsetCalculatorTypes = c10::variant<AT_FOR_8_CASES(DEFINE_CASE)>;
#undef DEFINE_CASE
...
#define DEFINE_CASE(index)                                    \
  case index:                                                 \
    v = make_unique_offset_calculator<IS_INPUT, index>(iter); \
    break;
...
#undef DEFINE_CASE
      default:
        TORCH_CHECK(
            false,
            "OffsetCalculatorVariant is not implemented for num_tensor = ",
            num);
    }
  }
...
#define DEFINE_CASE(index) \
  at::detail::Array<char*, index>, at::detail::Array<char*, index + 8>,
  using ArrayTypes = c10::variant<AT_FOR_8_CASES(DEFINE_CASE)>;
#undef DEFINE_CASE
...
#define DEFINE_CASE(index)                         \
  case index:                                      \
    array = at::detail::Array<char*, index>{};     \
    break;                                         \
  case index + 8:                                  \
    array = at::detail::Array<char*, index + 8>{}; \
    break;
...
#undef DEFINE_CASE
      ...
      default:
        TORCH_CHECK(
            false, "ArrayVariant is not implemented for ntensors = ", ntensors);
    }
...
#define DEFINE_CASE(index) TrivialOffsetCalculator<index>,
  using TrivialOffsetCalculatorTypes =
      c10::variant<AT_FOR_8_CASES(DEFINE_CASE)>;
#undef DEFINE_CASE
...
#define DEFINE_CASE(index)                \
  case index:                             \
    v = TrivialOffsetCalculator<index>(); \
    break;
...
#undef DEFINE_CASE
      ...
      default:
        TORCH_CHECK(
            false,
            "TrivialOffsetCalculatorVariant is not implemented for num_tensors = ",
            num);
    }
  }
...
#define DEFINE_CASE(index) std::unique_ptr<memory::LoadWithCast<index>>,
  using LoadWithCastPtr = c10::variant<AT_FOR_8_CASES(DEFINE_CASE)>;
#undef DEFINE_CASE
...
#define DEFINE_CASE(index)                                   \
  case index:                                                \
    v = std::make_unique<memory::LoadWithCast<index>>(iter); \
    break;
...
#undef DEFINE_CASE
      ...
      default:
        TORCH_CHECK(
            false,
            "LoadWithCastVariant is not implemented for ninputs = ",
            arity);
    }
  }
...
#define DEFINE_CASE(index) std::unique_ptr<memory::StoreWithCast<index>>,
  using StoreWithCastPtr = c10::variant<AT_FOR_8_CASES(DEFINE_CASE)>;
#undef DEFINE_CASE
...
#define DEFINE_CASE(index)                                    \
  case index:                                                 \
    v = std::make_unique<memory::StoreWithCast<index>>(iter); \
    break;
...
#undef DEFINE_CASE
      ...
      default:
        TORCH_CHECK(
            false,
            "StoreWithCastVariant is not implemented for noutputs = ",
            num);
    }
  }
