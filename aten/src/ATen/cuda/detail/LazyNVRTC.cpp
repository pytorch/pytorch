...
static std::string getLibVersion() {
  // [NVRTC versioning]
  // Quote of https://docs.nvidia.com/cuda/nvrtc/index.html Section 8.1. NVRTC
  // library versioning
  //
  // In the following, MAJOR and MINOR denote the major and minor versions of
  // the CUDA Toolkit. e.g. for CUDA 11.2, MAJOR is "11" and MINOR is "2".
  //
  // Linux:
  //   - In CUDA toolkits prior to CUDA 11.3, the soname was set to
  //   "MAJOR.MINOR".
  //   - In CUDA 11.3 and later 11.x toolkits, the soname field is set to
  //   "11.2".
  //   - In CUDA toolkits with major version > 11 (e.g. CUDA 12.x), the soname
  //   field is set to "MAJOR".
  //
  // Windows:
  //   - In CUDA toolkits prior to cuda 11.3, the DLL name was of the form
  //   "nvrtc64_XY_0.dll", where X = MAJOR, Y = MINOR.
  //   - In CUDA 11.3 and later 11.x toolkits, the DLL name is
  //   "nvrtc64_112_0.dll".
  //   - In CUDA toolkits with major version > 11 (e.g. CUDA 12.x), the DLL name
  //   is of the form "nvrtc64_X0_0.dll" where X = MAJOR.
  //
  // Consider a CUDA toolkit with major version > 11. The NVRTC library in this
  // CUDA toolkit will have the same soname (Linux) or DLL name (Windows) as an
  // NVRTC library in a previous minor version of the same CUDA toolkit.
  // Similarly, the NVRTC library in CUDA 11.3 and later 11.x releases will have
  // the same soname (Linux) or DLL name (Windows) as the NVRTC library in
  // CUDA 11.2.
...
static std::string getAltLibName() {
  ...
  return std::string("libnvrtc-") + C10_STRINGIZE(NVRTC_SHORTHASH) + ".so." +
      getLibVersion();
#else
  return {};
#endif
...
at::DynamicLibrary& getNVRTCLibrary() {
  ...
  static at::DynamicLibrary lib(
      libname.c_str(), alt_libname.empty() ? nullptr : alt_libname.c_str());
  return lib;
}
...
#define CUDA_STUB3(NAME, A1, A2, A3) \
  _STUB_3(CUDA, NAME, CUresult CUDAAPI, A1, A2, A3)
#define CUDA_STUB4(NAME, A1, A2, A3, A4) \
...
#define NVRTC_STUB3(NAME, A1, A2, A3) \
...
nvrtcResult nvrtcCreateProgram(
  ...
  auto fn = reinterpret_cast<decltype(&nvrtcCreateProgram)>(
      getNVRTCLibrary().sym(__func__));
  if (!fn)
...
CUresult CUDAAPI cuLaunchKernel(
    ...
    void** kernelParams,
    void** extra) {
  auto fn = reinterpret_cast<decltype(&cuLaunchKernel)>(
...
CUresult CUDAAPI cuModuleLoadDataEx(
    CUmodule* module,
    const void* image,
    unsigned int numOptions,
    CUjit_option* options,
    void** optionValues) {
  auto fn = reinterpret_cast<decltype(&cuModuleLoadDataEx)>(
      getCUDALibrary().sym(__func__));
  if (!fn)
...
CUresult CUDAAPI cuLinkAddData(
    ...
    CUjit_option* options,
    void** optionValues) {
  auto fn = reinterpret_cast<decltype(&cuLinkAddData)>(
      getCUDALibrary().sym(__func__));
  if (!fn)
