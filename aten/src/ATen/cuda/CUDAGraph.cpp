...
MempoolId_t graph_pool_handle() {
  ...
  TORCH_CHECK(
      false,
      "CUDA graphs may only be used in Pytorch built with CUDA >= 11.0 and not yet supported on ROCM");
  return {0, 0};
#endif
}
 ...
 *
 * A1: Convenience.
 * A2: To ensure valid numerics on replay, some native CUDA ops (like RNG ops
...
void CUDAGraph::capture_begin(MempoolId_t pool /*=0*/) {
  ...
  // CUDAGraph, but could be relaxed in the future: in principle, the underlying
  ...
  // When CUDACachingAllocator allocates while a capture is underway, it calls
  // cudaStreamGetCaptureInfo to get the current stream's capture id, if any.
  // Here we tell CUDACachingAllocator: if the stream has a capture id matching
  // this graph's id_, use the private pool mempool_id_ identifies.
  //
  // There's a small chance of a bad allocation here if another thread launches
  // a kernel on capture_stream_ between the call to cudaStreamBeginCapture
  // above and the call to notifyCaptureBegin below. But I don't think we need
  // to worry about it because that use case makes no sense: The user has no
  // business launching kernels on capture_stream_ from another thread while
  // calling capture_begin. They'll have no idea if their side thread's kernel
  // will end up as part of the capture or not.
  c10::cuda::CUDACachingAllocator::notifyCaptureBegin(
      capture_dev_, id_, mempool_id_);
#else
  TORCH_CHECK(
      false,
      "CUDA graphs may only be used in Pytorch built with CUDA >= 11.0 and not yet supported on ROCM");
...
void CUDAGraph::capture_end() {
  ...
  // who prefer not to report error message through these arguments moving
  // forward (they prefer return value, or errors on api calls internal to the
  // capture)
...
void CUDAGraph::reset() {
  ...
  // CUDAGraph, the generator, and the allocator could end up in all kinds of
  ...
  // Juptyer notebook, I don't see an easy way for reset() to gracefully fix all
