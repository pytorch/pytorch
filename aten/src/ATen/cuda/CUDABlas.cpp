...
static inline bool bgemm_internal_cublaslt(CUDABLAS_BGEMM_ARGTYPES_AND_C_DTYPE(Dtype, C_Dtype)) {
  ...
  if constexpr (std::is_same_v<Dtype, double>) {
    ...
    std::cout<<"aten/src/ATen/cuda/CUDABlas.cpp 407 "<<(at::globalContext().float32Precision("cuda", "matmul") == "tf32")<<"\n";
...
bool gemm_and_bias(
  ...
  if constexpr (std::is_same_v<Dtype, double>) {
    ...
    std::cout<<"aten/src/ATen/cuda/CUDABlas.cpp 1590 "<<(at::globalContext().float32Precision("cuda", "matmul") == "tf32")<<"\n";
    if (at::globalContext().float32Precision("cuda", "matmul") == "tf32") {
      computeType = CUBLAS_COMPUTE_32F_FAST_TF32;
    }
  } else if constexpr (std::is_same_v<Dtype, at::Half>) {
