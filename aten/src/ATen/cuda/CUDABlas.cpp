...
inline bool bgemm_tunable(CUDABLAS_BGEMM_ARGTYPES_AND_C_DTYPE(Dtype, C_Dtype)) {
  ...
  auto try_dispatch = [&](auto& bgemm_op) -> bool {
    ...
    if (result == at::cuda::tunable::ResultEntry::Null()
      ...
      // token pattern (same as the addmm path). AOTI's cpp_wrapper emits no
      // TunableDynamicDimsGuard, so the mask may be empty even when wildcards
      // were persisted at compile-time tuning.
