...
void scaled_gemm(
  ...
  // Handle specific Float8 data types first
...
#else
    TORCH_CHECK(false, "scaled_gemm with `torch.float8_e4m3fn` scales is only supported for CUDA 12.8 or ROCm 7.0(with gfx950) and above");
#endif // if CUDA_VERSION >= 12080
  } else {
