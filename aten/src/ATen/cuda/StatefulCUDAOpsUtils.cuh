...
// Also, whatever call unpacks philox_cuda_state_t in consumer kernels must be inlineable.
// Easiest thing that comes to mind is, define a free function here, in ATen/cuda.
// Any cuda consumer can include this header.
__device__ __forceinline__ std::pair<uint64_t, uint64_t> unpack(at::philox_cuda_state_t arg) {
  if (arg.has_device_ptrs_) {
    ...
    if (threadIdx.x == 0 && threadIdx.y == 0 && threadIdx.z == 0 &&
      ...
      *offset_next_launch = offset + args.increment_;
    }
