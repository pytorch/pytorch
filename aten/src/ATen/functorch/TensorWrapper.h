...
// There are two variants of makeTensorWrapper: one that accepts an is_alive_ptr
// (aka life handle) and one that doesn't
...
TORCH_API Tensor makeTensorWrapper(const Tensor& tensor, int64_t level, bool is_immutable, std::shared_ptr<bool> is_alive_ptr);
