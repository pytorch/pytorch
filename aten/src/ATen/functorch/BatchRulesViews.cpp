...
std::tuple<Tensor, optional<int64_t>> _lazy_clone_alias_batching_rule(
    const Tensor &self, optional<int64_t> self_bdim) {
  return std::make_tuple(self._lazy_clone_alias(), 0);
}
...
TORCH_LIBRARY_IMPL(aten, FuncTorchBatched, m) {
  ...
  VMAP_SUPPORT(_lazy_clone_alias, _lazy_clone_alias_batching_rule);
