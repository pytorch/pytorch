...
std::tuple<Tensor,optional<int64_t>>
linalg_vander_batch_rule(const Tensor& self, optional<int64_t> N, optional<int64_t> self_bdim) {
  // guard against the user passing in a batch of scalar tensors with batch
  // size equal to 2.
  TORCH_CHECK(self.sizes().size() > 1, "Input tensor must have one or more dimensions");

  auto self_ = moveBatchDimToFront(self, self_bdim);
  auto result = linalg_vander(self_, N);
  return std::make_tuple(result, 0);
}
...
#define UNARY_POINTWISE_ALL(op) \
  ...
  VMAP_SUPPORT(linalg_vander, linalg_vander_batch_rule);
