...
TensorIteratorConfig& TensorIteratorConfig::declare_static_shape(
  ...
  //   call this method are expected to check shapes before calling `add_input`
  //   or `add_output`.
  TORCH_CHECK(
      !resize_outputs_,
  ...
  for (auto& op : operands_) {
    ...
    if (config.check_all_same_device_) {
      ...
      if (!common_device.is_cpu() && config.allow_cpu_scalars_ &&
          !op.is_output && op.tensor->dim() == 0 && op.tensor->is_cpu()) {
        TORCH_CHECK(
    ...
    if (common_device == kCPU) {
      ...
      if (config.cast_common_dtype_to_outputs_ && op.is_output &&
        ...
        op.tensor = c10::MaybeOwned<Tensor>::owned(at::empty_like(
            *op.tensor,
      ...
      if (config.promote_inputs_to_common_dtype_ && !op.is_output &&
        ...
        op.original_tensor = op.tensor;
        op.tensor =
            c10::MaybeOwned<Tensor>::owned(op.tensor->to(common_dtype_));
        op.current_dtype = common_dtype_;
        op.target_dtype = common_dtype_;
      }
    }
...
SmallVector<char*, 4> TensorIteratorBase::get_data_ptrs(
    ArrayRef<char*> base,
  ...
  for (auto& op : operands_) {
    if (op.is_output && op.original_tensor->defined() &&
      ...
      if (op.original_tensor->sizes() != op.tensor->sizes()) {
        op.original_tensor->resize_as_(*op.tensor)
            .as_strided_(op.tensor->sizes(), op.tensor->strides());
      }
  ...
  for (auto& op : operands_) {
    if (op.tensor->defined()) {
      IntArrayRef original_shape =
          config.static_shape_ ? shape_ : op.tensor->sizes();
...
void TensorIteratorBase::build(TensorIteratorConfig& config) {
  ...
  // XLA tensors don't have storage, so they don't have an underlying data
  // pointer. Nothing beyond this point is important for meta functions, so it's
  // fine to exit early here.
...
void TensorIteratorBase::set_output(
    int64_t output_idx,
    IntArrayRef sizes,
    IntArrayRef strides,
