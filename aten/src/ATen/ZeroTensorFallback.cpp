...
// ZeroTensors are designed to be immutable. Thus, we error out when an in-place
// operation is performed on ZeroTensors
void zeroTensorFallback(
  ...
  if (is_write.has_value() && !*is_write) {
    ...
    op.redispatchBoxed(
        dispatch_keys &
            c10::DispatchKeySet(
                DispatchKeySet::FULL_AFTER, DispatchKey::ZeroTensor),
        stack);
    return;
  }
  ...
  for (const auto i : c10::irange(num_arguments)) {
    ...
    if (ivalue.isTensor()) {
      ...
      if (tensor._is_zerotensor()) {
        TORCH_CHECK(
            !mut_arg,
            "ZeroTensors are immutable. Please use the materialized zero tensor ",
...
TORCH_LIBRARY_IMPL(aten, ZeroTensor, m) {
  ...
  // The functions in the list below have a specific registeration in
  // native_functions.yaml and do not use the fallback. m.impl("mul.Tensor",
  // torch::CppFunction::makeFallthrough()); m.impl("add.Tensor",
  // torch::CppFunction::makeFallthrough()); m.impl("linalg_cross",
