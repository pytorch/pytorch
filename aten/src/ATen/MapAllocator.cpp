  ...
  , base_ptr_(nullptr)
{
  ...
  if (flags_ & ALLOCATOR_MAPPED_SHAREDMEM) {
    ...
    if (event_ == nullptr) {
      TORCH_CHECK(false, "Couldn't open shared event: <", eventname_, ">, error code: <", GetLastError(), ">");
    }
    ...
    if (handle_ == nullptr) {
      TORCH_CHECK(false, "Couldn't open shared file mapping: <", filename_, ">, error code: <", GetLastError(), ">");
    }

    size_ = size;
    base_ptr_ = MapViewOfFile(handle_, FILE_MAP_ALL_ACCESS, 0, 0, size);
    if (!base_ptr_) {
      TORCH_CHECK(false, "Couldn't map view of shared file <", filename_, ">, error code: <", GetLastError(), ">");
    }
  } else {
