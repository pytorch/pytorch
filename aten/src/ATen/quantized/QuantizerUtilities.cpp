...
Tensor quantize_tensor_cpu(Tensor rtensor, Tensor qtensor, double scale, int64_t zero_point) {
  ...
  checkCPUTensor(fn_name, qtensor);
...
Tensor quantize_tensor(Tensor rtensor, Tensor qtensor, double scale, int64_t zero_point) {
  ...
  checkSameDevice(fn_name, qtensor, rtensor);
  checkZeroPoint<typename T::underlying>(fn_name, zero_point);
  // Hard-fix to enable QuantizedCUDA.
  // Should be resolved when Quantizer is exposed to the python, i.e. via native functions mechanism.
  switch (qtensor.device().type())
  {
  case DeviceType::CUDA:
...
template CAFFE2_API Tensor quantize_tensor_per_channel_affine<qint8>(Tensor, Tensor, const std::vector<double>&, const std::vector<int64_t>&, int64_t);
template CAFFE2_API Tensor quantize_tensor_per_channel_affine<quint8>(Tensor, Tensor, const std::vector<double>&, const std::vector<int64_t>&, int64_t);
template CAFFE2_API Tensor quantize_tensor_per_channel_affine<qint32>(Tensor, Tensor, const std::vector<double>&, const std::vector<int64_t>&, int64_t);
template CAFFE2_API Tensor dequantize_tensor_per_channel_affine<qint8>(Tensor, Tensor, const std::vector<double>&, const std::vector<int64_t>&, int64_t);
template CAFFE2_API Tensor dequantize_tensor_per_channel_affine<quint8>(Tensor, Tensor, const std::vector<double>&, const std::vector<int64_t>&, int64_t);
template CAFFE2_API Tensor dequantize_tensor_per_channel_affine<qint32>(Tensor, Tensor, const std::vector<double>&, const std::vector<int64_t>&, int64_t);
