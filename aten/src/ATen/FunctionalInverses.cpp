...
Tensor FunctionalInverses::_lazy_clone_inverse(const Tensor& base, const Tensor& mutated_view, InverseReturnMode inverse_return_mode) {
  if (inverse_return_mode != InverseReturnMode::NeverView) {
    return at::_lazy_clone(mutated_view);
  } else {
    return at::_lazy_clone_copy(mutated_view);
  }
}
