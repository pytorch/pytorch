...
Tensor FunctionalInverses::_nested_from_values_and_offsets_copy_inverse(const Tensor& base, const Tensor& mutated_view, bool reapply_views, const Tensor& jagged_offsets) {
    TORCH_INTERNAL_ASSERT(false, "Attempted to call _nested_from_values_and_offsets() during the functionalization pass. For now, nested tensors aren't supported during functionalization");
    return Tensor();
}
