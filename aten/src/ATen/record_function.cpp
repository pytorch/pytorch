...
void DefaultSavedVariableHooks::enable() {
  is_enabled = true;
}

void DefaultSavedVariableHooks::set_hooks(PyObject* pack_hook, PyObject* unpack_hook) {
  pack_hook_ = pack_hook;
  unpack_hook_ = unpack_hook;
}

std::pair<PyObject*, PyObject*> DefaultSavedVariableHooks::get_hooks() {
  if (!is_enabled) {
