...
bool FunctionalTensorWrapper::is_mutated() const {
  return functional_storage_impl()->is_mutated();
}
...
bool is_mutated(const Tensor& tensor) {
  TORCH_INTERNAL_ASSERT(at::functionalization::impl::isFunctionalTensor(tensor));
  auto functional_base_impl = at::functionalization::impl::unsafeGetFunctionalWrapper(tensor);
  return functional_base_impl->is_mutated();
}
