...
void FunctionalTensorWrapper::set_constructor_metadata() {
  ...
  // In general, the sizes/stride metadata on a tensor can change as it is
  // mutated, and these changes need to be reflected in the metadata of the
  // wrapper.
  ...
  // E.g. when running torch.compile under inference mode, we need to make sure
  // that for any inputs that were created outside of inference mode (so they
  // are not inference tensors), then the functional wrappers that we wrap them
  // with should also not be inference tensors.
...
FunctionalTensorWrapper::FunctionalTensorWrapper(const Tensor& value)
  ...
  TORCH_INTERNAL_ASSERT(
      !at::functionalization::impl::isFunctionalTensor(value_));
...
void FunctionalTensorWrapper::commit_update() {
  ...
  // stride/storage_offset though, and DCE should handle that optimization
  // anyway. generation_ = storage_impl->generation();
}
...
void FunctionalTensorWrapper::mutate_view_meta(
    const at::functionalization::ViewMeta& meta) {
  view_metas_.push_back(meta);
  // Manually track the fact that this tensor recieved a metadata mutation!
  has_metadata_mutation_ = true;
  // Mark this tensor as being symbolic if there are any symbolic inputs used by
  // the view operation.
  maybe_mark_symbolic(meta);
...
// tensor.
void FunctionalTensorWrapper::replace_(
    const Tensor& other,
    bool from_lazy_regenerate) {
  ...
  if (dtype() != value_.unsafeGetTensorImpl()->dtype() ||
    ...
    at::AutoDispatchSkipFunctionalize guard;
    // and we want _to_copy() to show up in the graph, not the composite .to()
    // operator (this can happen if autograd has already run by the time we
    // enter this code)
    value_ = at::_to_copy(
        value_, c10::TensorOptions().dtype(dtype()).layout(layout()));
    TORCH_INTERNAL_ASSERT(
        !value_.key_set().has(c10::DispatchKey::Functionalize));
  }
  // might not be until after the no_grad region is exited.
  // Therefore, replace_() is not unconditionally safe to check the current
  // no_grad state. If this is a lazy regeneration, then it is guaranteed that
  // we have already done the mutation for the storage alias (when we originally
  // performed the mutation), so no counter update may be needed. Example: if a
  // mutation happens to a view under a no_grad, we won't call replace_() on the
  // other alias until the alias is later used, which
...
void FunctionalTensorWrapper::replace_functional_storage_base(
    const Tensor& other) {
  functional_storage_impl()->set_base(other);
}
...
void FunctionalTensorWrapper::set__impl(const FunctionalTensorWrapper* other) {
  // self.set_(src) will cause self to have all of the tensor properties of
  // self.
  value_ = other->value_;
  ...
  // (We could check if the updated value has a new storage than the original
  // value, but this won't also let us uniquely determine if the tensor **also**
...
void FunctionalTensorWrapper::storage_resize_(const c10::SymInt& new_size) {
  auto curr_storage_size = value_.unsafeGetTensorImpl()
                               ->unsafe_storage()
                               .unsafeGetStorageImpl()
                               ->sym_nbytes();
  // storage resizing is severely limited: we only support resizing either to
  // zero, or from zero bytes.
  TORCH_CHECK(
      ...
      ". curr_storage_size: ",
      curr_storage_size);
  // The "functionalization rule" for storage resizing is a giant no-op, mainly
  // because we don't want resize_() calls to actualy emit any ops in the
  // functional graph. How does it work? Resizing up (old size == 0):
  //   We do nothing in this case.
  //   The expection is that for the user code to be valid, the next op that
  //   should run against the current tensor "x" will be a x.copy_(y) (or
  //   similar), that will fully overwrite the data of x. If there are any
  //   outstanding aliases of x, we expect them not to be used until after the
  //   copy_() call (otherwise the eager code would be invalid), and therefore
  //   functionalization will regenerate the aliases off of the result of
  //   `x.copy(y)`.
  // Resizing down (new size == 0):
  //   We also do nothing in this case. The assumption is that after resizing a
  //   tensor down, it is fully unused in the program (unless it is later
  //   resized back up first, has data copied in) Although it might be saved for
  //   backward, which happens in FSDP. The expected pattern is that the param
  //   will then be resized back up from zero in the backward.
...
void FunctionalTensorWrapper::maybe_replace_storage(const Tensor& other) {
  ...
  // run though)
  refresh_contiguous();
  // Swapping out the storage of a tensor (aka from a resize_() call) will
  // update the sizes and strides of the tensor, so we need to record the fact
  // that metadata was mutated.
  has_metadata_mutation_ = true;
}
...
void FunctionalTensorWrapper::_unsafe_reset_storage() {
  ...
  storage_ = c10::Storage(
      c10::make_intrusive<functionalization::FunctionalStorageImpl>(value_));
  // Reset the generation so that it matches the new storage
  generation_ = 0;
  // Clear any pre-existing view metas so that base and value_ are semantically
  // the same
  view_metas_.clear();
}
...
c10::intrusive_ptr<TensorImpl> FunctionalTensorWrapper::
...
void FunctionalTensorWrapper::shallow_copy_from(
...
std::optional<Tensor> to_functional_tensor(
...
std::optional<Tensor> from_functional_tensor(
...
std::vector<Tensor> from_functional_tensor(ITensorListRef t_list) {
  ...
  for (const auto& tensor : t_list) {
    ...
    outputs.push_back(
        from_functional_tensor(tensor, /*assert_functional=*/false));
  }
  return outputs;
}
...
void replace_functional_storage_base(
    const Tensor& functional_tensor,
    const Tensor& other) {
  TORCH_INTERNAL_ASSERT_DEBUG_ONLY(isFunctionalTensor(functional_tensor));
  unsafeGetFunctionalWrapper(functional_tensor)
      ->replace_functional_storage_base(other);
}

void regenerate_from_base(const Tensor& functional_tensor) {
  TORCH_INTERNAL_ASSERT_DEBUG_ONLY(isFunctionalTensor(functional_tensor));
  unsafeGetFunctionalWrapper(functional_tensor)->regenerate_from_base();
}
...
void propagate_xla_data_direct(
...
bool isFunctionalTensor(const c10::List<::std::optional<Tensor>>& t_list) {
  ...
  for (const auto i : c10::irange(t_list.size())) {
    if (!t_list[i].has_value() || !t_list[i]->defined())
      continue;
...
void set_sizes_strides_offset(const Tensor& out, const Tensor& reference_out) {
  out.unsafeGetTensorImpl()->set_sizes_and_strides(
      reference_out.sym_sizes(),
      reference_out.sym_strides(),
      reference_out.sym_storage_offset());
}
...
// That is, it will call the operator, but removing any intermediate
// views/mutations that are performed inside of it. This is useful for LTC/XLA,
// which would like to re-use some of our composite kernels from pytorch core
// but not have to worry about the view ops that they might call. e.g.
// at::block_diag
void functionalize_op_helper(
  ...
  // When we re-invoke the dispatcher, this will automatically enable the
  ...
  {
    // Today when you call at::empty(device=lazy), the lazy backend decides
    // whether or not to wrap the output in a functional tensor based on TLS. In
    // this code, we're re-entrantly entering functionalization in the same
    // call-stack, so we need to manually fix up TLS as if it hadn't already
    // been called.
    ...
    // So, we should probably provide a way to directly call a kernel registered
    // to the `CompositeExplicitAutograd` key. We can't do that today, so this
    // should be a reasonably good proxy (It won't work in cases where an op has
    // both a CompositeExplicitAutograd kernel AND a dedicated meta kernel, but
