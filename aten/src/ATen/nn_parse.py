...
def argument_to_declaration(param, func=None):
    ...
    match = re.match(r'IntListRef\[(\d+)\]', arg['type'])
    if match:
        arg['type'] = 'IntListRef'
...
def get_thnn_args(thnn_function, params, inplace):
    ...
    def arg_expr(prefix, suffix):
        ...
        if param['type'] == 'IntListRef' and 'size' in param:
            name = name + '_'
...
def backward_declaration(base, thnn_functions):
    ...
    if 'upsample' in base['name']:
        ...
        input_size_arg = {'type': 'IntListRef', 'name': 'input_size', 'size': size}
    ...
    if 'im2col' in base['name']:
        ...
        input_size_arg = {'type': 'IntListRef', 'name': 'input_size', 'size': 2}
