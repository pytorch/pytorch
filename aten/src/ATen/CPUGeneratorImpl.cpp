...
const Generator& getDefaultCPUGenerator() {
  static auto default_gen_cpu =
      createCPUGenerator(c10::detail::getNonDeterministicRandom());
...
CPUGeneratorImpl::CPUGeneratorImpl(uint64_t seed_in)
    : c10::
          GeneratorImpl{Device(DeviceType::CPU), DispatchKeySet(c10::DispatchKey::CPU)},
      engine_{seed_in},
...
void CPUGeneratorImpl::set_state(const c10::TensorImpl& new_state) {
  ...
  static_assert(
      std::is_pod<CPUGeneratorImplStateLegacy>::value,
      "CPUGeneratorImplStateLegacy is not a PODType");
  static_assert(
      std::is_pod<CPUGeneratorImplState>::value,
      "CPUGeneratorImplState is not a PODType");
  ...
  static_assert(
      size_legacy != size_current,
      "CPUGeneratorImplStateLegacy and CPUGeneratorImplState can't be of the same size");
  ...
  // Construct the state of at::CPUGeneratorImpl based on input byte tensor
  // size. NOLINTNEXTLINE(cppcoreguidelines-init-variables)
  CPUGeneratorImplStateLegacy* legacy_pod;
  ...
  if (new_state_size == size_legacy) {
    ...
    // Note that CPUGeneratorImplStateLegacy stores two uniform values
    // (normal_x, normal_y) and a rho value (normal_rho). These three values
    // were redundant and in the new DistributionsHelper.h, we store the actual
    // extra normal sample, rather than three intermediate values.
    ...
    if (rng_state->is_next_float_normal_sample_valid) {
      float_normal_sample =
          c10::optional<float>(rng_state->next_float_normal_sample);
    }

    // Update next_double_normal_sample.
    // Note that in getRNGState, we now return the actual normal sample in
    // normal_y and if it's valid in normal_is_valid. The redundant normal_x and
    // normal_rho are squashed to 0.0.
    if (legacy_pod->normal_is_valid) {
      double_normal_sample = c10::optional<double>(legacy_pod->normal_y);
    }
  } else {
    AT_ERROR(
        "Expected either a CPUGeneratorImplStateLegacy of size ",
        size_legacy,
        " or a CPUGeneratorImplState of size ",
        size_current,
  ...
  // construct engine_
  // Note that CPUGeneratorImplStateLegacy stored a state array of 64 bit uints,
  // whereas in our redefined mt19937, we have changed to a state array of 32
  // bit uints. Hence, we are doing a std::copy.
  // NOLINTNEXTLINE(cppcoreguidelines-pro-type-member-init)
  at::mt19937_data_pod rng_data;
  std::copy(
      std::begin(legacy_pod->state),
      std::end(legacy_pod->state),
...
c10::intrusive_ptr<c10::TensorImpl> CPUGeneratorImpl::get_state() const {
  ...
  std::copy(
      rng_data.state_.begin(),
      rng_data.state_.end(),
      std::begin(accum_state->legacy_pod.state));
  accum_state->legacy_pod.normal_x =
      0.0; // we don't use it anymore and this is just a dummy
  accum_state->legacy_pod.normal_rho =
      0.0; // we don't use it anymore and this is just a dummy
 ...
 * See Note [Acquire lock when using random generators]
 */
void CPUGeneratorImpl::set_next_float_normal_sample(
    c10::optional<float> randn) {
 ...
 * See Note [Acquire lock when using random generators]
 */
void CPUGeneratorImpl::set_next_double_normal_sample(
    c10::optional<double> randn) {
