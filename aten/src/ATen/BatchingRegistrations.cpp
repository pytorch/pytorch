...
Tensor sum_batching_rule(
  ...
  if (/*logical*/ self.dim() == 0 && dims.size() == 1 &&
      is_allowed_dim_on_scalar_tensor(dims[0])) {
    return self.clone();
  }
...
Tensor expand_batching_rule(
  ...
  TORCH_CHECK(
      self_physical_dim <= size_physical.size(),
...
#define BINARY_POINTWISE_VA(op, ...)                                     \
  ...
  {
    using Binop =
        Tensor (*)(const Tensor&, const Tensor&, c10::optional<std::string>);
    using Unop =
        Tensor (*)(const Tensor&, const Scalar&, c10::optional<std::string>);
    m.impl(
        "div.Tensor_mode",
