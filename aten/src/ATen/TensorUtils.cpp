...
void checkSameGPU(CheckedFrom c, const TensorArg& t1, const TensorArg& t2) {
  if (! (t1->is_cuda() || t1->is_mps()) || ! (t2->is_cuda() || t2->is_mps())) {
    std::ostringstream oss;
    if (! (t1->is_cuda() || t1->is_mps())) {
      oss << "Tensor for " << t1 << " is on CPU, ";
    }
    if (! (t2->is_cuda() || t2->is_mps())) {
      oss << "Tensor for " << t2 << " is on CPU, ";
    }
    oss << "but expected " << ((!((t1->is_cuda() || t2->is_cuda()) || (t1->is_mps() || t2->is_mps()))) ? "them" : "it")
