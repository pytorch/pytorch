...
#define AT_DISPATCH_QINT_TYPES(TYPE, NAME, ...)                             \
  [&] {                                                                     \
    ...
    switch (_st) {                                                          \
      ...
      AT_QINT_PRIVATE_CASE_TYPE(                                            \
          at::kQUInt4x2, at::quint8, at::kByte, uint8_t, __VA_ARGS__)       \
      default:                                                              \
        AT_ERROR(#NAME, " not implemented for '", toString(TYPE), "'");     \
    }                                                                       \
  }()

#define AT_DISPATCH_QINT_TYPES_SUB_BYTE(TYPE, NAME, ...)                    \
  [&] {                                                                     \
    const auto& the_type = TYPE;                                            \
    /* don't use TYPE again in case it is an expensive or side-effect op */ \
    at::ScalarType _st = ::detail::scalar_type(the_type);                   \
    switch (_st) {                                                          \
      ...
      AT_QINT_PRIVATE_CASE_TYPE(                                            \
          at::kQInt32, at::qint32, at::kInt, int, __VA_ARGS__)              \
      AT_QINT_PRIVATE_CASE_TYPE(                                            \
          at::kQUInt4x2, at::quint4x2, at::kByte, uint8_t, __VA_ARGS__)       \
      default:                                                              \
        AT_ERROR(#NAME, " not implemented for '", toString(TYPE), "'");     \
    }                                                                       \
  }()
