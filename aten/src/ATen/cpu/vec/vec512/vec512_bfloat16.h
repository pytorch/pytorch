...
public:
  ...
  Vectorized<T> trigamma() const {
    ...
    for (auto i = decltype(sz){0}; i < sz / 2; i++) {
      tmp1[i] = calc_trigamma(tmp1[i]);
      tmp2[i] = calc_trigamma(tmp2[i]);
    }
    const auto o1 = _mm512_loadu_ps(tmp1);
    const auto o2 = _mm512_loadu_ps(tmp2);
    return cvt_from_fp32<T>(o1, o2);
  ...
  Vectorized<T> polygamma(const Vectorized<T> &n) const {
    ...
    _mm512_storeu_ps(reinterpret_cast<float*>(tmpn1), nlo);
    _mm512_storeu_ps(reinterpret_cast<float*>(tmpn2), nhi);
    for (int64_t i = 0; i < size() / 2; ++i) {
