...
public:
  ...
  Vectorized<float> fexp_u20() const {
    const __m512 vec_c0 =
        _mm512_set1_ps(0.00010703434948458272f);
    const __m512 vec_c1 =
        _mm512_set1_ps(0.30354260500649682f);
    const __m512 vec_c2 =
        _mm512_set1_ps(-0.22433836478672356);
    const __m512 vec_c3 =
        _mm512_set1_ps(-0.079204240219773236);
    ...
    // exp(x) = 2**(x * log2(e))
    //        = 2**xi * 2**xf   - TIPS we are using  the EEEE floating point representation with identification to the exponent and the mentissa
    //  2**xf will be approximated to a polynomial of degree 3 computed with Horner method
