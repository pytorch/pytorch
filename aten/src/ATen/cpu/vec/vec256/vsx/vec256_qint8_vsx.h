...
struct Vectorize<c10::qint8> {
 ...
 public:
  ...
  C10_ALWAYS_INLINE Vectorize(const c10::qint8& val)
      : _vec0{vec_splats(val.val_)}, _vec1{vec_splats(val.val_)} {}

  C10_ALWAYS_INLINE Vectorize(const Vectorize<c10::qint8>& other)
      : _vec0{other._vec0}, _vec1(other._vec1) {}

  C10_ALWAYS_INLINE Vectorize(vint8 v) : _vec0{v}, _vec1{v} {}
  C10_ALWAYS_INLINE Vectorize(vbool8 vmask) : _vecb0{vmask}, _vecb1{vmask} {}
  C10_ALWAYS_INLINE Vectorize(vint8 v1, vint8 v2) : _vec0{v1}, _vec1{v2} {}
  ...
  static C10_ALWAYS_INLINE Vectorize<c10::qint8> loadu(
      const void* ptr,
      int count = size()) {
    if (count == size()) {
      return {
 ...
 public:
  ...
  Vectorize<c10::qint8> C10_ALWAYS_INLINE relu(Vectorize<c10::qint8> zero_point) const {
    return {vec_max(_vec0, zero_point._vec0), vec_max(_vec1, zero_point._vec1)};
  }

  Vectorize<c10::qint8> C10_ALWAYS_INLINE
  relu6(Vectorize<c10::qint8> zero_point, Vectorize<c10::qint8> q_six) const {
...
template <>
Vectorize<c10::qint8> inline maximum(
    const Vectorize<c10::qint8>& a,
    const Vectorize<c10::qint8>& b) {
  return a.maximum(b);
}

template <>
Vectorize<c10::qint8> inline minimum(
    const Vectorize<c10::qint8>& a,
    const Vectorize<c10::qint8>& b) {
  return a.minimum(b);
}
