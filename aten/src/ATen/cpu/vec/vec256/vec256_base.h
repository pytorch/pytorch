...
public:
  ...
  //    `_ZN2at6vec25612_GLOBAL__N_16VectorizeIdE4sizeE' can not be used when making
...
template <class T,
          typename std::enable_if<!c10::is_complex<T>::value, int>::type = 0>
Vectorize<T> inline clamp(const Vectorize<T> &a, const Vectorize<T> &min_vec, const Vectorize<T> &max_vec) {
  Vectorize<T> c = Vectorize<T>();
  for (int i = 0; i != Vectorize<T>::size(); i++) {
...
template <class T,
          typename std::enable_if<!c10::is_complex<T>::value, int>::type = 0>
Vectorize<T> inline clamp_max(const Vectorize<T> &a, const Vectorize<T> &max_vec) {
  Vectorize<T> c = Vectorize<T>();
  for (int i = 0; i != Vectorize<T>::size(); i++) {
...
template <class T,
          typename std::enable_if<!c10::is_complex<T>::value, int>::type = 0>
Vectorize<T> inline clamp_min(const Vectorize<T> &a, const Vectorize<T> &min_vec) {
  Vectorize<T> c = Vectorize<T>();
  for (int i = 0; i != Vectorize<T>::size(); i++) {
...
static inline Vectorize<T> bitwise_binary_op(const Vectorize<T> &a, const Vectorize<T> &b, Op op) {
  ...
  __at_align32__ T results[Vectorize<T>::size()];
...
// E.g., inputs: a           Vectorize<float>   = {a0, b0, a1, b1, a2, b2, a3, b3}
//               b           Vectorize<float>   = {a4, b4, a5, b5, a6, b6, a7, b7}
//       returns:            Vectorize<float>   = {a0, a1, a2, a3, a4, a5, a6, a7}
//                           Vectorize<float>   = {b0, b1, b2, b3, b4, b5, b6, b7}
template <typename T>
...
// E.g., inputs: a           Vectorize<float>   = {a0, a1, a2, a3, a4, a5, a6, a7}
//               b           Vectorize<float>   = {b0, b1, b2, b3, b4, b5, b6, b7}
//       returns:            Vectorize<float>   = {a0, b0, a1, b1, a2, b2, a3, b3}
//                           Vectorize<float>   = {a4, b4, a5, b5, a6, b6, a7, b7}
template <typename T>
inline std::enable_if_t<Vectorize<T>::size() % 2 == 0, std::pair<Vectorize<T>, Vectorize<T>>>
