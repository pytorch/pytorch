...
// Transpose a [4, 32] block to [32, 4] (with contiguous output, ld=4)
template <typename scalar_t, typename = std::enable_if_t<sizeof(scalar_t) == 1>>
static inline void transpose_pad_4x32_block(
    const scalar_t* src,
    scalar_t* dst,
    int64_t ld_src,
    int krem = 4,
  ...
  // Transpose 4x32 bytes using unpack and shuffle
  ...
  // Store output in two steps (32 x 4 = 128 bytes = 2 x 512-bit stores)
  if (nrem * 4 < 64) {
    __mmask64 mask = (1ULL << (nrem * 4)) - 1;
    _mm512_mask_storeu_epi8(dst, mask, u0);
  } else if (nrem == 16) {
    _mm512_storeu_si512(reinterpret_cast<__m512i*>(dst), u0);
  } else if (nrem < 32) {
    int n_bytes1 = 64;
    int n_bytes2 = (nrem * 4) - n_bytes1;
    __mmask64 mask2 = (1ULL << n_bytes2) - 1;
    _mm512_storeu_si512(reinterpret_cast<__m512i*>(dst), u0);
    _mm512_mask_storeu_epi8(reinterpret_cast<__m512i*>(dst + 64), mask2, u1);
  } else {
    _mm512_storeu_si512(reinterpret_cast<__m512i*>(dst), u0);
    _mm512_storeu_si512(reinterpret_cast<__m512i*>(dst + 64), u1);
  }
#else
  TORCH_CHECK(
      false,
      "transpose_pad_4x32_block is only supported when AVX-512 is supported")
#endif
}
