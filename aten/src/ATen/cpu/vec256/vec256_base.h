...
struct Vec256 {
 ...
 public:
  ...
  //    `_ZN2at6vec25612_GLOBAL__N_16Vec256IdE4sizeE' can not be used when
  ...
  // a variable is odr-used or not.  So best to just cut the problem at the
  // root.
  ...
  template <
      ...
      typename std::enable_if<
          !is_floating_point<other_t_abs>::value &&
  ...
  template <
      ...
      typename std::enable_if<is_floating_point<float_t_abs>::value, int>::
          type = 0>
  Vec256<T> abs() const {
  ...
  template <
      ...
      typename std::enable_if<c10::is_complex<complex_t_abs>::value, int>::
          type = 0>
  Vec256<T> abs() const {
    // complex_t_abs is for SFINAE and clarity. Make sure it is not changed.
    static_assert(
        std::is_same<complex_t_abs, T>::value, "complex_t_abs must be T");
  ...
  template <
      ...
      typename std::enable_if<c10::is_complex<other_t_sgn>::value, int>::type =
          0>
  Vec256<T> sgn() const {
  ...
  template <
      ...
      typename std::enable_if<!c10::is_complex<other_t_angle>::value, int>::
          type = 0>
  Vec256<T> angle() const {
    // other_t_angle is for SFINAE and clarity. Make sure it is not changed.
    static_assert(
        std::is_same<other_t_angle, T>::value, "other_t_angle must be T");
    return map(at::native::angle_impl<T>); // compiler is unable to resolve the
                                           // overload without <T>
  }
  template <
      typename complex_t_angle = T,
      typename std::enable_if<c10::is_complex<complex_t_angle>::value, int>::
          type = 0>
  Vec256<T> angle() const {
    // complex_t_angle is for SFINAE and clarity. Make sure it is not changed.
    static_assert(
        std::is_same<complex_t_angle, T>::value, "complex_t_angle must be T");
    return map([](T x) { return static_cast<T>(std::arg(x)); });
  }
  template <
      typename other_t_real = T,
      typename std::enable_if<!c10::is_complex<other_t_real>::value, int>::
          type = 0>
  Vec256<T> real() const {
    // other_t_real is for SFINAE and clarity. Make sure it is not changed.
    static_assert(
        std::is_same<other_t_real, T>::value, "other_t_real must be T");
    return *this;
  }
  template <
      typename complex_t_real = T,
      typename std::enable_if<c10::is_complex<complex_t_real>::value, int>::
          type = 0>
  Vec256<T> real() const {
    // complex_t_real is for SFINAE and clarity. Make sure it is not changed.
    static_assert(
        std::is_same<complex_t_real, T>::value, "complex_t_real must be T");
    return map([](T x) { return static_cast<T>(x.real()); });
  }
  template <
      typename other_t_imag = T,
      typename std::enable_if<!c10::is_complex<other_t_imag>::value, int>::
          type = 0>
  Vec256<T> imag() const {
    // other_t_imag is for SFINAE and clarity. Make sure it is not changed.
    static_assert(
        std::is_same<other_t_imag, T>::value, "other_t_imag must be T");
    return Vec256(0);
  }
  template <
      typename complex_t_imag = T,
      typename std::enable_if<c10::is_complex<complex_t_imag>::value, int>::
          type = 0>
  Vec256<T> imag() const {
    // complex_t_imag is for SFINAE and clarity. Make sure it is not changed.
    static_assert(
        std::is_same<complex_t_imag, T>::value, "complex_t_imag must be T");
    return map([](T x) { return static_cast<T>(x.imag()); });
  }
  template <
      typename other_t_conj = T,
      typename std::enable_if<!c10::is_complex<other_t_conj>::value, int>::
          type = 0>
  Vec256<T> conj() const {
    // other_t_conj is for SFINAE and clarity. Make sure it is not changed.
    static_assert(
        std::is_same<other_t_conj, T>::value, "other_t_conj must be T");
    return *this;
  }
  template <
      typename complex_t_conj = T,
      typename std::enable_if<c10::is_complex<complex_t_conj>::value, int>::
          type = 0>
  Vec256<T> conj() const {
    // complex_t_conj is for SFINAE and clarity. Make sure it is not changed.
    static_assert(
        std::is_same<complex_t_conj, T>::value, "complex_t_conj must be T");
  ...
  template <
      ...
      typename std::enable_if<!c10::is_complex<other_t_log2>::value, int>::
          type = 0>
  Vec256<T> log2() const {
    // other_t_log2 is for SFINAE and clarity. Make sure it is not changed.
    static_assert(
        std::is_same<other_t_log2, T>::value, "other_t_log2 must be T");
    return map(std::log2);
  }
  template <
      typename complex_t_log2 = T,
      typename std::enable_if<c10::is_complex<complex_t_log2>::value, int>::
          type = 0>
  Vec256<T> log2() const {
    // complex_t_log2 is for SFINAE and clarity. Make sure it is not changed.
    static_assert(
        std::is_same<complex_t_log2, T>::value, "complex_t_log2 must be T");
 ...
 public:
  Vec256<T> operator==(const Vec256<T>& other) const {
 ...
 public:
  Vec256<T> eq(const Vec256<T>& other) const {
    return binary_pred_bool(other, std::equal_to<T>());
  }
  Vec256<T> ne(const Vec256<T>& other) const {
    return binary_pred_bool(other, std::not_equal_to<T>());
  }
  Vec256<T> gt(const Vec256<T>& other) const {
    return binary_pred_bool(other, std::greater<T>());
  }
  Vec256<T> ge(const Vec256<T>& other) const {
    return binary_pred_bool(other, std::greater_equal<T>());
  }
  Vec256<T> lt(const Vec256<T>& other) const {
    return binary_pred_bool(other, std::less<T>());
  }
  Vec256<T> le(const Vec256<T>& other) const {
...
template <
    class T,
    typename std::enable_if<!c10::is_complex<T>::value, int>::type = 0>
Vec256<T> inline clamp(
    const Vec256<T>& a,
    const Vec256<T>& min_vec,
    const Vec256<T>& max_vec) {
  Vec256<T> c = Vec256<T>();
  for (int i = 0; i != Vec256<T>::size(); i++) {
...
static inline Vec256<T> bitwise_binary_op(
    const Vec256<T>& a,
    const Vec256<T>& b,
    Op op) {
  __m256i buffer;
  __m256i a_buffer =
      _mm256_loadu_si256(reinterpret_cast<const __m256i*>((const T*)a));
  __m256i b_buffer =
      _mm256_loadu_si256(reinterpret_cast<const __m256i*>((const T*)b));
...
template <
    class T,
    typename std::
        enable_if_t<!std::is_base_of<Vec256i, Vec256<T>>::value, int> = 0>
inline Vec256<T> operator&(const Vec256<T>& a, const Vec256<T>& b) {
  ...
  return bitwise_binary_op(
      a, b, [](__m256i a, __m256i b) { return _mm256_and_si256(a, b); });
}
template <
    class T,
    typename std::
...
inline Vec256<T> operator|(const Vec256<T>& a, const Vec256<T>& b) {
  ...
  return bitwise_binary_op(
      a, b, [](__m256i a, __m256i b) { return _mm256_or_si256(a, b); });
}
template <
    class T,
    typename std::
...
inline Vec256<T> operator^(const Vec256<T>& a, const Vec256<T>& b) {
  ...
  return bitwise_binary_op(
      a, b, [](__m256i a, __m256i b) { return _mm256_xor_si256(a, b); });
}
...
std::enable_if_t<
    ...
    Vec256<
        T>> inline gather(T const* base_addr, const Vec256<int_same_size_t<T>>& vindex) {
...
template <int64_t scale = 1, typename T = void>
std::
    enable_if_t<scale == 1 || scale == 2 || scale == 4 || scale == 8, Vec256<T>> inline mask_gather(
        const Vec256<T>& src,
        T const* base_addr,
...
template <typename T>
inline Vec256<int_same_size_t<T>> convert_to_int_of_same_size(
    const Vec256<T>& src) {
  static constexpr int size = Vec256<T>::size();
  T src_arr[size];
  src.store(static_cast<void*>(src_arr));
...
template <typename T>
inline std::
    enable_if_t<Vec256<T>::size() % 2 == 0, std::pair<Vec256<T>, Vec256<T>>>
  ...
  return std::make_pair(
      Vec256<T>::loadu(static_cast<void*>(buffer1)),
