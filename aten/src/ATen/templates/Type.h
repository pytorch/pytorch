...
struct CAFFE2_API Type {
  ...
  virtual Tensor tensorFromBlob(void * data, ScalarType dtype, IntArrayRef sizes, const std::function<void(void*)> & deleter=noop_deleter) const = 0;
  virtual Tensor tensorFromBlob(void * data, ScalarType dtype, IntArrayRef sizes, IntArrayRef strides, const std::function<void(void*)> & deleter=noop_deleter) const = 0;
  virtual Tensor tensorWithAllocator(ScalarType dtype, IntArrayRef sizes, Allocator* allocator) const = 0;
  virtual Tensor tensorWithAllocator(ScalarType dtype, IntArrayRef sizes, IntArrayRef strides, Allocator* allocator) const = 0;
  ...
  TensorOptions options(ScalarType dtype, int16_t device_index = -1) const {
  ...
  TensorOptions options(ScalarType dtype, c10::optional<Device> device_opt) const {
    if (!device_opt.has_value()) {
      return options(dtype, -1);
    } else {
      Device device = device_opt.value();
      AT_ASSERT(device.type() == device_type());
      return options(dtype, device.index());
    }
  }
