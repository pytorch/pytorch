...
class TORCH_API Tensor : public TensorBase {
 protected:
  ...
  explicit Tensor(unsafe_borrow_t, const TensorBase& rhs)
      : TensorBase(unsafe_borrow_t{}, rhs) {}
 ...
 public:
  ...
  c10::MaybeOwned<Tensor> expect_contiguous(
      MemoryFormat memory_format = MemoryFormat::Contiguous) const&;
  ...
  // Also MSVC will wrongly issue the following warning with the aforementioned
  // fix
  ...
  Tensor toBackend(Backend b) const {
    return to(
        options().device(backendToDeviceType(b)).layout(layout_from_backend(b)),
        /*non_blocking*/ false,
        /*copy*/ false);
  }
  ...
  Tensor cuda() const {
    return to(
        options().device(DeviceType::CUDA),
        /*non_blocking*/ false,
        /*copy*/ false);
  }
  ...
  Tensor vulkan() const {
    return to(
        options().device(DeviceType::Vulkan),
        /*non_blocking*/ false,
        /*copy*/ false);
  }
  ...
  Tensor metal() const {
    return to(
        options().device(DeviceType::Metal),
        /*non_blocking*/ false,
        /*copy*/ false);
  }
  ...
  Tensor meta() const {
    return to(
        options().device(DeviceType::Meta),
        /*non_blocking*/ false,
        /*copy*/ false);
  }
  ...
  /// leaf Tensors if they were created by the user. This means that they are
  /// not the result of an operation and so `grad_fn()` is `nullptr`.
  ///
  ...
  /// // b was created by the operation that cast a cpu Tensor into a cuda
  /// Tensor
  ///
  ...
  /// // d does not require gradients and so has no operation creating it (that
  ...
  /// \fn void backward(const Tensor & gradient={}, c10::optional<bool>
  /// retain_graph=c10::nullopt, bool create_graph=false,
  /// c10::optional<TensorList> inputs=c10::nullopt) const;
  ///
  ...
  ///     to a Tensor that does not require grad unless ``create_graph`` is
  ///     True. None values can be specified for scalar Tensors or ones that
  ...
  ///     the current implementation will call its grad_fn (even though it is
  ///     not strictly needed to get this gradients). It is an implementation
  ///     detail on which the user should not rely. See
  ...
  void backward(
    ...
    // 'backwards' api to accept the 'inputs' argument optionally. Since code
    // gen currently does not support optional of TensorList our approach is to
    // replace backward in native_functions.yaml with _backward and call it here
    // instead.
    if (inputs.has_value()) {
      TORCH_CHECK(
          inputs.value().size() > 0,
  ...
  ///
  /// Is ``true`` if this Tensor is non-leaf and its :attr:`grad` is enabled to
  /// be populated during :func:`backward`, ``false`` otherwise.
  ...
  /// This function returns an undefined tensor by default and returns a defined
  /// tensor the first time a call to `backward()` computes gradients for this
  /// Tensor. The attribute will then contain the gradients computed and future
  ...
  /// Note that the given new_grad might not be used directly if it has
  /// different metadata (size/stride/storage offset) compared to this Tensor.
  /// In that case, new_grad content will be copied into a new Tensor
  void _set_fw_grad(
      const TensorBase& new_grad,
      uint64_t level,
      bool is_inplace_op) const {
  ...
  /// is computed. The hook should have one of the following signature:
  /// ```
  ...
  ///
  /// This function returns the index of the hook in the list which can be used
  /// to remove hook.
  ///
  /// Example:
  /// @code
  /// auto v = torch::tensor({0., 0., 0.}, torch::requires_grad());
  /// auto h = v.register_hook([](torch::Tensor grad){ return grad * 2; }); //
  /// double the gradient v.backward(torch::tensor({1., 2., 3.}));
  /// // This prints:
  /// // ```
  /// //  2
  /// //  4
...
struct MaybeOwnedTraits<at::Tensor> {
  ...
  static borrow_type createBorrow(const owned_type& from) {
    ...
    // at::UndefinedTensorImpl>::reclaim(from.unsafeGetTensorImpl()));
    //
...
inline c10::MaybeOwned<Tensor> borrow_from_optional_tensor(
  ...
  return opt.has_value() ? c10::MaybeOwned<Tensor>::borrowed(*opt)
