...
namespace c10 {
  template<> class optional<at::Tensor>;
}
...
class TORCH_API Tensor {
 ...
 public:
  ...
  Tensor var(int dim) const {
    return var(IntArrayRef{dim}, true, false);
  }

  Tensor std(int dim) const {
    return std(IntArrayRef{dim}, true, false);
  }
  ...
  inline Tensor to(caffe2::TypeMeta type_meta, bool non_blocking=false, bool copy=false) const {
    return this->to(/*scalar_type=*/typeMetaToScalarType(type_meta), non_blocking, copy, c10::nullopt);
  }
  inline Tensor to(Device device, caffe2::TypeMeta type_meta, bool non_blocking=false, bool copy=false) const {
    return this->to(device, /*scalar_type=*/typeMetaToScalarType(type_meta), non_blocking, copy, c10::nullopt);
  }
...
namespace c10 {
  template<> class optional<at::Tensor> {
    public:
      ...
      const at::Tensor&& operator*() const&& {
        return std::move(tensorOrUndefined_);
      }
      ...
      template <class U = value_type, std::enable_if_t<
          std::is_constructible<at::Tensor, U&&>::value
          && !std::is_same<typename std::decay<U>::type, in_place_t>::value
          && !std::is_same<typename std::decay<U>::type, optional<at::Tensor>>::value, int> = 0>
      optional(U&& value)
      : tensorOrUndefined_(at::Tensor(std::move(value)).getIntrusivePtr()) {
      }
      ...
      bool has_value() const noexcept {
        return tensorOrUndefined_.defined();
      }
      ...
      template< class U >
      at::Tensor value_or( U&& default_value ) const& {
        return tensorOrUndefined_.defined() ? tensorOrUndefined_ : default_value;
      }

      template< class U >
      at::Tensor value_or( U&& default_value ) && {
        return tensorOrUndefined_.defined() ? std::move(tensorOrUndefined_) : std::forward<U>(default_value);
      }
    ...
    private:
      at::Tensor tensorOrUndefined_;
  };
}

namespace at {
  inline void Tensor::backward(const Tensor & gradient, c10::optional<bool> retain_graph, bool create_graph, c10::optional<TensorList> inputs) const {
