...
#if defined(AT_PER_OPERATOR_HEADERS) && \
...
// decltype(&ATEN_FN2(op, overload)) it. NB: the 2 means this macro takes 2
// args.
//
...
// Separately, ATEN_OP(op) and ATEN_OP2(op, overload) define a class containing
// compile-time metadata about a given aten operator. Notable data on the class
// includes:
// - ATEN_OP2(add, Tensor)::name // returns the string name: "add"
// - ATEN_OP2(add, Tensor)::overload_name // returns the string overload name:
// "Tensor"
// - ATEN_OP2(add, Tensor)::schema // returns the C++ schema type: at::Tensor
// (const at::Tensor &, const at::Tensor &, const at::Scalar &)
// - ATEN_OP2(add, Tensor)::schema_str // returns the string jit type:
