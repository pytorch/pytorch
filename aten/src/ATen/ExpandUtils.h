...
CAFFE2_API std::vector<int64_t> infer_size(IntListRef a, IntListRef b);
...
inline std::tuple<Tensor> expand_size(const Tensor &to_expand, IntListRef sizes) {
...
inline std::tuple<Tensor> expand_size(const Tensor &to_expand, IntListRef sizes, const char *api_name) {
...
static inline Tensor sum_to(Tensor tensor, const IntListRef shape) {
  if (shape.size() == 0) {
    return tensor.sum();
  }
  c10::SmallVector<int64_t, 8> reduce_dims;
  const at::IntListRef sizes = tensor.sizes();
...
static inline bool is_expandable_to(IntListRef shape, IntListRef desired) {
  int ndim = shape.size();
