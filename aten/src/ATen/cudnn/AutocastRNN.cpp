...
/********************************************************************************
Autocast wrapper for CuDNN RNNs (the weight reflattening needs special
attention)
********************************************************************************/

// To be registered for the "_cudnn_rnn(...)" schema.
// _cudnn_rnn is autograd-exposed (test_autocast_cudnn_rnn in test_cuda.py
// includes a test to confirm)
std::tuple<Tensor, Tensor, Tensor, Tensor, Tensor> _cudnn_rnn_cast_reflatten(
    const Tensor& input,
    TensorList weight,
  ...
  // There's an implicit contract here with native/cudnn/RNN.cpp:_cudnn_impl,
  // which calls at:_cudnn_rnn. Code here assumes if _cudnn_impl passes
  // weight_buf_opt containing a defined tensor, that tensor is valid flat
  // storage of the weights in their incoming dtype.
  if (weight_buf_opt.has_value()) {
    weight_buf = *weight_buf_opt;
  }
  bool needs_cast_and_flatten =
      (weight_buf.defined() ?
                            // weight_buf is valid.  Only change it if it's
                            // eligible and not already FP16.
           is_eligible(weight_buf) && (weight_buf.scalar_type() != at::kHalf)
                            :
                            // weight_buf is not valid.  Only create it if other
                            // weights are eligible and not already FP16.
           is_eligible(weight[0]) && (weight[0].scalar_type() != at::kHalf));
  if (needs_cast_and_flatten) {
    // Casts weight tensors to FP16 and ensures all weights for all layers are
    // views into a large flat buffer, with the right locations and layouts
    // expected by cudnn. This is (and should be) autograd-exposed.
    ...
    std::tie(redispatch_weight_buf, redispatch_weight) =
        at::native::cudnn_rnn::copy_weights_to_flat_buf_views(
            ...
            at::native::getCudnnDataTypeFromScalarType(
                at::kHalf), // could just hardcode CUDNN_DATA_HALF
  ...
  AT_ERROR(
      "autocast::_cudnn_rnn_cast_reflatten: ATen not compiled with cuDNN support");
  return {
      Tensor{},
      Tensor{},
      Tensor{},
      Tensor{},
      Tensor{}}; // never reached, placates the compiler
#endif // AT_CUDNN_ENABLED()
}

namespace {
TORCH_LIBRARY_IMPL(aten, Autocast, m) {
  m.impl("_cudnn_rnn", TORCH_FN((&at::autocast::_cudnn_rnn_cast_reflatten)));
}
} // anonymous namespace
