...
TEST(TestParallel, NestedParallel) {
  ...
  auto expected = a.sum();
  // check that calling sum() from within a parallel block computes the same
  // result
...
TEST(TestParallel, LocalMKLThreadNumber) {
  ...
  auto f = [](int nthreads) { set_num_threads(nthreads); };
  std::thread t(f, 1);
  t.join();
...
TEST(TestParallel, NestedParallelThreadId) {
  ...
  at::parallel_for(0, 10, 1, [&](int64_t begin, int64_t end) {
    auto num_threads = at::parallel_reduce(
        0,
        10,
        1,
        0,
...
TEST(TestParallel, IntraOpLaunchFuture) {
  ...
  auto fut1 = at::intraop_launch_future([&v1]() { v1 = 1; });

  auto fut2 = at::intraop_launch_future([&v2]() { v2 = 2; });
