...
#elif !defined(CPU_CAPABILITY_AVX) && !defined(CPU_CAPABILITY_AVX2) && \
...
#define RESOLVE_OVERLOAD(...)                                  \
  [](auto&&... args) -> decltype(auto) {                       \
    ...
    defined(CPU_CAPABILITY_AVX2) && (defined(__GNUC__) || defined(__GNUG__))
...
void test_unary(
  ...
  for (const CheckWithinDomains<UVT>& dmn : domains) {
    ...
    for (int trial = 0; trial < trialCount; trial++) {
      ...
      AssertVec256<vec_type> vecAssert(
