...
template <typename T>
using VecType = typename at::vec::Vectorize<T>;
...
template <typename T>
class AssertVectorize
{
public:
    AssertVectorize(const std::string& info, TestSeed seed, const T& expected, const T& actual, const T& input0)
        : additionalInfo(info), testSeed(seed), exp(expected), act(actual), arg0(input0), argSize(1)
    {
    }
    AssertVectorize(const std::string& info, TestSeed seed, const T& expected, const T& actual, const T& input0, const T& input1)
        : additionalInfo(info), testSeed(seed), exp(expected), act(actual), arg0(input0), arg1(input1), argSize(2)
    {
    }
    AssertVectorize(const std::string& info, TestSeed seed, const T& expected, const T& actual, const T& input0, const T& input1, const T& input2)
        : additionalInfo(info), testSeed(seed), exp(expected), act(actual), arg0(input0), arg1(input1), arg2(input2), argSize(3)
    {
    }
    AssertVectorize(const std::string& info, TestSeed seed, const T& expected, const T& actual) : additionalInfo(info), testSeed(seed), exp(expected), act(actual)
    {
    }
    AssertVectorize(const std::string& info, const T& expected, const T& actual) : additionalInfo(info), exp(expected), act(actual), hasSeed(false)
    {
    }
...
void test_unary(
    ...
    for (const CheckWithinDomains<UVT>& dmn : domains) {
        ...
        for (int trial = 0; trial < trialCount; trial++) {
            ...
            AssertVectorize<vec_type> vecAssert(testNameInfo, seed, vec_expected, actual, input);
...
void test_binary(
    ...
    for (const CheckWithinDomains<UVT>& dmn : testCase.getDomains()) {
        ...
        for (int trial = 0; trial < trialCount; trial++) {
            ...
            auto vec_expected = vec_type::loadu(expected);
            AssertVectorize<vec_type> vecAssert(testNameInfo, seed, vec_expected, actual, input0, input1);
            if (vecAssert.check(bitwise, dmn.CheckWithTolerance, dmn.ToleranceError))return;
        }// trial
        changeSeedBy += 1;
    }
...
void test_ternary(
    ...
    for (const CheckWithinDomains<UVT>& dmn : testCase.getDomains()) {
        ...
        for (int trial = 0; trial < trialCount; trial++) {
            ...
            auto vec_expected = vec_type::loadu(expected);
            AssertVectorize<vec_type> vecAssert(testNameInfo, seed, vec_expected, actual, input0, input1, input2);
            if (vecAssert.check(bitwise, dmn.CheckWithTolerance, dmn.ToleranceError)) return;
        }// trial
        changeSeedBy += 1;
    }
