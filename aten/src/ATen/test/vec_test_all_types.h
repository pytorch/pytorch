...
#define NAME_INFO(name) \
  TOSTRING(name) " " TOSTRING(__FILE__) ":" TOSTRING(__LINE__)
...
#if defined(CPU_CAPABILITY_ZVECTOR) || defined(CPU_CAPABILITY_VSX) || \
    defined(CPU_CAPABILITY_AVX2) ||                                   \
...
struct DomainRange {
  T start; // start [
  T end; // end is not included. one could use  nextafter for including his end
         // case for tests
};
...
template <typename T>
std::enable_if_t<std::is_floating_point<T>::value, bool> check_both_nan(
    T x,
    T y) {
...
template <typename T>
std::enable_if_t<std::is_floating_point<T>::value, bool> check_both_inf(
    T x,
    T y) {
...
template <typename T>
std::enable_if_t<!std::is_floating_point<T>::value, bool> check_both_big(
    T x,
    T y) {
  return false;
}
...
std::enable_if_t<std::is_floating_point<T>::value, bool> check_both_big(
    T x,
    T y) {
  T cmax = std::is_same<T, float>::value ? static_cast<T>(1e+30)
                                         : static_cast<T>(1e+300);
  T cmin = std::is_same<T, float>::value ? static_cast<T>(-1e+30)
                                         : static_cast<T>(-1e+300);
...
template <class T>
std::enable_if_t<std::is_floating_point<T>::value, bool> nearlyEqual(
    T a,
    T b,
    T tolerance) {
  if (check_both_nan<T>(a, b))
    return true;
  if (check_both_big(a, b))
    return true;
  T absA = std::abs(a);
  T absB = std::abs(b);
  T diff = std::abs(a - b);
...
template <typename T>
std::enable_if_t<std::is_floating_point<T>::value, void> filter_fmod(
    T& a,
    T& b) {
...
template <typename T>
std::enable_if_t<std::is_floating_point<T>::value, void> filter_fmadd(
    T& a,
    T& b,
    T& c) {
...
template <typename T>
std::enable_if_t<is_complex<Complex<T>>::value, void> filter_zero(
    Complex<T>& val) {
  T rr = val.real();
  T ii = val.imag();
...
template <typename T>
std::enable_if_t<is_complex<T>::value, void> filter_mult_overflow(
    T& val1,
    T& val2) {
  // missing
}
...
std::enable_if_t<!is_complex<T>::value, void> filter_div_ub(T& val1, T& val2) {
  if (is_zero(val2)) {
    val2 = 1;
      ...
      std::is_integral<T>::value && val1 == std::numeric_limits<T>::min() &&
      val2 == -1) {
    val2 = 1;
  }
}
...
template <
    ...
    bool is_complex = is_complex<T>::value>
struct ValueGen {
...
class TestingCase {
 public:
  ...
  static TestCaseBuilder<T, U> getBuilder() {
    return TestCaseBuilder<T, U>{};
  }
...
class TestCaseBuilder {
 ...
 public:
  ...
  operator TestingCase<T, U>&&() {
    return std::move(_case);
  }
};

template <typename T>
typename std::
    enable_if_t<!is_complex<T>::value && std::is_unsigned<T>::value, T>
...
template <typename T>
typename std::enable_if_t<is_complex<Complex<T>>::value, T> correctEpsilon(
    const Complex<T>& eps) {
  return std::abs(eps);
}
...
class AssertVectorized {
 public:
  ...
  bool check(
      bool bitwise = false,
      bool checkWithTolerance = false,
      ValueType<T> toleranceEps = {}) const {
    using UVT = UvalueType<T>;
    using BVT = BitType<UVT>;
    UVT absErr = correctEpsilon(toleranceEps);
    constexpr int sizeX =
        VecTypeHelper<T>::holdCount * VecTypeHelper<T>::unitStorageCount;
    ...
    if (bitwise) {
      ...
      for (const auto i : c10::irange(sizeX)) {
        EXPECT_EQ(nearlyEqual<UVT>(expArr[i], actArr[i], absErr), true)
            << expArr[i] << "!=" << actArr[i] << "\n"
      ...
      for (const auto i : c10::irange(sizeX)) {
        if (std::is_same<UVT, float>::value) {
          if (!check_both_nan(expArr[i], actArr[i])) {
            EXPECT_FLOAT_EQ(expArr[i], actArr[i])
                << getDetail(i / unitStorageCount);
          }
        } else if (std::is_same<UVT, double>::value) {
          if (!check_both_nan(expArr[i], actArr[i])) {
            EXPECT_DOUBLE_EQ(expArr[i], actArr[i])
                << getDetail(i / unitStorageCount);
          }
        } else {
...
void test_unary(
  ...
  UVT default_start = std::is_floating_point<UVT>::value
      ? std::numeric_limits<UVT>::lowest()
      : std::numeric_limits<UVT>::min();
  ...
  for (const CheckWithinDomains<UVT>& dmn : domains) {
    ...
    for (const auto trial : c10::irange(trialCount)) {
      ...
      if (vecAssert.check(bitwise, dmn.CheckWithTolerance, dmn.ToleranceError))
        return;
...
template <typename T>
std::enable_if_t<is_complex<Complex<T>>::value, Complex<T>> local_log2(
    Complex<T> x) {
  T ret = std::log(x);
...
template <typename T>
std::enable_if_t<is_complex<Complex<T>>::value, Complex<T>> local_abs(
    Complex<T> x) {
...
template <typename T>
std::enable_if_t<is_complex<Complex<T>>::value, Complex<T>> local_multiply(
    Complex<T> x,
    Complex<T> y) {
#if defined(TEST_AGAINST_DEFAULT)
  return x * y;
...
template <typename T>
std::enable_if_t<is_complex<Complex<T>>::value, Complex<T>> local_division(
    Complex<T> x,
    Complex<T> y) {
#if defined(TEST_AGAINST_DEFAULT)
  return x / y;
...
template <typename T>
std::enable_if_t<is_complex<Complex<T>>::value, Complex<T>> local_sqrt(
    Complex<T> x) {
  return std::sqrt(x);
}
...
template <typename T>
std::enable_if_t<is_complex<Complex<T>>::value, Complex<T>> local_asin(
    Complex<T> x) {
  return std::asin(x);
}
...
template <typename T>
std::enable_if_t<is_complex<Complex<T>>::value, Complex<T>> local_acos(
    Complex<T> x) {
  return std::acos(x);
}

template <typename T>
std::enable_if_t<!is_complex<T>::value, T> local_and(
    const T& val0,
    const T& val1) {
  using bit_rep = BitType<T>;
...
template <typename T>
std::enable_if_t<is_complex<Complex<T>>::value, Complex<T>> local_and(
    const Complex<T>& val0,
    const Complex<T>& val1) {
  using bit_rep = BitType<T>;
  T real1 = val0.real();
  T imag1 = val0.imag();
  T real2 = val1.real();
...
template <typename T>
std::enable_if_t<!is_complex<T>::value, T> local_or(
    const T& val0,
    const T& val1) {
  using bit_rep = BitType<T>;
  bit_rep ret = bit_cast<bit_rep>(val0) | bit_cast<bit_rep>(val1);
...
template <typename T>
std::enable_if_t<is_complex<Complex<T>>::value, Complex<T>> local_or(
    const Complex<T>& val0,
    const Complex<T>& val1) {
  using bit_rep = BitType<T>;
  T real1 = val0.real();
...
template <typename T>
std::enable_if_t<!is_complex<T>::value, T> local_xor(
    const T& val0,
    const T& val1) {
  using bit_rep = BitType<T>;
  bit_rep ret = bit_cast<bit_rep>(val0) ^ bit_cast<bit_rep>(val1);
...
template <typename T>
std::enable_if_t<is_complex<Complex<T>>::value, Complex<T>> local_xor(
    const Complex<T>& val0,
    const Complex<T>& val1) {
  using bit_rep = BitType<T>;
  T real1 = val0.real();
...
T quantize_val(float scale, int64_t zero_point, float value) {
  ...
  qvalue = static_cast<int64_t>(
      zero_point + at::native::round_impl<float>(value * inv_scale));
...
template <typename T>
TestingCase<T> createDefaultUnaryTestCase(
    TestSeed seed = TestSeed(),
    bool bitwise = false,
    bool checkWithTolerance = false,
    size_t trials = 0) {
  using UVT = UvalueType<T>;
  ...
  if (!bitwise && std::is_floating_point<UVT>::value) {
    ...
    testCase =
        TestingCase<T>::getBuilder()
            .set(bitwise, false)
            .addDomain(CheckWithinDomains<UVT>{
                {{(UVT)-10, (UVT)10}}, checkWithTolerance, tolerance})
            .addDomain(CheckWithinDomains<UVT>{
                {{(UVT)10, (UVT)100}}, checkWithTolerance, tolerance})
            .addDomain(CheckWithinDomains<UVT>{
                {{(UVT)100, (UVT)1000}}, checkWithTolerance, tolerance})
            .addDomain(CheckWithinDomains<UVT>{
                {{(UVT)-100, (UVT)-10}}, checkWithTolerance, tolerance})
            .addDomain(CheckWithinDomains<UVT>{
                {{(UVT)-1000, (UVT)-100}}, checkWithTolerance, tolerance})
            .addDomain(
                CheckWithinDomains<UVT>{{}, checkWithTolerance, tolerance})
...
template <typename T>
TestingCase<T> createDefaultBinaryTestCase(
    TestSeed seed = TestSeed(),
    bool bitwise = false,
    bool checkWithTolerance = false,
    size_t trials = 0) {
  ...
  if (!bitwise && std::is_floating_point<UVT>::value) {
    ...
    testCase = TestingCase<T>::getBuilder()
                   .set(bitwise, false)
                   .addDomain(CheckWithinDomains<UVT>{
                       {{(UVT)-10, (UVT)10}, {(UVT)-10, (UVT)10}},
                       checkWithTolerance,
                       tolerance})
                   .addDomain(CheckWithinDomains<UVT>{
                       {{(UVT)10, (UVT)100}, {(UVT)-10, (UVT)100}},
                       checkWithTolerance,
                       tolerance})
                   .addDomain(CheckWithinDomains<UVT>{
                       {{(UVT)100, (UVT)1000}, {(UVT)-100, (UVT)1000}},
                       checkWithTolerance,
                       tolerance})
                   .addDomain(CheckWithinDomains<UVT>{
                       {{(UVT)-100, (UVT)-10}, {(UVT)-100, (UVT)10}},
                       checkWithTolerance,
                       tolerance})
                   .addDomain(CheckWithinDomains<UVT>{
                       {{(UVT)-1000, (UVT)-100}, {(UVT)-1000, (UVT)100}},
                       checkWithTolerance,
                       tolerance})
                   .addDomain(CheckWithinDomains<UVT>{
                       {}, checkWithTolerance, tolerance})
                   .setTrialCount(trials)
                   .setTestSeed(seed);
  } else {
    testCase = TestingCase<T>::getBuilder()
...
template <typename T>
TestingCase<T> createDefaultTernaryTestCase(
    TestSeed seed = TestSeed(),
    bool bitwise = false,
    bool checkWithTolerance = false,
    size_t trials = 0) {
