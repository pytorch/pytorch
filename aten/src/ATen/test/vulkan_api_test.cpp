...
bool checkSoftShrink(
  ...
  for (int i = 0; i < ref.numel(); i++) {
    ...
    if (ref_val > abs_clamp_thresh) {
      ref_val -= abs_clamp_thresh;
    } else if (ref_val < -abs_clamp_thresh) {
      ref_val += abs_clamp_thresh;
    } else {
      ref_val = 0;
    }

    if (std::abs(ref_val - out_val) > kTolerance * max_val) {
      return false;
    }
  }
  return true;
...
static void gen_allpermutations(
...
static void slice_tests(
    const std::unordered_map<int64_t, std::vector<int64_t>>& dim2sizes) {
  for (const auto& dim2size : dim2sizes) {
    slice_test(
        ...
        10,
        30,
        1); // i.e., 4D tensor's equivalent indexing = [:,:,:,10:30:1]
    slice_test(
        dim2size.second,
        dim2size.first,
        10,
        30,
        7); // i.e., 4D tensor's equivalent indexing = [:,:,:,10:30:7]
    slice_test(
        dim2size.second,
        dim2size.first,
        10,
        50,
        2); // i.e., 4D tensor's equivalent indexing = [:,:,:,10:50:2] with
            // end=out of range
    slice_test(
        dim2size.second,
        dim2size.first,
        -60,
        60,
        2); // i.e., 4D tensor's equivalent indexing = [:,:,:,-60:60:2] with
    ...
    slice_test(
        ...
        -30,
        -10,
        1); // i.e., 4D tensor's equivalent indexing = [:,:,:,-30:-10:1] with
            // negative start/end
    slice_test(
        dim2size.second,
        dim2size.first,
        0,
        INT64_MAX,
        1); // i.e., 4D 's equivalent indexing = [:,:,:,0:9223372036854775807:1]
            // with end=INT64_MAX
    slice_test(
        dim2size.second,
        dim2size.first,
        -10,
        INT64_MAX,
        1); // i.e., 4D 's equivalent indexing =
            // [:,:,:,-10:9223372036854775807:1] with negative start and
            // end=INT64_MAX
    slice_test(
        dim2size.second,
        dim2size.first,
        ...
        INT64_MAX,
        1); // i.e., 4D 's equivalent indexing =
            // [:,:,:,-9223372036854775808:9223372036854775807:1] with
            // start=INT64_MIN and end=INT64_MAX
    slice_test(
        dim2size.second,
        dim2size.first,
        {},
        {},
        1); // i.e., 4D 's equivalent indexing = [:,:,:,::1] with empty
            // start/end
  }
}
...
TEST_F(VulkanAPITest, adaptive_avg_pool2d) {
  ...
  const auto in_cpu =
      at::rand({5, 7, 47, 31}, at::TensorOptions(at::kCPU).dtype(at::kFloat));
...
TEST_F(VulkanAPITest, add_broadcast0) {
  const auto a_cpu =
      at::rand({3, 5, 179, 221}, at::device(at::kCPU).dtype(at::kFloat));
  const auto a_vulkan = a_cpu.vulkan();

  const auto b_cpu =
      at::rand({3, 5, 1, 1}, at::device(at::kCPU).dtype(at::kFloat));
  const auto b_vulkan = b_cpu.vulkan();
...
TEST_F(VulkanAPITest, add_broadcast1) {
  ...
  const auto b_cpu =
      at::rand({3, 5, 1, 221}, at::device(at::kCPU).dtype(at::kFloat));
  const auto b_vulkan = b_cpu.vulkan();
...
TEST_F(VulkanAPITest, add_broadcast2) {
  const auto a_cpu =
      at::rand({3, 4, 179, 221}, at::device(at::kCPU).dtype(at::kFloat));
  const auto a_vulkan = a_cpu.vulkan();

  const auto b_cpu =
      at::rand({4, 1, 1}, at::device(at::kCPU).dtype(at::kFloat));
  const auto b_vulkan = b_cpu.vulkan();
...
TEST_F(VulkanAPITest, add_broadcast3) {
  const auto a_cpu =
      at::rand({3, 4, 41, 53}, at::device(at::kCPU).dtype(at::kFloat));
  const auto a_vulkan = a_cpu.vulkan();

  const auto b_cpu =
      at::rand({1, 1, 41, 53}, at::device(at::kCPU).dtype(at::kFloat));
  const auto b_vulkan = b_cpu.vulkan();
...
TEST_F(VulkanAPITest, add_broadcast4) {
  const auto a_cpu =
      at::rand({3, 4, 41, 1}, at::device(at::kCPU).dtype(at::kFloat));
  const auto a_vulkan = a_cpu.vulkan();

  const auto b_cpu =
      at::rand({1, 41, 53}, at::device(at::kCPU).dtype(at::kFloat));
  const auto b_vulkan = b_cpu.vulkan();
...
TEST_F(VulkanAPITest, add_) {
  auto a_cpu =
      at::rand({61, 17, 29, 83}, at::device(at::kCPU).dtype(at::kFloat));
...
TEST_F(VulkanAPITest, add_broadcast0_) {
  auto a_cpu =
      at::rand({16, 17, 29, 83}, at::device(at::kCPU).dtype(at::kFloat));
  auto a_vulkan = a_cpu.vulkan();

  const auto b_cpu =
      at::rand({16, 17, 29, 1}, at::device(at::kCPU).dtype(at::kFloat));
  const auto b_vulkan = b_cpu.vulkan();
...
TEST_F(VulkanAPITest, add_broadcast1_) {
  ...
  const auto b_cpu =
      at::rand({3, 8, 1, 1}, at::device(at::kCPU).dtype(at::kFloat));
  const auto b_vulkan = b_cpu.vulkan();
...
TEST_F(VulkanAPITest, add_scalar) {
  const auto a_cpu =
      at::rand({13, 23, 59, 73}, at::device(at::kCPU).dtype(at::kFloat));
  const auto a_vulkan = a_cpu.vulkan();
...
TEST_F(VulkanAPITest, add_scalar_) {
  auto a_cpu =
      at::rand({47, 2, 23, 97}, at::device(at::kCPU).dtype(at::kFloat));
  auto a_vulkan = a_cpu.vulkan();
...
TEST_F(VulkanAPITest, addmm) {
  ...
  const auto m1_cpu =
      at::rand({179, 67}, at::device(at::kCPU).dtype(at::kFloat));
  const auto m2_cpu =
      at::rand({67, 163}, at::device(at::kCPU).dtype(at::kFloat));
...
TEST_F(VulkanAPITest, addmm_expand) {
  ...
  const auto bias_cpu =
      at::rand({1000}, at::device(at::kCPU).dtype(at::kFloat));
  const auto m1_cpu =
      at::rand({1, 1280}, at::device(at::kCPU).dtype(at::kFloat));
  const auto m2_cpu =
      at::rand({1280, 1000}, at::device(at::kCPU).dtype(at::kFloat));
  const auto out_cpu = at::addmm(bias_cpu, m1_cpu, m2_cpu, beta, alpha);
...
TEST_F(VulkanAPITest, avg_pool2d) {
  const auto in_cpu =
      at::rand({3, 19, 43, 79}, at::TensorOptions(at::kCPU).dtype(at::kFloat));
  const auto out_cpu = at::avg_pool2d(in_cpu, {5, 3}, {1, 2}, {2, 0}, true);
  const auto out_vulkan =
      at::avg_pool2d(in_cpu.vulkan(), {5, 3}, {1, 2}, {2, 0}, true);
...
TEST_F(VulkanAPITest, batch_norm_invalid_inputs) {
  ...
  EXPECT_THROW(
      {
        at::batch_norm(
            at::rand({3, 8, 5, 7}, at::device(at::kCPU).dtype(at::kFloat))
                .vulkan(),
  ...
  EXPECT_THROW(
      {
        at::batch_norm(
            at::rand({3, 8, 5}, at::device(at::kCPU).dtype(at::kFloat))
                .vulkan(),
            at::rand({8}, at::device(at::kCPU).dtype(at::kFloat)).vulkan(),
            at::rand({8}, at::device(at::kCPU).dtype(at::kFloat)).vulkan(),
            at::rand({8}, at::device(at::kCPU).dtype(at::kFloat)).vulkan(),
  ...
  EXPECT_THROW(
      {
        at::batch_norm(
            at::rand({2, 8, 3, 5, 7}, at::device(at::kCPU).dtype(at::kFloat))
                .vulkan(),
            at::rand({8}, at::device(at::kCPU).dtype(at::kFloat)).vulkan(),
            at::rand({8}, at::device(at::kCPU).dtype(at::kFloat)).vulkan(),
            at::rand({8}, at::device(at::kCPU).dtype(at::kFloat)).vulkan(),
  ...
  EXPECT_THROW(
      {
        at::batch_norm(
            at::rand({4, 7, 4, 4}, at::device(at::kCPU).dtype(at::kFloat))
                .vulkan(),
  ...
  EXPECT_THROW(
      {
        at::batch_norm(
            at::rand({4, 8, 4, 4}, at::device(at::kCPU).dtype(at::kFloat))
                .vulkan(),
...
TEST_F(VulkanAPITest, batch_norm_small) {
  ...
  const auto input_cpu =
      at::rand({1, 4, 1, 1}, at::device(at::kCPU).dtype(at::kFloat));
  ...
  const auto running_mean_cpu =
      at::rand({4}, at::device(at::kCPU).dtype(at::kFloat));
  const auto running_mean_vulkan = running_mean_cpu.vulkan();

  const auto running_var_cpu =
      at::rand({4}, at::device(at::kCPU).dtype(at::kFloat));
  const auto running_var_vulkan = running_var_cpu.vulkan();

  const auto output_cpu = at::batch_norm(
      input_cpu,
      weight_cpu,
      bias_cpu,
      running_mean_cpu,
      ...
      1e-05,
      false);
  const auto output_vulkan = at::batch_norm(
      input_vulkan,
      weight_vulkan,
      bias_vulkan,
      running_mean_vulkan,
...
TEST_F(VulkanAPITest, batch_norm_medium) {
  ...
  const auto running_mean_cpu =
      at::rand({8}, at::device(at::kCPU).dtype(at::kFloat));
  const auto running_mean_vulkan = running_mean_cpu.vulkan();
...
TEST_F(VulkanAPITest, batch_norm_large) {
  ...
  const auto input_cpu =
      at::rand({79, 52, 139, 109}, at::device(at::kCPU).dtype(at::kFloat));
  const auto input_vulkan = input_cpu.vulkan();

  const auto weight_cpu =
      at::rand({52}, at::device(at::kCPU).dtype(at::kFloat));
  const auto weight_vulkan = weight_cpu.vulkan();
...
TEST_F(VulkanAPITest, cumsum) {
  ...
  const auto in_cpu =
      at::rand({1, 17, 37, 49}, at::TensorOptions(at::kCPU).dtype(at::kFloat));
  // 0 do nothing
  // 1 frame
  // not implemented
...
TEST_F(VulkanAPITest, div_broadcast0) {
  const auto a_cpu =
      at::rand({3, 5, 1, 1}, at::device(at::kCPU).dtype(at::kFloat)) + 0.01;
  const auto a_vulkan = a_cpu.vulkan();

  const auto b_cpu =
      at::rand({3, 5, 179, 221}, at::device(at::kCPU).dtype(at::kFloat)) + 0.01;
  const auto b_vulkan = b_cpu.vulkan();
...
TEST_F(VulkanAPITest, div_broadcast1) {
  ...
  const auto b_cpu =
      at::rand({3, 5, 1, 221}, at::device(at::kCPU).dtype(at::kFloat)) + 0.01;
  const auto b_vulkan = b_cpu.vulkan();
...
TEST_F(VulkanAPITest, div_broadcast2) {
  const auto a_cpu =
      at::rand({3, 4, 179, 221}, at::device(at::kCPU).dtype(at::kFloat)) + 0.01;
  const auto a_vulkan = a_cpu.vulkan();

  const auto b_cpu =
      at::rand({4, 1, 1}, at::device(at::kCPU).dtype(at::kFloat)) + 0.01;
  const auto b_vulkan = b_cpu.vulkan();
...
TEST_F(VulkanAPITest, div_broadcast3) {
  ...
  const auto b_cpu =
      at::rand({1, 1, 179, 221}, at::device(at::kCPU).dtype(at::kFloat)) + 0.01;
  const auto b_vulkan = b_cpu.vulkan();
...
TEST_F(VulkanAPITest, div_) {
  auto a_cpu =
      at::rand({61, 17, 29, 83}, at::device(at::kCPU).dtype(at::kFloat)) + 0.01;
  auto a_vulkan = a_cpu.vulkan();
...
TEST_F(VulkanAPITest, div_broadcast0_) {
  auto a_cpu =
      at::rand({12, 17, 29, 83}, at::device(at::kCPU).dtype(at::kFloat)) + 0.01;
  auto a_vulkan = a_cpu.vulkan();

  const auto b_cpu =
      at::rand({12, 17, 29, 1}, at::device(at::kCPU).dtype(at::kFloat)) + 0.01;
  const auto b_vulkan = b_cpu.vulkan();
...
TEST_F(VulkanAPITest, div_broadcast1_) {
  auto a_cpu =
      at::rand({3, 8, 29, 83}, at::device(at::kCPU).dtype(at::kFloat)) + 0.01;
  auto a_vulkan = a_cpu.vulkan();

  const auto b_cpu =
      at::rand({8, 1, 1}, at::device(at::kCPU).dtype(at::kFloat)) + 0.01;
  const auto b_vulkan = b_cpu.vulkan();
...
TEST_F(VulkanAPITest, div_to_scalar_wrapped) {
  ...
  const auto b_cpu =
      at::rand({2, 3, 5, 7}, at::device(at::kCPU).dtype(at::kFloat));
  const auto b_vulkan = b_cpu.vulkan();
...
TEST_F(VulkanAPITest, hardsigmoid) {
  const auto in_cpu =
      at::rand({17, 197, 302, 5}, at::device(at::kCPU).dtype(at::kFloat)) * 12 -
      6;
  const auto in_vulkan = in_cpu.vulkan();
...
TEST_F(VulkanAPITest, softshrink) {
  for (const auto lambd_value : {-4.2, -1.0, 0.42, 1.0, 4.2, 13.7}) {
    // Generate values between -10 and +10
    const auto in_cpu =
        (at::rand({3, 63, 79, 17}, at::device(at::kCPU).dtype(at::kFloat)) -
         0.5) *
        20;
    const auto in_vulkan = in_cpu.vulkan();

    const auto out_vulkan = at::softshrink(in_vulkan, lambd_value);

    const auto check = checkSoftShrink(in_cpu, out_vulkan.cpu(), lambd_value);
    ASSERT_TRUE(check);
  }
}
...
TEST_F(VulkanAPITest, layer_norm_invalid_inputs) {
  ...
  EXPECT_THROW(
      {
        at::layer_norm(
            at::rand({3, 5, 7}, at::device(at::kCPU).dtype(at::kFloat))
                .vulkan(),
            {8, 5},
  ...
  EXPECT_THROW(
      {
        at::layer_norm(
            at::rand({1, 2, 3, 5, 7}, at::device(at::kCPU).dtype(at::kFloat))
                .vulkan(),
            {3, 5, 7},
            at::rand({3, 5, 7}, at::device(at::kCPU).dtype(at::kFloat))
                .vulkan(),
...
TEST_F(VulkanAPITest, layer_norm_3d_small) {
  ...
  const auto output_cpu =
      at::layer_norm(input_cpu, {1, 1, 1}, weight_cpu, bias_cpu, 1e-05, false);
  const auto output_vulkan = at::layer_norm(
      input_vulkan, {1, 1, 1}, weight_vulkan, bias_vulkan, 1e-05, false);
...
TEST_F(VulkanAPITest, layer_norm_3d_medium) {
  ...
  const auto output_cpu =
      at::layer_norm(input_cpu, {3, 5, 7}, weight_cpu, bias_cpu, 1e-05, false);
  const auto output_vulkan = at::layer_norm(
      input_vulkan, {3, 5, 7}, weight_vulkan, bias_vulkan, 1e-05, false);
...
TEST_F(VulkanAPITest, layer_norm_3d_large) {
  ...
  const auto input_cpu =
      at::rand({53, 139, 109}, at::device(at::kCPU).dtype(at::kFloat));
  const auto input_vulkan = input_cpu.vulkan();
  ...
  const auto output_cpu = at::layer_norm(
      input_cpu, {53, 139, 109}, weight_cpu, bias_cpu, 1e-05, false);
  const auto output_vulkan = at::layer_norm(
      input_vulkan, {53, 139, 109}, weight_vulkan, bias_vulkan, 1e-05, false);
...
TEST_F(VulkanAPITest, layer_norm_4d_medium) {
  ...
  const auto input_cpu =
      at::rand({1, 3, 5, 7}, at::device(at::kCPU).dtype(at::kFloat));
  const auto input_vulkan = input_cpu.vulkan();
...
TEST_F(VulkanAPITest, layer_norm_4d_large) {
  ...
  const auto input_cpu =
      at::rand({1, 53, 139, 109}, at::device(at::kCPU).dtype(at::kFloat));
  const auto input_vulkan = input_cpu.vulkan();
...
TEST_F(VulkanAPITest, lerp_broadcast1) {
  ...
  const auto b_cpu =
      at::rand({4, 179, 221}, at::device(at::kCPU).dtype(at::kFloat));
  const auto b_vulkan = b_cpu.vulkan();
...
TEST_F(VulkanAPITest, threshold) {
  const auto in_cpu =
      at::rand({2, 11, 57, 23}, at::device(at::kCPU).dtype(at::kFloat)) * 12 -
      6;
  const auto in_vulkan = in_cpu.vulkan();
  ...
  const auto check =
      checkThreshold(out_cpu, out_vulkan.cpu(), threshold, value);
  ASSERT_TRUE(check);
}
...
TEST_F(VulkanAPITest, max_pool2d) {
  ...
  const auto out_vulkan =
      at::max_pool2d(in_cpu.vulkan(), {3, 4}, {2, 1}, {1, 1}, {1, 1}, false);
...
TEST_F(VulkanAPITest, mean) {
  const auto in_cpu =
      at::rand({17, 3, 79, 53}, at::TensorOptions(at::kCPU).dtype(at::kFloat));
...
TEST_F(VulkanAPITest, mean2d) {
  const auto in_cpu =
      at::rand({11, 7, 173, 37}, at::TensorOptions(at::kCPU).dtype(at::kFloat));
...
TEST_F(VulkanAPITest, mul_broadcast3) {
  ...
  const auto b_cpu =
      at::rand({1, 1, 179, 221}, at::device(at::kCPU).dtype(at::kFloat));
  const auto b_vulkan = b_cpu.vulkan();
...
TEST_F(VulkanAPITest, mul_broadcast4) {
  const auto a_cpu =
      at::rand({3, 4, 179, 1}, at::device(at::kCPU).dtype(at::kFloat));
  const auto a_vulkan = a_cpu.vulkan();

  const auto b_cpu =
      at::rand({1, 179, 221}, at::device(at::kCPU).dtype(at::kFloat));
  const auto b_vulkan = b_cpu.vulkan();
...
TEST_F(VulkanAPITest, mul_broadcast0_) {
  auto a_cpu =
      at::rand({12, 17, 29, 83}, at::device(at::kCPU).dtype(at::kFloat));
  auto a_vulkan = a_cpu.vulkan();

  const auto b_cpu =
      at::rand({12, 17, 29, 1}, at::device(at::kCPU).dtype(at::kFloat));
  const auto b_vulkan = b_cpu.vulkan();
...
TEST_F(VulkanAPITest, mul_broadcast1_) {
  ...
  const auto b_cpu =
      at::rand({8, 1, 1}, at::device(at::kCPU).dtype(at::kFloat));
  const auto b_vulkan = b_cpu.vulkan();
...
TEST_F(VulkanAPITest, reflection_pad2d) {
  const auto a_cpu =
      at::rand({2, 3, 47, 63}, at::device(at::kCPU).dtype(at::kFloat));
  const auto a_vulkan = a_cpu.vulkan();
...
TEST_F(VulkanAPITest, reshape) {
  ...
  const auto in_cpu =
      at::rand({7, 11, 8, 9}, at::device(at::kCPU).dtype(at::kFloat));
  const auto in_vulkan = in_cpu.vulkan();
...
TEST_F(VulkanAPITest, reshape_) {
  ...
  const auto cpu =
      at::rand({9, 4, 12, 6}, at::device(at::kCPU).dtype(at::kFloat));
  const auto vulkan = cpu.vulkan();
...
// TODO: Currently the op is not working correctly. Add it back when it is
// fixed.
...
TEST_F(VulkanAPITest, tanh) {
  const auto in_cpu =
      at::rand({17, 197, 302, 5}, at::device(at::kCPU).dtype(at::kFloat)) * 30;
  const auto in_vulkan = in_cpu.vulkan();
...
TEST_F(VulkanAPITest, DISABLED_cat_dim1_samefeature_success) {
  // Arrange
  const auto in_cpu1 =
      at::rand({3, 9, 221, 193}, at::device(at::kCPU).dtype(at::kFloat));
  const auto in_cpu2 =
      at::rand({3, 9, 221, 193}, at::device(at::kCPU).dtype(at::kFloat));
  const auto in_cpu3 =
      at::rand({3, 9, 221, 193}, at::device(at::kCPU).dtype(at::kFloat));
  ...
  const auto out_vulkan = at::cat(
      {in_cpu1.vulkan(), in_cpu2.vulkan(), in_cpu3.vulkan()},
      1); // dim=feature(channel)
...
TEST_F(VulkanAPITest, DISABLED_cat_dim1_difffeature_success) {
  // Arrange
  const auto in_cpu1 =
      at::rand({3, 3, 221, 193}, at::device(at::kCPU).dtype(at::kFloat));
  const auto in_cpu2 =
      at::rand({3, 8, 221, 193}, at::device(at::kCPU).dtype(at::kFloat));
  const auto in_cpu3 =
      at::rand({3, 11, 221, 193}, at::device(at::kCPU).dtype(at::kFloat));
...
TEST_F(VulkanAPITest, cat_dim1_texture2d_success) {
  ...
  const auto in_cpu1 =
      at::rand({2, 3, 2, 2}, at::device(at::kCPU).dtype(at::kFloat));
  const auto in_cpu2 =
      at::rand({2, 3, 2, 2}, at::device(at::kCPU).dtype(at::kFloat));
  const auto in_cpu3 =
      at::rand({2, 3, 2, 2}, at::device(at::kCPU).dtype(at::kFloat));
...
TEST_F(VulkanAPITest, cat_dim1_singledepth_success) {
  ...
  const auto in_cpu1 =
      at::rand({1, 1, 221, 193}, at::device(at::kCPU).dtype(at::kFloat));
  const auto in_cpu2 =
      at::rand({1, 1, 221, 193}, at::device(at::kCPU).dtype(at::kFloat));
  const auto in_cpu3 =
      at::rand({1, 1, 221, 193}, at::device(at::kCPU).dtype(at::kFloat));
...
TEST_F(VulkanAPITest, cat_dim1_singletensor_success) {
  ...
  const auto in_cpu1 =
      at::rand({3, 7, 221, 193}, at::device(at::kCPU).dtype(at::kFloat));
...
TEST_F(VulkanAPITest, DISABLED_cat_dim1_twotensors_success) {
  ...
  const auto out_vulkan =
      at::cat({in_cpu1.vulkan(), in_cpu2.vulkan()}, 1); // dim=feature(channel)
...
TEST_F(VulkanAPITest, cat_dim1_bat1_mult4ch_success) {
  ...
  const auto in_cpu1 =
      at::rand({1, 4, 221, 193}, at::device(at::kCPU).dtype(at::kFloat));
  const auto in_cpu2 =
      at::rand({1, 4, 221, 193}, at::device(at::kCPU).dtype(at::kFloat));
  const auto in_cpu3 =
      at::rand({1, 4, 221, 193}, at::device(at::kCPU).dtype(at::kFloat));
...
TEST_F(VulkanAPITest, cat_dim1_bat2_mult4ch_success) {
  ...
  const auto in_cpu1 =
      at::rand({2, 4, 221, 193}, at::device(at::kCPU).dtype(at::kFloat));
  const auto in_cpu2 =
      at::rand({2, 4, 221, 193}, at::device(at::kCPU).dtype(at::kFloat));
  const auto in_cpu3 =
      at::rand({2, 4, 221, 193}, at::device(at::kCPU).dtype(at::kFloat));
...
TEST_F(VulkanAPITest, cat_dim1_mult4ch_mixed_success) {
  ...
  const auto in_cpu1 =
      at::rand({3, 4, 221, 193}, at::device(at::kCPU).dtype(at::kFloat));
  const auto in_cpu2 =
      at::rand({3, 8, 221, 193}, at::device(at::kCPU).dtype(at::kFloat));
  const auto in_cpu3 =
      at::rand({3, 12, 221, 193}, at::device(at::kCPU).dtype(at::kFloat));
...
TEST_F(VulkanAPITest, DISABLED_cat_dim1_mult4ch_nonmult4ch_success) {
  // Arrange: batch=1 and channel (a mixed set of multiples and non-multiples of
  // 4)
  const auto in_cpu1 =
      at::rand({3, 3, 221, 193}, at::device(at::kCPU).dtype(at::kFloat));
  const auto in_cpu2 =
  ...
  const auto out_vulkan = at::cat(
      {in_cpu1.vulkan(), in_cpu2.vulkan(), in_cpu3.vulkan(), in_cpu4.vulkan()},
      1); // dim=feature(channel)
...
TEST_F(VulkanAPITest, cat_dim2_sameheight_success) {
  ...
  const auto out_vulkan =
      at::cat({in_cpu1.vulkan(), in_cpu2.vulkan(), in_cpu3.vulkan()}, 2);
...
TEST_F(VulkanAPITest, cat_dim2_diffheight_success) {
  ...
  const auto in_cpu1 =
      at::rand({3, 9, 221, 193}, at::device(at::kCPU).dtype(at::kFloat));
  const auto in_cpu2 =
      at::rand({3, 9, 112, 193}, at::device(at::kCPU).dtype(at::kFloat));
  const auto in_cpu3 =
      at::rand({3, 9, 331, 193}, at::device(at::kCPU).dtype(at::kFloat));
...
TEST_F(VulkanAPITest, cat_dim2_invalidinputs_exceptions) {
  // Arrange: Vulkan cat inputs must have matching sizes except concatenated
  // dimension
  {
    const auto in_cpu1 =
        at::rand({3, 5, 221, 193}, at::device(at::kCPU).dtype(at::kFloat));
    const auto in_cpu2 =
        at::rand({3, 9, 112, 193}, at::device(at::kCPU).dtype(at::kFloat));
    const auto in_cpu3 =
        at::rand({3, 9, 331, 193}, at::device(at::kCPU).dtype(at::kFloat));
    ...
    EXPECT_THROW(
        {
          const auto out_vulkan = at::cat(
              {in_cpu1.vulkan(), in_cpu2.vulkan(), in_cpu3.vulkan()}, 2);
        },
        ::c10::Error);
  }
  ...
  // Arrange: Vulkan cat expects 4 dimensional inputs
  {
    const auto in_cpu1 =
        at::rand({3, 9, 221, 193}, at::device(at::kCPU).dtype(at::kFloat));
    const auto in_cpu2 =
        at::rand({9, 112, 193}, at::device(at::kCPU).dtype(at::kFloat));
    const auto in_cpu3 =
        at::rand({3, 9, 331, 193}, at::device(at::kCPU).dtype(at::kFloat));
  ...
  // Arrange: Vulkan cat is implemented only for height dimension
  {
    ...
    EXPECT_THROW(
        {
          const auto out_vulkan = at::cat(
              {in_cpu1.vulkan(), in_cpu2.vulkan(), in_cpu3.vulkan()}, 0);
        },
        ::c10::Error);
    EXPECT_THROW(
        {
          const auto out_vulkan = at::cat(
              {in_cpu1.vulkan(), in_cpu2.vulkan(), in_cpu3.vulkan()}, 1);
        },
        ::c10::Error);
    EXPECT_THROW(
        {
          const auto out_vulkan = at::cat(
              {in_cpu1.vulkan(), in_cpu2.vulkan(), in_cpu3.vulkan()}, 3);
        },
        ::c10::Error);
  }
}
...
TEST_F(VulkanAPITest, permute_4d_success) {
  // Arrange
  const auto in_cpu =
      at::rand({2, 3, 4, 5}, at::device(at::kCPU).dtype(at::kFloat));
  std::vector<std::vector<int64_t>> all_dims;
...
TEST_F(VulkanAPITest, permute_4dmclaren_success) {
  ...
  const auto in_cpu =
      at::rand({1, 2, 1, 161}, at::device(at::kCPU).dtype(at::kFloat));
...
TEST_F(VulkanAPITest, permute_4dbig_success) {
  // Arrange
  const auto in_cpu =
      at::rand({3, 9, 89, 91}, at::device(at::kCPU).dtype(at::kFloat));
  std::vector<std::vector<int64_t>> all_dims;
  std::vector<int64_t> in{0, 1, 2, 3};
  gen_allpermutations(all_dims, in, 0);
...
TEST_F(VulkanAPITest, permute_negativedims_success) {
  // Arrange
  const auto in_cpu =
      at::rand({5, 4, 3, 2}, at::device(at::kCPU).dtype(at::kFloat));
...
TEST_F(VulkanAPITest, permute_invalidinputs_exceptions) {
  ...
  const auto in_cpu_5d =
      at::rand({1, 2, 1, 2, 161}, at::device(at::kCPU).dtype(at::kFloat));
  EXPECT_THROW(
      {
...
TEST_F(VulkanAPITest, slice_feature_success) {
  ...
  std::unordered_map<int64_t, std::vector<int64_t>> dim2sizes{
      ...
      {0,
       {40,
        13,
        14}}, // 3D tensors with dim=feature(channel)
...
TEST_F(VulkanAPITest, slice_batch_success) {
  ...
  std::unordered_map<int64_t, std::vector<int64_t>> dim2sizes{
      {0,
       {40, 3, 13, 14}}, // 4D tensors with dim=batch
...
TEST_F(VulkanAPITest, slice_invalidinputs_exceptions) {
  ...
  EXPECT_THROW({ slice_test({2, 3, 4, 5}, 3, 0, 3, 0); }, ::c10::Error);

  // Act: Vulkan doesn't support zero-sized slice (when start=end)
  EXPECT_THROW({ slice_test({2, 3, 4, 5}, 3, 0, 0, 1); }, ::c10::Error);

  // Act: Vulkan doesn't support zero-sized slice (when start > end)
  EXPECT_THROW({ slice_test({2, 3, 4, 5}, 3, 3, 2, 1); }, ::c10::Error);
}

TEST_F(VulkanAPITest, stack_invalid_inputs) {
  // Act: Vulkan stack expects at least one tensor
  EXPECT_THROW({ at::stack({}, 0); }, ::c10::Error);
  ...
  EXPECT_THROW(
      {
        at::stack(
            {at::rand({5, 7}, at::device(at::kCPU).dtype(at::kFloat)).vulkan(),
             at::rand({5, 7}, at::device(at::kCPU).dtype(at::kFloat)).vulkan(),
             at::rand({6, 7}, at::device(at::kCPU).dtype(at::kFloat)).vulkan()},
            0);
      },
      ::c10::Error);
}
...
TEST_F(VulkanAPITest, stack_1_tensor) {
  // Arrange
  const auto in_cpu1 =
      at::rand({221, 193}, at::device(at::kCPU).dtype(at::kFloat));
...
TEST_F(VulkanAPITest, stack_3_tensors) {
  ...
  const auto out_vulkan =
      at::stack({in_cpu1.vulkan(), in_cpu2.vulkan(), in_cpu3.vulkan()}, 0);
...
TEST_F(VulkanAPITest, stack_4_tensors) {
  ...
  const auto out_vulkan = at::stack(
      {in_cpu1.vulkan(), in_cpu2.vulkan(), in_cpu3.vulkan(), in_cpu4.vulkan()},
      0);
...
TEST_F(VulkanAPITest, clone_success) {
  // Arrange
  std::multimap<c10::optional<c10::MemoryFormat>, std::vector<int64_t>>
      mem2sizes{
          {c10::MemoryFormat::Preserve,
           {2, 3, 5, 161}}, // 4D tensors with MemoryFormat::Preserve
          {c10::MemoryFormat::Contiguous,
           {2, 3, 5, 161}}, // 4D tensors with MemoryFormat::Contiguous
          {{}, {2, 3, 5, 161}}, // 4D tensors with null
          {c10::MemoryFormat::Preserve,
           {3, 5, 161}}, // 3D tensors with MemoryFormat::Preserve
          {c10::MemoryFormat::Contiguous,
           {3, 5, 161}}, // 3D tensors with MemoryFormat::Contiguous
          {{}, {3, 5, 161}}, // 3D tensors with null
          {c10::MemoryFormat::Preserve,
           {5, 161}}, // 2D tensors with MemoryFormat::Preserve
          {c10::MemoryFormat::Contiguous,
           {5, 161}}, // 2D tensors with MemoryFormat::Contiguous
          {{}, {5, 161}}, // 2D tensors with null
          {c10::MemoryFormat::Preserve,
           {161}}, // 1D tensors with MemoryFormat::Preserve
          {c10::MemoryFormat::Contiguous,
           {161}}, // 1D tensors with MemoryFormat::Contiguous
...
class Addmm final : public BaseOp {
 public:
  Addmm(
      ...
      : BaseOp(OpType::addmm),
        m2_(at::rand(
            c10::IntArrayRef({m1W, m2W}),
            at::device(at::kCPU).dtype(at::kFloat))),
        b_(at::rand(
            c10::IntArrayRef({m1H, m2W}),
            at::device(at::kCPU).dtype(at::kFloat))),
        beta_(beta),
        alpha_(alpha) {}
...
TEST_F(VulkanAPITest, gru_invalidinputs_exceptions) {
  ...
  EXPECT_THROW(
      {
        const auto in_cpu_2d =
            at::rand({1, H_in}, at::device(at::kCPU).dtype(at::kFloat));
        at::gru(
            in_cpu_2d.vulkan(),
            h0_cpu.vulkan(),
            {weight_ih_l.get(0),
  ...
  EXPECT_THROW(
      {
        const auto h0_cpu_2d = at::rand(
            {num_layers, H_out}, at::device(at::kCPU).dtype(at::kFloat));
        at::gru(
            in_cpu.vulkan(),
            h0_cpu_2d.vulkan(),
            {weight_ih_l.get(0),
...
void test_linear(
  ...
  auto prepack = callOpByName(
      "vulkan_prepack::create_linear_context", "", weight.t(), bias);

  auto vulkan_output = callOpByName(
      "vulkan_prepack::run_linear_context", "", input_cpu.vulkan(), prepack[0]);
...
TEST_F(VulkanAPITest, lstm_success) {
  ...
  const bool batch_first = true;
  const auto in_cpu =
      at::rand({N, L, input_size}, at::device(at::kCPU).dtype(at::kFloat));
  const auto h0_cpu = at::rand(
      {num_layers, N, hidden_size}, at::device(at::kCPU).dtype(at::kFloat));
  const auto c0_cpu = at::rand(
      {num_layers, N, hidden_size}, at::device(at::kCPU).dtype(at::kFloat));
  ...
  for (int l = 0; l < num_layers; ++l) {
    if (l == 0) {
      weight_ih_l.emplace_back(at::rand(
    ...
    weight_hh_l.emplace_back(at::rand(
        {4 * hidden_size, hidden_size},
        at::device(at::kCPU).dtype(at::kFloat)));
    bias_ih_l.emplace_back(
        at::rand({4 * hidden_size}, at::device(at::kCPU).dtype(at::kFloat)));
    bias_hh_l.emplace_back(
        at::rand({4 * hidden_size}, at::device(at::kCPU).dtype(at::kFloat)));
  }
  ...
  // weights/biases should be always on CPU.
  const auto out_vulkan = at::lstm(
      in_cpu.vulkan(),
      {h0_cpu.vulkan(), c0_cpu.vulkan()},
      {weight_ih_l.get(0),
       weight_hh_l.get(0),
       bias_ih_l.get(0),
       bias_hh_l.get(0),
...
TEST_F(VulkanAPITest, lstm_prepack_success) {
  ...
  c10::List<at::Tensor>
      weight_ih_l; // shape (4 * hidden_size, l == 0 ? input_size : hidden_size)
  c10::List<at::Tensor> weight_hh_l; // shape (4 * hidden_size, hidden_size)
  c10::List<at::Tensor> bias_ih_l; // shape (4 * hidden_size)
  c10::List<at::Tensor> bias_hh_l; // shape (4 * hidden_size)
  for (int l = 0; l < num_layers; ++l) {
