...
void test_random_from_to(const at::Device& device) {
  ...
  if (std::is_same<T, bool>::value) {
    froms = {0L};
    tos = {1L, static_cast<c10::optional<int64_t>>(c10::nullopt)};
  } else if (std::is_signed<T>::value) {
    froms = {min_from, -42L, 0L, 42L};
    tos = {
    ...
    for (const c10::optional<int64_t> to : tos) {
      if (!to.has_value() || from < *to) {
        for (const uint64_t val : vals) {
          ...
          auto actual = torch::empty(
              {3, 3}, torch::TensorOptions().dtype(S).device(device));
          ...
          if (!to.has_value() && from == int64_min_val) {
            ...
            if (to.has_value()) {
              ...
              range = static_cast<uint64_t>(max_to) -
                  static_cast<uint64_t>(from) + 1;
              from_case_covered = true;
            }
            if (range < (1ULL << 32)) {
              exp = static_cast<T>(static_cast<int64_t>(
                  (static_cast<uint32_t>(val) % range + from)));
            } else {
          ...
          if (std::is_same<T, bool>::value) {
            ASSERT_TRUE(torch::allclose(
                actual.toType(torch::kInt), expected.toType(torch::kInt)));
          } else {
...
void test_random(const at::Device& device) {
  ...
  for (const uint64_t val : vals) {
    ...
    auto actual =
        torch::empty({3, 3}, torch::TensorOptions().dtype(S).device(device));
    actual.random_(gen);
