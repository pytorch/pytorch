...
TEST(TestQTensor, QuantDequantAPIs) {
  ...
  const auto* int_repr_data = int_repr.const_data_ptr<uint8_t>();
  for (const auto i : c10::irange(num_elements)) {
    ASSERT_EQ(int_repr_data[i], 3);
  }

  // Check for correct quantization
  auto r_data = r.const_data_ptr<float>();
  auto qr_data = qr.const_data_ptr<quint8>();
  for (const auto i : c10::irange(num_elements)) {
    ASSERT_EQ(
  ...
  auto rqr_data = rqr.const_data_ptr<float>();
  for (const auto i : c10::irange(num_elements)) {
...
TEST(TestQTensor, RoundingMode) {
  ...
  auto qx_data = qx.const_data_ptr<quint8>();
...
TEST(TestQTensor, EmptyQuantized) {
  ...
  // dequantize
  auto r = q.dequantize();
  const auto* r_data = r.const_data_ptr<float>();
  for (const auto i : c10::irange(numel)) {
...
TEST(TestQTensor, QuantizePerChannel4d) {
  ...
  const auto* q_data = reinterpret_cast<const uint8_t*>(q.const_data_ptr<quint8>());
  for (int c = 0, i = 0; c < C; ++c) {
...
TEST(TestQTensor, FromBlobQuantizedPerTensor) {
  ...
  const uint8_t* q_data = reinterpret_cast<const uint8_t*>(qtensor.const_data_ptr<quint8>());
  for (const auto i : c10::irange(numel)) {
...
TEST(TestQTensor, TestArmVectorizedQuantizeDequantize) {
  ...
  test_for_datatype(
    kQUInt8,
    [](Tensor q) { return q.const_data_ptr<quint8>(); },
  ...
  test_for_datatype(
    kQInt8,
    [](Tensor q) { return q.const_data_ptr<qint8>(); },
  ...
  test_for_datatype(
    kQInt32,
    [](Tensor q) { return q.const_data_ptr<qint32>(); },
