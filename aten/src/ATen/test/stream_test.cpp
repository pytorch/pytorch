...
TEST(TestStream, CopyAndMoveTest) {
  ...
  {
    auto s = at::cuda::getNewPooledStream();
    device = s.device();
...
TEST(TestStream, GetAndSetTest) {
  at::cuda::CUDAStream myStream = at::cuda::getNewPooledStream();
...
void thread_fun(at::cuda::CUDAStream& cur_thread_stream) {
  auto new_stream = at::cuda::getNewPooledStream();
...
TEST(TestStream, CUDAGuardTest) {
  ...
  std::vector<at::cuda::CUDAStream> streams0 = {
      at::cuda::getDefaultCUDAStream(), at::cuda::getNewPooledStream()};
  ...
  std::vector<at::cuda::CUDAStream> streams1;
  {
    ...
    streams1.push_back(at::cuda::getNewPooledStream());
  }
...
TEST(TestStream, CUDAGuardMovableTest) {
  ...
  const auto stream = at::cuda::getNewPooledStream();
...
TEST(TestStream, StreamPoolTest) {
  ...
  for (int i = 0; i < 200; ++i) {
    streams.emplace_back(at::cuda::detail::CUDAStream_getNewPooledStream());
  }
...
TEST(TestStream, MultiGPUTest) {
  ...
  at::cuda::CUDAStream s0 = at::cuda::getNewPooledStream(true, 0);
  at::cuda::CUDAStream s1 = at::cuda::getNewPooledStream(false, 1);
...
TEST(TestStream, CUDAEventSyncTest) {
  ...
  const auto wait_stream0 = at::cuda::getNewPooledStream();
  const auto wait_stream1 = at::cuda::getNewPooledStream();
...
TEST(TestStream, CrossDeviceTest) {
  ...
  const auto stream0 = at::cuda::getNewPooledStream();
  at::cuda::CUDAEvent event0;

  at::cuda::set_device(1);
  const auto stream1 = at::cuda::getNewPooledStream();
