...
TEST(IValueTest, Basic) {
  ...
  // ASSERT_EQ(foo1.use_count(), 3);
  // ASSERT_TRUE(foo12.isComplexList());
  ...
  //     at::ivalue::Tuple::create({IValue(c10::complex<float>(3.4, 4.7)), IValue(c10::complex<float>(0, 2.3)), bar1}));
  // ASSERT_EQ(foo.use_count(), 3);
  // ASSERT_TRUE(the_complex_list.isTuple());
  // auto first_complex_element = the_complex_list.toTuple()->elements()[1];
  // ASSERT_EQ(first.toComplexDouble(), c10::complex<double>(3.4, 4.7));
}
