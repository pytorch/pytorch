...
ThreadLocalState::ThreadLocalState()
    : dispatch_key_(c10::impl::tls_local_dispatch_key_set()),
      ...
      python_torch_function_state_(
          at::impl::PythonTorchFunctionTLS::get_state()) {
...
void ThreadLocalState::setThreadLocalState(const ThreadLocalState& state) {
  // Note that setting the InferenceMode TLS in this function is ONLY ok because
  // we always restore the dispatch key set TLS at the same time.
  ...
  at::impl::PythonTorchFunctionTLS::set_state(
      state.python_torch_function_state_);
