...
template <>
inline THHalf __device__
curand_uniform_type<THHalf>(curandStatePhilox4_32_10_t* state) {
...
curand_uniform_type<float>(curandStatePhilox4_32_10_t* state) {
  auto rand = curand_uniform4(state);
  return rand.x;
}

template <>
inline double __device__
curand_uniform_type<double>(curandStatePhilox4_32_10_t* state) {
...
struct RReLUUpdateOutputEval_functor {
  ...
  RReLUUpdateOutputEval_functor(T negSlope) : negSlope_(negSlope) {}
...
struct RReLUUpdateOutputEvalIP_functor {
  ...
  RReLUUpdateOutputEvalIP_functor(T negSlope) : negSlope_(negSlope) {}
