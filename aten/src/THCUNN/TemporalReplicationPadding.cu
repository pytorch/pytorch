...
__global__ void TemporalReplicationPadding_updateOutput(
  ...
  int64_t inputPointX = min(max(padL, outputPointX), input.getSize(2) + padL - 1) - oStartX + iStartX;
...
__global__ void TemporalReplicationPadding_updateGradInput(
  ...
  int64_t inputPointX = min(max(padL, outputPointX), gradInput.getSize(2) + padL - 1) - oStartX + iStartX;
