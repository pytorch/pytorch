...
struct gatedLinearCSigMul_functor {
  __device__ void operator()(
      Dtype* target,
      const Dtype* sigTensor,
      const Dtype* mulTensor) const {
    const Acctype sigNum = Acctype(1) /
        (Acctype(1) + exp(ScalarConvert<Dtype, Acctype>::to(-*sigTensor)));
...
struct gatedLinearDerivative {
  ...
  __device__ void operator()(Dtype* gI, const Dtype* gO, const Dtype* input)
      const {
    ...
    *gIsecond = ScalarConvert<Acctype, Dtype>::to(
        (Acctype(1) - sigNum) * sigNum * *gO * *input);
  }
};
