// updateOutput, updateGradInput Kernels ported from Sergey Zagoruyko's pyinn,
// which itself was a port from Caffe
...
__global__ void spatialDepthwiseConvolutionAccGradParameters(
  ...
  // Use warp per item.  In the original kernel, a threadblock was used to sum
  // over NHW. Here, we use a warp to sum values over HW dimension, and if
  // batchSize is larger than the number of warps, a warp would loop over
  // remaining batch items (e.g. if there are 8 warps, warp 0 would go over
  // 0-8-16 etc image, warp 1 over 1-9-17 etc). Later in blockReduce, all the
  // warps will be reduced anyway, thus the full reduction will be over NHW,
  // like it should be. That allows to get rid of one modulo operation inside
  // the loop (because n/batchIdx now does not have to be computed through
  // modulo, you are just looping over it), and bring a nice speed-up.
  for (int batchIdx = batch; batchIdx < batchSize; batchIdx += nwarps) {
    ...
    for (IndexType idx = laneId; idx < imageElements; idx += C10_WARP_SIZE) {
      // Need to calculate the following: batch position, and offset into the
      // gradOutput in height, and width. We can intuit the corresponding
      // position in the input from the other parameters we have
  ...
  AccT tval = reduceBlock<AccT, ReduceAdd<AccT>>(
      ...
      grad,
      ReduceAdd<AccT>(),
      ScalarConvert<float, AccT>::to(0));

  // After reduction, first thread in the block has the gradient, so its
  // responsible for writing it to gradWeight
  if (threadIdx.x == 0) {
    int weightOffset =
