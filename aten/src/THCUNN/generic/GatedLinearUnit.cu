...
void THNN_(GatedLinear_updateOutput)(
  ...
  THArgCheck(
      nIn % 2 == 0,
      2,
      "Halving dimension must be even. Dim %d is size %ld",
      dim,
      nIn);
  ...
  THCTensor* secondHalf =
      THCTensor_(newNarrow)(state, input, dim, inputSize, inputSize);
  ...
  THC_pointwiseApply3<scalar_t, scalar_t, scalar_t>(
      ...
      secondHalf,
      firstHalf,
      gatedLinearCSigMul_functor<scalar_t, accreal>());
...
void THNN_(GatedLinear_updateGradInput)(
  ...
  const int64_t inputSize = THCTensor_(size)(state, input, dim) / 2;
  THCTensor* firstHalf = THCTensor_(newNarrow)(state, input, dim, 0, inputSize);
  THCTensor* gradInputfirstHalf =
      THCTensor_(newNarrow)(state, gradInput, dim, 0, inputSize);
  const int64_t stride_i = THCTensor_(stride)(state, input, dim) * inputSize;
  const int64_t stride_gI =
      THCTensor_(stride)(state, gradInput, dim) * inputSize;
  THC_pointwiseApply3<scalar_t, scalar_t, scalar_t>(
      state,
      gradInputfirstHalf,
      gradOutput,
      firstHalf,
      gatedLinearDerivative<scalar_t, accreal>(stride_i, stride_gI));
  THCTensor_(free)(state, firstHalf);
