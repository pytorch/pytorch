...
void THNN_(RReLU_updateOutput)(
  ...
  if (train) {
    ...
    uint64_t counter_offset =
        ((n - 1) / (BLOCK_SIZE * grid.x) + 1) * curand4_engine_calls;
    ...
    if (inplace) {
      THC_pointwiseApply1<scalar_t>(
          state, input, RReLUUpdateOutputEvalIP_functor<scalar_t>(negSlope));
      THCTensor_(set)(state, output, input);
    } else {
      THCTensor_(resizeAs)(state, output, input);
      THC_pointwiseApply2<scalar_t, scalar_t>(
          ...
          output,
          input,
          RReLUUpdateOutputEval_functor<scalar_t>(negSlope));
    }
  }
}
#endif
