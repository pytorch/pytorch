...
void THNN_(MultiMarginCriterion_updateOutput)(
  ...
  if (THTensor_nDimensionLegacyNoScalars(input) == 1) {
    ...
    THArgCheck(
        !target->is_empty() &&
            (THTensor_nDimensionLegacyNoScalars(target) == 1) &&
            (THTensor_sizeLegacyNoScalars(target, 0) == nframe),
        3,
    ...
    if (p == 1) {
      cunn_MultiMarginCriterion_updateOutput_kernel<1, scalar_t, accreal>
      ...
      cunn_MultiMarginCriterion_updateOutput_kernel<2, scalar_t, accreal>
          <<<blocks, threads, 0, c10::cuda::getCurrentCUDAStream()>>>(
              THCTensor_(data)(state, output),
              THCTensor_(data)(state, input),
              THCIndexTensor_(data)(state, target),
    ...
    if (reduction == at::Reduction::None) {
      ...
      THCTensor_(resize0d)(state, output);
      THCTensor* output_ =
          THCTensor_(newWithSize1d)(state, input->size(0)); // tmp output buffer
      if (p == 1) {
        cunn_MultiMarginCriterion_updateOutput_kernel<1, scalar_t, accreal>
            <<<blocks, threads, 0, c10::cuda::getCurrentCUDAStream()>>>(
...
void THNN_(MultiMarginCriterion_updateGradInput)(
  ...
  if (THTensor_nDimensionLegacyNoScalars(input) == 1) {
    ...
    if (p == 1) {
      cunn_MultiMarginCriterion_updateGradInput_kernel<1, scalar_t, accreal>
          <<<blocks, threads, 0, c10::cuda::getCurrentCUDAStream()>>>(
              ...
              weights ? THCTensor_(data)(state, weights) : NULL,
              1,
              THTensor_sizeLegacyNoScalars(gradInput, 0),
              reduction == at::Reduction::Mean,
              margin,
              reduction != at::Reduction::None);
      C10_CUDA_KERNEL_LAUNCH_CHECK();
    } else if (p == 2) {
      cunn_MultiMarginCriterion_updateGradInput_kernel<2, scalar_t, accreal>
          <<<blocks, threads, 0, c10::cuda::getCurrentCUDAStream()>>>(
              THCTensor_(data)(state, gradInput),
              THCTensor_(data)(state, gradOutput),
              THCTensor_(data)(state, input),
