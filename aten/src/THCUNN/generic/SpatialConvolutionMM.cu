...
static inline void THNN_(SpatialConvolutionMM_shapeCheck)(
  ...
  if (weight != NULL) {
    THCUNN_argCheck(
        state,
        !weight->is_empty() && (weight->dim() == 2 || weight->dim() == 4),
        5,
        weight,
  ...
  THCUNN_argCheck(
      state,
      (!input->is_empty() || valid_empty) && (ndim == 3 || ndim == 4),
      2,
      input,
  ...
  if (exactInputHeight < kH || exactInputWidth < kW) {
    THError(
        "Calculated padded input size per channel: (%ld x %ld). "
  ...
  if (outputWidth < 1 || outputHeight < 1) {
    THError(
        "Given input size per channel: (%ld x %ld). "
...
void THNN_(SpatialConvolutionMM_updateOutput)(
  ...
  THArgCheck(
      !bias || THCTensor_(isContiguous)(state, bias),
      5,
  ...
  if (input->dim() == 3) {
    ...
    is_batch = 0;
    THCTensor_(resize4d)(
        state, input, 1, input->size(0), input->size(1), input->size(2));
  }
  ...
  // Resize output
  THCTensor_(resize4d)(
      state, output, batchSize, nOutputPlane, outputHeight, outputWidth);

  // Resize temporary columns
  THCTensor_(resize2d)(
      state, columns, nInputPlane * kW * kH, outputHeight * outputWidth);
  ...
  if (is_batch == 0) {
    THCTensor_(resize3d)(
        state, output, nOutputPlane, outputHeight, outputWidth);
...
void THNN_(SpatialConvolutionMM_updateGradInput)(
  ...
  THCUNN_assertSameGPU(
      state, 5, input, gradOutput, weight, gradColumns, gradInput);
  weight = THNN_(newViewWeightMM2d)(state, weight);
  ...
  // Params
  int nInputPlane =
      weight->dim() == 2 ? weight->size(1) / (kW * kH) : weight->size(1);
  ...
  // Resize output
  THCTensor_(resize4d)(
      state, gradInput, batchSize, nInputPlane, inputHeight, inputWidth);

  // Resize temporary columns
  THCTensor_(resize2d)(
      state, gradColumns, nInputPlane * kW * kH, outputHeight * outputWidth);
  ...
  if (is_batch == 0) {
    THCTensor_(resize3d)(
        state, gradOutput, nOutputPlane, outputHeight, outputWidth);
    THCTensor_(resize3d)(state, input, nInputPlane, inputHeight, inputWidth);
    THCTensor_(resize3d)(
        state, gradInput, nInputPlane, inputHeight, inputWidth);
  }
...
void THNN_(SpatialConvolutionMM_accGradParameters)(
  ...
  THCUNN_assertSameGPU(
      state, 5, input, gradOutput, gradWeight, gradBias, columns, ones);
  if (gradWeight) {
    THArgCheck(
        THCTensor_(isContiguous)(state, gradWeight),
        4,
        "gradWeight needs to be contiguous");
    gradWeight = THNN_(newViewWeightMM2d)(state, gradWeight);
  }
  if (gradBias) {
    THArgCheck(
        THCTensor_(isContiguous)(state, gradBias),
        5,
        "gradBias needs to be contiguous");
    THArgCheck(
        THCTensor_(isContiguous)(state, ones),
        7,
  ...
  for (int elt = 0; elt < batchSize; elt++) {
    ...
    if (gradBias) {
      ...
      at::cuda::blas::gemv<scalar_t>(
          ...
          1,
          ScalarConvert<int, scalar_t>::to(1),
          THCTensor_(data)(state, gradBias),
          1);
    }
  }
