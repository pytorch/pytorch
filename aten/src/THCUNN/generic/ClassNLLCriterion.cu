...
void THNN_(ClassNLLCriterion_updateOutput)(
  ...
  int64_t batch_size =
      n_dims == 1 ? 1 : THCTensor_(sizeLegacyNoScalars)(state, input, 0);
  ...
  if (weights && THCTensor_(nElement)(state, weights) != n_classes) {
    ...
    THError(
        "weight tensor should be defined either for all %d classes or no classes"
        " but got weight tensor of shape: %s",
        n_classes,
        s1.str);
  }
