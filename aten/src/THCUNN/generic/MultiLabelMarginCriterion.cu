...
void THNN_(MultiLabelMarginCriterion_updateGradInput)(
  ...
  if (gradInput->dim() <= 1) {
    ...
    int target_size = target->dim() == 0 ? 1 : target->size(0);
    THArgCheck(
        !target->is_empty() && (target->dim() <= 1) && (target_size == dim),
        3,
        "inconsistent target size");
    TORCH_CHECK(
        target->sizes() == istarget->sizes(), "inconsistent isTarget size");
    dim3 blocks(1);
    dim3 threads(MULTILABELMARGIN_THREADS);
    ...
    THArgCheck(
        (input->size(1) != 0) && (target->dim() == 2) &&
            (target->size(0) == nframe) && (target->size(1) == dim),
        3,
        "inconsistent target size");
    THArgCheck(
        (istarget->dim() == 2) && (istarget->size(0) == nframe) &&
            (istarget->size(1) == dim),
        3,
