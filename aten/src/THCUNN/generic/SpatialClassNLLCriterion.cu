...
void THNN_(SpatialClassNLLCriterion_shapeCheck)(
  ...
  TORCH_CHECK(
      ...
      input->sizes());
  if (THCTensor_(size)(state, input, 0) !=
          THCIndexTensor_(size)(state, target, 0) ||
      THCTensor_(size)(state, input, 2) !=
          THCIndexTensor_(size)(state, target, 1) ||
      THCTensor_(size)(state, input, 3) !=
          THCIndexTensor_(size)(state, target, 2)) {
    THCDescBuff input_size = THCTensor_(sizeDesc)(state, input);
    THCDescBuff target_size = THCIndexTensor_(sizeDesc)(state, target);
    THError(
        "input and target batch or spatial sizes don't match: target %s, input %s",
        target_size.str,
        input_size.str);
  }

  if (weights &&
      THCTensor_(nElement)(state, weights) !=
...
static void THNN_(SpatialClassNLLCriterion_gradOutput_no_reduce_shapeCheck)(
    ...
    THCIndexTensor* target) {
  TORCH_CHECK(
      THCTensor_(nDimensionLegacyNoScalars)(state, gradOutput) == 3,
      2,
      "gradOutput must have same dimension as target (3) but got dimension: ",
      gradOutput->sizes());
  if (THCTensor_(size)(state, gradOutput, 0) !=
          THCIndexTensor_(size)(state, target, 0) ||
      THCTensor_(size)(state, gradOutput, 1) !=
          THCIndexTensor_(size)(state, target, 1) ||
      THCTensor_(size)(state, gradOutput, 2) !=
          THCIndexTensor_(size)(state, target, 2)) {
    THCDescBuff gradOutput_size = THCTensor_(sizeDesc)(state, gradOutput);
    THCDescBuff target_size = THCIndexTensor_(sizeDesc)(state, target);
    THError(
        "gradOutput sizes don't match target sizes: target %s, gradOutput %s",
        target_size.str,
        gradOutput_size.str);
  }
}
...
void THNN_(SpatialClassNLLCriterion_updateOutput)(
  ...
  at::globalContext().alertNotDeterministic(
      "SpatialClassNLLCriterion_updateOutput");
  ...
  if (batch_size != 0) { // This guards from unnecessary operations and
                         // launching CUDA kernel with 0 blocks.
    THCIndex_t map_nelem =
        THCIndexTensor_(nElement)(state, target) / batch_size;
  ...
  if (reduction == at::Reduction::Mean) {
    cunn_SpatialClassNLLCriterion_sizeAverage_kernel<<<
        1,
        1,
        0,
        c10::cuda::getCurrentCUDAStream()>>>(
