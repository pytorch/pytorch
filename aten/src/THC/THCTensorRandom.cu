...
{
  THCudaCheck(THCudaMalloc(state, (void**)&gen->gen_state.gen_states, MAX_NUM_BLOCKS * sizeof(curandStateMtgp32)));
  THCudaCheck(THCudaMalloc(state, (void**)&gen->gen_state.kernel_params, sizeof(mtgp32_kernel_params)));
}
...
{
  if (curandMakeMTGP32Constants(mtgp32dc_params_fast_11213, gen->gen_state.kernel_params) != CURAND_STATUS_SUCCESS)
  {
    THError("Creating MTGP constants failed.");
  }
  if (curandMakeMTGP32KernelState(gen->gen_state.gen_states, mtgp32dc_params_fast_11213,
                                  gen->gen_state.kernel_params, MAX_NUM_BLOCKS, seed) != CURAND_STATUS_SUCCESS)
  {
...
__host__ void THCRandom_getRNGState(THCState* state, THByteTensor *rng_state)
{
  ...
  static const size_t seed_size = sizeof(gen->gen_state.initial_seed);
  static const size_t offset_size = sizeof(gen->gen_state.philox_seed_offset);
  static const size_t total_size = states_size + seed_size + offset_size;
  THByteTensor_resize1d(rng_state, total_size);
  THArgCheck(THByteTensor_nElement(rng_state) == total_size, 1, "RNG state is wrong size");
  THArgCheck(THByteTensor_isContiguous(rng_state), 1, "RNG state must be contiguous");
  THCudaCheck(cudaMemcpy(THByteTensor_data(rng_state), gen->gen_state.gen_states,
                         states_size, cudaMemcpyDeviceToHost));
  memcpy(THByteTensor_data(rng_state) + states_size, &gen->gen_state.initial_seed, seed_size);
  memcpy(THByteTensor_data(rng_state) + states_size + seed_size, &gen->gen_state.philox_seed_offset, offset_size);
}
...
__host__ void THCRandom_setRNGState(THCState* state, THByteTensor *rng_state)
{
  ...
  THCudaCheck(cudaMemcpy(gen->gen_state.gen_states, THByteTensor_data(rng_state),
                         states_size, cudaMemcpyHostToDevice));
  set_rngstate_kernel<<<1, MAX_NUM_BLOCKS, 0, THCState_getCurrentStream(state)>>>(
      gen->gen_state.gen_states, gen->gen_state.kernel_params);
  memcpy(&gen->gen_state.initial_seed, THByteTensor_data(rng_state) + states_size, seed_size);
  if (!no_philox_seed) {
    memcpy(&gen->gen_state.philox_seed_offset, THByteTensor_data(rng_state) + states_size + seed_size, offset_size);
  }
  else {
    gen->gen_state.philox_seed_offset = 0;
  }
}
