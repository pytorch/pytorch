...
extern "C" __host__ void THCRandom_getRNGState(at::GeneratorHolder gen_, THByteTensor *rng_state)
{
...
extern "C" __host__ void THCRandom_setRNGState(at::GeneratorHolder gen_, THByteTensor *rng_state)
{
  auto gen = at::check_generator<at::CUDAGenerator>(gen_);
  std::lock_guard<std::mutex> lock(gen->mutex_);
