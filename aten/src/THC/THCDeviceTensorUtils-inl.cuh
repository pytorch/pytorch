...
struct UpcastTHCRoot {
  static THCDeviceTensor<T, NewDim, IndexT, PtrTraits> make(
      THCState* state,
      THCudaTensor* t);
};
...
template <
    ...
    int NewDim,
    bool B>
struct UpcastTHC : UpcastTHCRoot<T, Dim, IndexT, PtrTraits, NewDim, B> {};
...
template <
    ...
    int NewDim,
    bool B>
struct DowncastTHC : DowncastTHCRoot<T, Dim, IndexT, PtrTraits, NewDim, B> {};
...
#define SWITCH_UNROLL_CUDA_CAST_FACTORY(i)                                  \
  case i:                                                                   \
    if (NewDim > i) {                                                       \
      return detail::                                                       \
          UpcastTHC<T, i, IndexT, PtrTraits, NewDim, (NewDim > i)>::make(   \
              state, t);                                                    \
    } else if (NewDim == i) {                                               \
      return toDeviceTensor<T, NewDim, IndexT, PtrTraits>(state, t);        \
    } else {                                                                \
      return detail::                                                       \
          DowncastTHC<T, i, IndexT, PtrTraits, NewDim, (NewDim < i)>::make( \
              state, t);                                                    \
    }                                                                       \
    /* break; */
...
template <
    ...
    template <typename U>
    class PtrTraits>
THCDeviceTensor<T, NewDim, IndexT, PtrTraits> toDeviceTensorCast(
    THCState* state,
    THCudaTensor* t) {
