...
THC_API void THCTensor_(bgetri)(THCState *state, THCTensor *ra_, THCTensor *a)
{
#if defined(THC_REAL_IS_FLOAT) || defined(THC_REAL_IS_DOUBLE)
  THAssert(THCTensor_(checkGPU)(state, 2, ra_, a));
  THArgCheck(THCTensor_(_nDimension)(state, a) == 3, 3, "expected 3D tensor");
  ...
  THCTensor *result = THCTensor_(newClone)(state, ra_);

  int64_t num_batches = ra__->size[0];

  THCudaIntTensor *info = THCudaIntTensor_new(state);
  THCudaIntTensor_resize1d(state, info, num_batches);
  int *info_gpu = THCudaIntTensor_data(state, info);
  int *ipiv_gpu;
  THCudaCheck(THCudaMalloc(state, (void**)&ipiv_gpu, num_batches * n * sizeof(int)));

  // Copy pointers to device.
  real **d_matrix1;
  real **d_matrix2;
  size_t matrices_size = num_batches * sizeof(real*);
  THCudaCheck(THCudaMalloc(state, (void**)&d_matrix1, matrices_size));
  THCudaCheck(THCudaMalloc(state, (void**)&d_matrix2, matrices_size));

  const int64_t block = 512;
  const int64_t grid = (num_batches + block - 1) / block;
  createBatchGemmBuffer<<<grid, block, 0, THCState_getCurrentStream(state)>>>(
    (const real**)d_matrix1, THCTensor_(data)(state, ra__),
    ra__->stride[0], num_batches);
  createBatchGemmBuffer<<<grid, block, 0, THCState_getCurrentStream(state)>>>(
    (const real**)d_matrix2, THCTensor_(data)(state, result),
    result->stride[0], num_batches);

  // LU factorization
#ifdef THC_REAL_IS_FLOAT
  THCudaBlas_Sgetrf(state, n, d_matrix1, lda, ipiv_gpu, info_gpu, num_batches);
#elif defined(THC_REAL_IS_DOUBLE)
  THCudaBlas_Dgetrf(state, n, d_matrix1, lda, ipiv_gpu, info_gpu, num_batches);
#endif

  // Did the factorization succeed?
  int min = THCudaIntTensor_minall(state, info);
  int max = THCudaIntTensor_maxall(state, info);
  ...
  THCudaBlas_Sgetri(state, n, (const real**)d_matrix1, lda, ipiv_gpu,
                    d_matrix2, n, info_gpu, num_batches);
#else
  THCudaBlas_Dgetri(state, n, (const real**)d_matrix1, lda, ipiv_gpu,
                    d_matrix2, n, info_gpu, num_batches);
#endif

  THCudaCheck(THCudaFree(state, d_matrix1));
  THCudaCheck(THCudaFree(state, d_matrix2));
  THCudaCheck(THCudaFree(state, ipiv_gpu));

  THCTensor_(freeCopyTo)(state, result, ra__);
  ...
  // Was it successful?
  min = (int)THCudaIntTensor_minall(state, info);
  max = (int)THCudaIntTensor_maxall(state, info);
  THCudaIntTensor_free(state, info);
  if (min != 0 || max != 0) {
    THError("failed to invert some batch elements (min info == %d, max info == %d)",
            min, max);
  }
#else
  THError("bgetri is only supported for floats and doubles");
#endif
}
