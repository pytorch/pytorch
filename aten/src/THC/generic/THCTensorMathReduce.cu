...
void THCTensor_(renorm)(
  ...
  THArgCheck(
      dimension >= 0 &&
          dimension < THCTensor_(nDimensionLegacyNoScalars)(state, src),
      3,
      "invalid dimension");
  THArgCheck(
      THCNumerics<scalar_t>::gt(value, scalar_cast<scalar_t>(0)),
      2,
      "non-positive-norm not supported");
  THArgCheck(
      THCTensor_(nDimensionLegacyNoScalars)(state, src) > 1,
      1,
  ...
  if (numel > 0) {
    ...
    // NOTE: only with this specific number of threads can this work on GPUs
    // with a warp size != 32 (such as AMD). Do not alter w/o changing buffer
    // size in kernel.
    dim3 threads(32);
    ...
    THCTensor_kernel_renorm<scalar_t, accreal>
        <<<grid, threads, 0, c10::cuda::getCurrentCUDAStream()>>>(
            ...
            scalar_cast<accreal>(value),
            size,
            scalar_cast<accreal>(maxnorm));
