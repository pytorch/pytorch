...
{
  THCudaCheck(cudaMemcpyAsync(THCStorage_(data)(state, self), src, self->size_ * sizeof(real), cudaMemcpyDeviceToDevice, THCState_getCurrentStream(state)));
}
...
{                                                                                       \
  ...
  THCTensor* selfTensor = THCTensor_(newWithStorage1d)(state, self, 0, self->size_, 1);  \
  struct THCuda##TYPECUDA##Tensor* srcTensor =                                          \
      THCuda##TYPECUDA##Tensor_newWithStorage1d(state, src, 0, src->size_, 1);           \
