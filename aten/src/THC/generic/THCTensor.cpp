...
{
  return c10::make_intrusive<at::TensorImpl, at::UndefinedTensorImpl>(at::Storage(THCStorage_(new)(state)), at::CUDATensorId(), false).release();
}

/* Pointer-copy init */
THCTensor *THCTensor_(newWithTensor)(THCState *state, THCTensor *tensor)
{
  THCTensor *self = c10::make_intrusive<at::TensorImpl, at::UndefinedTensorImpl>(at::Storage(THCStorage_(new)(state)), at::CUDATensorId(), false).release();
