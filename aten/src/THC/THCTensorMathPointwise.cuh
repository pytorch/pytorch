...
struct TensorDigammaOp {
  __device__ __forceinline__ void
  operator()(scalar_t* out, scalar_t* in) {
    using compute_type = typename std::conditional<std::is_same<scalar_t, half>::value, accreal, scalar_t>::type;
    ...
    if (x == 0) {
      *out = scalar_cast<scalar_t>(INFINITY);
      return;
    }
    ...
    if (x == 10) {
      *out = scalar_cast<scalar_t>(result + PSI_10);
      return;
    }
    ...
    *out = scalar_cast<scalar_t>(log(x) - (0.5 / x) - y + result);
    return;
  }
};
...
struct TensorTrigammaOp {
  ...
  operator()(scalar_t* out, scalar_t* in) {
    ...
    compute_type x = ScalarConvert<scalar_t, compute_type>::to(*in);
    compute_type sign = +1;
    compute_type result = 0;
    if (x < 0.5f) {
      sign = -1;
    ...
    *out = ScalarConvert<compute_type, scalar_t>::to(sign * result);
  }
};
