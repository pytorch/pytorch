...
TH_API THDescBuff THLongStorage_sizeDesc(const at::LongStorageImpl *size);
TH_API at::LongStorageImpl *THLongStorage_newInferSize(at::LongStorageImpl *size, ptrdiff_t nElement);

// Given the sizes of {2,N} tensors, write out the size when the tensors are expanded together.
TH_API int THLongStorage_inferSize2(at::LongStorageImpl *output, int64_t *sizesA, int64_t dimsA,
                                    int64_t *sizesB, int64_t dimsB, char *error_buffer, int buffer_len);
TH_API int THLongStorage_inferSizeN(at::LongStorageImpl *output, int n, int64_t **sizes, int64_t *dims,
