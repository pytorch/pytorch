...
static void THMemoryFile_grow(THMemoryFile *self, ssize_t size)
{
  ...
  else
  {
    ...
    {
      self->size = size;
      self->storage->data<int8_t>()[self->size] = '\0';
      return;
    }
  }
...
#define READ_WRITE_METHODS(TYPE, TYPEC, ASCII_READ_ELEM, ASCII_WRITE_ELEM, INSIDE_SPACING) \
  ...
  {                                                                     \
    ...
    {                                                                   \
      ...
      memmove(data, mfself->storage->data<int8_t>()+mfself->position, nread*sizeof(TYPE)); \
    ...
    {                                                                   \
      ...
      {                                                                 \
        ssize_t nByteRead = 0;                                             \
        int8_t spaceChar = 0;                                             \
        int8_t *spacePtr = THMemoryFile_strnextspace(mfself->storage->data<int8_t>()+mfself->position, &spaceChar); \
        ASCII_READ_ELEM;                                                \
        if(ret == EOF)                                                  \
        {                                                               \
          while(mfself->storage->data<int8_t>()[mfself->position])                \
            mfself->position++;                                         \
        }                                                               \
        else                                                            \
      ...
      {                                                                 \
        if( (mfself->position < mfself->size) && (mfself->storage->data<int8_t>()[mfself->position] == '\n') ) \
          mfself->position++;                                           \
      }                                                                 \
    }                                                                   \
                                                                        \
  ...
  {                                                                     \
    ...
    {                                                                   \
      ...
      memmove(mfself->storage->data<int8_t>()+mfself->position, data, nByte);     \
      mfself->position += nByte;                                        \
      if(mfself->position > mfself->size)                               \
      {                                                                 \
        mfself->size = mfself->position;                                \
        mfself->storage->data<int8_t>()[mfself->size] = '\0';                     \
      }                                                                 \
    }                                                                   \
    else                                                                \
    {                                                                   \
      ...
      {                                                                 \
        ...
        {                                                               \
          ...
          {                                                             \
            ...
            sprintf((char*) mfself->storage->data<int8_t>()+mfself->position, " "); \
            mfself->position++;                                         \
          }                                                             \
          if(i == n-1)                                                  \
          {                                                             \
...
at::CharStorageImpl *THMemoryFile_storage(THFile *self)
{
  THMemoryFile *mfself = (THMemoryFile*)self;
  THArgCheck(mfself->storage != NULL, 1, "attempt to use a closed file");
...
READ_WRITE_METHODS(uint8_t, Byte,
                   ...
                   nread = ret; \
                   i = n-1; \
                   memmove(data, mfself->storage->data<int8_t>()+mfself->position, nByteRead),
                   nByteWritten = (n < mfself->storage->size()-mfself->position ? n : -1); \
                   i = n-1; \
                   if(nByteWritten > -1)
                     memmove(mfself->storage->data<int8_t>()+mfself->position, data, nByteWritten),
                   0)
...
READ_WRITE_METHODS(int16_t, Short,
                   int nByteRead_; int ret = sscanf((char*) mfself->storage->data<int8_t>()+mfself->position, "%hd%n", &data[i], &nByteRead_); nByteRead = nByteRead_; if(ret <= 0) break; else nread++,
                   nByteWritten = snprintf((char*) mfself->storage->data<int8_t>()+mfself->position, mfself->storage->size()-mfself->position, "%hd", data[i]),
                   1)

READ_WRITE_METHODS(int32_t, Int,
                   int nByteRead_; int ret = sscanf((char*) mfself->storage->data<int8_t>()+mfself->position, "%d%n", &data[i], &nByteRead_); nByteRead = nByteRead_; if(ret <= 0) break; else nread++,
                   nByteWritten = snprintf((char*) mfself->storage->data<int8_t>()+mfself->position, mfself->storage->size()-mfself->position, "%d", data[i]),
                   1)

READ_WRITE_METHODS(float, Float,
                   int nByteRead_; int ret = sscanf((char*) mfself->storage->data<int8_t>()+mfself->position, "%g%n", &data[i], &nByteRead_); nByteRead = nByteRead_; if(ret <= 0) break; else nread++,
                   nByteWritten = snprintf((char*) mfself->storage->data<int8_t>()+mfself->position, mfself->storage->size()-mfself->position, "%.9g", data[i]),
                   1)

READ_WRITE_METHODS(THHalf, Half,
                   int nByteRead_; float buf; \
                   int ret = sscanf((char*) mfself->storage->data<int8_t>()+mfself->position, "%g%n", &buf, &nByteRead_); \
                   data[i] = TH_float2half(buf); nByteRead = nByteRead_; if(ret <= 0) break; else nread++,
                   nByteWritten = snprintf((char*) mfself->storage->data<int8_t>()+mfself->position, mfself->storage->size()-mfself->position, "%.9g", TH_half2float(data[i])),
                   1)

READ_WRITE_METHODS(double, Double,
                   int nByteRead_; int ret = sscanf((char*) mfself->storage->data<int8_t>()+mfself->position, "%lg%n", &data[i], &nByteRead_); nByteRead = nByteRead_; if(ret <= 0) break; else nread++,
                   nByteWritten = snprintf((char*) mfself->storage->data<int8_t>()+mfself->position, mfself->storage->size()-mfself->position, "%.17g", data[i]),
                   1)
...
static ssize_t THMemoryFile_readLong(THFile *self, int64_t *data, ssize_t n)
{
  ...
  if(mfself->file.isBinary)
  {
    if(mfself->longSize == 0 || mfself->longSize == sizeof(int64_t))
    {
      ...
      memmove(data, mfself->storage->data<int8_t>()+mfself->position, nread*sizeof(int64_t));
      mfself->position += nread*sizeof(int64_t);
    } else if(mfself->longSize == 4)
    {
      ssize_t nByte = 4*n;
      ssize_t nByteRemaining = (mfself->position + nByte <= mfself->size ? nByte : mfself->size-mfself->position);
      int32_t *storage = (int32_t *)(mfself->storage->data<int8_t>() + mfself->position);
  ...
  else
  {
    ...
    for(i = 0; i < n; i++)
    {
      ...
      int8_t spaceChar = 0;
      int8_t *spacePtr = THMemoryFile_strnextspace(mfself->storage->data<int8_t>()+mfself->position, &spaceChar);
      int nByteRead_; int ret = sscanf((char*) mfself->storage->data<int8_t>()+mfself->position, "%" PRId64 "%n", &data[i], &nByteRead_); nByteRead = nByteRead_; if(ret <= 0) break; else nread++;
      if(ret == EOF)
      {
        while(mfself->storage->data<int8_t>()[mfself->position])
          mfself->position++;
...
static ssize_t THMemoryFile_writeLong(THFile *self, int64_t *data, ssize_t n)
{
  ...
  else
  {
    ...
    for(i = 0; i < n; i++)
    {
      ...
      while (1)
      {
        nByteWritten = snprintf((char*) mfself->storage->data<int8_t>()+mfself->position, mfself->storage->size()-mfself->position, "%" PRId64, data[i]);
        if( (nByteWritten > -1) && (nByteWritten < mfself->storage->size()-mfself->position) )
        {
          mfself->position += nByteWritten;
          break;
...
static ssize_t THMemoryFile_readString(THFile *self, const char *format, char **str_)
{
  ...
  if(format[1] == 'a')
  {
    ...
    *str_ = (char*) THMemoryFile_cloneString(mfself->storage->data<int8_t>()+mfself->position, str_size);
    mfself->position = mfself->size;
  ...
  {
    int8_t *p = mfself->storage->data<int8_t>()+mfself->position;
    int eolFound = 0;
    ssize_t posEol;
    ssize_t i;
    ...
    if(eolFound)
    {
      *str_ = (char*) THMemoryFile_cloneString(mfself->storage->data<int8_t>()+mfself->position, posEol);
...
static ssize_t THMemoryFile_writeString(THFile *self, const char *str, ssize_t size)
{
  ...
  memmove(mfself->storage->data<int8_t>()+mfself->position, str, size);
...
THFile *THMemoryFile_newWithStorage(at::CharStorageImpl *storage, const char *mode)
{
  ...
  if(storage)
  {
    THArgCheck(storage->data<int8_t>()[storage->size()-1] == '\0', 1, "provided CharStorage must be terminated by 0");
