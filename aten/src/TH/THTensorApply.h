...
#define __TH_TENSOR_APPLYX_PREAMBLE(TYPE, TENSOR, DIM, ALLOW_CONTIGUOUS)      \
  ...
  int64_t *TENSOR##_counter = NULL, *TENSOR##_sizes = NULL,                   \
          *TENSOR##_strides = NULL, *TENSOR##_dimOffset = NULL;               \
  int64_t TENSOR##_stride = 0, TENSOR##_size = 0, TENSOR##_dim = 0,           \
          TENSOR##_i, TENSOR##_n;                                             \
  ...
  if (TENSOR->is_empty())                                                     \
    ...
    TENSOR##_data = THTensor_getStoragePtr(TENSOR)->data<TYPE>() +            \
        TENSOR->storage_offset();                                             \
    TENSOR##_size = 1;                                                        \
    TENSOR##_stride = 1;                                                      \
    for (TENSOR##_i = THTensor_nDimensionLegacyAll(TENSOR) - 1;               \
         TENSOR##_i >= 0;                                                     \
         TENSOR##_i--) {                                                      \
      if (THTensor_sizeLegacyNoScalars(TENSOR, TENSOR##_i) != 1) {            \
        if (THTensor_strideLegacyNoScalars(TENSOR, TENSOR##_i) ==             \
                TENSOR##_size &&                                              \
            TENSOR##_i != DIM)                                                \
    ...
    if (!TENSOR##_contiguous) {                                               \
      ...
      TENSOR##_dim = 1;                                                       \
      for (TENSOR##_i = THTensor_nDimensionLegacyAll(TENSOR) - 2;             \
           TENSOR##_i >= 0;                                                   \
           TENSOR##_i--) {                                                    \
        if (TENSOR->stride(TENSOR##_i) !=                                     \
                TENSOR->stride(TENSOR##_i + 1) *                              \
                    TENSOR->size(TENSOR##_i + 1) ||                           \
            TENSOR##_i == DIM || TENSOR##_i + 1 == DIM)                       \
          TENSOR##_dim++;                                                     \
      }                                                                       \
      /* Allocate an array of 3*dim elements, where dim is the number of      \
       * contiguous sections */                                               \
      TENSOR##_counter =                                                      \
          (int64_t*)THAlloc(sizeof(int64_t) * (3 * TENSOR##_dim));            \
      TENSOR##_sizes = TENSOR##_counter + TENSOR##_dim;                       \
      TENSOR##_strides = TENSOR##_counter + 2 * TENSOR##_dim;                 \
      TH_TENSOR_dim_index = TENSOR##_dim - 1;                                 \
      TENSOR##_dimOffset = (DIM == THTensor_nDimensionLegacyAll(TENSOR) - 1)  \
          ? &TENSOR##_i                                                       \
          : &TENSOR##_counter[DIM];                                           \
      TENSOR##_sizes[TH_TENSOR_dim_index] = THTensor_sizeLegacyNoScalars(     \
          TENSOR, THTensor_nDimensionLegacyAll(TENSOR) - 1);                  \
      TENSOR##_strides[TH_TENSOR_dim_index] = THTensor_strideLegacyNoScalars( \
          TENSOR, THTensor_nDimensionLegacyAll(TENSOR) - 1);                  \
      /* TENSOR##_counter tracks where we are in the storage. The offset into \
       * the */                                                               \
      /* storage is given by storage_offset + (i * j), where i is the stride  \
       */                                                                     \
      /* vector and j is tensor_counter vector. This sets the starting        \
      ...
      for (TENSOR##_i = THTensor_nDimensionLegacyAll(TENSOR) - 2;             \
        ...
        if (TENSOR->stride(TENSOR##_i) ==                                     \
                TENSOR->stride(TENSOR##_i + 1) *                              \
                    TENSOR->size(TENSOR##_i + 1) &&                           \
            TENSOR##_i != DIM && TENSOR##_i + 1 != DIM) {                     \
          TENSOR##_sizes[TH_TENSOR_dim_index] =                               \
              TENSOR->size(TENSOR##_i) * TENSOR##_sizes[TH_TENSOR_dim_index]; \
          if (DIM != THTensor_nDimensionLegacyAll(TENSOR) - 1 &&              \
              TENSOR##_i < DIM)                                               \
            TENSOR##_dimOffset--;                                             \
        } else {                                                              \
          --TH_TENSOR_dim_index;                                              \
    ...
    for (TENSOR##_i = TENSOR##_dim - 2; TENSOR##_i >= 0; TENSOR##_i--) {     \
      ...
      if (TENSOR##_counter[TENSOR##_i] == TENSOR##_sizes[TENSOR##_i]) {      \
        if (TENSOR##_i == 0) {                                               \
           ...
           * dimension */                                                    \
          TENSOR##_data -=                                                   \
              TENSOR##_counter[TENSOR##_i] * TENSOR##_strides[TENSOR##_i];   \
          TENSOR##_counter[TENSOR##_i] = 0;                                  \
        }                                                                    \
      } else                                                                 \
        break;                                                               \
...
#define TH_TENSOR_APPLY3_D(                                                  \
    TYPE1, TENSOR1, TYPE2, TENSOR2, TYPE3, TENSOR3, DIM, CODE)               \
  {                                                                          \
    ...
    while (!TH_TENSOR_APPLY_hasFinished) {                                   \
      ...
      for (; TENSOR1##_i < TENSOR1##_size && TENSOR2##_i < TENSOR2##_size && \
           TENSOR3##_i < TENSOR3##_size;                                     \
           TENSOR1##_i++,                                                    \
           TENSOR2##_i++,                                                    \
           TENSOR3##_i++,                                                    \
           TENSOR1##_data += TENSOR1##_stride,                               \
           TENSOR2##_data += TENSOR2##_stride,                               \
           TENSOR3##_data += TENSOR3##_stride) /* 0 et pas TENSOR##_dim! */  \
      {                                                                      \
        CODE                                                                 \
      }                                                                      \
  ...
  {                                                                         \
    ...
    while (!TH_TENSOR_APPLY_hasFinished) {                                  \
      ...
      for (; TENSOR1##_i < TENSOR1##_size && TENSOR2##_i < TENSOR2##_size;  \
           ...
           TENSOR2##_i++,                                                   \
           TENSOR1##_data += TENSOR1##_stride,                              \
           TENSOR2##_data += TENSOR2##_stride) /* 0 et pas TENSOR##_dim! */ \
      {                                                                     \
        CODE                                                                \
      }                                                                     \
      __TH_TENSOR_APPLYX_UPDATE_COUNTERS(TENSOR1, 0)                        \
...
#define TH_TENSOR_APPLY_D(TYPE, TENSOR, DIM, CODE)                        \
  {                                                                       \
    ...
    while (!TH_TENSOR_APPLY_hasFinished) {                                \
      /* Loop through the inner most region of the Tensor */              \
      for (; TENSOR##_i < TENSOR##_size;                                  \
           TENSOR##_i++,                                                  \
           TENSOR##_data += TENSOR##_stride) /* 0 et pas TENSOR##_dim! */ \
      {                                                                   \
        CODE                                                              \
      }                                                                   \
 ...
 *
 * 1. convert the line index(the index of the element) to the
 * indexs(coordinates) in the tensor. It can hinted by a classical problem:
 * Getting each individual digit from a whole integer(Decimal base). A N-digit
 * decimal base number could be view as a N-dimension tensor and the sizes of
 * the tensor are 10. So the value the whole integer is the line index. And the
 * digits could be viewed as the indexes in different dimensions.
 *
 ...
 *
 * The macro is only used in the first element in each thread. For the rest, the
 * memory offset could update according to info of the tensor in order to get
 * better performance. So we should also record the each indexs in coresponding
 * dimension of first element. The recorded info is stored in the
 * TENSOR##_counter_tmp.
 *
 */
#define __TH_TENSOR_APPLYX_CAL_MEMORY_OFFSET(TENSOR)                     \
  int64_t* TENSOR##_counter_tmp =                                        \
      (int64_t*)THAlloc(sizeof(int64_t) * TENSOR##_dim);                 \
  ...
  for (TENSOR##_i = TENSOR##_dim - 1; TENSOR##_i >= 0; --TENSOR##_i) {   \
    ...
    TENSOR##_quot /= TENSOR##_sizes[TENSOR##_i];                         \
    TENSOR##_memory_offset +=                                            \
        TENSOR##_counter_tmp[TENSOR##_i] * TENSOR##_strides[TENSOR##_i]; \
  }

/*
 * The macro update the indexes in each dimension of the elements except for the
 * first one allocated in each thread. For a tensor, if the index of some
 * dimension reaches the size of the corresponding dimension. It will carry and
 * clear. If the index of next high dimension does do, the index of next high
 * dimension should carry and clear, too.
 *
 * The momery offset calculatation is a little confusing. If current index
 * carries, the current index is set to 0. So the offset should decrease by
 * size*stride of the last dimension. Then the index next high dimension
 * increases by 1. So the offset should increase by stride of next high
 * dimension.
 */
#define __TH_TENSOR_APPLYX_UPDATE_COUNTERS_PARALLEL(TENSOR)                    \
  if (TENSOR##_i == TENSOR##_size && TENSOR##_dim > 1) { /*reaches the edge*/  \
    ...
    TENSOR##_data -=                                                           \
        TENSOR##_size * TENSOR##_stride; /*the momery offset reset to the      \
                                            first one in current dimension  */ \
    for (TENSOR##_i = TENSOR##_dim - 2;                                        \
         (TENSOR##_i >= 0) && (TENSOR##_carry_coord);                          \
         TENSOR##_i--) {                                                       \
      TENSOR##_counter_tmp[TENSOR##_i]++; /*the index of next high dimension   \
                                             update*/                          \
      TENSOR##_data +=                                                         \
          TENSOR##_strides[TENSOR##_i]; /*memory offset increase by stride of  \
                                           next high dimension*/               \
