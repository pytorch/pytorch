...
void THTensor_(bgetri)(THTensor *ra_, THTensor *a)
{
  ...
  int n = a->size[2];
  if (m != n) {
    THError("bgetri is only implemented for square matrices");
  }
  ...
  THIntTensor *ipiv = THIntTensor_newWithSize1d((int64_t)m);
  ...
  for (; batch < num_batches; ++batch) {
    ...
    // Factorize
    THLapack_(getrf)(n, n, THTensor_(data)(rai), lda, THIntTensor_data(ipiv), &info);
    if (info != 0) {
      break;
    }

    // Invert
    THLapack_(getri)(n, THTensor_(data)(rai), lda, THIntTensor_data(ipiv), &wkopt, -1, &info);
    lwork = (int) wkopt;
    work = THTensor_(newWithSize1d)(lwork);
  ...
  if (info != 0) {
    THError("failed to invert batch element %ld (info == %d)", batch, info);
  }
}
