...
{
  if(size && stride) {
    THArgCheck(size->size() == stride->size_, 4, "inconsistent size");
  }
...
void THTensor_(resize)(THTensor *self, THLongStorage *size, THLongStorage *stride)
{
  ...
  if(stride)
    THArgCheck(stride->size_ == size->size(), 3, "invalid stride");
...
{
  if(size_ && stride_)
    THArgCheck(size_->size_ == stride_->size_, 5, "inconsistent size/stride sizes");
