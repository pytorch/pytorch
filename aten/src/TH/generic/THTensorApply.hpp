...
#define TH_TENSOR_DIM_APPLY3_SIZE_SCATTER(                                  \
    TENSOR1, TENSOR2, TENSOR3, DIMENSION)                                   \
  {                                                                         \
    int shape_check_flag = 0;                                               \
    for (TH_TENSOR_DIM_APPLY_i = 0;                                         \
         TH_TENSOR_DIM_APPLY_i < THTensor_nDimensionLegacyAll(TENSOR2);     \
         TH_TENSOR_DIM_APPLY_i++) {                                         \
      int64_t TENSOR1##_dim_size =                                          \
          THTensor_sizeLegacyNoScalars(TENSOR1, TH_TENSOR_DIM_APPLY_i);     \
      if (TH_TENSOR_DIM_APPLY_i != DIMENSION) {                             \
        if (TENSOR1##_dim_size >                                            \
            THTensor_sizeLegacyNoScalars(TENSOR2, TH_TENSOR_DIM_APPLY_i)) { \
          shape_check_flag = 1;                                             \
          break;                                                            \
        }                                                                   \
      }                                                                     \
      if (TENSOR1##_dim_size >                                              \
          THTensor_sizeLegacyNoScalars(TENSOR3, TH_TENSOR_DIM_APPLY_i)) {   \
        shape_check_flag = 1;                                               \
        break;                                                              \
      }                                                                     \
    }                                                                       \
    if (shape_check_flag == 1) {                                            \
      AT_ERROR(                                                             \
          ...
          " ",                                                              \
          TENSOR1->sizes(),                                                 \
          " to be smaller size than ",                                      \
          #TENSOR3,                                                         \
          " ",                                                              \
          TENSOR3->sizes(),                                                 \
          " and to be smaller than ",                                       \
          #TENSOR2,                                                         \
          " ",                                                              \
          TENSOR2->sizes(),                                                 \
...
#define th_isnan(val) (std::isnan(val))
#else
...
#define TH_TENSOR_APPLY2_PARALLEL(                                             \
    SIZE, CONTIG1, CONTIG2, TYPE1, TENSOR1, TYPE2, TENSOR2, CODE, THRESHOLD)   \
  {                                                                            \
    /* for advanced searching index*/                                          \
    if (CONTIG1 && CONTIG2) {                                                  \
      TYPE1* rp = THTensor_getStoragePtr(TENSOR1)->data<TYPE1>() +             \
          TENSOR1->storage_offset();                                           \
      TYPE2* tp = THTensor_getStoragePtr(TENSOR2)->data<TYPE2>() +             \
          TENSOR2->storage_offset();                                           \
      if (tp != (TYPE2*)rp) {                                                  \
        at::parallel_for(                                                      \
            0, SIZE, (THRESHOLD * 10), [&](int64_t begin, int64_t end) {       \
              PRAGMA_IVDEP                                                     \
       ...
       * 1. Collapse the dimension of the tensors in order to decrease the     \
      ...
      if (0 == TH_TENSOR_APPLY_hasFinished) {                                  \
        ...
        at::parallel_for(0, SIZE, THRESHOLD, [&](int64_t begin, int64_t end) { \
          ...
          ptrdiff_t count = 0;                                                 \
          ptrdiff_t TENSOR2##_start =                                          \
              TENSOR2##_counter_tmp[TENSOR2##_dim - 1];                        \
          ptrdiff_t TENSOR1##_start =                                          \
              TENSOR1##_counter_tmp[TENSOR1##_dim - 1];                        \
          /* step 4*/                                                          \
          while (count < line_seg_length) {                                    \
            for (TENSOR2##_i = TENSOR2##_start, TENSOR1##_i = TENSOR1##_start; \
                 ((count < line_seg_length) &&                                 \
                  (TENSOR2##_i < TENSOR2##_size) &&                            \
                  (TENSOR1##_i < TENSOR1##_size));                             \
                 ++TENSOR2##_i, ++TENSOR1##_i, ++count) {                      \
              CODE TENSOR2##_data += TENSOR2##_stride;                         \
...
#define TH_TENSOR_APPLY3_PARALLEL(                                             \
    SIZE,                                                                      \
    CONTIG1,                                                                   \
    CONTIG2,                                                                   \
    CONTIG3,                                                                   \
  ...
  {                                                                            \
    ...
    if (CONTIG1 && CONTIG2 && CONTIG3) {                                       \
      ...
      TYPE2* tp = THTensor_getStoragePtr(TENSOR2)->data<TYPE2>() +             \
          TENSOR2->storage_offset();                                           \
      TYPE3* srcp = THTensor_getStoragePtr(TENSOR3)->data<TYPE3>() +           \
          TENSOR3->storage_offset();                                           \
      if (tp != (TYPE2*)rp) {                                                  \
        at::parallel_for(                                                      \
            0, SIZE, (THRESHOLD * 10), [&](int64_t begin, int64_t end) {       \
              PRAGMA_IVDEP                                                     \
      ...
      if (0 == TH_TENSOR_APPLY_hasFinished) {                                  \
        ...
        at::parallel_for(0, SIZE, THRESHOLD, [&](int64_t begin, int64_t end) { \
          ...
          ptrdiff_t TENSOR2##_start =                                          \
              TENSOR2##_counter_tmp[TENSOR2##_dim - 1];                        \
          ptrdiff_t TENSOR3##_start =                                          \
              TENSOR3##_counter_tmp[TENSOR3##_dim - 1];                        \
          while (count < line_seg_length) {                                    \
            for (TENSOR1##_i = TENSOR1##_start,                                \
                TENSOR2##_i = TENSOR2##_start,                                 \
                TENSOR3##_i = TENSOR3##_start;                                 \
                 (count < line_seg_length) &&                                  \
                 (TENSOR1##_i < TENSOR1##_size) &&                             \
                 (TENSOR2##_i < TENSOR2##_size) &&                             \
                 (TENSOR3##_i < TENSOR3##_size);                               \
                 ++TENSOR1##_i, ++TENSOR2##_i, ++TENSOR3##_i, ++count) {       \
              CODE TENSOR1##_data += TENSOR1##_stride;                         \
...
#define TH_TENSOR_APPLY_REDUCTION_SUM_PARALLEL(                                \
    TYPE, TENSOR, EXPR, OUTPUT, THRESHOLD)                                     \
  {                                                                            \
    int TENSOR##Contig = THTensor_(isContiguous)(TENSOR);                      \
    ptrdiff_t TENSOR##Size = THTensor_(nElement)(TENSOR);                      \
    if (TENSOR##Contig) {                                                      \
      TYPE* rp = THTensor_getStoragePtr(TENSOR)->data<TYPE>() +                \
          TENSOR->storage_offset();                                            \
      OUTPUT = at::parallel_reduce(                                            \
          0,                                                                   \
          TENSOR##Size,                                                        \
          (THRESHOLD * 10),                                                    \
          (accreal)0,                                                          \
          [&](int64_t begin, int64_t end, accreal ident) -> accreal {          \
            accreal r = ident;                                                 \
            for (auto iter = begin; iter < end; iter++) {                      \
      ...
      if (0 == TH_TENSOR_APPLY_hasFinished) {                                  \
        ...
        OUTPUT = at::parallel_reduce(                                          \
            ...
            [&](int64_t begin, int64_t end, accreal ident) -> accreal {        \
              ...
              ptrdiff_t count = 0;                                             \
              ptrdiff_t TENSOR##_start =                                       \
                  TENSOR##_counter_tmp[TENSOR##_dim - 1];                      \
              accreal r = ident;                                               \
              while (count < line_seg_length) {                                \
                for (TENSOR##_i = TENSOR##_start; (count < line_seg_length) && \
                     (TENSOR##_i < TENSOR##_size);                             \
                     ++TENSOR##_i, ++count) {                                  \
                  r += (EXPR);                                                 \
...
#define TH_TENSOR_APPLY3_CONTIG(                                               \
    TYPE1, TENSOR1, TYPE2, TENSOR2, TYPE3, TENSOR3, CODE)                      \
  {                                                                            \
    auto code_fn = [&](int64_t begin, int64_t end) {                           \
      ptrdiff_t TENSOR1##_len = end - begin;                                   \
      TYPE1* TENSOR1##_data = TENSOR1->data<scalar_t>() + begin;               \
