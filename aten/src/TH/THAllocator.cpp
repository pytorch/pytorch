...
THMapAllocator::THMapAllocator(
    ...
    : filename_(filename ? filename : unknown_filename),
      ...
      ,
      eventname_(
          filename ? std::string(filename) + "_event" : unknown_eventname)
#else
      ,
      fd_(fd)
#endif
  ...
  if ((flags ^ TH_ALLOCATOR_MAPPED_EXCLUSIVE) == 0) {
    AT_ERROR(
        "TH_ALLOCATOR_MAPPED_EXCLUSIVE flag requires opening the file in shared mode");
  }
#ifdef _WIN32
  if (fd != -1) {
  ...
  if (flags_ & TH_ALLOCATOR_MAPPED_SHAREDMEM) {
    ...
    if (flags_ & TH_ALLOCATOR_MAPPED_EXCLUSIVE) {
      ...
      AT_ERROR(
          "Expected either TH_ALLOCATOR_MAPPED_EXCLUSIVE or TH_ALLOCATOR_MAPPED_NOCREATE");
    }
      ...
      if (size > file_stat.st_size) {
        if (flags_) {
          if (ftruncate(fd, size) == -1) {
            AT_ERROR(
                "unable to resize file <", filename_, "> to the right size");
          }
          // NOLINTNEXTLINE(clang-diagnostic-sign-compare)
          if (fstat(fd, &file_stat) == -1 || file_stat.st_size < size) {
            ::close(fd);
            AT_ERROR(
                "unable to stretch file <", filename_, "> to the right size");
          }
 ...
 */
  ...
  c10::reportMemoryUsageToProfiler(
      base_ptr_, size_, c10::Device(c10::DeviceType::CPU));
}
...
#ifdef _WIN32
  if ((flags_ & TH_ALLOCATOR_MAPPED_KEEPFD) ||
      (flags_ & TH_ALLOCATOR_MAPPED_SHAREDMEM))
    CloseHandle(handle_);
...
THRefcountedMapAllocatorArgCheck::THRefcountedMapAllocatorArgCheck(int flags) {
  if (flags & TH_ALLOCATOR_MAPPED_FROMFD) {
    AT_ERROR(
        "THRefcountedMapAllocator doesn't support TH_ALLOCATOR_MAPPED_FROMFD flag");
  }
  if (flags & TH_ALLOCATOR_MAPPED_KEEPFD) {
    AT_ERROR(
        "THRefcountedMapAllocator doesn't support TH_ALLOCATOR_MAPPED_KEEPFD flag");
  }
  if (flags & TH_ALLOCATOR_MAPPED_UNLINK) {
    AT_ERROR(
        "THRefcountedMapAllocator doesn't support TH_ALLOCATOR_MAPPED_UNLINK flag");
  }
...
void THRefcountedMapAllocator::initializeAlloc() {
  ...
  if (!can_wait) {
    AT_ERROR(
        "Couldn't register wait on event, error code: <", GetLastError(), ">");
  }
#endif
...
THRefcountedMapAllocator* THRefcountedMapAllocator::fromDataPtr(
    const at::DataPtr& dptr) {
  return dptr.cast_context<THRefcountedMapAllocator>(
      &deleteTHRefcountedMapAllocator);
}

at::DataPtr THMapAllocator::makeDataPtr(
    const char* filename,
    int flags,
    size_t size,
...
at::DataPtr THRefcountedMapAllocator::makeDataPtr(
    const char* filename,
    int flags,
    size_t size,
    size_t* actual_size_out) {
  ...
  if (actual_size_out)
    *actual_size_out = context->size() - TH_ALLOC_ALIGNMENT;
  return {
      context->data(),
      context,
      &deleteTHRefcountedMapAllocator,
...
at::DataPtr THRefcountedMapAllocator::makeDataPtr(
    ...
    size_t* actual_size_out) {
  auto* context =
      new THRefcountedMapAllocator(WITH_FD, filename, fd, flags, size);
  if (actual_size_out)
    *actual_size_out = context->size() - TH_ALLOC_ALIGNMENT;
  return {
      context->data(),
