...
class TsNode : public torch::lazy::Node {
 public:
  TsNode(OpKind op, OpList operands, std::vector<lazy_tensors::Shape>&& shapes,
  ...
  c10::ArrayRef<lazy_tensors::Shape> shapes() const { return shapes_; }

  // Retrieves the shape of the output at a given index.
  const lazy_tensors::Shape& shape(size_t output_index = 0) const;
 ...
 private:
  ...
  std::vector<lazy_tensors::Shape> shapes_;
