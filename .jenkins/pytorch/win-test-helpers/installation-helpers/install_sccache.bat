...
if "%REBUILD%"=="" (
  ...
  %TMP_DIR_WIN%\bin\sccache.exe --show-stats || (
    ...
    if "%BUILD_ENVIRONMENT%"=="" (
      curl --retry 3 -k https://s3.amazonaws.com/ossci-windows/sccache-20220125.exe --output %TMP_DIR_WIN%\bin\sccache.exe
      curl --retry 3 -k https://s3.amazonaws.com/ossci-windows/sccache-cl.exe --output %TMP_DIR_WIN%\bin\sccache-cl.exe
    ) else (
      aws s3 cp s3://ossci-windows/sccache-20220125.exe %TMP_DIR_WIN%\bin\sccache.exe
