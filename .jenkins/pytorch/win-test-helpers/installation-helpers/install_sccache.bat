...
if "%REBUILD%"=="" (
  ...
  %TMP_DIR_WIN%\bin\sccache.exe --show-stats || (
    ...
    del %TMP_DIR_WIN%\bin\sccache-clang-cl.exe || ver > nul
    if "%BUILD_ENVIRONMENT%"=="" (
      ...
      curl --retry 3 -k https://s3.amazonaws.com/ossci-windows/sccache-clang-cl.exe --output %TMP_DIR_WIN%\bin\sccache-clang-cl.exe
    ) else (
      ...
      aws s3 cp s3://ossci-windows/sccache-clang-cl.exe %TMP_DIR_WIN%\bin\sccache-clang-cl.exe
    )
    goto :check_sccache
  )
)
