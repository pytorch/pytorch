...
with pushd('build'):
    ...
    try:
        ...
        subprocess.run('echo Executing Ninja for custom_backend test...', shell=True)
        subprocess.run('ninja -v', shell=True)

        subprocess.run('echo Ninja succeeded for custom_backend test.', shell=True)
...
try:
    ...
    subprocess.run('conda install -n test_env python test_custom_backend.py -v', shell=True)
    subprocess.run('conda install -n test_env python backend.py --export-module-to="build/model.pt"', shell=True)
    ...
    subprocess.run('test_custom_backend.exe model.pt', shell=True)
