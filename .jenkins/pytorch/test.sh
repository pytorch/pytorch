...
if [[ "${BUILD_ENVIRONMENT}" == *backward* ]]; then
...
elif [[ "${BUILD_ENVIRONMENT}" == *-test1 || "${JOB_BASE_NAME}" == *-test1 ||
        ( $NUM_TEST_SHARDS -gt 1 && "${TEST_CONFIG}" == 1 ) ]]; then
  ...
  test_python_shard1
  test_aten
elif [[ "${BUILD_ENVIRONMENT}" == *-test2 || "${JOB_BASE_NAME}" == *-test2  ||
        ( $NUM_TEST_SHARDS -gt 1 && "${TEST_CONFIG}" == 2 ) ]]; then
  install_torchvision
  test_python_shard2
  test_libtorch
  test_custom_script_ops
