...
def triton_(arg_Q, arg_K, arg_V, arg_LSE, out_ptr0, arg_m, arg_l, arg_acc, arg_rdct):
    ...
    tl.device_assert(Q_LEN <= BLOCK_M, "Query len must be smaller than block size static kernel parameter BLOCK_MMODEL")
    ...
    ACC_block_ptr = tl.make_block_ptr(
        base=ACC + acc_offset,
        shape=(SPLIT_KV, Q_LEN, BLOCK_DMODEL),          # (T, M, D)
        strides=(stride_acct, stride_accm, stride_accd),
        offsets=(off_t, 0, 0),
        block_shape=(1, BLOCK_M, BLOCK_DMODEL),
        order=(2, 1, 0)
    )

    # initialize offsets
    offs_m = tl.arange(0, BLOCK_M)      
    offs_qlen = tl.arange(0, Q_LEN) 
 ...
 # ~~~~~~~~~~~~~~~~~~~ Apply score modification  ~~~~~~~~~~~~~~~~~~~
    ...
    if my_ticket >= SPLIT_KV - 1: # Last CTA is responsible for the reduction
        ...
        index_ml = offs_t[:, None] * BLOCK_M + offs_qlen[None, :] # [T, M]
        index_acc = offs_t[:, None, None] *BLOCK_DMODEL*BLOCK_M + offs_qlen[None, :, None]*BLOCK_DMODEL + offs_d[None, None, :] #[T, M, D]
        ...
        g_acc = tl.zeros([BLOCK_M, BLOCK_DMODEL], dtype=tl.float32)
        ...
        idx_z = off_hz // H
        idx_h = off_hz % H
        idx_m = offs_qlen[:, None]
        idx_d = tl.arange(0, BLOCK_DMODEL)[None, :]
        # TODO generalize and add proper mask support
        mask = (idx_m != -1) & (idx_d != -1)
        xindex = idx_d + (64*idx_m) + (128*idx_h) + (1024*idx_z)
        tl.store(out_ptr0 + (tl.broadcast_to(xindex, mask.shape)), g_acc, mask) 
        # indentation hack https://github.com/pytorch/pytorch/pull/125515


        # TODO dont want to write this if we dont require grad
        if OUTPUT_LOGSUMEXP:
            l_ptrs = LSE + off_hz * Q_LEN + offs_qlen
            lse = g_m + tl.math.log2(g_l)
            tl.store(l_ptrs, lse)
        


meta0 = {'BLOCK_N': 64, 'SPLIT_KV': 32, 'BLOCK_M': 16, 'Q_LEN': 2, 'BLOCK_DMODEL': 64, 'SCORE_MOD_IS_LINEAR': False, 'ROWS_GUARANTEED_SAFE': False, 'OUTPUT_LOGSUMEXP': True}
...
def call(args):
    ...
    buf_m = torch.zeros(2, 8, 64, 2, device='cuda', dtype=torch.float32) # rowmax reduction buffer [B, H, SPLIT_KV, M]
    buf_l = torch.zeros(2, 8, 64, 2, device='cuda', dtype=torch.float32) # l_i reduction buffer [B, H, SPLIT_KV, M]
    buf_acc = torch.zeros(2, 8, 64, 2, 64, device='cuda', dtype=torch.float32) # acc reduction buffer [B, H, SPLIT_KV, M, D]
