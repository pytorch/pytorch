...
def partition_0(args):
    getitem_1, tangents_2, _conj_2, tangents_1 = args
    args.clear()
    with torch.cuda._DeviceGuard(0):
        torch.cuda.set_device(0)
        ...
        buf32 = torch.ops.aten.div.Tensor(buf19, buf25) ### buf19, bu25 matches, 
        del buf19
        del buf25
        buf33 = buf32  
        # in the correct order, buf33 is updated by `buf36 = torch.ops.aten.copy_.default(buf35, tangents_1)`
        # then buf33 is used in `buf42 = torch.ops.aten.mm.default(buf33, buf41)`
        # However, after reordering, we first do `buf42 = torch.ops.aten.mm.default(buf33, buf41)`
        # then buf33 is updated by `buf36 = torch.ops.aten.copy_.default(buf35, tangents_1)`
        # This violates the dependency and leads to error on buf42 values.
        # More details: buf34 is the diagonal of buf33, so it's alias.
        #       buf35 is buf34, then we copy tangents_1 to buf35 as buf36.
        #       finally, we compute mm on buf33 and buf41 to get buf42. So buf42 should depend on buf33, buf35, buf36.
        # (Pdb) nodes[40].read_writes
        # ReadWrites(reads=OrderedSet([StarDep(name='buf33', mode=None), StarDep(name='buf41', mode=None)]), writes=OrderedSet([StarDep(name='buf42', mode=None)]), index_exprs=OrderedSet([]), range_vars=None, var_ranges=None)
        # (Pdb) nodes[40].unmet_dependencies
        # OrderedSet([StarDep(name='buf33', mode=None), StarDep(name='buf41', mode=None)])
        # (Pdb) nodes[40].node.name
        # 'buf42'
        ...
        breakpoint()
        # Topologically Sorted Source Nodes: [], Original ATen: [aten.mm]
        buf42 = torch.ops.aten.mm.default(buf33, buf41)  # buf33 does not match
        del buf33
        del buf38
        del buf39
        del buf40
...
class Runner:
    ...
    def call(self, args):
        ...
        args.clear()
        partition0_args = [getitem_1, tangents_2, _conj_2, tangents_1]
        del getitem_1, tangents_2, _conj_2, tangents_1
