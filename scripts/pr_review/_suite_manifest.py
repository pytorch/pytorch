...
run and silently retires whatever that module asserted. For
`test_workflow_contract.py` that is every invariant about the workflow YAML — API
scopes, the egress allowlist, the gating label spelled in three places — so the
deletion would be exactly the change the CI wiring was added to prevent.
...
class TestTheSuiteIsWhole(unittest.TestCase):
    ...
    def test_no_module_hides_its_tests_behind_load_tests(self):
        ...
        purpose: in `test_workflow_contract.py` alone, a hook on that same file
        would remove this check together with every workflow contract it
        protects. Carried by all three, a surviving sibling still reports it —
        and it reads the module DISCOVERY imported, so a hook defined only under
        the real import context is still visible.
        """
        for name, mods in self.loaded_modules().items():
            for mod in mods:
                with self.subTest(module=name, loaded_as=mod.__name__):
