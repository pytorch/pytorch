...
def main():
    ...
    commit_hash = subprocess.check_output("git rev-parse HEAD".split(), encoding="utf-8").rstrip()

    out.writerow([commit_hash, args.log_url, ""])
    ...
    with open(args.file, newline='') as f:
        ...
        for row in reader:
            ...
            msg = msg.replace("\t", " ")
            # Remove specific prefixes from test names to clean up the output
