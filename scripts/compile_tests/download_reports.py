...
def download_reports(commit_sha):
    ...
    if os.path.exists(log_dir):
        ...
        return subdir_paths
    output = subprocess.check_output(["gh", "run", "list", "-c", commit_sha, "-w", "pull", "--json", "databaseId"]).decode()
    ...
    def download_report(job_name):
        ...
        for listing in listings:
            ...
            if name.endswith(f'_{job_id}.zip'):
                ...
                subprocess.run(["unzip", path_to_zip, "-d", path_to_zip[:-4]], check=True)
                return
