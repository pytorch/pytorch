...
def visualize_file(filename):
  ...
  if filename.endswith('summary'):
    ...
    return flask.jsonify(result='Unsupport file: {}'.format(filename),
                         script='')
...
def index():
  ...
  return flask.render_template(
      'index.html', root=args.root, names=names, debug_messages=names)
...
def main(argv):
  ...
  parser.add_argument('--max_curves', type=int, default=5,
