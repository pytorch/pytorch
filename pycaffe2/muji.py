...
def Allreduce2(net, blobs, reduced_affix, gpu_indices):
  ...
  a_reduced = net.Add([a, b], a + reduced_affix,
                      device_option=OnGPU(gpu_a))
  b_reduced = a_reduced.Copy([], b + reduced_affix,
...
def Allreduce8(net, blobs, reduced_affix, gpu_indices):
  ...
  # Reduction level 1
  for i in [0, 2, 4, 6]:
    reduced[i] = net.Add([blobs[i], blobs[i+1]], blobs[i] + reduced_affix,
  ...
  reduced[4] = reduced[0].Copy(
      [], reduced[4], device_option=OnGPU(gpu_indices[4]))
  # Broadcast level 2
  for i in [2, 6]:
...
def AllreduceFallback(net, blobs, reduced_affix, gpu_indices):
  ...
  # copy first
  reduced[0] = net.Copy(blobs[0], blobs[0] + reduced_affix,
                             device_option=OnGPU(gpu_indices[0]))
  # do temp copy and add
  temp_name = reduced[0] + '_temp_copy'
  for i in range(1, len(gpu_indices)):
    temp = net.Copy(
        blobs[i], temp_name, device_option=OnGPU(gpu_indices[0]))
    reduced[0] = reduced[0].Add(
        temp, reduced[0], device_option=OnGPU(gpu_indices[0]))
  # Broadcast to everyone else
  for i in range(1, len(gpu_indices)):
    reduced[i] = net.Copy(reduced[0], blobs[i] + reduced_affix,
        device_option=OnGPU(gpu_indices[i]))
  return reduced
