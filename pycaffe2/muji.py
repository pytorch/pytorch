...
def Allreduce2(net, blobs, reduced_affix, gpu_indices):
  ...
  a_reduced = net.Add([a, b], str(a) + reduced_affix,
                      device_option=CudaDeviceOption(gpu_a))
  b_reduced = a_reduced.Copy([], str(b) + reduced_affix,
                             device_option=CudaDeviceOption(gpu_b))
...
def Allreduce4(net, blobs, reduced_affix, gpu_indices):
  ...
  c_reduced = net.Add([c, d], str(d) + reduced_affix,
                      device_option=CudaDeviceOption(gpu_c))
  # a_reduced <- a_reduced + c_reduced
  a_reduced = a_reduced.Add(c_reduced, a_reduced,
                            device_option=CudaDeviceOption(gpu_a))
  # broadcast a_reduced to c_reduced
  c_reduced = a_reduced.Copy([], c_reduced,
                             device_option=CudaDeviceOption(gpu_c))
  # broadcast to b and d
  b_reduced = a_reduced.Copy([], str(b) + reduced_affix,
                             device_option=CudaDeviceOption(gpu_b))
  d_reduced = c_reduced.Copy([], str(d) + reduced_affix,
                             device_option=CudaDeviceOption(gpu_d))
...
def Allreduce8(net, blobs, reduced_affix, gpu_indices):
  ...
  # Reduction level 1
  for i in [0, 2, 4, 6]:
    reduced[i] = net.Add([blobs[i], blobs[i+1]], str(blobs[i]) + reduced_affix,
                device_option = CudaDeviceOption(gpu_indices[i]))
  # Reduction level 2
  for i in [0, 4]:
    reduced[i] = net.Add(
        [reduced[i], reduced[i+2]], str(blobs[i]) + reduced_affix,
        device_option = CudaDeviceOption(gpu_indices[i]))
  # Reduction level 3: this involves a copy.
  reduced_4_copy = reduced[4].Copy(
      [], reduced[4] + '_copy', device_option = CudaDeviceOption(gpu_indices[0]))
  reduced[0] = reduced[0].Add(
      reduced_4_copy, reduced[0],
      device_option = CudaDeviceOption(gpu_indices[0]))
  # Broadcast level 1
  reduced[4] = reduced[0].Copy(
      [], reduced[4], device_option=CudaDeviceOption(gpu_indices[4]))
  # Broadcast level 2
  for i in [2, 6]:
    reduced[i] = reduced[i-2].Copy(
        [], reduced[i], device_option=CudaDeviceOption(gpu_indices[i]))
  # Broadcast level 3
  for i in [1, 3, 5, 7]:
...
def AllreduceFallback(net, blobs, reduced_affix, gpu_indices):
  """A fallback option: everything gets carried out on the first gpu, and then
  gets copied."""
  reduced = [None] * len(gpu_indices)
  # copy first
  reduced[0] = net.Copy(blobs[0], str(blobs[0]) + reduced_affix,
                             device_option=CudaDeviceOption(gpu_indices[0]))
  # do temp copy and add
  temp_name = '_' + str(reduced[0]) + '_temp_copy'
  for i in range(1, len(gpu_indices)):
    temp = net.Copy(
        blobs[i], temp_name, device_option=CudaDeviceOption(gpu_indices[0]))
    reduced[0] = reduced[0].Add(
        temp, reduced[0], device_option=CudaDeviceOption(gpu_indices[0]))
  # Broadcast to everyone else
  for i in range(1, len(gpu_indices)):
    reduced[i] = reduced[0].Add(
        reduced[0], str(blobs[i]) + reduced_affix,
        device_option=CudaDeviceOption(gpu_indices[i]))
  return reduced
