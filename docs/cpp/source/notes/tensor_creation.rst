...
3. ``<sizes>`` is an object of type ``IntListRef`` and specifies the shape of the resulting tensor,
...
tensor? In general, an ``IntListRef`` -- the type of the size parameter of factory
...
constructing an ``IntListRef``. You can also pass an ``std::vector<int64_t>`` and
...
.. code-block:: cpp
  ...
  assert(tensor.sizes() == torch::IntListRef{3, 4, 5});

Notice how we use ``tensor.sizes()`` to get back an ``IntListRef`` containing the
