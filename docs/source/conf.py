...
def viewcode_follow_imported(app, modname, fullname):
    # By default, the `__module__` attribute is used to infer the source code location of an object. However, if one
    # uses the `@torch._functorch.utils.exposed_in` decorator, the attribute points to where the object is exposed
    # rather than where it is defined. This function implements an alternative way to extract the location without
    # relying on `__module__` and thus make the `@torch._functorch.utils.exposed_in` compatible with source view.
    ...
    try:
        ...
        # In case someone is using a partial build, some parts that should be documented are not accessible. `sphinx`
        # warns in these cases, but otherwise continues the build. Thus, we do the same and just disable source view.
        return None

    fallback_module = getattr(obj, "__module__", None)
    ...
    try:
        rel_path = pathlib.Path(abs_path).relative_to(pathlib.Path(torch.__path__[0]).parent)
    except ValueError:
        # The module is not relative to torch and thus cannot be documented with source view
        return fallback_module

    module_path = rel_path.parent if rel_path.name == "__init__.py" else rel_path.with_suffix("")
