...
class TorchCoreAutosummary(Autosummary):
    """
    This directive should only be used on ``torch.rst``, it is designed
    specifically to get around the fact that we desire common operations such
    as ``torch.no_grad`` and ``torch.autograd.no_grad`` (in ``autograd.rst``)
    to effectively be documented twice.  Using ``.. autosummary`` in both places
    will result in warnings from sphinx (documenting the same item twice is not
    allowed), so we redirect the ``torch.rst`` document to the original using
    two steps:

    1. ``autosummary_filename_map`` must declare any special cases, keys are the
       name used on ``torch.rst`` and values are the other location where this
       item is being documented.
    2. Hack autosummary to, if found in ``autosummary_filename_map``, import
       the other item.

    This means that the file ``generated/torch.no_grad.html`` that was being
    generated prior to this will no longer be generated.  Instead, the link in
    the summary table on ``torch.rst`` will redirect to
    ``generated/torch.autograd.no_grad.html``.
    """
    def import_by_name(self, name, prefixes):
        if name in autosummary_filename_map:
            name = autosummary_filename_map[name]
...
def setup(app):
    ...
    app.add_directive("torchsummary", TorchCoreAutosummary)
