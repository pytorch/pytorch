...
.. list-table::
  ...
  * - ``PYTORCH_ALLOC_CONF``
    - For a more in depth explanation of this environment variable, see :ref:`cuda-memory-management`. ``PYTORCH_CUDA_ALLOC_CONF`` and ``PYTORCH_HIP_ALLOC_CONF`` are its aliases and are provided only for backward compatibility.
