...
| `TORCH_CUBLAS_WORKSPACE_CACHE` | If set to `1`, retains ATen's cuBLAS and cuBLASLt workspaces per handle and stream instead of allocating them for each ATen operation, which is the default. This is separate from the CUDA allocator's caching of freed device memory. Persistent workspaces should not be used when capturing multiple CUDA graphs into a shared memory pool. |
