...
The forward and backward compatibility of StableIValue conversions will be handled appropriately and a user need not consider this when choosing `TORCH_TARGET_VERSION`. This means that if, for example, the memory layout of a `torch/headeronly` struct `T` was changed from version 2.9 to 2.10 and the user targets 2.9 when building, they can be assured that the StableIValue conversion will handle the conversion from the user extension `T` to libtorch `T` appropriately and vice versa.
