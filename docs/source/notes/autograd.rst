...
#. If the function is not a deterministic mapping (i.e. it is not a [mathematical function](https://en.wikipedia.org/wiki/Function_(mathematics))), it will be marked as non-differentiable, and it will error out in the backward if used on tensors that require grad outside of a ``no_grad`` environment.
