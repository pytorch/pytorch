...
.. code-block:: python
   ...
   def compiled_example(a, b):
       ...
       x, _ = __compiled_fn_0._torchdynamo_orig_callable(a, b)
       if not _:
...
Note that we pass a simple `my_compiler` function as the backend compiler, therefore the subgraph code `__resume_at_38_2`, `__resume_at_30_1`, and `__compiled_fn_0._torchdynamo_orig_callable` remain python code. However, if we use other backends like the built-in `torchinductor`, the subgraph code will be compiled CUDA kernels for GPU or C++ code for CPU.
