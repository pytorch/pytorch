...
How to retrieve artifacts generated by Dynamo?
----------------------------------------------

If you want to inspect the artifacts generated by Dynamo, here is what you can do:

First, we have an API `torch._dynamo.eval_frame._debug_get_cache_entry_list` to retrieve compiled code and guards out of a compiled function. A compiled function can have several cache entries, and each cache entry consists a lambda function to keep the guards, and a codeobject to keep the code to be executed if the guarding conditions are satisfied.
...
.. code-block:: python
   ...
   # the guard takes a input frame, and tells whether a re-compilation should be triggered.
   import inspect
...
Second, the compiled function for subgraphs like `__compiled_fn_0` has already been released into your python scope now! You can find all the functions in the output of `dis.dis(code)`.

.. code-block:: python

   # Try this out! You didn't define the function, but Dynamo implicitly defines for you!
   __compiled_fn_0

With the above utilities and explanations, you can now manually inspect the compiled functions and guards as you like, so long as you can read the python bytecode.

Manually decompilation of the bytecode looks like the following:
...
The `__compiled_fn_0` is more complicated, though. It will invoke the guards, and check the guarding conditions, trigger re-compilation if needed.
