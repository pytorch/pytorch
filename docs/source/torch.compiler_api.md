...
returns, the runnable a load returns, and the coverage and guard report it produces.
It is distinct from `torch._dynamo.config.caching_precompile` (a `torch.compile`
caching mode).
