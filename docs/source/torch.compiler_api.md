...
.. py:function:: precompile(fn, *example_inputs, backend="inductor", tracer="make_fx", decompositions=None)
   ...
   .. note::
      ...
      silently drops (pinned, mkldnn, sparse), and a nested one; and, on a STATIC capture,
      a data-dependent op (``.item()``, ``.nonzero()``, a Python branch over a tensor
      value). It also FAILS, rather than baking a wrong answer, on an op with no meta/fake
      kernel and on a read of a traced tensor's data (``.data_ptr()``, ``.numpy()``), and
      refuses to run inside another trace, whose fake mode would outrank its own.
