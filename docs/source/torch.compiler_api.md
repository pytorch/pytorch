...
.. py:function:: precompile(fn, *example_args, example_inputs=None, backend="inductor", tracer="make_fx", decompositions=None, recompile_limit=None, dynamic=None)
   ...
   :raises NotImplementedError: for a feature not yet supported (with
       ``tracer="dynamo"``: an ``nn.Module``, bound method, or non-function ``fn``, a
       function with closure cells, numpy arguments, or ``decompositions``).
   :raises TypeError: if ``example_inputs`` is not a sequence of positional-argument
       tuples, or both positional examples and ``example_inputs`` are passed.
   :raises ValueError: for an out-of-contract argument (e.g. empty ``example_inputs``,
       an unknown ``backend``/``tracer``, or ``recompile_limit``/``dynamic`` without
       ``tracer="dynamo"``).
