...
5. **Device validation**: Validates device indices and defaults to the current device when index is -1
...
.. literalinclude:: ../../../test/cpp_extensions/open_registration_extension/torch_openreg/csrc/runtime/OpenRegHooks.h
    :language: c++
    :start-after: LITERALINCLUDE START: OPENREG GENERATOR HOOKS
    :end-before: LITERALINCLUDE END: OPENREG GENERATOR HOOKS
    :linenos:
```

**Layer 5 responsibilities:**

1. **Hook implementation**: Overrides `getDefaultGenerator` and `getNewGenerator` from `PrivateUse1HooksInterface`
...
3. **New generator creation**: `getNewGenerator` calls `createOpenRegGenerator(device_index)` to create independent generator instances
...
3. **Generator creation**: Creates generator instances using `createOpenRegGenerator(i)` which internally calls `at::make_generator<OpenRegGeneratorImpl>(idx)`
4. **Initial seeding**: Seeds each generator during initialization
5. **Device validation**: Validates the device index and defaults to current device if index is -1
6. **Generator return**: Returns the const reference to the appropriate device's generator