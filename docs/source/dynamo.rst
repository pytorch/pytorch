...
TorchDynamo supports arbitrary PyTorch code, control flow, mutation and
dynamic shapes.
...
Here is a basic example of how to use TorchDynamo. You can decorate a
function or a method using ``torch._dynamo.optimize()`` and pass in the
name of a compiler or backend e.g: inductor and your code will run faster.
...
.. code:: py
   ...
   def fn(x, y):
    ...
    fn(torch.randn(1), torch.randn(1))

It’s also easy to define your own compiler backends in pure python
`custom backend <./documentation/custom-backend.md>`__
