...
By default, if optimization blacklist is None or empty, ``optimize_for_mobile`` will run the following optimizations:
    ...
    - **MobileOptimizerType::REMOVE_DROPOUT**: This optimization pass removes ``dropout`` and ``dropout_`` nodes from this module when training is false.
