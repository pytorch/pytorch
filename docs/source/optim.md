...
```{math}
W^\mathrm{EMA}_{t+1} = \alpha W^\mathrm{EMA}_{t} + (1 - \alpha) W^\mathrm{model}_{t+1}
```

where alpha is the EMA decay and t is the update step (with $W^\mathrm{EMA}_{0} = W^\mathrm{model}_{0}$ at initialization).
