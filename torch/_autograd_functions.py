...
class _LU(torch.autograd.Function):
    ...
    def backward(ctx, LU_grad, pivots_grad, infors_grad):
        ...
        L^{-1} dL = 1_L o (L^{-1} P^T dA U^{-1}),
        dU U^{-1} = 1_U o (L^{-1} P^T dA U^{-1}), where o denotes the Hadamard product.

        Hence we finally get:
        dL = L 1_L o (L^{-1} P^T dA U^{-1}),
        dU = 1_U o (L^{-1} P^T dA U^{-1}) U

        Backward AD:
        The backward sensitivity is then:
        Tr(B_grad^T dB) = Tr(B_grad^T dL) + Tr(B_grad^T dU) = [1] + [2].

        [1] = Tr(B_grad^T dL) = Tr(B_grad^T L 1_L o (L^{-1} P^T dA U^{-1}))
            = [using Tr(A (B o C)) = Tr((A o B^T) C)]
            = Tr((B_grad^T L o 1_L^T) L^{-1} P^T dA U^{-1})
            = [cyclic property of trace]
            = Tr(U^{-1} (B_grad^T L o 1_L^T) L^{-1} P^T dA)
            = Tr((P L^{-T} (L^T B_grad o 1_L) U^{-T})^T dA).
        Similary [2] can be rewritten as:
        [2] = Tr(P L^{-T} (B_grad U^T o 1_U) U^{-T})^T dA, hence
        Tr(A_grad^T dA) = [1] + [2]
                        = Tr((P L^{-T} (L^T B_grad o 1_L + B_grad U^T o 1_U) U^{-T})^T dA), so
        A_grad = P L^{-T} (L^T B_grad o 1_L + B_grad U^T o 1_U) U^{-T}.

        In the code below we use `LU` name instead of `B`, so that there is no confusion
