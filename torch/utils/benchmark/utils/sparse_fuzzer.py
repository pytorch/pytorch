...
class FuzzedSparseTensor(FuzzedTensor):
    def __init__(
        ...
        Args:
            ...
            density: The `nnz` value will be truncated to this value.
                This allows tensors of varying sparsities to be generated by the Fuzzer.  
            coalesced: 
    ...
    def _make_tensor(self, params, state):
        ...
        if 'nnz' not in params:
            density = params['density']
            nnz = math.ceil(sum(size) * density)
            assert nnz <= sum(size)
        else:
            nnz = params['nnz']
            density = nnz / sum(size)
