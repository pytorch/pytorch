...
class RemovableHandle:
    ...
    def __init__(self, hooks_dict: Any, *, extra_dict: Any = None) -> None:
        ...
        self.extra_dict_ref: Optional[Union[List[Any], Any]] = None
        if isinstance(extra_dict, dict):
            self.extra_dict_ref = weakref.ref(extra_dict)
        elif isinstance(extra_dict, list):
            self.extra_dict_ref = [weakref.ref(d) for d in extra_dict]
    ...
    def remove(self) -> None:
        ...
        if self.extra_dict_ref is not None:
            if isinstance(self.extra_dict_ref, list):
    ...
    def __getstate__(self):
        ...
        elif isinstance(self.extra_dict_ref, list):
            return (self.hooks_dict_ref(), self.id, [ref() for ref in self.extra_dict_ref])
        else:
            return (self.hooks_dict_ref(), self.id, self.extra_dict_ref())
    ...
    def __setstate__(self, state) -> None:
        ...
        if len(state) < 3:
            ...
            self.extra_dict_ref = weakref.ref(OrderedDict())
        elif isinstance(state[2], dict):
            self.extra_dict_ref = weakref.ref(state[2])
        else:
            assert isinstance(state[2], list)
            self.extra_dict_ref = [weakref.ref(d) for d in state[2]]
