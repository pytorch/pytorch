...
def register_flop_formula(targets, get_raw=False) -> Callable[[Callable[_P, _T]], Callable[_P, _T]]:
    ...
    Args:
        targets: OpOverloadPacket(s) to register the formula for (e.g., torch.ops.mylib.my_op),
                or Triton JITFunction(s). Can be a single target or a list of targets.
    ...
    Example::
        ...
        # Or calculate actual FLOPs based on shape
...
class FlopCounterMode:
    ...
    Args:
        ...
        skip_unsupported: If True, operations without registered FLOP formulas are executed
                         with a warning and tracked via get_unsupported_ops() (default: False).
                         This is particularly useful for Higher Order Operators (HOPs) that
                         would otherwise fail. Regular ops without formulas always execute
                         and count 0 FLOPs regardless of this setting.
...
class _FlopCounterMode(TorchDispatchMode):
    ...
    def _handle_higher_order_ops(self, func, types, args, kwargs):
        # Handle Triton kernels - look up in flop registry
        ...
        # Handle cond HOP - count upper bound of flops across branches
        ...
        # Check if HOP is in flop_registry (e.g., flex_attention)
        if func in self.counter.flop_registry:
            out = func(*args, **kwargs)
            return self.counter._count_flops(func, out, args, kwargs)

        # Unknown HOP - handle with skip_unsupported or return NotImplemented
    ...
    def __torch_dispatch__(self, func, types, args=(), kwargs=None):
        ...
        if func_packet not in self.counter.flop_registry and func is not torch.ops.prim.device.default:
            ...
            # Could not decompose. If skip_unsupported is enabled, execute without counting
            if self.counter.skip_unsupported:
