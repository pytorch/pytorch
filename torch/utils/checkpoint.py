...
def check_backward_validity(inputs: Iterable[Any]) -> None:
    if not any(inp.requires_grad for inp in inputs if isinstance(inp, torch.Tensor)):
        warnings.warn(
            "None of the inputs have requires_grad=True. Gradients will be None", stacklevel=1
        )
...
def _infer_device_type(*args):
    ...
    if len(device_types) > 1:
        warnings.warn(
            ...
            "detected, it will be prioritized; otherwise, the first device encountered will be selected.)", stacklevel=1
        )
    ...
    **kwargs
    ...
    if use_reentrant is None:
        warnings.warn(
            ...
            "differences between the two variants.", stacklevel=1
        )
        use_reentrant = True
