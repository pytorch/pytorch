...
class WebIterDataPipe(IterDataPipe[Tuple[str, IOBase]]):
    ...
    Iterable DataPipe to load file url(s) (web url(s) pointing to file(s)), yield file url and IO stream in a tuple
    args:
        ...
        for furl in self.source_datapipe:
            try:
                ...
                yield(furl, r)
                    ...
                    raise Exception("Could not get the file. [HTTP Error] {code}: {reason}."
                                    .format(code=e.code, reason=e.reason))
            except URLError as e:
                    raise Exception("Could not get the file at {url}. [URL Error] {reason}."
