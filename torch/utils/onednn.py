...
class OnednnLinear(torch.jit.ScriptModule):
    def __init__(self, dense_module, dtype):
        super().__init__()
        self.register_buffer('weight', dense_module.weight.to_onednn(dtype))
        if dense_module.bias is not None:
            # Bias can be fp32 or bf16 for OneDNN bf16 path, but for good accuracy,
            # we use fp32 dtype.
            self.register_buffer('bias', dense_module.bias.to_onednn())
        else:
            # TODO: Remove this once ScriptModule supports registering None buffer
            self.register_buffer(
                'bias',
                torch.zeros([dense_module.weight.size(0)], dtype=torch.float).to_onednn())
    ...
    def __setstate__(self, state):
        self.weight = state[0].to_onednn()
        self.bias = state[1].to_onednn()
        self.training = state[2]

    @torch.jit.script_method
    def forward(self, x):
        x_onednn = x if x.is_onednn else x.to_onednn()
        y_onednn = torch._C._nn.onednn_linear(x_onednn, self.weight, self.bias)
        y = y_onednn if x.is_onednn else y_onednn.to_dense()
        return y
...
class OnednnConv2d(_OnednnConvNd):
    def __init__(self, dense_module, dtype):
        ...
        self.register_buffer('weight', torch._C._nn.onednn_reorder_conv2d_weight(
            dense_module.weight.to_onednn(dtype),
            self.padding,
            self.stride,
            self.dilation,
            self.groups))
...
class OnednnBatchNorm(torch.jit.ScriptModule):
    ...
    def __init__(self, dense_module):
        ...
        self.register_buffer('weight', dense_module.weight.to_onednn())
        self.register_buffer('bias', dense_module.bias.to_onednn())
        self.register_buffer('running_mean', dense_module.running_mean.to_onednn())
        self.register_buffer('running_var', dense_module.running_var.to_onednn())
    ...
    def __setstate__(self, state):
        self.weight = state[0].to_onednn()
        self.bias = state[1].to_onednn()
        self.running_mean = state[2].to_onednn()
        self.running_var = state[3].to_onednn()
...
class OnednnPrelu(torch.jit.ScriptModule):
    ...
    def forward(self, x):
        x_onednn = x if x.is_onednn else x.to_onednn()
        y_onednn = torch.prelu(x_onednn, self.weight)
        y = y_onednn if x.is_onednn else y_onednn.to_dense()
        return y

def to_onednn(module, dtype=torch.float):
