...
def _get_context_intercept_specs() -> list[tuple[type, str, Callable | None]]:
    ...
    return [
        (no_grad, "no_grad", None),
        (enable_grad, "enable_grad", None),
        (set_grad_enabled, "set_grad_enabled", lambda *a, **kw: f"mode={a[0].mode}"),
        (inference_mode, "inference_mode", lambda *a, **kw: f"mode={a[0].mode}"),
        (
            autocast,
            "autocast",
...
def register_context_manager_intercept(
    cls: type, display_name: str, enter_args_fn: Callable | None = None
) -> None:
    ...
    Example::
        ...
        register_context_manager_intercept(
            MyCtx, "MyCtx", lambda *a, **kw: f"key={a[0].key}"
        )
    """
...
def register_function_intercept(
    owner: Any, attr: str, display_name: str, args_fn: Callable
) -> None:
    ...
    Example::
        ...
        register_function_intercept(
            torch, "manual_seed", "manual_seed", lambda *a, **kw: f"seed={a[0]}"
        )
    """
    spec = (owner, attr, display_name, args_fn)
    _USER_FUNCTION_SPECS.append(spec)
    if _ACTIVE_DEBUG_MODE_COUNT > 0:
        _install_function_intercept(*spec)


def unregister_external_intercept(owner: Any, attr: str) -> None:
    """Remove a previously registered interception."""
    _USER_CONTEXT_SPECS[:] = [s for s in _USER_CONTEXT_SPECS if s[0] is not owner]
    _USER_FUNCTION_SPECS[:] = [
        s for s in _USER_FUNCTION_SPECS if not (s[0] is owner and s[1] == attr)
    ]
    for key_attr in (attr, "__enter__", "__exit__"):
        key = (owner, key_attr)
        if key in _INTERCEPT_ORIGINALS:
            setattr(owner, key_attr, _INTERCEPT_ORIGINALS.pop(key))
