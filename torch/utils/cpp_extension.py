...
def _find_cuda_home() -> Optional[str]:
    ...
    if cuda_home and not torch.cuda.is_available():
        log.info("No CUDA runtime is found, using CUDA_HOME='%s'", cuda_home)
    return cuda_home
...
def _find_rocm_home() -> Optional[str]:
    ...
    if rocm_home and torch.version.hip is None:
        log.info("No ROCm runtime is found, using ROCM_HOME='%s'", rocm_home)
    return rocm_home
...
def _jit_compile(name,
    ...
    if version > 0:
        if version != old_version and verbose:
            log.info(
                "The input conditions for extension module %s have changed. "
                "Bumping to version %s and re-building as %s_v%s...", name, version, name, version)
    ...
    if version != old_version:
        ...
        log.info("No modifications detected for re-loaded extension module %s, skipping build step...", name)

    if verbose:
        log.info("Loading extension module %s...", name)
...
def _write_ninja_file_and_compile_objects(
    ...
    if verbose:
        log.info("Emitting ninja build file %s...", build_file_path)
    _write_ninja_file(
...
def _write_ninja_file_and_build_library(
    ...
    if verbose:
        log.info('Building extension module %s...', name)
    _run_ninja_build(
        build_directory,
        verbose,
...
def _prepare_ldflags(extra_ldflags, with_cuda, verbose, is_standalone):
    ...
    if with_cuda:
        if verbose:
            log.info('Detected CUDA files, patching ldflags')
        if IS_WINDOWS:
...
def _get_build_directory(name: str, verbose: bool) -> str:
    ...
    if verbose:
        log.info('Using %s as PyTorch extensions root...', root_extensions_directory)

    build_directory = os.path.join(root_extensions_directory, name)
    if not os.path.exists(build_directory):
        if verbose:
            log.info('Creating extension directory %s...', build_directory)
...
def _get_num_workers(verbose: bool) -> Optional[int]:
    ...
    if max_jobs is not None and max_jobs.isdigit():
        if verbose:
            log.info('Using envvar MAX_JOBS (%s) as the number of workers...', max_jobs)
        return int(max_jobs)
    if verbose:
        log.info('Allowing ninja to set a default number of workers... '
