...
class TreeSpec:
    ...
    @overload
    def __init__(self, type: list[_Node], context: int) -> None: ...
    ...
    def __init__(
        self,
        type: list[_Node] | PyTreeType | None = None,
        context: int | Context | None = None,
        children_specs: list["TreeSpec"] | None = None,
    ) -> None:
        if type is None:
            nodes = [_LEAF]
            index = -1
        elif isinstance(type, list) and isinstance(context, int):
            nodes = type
            index = context
        else:
            assert not isinstance(type, list)
            assert isinstance(children_specs, list)
            ...
            nodes.append(_Node(type, context, num_children, num_leaves, num_nodes))
            index = -1
    ...
    def __hash__(self) -> int:
        return hash((tuple(self.nodes), self.index))
    ...
    def children_specs_append(self, spec: Self) -> None:
        ...
        object.__setattr__(self._root, "num_children", self._root.num_children + 1)
        object.__setattr__(self._root, "num_nodes", self._root.num_nodes + 1)

        if spec.nodes[0].is_leaf():
            object.__setattr__(self._root, "num_leaves", self._root.num_leaves + 1)
    ...
    def _recalculate(self) -> None:
        ...
        object.__setattr__(self._root, "num_nodes", num_nodes)
        object.__setattr__(self._root, "num_leaves", num_leaves)
