...
# Not registering here as there's a better decomposition using
...
def _normalize_start_end(
    x: TensorLikeType, dim: int, start: Optional[int], end: Optional[int]
) -> Tuple[int, int]:
    """
...
def _validate_dim(x: TensorLikeType, dim: int):
    res = dim
    ndim = x.dim()
    if dim < 0:
        res += ndim
