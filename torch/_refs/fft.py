...
def _canonicalize_fft_shape_and_dim_args(
    ...
    if shape is not None:
        ...
        ret_shape = tuple(
            s if s != -1 else input_sizes[d] for (s, d) in zip(shape, ret_dims)  # type: ignore[possibly-undefined]  # TODO: audit
        )
    elif dim is None:
        # No shape, no dim
        ...
        # No shape, has dim
        ret_shape = tuple(input_sizes[d] for d in ret_dims)  # type: ignore[possibly-undefined]  # TODO: audit

    for n in ret_shape:
        torch._check(n > 0, lambda: f"Invalid number of data points ({n}) specified")

    return _ShapeAndDims(shape=ret_shape, dims=ret_dims)  # type: ignore[possibly-undefined]  # TODO: audit
