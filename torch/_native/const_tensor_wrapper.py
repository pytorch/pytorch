...
class ConstTensorWrapper:
    ...
    Pointer-based DSL adapters such as Triton and FlyDSL extract a tensor
    argument through ``data_ptr()`` and its metadata. This wrapper returns
    ``const_data_ptr()`` instead, so passing a copy-on-write tensor to an
    argument the kernel only reads does not materialize it.

    Only wrap arguments the kernel treats as read-only (loads, never stores).
    The DSL does not enforce this; wrapping a written argument is a bug.
