...
def _kernel_eligible(
    ...
    reads, so this is safe under FakeTensor tracing. An ineligible input never
    enters the override: the router falls back, so a profile shows whether the
    kernel ran. At fp16, eager's `compact` logits buffer is fp16 too, and
    aliasing `g` into it keeps the kernel at eager's memory.
    """
    return (
