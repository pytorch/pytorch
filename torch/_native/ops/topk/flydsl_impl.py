...
def _get_topk_kernels():
    ...
    if _TOPK_KERNELS is None:
        ...
        _TOPK_KERNELS = RadixSelectTopK, RadixSelectTopKOut, RegisterTopK, RegisterTopKOut
...
def _run(self: torch.Tensor, k: int) -> tuple[torch.Tensor, torch.Tensor]:
    ...
    values_2d, indices_2d = RadixSelectTopK(
        self_2d, k, deterministic=deterministic
    )
    return unflatten_last_dim(values_2d, indices_2d, self, k)
...
def _run_out(
    ...
    if kernel == "register":
        ...
        deterministic = torch.are_deterministic_algorithms_enabled()
        RadixSelectTopKOut(self_2d, k, values_2d, indices_2d, deterministic=deterministic)
    return values, indices
