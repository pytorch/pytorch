...
from flydsl.expr import Array, Float32, Int32, Int64, arith, buffer_ops, gpu, range_constexpr
...
def _build_radix_select_topk_module(n: int, k: int, deterministic: bool):
    ...
    def radix_select_topk_kernel(
        ...
        storage = fx.SharedAllocator().allocate(_make_topk_storage(k, block_threads)).peek()
        ...
        def ord_from_bits(bits):
            ...
            is_nan = arith.cmpi(
                arith.CmpIPredicate.ugt, abs_bits, fx.Int32(0x7F800000)
            )
        ...
        def block_excl_prefix_i32(packed_local):
            ...
            if warp == fx.Int32(0):
                ...
                if lane < fx.Int32(num_warps):
                    warp_val = fx.memref_load(s_scan, lane)
            ...
            packed_pfx = fx.memref_load(s_scan, warp) + excl_intra
            packed_total = fx.memref_load(s_scan, num_warps)
            gpu.barrier()
        ...
        for byte_pos in range_constexpr(4):
            ...
            prefix = fx.memref_load(s_prefix, 0)
            decided_mask = fx.memref_load(s_mask, 0)
            for step in range_constexpr(vec_iters):
                ...
                for vi in range_constexpr(_VEC):
                    ...
                    if byte_pos == 0 or (ords & decided_mask) == prefix:
                        byte_val = ((ords >> fx.Int32(shift)) & fx.Int32(255)) ^ fx.Int32(
                            xor_val
                        )
            ...
            if tid == fx.Int32(0):
                remaining_k = fx.memref_load(s_rem_k, 0)
                acc = fx.Int32(0)
                found = fx.Int32(0)
                sel_bin = fx.Int32(0)
                ...
                for b in range_constexpr(_N_HIST_BINS):
                    ...
                    count = fx.memref_load(s_hist, bin_idx)
                    if found == fx.Int32(0):
        ...
        threshold = fx.memref_load(s_prefix, 0)  # Values above threshold are in top-k.
        remaining_k = fx.memref_load(s_rem_k, 0)  # Number of threshold ties to keep.
        ...
        if deterministic:
            ...
            if tid < fx.Int32(k):
                ...
                if offset_into_eq >= fx.Int32(0) and offset_into_eq < remaining_k:
                    fx.memref_store(fx.memref_load(s_eq_vals, offset_into_eq), s_vals, tid)
                    fx.memref_store(fx.memref_load(s_eq_idxs, offset_into_eq), s_idxs, tid)
            gpu.barrier()
        else:
            for step in range_constexpr(vec_iters):
                tile_base = step * tile
                ...
                for vi in range_constexpr(_VEC):
                    ...
                    if ords > threshold:
                        pos = atomic_add_i32_fetch(s_write_ctr, fx.Int32(1), fx.Int32(0))
                        if pos < fx.Int32(k):
                            fx.memref_store(val, s_vals, pos)
                            fx.memref_store(idx, s_idxs, pos)
                    elif ords == threshold:
                        eq_pos = atomic_add_i32_fetch(s_eq_ctr, fx.Int32(1), fx.Int32(0))
        ...
        for stage in range_constexpr(num_stages):
            for sub_rev in range_constexpr(stage + 1):
                ...
                p_v = fx.memref_load(s_vals, partner)
                p_i = fx.memref_load(s_idxs, partner)
                gpu.barrier()
        ...
        if tid < fx.Int32(k):
            offset = base_out + tid
            buffer_ops.buffer_store(fx.memref_load(s_vals, tid), values_rsrc, offset)
            buffer_ops.buffer_store(
                fx.Int64(fx.memref_load(s_idxs, tid)), indices_rsrc, offset
            )
...
def _build_register_topk_module(n: int, k: int, rows_per_cta: int = 2):
    ...
    def register_topk_kernel(
        ...
        def cas_desc(arr, i: int, j: int):
            a = fx.memref_load(arr, i)
            b = fx.memref_load(arr, j)
            swap = a < b
            fx.memref_store(swap.select(b, a), arr, i)
            fx.memref_store(swap.select(a, b), arr, j)
        ...
        def topk_merge_desc(a, b):
            for i in range_constexpr(k):
                x = fx.memref_load(a, i)
                y = fx.memref_load(b, k - 1 - i)
                take_y = y > x
                fx.memref_store(take_y.select(y, x), a, i)
        ...
        for i in range_constexpr(vec):
            ...
            fx.memref_store(make_key(load_col(col), col), keys, i)
        ...
        if vec >= k:
            ...
            for i in range_constexpr(vec):
                fx.memref_store(fx.memref_load(keys, i), topk, i)
            for i in range_constexpr(k - vec):
                fx.memref_store(sentinel, topk, vec + i)
        ...
        for s in range_constexpr(log2_threads_per_row):
            ...
            for i in range_constexpr(k):
                peer = fx.memref_load(topk, i).shuffle_xor(
                    fx.Int32(1 << s), fx.Int32(threads_per_row)
                )
                fx.memref_store(peer, other, i)
            topk_merge_desc(topk, other)

        if lane == fx.Int32(0) and in_bounds:
            for i in range_constexpr(k):
                val, idx = decode_key(fx.memref_load(topk, i))
