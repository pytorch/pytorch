...
#
#   stage 1: reshape (M, N) -> (M*C, N/C). The vectorized tile row kernel
#            (kernel_rowtile) reduces each sub-row to a RAW per-field accumulator
...
#            re-reduce) keeps mean/var/norm correct, not just sum. It projects one
...
def _reduce_row_xcta(
    ...
    # for reduce-all. Returns a tuple of (M,) results. flatten=True (reduce-ALL of a 1-D input)
    ...
    # (Stage 1 casts its block-index tile coordinate to Int64, so a >2^31 M*N element
    # offset does not overflow at large M.)
    #
    ...
    # fused kernel is keyed on the sub-row's vec class and stage-1 launch config only.
    # Both stages take their extents as runtime args (stage 1 the sub-row length, stage
    # 2 C and N), so distinct N sharing a vec class share ONE kernel. subrow_target is
    # in gkey: it changes C.
...
def _build_geom(trait, trait_key, x, out_dtypes, nouts, M, N, block, subrow_target):
    ...
    # ONE stage-1 shape. The bucket-vs-exact split this used to carry existed so that a
    # STATIC per-N tile could be shared across N via a ceiling; the tile row kernel's fold is
    # rolled, so the sub-row length is a runtime arg and one compiled kernel already serves
    # the whole vec class. N is absent from the key for exactly that reason.
