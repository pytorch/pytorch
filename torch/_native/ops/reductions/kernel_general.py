...
class _K0Config(NamedTuple):
    # K0's OWN knob set (row/col/xcta have differently-shaped knobs). block is the sole
    # dim-reduce knob; reduce-all adds grid_mult. Explicit args to the reduce_* fns
    # override these (autotuner / per-machine retune).
    block: int = _K0_BLOCK
    all_block: int = _K0_ALL_BLOCK
    all_grid_mult: int = _K0_ALL_GRID_MULT
...
def reduce_row(trait, trait_key, x, out_dtype, block=_K0_BLOCK):
    assert x.dim() == 2  # noqa: S101
    return reduce_dim(trait, trait_key, x, -1, out_dtype, block=block)


def reduce_row_2out(trait, trait_key, x, out_dtypes, block=_K0_BLOCK):
    assert x.dim() == 2  # noqa: S101
    return reduce_dim2(trait, trait_key, x, -1, out_dtypes, block=block)


def reduce_col(trait, trait_key, x, out_dtype, block=_K0_BLOCK, **_legacy):
