...
def cute_tensor_dynMN(t, vec, align=None, read_only=False):
    ...
    # emitting vec-wide (up to 128-bit) loads. One compiled kernel then serves every
    # shape in a VEC CLASS instead of one exact N -- the recompile-minimization mode
    # the rolled folds rely on. Caller guarantees t is 2D row-major compact and
    # t.shape[1] % vec == 0 (its vec class).
    w = _ro(t, read_only)
    ct = (
