...
def load_metallib(data: bytes):
    r"""Loads a precompiled Metal library (.metallib) from bytes and returns
    a shader library object that allows invoking kernels defined in it.
    ...
    Example::
        ...
        >>> # xdoctest: +REQUIRES(env:TORCH_DOCTEST_MPS)
        >>> with open("kernels.metallib", "rb") as f:
    ...
    if not hasattr(torch._C, "_mps_loadMetalllib"):
        raise RuntimeError("MPS is not available")
    return torch._C._mps_loadMetalllib(data)
