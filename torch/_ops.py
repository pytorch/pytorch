...
@cache
def _out_arg_names_cached(
    qualified_op_name: str, overload_names: tuple[str, ...]
) -> frozenset[str]:
    return frozenset(
        arg.name
        for overload_name in overload_names
        for arg in torch._C._get_schema(qualified_op_name, overload_name).arguments
        if arg.is_out
    )


def _out_arg_names(op: "OpOverload | OpOverloadPacket") -> frozenset[str]:
    """Names of the kwarg-only mutable ("out=") arguments declared by `op`.
    ...
    outputs (aten.aminmax uses min/max). A positional mutable argument is not
    `is_out`, so this is not a "does this op mutate an argument" predicate --
    use `get_mutable_args_from_schema` for that. For a packet the answer is the
    union over its overloads, which is exact in tree: no name is an out
    argument of one overload and an ordinary argument of a sibling.

    Memoized on names rather than on `op` because a packet hashes on its `_op`
    (`OpOverloadPacket.__hash__`), which `_refresh_packet` reassigns in place;
    the freed address can be reused, aliasing a stranded entry and serving a
    stale answer. The name key is itself reused when a library is torn down and
    the op redefined with a different schema, so `_clear_torch_ops_cache`
    clears the memo. Entries are bounded by the registered overload sets.
    """
    if isinstance(op, OpOverload):
        return _out_arg_names_cached(op._schema.name, (op._schema.overload_name,))
    return _out_arg_names_cached(op._qualified_op_name, tuple(op._overload_names))
