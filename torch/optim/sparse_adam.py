        ...
        for group in self.param_groups:
            for p in group['params']:
                ...
                if self.fuse_kernels:
                    data_step, exp_avg_step, exp_avg_sq_step = \
                        torch._C._nn._sparse_adam_step(
                            grad, exp_avg, exp_avg_sq, group['lr'], beta1,
                            beta2, group['eps'], state['step'])
                    exp_avg.add_(exp_avg_step)
                    exp_avg_sq.add_(exp_avg_sq_step)
                    p.data.add_(data_step)
                else:
                    ...
                    old_exp_avg_sq_values = exp_avg_sq.sparse_mask(
                        grad)._values()
                    exp_avg_sq_update_values = grad_values.pow(
                        2).sub_(old_exp_avg_sq_values).mul_(1 - beta2)
