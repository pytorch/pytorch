...
def sparse_adam(params: List[Tensor],
    ...
    for i, param in enumerate(params):
        ...
        if grad_values.numel() == 0:
            ...
            continue
        if param.is_complex():
            grad_values = view_as_real(grad_values)
        size = grad.size()
        ...
        def make_sparse(values):
            ...
            if param.is_complex():
                return constructor(grad_indices, view_as_complex(values), size)
        ...
        if param.is_complex():
            old_exp_avg_values = view_as_real(old_exp_avg_values)
            old_exp_avg_sq_values = view_as_real(old_exp_avg_sq_values)
