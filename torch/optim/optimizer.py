...
_capturable_doc = r"""capturable (bool, optional): whether this instance is safe to
            capture in a graph, whether for CUDA graphs or for `torch.compile` support. Tensors
            are only capturable when on supported accelerators like CUDA, XPU, HPU, and XLA.
            Passing True can impair ungraphed performance, so if you don't intend to graph capture
