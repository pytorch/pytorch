...
def rprop(params: List[Tensor],
    ...
    torch._foreach_addcmul_(params_with_grad, grad_signs, step_sizes, value=-1)   # type: ignore[name-defined, arg-type]
...
def rmsprop(params: List[Tensor],
    ...
    if weight_decay != 0:
        torch._foreach_add_(grads, params_with_grad, alpha=weight_decay)   # type: ignore[name-defined]
    ...
    if momentum > 0:
        ...
        torch._foreach_add_(params_with_grad, buf, alpha=-lr)   # type: ignore[name-defined]
    else:
        torch._foreach_addcdiv_(params_with_grad, grads, avg, value=-lr)  # type: ignore[name-defined]
