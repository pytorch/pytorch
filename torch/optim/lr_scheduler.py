...
class CyclicLR(_LRScheduler):
    ...
    * "exp_range": A cycle that scales initial amplitude by :math:`\text{gamma}^{\text{cycle iterations}}`
    ...
    def __init__(self,
        ...
        if scale_fn is None:
            self.scale_fn_custom = None      
            if self.mode == 'triangular':
                self.scale_fn_ref = weakref.WeakMethod(self._triangular_scale_fn)
                self.scale_mode = 'cycle'
            elif self.mode == 'triangular2':
                self.scale_fn_ref = weakref.WeakMethod(self._triangular2_scale_fn)
                self.scale_mode = 'cycle'
            elif self.mode == 'exp_range':
                self.scale_fn_ref = weakref.WeakMethod(self._exp_range_scale_fn)
                self.scale_mode = 'iterations'
        else:
            self.scale_fn_custom = scale_fn
            self.scale_fn_ref = None
    ...
    def scale_fn(self, x):
        if self.scale_fn_custom is not None:
           return self.scale_fn_custom(x)

        else:
           return self.scale_fn_ref()(x)
