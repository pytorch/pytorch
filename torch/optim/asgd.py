...
class ASGD(Optimizer):
    ...
    def __setstate__(self, state):
        ...
        if not eta_is_tensor:
            for s in state_values:
                s["eta"] = torch.tensor(s["eta"], dtype=torch.float)
        mu_is_tensor = (len(state_values) != 0) and torch.is_tensor(
            state_values[0]["mu"]
        )
        if not mu_is_tensor:
            for s in state_values:
                s["mu"] = torch.tensor(float(s["mu"]), dtype=torch.float)
    ...
    def _init_group(self, group, params_with_grad, grads, mus, axs, etas, state_steps):
        ...
        for p in group["params"]:
            if p.grad is not None:
                ...
                if len(state) == 0:
                    state["step"] = torch.zeros((), device=p.device, dtype=torch.float)
                    state["eta"] = torch.tensor(group["lr"], device=p.device, dtype=torch.float)
                    state["mu"] = torch.ones((), device=p.device, dtype=torch.float)
