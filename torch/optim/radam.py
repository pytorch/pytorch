...
class RAdam(Optimizer):
                ...
                if len(p_state) != 0 and not torch.is_tensor(p_state["step"]):
                    ...
                    p_state["step"] = (torch.tensor(
                        step_val, dtype=_get_scalar_dtype(), device=p.device)
                                       if group["capturable"] else torch.tensor(
                                           step_val, dtype=_get_scalar_dtype()))

    def _init_group(self, group, params_with_grad, grads, exp_avgs, exp_avg_sqs,
                    state_steps):
        has_complex = False
        for p in group["params"]:
    ...
    def step(self, closure=None):
        ...
        for group in self.param_groups:
            ...
            has_complex = self._init_group(group, params_with_grad, grads,
...
def _single_tensor_radam(
    ...
    *,
    ...
    for i, param in enumerate(params):
        ...
        def _compute_rect():
            return ((rho_t - 4) * (rho_t - 2) * rho_inf /
        ...
        if capturable:
            update = torch.where(rho_t > 5.0,
                                 _compute_rect() * _compute_adaptive_lr(), 1.0)
            param.add_(bias_corrected_exp_avg * lr * update, alpha=-1.0)
        else:
            if rho_t > 5.0:
                param.add_(
                    bias_corrected_exp_avg * lr * _compute_adaptive_lr() *
                    _compute_rect(),
                    alpha=-1.0,
                )
            else:
    ...
    *,
    ...
    for (
        ...
        if grouped_state_steps[0].is_cpu:
            torch._foreach_add_(grouped_state_steps,
        ...
        if has_complex:
            _view_as_real(grouped_params, grouped_grads, grouped_exp_avgs,
                          grouped_exp_avg_sqs)
        ...
        if capturable:
            ...
            rho_t_list = [
                rho_inf - 2 * _get_value(step) * (beta2**_get_value(step)) /
        ...
        torch._foreach_addcmul_(grouped_exp_avg_sqs, grouped_grads,
                                grouped_grads, 1 - beta2)
        ...
        if capturable:
            ...
            rect = [
                ...
                for n, rho_t in zip(num, rho_t_list)
            ]
            del num
            del rho_t_list
            unrect_step_size = [
                torch.where(rect > 0, 0.0, 1.0) for rect in rect
            ]
            ...
            bias_correction2 = [
                ...
                (lr * rect / bc) * -1 for step, rect, bc in zip(
