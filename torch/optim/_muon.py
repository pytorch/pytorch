...
def param_groups_for_muon(
    ...
    * ``exclude_predicate(name, p)`` returns ``True`` (when provided).
    ...
    ``requires_grad=False`` are skipped. Shared parameters are routed to
    ``other_params`` if any of their names matches an exclusion.
    ...
    excluded_by_param: dict[torch.nn.Parameter, bool] = {}
    for name, p in module.named_parameters(remove_duplicate=False):
        if not p.requires_grad:
            continue
        excluded_by_param[p] = excluded_by_param.get(p, False) or (
            p.ndim < 2
    ...
    for p, is_other in excluded_by_param.items():
