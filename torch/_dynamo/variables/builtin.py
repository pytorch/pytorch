...
class BuiltinVariable(BaseBuiltinVariable):
    ...
    def call___build_class__(self, tx, *args, **kwargs):
        ...
        if isinstance(args[0], variables.NestedUserFunctionVariable):
            code = args[0].get_code()
            direct_source_backed_closure_names = set()
            nested_source_backed_closure_names = set()

            def contains_source_backed_closure(fn_var, seen):
                if id(fn_var) in seen:
                    return False
                seen.add(id(fn_var))
                if not fn_var.closure:
                    return False
                for (
                    cell_var
                ) in fn_var.closure.items:  # pyrefly: ignore[missing-attribute]
                    cell_contents = tx.output.side_effects.load_cell(cell_var)
                    if (
                        isinstance(cell_contents, variables.UserDefinedObjectVariable)
                        and cell_contents.source is not None
                        and not tx.output.side_effects.is_modified(cell_contents)
                    ):
                        return True
                    if isinstance(
                        cell_contents, variables.NestedUserFunctionVariable
                    ) and contains_source_backed_closure(cell_contents, seen):
                        return True
                return False

            if args[0].closure:
                for name, cell_var in zip(
                    code.co_freevars,
                    args[0].closure.items,  # pyrefly: ignore[missing-attribute]
                    strict=False,
                ):
                    cell_contents = tx.output.side_effects.load_cell(cell_var)
                    if (
                        ...
                        direct_source_backed_closure_names.add(name)
                    elif isinstance(
                        cell_contents, variables.NestedUserFunctionVariable
                    ) and contains_source_backed_closure(cell_contents, set()):
                        nested_source_backed_closure_names.add(name)

            instructions = list(dis.get_instructions(code))
            has_source_backed_closure = bool(
                direct_source_backed_closure_names or nested_source_backed_closure_names
            )
            closure_load_opnames = {
                "LOAD_CLASSDEREF",
                "LOAD_CLOSURE",
                "LOAD_DEREF",
                "LOAD_FAST",
            ...
            call_opnames = {"CALL", "CALL_KW", "CALL_FUNCTION_EX"}
            call_opnames.update({"CALL_FUNCTION", "CALL_FUNCTION_KW", "CALL_METHOD"})

            def source_backed_closure_stored_function_target(idx):
                end = idx
                while (
                    end < len(instructions)
                    and instructions[end].opname in closure_load_opnames
                ):
                    end += 1
                remaining_opnames = [inst.opname for inst in instructions[end:]]
                if remaining_opnames[:5] == [
                    "BUILD_TUPLE",
                    "LOAD_CONST",
                    "MAKE_FUNCTION",
                    ...
                    if instructions[end + 3].arg & 0x08:
                        return instructions[end + 4].argval
                    return None
                if remaining_opnames[:4] == [
                    "BUILD_TUPLE",
                    "LOAD_CONST",
                    "MAKE_FUNCTION",
                    "STORE_NAME",
                ]:
                    if instructions[end + 2].arg & 0x08:
                        return instructions[end + 3].argval
                    return None
                if remaining_opnames[:5] == [
                    "BUILD_TUPLE",
                    "LOAD_CONST",
            ...
            stored_source_backed_function_names = set()

            for idx, inst in enumerate(instructions):
                if (
                    inst.opname in closure_load_opnames
                    and inst.argval in direct_source_backed_closure_names
                ):
                    target = source_backed_closure_stored_function_target(idx)
                    if target is not None:
                        stored_source_backed_function_names.add(target)
                        continue
                    unimplemented(
                        gb_type="Unsupported source-backed closure direct read in class body",
                        context=f"{inst.opname} {inst.argrepr}",
                        explanation=(
                            "Dynamo cannot safely execute a class body that reads a "
                            "source-backed closure object while tracing __build_class__."
                        ),
                        hints=[*graph_break_hints.SUPPORTABLE],
                    )
                if has_source_backed_closure and inst.opname in call_opnames:
                    unimplemented(
                        gb_type="Unsupported source-backed closure call in class body",
                        context=f"{inst.opname} {inst.argrepr}",
                        explanation=(
                            "Dynamo cannot safely execute calls in a class body "
                            "with a source-backed closure."
                        ),
                        hints=[*graph_break_hints.SUPPORTABLE],
                    )
                if (
                    has_source_backed_closure
                    and inst.opname in function_load_opnames
                    and inst.argval in {"locals", "vars", "eval", "exec"}
                ):
                    unimplemented(
                        gb_type="Unsupported source-backed closure dynamic class body access",
                        context=f"{inst.opname} {inst.argrepr}",
                        explanation=(
                            "Dynamo cannot safely execute a class body that "
                            "uses dynamic introspection or execution with a "
                            "source-backed closure."
                        ),
                        hints=[*graph_break_hints.SUPPORTABLE],
                    )
                if inst.opname in function_load_opnames and inst.argval in (
                    nested_source_backed_closure_names
                    | stored_source_backed_function_names
                ):
                    unimplemented(
                        gb_type="Unsupported source-backed closure function read in class body",
                        context=f"{inst.opname} {inst.argrepr}",
                        explanation=(
                            "Dynamo cannot safely execute a class body that reads a "
                            "source-backed closure object while tracing __build_class__."
        ...
        try:
            if isinstance(args[0], variables.NestedUserFunctionVariable):
                fn = args[0].get_function(allow_source_backed_closure=True)
            else:
