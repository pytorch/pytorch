...
class BuiltinVariable(BaseBuiltinVariable):
    ...
    def call_method(
        ...
        if self.fn in (BaseException, Exception) and name == "__init__" and args:
            ...
            unimplemented(
                gb_type="Unsupported exception __init__ mutation",
                context=f"{self.fn.__name__}.__init__ on {receiver}",
                explanation="Dynamo cannot safely trace BaseException.__init__ mutating an existing exception object.",
                hints=[*graph_break_hints.SUPPORTABLE],
            )
