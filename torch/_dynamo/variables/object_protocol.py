...
Per-type hook implementations (bool_impl, tp_richcompare_impl, tp_getattro_impl,
...
type_implements_sq_length_impl = partial(
...
type_implements_sq_contains_impl = partial(
...
# PyMappingSlots
type_implements_mp_length_implh = partial(
...
def pymapping_size(
    ...
    if obj.tp_as_mapping.mp_length_impl:
        return obj.mp_length_impl(tx)

    if obj.tp_as_sequence.sq_length_impl is not None:
...
def generic_size(
    ...
    Dispatches to sq_length_impl (sequences) or mp_length_impl (mappings) depending on the VT type.
    """

    if obj.tp_as_sequence.sq_length_impl:
...
def generic_is_true(
    ...
    Resolution order: constants → nb_bool → mp_length_impl/sq_length_impl → truthy.
    """
    from .constant import ConstantVariable
...
def pysequence_getitem(
    ...
    iteration protocol.  Wraps negative indices via sq_length_impl before
...
def pysequence_setitem(
    ...
    if sq_ass_item is not None:
        ...
        if isinstance(i, ConstantVariable):
            ...
            if isinstance(index_val, int) and index_val < 0:
                if s.tp_as_sequence.sq_length_impl is not None:
...
def python_constant_tp_richcompare_impl(
    self: VariableTracker,
    tx: "InstructionTranslatorBase",
    other: VariableTracker,
    op: str,
...
def pysequence_contains(
    ...
    If the object has sq_contains_impl (i.e., __contains__), calls obj.sq_contains_impl(tx, item).
    ...
    sq_contains_impl = obj.tp_as_sequence.sq_contains_impl
    if sq_contains_impl is not None:
