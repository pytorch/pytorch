    ...
    def call_method(
        ...
        if name == "__getitem__":
            ...
            if value.python_type() not in (int, slice):
                ...
                raise_observed_exception(TypeError, tx, args=[SourcelessBuilder.create(tx, msg)])
            ...
            return SourcelessBuilder.create(tx, polyfills.list_cmp).call_function(
                tx,
                [SourcelessBuilder.create(tx, cmp_name_to_op_mapping[name]), left, right],
                {},
            )
        elif name == "__iter__":
...
class RangeVariable(BaseListVariable):
    ...
    def getitem_const(
        ...
        if isinstance(index, slice):
            ...
            msg = SourcelessBuilder.create(tx, "range indices must be integers or slices")
            raise_observed_exception(TypeError, tx, args=[msg])
...
class NamedTupleVariable(UserDefinedTupleVariable):
    ...
    def call_method(
        ...
        if self._is_method_overridden(name):
            ...
            return SourcelessBuilder.create(tx, self.is_python_equal(args[0]))
        elif name == "__ne__":
            if len(args) != 1 or kwargs:
                raise ValueError("Improper arguments for method.")
            return SourcelessBuilder.create(tx, not self.is_python_equal(args[0]))
