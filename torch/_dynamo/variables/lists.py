...
class ByteArrayIteratorVariable(IteratorVariable):
    ...
    def reconstruct(self, codegen: "PyCodegen") -> None:
        ...
        data = self.parent.data
        if self.is_exhausted or (self.step > 0 and self.index >= len(data)):
            remaining = b""
        elif self.step < 0 and self.index < 0:
            remaining = b""
        elif self.step == 1:
            remaining = bytes(data[self.index :])
        elif self.step == -1:
            remaining = bytes(data[: self.index + 1][::-1])
        else:
            remaining = bytes(data[self.index :: self.step])
