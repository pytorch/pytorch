...
class FunctoolsPartialVariable(VariableTracker):
    ...
    def is_python_object_hashable(self) -> bool:
        return (
            self.func.is_python_object_hashable()
            and all(arg.is_python_object_hashable() for arg in self.args)
            and all(
                value.is_python_object_hashable()
                for value in self.keywords.values()
            )
        )

    def get_python_object_hash(self):
        func_hash = self.func.get_python_object_hash()
        args_hash = (arg.get_python_object_hash() for arg in self.args)
        values_hash = (
            value.get_python_object_hash() for value in self.keywords.values()
        )
    ...
    def is_python_object_equal(self, other):
        ...
        return (
            self.func.is_python_object_equal(other.func)
            and all(
                arg_a.is_python_object_equal(arg_b)
                for (arg_a, arg_b) in zip(self.args, other.args)
            )
            and all(
                value_a.is_python_object_equal(value_b)
