...
class LocalGeneratorObjectVariable(VariableTracker):
    ...
    def call_method(
        ...
        if name == "__next__":
            ...
            # Call next(generator) to continue execution. User code can either
            # (1) yield a value, which is returned to the caller, or return
            retval = self.next_variable(tx)

            # Run the finally block if it exists.
            ...
            # TODO(guilhermeleobas): The finally block should be executed when the function
            # is collected. The code below is a workaround that is wrong.
            return retval

            # exc_type = type("__InternalThrowException", (BaseException,), {})

            # try:
            #     self._setup_and_raise_exception(
            #         tx, variables.ExceptionVariable(exc_type, ())
            #     )
