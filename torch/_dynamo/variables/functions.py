...
_SAFE_TO_RUN_IN_EAGER_FUNCTIONS = frozenset({inspect.Signature.bind})
    ...
    def call_function(
        ...
        if self.source and not kwargs and self.fn in _TRY_EAGER_FALLBACK_FUNCTIONS:
            ...
            try:
                ...
                pass
        elif self.fn in _SAFE_TO_RUN_IN_EAGER_FUNCTIONS:
            from torch._dynamo.variables import UserDefinedObjectVariable
            assert isinstance(self, UserMethodVariable)
            out_val = self.fn(self.obj.value, *args, **kwargs)
            out_type = type(out_val)
            # Build cls_source by chaining AttrSource for nested modules
            # e.g., "a.b.c" -> AttrSource(AttrSource(ImportSource("a"), "b"), "c")
            module_parts = out_type.__module__.split(".")
            ...
            out = tx.output.side_effects.track_eagerly_created_new_user_defined_object(
                UserDefinedObjectVariable(
                    out_val,
                    init_args=args,
                    cls_source=cls_source,
            ...
            out.call_method(tx, "__setattr__", [ConstantVariable.create("_signature"), self.obj], {})
            return out
