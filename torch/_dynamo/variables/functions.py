...
def _descriptor_defining_class_vt(
    ...
    """VT for the class that actually defines an unbound C descriptor found by
    walking class_vt's MRO. `descriptor.__objclass__` names the defining class,
    which can differ from class_vt itself for an inherited descriptor:
    ...
    if objclass is class_vt.as_python_constant():
        return class_vt
...
def get_source_by_walking_mro(
    ...
    """Source for `name` as found directly in some class in cls.__mro__'s own
    __dict__, bypassing the descriptor protocol.

    Unlike AttrSource(source, name), which resolves through __get__ if `name`
    is a descriptor, this always yields the raw descriptor object (a
    staticmethod/classmethod/property/... instance) -- the object our VT
    actually wraps. Installs DICT_NOT_CONTAINS guards on every class in the
    MRO searched before the hit, so a subclass later shadowing `name` earlier
    in the MRO triggers a recompile.
    """
    for idx, klass in enumerate(cls.__mro__):
        if name in klass.__dict__:
            ...
            for absent_idx in range(1, idx):
                absent_klass = cls.__mro__[absent_idx]
    ...
    raise RuntimeError(f"Attribute {name!r} not found in MRO of {cls}")
...
def _resolve_descriptor_get(
    ...
    descriptor type (plain class variable). This is the bound (obj != None)
    sibling of resolve_unbound_class_descriptor.
    """
    if isinstance(type_attr, property):
        prop_vt = PropertyVariable(type_attr, source=source)
        return prop_vt.tp_descr_get_impl(tx, obj, class_vt)
    if isinstance(type_attr, types.MemberDescriptorType):
        md_vt = MemberDescriptorVariable(type_attr, source=source)
        return md_vt.tp_descr_get_impl(tx, obj, class_vt)
    if isinstance(type_attr, types.GetSetDescriptorType):
        gs_vt = GetSetDescriptorVariable(type_attr, source=source)
    ...
    if isinstance(type_attr, _tuplegetter):
        tg_vt = TupleGetterVariable(type_attr, source=source)
        return tg_vt.tp_descr_get_impl(tx, obj, class_vt)
    if isinstance(type_attr, staticmethod):
        sm_vt = StaticMethodVariable(type_attr, source=source)
        return sm_vt.tp_descr_get_impl(tx, obj, class_vt)
    if isinstance(type_attr, classmethod):
        cm_vt = ClassMethodVariable(type_attr, source=source)
        return cm_vt.tp_descr_get_impl(tx, obj, class_vt)
    if isinstance(type_attr, types.ClassMethodDescriptorType):
        cmd_vt = ClassMethodDescriptorVariable(type_attr, source=source)
        return cmd_vt.tp_descr_get_impl(tx, obj, class_vt)
    if isinstance(type_attr, types.WrapperDescriptorType):
        wd_vt = WrapperDescriptorVariable(type_attr, owner=class_vt, source=source)
        return wd_vt.tp_descr_get_impl(tx, obj, class_vt)
    if isinstance(type_attr, types.MethodDescriptorType):
        md_vt = MethodDescriptorVariable(type_attr, owner=class_vt, source=source)
        return md_vt.tp_descr_get_impl(tx, obj, class_vt)
    if isinstance(type_attr, types.FunctionType):
        return UserMethodVariable(type_attr, obj, source=source)

    return None


def resolve_unbound_class_descriptor(
    tx: "InstructionTranslatorBase",
    cls_attr: object,
    class_vt: VariableTracker,
    name: str,
    source: Source | None,
    allow_c_slot_descriptors: bool = True,
) -> VariableTracker | None:
    """Resolve a class-MRO attribute for class-level (unbound, obj=NULL)
    access -- e.g. `list.__len__` or `SomeClass.some_attr` -- mirroring
    CPython's type_getattro step 4 (`descriptor.__get__(None, cls)`).

    `cls_attr` is the raw attribute as found by an MRO walk (e.g. via
    `mro_lookup`). Returns None when `cls_attr` isn't a descriptor kind
    handled here (plain class variable, Python function, user descriptor with
    a Python-level __get__, ...); the caller decides the fallback.

    `allow_c_slot_descriptors` gates wrapper_descriptor/method_descriptor
    handling, for callers (e.g. torch-internal classes) that have their own
    trace_rules for those and want to decline and fall through instead.

    Callers should check `name` against comparison dunders themselves before
    calling this (see cmp_name_to_op_mapping) -- this function doesn't special
    case them, but resolving __eq__/__lt__/etc. as a bound wrapper_descriptor
    here can cause VT type mismatches in identity checks elsewhere.
    """
    if isinstance(cls_attr, staticmethod):
        # sm_descr_get returns sm->sm_callable unconditionally, ignoring obj.
        descriptor_source = (
            get_source_by_walking_mro(
                tx, class_vt.as_python_constant(), class_vt.source, name
            )
            if class_vt.source is not None
            else None
        )
        sm_vt = StaticMethodVariable(cls_attr, source=descriptor_source)
        return sm_vt.tp_descr_get_impl(tx, class_vt, class_vt)

    if isinstance(cls_attr, classmethod):
        # cm_descr_get binds to the class, ignoring obj.
        descriptor_source = (
            get_source_by_walking_mro(
                tx, class_vt.as_python_constant(), class_vt.source, name
            )
        ...
        cm_vt = ClassMethodVariable(cls_attr, source=descriptor_source)
        return cm_vt.tp_descr_get_impl(tx, class_vt, class_vt)

    if isinstance(cls_attr, types.ClassMethodDescriptorType):
        # classmethod_get binds to the class, ignoring obj.
        cmd_vt = ClassMethodDescriptorVariable(cls_attr, source=source)
        return cmd_vt.tp_descr_get_impl(tx, class_vt, class_vt)

    # property_descr_get/member_get/getset_get with obj=NULL return self.
    if isinstance(cls_attr, property):
        ...
        return PropertyVariable(cls_attr, source=descriptor_source)
    ...
    if isinstance(cls_attr, types.MemberDescriptorType):
        ...
        return MemberDescriptorVariable(cls_attr, source=descriptor_source)

    if isinstance(cls_attr, types.GetSetDescriptorType):
        descriptor_source = (
            get_source_by_walking_mro(
                tx, class_vt.as_python_constant(), class_vt.source, name
            )
        ...
        return GetSetDescriptorVariable(cls_attr, source=descriptor_source)

    # tuplegetter_descr_get with obj=NULL returns self.
    _tuplegetter = collections._tuplegetter  # pyrefly: ignore[missing-attribute]
    if isinstance(cls_attr, _tuplegetter):
        descriptor_source = (
            get_source_by_walking_mro(
                tx, class_vt.as_python_constant(), class_vt.source, name
            )
        ...
        tg_vt = TupleGetterVariable(cls_attr, source=descriptor_source)
        return tg_vt.tp_descr_get_impl(tx, None, class_vt)

    if not allow_c_slot_descriptors:
        return None

    if name in ("__get__", "__set__", "__delete__"):
        return None

    # wrapperdescr_get/method_get with obj=NULL return the descriptor itself.
    if isinstance(cls_attr, types.WrapperDescriptorType):
        return WrapperDescriptorVariable(
            cls_attr,
            owner=_descriptor_defining_class_vt(tx, class_vt, cls_attr),
            source=source,
        )
