...
def extract_subgraph_and_wrap_with_hop(
    ...
    """
    Extract nodes added to the graph since start_marker_node and wrap them
    in a higher-order operator (HOP).

    This function:
    1. Collects all nodes added after start_marker_node
    2. Creates a new subgraph containing these nodes
    3. Replaces the nodes in the main graph with a call to wrapper_fn
    4. Transforms the original nodes to extract results from the HOP call

    Args:
        tx: The instruction translator
        start_marker_node: Node to start collecting from (or _EMPTY_GRAPH_MARKER)
        subgraph_name: Name for the installed subgraph
        wrapper_fn: The HOP function to wrap the subgraph with
        wrapper_kwargs: Kwargs to pass to the wrapper_fn call
        gmod_meta: Metadata to store on the created GraphModule
    """
    import operator
    ...
    # Step 1: Collect nodes added since start_marker_node and track external inputs
    nodes_to_move = []
    external_inputs = []
    external_input_set = set()
    started_collecting = start_marker_node is _EMPTY_GRAPH_MARKER
    marker_found = start_marker_node is _EMPTY_GRAPH_MARKER

    for node in main_graph.nodes:
        if not started_collecting:
            if node == start_marker_node:
    ...
    assert marker_found, (
        f"start_marker_node was not found in graph! start_marker_node={start_marker_node}"
    )
    ...
    body_name = tx.output.install_subgraph(subgraph_name, gmod)

    # Store metadata on the GraphModule
    gmod.meta.update(gmod_meta)

    # Step 4: Create the get_attr node and the call to wrapper_fn
...
class GenericContextWrappingVariable(UserDefinedObjectVariable):
    ...
    def exit(self, tx: "InstructionTranslator", *args):
        ...
        if self.hopify:
            # Extract the subgraph and wrap with the generic HOP
            extract_subgraph_and_wrap_with_hop(
                ...
                {},
                {
                    "_wrap_generic_cls_fqn": self.cls_fqn,
                    "_wrap_generic_arg_values": self.arg_values,
                    "_wrap_generic_kwarg_values": self.kwarg_values,
                },
            )
