...
class StringFormatVariable(VariableTracker):
    ...
    def _splitlines(
        ...
        pending_bare_cr_through_empty_fields = False
        ...
        try:
            parsed = string.Formatter().parse(self.format_string)
            for literal_text, field_name, format_spec, conversion in parsed:
                if pending_bare_cr_through_empty_fields:
                    if literal_text.startswith("\n"):
                        unimplemented(
                            gb_type="formatted field may be empty between CRLF splitlines boundary",
                            context=f"format string: {self.format_string!r}",
                            explanation="Dynamo cannot split a delayed format string when fields that may be empty occur between '\\r' and '\\n'.",
                            hints=[*graph_break_hints.SUPPORTABLE],
                        )
                    if literal_text:
                        pending_bare_cr_through_empty_fields = False
                ...
                if literal_text.endswith("\r") and field_nonempty is not True:
                    pending_bare_cr_through_empty_fields = True
        except ValueError as e:
            raise_observed_exception(type(e), tx, args=list(e.args))
