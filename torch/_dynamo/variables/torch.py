...
def _jit_isinstance_target_type(target_type: VariableTracker) -> Any | None:
    try:
        ...
        if isinstance(target_type, TupleVariable):
            types = []
            for typ in target_type.items:
                typ_value = _jit_isinstance_target_type(typ)
                if typ_value is None:
                    return None
                types.append(typ_value)
            return tuple(types)
        return None
...
def _jit_isinstance_container_bool(
    ...
    if origin_type in (Union, types.UnionType):
        ...
        for typ in get_args(target_type):
            if get_origin(typ):
                # Match torch._jit_internal.container_checker, including its
                # early return for the first parameterized Union member.
