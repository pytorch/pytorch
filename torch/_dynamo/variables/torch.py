...
def _schema_out_kwarg_names(fn: Any, kwargs: "dict[str, VariableTracker]") -> list[str]:
    """Names of `kwargs` that are out= arguments of `fn`, other than "out" itself.

    Out variants do not always spell the argument "out": autogenerated
    multi-output variants use out0/out1/..., and some ops name them after the
    outputs (aten.aminmax uses min/max). Those names are only discoverable from
    the schema, so the "out" in kwargs check alone silently skips them.
    """
    if not kwargs:
        return []
    ...
    names = {
        ...
        if arg.is_out and arg.name != "out" and arg.name in kwargs
    }
    return sorted(names)
...
"""
        ...
        out_kwarg_vt = None
        # The tensors checked by the `saved_out_shapes` list path below, in the
        # same order. Populated for out= sequences and for out= arguments the
        # schema spells with another name.
        out_list_vts: list[VariableTracker] = []
        if "out" in kwargs:
        ...
        # Same reasoning for out= arguments the schema spells with another name
        # (out0/out1/..., min/max, ...). Without these the resize check below is
        # skipped entirely and the caller silently observes an unresized tensor.
        if not isinstance(saved_out_shapes, torch.Size):
            for name in _schema_out_kwarg_names(self.value, kwargs):
                ...
                if vt.is_tensor():
                    if saved_out_shapes is None:
                        saved_out_shapes = []
                    out_list_vts.append(vt)
                    saved_out_shapes.append(
                        vt.as_proxy().node.meta["example_value"].shape
                    )
