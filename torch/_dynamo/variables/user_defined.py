...
class UserDefinedObjectVariable(UserDefinedVariable):
    ...
    def richcompare_impl(
        ...
        for cls in obj_type.__mro__:
            ...
            # C comparison method — check allowlist before calling.
            in_allowlist = False
            try:
...
class UserDefinedDictVariable(UserDefinedObjectVariable):
    ...
    def richcompare_impl(
        ...
        # Check if the subclass defines a Python-level comparison override
        # (e.g. Counter.__lt__). If so, use the UDOV MRO walk to trace it.
        # Otherwise, delegate to the base dict's richcompare_impl directly.
        obj_type = type(self.value)
        for cls in obj_type.__mro__:
            if cls is dict or cls is object:
                break
            if op in cls.__dict__ and isinstance(cls.__dict__[op], types.FunctionType):
