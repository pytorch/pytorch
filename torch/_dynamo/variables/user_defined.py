...
def call_runtime_value_fn(
    tx: "InstructionTranslatorBase",
    fn: Callable[..., Any],
    args: list[VariableTracker],
    kwargs: dict[str, VariableTracker],
    example_value: Any = _missing,
) -> VariableTracker:
    from .builder import VariableBuilder

    args = [x.as_python_constant() for x in args]
    kwargs = {k: v.as_python_constant() for k, v in kwargs.items()}
    runtime_call_index = len(tx.output.random_calls)
    ...
    source = RandomValueSource(runtime_call_index)
...
def call_random_fn(
    ...
    return call_runtime_value_fn(tx, fn, args, kwargs)
