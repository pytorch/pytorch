...
@functools.cache
def _get_manual_torch_name_rule_map() -> dict[
    str,
    Union[
    ...
    return _manual_torch_name_rule_map


_manual_torch_name_rule_map: dict[
    str,
    Union[
        type[TorchInGraphFunctionVariable],
        type[SkipFunctionVariable],
...
@functools.cache
def _get_torch_c_binding_in_graph_functions() -> dict[
    str,
    type[TorchInGraphFunctionVariable],
]:
    d = dict.fromkeys(_torch_c_binding_in_graph_function_names, TorchInGraphFunctionVariable)
    if sys.version_info >= (3, 11):
        d["math.exp2"] = TorchInGraphFunctionVariable
        d["math.cbrt"] = TorchInGraphFunctionVariable
    if sys.version_info >= (3, 13):
        d["math.fma"] = TorchInGraphFunctionVariable
    return d


# In graph functions (including constant folding) that are C bindings
_torch_c_binding_in_graph_function_names = [
    "math.acos",
    "math.acosh",
    "math.asin",
    "math.asinh",
...
@functools.cache
def _get_torch_non_c_binding_in_graph_functions() -> dict[
    str,
    type[TorchInGraphFunctionVariable],
]:
    return dict.fromkeys(_torch_non_c_binding_in_graph_function_names, TorchInGraphFunctionVariable)
...
_torch_non_c_binding_in_graph_function_names = [
...
@functools.cache
def _get_torch_name_rule_map() -> list[dict[str, type[VariableTracker]]]:
    return [
        _get_manual_torch_name_rule_map(),
        _get_torch_c_binding_in_graph_functions(),
        _get_torch_non_c_binding_in_graph_functions(),
    ]
...
def get_torch_obj_rule_map() -> dict[Any, type["VariableTracker"]]:
    ...
    for m in _get_torch_name_rule_map():
...
def clear_lru_cache() -> None:
    ...
    torch._dynamo.trace_rules.get_mod_skiplist.cache_clear()
    torch._dynamo.trace_rules.dynamo_dir.cache_clear()
    torch._dynamo.trace_rules._get_manual_torch_name_rule_map.cache_clear()
    torch._dynamo.trace_rules._get_torch_c_binding_in_graph_functions.cache_clear()
    torch._dynamo.trace_rules._get_torch_non_c_binding_in_graph_functions.cache_clear()
    torch._dynamo.trace_rules._get_torch_name_rule_map.cache_clear()


def __getattr__(name: str) -> Any:
    if name == "manual_torch_name_rule_map":
        return _get_manual_torch_name_rule_map()
    if name == "torch_c_binding_in_graph_functions":
        return _get_torch_c_binding_in_graph_functions()
    if name == "torch_non_c_binding_in_graph_functions":
        return _get_torch_non_c_binding_in_graph_functions()
    if name == "torch_name_rule_map":
        return _get_torch_name_rule_map()
    if name == "torch_obj_rule_map":
        return get_torch_obj_rule_map()
    if name == "tensor_method":
        return get_tensor_method()
    if name == "legacy_mod_inlinelist":
        return get_legacy_mod_inlinelist()
    if name == "mod_inlinelist":
        return get_mod_inlinelist()
    if name == "mod_skiplist":
        return get_mod_skiplist()
