...
# Sentinel object used to signal a missing attribute/key
_not_present = object()
...
@substitute_in_graph(operator.contains, can_constant_fold_through=False)  # type: ignore[arg-type,misc]
...
def index(a: SupportsIndex, /) -> int:
    nb_index = getattr(a, "__index__", _not_present)
    if nb_index is _not_present:
        raise TypeError(
            f"{type(a).__name__!r} object cannot be interpreted as an integer"
        )
    out = nb_index()  # type: ignore[not-callable]
    if not isinstance(out, int):
        raise TypeError(f"__index__ returned non-int, (type {type(out).__name__})")
    return int(out)
