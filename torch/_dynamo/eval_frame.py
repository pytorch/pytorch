...
def _maybe_wrap_dynamo_generated_runtime_error(
    exc: Exception,
) -> InternalTorchDynamoError | None:
    ...
    msg = (
        ...
        "A runtime exception was raised while executing Dynamo-generated "
        f"bytecode for {context.name} "
        f"({context.filename}:{context.firstlineno}). "
        f"Failing bytecode offset: {tb.tb_lasti}. "
        "This usually indicates a TorchDynamo code generation bug. "
        'Set TORCH_LOGS="bytecode" for the generated bytecode.'
    )
    return InternalTorchDynamoError(msg)
...
class _TorchDynamoContext:
    ...
    def __exit__(
        ...
        if exc_val is not None and isinstance(exc_val, Exception):
            if wrapped := _maybe_wrap_dynamo_generated_runtime_error(exc_val):
                raise wrapped.with_traceback(exc_val.__traceback__) from None
        return None
            ...
            try:
                ...
                with torch._C._ForceDispatchKeyGuard(
                    ...
                    try:
                        ...
                        if wrapped := _maybe_wrap_dynamo_generated_runtime_error(e):
