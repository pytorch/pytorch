...
class DisableContext(_TorchDynamoContext):
    ...
    def __call__(self, fn: Callable[..., Any]) -> Callable[..., Any]:
        ...
        # DisableWrapper is a C-level callable whose tp_call toggles Dynamo's
        # eval-frame callback off around fn without the cost of a Python wrapper
        # frame -- disable is used on hot paths (e.g. around custom operators).
...
def _disable_wrapper_export_call(
    fn: Callable[..., Any], *args: Any, **kwargs: Any
) -> Any:
    # Export fallback for DisableWrapper (torch._C._dynamo.eval_frame). The C
    # tp_call handles the common non-export toggle-and-call inline; when
    # torch.compiler._is_exporting_flag is set it delegates here so the
    # fx_traceback annotation is applied. Toggling the eval-frame callback is
    # done here (not in C) so the whole export path lives in one place.
    prior = set_eval_frame(None)
    try:
        fn_name = getattr(fn, "__name__", type(fn).__name__)
        # Skip annotation for __torch_dispatch__ to avoid polluting node
        # metadata during export. The disable on __torch_dispatch__ is an
        # internal implementation detail, not user-facing.
        # TODO: Ideally we shouldn't need this check because nested annotate()
        # calls shouldn't override existing keys.
