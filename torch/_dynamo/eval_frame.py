            ...
            if OptimizeContext._different_backend(most_recent_backend, compiler_fn):
                if config.raise_on_backend_change:
                    ...
                    warnings.warn(
                        ...
                        "calling `torch._dynamo.reset()` to take effect", stacklevel=1
                    )
...
def explain(f, *extra_args, **extra_kwargs):
    ...
    if extra_args or extra_kwargs:
        warnings.warn(
            ...
            "conflict with future kwargs added to explain(f).", stacklevel=1
        )
    ...
    **extra_kwargs,
    ...
    if extra_args or extra_kwargs:
        warnings.warn(
            ...
            "conflict with future kwargs added to export(f).", stacklevel=1
        )
        return inner(*extra_args, **extra_kwargs)
    else:
        return inner
