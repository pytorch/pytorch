...
class _TorchDynamoContext:
    ...
    def __call__(self, fn):
        ...
        def do_nothing(*arg, **kwargs) -> None:
            pass
...
def is_dynamo_supported() -> Optional[bool]:
    try:
...
def is_inductor_supported() -> Optional[bool]:
    try:
...
def explain(f, *extra_args, **extra_kwargs):
    def inner(*args, **kwargs):
        ...
        def guard_export_print(guards) -> None:
            nonlocal out_guards
...
def rewrite_signature(
    ...
    def check_user_input_output(flat_values, error_type) -> None:
        supported_types = [
            torch.Tensor,
            torch.SymInt,
            torch.SymFloat,
