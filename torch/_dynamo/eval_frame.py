...
if TYPE_CHECKING:
    ...
    from torch._dynamo.package import _DynamoCacheEntry as DynamoArtifacts
...
@dataclass
class CaptureOutput:
    dynamo_artifacts: DynamoArtifacts
    backend_inputs: dict[str, BackendInput]

def aot_capture(model) -> CaptureOutput:
    """
    ...
    1. Dynamo specific information from CompilePackage, which includes:
        - guards
        - generated bytecode
    ...
    def _backend(gm, example_inputs):
        ...
        backend_inputs[gm._backend_id] = BackendInput(gm, example_inputs, TracingContext.get().fake_mode)
        return gm
    ...
    capture_context.get_artifacts = lambda: CaptureOutput(dynamo_artifacts=package.cache_entry(), backend_inputs=backend_inputs)
