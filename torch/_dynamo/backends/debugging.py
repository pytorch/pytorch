...
class ExplainOutput:
    ...
    def __str__(self) -> str:
        output_parts = [f"Graph Count: {self.graph_count}\n"]
        output_parts.append(f"Graph Break Count: {self.graph_break_count}\n")
        output_parts.append(f"Op Count: {self.op_count}\n")

        output_parts.append("Break Reasons:\n")
        for idx, break_reason in enumerate(self.break_reasons):
            output_parts.append(f"  Break Reason {idx + 1}:\n")
            output_parts.append(f"    Reason: {break_reason.reason}\n")
            output_parts.append("    User Stack:\n")
            for frame_summary in break_reason.user_stack:
                output_parts.append(f"      {frame_summary}\n")

        if self.ops_per_graph is not None:
            output_parts.append("Ops per Graph:\n")
            for idx, ops in enumerate(self.ops_per_graph):
                output_parts.append(f"  Ops {idx + 1}:\n")
                for op in ops:
                    output_parts.append(f"    {op}\n")

        if self.out_guards is not None:
            output_parts.append("Out Guards:\n")
            for i, guard in enumerate(self.out_guards):
                output_parts.append(f"  Guard {i + 1}:\n")
                output_parts.append(f"    {str(guard)}")

        if self.compile_times is not None:
            output_parts.append(f"Compile Times: {self.compile_times}\n")
