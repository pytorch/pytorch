...
def _generic_device_system_info_comment(device_type: str) -> str:
    """
    Best-effort system info comment for non-CUDA accelerator devices.
    Display-only: every failure degrades to comment text so that debug
    tooling never breaks repro generation.
    """
    try:
        lines: list[str] = []
