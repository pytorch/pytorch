...
class GuardsStatePickler(pickle.Pickler):
    ...
    @staticmethod
    def _unpickle_type_parameter(kind: str, name: str) -> object:
        import typing

        constructor = getattr(typing, kind)
        if kind in ("TypeVar", "ParamSpec"):
            return constructor(name, infer_variance=True)
    ...
    def reducer_override(
        ...
        if isinstance(obj, torch.Tensor) and obj.device.type != "meta":
            ...
            type(obj).__module__ == "typing"
            and type(obj).__name__ in ("TypeVar", "TypeVarTuple", "ParamSpec")
        ):
            return type(self)._unpickle_type_parameter, (
                type(obj).__name__,
                obj.__name__,
            )
