...
class GuardsStatePickler(FunctionPicklerBase):
    ...
    def reducer_override(
        ...
        if isinstance(obj, torch.Tensor) and obj.device.type != "meta":
            ...
            pytype = type(obj)
            # _dispatch_keys() on a fake reports the Python/PythonTLSSnapshot
            # (or Fake) keys of the fake itself; the converter may have
            # recorded the real tensor's keys (from_meta_and_device always
            # does, from_real_tensor only for an mkldnn source).
            ...
            if isinstance(  # noqa: ISINSTANCE_FAKE_TENSOR
                ...
                template_ctx = torch._C._ExcludeDispatchKeyGuard(
                    torch._C.DispatchKeySet(torch._C.DispatchKey.Fake)
                )

            # A fake answers empty_like with another fake (a Python fake through
            # its own __torch_dispatch__, whether or not its FakeTensorMode is
            # active), and that fake would drag the mode and its converters
            # into the pickle; template_ctx makes the template a plain meta
            # tensor.
            with template_ctx:
