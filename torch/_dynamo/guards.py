...
class GuardsStatePickler(FunctionPicklerBase):
    ...
    # default -- is pruned per value, since those guards survive it. Being
    # registered is not what decides this: a whole-tuple EQUALS_MATCH and a
    # per-element guard through the same tuple coexist in ordinary code (a
    ...
    def _reduce_function_by_value(self, obj: types.FunctionType) -> tuple[Any, ...]:
        ...
        if closure is not None:
            # No _keep_container_verbatim gate like the other containers: a
            # guard through __closure__ always ends at cell_contents, which
            # registers the contents _prune_cell keys on, so per-cell pruning
            # already keeps what is read, and _prune_cell is length-preserving.
...
def _guards_type_of_random_value(guard: Guard) -> bool:
    if guard.create_fn_name() == "TYPE_MATCH":
        return True
    source: Source | None = guard.originating_source
    while isinstance(source, ChainedSource):
        if isinstance(source, TypeSource) or (
            isinstance(source, AttrSource) and source.member == "__class__"
        ):
            return True
        source = source.base
    return False
...
def install_guard(*guards: Guard, skip: int = 0) -> None:
    ...
    for guard in guards:
        ...
        if is_from_skip_guard_source(guard.originating_source):
            continue
        # Random values are regenerated by the prologue on every call and are
        # not reachable from the frame's locals or globals. Their type is
        # fixed, so type guards are dropped; any other guard would freeze the
        # value.
        if guard.originating_source.guard_source is GuardSource.RANDOM_VALUE:
            if not _guards_type_of_random_value(guard):
                unimplemented(
                    gb_type="Guard on a runtime random value",
                    context=f"{guard.create_fn_name()} on {guard.name}",
                    explanation="Random values are regenerated on every call, so "
                    "code that depends on a specific value cannot be compiled.",
                    hints=["Avoid specializing on the result of a random call."],
                )
            continue
