...
starts_line_log = torch._logging.getArtifactLogger(__name__, "starts_line")
...
class InstructionTranslatorBase(Checkpointable[InstructionTranslatorGraphState]):
    ...
    def log_starts_line(self):
        ...
        starts_line_log.debug(
            "TRACE starts_line %s%s %s:%s",
            inline_depth_str,
            self.f_code.co_name,
            self.f_code.co_filename,
            self.lineno,
        )
        line = linecache.getline(self.f_code.co_filename, self.lineno)
        if line:
            # strip the newline at the end
            line = line[:-1]
        starts_line_log.debug("    %s", (" " * 4 * self.inline_depth) + line)
