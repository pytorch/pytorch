...
class InstructionTranslatorBase(
    ...
    def import_source(
        ...
        if alias in f_globals and f_globals[alias] is not value:
            ...
            # The codegen callers keep the hard error because an Unsupported there
            # is not a graph break but a frame skipped after the backend ran, silent
            # at default log levels; the error is loud but unlocated, codegen running
            # under TracingContext.clear_frame, which attaches no user stack. The
            # tracing-phase callers keep it too, for now: whether they should break
            # instead is deferred, so for a module a frame both imports and inlines
            # from, whichever caller gets here first decides.
