...
# Out-of-tree backends are reached through the torch.accelerator channel,
# not by name, so the fallback chain only lists in-tree devices.
_REPRO_DEVICE_PROBE_ORDER: tuple[str, ...] = ("cuda", "xpu", "mps")
...
def _probe_default_device() -> str:
    ...
    register themselves there), then falls back to
...
def _normalize_device_type(device_type: str) -> str:
    """Reduce any device-like string (``"xpu:0"``) to a bare type (``"xpu"``).

    ``torch.device`` rejects type names that no compiled/registered backend
    owns (e.g. an unregistered backend name), so fall back to the
...
def _generic_device_system_info_comment(device_type: str) -> str:
    ...
    built-in and out-of-tree device modules alike.
    """
    module = getattr(torch, device_type, None)
    is_available = getattr(module, "is_available", None)
