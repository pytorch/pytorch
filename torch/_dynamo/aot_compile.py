...
class AOTCompiledFunction:
    ...
    def _seed_guard_scope(
        ...
        # Two dicts can be holding that recording, and only on the default path
        # are they one: the guard scope, and fn.__globals__, which is what the
        # generated bytecode subscripts. A caller-supplied guard scope is not the
        # bytecode's, so re-deriving only there would leave the guards reading the
        # live builtins while the bytecode went on reading the recording. Visit
        # fn.__globals__ only when there is a recording to replace -- it needs no
        # seeding of its own -- and derive each dict from its OWN __builtins__, so
        # a guard-only scope still cannot rewire what the bytecode resolves.
    ...
    def _serve(self, *args: Any, **kwargs: Any) -> Any:
        ...
        from there, and the guard rooted at it refuses the call anyway.
        ...
        The write goes into ``self.fn.__globals__``, which every call shares, so
        an artifact served from several threads hands each graph whatever the
        last write left there; load one per thread if that matters."""
...
class AOTCompiledModel:
        ...
        def warn_swallowed(served: int) -> None:
            ...
            for i, e in raised.items():
                ...
                log.warning(
                    ...
                    "stale, so its next check can reject a call it fits or "
    ...
    def _no_match_report(
        ...
        if covered:
            ...
            # withheld line has already said what happened, and an artifact
            # holding no inputs at all -- which deserialize() accepts -- has no
            # raise to describe either.
            lines.append(
