    ...
    def codegen_save_tempvars(self, cg: PyCodegen):
        ...
        for var in self._get_modified_vars():
            if isinstance(var.mutation_type, AttributeMutationNew) and isinstance(
                ...
                # Tensor variables are a bit special, since they will be
                # reconstructed from the output of the fx graph (see special
                # cased logic in PyCodegen). However, we still need to handle
                # them here for attribute mutations.
                if isinstance(var, variables.TensorVariable):
                    # NOTE: for historical reasons we never assigned local
                    # sources to newly constructed tensor object or added them
                    # to codegen cache (the PyCodegen logic also slightly
                    # depends on this). TODO fix this for cleaner code and
                    # cleaner + faster generated bytecode.
                    cg(var)
                else:
                    ...
                    cg.extend_output(
                        create_call_function(1 + len(var.init_args), False)
                    )
