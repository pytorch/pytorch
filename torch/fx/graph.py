...
class PythonCode:
    ...
    src: str
    # Values in global scope during execution of `src`.
    globals: Dict[str, Any]
    # Maps nodes in the fx graph to line offsets in `src`. For FX internal use only.
    _offset_to_stack: Optional[Dict[int, Optional[List[str]]]] = None
    ...
    def _gen_python_code(self, nodes, root_module: str, namespace: _Namespace, *, verbose: bool = False) -> PythonCode:
        ...
        def delete_unused_values(user : Node):
            ...
            if len(nodes_to_delete):
                ...
                # Append the deletion to the end of the current line
                body[-1] = body[-1] + f';  {to_delete_str}'
        ...
        def emit_node(node : Node) -> int:
            """Returns the number of lines emitted for this node, to compute line offsets"""
        ...
        offset_to_stack = {}
        for node in nodes:
            offset_to_stack[node_offset] = node.meta.get("stack_trace")
            if verbose:
                append_stacktrace_summary(node)
            len_before = len(body)
            lines_added = emit_node(node)
            # NOTE: we expect delete_unused_values to add no additional lines
            delete_unused_values(node)
            assert len(body) == len_before + lines_added
            node_offset += lines_added
        ...
        if len(wrapped_fns) > 0:
            ...
            wrap_stmts = [f'{wrap_name}("{name}")' for name in wrapped_fns]
        ...
        fn_code = "\n" + "\n".join(wrap_stmts + [prologue] + body)
        return PythonCode(fn_code, globals_, offset_to_stack)
