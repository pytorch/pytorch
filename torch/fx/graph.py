...
class Graph:
    ...
    def erase_node(self, to_erase : Node) -> None:
        ...
        if to_erase._erased:
            warnings.warn(f"erase_node({to_erase}) on an already erased node", stacklevel=1)
            return
    ...
    def call_module(self,
        ...
        if (self.owning_module and
            ...
            warnings.warn("Attempted to insert a call_module Node with "
                          ...
                          "GraphModule! Call "
                          "GraphModule.add_submodule to add the "
                          "necessary submodule", stacklevel=1)
                    ...
                    for i, atom in enumerate(target_atoms):
                        ...
                        if (node.op == "call_module"
                            ...
                            warnings.warn(f'Node {node} target {node.target} {atom} of {seen_qualname} does '
                                          ...
                                          'what \'get_attr\' Nodes typically target', stacklevel=1)
                        else:
                            m_itr = new_m_itr
