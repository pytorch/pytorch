...
class GraphTransformObserver:
    ...
    def __exit__(self, type, value, tb):
        ...
        for node in self.erased_nodes:
            self.input_dot_graph.get_node(node.name).attrs['color'] = 'red'
        self.input_dot_graph.write_pdf(os.path.join(self.log_dir, f'{self.passname}_input_graph.pdf'))

        from .passes.graph_drawer import FxGraphDrawer
        output_dot_graph = FxGraphDrawer(self.graphModule, self.passname).get_dot_graph()
        for node in self.created_nodes:
            output_dot_graph.get_node(node.name).attrs['color'] = 'red'
        output_dot_graph.write_pdf(os.path.join(self.log_dir, f'{self.passname}_output_graph.pdf'))
...
class Graph:
    ...
    def erase_node(self, to_erase : Node) -> None:
        ...
        if self.transfrom_observer is not None:
            self.transfrom_observer.on_node_erase(n)
