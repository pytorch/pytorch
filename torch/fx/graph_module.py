...
class GraphModule(torch.nn.Module):
    ...
    def __new__(cls: 'Type[GraphModule]', *args, **kwargs):
        ...
        # If possible, change the calling function name to match the
        # user's original code for better error reporting
        parent_frame = inspect.currentframe().f_back
        calling_name = cls.__name__
        if "root" in parent_frame.f_locals:
            calling_name = parent_frame.f_locals["root"].__class__.__name__
            if calling_name == "function":
                calling_fn_name = parent_frame.f_locals["root"].__name__
                calling_name = "nnModule_with_autogenerated_forward_wrapping_user_function_" + calling_fn_name
        GraphModuleImpl.__name__ = calling_name
