...
PyObject* replacement_method(PyObject* self, PyObject* args, PyObject* kwargs) {
    // restore the implementation immediately so that patch_fn lives for as little as possible
    ToRestore* to_restore = (ToRestore*) PyBytes_AsString(self);
    PyCFunctionObject* patch_method_c = ((PyCFunctionObject*)to_restore->patched_method);
    ...
    PyObject* args_ = Py_BuildValue("(OOO)", to_restore->patched_method, args, kwargs);
    if (!args_) {
        goto exit;
    }
...
static PyMethodDef ReplacementMethod = {"replace", (PyCFunction)(void(*)(void)) replacement_method, METH_VARARGS | METH_KEYWORDS, "Replaced method implementation."};

static PyObject * patch_function(PyObject *self, PyObject *args) {
    ToRestore to_restore;
    if (!PyArg_ParseTuple(args, "OO", &to_restore.patched_method, &to_restore.patch_fn)) {
        return nullptr;
    }
    ...
    patch_method_c->m_self = PyBytes_FromStringAndSize((const char*)&to_restore, sizeof(ToRestore));
...
PyMODINIT_FUNC PyInit_patch(void) {
    return PyModule_Create(&path);
}
