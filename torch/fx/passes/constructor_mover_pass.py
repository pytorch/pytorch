...
class ConstructorMoverPass(PassBase):
    ...
    def gather_constructors_and_target_devices(
        ...
        for node in graph.nodes:
            ...
            # there's a device keyword-argument and it is "cpu".
            if not node.kwargs.get("device").type == "cpu":
                continue
    ...
    def find_movable_constructors(
        ...
        while queue:
            ...
            for user in node.users:
                ...
                # We've reached user through a constructor.
                # Decrement the in-degree.
                cpu_indeg[user] -= 1

                # If all constructors have reached user, enqueue it.
                if cpu_indeg[user] == 0:
                    del cpu_indeg[user]

                    # this node was used on a op which takes in multiple devices and output a tensor
                    # on the target device. We can convert its cpu input to the target device without
                    ...
                    if not (
                        ...
                        # Enqueue only if the result of user is still a CPU tensor.
                        queue.append(user)
    ...
    def call(self, graph_module: torch.fx.GraphModule) -> Optional[PassResult]:
        ...
        self.move_constructors_to_device(movable_constructors, target_device)
