...
class _SplitterBase:
        ...
        for t, dtypes in supported_node_types.items():
            for arg_dtypes_tuple, kwarg_dtypes_tuple in dtypes:
                reports_parts.append(
                    f"{t}: ({arg_dtypes_tuple}, {dict(kwarg_dtypes_tuple)})\n"
                )
    ...
    def split_preview(self, dump_graph: bool = False):
        ...
        reports_parts.append(
            f"Before removing small acc subgraphs, total {len(subgraphs)} subgraphs are created:"
        )
        reports_parts.append(
            f" {acc_subgraphs_num} acc subgraphs and {cpu_subgraphs_num} cpu subgraphs.\n"
        )
        ...
        cpu_subgraphs_num = len(subgraphs) - acc_subgraphs_num
        reports_parts.append(
            f"After removing small acc subgraphs, total {len(subgraphs)} subgraphs are created:"
        )
        reports_parts.append(
            f" {acc_subgraphs_num} acc subgraphs and {cpu_subgraphs_num} cpu subgraphs.\n"
        )
        ...
        for i, subgraph in enumerate(subgraphs):
            ...
            reports_parts.append(f"{len(subgraph.nodes)} node(s)\n")
        ...
        for node in split_mod.graph.nodes:
            if node.op == "call_module" and "acc" in node.target:
                ...
                reports_parts.append("Checking inputs...\n")
                for n in submod.graph.nodes:
                    if n.op == "placeholder":
                        if not is_node_output_tensor(n):
                            reports_parts.append(
                                f"Input {n.name} is not a tensor, this might cause problems during lowering!\n"
                            )
                        else:
                ...
                reports_parts.append("Checking outputs...\n")

                def get_bytes(node: torch.fx.Node):
                    nonlocal total_output_bytes
                    nonlocal reports_parts
                    if not is_node_output_tensor(node):
                        reports_parts.append(
                            f"Output {node.name} is not a tensor, this might cause problems during lowering!\n"
                        )
                    else:
                ...
                reports_parts.append(
                    f"Total input size in bytes is {total_input_bytes}, total output size in bytes is {total_output_bytes},"
                )
                reports_parts.append(
                    f" theoretical max qps (bounds by PCIe bandwidth) for this submodule is {qps}.\n"
                )
                ...
                try:
                    ...
                    reports_parts.append("Run into an error during lowering!\n")
                    reports_parts.append(self._find_culprit(submod, submod_inputs))
                    continue

                try:
                    lowered_submod(*submod_inputs)
                except RuntimeError:
                    reports_parts.append("Run into an error during inference!\n")
                    reports_parts.append(self._find_culprit(submod, submod_inputs))
                else:
                    reports_parts.append("Lowering and running succeed!\n")

        reports_parts.append(
            f"\nTheoretical max qps (bounds by PCIe bandwidth) for this model is {max_qps},"
        )
        reports_parts.append(f" bottleneck is submodule {bottleneck_module}.")
