...
def split_module(
    ...
    # Super Nodes are nodes which by their global state effects,
    ...
    for node in m.graph.nodes:
        ...
        if node.op == "call_function" and node.target in SUPER_NODE_TARGETS:
    ...
    for node in m.graph.nodes:
        ...
        if node.target not in SUPER_NODE_TARGETS:
            torch.fx.graph.map_arg(
