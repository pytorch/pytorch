...
class TestMoveNonTensorNodesOnBoundary(unittest.TestCase):
    ...
    def _create_mock_node(self, name: str, op: str, target=None, is_tensor: bool = True) -> torch.fx.Node:
        ...
        # Mock users list (this can be set directly)
        node.users = []

        # Initialize the mock input nodes that we'll patch later
        node._mock_input_nodes = []

        return node

    def test_move_non_tensor_nodes_basic_case(self):
        ...
        node1.users = [node2, node3]
        node2._mock_input_nodes = [node1]
        node3._mock_input_nodes = [node1]
        ...
        with patch('torch.fx.passes.split_utils.is_node_output_tensor') as mock_is_tensor:
            ...
            with patch.object(type(node2), 'all_input_nodes', new_callable=PropertyMock) as mock_node2_inputs, \
                 patch.object(type(node3), 'all_input_nodes', new_callable=PropertyMock) as mock_node3_inputs:
                mock_node2_inputs.return_value = node2._mock_input_nodes
                mock_node3_inputs.return_value = node3._mock_input_nodes
    ...
    def test_no_movement_for_tensor_nodes(self):
    ...
    def test_no_movement_for_non_acc_subgraph(self):
    ...
    def test_multiple_target_subgraphs_no_movement(self):
    ...
    def test_dependency_chain_movement(self):
        ...
        node2.users = [node3]
        node3.users = [node4]
        node1._mock_input_nodes = []  # node1 has no inputs
        node2._mock_input_nodes = [node1]
        node3._mock_input_nodes = [node2]
        node4._mock_input_nodes = [node3]
        ...
        with patch('torch.fx.passes.split_utils.is_node_output_tensor') as mock_is_tensor:
            ...
            with patch.object(type(node1), 'all_input_nodes', new_callable=PropertyMock) as mock_node1_inputs, \
                 patch.object(type(node2), 'all_input_nodes', new_callable=PropertyMock) as mock_node2_inputs, \
                 patch.object(type(node3), 'all_input_nodes', new_callable=PropertyMock) as mock_node3_inputs, \
                 patch.object(type(node4), 'all_input_nodes', new_callable=PropertyMock) as mock_node4_inputs:
                mock_node1_inputs.return_value = node1._mock_input_nodes
                mock_node2_inputs.return_value = node2._mock_input_nodes
                mock_node3_inputs.return_value = node3._mock_input_nodes
                mock_node4_inputs.return_value = node4._mock_input_nodes
                ...
                self.assertLessEqual(len(subgraph1.nodes), 3)  # Some nodes should be moved
                self.assertGreaterEqual(len(subgraph2.nodes), 1)  # At least node4 should be there

    def test_parent_node_processing(self):
        ...
        parent.users = [node1]
        node1.users = [child]
        parent._mock_input_nodes = []  # parent has no inputs
        node1._mock_input_nodes = [parent]
        child._mock_input_nodes = [node1]
        ...
        with patch('torch.fx.passes.split_utils.is_node_output_tensor') as mock_is_tensor:
            ...
            with patch.object(type(parent), 'all_input_nodes', new_callable=PropertyMock) as mock_parent_inputs, \
                 patch.object(type(node1), 'all_input_nodes', new_callable=PropertyMock) as mock_node1_inputs, \
                 patch.object(type(child), 'all_input_nodes', new_callable=PropertyMock) as mock_child_inputs:
                mock_parent_inputs.return_value = parent._mock_input_nodes
                mock_node1_inputs.return_value = node1._mock_input_nodes
                mock_child_inputs.return_value = child._mock_input_nodes
                ...
                self.assertIn(child, subgraph2.nodes)  # child should remain in subgraph2
                # Allow flexibility in how many nodes are moved based on algorithm behavior
                self.assertLessEqual(len(subgraph1.nodes), 2)  # Some nodes should be moved
