...
# Note that Pattern-matching is done based on use-def relationships,
...
# for the two instances of `torch.cat([w1, w2]).sum()` above. The last
# statement in the pattern must be returned by the Callable.
def pattern(a1, a2):
    val1 = torch.neg(a1)
