...
def add_inference_rule(n: Node, symbols, constraints, counter):
    ...
    if isinstance(n.args[0], Node) and isinstance(n.args[1], Node):
        if isinstance(symbols[n.args[0]], TVar) and isinstance(symbols[n.args[1]], TVar):
            my_outut, counter = gen_tvar(counter)
            symbols[n] = my_outut
            e1 = symbols[n.args[0]]
            e2 = symbols[n.args[1]]

            return gen_broadcasting_constraints(e1, e2, symbols, counter, my_outut)
        else:
            raise NotImplementedError('Method not yet implemented')
        ...
        if isinstance(symbols[n.args[0]], TVar):
            ...
            symbols[n] = my_outut
            e1 = symbols[n.args[0]]
            return [BinConstraintT(my_outut, e1, op_eq)], counter
        elif isinstance(symbols[n.args[0]], DVar):
            my_outut, counter = gen_dvar(counter)
            symbols[n] = my_outut
            e1 = symbols[n.args[0]]

            # we will propagate the runtime value here since this is regular addition
            c = Conj([BinConstraintD(my_outut, BinConstraintD(e1, n.args[1], op_add), op_eq),
                      BinConstraintD(0, my_outut, op_leq)])
            return [c], counter
        ...
        if isinstance(symbols[n.args[1]], TVar):
            ...
            # we will propagate the runtime value here since this is regular addition
            c = Conj([BinConstraintD(my_outut, BinConstraintD(e2, n.args[0], op_add), op_eq),
                      BinConstraintD(0, my_outut, op_leq)])
            return [c], counter
