...
def gen_tvar(curr: int) -> tuple[TVar, int]:
    ...
    """
    curr += 1
    return TVar(curr), curr  # type: ignore[no-untyped-call]
...
def gen_dvar(curr: int) -> tuple[DVar, int]:
    ...
    """
    curr += 1
    return DVar(curr), curr  # type: ignore[no-untyped-call]
...
def gen_bvar(curr: int) -> tuple[BVar, int]:
    ...
    """
    curr += 1
    return BVar(curr), curr  # type: ignore[no-untyped-call]
...
def gen_nat_constraints(list_of_dims: list[DVar]) -> list[BinConstraintD]:
    ...
    """
    return [BinConstraintD(0, d, op_leq) for d in list_of_dims]  # type: ignore[no-untyped-call]
