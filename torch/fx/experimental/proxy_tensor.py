...
def wrap_key(f, tensors, tracer, pre_dispatch: bool):
    ...
    def wrapped(*proxies):
        ...
        out = pytree.tree_map_only(
            (SymInt, SymFloat, SymBool), lambda t: get_proxy_slot(t, tracer)(), out
        )
        return out
...
class _ModuleStackTracer(PythonKeyTracer):
    ...
    def getattr(self, attr, attr_val, parameter_proxy_cache):
        if not isinstance(attr_val, torch.nn.Module) or isinstance(
            attr_val, torch.fx.GraphModule
        ):
    ...
    *,
    ...
    def wrapped(*args):
        ...
        with decompose(
            ...
            pre_dispatch_mode
        ), proxy_function_mode, sym_mode, proxy_mode, disable_autocast_cache():
            t = dispatch_trace(
