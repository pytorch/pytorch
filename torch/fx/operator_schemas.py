...
def create_type_hint(x):
    try:
        ...
        warnings.warn(f"We were not able to successfully create type hint from the type {x}", stacklevel=1)
        pass
    return x
...
def type_matches(signature_type : Any, argument_type : Any):
    ...
    if getattr(signature_type, '__origin__', None) in {list, List}:
        ...
        if not inspect.isclass(sig_el_type):
            warnings.warn(
                f"Does not support nested parametric types, got {signature_type}. Please file a bug.", stacklevel=1)
            return False
