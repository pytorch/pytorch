...
# The emitted inductor code binds one buffer allocator per device type
# (empty_strided_cuda, empty_strided_cpu, empty_strided_cpu_pinned, ...). Rebinding
# them in the artifact's namespace is what lets a donating call hand the generated
# code memory instead of letting it allocate (see _BufferDonationPool). Matched by
# prefix rather than by an explicit list because the pool replays allocations BY
# ORDER: missing one device's allocator would desync the whole plan.
...
class _BufferDonationPool:
    ...
    The emitted ``call`` gets every buffer it needs from a module-level
    ``empty_strided_*`` in the artifact's own namespace, so rebinding that name is
    enough to hand it memory rather than let it allocate. The first donating call
    records the ordered requests and which of them come back as graph outputs; later
    donating calls replay that recording, serving scratch buffers from the pool and
    outputs from the caller, so a steady-state donating call allocates nothing.
    ...
    def __init__(self, ns: dict[str, Any]) -> None:
        ...
        if not allocators:
            raise _precompile_error(
                ...
                "buffers through the inductor allocator, which this artifact does not "
                "(backend='eager' emits a bare graph call). Use backend='inductor', or "
                "drop out=."
            )
