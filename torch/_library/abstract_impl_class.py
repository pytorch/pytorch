...
class AbstractClassRegistry:
    ...
    def _check_registered(self, full_qualname: str):
        if full_qualname not in self._registered_class:
            raise RuntimeError(f"{full_qualname} is not registered. Please use impl_abstract_class to register it first.")
...
def impl_abstract_class(qualname, fake_class=None):
    def inner(fake_class):
        ...
        global_abstract_class_registry.register(_full_qual_class_name(qualname), fake_class)
        return fake_class
