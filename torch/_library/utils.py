...
def check_aliasing_constraint(name, prev, result, get_module=lambda: "???"):
    ...
    """
    storages = {t.untyped_storage().cdata for t in prev if isinstance(t, torch.Tensor)}
    tuple_result = result
    if not isinstance(result, tuple):
        tuple_result = (result,)
    for tensor in iter_tensors(tuple_result, {}):
        key = tensor.untyped_storage().cdata
        if tensor.untyped_storage().cdata in storages:
            raise RuntimeError(
