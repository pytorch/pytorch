...
from torch import _SymExprHash, SymInt
...
class FakeTensorConverter:
    ...
    def invalidate_constant_aliases(self, tensor: torch.Tensor) -> None:
    ...
    def _get_memo(self, t: torch.Tensor) -> Optional["FakeTensor"]:
        tid = self.meta_converter.describer.lookup_tensor.get(t)
        if tid is None:
            return None
        return self.tensor_memo.get(tid)

    def set_tensor_memo(self, t: torch.Tensor, v: "FakeTensor") -> None:
...
class FakeTensor(torch.Tensor):
    ...
    def __new__(
        ...
        real_tensor: Optional[torch.Tensor] = None,
    ) -> Self:
    ...
    @staticmethod
    def from_tensor(t: torch.Tensor, fake_mode: "FakeTensorMode") -> "FakeTensor":
    ...
    def _find_common_device(
        ...
        def cpu_zero_dim(t: torch.Tensor) -> bool:
    ...
    def tolist(self) -> List[torch.SymInt]:
...
class TensorMetadata:
    ...
    dtype: torch.dtype
    shape: Tuple[Union[int, _SymExprHash], ...]
    stride: Tuple[Union[int, _SymExprHash], ...]
    device: torch.device
    layout: torch.layout
...
def extract_tensor_metadata(t: torch.Tensor) -> "TensorMetadata":
    ...
    def convert_to_sym_hash(x: Union[int, SymInt]) -> Union[int, _SymExprHash]:
        if isinstance(x, torch.SymInt):
            return _SymExprHash(x)
        else:
            return x

    stride_ = t.stride() if t.layout == torch.strided else ()
    if t._has_symbolic_sizes_strides:
        shape: Tuple[Union[int, _SymExprHash], ...] = tuple(
            convert_to_sym_hash(x) for x in t.shape
        )
...
class FakeTensorMode(TorchDispatchMode):
    ...
    def _prep_args_for_hash(
        ...
        for arg in args:
            if isinstance(arg, FakeTensor):
                ...
                result.append(_SymExprHash(arg))
    ...
    def _output_from_cache_entry(
        ...
        def convert_from_sym_hash(x: Union[int, _SymExprHash]) -> Union[int, SymInt]:
            if isinstance(x, _SymExprHash):
                return cast(SymInt, x.sym_obj)
            else:
                return x

        if metadata.has_symbolic_sizes_strides:
            shape: Sequence[Union[int, SymInt]] = tuple(
                convert_from_sym_hash(x) for x in metadata.shape
            )
            stride: Sequence[Union[int, SymInt]] = tuple(
                convert_from_sym_hash(x) for x in metadata.stride
            )
        else:
            shape = cast(Sequence[int], metadata.shape)
            stride = cast(Sequence[int], metadata.stride)

        storage_bytes = metadata.storage_bytes and convert_from_sym_hash(
        ...
        if torch.fx.experimental.symbolic_shapes.guard_size_oblivious(
            storage_offset != 0
        ):
    ...
    def _dispatch_impl(
        ...
        def maybe_to_real_tensor(t: T) -> Optional[Union[T, torch.Tensor]]:
            if isinstance(t, FakeTensor):
        ...
        def maybe_propagate_real_tensors(fake_out: T) -> T:
            ...
            def go(t: object, real_t: torch.Tensor) -> None:
                if isinstance(t, FakeTensor):
    ...
    def may_turn_const(self, t: torch.Tensor) -> bool:
        return (
...
def run_fallback_kernel(
    ...
    with no_dispatch():
        ...
        def to_real_tensor(e: T) -> Union[T, torch.Tensor]:
...
_DISPATCH_META_HANDLERS = {
    ...
    torch.ops.aten.size.default: lambda args: tuple(
        int(s) for s in cast(torch.Tensor, args[0]).size()
    ),
    torch.ops.aten.stride.default: lambda args: tuple(
        int(s) for s in cast(torch.Tensor, args[0]).stride()
    ),
    torch.ops.aten.storage_offset.default: lambda args: int(
        cast(torch.Tensor, args[0]).storage_offset()
    ),
}
