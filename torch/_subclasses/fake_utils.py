...
class CrossRefFakeMode(TorchDispatchMode):
    ...
    def __torch_dispatch__(self, func, types, args=(), kwargs=None):
        ...
        if fake_r is not None:
            r_flat = tree_flatten(r)[0]
            f_flat = tree_flatten(fake_r)[0]
            assert len(r_flat) == len(
                r_flat
            ), f"Mismatch {len(r_flat)} != {len(r_flat)} on {func}"

            if self.check_aliasing:
                assert outputs_alias_inputs(r, (args, kwargs)) == outputs_alias_inputs(
                    fake_r, (fake_args, fake_kwargs)
                ), f"Mismatch on {func}"
                assert outputs_are_inputs(r, (args, kwargs)) == outputs_are_inputs(
                    fake_r, (fake_args, fake_kwargs)
                ), f"Mismatch on {func}"
