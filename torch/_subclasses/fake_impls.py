...
def infer_size(a, b):
    ...
    for i in range(ndim - 1, -1, -1):
        ...
        # to be statically known, but the sizeA == sizeB test may not be
        # statically known. However, once we have established that no
        # broadcasting is happening, the sizeA == sizeB is now expect_true
        # and we can defer it as a runtime assert (this works because Python
        # will return the terminal expression of an or statement as-is,
        # without bool()'ing it; if this were not the case, we'd need to write
        # this using torch.sym_or() or something like that).
        torch._check(
