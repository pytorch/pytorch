...
def _output_mask(op, input: Tensor, *args, **kwargs) -> Tensor:
    ...
    """
    if callable(op):
        is_reduction = op.__name__ in {'sum', 'prod', 'amax', 'amin', 'argmax', 'argmin', 'mean', 'median', 'norm', 'var', 'std'}
...
{reduction_example}"""
    if dtype is None:
        dtype = input.dtype
    if input.layout == torch.strided:
        identity = input.new_full([], _reduction_identity('argmax', input))
        inmask = _input_mask(input, mask=mask)
        mask_input = torch.where(inmask, input, identity)
...
{reduction_example}"""
    if dtype is None:
        dtype = input.dtype
    if input.layout == torch.strided:
        identity = input.new_full([], _reduction_identity('argmin', input))
        inmask = _input_mask(input, mask=mask)
        mask_input = torch.where(inmask, input, identity)
