...
def _replace_with_hop(node: torch.fx.Node):
    ...
    if len(set_grad_nodes) > 0:
        ...
        with graph.inserting_before(node):
            ...
            call_func_node = graph.call_function(wrap_with_set_grad_enabled, (enable_grad, get_attr_node, *node.args))
        node_replace_(node, call_func_node, delete_old=True)


def replace_set_grad_with_hop_pass(gm: torch.fx.GraphModule):
    new_gm = sequential_split(gm, _is_set_grad_enabled_node)
    call_module_nodes = nodes_filter(new_gm.graph.nodes, functools.partial(_is_set_grad_enabled_sub_mod, omit_if_same_with_ambient=True))
