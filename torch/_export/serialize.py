...
_DTYPES_MAPPING = _reverse_map(_SCALAR_TYPES_MAPPING)
...
def export_tensor_meta(t: torch.Tensor) -> TensorMeta:
    ...
    """
    return TensorMeta(
        dtype=_SCALAR_TYPES_MAPPING[t.dtype],
        ...
        storage_offset=0,
        layout=_LAYOUT_MAPPING[t.layout],
    )
...
class Serializer:
    ...
    def export_input(self, arg) -> Argument:
        if isinstance(arg, torch.fx.Node):
            ...
            return Argument.create(as_scalar_type=_SCALAR_TYPES_MAPPING[arg])
        elif isinstance(arg, torch.device):
            return Argument.create(as_device=Device(type=arg.type, index=arg.index))
        elif isinstance(arg, torch.memory_format):
            return Argument.create(as_memory_format=_MEMORY_FORMAT_MAPPING[arg])
        elif isinstance(arg, torch.layout):
            return Argument.create(as_layout=_LAYOUT_MAPPING[arg])
        else:
    ...
    def export_outputs(self, node: torch.fx.Node) -> List[Argument]:
        ...
        if len(returns) == 1 and isinstance(returns[0].real_type, torch.TensorType):
