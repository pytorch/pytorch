...
class _Package:
    def __init__(
        ...
        *,
        ...
        if os.path.exists(self.path) and os.path.isdir(self.path):
            raise RuntimeError(f"File {self.path} already exists as a directory. Please specify a file path.")
        self.frontend = frontend
        self._compilations = {}

    def __call__(self, model, name: str, version: int):
        ...
        def _(*args, **kwargs):
            ...
            if self.mode == "package":
                ...
                path = torch._inductor.aot_compile(ep.module(), args, kwargs, options=options)
                ...
                raise RuntimeError(f"Unknown mode {self.mode}. Supported options: package, load")
    ...
    def finalize(self):
        torch._inductor.package.package_aoti(self.path, {c.name: c.path for c in self._compilations.values()})
        self._compilations.clear()


_optimized_versions = weakref.WeakKeyDictionary()

def _bump_optimized_version(obj) -> int:
