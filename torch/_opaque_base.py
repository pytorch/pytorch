            ...
            if active_fake_mode() is not None:
                ...
                if get_proxy_mode() is not None:
                    raise RuntimeError(
                        ...
                        f"before torch.compile and pass it as an argument. If the "
                        f"creation is safe to specialize, wrap it in "
                        f"`torch._subclasses.fake_tensor.unset_fake_temporarily()`. "
                        f"If instances of this type are always safe to create "
                        f"mid-trace, set "
