...
def register_run_dtensor_rng_op():
    ...
    def impl_cuda(start_offset_incr, end_offset_incr, op, *args, **kwargs):
        ...
        # Compute start state: advance offset by shard-specific increment
        start = _PhiloxState(current_state.clone())
        old_offset = start.offset.clone()
        start.offset = old_offset + start_offset_incr

        # Compute end state: advance offset by full DTensor numel
        end = _PhiloxState(current_state.clone())
        end.offset = old_offset + end_offset_incr
        ...
        with torch.random.fork_rng(devices=[device], device_type="cuda"):
            torch.cuda.set_rng_state(start.state)
            out = op(*args, **kwargs)

        torch.cuda.set_rng_state(end.state)
        return out
