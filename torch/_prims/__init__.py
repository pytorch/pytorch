...
def _slice_meta(
    ...
    if a.ndim != len(_strides):
        msg = (
            "Attempting to slice tensor of rank {} with strides of length {}!".format(
                a.ndim, len(limit_indices)
            )
        )
        raise ValueError(msg)
    ...
    for x, y, z in zip(limit_indices, a.shape, start_indices):
        if x < 0:
            msg = "Attempting to slice a tensor with a negative stop index of {}!".format(
                x
            )
            raise ValueError(msg)
        if x > y:
    ...
    for x in _strides:
        if x <= 0:
            msg = (
                "Attempting to slice a tensor with a non-positive step of {}!".format(
                    x
                )
            )
            raise ValueError(msg)
