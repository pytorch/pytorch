...
def get_lane_warp_layouts(tiled_copy, reference_src=True):
    ...
    ref_layout_mn = cute.composition(
        ref_layout, cute.make_layout((tile_m, tile_n))
    )

    num_warps = cute.size(tiled_copy) // cute.arch.WARP_SIZE
    tv_to_lane = cute.make_layout(
        (cute.arch.WARP_SIZE, num_warps, 1), stride=(1, 0, 0)
    )
    ...
    tv_to_warp = cute.make_layout(
        (cute.arch.WARP_SIZE, num_warps, 1), stride=(0, 1, 0)
    )
