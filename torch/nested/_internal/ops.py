import torch
import functools
from .nested_tensor import NestedTensor, is_jagged
...
def check_schema(schema_str: str, func, *args, **kwargs) -> None:
    ...
    for i, named_arg_type in enumerate(named_arg_types):
        ...
        if not normalized_arg_type in arg_type_check_fns.keys():
            raise AssertionError(f"Unknown arg type: {normalized_arg_type}")

        if i >= len(args):
            if not is_optional:
                raise ValueError(f"NestedTensor {func.__name__}({schema_str}) "
...
def jagged_unary_pointwise(func, *args, **kwargs):
    return NestedTensor(func(args[0].buffer, **kwargs), **extract_kwargs(args[0]))

def jagged_binary_pointwise(func, *args, **kwargs):
    check_ragged_dim_same(func, args[0], "lhs", args[1], "rhs")
    return NestedTensor(func(args[0].buffer, args[1].buffer, **kwargs), **extract_kwargs(args[0]))
...
def linear_default(func, *args, **kwargs):
    buffer = torch.mm(args[0].buffer, args[1])
    if len(args) == 3:
        buffer += args[2]
    return NestedTensor(buffer, **extract_kwargs(args[0]))
...
def linear_backward_default(func, *args, **kwargs):
    ...
    ds = NestedTensor(torch.mm(args[1].buffer, args[2].T), **extract_kwargs(args[1]))
    dw = torch.mm(args[0].buffer.T, args[1].buffer)
