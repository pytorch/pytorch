...
def _iter_record_chunks(path: Path):
    ...
    offset = 0
    while offset < len(data):
        magic, version, count, payload_size, min_ts, max_ts = (
            _mon._CHUNK_HEADER.unpack_from(data, offset)
        )
...
def export_monitor_trace(
    ...
    *,
    ...
    base_ns = _trimester_base_ns()
    min_ts = min(_as_int(e["start_ns"]) for e in events)
    max_end_ts = max(_as_int(e["end_ns"]) for e in events)
