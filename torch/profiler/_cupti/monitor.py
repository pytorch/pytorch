...
class CuptiMonitor:
    ...
    def _deliver_pm(
        ...
        if frame is None:
            return
        frame = dict(frame)
        # PM samples are always on the native cuptiGetTimestamp clock (the timestamp callback only
        # restamps activity records), so convert via the native offset, not the record-clock path.
        frame["start_ns"] = self._convert_native_time_array(frame["start_ns"])
        try:
            sink(frame)
        except Exception:
