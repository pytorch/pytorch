...
def extract_gradients(node: _ProfilerEvent) -> Iterator[Tuple[Optional[Key], Key]]:
    ...
    if (
        isinstance(e, _ExtraFields_TorchOp)
        and e.scope == RecordScope.BACKWARD_FUNCTION
        # TODO: Move away from load bearing names
        and node.name == "torch::autograd::AccumulateGrad"
        and children
        and children[0].tag == _EventType.TorchOp
        and children[0].name in ("aten::detach", "aten::add_")
        and children[0].extra_fields.inputs.tensor_metadata
        and children[0].extra_fields.inputs.tensor_metadata[0].id is not None
    ):
    ...
    #       a particular time.
    elif isinstance(e, _ExtraFields_PyCall):
        assert e.module is None or e.optimizer is None
        if e.module is not None:
            for _, p, p_grad in e.module.parameters:
                p_grad_key = Key.from_tensor(p_grad)
                if p_grad_key is not None:
                    yield Key.from_tensor(p), p_grad_key

        if e.optimizer is not None:
            for p, p_grad, _ in e.optimizer.parameters:
                p_grad_key = Key.from_tensor(p_grad)
                if p_grad_key is not None:
                    yield Key.from_tensor(p), p_grad_key
