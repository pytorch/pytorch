...
class SizeMap:
    def __init__(self, op_tree: OpTree) -> None:
        ...
        for node in op_tree.sorted_nodes:
            if node.tag == _EventType.Allocation:
                key = TensorKey.from_allocation(node.extra_fields)
                if key:
                    new_size = abs(node.extra_fields.alloc_size)
    ...
    def _update_values(self, t: Optional[_TensorMetadata]) -> None:
        ...
        if key is not None and t.layout == torch.strided:
