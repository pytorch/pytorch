...
def schedule(
    *, wait: int, warmup: int, active: int, repeat: int = 0, skip_first: int = 0
    ...
    if warmup == 0:
        warn("Profiler won't be using warmup, this can skew profiler results", stacklevel=1)
    return schedule_fn
    ...
    def __init__(
        ...
        *,
        ...
        if use_cuda is not None:
            warn("use_cuda is deprecated, use activities argument instead", stacklevel=1)
            if use_cuda:
