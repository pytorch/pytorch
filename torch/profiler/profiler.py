...
class profile(_KinetoProfile):
    ...
    .. code-block:: python
        et = ExecutionTraceObserver().register_callback("./execution_trace.json") # create an ET observer
        et.set_extra_resource_collection(True) # enable extra resource collection like Triton Kernels
        with torch.profiler.profile(
            ...
    ...
    def __init__(
        ...
        *,
        ...
        super().__init__(
            ...
            execution_trace_observer=execution_trace_observer if execution_trace_observer 
...
class ExecutionTraceObserver(_ITraceObserver):
    ...
    @staticmethod
    def build_execution_trace_obs_from_env() -> "ExecutionTraceObserver":
        """
        ...
        if os.environ.get("ENABLE_PYTORCH_EXECUTION_TRACE", "0") == "1":
            ...
            et.set_extra_resource_collection()
        return None

    def set_extra_resource_collection(self, val=False) -> None:
        """
        ...
        if not val:
            ...
            if os.environ.get("ENABLE_PYTORCH_EXECUTION_TRACE_EXTRAS", "0") == "1":
                self.extra_resources_collection = True
        else:
    ...
    def generate_resources_dir(self) -> Optional[str]:
        """
        ...
        Only works if the observer has called set_extra_resource_collection() with val=True.
        """
        ...
        return self.get_resources_dir_for_et_path(self._output_file_path, create_dir=True)
    
    @staticmethod
    def get_resources_dir_for_et_path(trace_path, create_dir: bool = False) -> Optional[str]:
    ...
    def unregister_callback(self):
        ...
        def _save_triton_kernels():
            try:
                resource_dir = self.generate_resources_dir()
            except Exception as e:
                warn(f"Execution trace exception when generating resource directory: {e}")
                return
            if not resource_dir:
                return
