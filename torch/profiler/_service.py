...
class PyTorchServiceWSGIApp(object):
    ...
    def start_profiling(self, request):
        ...
        if self.is_master_server:
            logger.info("Master server checks status of slave servers before start profiling")
        ...
        if start_threads:
            logger.info("Master server waits for slave servers' profiling to be started")
            for thread in start_threads:
                thread.join()
            self.profiling_started = True
            logger.info("Slave servers' profiling has been stopped")
        else:
            logger.info("Start self profiling")
            self.prof = profile(
            ...
            if warmup_dur > 0:
                ...
                thread = Thread(target=self.start_profiling_with_warmup, args=(warmup_dur, _ThreadLocalState(), ))
                thread.start()
            else:
                self.prof.start()
                self.profiling_started = True
            logger.info("Start self profiling completed")
    ...
    def stop_profiling(self, request):
        ...
        if self.is_master_server:
            logger.info("Master server checks status of slave servers before stop profiling")
        ...
        if stop_threads:
            logger.info("Master server waits for slave servers' profiling to be stopped")
            for thread in stop_threads:
                thread.join()
            logger.info("Slave servers' profiling has been stopped")
        else:
            logger.info("Stop self profiling")
            self.prof.stop()
            logger.info("Stop self profiling completed")
    ...
    def start_child_profiling(self, host, port, request_data):
        logger.info("Start child {}:{} profiling".format(host, port))
        ...
        try:
            ...
            logger.error("Start child {}:{} profiling faild: {}".format(host, port, repr(e)))
        logger.info("Start child {}:{} profiling completed".format(host, port))
    
    def stop_child_profiling(self, host, port):
        logger.info("Stop child {}:{} profiling".format(host, port))
        baseUrl = 'http://{}:{}'.format(host, port)
        try:
            r = requests.put(
                url="/".join([baseUrl, "service"]),
            ...
            logger.error("Stop child {}:{} profiling faild: {}".format(host, port, repr(e)))
        res = r.json()
        if res['success']:
        ...
        logger.info("Stop child {}:{} profiling completed".format(host, port))
