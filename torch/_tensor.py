...
class Tensor(torch._C.TensorBase):
    ...
    def module_load(self, other):
        ...
        After applying ``result = param.module_load(other)``, if ``self`` is an ``nn.Parameter``,
        ...
        .. note::
            If both ``self`` and ``other`` have a ``__torch_function__`` implementation,
            the ``__torch_function__`` implementation of ``self`` takes precedence.

        Args:
            other (Tensor): value in state dict with key corresponding to ``self``

        """
        if has_torch_function({self, other}):
            # handle_torch_function first checks ``self`` for __torch_function__ impl
            # if so it will call this impl and return the result. Otherwise it will
            # check ``other`` for __torch_function__ impl
