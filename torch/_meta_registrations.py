...
def meta_native_multi_head_attention(
    ...
    torch._check( not mask or not query.is_nested, "NestedTensor with mask is not supported yet" )
    torch._check(query.dim() == 3, f"expect 3-D `query`, got {query.dim()}-D tensor")
    torch._check(query.is_nested or query.size()[2] == embed_dim, f"passed-in embed_dim {embed_dim} didn't match last dim of query {query.size()[2]}")
    ...
    torch._check(query.is_nested or key.is_nested or value.is_nested or (query.size() == key.size() and key.size() == value.size()), "expected `query`/`key`/`value` shapes to match")
    torch._check(qkv_weight.dim() == 2, f"expect 2-D `qkv_weight`, got {qkv_weight.dim()}-D tensor")
    torch._check(embed_dim * 3 == qkv_weight.size()[0], f"expected `qkv_weight` first dim to be 3x embed_dim")
    torch._check(embed_dim == qkv_weight.size()[1], f"expected `qkv_weight` second dim to be embed_dim")
    torch._check(qkv_bias.dim() == 1, f"expect 1-D `qkv_bias`, got {qkv_bias.dim()}-D tensor")
    torch._check(qkv_bias.size()[0] == 3 * embed_dim, f"expected `qkv_bias` first dim and first dim of query to be equal")
    torch._check(embed_dim % num_head == 0, "expected `embed_dim` to be divisible by `num_head`")
    ...
    return query.new_empty([B, T, embed_dim]), query.new_empty([B, num_head, T, T])
