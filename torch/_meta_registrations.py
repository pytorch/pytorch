   ...
   # Check dimensions
    ...
    if a_is_2d:
        if b_is_2d:
            ...
            torch._check(offs.size(0) == mat_b.size(0), lambda: "matrix batch sizes have to match")
            out_size = [mat_a.size(0), mat_b.size(-1)]
    else:
        if b_is_2d:
            torch._check(offs.size(0) == mat_a.size(0), lambda: "matrix batch sizes have to match")
            out_size = [mat_a.size(1), mat_b.size(1)]
        else:  # regular bmm
            torch._check(mat_a.size(0) == mat_b.size(0), lambda: "batched dimension has to match")
            out_size = [mat_a.size(0), mat_a.size(1), mat_b.size(-1)]
    
    # Create output tensor with the computed size and dtype
    return mat_a.new_empty(out_size, dtype=out_dtype_)
