...
def meta__mixed_dtypes_linear(input, weight, scale, bias=None, activation=None):
    ...
    torch._check(
        weight.dtype == torch.uint8, lambda: f"weight must be uint8, not {weight.dtype}"
    )
    torch._check(
    ...
    torch._check(
        input.size(-1) == weight.size(0) or input.size(1) == weight.size(0) / 2,
        lambda: (
    ...
    torch._check(
        activation is None or activation in ["relu", "silu"],
        lambda: f"_mixed_dtypes_linear does not support activation {activation}",
    )
    return input.new_empty(
        (
            *input.shape[:-1],
            weight.size(1) * 1 if input.size(-1) == weight.size(0) else 2,
        )
    )
