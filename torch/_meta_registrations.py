...
def _pad1d_backward_common(grad_output, input, padding, *, is_reflection):
    ...
    if not is_reflection:
        check(len(padding) == 2, lambda: "padding size is expected to be 2")
...
def _pad2d_common(input, padding, *, is_reflection):
    ...
    if is_reflection:
        check(
            ...
            lambda: (
                f"Argument #4: Padding size should be less than the corresponding input dimension, "
                f"but got: padding ({pad_l}, {pad_r}) at dimension {dim_w} of input {ndim}"
            ),
        )
        check(
            ...
            lambda: (
                ...
                f"but got: padding ({pad_t}, {pad_b}) at dimension {dim_h} of input {ndim}"
            ),
        )
...
def meta_pad3d_backward(grad_output, input, padding):
    check(len(padding) == 6, lambda: "padding size is expected to be 6")
    assert input.ndim > 3
...
def meta__pdist_forward(self: Tensor, p: float = 2) -> Tensor:
    check(self.is_contiguous(), lambda: "_pdist_forward requires contiguous input")
    n = self.size(0)
    if n <= 1:
