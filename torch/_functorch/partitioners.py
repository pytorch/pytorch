...
def _extract_graph_with_inputs_outputs_special(joint_graph, inputs, outputs, modified_rand_nodes, is_fwd):
    """
    Given a graph, extracts out a subgraph that takes the specified nodes as
    inputs and returns the specified outputs.
        ...
        if node in inputs:
            ...
            if is_fwd:
                if hasattr(node.target, "tags") and torch.Tag.nondeterministic_seeded in node.target.tags:
                    ...
                    env[node] = new_output
                    modified_rand_nodes[node] = state
                else:
                    env[node] = new_graph.node_copy(node, lambda x: env[x])
            else:
                if hasattr(node.target, "tags") and torch.Tag.nondeterministic_seeded in node.target.tags:
    ...
    for x in outputs:
        if isinstance(x, fx.Node):
            ...
            output_values.append(x)
    output_values.extend(rand_states)
    new_graph.output(output_values)

    new_graph.eliminate_dead_code()
    new_graph.lint()
    return new_graph, rand_states
...
def _extract_fwd_bwd_modules_special(joint_module: fx.GraphModule, saved_values, saved_sym_nodes=(), *, num_fwd_outputs):
    ...
    modified_rand_nodes = dict()
    fwd_graph, rand_states = _extract_graph_with_inputs_outputs_special(
        joint_module.graph,
    ...
    bwd_graph, _ = _extract_graph_with_inputs_outputs_special(
        joint_module.graph,
        saved_sym_nodes + saved_values + rand_states + tangent_inputs + bwd_seed_offset_inputs,
        bwd_outputs,
        modified_rand_nodes,
        is_fwd=False,
    )
