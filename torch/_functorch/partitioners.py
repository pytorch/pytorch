...
def should_offload(node: torch.fx.Node) -> bool:
    ...
    Also avoid offloading nodes that would require expensive layout conversions.
    """
    ...
    # Skip offloading for nodes that come from non-contiguous operations
    # to avoid expensive CPU layout conversions
    if hasattr(node, "meta") and "val" in node.meta:
        val = node.meta["val"]
        if isinstance(val, torch.Tensor) and not val.is_contiguous():
            # For now, skip non-contiguous tensors to avoid round-trip layout conversion costs
            # TODO: Make this more sophisticated - could check if the layout conversion cost
            # is worth the memory savings
            return False

    if _size_of(node) < 50 * 1024 * 1024:
        return False
...
def offload_activation_fw(graph: torch.fx.Graph) -> None:
    ...
    def find_all_effective_users(
        node: fx.Node, visited: OrderedSet = OrderedSet()
    ) -> set[fx.Node]:
        """
    ...
    for node in fwd_outputs:
        ...
        if node.meta.get("saved_for_offloading", False):
            ...
            if all_effective_users:
                ...
                # If no users, set last_user to the node itself so we insert after it
                last_user = node
            ...
            with graph.inserting_after(last_user):
                ...
                # Mark this device_put node as activation offloading
                cpu_node.meta["is_activation_offloading"] = True
...
def offload_activation_bw(graph: torch.fx.Graph) -> None:
    ...
    for node in bw_inputs:
        if node.meta.get("saved_for_offloading", False):
            ...
            # Move tensor back to original device (GPU) using prims.device_put with dtype preserved
            ...
            with insert_method(insert_point):
                ...
                # Mark this device_put node as activation offloading
                gpu_node.meta["is_activation_offloading"] = True
...
def enable_activation_offloading(
    ...
    count = 0
    for node in reversed(fwd_module.graph.nodes):
        count += 1
        if count < 100 or node not in fwd_module_outputs:
            continue
        # this is the case of fwd output
        if node.name not in saved_values_names:
            continue
        # this is the case of graph inputs (i.e., not intermediate activations)
        if node.name in static_input_names or "primals" in node.name:
            continue
