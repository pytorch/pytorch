...
def _collect_fwd_nodes_from_subgraph(fx_g, fwd_seq_nr_to_node):
...
def _copy_metadata_to_bw_nodes_in_subgraph(fx_g, fwd_seq_nr_to_node):
...
def copy_fwd_metadata_to_bw_nodes(fx_g):
    ...
    def collect_all_submodules(module):
        """Recursively collect all GraphModule submodules."""
        submods = [module]
        for node in module.graph.nodes:
            if node.op == "get_attr":
                submod = getattr(module, node.target)
                if isinstance(submod, torch.fx.GraphModule):
                    submods.extend(collect_all_submodules(submod))
        return submods

    all_submods = collect_all_submodules(fx_g)
