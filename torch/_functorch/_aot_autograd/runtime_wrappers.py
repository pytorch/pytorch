...
def _codegen_compiled_forward(
    ...
    # The compiled forward's output layout starts with all num_forward returns
    # (mutated inputs, user outputs, intermediate bases, and any functionalized-RNG
    # offset), followed by tensors, opaque objects, then symints saved for backward.
    # Keep this in sync with ViewAndMutationMeta's saved-for-backward slices.
    ...
    if len(fw_metadata.saved_tensor_is_graph_input) != num_saved_tensors:
        raise AssertionError(
            "expected one saved_tensor_is_graph_input entry per saved tensor, "
            f"got {len(fw_metadata.saved_tensor_is_graph_input)} != {num_saved_tensors}"
        )

    num_forward = fw_metadata.num_forward
    vc_indices = range(num_forward, num_forward + num_vc)
    no_vc_indices = range(num_forward + num_vc, num_forward + num_saved_tensors)
    opaque_indices = range(
        num_forward + num_saved_tensors,
        num_forward + num_saved_tensors + num_opaque,
    )
    symint_indices = range(
        num_forward + num_saved_tensors + num_opaque,
    ...
    buf = PySourceBuilder(
        "_compiled_forward",
        args="ctx, args, _compiled_fw_, _rng_add_",
    ...
    with buf.indent():
        ...
        for i, idx in enumerate(vc_indices):
            buf.writeline(f"_saved_{i} = fw_outs[{idx}]")
            buf.writeline(f"if not isinstance(_saved_{i}, Tensor):")
            with buf.indent():
                buf.writeline(
                    "raise AssertionError("
                    f"f'expected saved tensor at fw_outs[{idx}], "
                    f"got {{type(_saved_{i})}}')"
                )
            if not fw_metadata.saved_tensor_is_graph_input[i]:
                buf.writeline(
                    f"_saved_{i} = "
                    f"_saved_{i}.detach() if _saved_{i}._is_view() else _saved_{i}"
                )

        for i, idx in enumerate(no_vc_indices):
            saved_idx = num_vc + i
            buf.writeline(f"_saved_no_vc_{i} = fw_outs[{idx}]")
            buf.writeline(f"if not isinstance(_saved_no_vc_{i}, Tensor):")
            with buf.indent():
                buf.writeline(
                    "raise AssertionError("
                    f"f'expected saved tensor at fw_outs[{idx}], "
                    f"got {{type(_saved_no_vc_{i})}}')"
                )
            if not fw_metadata.saved_tensor_is_graph_input[saved_idx]:
                buf.writeline(
                    f"_saved_no_vc_{i} = _saved_no_vc_{i}.detach() "
                    f"if _saved_no_vc_{i}._is_view() else _saved_no_vc_{i}"
                )

        if fw_metadata.dynamic_saved_tensors_idxs:
            buf.bind(_mark_dynamic_=mark_dynamo_propagated_dynamic_indices)
            for idx, dims in fw_metadata.dynamic_saved_tensors_idxs.items():
                dims_name = buf.bind_value("_dynamic_dims", dims)
                if idx < num_vc:
                    buf.writeline(f"_mark_dynamic_(_saved_{idx}, {dims_name})")
        ...
        if num_vc > 0:
            ...
            buf.writeline(f"ctx.save_for_backward({saved_args})")

        if num_no_vc > 0:
            no_vc_args = ", ".join(f"_saved_no_vc_{i}" for i in range(num_no_vc))
            buf.writeline(f"ctx._tensors_no_vc_check = [{no_vc_args}]")

        if num_symints > 0:
            for i, idx in enumerate(symint_indices):
                buf.writeline(f"_symint_{i} = fw_outs[{idx}]")
                buf.writeline(
                    f"if not isinstance(_symint_{i}, "
                    "(int, float, torch.SymInt, torch.SymFloat)):"
                )
                with buf.indent():
                    buf.writeline(
                        "raise AssertionError("
                        f"f'expected symint at fw_outs[{idx}], "
                        f"got {{type(_symint_{i})}}')"
                    )
            symint_args = ", ".join(f"_symint_{i}" for i in range(num_symints))
            buf.writeline(f"ctx.symints = [{symint_args}]")
        else:
            buf.writeline("ctx.symints = ()")

        if num_opaque > 0:
            buf.bind(
                _is_custom_class_=is_custom_class,
                CustomClassBase=CustomClassBase,
            )
            for i, idx in enumerate(opaque_indices):
                buf.writeline(f"_opaque_{i} = fw_outs[{idx}]")
                buf.writeline(
                    f"if not (_is_custom_class_(type(_opaque_{i})) "
                    f"or isinstance(_opaque_{i}, CustomClassBase)):"
                )
                with buf.indent():
                    buf.writeline(
                        "raise AssertionError("
                        f"f'expected opaque object at fw_outs[{idx}], "
                        f"got {{type(_opaque_{i})}}')"
                    )
            opaque_args = ", ".join(f"_opaque_{i}" for i in range(num_opaque))
            buf.writeline(f"ctx.opaque_objects = [{opaque_args}]")
        else:
            buf.writeline("ctx.opaque_objects = ()")
        ...
        needs_raw_returns_list |= bool(non_diff_indices)

        if needs_raw_returns_list:
            raw_return_args = ", ".join(
                f"fw_outs[{i}]" for i in range(num_forward_returns)
            )
            buf.writeline(f"raw_returns = [{raw_return_args}]")

            if num_mutated_runtime_inps > 0:
                buf.bind(_config_=config)
                num_mutated_inp_infos = sum(
                    info.mutates_data or info.mutates_metadata
                    for info in fw_metadata.input_info
                )
                buf.writeline("if _config_.debug_assert:")
                with buf.indent():
                    buf.writeline(
                        "_user_mutated_inputs_raw = "
                        f"raw_returns[0:{num_mutated_runtime_inps}]"
                    )
                    buf.writeline(
                    ...
                    with buf.indent():
                        ...
                        with buf.indent():
                            buf.writeline(
                                "'expected len(user_mutated_inputs_raw) == "
                                "len(mut_inp_infos), '"
                            )
                            buf.writeline(
                                f"f'got {{len(_user_mutated_inputs_raw)}} "
                                f"!= {num_mutated_inp_infos}'"
                            )
                        buf.writeline(")")
            ...
            for idx in fw_metadata.unsafe_view_out_indices:
                ...
                buf.writeline(f"_unsafe_view_{idx} = raw_returns[{raw_return_idx}]")
                buf.writeline(
            ...
            if fw_metadata.num_outputs_aliased > 0:
                for idx in fw_metadata.aliased_out_indices:
                    raw_return_idx = num_mutated_runtime_inps + idx
                    buf.writeline(
                        f"raw_returns[{raw_return_idx}] = "
                        f"TensorAlias(raw_returns[{raw_return_idx}])"
                    )
                ...
                with buf.indent():
                    ...
                    buf.writeline(
                        "if any(isinstance(x, TensorAlias) "
                        "for x in _intermediates_raw):"
                    )
                    with buf.indent():
                        buf.writeline(
                            "raise AssertionError("
            ...
            if non_diff_indices:
                checks = " + ".join(
                    f"([raw_returns[{i}]] "
                    f"if isinstance(raw_returns[{i}], Tensor) else [])"
                    for i in non_diff_indices
                )
                buf.writeline(f"_non_diff = {checks}")
                buf.writeline("if _non_diff:")
                with buf.indent():
                    buf.writeline("ctx.mark_non_differentiable(*_non_diff)")
            ...
            if num_forward_returns == 0:
                buf.writeline("return ()")
            else:
                return_values = ", ".join(
                    f"fw_outs[{i}]" for i in range(num_forward_returns)
                )
                trailing_comma = "," if num_forward_returns == 1 else ""
                buf.writeline(f"return ({return_values}{trailing_comma})")
