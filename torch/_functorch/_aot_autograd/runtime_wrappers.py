...
def _backward_prologue_functional(
    ...
    assert len(metadata.output_types) == num_flat_bw_args_with_grads
...
Got grad_output types: {str(grad_output_types)}"""
    ...
    if maybe_subclass_metadata is not None:
        ...
        if len(runtime_subclass_info) != len(metadata.subclass_tangent_meta):
            raise RuntimeError(
        ...
        assert metadata.traced_tangent_metas is not None
        all_args = [
            (
                AOTDispatchAutograd.coerce_runtime_tangent(
                    t,
                    metadata.traced_tangent_metas[i - tangents_start_idx],
                )
        ...
        assert metadata.traced_tangent_memory_formats is not None
        all_args = [
            (
                AOTDispatchAutograd.coerce_runtime_tangent_tracing_memory_format(
                    t,
                    metadata.traced_tangent_memory_formats[i - tangents_start_idx],
                )
                if tangents_start_idx <= i < tangents_end_idx
                else t
            )
...
def _backward_epilogue_functional(metadata, maybe_subclass_metadata, out):
    ...
    if maybe_subclass_metadata is not None:
        ...
        outs_wrapped = wrap_tensor_subclasses(
            out,
            subclass_metas=maybe_subclass_metadata.grad_input_metas,  # type: ignore[call-arg]
