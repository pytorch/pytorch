    ...
    def inner_fn(*args):
        ...
        for i, (o, info) in enumerate(zip(outs, meta.output_info)):
            if info.output_type == OutputType.alias_of_intermediate_save_as_output:
                ...
                intermediate_bases_descs.append(IntermediateBaseAOTOutput(o))
...
def create_joint(
    fn: PreppedForAutogradTraceFn,
    primals_descs: list[AOTInput],
    *,
    ...
    # post_forward
    def inner_fn(primals: list[FxValue], tangents: list[FxValue]):
...
def create_functionalized_rng_ops_wrapper(
    func: Union[JointTraceFn, TraceFn], args, args_descs, trace_joint=True
) -> Any:
...
def create_functionalized_fn(
    ...
    *,
    ...
    def _functionalized_f_helper(
        *args: list[FxValue],
    ) -> tuple[tuple[Sequence[FxValue], Sequence[Tensor]], Sequence[AOTOutput]]:
...
def handle_effect_tokens_fn(
    ...
    *,
    ...
    if trace_joint:
        ...
        args_descs = (
            [*additional_fwd_token_inputs_descs, *args_descs[0]],
            *args_descs[1:],
        )
    else:
...
def aot_dispatch_subclass(
    flat_fn_maybe_joint: Union[JointTraceFn],  # TODO: add the other types
    args: list[Any],
    args_descs: list[AOTDescriptor],
    *,
    ...
    if is_joint_structure:
        ...
        tangents_unwrapped = unwrap_tensor_subclasses(
            args[1], args_descs[1], append_symints=False
        )

        args_unwrapped = (primals_unwrapped[0], tangents_unwrapped[0])
        args_descs_unwrapped = (primals_unwrapped[1], tangents_unwrapped[1])
    else:
        args_unwrapped, args_descs_unwrapped = unwrap_tensor_subclasses(
            args, args_descs, append_symints=True
        )
