...
def _run_forward_in_functionalized_mode(
    f: Callable[..., Any],
    flat_args: tuple[Any, ...],
    ...
    with disable_above, mode, suppress_pending, disable_autocast_cache():
        ...
        flat_f_args = pytree.tree_map(to_fun_fn, flat_args)
    ...
    return flat_f_args, flat_f_outs, flat_f_outs_descs, mode, prior_grad_enabled


def _collect_input_alias_info(
    flat_args: tuple[Any, ...],
    flat_f_args: Any,
    *,
    ...
    # Inspect the state of the input tensor functional wrapper to detect input
    # mutation info. If inp[i] has a metadata-only mutation, then
    # maybe_inputs_with_mutated_metadata[i] contains the updated version.
...
def _collect_output_alias_info(
    ...
    # If a function involves creating a tensor, and returning a view of it, such
    # that its _base is the intermediate, we need to make sure our graph returns
    # the _base as a graph output, and we manually recreate the view to return to
    # the user. Why? The backend compiler is free to (incorrectly) not set
    # requires_grad on the base tensor, but we are obligated to properly set
    # requires-gradness on the real output.
    inp_storage_refs = {
    ...
    # We need output tensor id's to tell if any output._base` attributes **are**
    # other outputs. (This is also a dict because we need to know that output's
    # index, so we can regenerate the alias from it).
    ...
    # This tells us, for a given group of outputs that alias each other, whether
    # they e.g. all came from an unbind call.
    ...
    for o, desc in zip(flat_f_outs, flat_f_outs_descs):
        ...
        if not isinstance(o, Tensor):
            ...
            base_idx = None
        elif curr_storage in inp_storage_refs and not functional_tensor_storage_changed:
...
from a multi-output view call"
    ...
    return output_info, intermediate_bases, intermediate_bases_descs


# See Note [AOT Autograd: Views to avoid tangents aliasing inputs]
def _view_avoid_dupes_with_primals(t: object) -> object:
    if isinstance(t, Tensor) and is_traceable_wrapper_subclass(t):
        return transform_subclass(
            t, lambda _, inner_t: _view_avoid_dupes_with_primals(inner_t)
        )
...
def _plain_fake_tensor_like_subclass(x: Any) -> torch.Tensor:
    ...
    with detect_fake_mode():
        return torch.empty(x.shape, dtype=x.dtype, device=x.device, layout=x.layout)
...
def _collect_traced_tangents_and_fw_graph_outs(
    flat_f_args: Any,
    ...
    intermediate_bases: list[torch.Tensor],
    intermediate_bases_descs: list[AOTInput],
) -> tuple[list[Any], list[AOTInput], Any]:
    # This analysis function returns *only* the outputs that are meant to be
    # tangents to the backwards. Anything that aliases (inputs returned in the fw
    # due to metadata mutations, or outputs that alias inputs/intermediates) are
    # regenerated later, and not used directly in the autograd graph.
    f_input_tangents_pairs = [
        ...
        for inp, inp_desc, info in zip(flat_f_args, flat_args_descs, input_info)
    ...
    return traced_tangents, traced_tangents_descs, fw_graph_outs


def _normalize_static_input_indices(
...
def _restore_grad_mode_and_get_mutation(
    prior_grad_enabled: bool,
) -> bool | None:
...
def run_functionalized_fw_and_collect_metadata(
    ...
    *,
    ...
    def inner(*flat_args: Any) -> ViewAndMutationMeta:
        ...
        # Phase 1: run the forward under functionalization.
        (
            flat_f_args,
            flat_f_outs,
            flat_f_outs_descs,
            mode,
            prior_grad_enabled,
        ) = _run_forward_in_functionalized_mode(f, flat_args, _to_fun)

        # Phase 2: detect input mutations after functionalization has run.
        input_info = _collect_input_alias_info(
            flat_args,
            flat_f_args,
            keep_input_mutations=keep_input_mutations,
        )

        # Phase 3: classify outputs and record the extra bases we must preserve.
        output_info, intermediate_bases, intermediate_bases_descs = (
            _collect_output_alias_info(
                flat_f_args,
                flat_f_outs,
                flat_f_outs_descs,
                input_info,
        ...
        # Phase 4: build tangent and subclass metadata from the analyzed graph.
        traced_tangents, traced_tangents_descs, fw_graph_outs = (
            _collect_traced_tangents_and_fw_graph_outs(
                flat_f_args,
                flat_args_descs,
                flat_f_outs,
                flat_f_outs_descs,
