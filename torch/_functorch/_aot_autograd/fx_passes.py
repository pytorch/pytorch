...
def move_resize_zero_to_end_of_graph(graph):
    ...
    for n in node_list:
        if n.op == "call_function" and n.target == torch.ops.inductor.resize_storage_bytes_.default and n.args[1] == 0:
            resize_node = n
            with graph.inserting_before(return_node):
                new_resize_node = graph.call_function(torch.ops.inductor.resize_storage_bytes_.default, resize_node.args)
...
def refunctionalize_set(graph):
    ...
    1. `.set_(X, Y)` exists in graph, and X is primal input of graph
    2. `.set_(Y, ...)` is not called between primal input X's this `.set_(X, Y)` vs. its next `.set_(X, ...)` (or end of graph)
        - This ensures X and Y are semantically always meant to be the same tensor within this section of the graph.
    If the above two conditions are met, then within this section of the graph
    we will replace usage of output of this `.set_(X, Y)` node with Y, and delete this `.set_(X, Y)` node.
    For any primal input, if we have deleted the last `.set_(X, ...)`, we will re-insert `.set_(X, Y_last)` at the end of the graph.

    TODO: ok, it seems that we have to do this pass after parititoner, because we can't control where the partitioner cuts the graph (right now it's saving root of view but we actually want some view to be saved)

    TODO: need to add unit test for "between primal input's this .set_ vs. its next .set_ (or end of graph)" case
    """
    node_list = list(graph.nodes)
    return_node = node_list[-1]
    assert return_node.target == "output"
    primal_inputs = [*filter(_is_primal, node_list)]
    ...
    for i, n in enumerate(node_list):
        ...
        if n.op == "call_function" and n.target == torch.ops.aten.set_.source_Tensor and n.args[0] in primal_inputs:
    ...
    for primal_input, set_idx_list in primal_input_to_set_idx.items():
        for i in range(len(set_idx_list)):
            ...
            if not any(node_list[idx].target == torch.ops.aten.set_.source_Tensor and node_list[idx].args[0] == Y_input for idx in range(set_node_idx + 1, next_set_node_idx)):
                ...
                set_node.replace_all_uses_with(Y_input, delete_user_cb=lambda n: node_to_idx[n] > set_node_idx and node_to_idx[n] < next_set_node_idx)
        ...
        if last_set_node in set_nodes_to_be_deleted:
            with graph.inserting_before(return_node):
                new_last_set_node = graph.call_function(last_set_node.target, last_set_node.args)
