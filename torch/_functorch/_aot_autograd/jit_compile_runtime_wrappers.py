    ...
    with torch.no_grad():
        ...
        with track_graph_compiling(aot_config, "joint"):
            ...
            def insert_primal_resize(mod):
                ...
                primal_inputs_tensor_only = [x for x in list(filter(torch._functorch.partitioners._is_primal, fw_module.graph.nodes)) if isinstance(x.meta.get('val', None), torch.Tensor)]
                ...
                for primal_input in list(primal_inputs_resized):
                    ...
                    with mod.graph.inserting_before(first_non_placeholder_op):
                        ...
                        assert full_size > 0
                        primal_input_resized_to_full = mod.graph.call_function(torch.ops.inductor.resize_storage_bytes_, (primal_input, full_size), {})
                    ...
                    with mod.graph.inserting_before(return_op):
                        primal_input_resized_to_0 = mod.graph.call_function(torch.ops.inductor.resize_storage_bytes_, (primal_input, 0), {})
                    mod.graph.lint()
                    mod.recompile()

            insert_primal_resize(fw_module)
