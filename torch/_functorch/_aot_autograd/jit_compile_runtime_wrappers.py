...
def aot_dispatch_autograd(
    ...
    *,
    ...
    if torch._dynamo.config.compiled_autograd_enabled:
        from torch.fx._lazy_graph_module import _LazyGraphModule

        _LazyGraphModule.force_recompile(bw_module)
        torch._dynamo.compiled_autograd.initialize()
    ...
    class CompiledFunction(torch.autograd.Function):
        ...
        def forward(ctx, *deduped_flat_tensor_args):
            ...
            if torch._dynamo.config.compiled_autograd_enabled:
                assert saved_context, "TracingContext must be set for compiled autograd"
                ctx._compiled_autograd_compiler_fn = saved_context.torch_compile_wrapper
