        ...
        if grad_primals:
            with fx_traceback.preserve_node_meta():
                ...
                if aot_config.no_tangents:
                    ...
                    print("Running backwards, no tangents")
                    ...
                    # """
                    print("Running backwards, tangents")
                    backward_out = torch.autograd.grad(
                        needed_outs,
                        grad_primals,
...
def create_runtime_wrapper(
    ...
    *,
    ...
    def runtime_wrapper(*args):
        if trace_joint:
            args_ = list(args)
            # print("Runtime wrapper args:", [_sizes(x) for x in args_])
        ...
        sizes = [_sizes(x) for x in all_outs]
        # print("All outs sizes:", sizes)
...
def aot_dispatch_autograd(flat_fn, flat_args: List[Any], aot_config: AOTConfig, *, fw_metadata: ViewAndMutationMeta):
    ...
    class CompiledFunction(torch.autograd.Function):
        ...
        def forward(ctx, *deduped_flat_tensor_args):
            ...
            outs =  tuple(raw_returns) + tuple(marked_dirty_inps)
            sizes = [_sizes(x) for x in outs]
            # print("Compiled autograd function forward out sizes:", sizes)
            return outs

        @staticmethod
        def backward(ctx, *flat_args):
            sizes = [_sizes(x) for x in flat_args]
            # print("Compiled autograd function backward in sizes:", sizes)
...
Got grad_output types: {str(grad_output_types)}"""
            ...
            def call_compiled_backward():
                # print("Calling compiled backwards")
                if ctx._is_compiled_autograd_tracing():
                    # print("Autograd tracing - start")
                    # For compiled autograd, run raw FX graph so that it can be inlined into the larger graph
                    symints = ctx._get_compiled_autograd_symints()
                    # print("Autograd tracing - got symints")
                    assert len(symints) == len(ctx.symints)
                    all_args[:len(symints)] = symints
                    context = torch._C._DisableAutocast if disable_amp else nullcontext
                    with context():
                        # print("Autograd tracing - run bw_module")
                        sizes = [_sizes(x) for x in all_args]
                        # print("All args sizes:", sizes)
                        out = normalize_as_list(bw_module(*all_args))
                        bw_module.print_readable()
                        # print("Autograd tracing - ran bw_module")

                    out = functionalized_rng_runtime_epilogue(CompiledFunction.metadata, out)
                    # print("Autograd tracing - epilogue")
                    sizes = [_sizes(x) for x in out]
                    # print("Autograd tracing - out", sizes)
                    return tuple(out)
                # print("Autograd tracing - clear saved tensors")
                ctx.maybe_clear_saved_tensors()
                if CompiledFunction.compiled_bw is None:
                    # print("Autograd tracing - no compiled_bwd")
                    context = torch._C._DisableAutocast if disable_amp else nullcontext
                    with tracing(saved_context), context(), track_graph_compiling(aot_config, "backward"):
                        # print("With tracing, etc, calling aot bw_compiler")
                        CompiledFunction.compiled_bw = aot_config.bw_compiler(
                            bw_module, placeholder_list
                        )

                # print("Autograd tracing - invoke")
            ...
            if torch.is_grad_enabled() and any(t.requires_grad for t in all_args if isinstance(t, torch.Tensor)):
                ...
                class InnerCompiledFunctionBackward(torch.autograd.Function):
                    @staticmethod
                ...
                InnerCompiledFunctionBackward._compiled_autograd_key = CompiledFunction._compiled_autograd_key

                # Pass args even though they're unused, so that the graph is built
                out = InnerCompiledFunctionBackward.apply(*all_args)
            else:
