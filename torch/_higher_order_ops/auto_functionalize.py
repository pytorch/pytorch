...
class ViewInfo:
    ...
    # When is_alias is true, the tensor is an alias of the base tensor, that is regenerated by calling aten.alias
    is_alias: bool = False

    def regenerate_view(self, bases_list: List[Tensor]):
        base = bases_list[self.base_index]

        if not self.is_view:
            return base

        if self.is_alias:
            return torch.ops.aten.alias.default(base)
...
def read_view_information_from_args(
    ...
    def read_single_view(prefix):
        ...
        if base_index is None:
            ...
            return ViewInfo(base_index, all_bases[base_index], is_alias=True)
            ...
            return ViewInfo(base_index, size, stride, storage_offset)
