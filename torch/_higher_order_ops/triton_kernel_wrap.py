...
def analyze_kernel_access(
    ...
    for op_list in ops.values():
        for op in op_list:
            ...
            if op.name == "tt.call":
                ...
                read_writes = analyze_kernel_access(
                    functions,
                    # pyrefly: ignore [bad-argument-type]
                    op.fn_call_name,
                    len(op.args),
                ...
                written_set = {dep.name for dep in read_writes.writes}
                read_set = {dep.name for dep in read_writes.reads}
    ...
    def _dfs_traverse(
        ...
        """DFS traversal to find which argument indices are accessed."""
        result = set()
        stack = initial_stack[:]
        visited = set()
    ...
    written_indices = _dfs_traverse(write_stack, skip_loads=True)
    read_indices = _dfs_traverse(read_stack, skip_loads=False)

    writes: OrderedSet[Dep] = OrderedSet(
        StarDep(tensor_names[i]) for i in sorted(written_indices)
    )
    reads: OrderedSet[Dep] = OrderedSet(
        StarDep(tensor_names[i]) for i in sorted(read_indices)
    )
...
def get_mutated_tensors(
    ...
    kernel = kernel_side_table.get_kernel(kernel_idx)
    constant_args = kernel_side_table.get_constant_args(constant_args_idx)
    read_writes = identify_accessed_tensors(
        kernel, {**kwargs, **constant_args}, tma_descriptor_metadata
    )
    return [dep.name for dep in read_writes.writes]
