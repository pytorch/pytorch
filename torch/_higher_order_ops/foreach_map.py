...
class ForeachMap(BaseHOP):
    ...
    def __call__(self, fn, *operands, assert_fused=False, **kwargs):  # type: ignore[override]
        ...
        return super().__call__(fn, *operands, assert_fused=assert_fused, **kwargs)


_foreach_map = ForeachMap()


def foreach_map(op: Callable, *operands: Any, assert_fused: bool = False, **kwargs: dict[str, Any]):
    from torch._dynamo.polyfills import foreach_map_fn

    return _foreach_map(foreach_map_fn, op, *operands, assert_fused=assert_fused, **kwargs)
