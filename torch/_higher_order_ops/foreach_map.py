...
class ForeachMap(BaseHOP):
    ...
    def __call__(self, fn, *operands, _debug_assert_fused=False, **kwargs):  # type: ignore[override]
        ...
        return super().__call__(fn, *operands, _debug_assert_fused=_debug_assert_fused, **kwargs)


_foreach_map = ForeachMap()


def foreach_map(op: Callable, *operands: Any, _debug_assert_fused: bool = False, **kwargs: dict[str, Any]):
    """
    ...
    return _foreach_map(foreach_map_fn, op, *operands, _debug_assert_fused=_debug_assert_fused, **kwargs)
