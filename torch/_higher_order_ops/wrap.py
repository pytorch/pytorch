...
def wrap_dispatch_mode(mode, func, *args, **kwargs):
    # create & register subgraph
    subgraph = reenter_make_fx(func)(*args, **kwargs)
    i, graph_name = unique_graph_id(mode, prefix="wrap_body")
    mode.tracer.root.register_module(graph_name, subgraph)

    # proxify
    proxy_args = pytree.tree_map(mode.tracer.unwrap_proxy, (subgraph,) + args)
    proxy_kwargs = pytree.tree_map(mode.tracer.unwrap_proxy, kwargs)
    out_proxy = mode.tracer.create_proxy(
        "call_function", wrap, proxy_args, proxy_kwargs
    )
