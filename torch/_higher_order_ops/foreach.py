...
class ForeachOp(HigherOrderOperator):
    def __init__(self):
        super().__init__("cond")
...
foreach_op = ForeachOp()

# Mapping of non-foreach op to the foreach impl
# this allows us to utilize existing foreach impls
# when running the HOP in eager
OP_TO_FOREACH_OP = {torch.add: torch.foreach_add}


@exposed_in("torch")
def foreach(op, operands):
    r"""
    ...
    def _validate_input(pred, true_fn, false_fn, operands):
        # TODO: implement this to check for lists, scalars and tensors and a valid op arg
        return

    _validate_input(op, operands)

    if not torch._dynamo.is_dynamo_supported():
        raise RuntimeError("torch.foreach requires dynamo support.")

    # Dynamo is expecting a callable with "__code__" attribute.
    # We cannot directly pass cond_op to it. So we wrap it in a dummy function.
    def _foreach_op_wrapper(*args, **kwargs):
        return foreach_op(*args, **kwargs)
    ...
    with _set_compilation_env(), torch._dynamo.utils.disable_cache_limit(), _temp_remove_pre_dispatch_torch_function_mode():
        with _temp_remove_metadata_torch_function_mode() as metadata_mode:
            if metadata_mode:
                ...
                backend = "eager"
            return torch.compile(_foreach_op_wrapper, backend=backend, fullgraph=True)(
                op, operands
            )
...
@foreach_op.py_impl(DispatchKey.CompositeExplicitAutograd)
def foreach_op_dense(op, operands):
    mode = _get_current_dispatch_mode()
    assert mode is None, "Mode should never be enabled for CPU/CUDA key"


class ForeachAutogradOp(torch.autograd.Function):
    @staticmethod
    def forward(
        ctx,
        op,
