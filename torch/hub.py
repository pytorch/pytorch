    ...
    try:
        ...
        with tqdm(total=file_size, disable=not progress,
            ...
            while True:
                ...
                if len(buffer) == 0:
                    break
                f.write(buffer)  # type: ignore[possibly-undefined]  # TODO: audit
                if hash_prefix is not None:
                    sha256.update(buffer)  # type: ignore[possibly-undefined]  # TODO: audit
                pbar.update(len(buffer))

        f.close()
        if hash_prefix is not None:
            digest = sha256.hexdigest()  # type: ignore[possibly-undefined]  # TODO: audit
