...
def use_deterministic_algorithms(
    ...
    *,
    ...
    :class:`RuntimeError` when ``mode=True``:
        ...
        * :func:`torch.Tensor.scatter_reduce` with ``reduce='sum'\'prod'\'mean'``
        ...
        * :func:`torch.nn.MaxPool2d` when attempting to differentiate an MPS tensor
        * :func:`torch.nn.MaxPool3d` when attempting to differentiate an MPS tensor
        * :func:`torch.nn.AvgPool3d` when attempting to differentiate an MPS tensor
        * :func:`torch.nn.functional.interpolate` when attempting to differentiate an MPS tensor
          in one of the following cases:

          - ``mode='bilinear'`` and ``antialias=True``
          - ``mode='bicubic'``
          - ``mode='trilinear'``
          - ``mode='nearest'`` with 5-D input
          - ``mode='nearest-exact'`` with 5-D input
