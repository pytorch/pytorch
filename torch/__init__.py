...
class _TorchCompileInductorWrapper:
    ...
    def apply_options(self, options: dict[str, _Any] | None) -> None:
        ...
        for key, val in options.items():
            ...
            owner_config, target_key = get_compile_option_owner(attr_name) or (config, attr_name)
            if target_key not in owner_config._config:
                raise RuntimeError(
                    f"Unexpected optimization option {key}, known options are {list(config._config)}"
                )
