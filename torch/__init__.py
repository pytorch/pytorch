...
if sys.platform == 'win32':
    ...
    if all(map(lambda p: not os.path.exists(os.path.join(p, 'nvToolsExt64_1.dll')), dll_paths)):
    ...
    import glob
    if cuda_version and all(map(lambda p: len(glob.glob(os.path.join(p, 'cudart64*.dll'))) == 0, dll_paths)):
