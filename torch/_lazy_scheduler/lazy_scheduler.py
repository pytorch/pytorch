...
class LazySchedulerGraphModule(torch.nn.Module):
  ...
  def __call__(self, *args):
    ...
    LazyScheduler._current_scheduler._segment_to_gm_map[self.segment_name] = self.gm
...
class LazyScheduler:
  ...
  def __init__(self, module, *, segments=[], schedule=[], compile_options=None):
    ...
    # NOTE: we intentionally don't reset this in reset(), because we can only populate it in 1st iteration compile time.
    self._has_inplace_op_segment_prefixes = set()
    self.reset()
    LazyScheduler._current_scheduler = self

  def reset(self):
    # NOTE: must call this `reset()` function at beginning of each iteration
    global next_unnamed_segment_id
    next_unnamed_segment_id = 0
    ...
    # We assume one segment maps to one GraphModule (i.e. no graph-break within a segment)
    self._segment_to_gm_map = OrderedDict()
...
"""
  ...
  def _register_segment(self, segment):
    ...
    if segment_prefix not in self._registered_segment_prefixes:
      ...
      # Reuse eager and compiled methods from previous iterations, to avoid recompile when possible.
      if not (hasattr(nn_module, stashed_eager_method_name) and hasattr(nn_module, compiled_method_name)):
  ...
  def maybe_run(self, gm, compiled_fn, cur_segment_name, *args):
    ...
    while _next_segment_index < len(self._schedule):
      ...
      if _segment_name not in self._segment_to_gm_map:
        all_preceding_graph_handles_are_created = False
        break
      else:
        g = self._segment_to_gm_map[_segment_name]
...
If you find that removing a specific segment from the schedule fixes the issue, then audit that segment and other functions that have data dependency with it.
-----------------------------------------------------------------------------------------------------------------------
"""
