...
# Custom module wrapper that wraps a free function
class FunctionModuleWrapper(torch.nn.Module):
    def __init__(self, func):
        super(FunctionModuleWrapper, self).__init__()
        self.func = func

    def forward(self, *args, **kwargs):
        return self.func(*args, **kwargs)

def process_callable_into_module(func):
    if isinstance(func, torch.nn.Module):
        return func
    if hasattr(func, "__self__") and isinstance(func.__self__, torch.nn.Module):
...
def trace(
    ...
    * The resulting recording of `nn.Module.forward` or `nn.Module` produces
    ...
    mod = process_callable_into_module(func)
    exported_program = torch.export.export(mod, export_example_inputs, example_kwarg_inputs)
    return _trace_impl(exported_program.module, example_inputs, optimize, check_trace, check_inputs, check_tolerance, strict, _force_outplace, _module_class, _compilation_unit, example_kwarg_inputs, _store_inputs)
