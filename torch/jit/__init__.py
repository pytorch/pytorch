...
def freeze(mod, preserved_attrs : Optional[List[str]] = None):
    ...
    module's submodules, parameters, and attributes as constants
    in the TorchScript IR Graph. By default, `forward` will be preserved,
    as well as attributes specified in `preserved_attrs`. Additionally, attributes which
    are modified in the `forward` or in a method specified in `preserved_attrs`
    will be preserved.

    Freezing currently only accepts ScriptModules which are in eval mode.

    Arguments:
        mod (:class:`ScriptModule`): a module to be frozen

        preserved_attrs (Optional[List[str]]): a list of additional attributes to `forward` to preserve.
        Attributes modified in methods specified in `preserved_attrs` will also be preserved.
    ...
    .. testcode::
        ...
        class MyModule2(torch.nn.Module):
            def __init__(self):
                ...
                self.version = 1
                self.unused_tensor = torch.tensor(5.)
        ...
        # we've manually preserved `version`, soit still exists on the frozen module and can be modified
        assert frozen_module.version == 1
        frozen_module.version = 2
        # `modified_tensor` is detected as being mutated in the forward, so freezing preserves it to
        # retain model semantics
