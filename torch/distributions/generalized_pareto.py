...
class GeneralizedPareto(Distribution):
    ...
    def icdf(self, value):
        ...
        m = self.loc
        s = self.scale
        is_k_zero = torch.isclose(k, torch.tensor(0.))
        safe_k = torch.where(is_k_zero, torch.ones_like(k), k)
        neglog1mp = -torch.log1p(-value)
        return m + s * torch.where(is_k_zero, neglog1mp, torch.expm1(k * neglog1mp) / safe_k)
    ...
    def mean(self):
        ...
        safe_conc = torch.where(valid, concentration, torch.tensor(0.5, dtype=self.dtype))
    ...
    def variance(self):
        ...
        safe_conc = torch.where(valid, concentration, torch.tensor(0.25, dtype=self.dtype))
