...
def _kl_lowrankmultivariatenormal_lowrankmultivariatenormal(p, q):
    ...
    A = torch.linalg.triangular_solve(q._capacitance_tril, qWt_qDinv, upper=False)
...
def _kl_lowrankmultivariatenormal_multivariatenormal(p, q):
    ...
    term21 = _batch_trace_XXT(torch.linalg.triangular_solve(q_scale_tril, p_cov_factor, upper=False))
    term22 = _batch_trace_XXT(torch.linalg.triangular_solve(q_scale_tril, p_cov_diag, upper=False))
    term2 = term21 + term22
    return 0.5 * (term1 + term2 + term3 - p.event_shape[0])
...
def _kl_multivariatenormal_multivariatenormal(p, q):
    ...
    term2 = _batch_trace_XXT(torch.linalg.triangular_solve(q_scale_tril, p_scale_tril, upper=False))
    term3 = _batch_mahalanobis(q._unbroadcasted_scale_tril, (q.loc - p.loc))
