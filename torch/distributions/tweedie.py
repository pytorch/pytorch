...
class Tweedie(Distribution):
    ...
    :math:`Var(Y) = dispersion * \mu^{power}` where :math:`1 < power < 2`. It can be represented as a compound Poisson-Gamma distribution.
    Example::
    ...
    def __init__(self, mu, dispersion, power, validate_args=None):
        ...
        if isinstance(mu, _Number) and isinstance(dispersion, _Number) and isinstance(power, _Number):
    ...
    def log_prob(self, value):
        ...
        def log_prob_nonzero(y, mu, dispersion, power):
            ...
            def get_alpha(power): return (2-power)/(1-power)

            def get_jmax(y, dispersion, power): return torch.pow(y, 2 - power)/(dispersion*(2 - power))

            def get_log_z(y, dispersion, power):
                alpha = get_alpha(power)
                return -alpha*torch.log(y) + alpha*torch.log(power-1) - torch.log(2 - power) - (1-alpha)*torch.log(dispersion)
            ...
            return max_log_W + torch.log(sum_W) - torch.log(y) + (((y * torch.pow(mu, 1 -power)/(1 -power)) -
                                                            torch.pow(mu, 2 -power)/(2 -power)) / dispersion)

        value, mu, dispersion, power = broadcast_all(value, self.mu, self.dispersion, self.power)
        ...
        if torch.any(zeros):
            log_p[zeros] = -(torch.pow(mu[zeros], 2 - power[zeros]) / (dispersion[zeros] * (2 - power[zeros])))

        if torch.any(non_zeros):
            log_p[non_zeros] = log_prob_nonzero(value[non_zeros], mu[non_zeros],
                                                dispersion[non_zeros], power[non_zeros])

        return log_p

    @lazy_property
    def poisson_rate(self): return torch.pow(self.mu, 2-self.power)/(self.dispersion*(2 - self.power))

    @lazy_property
    def gamma_concentration(self): return (2-self.power)/(self.power-1)

    @lazy_property
    def gamma_rate(self): return 1/(self.dispersion*(self.power -1)*torch.pow(self.mu, self.power -1))
