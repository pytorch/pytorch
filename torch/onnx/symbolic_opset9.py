...
def batch_norm(g, input, weight, bias, running_mean, running_var, training, momentum, eps, cudnn_enabled):
    ...
    if running_mean is None or sym_help._is_none(running_mean) or running_var is None or sym_help._is_none(running_var):
        assert len(input_sizes) > 1
        reshape_in = g.op("Reshape", input, g.op("Constant", value_t=torch.tensor([input_sizes[0], input_sizes[1], -1], dtype=torch.int64)))
        trans_in = g.op('Transpose', reshape_in, perm_i=[1, 0, 2])
        reshape_in = g.op("Reshape", trans_in, g.op("Constant", value_t=torch.tensor([input_sizes[1], -1], dtype=torch.int64)))
        running_var, running_mean = _var_mean(g, reshape_in, g.op("Constant", value_t=torch.tensor([1], dtype=torch.int64)), False, False)
