...
def softmax(g: jit_utils.GraphContext, input, dim, dtype=None):
    ...
    if input_dim is not None:
        ...
        if is_transpose_required:
            softmax = g.op("Transpose", softmax, perm_i=axes)  # type: ignore[possibly-undefined]  # TODO: audit
        return softmax
...
def log_softmax(g: jit_utils.GraphContext, input, dim, dtype=None):
    ...
    if is_transpose_required:
        return_op = g.op("Transpose", return_op, perm_i=axes)  # type: ignore[possibly-undefined]  # TODO: audit
    return return_op
...
def native_layer_norm(
    ...
    if is_type_half:
        denominator = g.op(
            "Cast", denominator, to_i=_type_utils.JitScalarType(input_dtype).onnx_type()  # type: ignore[possibly-undefined]  # TODO: audit
        )
...
def _generic_rnn(
    ...
    def transform_weights_no_bias(layer_index):
        ...
        return tuple(
            symbolic_helper._unsqueeze_helper(g, x, [0]) for x in (weight_ih, weight_hh)  # type: ignore[possibly-undefined]  # TODO: audit
        )
    ...
    def transform_weights(layer_index):
        ...
        bias_concat = g.op("Concat", bias_ih, bias_hh, axis_i=0)  # type: ignore[possibly-undefined]  # TODO: audit
        return tuple(
            symbolic_helper._unsqueeze_helper(g, x, [0])
            for x in (weight_ih, weight_hh, bias_concat)  # type: ignore[possibly-undefined]  # TODO: audit
        )
    ...
    for i in range(num_layers):
        ...
        inputs.append(retrieve_state(h0, *state_indices))  # type: ignore[possibly-undefined]  # TODO: audit
        if variant == "LSTM":
            inputs.append(retrieve_state(c0, *state_indices))  # type: ignore[possibly-undefined]  # TODO: audit

        extra_kwargs = {} if unidirectional else {"direction_s": "bidirectional"}
        if variant == "RNN":
            if bidirectional:
                activation = [nonlinearity, nonlinearity]  # type: ignore[possibly-undefined]  # TODO: audit
            else:
                activation = [nonlinearity]  # type: ignore[possibly-undefined]  # TODO: audit
        ...
        h_outs.append(h_out)  # type: ignore[possibly-undefined]  # TODO: audit
        if variant == "LSTM":
            c_outs.append(c_out)  # type: ignore[possibly-undefined]  # TODO: audit
    if batch_first:
    ...
    h_outs = h_out if num_layers == 1 else g.op("Concat", *h_outs, axis_i=0)  # type: ignore[possibly-undefined]  # TODO: audit
    if variant == "RNN" or variant == "GRU":
        return prev_output, h_outs
    elif variant == "LSTM":
        c_outs = c_out if num_layers == 1 else g.op("Concat", *c_outs, axis_i=0)  # type: ignore[possibly-undefined]  # TODO: audit
