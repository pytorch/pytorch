...
def _symbolic_override_wrapper_maker(symbolic_fn, arg_test, fn):

    def wrapper(*args, **kwargs):
        output = fn(*args, **kwargs)
        # fast path
        import torch.jit
        if not arg_test(args):
            return output
...
def symbolic_override_with_arg_test(symbolic_fn, arg_test):
    return functools.partial(_symbolic_override_wrapper_maker, symbolic_fn, arg_test)
...
def symbolic_override_first_arg_based(symbolic_fn):
    ...
    return symbolic_override_with_arg_test(symbolic_fn, arg_test)
