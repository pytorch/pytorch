...
from torch.onnx._symbolic_opsets.symbolic_opset9 import _reshape_from_tensor, permute
...
def binary_cross_entropy_with_logits(g, input, target, weight, pos_weight, reduction):
    from torch.onnx._symbolic_opsets.symbolic_opset9 import add, log, mul, neg, sigmoid, sub
...
def unfold(g, input, dimension, size, step):
    ...
    if not sym_help._is_value(const_size) and not sym_help._is_value(const_step):
        from torch.onnx._symbolic_opsets.symbolic_opset9 import unfold as _unfold
