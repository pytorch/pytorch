...
class UpsampleBilinear2DDecompSkip(DecompSkip):
    ...
    def register(cls, export_options: torch.onnx.ExportOptions):
        ...
        registry.register_op(function=cls.onnxscript_function, namespace=cls.new_op_namespace, op_name=cls.new_op_name)
    ...
    def abstract(cls, input, output_size, align_corners, scale_factors):
        if output_size is not None:
            ...
            return torch.empty((input.size(0), input.size(1), h, w), dtype=input.dtype, device=input.device)
...
def enable_decomposition_skips(export_options: torch.onnx.ExportOptions, skips: Sequence[DecompSkip] = _DEFAULT_SKIP_LIST):
