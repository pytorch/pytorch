...
class FxOnnxInterpreter:
    ...
    def run(
        ...
        with diagnostic.log_section(logging.DEBUG, "ONNX Graph:"):
            diagnostic.debug("```\n%s\n```", onnxscript_graph.torch_graph)  # type: ignore[attr-defined, unused-ignore]
    ...
    def call_function(
        ...
        symbolic_fn = onnxfunction_dispatcher.dispatch(
            node=node,
            onnx_args=onnx_args,  # type: ignore[arg-type, unused-ignore]
    ...
    def call_module(
        ...
        outputs: (
            onnxscript_graph_building.TorchScriptTensor
            | tuple[onnxscript_graph_building.TorchScriptTensor, ...]
        ) = parent_onnxscript_graph.add_module_call(  # type: ignore[assignment, unused-ignore]
