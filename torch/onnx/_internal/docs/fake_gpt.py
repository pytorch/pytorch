...
with ftm:
    ...
    onnx_model, graph_module, bound_args, replaced_attrs = fx_onnx.export_without_parameters_and_buffers(model, use_binary_format=False, **inputs)
...
def test_one(graph_module, onnx_model, bound_args, replaced_attrs):
    ...
    ort_sess = ort.InferenceSession("model_tiny_gpt.onnx", providers=['CUDAExecutionProvider', 'CPUExecutionProvider'])
    ort_input_dict = {}
    for ort_input, t in zip(onnx_model.graph.input, [arg for arg in pth_args if arg is not None]):
        ort_input_dict[ort_input.name] = t.numpy()
    ort_out = ort_sess.run(None, ort_input_dict)

    np.testing.assert_allclose(ort_out[0], pth_out["last_hidden_state"].numpy(), atol=1e-4, rtol=1e-3)
    for ort_value, pth_value in zip(ort_out[1:], pth_out["past_key_values"][0] + pth_out["past_key_values"][1]):
