...
def index_put(g, self, indices_list_value, values, accumulate=False):
    ...
    values = sym_help._reshape_helper(g, values, values_shape, allowzero=0)
...
def masked_scatter(g, self, mask, source):
    ...
    source = sym_help._reshape_helper(g, source, torch.LongTensor([-1]), allowzero=0)
...
def _prepare_onnx_paddings(g, dim, pad):
    ...
    paddings = sym_help._reshape_helper(g, paddings, g.op("Constant", value_t=torch.tensor([-1, 2])), allowzero=0)
    paddings = g.op("Transpose", torch.onnx.symbolic_opset10.flip(g, paddings, [0]), perm_i=[1, 0])
    paddings = sym_help._reshape_helper(g, paddings, g.op("Constant", value_t=torch.tensor([-1])), allowzero=0)
...
def _get_im2col_indices_along_dim(g, input_d, kernel_size_d, dilation_d, padding_d, stride_d):
    ...
    kernel_mask = sym_help._reshape_helper(g, kernel_grid, g.op("Constant", value_t=torch.tensor([-1, 1])), allowzero=0)
...
def im2col(g, input, kernel_size, dilation, padding, stride):
    ...
    return sym_help._reshape_helper(g, output, output_shape, allowzero=0)
