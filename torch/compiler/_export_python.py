...
def _mark_editable(code: str) -> str:
    ...
    return code.replace(
        ...
        "# supported: this source is what runs. The kernels below are yours -- change a\n"
        "# Triton kernel's launch config in KERNEL_CONFIGS (nothing retunes it at runtime),\n"
        "# or delete inductor's machinery and hand-write the kernel. Four things to know:\n"
        "#\n"
        "#   1. The launch site uses KERNEL.run(...), which only exists on inductor's\n"
        "#      autotuner. If you drop the decorator, switch it to\n"
        "#      KERNEL[grid](..., XBLOCK=..., num_warps=...) AND delete the\n"
        "#      stream=raw_stream0 argument -- triton rejects it, and the line you are\n"
        "#      replacing passes it, so copying it forward is an immediate error.\n"
        "#   2. @triton.jit finds its own source by filename, so load the artifact with\n"
        "#      exec(compile(src, path, 'exec'), ns) rather than exec(src, ns).\n"
        "#   3. The generated kernel is unmasked with a grid sized for this exact shape.\n"
        "#      Changing the grid or the block size writes past the end of the output,\n"
        "#      and neither CUDA nor COMPILER_EXPORT_PYTHON_CHECK will say so -- the\n"
        "#      checksum still matches. Run compute-sanitizer once after such an edit.\n"
        "#   4. COMPILER_EXPORT_PYTHON_CHECK compares against eager at this dtype's\n"
        "#      resolution. It catches an edit that changes the answer; it does not prove\n"
        "#      the edit computes the same function.",
        1,
    )
...
def _explain_kernel_compile_error(
    ...
    cpp_pybinding path -- where the source is still compiled out of a cache file, and for
    a hand-edited Triton kernel that gets past Python and dies in ptxas. A hoisted Triton
    kernel with a plain syntax error needs none of this: Python reports the error against
    this artifact at the right line by itself.
    ...
    # The cache file is .py for a Triton kernel but .cpp/.h for the CPU path, and the
    # location reads "line 56" from Python but "main.cpp:56:27" from g++. Matching only
    # the Python shapes meant this never fired for the one backend it exists for.
    match = re.search(r"(/\S*torchinductor\S*?\.(?:py|cpp|cc|h|hpp))", message)
    if match is None:
        return hint
    try:
        ...
        return hint
    line_match = re.search(r"line (\d+)", message) or re.search(
        r"\.(?:py|cpp|cc|h|hpp):(\d+)[:\s]", message
    )
