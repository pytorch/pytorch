...
def build_table(
    ...
    def auto_scale_flops(flops):
        ...
        log_flops = min(math.log10(flops) / 3, float(len(flop_headers) - 1))
        return (pow(10, (log_flops * -3.0)), flop_headers[log_flops])
    ...
    if with_flops:
        ...
        (flops_scale, flops_header) = auto_scale(min([evt.flops for evt in events]))
    ...
    for evt in events:
        ...
        if with_flops:
            if evt.flops == 0:
                ...
                row_values.append('{0:8.3f}'.format(evt.flops * flops_scale / evt.self_cpu_time_total))
        if has_stack:
            src_field = ""
            if len(evt.stack) > 0:
