            ...
            if fe.device_type == DeviceType.CPU and not fe.is_async:
                if self.use_device:
                    ...
                    if privateuse1_time > 0:
                        fe.append_kernel(
                            fe.name, f"privateuse1:{fe.device_index}", privateuse1_time
                        )
                        fe.is_legacy = True
                else:
                    ...
                    if cuda_time > 0:
                        fe.append_kernel(fe.name, f"cuda:{fe.device_index}", cuda_time)
                        fe.is_legacy = True
            ...
            if (
                ...
                for f_evt in device_corr_map[fe.id]:
                    if f_evt.device_type in [DeviceType.CUDA, DeviceType.XPU]:
                        fe.append_kernel(
                            f_evt.name,
                            f"cuda:{f_evt.device_index}"
                            if f_evt.device_type == DeviceType.CUDA
                            else f"xpu:{f_evt.device_index}",
...
def parse_nvprof_trace(path):
    ...
    for row in conn.execute(kernel_query):
        ...
        evt.append_kernel(
            row["kernel_name"], "cuda:0", row["kernel_end"] - row["kernel_start"]
        )
