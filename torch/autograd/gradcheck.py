...
def failed_batched_grad_test_msg(output_idx, input_idx, res, exp):
    return f"""
For output {output_idx} and input {input_idx}, batched grad computation failed.

If you are adding a new operator, a fast workaround is to use gradcheck with
`check_batched_grad=False` and filing an issue. If the test is autogenerated
from e.g. common_method_invocations, there should be a deny list you can add
the test to. If you're interested in making your new operator work with
batched grad computation, read on.
...
def test_batched_grad(fail_test, input, output, output_idx):
    ...
    for input_idx, (res, exp) in enumerate(zip(result, expected)):
        if torch.allclose(res, exp):
            continue
        return fail_test(failed_batched_grad_test_msg(output_idx, input_idx, res, exp))
