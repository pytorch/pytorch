...
class FusedMovingAvgObsFakeQuantize(FakeQuantize):
    ...
    def __init__(
        ...
        **observer_kwargs: Any
        ...
        assert isinstance(
            self.activation_post_process, MovingAverageMinMaxObserver) or isinstance(
            self.activation_post_process, MovingAveragePerChannelMinMaxObserver) , \
            "Fused observer+fake_quant module only works with MovingAverageMinMaxObserver or MovingAveragePerChannelMinMaxObserver"
        ...
        if self.is_per_channel:
            self.fused_min_val = self.activation_post_process.min_vals
            self.fused_max_val = self.activation_post_process.max_vals
        else:
            self.fused_min_val = self.activation_post_process.min_val
            self.fused_max_val = self.activation_post_process.max_val
