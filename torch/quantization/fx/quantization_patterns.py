...
class BinaryOpQuantizeHandler(QuantizeHandler):
    def __init__(self, quantizer: QuantizerCls, node: Node):
        ...
        for arg_idx in range(len(self.binary_op_node.args)):
            ...
            if isinstance(arg, Node) and (not all_node_args_have_no_tensors(arg, quantizer.modules, cache)):
    ...
    def convert(self, quantizer: QuantizerCls, node: Node, load_arg: Callable,
        ...
        if dtypes in [(torch.quint8, torch.qint8, None)]:
            ...
            if self.num_tensor_args == 1:
                ...
                if isinstance(first_arg, Node) and (not all_node_args_have_no_tensors(first_arg, quantizer.modules, cache)):
                    quantized_index = 0
                else:
                    quantized_index = 1
