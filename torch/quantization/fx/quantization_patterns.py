...
class QuantizeHandler(ABC):
    ...
    def convert(self, quantizer: QuantizerCls, node: Node, load_arg: Callable,
                output_a_reference_model: bool = False,
...
class ConvRelu(QuantizeHandler):
    ...
    def convert(self, quantizer: QuantizerCls, node: Node, load_arg: Callable,
        ...
        # TODO: output_a_reference_model option for conv module
        ...
        if self.conv_node.op == 'call_module':
            ...
            if output_a_reference_model:
                ...
                assert len(self.conv_node.args) >= 7, \
                    "only conv2d calls with all arguments specified is supported right now in output_a_reference_model=False option"
                args = load_arg(quantized=[0, 1])(self.conv_node.args)
                # pack weight
...
class StandaloneModuleQuantizeHandler(QuantizeHandler):
    ...
    def convert(self, quantizer: QuantizerCls, node: Node, load_arg: Callable,
        ...
        quantized_standalone_module = convert(observed_standalone_module, output_a_reference_model=output_a_reference_model)
        parent_name, name = _parent_name(node.target)
