...
class Quantizer:
    ...
    def _convert(self, model: GraphModule, output_a_reference_model: bool = False,
        ...
        for node in model.graph.nodes:
            ...
            if root_node is node:
                ...
                if qconfig is None and not is_observed_standalone_module_node:
                    ...
                    result = obj.convert(
                        self, node, load_arg, output_a_reference_model=output_a_reference_model,
    ...
    def convert(self, model: GraphModule, output_a_reference_model: bool = False,
                convert_custom_config_dict: Dict[str, Any] = None,
                is_standalone_module: bool = False) -> GraphModule:
        quantized = self._convert(
            model, output_a_reference_model, convert_custom_config_dict, is_standalone_module)
        if not output_a_reference_model:
