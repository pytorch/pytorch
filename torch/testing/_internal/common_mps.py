    ...
    for op in ops:
        ...
        if key in EMPTY_OPS_SKIPLIST:
            addDecorator(
                op,
                DecorateInfo(
                    unittest.skip("Skipping empty ops."), dtypes=EMPTY_OPS_SKIPLIST[key]
                ),
            )
        if key in SKIPLIST:
            addDecorator(
        ...
        for xfaillist in [
            ...
            if key in xfaillist and key not in xfail_exclusion:
                addDecorator(
                    op, DecorateInfo(unittest.expectedFailure, dtypes=xfaillist[key])
                )
        ...
        if (
            ...
            addDecorator(
                op,
                DecorateInfo(
                    unittest.expectedFailure, dtypes=MACOS_BEFORE_14_4_XFAILLIST[key]
                ),
            )
        ...
        if (
            ...
            addDecorator(
                op,
                DecorateInfo(
                    unittest.expectedFailure, dtypes=MACOS_BEFORE_13_3_XFAILLIST[key]
                ),
            )
        ...
        if key not in SUPPORTED_COMPLEX_OPS and (
            key not in AFTER_MACOS_14_0_SUPPORTED_COMPLEX_OPS or MACOS_VERSION < 14.0
        ):
            addDecorator(
                op,
                DecorateInfo(
                    unittest.expectedFailure, dtypes=[torch.complex32, torch.complex64]
                ),
            )
...
def mps_ops_grad_modifier(ops: Sequence[OpInfo]) -> Sequence[OpInfo]:
    ...
    for op in ops:
        ...
        if key in XFAILLIST_GRAD:
            addDecorator(
                op, DecorateInfo(unittest.expectedFailure, dtypes=XFAILLIST_GRAD[key])
            )
        ...
        if key in ON_MPS_XFAILLIST:
            addDecorator(
                op, DecorateInfo(unittest.expectedFailure, dtypes=ON_MPS_XFAILLIST[key])
            )
