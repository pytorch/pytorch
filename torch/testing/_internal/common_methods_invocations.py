...
op_db: List[OpInfo] = [
    ...
    BinaryUfuncInfo('pow',
                    ...
                    supports_forward_ad=True,
                    supports_fwgrad_bwgrad=True,
                    # Note: pow on Sparse CSR inputs doesn't support complex inputs yet
                    # and only pow(Tensor, Scalar) is supported, hence removed from the generic tests
                    # as it requires a lot of special handling. Separate tests are however present in
                    # test/test_sparse_csr.py for pow(Tensor, Scalar) variant
    ...
    UnaryUfuncInfo('square',
                   ...
                   # hence, square is temporarily filtered from the list,
                   # see: https://github.com/pytorch/pytorch/pull/69603#discussion_r793300941
                   supports_sparse_csr=False,
                   supports_fwgrad_bwgrad=True,
                   skips=(
...
sparse_csr_unary_ufuncs = [op for op in op_db if isiinstance(op, UnaryUfuncInfo) and op.supports_sparse_csr]
