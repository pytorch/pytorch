...
def method_tests():
    ...
    return [
        ...
        ('det', lambda dtype, device: random_symmetric_matrix(S),
         NO_ARGS, 'symmetric', (), NO_ARGS, [skipCPUIfNoLapack, skipCUDAIfNoMagma]),
        ...
        ('det', lambda dtype, device: random_square_matrix_of_rank(S, 1),
         NO_ARGS, 'rank1', (), NO_ARGS, [skipCPUIfNoLapack, skipCUDAIfNoMagma]),
        ('det', lambda dtype, device: random_square_matrix_of_rank(S, 2),
         NO_ARGS, 'rank2', (), NO_ARGS, [skipCPUIfNoLapack, skipCUDAIfNoMagma]),
        ...
        ('svd', lambda dtype, device: random_fullrank_matrix_distinct_singular_value(S, dtype=dtype).to(device),
         NO_ARGS, 'check_grad_uv', (), NO_ARGS, [skipCPUIfNoLapack, skipCUDAIfNoMagma],
         lambda usv: usv[0][0, 0] * usv[2][0, 0]),
        ('svd',
         lambda dtype, device: random_fullrank_matrix_distinct_singular_value(S, dtype=dtype).to(device)[:(S - 2)],
         NO_ARGS, 'wide', (), NO_ARGS, [skipCPUIfNoLapack, skipCUDAIfNoMagma],
         lambda usv: (abs(usv[0]), usv[1], abs(usv[2][..., :, :(S - 2)]))),
        ('svd',
         lambda dtype, device: random_fullrank_matrix_distinct_singular_value(S, dtype=dtype).to(device)[:, :(S - 2)],
         NO_ARGS, 'tall', (), NO_ARGS, [skipCPUIfNoLapack, skipCUDAIfNoMagma],
         lambda usv: (abs(usv[0]), usv[1], abs(usv[2]))),
        ('svd',
         lambda dtype, device: random_fullrank_matrix_distinct_singular_value(S, dtype=dtype).to(device)[:(S - 2)],
         (False,), 'wide_all', (), NO_ARGS, [skipCPUIfNoLapack, skipCUDAIfNoMagma],
         lambda usv: (abs(usv[0]), usv[1], abs(usv[2][:, :(S - 2)]))),
        ('svd',
         lambda dtype, device: random_fullrank_matrix_distinct_singular_value(S, dtype=dtype).to(device)[:, :(S - 2)],
         (False,), 'tall_all', (), NO_ARGS, [skipCPUIfNoLapack, skipCUDAIfNoMagma],
         lambda usv: (abs(usv[0][:, :(S - 2)]), usv[1], abs(usv[2]))),
        ('svd', lambda dtype, device: random_fullrank_matrix_distinct_singular_value(M, dtype=dtype).to(device),
         NO_ARGS, 'large', (), NO_ARGS, [skipCPUIfNoLapack, skipCUDAIfNoMagma],
         lambda usv: (abs(usv[0]), usv[1], abs(usv[2]))),
        ('svd', lambda dtype, device: random_fullrank_matrix_distinct_singular_value(S, 3, dtype=dtype).to(device),
         NO_ARGS, 'batched', (), NO_ARGS, [skipCPUIfNoLapack, skipCUDAIfNoMagma],
         lambda usv: (abs(usv[0]), usv[1], abs(usv[2]))),
        ('svd', lambda dtype, device:
         random_fullrank_matrix_distinct_singular_value(S, 3, dtype=dtype).to(device)[..., :(S - 2), :],
         NO_ARGS, 'wide_batched', (), NO_ARGS, [skipCPUIfNoLapack, skipCUDAIfNoMagma],
         lambda usv: (abs(usv[0]), usv[1], abs(usv[2]))),
        ('svd', lambda dtype, device:
         random_fullrank_matrix_distinct_singular_value(S, 3, dtype=dtype).to(device)[..., :, :(S - 2)],
         NO_ARGS, 'tall_batched', (), NO_ARGS, [skipCPUIfNoLapack, skipCUDAIfNoMagma],
         lambda usv: (abs(usv[0]), usv[1], abs(usv[2]))),
        ('svd', lambda dtype, device:
         random_fullrank_matrix_distinct_singular_value(S, 3, 3, dtype=dtype).to(device)[..., :(S - 2), :],
         (False,), 'wide_all_batched', (), NO_ARGS, [skipCPUIfNoLapack, skipCUDAIfNoMagma],
         lambda usv: (abs(usv[0]), usv[1], abs(usv[2][..., :, :(S - 2)]))),
        ('svd', lambda dtype, device:
         random_fullrank_matrix_distinct_singular_value(S, 3, 3, dtype=dtype).to(device)[..., :, :(S - 2)],
         (False,), 'tall_all_batched', (), NO_ARGS, [skipCPUIfNoLapack, skipCUDAIfNoMagma],
         lambda usv: (abs(usv[0][..., :, :(S - 2)]), usv[1], abs(usv[2]))),
        ('pinverse', lambda dtype, device: random_fullrank_matrix_distinct_singular_value(S, dtype=dtype).to(device),
         NO_ARGS, '', (), NO_ARGS, [skipCPUIfNoLapack, skipCUDAIfNoMagma]),
        ('pinverse', lambda dtype, device:
