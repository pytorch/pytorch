...
def sample_inputs_masked_std_var(op_info, device, dtype, requires_grad, **kwargs):
    """Sample inputs for masked std and var.
    """
    inputs: List[SampleInput] = []
        ...
        for sample_input in sample_inputs_masked_reduction(op_info, device, dtype, requires_grad, **kwargs):
            ...
            if requires_grad:
                ...
                if orig_count.min() <= int(unbiased):
                    ...
                    # correctly.
                    continue
                if sample_input.input.dim() == 0 or orig_count.min() == 1:
                    # Skip samples when the autograd output for std could not be handled correctly
                    # due to torch.sqrt
                    continue
            inputs.append(SampleInput(sample_input.input.clone().requires_grad_(requires_grad),
                                      args=sample_input_args, kwargs=sample_input_kwargs))
...
def sample_inputs_masked_logaddexp(op_info, device, dtype, requires_grad, **kwargs):
    ...
    for shape, input_masks, other_masks in zip(shapes, input_mask_lists, other_mask_lists):
        for input_mask, other_mask in zip(input_masks, other_masks):
            input = make_tensor(shape, device, dtype, requires_grad=requires_grad)
            other = make_tensor(shape, device, dtype, requires_grad=requires_grad)
...
op_db: List[OpInfo] = [
    ...
    ReductionOpInfo(
        ...
        supports_autograd=False,
        dtypes=all_types_and(torch.float16, torch.bfloat16),
        ref=reference_reduction_numpy(np.argmax),
        skips=(
    ...
    ReductionOpInfo(
        ...
        supports_autograd=False,
        dtypes=all_types_and(torch.float16, torch.bfloat16),
        ref=reference_reduction_numpy(np.argmin),
        skips=(
    ...
    OpInfo(
        '_masked.logaddexp',
        ref=np.logaddexp if np.lib.NumpyVersion(np.__version__) >= '1.20.2' else None,
        dtypes=floating_types_and(torch.bfloat16),
        supports_out=False,
        supports_forward_ad=True,
        supports_fwgrad_bwgrad=True,
