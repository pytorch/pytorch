...
def sample_inputs_linalg_det_singular(op_info, device, dtype, requires_grad):
    kw = dict(device=device, dtype=dtype)
    singular_det_cutoff = 1e-8

    # Create a batch of randomized square singular matrices. Each matrix is
    # generated by performing a matrix multiply of two random matrices of
    # size (size[-1], rank) and (rank, size[-1])
    def make_singular_matrix_batch_base(size, rank, device, dtype):
        assert size[-1] == size[-2]
        assert rank > 0 and rank <= size[-1]

        batch = torch.empty(size, device=device, dtype=dtype)

        matrix_size = size[-2:]
        view_size = (int(np.prod(size[:-2])),) + matrix_size
        batch_view = batch.view(view_size)

        a_size = (size[-1], rank)
        b_size = (rank, size[-1])

        for matrix_idx in range(batch_view.size(0)):
            num_tries = 0
            max_tries = 50
            is_matrix_acceptable = False

            while not is_matrix_acceptable:
                assert num_tries < max_tries, (
                    f'could not create singular matrix of size {matrix_size}'
                    f' after trying {max_tries} times')

                lo = -10
                hi = 10
                if dtype.is_complex:
                    a = torch.randint(lo, hi, a_size, device=device).to(dtype).add(
                        1j * torch.randint(lo, hi, a_size, device=device).to(dtype))
                    b = torch.randint(lo, hi, b_size, device=device).to(dtype).add(
                        1j * torch.randint(lo, hi, b_size, device=device).to(dtype))

                    # Avoiding zeros in a and b decreases likelihood of
                    # producing an unstable gradient
                    a[a.eq(0)] = torch.tensor(random.choice([hi, 1j * hi]), device=device, dtype=dtype)
                    b[b.eq(0)] = torch.tensor(random.choice([hi, 1j * hi]), device=device, dtype=dtype)
                else:
                    a = torch.randint(lo, hi, a_size, device=device, dtype=dtype)
                    b = torch.randint(lo, hi, b_size, device=device, dtype=dtype)
                    a[a.eq(0)] = hi
                    b[b.eq(0)] = hi
                ...
                if torch.matrix_rank(matrix) != rank:
                    is_matrix_acceptable = False

                # Ensure that linalg_det_backward will consider the matrix as singular
                elif matrix.det().abs().gt(singular_det_cutoff):
                    is_matrix_acceptable = False

                elif (torch.device(device).type == 'cpu' and rank == 1) or (rank > 2):
                    # TODO: linalg_det_backward can sometimes unstable if
                    # any singular values are 0 if rank is greater than
                    # 2 or CPU rank 1.  This is likely due to its
                    # dependence upon svd_backward. Therefore, we must
                    # avoid testing such matrices. Once lu_backward is
                    # available, linalg_det_backward can be updated to use
                    # it so that we hopefully no longer have this problem:
                    # https://github.com/pytorch/pytorch/issues/53364
                    is_matrix_acceptable = bool(matrix.svd()[1].ne(0).all())

                else:
                    is_matrix_acceptable = True

                num_tries += 1

            batch_view[matrix_idx] = matrix

        return batch

    def make_singular_matrix_batch(size, rank=1, device=None, dtype=None):
        num_tries = 0
        max_tries = 50
        is_batch_acceptable = False
        while not is_batch_acceptable:
            assert num_tries < max_tries, (
                f'could not create batch of singular matrices of size {size}'
                f' after trying {max_tries} times')
            batch = make_singular_matrix_batch_base(size, rank, device, dtype)

            # CUDA torch.det can give a different result for batched
            # matrices compared to an unbatched matrix. So even though we
            # already checked if each matrix is singular, we need to also
            # check if the batched call is fully singular.
            if torch.device(device).type == 'cuda':
                is_batch_acceptable = bool(batch.det().abs().lt(singular_det_cutoff).all())
            else:
                is_batch_acceptable = True
            num_tries += 1

        return batch

    def make_nonsingular_matrix_batch(size, device=None, dtype=None):
        batch = None
        num_tries = 0
        max_tries = 10
        is_batch_nonsingular = False
        while not is_batch_nonsingular:
            assert num_tries < max_tries, (
                f'could not create nonsingular matrix batch of size {size}'
                f' after trying {max_tries} times')
            batch = make_tensor(size, device, dtype, low=None, high=None, requires_grad=False)
            is_batch_nonsingular = bool(batch.det().abs().gt(singular_det_cutoff).all())
        return batch

    inputs = [
        # Fully singular
        make_singular_matrix_batch((4, 4), 1, **kw),
        make_singular_matrix_batch((4, 4), 2, **kw),
        make_singular_matrix_batch((4, 4), 3, **kw),
        make_singular_matrix_batch((2, 4, 4), 1, **kw),
        make_singular_matrix_batch((2, 4, 4), 2, **kw),
        make_singular_matrix_batch((2, 4, 4), 3, **kw),
        make_singular_matrix_batch((3, 1, 4, 4), 1, **kw),
        make_singular_matrix_batch((3, 1, 4, 4), 2, **kw),
        make_singular_matrix_batch((3, 1, 4, 4), 3, **kw),
        # Mixed singular and nonsingular
        torch.stack([
            make_nonsingular_matrix_batch((4, 4), **kw),
            make_singular_matrix_batch((4, 4), rank=2, **kw),
            make_singular_matrix_batch((4, 4), rank=1, **kw)]),
        torch.stack([
            make_singular_matrix_batch((4, 4), rank=1, **kw),
            make_nonsingular_matrix_batch((4, 4), **kw),
            make_singular_matrix_batch((4, 4), rank=2, **kw)]),
        torch.stack([
            make_nonsingular_matrix_batch((2, 4, 4), **kw),
            make_singular_matrix_batch((2, 4, 4), rank=2, **kw)]),
        torch.stack([
            make_nonsingular_matrix_batch((2, 4, 4), **kw),
            make_singular_matrix_batch((2, 4, 4), rank=1, **kw)]),
        torch.stack([
            torch.stack([
                make_nonsingular_matrix_batch((4, 4), **kw),
                make_singular_matrix_batch((4, 4), rank=2, **kw),
...
op_db: List[OpInfo] = [
    ...
    OpInfo('linalg.det',
           ...
           # Many of the tests fail on ROCm. This is probably
