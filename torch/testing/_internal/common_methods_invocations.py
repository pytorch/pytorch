...
op_db: List[OpInfo] = [
    ...
    OpInfo('broadcast_shapes',
           ...
           skips=(
               ...
               DecorateInfo(unittest.expectedFailure, 'TestCommon', 'test_composite_compliance', dtypes=[torch.float32]),
               DecorateInfo(unittest.expectedFailure, "TestCommon", "test_noncontiguous_samples", dtypes=[torch.complex64, torch.int64, torch.float32]),
               DecorateInfo(unittest.skip("Skipped!"), 'TestCommon', 'test_composite_compilance', dtypes=[torch.float32]),
               DecorateInfo(unittest.skip("Skipped!"), 'TestGradients', 'test_nondifferentiable', dtypes=[torch.float64]),
               DecorateInfo(unittest.skip("Skipped!"), 'TestCommon', 'test_variant_consistency_eager', dtypes=[torch.float32, torch.complex64]), 
               # JIT does not support variadic tensors.
               DecorateInfo(unittest.skip("Skipped!"), 'TestJit', 'test_variant_consistency_jit', dtypes=[torch.float32, torch.complex64]),
               DecorateInfo(unittest.expectedFailure, 'TestMathBits', 'test_neg_view', dtypes=[torch.float64]),
               DecorateInfo(unittest.expectedFailure, 'TestMathBits', 'test_conj_view', dtypes=[torch.complex64]),
               DecorateInfo(unittest.expectedFailure, 'TestMathBits', 'test_neg_conj_view', dtypes=[torch.complex128]),
           ),
