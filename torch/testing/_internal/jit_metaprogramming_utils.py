...
nn_functional_tests = [
    ...
    ('max_unpool1d', torch.tensor([[[2., 4]]], dtype=default_dtype), (torch.tensor([[[1, 3]]]), 2, 2, 0)),
    ('max_unpool2d', torch.tensor([[[[2., 4]]]], dtype=default_dtype), (torch.tensor([[[[1, 3]]]]), 2, 2, 0)),
    ('max_unpool3d', torch.tensor([[[[[2., 4]]]]], dtype=default_dtype), (torch.tensor([[[[[1, 3]]]]]), 2, 2, 0)),
    ...
    ('bilinear', (S, S, S), ((S, S, M), torch.zeros(M, S, M, dtype=default_dtype),),),
    ('embedding', torch.tensor([[1, 2, 4, 5], [4, 3, 2, 5]]), (torch.rand(6, 3, dtype=default_dtype), ), '', (True,)),
    ('embedding_bag', torch.tensor([1, 2, 4, 2]), (torch.rand(5, 3, dtype=default_dtype), torch.tensor([0, 4]),),),
    ('batch_norm', (S, S),
        (non_differentiable(torch.randn(S, dtype=default_dtype)),
         non_differentiable(torch.ones(S, dtype=default_dtype)), None, None, True, ),
        'training', (True, 'aten::_batch_norm_impl_index')),
    ('batch_norm', (0, S, S, S),
        (non_differentiable(torch.randn(S, dtype=default_dtype)),
         non_differentiable(torch.ones(S, dtype=default_dtype)),
         non_differentiable(torch.randn(S, dtype=default_dtype)),
         non_differentiable(torch.ones(S, dtype=default_dtype)), True, ),
    ...
    ('batch_norm', (S, S), (non_differentiable(torch.randn(S, dtype=default_dtype)),
                            non_differentiable(torch.ones(S, dtype=default_dtype)),
                            None, non_differentiable(torch.ones(S, dtype=default_dtype)), True, ),
        'with_only_bias_training', (True, 'aten::_batch_norm_impl_index')),
    ('batch_norm', (S, S), (non_differentiable(torch.randn(S, dtype=default_dtype)),
                            non_differentiable(torch.ones(S, dtype=default_dtype)),
                            non_differentiable(torch.randn(S, dtype=default_dtype)), None, True, ),
    ...
    ('batch_norm', (S, S), (non_differentiable(torch.randn(S, dtype=default_dtype)),
                            non_differentiable(torch.ones(S, dtype=default_dtype)),
                            non_differentiable(torch.randn(S, dtype=default_dtype)),
                            non_differentiable(torch.ones(S, dtype=default_dtype)), False, ),
        'with_weight_and_bias_inference', (True, 'aten::_batch_norm_impl_index')),
    ('batch_norm', (S, S), (non_differentiable(torch.randn(S, dtype=default_dtype)),
                            non_differentiable(torch.ones(S, dtype=default_dtype)),
                            None, non_differentiable(torch.ones(S, dtype=default_dtype)), False, ),
        'with_only_bias_inference', (True, 'aten::_batch_norm_impl_index')),
    ('batch_norm', (S, S), (non_differentiable(torch.randn(S, dtype=default_dtype)),
                            non_differentiable(torch.ones(S, dtype=default_dtype)),
                            non_differentiable(torch.randn(S, dtype=default_dtype)), None, False, ),
        'with_only_weight_inference', (True, 'aten::_batch_norm_impl_index')),
    ('instance_norm', (S, S, S), (non_differentiable(torch.zeros(S, dtype=default_dtype)),
                                  non_differentiable(torch.ones(S, dtype=default_dtype))),),
    ('layer_norm', (S, S, S, S), ([5],), '',
     (False, ['aten::contiguous', 'aten::_batch_norm_impl_index'])),
    ('layer_norm', (S, S, S, S), ([5], non_differentiable(torch.rand(S, dtype=default_dtype)),), 'with_only_weight',
     (False, ['aten::contiguous', 'aten::_batch_norm_impl_index'])),
    ('layer_norm', (S, S, S, S), ([5], None, non_differentiable(torch.rand(S, dtype=default_dtype)),), 'with_only_bias',
     (False, ['aten::contiguous', 'aten::_batch_norm_impl_index'])),
    ('layer_norm', (S, S, S, S), ([5], non_differentiable(torch.rand(S, dtype=default_dtype)),
                                  non_differentiable(torch.rand(S, dtype=default_dtype))), 'with_weight_and_bias',
     (False, ['aten::contiguous', 'aten::_batch_norm_impl_index', 'aten::addcmul'])),
    ('group_norm', (S, S, S), (1, torch.rand(5, dtype=default_dtype),),),
    ('local_response_norm', (S, S, S), (2, ),),
    ('nll_loss', F.log_softmax(torch.randn(3, 5, dtype=default_dtype), dim=0), (torch.tensor([1, 0, 4]),), '',),
    ('poisson_nll_loss', torch.rand(S, 2, dtype=default_dtype), (torch.rand(S, 2, dtype=default_dtype),),),
    ('poisson_nll_loss', torch.rand(S, 2, dtype=default_dtype), (torch.rand(S, 2, dtype=default_dtype), True, True), 'full'),
    ('kl_div', F.log_softmax(torch.randn(S, 10, dtype=default_dtype), 1),
        (F.softmax(torch.randn(S, 10, dtype=default_dtype), 1),),),
    ('cross_entropy', (3, S), (torch.randint(S, (3,), dtype=torch.int64),),),
    ('binary_cross_entropy_with_logits', (3,), (torch.empty(3, dtype=default_dtype).random_(2), ),),
    ('smooth_l1_loss', (3, S), (non_differentiable(torch.rand(3, S, dtype=default_dtype)),),),
    ('huber_loss', (3, S), (non_differentiable(torch.rand(3, S, dtype=default_dtype)),),),
    ('l1_loss', (3, S), (non_differentiable(torch.rand(3, S, dtype=default_dtype)),),),
    ('mse_loss', (3, S), (non_differentiable(torch.rand(3, S, dtype=default_dtype)),),),
    ('smooth_l1_loss', (3, S), ((torch.rand(3, S, dtype=default_dtype)),), 'with_grad'),
    ('huber_loss', (3, S), ((torch.rand(3, S, dtype=default_dtype)),), 'with_grad'),
    ('l1_loss', (3, S), ((torch.rand(3, S, dtype=default_dtype)),), 'with_grad'),
    ('mse_loss', (3, S), ((torch.rand(3, S, dtype=default_dtype)),), 'with_grad'),
    ('margin_ranking_loss', (S,), ((S,), (S,)),),
    ('hinge_embedding_loss', (3, S), (non_differentiable(torch.rand(3, S, dtype=default_dtype)),),),
    ('soft_margin_loss', (3, S), (non_differentiable(torch.rand(3, S, dtype=default_dtype)),),),
    ('multilabel_soft_margin_loss', (3, S), (non_differentiable(torch.rand(3, S, dtype=default_dtype)),),),
    ('cosine_embedding_loss', (S, S), ((S, S), non_differentiable(torch.rand(S, dtype=default_dtype))),),
    ...
    ('grid_sample', (S, S, S, S), (non_differentiable(torch.rand(S, S, S, 2, dtype=default_dtype)),),),
    ...
    ('multilabel_margin_loss', torch.tensor([[0.2, -0.2, 0.07]], dtype=default_dtype), (torch.tensor([[0, 0, 1]]),),),
    ('multi_margin_loss', (S, S), (non_differentiable(torch.randint(S, (S, ), dtype=torch.int64)),
                                   1, 1., non_differentiable(torch.randn(S, dtype=default_dtype))),),
    ('binary_cross_entropy', torch.randn(3, 2, dtype=default_dtype).sigmoid(),
        (non_differentiable(torch.rand(3, 2, dtype=default_dtype)),
         non_differentiable(torch.randn(3, 2, dtype=default_dtype))),),
    ('binary_cross_entropy', torch.randn(3, 2, dtype=default_dtype).sigmoid(),
        (non_differentiable(torch.rand(3, 2, dtype=default_dtype)),
         non_differentiable(torch.randn(3, 2, dtype=default_dtype)), None, None, 'mean'), 'size_average'),
    ('ctc_loss', torch.rand(S, S, S, dtype=default_dtype).log_softmax(2).detach().requires_grad_(),
     (torch.randint(1, S, (S, S), dtype=torch.long), torch.full((S,), S, dtype=torch.long),
      torch.randint(1, S, (S,), dtype=torch.long))),
    ('upsample', torch.randn(S, S, M, M, dtype=default_dtype), (None, 2.), 'with_scale'),
    ('upsample', torch.randn(S, S, M, M, dtype=default_dtype), (4,), 'with_size'),
    ('interpolate', torch.zeros(3, 3, dtype=default_dtype).view(1, 1, 3, 3), (2,), 'nearest_4d'),
    ('interpolate', torch.randn(S, S, M, M, dtype=default_dtype), (None, 2.), 'nearest_4d_with_scale'),
    ('interpolate', torch.randn(S, S, M, M, dtype=default_dtype), (4,), 'nearest_4d_with_size'),
    ('interpolate', torch.zeros(3, 3, dtype=default_dtype).view(1, 1, 3, 3), (2,), 'area_4d'),
    ('interpolate', torch.randn(S, S, M, M, dtype=default_dtype), (None, 2.), 'area_4d_with_scale'),
    ('interpolate', torch.randn(S, S, M, M, dtype=default_dtype), (4,), 'area_4d_with_size'),
    ('interpolate', torch.zeros(3, 3, dtype=default_dtype).view(1, 1, 3, 3), (2,), 'bilinear_4d'),
    ('interpolate', torch.randn(S, S, M, M, dtype=default_dtype), (None, 2.), 'bilinear_4d_with_scale'),
    ('interpolate', torch.randn(S, S, M, M, dtype=default_dtype), (4,), 'bilinear_4d_with_size'),
    ('interpolate', torch.zeros(3, 3, dtype=default_dtype).view(1, 1, 3, 3), (2,), 'bicubic_4d'),
    ('interpolate', torch.randn(S, S, M, M, dtype=default_dtype), (None, 2.), 'bicubic_4d_with_scale'),
    ('interpolate', torch.randn(S, S, M, M, dtype=default_dtype), (4,), 'bicubic_4d_with_size'),
    ('interpolate', torch.zeros(3, 3, dtype=default_dtype).view(1, 3, 3), (2,), 'nearest_3d'),
    ('interpolate', torch.randn(S, M, M, dtype=default_dtype), (None, 2.), 'nearest_3d_with_scale'),
    ('interpolate', torch.randn(S, M, M, dtype=default_dtype), (4,), 'nearest_3d_with_size'),
    ('interpolate', torch.zeros(3, 3, dtype=default_dtype).view(1, 3, 3), (2,), 'area_3d'),
    ('interpolate', torch.randn(S, M, M, dtype=default_dtype), (None, 2.), 'area_3d_with_scale'),
    ('interpolate', torch.randn(S, M, M, dtype=default_dtype), (4,), 'area_3d_with_size'),
    ('interpolate', torch.zeros(3, 3, dtype=default_dtype).view(1, 3, 3), (2,), 'linear_3d'),
    ('interpolate', torch.randn(S, M, M, dtype=default_dtype), (None, 2.), 'linear_3d_with_scale'),
    ('interpolate', torch.randn(S, M, M, dtype=default_dtype), (4,), 'linear_3d_with_size'),
    ('interpolate', torch.randn(S, M, M, M, M, dtype=default_dtype), (None, 2.), 'nearest_5d_with_scale'),
    ('interpolate', torch.randn(S, M, M, M, M, dtype=default_dtype), (4,), 'nearest_5d_with_size'),
    ('interpolate', torch.zeros(3, 3, 3, dtype=default_dtype).view(1, 1, 3, 3, 3), (2,), 'area_5d'),
    ('interpolate', torch.randn(S, M, M, M, M, dtype=default_dtype), (None, 2.), 'area_5d_with_scale'),
    ('interpolate', torch.randn(S, M, M, M, M, dtype=default_dtype), (4,), 'area_5d_with_size'),
    ('interpolate', torch.zeros(3, 3, 3, dtype=default_dtype).view(1, 1, 3, 3, 3), (2,), 'trilinear_5d'),
    ('interpolate', torch.randn(S, M, M, M, M, dtype=default_dtype), (None, 2.), 'trilinear_5d_with_scale'),
    ('interpolate', torch.randn(S, M, M, M, M, dtype=default_dtype), (4,), 'trilinear_5d_with_size'),
    ('interpolate', torch.zeros(3, 3, dtype=default_dtype).view(1, 1, 3, 3), (2, None, 'nearest', None, False),
     'nearest_4d_not_recompute_scale_factor'),
    ('interpolate', torch.randn(S, S, M, M, dtype=default_dtype), (4, None, 'nearest', None, False),
     'nearest_4d_with_size_not_recompute_scale_factor'),
    ('interpolate', torch.randn(S, S, M, M, dtype=default_dtype), (None, 2., 'bilinear', None, False),
     'bilinear_4d_with_scale_not_recompute_scale_factor'),
    ('interpolate', torch.randn(S, S, M, M, dtype=default_dtype), (4, None, 'bilinear', None, False),
     'bilinear_4d_with_size_not_recompute_scale_factor'),
    ('interpolate', torch.randn(S, S, M, M, dtype=default_dtype), (None, 2., 'bicubic', None, False),
     'bicubic_4d_with_scale_not_recompute_scale_factor'),
    ('interpolate', torch.randn(S, S, M, M, dtype=default_dtype), (4, None, 'bicubic', None, False),
     'bicubic_4d_with_size_not_recompute_scale_factor'),
    ('interpolate', torch.randn(S, M, M, dtype=default_dtype), (None, 2., 'nearest', None, False),
     'nearest_3d_with_scale_not_recompute_scale_factor'),
    ('interpolate', torch.randn(S, M, M, dtype=default_dtype), (4, None, 'nearest', None, False),
     'nearest_3d_with_size_not_recompute_scale_factor'),
    ('interpolate', torch.randn(S, M, M, dtype=default_dtype), (None, 2., 'linear', None, False),
     'linear_3d_with_scale_not_recompute_scale_factor'),
    ('interpolate', torch.randn(S, M, M, dtype=default_dtype), (4, None, 'linear', None, False),
     'linear_3d_with_size_not_recompute_scale_factor'),
    ('interpolate', torch.randn(S, M, M, M, M, dtype=default_dtype), (None, 2., 'nearest', None, False),
     'nearest_5d_with_scale_not_recompute_scale_factor'),
    ('interpolate', torch.randn(S, M, M, M, M, dtype=default_dtype), (4, None, 'nearest', None, False),
     'nearest_5d_with_size_not_recompute_scale_factor'),
    ('interpolate', torch.randn(S, M, M, M, M, dtype=default_dtype), (None, 2., 'trilinear', None, False),
     'trilinear_5d_with_scale_not_recompute_scale_factor'),
    ('interpolate', torch.randn(S, M, M, M, M, dtype=default_dtype), (4, None, 'trilinear', None, False),
...
additional_module_tests = [
    ...
    },
    {
        ...
        'input_size': (10, 8, 128),
        'extra_args': (torch.randn(10, 8, 128, dtype=default_dtype), torch.randn(10, 8, 128, dtype=default_dtype)),
        'slowTest': True
    },
    {
        ...
        'input_size': (3, 1, 1),
        'extra_args': (torch.randn(1, 1, 1, dtype=default_dtype),),
        'slowTest': True
    }
]
