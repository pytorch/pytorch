...
class PrivateUse1TestBase(DeviceTypeTestBase):
    ...
    def setUpClass(cls):
        ...
        if cls.device_mod is None:
            raise AssertionError(f"""torch has no module of `{cls.device_type}`, you should register
...
def largeTensorTest(size, device=None, inductor=TEST_WITH_TORCHINDUCTOR):
    ...
    if isinstance(size, str):
        if not size.endswith(("GB", "gb")):
            raise AssertionError("only bytes or GB supported")
...
class precisionOverride:
    def __init__(self, d):
        if not isinstance(d, dict):
            raise TypeError("precisionOverride not given a dtype : precision dict!")
        for dtype in d:
            if not isinstance(dtype, torch.dtype):
                raise TypeError(f"precisionOverride given unknown dtype {dtype}")
...
class toleranceOverride:
    def __init__(self, d):
        if not isinstance(d, dict):
            raise TypeError("toleranceOverride not given a dtype : tol dict!")
        for dtype, prec in d.items():
            if not isinstance(dtype, torch.dtype):
                raise TypeError(f"toleranceOverride given unknown dtype {dtype}")
    ...
    def __init__(self, *args, device_type="all"):
        if len(args) > 0 and isinstance(args[0], (list, tuple)):
            for arg in args:
                ...
                if any(not isinstance(dtype, torch.dtype) for dtype in arg):
                    raise TypeError(f"Unknown dtype in {str(arg)}")
        else:
            if any(not isinstance(arg, torch.dtype) for arg in args):
                raise TypeError(f"Unknown dtype in {str(args)}")
