...
def wait_until_pending_futures_and_users_flushed(timeout: int = 20) -> None:
    ...
    while True:
        ...
        if time.time() - start > timeout:
            raise ValueError(
                f"Timed out waiting to flush pending futures and users, had {num_pending_futures} pending futures and {num_pending_users} pending users"
            )
...
def wait_until_owners_and_forks_on_rank(
    ...
    while True:
        ...
        if time.time() - start > timeout:
            raise ValueError(
                f"Timed out waiting {timeout} sec for {num_owners} owners and {num_forks} forks on rank, had {num_owners_on_rank} owners and {num_forks_on_rank} forks"
            )
