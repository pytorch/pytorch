...
class DTensorTestBase(MultiProcessTestCase):
    ...
    def _test_op_on_dtensor(self, dtensor: DTensor, op_call) -> None:
        """
        ...
        Note: for in-place ops, the function may not work as expected if the DTensor
        placement is Replicate because the full_tensor() could share the same object
        with DTensor._local_tensor.
        """
        out_flattened, _ = tree_flatten(op_call(dtensor.full_tensor()))
        d_out_flattened, _ = tree_flatten(op_call(dtensor))
