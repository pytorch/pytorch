...
class DistributedTest:
    class _DistTestBase:
        ...
        @skip_but_pass_in_sandcastle_if(
            ...
            f"The {BACKEND} backend does not support creating subgroups on accelerator devices",
        )
        ...
        @skip_but_pass_in_sandcastle_if(
            ...
            "NCCL or XCCL Batch Send Recv Only",
        )
        ...
        @skip_but_pass_in_sandcastle_if(
            ...
            "NCCL or XCCL send/recv tested by test_send_recv_nccl",
        )
        ...
        @skip_but_pass_in_sandcastle_if(
            (BACKEND == "nccl" or BACKEND == "xccl"),
            "NCCL or XCCL does not support isend",
        )
        def test_isend(self):
        ...
        @skip_but_pass_in_sandcastle_if(
            (BACKEND == "nccl" or BACKEND == "xccl"),
            "NCCL or XCCL does not support irecv",
        )
        def test_irecv(self):
            rank = dist.get_rank()
            world_size = dist.get_world_size()
        ...
        @skip_but_pass_in_sandcastle_if(
            (BACKEND == "nccl" or BACKEND == "xccl"),
            "NCCL or XCCL does not support CPU tensors",
        )
        ...
        def test_nccl_high_priority_stream(self):
            ...
            opts = (
                ...
                if TEST_CUDA
                else dist.ProcessGroupXCCL.Options()
            )
            opts.is_high_priority_stream = False
            group_id = (
                dist.ProcessGroupNCCL(store, rank, size, opts)
                if TEST_CUDA
                else dist.ProcessGroupXCCL(store, rank, size, opts)
            )
        ...
        def _test_reduce_twice_helper(
            ...
            for src in group:
                ...
                if cuda:
                    for i in range(2):
                        tensors[i] = tensors[i].to(rank_to_GPU[rank][0])
                self.call_dist_op(
                    ":reduce",
                    False,
                    dist.reduce,
        ...
        @skip_but_pass_in_sandcastle_if(
            BACKEND != "nccl" and BACKEND != "xccl",
            "Only NCCL or XCCL supports reduce_scatter_v",
        )
        @skip_but_pass_in_sandcastle_if(
            BACKEND in DistTestCases.skip_collective["reduce"],
            f"{BACKEND} does not support reduce",
        ...
        def test_reduce_scatter_v_cuda(self):
            ...
            for async_val in [True, False]:
                ...
                expected_tensor = expected_tensor.fill_(expected_value).to(device_id)
        ...
        def _reduce_scatter_tensor_helper(
            ...
            if cuda:
                tensor_in = tensor_in.to(rank_to_GPU[rank][0])
                tensor_out = tensor_out.to(rank_to_GPU[rank][0])
        ...
        @skip_but_pass_in_sandcastle_if(
            BACKEND != "nccl" and BACKEND != "xccl",
            "XCCL and NCCL support all_gather_v",
        )
        @skip_if_no_gpu
        ...
        def _test_all_to_all_single_equal_split_helper(
            ...
            if group_id is not None:
                ...
                if cuda:
                    in_tensor = in_tensor.to(rank_to_GPU[rank][0])
                    expected_tensor = expected_tensor.to(rank_to_GPU[rank][0])
                    out_tensor = out_tensor.to(rank_to_GPU[rank][0])
                if dtype == torch.complex64:
        ...
        def _test_all_to_all_helper(
            ...
            if group_id is not None:
                ...
                if cuda:
                    in_tensors = [t.to(rank_to_GPU[rank][0]) for t in in_tensors]
                    expected_tensors = [
        ...
        def _test_DistributedDataParallel(
            ...
            self._test_DDP_niter(
                model_gpu,
                model_DDP,
                input_cpu.to(gpu_subset[0]),
                target.to(gpu_subset[0]),
                loss,
                local_bs,
                rank,
                global_bs,
        ...
        @skip_but_pass_in_sandcastle_if(
            BACKEND == "nccl" or BACKEND == "xccl",
            "nccl does not support DDP on CPU models",
        )
        ...
        def test_DistributedDataParallel_non_default_stream(self):
            ...
            rank = self.rank
            stream_call = (
                torch.xpu.stream(stream) if TEST_XPU else torch.cuda.stream(stream)
            )
            with stream_call:
            ...
            if HAS_TORCHVISION:
                models_to_test.append(
                    (
                        ...
                        torch.randn(1, 3, 3, 1000).to(device_type),
                    )
                )
        ...
        def _test_ddp_apply_optim_in_backward(
            ...
            models_to_test = [
                nn.Sequential(nn.Linear(3, 3), nn.Linear(3, 3), nn.Linear(3, 3)).to(
                    device_type
                ),
        ...
        def _test_accumulate_gradients_no_sync(
            ...
            if BACKEND == "xccl" or BACKEND == "nccl":
                rank_to_GPU = init_multigpu_helper(dist.get_world_size(), BACKEND)
                int_devices = rank_to_GPU[rank][:1]
                device_type = "xpu" if BACKEND == "xccl" else "cuda"
                devices = [torch.device(f"{device_type}:{i}") for i in int_devices]
                global_batch_size = world_size
        ...
        @skipIfXpu(
            msg="test_DistributedDataParallel fails with static_graph=True, issue #112277"
        )
        ...
        @unittest.skipIf(
            BACKEND != "nccl" and BACKEND != "xccl",
            "Tests nccl/xccl metadata primarily.",
        )
        ...
        def test_ddp_uneven_inputs_stop_iteration_sync_bn(self):
            ...
            model_input = torch.randn(10, 2).to(
        ...
        def _test_broadcast_object_list(self, group=None):
            ...
            if backend != "nccl" and torch.accelerator.device_count() == int(
                self.world_size
            ):
                single_obj_list = [objects[0]]
                if self.rank != src_rank:
        ...
        def test_ddp_has_finalized(self):
            ...
            class MyModel(nn.Module):
                def __init__(self, rank):
                    super().__init__()
                    self.rank = rank
                    self.fc1 = nn.Linear(1024, 1024).to(rank)
                    self.fc2 = nn.Linear(1024, 2 * 1024).to(rank)
