...
class ParameterServerTest2(RpcAgentTestFixture):
    @property
    def world_size(self):
        return 2
    ...
    def test_parameter_server(self):
        if self.rank == 0:            
            # run_parameter_server(self.rank, self.world_size)
            pass
        else:
            # Get data to train on
            mnist = FakeMNIST(10000)
            train_loader = torch.utils.data.DataLoader(
                mnist, batch_size=32, shuffle=True)
            test_loader = torch.utils.data.DataLoader(
                mnist, batch_size=32, shuffle=True)
            run_worker(self.rank,
                       self.world_size,
                       0,  # TODO args.num_gpus,
                       train_loader,
                       test_loader)
