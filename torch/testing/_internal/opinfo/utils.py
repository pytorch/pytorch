...
def get_supported_dtypes(op, sample_inputs_fn, device_type):
    ...
    if not TEST_CUDA and device_type == "cuda":
        warnings.warn(
            "WARNING: CUDA is not available, empty_dtypes dispatch will be returned!", stacklevel=1
        )
    ...
    for dtype in all_types_and_complex_and(torch.bool, torch.bfloat16, torch.half):
        try:
            ...
            warnings.warn(
                f"WARNING: Unable to generate sample for device:{device_type} and dtype:{dtype}", stacklevel=1
            )
            continue
