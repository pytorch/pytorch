...
def validate_failures_dict_formatting(failures_dict_path: str) -> None:
    ...
    raise RuntimeError(
        ...
        f"this either manually or by re-running any single test with "
...
class OpCheckMode(TorchFunctionMode):
    ...
    def maybe_raise_errors_on_exit(self) -> None:
        ...
        repro_command = generate_repro(
            ...
            args,
            kwargs,
            better_repro=should_print_better_repro(),
        )
        repro_command = f"\n\n# BEGIN REPRO SCRIPT:{repro_command}\n# END REPRO SCRIPT"
        raise OpCheckError(
            ...
            f"incorrect behavior. Please see the following command for how to reproduce "
            f"this failure or please see {GDOC} for more recommendations. "
            f"{repro_command}"
        ) from ex
...
def should_print_better_repro() -> bool:
    ...
    """
    key = "PYTORCH_OPCHECK_BETTER_REPRO"
...
def generate_repro(
    ...
    *,
    ...
    if better_repro:
        ...
        args_kwargs = 'args, kwargs = torch.load("{filepath}")'
    else:
        args_kwargs = (
            "# To fill these in with the same (args, kwargs) as your test, rerun with PYTORCH_OPCHECK_BETTER_REPRO=1\n"
