...
def random_topology_test(seed, *inp_tensor_list):
  ...
  while num_operations >= 0 or num_sets > 1:
    ...
    if DEBUG_PRINT:
      print("iteration {0}, num_sets{1}, candidates {2}, tensor_list {3}, lh_index {4}, op_index {5}".format(num_operations, num_sets, candidate, len(tensor_list), lh_index, op_index))
    if num_operations >= 0:
      num_operations -= 1
...
def runTest(seed, args):
  ...
  try:
    ...
    raise Exception("Testing script failure, repro by running:\n"
