...
from torch.testing._internal.opinfo.core import OpInfo, DecorateInfo
...
class MPSSkipInfo:
    ...
    def __init__(
        self,
        tests: type[str | List[str]],
        variant: Optional[str] = None,
        dtypes: Optional[type[torch.dtype | List[torch.dtype]]] = None,
        skip: bool = False,
...
MPS_OPINFO_SKIPLIST = {
    ...
    'linalg.inv_ex': [
        ...
        MPSSkipInfo(
            NONCONTIGUOUS,
            skip=True,
            skip_msg="Crashes on MPS with err: MPSMatrixDecompositionLU.mm:1146: failed assertion `Number of columns in source exceeds source matrix size.'",
    ...
    'trace': MPSSkipInfo(
        NONCONTIGUOUS,
        skip=True,
        skip_msg="Crashes on MPS with err: 'mps.scatter' op operand #0 must be tensor of mps native type values, but got 'tensor<25xcomplex<f32>>"
    ),
    ...
    for op in op_db:
        if op.name in MPS_OPINFO_SKIPLIST:
            if not isinstance(MPS_OPINFO_SKIPLIST[op.name], Iterable):
                skips: Iterable[MPSSkipInfo] = [MPS_OPINFO_SKIPLIST[op.name]]
            else:
                skips: Iterable[MPSSkipInfo] = MPS_OPINFO_SKIPLIST[op.name]
            ...
            for skip in skips:
                ...
                if (
                    ...
                    and (
                        ...
                        or (op.variant_test_name == None and skip.variant == "")
                    )
                ):
                    ...
                    for test in skip.tests:
                        ...
                        decorator = DecorateInfo(
                            unittest.skip(skip.skip_msg) if skip.skip else unittest.expectedFailure,
                            test_module,
                            test,
                            device_type="mps",
                            dtypes=skip.dtypes,
