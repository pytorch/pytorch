...
"""
Python wrapper for NVSHMEM symmetric memory communication.

This module provides a high-level Python interface for the NVSHMEMSymmComm class
that handles NVSHMEM initialization, symmetric memory allocation, and device context
creation.

Usage:
    from torch._extern_triton._nvshmem_symm_comm import NVSHMEMSymmComm

    # Initialize communicator (call after process group is initialized)
    comm = NVSHMEMSymmComm(group_name="0", buffer_size=1024*1024, device_idx=0)
...
def is_nvshmem_symm_mem_available() -> bool:
    """Check if NVSHMEM symmetric memory is available."""
    try:
        from torch.distributed._symmetric_memory import is_nvshmem_available

        return is_nvshmem_available()
    except (AttributeError, RuntimeError, ImportError):
        return False


class NVSHMEMSymmComm:
    """
    High-level Python wrapper for NVSHMEM symmetric memory communication.

    This class wraps the C++ NVSHMEMSymmComm class and provides a Pythonic interface
    for initializing NVSHMEM symmetric memory communication and obtaining pointers
    ...
    def __init__(
        ...
        """
        Initialize NVSHMEM symmetric memory communication.
        ...
        Raises:
            RuntimeError: If NVSHMEM symmetric memory is not available or
                          if distributed is not initialized.
        """
        if not is_nvshmem_symm_mem_available():
            raise RuntimeError(
                "NVSHMEM symmetric memory is not available. "
                "Requires PyTorch built with USE_NVSHMEM=1."
            )

        if not dist.is_initialized():
            raise RuntimeError(
                "torch.distributed must be initialized before creating NVSHMEMSymmComm. "
        ...
        try:
            from torch._C._distributed_c10d import (  # type: ignore[attr-defined]
                NVSHMEMSymmComm as _NVSHMEMSymmComm,
            )

            self._comm = _NVSHMEMSymmComm(group_name, buffer_size, device_idx)
        except ImportError as e:
            raise RuntimeError(
                f"Failed to import NVSHMEMSymmComm: {e}. "
                "Make sure PyTorch is built with USE_NVSHMEM=1."
            ) from e
    ...
    def get_context_ptr(self) -> int:
        ...
        Returns:
            int64: Device pointer to NVSHMEMSymmContext
        """
        if self._comm is None:
    ...
    def __repr__(self) -> str:
        return (
            f"NVSHMEMSymmComm(group_name='{self.group_name}', "
