...
from torch.distributed._distributed_c10d import Backend as C10dBackend, HAS_DISTRIBUTED
...
class _MeshEnv(threading.local):
    ...
    def get_mesh_dim_by_name(
        ...
        if device_mesh.mesh_dim_names is None or len(device_mesh.mesh_dim_names) == 0:
            raise KeyError(
    ...
    def _get_slice_mesh_dims(
        ...
        if not all(
            mesh_dim_name in valid_mesh_dim_names for mesh_dim_name in mesh_dim_names
        ):
            raise KeyError(
    ...
    def from_group(
        ...
        *,
        ...
        if isinstance(group, ProcessGroup):
            ...
            if (isinstance(mesh, torch.Tensor) and mesh.tolist() != group_ranks) or (
                mesh is not None
