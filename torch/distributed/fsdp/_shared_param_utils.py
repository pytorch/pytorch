...
class SharedParamInfo(NamedTuple):
    """
    This includes information about shared parameters needed for higher-level
...
def get_shared_param_info_to_lca(
    ...
    algorithm based on a _union-_find data structure.
    """
    ...
    lca_query_to_lca = _run_tarjan_lca(root_module, module_to_sharing_modules)
...
def _run_tarjan_lca(
    root_module: nn.Module,
    edge_list: Dict[nn.Module, List[nn.Module]],
) -> Dict[Tuple[nn.Module, nn.Module], nn.Module]:
    ...
    def tarjan_lca(module: nn.Module):
        _make_set(module, module_to_parent, module_to_rank)
        for child_module in module.children():
            tarjan_lca(child_module)
            _union(module, child_module, module_to_parent, module_to_rank)
            module_to_ancestor[_find(module, module_to_parent)] = module
        visited_modules.add(module)
        if module not in edge_list:
            return
        for other_module in edge_list[module]:
            if other_module in visited_modules:
                ...
                lca_query_to_lca[lca_query] = module_to_ancestor[
                    _find(other_module, module_to_parent)
                ]

    tarjan_lca(root_module)
    return lca_query_to_lca
    ...
    for module in root_module.modules():
        for param in module.parameters(recurse=False):
            ...
            if param in visited_param_to_module:  # shared parameter
                query = _get_lca_query(visited_param_to_module[param], module)
...
def _union(
    ...
    root1 = _find(module1, module_to_parent)
    root2 = _find(module2, module_to_parent)
...
def _find(
    ...
    if module_to_parent[module] != module:
        module_to_parent[module] = _find(module_to_parent[module], module_to_parent)
