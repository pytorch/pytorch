...
def _get_param_to_fqns(
    ...
    based on its position in the nn.Module hierarchy. This is in contrast to FQNs
    that may be generated by other parallelisms or wrappers.
    ...
    ``FlatParameter`` s in the model and this mapping will only contain mappings from
    ``nn.Parameter`` s to singleton FQN lists.

    It is only in the case where FSDP was applied with ``use_orig_params=False`` where
    the original parameters are swapped out for a ``FlatParameter`` and there will be
    mappings from each ``FlatParameter`` to lists of FQNs corresponding to the original
    parameters.
