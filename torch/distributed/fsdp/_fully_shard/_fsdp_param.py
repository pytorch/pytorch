...
def free_storage(tensor: torch.Tensor) -> None:
    if (storage := tensor.untyped_storage()).size() != 0:
        storage.resize_(0)
        # Instrumentation: accesses to the freed (null) data pointer are
        # recorded so we can find them before turning this into an error
        torch._C._set_storage_data_ptr_access_log_msg(storage._cdata, _FREED_MSG)
