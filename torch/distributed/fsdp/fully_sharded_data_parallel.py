...
class FullyShardedDataParallel(nn.Module):
        ...
        with torch.autograd.profiler.record_function("FullyShardedDataParallel._post_backward_hook"):
            ...
            with torch.cuda.stream(self._streams["post_backward"]):
                ...
                if param._is_sharded:  # type: ignore[attr-defined]
                    ...
                    self._cast_grad_to_full_param_dtype(output, param.dtype)
                    ...
                    self._cast_grad_to_full_param_dtype(grad, param.dtype)
    ...
    def _cast_grad_to_full_param_dtype(
        self,
        grad: torch.Tensor,
        full_param_dtype: torch.dtype,
    ):
        """
        Casts gradients back to the full parameter dtype so that the optimizer
        step runs with that dtype. This performs an actual cast if
        ...
        if (
            ...
            grad.data = grad.data.to(dtype=full_param_dtype)
