...
class FullyShardedDataParallel(nn.Module):
    ...
    def _need_prefetch_pre_backward_hook(self) -> bool:
        if (
            ...
            and self.fsdp_graph_order[self._my_fsdp_idx_in_graph - 1].training_state != TrainingState_.BACKWARD_POST
        ):
            return True
        else:
            return False
    ...
    def _need_prefetch_post_backward_hook(self) -> bool:
        if (
            ...
            and self._my_fsdp_idx_in_graph is not None and self._my_fsdp_idx_in_graph > 0
            and self.fsdp_graph_order[self._my_fsdp_idx_in_graph - 1].training_state != TrainingState_.BACKWARD_POST
            and self.fsdp_graph_order[self._my_fsdp_idx_in_graph - 1]._need_rebuild_full_params
        ):
            return True
        else:
            return False
    ...
    def forward(self, *args: Any, **kwargs: Any) -> Any:
        ...
        if self not in self.fsdp_graph_order:
            self._my_fsdp_idx_in_graph = len(self.fsdp_graph_order)
    ...
    def _register_pre_backward_hooks(self, outputs: Any) -> Any:
        ...
        def _pre_backward_hook(*unused: Any) -> None:
            ...
            if self._need_prefetch_pre_backward_hook():
                self.fsdp_graph_order[self._my_fsdp_idx_in_graph - 1]._rebuild_full_params()  # type: ignore[operator]
