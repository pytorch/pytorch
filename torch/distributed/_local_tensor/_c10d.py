...
def _local_allreduce_coalesced_(
    tensors, process_group_so, reduce_op_so, async_op=True, timeout=-1
):
...
def _local_allgather_into_tensor_coalesced_(
    outputs, inputs, process_group_so, async_op=True
):
...
def _local_gather_(
    ...
    raise NotImplementedError(
        "LocalTensor does not support MPMD operations like gather (only root rank receives data). Use SPMD collective operations like allgather instead."
    )
...
def _local_alltoall_(
    ...
    assert len(input_tensors) == len(output_tensors) == len(ranks), (
        f"Number of input tensors ({len(input_tensors)}), output tensors ({len(output_tensors)}), "
        f"and ranks ({len(ranks)}) must match"
    )
...
def _local_barrier(tensor, process_group_so, device_ids, async_op=True, timeout=-1):
...
def _local_monitored_barrier_(
    tensor, process_group_so, device_ids, timeout, wait_all_ranks
):
...
def _local_send(tensors, process_group_so, dst, tag):
    ...
    raise NotImplementedError(
        "LocalTensor does not support MPMD operations like send. Use SPMD collective operations instead."
    )


def _local_recv_(tensors, process_group_so, src, tag):
    ...
    raise NotImplementedError(
        "LocalTensor does not support MPMD operations like recv. Use SPMD collective operations instead."
    )


def _local_recv_any_source_(tensors, process_group_so, tag):
    ...
    raise NotImplementedError(
        "LocalTensor does not support MPMD operations like recv_any_source. Use SPMD collective operations instead."
    )
