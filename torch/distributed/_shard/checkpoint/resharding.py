...
def _prepare_tensor_read(
    metadata: ShardedTensorStorageMetadata, tensor_out: Tensor
) -> List[TensorReadRequest]:
    """
    Prepare reading sharded tensor into tensor.

    Args:
        metadata: Metadata describing the persisted sharded tensor. Normally,
                  this is generated by func::`_prepare_sharded_tensor_write`.
        tensor_out: The dest tensor.

    Returns:
        A list of class::`TensorReadRequest`. When fullfilled,
        `tensor_out` load from the persisted sharded tensor.
    """
    read_reqs = []
    for storage_md in metadata.storage_metadata:
        shard_md_from_storage = storage_md.shard_metadata
        ...
        storage_key = storage_md.storage_key
        target_tensor = tensor_out.detach()

        for dim, (offset, length) in enumerate(zip(
            shard_md_from_storage.shard_offsets,
            shard_md_from_storage.shard_sizes,
        )):
            target_tensor = torch.narrow(
                target_tensor, dim, offset, length
            )
        ...
        read_reqs.append(
            TensorReadRequest(
                tensor=target_tensor,
                storage_key=storage_key,
                offsets=tuple([0] * len(target_tensor.size())),
                lengths=tuple(shard_md_from_storage.shard_sizes),
            )
        )
    return read_reqs
