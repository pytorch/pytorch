...
def _compute_local_shape_and_global_offset(
    ...
    # ---- Branch 1: keep legacy behavior for _StridedShards ----
    if any(isinstance(p, _StridedShard) for p in placements):
        ...
        for mesh_dim, placement in ordered_placements:
            ...
            if isinstance(placement, Shard):
                ...
                # Use placement helper that existing tests depend on
    ...
    # ---- Branch 2: fixed stage-wise logic for plain Shard/Replicate ----
    local_shape = list(global_shape)
    global_offset = [0] * len(global_shape)

    # We want to group meshes by tensor dimensions to coalesce sharding
    groups: defaultdict[int, list[int]] = defaultdict(list)

    for mesh_dim, placement in ordered_placements:
        if isinstance(placement, Shard):
            groups.setdefault(placement.dim, []).append(mesh_dim)
            shard_dim = placement.dim
            assert shard_dim < len(list(global_shape)), (
                f"Sharding dim {shard_dim} greater than tensor ndim {len(local_shape)}"
            )
    ...
    for d in range(len(global_shape)):
        ...
        if not axes:
            # not sharded
            local_shape[d] = global_shape[d]
            global_offset[d] = 0
            continue

        # Flatten coordinates into composite indices
        dims = [mesh_shape[ax] for ax in axes]
        coords = [my_coordinate[ax] for ax in axes]

        comp_idx = 0
        for dim, coord in zip(dims, coords):
            comp_idx = comp_idx * dim + coord
        ...
        local_len = base + (1 if comp_idx < rem else 0)
        start = base * comp_idx + min(comp_idx, rem)

        local_shape[d] = local_len
        global_offset[d] = L if local_len == 0 else start
