...
def compute_local_shape_and_global_offset(
    ...
    flat_sharding = any(isinstance(p, _FlatShard) for p in placements)
    if flat_sharding:
        if len(placements) > 1:
            raise NotImplementedError
        idx = 0
        placement = placements[idx]
        global_shape_list = list(global_shape)
        global_numel = math.prod(global_shape_list)
        flat_global_shape = torch.Size([global_numel])
        assert isinstance(placement, _FlatShard)
        shard_size, shard_offset = placement._local_shard_size_on_dim(
            flat_global_shape[placement.dim],
            mesh.size(idx),
            my_coordinate[idx],
            return_offset=True,
        )
        assert isinstance(shard_size, int), f"{shard_size=}"
        assert isinstance(shard_offset, int), f"{shard_offset=}"
    ...
    if strided_sharding:
        ...
        shard_idx = [
            ...
            for shard_dim, shard_idx_stride in enumerate(shard_idx_stride_by_mesh_dim)
        ]
