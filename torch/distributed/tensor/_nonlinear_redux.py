...
class NonLinearReductionsBase:
    def __init__(self):
        self._input_dtensor: None | dtensor.DTensor = None # type: ignore[assignment]
        self._local_tensor: None | torch.Tensor = None # type: ignore[assignment]
        self._dim: None | int = None # type: ignore[assignment]
        self._device_mesh: None | dtensor.DeviceMesh = None # type: ignore[assignment]
        self._placements: None | list[dtensor.Placement] = None # type: ignore[assignment]
    ...
    def _gather_tensors(self, gather_dim: int, gathered_idxs: torch.Tensor, local_redux: torch.Tensor):
        gathered_redux = local_redux
        for mesh_dim in self._shard_mesh_dims:
            gathered_redux = funcol.all_gather_tensor(
                gathered_redux, gather_dim=gather_dim, group=(self._device_mesh, mesh_dim)
            )
            gathered_idxs = funcol.all_gather_tensor(
                gathered_idxs, gather_dim=gather_dim, group=(self._device_mesh, mesh_dim)
            )
    ...
    def _convert_to_global_idxs(self, local_idx: torch.Tensor):
        ...
        if self._dim is None:
            ...
            gathered_idxs = torch.tensor(0, device=self._local_tensor.device, dtype=torch.long)
    ...
    def _collect_shard_mesh_dims(self):
        for mesh_dim, p in enumerate(self._placements):
            if isinstance(p, Shard):
                if self._dim is None or p.dim == (self._dim if self._dim >= 0 else self._local_tensor.ndim + self._dim):
...
class ArgMinMaxHandler(NonLinearReductionsBase):
    ...
    def __call__(
        ...
        gathered_redux, gather_idxs = self._gather_tensors(gather_dim, gathered_idxs, local_redux)
    ...
    def _compute_local_reduction(self, op_call: torch._ops.OpOverload):
        if self._dim is None:
            ...
            local_redux, local_idx = val_op(self._local_tensor, dim=self._dim, keepdim=True)
        return local_redux, local_idx
    

class MinMaxDimHandler(NonLinearReductionsBase):
    """Hanlder for aten.min.dim and aten.max.dim ops."""
    ...
    def __call__(
        ...
        if not self._shard_mesh_dims:
            return (
                dtensor.DTensor._op_dispatcher.wrap(
                    local_redux.reshape(self._expected_shape), output_sharding.output_spec
                ),
                dtensor.DTensor._op_dispatcher.wrap(
                    local_idx.reshape(self._expected_shape), output_sharding.output_spec
                ),
        ...
        return (
            dtensor.DTensor._op_dispatcher.wrap(
                final_redux.reshape(self._expected_shape), output_sharding.output_spec
            ),
            dtensor.DTensor._op_dispatcher.wrap(
                final_idx.reshape(self._expected_shape), output_sharding.output_spec
            ),
