...
def expand_to_full_mesh_op_strategy(
    ...
    *,
    ...
    if kwargs_strategy and op_schema.is_out_variant_op():
        ...
        expected_len = input_index + len(args_strategy) + len(kwargs_strategy)
        ...
        for strategy in single_mesh_dim_strategies:
            if len(strategy) < expected_len:
                ...
                output_placements = strategy[:input_index]
                out_placement = output_placements[0] if output_placements else None
                if out_placement is not None:
                    num_to_append = expected_len - len(strategy)
                    expanded_strategies.append(
                        list(strategy) + [out_placement] * num_to_append
                    )
                else:
