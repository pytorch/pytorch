...
def _expand_single_dim_strategy_to_mesh(
    ...
    single_dim_strategy: Callable[
        [ArgsType, KwargsType], list[list[Placement | PlacementPlaceholder]]
    ],
    ...
    def expanded_strategy(
        ...
        for spec in op_schema.args_spec:
            ...
            for p in spec.placements:
                # This is ugly, the point is we don't care about the dim,
                # because we're going to replace the dim with the placeholder's dim when expanding the sharding rule,
                # but we do care about the split-factor so we can't just store the unique types
                # (e.g. Shard, _StridedShard).
                if isinstance(p, _StridedShard):
                    sharding_types.add(_StridedShard(1234, split_factor=p.split_factor))
                elif isinstance(p, Shard):
                    sharding_types.add(Shard(1234))
        ...
        for s in strategies_over_one_mesh_dim:
            if not any(isinstance(p, PlacementPlaceholder) for p in s):
                continue
            for placement_base in sharding_types:
                expanded_strategy = []
                success = True
                for maybe_placeholder in s:
                    if isinstance(maybe_placeholder, PlacementPlaceholder):
                        # TODO: too lazy to modify Placement base in C++, making mypy happy this way for now
                        if isinstance(
                            placement_base,
                            (
                                Shard,
                        ...
                        ) and placement_base._supports_placeholder(maybe_placeholder):
                            # we combine the tensor dim to shard from the placeholder
                            # with other metadata (e.g. split_factor) from the sharding class
                            expanded_strategy.append(
                                placement_base._from_placeholder(maybe_placeholder)
                            )
                        else:
                            success = False
                            break
