...
class OffsetBasedRNGTracker(_RNGStateTracker):
    ...
    def __init__(
        ...
        if fake_mode is not None:
            ...
            self._initial_rng_state: Optional[torch.Tensor] = None
        else:
            ...
            self._initial_rng_state = rng_state

    def _get_device_state(self) -> Optional[torch.Tensor]:
        from torch._subclasses.fake_tensor import FakeTensorMode
        
        # Return None during tracing
    ...
    def _distribute_region(
        ...
        if self._initial_rng_state is None and FakeTensorMode.current_mode() is None:
            self._initial_rng_state = self._get_device_state()
