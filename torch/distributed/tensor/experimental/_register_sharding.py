...
def register_sharding(op: OpOverload | list[OpOverload]):
    ...
    def custom_strategy(
        ...
        # filter out "out" kwargs for custom ops
        out_kwarg = op_schema.kwargs_schema.get("out")
        if out_kwarg is not None and isinstance(out_kwarg, TupleStrategy):
            filtered_kwargs_schema = {
                k: v for k, v in op_schema.kwargs_schema.items() if k != "out"
            }
            op_schema = OpSchema(
                op=op_schema.op,
