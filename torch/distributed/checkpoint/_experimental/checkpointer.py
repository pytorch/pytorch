...
class SyncCheckpointer(Checkpointer):
    ...
    def load(
        ...
        **kwargs: Any,
        ...
        if strict and missing_keys is not None and missing_keys != []:
            raise RuntimeError(
                "Checkpoint at %s is missing keys: %s" % (path, missing_keys)
            )
    ...
    def close(self) -> None:
        ...
        For SyncCheckpointer, this is a no-op as there are no async resources to clean up.
        """
...
class AsyncCheckpointer(Checkpointer):
    ...
    def __init__(
        ...
        reader: CheckpointReader = CheckpointReader,
    ):
        """
