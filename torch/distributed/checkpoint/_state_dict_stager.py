...
class StateDictStager:
    ...
    def __init__(self, pin_memory: bool = False, share_memory: bool = False):
        ...
        def _deepcopy_dict(x, memo):
            ...
            for key, value in x.items():
                y[self.deepcopy_with_tensor_offload(key, memo)] = self.deepcopy_with_tensor_offload(value, memo)
            return y

        def _deepcopy_method(x, memo): # Copy instance methods
            return type(x)(x.__func__, self.deepcopy_with_tensor_offload(x.__self__, memo))
    ...
    def _stage_untyped_storage(self, storage : Storage, non_blocking: bool = False):
        """
        ...
        if self.pin_memory:
            ...
            f = weakref.finalize(new_storage, pin_memory_utils.unpin_memory, new_storage.data_ptr())
    ...
    def _offload_tensor(self, x, memo, non_blocking=False):
        ...
        if type(x) is torch.Tensor or x.data_ptr() != 0:
            ...
            copied_storage = self._stage_untyped_storage(untyped_storage, non_blocking=non_blocking)
            # Set the tensor data using the optimized storage
            y.set_(
                copied_storage, x.storage_offset(), x.size(), x.stride()
            )

        # Copy any attributes the tensor might have
        if hasattr(x, '__dict__'):
            for attr_name, attr_value in x.__dict__.items():
                setattr(y, attr_name, self.deepcopy_with_tensor_offload(attr_value, memo, non_blocking=non_blocking))
                
        if hasattr(x, '__slots__'):
            for slot in x.__slots__:
                if hasattr(x, slot):
                    setattr(y, slot, self.deepcopy_with_tensor_offload(getattr(x, slot), memo, non_blocking=non_blocking))
    ...
    def _reconstruct(self, x, memo, func, args,
