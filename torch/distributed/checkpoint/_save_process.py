...
class _ProcessGroupInitInfo:
    ...
    global_rank: int
    world_size: int
    tcp_store_master_addr: str | None
    tcp_store_master_port: int | None
    ...
    def __init__(self, process_group: Optional[dist.ProcessGroup] = None):
        ...
        self.tcp_store_master_addr = os.environ.get("MASTER_ADDR")

        # Let coordinator rank find a free port on the localhost. 
        # Broadcast the free port to all ranks; each rank in the
...
class _AsyncCheckpointProcess:
    ...
    def __init__(
        ...
        init_fn: Optional[Callable[None, None]] = None, 
    ):
        
        self.ctx = mp.get_context("spawn")
        self._mp_queue_send: mp.Queue = self.ctx.Queue()
        self._mp_queue_recv: mp.Queue = self.ctx.Queue()
        self._mp_manager: mp.Manager = self.ctx.Manager()
        self._response_dict: dict = self._mp_manager.dict()
        ...
        response = self._wait_for_response(_CheckpointSaveProcessControlOpts.INIT_COMPLETE)
    ...
    def _checkpointing_subprocess(
        ...
        try:
            ...
            logger.error(f"Checkpoint background process encountered an exception during init: {e}")
        ...
        try:
            while True:
                ...
                obj = recv.get()
                if isinstance(obj, _CheckpointSaveProcessControlOpts) and obj == _CheckpointSaveProcessControlOpts.TERMINATE:
                    logger.info("Terminating the checkpoint background process.")
                    return
                assert isinstance(obj, _AsyncCheckpointRequest)
                logger.info(f"Received async checkpoint request with id={obj.checkpoint_request_id.checkpoint_id}")
                ...
                logger.info(f"Submitted checkpoint save request for checkpoint_id={obj.checkpoint_request_id}")
        except BaseException as e:
            logger.error(f"Checkpoint background process encountered an exception: {e}")
            raise
        finally:
...
def execute_save_in_process_impl(
    *,
