...
class _ProcessGroupInitInfo:
    ...
    def __init__(self, process_group: Optional[dist.ProcessGroup] = None):
        ...
        self.tcp_store_master_addr = dist_wrapper.broadcast(
            step="get_master_addr",
            map_fun=get_master_addr,
        )
        self.tcp_store_master_port = dist_wrapper.broadcast(
            step="get_free_port",
            map_fun=get_free_port,
        )
...
class _AsyncCheckpointProcess:
    def __init__(
        ...
        self._mp_manager = self.ctx.Manager()
        self._response_dict = self._mp_manager.dict()
        ...
        response = self._wait_for_response(
            _CheckpointSaveProcessControlOpts.INIT_COMPLETE
        )
    ...
    def save(
        ...
        *,
        ...
        result = self._wait_for_response(checkpoint_request_id)
        assert isinstance(result, Metadata)
        return result

    def _wait_for_response(self, key: Any) -> Any:
        while key not in self._response_dict:
            ...
            continue
        if isinstance(self._response_dict[key], Exception):
            raise self._response_dict[key]
    ...
    def _execute_save(
        ...
        *,
        ...
        try:
            ...
            response_dict[checkpoint_request_id] = metadata
        except Exception as e:
            response_dict[checkpoint_request_id] = e
            raise e
    ...
    def _checkpointing_subprocess(
        ...
        try:
            ...
            response_dict[_CheckpointSaveProcessControlOpts.INIT_COMPLETE] = True
        except Exception as e:
            logger.error(
                f"Checkpoint background process encountered an exception during init: {e}"  # noqa: G004
            )
            response_dict[_CheckpointSaveProcessControlOpts.INIT_COMPLETE] = e
            raise
        ...
        try:
            while True:
                ...
                f: Future = executor.submit(
                    _AsyncCheckpointProcess._execute_save,
                    response_dict,
...
class _AsyncCheckpointExecutor(abc.ABC):
    ...
    def execute_save(
        ...
        *,
        ...
        This method is intended to be used as an abastraction for
