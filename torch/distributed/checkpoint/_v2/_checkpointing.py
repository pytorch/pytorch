...
from typing import Any, Callable, Dict, List, Optional, TypeAlias, Union, BinaryIO
...
from torch.distributed.checkpoint._v2._metadata import (
    Metadata
)
...
class CheckpointLayout(abc.ABC):
    ...
    def get_file_mappings_for_write(
        self, rank, state_dict: Dict[str, Any]
    ) -> Dict[str, Any]:
        """
        Examples usecases:
...
class SerializationFormat(abc.ABC):
    ...
    @abc.abstractmethod
    def serialize(self, obj: object) -> io.BytesIO:
        pass
...
class CheckpointWriter:
    ...
    def write_checkpoint(
        ...
        if self._config.save_manifest_with_checkpoint:
            self._storage.write(
                os.path.join(Path(root_dir) / metadata_path, "METADATA"),
                io.BytesIO(json.dumps(asdict(metadata)).encode("utf-8")),
            )

        file_paths = self._layout.get_file_mappings_for_write(self._rank_info, state_dict)
        for file_path, obj in file_paths.items():
            data = self._serialization_format.serialize(obj)
            file_path = os.path.join(root_dir, file_path)
            self._storage.write(file_path, data) 
