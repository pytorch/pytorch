...
CACHED_STATE_DICT: Optional[STATE_DICT_TYPE] = None
    ...
    def stage_state_dict() -> Future[STATE_DICT_TYPE]:
        ...
        if isinstance(storage_writer, AsyncStager) and not use_default_staging:
            ...
            if not block_on_staging:
                global CACHED_STATE_DICT
                if not CACHED_STATE_DICT:
                    CACHED_STATE_DICT = _create_cpu_state_dict(state_dict, pin_memory=True, share_memory=True)
                cpu_state_dict = CACHED_STATE_DICT
            else:
                cpu_state_dict = _create_cpu_state_dict(state_dict, pin_memory=True, share_memory=True)
            staging_future = staging_executor.submit(default_stage, state_dict, cpu_state_dict)
