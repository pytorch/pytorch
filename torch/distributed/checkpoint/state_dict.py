...
class StateDictOptions:
    ...
    fqn_modifier_name: str = "fqn_modifiers"
...
def _get_fqns(
    model: nn.Module,
    name: str,
    fqn_prefix_modifiers: str = "fqn_modifiers",
    ...
    for i, curr_obj_name in enumerate(obj_names):
        if isinstance(curr_obj, DDP):
            ...
            if hasattr(curr_obj, fqn_prefix_modifiers):
                if removed_fqn := getattr(curr_obj, fqn_prefix_modifiers)().get(
                    curr_obj_name
                ):
...
def _iterate_valid_model_state(model, fqn_prefix_modifiers="fqn_modifiers"):
    ...
    def recurse(module: nn.Module, curr_fqn: str) -> Generator:
        ...
        for name, submodule in module.named_children():
            ...
            # at fqn_prefix_modifiers in input to align with the function of state_dict_hook
            if (
                hasattr(module, fqn_prefix_modifiers)
                and name in getattr(module, fqn_prefix_modifiers)().values()
            ):
...
def _load_model_state_dict(
    ...
    local_state_dict = {}
    for key, value in _iterate_valid_model_state(model, info.fqn_modifier_name):
        fqns = _get_fqns(model, key, info.fqn_modifier_name)
        fqns_with_prefix = _get_fqns(
            model,
            key,
            info.fqn_modifier_name,
