...
def _flatten_optim_state_dict(state_dict: OptimizerStateType) -> dict[str, ValueType]:
    ...
    def _flatten_state_nested_dict(nested_dict: dict[str, Any], prefix: str) -> dict[str, ValueType]:
        """
    ...
    for fqn, state in cast(DictValueType, state_dict[_STATE]).items():
        ...
        ret.update(_flatten_state_nested_dict(cast(dict[str, Any], state), state_prefix))

    # Handle the "param_groups" section with two-level flattening (original approach)
...
def _unflatten_optim_state_dict(
    ...
    """
    def _reconstruct_nested_value(flattened_key: str, flattened_dict: dict[str, ValueType]) -> Any:
        """
        ...
        """
        if flattened_key in flattened_dict:
            # Existing keys mean no nesting, directly return.
            return flattened_dict[flattened_key]
        ...
        # Return the reconstructed nested dictionary, or None if no nested keys were found
        ...
        for param in param_group[_PARAMS]:
            for fqn in info.fqn_param_mapping[param]:
                ...
                for state_name in optim.state[param].keys():
                    ...
                    reconstructed_value = _reconstruct_nested_value(flattened_state_key, state_dict)
                    if reconstructed_value is not None:
                        cast(DictValueType, state[fqn])[state_name] = reconstructed_value
