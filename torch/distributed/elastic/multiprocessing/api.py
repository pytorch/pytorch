...
class PContext(abc.ABC):
    ...
    def __init__(
        ...
        duplicate_stdout_filters: list[str] = [],
        duplicate_stderr_filters: list[str] = [],
    ):
        self.name = name
        ...
        if duplicate_stdout_filters:
            self._tail_logs.append(
                TailLog(
                    ...
                    log_line_prefixes,
                    log_line_filter=lambda line: any(needle in line for needle in duplicate_stdout_filters),
                )
            )
        ...
        if duplicate_stderr_filters:
            self._tail_logs.append(
                TailLog(
                    ...
                    log_line_prefixes,
                    log_line_filter=lambda line: any(needle in line for needle in duplicate_stderr_filters),
                )
            )
