...
def _parse_rendezvous_config(config_str: str) -> Dict[str, str]:
    """Extracts key-value pairs from a rendezvous configuration.
    ...
    for kv in key_values:
        ...
        if not key:
            raise ValueError(
                "The rendezvous config string must be in format "
        ...
        if values:
            ...
            value = None
        if not value:
            raise ValueError(f"The '{key}' rendezvous config must have a value specified.")
...
def _try_parse_port(port_str: str) -> Optional[int]:
    """Tries to extract the port number from a string."""
...
def _parse_rendezvous_endpoint(endpoint: Optional[str], default_port: int) -> Tuple[str, int]:
    ...
    if len(rest) == 1:
        ...
        if port is None or port >= 2 ** 16:
            raise ValueError(
                "The port number of the rendezvous endpoint must be an integer between 0 and 65536."
            )
    else:
        port = default_port
    ...
    if not re.match(r"^[\w\.:-]+$", host):
        raise ValueError(
            "The hostname of the rendezvous endpoint must be a dot-separated list of labels, an "
