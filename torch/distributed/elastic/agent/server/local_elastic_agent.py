...
from torch.distributed.elastic.agent.server.health_check_server import start_healthcheck_server
...
class LocalElasticAgent(SimpleElasticAgent):
    ...
    def __init__(
        ...
        self._stop_healthcheck_cbk: Optional[Callable[[], None]] = None
    ...
    def _setup_healthcheck(self) -> None:
        ...
        if healthcheck_port is not None:
            ...
            if self._worker_watchdog is None:
                log.info("FileTimerServer doesn't exist, using current time as dummy callback")
                alive_callback = LocalElasticAgent._get_current_time_secs
            else:
                alive_callback = self._worker_watchdog.get_last_progress_time

            self._stop_healthcheck_cbk = start_healthcheck_server(
    ...
    def _shutdown(self, death_sig: signal.Signals = signal.SIGTERM) -> None:
        ...
        if self._stop_healthcheck_cbk is not None:
            self._stop_healthcheck_cbk()
            self._stop_healthcheck_cbk = None
        if self._pcontext:
