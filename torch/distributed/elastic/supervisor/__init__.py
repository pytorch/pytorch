...
class Future(Generic[T]):
    ...
    def __repr__(self) -> str:
        status = (
            ...
            else "complete" if self._complete else "incomplete"
        )
...
def wait_on(
    ...
    """
    coll: MutableSequence[Tuple[T, Future[V]]] = deque()
    ...
    try:
        wait([_capture(el) for el in l], timeout, return_when)
    except TimeoutError:
        pass
...
class Host:
    ...
    def __init__(self, context: "Context") -> None:
        ...
        self._proc_table: weakref.WeakValueDictionary[int, Process] = (
...
class Context:
    ...
    def _heartbeat_ttl(self, ttl):
        self._heartbeats += 1
        self._heartbeat_ttl_sum += ttl
        self._heartbeat_min_ttl: int = min(self._heartbeat_min_ttl, ttl)
