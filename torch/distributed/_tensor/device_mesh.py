...
class _MeshEnv:
    ...
    def create_child_mesh(
        ...
        for mesh_1d in pg_ranks_by_dim:
            sub_mesh = DeviceMesh(
                device_mesh.device_type, mesh_1d, mesh_dim_names=(mesh_dim_name, ), _init_process_groups=False
            )
    ...
    def get_parent_mesh_dim(self, device_mesh: "DeviceMesh") -> int:
        """
        Return the index of the mesh dim in the parent mesh.
        The device_mesh passed in is sliced out from the parent mesh.
        """
        cur_mesh_dim_names = device_mesh.mesh_dim_names
        assert len(cur_mesh_dim_names) == 1, "The child mesh can only be a 1D mesh."
        cur_mesh_dim_name = cur_mesh_dim_names[0]
        return self.get_parent_mesh(device_mesh).mesh_dim_names.index(cur_mesh_dim_name)
