...
class ShardingPropagator:
    ...
    ) -> Union[None, TensorMeta, List[Optional[TensorMeta]], Tuple[Optional[TensorMeta], ...]]:
        """
        ...
        if op_schema.op in self.op_strategy_funcs:
            ...
            if isinstance(op_strategy, OpStrategy):
                ...
                if op_schema.return_type_tuple_tensors():
                    ...
                    # TODO: don't assign meta
                    output_spec: OutputSpecType = tuple(
                        [
                            # output_strategy.output_spec # don't reuse but create from it w/o tensor meta
                            DTensorSpec(
                ...
                for strategy in op_strategy.childs:
                    # TODO: how to handle None output in tuple?
                    # output sharding should be None for None output in tuple.
                ...
                print(f"input spec lists = {input_specs_list}")
                ...
                if op_strategy.op_type == 0:
                    ...
                    idx = 0
                    print(f"sharding prop, op_call = {op_schema.op}")
                    for arg_spec in op_schema.args_schema:
                        # TODO: if output is None, do we need to redistribute?
                        print(arg_spec)
                        if isinstance(arg_spec, DTensorSpec) and input_specs_list:
                            print(f"idx={idx}, arg_spec={arg_spec}, input_specs_list[{idx}]={input_specs_list[idx]}")
                            if arg_spec.placements != input_specs_list[idx].placements:
                                needs_redistribute = True
                    ...
                    raise ValueError(f"Invalid TupleStrategy op type {op_strategy.op_type}")
