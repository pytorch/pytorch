# Copyright (c) Meta Platforms, Inc. and affiliates
from typing import Callable, List, Optional, Sequence, Tuple, Union

import torch
from torch.distributed._tensor import DeviceMesh, DTensor
from torch.distributed._tensor.placement_types import Placement
from torch.utils._pytree import tree_flatten, tree_unflatten


PlacementType = Sequence[Placement]
InputPlacements = Union[PlacementType, Tuple[PlacementType, ...]]
OutputPlacements = Union[PlacementType, Tuple[PlacementType, ...]]


def dmap(
    func: Callable,
    out_placements: OutputPlacements,
    device_mesh: Optional[DeviceMesh] = None,
    in_placements: Optional[InputPlacements] = None,
    resharding: bool = False,
):
    """
    ``dmap`` is an experimental API that allows users to apply on :class:`DTensors`
    a function that is written to be applied on :class:`~torch.Tensors`.

    Args:
        func (Callable): the function to be applied on each local shard of :class:`DTensor`s.
        out_placements (Union[`PlacementType`, Tuple[`PlacementType`, ...]]):
            the desired placements of the output :class:`DTensor`s.
        device_mesh (:class:`DeviceMesh`, optional):
            the device mesh that all the :class:`DTensor`s are placed on. If not specified,
            this will be inferred from the input :class:`DTensor`s' device mesh. `dmap` requires
            every :class:`DTensor`s to be placed on the same device mesh.
        in_placements (Union[`PlacementType`, Tuple[`PlacementType`, ...]], optional):
            the required placements of the input :class:`DTensor`s. If not specified,
            this will be inferred from the input :class:`DTensor`s' device mesh.
        resharding (bool, optional):
            the bool value indicating whether to reshard the input :class:`DTensor`s when
            their placements are different from the required input placements. If this value
            is `False` and a :class:`DTensor` has a different placement, an exception will be
            raised. Default: `False`.

    Returns:
        A `Callable` that applies `func` to each local shard of the input :class:`DTensor`
        and returns a :class:`DTensor` constructed from the return value of `func`.

    Raises:
        AssertionError: If the input :class:`DTensor`s are not placed on the same device mesh, or
        if they are placed on a different device mesh than the `device_mesh` argument passed in.

        AssertionError: If the placements in `out_placements` for non-:class:`DTensor` return value
        is not `None`.

        ValueError: If `resharding=False` but the input :class:`DTensor` needs resharding according to
        `in_placements`.

    Example:
        >>> # xdoctest: +SKIP("distributed")
        >>> def mm_allreduce_forward(device_mesh, W, X):
        >>>     partial_sum_tensor = torch.mm(W, X)
        >>>     reduced_tensor = funcol.all_reduce(partial_sum_tensor, "sum", device_mesh)
        >>>     return reduced_tensor
        >>>
        >>> W = torch.randn(12, 8, requires_grad=False)
        >>> X = torch.randn(8, 16, requires_grad=False)
        >>> Y = torch.mm(W, X)
        >>> row_wise = [Shard(0)]  # row-wise sharding placements on 1-d mesh
        >>> col_wise = [Shard(1)]  # col-wise sharding placements on 1-d mesh
        >>>
        >>> # d_mm_allreduce_forward is the function wrapped with DTensor/Tensor convertion
        >>> d_mm_allreduce_forward = dmap(
        >>>     mm_allreduce_forward,
        >>>     out_placements=[Replicate()],
        >>>     device_mesh=device_mesh,
        >>>     in_placements=[col_wise, row_wise],
        >>> )
        >>>
        >>> dW = distribute_tensor(W, device_mesh, col_wise)  # col-wisely sharded W tensor
        >>> dX = distribute_tensor(X, device_mesh, row_wise)  # row-wisely sharded X tensor
        >>> dY = d_mm_allreduce_forward(dW, dX)  # apply d_mm_allreduce_forward to DTensors

    NOTE: This API is currently experimental and subject to change
    """

    def wrapped(*args, **kwargs):
        # process input args
        flat_args, args_spec = tree_flatten(args)

        # we assume every DTensor object is placed on the same device mesh
        flat_local_args = []
        nonlocal device_mesh  # access var device_mesh from the outer scope
        for idx, arg in enumerate(flat_args):
            if isinstance(arg, DTensor):
                if device_mesh is None:  # infer device mesh from the DTensor arg
                    device_mesh = arg.device_mesh

                assert arg.device_mesh == device_mesh, (
                    f"arg {arg} in dmap has a mismatched device mesh:"
                    f"{arg} has device mesh {arg.device_mesh} while"
                    f"the input device mesh is {device_mesh}!"
                )
                if in_placements is not None:
                    spec = (
                        in_placements[idx]
                        if isinstance(in_placements, tuple)
                        else in_placements
                    )
                    if not isinstance(spec, tuple):
                        spec = tuple(spec)

                    if arg.placements != spec:
                        if resharding:
                            # redistribute to input placements
                            arg = arg.redistribute(device_mesh, spec)
                        else:
                            raise ValueError(
                                f"arg {arg} in dmap has a mismatched placements:"
                                f"arg placements is {arg.placements} but the input"
                                f"placements is {spec}!"
                                "If resharding is wanted, set resharding=True to dmap."
                            )

                flat_local_args.append(arg.to_local())
            else:
                flat_local_args.append(arg)

        local_args = tree_unflatten(flat_local_args, args_spec)

        out = func(device_mesh, *local_args, **kwargs)

        # process output
        flat_out, out_spec = tree_flatten(out)
        flat_dist_out: List[Optional[DTensor]] = []
        for idx, out in enumerate(flat_out):
            spec = (
                out_placements[idx]
                if isinstance(out_placements, tuple)
                else out_placements
            )
            if isinstance(out, torch.Tensor):
                assert not isinstance(
                    out, DTensor
                ), f"torch.Tensor output expected but received {type(out)}: {out}"

                flat_dist_out.append(DTensor.from_local(out, device_mesh, spec))
            else:
                assert (
                    out is None
                ), f"Non-tensor output {out} expects None placements but received {spec}!"

                flat_dist_out.append(out)

        return tree_unflatten(flat_dist_out, out_spec)

    return wrapped
