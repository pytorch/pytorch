...
def _on_leader_follower_report_shutdown_intent(sequence_id, worker_name):
    ...
    intent_worker_names = _wait_all_workers_sequence_id_to_states[sequence_id].intent_worker_names
    assert (
...
def _set_proceed_shutdown_signal(sequence_id):
    ...
    assert not proceed_signal.is_set(), "Termination signal sequence id {} got set twice.".format(
        sequence_id=sequence_id
    )
    proceed_signal.set()
