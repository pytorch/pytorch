...
class PipelineStage(_PipelineStageBase):
    ...
    def _initial_model_run_for_input_outputs(self, args, kwargs):
        if self.is_first:
            ...
            logger.debug(
                "%s received %s from stage %s",
                self.stage_index,
                objects,
                self.prev_stage,
            )
            recv_args, input_kwargs = objects[0], {}
            assert isinstance(recv_args, Iterable)
            input_args = [
        ...
        with torch.no_grad():
            logger.debug(
                "%s running with %s, %s", self.stage_index, input_args, input_kwargs
            )
            outputs = self.submod(*input_args, **input_kwargs)
            logger.debug("%s finished fwd", self.stage_index)
        ...
        if not self.is_last:
            ...
            logger.debug(
                "%s sending %s to stage %s", self.stage_index, outputs, self.next_stage
            )
        dist.barrier()
