...
try:
    ...
    # (selected via the "nccl2-lazy" backend / entry point).
    from torch._C._distributed_c10d import ProcessGroupNCCL2Lazy

    ProcessGroupNCCL2Lazy.__module__ = "torch.distributed.distributed_c10d"
    __all__ += ["ProcessGroupNCCL2Lazy"]
except ImportError:
    pass
...
def _create_nccl2_lazy_process_group(
    opts: _DistributedBackendOptions, backend_options: Any | None
) -> C10DBackend:
    if not is_nccl_available():
        raise RuntimeError("Distributed package doesn't have NCCL built in")
...
def _register_builtin_nccl2_lazy_backend() -> None:
    Backend.register_backend(
        "nccl2-lazy",
        _create_nccl2_lazy_process_group,
        extended_api=True,
        devices=["cuda"],
        _backend_type=ProcessGroup.BackendType.CUSTOM,
    )
