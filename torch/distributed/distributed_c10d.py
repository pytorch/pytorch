...
_pg_names: Dict[ProcessGroup, Tuple[str, str]] = {}
...
class _World:
    ...
    @property
    def pg_names(self) -> Dict[ProcessGroup, Tuple[str, str]]:
        """
        Process group's names, map from ProcessGroup to pg_name: (group_name, group_uid).
        group_name is user-defined group name
        group_uid is a global unique id generated by c10d
...
@_time_logger
def _store_based_barrier(rank, store, group_uid, rendezvous_count, timeout, logging_interval=timedelta(seconds=10)) -> None:
    """
    ...
    """
    store_key = f"{STORE_BASED_BARRIER_PREFIX}:{group_uid}"
...
def _get_group_size_by_name(group_uid: str) -> int:
    group = _resolve_process_group(group_uid)
    return group.size()
...
def init_process_group(
    ...
    """
    pg_name (group_name, group_uid) is not visible to users unless they access
    internals of c10d. This means we can ignore the value
    they provide as it not exposed in a public way.
    """
    group_uid = _process_group_uid([], use_hashed_uid=False)
    if backend == Backend.MPI:
        ...
        default_pg, _ = _new_process_group_helper(
            -1, -1, [], backend, None, ("default_pg", group_uid), timeout=timeout
        )
    ...
    if _is_barrier_after_init() == 1:
        ...
        if backend == Backend.MPI:
            ...
            _store_based_barrier(rank, store, group_uid, world_size, timeout)
...
def _new_process_group_helper(
    ...
    (group_name, group_uid) = pg_name

    if _check_duplicate_group_uid(group_uid):
        raise ValueError(
            f"The specified unique pg identifier (group_uid) has already been "
            f"created, please use a different group_uid"
        )
    ...
    prefix_store = PrefixStore(f"{group_uid}/", store)
    ...
    for device, backend_str in backend_config.get_device_backend_map().items():
        ...
        if backend_str == Backend.MPI:
            ...
            if not extended_api:
                ...
                dist_backend_opts.group_id = group_uid
    ...
    _world.pg_names[pg] = pg_name
    pg._set_pg_name(pg_name)
    _register_process_group(group_uid, pg)

    _world.pg_backend_config[pg] = str(backend_config)
    # "" is the default tag for user PGs
    if pg_tag in [None, ""]:
        pg_tag = f"ptd:{group_uid}"
    ...
    if group is None or group == GroupMember.WORLD:
        ...
        if tag is not None:
            try:
                ...
                pass
        _unregister_process_group(pg.group_uid)
...
def _check_duplicate_group_uid(group_uid) -> bool:
    for (_, uid) in _world.pg_names.values():
        if group_uid == uid:
            return True
    return False

def _process_group_uid(ranks, use_hashed_uid) -> str:
    global _world
    if use_hashed_uid:
        group_uid = _hash_ranks(ranks)
        while _check_duplicate_group_uid(group_uid):
            group_uid = hashlib.sha1(bytes(group_uid + "_", "utf-8")).hexdigest()
    else:
        group_uid = str(_world.group_count)
...
@_time_logger
def new_group(ranks=None, timeout=None, backend=None, pg_options=None, group_name=None, use_local_synchronization=False):
    """
    ...
    Args:
        ...
        use_local_synchronization (bool, optional): perform a group-local
            ...
            join the barrier.
        group_name (str, optional): name of the group user want to specify, e.g. FSDP.
            If not specified, it will be empty.
    ...
    """
    return _new_group_with_tag(ranks, timeout, backend, pg_options, None, group_name, use_local_synchronization=use_local_synchronization)
...
def _new_group_with_tag(
    ...
    group_uid = _process_group_uid(ranks, use_hashed_uid=use_local_synchronization)
    ...
    if _is_barrier_after_init() == 1:
        ...
        if backend == Backend.MPI:
            ...
            # Use store based barrier here since barrier() used a bunch of
            # default devices and messes up NCCL internal state.
            _store_based_barrier(global_rank, barrier_store, group_uid, world_size, timeout)
...
def _get_process_group_name(pg: ProcessGroup) -> Tuple[str, str]:
    return _world.pg_names.get(pg, ("None", "None"))
