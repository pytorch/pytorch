...
def shrink_group(
    ...
    new_nccl_backend = nccl_backend.shrink_process_group(sorted(excluded_ranks_set), shrink_flags)

    # Step 6: Handle cleanup and creation of new process group
    return _finalize_shrunk_group(target_group_info, excluded_ranks_set, new_nccl_backend)
...
def _validate_nccl_shrink_requirements(group_info: dict) -> 'ProcessGroupNCCL':
    ...
    if nccl_version < NCCL_MIN_VERSION_FOR_SHRINK:
        raise RuntimeError(
            f"ProcessGroupNCCL.shrink_process_group requires NCCL version >= {NCCL_MIN_VERSION_FOR_SHRINK}. "
...
def _validate_and_process_excluded_ranks(ranks_to_exclude: List[int], group_info: dict) -> set:
...
def _finalize_shrunk_group(group_info: dict, excluded_ranks_set: set, new_nccl_backend) -> ProcessGroup:
    ...
    remaining_ranks = [rank for rank in original_ranks if rank not in excluded_ranks_set]
    ...
    rank_mapping = {global_rank: group_rank for group_rank, global_rank in enumerate(remaining_ranks)}
...
def _destroy_all_other_groups(exclude_group: Optional[ProcessGroup] = None) -> None:
    ...
    for pg in groups_to_destroy:
        try:
            ...
            logger.warning(f"Failed to abort process group {_get_process_group_name(pg)}: {e}")
