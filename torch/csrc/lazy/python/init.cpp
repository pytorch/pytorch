...
#if defined(BUILD_LAZY_TS_BACKEND) && \
    !(defined(FBCODE_CAFFE2) || defined(OVRSOURCE))
#include <torch/csrc/lazy/ts_backend/ts_backend_impl.h>
#include <torch/csrc/lazy/ts_backend/ts_lowering_context.h>
#endif // BUILD_LAZY_TS_BACKEND && !(FBCODE_CAFFE2 || OVRSOURCE)
#include <string>
#include <utility>
#include <vector>
...
void initLazyBindings(PyObject* module) {
  ...
  lazy.def("_counter_value", [](const std::string& name) -> py::object {
    const torch::lazy::CounterData* data = torch::lazy::GetCounter(name);
...
#else
      TORCH_CHECK(false, "TorchScript backend not available");
#endif // BUILD_LAZY_TS_BACKEND && !(FBCODE_CAFFE2 || OVRSOURCE)
  });
