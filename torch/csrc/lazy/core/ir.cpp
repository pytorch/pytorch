...
void Node::addShapeFromFunction(const std::function<Shape()>& shape_fn) {
  shapes_.emplace_back(getShapeFromFunction(shape_fn));
}

// Add a shape generated by the shape_fn
Shape Node::getShapeFromFunction(const std::function<Shape()>& shape_fn) {
  if (hasBackend()) {
    return getBackend()->GetShapeFromFunction(shape_fn, hash_with_sizes());
  } else {
    return shape_fn();
  }
}
