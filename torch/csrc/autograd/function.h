 ...
 public:
  ...
  explicit Node(uint64_t sequence_nr, edge_list&& next_edges = edge_list())
    ...
    if (AnomalyMode::should_save_traceback()) {
      ...
      // If traceback saving is enabled and graph is constructed, then assign
      // the currently evaluating node as the parent of this node. A parent is a
      // Node where this Node is created. We are tracking the parents to track
      // multiple backward operations.
      assign_parent();
    }
