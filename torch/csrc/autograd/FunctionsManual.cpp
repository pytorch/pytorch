...
// The kernels' nonfinite_to_far.
static Tensor gs_pixel_far(const Tensor& coord) {
  return at::nan_to_num(coord, -100.0, 1.0e15, -1.0e15);
}
...
static std::pair<Tensor, Tensor> gs_compute_coords(
  ...
  Tensor ix = pixel_coords ? gs_pixel_far(coord)
...
std::tuple<Tensor, Tensor, Tensor> grid_sampler_2d_double_backward(
  ...
  if (interpolation == GridSamplerInterpolation::Bilinear) {
    ...
    auto raw = [&](int64_t axis, double scale) {
      if (pixel_coords) {
        // past 2^63 floor() has no index to convert to
        return gs_pixel_far(grid_acc.select(-1, axis)).clamp(-9.0e18, 9.0e18);
      }
