...
Tensor mul_tensor_backward(
  ...
  // NOTE: alternatively, we could use _sparse_sum, but _sparse_sum
  // does not support keepdim which is very much needed in this case.
  const auto handle_sparse_sparse_case = [](const Tensor& self_grad,
                                            IntArrayRef self_sizes) -> Tensor {
    // No broadcasting in dense dims, no need to modify anything.
    if (self_grad.sizes() == self_sizes) {
      return self_grad;
    }
    ...
    const auto self_sparse_dim = self_grad.sparse_dim();
    ...
    for (const auto d : c10::irange(self_sparse_dim, self_grad.dim())) {
      // If d broadcasts...
      if (self_sizes[d] == 1) {
  ...
  if (other.is_sparse() && self_layout == c10::kSparse) {
    self_grad = handle_sparse_sparse_case(self_grad, self_sizes);
  }
