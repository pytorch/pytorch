...
Tensor index_add_source_backward(const Tensor& grad, int64_t dim, const Tensor& index, IntArrayRef source_size, Scalar alpha) {
  ...
  if (source_size.size() == 0) {
    return maybe_multiply(grad.index_select(dim, index.squeeze(0)), alpha);
  }
  ...
  return maybe_multiply(source_grad, alpha);
}
