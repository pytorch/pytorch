...
std::tuple<Tensor, Tensor, Tensor> attn_backward(
  ...
  if (grad_o.defined()) {
    Tensor q_o =
        at::matmul(at::matmul(grad_o, v.transpose(0, 1)) * (1 - a.pow(2)), k);
    Tensor k_o = at::matmul(
        (at::matmul(grad_o, v.transpose(0, 1)) * (1 - a.pow(2)))
            .transpose(0, 1),
        q);
  ...
  Tensor k_a = at::matmul((grad_a * (1 - a.pow(2))).transpose(0, 1), q);
  return std::tuple<Tensor, Tensor, Tensor>(
      q_a, k_a, at::_efficientzerotensor(v.sizes(), v.options()));
}
