...
Tensor sgn_backward(Tensor result, Tensor grad, Tensor self) {
  if (self.is_complex()) {
    ...
    return at::where(
        abs == 0.0,
        at::zeros({}, grad.options()),
        (grad / abs - (at::real(grad / self) * result)));
  } else {
    return at::zeros_like(self, at::MemoryFormat::Preserve);
  }
}
...
Tensor var_jvp(
  ...
  if (self_p.dim() == 0 || !dim_opt.has_value()) {
    return var_backward(self_t.conj(), self_p, correction)
        .sum()
        .expand_as(result)
        .conj();
  }
  ...
  return ((2.0 / dof) * self_t.conj() *
          (self_p - self_p.mean(dim, /*keepdim=*/true)))
      .sum(dim, keepdim)
      .conj();
}
...
Tensor var_std_mean_backward(
  ...
  if (grads[1].defined()) {
    Tensor mean_grad =
        mean_backward(grads[1], self.sizes(), self.numel(), dim, keepdim);
...
Tensor smooth_l1_loss_double_backward_grad_output(
    const Tensor& grad,
    const Tensor& grad_output,
    const Tensor& input,
    const Tensor& target,
  ...
  auto r = smooth_l1_loss_backward(
      ones_like(grad_output), input, target, reduction, beta);
  return (r * grad).sum();
}
...
Tensor mse_loss_double_backward_grad_output(
    const Tensor& grad,
    const Tensor& grad_output,
    const Tensor& input,
    const Tensor& target,
...
std::tuple<Tensor, Tensor> lu_solve_backward(
  ...
  // const auto trans = grad.is_complex() ? TransposeType::ConjTranspose :
  // TransposeType::Transpose; const Tensor B_grad = at::_lu_solve_trans(grad,
  // LU_data, LU_pivots, trans); return std::make_pair(B_grad, Tensor{});
  //
  // We'll be able to use it once we ahve migradet lu_solve to linalg and has an
  // `adjoint` flag. This formula avoids the instantiation of P explicitly and
  // may have better numerical properties
  ...
  if (B_requires_grad) {
    ...
    std::tie(std::ignore, L, U) = at::lu_unpack(
        LU_data,
        LU_pivots,
        /*unpack_data=*/true,
        /*unpack_pivots=*/false);
  ...
  if (B_requires_grad) {
    ...
    // Z = L^{-H}U^{-H}X_grad
    const Tensor Z = at::linalg_solve_triangular(
        L_H,
        Y,
        /*upper=*/true,
        /*left=*/true,
    ...
    // Since when nothing needs to be computed was handled at the start, here we
    // have the case when just LU_data requires grad

    // U^{-H}X_grad X^H
    const Tensor U_grad =
        at::linalg_solve_triangular(U_H, grad.matmul(X_H), /*upper=*/false);
    // L^{-H}U^{-H}X_grad X^H U^H
    const Tensor L_grad = at::linalg_solve_triangular(
        L_H,
        U_grad.matmul(U_H),
        /*upper=*/true,
        /*left=*/true,
...
Tensor lu_solve_jvp(
  ...
  return generic_solve_jvp(
      [&](const Tensor& A, const Tensor& dB, const Tensor& dA_contrib) {
        // We exploit the structure in the computation of A^{-1} dA, where the
        // permutation matrix P such that A = P L U cancels itself. Because of
        // that we use lu_solve with the identity permutation input. The
        // identity permutation pivots are 1-based because of the Fortran-like
        // LAPACK interfaces. More details on the permutation matrix canceling
        // note: as part of forward AD we need to compute A^{-1} dA. Since A = P
        // L U and P is locally constant for full-rank matrices, we get dA = P
        // d(L U), A^{-1} = (L U)^{-1} P^T, so A^{-1} dA = (L U)^{-1} d(L U),
        // which is lu_solve with the pivots set to the identity permutation
        auto identity_pivots =
            at::arange(1, LU_data.size(-1) + 1, LU_pivots.options())
                .expand(LU_pivots.sizes());
        return at::lu_solve(dB, A, LU_pivots) -
            at::lu_solve(dA_contrib, A, identity_pivots);
      },
      X,
      /*A=*/LU_data,
      dA,
