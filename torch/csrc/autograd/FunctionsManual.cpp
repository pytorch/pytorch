...
Tensor _grouped_mm_mat1_backward(
    ...
    const Scalar& alpha) {
  TORCH_CHECK(grad.layout() == c10::kStrided && mat2.layout() == c10::kStrided 
  && mat1_layout == c10::kStrided, "only strided layout supported for grouped mm");
    // if input was column-major, return grad as column-order for efficiency
  ...
  if (mat1_strides[mat1_dim - 2] == 1 && mat1_strides[mat1_dim - 1] == mat1_sizes[mat1_dim - 2]) {
    auto grad_inp = (at::_grouped_mm(mat2, grad.transpose(-2, -1), offs)).transpose(-2, -1);
...
Tensor _grouped_mm_mat2_backward(
    ...
    std::optional<Tensor> offs,
    const Scalar& alpha) {
  TORCH_CHECK(grad.layout() == c10::kStrided && mat1.layout() == c10::kStrided 
  && mat2_layout == c10::kStrided, "only strided layout supported for grouped mm");
    // if input was column-major, return grad as column-order for efficiency
  ...
  if (mat2_strides[mat2_dim - 2] == 1 && mat2_strides[mat2_dim - 1] == mat2_sizes[mat2_dim - 2]) {
    auto grad_inp = at::_grouped_mm(grad.transpose(-2, -1), mat1, offs).transpose(-2, -1);
    return maybe_multiply(grad_inp, alpha.conj());
  } else {
