...
// The derivative w.r.t. the first argument (self) of the regularized upper
// incomplete gamma function and the regularized lower incomplete gamma function
...
Tensor igammac_backward(
  ...
  //
  //   variables:
  //     sum_asympt = \frac{(d/da)u_k}{z^k} incrementing
  //     uk = u_k
  //     du = (d/da)u_k
  //     u_term = (a-k)
  if (other_is_large.any().item<bool>()) {
    ...
    auto u_term = self.clone();

    for (int k = 1; k <= n_asympt_terms; ++k) {
      // update u_term
      u_term -= 1;

      if (k > 1) {
        // du(this k) = u(last k) + du(last k) * (this u_term)
        du = uk + du * u_term;
      }
      ...
      sum_asympt += du * other.pow(k).reciprocal();
    }
  ...
  // case: small other (z)
  // - use series expansion of gammac: dlmf 8.7.3
  //
  //   gammac(a,z) = (1 -
  ...
  //
  //   variables:
  //     sum_series = \frac{(d/da)u_k}{z^k} incrementing
  //     term_sign = (-1)^k
  ...
  if (other_is_large.logical_not().any().item<bool>()) {
    ...
    for (int k = 0; k <= max_series_terms; ++k) {
      ...
      // and subtracting log(k) which is the next term in k!
      if (k > 0) {
        subseries_term += log_other - std::log(k);
      }

      // calculate series term
      series_term = subseries_term - 2 * at::log(self + k);

      // update sum term
      sum_series += term_sign * at::exp(series_term);

      // toggle sign for next step
      term_sign *= -1;
      ...
      if (k == max_series_terms) {
        // if we reach this, we have not converged in n_series_steps
        TORCH_WARN(
          "igammac_backward did not converge to desired precision. "
          "Consider increasing the number of steps in the series expansion."
        );
      }
    }
  }
...
Tensor igamma_backward(
  ...
  // case: other (z) is large or other specific self (a) and other (z)
  // - use negative of gradient of upper incomplete gamma function
  ...
  // case: otherwise
  // - use series expansion of gamma: gautschi 5.4
  //
  //   gamma(a,z) = z^{a} e^{-z} \sum_{k=0}^{\infty} \frac{z^k}{\Gamma(a+k+1)}
  //     = e^{-z} \sum_{k=0}^{\infty} exp[(a+k) log(z) - log(\Gamma(a+k+1))]
  //
  //  (d/da)gamma = (
  //    e^{-z} \sum_{k=0}^{\infty} exp[...] log(z)
  //    - e^{-z} \sum_{k=0}^{\infty} exp[...] (d/da)log(\Gamma(a+k+1))
  //  )
  //  second sum:
  //    (d/da)log(\Gamma(a+k+1)) = 1/(a+k) + (d/da)log(\Gamma(a+k))
  //    for k=0: 1/a + (d/da)log(\Gamma(a)) = 1/a + digamma(a)
  //    for k=1: 1/(a+1) + (d/da)log(\Gamma(a+1)) = 1/(a+1) + 1/a + digamma(a)
  //    ...
  //
  //   variables:
  ...
  if (use_gammac_grad.logical_not().any().item<bool>()) {
    ...
    auto term_exp = at::zeros_like(self);
    auto term_a = at::zeros_like(self);
    ...
    for (int k = 0; k <= max_terms; ++k) {
      ...
      if (k == max_terms) {
        // if we reach this, we have not converged in n_series_steps
        TORCH_WARN(
          "igamma_backward did not converge to desired precision. "
          "Consider increasing the number of steps in the series expansion."
        );
      }
    }
