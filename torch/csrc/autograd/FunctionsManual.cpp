...
Tensor log1p_backward(const Tensor& grad, const Tensor& self) {
  if (self.layout() == c10::kSparse || self.layout() == c10::kSparseCsr ||
    ...
    TORCH_WARN(
        "log1p_backward: self has sparse layout but gradient does not preserve sparsity dense gradient will be materialized.");
    // Grad may also be sparse, if not to_dense will not create new storage, but
    // this is required due to incomplete implementation of sparse/dense
    // division
    return grad.to_dense() / (self.to_dense() + 1).conj();
  }
