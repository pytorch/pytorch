...
Tensor renorm_jvp(
  ...
  // factor =  (norm > maxnorm_s) ?
  //          (norm + eps)^(-1) * maxnorm_s : static_cast<scalar_t>(1.0);
  // return self * factor
  // dfactor = -1 (norm + eps)^(-2) * dnorm * max_norm
  ...
  return where(
      norm > double_maxnorm,
      invnorm * double_maxnorm -
          norm_jvp(self_p, self_t, p, norm, reduce_dims, /*keepdim=*/true) *
              invnorm.pow(2) * double_maxnorm * self_p,
      self_t);
}
