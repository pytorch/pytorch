...
auto CopySlices::apply(variable_list&& inputs) -> variable_list {
  ...
  // Since the gradient edge for the 0th input is different between `this` and `fn`,
  // make sure that the one from `fn` has the same metadata in the current GraphTask's
  // exec_info as the one on `this`.
    ...
    if (fn_edge.is_valid()) {
      ...
      if (it == exec_info->end()) {
        ...
        if (task_should_compute_output(0)) {
          ...
          // There is no need to remove this after execution because we are guaranteed that
          // this->next_edge(0) must be in the history of fn->next_edge(0) (we cannot easily assert
          // this as it might be far away if there were many chained views). This means that,
          // since fn->next_edge(0) was not needed (no exec_info entry for it), we know that nothing
          // downstream of fn->next_edge(0) is needed either (otherwise the whole path from that
          // Node to this->next_edge(0) would be needed as well).
          // This means that no other Node will ever look at fn->next_edge(0) metadata and thus there
          // is no need to clean them up.
        }
      } else {
  ...
  // Sanity check that the graph was never modified after the fact (it is read-only!)
