...
namespace autograd {
   ...
   variable_list run_backward(const variable_list& outputs,
    ...
    return Engine::get_default_engine().execute(roots, grad_outputs, keep_graph, create_graph, output_edges);
   }
  ...
  variable_list grad(
    ...
    variable_list grad_inputs = run_backward(outputs, gradients, keep_graph.value(), create_graph, inputs);
