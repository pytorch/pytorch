...
static void general_trace_function(
  ...
  if (tracer::isTracing()) {
    ...
    int i = 0;
    for (auto iter = stack->end() - static_cast<std::ptrdiff_t>(input_size); iter != stack->end();
         ++iter, ++i) {
  ...
  if (tracer_state) {
    ...
    int i = 0;
    for (auto iter = stack->end() - static_cast<std::ptrdiff_t>(output_size); iter != stack->end();
         ++iter, ++i) {
