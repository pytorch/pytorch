...
variable_list MeanBackward1::apply(variable_list&& grads) {
  ...
  auto self = self_.unpack();
  if (should_compute_output({ self_ix })) {
    auto grad_result = grad.expand(self_sizes).to(self.type().scalarType()) / self_numel;
    copy_range(grad_inputs, self_ix, grad_result);
  }
  return grad_inputs;
}
...
variable_list MeanBackward3::apply(variable_list&& grads) {
  ...
  auto self = self_.unpack();
  if (should_compute_output({ self_ix })) {
    auto grad_result = sum_backward(grad, self_sizes, dim, false).to(self.type().scalarType()) / _safe_size(self_sizes, dim);
    copy_range(grad_inputs, self_ix, grad_result);
  }
  return grad_inputs;
}
variable_list MeanBackward4::apply(variable_list&& grads) {
  ...
  auto self = self_.unpack();
  if (should_compute_output({ self_ix })) {
    auto grad_result = sum_backward(grad, self_sizes, dim, keepdim).to(self.type().scalarType()) / _safe_size(self_sizes, dim);
    copy_range(grad_inputs, self_ix, grad_result);
  }
  return grad_inputs;
}
