...
auto Engine::execute(
  ...
  // re-entrant backward calls), then memorize the local_ready_queue in
  // GraphTask
...
std::shared_ptr<at::ivalue::Future> Engine::execute_with_graph_task(
  ...
  ready_queue(graph_task->cpu_ready_queue_, at::kCPU)
      ->push(NodeTask(graph_task, std::move(graph_root), InputBuffer(0)));
