...
auto Engine::thread_main(GraphTask *graph_task) -> void {
  ...
  while (!graph_task || graph_task->outstanding_tasks > 0) {
    ...
    if (worker_device == NO_DEVICE && queue->heap.empty()) {
      continue;
    }
...
auto Engine::execute(const edge_list& input_roots,
  ...
  if (worker_device == NO_DEVICE) {
    ...
    // NO_DEVICE worker do the job. Note graph_task always resets GradMode in thread_main()
    //     thus I store the GradMode before thread_main() here.
    lock.unlock();
    auto mod = GradMode::is_enabled();
    thread_main(&graph_task);
    GradMode::set_enabled(mod);
...
auto Engine::start_threads() -> void {
  ...
  // Not new thread for CPU, use NO_DEVICE as CPU thread
