...
static at::Tensor _view_as_self_with_no_grad(
    ...
    const _view_as_fn_t& view_as_fn) {
  ...
  // We thread through this view_as_fn lambda so that in the case we are a
    ...
    if (!is_differentiable) {
      if (!var.requires_grad()) {
        if (is_input && !is_modified) {
          ...
          var = _view_as_self_with_no_grad(var, view_as_fn);
        }
        return;
      }
