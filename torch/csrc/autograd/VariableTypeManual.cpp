...
static auto registry = torch::RegisterOperators()
  .op(torch::RegisterOperators::options()
    ...
    .impl_unboxedOnlyKernel(DispatchKey::Autograd, TORCH_FN(VariableType::resize_)))
  .op(torch::RegisterOperators::options()
    .schema("aten::resize_as_(Tensor(a!) self, Tensor the_template, *, MemoryFormat? memory_format=None) -> Tensor(a!)")
    .aliasAnalysis(AliasAnalysisKind::FROM_SCHEMA)
    .impl_unboxedOnlyKernel(DispatchKey::Autograd, TORCH_FN(VariableType::resize_as_)))
  .op(torch::RegisterOperators::options()
    .schema("aten::detach(Tensor self) -> Tensor")
    .aliasAnalysis(AliasAnalysisKind::FROM_SCHEMA)
    .kernel(DispatchKey::Autograd, TORCH_FN(VariableType::detach)))
  .op(torch::RegisterOperators::options()
    .schema("aten::detach_(Tensor(a!) self) -> Tensor(a!)")
    .aliasAnalysis(AliasAnalysisKind::FROM_SCHEMA)
    .impl_unboxedOnlyKernel(DispatchKey::Autograd, TORCH_FN(VariableType::detach_)))
  .op(torch::RegisterOperators::options()
    .schema("aten::copy_(Tensor(a!) self, Tensor src, bool non_blocking=False) -> Tensor(a!)")
    .aliasAnalysis(AliasAnalysisKind::FROM_SCHEMA)
    .impl_unboxedOnlyKernel(DispatchKey::Autograd, TORCH_FN(VariableType::copy_)))
  .op(torch::RegisterOperators::options()
    ...
    .impl_unboxedOnlyKernel(DispatchKey::Autograd, TORCH_FN(VariableType::backward))
    .impl_unboxedOnlyCatchAllKernel(TORCH_FN(VariableType::backward)))
  .op(torch::RegisterOperators::options()
    .schema("aten::set_data(Tensor(a!) self, Tensor new_data) -> ()")
    .aliasAnalysis(AliasAnalysisKind::FROM_SCHEMA)
    .catchAllKernel(TORCH_FN(VariableType::set_data)))
  .op(torch::RegisterOperators::options()
    .schema("aten::data(Tensor self) -> Tensor")
    .aliasAnalysis(AliasAnalysisKind::FROM_SCHEMA)
    .catchAllKernel(TORCH_FN(VariableType::data)))
  .op(torch::RegisterOperators::options()
    .schema("aten::is_leaf(Tensor self) -> bool")
    .aliasAnalysis(AliasAnalysisKind::FROM_SCHEMA)
    .catchAllKernel(TORCH_FN(VariableType::is_leaf)))
  .op(torch::RegisterOperators::options()
    .schema("aten::output_nr(Tensor self) -> int")
    .aliasAnalysis(AliasAnalysisKind::FROM_SCHEMA)
    .catchAllKernel(TORCH_FN(VariableType::output_nr)))
  .op(torch::RegisterOperators::options()
    .schema("aten::_version(Tensor self) -> int")
    .aliasAnalysis(AliasAnalysisKind::FROM_SCHEMA)
    .catchAllKernel(TORCH_FN(VariableType::_version)))
  .op(torch::RegisterOperators::options()
    ...
    //      and remove the backend Autograd kernel here, only leaving the catch-all kernel.
    .impl_unboxedOnlyKernel(DispatchKey::Autograd, TORCH_FN(VariableType::requires_grad_))
    .impl_unboxedOnlyCatchAllKernel(TORCH_FN(VariableType::requires_grad_)))
  .op(torch::RegisterOperators::options()
    .schema("aten::retain_grad(Tensor(a!) self) -> ()")
    .aliasAnalysis(AliasAnalysisKind::FROM_SCHEMA)
    .catchAllKernel(TORCH_FN(VariableType::retain_grad)))
  ;
