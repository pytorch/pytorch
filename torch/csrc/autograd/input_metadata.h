...
struct InputMetadata {
  ...
  InputMetadata(const at::TypeProperties type, at::IntArrayRef shape, const int64_t device)
  ...
  bool is_valid() const {
    return type_.is_defined();
  }

  const at::TypeProperties type() const {
    return type_;
  }
