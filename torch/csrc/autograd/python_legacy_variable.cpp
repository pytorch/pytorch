...
static PyObject *THPVariable_pynew(PyTypeObject* type, PyObject *args, PyObject *kwds) {
  ...
  if (!data || data == Py_None) {
    ...
    auto scalar_type = static_cast<at::ScalarType>(torch::tensors::get_default_tensor_type().scalar_type);
    auto var = at::empty({0}, torch::tensors::get_default_tensor_type().aten_type()->options(scalar_type));
