...
void Variable::Impl::set_data(const at::Tensor &new_data) {
  ...
  // `var.set_data(...)` is not supposed to affect `var`'s version counter, so we save the
  // original version counter and restore it after replacing the underlying tensor.
