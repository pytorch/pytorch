...
void Variable::Impl::set_data(Tensor new_data) {
  ...
  // Change ownership of the AutogradMeta pointer (moving it from the old TensorImpl to the new TensorImpl)
  new_data_copy.unsafeGetTensorImpl()->set_autograd_meta(
    std::move(data_.unsafeGetTensorImpl()->detach_autograd_meta()));
...
Variable::DifferentiableViewImpl::DifferentiableViewImpl(Variable base, at::Tensor data, Edge gradient_edge)
  ...
  Variable::DifferentiableViewMeta& diff_view_meta = static_cast<Variable::DifferentiableViewMeta&>(get_autograd_meta());
