...
static PyObject* THPVariable_make_wrapper_subclass(
  ...
  static PythonArgParser parser({
      ...
      "DispatchKeySet _extra_dispatch_keys=None, int? _override_storage_size=None)",
  });
...

  {
    ...
    std::optional<int64_t> override_storage_size = r.toInt64Optional(14);

    if (override_storage_size.has_value()) {
      size_bytes = override_storage_size.value();
