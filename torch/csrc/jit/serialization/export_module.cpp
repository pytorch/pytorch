...
std::pair<IValue, IValue> getFunctionTuple(
    const Module& module,
    const Function& func,
    std::unique_ptr<BackendDebugHandleManager>& debug_handle_manager_ptr) {
  ...
  // We always save debug handles. It maybe the case
  // that sometimes we do not have a valid debug handle
  // (invalid debug handle = -1 for now)
  // but they are saved nonetheless
  // valid debug handles generated by debug_handle_manager
  // will correspond to inlinedCallStackPtrs which we will
...
void setstateTuple(
    ...
    std::unique_ptr<BackendDebugHandleManager>& debug_handle_manager) {
  if (!ivalue.isObject())
    return;
 ...
 private:
  ...
  void writeByteCode(const Module& module, const bool save_mobile_debug_info) {
    ...
    if (save_mobile_debug_info) {
      debug_handle_manager = std::make_unique<BackendDebugHandleManager>();
    }
...
void export_opnames(const script::Module& m, std::set<std::string>& opnames) {
  std::vector<c10::IValue> elements;
  std::vector<c10::IValue> debug_info_elements;
  std::unique_ptr<BackendDebugHandleManager> dummy;
