...
Value* to_ir::emitListLiteral(ListLiteral ll, TypePtr type_hint) {
  ...
  TypePtr annotated_union_type =
      refined_type_hint && refined_type_hint->isUnionType() ? refined_type_hint
                                                            : nullptr;
...
Value* to_ir::emitDictLiteral(DictLiteral dl, const TypePtr& type_hint) {
...
Value* to_ir::emitSimpleExpr(const TreeRef& tree, TypePtr type_hint) {
  switch (tree->kind()) {
    case TK_FLOOR_DIV:
    case '@': {
      const auto& inputs = tree->trees();
