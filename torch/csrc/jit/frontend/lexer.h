...
#define TC_CASE_ASCII_ALPHA \
  ...
  case 'z':
      ...
      case '<':
        if (input.starts_with("<=>"))
          TC_RETURN_TOKEN(TK_EQUIVALENT, 3);
        if (input.starts_with("<<="))
          TC_RETURN_TOKEN(TK_LSHIFT_EQ, 3);
        if (input.starts_with("<<"))
          TC_RETURN_TOKEN(TK_LSHIFT, 2);
        if (input.starts_with("<="))
          TC_RETURN_TOKEN(TK_LE, 2);
        TC_RETURN_TOKEN('<', 1);
      case '>':
        if (input.starts_with(">>="))
          TC_RETURN_TOKEN(TK_RSHIFT_EQ, 3);
        if (input.starts_with(">>"))
          TC_RETURN_TOKEN(TK_RSHIFT, 2);
        if (input.starts_with(">="))
          TC_RETURN_TOKEN(TK_GE, 2);
        TC_RETURN_TOKEN('>', 1);
      case '*':
        if (input.starts_with("**="))
          TC_RETURN_TOKEN(TK_POW_EQ, 3);
        if (input.starts_with("**"))
          TC_RETURN_TOKEN(TK_POW, 2);
        if (input.starts_with("*="))
          TC_RETURN_TOKEN(TK_TIMES_EQ, 2);
        TC_RETURN_TOKEN('*', 1);
      case '/':
        if (input.starts_with("//"))
          TC_RETURN_TOKEN(TK_FLOOR_DIV, 2);
        if (input.starts_with("/="))
          TC_RETURN_TOKEN(TK_DIV_EQ, 2);
        TC_RETURN_TOKEN('/', 1);
      case '+':
        if (input.starts_with("+="))
          TC_RETURN_TOKEN(TK_PLUS_EQ, 2);
        TC_RETURN_TOKEN('+', 1);
      case '-':
        if (input.starts_with("->"))
          TC_RETURN_TOKEN(TK_ARROW, 2);
        if (input.starts_with("-="))
          TC_RETURN_TOKEN(TK_MINUS_EQ, 2);
        TC_RETURN_TOKEN('-', 1);
      case '%':
        if (input.starts_with("%="))
          TC_RETURN_TOKEN(TK_MOD_EQ, 2);
        TC_RETURN_TOKEN('%', 1);
      case '|':
        if (input.starts_with("|="))
          TC_RETURN_TOKEN(TK_BIT_OR_EQ, 2);
        TC_RETURN_TOKEN('|', 1);
      case '&':
        if (input.starts_with("&="))
          TC_RETURN_TOKEN(TK_BIT_AND_EQ, 2);
        TC_RETURN_TOKEN('&', 1);
      case '^':
        if (input.starts_with("^="))
          TC_RETURN_TOKEN(TK_BIT_XOR_EQ, 2);
        TC_RETURN_TOKEN('^', 1);
      case '!':
        if (input.starts_with("!="))
          TC_RETURN_TOKEN(TK_NE, 2);
        TC_RETURN_TOKEN('!', 1);
      case '=':
        if (input.starts_with("=="))
          TC_RETURN_TOKEN(TK_EQ, 2);
        TC_RETURN_TOKEN('=', 1);
      case '#':
        if (input.starts_with("# type:"))
          TC_RETURN_TOKEN(TK_TYPE_COMMENT, 7);
        break;
      case '@':
      case '(':
      case ')':
      ...
      case '_': {
        ...
        if (ident == "is" && input.starts_with("is not")) {
          TC_RETURN_TOKEN(TK_ISNOT, 6);
        }
        if (ident == "not" && input.starts_with("not in")) {
