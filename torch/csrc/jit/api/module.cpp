...
IValue Module::operator()(std::vector<IValue> inputs) {
  const auto& pre_forward_hooks = type()->getForwardPreHooks(); // vector of function pointers
  const auto& forward_hooks = type()->getForwardHooks(); // vector of function pointers

  // Let's go over each forward pre_hook and call them
  for (const auto& pre_hook : pre_forward_hooks) {
    std::cout << "calling pre hook!" << std::endl;
    ...
    std::cout << " donw with pre hook call!" << std::endl;
    // Check the result
    if (!result.isNone() && result.isTuple()) { // TODO does this work for None results without the isNone()?
  ...
  for (const auto& hook : forward_hooks) {
    ...
    std::cout << "done calling hook!" << std::endl;
  }
