...
inline const std::unordered_map<std::string, std::vector<UpgraderEntry>>&
get_operator_version_map() {
  return operatorVersionMap;
}

inline void test_only_add_entry(std::string op_name, UpgraderEntry entry) {
  operatorVersionMap[op_name].push_back(entry);
}

inline void test_only_remove_entry(std::string op_name) {
