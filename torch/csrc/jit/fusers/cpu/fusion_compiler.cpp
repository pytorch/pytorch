...
* CPUFusionFunction
*/
struct CPUFusionFunction : public CompiledCPUFusionFunction {
  CPUFusionFunction(
    const std::string& name
  , AnnotatedGraph& agraph
  , CPUFusionCompilerConfig& config)
  : CompiledCPUFusionFunction(name, agraph) {
...
/*
* CPUFusionCompiler implementation.
*/
...
std::shared_ptr<CompiledCPUFusionFunction> CPUFusionCompiler::getOrCompile(
  ...
  if (it == cache.end()) {
    ...
    CompiledCPUFusionFunction* raw_func = new CPUFusionFunction(name, agraph, config_);
    it = cache.emplace(key_, std::shared_ptr<CompiledCPUFusionFunction>(raw_func)).first;
  }
  return it->second;
}
...
/*
* CompiledCPUFusionFunction implementation.
*/
