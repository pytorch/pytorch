...
at::optional<int> GraphFuser::getDevice(Node* node) {
  if (node->kind() == prim::FusionGroup) return node->i(attr::device);
...
bool GraphFuser::isSupportedValue(Value* val) {
  if (auto tt = val->type()->cast<TensorType>()) {
...
bool GraphFuser::allValuesSupported(at::ArrayRef<Value*> list) {
  for (Value* v : list) 
...
bool GraphFuser::areTensorsOfSameShape(at::ArrayRef<Value*> values) {
  auto expected_type = values.at(0)->type()->cast<TensorType>();

  // Short-circuits if first element isn't a tensor
...
bool GraphFuser::isSupportedNode(Node* node) {
  return 
      areTensorsOfSameShape(node->inputs()) 
...
bool GraphFuser::isFusable(Node* node) {
  if (node->owningBlock() != block) return false;
  if (!isSimpleMap(node)) return false;

  if (
      node->kind() == prim::FusionGroup
  && node->i(attr::device) == kCPUDevice) return true;
  ...
  if (node->matches("aten::add(Tensor self, Tensor other, *, Scalar alpha) -> Tensor", /*const=*/attr::alpha)) {
    ...
    return isSupportedNode(node);
  }
}

bool GraphFuser::isFusableCatNode(Node* node) {
  ...
  return std::all_of(tensors.begin(), tensors.end(), [&expected](Value *v) {
      ...
      return tt && tt->sizes() == expected->sizes() && tt->device() == kCPUDevice;
  });
}

bool GraphFuser::isFusableOnlyAsExitNode(Node* node) {
...
bool GraphFuser::isFusableAsExitNode(Node* node) {
...
bool GraphFuser::allUsersAreThisConsumerOrOccurAfterIt(Node* consumer, Value* producer) {
...
bool GraphFuser::allUsersAreThisConsumer(Node* consumer, Value* producer) {
  auto defining_node = producer->node();
  for (auto o : defining_node->outputs()) {
    for (auto u : o->uses()) {
      if (u.user != consumer)
...
bool GraphFuser::mustRemainAsFusionGroupOutput(Value* producer) {
...
bool GraphFuser::shouldFuse(Node* consumer, Value* producer) {
...
Graph& GraphFuser::getSubgraph(Node* n) {
  JIT_ASSERT(n->kind() == prim::FusionGroup);
  return *n->g(attr::Subgraph);
}

void GraphFuser::mergeFusionGroups(Node* consumer_group, Node* producer_group) {
...
Node* GraphFuser::mergeNodeIntoGroup(Node* group, Node* n) {
  JIT_ASSERT(n->kind() != prim::FusionGroup);
...
Node* GraphFuser::createSingletonFusionGroup(Node* n) {
...
void GraphFuser::insertAfter(Node* n, Node* after) {
  n->insertAfter(after);
  topological_index[n] = topological_index[after];
}

void GraphFuser::insertAt(Node** insertion_point, Node* n) {
  insertAfter(n, *insertion_point);
  *insertion_point = n;
}

Node* GraphFuser::fuse(Node* consumer, Value* producer) {
  auto group = consumer;
...
bool GraphFuser::isChunk(Node* node) {
...
bool GraphFuser::tryToMoveChunk(Node* consumer, Value* producer) {
...
std::pair<graph_node_list::iterator, bool> GraphFuser::scanNode(Node* consumer) {
