...
struct CompiledCUDAFusionFunction : public CompiledFusionFunction {
  TH_DISALLOW_COPY_AND_ASSIGN(CompiledCUDAFusionFunction);

  CompiledCUDAFusionFunction(
    const std::string& name
  , AnnotatedGraph& agraph);

  virtual ~CompiledCUDAFusionFunction() override = default;
...
struct CUDAFusionCompiler {
  ...
  std::shared_ptr<CompiledCUDAFusionFunction> getOrCompile(
    AnnotatedGraph& agraph);
...
private:
  ...
  std::unordered_map<
    std::string
  , std::shared_ptr<CompiledCUDAFusionFunction>> cache;
};
