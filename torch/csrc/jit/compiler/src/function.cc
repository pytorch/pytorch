...
static std::vector<Var> arg_name_hints_to_args(int ndim, std::vector<std::string>& arg_name_hints) {
  std::vector<Var> args;
...
Tensor Compute(const std::vector<Expr>& dims, std::vector<std::string> arg_name_hints,
  ...
  Function func = Function(dims, std::move(args), std::move(body));
  return Tensor(func, 0);
}

Tensor Compute(const std::vector<Expr>& dims, std::vector<std::string> arg_name_hints,
               std::function<Expr(const Var&)> body_func) {
  // TODO: CHEKC(dims.size() == 1
  std::vector<Var> args = arg_name_hints_to_args(dims.size(), arg_name_hints);
