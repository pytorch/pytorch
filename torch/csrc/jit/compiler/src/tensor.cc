...
void TensorOperationNode::SplitWithTail(const Var& loop_var, int factor, bool factor_on_inner,
  ...
  schedule->SplitWithTail(expr_node_, loop_var, factor, factor_on_inner, outer_var,
			  inner_var,
			  tail_var, &tail_expr_node);
  if (!tail_expr_node) {
