...
bool needTrimGrad(Node *n) {
  static OperatorSet need_trim_grad_ops = {
      "aten::kthvalue(Tensor self, int k, int dim, bool keepdim) -> (Tensor, "
      "Tensor)",
  };
...
bool isDifferentiable(Node *n) {
  ...
  static OperatorSet differentiable_ops = {
      ...
      "aten::var(Tensor self, int[] dim, bool unbiased, bool keepdim) -> "
      "Tensor",
      "aten::std(Tensor self, bool unbiased) -> Tensor",
      "aten::std(Tensor self, int[] dim, bool unbiased, bool keepdim) -> "
      "Tensor",
      "aten::sigmoid(Tensor self) -> Tensor",
      "aten::tanh(Tensor self) -> Tensor", "aten::relu(Tensor self) -> Tensor",
      "aten::threshold(Tensor self, Scalar threshold, Scalar value) -> Tensor",
      "aten::erf(Tensor self) -> Tensor", "aten::erfc(Tensor self) -> Tensor",
      "aten::exp(Tensor self) -> Tensor", "aten::t(Tensor self) -> Tensor",
      ...
      "aten::abs(Tensor self) -> Tensor", "aten::acos(Tensor self) -> Tensor",
      "aten::asin(Tensor self) -> Tensor", "aten::atan(Tensor self) -> Tensor",
      "aten::ceil(Tensor self) -> Tensor", "aten::cos(Tensor self) -> Tensor",
      "aten::cosh(Tensor self) -> Tensor", "aten::exp(Tensor self) -> Tensor",
      "aten::expm1(Tensor self) -> Tensor",
      "aten::floor(Tensor self) -> Tensor",
      "aten::fmod(Tensor self, Scalar other) -> Tensor",
      "aten::frac(Tensor self) -> Tensor", "aten::log(Tensor self) -> Tensor",
      "aten::log10(Tensor self) -> Tensor",
      "aten::log1p(Tensor self) -> Tensor", "aten::log2(Tensor self) -> Tensor",
      "aten::reciprocal(Tensor self) -> Tensor",
      "aten::remainder(Tensor self, Scalar other) -> Tensor",
      "aten::round(Tensor self) -> Tensor",
      "aten::rsqrt(Tensor self) -> Tensor", "aten::sin(Tensor self) -> Tensor",
      "aten::sinh(Tensor self) -> Tensor", "aten::tan(Tensor self) -> Tensor",
      ...
      "aten::max_pool2d_with_indices(Tensor self, int[] kernel_size, int[] "
      "stride, int[] padding, int[] dilation, bool ceil_mode) -> (Tensor, "
      "Tensor)",
      "aten::thnn_conv2d_forward(Tensor self, Tensor weight, int[] "
      "kernel_size, Tensor? bias, int[] stride, int[] padding) -> (Tensor, "
      "Tensor, Tensor)",
      "aten::native_batch_norm(Tensor input, Tensor? weight, Tensor? bias, "
      "Tensor? running_mean, Tensor? running_var, bool training, float "
      "momentum, float eps) -> (Tensor, Tensor, Tensor)",
  };
  ...
  if (n->matches("aten::expand(Tensor self, int[] size, *, bool implicit) -> "
                 "Tensor")) {
  ...
  if (n->matches("aten::nll_loss(Tensor self, Tensor target, Tensor? weight, "
  ...
  if (n->kind() == prim::GradOf) {
    ...
    return std::all_of(body->nodes().begin(), body->nodes().end(),
...
// in this file.
static c10::optional<std::vector<Value *>>
build_script_grad(Node *node, const ArrayRef<Value *> &grads) {
  ...
  value_list new_outputs;
  {
    ...
    new_outputs = inlineCallTo(*graph, *fw_graph, node->inputs(),
                               /*unpack_outputs=*/true);
...
private:
  ...
  std::vector<SymbolicVariable>
  buildSymbolicGradient(const std::vector<SymbolicVariable> &grads) {
    ...
    if (node->matches("aten::add(Tensor self, Tensor other, *, Scalar alpha) "
      ...
      return {gradSumToSizeOf(grads.at(0), attr::self),
              gradSumToSizeOf(grads.at(0) * node->namedInput(attr::alpha),
                              attr::other),
              nullptr};

    } else if (node->matches("aten::add(Tensor self, Scalar other, Scalar "
                             "alpha) -> Tensor")) {
    ...
    } else if (node->matches("aten::sub(Tensor self, Tensor other, *, Scalar "
                             "alpha) -> Tensor")) {
      return {gradSumToSizeOf(grads.at(0), attr::self),
              gradSumToSizeOf(-grads.at(0) * node->namedInput(attr::alpha),
                              attr::other),
    ...
    } else if (node->matches("aten::sub(Tensor self, Scalar other, Scalar "
                             "alpha) -> Tensor")) {
      return {grads.at(0), nullptr, nullptr};
      ...
      return {grads.at(0) * inputs.at(1), nullptr};
                   ...
                   "aten::div(Tensor self, Tensor other) -> Tensor")) {
      return {gradSumToSizeOf(grads.at(0) / inputs.at(1), attr::self),
              gradSumToSizeOf(-grads.at(0) * inputs.at(0) /
                                  (inputs.at(1) * inputs.at(1)),
                              attr::other)};
    ...
    } else if (node->matches("aten::where(Tensor condition, Tensor self, "
                             "Tensor other) -> Tensor")) {
      return {
          nullptr,
          gradSumToSizeOf(grads.at(0) * inputs.at(0).type_as(grads.at(0)),
                          attr::self),
          gradSumToSizeOf(grads.at(0) * (1 - inputs.at(0)).type_as(grads.at(0)),
                          attr::other)};
    ...
    } else if (node->matches("aten::clamp(Tensor self, Scalar? min, Scalar? "
                             "max) -> Tensor")) {
    ...
    } else if (node->matches("aten::threshold(Tensor self, Scalar threshold, "
                             "Scalar value) -> Tensor")) {
    ...
    } else if (node->matches("aten::_grad_sum_to_size(Tensor(a) self, int[] "
                             "size) -> Tensor(a)")) {
      Value *self_size;
      {
        WithInsertPoint insert_guard{node};
      ...
      return {gradSumToSizeOf(grads.at(0) * node->namedInput(attr::beta),
                              attr::self),
    ...
    } else if (node->matches("aten::avg_pool2d(Tensor self, int[] kernel_size, "
                             "int[] stride, int[] padding, bool ceil_mode, "
                             "bool count_include_pad) -> Tensor")) {
      AT_ASSERT(grads.size() == 1);
      auto graph = node->owningGraph();
      auto backward_value = graph->insert(
          aten::avg_pool2d_backward,
          {grads.at(0).value(), node->namedInput(attr::self),
           node->namedInput(attr::kernel_size), node->namedInput(attr::stride),
           node->namedInput(attr::padding), node->namedInput(attr::ceil_mode),
    ...
    } else if (node->matches("aten::max_pool2d_with_indices(Tensor self, int[] "
                             "kernel_size, int[] stride, int[] padding, int[] "
                             "dilation, bool ceil_mode) -> (Tensor, Tensor)")) {
      ...
      auto backward_value = graph->insert(
          ...
          {grads.at(0).value(), node->namedInput(attr::self),
           node->namedInput(attr::kernel_size), node->namedInput(attr::stride),
           node->namedInput(attr::padding), node->namedInput(attr::dilation),
           node->namedInput(attr::ceil_mode), outputs.at(1).value()});
      return {backward_value->node()->output(0),
              nullptr,
              nullptr,
              nullptr,
    ...
    } else if (node->matches("aten::thnn_conv2d_forward(Tensor self, Tensor "
                             "weight, int[] kernel_size, Tensor? bias, int[] "
                             "stride, int[] padding) -> (Tensor, Tensor, "
                             "Tensor)")) {
      auto graph = node->owningGraph();
      auto backward_value = graph->insert(
          aten::thnn_conv2d_backward,
          {grads.at(0).value(), inputs.at(0).value(), inputs.at(1).value(),
           node->namedInput(attr::kernel_size), node->namedInput(attr::stride),
           node->namedInput(attr::padding), outputs.at(1).value(),
           outputs.at(2).value(),
      ...
      return {tuple_outputs[0], tuple_outputs[1], nullptr,
              tuple_outputs[2], nullptr,          nullptr};

    } else if (node->matches("aten::native_batch_norm(Tensor input, Tensor? "
                             "weight, Tensor? bias, Tensor? running_mean, "
                             "Tensor? running_var, bool training, float "
                             "momentum, float eps) -> (Tensor, Tensor, "
                             "Tensor)")) {
      auto graph = node->owningGraph();
      auto backward_value = graph->insert(
          aten::native_batch_norm_backward,
          {grads.at(0).value(), inputs.at(0).value(), inputs.at(1).value(),
           inputs.at(3).value(), inputs.at(4).value(), outputs.at(1).value(),
           outputs.at(2).value(), inputs.at(5).value(), inputs.at(7).value(),
           graph->insertConstant(std::vector<bool>{true, true, true})});
      // graph->insert returns a tuple automatically if multiple outputs are
      // returned. So unpack them again.
      Node *tuple_unpack_node =
      ...
      auto tuple_outputs = tuple_unpack_node->outputs();
      AT_ASSERT(tuple_outputs.size() == size_t(3));
      return {tuple_outputs[0], tuple_outputs[1], tuple_outputs[2], nullptr,
              nullptr,          nullptr,          nullptr,          nullptr};

    } else if (node->matches("aten::nll_loss(Tensor self, Tensor target, "
      ...
      auto backward_value = graph->insert(
          ...
          {grads.at(0).value(), inputs.at(0).value(), inputs.at(1).value(),
           weight->output(), inputs.at(3).value(), inputs.at(4).value(),
           total_weight->output()});
      return {backward_value->node()->output(0), nullptr, nullptr, nullptr,
              nullptr};
      ...
      auto backward_value = graph->insert(
          ...
          {grads.at(0).value(), outputs.at(0).value(),
           node->namedInput(attr::dim), node->namedInput(attr::self)});
      return {backward_value->node()->output(0), nullptr};

    } else if (node->kind() == prim::Constant ||
               node->kind() == prim::Undefined || node->kind() == prim::None) {
      return {};
    }
    throw std::runtime_error(std::string("failed to differentiate `") +
...
linearGradientForNode(Node *node, ArrayRef<Value *> grad_values) {
  auto &graph = *node->owningGraph();
...
static ReverseDetails addReverseInline(Gradient &grad_desc) {
  ...
  for (auto it = graph.nodes().rbegin(), end = graph.nodes().rend(); it != end;
    ...
    if (std::all_of(outputs.begin(), outputs.end(),
                    [](Value *v) { return !v->requires_grad(); })) {
      continue;
    }
...
static void deduplicateSizeCaptures(Gradient &grad_desc,
...
static void lambdaLiftReverse(Gradient &grad_desc, ReverseDetails &rev_info) {
  ...
  for (Value *capture_val : reverse_captures) {
    ...
    if (orig_primal_outputs_idx.count(capture_val) > 0) {
      ...
      grad_desc.df_input_captured_outputs.emplace_back(graph.outputs().size() -
                                                       1);
    }
  }
