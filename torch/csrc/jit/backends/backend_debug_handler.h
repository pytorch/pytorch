 ...
 * using atomic variable. However this is only used for compilation so we assume
 * to absorb some of that penalty. Plus if there is no contention then we should
 * have less to worry about. B. If repeated compilation is part of a long
 * running process then we may overflow int64_t. We may detect and fail on this.
 * Not sure if this is good. Will seek opinions about this.
 *
 *  Now about 2:
 ...
 * During backend lowering, each backend's preprocess/compile method can compile
 * method's graph and serialize those methods. Once the method is lowered to
 * backend, graph is essentially lost. Without access to graph it is hard to
 * generate model level debug info. Thus the debug handles provide a way to map
 * nodes of the graph to the model level debug info. Following diffs will
 ...
 * available in its inlined CS. It is rather part of node's source range instead
 * of inlined CS. Thus to get full stack: [N.forward, source range] ->
