 ...
 * inlined-callstack-ptr. Thus simplifies the runtime API for throwing
 * exception. Exception throwing only needs to know debug_handle and not which
 * module or method threw it. There are 2 issues to keep in mind, though,for
 * static std::atomic debug_handle: A. Performance implications of using atomic
 * variable. However this is only used for compilation so we assume to absorb
 * some of that penalty. Plus if there is no contention then we should have less
 * to worry about. B. If repeated compilation is part of a long running process
 * then we may overflow int64_t. We may detect and fail on this. Not sure if
 * this is good. Will seek opinions about this.
 *
 *  Now about 2:
 ...
 *     the debug handles provide a way to map nodes of the graph to the model
 * level debug info. Following diffs will provide a API to generate debug
 * handles given inlined graph. This API will, given an inlined graph of a
 * method, returns a map of node*-to-debug-handles. Backends will serialize
 * these debug handles and use them to raise exceptionm, much like lite
 * interpreter. Underneath the API will utilize BackendDebugHandleManager, to
 * generate map of debug-handles-to-inlined-callstack-ptrs.
 *
 *  During byte-code model serialization,
 * debug-handles-to-inlined-callstack-ptrs is serialized. Now we know a. debug
 ...
 *
 *  Note that it is not necessary to serialize
 * debug-handles-to-inlined-callstack-ptrs lowered backend if the lowering
 ...
 *     and
 *  2. getCallStackPtrMap which returns the map of
 * debug-handle-to-inlined-callstack-ptr or more precisely map of
 * debug-handle-to-tuple(source range, inlined-callstack-ptr)
 *
