...
MemoryPlanner::MemoryPlanner(
  ...
  for (const auto& ms : managed_tensors_) {
    num_managed_tensors_ += ms.group().size();
  }
}
...
#ifndef NDEBUG
    DCHECK_EQ(tensor_size, managed_tensors_[group_idx].size);
    for (auto* tensor : managed_tensors_[group_idx].group()) {
      DCHECK_EQ(storageImpl, tensor->storage().unsafeGetStorageImpl());
    }
#endif
    DCHECK_NE(managed_tensors_[group_idx].group().size(), 0);
    reused_tensors_ += managed_tensors_[group_idx].group().size() - 1;
...
void MemoryPlanner::deallocate() {
  ...
  for (auto& ms : managed_tensors_) {
    ...
    managed_tensor_storage_impls_[group_idx++].first = ms.size = max;
    managed_bytes_ += max;
  }
