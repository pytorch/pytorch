...
c10::IValue BlockRunner::run_impl_record_functions(
  ...
  if (!step_callbacks.empty()) {
    ...
    guard.needsInputs() ? guard.before("forward", c10::ArrayRef<const IValue>(args.data(), args.size()))
...
void ProcessedNode::run() {
  ...
  if (!step_callbacks.empty()) {
    ...
    if (guard.needsInputs()) {
      ...
      guard.before(get_op_name(), c10::ArrayRef<const IValue>(inputs.data(), inputs.size()));
    } else {
