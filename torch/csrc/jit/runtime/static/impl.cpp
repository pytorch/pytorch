...
// from "optimzable" set.  GetLivenessMap and FindShared
...
std::unordered_map<const Value*, std::vector<const Value*>> FindShared(
  ...
  for (const auto* v : all_values) {
    ...
    for (const auto& p : shared) {
      ...
      // TODO: add a more robust check of this behavior
      if (mayContainAlias(db, p.first, v)) {
        share(v, p.first);
      }
    }
  }
  ...
  for (const auto* v : optimizable_values) {
    ...
    for (const auto* sv : shared.at(v)) {
      auto l = lm.first.count(sv) ? lm.first.at(sv) : std::set<const Value*>{};
  ...
  auto unshare = [&](const Value* v) {
    if (!shared.count(v)) {
      return;
    }
    auto vs = shared.at(v);
    (void)std::remove_if(
        vs.begin(), vs.end(), [&](const Value* _) { return _ == v; });
    for (const auto* other_v : vs) {
      shared.at(other_v) = vs;
    }
    shared.erase(v);
  };

  std::unordered_set<const Value*> keep{
      optimizable_values.begin(), optimizable_values.end()};
  for (const auto* v : all_values) {
    if (keep.count(v)) {
      continue;
    }
...
StaticRuntime::StaticRuntime(
  ...
  // memory optimization pass (GetLivenessMap + AssignRegisters) is
  ...
  auto lm = GetLivenessMap(module_->graph, alias_db);
