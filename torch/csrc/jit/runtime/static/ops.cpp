...
REGISTER_OPERATOR_FUNCTOR(static_runtime::clamp_nan_to_num, static_runtime_clamp_nan_to_num, [](Node* n) -> SROperator {
    ...
    return [te = createClampNanToNum(), clamp_min = clamp_min_opt->to<float>(), clamp_max = clamp_max_opt->to<float>()](ProcessedNode* p_node) mutable {
      ...
      if (!te || !te->checkInput<float>(in0_t)) {
        at::cpu::nan_to_num_out(out_t, at::cpu::clamp(in0_t, clamp_min, clamp_max), in3_s, c10::nullopt, c10::nullopt);
        return;
      }
      ...
      // This might be UB if in3_s is absurdly large, but most implementations just turn
      // it into `inf` in that case. The PyTorch core nan_to_num kernel just static_cast()s the limits to
      // the destination type, so we'll ignore overflow issues here as well.
      auto nan = in3_s.has_value() ? static_cast<float>(*in3_s)
                                   : 0.f;

      te->call({out_t.data_ptr(), in0_t.data_ptr(), &clamp_min, &clamp_max, &nan, &output_size});
    };
});
