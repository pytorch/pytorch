...
#ifdef _MSC_VER
template <typename T>
void toCopyOutFastPath(Tensor& out, const Tensor& self) {
  TO_COPY_OUT_FAST_PATH_LOGIC(out, self, T);
}

#define TO_COPY_OUT_FAST_PATH_BODY(out, self) \
  toCopyOutFastPath<scalar_t>(out, self)
#else
#define TO_COPY_OUT_FAST_PATH_BODY(out, self) \
...
at::Tensor& dequantizeCopyOut(Tensor& out, const Tensor& self) {
...
REGISTER_OPERATOR_FUNCTOR(aten::clamp, aten_clamp, [](Node* n) -> SROperator {
  if (n->matches(torch::schema(
    ...
    return [](ProcessedNode* p_node) {
      const auto& in0_t = p_node->input(0).toTensor();
      if (p_node->output(0).isNone()) {
        p_node->output(0) = createEmptyFrom(in0_t);
      }
      auto& out_t = p_node->output(0).toTensor();
      fastResizeToZero(out_t);
...
void varStackFastOut(
  ...
  const auto numInputs = static_cast<int64_t>(inputs.size());
  TORCH_CHECK(numInputs > 0, "stack expects a non-empty list of tensors");

  const auto first_tensor_shape = inputs[0].sizes();
  for (const auto i : c10::irange(1, numInputs)) {
  ...
  const std::array<int64_t, 2> output_size = (dim == 0 || dim == -2)
      ? std::array<int64_t, 2>{numInputs, 1}
      : std::array<int64_t, 2>{1, numInputs};
...
void varStackOut(ProcessedNode& pnode, int64_t dim) {
  ...
  TORCH_CHECK(numInputs > 1, "stack expects a non-empty list of tensors");
...
REGISTER_OPERATOR_FUNCTOR(
    ...
    [](Node* n) -> SROperator {
      return [](ProcessedNode* p_node) {
        const size_t numInputs = p_node->numInputs();
        const auto dim = p_node->input(numInputs - 1).toInt();

        if (p_node->output(0).isNone()) {
          p_node->output(0) = createEmptyFrom(p_node->input(0).toTensor());
        }
...
REGISTER_OPERATOR_FUNCTOR(
    ...
    [](Node* n) -> SROperator {
      ...
      return [code](ProcessedNode* p_node) {
        auto num_outputs = p_node->numOutputs();
        Stack stack;
        if (p_node->output(0).isNone()) {
          stack.reserve(p_node->numInputs());
        } else {
          stack.reserve(p_node->numInputs() + num_outputs);
          for (const auto& o : p_node->outputs()) {
            stack.emplace_back(o);
          }
        }
        for (auto i : c10::irange(p_node->numInputs())) {
  ...
  return [](ProcessedNode* p_node) {
    ...
    if (p_node->Output(0).isNone()) {
      if (memory_format == c10::MemoryFormat::Preserve &&
        ...
        p_node->Output(0) = createEmptyFrom(src, memory_format);
      }
    }
    auto& out_t = p_node->Output(0).toTensor();
    at::native::resize_impl_cpu_(
...
REGISTER_OPERATOR_FUNCTOR(
    ...
    [](Node* n) -> SROperator {
      ...
      return [](ProcessedNode* p_node) {
        ...
        if (p_node->output(0).isNone()) {
          p_node->output(0) = createEmptyFrom(weight, at::kFloat);
        }
        auto& out_t = p_node->output(0).toTensor();
        fastResizeToZero(out_t);
  ...
  if (n->matches(torch::schema(
    ...
    return [](ProcessedNode* p_node) {
      if (p_node->output(0).isNone()) {
        ...
        p_node->output(0) = createEmptyFrom(in0_t, dtype);
      }
      auto& out_t = p_node->output(0).toTensor();
      fastResizeToZero(out_t);
      at::cpu::pow_out(
...
ToArgs extractToArgs(ProcessedNode* p_node) {
  ToArgs result;
...
// at runtime.
template <bool has_constant_non_tensor_dtype_and_flags, bool has_memory_format>
C10_ALWAYS_INLINE void toCopyFunctorImpl(
    ProcessedNode* p_node,
    const ToArgs* args) {
  const auto& self = p_node->input(0).toTensor();
  // ignore input 3 (copy)
...
template <bool has_constant_non_tensor_dtype_and_flags, bool has_memory_format>
void toMaybeCopyOutFunctor(ProcessedNode* p_node) {
...
// case.
template <>
void toMaybeCopyOutFunctor<true, false>(ProcessedNode* p_node) {
  const auto& self = p_node->input(0).toTensor();
  if (CheckToWillAlias<true, false>::call(p_node, self)) {
    p_node->output(1) = false;
  } else {
    p_node->output(1) = true;
    auto args = extractToArgs<true, false>(p_node);
    toCopyFunctorImpl<true, false>(p_node, &args);
  }
}

bool nodeHasConstantNonTensorDtypeAndFlags(Node* n) {
...
auto getToCopyFunctor(
    bool has_constant_non_tensor_dtype_and_flags,
    bool has_memory_format) {
  if (has_constant_non_tensor_dtype_and_flags) {
    if (has_memory_format) {
      return toCopyFunctor<true, true>;
    } else {
      return toCopyFunctor<true, false>;
    }
  } else {
    if (has_memory_format) {
      return toCopyFunctor<false, true>;
    } else {
      return toCopyFunctor<false, false>;
    }
  }
}
...
REGISTER_OPERATOR_FUNCTOR(
    ...
    [](Node* n) -> SROperator {
      ...
      const bool has_constant_non_tensor_dtype_and_flags =
          nodeHasConstantNonTensorDtypeAndFlags(n);
      ...
      if (has_constant_non_tensor_dtype_and_flags) {
        ...
        if (*copyArg) {
          return getToCopyFunctor(
              has_constant_non_tensor_dtype_and_flags, has_memory_format);
        }
      }
      if (has_constant_non_tensor_dtype_and_flags) {
        if (has_memory_format) {
          return toMaybeCopyOutFunctor<true, true>;
        } else {
          return toMaybeCopyOutFunctor<true, false>;
        }
      } else {
        if (has_memory_format) {
          return toMaybeCopyOutFunctor<false, true>;
        } else {
          return toMaybeCopyOutFunctor<false, false>;
        }
      }
    });
...
REGISTER_OPERATOR_FUNCTOR(
    ...
    [](Node* n) -> SROperator {
      ...
      return [](ProcessedNode* p_node) {
        ...
        if (p_node->output(0).isNone()) {
          p_node->output(0) =
              createEmptyFrom(self, at::kFloat, self.suggest_memory_format());
        }

        auto& out_t = p_node->output(0).toTensor();
        fastResizeToZero(out_t);
        at::native::dequantizeCopyOut(out_t, self);
      };
    });
...
REGISTER_OPERATOR_FUNCTOR(
    ...
    [](Node* n) -> SROperator {
      ...
      return [](ProcessedNode* p_node) {
        ...
        if (p_node->output(0).isNone()) {
          p_node->output(0) = createEmptyFrom(self);
        }
        auto& out = p_node->output(0).toTensor();
...
REGISTER_OPERATOR_FUNCTOR(aten::argmin, aten_argmin, [](Node* n) -> SROperator {
  ...
  return [](ProcessedNode* p_node) {
    ...
    if (in0_t.is_contiguous() && dim.has_value()) {
      at::native::caffe2ArgminOut(out_t, in0_t, dim.value(), keepdim);
      return;
    }
    ...
    [](Node*) -> SROperator {
      return [](ProcessedNode* p_node) {
        ...
        c10::MaybeOwned<at::Tensor> weight_maybe_owned =
            borrowFromOptionalTensorIValue(p_node->input(2));
        const at::Tensor& weight = *weight_maybe_owned;
        c10::MaybeOwned<at::Tensor> bias_maybe_owned =
            borrowFromOptionalTensorIValue(p_node->input(3));
...
template <>
void applyDynamicOutFunctor<false>(
    c10::intrusive_ptr<LinearPackedParamsBase> packed_weight,
    const at::Tensor& input,
    at::Tensor& out,
    bool reduce_range) {
...
template <>
void applyDynamicOutFunctor<true>(
    c10::intrusive_ptr<LinearPackedParamsBase> packed_weight,
    const at::Tensor& input,
    at::Tensor& out,
    bool reduce_range) {
...
template <bool has_relu>
SROperator quantizedLinearDynamicFP16Impl(Node* n) {
  ...
  if (packed_weight) {
    return [packed_weight](ProcessedNode* p_node) {
      const auto& input = p_node->input(0).toTensor();
      if (p_node->output(0).isNone()) {
        p_node->output(0) = createEmptyFrom(input, at::kFloat);
      }
      auto& out_t = p_node->output(0).toTensor();
      fastResizeToZero(out_t);
      applyDynamicOutFunctor<has_relu>(packed_weight, input, out_t, false);
    };
  } else {
    return [](ProcessedNode* p_node) {
      const auto& input = p_node->input(0).toTensor();
...
REGISTER_OPERATOR_FUNCTOR(
    ...
    [](Node* n) -> SROperator {
      ...
      return quantizedLinearDynamicFP16Impl<false>(n);
    });
...
REGISTER_OPERATOR_FUNCTOR(
    ...
    [](Node* n) -> SROperator {
      ...
      return quantizedLinearDynamicFP16Impl<true>(n);
    });
...
REGISTER_OPERATOR_FUNCTOR(aten::linear, aten_linear, [](Node* n) -> SROperator {
  ...
  return [](ProcessedNode* p_node) {
    ...
    auto& out_t = p_node->output(0).toTensor();
    fastResizeToZero(out_t);
    at::native::linearOut(out_t, in0_t, in1_t, in2_t);
  };
});
...
REGISTER_OPERATOR_FUNCTOR(
    ...
    [](Node* n) -> SROperator {
      return [](ProcessedNode* p_node) {
        const size_t numInputs = p_node->numInputs();
        std::vector<at::Tensor> inputs(numInputs - 1);
        for (const auto i : c10::irange(numInputs - 1)) {
          inputs[i] = p_node->input(i).toTensor();
        }
        auto dim = p_node->input(numInputs - 1).toInt();
        if (p_node->output(0).isNone()) {
          p_node->output(0) = at::native::_cat_cpu(inputs, dim);
          return;
        }
...
template <>
unsigned char absIfSigned<unsigned char>(unsigned char val) {
  return val;
}
...
void signed_log1p_out(at::Tensor& out, const at::Tensor& input) {
  ...
  AT_DISPATCH_ALL_TYPES(input.scalar_type(), "signed_log1p_kernel", [&]() {
    ...
    for (const auto i : c10::irange(N)) {
      ...
      output_data[i] = std::log1p(absIfSigned(input_data[i])) * sign;
    }
  });
}

at::Tensor signed_log1p(const at::Tensor& input) {
  auto out = createEmptyFrom(input);
...
REGISTER_OPERATOR_FUNCTOR(
    ...
    [](Node* n) -> SROperator {
      ...
      return [te](ProcessedNode* p_node) {
        const auto& input = p_node->input(0).toTensor();
        if (p_node->output(0).isNone()) {
          p_node->output(0) = createEmptyFrom(input);
        }
        auto& out = p_node->output(0).toTensor();
