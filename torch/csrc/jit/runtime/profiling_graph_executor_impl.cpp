...
void runDiffGraphPasses(std::shared_ptr<Graph>& graph) {
  ...
  // runNondiffOptimization
  {
    ...
    // Remove prim::profile nodes and embed the profile info directly in the IR
    // in value types. We're doing such transformation as optimizations that try
    // to merge/fuse nodes in the graph (e.g. BatchMM and GraphFuser) work worse
    // in the presence of intermittent prim::profile nodes.
    // Optimizations relying on the type info are also responsible for inserting
    // proper type checks. Once we're done with these optimizations we will wipe
    // the tensor type information from the IR, so that it's not accidentally
    // used by any other pass.
