...
RegisterOperators reg({
    ...
    // The self argument does not have a mutability annotation because we use
    // AliasAnalysisKind::CONSERVATIVE which assumes everything is mutable by default.
    Operator(
        "aten::backward(Tensor[] tensors, Tensor?[]? grad_tensors=None, bool? retain_graph=None, bool create_graph=False) -> ()",
        [](Stack& stack) {
          bool create_graph = pop(stack).toBool();
          auto retain_graph = pop(stack).toOptional<bool>();
    ...
    Operator(
        //NB: self argument does not have a mutability annotation because we use AliasAnalysisKind::CONSERVATIVE
        //    which assumes everything is mutable by default.
