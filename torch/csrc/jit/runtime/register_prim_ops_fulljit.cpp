...
#define DEFINE_CONVERT_BASE_OP(op_name, prefix, char_op) \
  Operator(                                              \
    ...
    DEFINE_GENERIC_BINARY_OP(aten::log, std::log(a) / std::log(b), float, complex),
    ...
    DEFINE_UNARY_COMPLEX_OP(aten::isnan, std::isnan(a.real()) || std::isnan(a.imag()), bool),
    DEFINE_UNARY_COMPLEX_OP(aten::isfinite, std::isfinite(a.real()) && std::isfinite(a.imag()), bool),
    DEFINE_UNARY_COMPLEX_OP(aten::isinf, std::isinf(a.real()) || std::isinf(a.imag()), bool),
