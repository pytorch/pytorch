...
// share the same shape compute graph, which is memory efficient and also will help
// speed up shape analysis by caching the result of running consecutive ops 
...
// is not recoverable from the shape compute graph, since the shape compute graph
// replaces Tensor inputs with List[int] and there are operators like Conv
...
// TODO: we are matching on exact string here, which is brittle and can fail in non-obvious ways,
// e.g., you have to return (Tensor) instead of Tensor. It would be nice to use something like
// OperatorMap<T> like `OperatorSet` but for mapping Operators to T
// TODO: if a unary function has an equivalent inplace operation, then the shape function must 
// be an identity. We could add those automatically. 
...
std::unordered_map<std::string, std::shared_ptr<Graph>> schema_string_to_function_graph;

std::unordered_map<const FunctionSchema*, std::shared_ptr<Graph>> cached_schema_to_graph;
...
void loadModule(const CompilationUnit& module) {
  ...
  for (const auto& pair: schema_string_to_function_name_mappings) {
    ...
    if (reused_functions.count(shape_compute_function_name)) {
      schema_string_to_function_graph[schema_string] = reused_functions[shape_compute_function_name];
      continue;
    }

    Function& shape_compute_function = module.get_function(shape_compute_function_name);
...
void loadFunctions() {
  compilation_unit.define(c10::nullopt, shape_compute_functions, nativeResolver(), nullptr);
