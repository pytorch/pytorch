...
static const OperatorMap<std::string>& get_schema_to_function_graph() {
  ...
  static const OperatorMap<std::string> schema_to_function_graph{
      ...
      {"aten::zeros(int[] size, *, ScalarType? dtype=None, int? layout=None, Device? device=None, bool? pin_memory=None) -> (Tensor)", "unary"},
      {"aten::to.dtype(Tensor(a) self, ScalarType dtype, bool non_blocking=False, bool copy=False, int? memory_format=None) -> (Tensor(a))", "unary"},
      {"aten::arange(Scalar end, *, ScalarType? dtype=None, int? layout=None, Device? device=None, bool? pin_memory=None) -> (Tensor)", "arange_end"},
