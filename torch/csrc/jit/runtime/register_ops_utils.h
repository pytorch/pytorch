...
// define implementations for primitive number ops
#define DEFINE_GENERIC_OP_WITH_COMPLEX(aten_op, int_op, float_op, complex_op, int_result, float_result, complex_result) \
  OperatorGenerator(                                                           \
      TORCH_SELECTIVE_SCHEMA(#aten_op ".int(int a, int b) -> " #int_result),   \
      [](Stack* stack) {                                                       \
        int64_t a, b;                                                          \
      ...
      aliasAnalysisFromSchema()),                                              \
  OperatorGenerator(                                                           \
      TORCH_SELECTIVE_SCHEMA(#aten_op ".complex(complex a, complex b) -> " #complex_result), \
      [](Stack* stack) {                                                       \
...
#define DEFINE_INT_COMPLEX_OP(aten_op, op, result)                            \
  OperatorGenerator(                                                        \
      ...
      OperatorGenerator(                                                    \
          TORCH_SELECTIVE_SCHEMA(#aten_op                                   \
                                 ".complex_int(complex a, int b) -> " #result), \
          [](Stack* stack) {                                                \
...
#define DEFINE_FLOAT_COMPLEX_OP(aten_op, op, result)                        \
  OperatorGenerator(                                                        \
      TORCH_SELECTIVE_SCHEMA(#aten_op                                       \
                             ".float_complex(float a, complex b) -> " #result),     \
      [](Stack* stack) {                                                    \
        double a;                                                            \
        c10::complex<double> b;                                             \
        pop(stack, a, b);                                                   \
      ...
      OperatorGenerator(                                                    \
          TORCH_SELECTIVE_SCHEMA(#aten_op                                   \
                                 ".complex_int(complex a, float b) -> " #result), \
          [](Stack* stack) {                                                \
            c10::complex<double> a;                                         \
            double b;                                                        \
            pop(stack, a, b);                                               \
...
#define DEFINE_SCALAR_BINARY_OP_WITH_COMPLEX(aten_op, int_op, float_op, complex_op, result) \
  OperatorGenerator(                                                      \
      TORCH_SELECTIVE_SCHEMA(                                             \
