...
+ std::string(R"=====(def embedding_bag_padding_idx(weight: List[int],
    indices: List[int],
    offsets: List[int],
    mode: int,
    include_last_offset: bool) -> Tuple[List[int], List[int], List[int], List[int]]:
  if torch.eq(torch.len(weight), 2):
    pass
  else:
    ops.prim.RaiseException("AssertionError: ")
  if torch.eq(torch.len(indices), 1):
    ...
    ops.prim.RaiseException("AssertionError: ")
  if torch.eq(torch.len(offsets), 1):
    pass
  else:
    ops.prim.RaiseException("AssertionError: ")
  output_bag_shape = annotate(List[int], [])
  out_dim0 = offsets[0]
  if include_last_offset:
    out_dim00 = torch.sub(out_dim0, 1)
  else:
    out_dim00 = out_dim0
  out_dim1 = weight[1]
  ...
  if torch.eq(mode, 1):
    ...
    offset2bag_shape1 = annotate(List[int], [])
    for _3 in range(torch.len(indices)):
      elem = indices[_3]
      _4 = torch.append(offset2bag_shape1, elem)
    offset2bag_shape0 = offset2bag_shape1
  bag_size_shape = annotate(List[int], [])
  for _5 in range(torch.len(offsets)):
    elem0 = offsets[_5]
    _6 = torch.append(bag_size_shape, elem0)
  if torch.eq(mode, 2):
    max_indices_shape0 = annotate(List[int], [])
    for _7 in range(torch.len(output_bag_shape)):
      elem1 = output_bag_shape[_7]
      _8 = torch.append(max_indices_shape0, elem1)
    max_indices_shape = max_indices_shape0
  else:
    max_indices_shape1 = annotate(List[int], [])
    for _9 in range(torch.len(offsets)):
      elem2 = offsets[_9]
      _10 = torch.append(max_indices_shape1, elem2)
    max_indices_shape = max_indices_shape1
  _11 = (output_bag_shape, offset2bag_shape0, bag_size_shape, max_indices_shape)
  return _11

)=====")
+ std::string(R"=====(def mm(self: List[int],
...
const OperatorMap<std::string>& GetShapeFunctionMappings() {
 static const OperatorMap<std::string> shape_mappings {
    ...
    {"aten::embedding_bag.padding_idx(Tensor weight, Tensor indices, Tensor offsets, bool scale_grad_by_freq, int mode, bool sparse, Tensor? per_sample_weights, bool include_last_offset, int? padding_idx) -> (Tensor, Tensor, Tensor, Tensor)", "embedding_bag_padding_idx"},
