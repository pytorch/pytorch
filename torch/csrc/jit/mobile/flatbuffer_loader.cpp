...
InlinedCallStackPtr torch::jit::FlatbufferLoader::parseInlinedCallStack(
  ...
  auto type_ptr =
      module_type_name.empty() ? nullptr : cu->get_class(module_type_name);
  ...
  if (inlinedCallStack->calle()) {
    callee =
        parseInlinedCallStack(inlinedCallStack->calle_as_InlinedCallStack());
  }
...
ska::flat_hash_map<int64_t, DebugInfoTuple> torch::jit::FlatbufferLoader::
    parseMobileDebugInfo(
  ...
  for (uint32_t i = 0; i < mobile_debug_infos->size(); ++i) {
    ...
    if (mobileDebugInfo->inlined_call_stack()) {
      inlinedCallStackPtr =
          parseInlinedCallStack(mobileDebugInfo->inlined_call_stack());
    }
    ...
    SourceRange source_range;
    callstack_ptrs[debug_handle] =
        std::make_tuple(source_range, node_name, inlinedCallStackPtr);
  }
