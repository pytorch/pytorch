...
static std::string variableType(const std::shared_ptr<c10::Type>& t) {
  if (t->kind() == TypeKind::IntType) {
    ...
    return "bool";
  } else if (t->kind() == TypeKind::DimentionedTensorType) {
    auto const tt = t->cast<DimentionedTensorType>();
...
static std::string encodeRHS(const Node* n) {
  ...
  size_t i = 0;
  auto outtype =
      n->output()->type()->expect<c10::DimentionedTensorType const>()->scalarType();
