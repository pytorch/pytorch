...
at::Tensor interpolate(
    ...
    c10::optional<bool> recompute_scales) {
...
Operation interpolate_op(const Node* n) {
  return [](Stack& stack) {
    ...
    IValue recompute_scales;
    pop(stack, input, size, scale_factors, mode, align_corners, recompute_scales);
    at::Tensor res = interpolate(
        input, size, scale_factors, mode, align_corners.toOptional<bool>(), recompute_scales.toOptional<bool>());
...
RegisterOperators reg3({
    Operator(
        "aten::__interpolate(Tensor input, int? size = None, float[]? scale_factor = None, str mode = 'nearest', bool? align_corners = None, bool? recompute_scales = None) -> Tensor",
        interpolate_op,
        aliasAnalysisFromSchema()),
    Operator(
        "aten::__interpolate(Tensor input, int[]? size = None, float[]? scale_factor = None, str mode = 'nearest', bool? align_corners = None, bool? recompute_scales = None) -> Tensor",
