...
struct C10_EXPORT RegisterLlgaFuseGraph
  ...
  static bool setEnabled(bool enabled) {
    ...
    if (enabled) {
      ...
      // From oneDNN v3.3, it's now disabled by default to avoid potential OOM
      // issues since there's no cap on its capacity. But, in practice, OOM
      // issues do not manifest.
