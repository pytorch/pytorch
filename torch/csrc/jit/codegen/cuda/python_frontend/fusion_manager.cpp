...
FusionCacheEntry::FusionCacheEntry() :
  ...
  record_hash_map(),
  is_terminal(true),
  fusion_executor_cache(std::make_unique<NvfFusionExecutorCache>(std::make_unique<NvfFusion>())) {}

FusionManager::FusionManager() :
  fusion_cache_start_(std::make_unique<FusionCacheEntry>(std::make_shared<RecordFunctor>(new StartRecord()))),
  fusion_cache_ptr_(fusion_cache_start_.get()) {}
  
std::vector<at::Tensor> FusionManager::execute(const at::ArrayRef<c10::IValue>& inputs) {
...
c10::optional<FusionCacheEntry*> FusionManager::lookupFusionCacheEntry(RecordFunctor* rec) const {
...
void FusionManager::createFusionCacheEntry(std::shared_ptr<RecordFunctor> &rec) {
  fusion_cache_ptr_->record_hash_map[rec.get()] = std::make_unique<FusionCacheEntry>(rec);
}
void FusionManager::createTerminalFusionCacheEntry(std::shared_ptr<RecordFunctor> &rec) {
  fusion_cache_ptr_->record_hash_map[rec.get()] = std::make_unique<FusionCacheEntry>();
}
