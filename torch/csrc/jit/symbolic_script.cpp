...
const std::vector<std::string> functions = {
    ...
    R"(
        ...
        def AD_interpolate_scales_list(input,
                                       ...
                                       recompute_scales: bool):
            input_dim = len(input.size())
            if scale_factor is None or recompute_scales:
        ...
        def __interpolate_0(input,
                            ...
                            recompute_scales: Optional[bool]):
            def backward(grad_output):
                if align_corners is None:
                    align_corners = False
                if recompute_scales is None:
                    recompute_scales = True
                scales = AD_interpolate_scales_list(input, scale_factor, recompute_scales)
        ...
        def __interpolate_2(input,
            ...
            def backward(grad_output):
                ...
                if recompute_scales is None:
                    recompute_scales = True
                scales = AD_interpolate_scales_float(input, scale_factor, recompute_scales)
                grad_self = AD_interpolate_backward(grad_output, input, mode, align_corners, scales)
                return grad_self, None, None, None, None, None
