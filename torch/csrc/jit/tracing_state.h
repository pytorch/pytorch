...
struct ArgumentStash {
  struct IntListRefTrace : std::vector<Value*> {
    IntListRefTrace(int size) : std::vector<Value*>(size, nullptr) {}
  };
  ...
  TORCH_API static void stashIntListRefElem(
      const std::string& arg_name,
      size_t size,
      size_t idx,
      const Variable& var);

  static bool hasIntListRef(const std::string& arg_name) {
    return stash.intlists.count(arg_name) > 0;
  }

  static IntListRefTrace popIntListRef(const std::string& arg_name) {
  ...
  // handling like in the case of IntListRefs
 ...
 private:
  ...
  std::unordered_map<std::string, IntListRefTrace> intlists;
