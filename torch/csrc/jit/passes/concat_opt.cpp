 ...
 private:
  void handleBlock(Block* block) {
    for (auto node : block->nodes()) {
      if (node->kind() == prim::ConstantConcat) {
        handleCat(node);
      }
  ...
  void handleCat(Node* node) {
    ...
    //
    // Example:
    //    %11 = prim::ConstantConcat(%0, %1, <dim>)
    //    ...
    //    %13 = prim::ConstantConcat(%0, %1, %2, <dim>) // first 2 inputs same
    //                                                  // as %11
    //    ...
    //        = %13 ... // Use %13
    //
    ...
    //    %11 = prim::ConstantConcat(%0, %1, <dim>)
    //    ...
    //    %14 = prim::ConstantConcat(%11, %2, <dim>) // Replace first 2 inputs
    //                                               // with %11
    //    ...
    ...
    for (const auto& prev : concated_outputs_) {
      ...
      if (equal(curr_tensor_inputs_prefix, prev_tensor_inputs) &&
        ...
        auto new_concat =
            node->owningGraph()->create(prim::ConstantConcat, new_inputs);
...
class VariadicCatUpdater {
 ...
 private:
  ...
  bool replaceWithVariadicCat(Node* cat) {
    ...
    auto var_cat = cat->owningGraph()->create(prim::ConstantConcat, inputs);
    GRAPH_UPDATE("Adding\n", *var_cat);
