...
void RegisterIndexPutInBlocks(Value* orig_data, Value* new_index_put,
  Node* block_node, Block* outer_block, Node* next_node) {
  auto cur_node = next_node;
  while (nullptr != cur_node) {
    if (cur_node->kind() != ::c10::onnx::Loop)
      return;
...
void SquashSliceAndSelect(Node* index_put_node) {
  ...
  RegisterIndexPutInBlocks(orig_data, new_index_put,
    block_node, outer_block, next_node);
}
