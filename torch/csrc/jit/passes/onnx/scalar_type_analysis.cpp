...
static c10::optional<c10::ScalarType> LowPrecisionCastForStandardOps(
  ...
  if (n->kind() != onnx::Gemm &&
        ...
        (scalar_type == c10::ScalarType::Byte ||
          scalar_type == c10::ScalarType::Char ||
          scalar_type == c10::ScalarType::Short)) {
      return c10::kLong;
    }
  return scalar_type;
}
...
static void ImplicitCastNodeForONNX(Node* n) {
  if (IsImplicitCastSupported(n->kind())) {
    ...
    if (expected_scalar_type) {
      expected_scalar_type = LowPrecisionCastForStandardOps(n, *expected_scalar_type);
