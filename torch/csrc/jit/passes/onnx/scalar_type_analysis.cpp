...
static c10::optional<c10::ScalarType> InferExpectedScalarType(
    const Node* n,
  ...
  std::for_each(
      n->inputs().begin(), n->inputs().end(), [&](const Value* input) {
        ...
        // StandardOps do not support Byte/Char/Short in ORT, cast input to Long
        // type, and cast output back again. onnx::Geem only support
        // float/double now.
        if (n->kind() != onnx::Gemm &&
            standardOps.find(n->kind()) != standardOps.end() &&
            scalar_type_out.has_value() &&
            (scalar_type_out.value() == c10::ScalarType::Byte ||
             scalar_type_out.value() == c10::ScalarType::Char ||
...
static void ImplicitCastNodeForONNX(Node* n) {
  if (IsImplicitCastSupported(n->kind())) {
    ...
    if (opUnsupportedType.find(n->kind()) != opUnsupportedType.end()) {
      UpdateScalarTypeForOutput(n, opUnsupportedType[n->kind()]);
      opUnsupportedType.erase(n->kind());
    }
  }
}
