...
std::string nameFromRoot(torch::jit::ScopePtr scope, const std::string& layer_separator, NameFunc nameFunc) {
...
std::pair<std::string, std::string> parseNameFromScope(torch::jit::ScopePtr scope) {
  std::string full_name = scope->name().toUnqualString();
  auto pos = full_name.find(name_separator);
  TORCH_CHECK(pos != std::string::npos, "Scope name (" + full_name + ") does not contain '" + name_separator + "'");
  return std::make_pair(full_name.substr(0, pos), full_name.substr(pos + 2));
}

} // namespace

std::string createFullScopeName(const std::string& class_name, const std::string& variable_name) {
...
std::string variableNameFromRoot(torch::jit::ScopePtr scope, const std::string& layer_separator) {
...
std::string classNameFromRoot(torch::jit::ScopePtr scope, const std::string& layer_separator) {
  return nameFromRoot(scope, layer_separator, &className);
}

bool isCompatibleScope(torch::jit::ScopePtr scope) {
  return !scope->isRoot() && !scope->isBlank() && (std::string(scope->name().toUnqualString()).find(name_separator) != std::string::npos);
}
} // namespace ONNXScopeName
...
class NodeNameGenerator {
  ...
  protected:
    std::string UpdateBaseName(std::unordered_map<std::string, size_t> &base_name_count, std::string base_name);
...
std::string NodeNameGenerator::UpdateBaseName(std::unordered_map<std::string, size_t> &base_name_count, std::string base_name) {
...
bool NodeNameGenerator::IsGraphOutput(const Value* v, const std::shared_ptr<Graph> graph) const {
...
c10::optional<std::string> ScopedNodeNameGenerator::GetName(const Node* n) const {
  if (node_names_.find(n) == node_names_.end()) {
    return c10::nullopt;
  }
...
std::string ScopedNodeNameGenerator::GetFullScopeName(ScopePtr scope) {
  if (full_scope_names_.find(scope) == full_scope_names_.end()) {
    auto full_scope_name = ONNXScopeName::variableNameFromRoot(scope, layer_separator_);
    full_scope_names_[scope] = UpdateBaseName(base_scope_name_counts_, full_scope_name);
  }
