...
std::tuple<bool, at::optional<size_t>> fusibleExpandTo(at::IntList from, at::IntList to) {
  if (from.size() > to.size()) {
    return std::make_tuple<bool, at::optional<size_t>>(false, {});
  }
  ...
  if (trailing_expand && f <= from_dim_start) {
    return std::make_tuple<bool, at::optional<size_t>>(true, {});
  }
  ...
  if (leading_expand && f >= from_dim_end) {
    return std::make_tuple<bool, at::optional<size_t>>(true, 0);
  }
