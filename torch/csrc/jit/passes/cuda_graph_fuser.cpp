...
struct CudaGraphFuser {
  ...
  at::optional<Node*> tryFuse(Node* consumer, Value* producer) {
    ...
    bool shouldFuse =
#ifndef C10_MOBILE
        fuser::cuda::isFusable(consumer, producer->node()) &&
...
#else
        // WAR for linker error for accessing APIs under
        // codegen/cuda/interface.h on MOBILE build.
        false;
...
#ifndef C10_MOBILE
          return fuser::cuda::isFusable(consumer, producer_for_chunk->node()) &&
#else
          // WAR for linker error for accessing APIs under
          // codegen/cuda/interface.h on MOBILE build.
          TORCH_INTERNAL_ASSERT(false);
...
#ifndef C10_MOBILE
    if (fuser::cuda::isFusable(consumer)) {
#else
    // WAR for linker error for accessing APIs under
    // codegen/cuda/interface.h on MOBILE build.
    TORCH_INTERNAL_ASSERT(false);
