...
static std::string GetLintDescriptionString(ModuleLintCode lint_code) {
  ...
  switch (lint_code) {
    case ModuleLintCode::ML_BUNDLED_INPUT:
      lint_str =
          "Please add bundled inputs before saving the module using "
...
static void checkBundledInputs(
    std::map<std::string, std::string>& lints,
    const script::Module& module) {
  auto get_method = module.find_method("get_all_bundled_inputs");
  if (!get_method) {
    lints[GetLintNameString(ModuleLintCode::ML_BUNDLED_INPUT)] =
        GetLintDescriptionString(ModuleLintCode::ML_BUNDLED_INPUT);
  }
}

std::map<std::string, std::string> GenerateModuleLints(
    const jit::script::Module& module) {
