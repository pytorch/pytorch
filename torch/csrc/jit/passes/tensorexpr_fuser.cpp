...
#define REQ(cond)                           \
  ...
  bool canMerge(Node* consumer, Node* producer) {
    ...
    // Device checks
    if (consumer->kind() != aten::cat && producer->kind() != aten::cat && consumer->kind() != aten::stack && producer->kind() != aten::stack) {
    ...
    } else if (consumer->kind() == aten::cat || consumer->kind() == aten::stack) {
