...
struct GraphFuser {
  ...
  bool isFusableGradSumToSize(Node * node, Node * consumer) {
    if (node->owningBlock() != block_
	|| node->kind() != prim::GradSumToSize
	|| consumer->kind() == aten::cat
	|| consumer->kind() == prim::FusedConcat) return false;
    if (consumer->kind() == prim::FusionGroup) {
      std::cerr << "fusion group as consumer" << std::endl;
  ...
  at::optional<Node*> tryFuse(
    ...
    bool shouldFuse = (isFusable(producer->node()) ||
		       isFusableGradSumToSize(producer->node(), consumer)) &&
