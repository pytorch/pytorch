...
struct GraphFuser {
  ...
  bool containsFusedConcat(Node * fusion_group) {
  ...
  bool isFusable(Node * node, Node * consumer) {
    // We don't want to bother with cross-block node movements, as they
    // are not necessarily correct.
    if (node->owningBlock() != block_) return false;
    if (isSimpleMap(node)) return true;
    if (node->kind() == prim::FusionGroup) {
      if (! consumer) return true;
      if (containsGradSumToSize(node)) {
	return !(consumer->kind() == aten::cat
		 || consumer->kind() == prim::FusedConcat
		 || (consumer->kind() == prim::FusionGroup
		     && containsFusedConcat(consumer)));
      }
      if (containsFusedConcat(node)) {
	return !(consumer->kind() == prim::FusionGroup
		 && containsGradSumToSize(consumer));
      }
      return true;
    }
    if (node->kind() == prim::GradSumToSize) {
      if (! consumer) return false; // we currently don't want to fue it
      return !(consumer->kind() == aten::cat
	       || consumer->kind() == prim::FusedConcat
	       || (consumer->kind() == prim::FusionGroup
		   && containsFusedConcat(consumer)));
  ...
  bool isFusableAsExitNode(Node * node) {
    return isFusable(node, nullptr) || isFusableOnlyAsExitNode(node);
  }
  ...
  at::optional<Node*> tryFuse(
    ...
    Node* real_consumer = consumer->kind() == aten::cat
        ...
        : consumer;
    bool shouldFuse = isFusable(producer->node(), consumer) &&
  ...
  bool tryToMoveChunk(Node * consumer, Value * producer) {
    ...
    auto it = std::find_if(
        ...
        [&](Value * producer_for_chunk) {
          return isFusable(producer_for_chunk->node(), consumer) &&
