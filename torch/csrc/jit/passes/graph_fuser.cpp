...
struct GraphFuser {
  ...
  bool shouldFuse(Node * consumer, Value * producer) {
    ...
    return isFusable(producer->node()) &&
      ...
      // Rearrange nodes such that all uses of producer are after the consumer.
      // Fusion will rewrite those later uses to use the version of producer
      // generated by the fused blob. In this case, producer becomes an output
      // of the fusion group.
