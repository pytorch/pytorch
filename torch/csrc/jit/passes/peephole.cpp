...
void PeepholeOptimizeImpl(Block* block, bool addmm_fusion_enabled) {
  for (auto it = block->nodes().begin(); it != block->nodes().end(); ++it) {
    ...
    if (node->matches(
      ...
      auto self_type = node->input(0)->type()->cast<DimentionedTensorType>();
      auto other_type = node->input(1)->type()->cast<DimentionedTensorType>();
      ...
      if (addmm_fusion_enabled &&
        ...
        for (size_t mm_side = 0; mm_side < 2; mm_side++) {
          ...
          auto add_mat_type =
              node->input(1 - mm_side)->type()->cast<DimentionedTensorType>();
          if (!add_mat_type)
            continue;
          ...
          if (node->input(mm_side)->node()->matches(
            ...
            auto mat_type = mat1.value()->type()->cast<DimentionedTensorType>();
            if (!mat_type) {
              mat_type = mat2.value()->type()->cast<DimentionedTensorType>();
            }
