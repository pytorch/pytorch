...
IValue representativeValue(Value* v) {
  ...
  if (CompleteTensorTypePtr type = type_->cast<CompleteTensorType>()) {
    // TODO: making assumption on backend
    auto backend = type->device().type() == at::DeviceType::CPU
        ? at::Backend::CPU
        : at::Backend::CUDA;
    at::DeviceGuard device_guard(type->device());
    auto& attype = at::getNonVariableType(backend, type->scalarType());
    auto t = at::empty_strided(type->sizes(), type->strides(), attype.options())
                 .zero_();
...
bool PropagateTensorShapeOnNode(Node* node, bool insert_expands) {
  ...
  // tensor outputs of the node, or an empty vector which implies that it failed
  // to propagate.
  ...
  //   scalar type    : preserved from the first argument (doesn't have to match
  //   other arguments) device         : always matching and preserved tensor
  //   inputs  : * tensor outputs : 1
  ...
  //   scalar type    : preserved for first output, byte/uint8 for second output
  //   if exists device         : preserved tensor inputs  : 1 tensor outputs :
  //   1 or 2
  // Additionally:
  //   - First input should be the only tensor input
  ...
  static const register_formula_for dim_reduce_ops{
      ...
      [](Node* node) -> type_vec_t {
        // NB: Note that while this function is generally meant to be used with
        // ops that have a single output, we will fix up its return right below.
  ...
  if (node->matches(
    ...
    // index_select behaves very weirdly when self.dim() == 0. It allows both 0D
    // and 1D indices, and returns a value that has as many dimensions as index.
    if (type && index_type) {
      if (type->dim() == 0) {
    ...
    auto type = input_type(0);
    auto index_type = input_type(1);
    // Gather has this annoying edge case where index always needs to match the
    // number of dims of self, **except** when self is 1D and index is 0D in
    // which case we return a 0D output.
    if (type && index_type) {
      if (index_type->dim() == 0) {
        node->output()->setType(type->withDim(0));
      } else {
...
bool PropagateCompleteShapeOnNode(
  ...
  if (node->matches(
    ...
    // return!), because the type casting logic in scalar cases is non-trivial.
    ...
    node->output()->setType(CompleteTensorType::create(at::kLong, -1, dims));
    return true;
