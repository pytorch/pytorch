 ...
 private:
  ...
  void PropagateCatShape(Node* cat_node) {
    ...
    static const auto propagate = [](Node* node,
      ...
      for (Value* v : tensors) {
        if (auto type = v->type()->cast<DimentionedTensorType>()) {
    ...
    switch (node->kind()) {
      ...
      case prim::NumToTensor: {
        ...
        if (typ->isSubtypeOf(IntType::get()) ||
          ...
          node->output()->setType(DimentionedTensorType::create(at::kLong, at::kCPU, 0));
        } else if (node->input()->type()->isSubtypeOf(FloatType::get())) {
          node->output()->setType(DimentionedTensorType::create(at::kDouble, at::kCPU, 0));
        }
        return;
      }
      ...
      case prim::ConstantChunk: {
        ...
        if (auto type = tensor->type()->cast<DimentionedTensorType>()) {
  ...
  bool PropagateTensorShapeOnNode(Node* node, bool insert_expands) {
    ...
    static const register_formula_for simple_unary_ops{
        ...
        [](Node* node) -> type_vec_t {
          auto input_type = node->input(0)->type()->cast<DimentionedTensorType>();
    ...
    static const register_formula_for broadcasting_ops{
        ...
        [this](Node* node) -> type_vec_t {
          if (auto maybe_tensor_types = gatherTensorTypes<DimentionedTensorType>(node)) {
    ...
    static const register_formula_for nn_ops_first_input_preserving{
        ...
        [](Node* node) -> type_vec_t {
          if (auto type = node->input(0)->type()->cast<DimentionedTensorType>()) {
            return {type};
          }
          return {};
        }};
    ...
    static const auto factory_with_ndim = [](Node* node,
      ...
      if (!maybe_scalar_type)
        return {};
      return {DimentionedTensorType::create(*maybe_scalar_type, *maybe_device, dim)};
    };
    ...
    static const register_formula_for like_factories_with_options{
        ...
        [](Node* node) -> type_vec_t {
          if (auto type =
                  node->namedInput(attr::self)->type()->cast<DimentionedTensorType>()) {
    ...
    const auto input_type = [node](size_t index) {
      return node->input(index)->type()->cast<DimentionedTensorType>();
    };
    if (node->matches(
