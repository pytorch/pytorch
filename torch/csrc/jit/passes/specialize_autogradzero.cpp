...
void specializeAutogradZero(Graph &g, bool in_profiling_mode) {
  ...
  for (auto it = g.nodes().begin(); it != g.nodes().end(); ++it) {
    ...
    switch (n->kind()) {
      ...
      case prim::profile: {
        if (n->inputs().size() > 0) {
          std::cout << "propagating state " << (int)state[n->input()]
                    << " from " << n->input()->debugName() << " to "
                    << n->output()->debugName() << std::endl;
      ...
      case prim::Guard: {
        ...
        std::cout << "setting guard " << n->output()->debugName()
                  << " type = " << n->output()->type().get() << std::endl;
