...
template <typename T>
T unionRefinements(const T& ref1, const T& ref2);

template <typename T>
T intersectRefinements(const T& ref1, const T& ref2);
...
using ListRefinement = std::unordered_map<Value*, int64_t>;
using BooleanListRefinement = BooleanRefinementMapping<ListRefinement>;
...
void joinIfRefinements(
    ...
    std::unordered_map<Value*, BooleanRefinementMapping<T>>&
  ...
  for (const auto i : c10::irange(if_n.outputs().size())) {
    ...
    BooleanRefinementMapping<T> out;
    if (auto maybe_bool = constant_as<bool>(true_v)) {
      if (*maybe_bool) {
        out = BooleanRefinementMapping<T>::FalseRefinements(unionRefinements<T>(
            boolean_value_refinements[false_v].false_refine(),
            false_block_refinements));
      } else {
        out = BooleanRefinementMapping<T>::TrueRefinements(unionRefinements<T>(
...
bool handleCommonRefinentOperators(
    Node* n,
    std::unordered_set<Block*>& throwing_blocks,
    std::unordered_map<Value*, BooleanRefinementMapping<T>>& info) {
  ...
  if (n->kind() == aten::__not__ &&
    ...
    if (info.count(n->input())) {
      ...
      info[n->output()] = BooleanRefinementMapping<T>(
