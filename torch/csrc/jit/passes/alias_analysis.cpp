...
void AliasDb::makePointerTo(const Value* from, const Value* to) {
  ...
  if (!aliasingType(from)) {
    TORCH_INTERNAL_ASSERT(!aliasingType(to));
    return;
  }
  ...
  TORCH_INTERNAL_ASSERT(aliasingType(from) && aliasingType(to));
...
bool AliasDb::mayAlias(const Value* a, const Value* b) const {
  if (!aliasingType(a) || !aliasingType(b)) {
    return false;
  }
...
Element* AliasDb::getOrCreateWildcard(const TypePtr& type) {
  TORCH_INTERNAL_ASSERT(aliasingType(type));
