...
#define SYMNODE_SIZES_STRIDES(n)                \
  ...
  m.def(
      ...
      [](const FunctionSchema& schema, py::args args, py::kwargs kwargs) {
        // checkSchemaAllowFakeScriptObject will throw runtime error if there is a schema mismatch.
        // we drop the returned stack as it's not useful when callling in python.
