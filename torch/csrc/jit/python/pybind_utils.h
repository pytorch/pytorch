...
inline py::object _get_operation_for_overload_or_packet(
  const std::vector<std::shared_ptr<Operator>>& operations, Symbol symbol,
  py::args args, const py::kwargs& kwargs, bool is_overload) {
    std::vector<py::handle> overloaded_args;
      size_t total_arg_num = args.size() + kwargs.size();
      ...
      if (overloaded_args.size() > 0) {
        ...
        if (is_overload) {
          self_func = self_func.attr(operations[0]->schema().overload_name().c_str());
        }
