...
SugaredValuePtr ModuleValue::getitem(
  ...
  if (concreteType_->getIterableModuleKind() == IterableModuleKind::LIST) {
    if (type_hint) {
      ...
      // Emit a prim::ModuleContainerIndex operator. This is needed because it's
      // difficult to construct a list in the graph representing the ModuleList
      // and use aten::__getitem__ ops to index into it because any call to
      // ModuleList.setitem would invalidate that emitted list.
      auto graph = m.graph();
