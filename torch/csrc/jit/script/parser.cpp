...
Decl mergeTypesFromTypeComment(
  ...
  for (; i < decl.params().size(); ++i, ++j) {
    new_params.push_back(
        Param::create(old[i].range(), old[i].ident(), _new[j].type()));
  }
  return Decl::create(
      decl.range(),
...
TreeRef Parser::createApply(Expr expr) {
  TreeList attributes;
...
// exp | expr, | expr, expr, ...
TreeRef Parser::parseExpOrExpTuple(int end) {
...
TreeRef Parser::parseOptionalReduction() {
  auto r = L.cur().range;
  switch (L.cur().kind) {
    case TK_PLUS_EQ:
    case TK_MINUS_EQ:
...
Expr Parser::parseExp(int precedence) {
  ...
  while (shared.isBinary(L.cur().kind, &binary_prec)) {
    if (binary_prec <= precedence) {
      // not allowed to parse something which is
    ...
    switch (ret_str[pos + 1]) {
      ...
      default:
        throw ErrorReport(range)
            << " octal and hex escaped sequences are not supported";
    }
...
Expr Parser::parseAttributeValue() {
  return parseExp();
}

void Parser::parseOperatorArguments(TreeList& inputs, TreeList& attributes) {
  L.expect('(');
  if (L.cur().kind != ')') {
    do {
...
TreeRef Parser::parseSubscript(TreeRef value) {
...
Param Parser::parseBareTypeAnnotation() {
  auto type = parseExp();
  return Param::create(type.range(), Ident::create(type.range(), ""), type);
}

TreeRef Parser::parseTypeComment(bool parse_full_line) {
  auto range = L.cur().range;
  if (parse_full_line) {
...
// alone on a line:
// first[,other,lhs] = rhs
Assign Parser::parseAssign(List<Expr> list) {
...
TreeRef Parser::parseStmt() {
  switch (L.cur().kind) {
    ...
    default: {
      List<Expr> exprs =
          parseList(TK_NOTHING, ',', TK_NOTHING, &Parser::parseExp);
...
TreeRef Parser::parseOptionalIdentList() {
...
TreeRef Parser::parseIf(bool expect_if) {
  auto r = L.cur().range;
  if (expect_if)
    L.expect(TK_IF);
  auto cond = parseExp();
...
TreeRef Parser::parseStatements(bool expect_indent) {
  auto r = L.cur().range;
  if (expect_indent)
    L.expect(TK_INDENT);
  TreeList stmts;
...
Decl Parser::parseDecl() {
  ...
  L.expect(':');
  return Decl::create(
      paramlist.range(), List<Param>(paramlist), Maybe<Expr>(return_type));
}
...
TreeRef Parser::c(int kind, const SourceRange& range, TreeList&& trees) {
  return Compound::create(kind, range, std::move(trees));
}
TreeRef Parser::makeList(const SourceRange& range, TreeList&& trees) {
