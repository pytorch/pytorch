...
As the trace runs, individual operators create `Nodes` in the `Graph` being traced to record what happens. This code is currently generated per operator in [torchgen/autograd/gen_variable_type.py](/torchgen/autograd/gen_variable_type.py). It results in code that looks like the following:
