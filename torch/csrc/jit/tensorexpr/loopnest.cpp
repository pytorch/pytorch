...
class FunctionInliner : public IRMutator {
 public:
  FunctionInliner(const StorePtr& producer, std::unordered_set<BufPtr> outputs)
      : buf_(producer->buf()),
        producer_(producer),
...
void LoopNest::splitWithMask(const ForPtr& f, int factor, ForPtr* inner) {
  ...
  ExprPtr split_count = alloc<Div>(
      alloc<Sub>(alloc<Add>(size, factor_expr), immLike(std::move(size), 1)), factor_expr);
  ...
  if (tail_is_needed) {
    ...
    ExprPtr predicate =
        CompareSelect::make(ExprHandle(f->var()), ExprHandle(f->stop()), kLT)
            .node();
    body_inner = Cond::make(ExprHandle(std::move(predicate)), body_inner, nullptr);
  }
...
bool areEqual(const ExprPtr& expr1, const ExprPtr& expr2) {
  ...
  *rfac_buf_ptr = alloc<Buf>(
  ...
  outer_reduction_for->body()->insert_stmt_after(
      alloc<Store>(
          ...
          orig_buf_indices,
          reduce_op->reducer()(
              orig_buf, std::move(final_reduce_load), orig_buf_indices, {std::move(reduction_var)})),
