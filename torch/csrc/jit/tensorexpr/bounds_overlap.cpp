  ...
  if (!lowDiff->isConstant()) {
    ...
    if (vars.size() == 1) {
      lowDiff = IRSimplifier::simplify(alloc<Sub>(
          Substitute(b.start, {{*vars.begin(), alloc<IntImm>(1)}}),
          Substitute(a.start, {{*vars.begin(), alloc<IntImm>(1)}})));
    }
  }
  ...
  if (!highDiff->isConstant()) {
    ...
    if (vars.size() == 1) {
      highDiff = IRSimplifier::simplify(alloc<Sub>(
          Substitute(b.end, {{*vars.begin(), alloc<IntImm>(1)}}),
          Substitute(a.end, {{*vars.begin(), alloc<IntImm>(1)}})));
    }
  }
