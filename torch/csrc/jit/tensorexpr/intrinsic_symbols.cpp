...
#if !defined(_MSC_VER)
// Here follows a huge hack: if we're building with sleef, we want the
// vectorized function declarations; that's the whole point of sleef.
// But we only compile a very small part of the codebase with AVX, and
// access it through a dispatcher that checks cpuinfo at runtime.
//
// Here, we don't actually need to compile anything with AVX, we just
// need the addresses of the sleef library functions so that we can
// link jitted code against them, so we **temporarily** define
// __AVX__, include the sleef header, and then undef __AVX__.  This
// gets us the symbols without actually mucking up the build.
#  ifndef __AVX__
#    define __AVX__
#    include <sleef.h>
#    undef __AVX__
