...
const Expr* IRSimplifier::mutate(const Add* v) {
  ...
  if (lhsLinear && rhs_new->isConstant()) {
    ...
    LinearForm* linear =
        new LinearForm(lhsLinear->getX(), lhsLinear->getA(), evaluateOp(B_Add));
    return linear;
  }
  ...
  if (rhsLinear && lhs_new->isConstant()) {
    ...
    LinearForm* linear =
        new LinearForm(rhsLinear->getX(), rhsLinear->getA(), evaluateOp(B_Add));
    return linear;
  }
...
const Expr* IRSimplifier::mutate(const Sub* v) {
  const Expr* lhs = v->lhs();
  const Expr* rhs = v->rhs();
  const Expr* lhs_new = lhs->accept_mutator(this);
  const Expr* rhs_new = rhs->accept_mutator(this);
  ...
  if (lhsLinear && rhs_new->isConstant()) {
    ...
    LinearForm* linear =
        new LinearForm(lhsLinear->getX(), lhsLinear->getA(), evaluateOp(B_Sub));
    return linear;
  }
  ...
  if (rhsLinear && lhs_new->isConstant()) {
    ...
    LinearForm* linear =
        new LinearForm(rhsLinear->getX(), evaluateOp(A_Sub), evaluateOp(B_Sub));
    return linear;
  }
...
const Expr* IRSimplifier::mutate(const Mul* v) {
  const Expr* lhs = v->lhs();
  const Expr* rhs = v->rhs();
  const Expr* lhs_new = lhs->accept_mutator(this);
  const Expr* rhs_new = rhs->accept_mutator(this);
  ...
  if (lhsLinear && rhs_new->isConstant()) {
    ...
    LinearForm* linear =
        new LinearForm(lhsLinear->getX(), evaluateOp(A_Mul), evaluateOp(B_Mul));
    return linear;
  }
  ...
  if (rhsLinear && lhs_new->isConstant()) {
    ...
    LinearForm* linear =
        new LinearForm(rhsLinear->getX(), evaluateOp(A_Mul), evaluateOp(B_Mul));
    return linear;
  }
...
const Expr* IRSimplifier::mutate(const Div* v) {
  // TODO div simplification will require a rational node.
  return mutateBinaryOp(v, this);
}

const Expr* IRSimplifier::mutate(const Mod* v) {
  return mutateBinaryOp(v, this);
}

const Expr* IRSimplifier::mutate(const And* v) {
  return mutateBinaryOp(v, this);
}

const Expr* IRSimplifier::mutate(const Xor* v) {
  return mutateBinaryOp(v, this);
}

const Expr* IRSimplifier::mutate(const Lshift* v) {
  return mutateBinaryOp(v, this);
}

const Expr* IRSimplifier::mutate(const Rshift* v) {
  return mutateBinaryOp(v, this);
}

const Expr* IRSimplifier::mutate(const Max* v) {
  return mutateBinaryOp(v, this, v->propagate_nans());
}

const Expr* IRSimplifier::mutate(const Min* v) {
  return mutateBinaryOp(v, this, v->propagate_nans());
}

const Expr* IRSimplifier::mutate(const Intrinsics* v) {
...
const Expr* IRSimplifier::mutate(const Cast* v) {
...
const Expr* IRSimplifier::simplify(const Expr* e) {
...
ExprHandle IRSimplifier::simplify(const ExprHandle& e) {
  return ExprHandle(simplify(e.node()));
}

Stmt* IRSimplifier::simplify(Stmt* s) {
  IRSimplifier simplifier;
 ...
 * the new op. */
const Expr* IRSimplifier::expandAndRecurse(
    IRNodeType expr_type,
    const Expr* lhs,
    const Expr* rhs) {
...
const Expr* IRSimplifier::handleBroadcastAdd(
    const Broadcast* bc,
    const Expr* other) {
...
/* Handles optimization cases for Broadcast() * Other */
const Expr* IRSimplifier::handleBroadcastMul(
    const Broadcast* bc,
    const Expr* other) {
