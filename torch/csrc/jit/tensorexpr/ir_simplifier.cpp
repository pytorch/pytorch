...
// Insert a alloc<Term> into the provided polynomial. If the new term has common
...
ExprPtr PolynomialTransformer::mutate(AddPtr v) {
  ...
  // If all else fails we have a alloc<Polynomial> with two new variable Terms.
  return alloc<Polynomial>(
      hasher_, getImmediateByType(v->dtype(), 0), lhsTerm, rhsTerm);
}
...
ExprPtr PolynomialTransformer::mutate(MulPtr v) {
  ...
  // Two variables, create a alloc<Term>.
  return alloc<Term>(
...
ExprPtr combineMinMaxTerms(
  ...
  auto add_expr_to_opterm = [&](ExprPtr expr, OpTerm* opterm) {
    ExprPtr scalar = nullptr;
    std::vector<ExprPtr> variables;
    if (opterm) {
