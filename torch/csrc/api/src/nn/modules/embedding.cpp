...
EmbeddingBag::EmbeddingBag(EmbeddingOptions options, string mode) : options(options), mode(mode) {
  EmbeddingBag::reset();
}

EmbeddingImpl& EmbeddingImpl::from_pretrained(Tensor embeddings, bool freeze = true, c10::optional<int64_t> padding_idx = c10::nullopt,
        c10::optional<float> mex_norm = c10::nullopt, float norm_type = 2., bool scale_grad_by_freq = false, bool sparse = false) {
    TORCH_CHECK(embeddings.dim() == 2, "Embeddings parameter is expected to be 2-embedding_dimal");
    EmbeddingImpl embedding = EmbeddingImpl(
...
std::tuple<Tensor, Tensor, Tensor, Tensor> EmbeddingBagImpl::forward(const Tensor& input, c10::optional<torch::Tensor> offsets,
  ...
  EmbeddingBagImpl& EmbeddingBagImpl::from_pretrained(Tensor embeddings, bool freeze = true, c10::optional<float> mex_norm = c10::nullopt,
     float norm_type = 2., bool scale_grad_by_freq = false, string mode = "sum", bool sparse = false) {
      TORCH_CHECK(embeddings.dim() == 2, "Embeddings parameter is expected to be 2-embedding_dimal");
      EmbeddingImpl embeddingbag = EmbeddingImpl(
              num_embeddings=embeddings.size(0),
              embedding_dim=embeddings.size(1),
              _weight=embeddings,
              max_norm=max_norm,
