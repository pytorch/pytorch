...
inline std::tuple<Tensor, Tensor> fractional_max_pool2d_with_indices(
  ...
  if (output_size_ == c10::nullopt) {
    ...
    output_size_ = {(int64_t)(input.sizes()[input_dims - 2] * (*output_ratio.value())[0]),
                    (int64_t)(input.sizes()[input_dims - 1] * (*output_ratio.value())[1])};
  }

  Tensor _random_samples_ = _random_samples;
  if (!_random_samples_.defined()) {
    auto n_batch = input.sizes()[0];
    if (input_dims == 3) {
      // no batch dimension
      n_batch = 1;
    }
    _random_samples_ = torch::rand({n_batch, input.sizes()[input_dims - 3], 2}, torch::TensorOptions().dtype(input.dtype()).device(input.device()));
  }
