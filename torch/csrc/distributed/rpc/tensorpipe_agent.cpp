...
using nss = std::chrono::nanoseconds;
...
TensorPipeAgent::TensorPipeAgent(
    ...
    : RpcAgent(
          WorkerInfo(std::move(selfName), std::move(selfId)),
  ...
  for (const std::pair<std::string, worker_id_t>& kv : opts_.workerNameToId) {
...
void TensorPipeAgent::pipeRead(
  ...
  pipe->readDescriptor([fn{std::move(fn)}, pipe](
    ...
    for (int i = 0; i < rpcMessage.tensors().size(); i++) {
