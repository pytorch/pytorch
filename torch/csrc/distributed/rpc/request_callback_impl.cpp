...
c10::intrusive_ptr<JitFuture> RequestCallbackImpl::processScriptCall(
  ...
  return future->then(
      [](JitFuture& jitFuture) {
        return withStorages(ScriptResp(jitFuture.value()).toMessage("ScriptResp meta"));
      },
...
c10::intrusive_ptr<JitFuture> RequestCallbackImpl::processPythonCall(
  ...
  return future->then(
      [](JitFuture& future) {
        return withStorages(
            PythonResp(serializePyObject(future.value())).toMessage("PythonResp resp"));
      },
      c10::getCustomClassType<c10::intrusive_ptr<OutgoingMessage>>());
}
...
c10::intrusive_ptr<JitFuture> RequestCallbackImpl::processPythonRRefFetchCall(
  ...
  return future->then(
      [](JitFuture& future) {
        ...
        return withStorages(
            PythonRRefFetchRet(std::move(result).toIValues()).toMessage("PythonRRefFetchRet meta"));
      },
      c10::getCustomClassType<c10::intrusive_ptr<OutgoingMessage>>());
}
  ...
  return future->then(
      [autogradContextId = rrefBackwardReq.getAutogradContextId(),
       retainGraph = rrefBackwardReq.retainGraph()](JitFuture& future) {
        ...
        return withStorages(RRefBackwardResp().toMessage("RRefBackwardResp meta"));
      },
      c10::getCustomClassType<c10::intrusive_ptr<OutgoingMessage>>());
}
