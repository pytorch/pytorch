...
std::tuple<tensorpipe::Message, TensorpipeWriteBuffers> tensorpipeSerialize(
  ...
  // The corresponding unpickler in `tensorpipeDeserialize` uses tensor id as an
  // index in `buffers.tensors` (see tensorReadFunc). Meta tensors don't have
  // data and are not present in buffers.tensors, so to skip meta tensors
  // `non_meta_idx` is used to generate consecutive indices for non-meta tensors
  // in `buffers.tensors`. The `meta_idx` is used to generate unique ids for the
  // remaining meta tensors, but they are unused.
  int non_meta_idx = 0;
  int meta_idx = std::count_if(
      buffers.tensors.begin(), buffers.tensors.end(), [](auto& t) {
        return !t.is_meta();
      });
  ...
  int nonMetaTensorsSize =
      std::count_if(tensorDataVec.begin(), tensorDataVec.end(), [](auto& t) {
        return !t.is_meta();
      });
  ...
  for (const auto i : c10::irange(tensorDataVec.size())) {
    ...
    TORCH_INTERNAL_ASSERT(
        tpMessage.tensors.size() == i + 1 - metaTensorsCounter);
