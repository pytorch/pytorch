...
static void initialize_nvshmem_with_store(
  ...
  // Set NVSHMEM symmetric heap size to 8GB to avoid NVLS multicast allocation
  // failures on H100 systems. This must be set before nvshmem init.
  auto symm_size_env = c10::utils::get_env("NVSHMEM_SYMMETRIC_SIZE");
  if (!symm_size_env.has_value()) {
    // Default to 8GB if not set
    c10::utils::set_env("NVSHMEM_SYMMETRIC_SIZE", "8589934592");
  }
