...
// NCCL uses Non-negative int to represent in-group according to API requirement.
// We take a list of ranks and generate a hash value based on the list and ensure
// its range of 32-bit int.
int genNcclSplitColor(const std::vector<int>& ranks) {
    ...
    std::size_t combined_hash = std::accumulate(
        ranks.begin(), ranks.end(), std::size_t(0),
