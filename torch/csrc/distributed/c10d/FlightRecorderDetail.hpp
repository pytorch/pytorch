...
std::string FlightRecorder<EventType>::dump_json(
  ...
  result[nccl_version_key_str] = ccl_version_; // Keep for BC
  result[ccl_version_key_str] = ccl_version_;
...
std::string FlightRecorder<EventType>::dump(
  ...
  result.insert(nccl_version_key_str, ccl_version_);
  result.insert(ccl_version_key_str, ccl_version_);
