...
typename FlightRecorder<EventType>::TraceIdentifier FlightRecorder<EventType>::record(
  ...
  // Initialize the hash map entry for the current epoch if it doesn't exist
  if (reset_epoch_start_idx_.find(reset_epoch_) == reset_epoch_start_idx_.end()) {
    reset_epoch_start_idx_[reset_epoch_] = 0;
  }
...
size_t FlightRecorder<EventType>::getIdxFromId(size_t id, size_t reset_epoch) const {
  ...
  if (it == reset_epoch_start_idx_.end()) {
    // If the reset epoch is not found, the entry may have been overwritten
    // Return a best-guess idx, caller should verify the entry matches
    return id % max_entries_;
  }
