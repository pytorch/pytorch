...
#pragma unroll
  ...
  size_t offset = (blockDim.x * blockIdx.x + threadIdx.x) * numelPerThread;
  size_t stride = blockDim.x * gridDim.x * numelPerThread;
...
at::Tensor oneShotAllReduce(
  ...
  // narrowed later.
  size_t N = divUp(input.numel(), numelPerWarp) * numelPerWarp;
  TORCH_CHECK(N % numelPerWarp == 0);
  TORCH_CHECK(N <= kMaxIntraNodeSize * 4);
  ...
  if (N < numelPerThread * kThreadsPerBlock) {
    threads.x = divUp(N, numelPerWarp);
    blocks.x = 1;
  } else {
    auto warpsRequired = divUp(N, numelPerWarp);
    auto threadsRequired = divUp(N, numelPerThread);
    blocks.x = std::min<uint32_t>(divUp(threadsRequired, kThreadsPerBlock), kMaxAllReduceBlocks);
    auto warpsPerBlock = divUp(warpsRequired, blocks.x);
    threads.x = std::min<uint32_t>(kThreadsPerBlock, warpsPerBlock * kWarpSize);
  }
...
#define X(kWorldSize)                                               \
  ...
  X(8);
#undef X
  return output.narrow(0, 0, input.numel()).reshape(input.sizes());
}
