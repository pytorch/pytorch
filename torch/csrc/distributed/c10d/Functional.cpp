...
namespace {
// DTensor related comm operations, sharing code with functional collective for now
at::Tensor shard_dim_alltoall(const at::Tensor& input, int64_t gather_dim, int64_t shard_dim, std::string group_name) {
  auto group = c10d::resolve_process_group(group_name);
  ...
  if (output_sizes[shard_dim] % group_size != 0) {
    LOG(WARNING) << "The first dimension of the shard_dim_alltoall input ("
                 << output_sizes[shard_dim] << ") is not divisible by the group size ("
                 << group_size << ").";
  }
  ...
  work->wait();
  // TODO: it's very tricky to get the current async behavior work for shard dim alltoall
  // so for now we just keep this comm op to be synchronous.
