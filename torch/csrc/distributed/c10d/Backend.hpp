...
typedef std::tuple<
    std::string /* group_name specified by user, by default empty*/,
    std::string /* group_uid: a global unique id generated by c10d */>
    ProcessGroupName;
...
class TORCH_API Backend : public torch::CustomClassHolder {
 public:
  ...
  const ProcessGroupName& getProcessGroupNameTuple() const {
    // Get the PG name tuple <group_name, group_uid>
    return pg_name_;
  }

  const std::string& getGroupName() const {
    // Get PG group_name defined by user
    return std::get<0>(pg_name_);
  }

  const std::string& getGroupUid() const {
    // Get PG unique ID generated by c10d
    return std::get<1>(pg_name_);
  }
