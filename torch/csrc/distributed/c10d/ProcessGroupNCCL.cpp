...
c10::intrusive_ptr<Work> ProcessGroupNCCL::collective(
...

  {
    ...
    for (const auto i : c10::irange(inputs.size())) {
      ...
      // See [Sync Streams].
      if (!avoidRecordStreams_) {
        // TODO: fix for sparse tensors
        // C++ exception with description "Cannot access storage of
        // SparseTensorImpl
...
c10::intrusive_ptr<Work> ProcessGroupNCCL::allreduce_sparse(
  ...
  // TODO: we are allocating new memory for output tensors
      ...
      [&](std::vector<at::cuda::CUDAStream>& ncclStreams,
        ...
        for (const auto i : c10::irange(outputTensors.size())) {
          ...
          if (opts.sparseIndices.has_value()) {
            tensors[i] = at::sparse_coo_tensor(opts.sparseIndices.value(), outputTensors[i], tensors[i].sizes());
          } else {
            tensors[i] = outputTensors[i].to_sparse();
            // int num_rows = tensors[i].numel() / tensors[i].sizes()[1];
            // tensors[i] = at::sparse_coo_tensor(tensors[i].indices(), outputTensors[i].reshape({num_rows, -1}), tensors[i].sizes());
          }
          LOG(INFO) << tensors[i];
        }
      },
