...
// in which case we don't want to spam the logs.
    ...
    for (auto it = pg_->workMetaList_.begin(); it != pg_->workMetaList_.end();
      ...
      if (work.isCompleted()) { 
        ...
        if (enableTiming_ && logger) {
          ::c10d::C10dLoggingData data;
          // logging integers
          data.strings["collective_duration"] = std::to_string(work.getDuration());
          ...
          data.integers["pg_rank"] = rank_;
          data.integers["world_size"] = size_;
          data.strings["comm_backend"] = "nccl";
          data.strings["pg_decomm_backend_version"] = getNcclVersion();
