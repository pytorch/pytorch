...
class PythonStore : public ::c10d::Store {
 public:
  ...
  virtual std::vector<std::vector<uint8_t>> multiGet(const std::vector<std::string>& keys)
    ...
    std::vector<std::string> py_list = pybind11::cast<std::vector<std::string>>(fn(keys));
  ...
  virtual void multiSet(const std::vector<std::string>& keys, const std::vector<std::vector<uint8_t>>& values)
      override {
    ...
    for(auto& value : values){
        bytes.emplace_back(py::bytes(reinterpret_cast<const char*>(value.data()), value.size()));
    }
  ...
  bool hasExtendedApi() const override {
    PYBIND11_OVERLOAD_NAME(bool, ::c10d::Store, "has_extended_api", hasExtendedApi);
  }
};
    ...
    >>> store.get("first_key")
         ...
         .def(
              ...
              [](::c10d::Store& store,
                ...
                for(auto &value : values) {
                    auto bytes = py::bytes(reinterpret_cast<const char*>(value.data()), value.size());
                    res.push_back(bytes);
                }
                return res;
              },
    ...
    >>> store.multi_get(["first_key", "second_key"])
         ...
         .def(
              ...
              [](::c10d::Store& store,
                ...
                for(auto& value : values) {
                    vals.push_back(std::vector<uint8_t>(value.begin(), value.end()));
                }
