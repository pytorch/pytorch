...
static PyObject* NodeList_iter(PyObject* self) {
  NodeList* nl = reinterpret_cast<NodeList*>(self);
  // Get _NodeIter type and _root from graph
  ...
  PyObject* node_iter_type = PyObject_GetAttrString(torch_c.get(), "_NodeIter");
  if (!node_iter_type) {
    Py_DECREF(root);
    return nullptr;
  }
  ...
  Py_DECREF(root);
  if (!args) {
    Py_DECREF(node_iter_type);
    return nullptr;
  }
  PyObject* result = PyObject_Call(node_iter_type, args.get(), nullptr);
  Py_DECREF(node_iter_type);
  return result;
}
...
static PyObject* NodeList_reversed(PyObject* self, PyObject* Py_UNUSED(args)) {
  ...
  PyObject* node_list_type =
      PyObject_GetAttrString(torch_c.get(), "_node_list");
  if (!node_list_type) {
    return nullptr;
  }

  // Create reversed node_list
  const char* new_direction = nl->reversed ? "_next" : "_prev";
  THPObjectPtr args(Py_BuildValue("(Os)", nl->graph, new_direction));
  if (!args) {
    Py_DECREF(node_list_type);
    return nullptr;
  }
  PyObject* result = PyObject_Call(node_list_type, args.get(), nullptr);
  Py_DECREF(node_list_type);
  return result;
}
