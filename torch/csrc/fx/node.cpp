...
static PyObject* import_immutable_collection(const char* name) {
  THPObjectPtr module(PyImport_ImportModule("torch.fx.immutable_collections"));
  if (!module) {
...
template <typename F>
static PyObject* map_aggregate(PyObject* a, F fn) {
  ...
  // Case 2: a is a list.
    ...
    if (!immutable_list_cls) {
      immutable_list_cls = import_immutable_collection("immutable_list");
    }
    Py_ssize_t n = PyList_GET_SIZE(a);
    if (n == 0 &&
        reinterpret_cast<PyObject*>(Py_TYPE(a)) == immutable_list_cls) {
      Py_INCREF(a);
      return a;
    }
  ...
  // Case 3: a is a dict.
    ...
    if (!immutable_dict_cls) {
      immutable_dict_cls = import_immutable_collection("immutable_dict");
    }
    if (reinterpret_cast<PyObject*>(Py_TYPE(a)) == immutable_dict_cls &&
        PyDict_GET_SIZE(a) == 0) {
      Py_INCREF(a);
      return a;
    }
...
static PyObject* py_map_arg(
  ...
  try {
    ...
    return map_aggregate(args[0], [fn](PyObject* a) {
      if (PyObject_TypeCheck(a, &NodeBaseType)) {
...
static PyMethodDef extra_methods[] = {
    {"_fx_map_aggregate",
     reinterpret_cast<PyCFunction>(py_map_aggregate),
     METH_FASTCALL,
     "Recursively apply a function to every element in an aggregate object."},
    {"_fx_map_arg",
     reinterpret_cast<PyCFunction>(py_map_arg),
     METH_FASTCALL,
