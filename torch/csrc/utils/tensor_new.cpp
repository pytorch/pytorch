...
Tensor dispatch_zeros(const Type& type, ScalarType scalar_type, optional<Device> device, IntArrayRef sizes) {
  maybe_initialize_cuda(type);
  AutoNoGIL no_gil;
  return torch::zeros(sizes, type.options(scalar_type, std::move(device)));
}

Tensor dispatch_ones(const Type& type, ScalarType scalar_type, optional<Device> device, IntArrayRef sizes) {
  maybe_initialize_cuda(type);
  AutoNoGIL no_gil;
  return torch::ones(sizes, type.options(scalar_type, std::move(device)));
}

Tensor dispatch_full(const Type& type, ScalarType scalar_type, Scalar fill_value, optional<Device> device, IntArrayRef sizes) {
  maybe_initialize_cuda(type);
  AutoNoGIL no_gil;
  return torch::full(sizes, fill_value, type.options(scalar_type, std::move(device)));
}

Tensor new_with_sizes(const Type& type, ScalarType scalar_type, optional<Device> device, IntArrayRef sizes) {
  maybe_initialize_cuda(type);
  AutoNoGIL no_gil;
  return torch::empty(sizes, type.options(scalar_type, std::move(device)));
}

Tensor new_with_storage(const Type& type, ScalarType scalar_type, Storage storage) {
...
Tensor sparse_coo_tensor_ctor(const Type& default_type, ScalarType scalar_type, PyObject* args, PyObject* kwargs) {
  ...
  if (r.idx == 0) {
    ...
    Tensor values = internal_new_from_data(values_type, scalar_type, r.deviceOptional(3), r.pyobject(1), false, true, type_inference);
    const auto& indices_type = values.type().toScalarType(kLong);
    Tensor indices = internal_new_from_data(indices_type, scalar_type, r.deviceOptional(3), r.pyobject(0), false, true, false);
    ...
    Tensor values = internal_new_from_data(values_type, scalar_type, r.deviceOptional(4), r.pyobject(1), false, true, type_inference);
    const auto& indices_type = values.type().toScalarType(kLong);
    Tensor indices = internal_new_from_data(indices_type, scalar_type, r.deviceOptional(4), r.pyobject(0), false, true, false);
    ...
    return at::sparse_coo_tensor(r.intlist(0), type.options(scalar_type).layout(at::kSparse)).set_requires_grad(r.toBool(3));
  }
...
Tensor as_tensor(const Type& type, ScalarType scalar_type, PyObject* args, PyObject* kwargs) {
  ...
  if (r.idx == 0) {
    ...
    return internal_new_from_data(
        typeWithDefault(r, 1, 2, type, scalar_type), scalar_type, r.deviceOptional(2), r.pyobject(0), false, false, type_inference);
  }
  throw std::runtime_error("tensor(): invalid arguments");
}
...
Tensor new_empty(const Type& type, ScalarType scalar_type, PyObject* args, PyObject* kwargs) {
  ...
  if (r.idx == 0) {
    ...
    return new_with_sizes(actual_type, actual_scalar_type, r.deviceOptional(2), r.intlist(0)).set_requires_grad(r.toBool(3));
  }
