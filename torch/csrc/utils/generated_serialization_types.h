...
// checksum<<0ea757232b33f83b309cbf6e07f204f7c3eccb304fc7aa67aee766a940be2ecd>>
// clang-format off
...
class InputToSymbolicAttrSpec {
 private:
  SymIntArgument arg;
  std::string symbolic_attr_name;

 public:

  const SymIntArgument& get_arg() const {
    return arg;
  }

  void set_arg(SymIntArgument def) {
    arg = std::move(def);
  }

  const std::string& get_symbolic_attr_name() const {
    return symbolic_attr_name;
  }

  void set_symbolic_attr_name(std::string def) {
    symbolic_attr_name = std::move(def);
  }

  friend void to_json(nlohmann::json& nlohmann_json_j, const InputToSymbolicAttrSpec& nlohmann_json_t);
  friend void from_json(const nlohmann::json& nlohmann_json_j, InputToSymbolicAttrSpec& nlohmann_json_t);
};
...
class InputSpec {
 ...
 public:
  enum class Tag {
    USER_INPUT, PARAMETER, BUFFER, TENSOR_CONSTANT, CUSTOM_OBJ, TOKEN, CONSTANT_INPUT, SYMBOLIC_ATTR
  };

 private:
  std::variant<Void, UserInputSpec, InputToParameterSpec, InputToBufferSpec, InputToTensorConstantSpec, InputToCustomObjSpec, InputTokenSpec, InputToConstantInputSpec, InputToSymbolicAttrSpec> variant_;
  Tag tag_;
 ...
 public:
  ...
  const InputToSymbolicAttrSpec& get_symbolic_attr() const {
    return std::get<8>(variant_);
  }

  void set_symbolic_attr(InputToSymbolicAttrSpec def) {
    variant_.emplace<8>(std::move(def));
    tag_ = Tag::SYMBOLIC_ATTR;
  }
  ...
  friend void to_json(nlohmann::json& nlohmann_json_j, const InputSpec& nlohmann_json_t) {
    ...
    if (nlohmann_json_t.tag_ == Tag::SYMBOLIC_ATTR) {
      nlohmann_json_j["symbolic_attr"] = nlohmann_json_t.get_symbolic_attr();
      return;
    }
  }
  ...
  friend void from_json(const nlohmann::json& nlohmann_json_j, InputSpec& nlohmann_json_t) {
    ...
    if (nlohmann_json_j.contains("symbolic_attr")) {
      nlohmann_json_t.variant_.emplace<8>(nlohmann_json_j.at("symbolic_attr").template get<InputToSymbolicAttrSpec>());
      nlohmann_json_t.tag_ = Tag::SYMBOLIC_ATTR;
      return;
    }
  }
};
...
inline std::string_view printEnum(const InputSpec::Tag& e) {
  switch (e) {
    ...
    case InputSpec::Tag::SYMBOLIC_ATTR: return "SYMBOLIC_ATTR";
    default:
      throw std::runtime_error("Unknown enum value");
  }
}
...
inline void parseEnum(std::string_view s, InputSpec::Tag& t) {
  ...
  if (s == "SYMBOLIC_ATTR") { t = InputSpec::Tag::SYMBOLIC_ATTR; return; }
  throw std::runtime_error("Unknown enum value: " + std::string{s});
}
...
inline void to_json(nlohmann::json& nlohmann_json_j, const InputToSymbolicAttrSpec& nlohmann_json_t) {
  nlohmann_json_j["arg"] = nlohmann_json_t.arg;
  nlohmann_json_j["symbolic_attr_name"] = nlohmann_json_t.symbolic_attr_name;
}

inline void from_json(const nlohmann::json& nlohmann_json_j, InputToSymbolicAttrSpec& nlohmann_json_t) {
  InputToSymbolicAttrSpec nlohmann_json_default_obj;
  nlohmann_json_t.arg = nlohmann_json_j.value("arg", nlohmann_json_default_obj.arg);
  nlohmann_json_t.symbolic_attr_name = nlohmann_json_j.value("symbolic_attr_name", nlohmann_json_default_obj.symbolic_attr_name);
}
