...
// checksum<<60c038c990f7ccde2bafc12ccf0f4aa027e170dec255df8bbd8b68898c6cdbbb>>
// clang-format off
...
class Model {
 private:
  std::string name;
  std::unordered_map<std::string, ExportedProgram> programs;
 ...
 public:
  ...
  const std::unordered_map<std::string, ExportedProgram>& get_programs() const {
    return programs;
  }

  void set_programs(std::unordered_map<std::string, ExportedProgram> def) {
    programs = std::move(def);
  }
...
inline void to_json(nlohmann::json& nlohmann_json_j, const Model& nlohmann_json_t) {
  nlohmann_json_j["name"] = nlohmann_json_t.name;
  nlohmann_json_j["programs"] = nlohmann_json_t.programs;
}

inline void from_json(const nlohmann::json& nlohmann_json_j, Model& nlohmann_json_t) {
  Model nlohmann_json_default_obj;
  nlohmann_json_t.name = nlohmann_json_j.value("name", nlohmann_json_default_obj.name);
  nlohmann_json_t.programs = nlohmann_json_j.value("programs", nlohmann_json_default_obj.programs);
}
