...
// checksum<<299cb63e37accb6395a485e87a0a56a0876d5b1706641ebf51eed409aca2ec06>>
// clang-format off
...
inline void parseEnum(std::string_view s, ScalarType& t) {
  ...
  if (s == "UINT32") { t = ScalarType::UINT32; return; }
  if (s == "UINT64") { t = ScalarType::UINT64; return; }
  throw std::runtime_error("Unknown enum value: " + std::string{s});
}
...
class Argument {
 ...
 public:
  ...
  const std::vector<ForwardRef<Argument>>& get_as_tuple() const {
    return std::get<32>(variant_);
  }

  template <typename U>
  void set_as_tuple(U&& def) {
    variant_.emplace<32>(std::forward<U>(def));
    tag_ = Tag::AS_TUPLE;
  }
  ...
  friend void to_json(nlohmann::json& nlohmann_json_j, const Argument& nlohmann_json_t) {
    ...
    if (nlohmann_json_t.tag_ == Tag::AS_TUPLE) {
      nlohmann_json_j["as_tuple"] = nlohmann_json_t.get_as_tuple();
      return;
    }
  }
  ...
  friend void from_json(const nlohmann::json& nlohmann_json_j, Argument& nlohmann_json_t) {
    ...
    if (nlohmann_json_j.contains("as_tuple")) {
      nlohmann_json_t.variant_.emplace<32>(nlohmann_json_j.at("as_tuple").template get<std::vector<ForwardRef<Argument>>>());
      nlohmann_json_t.tag_ = Tag::AS_TUPLE;
      return;
    }
  }
};
...
inline std::string_view printEnum(const Argument::Tag& e) {
  switch (e) {
    ...
    case Argument::Tag::AS_TUPLE: return "AS_TUPLE";
    default:
      throw std::runtime_error("Unknown enum value");
  }
}
...
inline void parseEnum(std::string_view s, Argument::Tag& t) {
  ...
  if (s == "AS_TUPLE") { t = Argument::Tag::AS_TUPLE; return; }
  throw std::runtime_error("Unknown enum value: " + std::string{s});
}
