...
static std::unordered_map<std::string, ParameterType> type_map = {
  ...
  {"IntListRef", ParameterType::INT_LIST},
...
bool FunctionParameter::check(PyObject* obj) {
  switch (type_) {
    ...
    case ParameterType::INT_LIST: {
      ...
      // if a size is specified (e.g. IntListRef[2]) we also allow passing a single int
...
/*
Parse default value of IntListRef declared at native_functions.yaml

There are two kinds of default values:
1. IntListRef[2] x=1 (where size=2, value={1,1}
2. IntListRef x={1,2,3} (where size=3, value={1,2,3}, note that there cannot be space after comma since native_parse.py uses ', ' to split args)
*/
static inline std::vector<int64_t> parse_intlist_args(const std::string& s, int64_t size) {
  ...
  AT_CHECK(s[n - 1] == '}', "Default value of IntListRef is missing right brace '}', found ", s[n - 1]);
...
bool FunctionSignature::parse(PyObject* args, PyObject* kwargs, PyObject* dst[],
  ...
  // if there is a single positional IntListRef argument, i.e. expand(..), view(...),
  // allow a var-args style IntListRef, so expand(5,3) behaves as expand((5,3))
  ...
  for (auto& param : params) {
    ...
    bool is_kwd = false;
    if (arg_pos < nargs) {
      // extra positional args given after single positional IntListRef arg
      if (param.keyword_only) {
        if (raise_exception) {
          extra_args(*this, nargs);
        }
