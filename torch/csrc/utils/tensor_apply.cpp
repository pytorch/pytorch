...
const Tensor& apply_(const Tensor& self, PyObject* fn) {
  ...
  if (!self.device().is_cpu()) {
    C10_THROW_ERROR(TypeError, "apply_ is only implemented on CPU tensors");
  }
...
const Tensor& map_(const Tensor& self, const Tensor& other_, PyObject* fn) {
  if (!other_.options().type_equal(self.options())) {
    C10_THROW_ERROR(
        TypeError,
        c10::str("map_: expected ", self.toString(), " for 'other' (got ", other_.toString(), ")"));
  }
  if (self.is_meta()) {
    return self; // Just skip
  }
  if (!self.device().is_cpu()) {
    C10_THROW_ERROR(TypeError, "map_ is only implemented on CPU tensors");
  }
...
const Tensor& map2_(
  ...
  if (!self.device().is_cpu() || !x_.device().is_cpu() ||
      !y_.device().is_cpu()) {
    C10_THROW_ERROR(TypeError, "map2_ is only implemented on CPU tensors");
  }
