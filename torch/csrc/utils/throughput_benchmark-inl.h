...
BenchmarkExecutionStats BenchmarkHelper<Input, Output, Model>::benchmark(
  ...
  for (auto thread_id = 0; thread_id < config.num_calling_threads;
    ...
    callers.emplace_back([&, thread_id]() {
      ...
      }
      {
        ...
        ++initialized;
        LOG(INFO) << "Initialized thread " << thread_id
                  << ". Total number of initialized threads: " << initialized;
        worker_main_cv.notify_one();
        while (!start) {
          LOG(INFO) << "Not starting this thread yet";
...

  {
    ...
    while (initialized != config.num_calling_threads) {
      LOG(INFO) << "Not launching start yet as initialized = " << initialized
                << " while total number is " << config.num_calling_threads;
