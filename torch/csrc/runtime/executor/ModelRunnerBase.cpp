...
#include "torch/csrc/runtime/executor/ModelRunnerBase.h"
...
#include "torch/csrc/runtime/graph/TensorMeta.h"
...
c10::IValue ModelRunnerBase::run(
    ...
    const std::unordered_map<std::string, c10::IValue>& kwargs,
    const RunConfigs& runConfigs) {
  RecordFunction recordFunction("runtime::ModelRunner::run");
...
std::vector<c10::IValue> ModelRunnerBase::runWithFlatInputsAndOutputs(
  ...
  RecordFunction recordFunction(
      "runtime::ModelRunner::runWithFlatInputsAndOutputs");
...
void ModelRunnerBase::initializeExecutor(
  ...
  if (executorType_ == ExecutorType::INTERPRETER) {
    executor_ = std::make_unique<Executor>(config, graph_, weights, placement_);
