...
Weights::Weights(
  ...
  auto loadAndInsertParamsBuffers = [&](const std::string& tensorName) {
    return loadAndInsert(tensorName, stateDictPathPrefix, stateDictPaths);
  };

  for (const auto& tensorName : graph->signature().parameters()) {
    loadAndInsertParamsBuffers(tensorName);
  }
  ...
  auto loadConstants = [&](const std::vector<std::string>& constants) {
    for (const auto& constantName : constants) {
      ...
      if (starts_with(
        ...
        // tensor constants
        loadAndInsert(constantName, constantPathPrefix, constantPaths);
      } else {
