...
class TORCH_API ModelRunnerBase {
 public:
  ModelRunnerBase(
      ...
      // loading the weights.
      const std::function<Placement(const torch::runtime::Graph& graph)>&
          buildPlacementFn);
...
template <>
struct fmt::formatter<torch::runtime::ExecutorType> {
  ...
  auto format(torch::runtime::ExecutorType et, FormatContext& ctx) const {
    using namespace torch::runtime;
    switch (et) {
