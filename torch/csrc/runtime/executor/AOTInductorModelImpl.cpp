#include "torch/csrc/runtime/executor/AOTInductorModelImpl.h" // @manual
...
#include "torch/csrc/runtime/common/FileUtil.h"
...
inline bool fileExists(const std::string& name) {
  const auto fd =
      torch::runtime::openNoInt(name.c_str(), O_RDONLY | O_CLOEXEC);
  if (fd == -1) {
    return false;
  }
  torch::runtime::closeNoInt(fd);
  return true;
}
...
std::unique_ptr<ProxyExecutor> makeProxyExecutor(
    ...
    bool is_cpu) {
#ifdef FBCODE_CAFFE2
  return std::make_unique<FbProxyExecutor>(filename, is_cpu);
#else
...
AOTInductorModelImpl::AOTInductorModelImpl(
  ...
  if (extern_kernel_nodes_path.has_value()) {
    ...
    if (fileExists(filename)) {
      ...
      proxyExecutor_ = makeProxyExecutor(filename, is_cpu());
    }
  }
