...
c10::IValue treeUnflatten(
    std::vector<c10::IValue> leaves,
    const TreeSpec& spec) {
  RecordFunction recordFunction("runtime::treeUnflatten");
...
std::vector<c10::IValue> treeFlattenFromArgs(
    ...
    const TreeSpec& spec) {
  RecordFunction recordFunction("runtime::treeFlattenFromArgs");
...
void leafApplyFromArgs(
    ...
    const std::unordered_map<std::string, c10::IValue>& kwargs,
    const TreeSpec& spec) {
  RecordFunction recordFunction("runtime::leafApplyFromArgs");
