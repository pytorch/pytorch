        ...
        dl_iterate_phdr([](struct dl_phdr_info *info,
            ...
            for (auto i : c10::irange(info->dlpi_phnum)) {
                ...
                if (segments[i].p_type == PT_GNU_EH_FRAME) {
                    ...
                    auto eh_frame_hdr = (void*)(segments[i].p_vaddr + info->dlpi_addr);
                    self->all_libraries_.emplace_back(std::move(library_name), info->dlpi_addr, last_addr, eh_frame_hdr);
                    return 0;
                }
            }
            throw std::runtime_error("did not find PT_GNU_EH_FRAME segment?");
        }, this);
        std::sort(all_libraries_.begin(), all_libraries_.end(), [](const LibraryInfo& lhs, const LibraryInfo& rhs)  {
...
extern "C"
void unwind_c(std::vector<void*>* result, int64_t rsp, int64_t rbp) {
    UnwindState state;
...
extern "C"
void unwind_entry(std::vector<void*>* result);
...
std::vector<Frame> symbolize(const std::vector<void*>& frames) {
    ...
    auto get_or_create = [&](const LibraryInfo& info) -> Entry& {
        ...
        const char* args[] =  {"addr2line", "-C", "-f", "-e", info.name().c_str(), nullptr};
        entries.emplace_back(Entry {&info,std::make_unique<Communicate>("addr2line", args), {}});
        return entries.back();
    };
    for (auto f : frames) {
        if (f == nullptr) {
