...
struct Unwinder {
    ...
    : kind_(rip.kind == A_UNDEFINED ? END : STANDARD),
      ...
      off_(rsp.data),
      rip_off_(rip.data),
      rbp_off_(rbp.kind == A_UNDEFINED ? std::numeric_limits<int64_t>::max() : rbp.data) {
        bool standard = rsp.kind == A_REG_PLUS_DATA && (rip.kind == A_UNDEFINED || rip.kind == A_LOAD_CFA_OFFSET) && (rbp.kind == A_LOAD_CFA_OFFSET || rbp.kind == A_UNDEFINED);
        if (!standard) {
    ...
    UnwindState run(const UnwindState& cur) const {
        ...
        r.rbp = rbp_off_ == std::numeric_limits<int64_t>::max() ? cur.rbp : *(int64_t*)(r.rsp + rbp_off_);
        return r;
    }
private:
    Unwinder()
    : kind_(UNKNOWN), reg_(0), off_(0), rip_off_(0), rbp_off_(0) {}
    // map from register numbers in info to UnwindState register array
    static int32_t reg2us(int32_t reg) {
        switch(reg) {
            case D_RBP: return US_RBP;
            case D_RSP: return US_RSP;
            case D_RIP: return US_RIP;
            // unused register
            case US_UNDEF: return US_UNDEF;
            default:
