...
class TORCH_API CuptiMonitorDecoder {
 public:
  ...
  //
  // flush_fn is the address of cuptiActivityFlushAll; when nonzero the decode
  // thread also drives the periodic plain flush itself (every flush_period_ns,
  // called as fn(0) -- completed buffers only), so no separate flush thread is
  // needed. flush_fn == 0 disables it (the caller drives flush()).
  void configure(
 ...
 private:
  ...
  // Address of cuptiActivityFlushAll (0 = the decode thread does not
  // self-flush) and the plain-flush cadence the worker drives.
