...
template <typename T>
py::class_<T, std::shared_ptr<T>> registerStore(py::module& m, const char* name) {
  return py::class_<T, std::shared_ptr<T>>(m, name)
    // Convert from std::string to std::vector<uint8>.
    .def("set", [] (T& store, const std::string& key, const std::string& value) {
    ...
    .def("get", [] (T& store, const std::string& key) -> py::bytes {
        auto value = store.get(key);
        return py::bytes(reinterpret_cast<char*>(value.data()), value.size());
      }, py::call_guard<py::gil_scoped_release>())
