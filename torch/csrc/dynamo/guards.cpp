...
/**
 * Base class representing how to access the child value from the py::object
 * given to the parent check function.
 *
 * These will have one to one mapping with Source objects in Dynamo. Source
 * provides a way to access the underlying object using its name() function.
 * GuardAccessor is basically its C++ tree equivalent.
 */
class GuardAccessor {
 public:
...
class AttrGuardAccessor : public GuardAccessor {
  ...
  class GuardManager;
  typedef std::
      pair<std::unique_ptr<GuardAccessor>, std::unique_ptr<GuardManager>>
          ChildGuardType;
   ...
   * 2) Child guards - Guards that run on the values accessed by Accessors on
   * the py::object.
   *
   ...
   * functino is called, parent GuardManager calls getattr(value, "x") on its
   * value passed to the check function to call the check function of the child
   * guard manager.
   *
   * Sorting the guards based on failure count - An optimization for runtime
   * here is to reshuffle the order of child guards depending on the failure
   * count. This ensures that we run the guards that are more prone to fail
   * statistically first. This can improve the cache lookup time when we have
   ...
   public:
    ...
    GuardManager* get_child_manager(py::object accessor_key) {
      ...
      for (auto& child_accessor_and_manager : _child_managers) {
        ...
        auto maybe_attr_accessor =
            dynamic_cast<GuardAccessorT*>(child_accessor);
    ...
    std::pair<bool, GuardDebugInfo> run_guards(py::object value) {
      ...
      if (result == false) {
        ...
        // for the next check_with_debug_info.
