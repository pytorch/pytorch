...
bool TensorCheck::check(
  ...
  // Mask to exclude Python keys, that can be artificially off during
  ...
  auto stored_keys =
      c10::DispatchKeySet::from_raw_repr(dispatch_key_) - mode_key_mask;
  auto current_keys = state.apply(dispatch_key_set) - mode_key_mask;

  if (stored_keys.raw_repr() != current_keys.raw_repr() || dtype_ != dtype ||
      device_index_ != device.index() || requires_grad_ != requires_grad) {
    return false;
  }
