...
#include "torch/csrc/nativert/passes/SubgraphRewriter.h"
...
SubgraphMatcher::SubgraphMatcher(const Graph* pattern) : pattern_(pattern) {
  pattern_root_ = findRootNode(pattern_);
}
...
std::optional<Match> SubgraphMatcher::match(Node* target_node) {
  ...
  if (tryMatchNode(pattern_root_, target_node, current_match)) {
    for (const Value* output : pattern_->outputs()) {
      CHECK(current_match.value_map.count(output))
...
auto findAttributeByName(const Node* target_node, const std::string& attrName) {
  return std::find_if(
      target_node->attributes().begin(),
      target_node->attributes().end(),
      [&](const Attribute& targetAttr) { return attrName == targetAttr.name; });
}
...
bool SubgraphMatcher::tryMatchNodeInputs(
  ...
  TORCH_CHECK_LE(target_node->numInputs(), pattern_node->numInputs());
  TORCH_CHECK_LE(pattern_node->attributes().size(), target_node->numInputs());
  ...
  for (const auto i : c10::irange(target_node->attributes().size())) {
    ...
    Value* dummyOutput =
        dummyNode->addOutput(targetGraph->getUniqueValueName(), Type::None);
    targetGraph->insertBefore(dummyNode, target_node);
    if (match.value_map.count(patternInput->value)) {
...
bool SubgraphMatcher::tryMatchNode(
    ...
    Node* target_node,
    Match& match) {
  if (match.node_map.count(pattern_node)) {
  ...
  auto deltaInputCount = pattern_node->numInputs() - target_node->numInputs();
  auto deltaAttributesCount =
      pattern_node->attributes().size() - target_node->attributes().size();
...
bool SubgraphRewriter::overlapsWithUsedNodes(
  ...
  for (auto& kv : match.node_map) {
    ...
    if (usedNodes.count(target_node)) {
      return true;
    }
  }
  return false;
...
void SubgraphRewriter::rewriteMatch(
  ...
  TORCH_CHECK_EQ(outputs.size(), newOutputs.size());
  ...
  for (auto& patternNode : pattern.nodes()) {
    if (match.node_map.count(&patternNode)) {
