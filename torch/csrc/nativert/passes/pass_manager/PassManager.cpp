...
#include <torch/csrc/nativert/passes/pass_manager/PassManager.h>
...
GraphPassManager::GraphPassManager(
    ...
    : pipeline_(std::move(pipeline)), opts_(std::move(opts)) {
  if (static bool registered = false; !std::exchange(registered, true)) {
    register_base_passes();
  }
}
