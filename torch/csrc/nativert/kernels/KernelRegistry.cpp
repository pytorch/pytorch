...
#include <torch/csrc/jit/runtime/static/ops.h> // @manual=//caffe2:torch-cpp-cpu
  ...
  void computeInternal(ExecutionFrame& executionFrame) const override final {
    {
      ...
      if (KernelInput(numCatInps - 1).isInt()) {
        dim = KernelInput(numCatInps).toInt();
        numCatInps--;
      }
...
class OpKernel_aten__to_copy : public C10Kernel {
 public:
  explicit OpKernel_aten__to_copy(const Node* node, c10::Device device)
    ...
    kind_ = Kind::kStaticDispatchKernel;
    ...
    if (memory_format_.has_value()) {
      TORCH_CHECK(
          ...
          "Static Kernel for aten._to_copy doesn't correctly handle the ChannelsLast(3d) memory format. If you are running into this error, please report to Sigmoid oncall.");
    }
