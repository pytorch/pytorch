...
c10::intrusive_ptr<c10::TensorImpl> ConcretePyInterpreterVTable::to_meta_tensor(
  ...
  at::Tensor fake_tensor;
  {
    ...
    // fake fallback (the meta factory ops it runs are otherwise plain). Skip the
    // exclusion when re-fakeifying an already-fake input (e.g. adopting a fake
    // from another mode): with Fake excluded, device_custom() reports the fake's
    // meta backing device instead of its fake device, so meta conversion would
    // build the shadow real on the wrong device. The meta factory ops inside
    // from_real_tensor suspend the fake layer on their own.
    std::optional<c10::impl::ExcludeDispatchKeyGuard> exclude_fake;
    if (!real.is_fake()) {
      exclude_fake.emplace(c10::DispatchKeySet(c10::DispatchKey::Fake));
    }
