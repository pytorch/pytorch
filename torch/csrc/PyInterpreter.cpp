...
std::function<py::object(py::object)> make_fake_device_stamp(
  ...
  return [common_device, mode = std::move(mode), skip_fake](
    ...
    at::set_and_normalize_fake_device(
        t.unsafeGetTensorImpl(), common_device);
...
bool ConcretePyInterpreterVTable::fake_is_not_implemented_error(
    const std::exception_ptr& e) const {
  py::gil_scoped_acquire gil;
  try {
    std::rethrow_exception(e);
  } catch (py::error_already_set& err) {
    return err.matches(PyExc_NotImplementedError);
  } catch (python_error& err) {
    err.persist();
    return PyErr_GivenExceptionMatches(err.type, PyExc_NotImplementedError);
  } catch (...) {
    return false;
  }
}
  ...
  at::Tensor tensor(
      c10::intrusive_ptr<c10::TensorImpl, c10::UndefinedTensorImpl>::
          ...
          unsafe_reclaim_from_nonowning(
              const_cast<c10::TensorImpl*>(self)));
