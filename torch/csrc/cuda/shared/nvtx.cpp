...
public:
    ...
    uint64_t getCategory(const std::string &category_name, std::optional <std::string> domain_name) {
        ...
        if (domain_name)  // category local to domain
            nvtxDomainNameCategoryA(getDomain(domain_name.value()), category_offset, category_name.c_str());
...
nvtxEventAttributes_t getAttributes(const std::string &msg, std::optional <std::string> domain,
                                    std::optional <std::string> category, std::optional <uint32_t> color) {
...
int rangePush(const std::string &msg, std::optional <std::string> domain, std::optional <std::string> category,
              std::optional <uint32_t> color) {
    nvtxEventAttributes_t eventAttrib = getAttributes(msg, domain, category, color);

    if (domain)
        return nvtxDomainRangePushEx(registry.getDomain(domain.value()), &eventAttrib);
    else
...
void mark(const std::string &msg, std::optional <std::string> domain, std::optional <std::string> category,
          std::optional <uint32_t> color) {
    nvtxEventAttributes_t eventAttrib = getAttributes(msg, domain, category, color);
...
nvtxRangeId_t rangeStart(const std::string &msg, std::optional <std::string> domain,
                         std::optional <std::string> category, std::optional <uint32_t> color) {
    nvtxEventAttributes_t eventAttrib = getAttributes(msg, domain, category, color);

    if (domain)
        return nvtxDomainRangeStartEx(registry.getDomain(domain.value()), &eventAttrib);
    else
        return nvtxRangeStartEx(&eventAttrib);
}

void rangeEnd(const nvtxRangeId_t &range_id, std::optional <std::string> domain) {
    if (domain)
...
void initNvtxBindings(PyObject *module) {
    ...
    nvtx.def("rangePushA", rangePush, "NVTX range push",
             py::arg("message"),
             py::arg("domain"),
             py::arg("category"),
             py::arg("color"));
    nvtx.def("rangePop", rangePop, "NVTX range pop",
             py::arg("domain"));
    nvtx.def("rangeStartA", rangeStart, "NVTX range start",
             py::arg("message"),
             py::arg("domain"),
             py::arg("category"),
             py::arg("color"));
    nvtx.def("rangeEnd", rangeEnd, "NVTX range end",
             py::arg("id"),
             py::arg("domain"));
    nvtx.def("markA", mark, "NVTX mark",
             py::arg("message"),
             py::arg("domain"),
             py::arg("category"),
             py::arg("color"));
