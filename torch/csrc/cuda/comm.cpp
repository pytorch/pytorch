...
std::vector<Tensor> broadcast(const Tensor& tensor, IntListRef devices) {
...
#else
  {
    ...
    IntListRef loop_devices = type.is_cuda() ? devices.slice(1) : devices;
...
// Variables.
tensor_list2d broadcast_coalesced(TensorList tensors, IntListRef devices, size_t buffer_size) {
  ...
  for (const auto& tensor : tensors) {
    ...
    for (size_t dimension = 0; dimension < expected_size.size(); ++dimension) {
      AT_CHECK(
          ...
          tensor.sizes(), ", but expected ", at::IntListRef(expected_size));
    }
