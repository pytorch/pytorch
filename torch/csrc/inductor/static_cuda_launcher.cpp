...
inline void launchKernelEx(
  ...
  // TODO: support launch_cooperative_grid
  std::array<CUlaunchAttribute, 2> launchAttr = {
      {{.id = CU_LAUNCH_ATTRIBUTE_CLUSTER_DIMENSION,
        .value = {.clusterDim = {clusterDimX, clusterDimY, clusterDimZ}}},
  ...
  CUlaunchConfig config{
      .gridDimX = gridX * clusterDimX,
      .gridDimY = gridY * clusterDimY,
      .gridDimZ = gridZ * clusterDimZ,
      .blockDimX = 32 * numWarps,
      .blockDimY = 1,
      .blockDimZ = 1,
      .sharedMemBytes = sharedMemBytes,
      .hStream = stream,
      .attrs = launchAttr.data(),
      .numAttrs = 2};
  AT_CUDA_DRIVER_CHECK(nvrtc().cuLaunchKernelEx(
      &config, func, args, nullptr /* void** extra */
      ));
}
...
PyObject* launch_kernel(PyObject* self, PyObject* args) {
  ...
  int num_ctas = 1, clusterDimX = 1, clusterDimY = 1, clusterDimZ = 1;
