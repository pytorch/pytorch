...
class AOTInductorModelBase {
 public:
  ...
  std::vector<int64_t> max_input_shape(int64_t idx) const {
    return max_shape<ParamInfo>(inputs_info_, idx);
  }

  std::vector<int64_t> max_output_shape(int64_t idx) const {
    return max_shape<ParamInfo>(outputs_info_, idx);
  }

  std::vector<int64_t> max_constant_shape(int64_t idx) const {
    return max_shape<ConstInfo>(constants_info_, idx);
  }
 ...
 private:
  ...
  std::vector<int64_t> max_shape(const std::vector<InfoT>& params, int64_t idx)
      const {
    std::vector<int64_t> max_shape;
    const InfoT& param = params.at(idx);
