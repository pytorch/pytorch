...
struct OSSOpKernel {
  explicit OSSOpKernel(const std::string& target) : target_(target) {}
...
struct OSSOpKernelOperator : public OSSOpKernel {
  OSSOpKernelOperator(
      ...
      const c10::OperatorHandle& op_handle)
      : OSSOpKernel(target), op_handle_(op_handle) {}
...
struct OSSCallTorchBindKernel : public OSSOpKernel {
  OSSCallTorchBindKernel(
      const std::string& target,
      torch::jit::Function* method)
      : OSSOpKernel(target), method_(method) {}
