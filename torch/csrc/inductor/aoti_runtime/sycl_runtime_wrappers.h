...
static ze_module_handle_t create_module(ze_context_handle_t context,
                                 ze_device_handle_t device,
                                 const uint8_t *binary_ptr, size_t binary_size) {
...
static ze_module_handle_t loadModule(sycl::queue& queue, std::string &spv_path) {
  ...
  return create_module(l0_context, l0_device,
          reinterpret_cast<const uint8_t*>(data.c_str()),
          data.size());
}


static sycl::kernel *getKernel(sycl::queue& queue, ze_module_handle_t l0_module,
                               const char *kernel_name) {
  assert(l0_module);
  assert(kernel_name);
  auto l0_kernel = create_function(l0_module, ZE_KERNEL_FLAG_FORCE_RESIDENCY, kernel_name);
...
static void launchKernel(sycl::kernel& kernel_ptr, uint32_t gridX, uint32_t gridY,
        uint32_t gridZ, int num_warps, int shared_memory,
        std::vector<void*>& params, sycl::queue& stream) {
