...
static int16_t fresh_interpreter_id() {
  static std::atomic<uint16_t> next_id(1);
  auto ret = next_id++;
  TORCH_INTERNAL_ASSERT(
    ret <= INT16_MAX,
    "torchdeploy interpreter ids overflowed.  Are you allocating torch::deploy::Interpreters"
    "in a loop?  Don't do that, or file a bug if you have a legitimate use case."
  );
...
Interpreter::Interpreter(InterpreterManager* manager)
  ...
  pImpl_ = std::unique_ptr<InterpreterImpl>(
      ...
      ((InterpreterImpl * (*)(int16_t)) new_interpreter_impl)(fresh_interpreter_id()));
}
