    ...
    with setup_compilation_env() as backend:
        ...
        if skip_compile:
            if torch.is_grad_enabled():
                _warn_once(
                    warning_id="flex_attention_eager_closure_grads",
                    message=(
                        "flex_attention is running eagerly because a torch dispatch mode is "
                        "active; tensors captured by score_mod will not receive gradients."
                    ),
                )
