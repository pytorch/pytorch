...
def _create_block_mask_from_mask(
    ...
    full_blocks, partial_blocks = _convert_mask_to_block_mask(
        mask, KV_BLOCK_SIZE, Q_BLOCK_SIZE, mask_fn
    )
    ...
    if mask_fn is not None:
        ...
        partial_bm = (
            torch.zeros([1, 1, 1], dtype=torch.int32, device=mask.device),
            torch.zeros([1, 1, 1, 1], dtype=torch.int32, device=mask.device),
            torch.zeros([1, 1, 1], dtype=torch.int32, device=mask.device),
            torch.zeros([1, 1, 1, 1], dtype=torch.int32, device=mask.device),
        )
...
def _create_block_mask(
    ...
    """
    if len(inspect.signature(fn).parameters) + 1 == len(_score_mod_signature.__args__):
        # fn is a score_mod function
        mask_fn = None
        mask_tensor = _create_mask_from_score_mod(fn, B, H, M, N, device)
    else:
        # fn is a mask function, we can use the partial mask optimization.
        mask_fn = fn
        mask_tensor = _create_mask_from_mask_fn(fn, B, H, M, N, device)
    block_mask = _create_block_mask_from_mask(
        mask_tensor, KV_BLOCK_SIZE, Q_BLOCK_SIZE, mask_fn
    )
    return block_mask
