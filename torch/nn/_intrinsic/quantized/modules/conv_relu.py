...
class ConvReLU2d(nnq.Conv2d):
    ...
    def weight(self):
        return torch.ops.quantized.quantized_conv_unpack(self._packed_weight).permute([0, 3, 1, 2])

    def set_weight(self, w):
        self._packed_weight = torch.ops.quantized.quantized_conv_prepack(w.permute([0, 2, 3, 1]),
                                                                      self.stride,
                                                                      self.padding,
                                                                      self.dilation,
                                                                      self.groups)
    ...
    def forward(self, input):
        ...
        output = torch.ops.quantized.quantized_conv2d_relu(input.permute([0, 2, 3, 1]),
