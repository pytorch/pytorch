...
class Conv2dBackward(_ConvTransposeMixin, _ConvNd):
    r"""Computes the gradient of Conv2d with respect to the input of the convolution.
    This is same as the 2D transposed convolution operator under the hood but requires
    the shape of the gradient w.r.t. input ot be specified explicitly.
    ...
    * :attr:`groups` controls the connections between inputs and outputs.
    ...
    Shape:
         Different input shapes can result in the same output shape when stride is greater than 1 during the forward
         pass. With a given gradient w.r.t. output, there is no way of knowing the shape for the gradient w.r.t. input
         from kernel size, stride, padding alone. Hence it is necessary to specify the `output_shape` for backward
         convolution. With the output_shape an output_padding is implicitly calculated as:
         - Input: :math:`(N, C_{in}, H_{in}, W_{in})`
         - Output: :math:`(N, C_{out}, H_{out}, W_{out})` where
    ...
    Args:
        in_channels (int): Number of channels in the gradient w.r.t. the output of the convolution.
        out_channels (int): Number of channels in the gradient w.r.t. the input of the convolution.
        kernel_size (int or tuple): Size of the convolving kernel
        stride (int or tuple, optional): Stride of the convolution. Default: 1
        padding (int or tuple, optional): Zero-padding added to both sides of the input. Default: 0
        groups (int, optional): Number of blocked connections from input channels to output channels. Default: 1
    ...
    Examples::
        ...
        >>> # exact size of the gradient w.r.t. input needs to be specified in argument output_shape
        >>> input = autograd.Variable(torch.randn(1, 16, 12, 12))
        >>> forward_pass = nn.Conv2d(16, 16, 3, stride=2, padding=1)
        >>> h = forward_pass(input)
        >>> h.size()
        torch.Size([1, 16, 6, 6])
        >>> output_grad = autograd.Variable(torch.randn(1, 16, 6, 6))
        >>> grad = nn.Conv2dBackward(16, 16, 3, stride=2, padding=1)
        >>> grad_input = grad(output_grad, output_shape=input.size())
        >>> grad_input.size()
        torch.Size([1, 16, 12, 12])
    ...
    def forward(self, input, output_shape):
        output_padding = self._output_padding(input, output_shape)
        return F.conv_transpose2d(
            input, self.weight, self.bias, self.stride, self.padding,
            output_padding, self.groups, self.dilation)
