    ...
    def convert_sync_batchnorm(cls, module, process_group=None):
        ...
        if isinstance(module, torch.nn.modules.batchnorm._BatchNorm):
            module_output = torch.nn.SyncBatchNorm(module.num_features,
                                                   ...
                                                   process_group)
            if module.affine:
                module_output.weight.data = module.weight.data.clone(memory_format=torch.contiguous_format).detach()
                module_output.bias.data = module.bias.data.clone(memory_format=torch.contiguous_format).detach()
