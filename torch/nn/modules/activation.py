...
class MultiheadAttention(Module):
    ...
    Args:
        ...
        value: Value embeddings of shape :math:`(S, N, E_v)` when ``batch_first=False`` or :math:`(N, S, E_v)` when
            ``batch_first=True``, where :math:`S` is the source sequence length, :math:`N` is the batch size, and
            :math:`E_v` is the value embedding dimension ``vdim``. For unbatched 2D `query`, shape should be :math:`(S, E_v)`.
            See "Attention Is All You Need" for more details.
    ...
    Outputs:
        - **attn_output** - Attention outputs of shape :math:`(L, N, E)` when ``batch_first=False`` or
          ...
          the batch size, and :math:`E` is the embedding dimension ``embed_dim``. If inputs are unbatched then shape is
          :math:`(L, E)`.
        - **attn_output_weights** - Attention output weights of shape :math:`(N, L, S)`, where :math:`N` is the batch
          size, :math:`L` is the target sequence length, and :math:`S` is the source sequence length. Only returned
          when ``need_weights=True``. If inputs are unbatched then shape is :math:`(L, S)`.
