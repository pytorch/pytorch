...
class MultiheadAttention(Module):
    ...
    def __init__(self, embed_dim, num_heads, dropout=0., bias=True, add_bias_kv=False, add_zero_attn=False,
        ...
        self.out_proj_weight = Parameter(torch.empty((embed_dim, embed_dim), **factory_kwargs))
    ...
    def __setstate__(self, state):
        # FC: checkpoints before v1.1.0
        if '_qkv_same_embed_dim' not in state:
            state['_qkv_same_embed_dim'] = True
        # FC: checkpoints that still have 'out_proj' as a linear layer
        if 'out_proj.weight' in state:
            state['out_proj_weight'] = state.pop('out_proj_weight')
        if 'out_proj.bias' in state:
            state['out_proj_bias'] = state.pop('out_proj_bias')
