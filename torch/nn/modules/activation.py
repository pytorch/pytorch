...
class MultiheadAttention(Module):
    ...
    Shape:
        ...
        - key_padding_mask: :math:`(N, S)` where N is the batch size, S is the source sequence length.
          If a ByteTensor is provided, the positions with the value of non-zero will be ignored while the position
          with the value of zero will be unchanged. If a bool tensor is provided, the positions with the
          value of ``True`` will be ignored while the position with the value of ``False`` will be unchanged.
        - attn_mask: 2D mask :math:`(L, S)` where L is the target sequence length, S is the source sequence length.
          3D mask :math:`(N*num_heads, L, S)` where N is the batch size, L is the target sequence length,
          S is the source sequence length. attn_mask ensure that position i is allowed to attend the unmasked
          positions. If a ByteTensor is provided, positions with non-zero is not allowed to attend
          while zero values will be unchanged. If a bool Tensor is provided, positions with ``True``
