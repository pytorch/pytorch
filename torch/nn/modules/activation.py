...
try:  # pragma: no cover - optional dependency
    from torchtune.modules import (  # type: ignore[import]
        RotaryPositionalEmbeddings as _TorchtuneRotaryPositionalEmbeddings,
    )
except ModuleNotFoundError:  # pragma: no cover - torchtune not installed
    _TorchtuneRotaryPositionalEmbeddings = None


if _TorchtuneRotaryPositionalEmbeddings is None:
...
else:  # pragma: no cover - torchtune available
    RotaryPositionalEmbeddings = _TorchtuneRotaryPositionalEmbeddings  # type: ignore[assignment]
...
class MultiheadAttention(Module):
    ...
    Args:
        ...
        use_rotary: If ``True``, then applies rotary positional embeddings to query and key. Default: ``False``.
    ...
    rotary_pos_emb: Callable[[Tensor], Tensor] | None
        ...
        if not self._qkv_same_embed_dim:
            if self.use_rotary and self.rotary_pos_emb is not None:
                attn_output, attn_output_weights = F.multi_head_attention_forward(
                    ...
                    is_causal=is_causal,
                    rotary_pos_emb=self.rotary_pos_emb,  # RoPE modification
                )
            else:
                attn_output, attn_output_weights = F.multi_head_attention_forward(
                    query,
                    ...
                    average_attn_weights=average_attn_weights,
                    is_causal=is_causal,
                    rotary_pos_emb=None,  # RoPE modification
                )
        else:
            if self.use_rotary and self.rotary_pos_emb is not None:
                attn_output, attn_output_weights = F.multi_head_attention_forward(
