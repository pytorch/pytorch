...
class LinearCrossEntropyLoss(_WeightedLoss):
    ...
    If
      linear_loss = LinearCrossEntropy(linear_weight, linear_bias, **cross_entropy_kwargs)
    and
      loss = CrossEntropy(**cross_entropy_kwargs)
      linear = Linear(linear_weight, bias=linear_bias)
    then the following invariant holds:
      linear_loss(input, target) == loss(linear(input), target)

    See :class:`~torch.nn.CrossEntropyLoss` for the definitions of
