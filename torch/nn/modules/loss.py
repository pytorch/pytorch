...
class NLLLoss(_WeighedLoss):
    ...
    Args:
        weight: a manual rescaling weight given to each class. If given, has to be a Tensor of size "nclasses". Default: None
    ...
    Input Shape: [ * , * ] : Input is minibatch x nClasses
...
class NLLLoss2d(_Loss):
    ...
    Args:
        size_average: By default, the losses are averaged over observations for each minibatch. However, if the field sizeAverage is set to False, the losses are instead summed for each minibatch. Default: True
    Input Shape: [ * , * , * , *] : Input is minibatch x nClasses x height x width
