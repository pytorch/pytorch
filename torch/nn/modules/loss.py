...
class HuberLoss(_Loss):
    r"""Huber loss criterion that uses a squared term if the absolute
    element-wise error falls below 1 and an L1 term otherwise.
    It is less sensitive to outliers than the `MSELoss` and in some cases
    ...
    def __init__(self, size_average=None, reduce=None, reduction='mean'):
        super(HuberLoss, self).__init__(size_average, reduce, reduction)

    def forward(self, input, delta, target):
        return F.huber_loss(input, target, delta=1., reduction=self.reduction)
