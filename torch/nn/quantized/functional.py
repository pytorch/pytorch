...
def linear(input, weight, bias=None, scale=None, zero_point=None):
    ...
    _packed_weight = torch.ops.quantized.quantized_linear_prepack(weight)
    if bias is not None:
        bias = torch.quantize_linear(bias.dequantize(), weight.q_scale() * input.q_scale(), 0, torch.qint32)
    return torch.ops.quantized.quantized_linear(input, _packed_weight, bias, scale,
                                             zero_point)
...
def conv2d(input, weight, bias,
    ...
    prepacked_weight = torch.ops.quantized.quantized_conv_prepack(
    ...
    return torch.ops.quantized.quantized_conv2d(input.permute([0, 2, 3, 1]),
