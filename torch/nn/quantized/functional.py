...
def dropout(input: Tensor, scale: float, zero_point: int, p: float = 0.5, training: bool = False) -> Tensor:
    ...
    if not input.is_quantized:
        raise ValueError("Input to 'quantized.elu' must be quantized!")
    return torch.ops.quantized.dropout(input, scale, zero_point, p, training)
