...
class Conv2d(torch.nn.Module):
    ...
    def set_weight(self, w):
        self._packed_weight = torch.ops.quantized.quantized_conv_prepack(
            w.permute([0, 2, 3, 1]), self.stride, self.padding, self.dilation, self.groups)
        self.weight_scale = w.q_scale()

    def weight(self):
        return torch.ops.quantized.quantized_conv_unpack(
    ...
    def forward(self, input):
        ...
        output = ops.quantized.quantized_conv2d(input.permute([0, 2, 3, 1]),
