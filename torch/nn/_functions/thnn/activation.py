...
class Softmin(Function):
    ...
    def forward(ctx, input):
        ctx._backend = type2backend[type(input)]
        ctx.mininput = input.clone().mul(-1)
        output = input.new()
        ctx._backend.SoftMax_updateOutput(
            ctx._backend.library_state,
            ctx.mininput,
            output
        )
    ...
    def backward(ctx, grad_output):
        input, output = ctx.saved_variables
        return SoftminBackward.apply(input, output, grad_output, ctx.mininput)


class SoftminBackward(Function):

    @staticmethod
    def forward(ctx, input, output, grad_output, mininput):
        ctx._backend = type2backend[type(output)]
        ctx.mininput = mininput
        # we don't technically need input but this uses the same structure as the autogenerated SoftMax.
        ctx.save_for_backward(input, grad_output, output)
        grad_input = grad_output.new()
        ctx._backend.SoftMax_updateGradInput(
            ctx._backend.library_state,
            ctx.mininput,
            grad_output,
            grad_input,
    ...
    @staticmethod
    def backward(ctx, ggI):
        ret = softmax_double_backwards(ctx, ggI)
        gI = ret[0]
        ggO = -1 * ret[1]
...
_all_functions.append(SoftminBackward)
