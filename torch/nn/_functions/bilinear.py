...
class Bilinear(Function):
    ...
    def forward(self, input1, input2, weight, bias=None):
        ...
        for k, w in enumerate(weight):
            torch.mm(input1, w, out=self.buff2)
    ...
    def backward(self, grad_output):
        ...
        if self.needs_input_grad[0] or self.needs_input_grad[1]:
            ...
            for k in range(1, weight.size(0)):
                ...
                self.buff1.mul_(grad_output.narrow(1, k, 1).expand(grad_input1.size()))
                grad_input1.add_(self.buff1)

                torch.mm(input1, weight[k], out=self.buff2)
                self.buff2.mul_(grad_output.narrow(1, k, 1).expand(grad_input2.size()))
