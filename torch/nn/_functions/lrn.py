...
class SpatialCrossMapLRN(Function):

    @staticmethod
    def forward(ctx, input, size, alpha=1e-4, beta=0.75, k=1):
        
        assert input.dim() == 4, "Expected a 4-dim input. Got instead a {}-dim one".format(input.dim())
        ...
        if ctx._backend is not None:
            ...
            ctx.scale.mul_(alpha / ctx.size).add_(k)

            torch.pow(ctx.scale, -beta, out=output)
            output.mul_(input)
