...
from torch.onnx import symbolic_override_first_arg_based, symbolic_override_packed_sequence_based
...
def _symbolic_pack_padded_sequence(g, sequence, lengths, batch_first=False):
    if batch_first:
        from torch.onnx import symbolic
        sequence = symbolic.t(sequence)
    # There currently is no PackPadded operator in ONNX. We rely on an
    # optimization pass to remove this later. It is an error if all
    # PackPadded operators cannot be optimized out.
    return g.op("PackPadded", sequence, lengths, outputs=2)


@symbolic_override_first_arg_based(_symbolic_pack_padded_sequence)
def pack_padded_sequence(sequence, lengths, batch_first=False):
    ...
    data, batch_sizes = PackPadded.apply(sequence, lengths, batch_first)
...
@symbolic_override_packed_sequence_based(_symbolic_pad_packed_sequence)
