...
class Capture:
    ...
    it is refused, and so is saving -- except to retry a block-exit write that
    failed.
    """
...
def capture(
    ...
    *,
    ...
    the files before it exits, and after an exit whose WRITE failed to retry it. The
...
class _PrecompileApi:
    ...
    def __call__(
        ...
        *example_inputs: object,
        ...
        without a ``register_fake``), a ``.data_ptr()`` read, an example input the fake
        trace cannot represent
        ...
        device per loaded artifact; such a device can hold no autocast region to begin
        with, so the skip only costs a dtype divergence in the narrower case the same
        warning covers -- the device probe itself failing, e.g. raising a device
        deprecation warning under ``-W error``.
        """
