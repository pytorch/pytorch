...
def _check_path_pair(
    ...
    are typed) but turn ``os.path.abspath(None)``'s bare ``TypeError`` into one naming
    the pair.
    """
...
class _PrecompileApi:
    ...
    def __call__(
        ...
        *example_inputs: object,
        ...
        construct (a registered device module with no autocast support) is skipped rather
        than failed on, with one logged warning per device per loaded artifact: that is
