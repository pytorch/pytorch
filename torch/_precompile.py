...
def capture(
    ...
    *,
    ...
    the files before it exits, and after a WRITE that failed to retry it. The
    contract is Note [precompile programming model] in this module; see
...
class _PrecompileApi:
    ...
    def __call__(
        ...
        *example_inputs: object,
        ...
        kernel and none that can be synthesized (a ``torch.library.custom_op`` that only
        mutates its arguments and returns nothing gets a trivial one, so it captures
        without a ``register_fake``), a read of a traced tensor's data (``.data_ptr()``,
        ...
        autocast is skipped rather than failed on, with one logged warning per device
        per loaded artifact. That skip costs a dtype divergence when the device REPORTS
        autocast enabled and ``torch.amp.autocast`` then refuses to construct the
        disable -- an out-of-tree backend with no registered device module -- which is
        the case the warning covers. A device whose probe RAISES cannot have autocast
        enabled at all, since the same lookup backs ``torch.set_autocast_enabled``.
        """
