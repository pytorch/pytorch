...
def _check_path_pair(
    ...
    Three ways it can be unusable: artifact CONTENTS passed where a path belongs (the
    in-memory ``(python_code, cache)`` pair an earlier ``load`` took under the same
    name), one file named for both halves after resolving links (the write would
    clobber the source), and a path that exists but is not a regular file.
    """
