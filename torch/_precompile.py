...
def capture(
    ...
    *,
    ...
    nothing further -- files an in-block ``cap.save()`` already wrote stay -- and so
    does a clean exit with nothing captured -- no call made, or the only call raised
    and was caught -- which raises instead)::
