...
def _parse_artifact_metadata(python_code: str) -> dict[str, object]:
    ...
    for node in tree.body:
        ...
        if target.id in wanted:
            ...
            # call), so this fires many times on an inductor artifact. Skipped by
            # design, but logged at debug so a malformed / renamed artifact is
...
def capture(
    ...
    *,
    ...
    (inline the captured ATen graph as readable source). Call ``cap.save()`` inside the
    block to write the files before it exits, and after an exit whose WRITE failed to
    retry it. The contract is Note [precompile programming model] in this module; see
