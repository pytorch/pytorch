...
class _StorageBase(object):
    ...
    def __str__(self):
        ...
        return data_str + (
            f"\n[{torch.typename(self)}(device={self.device}) " f"of size {len(self)}]"
        )
...
class _TypedStorage:
    ...
    def __new__(cls, *args, wrap_storage=None, dtype=None, device=None):
        ...
        if cls == _TypedStorage:
            ...
            if wrap_storage is None:
                ...
                return _TypedStorage(*args, wrap_storage=wrap_storage, dtype=cls.dtype)
    ...
    def __init__(self, *args, device=None, dtype=None, wrap_storage=None):
        ...
        if wrap_storage is not None:
            ...
            if len(args) == 0:
                ...
                if _isint(args[0]):
                    self._storage = torch._UntypedStorage(
    ...
    def __setitem__(self, idx, value):
        ...
        if self.dtype in [
            ...
            tmp_tensor = torch.tensor([], dtype=tmp_dtype, device=self.device).set_(
                _TypedStorage(wrap_storage=self._storage, dtype=tmp_dtype)
            )
        else:
            tmp_tensor = torch.tensor([], dtype=self.dtype, device=self.device).set_(
                self
            )
    ...
    def __getitem__(self, idx):
        ...
        if isinstance(idx, slice):
            raise RuntimeError(
                "slices are only supported in _UntypedStorage.__getitem__"
            )
        ...
        if self.dtype in [
            ...
            return _TypedStorage(
                wrap_storage=self._storage, dtype=interpret_dtypes[self.dtype]
            )[idx]
    ...
    def cuda(self, device=None, non_blocking=False, **kwargs) -> T:
        ...
        cuda_storage: torch._UntypedStorage = self._storage.cuda(
    ...
    def _new_shared(self, size, *, device=None):
        ...
        if device is None:
            device = "cpu"
        device = torch.device(device)
        untyped_storage = torch._UntypedStorage._new_shared(
            size * self.element_size(), device=device
        )
        return _TypedStorage(wrap_storage=untyped_storage, dtype=self.dtype)
    ...
    def from_buffer(cls, *args, dtype=None, device=None, **kwargs):
        if cls == _TypedStorage:
            ...
            if device.type != "cpu":
                raise RuntimeError(
                    f"_TypedStorage.from_buffer: Not available for device {device.type}"
                )
            untyped_storage: torch._UntypedStorage = torch._UntypedStorage.from_buffer(
            ...
            dtype = cls.dtype
            untyped_storage = torch._UntypedStorage.from_buffer(
                *args, dtype=dtype, **kwargs
            )
    ...
    def _share_filename_cpu_(self, *args, **kwargs):
        manager_handle, storage_handle, size = self._storage._share_filename_cpu_(
            *args, **kwargs
        )
...
class _LegacyStorage(_TypedStorage, metaclass=_LegacyStorageMeta):
    ...
    def _new_shared_filename(cls, manager, obj, size):
        ...
        return cls(
            wrap_storage=torch._UntypedStorage._new_shared_filename_cpu(
