        ...
        NodeDef{
            [](const c10::IValue& nested,
              ...
              for (size_t i = 0; i < tuple.size(); i++) {
                nestedFlatten(tuple[i], spec.children(i), ivalues);
              }
            },
        ...
        NodeDef{
            [](const c10::IValue& nested,
              ...
              for (size_t i = 0; i < list.size(); i++) {
                nestedFlatten(list[i], spec.children(i), ivalues);
              }
            },
            [](std::vector<c10::IValue> flats,
               const nlohmann::json& obj) -> c10::IValue {
        ...
        NodeDef{
            [](const c10::IValue& nested,
              ...
              for (const auto& key : contextKeys) {
                ...
                if (it != dict.end()) {
                  nestedFlatten(it->value(), spec.children(i), ivalues);
                } else {
...
NestedSpec makeNestedSpec(
  ...
  if (obj["type"].is_null()) {
    ...
    if (value) {
      ...
      return NestedSpec(value, isUsed);
    } else {
      return NestedSpec(value, false);
    }
  }
  ...
  for (const auto& child : childrenSpec) {
    children.push_back(makeNestedSpec(child, values, start + offset));
    // NOLINTNEXTLINE(*-narrowing-conversions)
    offset += children.back().numIValues();
  }

  return NestedSpec(name, context, std::move(children), nodeDefCache);
}
...
NestedSpec nestedSpecLoads(
  ...
  auto result = makeNestedSpec(obj[1], values, 0);
...
c10::IValue nestedUnflatten(
    std::vector<c10::IValue> ivalues,
    const NestedSpec& spec) {
  RECORD_USER_SCOPE("nativert::nestedUnflatten");
  ...
  for (const auto& child : spec.children()) {
    ...
    childrenPytrees.push_back(nestedUnflatten(std::move(slice), child));
    start += numIValues;
  }
...
std::vector<c10::IValue> nestedFlatten(
  ...
  nestedFlatten(nested, spec, ivalues);
  return ivalues;
}
...
std::vector<c10::IValue> nestedFlattenFromArgs(
    ...
    const NestedSpec& spec) {
  RECORD_USER_SCOPE("nativert::nestedFlattenFromArgs");
  ...
  for (size_t i = 0; i < args.size(); i++) {
    nestedFlatten(args[i], specArgs.children(i), ivalues);
  }
...
std::vector<at::Tensor> nestedFlattenToTensorList(
    const c10::IValue& nested,
    const NestedSpec& spec) {
  auto flats = nestedFlatten(nested, spec);
...
c10::IValue nestedMap(
    ...
    const c10::IValue& nested,
    const NestedSpec& spec) {
  const auto flats = nestedFlatten(nested, spec);
  std::vector<c10::IValue> mapped;
  mapped.reserve(flats.size());
  for (const auto& flat : flats) {
    mapped.push_back(f(flat));
  }
  return nestedUnflatten(std::move(mapped), spec);
}
...
std::
  ...
  const auto mapVal = nestedMap(f, val, spec);
  auto mapArgs =
...
c10::TypePtr NestedSpec::toAtenType() const {
  if (isIValue()) {
