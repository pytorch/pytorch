...
CudaTritonKernelManager::~CudaTritonKernelManager() {
  for (auto& mod : loaded_modules_) {
    if (CUresult err = cuModuleUnload(mod); err != 0) {
      CU_LOG_ERROR(cuModuleLoad, err);
    }
  }
}
...
CUfunction CudaTritonKernelManager::load() {
  ...
  if (CUresult err = cuModuleLoad(&mod_ptr, kernel_bin_path_.c_str());
      err != 0) {
    CU_LOG_ERROR(cuModuleLoad, err);
    return nullptr;
  }
  ...
  if (CUresult err = cuModuleGetFunction(&func, mod_ptr, kernel_name_.c_str());
      err != 0) {
    CU_LOG_ERROR(cuModuleGetFunction, err);
    return nullptr;
  }
