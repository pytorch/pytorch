...
// Optional validator function can be provided to validate the value before assignment.
...
// Overload without validator - always assigns if name matches and type is correct.
template <typename VariantT, typename TargetT>
bool set_from_variant(TargetT& target, const std::string& name, const Attribute& attr) {
  if (attr.name == name) {
    if (auto* ptr = std::get_if<VariantT>(&attr.value)) {
      target = *ptr;
      return true;
