...
#include <torch/nativert/detail/ITree.h> // @manual
...
using torch::nativert::Executor;
using torch::nativert::Graph;
using torch::nativert::MakeProxyExecutorFn;
using torch::nativert::Placement;
using torch::nativert::ProfileMetrics;
using torch::nativert::Value;
using torch::nativert::Weights;
...
struct BaseRuntimeConfigs {
  ...
  // whether to free IValues between runs
  bool tryFreeUnmanagedValuesAfterUse = true;
  // memory planner settings
  torch::nativert::LayoutPlannerSettings layoutPlannerSettings;
  ...
  // whether to cleanup execution frames from pool
  // If true, the execution frame pool will be reset to minNumExecutionFrames
  // every executionFramePoolCleanupIntervalSec seconds
  ...
  // min number of executions frames to keep in the pool
  int64_t minNumExecutionFrames = 1;

  // interval to clean up the execution frame pool
  int64_t executionFramePoolCleanupIntervalSec = 600;

  // whether to enable graph runtime passes
  bool enableRuntimeGraphPasses = false;
};
...
class TORCH_API ModelRunnerBase {
 public:
  using GraphPassFn =
      std::function<void(const BaseRuntimeConfigs&, torch::nativert::Graph&)>;
  ModelRunnerBase(
      std::shared_ptr<caffe2::serialize::PyTorchStreamReader>
          pytorchStreamReader,
  ...
  const ITreeSpec& getOutputSpec() const;
  const ITreeSpec& getInputSpec() const;
  ...
  const BaseRuntimeConfigs& getRuntimeConfigs() const {
  ...
  void setTryFreeUnmanagedValuesAfterUse(bool enabled) {
    runtimeConfigs_.tryFreeUnmanagedValuesAfterUse = enabled;
  }

  void setLayoutPlannerSettings(
      const torch::nativert::LayoutPlannerSettings& settings) {
    runtimeConfigs_.layoutPlannerSettings = settings;
  }

  void setEnableRuntimeConstFolding(bool enable) {
    runtimeConfigs_.enableRuntimeConstFolding = enable;
  }

  // Get the number of execution frames in the pool from the executor, used for
  // testing (i.e. ModelRunnerTest)
  ...
  void initialize(
      ...
      const GraphPassFn& graphPassFn = nullptr,
  ...
  auto format(torch::nativert::ExecutorType et, FormatContext& ctx) const {
    ...
    switch (et) {
      case ExecutorType::INTERPRETER:
        return fmt::format_to(ctx.out(), "INTERPRETER");
      case ExecutorType::AOTINDUCTOR:
        return fmt::format_to(ctx.out(), "AOTINDUCTOR");
      case ExecutorType::MTIA:
        return fmt::format_to(ctx.out(), "MTIA");
      default:
