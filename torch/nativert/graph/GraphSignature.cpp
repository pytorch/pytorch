...
void checkInputNames(
    const std::unordered_set<std::string>& sigNames,
    const std::unordered_set<std::string>& graphNames) {
...
void checkOutputNames(
    const std::unordered_set<std::optional<std::string>>& sigNames,
    const std::unordered_set<std::string>& graphNames) {
...
GraphSignature::GraphSignature(const torch::_export::GraphSignature& storage) {
  ...
  if (FLAGS_caffe2_log_level > 2) {
    fmt::print("[{}]\n", fmt::join(userInputs_, ", "));
    fmt::print("[{}]\n", fmt::join(keys_of(inputsToParameters_), ", "));
    fmt::print("[{}]\n", fmt::join(keys_of(inputsToBuffers_), ", "));
    fmt::print("[{}]\n", fmt::join(keys_of(inputsToTensorConstants_), ", "));
    fmt::print("[{}]\n", fmt::join(get_valid(userOutputs_), ", "));
    fmt::print("[{}]\n", fmt::join(keys_of(buffersToMutate_), ", "));
    fmt::print("[{}]\n", fmt::join(keys_of(gradientsToParameters_), ", "));
    fmt::print("[{}]\n", fmt::join(keys_of(gradientsToUserInputs_), ", "));
  }
}

std::unordered_set<std::string> GraphSignature::inputNames() const {
...
std::unordered_set<std::optional<std::string>> GraphSignature::outputNames()
    const {
  std::unordered_set<std::optional<std::string>> ret;
