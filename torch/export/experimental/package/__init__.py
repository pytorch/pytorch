...
###############################################################################
import torch
from torch._C._nativert.pt2_archive_constants import (  # @manual=//sigmoid/core/package:pt2_archive_constants_pybind
    CONSTANTS_DIR,
...
__all__ = ["package_model", "PT2ArchiveWriter", "get_raw_tensor_bytes"]


def get_raw_tensor_bytes(value: torch.Tensor) -> bytes:
    """
    Get the raw bytes of a tensor. This is used to save the tensor in the zip archive.
    """
...
def _package_state_dict(
    ...
    for name, tensor in exported_program.state_dict.items():
        if tensor.is_meta:
            logger.error(
                "Skipping state_dict packing of %s since it's a meta tensor", name
            )
            continue
...
def _package_constants(
    ...
    for name, constant in exported_program.constants.items():
        if isinstance(constant, torch.Tensor):
            ...
            # functions.
            logger.info("saving script object %s", name)
...
def package_model(
    ...
    _package_sample_inputs(example_args, example_kwargs, zip_file, model_name)
