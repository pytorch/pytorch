...
def _inplace_buffer_and_input_mutations(
    ...
    for k, buffer_fqn in graph_signature.buffers_to_mutate.items():
        mutation_name_to_input_name[k] = buffer_fqn_to_input_name[buffer_fqn]
    # Collect mutated user inputs.
    for k, user_input_name in graph_signature.user_inputs_to_mutate.items():
        mutation_name_to_input_name[k] = user_input_name
...
class _IVals:
    ...
    def _is_mutable(self, target):
        ...
        if isinstance(target, torch._ops.OpOverloadPacket):
            return all(schema.is_mutable for schema in target._schemas.values())
        return False
