...
def _split_values(value: Any, count: int, name: str) -> tuple[Any, ...]:
        ...
        *,
        ...
        Args:
            ...
            coscheduled_sm_count (int or sequence of int, optional): Co-scheduled
                SM grouping size for thread-block clusters. Zero selects the
                device default. Default: ``0``.
        ...
        _ensure_driver_version(13010, message)
        _ensure_cuda_bindings_version(13010, message)
        counts = tuple(num_sms) if isinstance(num_sms, Sequence) else (num_sms,)
        if not counts:
            raise ValueError("num_sms must contain at least one group")
        n = len(counts)
        co_counts = _split_values(coscheduled_sm_count, n, "coscheduled_sm_count")
        preferred = _split_values(
