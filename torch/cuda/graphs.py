...
def make_graphed_callables(
    ...
    for c, args in zip(callables, sample_args):
        ...
        flatten_arg = torch.utils.pytree.tree_leaves(args)
    ...
    with torch.cuda.stream(torch.cuda.Stream()):
        for func, args, static_input_surface in zip(
            ...
            for _ in range(num_warmup_iters):
                outputs = torch.utils.pytree.tree_leaves(func(*args))
    ...
    for func, args, fwd_graph in zip(callables, sample_args, fwd_graphs):
        ...
        flatten_outputs, spec = torch.utils.pytree.tree_flatten(outputs)
    ...
    def make_graphed_autograd_function(
        ...
        def functionalized(*user_args):
            ...
            flatten_user_args = torch.utils.pytree.tree_leaves(user_args)
            out = Graphed.apply(*(tuple(flatten_user_args) + module_params))
            return torch.utils.pytree.tree_unflatten(out, output_unflatten_spec)
