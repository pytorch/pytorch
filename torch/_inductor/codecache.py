            ...
            if not _IS_WINDOWS:
                ...
                for kernel_name, value in CudaKernelParamCache.cache.items():
                    ...
                    if (
                        ...
                        # PTX generated by Triton targets the current architecture and may use
                        # architecture-specific instructions. PTX is forward-compatible but not
                        # backward-compatible, so we can only compile for architectures >= current arch.
                        #
                        # Example: PTX generated on A100 (sm_80) can run on:
                        #   ✓ A100 (sm_80), L4 (sm_89), H100/H200 (sm_90)
                        ...
                        # If no compatible archs (shouldn't happen), use current arch only
...
def _get_multi_arch_target_list() -> list[str]:
    """
    Get list of target architectures for multi-arch fat binary compilation.
    Returns a list of compute capabilities (e.g., ["70", "75", "80", "86", "90"]).

    This reads from TORCH_CUDA_ARCH_LIST environment variable if set,
    otherwise returns a default list covering common GPU architectures.
    """
    arch_list_str = os.environ.get("TORCH_CUDA_ARCH_LIST", "")

    if arch_list_str:
        # Parse TORCH_CUDA_ARCH_LIST which can be in formats like:
        # "7.0;7.5;8.0;8.6;9.0+PTX" or "7.0 7.5 8.0 8.6 9.0+PTX"
        # Convert to ["70", "75", "80", "86", "90"]
        archs = []
        # Split by semicolon or space
        for item in re.split(r'[;\s]+', arch_list_str):
            item = item.strip()
            if not item:
                continue
            # Remove +PTX suffix if present
            item = item.replace('+PTX', '')
            # Convert "7.0" to "70", "8.6" to "86", etc.
            if '.' in item:
    ...
    # Default list covering V100 (sm_70), A100 (sm_80), and H100/H200 (sm_90)
    # These are the most common datacenter GPUs mentioned in issue #160303
