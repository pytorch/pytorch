    ...
    def _lookup_graph(
        ...
        for path in os.listdir(subdir):
            ...
            with shape_env.suppress_guards():
                res = shape_env.evaluate_guards_expression(graph.guards_expr, symints)
                hit = definitely_true(res)

            log.debug(
                "fx graph evaluating guards for key %s: (%s) = %s",
                key,
                graph.guards_expr,
                hit,
            )
