...
@pw_cast_for_opmath
def mm(self, input2):
    from torch.fx.experimental.symbolic_shapes import guard_size_oblivious, definitely_true
    ...
    if self.device.type == "cpu":
        ...
        if guard_size_oblivious(self.size(0) == 1) and guard_size_oblivious(input2.size(-1) == 1):
            return torch.sum(
