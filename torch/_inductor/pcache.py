        ...
        if env_val is not None:
            for kv_pair in env_val.split(";"):
                if kv_pair == "":
                    ...
                    continue
                try:
                    key, str_value = kv_pair.split(",")
                except ValueError as err:
                    raise ValueError(
                        f"Malformed kv_pair {kv_pair} in env_var {env_var}!"
                    ) from err
                # check that str_value is a str repr of bytes
                if (not str_value.startswith("b'")) or (not str_value.endswith("'")):
                    raise ValueError(
                        f"Malformed value {str_value} in kv_pair {kv_pair}!"
                    )
                # remove b' prefix and ' suffix
                str_value = str_value[2:-1]
                value = str_value.encode()
                # duplicatess are ok, so long as the key does not point to two distinct values
