...
def get_cpp_torch_options(
    ...
    ldflags = omp_ldflags
    libraries_dirs = python_libraries_dirs + omp_lib_dir_paths
    libraries =  omp_lib

    if include_pytorch:
        libraries_dirs += torch_libraries_dirs
...
def get_cpp_torch_device_options(
    ...
    if not config.is_fbcode() and include_pytorch:
        libraries += ["c10"]
    if device_type == "cuda":
        ...
        if torch.version.hip is not None:
            ...
            if config.is_fbcode() or not include_pytorch:
                libraries += ["cuda"]
            else:
        ...
        if device_type == "cuda" and torch.version.hip is None:
            ...
            if config.aot_inductor.cross_target_platform == "windows":
                ...
                if config.aot_inductor.embed_kernel_binary:
                    contents += textwrap.indent(textwrap.dedent(
                        """
