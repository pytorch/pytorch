    ...
    for graph in all_graphs:
        if debug:
            ...
            for n in graph.nodes:
                if _is_to_dtype_node(n):
                    ...
                    print(f"  {n.name}: to_dtype({inp_name}, {dtype}, uct={uct}), users={user_names}")
        ...
        while modified:
            ...
            for node in list(graph.nodes):
                ...
                if _produces_fp32(inner_input):
                    ...
                    if debug:
                        inp_name = inner_input.name if hasattr(inner_input, 'name') else str(inner_input)
        ...
        while modified:
            ...
            for node in list(graph.nodes):
                ...
                if debug:
                    print(f"Pass2: {node.name} <- {inner_node.name} <- {inner_input.name}")
                total_patterns += 1
                node.replace_all_uses_with(inner_input)
                processed_nodes.add(node)
        ...
        if debug:
            ...
            for n in graph.nodes:
                if _is_to_dtype_node(n):
                    ...
                    inp = _get_input_node(n)
                    inp_name = inp.name if inp and hasattr(inp, 'name') else str(inp)
                    print(f"  {n.name}: to_dtype({inp_name}, {dtype}, uct={uct}), users={len(n.users)}")
