...
class ExternKernel(InputsKernel):
    ...
    def codegen_args(self, code: Optional[IndentedBuffer] = None):  # type: ignore[no-untyped-def]
        ...
        for i, x in enumerate(inputs):
            if V.graph.cpp_wrapper:
                ...
                args.append(V.graph.wrapper_code.val_to_arg_str(x, type_, code=code))
            else:
                args.append(V.graph.wrapper_code.val_to_arg_str(x, code=code))
        if need_codegen_constant_args:
            args.extend(self.codegen_const_args(code=code))
        return args
    ...
    def codegen_kwargs(self, code: Optional[IndentedBuffer] = None, skip_out=False):  # type: ignore[no-untyped-def]
        if V.graph.cpp_wrapper:
            ...
            for arg_name in self.ordered_kwargs_for_cpp_kernel:
                ...
                if isinstance(v, sympy.Expr):
                    ...
                    kwargs.append(
                        V.graph.wrapper_code.val_to_arg_str(v, type_, code=code)
                    )
        else:
            kwargs = [
...
class FallbackKernel(ExternKernelAlloc):
    ...
    def codegen_args(self, code: Optional[IndentedBuffer] = None):  # type: ignore[no-untyped-def]
        ...
        tensor_args = [Shim(x.codegen_reference(code)) for x in self.inputs]
        args, kwargs = self.unflatten_args(tensor_args, self.constant_args)
        if V.graph.cpp_wrapper and isinstance(self.op_overload, torch._ops.OpOverload):
            args = self.fill_non_provided_args(args, kwargs)
            args = [
                V.graph.wrapper_code.val_to_arg_str(x, param.real_type, code=code)
                for param, x in zip(self.op_overload._schema.arguments, args)
            ]
        else:
            args = [V.graph.wrapper_code.val_to_arg_str(x, code=code) for x in args]
