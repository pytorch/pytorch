...
class MultiTemplateBuffer(TritonTemplateBuffer):
    ...
    def __init__(
        ...
        self._choice_timings: dict[Optional[int], Optional[dict[ChoiceCaller, float]]] = {}
    ...
    def choice_timings(self, hint_override: Optional[int] = None) -> dict[ChoiceCaller, float]:
        if hint_override not in self._choice_timings or self._choice_timings[hint_override] is None:
    ...
    def finalize_as_triton_callers(self, callers: list[TritonTemplateCallerBase]) -> None:
        """Finalize multiple callers for multi-kernel dispatch"""
        # For now, just use the first caller as the primary one
        # TODO: Implement proper multi-kernel dispatch logic
        if callers:
            self.finalize_as_triton_caller(callers[0])
