...
class UserDefinedTritonKernel(ExternKernel):
    ...
    def can_fuse_epilogue(self) -> bool:
        ...
        written_arg_name = next(iter(self.args_read_writes.writes)).name
        #  cannot fuse if the kernel also reads from the output buffer
        if any(
            read_dep.name == written_arg_name
            for read_dep in self.args_read_writes.reads
        ):
            return False
    ...
    def _codegen(
        ...
        if epilogue_fusion:
            assert len(self.args_read_writes.writes) == 1
            mutable_arg_name = next(iter(self.args_read_writes.writes)).name
    ...
    def __init__(
        ...
        *,
        ...
        # names in this ReadWrites are names of formal arguments in the kernel's prototype
        self.args_read_writes = identify_accessed_tensors(
            kernel,
        ...
        self.mutable_args = [
            kernel_args[key.name] for key in self.args_read_writes.writes
        ]
    ...
    def get_read_writes(self) -> dependencies.ReadWrites:
        ...
        read_renames = {
            ...
            for formal_arg_dep in self.args_read_writes.reads
        }

        formal_arg_writes = list(self.args_read_writes.writes)
        write_renames = {
        ...
        read_writes = dependencies.ReadWrites(
            reads=OrderedSet(
                [dep.rename(read_renames) for dep in self.args_read_writes.reads]
            ),
            writes=OrderedSet(
                [dep.rename(write_renames) for dep in self.args_read_writes.writes]
            ),
            index_exprs=OrderedSet(),
        )
        return read_writes
