...
class ExternKernel(InputsKernel):
    ...
    def collect_arg_kwarg_properties(self):  # type: ignore[no-untyped-def]
        ...
        if isinstance(self.op_overload, torch._ops.OpOverload):
            ...
            self._schema_kwargs_defaults = {
                x.name: x.default_value
                for x in self.op_overload._schema.arguments
                if x.kwarg_only
            }
    ...
    def codegen_kwargs(self, skip_out=False):  # type: ignore[no-untyped-def]
        if V.graph.cpp_wrapper:
            if self.op_overload is not None and len(self._schema_kwargs_defaults) == 0:
...
class FallbackKernel(ExternKernelAlloc):
    ...
    def export_extern_kernel_node(self):  # type: ignore[no-untyped-def]
        ...
        args = self.fill_non_provided_args(args, kwargs)
        ordered_kwargs = [
            kwargs.get(key, self._schema_kwargs_defaults.get(key))
