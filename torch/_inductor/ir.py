...
class WhileLoop(ExternKernel):
    ...
    def create(
        ...
        if not isinstance(fx_carried_inputs_arg, Sequence):
            raise AssertionError(type(fx_carried_inputs_arg))
        if not isinstance(fx_additional_inputs_arg, Sequence):
            raise AssertionError(type(fx_additional_inputs_arg))
        ...
        if len(body_fake_all_inputs) != len(fx_all_inputs):
            raise AssertionError((body_fake_all_inputs, fx_all_inputs))
        if len(cond_fake_all_inputs) != len(fx_all_inputs):
            raise AssertionError((cond_fake_all_inputs, fx_all_inputs))
        ...
        for i, (op, bo) in enumerate(zip(carried_inputs_, body_outputs)):
            if isinstance(op, ShapeAsConstantBuffer) or isinstance(
                ...
                if not (
                    isinstance(op, ShapeAsConstantBuffer)
                    and isinstance(bo, ShapeAsConstantBuffer)
                ):
