...
def optimize_memory_usage(
    ...
    Move consumer ops earlier if it satisfies the following conditions:
        - Consumer op is the first use site of its out tensor
            ...
            - For now, we only target consumer ops with .writes containing only itself
        - This consumer op is last use site for all its inputs args (by checking .last_usage)
        - The out tensor is smaller than the sum memory of all its input args
        - Ignore consumer ops that
            - contain collective or wait
            - or, is MultiOutput node
            - or, has GroupedSchedulerNode as one of inputs
    If all above conditions are satisfied, we can move the consumer op up to right after the last write site of all of its input args
    (i.e. the point where all input args have finished being written into).
    After moving these consumer ops up, we can release the memory of their input args much earlier in the graph.
    """
