...
def apply_var_mapping(iter_vars, red_vars, norm_pw_vars, norm_red_vars, new_ranges, return_getters_groups):
    ...
    return {k: sympy_subs(v, flat_vars_to_new_vars) for k, v in iter_vars_to_flat_vars.items()}
...
def extract_normalized_read_writes(
    ...
    outputs = OrderedSet(
        buf for buf in all_output_names if not V.graph.scheduler.can_buffer_be_removed_through_fusion(buf, op_names)
    )
    ...
    reads = {V.graph.sizevars.simplify_with_ranges(r, ranges): v for r, v in reads.items()}
    writes = {V.graph.sizevars.simplify_with_ranges(w, ranges): v for w, v in writes.items()}

    out = FusedNormalizedReadsWrites(
    ...
    loop_tiling_log.info("Normalized Fused reads: %s", out)
    return out
