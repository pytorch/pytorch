...
def fuse_conv_bn(gm: torch.fx.GraphModule, inplace=True):
    """
    Fuses Convolution/BN layers.
    This function transforms Conv+BN calls into Conv with BN normalized weight kernels,
    and can be used for both training and inference. The optimization for training requires
     the BN module in eval mode, and the optimization for inference requires one additional
     step of constant folding.
    """
    if not inplace:
        gm = copy.deepcopy(gm)
