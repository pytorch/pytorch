...
def normalize_cat_base(match: Match):
    cat_node = match.nodes[0]
    graph = match.graph
    tensors = get_arg_value(cat_node, 0)
    cat_dim = get_arg_value(cat_node, 1, "dim") or 0
    out = get_arg_value(cat_node, 2, "out")
...
@register_graph_pattern(
    CallMethodVarArgs("cat", users=MULTIPLE),
    pass_dict=normalize_split_pass,
    extra_check=config_flag("split_cat_fx_passes"),
)
def normalize_cat_default(match: Match, *args, **kwargs):
    return normalize_cat_base(match)
