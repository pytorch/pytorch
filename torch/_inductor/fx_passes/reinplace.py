    ...
    for node in graph.nodes:
        if (inplaceable_op := inplaceable_ops.get(node.target, None)) is not None:
            ...
            if "original_arg_kwarg_vals" in node.meta:
                # We changed the kwargs, so we need to update original_arg_kwarg_vals
                # to something sane.
                args, kwargs = node.meta["original_arg_kwarg_vals"]
                new_kwargs = {**kwargs}
                new_kwargs["tensors_to_clone"] = immutable_list(tensors_to_clone)
                new_kwargs = immutable_dict(new_kwargs)
                node.meta["original_arg_kwarg_vals"] = (args, new_kwargs)
        elif (
