...
def _sfdp_pattern_24(query, key, value, attn_mask, inv_scale):
    ...
    fill_value = torch.full((), -float("inf"), dtype=query.dtype, device=query.device)
    return torch.bmm(torch.softmax(scores.masked_fill(attn_mask.bool(), fill_value), dim=-1), v)
...
def _sfdp_replacement_24(query, key, value, attn_mask, inv_scale):
    ...
    key = key.permute(0, 2, 1, 3)
    value = value.permute(0, 2, 1, 3)
    mask = attn_mask.view(bs, n_head, attn_mask.size(-2), attn_mask.size(-1))
    scale = 1.0 / inv_scale
    return _scaled_dot_product_attention(
            query,
            key,
...
def _get_sfdp_patterns():
    ...
    m_inp_3d = functools.partial(torch.empty, (16, 4, 4), device=device)
    # inv_scale
