...
def _add_chunking_meta_to_source_node(source_node):
    ...
    meta = ChunkingMeta(chunking_dim=0)
...
def copy_chunking_meta(dst_node: torch.fx.Node, source_node: torch.fx.Node):
    source_meta = _get_chunking_meta(source_node)
    assert source_meta
    dst_node.meta["chunking"] = ChunkingMeta(**dataclasses.asdict(source_meta))
...
def propagate_unary_pointwise(nd):
    node_args = [x for x in _pytree.tree_flatten((nd.args, nd.kwargs))[0] if isinstance(x, torch.fx.Node)]
...
class AutoChunker:
    ...
    def _collect_source_nodes(self) -> Sequence[torch.fx.Node]:
        ...
        for nd in self.node_list[: self.last_fwd_node_index + 1]:
            ...
            if nd.op != "call_function" or nd.target not in eligible_source_node_op_to_idx:
                continue

            argidx = eligible_source_node_op_to_idx[nd.target]
            ...
            if (
                ...
                source_nodes.append((nd.args[argidx], output_size / input_size))
