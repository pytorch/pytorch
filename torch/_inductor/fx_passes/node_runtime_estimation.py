...
def _log_collective_estimations(
    collective_nodes: list[fx.Node],
    artifact_name: str,
) -> None:
    """Log NCCL and Inductor analytical estimations for collectives (no benchmarking required)."""
    headers = [
        "Collective Key",
        "NCCL Est(ms)",
        "Inductor Est(ms)",
    ...
    rows = []
    for coll_node in collective_nodes:
        key = _get_collective_key(coll_node)
        nccl_ms, inductor_ms = _get_collective_estimations(coll_node)
