...
def move_cpu_scalar_tensor_to_cuda(gm: torch.fx.GraphModule) -> None:
    ...
    we get the gpu_device, and move all cpu scalar tensors to gpu_device.
    """
    graph = gm.graph

    # mapping from cpu node to gpu node such that we don't duplicate
    # data movement
    ...
    for node in graph.nodes:
        if node.op == "call_function":
            # gpu_device specifies both device type and device index
        ...
        while queue:
            ...
            for user in node.users:
                ...
                if self.allow_cpu_device(user) and self.is_on_target_device(user):
                    ...
                    # this node takes only cpu scalar tensor or gpu tensor as input
                    # and output a gpu tensor. we can convert its cpu scakar input to gpu safely
                    del cpu_indeg[user]
                else:
