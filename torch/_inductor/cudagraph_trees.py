...
def _flatten_tensor_leaves(inputs: Any) -> list[Tensor]:
    """Return all tensor leaves from a possibly-nested input structure.

    Partition functions generated by Inductor can receive lists of tensors
    (e.g. while_loop carried state) alongside regular tensor arguments.
    A simple top-level ``isinstance(t, Tensor)`` check misses tensors nested
    inside lists/tuples.
    """
    # Materialize iterators (e.g. itertools.chain) so pytree can traverse them.
    if not isinstance(inputs, (list, tuple)):
        inputs = list(inputs)
    return [t for t in pytree.tree_leaves(inputs) if isinstance(t, Tensor)]
...
class CUDAWarmupNode:
    ...
    def run(self, new_inputs: Any) -> OutputType:
        ...
        # Collect weakrefs to non-pool input storages so add_ref can detect
        # pass-through outputs.  We must recurse into nested containers
        # (lists/tuples) because partition functions can receive carried state
        # from while_loop/scan as a list of tensors.
        non_cudagraph_inps_storages = [
            weakref.ref(t.untyped_storage())
            for t in _flatten_tensor_leaves(
...
class CUDAGraphNode:
    ...
    def __init__(
        ...
        self.reconstructed_inputs: list[InputType] = [
            self._reconstruct_input(x) for x in recording_inputs
        ]
    ...
    def _copy_inputs_and_remove_from_src(
        ...
        for idx in self.non_static_input_idx:
            ...
            if isinstance(src, torch.Tensor):
                ...
                dst_tensors.append(index_expanded_dims(dst, expanded_dims))  # type: ignore[arg-type]
                src_tensors.append(index_expanded_dims(src, expanded_dims))  # type: ignore[arg-type]
            else:
                flat_dst = pytree.tree_leaves(dst)
                flat_src = pytree.tree_leaves(src)
                for d, s in zip(flat_dst, flat_src):
                    if isinstance(s, torch.Tensor):
                        dst_tensors.append(d)
    ...
    def _record(self, model: ModelType, inputs: list[InputType]) -> OutputType:
        ...
        if config.triton.slow_path_cudagraph_asserts:
            ...
            memory += [
                ...
                for t in _flatten_tensor_leaves(inputs)
                if t.is_cuda
                and t.untyped_storage().data_ptr() not in static_ptrs
                and t.untyped_storage().data_ptr() != 0
            ]
    ...
    def _reconstruct_input(self, x: Any) -> Any:
        """Reconstruct a recording input, recursing into containers via pytree."""

        def _reconstruct_leaf(leaf: Any) -> Any:
            if isinstance(leaf, torch.Tensor):
                return self._reconstruct_from_tensor_metadata(
                    self._tensor_metadata(leaf)
                )
            return leaf

        return pytree.tree_map(_reconstruct_leaf, x)
    ...
    @staticmethod
    def _allocate_recording_input(inp: Any) -> Any:
        """Allocate pool storage for a single input, recursing into containers."""
        return pytree.tree_map(
            lambda x: static_input(x) if isinstance(x, torch.Tensor) else x, inp
        )
        ...
        with (
            ...
            for i, inp in enumerate(inputs):
                if isinstance(inp, torch.Tensor):
                    ...
                    recording_inputs.append(self._allocate_recording_input(inp))
                else:
                    recording_inputs.append(inp)
