...
@dataclass
class CKAddMMOperation:
    # The fields are same as CKGemmOperation unless specified otherwise
    a_layout: str
    b_layout: str
    ...
    a_lds_dtype: Optional[str] = None  # addmm specific
    b_lds_dtype: Optional[str] = None  # addmm specific
...
class CKAddMMTemplate(CKTemplate):
    ...
    def globals(self) -> IndentedBuffer:
        ...
        res.splice(
            """
                ...
                using ck::tensor_operation::element_wise::Add;
                ...
                struct ScaledAdd {
                    ...
                    operator()(E& e, const C& c, const D& d) const {
                        e = ck::type_convert<F16>(alpha * ck::type_convert<F32>(c) + beta * ck::type_convert<F32>(d));
                    }
...
"""
    ...
    def render(self, kernel: ROCmTemplateKernel, op: "CKGemmOperation", **kwargs) -> str:  # type: ignore[override]
        ...
        X, W, Bias = self.input_nodes
        Y = self.output_node
        addmm_op = CKAddMMOperation(**dict(op.dict_items()),
                                    ds_layouts=(torch_layout_to_ck_layout(Bias.get_layout()),),
                                    ds_element_dtypes=((self._TORCH_DTYPE_TO_CK[Bias.get_layout().dtype]),))
        addmm_op.c_elementwise_op = "ScaledAdd"
        addmm_op.c_shuffle_block_transfer_scalar_per_vector_n_per_block = (op.c_shuffle_block_transfer_scalar_per_vector_n_per_block,) * 2
        instance_definition, instance_type = self.emit_ck_instance(addmm_op)

        version_comment = rf"""/**
* Generated code for CK inductor backend
* See {type(self).__module__}.{type(self).__qualname__}
*
* Problem size M={X.get_layout().size[-2]} N={W.get_layout().size[-1]} K={X.get_layout().size[-1]} Bias={Bias.get_layout().size}
* Template instance {addmm_op}
*
* {torch.__version__=}
...
"""
        ...
        return self._template_from_string(self.gemm_template).render(
            ...
            K=kernel.size(X, -1),
            N=kernel.size(W, -1),
            a_elementwise_op=addmm_op.a_elementwise_op,
            b_elementwise_op=addmm_op.b_elementwise_op,
            c_elementwise_op=addmm_op.c_elementwise_op,
            a_element_dtype=addmm_op.a_element_dtype,
            b_element_dtype=addmm_op.b_element_dtype,
            c_element_dtype=addmm_op.c_element_dtype,
            bias_element_dtype=addmm_op.ds_element_dtypes[0],
            a_stride=kernel.stride(X, -2),
            b_stride=kernel.stride(W, -2),
            c_stride=kernel.stride(Y, -2),
            d_stride=kernel.stride(Bias, -2),
            alpha=self.alpha,
            beta=self.beta,
            null_checks="".join(kernel.check_not_null(node) for node in (X, W, Y, Bias)),
