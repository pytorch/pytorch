...
class CppWrapperGpu(CppWrapperCpu):
    ...
    def write_get_raw_stream(self, device_idx: int, graph_name: str) -> str:
        ...
        # The JIT stream is declared at the entry function prologue (see
        # _codegen_entry_impl_prologue) so it stays in scope across every kernel
        # call site. Declaring it here would put it inside whichever profiling
        # block contained the first call, and the calls after that block would
        # name a variable that no longer exists.
