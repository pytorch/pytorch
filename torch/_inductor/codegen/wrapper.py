...
class PythonWrapperCodegen(CodeGen):
    ...
    def define_user_defined_triton_kernel(
        ...
        if len(grids) == 1:
            ...
            # Pad grid to 3 dimensions (Triton always uses 3D grids)
            grid = [*map(sympy.sympify, grids[0])]
            while len(grid) < 3:
                grid.append(sympy.Integer(1))
            extra_launcher_call_args = grid
        else:
        ...
        if (
            ...
            for i, (arg, arg_type, raw_key, raw_arg) in enumerate(
                ...
                if triton_input:
                    ...
                    raw_key == ""
                    and not isinstance(raw_arg, (int, sympy.Integer))
                    and infer_arg_by_inputs(raw_keys, raw_args, i, reused_args)
                ):
                    # Empty raw_key means this is a arg that's not native to the triton kernel,
                    # and is being added by inductor. Skip inference for literal constants.
