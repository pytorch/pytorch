    ...
    def define_user_defined_triton_kernel(
        ...
        if autotune_keys:
            ...
            if epilogue_fusion is not None:
                # Include the modified Triton source produced by epilogue fusion.
                kernel_cache_key = hashlib.sha256(
            ...
            if triton_version_uses_attrs_dict():
                ...
                declared_constexpr_names = OrderedSet()
            user_autotune_arg_names = tuple(
                name
                for name in triton_signature
                if name not in declared_constexpr_names
            )
            inductor_meta["user_autotune_keys"] = tuple(autotune_keys)
            inductor_meta["user_autotune_arg_names"] = user_autotune_arg_names
