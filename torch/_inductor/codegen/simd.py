...
class SIMDScheduling(BaseScheduling):
    ...
    def codegen_template(
        ...
        # For multi-kernel dispatch, create multiple kernel variants
        if hasattr(template_node.node, '_multi_kernel_renders') and template_node.node._multi_kernel_renders:
            kernels = []
            src_codes = []
            
            # Create kernel for each hint and default
            for hint_override, make_kernel_render in template_node.node._multi_kernel_renders.items():
            ...
            if only_gen_src_code:
                # Return combined source codes for multi-kernel
                return "\n\n".join(src_codes)
            else:
                # Create MultiKernel wrapper for runtime dispatch
                if len(kernels) > 1:
                    MultiKernel.merge_workspaces_inplace(kernels)
