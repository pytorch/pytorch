...
def register_backend_for_device(
    ...
    if device_compile_options:
        ...
        for name, key in device_compile_options.items():
            ...
            if normalized in config._config:
                raise AssertionError(
...
def patch_compile_options(
    ...
    for name, value in (config_patches or {}).items():
        owner, key = get_compile_option_owner(name) or (config, name)
        grouped.setdefault(owner, {})[key] = value
    routed = {owner.__name__: values for owner, values in grouped.items() if owner is not config}
