...
class OpDecompositions:
    ...
    def _chbevl(x: OpVarT, coefficients: tuple[float, ...]) -> OpVarT:
        ...
        for coefficient in coefficients[1:]:
            ...
            b0 = ops.add(b0, ops.constant(coefficient, torch.float32))
        return ops.mul(
            ops.constant(0.5, torch.float32),
            ops.sub(b0, b2),
        )
    ...
    def i0(x: OpVarT) -> OpVarT:
        ...
        small = ops.mul(ops.exp(abs_x), OpDecompositions._chbevl(small_z, a))
        ...
        result = ops.where(
            ops.le(abs_x, ops.constant(8.0, torch.float32)), small, large
        )
    ...
    def i1(x: OpVarT) -> OpVarT:
        ...
        result = ops.where(
            ops.lt(x, ops.constant(0.0, torch.float32)),
            ops.neg(magnitude),
            magnitude,
        )
