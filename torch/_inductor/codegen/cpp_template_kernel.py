...
class CppTemplateKernel(CppKernel):
    ...
    def index(self, node: ir.Buffer, indices: List[Any]) -> str:
        indexer = node.layout.as_fixed().make_indexer()  # type: ignore[union-attr]  # type: ignore[union-attr]
    ...
    def store_pointwise_nodes(
        ...
        output_index = dst.get_layout().make_indexer()(var_ranges.keys())  # type: ignore[arg-type]  # type: ignore[arg-type]
        ...
        for i, node in enumerate(nodes):
            ...
            def fn(*args):
                ...
                V.ops.store(
                    output_name,
                    output_index,  # type: ignore[arg-type]  # type: ignore[arg-type]
    ...
    def output_node(self) -> ir.TensorBox:
        return ir.TensorBox.create(
            ir.CppTemplateBuffer(
                layout=self.layout,  # type: ignore[arg-type]  # type: ignore[arg-type]
