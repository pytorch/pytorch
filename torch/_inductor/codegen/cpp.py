...
class CppTileKernel(CppKernel):
    ...
    def __enter__(self):
        ...
        class Wrapper:
            ...
            def writeline(self, line):
                ...
                self.wrapped.writeline(
                    V.kernel.current_slice_store_name, compute_at, line
                )
            ...
            def writelines(self, lines):
                ...
                self.wrapped.writelines(
                    V.kernel.current_slice_store_name, compute_at, lines
                )
        ...
        self.exit_stack.enter_context(
            self.swap_buffers(
                Wrapper(self.tile_loads),
                cb=Wrapper(self.tile_compute),
                sb=DeferredWrapper(self.tile_stores),
            )
        )
    ...
    def __exit__(self, *args):
        ...
        with patch.object(self.code, "prefix", lambda: prefix_override(self.code)):
            ...
            self.tile_loads.lines = [
                line
                for line in self.tile_loads.lines
                if line.name is None or line.name in self.tile_bufs_used
            ]
            self.tile_compute.lines = [
                line
                for line in self.tile_compute.lines
                if line.name is None or line.name in self.tile_bufs_used
            ]
            ...
            self.tile_loads.lines.sort(
                reverse=False, key=lambda line: len(line.compute_at)
            )
            self.tile_stores.lines.sort(
                reverse=True, key=lambda line: len(line.compute_at)
            )

            # Generate inner loops
            loop_indices = []
            for line in itertools.chain(
                self.tile_loads.lines, self.tile_compute.lines, self.tile_stores.lines
            ):
