...
class CppScheduling(BaseScheduling):
    ...
    def _can_fuse_horizontal_impl(self, node1, node2):
        assert isinstance(node1, (FusedSchedulerNode, SchedulerNode, ExternKernelSchedulerNode))
        assert isinstance(node2, (FusedSchedulerNode, SchedulerNode))
        if any(
            isinstance(node, (OuterLoopFusedSchedulerNode, ExternKernelSchedulerNode)) for node in (node1, node2)
        ):
            return False
