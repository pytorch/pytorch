...
class CppKernelDispatcher(CppKernel):
    ...
    def gen_if_conditions(self, conditions):
        if len(conditions) == 1:
            return f"if({self.if_wrapper}({conditions[0]}))"
        elif len(conditions) == 2:
            return f"if({self.if_wrapper}({conditions[0]} && {conditions[1]}))"
            ...
            if split_outer_reduction_inner:
                ...
                if self.vec_kernel:
                    with contextlib.ExitStack() as stack:
                        if split_outer_reduction_inner:
                            buf.writeline(
                                self.gen_if_conditions(
                                    [f"{outer_var} < {cexpr_index(tiling_offset)}"]
                                )
                            )
