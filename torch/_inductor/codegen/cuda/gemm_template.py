...
class CUTLASSGemmTemplate(CUTLASSTemplate):
    ...
    def __init__(
        ...
        assert self._are_inputs_layout_compatible(
            [node.get_layout() for node in input_nodes]  # type: ignore[misc]  # type: ignore[misc]
        )
    ...
    def fix_op_layout(
        ...
        match_list = [
            CUTLASSGemmTemplate.layout_match(buf.get_layout(), op_layout)  # type: ignore[arg-type]  # type: ignore[arg-type]
        ...
        if a_layout is not None:
            new_op.A.layout = CUTLASSGemmTemplate.cutlass_layout(a_layout)  # type: ignore[arg-type]  # type: ignore[arg-type]
        if b_layout is not None:
            new_op.B.layout = CUTLASSGemmTemplate.cutlass_layout(b_layout)  # type: ignore[arg-type]  # type: ignore[arg-type]
        if c_layout is not None:
            new_op.C.layout = CUTLASSGemmTemplate.cutlass_layout(c_layout)  # type: ignore[arg-type]  # type: ignore[arg-type]
            new_op.C.element = cutlass_utils.torch_dtype_to_cutlass_type(c_layout.dtype)
        if d_layout is not None:
            new_op.D.layout = CUTLASSGemmTemplate.cutlass_layout(d_layout)  # type: ignore[arg-type]  # type: ignore[arg-type]
        return new_op
    ...
    def filter_op(
        ...
        if not (
            self.layout_match(X.get_layout(), op.A.layout)  # type: ignore[arg-type]  # type: ignore[arg-type]
            and self.layout_match(W.get_layout(), op.B.layout)  # type: ignore[arg-type]  # type: ignore[arg-type]
        ):
            return None
        ...
        # Set output layout.
        op.D.layout = CUTLASSGemmTemplate.cutlass_layout(self.output_node.get_layout())  # type: ignore[arg-type]  # type: ignore[arg-type]
        ...
        if len(self.input_nodes) >= 3 and self.input_nodes[2] is not None:
            ...
            bias_layout = CUTLASSGemmTemplate.cutlass_layout(Bias.get_layout())  # type: ignore[arg-type]  # type: ignore[arg-type]
            if op.gemm_kind != cutlass_lib.GemmKind.Universal3x:
    ...
    def render_gemm_arguments(
        ...
        if should_swap_xw:
            ...
            def clone_with_transposed_stride(node: IRNode) -> IRNode:
                ...
                new_stride = list(old_layout.stride)  # type: ignore[union-attr]  # type: ignore[union-attr]
                new_stride[-2], new_stride[-1] = new_stride[-1], new_stride[-2]
                new_layout = FixedLayout(
                    old_layout.device,
                    old_layout.dtype,
                    list(old_layout.size),  # type: ignore[union-attr]  # type: ignore[union-attr]
                    new_stride,
                    old_layout.offset,  # type: ignore[union-attr]  # type: ignore[union-attr]  # type: ignore[union-attr]  # type: ignore[union-attr]
                )
