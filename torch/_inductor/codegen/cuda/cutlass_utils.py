...
XW_DTYPES: OrderedSet[torch.dtype] = OrderedSet(
    [torch.half, torch.bfloat16, torch.float8_e4m3fn, torch.float8_e5m2, torch.int8]
)
...
def get_accumulator_dtype(
    ...
    if torch_dtype in (torch.float16, torch.bfloat16, torch.float, torch.float8_e4m3fn, torch.float8_e5m2):
...
def get_alignments(torch_dtype: torch.dtype) -> list[int]:
    ...
    if torch_dtype in (torch.half, torch.bfloat16):
        ...
        return [4, 2, 1]
    elif torch_dtype in (torch.uint8, torch.int8, torch.float8_e4m3fn, torch.float8_e5m2):
        return [16, 8, 4, 2]
    elif torch_dtype == torch.int32:
        return [4, 2, 1]
    else:
