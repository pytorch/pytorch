...
class CUDATemplate(KernelTemplate):
    ...
    def generate_input_layout_combinations(self, input_nodes) -> List[List[TensorMeta]]:
        ...
        return all_variant_combinations

    def generate_variants_after_fusion(
        self,
        template_buffer: CUDATemplateBuffer,
        epilogue_nodes: List[IRNode],
        kwarg_override_variants: Sequence[Dict],
    ) -> Generator[CUDATemplateCaller, None, None]:
        """
        Generates variants of the given CUDATemplateBuffer after fusion with the given epilogue nodes. May be used to determine the best configuration for a fused kernel, which may differ from
        ...
        Args:
            ...
            kwarg_override_variants (Sequence[Dict]): A sequence of keyword argument overrides to use for generating the variants. Will typically be used to override the "op" argument for CUTLASSGemmTemplates and provide a sequence of ops to try.
        """
