...
def rewrite_index_for_function(
    ...
    indices_to_keep = [
        ...
        for idx in range(len(local_buf.get_layout().size))  # type: ignore[union-attr]  # type: ignore[union-attr]
    ]
...
def rewrite_index_for_nodes(
    ...
    index = local_buf.layout.make_indexer()(index_vars)  # type: ignore[arg-type]
    return index
