            ...
            for arg in numel_args:
                if isinstance(arg, int):
                    ...
                    hint = V.graph.sizevars.size_hint(
                        arg.inner_expr, hint_override=self.hint_override, fallback=config.unbacked_symint_fallback
                    )
                    args.append(str(hint))
                elif isinstance(arg, sympy.Expr):
                    hint = V.graph.sizevars.size_hint(
                        arg, hint_override=self.hint_override,fallback=config.unbacked_symint_fallback
                    )
                    args.append(str(hint))
                else:
