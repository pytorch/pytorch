...
from .codegen_device_driver import xpu_kernel_driver, xpu_kernel_header
...
class CppWrapperXpu(CppWrapperCpu):
    """
    ...
    def generate_load_kernel_once(
        ...
        if V.graph.aot_mode:
            ...
            self.writeline(
                f"""    {name} = loadKernel({stream}.queue(), "{cubin_path}", "{mangled_name}", {shared_mem});"""
            )
            self.writeline("}")
                    ...
                    if config.abi_compatible:
                        ...
                        self.writeline(
                            maybe_hipify_code_wrapper(
                                f"void* {var_name} = reinterpret_cast<void*>({arg}.data_ptr());"
                            )
                        )
    ...
    def generate_kernel_call(
        ...
        self.generate_load_kernel_once(
            name, mangled_name, cubin_path, shared_mem, V.graph, device_index
        )
        ...
        self.writeline(f"std::vector<void*> {kernel_args_var} = {{{call_args}}};")
        stream = (
            "stream"
            if V.graph.aot_mode
            else self.write_get_raw_stream(device_index, V.graph)
