...
{%- set kernel_args = {"X": X, "W": W, "scale": scale} %}
        ...
        for (int64_t mc = m_block_start; mc < m_block_end; mc += Mc_blocks) {
            ...
            for (int64_t nc = n_block_start; nc < n_block_end; ++nc) {
                ...
                for (int64_t kc = k_block_start; kc < k_block_end; kc += Kc_blocks) {
                    ...
                    {%- set tile_W_3d = kernel.slice_nd(W, [("nc", "nc + 1"), ("k_start", "k_end"), ()]) %}
                    {%- set tile_W = kernel.view(tile_W_3d, ["k_end - k_start", micro_gemm.register_blocking.block_n]) %}
                    {%- set tile_scale = kernel.slice_nd(scale, [("n_start", "n_start + N0")]) %}
                    if (kc == k_block_start) {
                        {{ micro_gemm.codegen_call(kernel, tile_X, tile_W, tile_scale, acc, accum=False)|indent(24, false) }}
                    } else {
                        {{ micro_gemm.codegen_call(kernel, tile_X, tile_W, tile_scale, acc, accum=True)|indent(24, false) }}
                    }
                }
                {%- set tile_Y = kernel.slice_nd(Y_2d, [("m_start", "m_end"), ("n_start", "n_start + N0")]) %}
                {{ kernel.store_output(
...
class CppPackedWoQGemmTemplate(CppTemplate):
    def __init__(
        self,
        input_nodes,
        layout: ir.Layout,
    ...
    def add_choices(
        ...
        num_threads = parallel_num_threads()
        new_inputs, _ = normalize_shapes(
            *maybe_to_dense([input_nodes[idx] for idx in input_indices], layout)
        )
        m, n, k, *_ = mm_args(
            new_inputs[0], new_inputs[1], new_inputs[2], is_woq_gemm=True
        )
        output_dtype, compute_dtype = new_inputs[0].get_dtype(), torch.float32
        micro_gemm = create_micro_gemm(
            "micro_gemm",
            m,
            n,
        ...
        def pack_weight(inputs, layout_or_out):
            ...
            if isinstance(W, ir.IRNode):
                ...
                n, k = list(W.shape)
                blocked_w = (
                    W.reshape(n // block_n, block_n, k).permute([0, 2, 1]).contiguous()
                )
                if micro_gemm.get_b_layout() != LayoutType.NORMAL:
                    layout_str = (
                        "VNNI4"
        ...
        def preprocessor(inputs, layout):
            return pack_weight(
                *normalize_shapes(
                    *maybe_to_dense([inputs[idx] for idx in input_indices], layout)
                )
            )
    ...
    def render(  # type: ignore[override]
        ...
        **kwargs,
        ...
        X, W, S = self.input_nodes[0], self.input_nodes[1], self.input_nodes[2]
        Y = self.output_node
        ...
        output_dtype, compute_dtype = X.get_dtype(), torch.float32
        micro_gemm = create_micro_gemm(
            f"{kernel.kernel_name}_micro_gemm",
            self.m,
            self.n,
        ...
        assert micro_gemm is not None
        assert isinstance(micro_gemm, CppMicroGemmWoQGEMM)
        assert self.register_blocking == micro_gemm.register_blocking
        if isinstance(micro_gemm, CppMicroGemmAMX):
            counters["inductor"]["cpp_micro_gemm_amx_counter"] += 1
        ...
        return self._template_from_string(WOQ_GEMM_TEMPLATE).render(**options)
