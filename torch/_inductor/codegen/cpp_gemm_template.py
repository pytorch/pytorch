...
def gen_2d_view_of_epilogue_buf(
    ...
    if (
        ...
        def get_reindexer(epilogue_node, default_reindexer=None):
            ...
            stride_order = list(
                ir.get_stride_order(
                    V.graph.sizevars.size_hints(epilogue_node.get_stride(), fallback=1)
                )
            )
        ...
        def normalize_shapes(inputs, layout_or_out):
            ...
            if not is_mkldnn_wgt and isinstance(new_inputs[1], torch.Tensor):
                if has_free_symbols(view_size):
                    ...
                    view_size[:] = V.graph.sizevars.size_hints(view_size, fallback=1)
                    view_stride[:] = V.graph.sizevars.size_hints(view_stride, fallback=1)
