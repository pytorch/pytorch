...
class CppWrapperCpu(PythonWrapperCodegen):
    ...
    def codegen_dynamic_select_index(self, node):
        ...
        index_compute_str = f"{index_cpp_str} < 0 ? {index_cpp_str} + {self.val_to_arg_str_for_prim_type(node.size, int)}:  {index_cpp_str}"
        self.writeline(
            f"auto {node.unbacked_offset_symbol} = {self.val_to_arg_str_for_prim_type(node.base_offset, int)} + {self.val_to_arg_str_for_prim_type(node.base_dim_stride, int)} * ({index_compute_str});"
        )
        # record in unbacked_symbol_decls so we won't generate a declaration of the symbol again
