...
from ..runtime_utils import conditional_product, last_power_of_2, next_power_of_2
from ..triton_compat import Config as TritonConfig, triton
from ...utils import prefix_is_reduction

Config = cast(Any, TritonConfig)

if TYPE_CHECKING:
    from ..triton_heuristics import InductorConfig
from ...utils import prefix_is_reduction
...
def triton_config(*args, **kwargs):
    from ..triton_heuristics import triton_config as _triton_config

    return _triton_config(*args, **kwargs)
...
def _handle_combo_kernel_per_subkernel_blocks(
    ...
    from ..triton_heuristics import pointwise, reduction, persistent_reduction
...
def triton_config_tiled_reduction(
    ...
    from ..triton_heuristics import _num_warps, check_config, check_max_block
...
def make_matmul_triton_config(sizes: dict[str, int], num_warps: int, num_stages: int):
    from ..triton_heuristics import check_max_block
...
def filter_reduction_configs_for_determinism(
    ...
    """
    from ..triton_heuristics import unique_configs
...
def cooperative_reduction_impl(
    size_hints,
    reduction_hint,
    triton_meta,
    filename,
