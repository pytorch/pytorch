...
Params: TypeAlias = tuple[tuple[Any], dict[str, Any]]
...
class _CacheIntf(ABC):
    ...
    def get(
        ...
        with self.lock():  # type: ignore[call-arg]
            result: impls.Hit | None = self._get(fn, params, ischema=ischema, custom_params_encoder=custom_params_encoder, custom_result_decoder=custom_result_decoder)
...
class _DeterministicCacheIntf(_CacheIntf):
    def __init__(self) -> None:
        ...
        if config.STRICTLY_PRE_POPULATED_DETERMINISM:
            # we'll never need a syncronization cache if we're in strictly pre-populated mode,
            ...
            # consistency as the syncronization cache
            ...
            # local determinism can use the on-disk cache as the syncronization cache,
    ...
    def _dump_imc_to_disk(self) -> Path | None:
        with self.lock():  # type: ignore[call-arg]
            ...
            odc: impls._OnDiskCacheImpl = impls._OnDiskCacheImpl(sub_dir=Path("dcache_dump"))
    ...
    def _get(
        ...
        key: Any = self._make_key(fn, params, ischema=ischema, custom_params_encoder=custom_params_encoder)
        sc: impls._OnDiskCacheImpl | impls._RemoteCacheImpl | None = self._get_sc_from_callee(fn.__name__)
