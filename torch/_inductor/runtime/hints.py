...
class TritonConstexprStruct:
    ...
    NamedTuples stringify as ``MyType(field=...)``, which is not defined in the
    generated module and raises ``NameError`` on import. This type is imported
    into those modules (see ``gen_common_triton_imports``) and preserves both
    attribute and index access so Triton's constexpr specialization matches
    eager NamedTuple field access.

    See https://github.com/pytorch/pytorch/issues/192288.
    """

    def __init__(self, **fields: typing.Any) -> None:
        # Preserve declaration order for stable repr / indexing.
        object.__setattr__(self, "_fields", tuple(fields.keys()))
    ...
    def __repr__(self) -> str:
        ...
        return f"TritonConstexprStruct({body})"
    ...
    def __eq__(self, other: object) -> bool:
        if isinstance(other, TritonConstexprStruct):
...
def serialize_triton_constexpr(value: typing.Any) -> typing.Any:
    """Make a constexpr value safe to embed via ``repr`` in generated modules.

    Recursively replaces NamedTuple instances with
    :class:`TritonConstexprStruct`. Plain tuples/lists/dicts are walked so
    nested NamedTuples are rewritten; other values are returned unchanged.
    """
    if _is_namedtuple_instance(value):
        return TritonConstexprStruct(
            **{
                name: serialize_triton_constexpr(getattr(value, name))
                for name in type(value)._fields
            }
        )
    if isinstance(value, list):
        return [serialize_triton_constexpr(v) for v in value]
    if isinstance(value, tuple):
        return tuple(serialize_triton_constexpr(v) for v in value)
    if isinstance(value, dict):
        return {k: serialize_triton_constexpr(v) for k, v in value.items()}
    return value
