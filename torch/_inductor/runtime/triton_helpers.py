...
@triton.jit
def powf_cuda(base, exp):
    """
    Compute pow(base, exp) using CUDA's exact algorithm via inline PTX.

    This achieves bitwise parity with CUDA's powf by using non-FTZ (flush-to-zero)
    instructions. Triton's libdevice.pow uses FTZ instructions which cause 1-3 ULP
    differences compared to eager PyTorch (which uses CUDA's powf).

    Handles positive base values. Returns correct result for:
    - exp == 0 -> 1.0
    - base == 1.0 -> 1.0
    - General positive base and finite exp
    """
    # fmt: off
    result = tl.inline_asm_elementwise(
        asm="""
        {
            ...
            mov.f32 base_in, $1;
            mov.f32 exp_in, $2;
            mov.f32 result_out, 0f3F800000;

            setp.eq.f32 p7, exp_in, 0f00000000;
            setp.eq.f32 p6, base_in, 0f3F800000;
            or.pred p8, p6, p7;
            @p8 bra DONE;
            ...
            selp.f32 f17, f16, f4, p2;
            selp.f32 f18, 0fC1C00000, 0f00000000, p2;

            mov.b32 r6, f17;
            add.s32 r7, r6, 0xC0B33333;
            and.b32 r8, r7, 0xFF800000;
            sub.s32 r9, r6, r8;
            mov.b32 f19, r9;
            ...
            mul.rn.f32 f57, f52, exp_in;
            neg.f32 f58, f57;
            fma.rn.f32 f59, f52, exp_in, f58;
            fma.rn.f32 f60, f56, exp_in, f59;
            ...
            selp.s32 r11, 0, 0x83000000, p3;
            add.s32 r12, r11, 0x7F000000;
            mov.b32 f76, r12;
            mul.f32 f77, f75, f76;
            shl.b32 r13, r10, 23;
            sub.s32 r14, r13, r11;
            ...
            selp.f32 result_out, 0f00000000, 0f7F800000, p5;
            selp.f32 result_out, result_out, f79, p4;

        DONE:
            mov.f32 $0, result_out;
        }
        """,
        constraints="=f,f,f",
        args=[base, exp],
