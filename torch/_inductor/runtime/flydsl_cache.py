...
def run_cached_flydsl(
    ...
    *compile_args: Any,
    ...
    """Cache a layout-dynamic FlyDSL dispatcher by device and constexpr param."""
    if compile_args and compile_args_factory is not None:
        raise ValueError(
            "pass either compile_args or compile_args_factory, not both"
        )
    ...
    with _compiled_cache_lock:
        ...
        compiled = compiled_cache.get(cache_key)
        if compiled is None:
            resolved_compile_args = (
                compile_args_factory()
                if compile_args_factory is not None
                else compile_args
            )
            compiled = compiler(jit_func, *resolved_compile_args)
...
def ensure_flydsl_cache_dir() -> str:
    """Route FlyDSL's disk cache through TorchInductor's cache root."""
