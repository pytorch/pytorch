...
# Per-state dispatch budget for incremental autotuning. A torch.compile option
# overrides this default for every kernel generated by that compilation. A
# malformed env var falls back to the default instead of breaking import.
try:
    incremental_autotune_max_dispatches = max(
        1, int(os.getenv("TORCHINDUCTOR_INCREMENTAL_AUTOTUNE_MAX_DISPATCHES", 200))
    )
except ValueError:
    incremental_autotune_max_dispatches = 200
