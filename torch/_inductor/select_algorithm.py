...
# very loose correctness checks by default, needed due to tf32
VERIFY = dict(atol=1, rtol=0.05)
PRINT_AUTOTUNE = True
...
class TritonTemplateKernel(TritonKernel):
    ...
    def def_kernel(self, *argnames):
        ...
        # TODO(jansel): apply offsets here, hide real variable names
        self.named_input_nodes.clear()
...
class TritonTemplate:
    ...
    def generate(self, input_nodes, layout, num_stages, num_warps, **kwargs):
        defines = StringIO()
...
class AlgorithmSelectorCache(DiskCache):
    ...
    def make_benchmark_fn(
        ...
        if VERIFY:
            choices[0].to_callable()(*example_inputs_extern, out=out)
            expected = out.clone()

        def benchmark(algo, is_extern):
            out.zero_()
            if is_extern:
                result = do_bench(lambda: algo(*example_inputs_extern, out=out))
            else:
                result = do_bench(lambda: algo(*example_inputs, out=out))
            if VERIFY:
                torch.testing.assert_close(out, expected, **VERIFY)
