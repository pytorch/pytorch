...
Where the boundaries go, NOT whether a node is safe to capture. Capture safety
lives in cudagraph_capture.py and is shared with whole-graph mode; the
predicates here are the extra reasons a REGIONAL partition needs an edge: a
partition carries at most one dynamic symbol, and a buffer's writers and readers
cannot straddle its boundary. That is why gathers, shared-storage concats and
batch-dim transitions appear here rather than there.

Also holds the agglomerative convex clustering of the scheduler node order and
the cross-partition handoff/scratch slab planning. Kept free of
...
def plan_cudagraph_handoff_slab() -> None:
    ...
    # _codegen_create_cudagraph_cached and cudagraph_runtime.h encode()). With a
...
def _freeze_nbytes_to_const(sym_nbytes: Any) -> int | None:
    """Freeze a (possibly symbolic) byte count to its constant finite upper
    bound so a scratch allocation packs at a CONSTANT offset (buffers of ANY
    dynamic symbol then share ONE pool). Returns None if a contributing symbol
    has no finite upper bound -- the caller then leaves that buffer on the
    per-partition path. Mirrors (and MUST stay in sync with) the single-max-slab
    freeze in AllocationPool._codegen_create_cudagraph_cached; duplicated here
    rather than shared so that baseline (flag-off) path stays zero-touch.
    """
    import math
    ...
    from .utils import align, ALIGN_BYTES

    if isinstance(sym_nbytes, (int, sympy.Integer)):
        return int(sym_nbytes)
    if not isinstance(sym_nbytes, sympy.Expr):
        return None
    var_to_range = V.graph.sizevars.shape_env.var_to_range
    expr = sym_nbytes.replace(align, lambda e: e + (ALIGN_BYTES - 1))
...
# Reserved slab-cache id for the single shared scratch pool. Sits one past the
# handoff pool id range [1<<22 .. (1<<22)+(1<<17)) so it never aliases a handoff
# slab, stays < 1<<23 so (id << 40) < 2**63 (positive signed int64 cache key --
# see _codegen_create_cudagraph_cached), and is above the 1<<20 legacy hash
# bucket and the small ordinary pool ids. One pool holds all scratch, so no
# per-index offset is needed.
_CUDAGRAPH_SCRATCH_POOL_ID = (1 << 22) + (1 << 17)
_CUDAGRAPH_SCRATCH_POOL_NAME = "cudagraph_scratch_pool"


def plan_cudagraph_scratch_slab() -> None:
    """Pack intra-partition SCRATCH of captured partitions into ONE outer-owned
    slab shared across partitions.

    Companion to plan_cudagraph_handoff_slab, gated additionally on
    config.aot_inductor.cudagraph_cross_partition_memory. A scratch buffer is
    produced and dead within one captured partition (the scheduler pre-pass
    publishes V.graph._cudagraph_scratch_liveness[name] = pid). Captured
    partitions replay sequentially, so scratch of partition P is dead before P+1
    runs and partitions can share its bytes.

    Each buffer gets LiveRange(pid, pid+1): all scratch of one partition overlaps
    (disjoint slots), scratch of different partitions is disjoint in pid time
    (shares slots). Pool size -> max over pid of that partition's scratch working
    set, instead of the SUM of per-partition slabs.

    Offsets are FROZEN to constants (each nbytes -> its finite upper bound) so a
    buffer of ANY dynamic symbol packs into the same pool: a producer body has
    only its single symbol in scope, so a symbolic cross-symbol offset would be
    out of scope, but a constant offset is always in scope. Buffers whose nbytes
    has no finite bound are skipped (left on the per-partition path).

    Publishes V.graph._cudagraph_scratch_offsets[name] = (pool_name, offset) for
    AllocFromPoolLine.codegen and appends the pool to
    V.graph._cudagraph_handoff_pools so the outer wrapper emits its slab once.
    """
    if not _cudagraph_handoff_slab_planning_enabled():
        return
    if not config.aot_inductor.cudagraph_cross_partition_memory:
        return

    from .codegen.memory_planning import Allocation, BufferGroup, LiveRange

    scratch_liveness: dict[str, int] = (
        getattr(V.graph, "_cudagraph_scratch_liveness", None) or {}
    )
    if not scratch_liveness:
        V.graph._cudagraph_scratch_offsets = {}
        return

    named_allocations: list[tuple[str, Allocation]] = []
    for name, pid in scratch_liveness.items():
        node = _resolve_handoff_buffer(name)
        if node is None:
            continue
        frozen = _freeze_nbytes_to_const(BufferGroup(node).sym_nbytes())
        if frozen is None:
            continue
        named_allocations.append(
            (
                name,
                Allocation(
                    node,
                    LiveRange(pid, pid + 1),
                    size_hint=frozen,
                    symbolic_size=sympy.Integer(frozen),
                ),
            )
        )

    if not named_allocations:
        V.graph._cudagraph_scratch_offsets = {}
        return

    pool = _pack_handoff_pool(
        _CUDAGRAPH_SCRATCH_POOL_NAME, named_allocations, _CUDAGRAPH_SCRATCH_POOL_ID
    )
    scratch_offsets: dict[str, tuple[str, Any]] = {}
    for name, allocation in named_allocations:
        offset = allocation.offset
        if offset is None:
            raise AssertionError("scratch allocation not finalized")
        if isinstance(offset, sympy.Expr) and any(
            isinstance(s, sympy.Symbol) for s in offset.free_symbols
        ):
            # Defensive: a frozen-constant pack must yield symbol-free offsets;
            # anything else would emit an out-of-scope symbol in a producer body.
            continue
        scratch_offsets[name] = (_CUDAGRAPH_SCRATCH_POOL_NAME, offset)
    V.graph._cudagraph_scratch_offsets = scratch_offsets

    handoff_pools = getattr(V.graph, "_cudagraph_handoff_pools", None) or {}
    handoff_pools[_CUDAGRAPH_SCRATCH_POOL_NAME] = pool
    V.graph._cudagraph_handoff_pools = handoff_pools
