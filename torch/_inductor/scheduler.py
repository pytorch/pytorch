...
class ForeachKernelSchedulerNode(FusedSchedulerNode):
    ...
    def fuse(cls, producer, consumer):
        ...
        return cls(producer.scheduler, fused_nodes, prev_node_1, prev_node_2)  # type: ignore[possibly-undefined]  # TODO: audit
    ...
    def codegen(self):
        for node in self.nodes:
            ...
            if node.is_template():
                ...
                self.get_backend(device).codegen_template(node, epilogue)  # type: ignore[possibly-undefined]  # TODO: audit
            elif node.is_extern():
                self.codegen_extern_call(node)
            elif node.is_foreach():
                self.get_backend(device).codegen_foreach(node)  # type: ignore[possibly-undefined]  # TODO: audit
            elif isinstance(node, (FusedSchedulerNode, SchedulerNode)):
                self.get_backend(device).codegen_nodes(node.get_nodes())  # type: ignore[possibly-undefined]  # TODO: audit
            else:
            ...
            if config.triton.debug_sync_kernel:
                self.get_backend(device).codegen_sync()  # type: ignore[possibly-undefined]  # TODO: audit
