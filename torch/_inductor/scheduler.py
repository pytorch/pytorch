...
class FusionMemoryContext:
    ...
    peak_regression_cache: dict[tuple[int, int, int], bool] = dataclasses.field(
        default_factory=dict
    )
    peak_span_cache: dict[tuple[int, int, int], bool] = dataclasses.field(
        default_factory=dict
    )
...
class Scheduler:
        ...
        for node1, node2 in possible_fusion_pairs:
            ...
            if self.can_fuse(
                ...
                if peak_allowed_increase is not None:
                    ...
                    extends_peak = False
                    if not regresses_peak:
                        extends_peak = self.fusion_extends_large_output_across_peak(
                            node1, node2, peak_allowed_increase
                        )
                    if debug:
                        ctx.timings["guard_s"] += time.perf_counter() - guard_start
                    if regresses_peak or extends_peak:
                        continue
    ...
    def _log_fusion_memory_timeline_stats(
        ...
        print(
            ...
            f"reg_cache={stats['regression_cache_hits']:.0f} "
            ...
            f"span_calls={stats['span_calls']:.0f} "
            f"span_cache={stats['span_cache_hits']:.0f} "
            f"span_rejects={stats['span_rejects']:.0f} "
            f"span_time={timings['span_s']:.3f}s "
    ...
    def fusion_regresses_estimated_peak_memory(
        ...
        if node1 not in ctx.node_set or node2 not in ctx.node_set:
            return finish(False)
        cache_key = (id(node1), id(node2), peak_allowed_increase)
        if cache_key in ctx.peak_regression_cache:
            if debug:
                ctx.stats["regression_cache_hits"] += 1
            return finish(ctx.peak_regression_cache[cache_key])
        ...
        if region_end - region_start > max_exact_span:
            ...
            if not peak_steps:
                ctx.peak_regression_cache[cache_key] = False
                return finish(False)
        ...
        if debug:
            ctx.timings["regression_exact_fuse_s"] += time.perf_counter() - fuse_start
    ...
    def fusion_extends_large_output_across_peak(
        ...
        debug = _debug_fusion_memory_timeline()
        if debug:
            ctx.stats["span_calls"] += 1
            span_start = time.perf_counter()

            def finish(result: bool) -> bool:
                ctx.timings["span_s"] += time.perf_counter() - span_start
                return result
        ...
        if cache_key in ctx.peak_span_cache:
            if debug:
                ctx.stats["span_cache_hits"] += 1
            return finish(ctx.peak_span_cache[cache_key])
