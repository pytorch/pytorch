...
class Scheduler:
    ...
    def should_partition(self, node: BaseSchedulerNode) -> str | None:
        ...
        if config.aot_inductor.cudagraph_mode == "regional":
            if reason := cudagraph_partition._is_batch_dim_transition(node):
                return reason
            if cudagraph_capture._node_uses_rng(node):
                return "RNG ops (philox seed/offset frozen under AOTI cuda graph)"
            if cudagraph_capture._node_uses_fp8_gemm(node):
                return (
                    "fp8 gemm (cublasLt allocates its workspace lazily, illegal "
    ...
    def _cudagraph_partitions_from_clusters(
        ...
        min_cg_size = config.aot_inductor.cudagraph_min_partition_size
    ...
    def _log_graph_partitions(
        ...
        for i, (partition, signature) in enumerate(zip(partitions, signatures)):
            # The symbol list is what explains most SKIPs under regional cuda
            # graph: a partition carrying two dynamic symbols cannot be captured.
            cudagraphs_log.debug(
                ...
                [s.name for s in signature.symbol_inputs],
            )
            if signature.skip_cudagraph:
    ...
    def _codegen_partitions(self) -> None:
        ...
        # (cudagraph_runtime.h). Captured partitions are indexed in execution
        ...
        if config.aot_inductor.cudagraph_mode == "regional":
            ...
            # Intra-partition SCRATCH liveness for the shared cross-partition
            # scratch slab (gated). A scratch buffer is produced by a captured
            # partition's nodes and neither escapes it (cap_out) nor enters it
            # (cap_in) nor is extern-produced, so it is dead at that partition's
            # end; captured partitions replay sequentially, so they can share its
            # bytes. Under-inclusion is safe (stays per-partition). The set
            # subtraction already drops model outs, escape outs, inputs, handoffs
            # and passthroughs; shared-storage (alias/mutation) buffers are
            # dropped too since their storage may outlive the partition.
            if config.aot_inductor.cudagraph_cross_partition_memory:
                captured_partitions = [
                    p
                    for p, sig in zip(partitions, signatures)
                    if not sig.skip_cudagraph
                ]
                shared_storage = cudagraph_partition._cudagraph_shared_storage_names(
                    self.nodes
                )
                scratch_liveness: dict[str, int] = {}
                for cpid, part_nodes in enumerate(captured_partitions):
                    out_set = OrderedSet(cap_out_names[cpid])
                    in_set = cap_in_names[cpid]
                    for pnode in part_nodes:
                        for bname in pnode.get_buffer_names():
                            if (
                                bname in out_set
                                or bname in in_set
                                or bname in extern_output_names
                                or bname in boundary_names
                                or bname in shared_storage
                            ):
                                continue
                            scratch_liveness[bname] = cpid
                V.graph._cudagraph_scratch_liveness = scratch_liveness
