...
class Scheduler:
    ...
    def _can_fuse_impl(
        ...
        if (
            ...
            and self._fusion_would_break_outer_reduction_plan(node1, node2)
        ):
            return False
    ...
    @staticmethod
    def _outer_reduction_plan_roles(node: BaseSchedulerNode) -> OrderedSet[str]:
        roles = OrderedSet()
        for snode in node.get_nodes():
            if not (
            ...
            if snode.node._whole_plan_outer_reduction:
                ...
                continue
            is_final = False
            if isinstance(snode.node.data, ir.Reduction):
