...
def is_mkldnn_conv(node: Node) -> bool:
    ...
    if (
        getattr(torch.ops, "onednn", None) is not None
        and getattr(torch.ops.onednn, "_convolution_pointwise", None) is not None
...
class GraphLowering(torch.fx.Interpreter):
    ...
    def decide_layout_opt(gm: GraphModule, *, is_inference: bool) -> bool:
        ...
        if (
            torch.backends.onednn.enabled  # pyrefly: ignore [unbound-name]
            and torch.backends.onednn.is_available()  # pyrefly: ignore [unbound-name]
            and all(
