...
# Remedies differ by where a header is supposed to come from, so the advice has
# to as well: ck/config.h is generated by CK's CMake and is never in the wheel,
# so "install a wheel" would be wrong for the header that most often goes
# missing now that ROCm no longer installs CK system-wide.
_DEFAULT_CK_HEADER_REMEDY = (
    "Check that $ROCM_HOME points at a complete CK install, or install a "
    "matching ck4inductor wheel."
)
_CK_HEADER_REMEDIES = {
    "ck/config.h": (
        "This header is generated by CK's build and is not shipped in the "
        "ck4inductor wheel; it comes from a full CK install under "
        "$ROCM_HOME/include/ck."
    ),
}


@functools.lru_cache(None)
def _warn_missing_ck_header(
    header: str, ck_dir: str | None, rocm_home: str | None
) -> None:
    ...
    ``ck_dir`` and ``rocm_home`` are part of the cache key, not just the header:
    they are what the answer actually depends on, and both can change within a
    process (the test suite patches them per case). Keying on the header alone
    would pin the first verdict for the lifetime of the process.
    """
    try:
        from ck4inductor import check_headers  # type: ignore[import]

        result = check_headers(headers=(header,), try_compile=False)
        resolved = result["headers"][header]["resolved"]
        roots = result["include_roots"]
    except ImportError:
        # Older ck4inductor without check_headers(). Search the same roots the
        # compiler will, rather than a private approximation of them: those roots
        # include a TheRock-packaged SDK and a hipcc-relative path that an
        # inline ROCM_HOME/ROCM_PATH/opt/rocm list would miss, which would report
        # a header as missing that the compile then resolves.
        from .codegen.rocm.compile_command import _rocm_header_search_dirs

        try:
            roots = _rocm_header_search_dirs(rocm_home, ck_dir)
        except OSError:
            # No resolvable ROCm root, so there is nothing to report against.
            # Silence is the contract: this is a diagnostic, and raising out of
            # a gate function would turn a missing header into a failed compile.
            return
    ...
    if not resolved:
        log.warning(
            ...
            header,
            roots,
            _CK_HEADER_REMEDIES.get(header, _DEFAULT_CK_HEADER_REMEDY),
        )


def _warn_missing_ck_headers(*headers: str) -> None:
    """Run the header diagnostic against the current CK / ROCm configuration."""
    for header in headers:
        _warn_missing_ck_header(header, config.rocm.ck_dir, config.rocm.rocm_home)


# ck/ck.hpp is shipped by the ck4inductor wheel, but ck/config.h is generated by
# CK's CMake and comes only from a full ROCm install -- so the two resolve
# independently and both are worth reporting.
_CLASSIC_CK_HEADERS = ("ck/ck.hpp", "ck/config.h")
...
def use_ck_gemm_template(layout: Layout, m: int, n: int, k: int) -> bool:
    ...
    if enabled:
        _warn_missing_ck_headers(*_CLASSIC_CK_HEADERS)
    return enabled
...
def use_ck_tile_gemm_template(layout: Layout, m: int, n: int, k: int) -> bool:
    ...
    if enabled:
        _warn_missing_ck_headers("ck_tile/core.hpp")
    return enabled
