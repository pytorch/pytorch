...
def ensure_nv_universal_gemm_available() -> bool:
    ...
    Call ensure_nv_universal_gemm_available.cache_clear() after installing cutlass.operators
    in the same interpreter to retry the import.
    """
    try:
        return importlib.util.find_spec("cutlass.operators") is not None
    except ImportError:
        return False
...
def use_nv_universal_gemm_template(
    ...
    Note:
        - Shape and stride constraints are handled internally by
          cutlass.operators.get_operators() which filters incompatible operators.
