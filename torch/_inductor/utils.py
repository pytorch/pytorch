...
def can_use_tma(*matrices: IRNode, output_layout: Optional[Layout] = None, add_guards: bool = False) -> bool:
    """
    ...
    * https://docs.nvidia.com/cuda/cuda-driver-api/group__CUDA__TENSOR__MEMORY.html
    ...
    def _is_tma_compatible(sizes: Iterable[int], strides: Iterable[int], dtype: torch.dtype) -> bool:
        rank = len(sizes)
    ...
    return has_triton_tma_device() and all(_is_tma_compatible_matrix(m) for m in matrices) and _is_tma_compatible_layout(output_layout)


def use_triton_tma_template(*matrices: IRNode, output_layout: Layout, add_guards: bool = False) -> bool:
    return (
