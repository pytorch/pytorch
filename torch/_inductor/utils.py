...
def get_k_splits(
    ...
    """Return bounded exact-divisor split choices for ATen decompose-K."""
    ...
    if (
        ...
        valid_divisors = [
            split for split in valid_divisors if split >= min_occupancy_split
        ]
        ...
        occupancy_splits.extend(
            _closest_decompose_k_splits(valid_divisors, wave_targets[3], count=1)
        )
        ...
        valid_divisor_set = OrderedSet(valid_divisors)
        aligned_splits = [
            split
        ...
        if aligned_splits:
            aligned_fallback = min(
                aligned_splits,
                key=lambda split: (
                    min(abs(math.log2(split / target)) for target in wave_targets),
                    split,
                ),
            )
