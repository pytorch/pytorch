...
def expr_fits_within_32bit(e: sympy.Expr) -> bool:
    ...
    runtime_asserts_enabled = (
        not config.do_not_emit_runtime_assertions
        and config.scalar_asserts
        and not config.unsafe_skip_scalar_range_asserts
        and not getattr(V.graph, "fx_wrapper", False)
    )
    if (
        not runtime_asserts_enabled
        or shape_env.frozen
        or shape_env._suppress_guards_tls()
    ):
        # Do not trust facts added after dispatch guards were emitted, or facts
        # whose runtime assertions are disabled. Cover every free symbol so
        # bound_sympy cannot fill in newer ranges from the tracing context.
        ranges = {}
        if runtime_asserts_enabled and shape_env.frozen:
            ranges = shape_env.var_to_range_at_freeze or {}
        ranges = {s: ranges.get(s, ValueRanges.unknown()) for s in e.free_symbols}
        return bool(bound_sympy(e, ranges).upper <= int_max)
    ...
    # AOTI has no Dynamo dispatch that can enforce a new hint-based guard.
    if V.aot_compilation:
        return False
