...
def eager_padded_blockwise_scatter(
    values: Tensor,
    padded_rows: int,
    padded_cols: int,
    ...
    destination_offsets = (
        ...
        ) * xdl + row_inner % xdl) * 2 + col_inner_index // col_inner) * 2
        + row_inner // xdl
    )
    result = torch.full(
...
padded_blockwise_scatter = make_prim(
    "padded_blockwise_scatter(Tensor values, SymInt padded_rows, SymInt padded_cols, int logical_row_chunk, int physical_row_chunk, int xdl, int col_chunk, int col_inner, Scalar padding_value) -> Tensor",
    eager_padded_blockwise_scatter,
    doc="Store a matrix in a padded blocked layout and initialize unused elements",
)
