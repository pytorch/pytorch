...
class GroupedGemmKernel:
    def __init__(
        ...
        - tensormap_update_mode: Specifies whether the tensormap is
           ...
           The 2 modes are functionally equivalent and the difference are:
            - We buffer 3 tensormaps in SMEM for A, B, and C tensors
                (each TMA descriptor takes 128B) when TMA updates performed on SMEM.
    ...
    def _setup_attributes(self):
        ...
        # TODO(shikaili): Further tune configurations.
        mma_inst_tile_k = 8
        self.mma_tiler = (
            self.mma_tiler[0],
            self.mma_tiler[1],
        ...
        (
            ...
            tiled_mma,
            self.mma_tiler,
            self.a_dtype,  # pyrefly: ignore [bad-argument-type]
            self.b_dtype,  # pyrefly: ignore [bad-argument-type]
            self.epi_tile,
            self.c_dtype,  # pyrefly: ignore [bad-argument-type]
            self.c_layout,
            self.smem_capacity,
            self.occupancy,
        )
        ...
        tensor_smem_bytes = self._get_tensor_smem_bytes(  # noqa: F841
    ...
    def __call__(
        ...
        class SharedStorage:
            tensormap_buffer: (
                cute.struct.MemRange[  # pyrefly: ignore [bad-specialization]
                    cutlass.Int64,
                    self.size_tensormap_in_i64,  # pyrefly: ignore [not-a-type]
                ]
            )
            ab_full_mbar_ptr: (
                cute.struct.MemRange[  # pyrefly: ignore [bad-specialization]
                    cutlass.Int64, self.num_ab_stage  # pyrefly: ignore [not-a-type]
                ]
            )
            ab_empty_mbar_ptr: (
                cute.struct.MemRange[  # pyrefly: ignore [bad-specialization]
            ...
            acc_full_mbar_ptr: (
                cute.struct.MemRange[  # pyrefly: ignore [bad-specialization]
                    cutlass.Int64, self.num_acc_stage  # pyrefly: ignore [not-a-type]
                ]
            )
            acc_empty_mbar_ptr: (
                cute.struct.MemRange[  # pyrefly: ignore [bad-specialization]
            ...
            sC: cute.struct.Align[  # pyrefly: ignore [bad-specialization]
                cute.struct.MemRange[  # pyrefly: ignore [bad-specialization]
                    self.c_dtype,
                    cute.cosize(self.epi_smem_layout_staged.outer),
                ],
                self.buffer_align_bytes,  # pyrefly: ignore [not-a-type]
            ]
            # (MMA, MMA_M, MMA_K, STAGE)
            sA: cute.struct.Align[  # pyrefly: ignore [bad-specialization]
                cute.struct.MemRange[  # pyrefly: ignore [bad-specialization]
            ...
            sB: cute.struct.Align[  # pyrefly: ignore [bad-specialization]
                cute.struct.MemRange[  # pyrefly: ignore [bad-specialization]
    ...
    def kernel(
        ...
        tile_sched_params = self._compute_tile_sched(
            total_num_clusters[0],  # pyrefly: ignore [bad-argument-type]
            self.cluster_shape_mn,
        )
        ...
        storage = smem.allocate(  # pyrefly: ignore [no-matching-overload]
            self.shared_storage
        )
        ...
        # Ensure all mbarrier inits are visible before cluster sync
        ...
        sC = storage.sC.get_tensor(
            epi_smem_layout_staged.outer,  # pyrefly: ignore [missing-attribute]
            swizzle=epi_smem_layout_staged.inner,  # pyrefly: ignore [missing-attribute]
        )
        # (MMA, MMA_M, MMA_K, STAGE)
        ...
        tensormap_a_ptr = tensormap_manager.get_tensormap_ptr(
            tensormaps[  # pyrefly: ignore [missing-attribute]
                (tensormap_workspace_idx, 0, None)
            ].iterator
        )
        tensormap_b_ptr = tensormap_manager.get_tensormap_ptr(
            tensormaps[  # pyrefly: ignore [missing-attribute]
                (tensormap_workspace_idx, 1, None)
            ].iterator
        )
        tensormap_c_ptr = tensormap_manager.get_tensormap_ptr(
            tensormaps[  # pyrefly: ignore [missing-attribute]
                (tensormap_workspace_idx, 2, None)
            ].iterator
        )
        ...
        if warp_idx == self.tma_warp_id:
            ...
            if cutlass.const_expr(self.delegate_tensormap_ab_init == False):  # noqa: E712
            ...
            group_gemm_ts_helper = utils.GroupedGemmTileSchedulerHelper(
                group_count,  # pyrefly: ignore [bad-argument-type]
                tile_sched_params,
                self.cluster_tile_shape_mnk,  # pyrefly: ignore [bad-argument-type]
            ...
            while work_tile.is_valid_tile:  # pyrefly: ignore [bad-assignment]
                ...
                if is_group_changed:
                    ...
                    if tensormap_init_done == False:  # noqa: E712
            ...
            while work_tile.is_valid_tile:  # pyrefly: ignore [bad-assignment]
                ...
                for k_tile in range(cur_k_tile_cnt):
                    ...
                    mma_rd_ab_full_phase_next = (
                        mma_rd_ab_full_phase  # pyrefly: ignore [unsupported-operation]
                        ^ 1
        ...
        if warp_idx < self.mma_warp_id:
            ...
            while work_tile.is_valid_tile:  # pyrefly: ignore [bad-assignment]
                ...
                subtile_cnt = cute.size(
                    tTR_tAcc.shape,  # pyrefly: ignore [missing-attribute]
                    mode=[3],
                )
                ...
                for subtile_idx in range(subtile_cnt):
                    ...
                    #
                    tTR_tAcc_mn = tTR_tAcc[  # pyrefly: ignore [bad-index]
                    ...
                    if warp_idx == self.epilog_warp_id[0]:
                        cute.copy(
                            ...
                            bSG_gC[(None, subtile_idx)],  # pyrefly: ignore [bad-index]
                            tma_desc_ptr=tensormap_manager.get_tensormap_ptr(
                                tensormap_c_ptr,
                                cute.AddressSpace.generic,
                            ),
    ...
    def _compute_grid(
        ...
        grid = utils.StaticPersistentTileScheduler.get_grid_shape(
            tile_sched_params,
            max_active_clusters,  # pyrefly: ignore [bad-argument-type]
        )

        return grid  # pyrefly: ignore [bad-return]
