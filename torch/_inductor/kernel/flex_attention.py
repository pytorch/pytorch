...
def create_num_blocks_fake_generator(sparse_indices):
    ...
    def create_num_blocks_fake(x) -> torch.Tensor:
        num_blocks_for_autotuning = V.graph.sizevars.size_hint(sparse_indices.shape[-1], fallback=1)
        size = [V.graph.sizevars.size_hint(i, fallback=1) for i in x.get_size()]
        return torch.full(
            size,
