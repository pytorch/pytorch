...
def lower_quack_flex_gemm(gemm_op, subgraph, args, gemm_kwargs, kernel_options):
    ...
    packed_interleaved_b16x2 = epilogue_analysis.packed_interleaved_b16x2
    packed_capture = None
    if packed_interleaved_b16x2 is not None:
        if (
            ...
            raise NotImplementedError(
                "FlexGEMM packed interleaved BF16x2 currently requires plain aten.mm"
            )
        packed_capture = placeholder_args[packed_interleaved_b16x2.capture]
        if not isinstance(packed_capture, TensorBox):
            raise NotImplementedError(
                "FlexGEMM packed interleaved BF16x2 requires a tensor capture"
            )
        ordinary_epilogue_pairs = tuple(
            ...
            if placeholder is not packed_interleaved_b16x2.capture
        )
    ...
    if packed_interleaved_b16x2 is not None:
        if len(outputs.aux_outputs) != 1:
            raise AssertionError("packed FlexGEMM requires one auxiliary output")
        aux_meta = outputs.aux_outputs[0].meta.get("val")
        if not isinstance(aux_meta, torch.Tensor):
            raise NotImplementedError(
                "FlexGEMM packed interleaved BF16x2 requires auxiliary metadata"
            )
        aux_metas = (aux_meta,)
    ...
    packed_capture_input_index = (
        ...
        if packed_capture_input_nodes
        else None
    )
    indexed_index_input_index = (
        len(gemm_input_nodes)
        + len(epilogue_input_nodes)
        + len(packed_capture_input_nodes)
        if indexed_index_input_nodes
        else None
    )
    aux_out_start = (
        len(gemm_input_nodes)
        + len(epilogue_input_nodes)
        + len(packed_capture_input_nodes)
        + len(indexed_index_input_nodes)
    )
    aux_out_indices = tuple(range(aux_out_start, aux_out_start + len(aux_input_nodes)))
    indexed_out_index = (
        aux_out_start + len(aux_input_nodes) if indexed_out_input_nodes else None
    )
    local_reduce_out_index = (
        aux_out_start + len(aux_input_nodes) + len(indexed_out_input_nodes)
