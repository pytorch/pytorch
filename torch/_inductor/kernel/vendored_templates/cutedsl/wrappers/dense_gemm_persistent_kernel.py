"""Kernel wrapper for dense_gemm_persistent vendored template."""
...
try:
    from ..kernels.dense_gemm_persistent import (  # pyrefly: ignore[missing-import]
        PersistentDenseGemmKernel as PersistentDenseGemmKernelImpl,
    )
except ImportError:
    PersistentDenseGemmKernelImpl = None  # type: ignore[misc, assignment]


class VendoredPersistentDenseGemmKernel(CuteDslKernel):
    """Wrapper for vendored persistent dense GEMM template for SM100 GPUs."""
    ...
    def _valid_metadata(metadata: KernelMetadata) -> bool:
        ...
        if not VendoredPersistentDenseGemmKernel._valid_operands(metadata.operands):
            return False
    ...
    def generate_kernels(
        ...
        epilogue_args=None,
        cc: int | None = None,
    ) -> list[VendoredPersistentDenseGemmKernel]:
        ...
        for (
            operands
        ) in VendoredPersistentDenseGemmKernel._metadata_operand_combinations():
                    ...
                    for tile_n in tile_n_values:
                        for cluster_m in cluster_m_values:
                            for cluster_n in cluster_n_values:
                                ...
                                kernel_name = (
                                    f"inductor_vendored.PersistentDenseGemmKernel_sm100_"
                                ...
                                metadata = KernelMetadata(
                                    ...
                                    kernel_class=VendoredPersistentDenseGemmKernel,
                                    min_cc=100,
                                    epilogue=None,
                                )

                                if VendoredPersistentDenseGemmKernel._valid_metadata(
                                    metadata
                                ):
                                    if metadata_filter is None or metadata_filter(
                                        ...
                                        kernel_list.append(
                                            VendoredPersistentDenseGemmKernel(metadata)
                                        )

        log.debug(
            "Generated %d PersistentDenseGemmKernel configurations", len(kernel_list)
        )
        return kernel_list


# Only register if kernel implementation is available
if PersistentDenseGemmKernelImpl is not None:
    cutlass_api.providers.cutedsl.CuTeDSLProvider.register(
        VendoredPersistentDenseGemmKernel
    )
