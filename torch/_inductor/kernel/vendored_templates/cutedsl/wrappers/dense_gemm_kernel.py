"""Kernel wrapper for dense_gemm vendored template."""
...
try:
    from ..kernels.dense_gemm import (  # pyrefly: ignore[missing-import]
        DenseGemmKernel as DenseGemmKernelImpl,
    )
except ImportError:
    DenseGemmKernelImpl = None  # type: ignore[misc, assignment]


class VendoredDenseGemmKernel(CuteDslKernel):
    """Wrapper for vendored dense GEMM template for SM100 (Blackwell) GPUs."""
    ...
    def __init__(self, metadata: KernelMetadata):
        ...
        self.impl = DenseGemmKernelImpl(  # pyrefly: ignore[not-callable]
    ...
    def _valid_metadata(metadata: KernelMetadata) -> bool:
        ...
        if not VendoredDenseGemmKernel._valid_operands(metadata.operands):
            return False
    ...
    def generate_kernels(
        ...
        for operands in VendoredDenseGemmKernel._metadata_operand_combinations():
                    ...
                    for tile_n in tile_n_values:
                        for cluster_m in cluster_m_values:
                            for cluster_n in cluster_n_values:
                                ...
                                kernel_name = (
                                    f"inductor_vendored.DenseGemmKernel_sm100_"
                                ...
                                metadata = KernelMetadata(
                                    ...
                                    kernel_class=VendoredDenseGemmKernel,
                                    min_cc=100,
                                    epilogue=None,
                                )

                                if VendoredDenseGemmKernel._valid_metadata(metadata):
                                    if metadata_filter is None or metadata_filter(
                                        metadata
                                    ):
                                        kernel_list.append(
                                            VendoredDenseGemmKernel(metadata)
                                        )

        log.debug("Generated %d DenseGemmKernel configurations", len(kernel_list))
        return kernel_list


# Only register if kernel implementation is available
if DenseGemmKernelImpl is not None:
    cutlass_api.providers.cutedsl.CuTeDSLProvider.register(VendoredDenseGemmKernel)
