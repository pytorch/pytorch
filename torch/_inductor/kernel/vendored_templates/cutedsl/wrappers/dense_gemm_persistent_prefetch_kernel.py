"""Kernel wrapper for dense_gemm_persistent_prefetch vendored template."""
...
try:
    from ..kernels.dense_gemm_persistent_prefetch import (  # pyrefly: ignore[missing-import]
        PersistentDenseGemmKernel as PersistentDenseGemmKernelPrefetchImpl,
    )
except ImportError:
    PersistentDenseGemmKernelPrefetchImpl = None  # type: ignore[misc, assignment]


@dataclass
class Sm100DesignMetadataWithPrefetch(Sm100DesignMetadata):
    """Design parameters for SM100 kernels with prefetch support."""

    prefetch_dist: int | None = None  # None=auto, 0=disabled, >0=explicit


class VendoredPersistentDenseGemmKernelPrefetch(CuteDslKernel):
    """Wrapper for vendored persistent dense GEMM template with TMA prefetch for SM100."""
    ...
    def __init__(self, metadata: KernelMetadata):
        ...
        prefetch_dist = getattr(metadata.design, "prefetch_dist", None)

        self.impl = (
            PersistentDenseGemmKernelPrefetchImpl(  # pyrefly: ignore[not-callable]
    ...
    def compile(self, args: GemmArguments, cc: int | None = None) -> CompiledArtifact:
        ...
        compiled_kernel = self.cute_compile(
            self.impl, a_tw, b_tw, out_tw, max_active_clusters, stream
        )
    ...
    def _valid_metadata(metadata: KernelMetadata) -> bool:
        ...
        if not VendoredPersistentDenseGemmKernelPrefetch._valid_operands(
            metadata.operands
        ):
            return False
    ...
    def generate_kernels(
        ...
        epilogue_args=None,
        cc: int | None = None,
    ) -> list[VendoredPersistentDenseGemmKernelPrefetch]:
        ...
        prefetch_dist_values = [None]

        kernel_list = []

        for (
            operands
        ) in VendoredPersistentDenseGemmKernelPrefetch._metadata_operand_combinations():
                        ...
                        for cluster_m in cluster_m_values:
                            for cluster_n in cluster_n_values:
                                for prefetch_dist in prefetch_dist_values:
                                    ...
                                    design = Sm100DesignMetadataWithPrefetch(
                                        use_2cta_mma=use_2cta,  # pyrefly: ignore [unexpected-keyword]
                                        # pyrefly: ignore [unexpected-keyword]
                                        tile_shape=(tile_m, tile_n, 64),
                                        # pyrefly: ignore [unexpected-keyword]
                                        cluster_shape=(cluster_m, cluster_n, 1),
                                        use_tma_store=True,  # pyrefly: ignore [unexpected-keyword]
                                    ...
                                    prefetch_str = (
                                        ...
                                        else str(prefetch_dist)
                                    )
                                    kernel_name = (
                                        f"inductor_vendored.PersistentDenseGemmKernelPrefetch_sm100_"
                                    ...
                                    metadata = KernelMetadata(
                                        ...
                                        kernel_class=VendoredPersistentDenseGemmKernelPrefetch,
                                        min_cc=100,
                                        epilogue=None,
                                    )

                                    if VendoredPersistentDenseGemmKernelPrefetch._valid_metadata(
                                        metadata
                                    ):
                                        if metadata_filter is None or metadata_filter(
                                            ...
                                            kernel_list.append(
                                                VendoredPersistentDenseGemmKernelPrefetch(
                                                    metadata
                                                )
                                            )

        log.debug(
            "Generated %d PersistentDenseGemmKernelPrefetch configurations",
            len(kernel_list),
        )
        return kernel_list
...
if PersistentDenseGemmKernelPrefetchImpl is not None:
