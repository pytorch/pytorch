...
def build_flex_attn_fwd_mfma32_module(
    *,
    batch_size: int,
    num_q_heads: int,
    num_kv_heads: int,
    ...
    """Build the gfx950 prefill fast path.
    ...
    if (qk_head_dim, v_head_dim) not in ((128, 128), (192, 128)):
        raise ValueError(
            "FlyDSL MFMA32 forward requires (qk_head_dim, v_head_dim) "
    ...
    if sparse_q_block_size != BM or sparse_kv_block_size != 128:
        raise ValueError("FlyDSL MFMA32 forward requires sparse block size 128")
    if seq_q % BM or seq_kv % sparse_kv_block_size:
        raise ValueError("FlyDSL MFMA32 forward requires Sq/Sk divisible by 128")
    ...
    launch.compile_hints = dict(_FWD_MFMA32_COMPILE_HINTS)
    return launch
