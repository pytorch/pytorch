...
{{def_kernel("A", "B")}}
    ...
    for _ in range(0, k_tiles * tiles_per_SM):
        ...
        if ki == k_tiles - 1:
            ...
            {%- else %}
            {{store_output(("rm", "rn"), "acc", indent_width=12, tma_block_sizes=("BLOCK_M", "BLOCK_N"))}}
            {%- endif %}
            acc = tl.zeros((BLOCK_M, BLOCK_N), dtype=ACC_TYPE)
...
{{def_kernel("A", "B", "A_inverse_scale", "B_inverse_scale")}}
    ...
    for _ in range(0, k_tiles * tiles_per_SM):
        ...
        if ki == k_tiles - 1:
            ...
            {%- else %}
            {{store_output(("offs_am", "offs_bn"), "accumulator", indent_width=12, tma_block_sizes=("BLOCK_M", "BLOCK_N"))}}
            {%- endif %}
