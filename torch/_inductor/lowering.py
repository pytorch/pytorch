...
def fuse_or_err(
    ...
    for i, node in enumerate(subgraph_fn.graph_module.graph.nodes):
        if node.op == "placeholder":
            ...
            args, kwargs = V.graph.fetch_args_kwargs_from_env(node)
            # Realize outputs (flattening any containers) so the region's ops
            # become materialized scheduler nodes that we can look up after
            # fusion.
            for v in pytree.tree_leaves((args, kwargs)):
