...
def _flatten_control_deps(deps, dep_names):
    """Recursively flatten containers in additional_deps and collect buffer names.

    Nested control_deps produce tuple/list outputs that appear in the
    additional_deps of outer control_deps nodes.  A flat scan misses them.
    """
    for dep in deps:
        if isinstance(dep, (list, tuple)):
            _flatten_control_deps(dep, dep_names)
        elif isinstance(dep, IRNode):
            dep.realize()
