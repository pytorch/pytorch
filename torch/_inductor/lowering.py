...
def _compute_slice_index(index, size, default=None):
    ...
    if guard_or_false(sympy.And(sympy.Ge(index, 0), sympy.Le(index, size))):
        return index
    elif guard_or_false(sympy.And(sympy.Lt(index, 0), sympy.Ge(index, -size))):
        return index + size
    elif guard_or_false(sympy.Gt(index, size)):
        return size
    elif guard_or_false(sympy.Lt(index, -size)):
        return 0
