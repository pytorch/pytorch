...
class Match:
    ...
    def replace_by_example(
        ...
        unsafe_drop_arg_kwarg_vals_meta: bool = False,
    ) -> list[torch.fx.Node]:
        ...
        with context:
            ...
            if len(self.nodes) == 1:
                if (
                    not unsafe_drop_arg_kwarg_vals_meta
                    and "arg_kwarg_vals" in self.nodes[0].meta
                    and len(replacement.graph.nodes) > 0
                ):
                    raise RuntimeError(
                        "pattern_matcher.replace_by_example attempted to replace a node with "
                        "'args_kwargs_val' metadata with 1+ new nodes. "
                        "The 'args_kwargs_val' metadata exists because the node has "
                        "strict stride requirements - you may need to manually propagate "
                        "this metadata to the new nodes or risk silent incorrectness. "
                        "If the metadata propagation is already being manually handled, then "
                        "pass unsafe_drop_args_kwargs_val_meta=True."
                    )
                for n in replacement.graph.nodes:
                    # NB: this does NOT transfer 'args_kwargs_val' metadata.
                    _transfer_meta(
                        new_meta=n.meta,
