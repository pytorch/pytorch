...
class Match:
    ...
    def replace_by_example(
        ...
        If the matched node is consumed through `getitem`, replacement_fn must return
        a pack of those outputs, which is rewired onto the `getitem` users. Otherwise
        it returns a single value that takes the place of the matched node.
...
class ReplacementPatternEntry(PatternEntry):
    ...
    def replace_with_graph(
        ...
        with graph.inserting_before(last_node):
            ...
            # Record before normalizing: that wraps a bare node into a one-element list.
            replacement_is_packed = isinstance(replacement, (list, tuple))
                ...
                for user in old_uses:
                    ...
                    if idx is None:
                        ...
                        try:
                            ...
                            raise AssertionError(
                                f"replacement for {old.target} has {len(new)} outputs, "
                                f"but it is indexed with {idx!r}"
                            ) from e
                        replace(user, value)
                graph.erase_node(old)

            # A one-element pack replacing a single matched node also satisfies
            # `len(output_nodes) == len(replacement)`, but it is read through `getitem`
            # and needs `replace()` to rewire the pack's value onto those users; the zip
            # branch leaves behind a stray `getitem` that fails to lower. A pack holding
            # a container (`triton_kernel_wrapper_functional`) stays on the zip branch,
            # which already maps the container onto the users' keys.
            packed_match = (
                len(output_nodes) == 1
                ...
                and output_nodes[0].users
                and all(
                    maybe_getitem(user) is not None for user in output_nodes[0].users
                )
            )
            if len(output_nodes) == len(replacement) and not packed_match:
