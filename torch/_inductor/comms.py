...
def _reorder_communication_preserving_peak_memory_internal(
    ...
    print(f"XXX BEFORE_REORDER CURRENT_MEMORY:{curr_memory}")
    print(f"XXX BEFORE_REORDER PEAK_MEMORY:{peak_memory}")
    ...
    print(f"XXX REORDER_INIT {[n.get_name() for n in snodes]}")
    curr = snodes[0]
    while _next[curr] is not None:
        print(f"XXX curr:{curr.get_name()}")
        if contains_collective(curr):
            print(f"XXX contains_collective")
            ...
            while candidate is not None:
                print(f"XX candidate:{candidate.get_name()}")
                ...
                if data_dep is not None:
                    ...
                    if is_grp:
                        group_head = candidate
                        group_peak_memory = max(group_peak_memory, _curr_memory[candidate])
                        reorder_info.grouped += 1
                        reorder_info.grouped_info = gsnode.get_name()
                        print(f"XXX GROUP")
                        print(f"XXX AFTER_GROUP {[_group_name(n) for n in _group_names(_head, None)]}")
                        candidate = _prev[candidate]
                        continue
                    else:
                        msg = (
                            f"data dependency {data_dep}(dep_names:{dep_names})"
                            f" candidate.outputs:{[o.get_name() for o in prev_outs]}"
                        )
                        reorder_info.limiting_factor = msg
                        break

                delta_memory_candidate = _curr_memory[candidate] - _curr_memory[_prev[candidate]]
                ...
                print(f"XXX BEFORE_SWAP {[n.get_name() for n in _group_nodes(_head, None)]}")
                print(f"XXX SWAP {candidate.get_name()} vs {[n.get_name() for n in _group_nodes(group_head, group_tail)]}")

                # def swap_curr_memory(snode_a, snode_b):
                #     assert _prev[snode_b] == snode_a
                #     assert _next[snode_a] == snode_b
                #     snode_prev = _prev[snode_a]
                #     snode_next = _next[snode_b]
                #     # snode_prev -> snode_a -> snode_b -> snode_next
                #     snode_a_delta_mem = _curr_memory[snode_a] - _curr_memory[snode_prev]
                #     snode_b_delta_mem = _curr_memory[snode_b] - _curr_memory[snode_a]
                #     # recalculating new curr_memory
                #     _curr_memory[snode_b] = _curr_memory[snode_prev] + snode_b_delta_mem
                #     _curr_memory[snode_a] = _curr_memory[snode_b] + snode_a_delta_mem
                ...
                #         swap_curr_memory(snode_j, snode_i)
                ...
                for n in [candidate, *_group_nodes(group_head, group_tail)]:
                    mem_deltas[n] = _curr_memory[n] - _curr_memory[_prev[n]]
                # SWAP
                # Before:
                ...
                if group_tail_next:
                    _prev[group_tail_next] = group_tail
                _next[group_tail] = group_tail_next
                ...
                print(f"XXX AFTER_SWAP {[n for n in new_nodes]}")
                new_peak_memory, _ = estimate_peak_memory(new_nodes, name_to_freeable_input_buf, graph_outputs)
                print(f"XXX peak_memory:{peak_memory}")
                print(f"XXX new_peak_memory:{new_peak_memory}")
                assert new_peak_memory <= peak_memory

                reorder_info.final_exposed = exposed_communication_time(
                    curr,
                    _group_nodes(_next[curr], None)
                )
                # Recompute curr_memory
                _prev_curr_memory = _curr_memory[_prev[candidate]]
                for n in _group_nodes(candidate, group_tail):
                    _curr_memory[n] = _prev_curr_memory = _prev_curr_memory + mem_deltas[n]
    ...
    flatten_snodes: list[BaseSchedulerNode] = _flatten_nodes()
    ...
    assert len(flatten_snodes) == original_snodes_num
    peak_memory, curr_memory = estimate_peak_memory(
        flatten_snodes, name_to_freeable_input_buf, graph_outputs
    )
    print(f"XXX AFTER_REORDER CURRENT_MEMORY:{curr_memory}")
    print(f"XXX AFTER_REORDER PEAK_MEMORY:{peak_memory}")
...
def _sink_waits_iterative_internal(
    ...
    print(f"XXX BEFORE_SINK:CURRENT_MEMORY:{curr_memory}")
    print(f"XXX BEFORE_SINK:PEAK_MEMORY:{peak_memory}")
    ...
    for i in range(n - 1, -1, -1):
        ...
        if contains_wait(gsnode):
            ...
            for j in range(i + 1, n):
                ...
                assert contains_wait(wait_gsnode)
                ...
                for o in wait_outs:
                    if o.get_name() in dep_names:
                        data_dep = o.get_name()
                        break
                wait_group_contains_collective = contains_collective(wait_gsnode)
                ...
                if data_dep is not None or (
                    both_contain_comms := (
                        wait_group_contains_collective
                    ...
                    def is_groupable(snode):
                        # if contains_collective(snode):
                        #     return False, f"candidate contains collective {snode.get_name()}"
                        if contains_gemm_like(snode):
                            return False, f"candidate contains gemm_like {snode.get_name()}"
                        return True, None

                    is_grp, grp_reason = is_groupable(next_gsnode)
                    if is_grp:
                        ...
                        info.limiting_factor = (
                            f"data dependency {data_dep}(dep_names:{list(dep_names)})"
                            f" candidate:{_group_name(next_gsnode)} dep on {_group_name(wait_gsnode)}"
                            f" outs:{[o.get_name() for o in wait_outs]} non_group_reason:{grp_reason}"
                        )
                        break
                # Candidate does not have data dependency
                # But if we already grouped with collective
                # We do not want to move that collective later of the heavy_ops
                if wait_group_contains_collective and contains_gemm_like(next_gsnode):
                    info.limiting_factor = (
                        f"contains_collective {_group_name(wait_gsnode)} candidate gemm_like:{_group_name(next_gsnode)}"
                    )
                    break
                info.moves += 1
    ...
    print(f"XXX AFTER_SINK:CURRENT_MEMORY:{curr_memory}")
    print(f"XXX AFTER_SINK:PEAK_MEMORY:{peak_memory}")
    return flatten_snodes, stats


def _sink_waits_iterative_internal2(
    snodes: list[BaseSchedulerNode],
) -> tuple[list[BaseSchedulerNode], dict[BaseSchedulerNode, SinkWaitInfo]]:
    from torch._inductor.scheduler import GroupedSchedulerNode
    n = len(snodes)
            ...
            while candidate is not None:
                ...
                if data_dep is not None or (
                    ...
                    if is_grp:
                        ...
                        break
                print(f"XXX SWAP candidate:{candidate.get_name()} vs {[n.get_name() for n in _group_nodes(group_head, group_tail)]}")
                candidate_delta_memory = _curr_memory[candidate] - _curr_memory[_prev[candidate]]
                print(f"XXX candidate_delta_memory:{candidate_delta_memory}")
                print(f"XXX group_peak_memory:{group_peak_memory}")
                print(f"XXX peak_memory:{peak_memory}")
                ...
                new_nodes = _group_nodes(_head, None)
                print(f"XXX BEFORE_SWAP {[n.get_name() for n in new_nodes]}")
                mem_deltas = {}
                for n in [candidate, *_group_nodes(group_head, group_tail)]:
                    mem_deltas[n] = _curr_memory[n] - _curr_memory[_prev[n]]
                # 0:
                ...
                # XXX DEBUG
                new_nodes = _group_nodes(_head, None)
                print(f"XXX AFTER_SWAP {[n.get_name() for n in new_nodes]}")
                new_peak_memory, _ = estimate_peak_memory(new_nodes, name_to_freeable_input_buf, graph_outputs)
                print(f"XXX peak_memory:{peak_memory}")
                print(f"XXX new_peak_memory:{new_peak_memory}")
                assert new_peak_memory <= peak_memory
    ...
    print(f"XXX ORIGINAL_N:{n}")
    print(f"XXX NEW:{len(new_snodes)}")
    print(f"XXX NEW_SNODES:{[s.get_name() for s in new_snodes]}")
    assert len(new_snodes) == n
    peak_memory, curr_memory = estimate_peak_memory(
...
def sink_waits_iterative2(
    snodes: list[BaseSchedulerNode],
) -> list[BaseSchedulerNode]:
    return _sink_waits_iterative_internal2(snodes)[0]
