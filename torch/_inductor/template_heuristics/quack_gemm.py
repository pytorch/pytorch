...
quack_gemm_config = importlib.import_module("quack.gemm_config")
...
def gemm_config_key(config: quack_gemm_config.GemmConfig) -> GemmConfigKey:
    """Use QuACK's GemmConfig schema as the explicit generated-code key."""
    return tuple(
...
def dense_gemm_config_priority_keys() -> tuple[GemmConfigKey, ...]:
    """Return the measured dense FlexGEMM QuACK preference order for Blackwell."""
    configs = (
...
def candidate_gemm_configs_for_device(device: torch.device):
    """Return dense-safe QuACK configs sorted by the reference branch priority."""
...
def config_from_key(config_key: GemmConfigKey) -> Any:
    """Reconstruct a QuACK config from a lossless generated-code key."""
    return quack_gemm_config.GemmConfig(**dict(config_key))


def default_gemm_config_key(device: torch.device, m, n) -> GemmConfigKey:
    """Return the untuned QuACK config key for generated dense code."""
