...
class CUDAConfigHeuristic(BaseConfigHeuristic):
    ...
    def get_flex_attn_bwd_configs(
        ...
        capability_table: dict[str, dict[Optional[int], FlexBwDConfig]] = {
            "float32": {
                None: FlexBwDConfig(16, 16, 16, 16, 1, 4),
            },
            "baseline": {
                None: FlexBwDConfig(16, 16, 16, 16, 1, 4),
            },
            "h100": {
                64: FlexBwDConfig(64, 64, 64, 64, 3, 4),
                128: FlexBwDConfig(64, 128, 128, 64, 3, 8),
            ...
            "a100": {
                ...
                128: FlexBwDConfig(64, 64, 64, 64, 2, 4),
                None: FlexBwDConfig(64, 64, 64, 64, 2, 4),
            },
        }

        if dtype == torch.float32:
            default_config = capability_table["float32"][None]
        else:
            major, minor = capability
            if major >= 10:
                capability_class = "b200"
            elif capability == (9, 0):
                capability_class = "h100"
            elif major >= 8:
                capability_class = "a100"
            else:
                capability_class = "baseline"

            if capability_class == "a100" and head_dim == 128:
                # SM86/89 have smaller shared memory sizes
                num_stages = 3 if minor == 0 else 2
                default_config = FlexBwDConfig(64, 64, 64, 64, num_stages, 4)
            elif head_dim <= 256:
                tier = capability_table.get(
                    capability_class, capability_table["baseline"]
                )
                default_config = tier.get(head_dim, tier[None])
            else:
                default_config = capability_table["baseline"][None]
