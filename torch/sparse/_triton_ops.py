...
if has_triton():
    ...
    def _scatter_mm6_kernel(
        ...
        if is_compressed:
            A_ptr += r0 * blocks_stride_P  # type: ignore[possibly-undefined]  # TODO: audit
            for _ in range(nnz):
                q = tl.load(q_ptr)
                B = tl.load(B_ptr + q)
                A = tl.load(A_ptr)
    ...
    def _bsr_strided_addmm_kernel(
        ...
        # alpha is never 0
        if beta_is_nonzero:
            output_acc_block = tl.load(input_ptrs).to(acc_dtype)  # type: ignore[possibly-undefined]  # TODO: audit
