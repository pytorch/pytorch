...
from torch.sparse._triton_utils import triton_jit, Annotated
...
if _has_triton():
    ...
    def _bsr_strided_dense_rowspace_kernel(
            BLOCKSIZE_ROW: tl.constexpr,
            BLOCKSIZE_COL: tl.constexpr,
            values: Annotated[torch.Tensor, dict(ndim=4)],
            crow_indices: Annotated[torch.Tensor, dict(ndim=2)],
            col_indices: Annotated[torch.Tensor, dict(ndim=2)],
            dense: Annotated[torch.Tensor, dict(ndim=5)],
            output: Annotated[torch.Tensor, dict(ndim=5)],
            acc_dtype: tl.constexpr,
            allow_tf32: tl.constexpr,
        ...
        nnz_offset = crow_indices[batch_pid, row_block_pid]
        nnz_offset_next = crow_indices[batch_pid, row_block_pid + 1]
        row_nnz = nnz_offset_next - nnz_offset
        if row_nnz == 0:
            return
        ...
        for n in range(row_nnz):
            values_block = values[batch_pid, nnz_offset + n, row_block_arange[:, None], col_block_arange[None, :]]

            # find which row of dense needs to get loaded
            # for multiplication with values_block.
            dense_row_idx = col_indices[batch_pid, n]
            dense_block = dense[batch_pid, dense_row_idx, col_block_pid, col_block_arange[:, None], row_block_arange[None, :]]

            # do block mm
            output_acc_block += tl.dot(values_block, dense_block, allow_tf32=allow_tf32, out_dtype=acc_dtype)

        # write back the result
        output[batch_pid, row_block_pid, col_block_pid,
               row_block_arange[:, None], row_block_arange[None, :]].copy_(output_acc_block.to(output.dtype.element_ty))
