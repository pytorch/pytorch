...
class SparseSemiStructuredTensor(torch.Tensor):
    ...
    def __init__(
        ...
        if original_tensor is not None:
            ...
            if self._FORCE_CUTLASS:
                dtype_to_semi_structured_sparse_config = _DTYPE_TO_SEMI_STRUCTURED_SPARSE_CONFIG_CUTLASS
            else:
                dtype_to_semi_structured_sparse_config = _DTYPE_TO_SEMI_STRUCTURED_SPARSE_CONFIG_CUSPARSELT

            if original_tensor.dtype not in dtype_to_semi_structured_sparse_config:
                raise RuntimeError(
                    f"Error original_tensor.dtype {original_tensor.dtype} is not a supported dtype! "
                    "dtype must be one of: {dtype_to_semi_structured_sparse_config}"
                )

            # check shape
            m, n = original_tensor.shape
            min_rows = dtype_to_semi_structured_sparse_config[
                original_tensor.dtype
            ].min_rows
            min_cols = dtype_to_semi_structured_sparse_config[
                original_tensor.dtype
            ].min_cols
    ...
    def _pad_tensor_for_matmul(self, original_tensor : torch.Tensor, min_rows : int, min_cols : int) -> torch.Tensor:
        """
        ...
        if func is torch.ops.aten.addmm.default:
            ...
            if isinstance(input_B, cls) and input_B.transposed:
                ...
                if input_B.compressed_tensor_cusparselt is None:
                    ...
                    dtype_config = _DTYPE_TO_SEMI_STRUCTURED_SPARSE_CONFIG_CUTLASS[input_B.dtype]
                    min_rows, min_cols = dtype_config.min_rows, dtype_config.min_cols
                    input_A_padded = input_B._pad_tensor_for_matmul(input_A, min_rows, min_cols)
                    ...
                    dtype_config = _DTYPE_TO_SEMI_STRUCTURED_SPARSE_CONFIG_CUSPARSELT[input_B.dtype]
                    min_rows, min_cols = dtype_config.min_rows, dtype_config.min_cols
                    input_A_padded = input_B._pad_tensor_for_matmul(input_A, min_rows, min_cols)
        ...
        if func is torch.ops.aten.mm.default:
            ...
            if isinstance(input_A, cls) and not input_A.transposed:
                ...
                if input_A.compressed_tensor_cusparselt is None:
                    ...
                    dtype_config = _DTYPE_TO_SEMI_STRUCTURED_SPARSE_CONFIG_CUTLASS[input_A.dtype]
                    min_rows, min_cols = dtype_config.min_rows, dtype_config.min_cols
                    input_B_padded = input_A._pad_tensor_for_matmul(input_A, min_rows, min_cols)
                    res = torch._sparse_semi_structured_linear(
                        input_B_padded.t(), input_A.sparse_tensor_cutlass, input_A.meta_tensor_cutlass
                    ).t()
                else:
                    dtype_config = _DTYPE_TO_SEMI_STRUCTURED_SPARSE_CONFIG_CUSPARSELT[input_A.dtype]
                    min_rows, min_cols = dtype_config.min_rows, dtype_config.min_cols
                    input_B_padded = input_A._pad_tensor_for_matmul(input_A, min_rows, min_cols)
                    res = torch._cslt_sparse_mm(
        ...
        if func is torch.ops.aten.linear.default:
            ...
            if isinstance(weight, cls):
                if weight.compressed_tensor_cusparselt is None:
                    ...
                    dtype_config = _DTYPE_TO_SEMI_STRUCTURED_SPARSE_CONFIG_CUTLASS[input_tensor.dtype]
                    min_rows, min_cols = dtype_config.min_rows, dtype_config.min_cols
                    input_tensor_2d_padded = weight._pad_tensor_for_matmul(input_tensor_2d, min_rows, min_cols)
                    res = torch._sparse_semi_structured_linear(
                    ...
                    dtype_config = _DTYPE_TO_SEMI_STRUCTURED_SPARSE_CONFIG_CUSPARSELT[input_tensor.dtype]
                    min_rows, min_cols = dtype_config.min_rows, dtype_config.min_cols
                    input_tensor_2d_padded = weight._pad_tensor_for_matmul(input_tensor_2d, min_rows, min_cols)
                    res = torch._cslt_sparse_mm(
