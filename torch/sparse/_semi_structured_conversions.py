...
def sparse_semi_structured_from_dense_cutlass(dense):
    ...
    if dense.dtype != torch.float:
        sparse0 = dense_4.gather(-1, idxs0.unsqueeze(-1))  # type: ignore[possibly-undefined]  # TODO: audit
        sparse1 = dense_4.gather(-1, idxs1.unsqueeze(-1))
        sparse = torch.stack((sparse0, sparse1), dim=-1).view(m, k // 2)
    else:
        sparse = dense_2.gather(-1, idxs0.unsqueeze(-1) // 2).view(m, k // 2)  # type: ignore[possibly-undefined]  # TODO: audit
    ...
    meta_reordered = meta.new_empty((m * meta_ncols,))  # type: ignore[possibly-undefined]  # TODO: audit
