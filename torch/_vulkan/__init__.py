...
def _compile_shader(name: str, source: str, use_buffers: bool):
    ...
    - Binding 0 is a buffer restrict writeonly containing a float[] where the output is
      to be written.
    - Bindings 1-N (where N is the arity of the op) are buffers containing a float[]
      where the input argument is stored.
    - Binding N+1 is a uniform block that may contain an arbitrary number of ints. The
      caller must pass that many ints when calling the compiled shader.
