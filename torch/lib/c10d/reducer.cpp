...
void Reducer::mark_variable_ready_dense(VariableIndex index) {
  ...
  runGradCallbackForVariable(variable, [&](auto& grad) {
    if (grad.defined()) {
      ...
      if (!c10d::viewSameMemory(grad, bucket_view)) {
  ...
  for (size_t i = 0; i < replica.variables.size(); i++) {
    ...
    if (gradient_as_bucket_view_) {
      ...
      runGradCallbackForVariable(v, [&](auto& grad) {
        if (grad.defined() && !c10d::viewSameMemory(grad, bucket_view)) {
          bucket_view.copy_(grad);
          grad = bucket_view;
  ...
  for (size_t replica_index = 0; replica_index < bucket.replicas.size();
    ...
    for (size_t intra_bucket_index = 0;
      ...
      if (!gradient_as_bucket_view_) {
        ...
        // grad.
        if (!c10d::viewSameMemory(bucket_view_in, bucket_view_out)) {
        ...
        runGradCallbackForVariable(variable, [&](auto& grad) {
          ...
          if (!global_unused) {
            ...
            if (!grad.defined()) {
              ...
              if (!c10d::viewSameMemory(grad, bucket_view_in)) {
