...
class AsyncAllgatherWork : public ProcessGroupGloo::AsyncWork {
 public:
  AsyncAllgatherWork(
      ...
      std::vector<at::Tensor>& inputs,
      uint32_t tag)
      : ProcessGroupGloo::AsyncWork(outputs[0], "gloo:all_gather", inputs),
        context(context),
        outputs(outputs),
        inputs(inputs),
        tag(tag) {}
 ...
 public:
  AsyncAllgatherCoalescedWork(
      ...
      : ProcessGroupGloo::AsyncWork(
            std::vector<at::Tensor>(), "gloo:all_gather", input_list),
        context(context),
...
class AsyncGatherWork : public ProcessGroupGloo::AsyncWork {
 public:
  AsyncGatherWork(
      ...
      int root,
      uint32_t tag)
      : ProcessGroupGloo::AsyncWork(outputs[0], "gloo:gather", inputs),
        context(context),
        outputs(outputs),
        inputs(inputs),
        root(root),
...
class AsyncBarrierWork : public ProcessGroupGloo::AsyncWork {
 public:
  AsyncBarrierWork(
      ...
      uint32_t tag)
      : ProcessGroupGloo::AsyncWork(
            std::vector<at::Tensor>(), "gloo:barrier", c10::nullopt),
        context(context),
