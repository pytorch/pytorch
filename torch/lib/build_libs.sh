...
function build() {
  ...
  ${CMAKE_VERSION} ../../$1 -DCMAKE_MODULE_PATH="$BASE_DIR/cmake/FindCUDA" \
              ...
              -DTH_LIBRARIES="$INSTALL_DIR/lib/libTH.a" \
              -DATEN_LIBRARIES="$INSTALL_DIR/lib/libATen$LD_POSTFIX" \
              -DTHNN_LIBRARIES="$INSTALL_DIR/lib/libTHNN.a" \
              -DTHCUNN_LIBRARIES="$INSTALL_DIR/lib/libTHCUNN.a" \
              -DTHS_LIBRARIES="$INSTALL_DIR/lib/libTHS.a" \
              -DTHC_LIBRARIES="$INSTALL_DIR/lib/libTHC.a" \
              -DTHCS_LIBRARIES="$INSTALL_DIR/lib/libTHCS.a" \
              -DTH_SO_VERSION=1 \
              -DTHC_SO_VERSION=1 \
              -DTHNN_SO_VERSION=1 \
