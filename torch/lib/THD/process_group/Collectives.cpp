...
void THDAllReduceMultiGPU(THDTensorDescriptor** data,
                          ...
                          THDGroup group) {
  std::vector<thpp::Tensor*> dataVec(data, data + len);
...
void THDReduceMultiGPU(THDTensorDescriptor** desc,
                       ...
                       int dst_rank,
                       THDGroup group) {
  std::vector<thpp::Tensor*> dataVec(desc, desc + len);
...
void THDAllGatherMultiGPU(THDTensorDescriptor** output,
                          ...
                          size_t inputLen,
                          THDGroup group) {
  std::vector<thpp::Tensor*> outputVec(output, output + outputLen);
  std::vector<thpp::Tensor*> inputVec(input, input + inputLen);
