...
struct SparseBitVectorElement {
 ...
 public:
  ...
  size_type count() const {
    unsigned NumBits = 0;
    for (unsigned i = 0; i < BITWORDS_PER_ELEMENT; ++i)
      NumBits += c10::llvm::countPopulation(Bits[i]);
    return NumBits;
  }
  ...
  int find_first() const {
    for (unsigned i = 0; i < BITWORDS_PER_ELEMENT; ++i)
      if (Bits[i] != 0)
        return i * BITWORD_SIZE + c10::llvm::countTrailingZeros(Bits[i]);
    ...
    for (unsigned I = 0; I < BITWORDS_PER_ELEMENT; ++I) {
      ...
      if (Bits[Idx] != 0)
        return Idx * BITWORD_SIZE + BITWORD_SIZE -
            c10::llvm::countLeadingZeros(Bits[Idx]);
    }
    throw std::runtime_error("Illegal empty element");
  }
  ...
  int find_next(unsigned Curr) const {
    ...
    if (Copy != 0)
      return WordPos * BITWORD_SIZE + c10::llvm::countTrailingZeros(Copy);
...
using torch::headeronly::SparseBitVector;
using torch::headeronly::SparseBitVectorElement;
} // namespace c10
