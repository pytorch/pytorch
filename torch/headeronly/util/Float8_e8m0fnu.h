...
inline C10_HOST_DEVICE uint8_t fp8e8m0fnu_from_fp32_value(float f) {
  ...
  uint32_t f_bits = torch::headeronly::detail::fp32_to_bits(f);
...
inline C10_HOST_DEVICE Float8_e8m0fnu::operator float() const {
  ...
  if (x == 0) {
    return torch::headeronly::detail::fp32_from_bits(0x00400000);
  }

  // if exponent is NaN, need to special case to return properly encoded NaN
  if (isnan()) {
    return torch::headeronly::detail::fp32_from_bits(0x7f800001);
  }

  // leave sign at 0, set the exponent bits, leave stored mantissa at 0
  uint32_t res = x << 23;

  return torch::headeronly::detail::fp32_from_bits(res);
}
...
namespace c10 {
  using torch::headeronly::Float8_e8m0fnu;
