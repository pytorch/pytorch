...
bool safe_multiplies_u64(It first, It last, uint64_t* out) {
  ...
  for (; first != last; ++first) {
    overflow |= mul_overflows(prod, *first, &prod);
  }
  *out = prod;
  return overflow;
#else
...
namespace c10 {
using torch::headeronly::add_overflows;
using torch::headeronly::mul_overflows;
using torch::headeronly::safe_multiplies_u64;
} // namespace c10
