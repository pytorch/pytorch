...
constexpr torch::headeronly::complex<fT> operator+(const torch::headeronly::complex<fT>& a, const iT& b) {
  return a + static_cast<fT>(b);
}

template <typename fT, typename iT, COMPLEX_INTEGER_OP_TEMPLATE_CONDITION>
constexpr torch::headeronly::complex<fT> operator+(const iT& a, const torch::headeronly::complex<fT>& b) {
...
struct alignas(4) complex<Half> {
  ...
  C10_HOST_DEVICE inline complex(const torch::headeronly::complex<float>& value)
      : real_(value.real()), imag_(value.imag()) {}

  // Conversion operator
  inline C10_HOST_DEVICE operator torch::headeronly::complex<float>() const {
    return {real_, imag_};
  }
...
namespace c10 {
  ...
  using namespace torch::headeronly::complex_literals;
}
