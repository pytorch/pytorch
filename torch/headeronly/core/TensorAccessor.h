...
class TensorAccessorBase {
 public:
  ...
  using ArrayRefCls = ArrayRef<int64_t>;
...
// strides() return HeaderOnlyArrayRef<int64_t>
template <
    typename T,
    size_t N,
    template <typename U> class PtrTraits = DefaultPtrTraits,
    typename index_t = int64_t>
using TensorAccessorBase = detail::
    TensorAccessorBase<c10::HeaderOnlyArrayRef, T, N, PtrTraits, index_t>;
...
// sizes() and strides() return HeaderOnlyArrayRef<int64_t>, and bounds check
// uses STD_TORCH_CHECK instead of TORCH_CHECK_INDEX
template <
    typename T,
    size_t N,
    template <typename U> class PtrTraits = DefaultPtrTraits,
