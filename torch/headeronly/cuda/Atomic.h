...
template <>
struct AtomicFPOp<torch::headeronly::Half> {
  template <typename func_t>
  inline __device__ torch::headeronly::Half operator() (torch::headeronly::Half *address, torch::headeronly::Half val, const func_t& func) {
...
template <>
struct AtomicFPOp<torch::headeronly::BFloat16> {
  template <typename func_t>
  inline __device__ torch::headeronly::BFloat16 operator() (torch::headeronly::BFloat16 *address, torch::headeronly::BFloat16 val, const func_t& func) {
    unsigned int * address_as_ui =
      (unsigned int *) ((char *)address - ((size_t)address & 2));
    unsigned int old = *address_as_ui;
    unsigned int assumed;

    torch::headeronly::BFloat16 bsum;
    do {
      assumed = old;
...
inline  __device__ torch::headeronly::Half gpuAtomicAdd(torch::headeronly::Half *address, torch::headeronly::Half val) {
#if defined(USE_ROCM) || ((defined(__CUDA_ARCH__) && (__CUDA_ARCH__ < 700)))
  return AtomicFPOp<torch::headeronly::Half>()(address, val,
                                [](torch::headeronly::Half hsum, torch::headeronly::Half val) {
                                  return hsum + val;
                                });
#else
...
inline __device__ torch::headeronly::BFloat16 gpuAtomicAdd(torch::headeronly::BFloat16 *address, torch::headeronly::BFloat16 val) {
#if defined(USE_ROCM) || ((defined(__CUDA_ARCH__) && (__CUDA_ARCH__ < 800)))
return AtomicFPOp<torch::headeronly::BFloat16>()(address, val,
                                  [](torch::headeronly::BFloat16 bsum, torch::headeronly::BFloat16 val) {
                                    return bsum + val;
                                  });
#else
  __nv_bfloat16 r = atomicAdd(reinterpret_cast<__nv_bfloat16*>(address), *reinterpret_cast<__nv_bfloat16*>(&val));
  return *reinterpret_cast<torch::headeronly::BFloat16*>(&r);
#endif
}
 ...
 */
inline __device__ torch::headeronly::Half atomicAdd(torch::headeronly::Half *address, torch::headeronly::Half val) {
  return gpuAtomicAdd(address, val);
}

inline __device__ torch::headeronly::BFloat16 atomicAdd(torch::headeronly::BFloat16 *address, torch::headeronly::BFloat16 val) {
  return gpuAtomicAdd(address, val);
}
 ...
 */
template<typename T>
inline __device__ void gpuAtomicAddNoReturn(torch::headeronly::complex<T> *address, torch::headeronly::complex<T> val) { gpuAtomicAdd(address, val); }
...
inline __device__ void gpuAtomicAddNoReturn(torch::headeronly::Half *address, torch::headeronly::Half val) { gpuAtomicAdd(address, val); }
inline __device__ void gpuAtomicAddNoReturn(torch::headeronly::BFloat16 *address, torch::headeronly::BFloat16 val) { gpuAtomicAdd(address, val); }
