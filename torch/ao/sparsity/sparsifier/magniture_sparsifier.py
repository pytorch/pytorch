...
class MagnitudeSparsifier(BaseSparsifier):
    r"""Magnitude Sparsifier

    This sparsifier computes the absolute value mean of every sparse block and "zeroes-out"
    the ones with the lowest absolute value. The level of sparsity defines how many of the
    blocks is removed.
    ...
    TODO(z-a-f): Factor out common code with the WeightNormSparsifiers
    TODO(z-a-f): Confirm that the weight used is not masked
    ...
    Note::
        All arguments to the MagnitudeSparsifier constructor are "default"
    ...
    def update_mask(self, layer, sparsity_level, sparse_block_shape,
        ...
        if sparsity_level <= 0 or zeros_per_block == 0:
            ...
            for row, col in zip(rows, cols):
                ...
                subweight = ww[row:row + sparse_block_shape[0],
                               col:col + sparse_block_shape[1]]
