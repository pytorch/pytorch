...
class DataNormSparsifier(BaseDataSparsifier):
    ...
    def __init__(self, data_list: List[Tuple[str, Any]] = None, sparsity_level: float = 0.5,
        ...
        assert norm in ['L1', 'L2'], "only L1 norm supported at the moment"
    ...
    def _get_scatter_folded_mask(self, data, dim, indices, output_size, sparse_block_shape):
    ...
    def _get_block_level_mask(self, data,
        ...
        padded_data = padded_data * float('inf')  # can also be replaced with 0 to stop the removal of edge data
        padded_data[0:height, 0:width] = data
        unfolded_data = F.unfold(padded_data.unsqueeze(0).unsqueeze(0), kernel_size=sparse_block_shape,
                                 stride=sparse_block_shape)

        _, sorted_idx = torch.sort(unfolded_data, dim=1)
        sorted_idx = sorted_idx[:, 0:zeros_per_block, :]  # zero out zeros_per_block number of elements

        mask = self._get_scatter_folded_mask(data=unfolded_data, dim=1, indices=sorted_idx, output_size=padded_data.shape,
                                             sparse_block_shape=sparse_block_shape)

        mask = mask.squeeze()[0:height, 0:width]  # remove padding
        return mask

    def _get_data_level_mask(self, data, sparsity_level,
                             sparse_block_shape):
        ...
        data_norm = F.avg_pool2d(data.unsqueeze(0).unsqueeze(0), kernel_size=sparse_block_shape,
        ...
        sorted_idx = sorted_idx[:, :, 0:threshold_idx]

        mask = self._get_scatter_folded_mask(data=data_norm, dim=2, indices=sorted_idx,
                                             output_size=(height + dh, width + dw),
                                             sparse_block_shape=sparse_block_shape)

        mask = mask.squeeze()[0:height, 0:width]
        return mask
    ...
    def update_mask(self, name, data, sparsity_level,
        ...
        data_lvl_mask = self._get_data_level_mask(data=data_norm, sparsity_level=sparsity_level,
                                                  sparse_block_shape=sparse_block_shape)

        # Fetch block level mask that zeros out 'zeros_per_block' number of elements in every block
        block_lvl_mask = self._get_block_level_mask(data=data_norm, sparse_block_shape=sparse_block_shape,
                                                    zeros_per_block=zeros_per_block)
