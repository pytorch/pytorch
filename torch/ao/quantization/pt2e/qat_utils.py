...
def _fuse_conv_bn_qat_helper(
    ...
    match_pattern = get_aten_graph_module_for_pattern(conv_bn_pattern, example_inputs, is_cuda)
    ...
    replacement_pattern_with_conv_bias = get_aten_graph_module_for_pattern(
        qat_conv_bn_pattern,
        example_inputs,
        is_cuda,
    )
    ...
    replacement_pattern_no_conv_bias = get_aten_graph_module_for_pattern(
...
def _fold_conv_bn_qat_helper(
    ...
    for is_per_channel, has_bias, bias_is_quantized, bn_is_training in replacement_options:
        ...
        match_pattern = get_aten_graph_module_for_pattern(match_pattern, example_inputs, is_cuda, **kwargs)
        replacement_pattern = _get_folded_quantized_qat_conv_bn_pattern(
            is_per_channel, has_bias, bias_is_quantized, conv_fn, bn_is_training
        )
        replacement_pattern = get_aten_graph_module_for_pattern(replacement_pattern, example_inputs, is_cuda, **kwargs)
        replacements.extend(
            replace_pattern_with_filters(
                m,
                match_pattern,
