    ...
    def __new__(cls, activation=torch.nn.Identity, weight=torch.nn.Identity):
        ...
        if isinstance(weight, nn.Module):
            raise ValueError("QConfigDynamic received observer instance, please pass observer class instead. " +
                             "Use MyObserver.with_args(x=1) to override arguments to constructor if needed")
        warnings.warn("QConfigDynamic is going to be deprecated in PyTorch 1.12, please use QConfig instead", stacklevel=1)
        return super().__new__(cls, activation, weight)
        ...
        if backend == 'fbgemm':
            ...
            if not torch.cpu._is_cpu_support_vnni():
                warnings.warn(
                    ...
                    "on CPU without Vector Neural Network Instruction support.", stacklevel=1)
            qconfig = QConfig(activation=HistogramObserver.with_args(reduce_range=False),
                              weight=default_per_channel_weight_observer)
        elif backend == 'x86':
            qconfig = QConfig(activation=HistogramObserver.with_args(reduce_range=True),
...
def get_default_qconfig_dict(backend='x86', version=0):
    warnings.warn(
        ...
        "a future version. Please use torch.ao.quantization.get_default_qconfig_mapping instead.", stacklevel=1)
...
def get_default_qat_qconfig_dict(backend='x86', version=1):
    warnings.warn(
        ...
        "a future version. Please use torch.ao.quantization.get_default_qat_qconfig_mapping instead.", stacklevel=1)
