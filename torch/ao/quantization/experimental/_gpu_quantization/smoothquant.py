...
# # wrapper of https://github.com/facebookresearch/fairscale/blob
# # /5b38de380e4407c2ef02f357ebc640f53470ea24/fairscale/nn/model_parallel/layers.py#L218
# class SmoothFakeDynQuantColumnParallelLinear(SmoothFakeDynQuantMixin, ColumnParallelLinear):
#     def __init__(self, *args, **kwargs):
#         alpha = kwargs.pop('alpha')
#         super().__init__(*args, **kwargs)
#         self.init_smoothquant_variables(alpha)
...
#         # print('col', 'input_.shape', input_.shape, 'weight.shape', self.weight.shape)
#         # Set up backprop all-reduce.
#         input_parallel = copy_to_model_parallel_region(input_)
#         # log_with_rank('input_.shape', input_.shape, 'input_parallel.shape', input_parallel.shape)

#         # SmoothQuant calibration/scaling
#         if self.calibrating:
#             self.update_x_running_abs_max(input_parallel)
#             # Matrix multiply.
...
#             else:
#                 X = input_parallel
#             W_int_repr_t = self.W_int_repr if self.store_w_int_repr_t else self.W_int_repr.t()
#             output_parallel = quant_int8_dynamic_per_token_linear(
#                 X, W_int_repr_t, self.W_scales, self.bias, X.dtype)
...
#         # log_with_rank('output_parallel.shape', output_parallel.shape, 'output.shape', output.shape)
#         return output
...
#         self.calibrating = False
#         W_absmax = torch.max(torch.abs(self.weight.transpose(0, 1)), dim=1).values
#         # log_with_rank('W_absmax before reduce', W_absmax)
#         W_absmax = _max_reduce(W_absmax)
#         # log_with_rank('W_absmax after reduce', W_absmax)
#         self.smooth_scale = get_scale(
#             self.x_running_abs_max,
#             W_absmax,
#             alpha=self.alpha)
...
# # wrapper of https://github.com/facebookresearch/fairscale/
# # blob/5b38de380e4407c2ef02f357ebc640f53470ea24/fairscale/nn/model_parallel/layers.py#L299
# # TODO(before review): test in distrubuted setting with >1 GPU, this
# # will need an updated scale calculation
# class SmoothFakeDynQuantRowParallelLinear(SmoothFakeDynQuantMixin, RowParallelLinear):
#     def __init__(self, *args, **kwargs):
#         alpha = kwargs.pop('alpha')
#         super().__init__(*args, **kwargs)
#         self.init_smoothquant_variables(alpha)
...
#     def forward(self, input_):
#         # print('row', 'input_.shape', input_.shape, 'weight.shape', self.weight.shape)
#         # Set up backprop all-reduce.
#         # log_with_rank('input_', input_, input_.shape)
#         if self.input_is_parallel:
#             input_parallel = input_
#         else:
#             input_parallel = scatter_to_model_parallel_region(input_)
#         # log_with_rank('input_parallel', input_parallel, input_parallel.shape)
#         # log_with_rank('self.weight', self.weight, self.weight.shape)

#         # SmoothQuant calibration/scaling
#         if self.calibrating:
#             if False:
#                 # for RowParallelLinear, the activation calibration needs to
#                 # happen on the full activation. For now, do it the simple way
#                 # by doing the update before scatter. A future performance
#                 # optimization would be to do the update after the scatter,
#                 # with less code reuse with the other layers.
#                 self.update_x_running_abs_max(input_)
#             self.update_x_running_abs_max(input_parallel)
#             # Matrix multiply.
...
#                 # TODO(future): fuse this into previous layer (LayerNorm,
#                 # RMSNorm, etc) where appropriate
#                 X = input_parallel / self.smooth_scale
#             else:
#                 X = input_parallel
#             # Matrix multiply.
#             # Note: bias is added later after all-reduce, so we skip it here
#             bias_to_use = None
#             W_int_repr_t = self.W_int_repr if self.store_w_int_repr_t else self.W_int_repr.t()
#             output_parallel = quant_int8_dynamic_per_token_linear(
#                 X, W_int_repr_t, self.W_scales, self.bias, X.dtype)

#         # log_with_rank('output_parallel', output_parallel, output_parallel.shape)
#         # All-reduce across all the partitions.
#         output_ = reduce_from_model_parallel_region(output_parallel)
#         # log_with_rank('output_', output_, output_.shape)
...
#         self.calibrating = False
#         W_absmax = torch.max(torch.abs(self.weight.transpose(0, 1)), dim=1).values
#         # log_with_rank('W_absmax before reduce', W_absmax, W_absmax.shape)
#         # W_absmax = _max_reduce(W_absmax)
#         # log_with_rank('W_absmax after reduce', W_absmax, W_absmax.shape)
#         # log_with_rank('self.x_running_abs_max', self.x_running_abs_max, self.x_running_abs_max.shape)
#         # Note: this is the local smooth scale which is enough for emulating
#         # numerics. To get the global one to fuse to previous layer, we'll
#         # need to all-gather.
#         # TODO: add global smooth scale when we need fusion.
#         self.smooth_scale = get_scale(
#             self.x_running_abs_max,
#             W_absmax,
#             alpha=self.alpha)
...
source_cls_to_target_cls = {
    ...
    # RowParallelLinear: SmoothFakeDynQuantRowParallelLinear,
    # ColumnParallelLinear: SmoothFakeDynQuantColumnParallelLinear,
}
