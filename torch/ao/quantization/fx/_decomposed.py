...
quantized_decomposed_lib.define(
    "int4mm_packed_weight_cpu(Tensor input, Tensor weight, Tensor qGroupSize, Tensor scalesAndZeroPoints) -> Tensor"
)


@impl(quantized_decomposed_lib, "int4mm_packed_weight_cpu", "CPU")
def int4mm_packed_cpu(
    input: torch.Tensor,
    weight: torch.Tensor,
    qGroupSize: torch.Tensor,
    scalesAndZeroPoints: torch.Tensor,
) -> torch.Tensor:
    """
    A wrapper of `torch.ops.aten._weight_int4pack_mm_for_cpu` to support group size as a tensor.
    Args:
       input (torch.Tensor): bfloat16 activation Tensor
       weight (torch.Tensor): int4 weights packed in int32 dtype Tensor
       qGroupSize (torch.Tensor): group size for quantization
       scaleAndZeroPoints (torch.tensor): scales & zero points in BF16.
    Returns:
       GEMM output
    """
    assert qGroupSize.numel() == 1, "qGroupSize must be a scalar"
    assert qGroupSize.dtype == torch.int64, "qGroupSize must be int64"
    return torch.ops.aten._weight_int4pack_mm_for_cpu(
        input, weight, qGroupSize.item(), scalesAndZeroPoints
    )


@impl(quantized_decomposed_lib, "int4mm_packed_weight_cpu", "Meta")
def int4mm_packed_meta(
    ...
    torch._check(weights.dim() == 2, lambda: "w must be a 2D tensor")
    torch._check(
        weights.dtype is torch.uint8,
        lambda: f"expected w to be uint8, got {weights.dtype}",
    )
    return input.new_empty(input.size(0), weights.size(0) * 2, dtype=input.dtype)
