...
def node_arg_is_weight(node: Node, arg: Any, backend_config: BackendConfig) -> bool:
    ...
    if isinstance(node, Node) and node.op == "call_function" and node.target in backend_config._config_dict:
        weight_index = backend_config._config_dict[node.target]._input_type_to_index.get("weight")
...
def node_arg_is_bias(node: Node, arg: Any, backend_config: BackendConfig) -> bool:
    ...
    if isinstance(node, Node) and node.op == "call_function" and node.target in backend_config._config_dict:
        bias_index = backend_config._config_dict[node.target]._input_type_to_index.get("bias")
