...
def is_qconfig_supported_by_dtype_configs(qconfig: QConfig, dtype_configs: List[DTypeConfig]):
    for dtype_config in dtype_configs:
        ...
        if is_dynamic is None:
            is_dynamic = False
        input_dtype = dtype_config.input_dtype.dtype or torch.float
        weight_dtype = dtype_config.weight_dtype.dtype or torch.float
        bias_dtype = dtype_config.bias_dtype or torch.float
        output_dtype = dtype_config.output_dtype.dtype or torch.float
