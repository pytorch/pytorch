...
def is_qconfig_supported_by_dtype_configs(qconfig: QConfig, dtype_configs: List[DTypeConfig]):
    for dtype_config in dtype_configs:
        ...
        if is_dynamic is None:
            is_dynamic = False
        input_dtype = dtype_config.get_input_dtype() or torch.float
        weight_dtype = dtype_config.get_weight_dtype() or torch.float
        bias_dtype = dtype_config.get_bias_dtype() or torch.float
        output_dtype = dtype_config.get_output_dtype() or torch.float
