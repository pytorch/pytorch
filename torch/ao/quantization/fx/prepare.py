...
from torch.ao.quantization.quantize import convert, is_activation_post_process
from torch.fx import GraphModule
from torch.fx.graph import Graph, Node
from torch.fx.node import Argument

from ..backend_config import BackendConfig, DTypeConfig, get_native_backend_config
...
from ..qconfig_mapping import _FIXED_QPARAMS_OP_TO_OBSERVER, QConfigMapping
from ..qconfig_mapping_utils import get_flattened_qconfig_dict, update_qconfig_for_qat

from ..quantize import propagate_qconfig_
...
from .backend_config_utils import get_pattern_to_quantize_handlers
...
from .match_utils import _MatchResultWithQConfig, find_matches

from .pattern_utils import sorted_patterns_dict
from .qconfig_utils import generate_qconfig_map, update_qconfig_for_fusion
...
def is_activation_post_process_node(
    ...
    return (
        ...
        and is_activation_post_process(modules[str(node.target)])
    )
...
def node_arg_is_weight(node: Node, arg: Any, weight_index_dict: Dict[str, List[int]]) -> bool:
    if (
        ...
        and node.target in weight_index_dict
    ):
        for i, node_arg in enumerate(node.args):
            if (
                arg is node_arg and i in weight_index_dict[node.target]
            ):  # type: ignore[index]
                return True
...
def node_arg_is_bias(node: Node, arg: Any, bias_index_dict: Dict[str, List[int]]) -> bool:
    if (
        ...
        or node.target not in bias_index_dict
    ):
        return False

    for i, node_arg in enumerate(node.args):
        if arg is node_arg and i in bias_index_dict[node.target]:  # type: ignore[index]
            return True
...
def is_input_arg_dtype_supported_by_backend(
    ...
    if isinstance(arg, (list, tuple)):
        return all(
            map(
                lambda a: is_input_arg_dtype_supported_by_backend(
                    a,
                    node,
    ...
    if is_activation:
        ...
        if is_dynamic:
            ...
            compute_dtype = node_name_to_target_dtype[node.name].get(
                "input_activation_compute_dtype", None
            )
            return (
                input_activation_dtype is None
                or compute_dtype == input_activation_dtype
            )
        else:
            input_activation_dtype = dtype_config.input_dtype
            return (
                input_activation_dtype is None
                or node_name_to_target_dtype[node.name]["input_activation_dtype"]
        ...
        return (
            ...
            or node_name_to_target_dtype[node.name]["weight_dtype"] == weight_dtype
        )
    else:  # bias
        bias_dtype = dtype_config.bias_dtype
        return (
            bias_dtype is None
            or node_name_to_target_dtype[node.name]["bias_dtype"] == bias_dtype
        )
...
def is_output_dtype_supported_by_backend(
    ...
    return (
        ...
        or output_dtype
        == node_name_to_target_dtype[node.name]["output_activation_dtype"]
    )
...
def is_observer_in_same_graph(node, modules, node_name_to_target_dtype):
    ...
    """
    node_output_dtype = get_arg_target_dtype_as_output(
        node, modules, node_name_to_target_dtype
    )
...
def get_target_activation_dtype_for_node(
    ...
    if node.op == "placeholder":
        ...
        is_getitem = node.op == "call_function" and node.target == operator.getitem
        if is_getitem:
            return {
                "input_activation_dtype": torch.float,
                "output_activation_dtype": torch.float,
        ...
        if qconfig is not None:
            if qhandler is not None and qhandler.input_output_observed():
                act_dtype, weight_dtype, act_compute_dtype = get_qconfig_dtypes(qconfig)
                bias_dtype = (
                    torch.float16
...
def get_arg_target_compute_dtype_as_input_to_node(
    ...
    if (
        is_activation
        and "input_activation_compute_dtype" in node_name_to_target_dtype[node.name]
    ):
...
def maybe_insert_input_observer_for_arg_or_kwarg(
    ...
    if not is_standalone_module:
        ...
        arg_as_output_target_dtype = get_arg_target_dtype_as_output(
            arg, modules, node_name_to_target_dtype
        )
        arg_as_input_target_dtype = get_arg_target_dtype_as_input_to_node(
            arg,
            node,
            modules,
            node_name_to_target_dtype,
        ...
        needs_obs = (
            ...
            (arg_as_output_target_dtype not in DO_NOT_OBS_DTYPE_LIST)
            and
            ...
            (
                arg_as_input_target_compute_dtype
                in [torch.quint8, torch.int8, torch.float16]
            )
        ...
        _, _, sm_prepare_custom_config, _ = get_standalone_module_configs(
    ...
    if needs_obs:
        ...
        if existing_obs_node is None:
            new_obs_node = insert_observer(arg, new_obs_mod, model, modules, graph)
...
def maybe_insert_input_equalization_observers_for_node(
    ...
    for arg in node.args:
        ...
        new_eq_obs_node = insert_observer(arg, new_eq_obs_mod, model, modules, graph)
...
def maybe_insert_output_observer_for_node(
    ...
    should_insert_observer = (
        should_insert_observer and activation_is_statically_quantized(qconfig)
    )
    ...
    if should_insert_observer:
        ...
        if activation_is_int8_quantized(qconfig):
            act_post_process_ctr = qhandler.get_activation_ctr(
                qconfig, matched_pattern, is_qat
            )
...
def maybe_insert_observers_before_graph_output(
    ...
    assert (
        output_quantized_idxs == [0] or output_quantized_idxs == []
    ...
    def _recursive_maybe_replace_node_with_obs(
        ...
        node_name_to_target_dtype: Dict[
            str, Dict[str, Optional[Union[torch.dtype, type]]]
        ],
        qconfig_map: Dict[str, QConfigAny],
        model: torch.nn.Module,
        modules: Dict[str, torch.nn.Module],
...
def insert_observers_for_model(
    ...
    propagate_dtypes_for_known_nodes(model.graph, node_name_to_target_dtype, matches)
    ...
    for node in nodes_before_observation:
        ...
        if node.op == "placeholder":
            ...
            skip_inserting_observers = (
                (qconfig is None) or output_not_a_tensor or is_getitem
                ...
                if node.op != "output":
                    ...
                    if (
                        ...
                        for user in node.args[0].users:
                            ...
                            is_user_quantized = qconfig_map.get(
                                user.name, None
                            ) is not None or (
                                user.op == "call_module"
                                and isinstance(modules[str(user.target)], ObserverBase)
                            )
                    ...
                    if is_last_node_of_pattern:
                        ...
                        if maybe_output_obs_node is not None:
                            ...
                            if (
                                is_general_tensor_value_op
                                and is_observer_in_same_graph_
                            ) or is_reuse_input_qconfig_:
                                if not maybe_make_input_output_share_observers(
                                    node, model, modules
                                ):
                            ...
                            if qhandler is not None and qhandler.is_custom_module():
                                swap_custom_module_to_observed(
                                    node, qconfig, modules, prepare_custom_config
                                )
        ...
        if module_type_or_function_or_method in _FIXED_QPARAMS_OP_TO_OBSERVER:
            ...
            if qconfig is None:
                ...
                for observer_ctr in allowed_observer_ctrs + [
                    _FIXED_QPARAMS_OP_TO_OBSERVER[module_type_or_function_or_method]
                ]:
                    if obs_or_fq_ctr_equals(
                        qconfig.activation,
                        FixedQParamsFakeQuantize.with_args(observer=observer_ctr),
                    ) or obs_or_fq_ctr_equals(qconfig.activation, observer_ctr):
                        bad_observer = False
            if bad_observer:
                raise ValueError(
                    "QConfigMapping must specify fixed qparams observer for fixed qparams op "
...
def run_prepare_fx_on_standalone_modules(
    ...
    for (
        ...
        observed_standalone_module = ObservedStandaloneGraphModule(
            ...
            observed_standalone_module.graph,
...
def prepare(
    ...
    qconfig_map = generate_qconfig_map(
        model, modules, model.graph, qconfig_mapping, node_name_to_scope
    )
