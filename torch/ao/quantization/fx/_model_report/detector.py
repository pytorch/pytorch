...
# Mapping information
# Adding class for returning QConfigMapping by Detector
class DetectorQConfigMapping():
    r"""
    Class for returning the QConfigMapping generated by the detectors.

    This class contains the QConfigMapping suggestion that is generated by the
    different detectors. It takes care of merging the different QConfigMapping
    suggestions generated by each of the individual detectors so the user only
    needs to deal with one QConfigMapping that they can just use.

    Args:
        detector_classes ([DetectorBase]): The list of detector classes this
            QConfigMapping was generated by.
        mapping (QConfigMapping): The qconfig_mapping based on the suggestions
            given by this class
    """

    def __init__(self, detector_classes: List[DetectorBase], mapping: QConfigMapping):
        super().__init__()
    ...
    def merge_detector_mappings(self, mapping_to_merge: DetectorQConfigMapping) -> DetectorQConfigMapping:
        ...
        The main restriction is that mapping_to_merge has to only contain a single
        Detector class in its detector_classes attribute; it will not work if it has
        multiple. This way, we are always merging smaller single detector mappings into
