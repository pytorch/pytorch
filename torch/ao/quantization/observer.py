        ...
        if reduce_range:
            warnings.warn(
                "Please use quant_min and quant_max to specify the range for observers. \
                    reduce_range will be deprecated in a future release of PyTorch.", stacklevel=TO_BE_DETERMINED
            )
        ...
        for name in local_state:
            ...
            if key in state_dict:
                ...
                if name == expected_min_name:
                    ...
                    warnings.warn(f"Observer load_from_state_dict got unexpected name {name}", stacklevel=TO_BE_DETERMINED)
        ...
        if is_uninitialized:
            warnings.warn(
                "must run observer before calling calculate_qparams.\
                                    Returning default scale and zero point ", stacklevel=TO_BE_DETERMINED
            )
        ...
        if compute_dtype:
            ...
            warnings.warn(
                "Please use `is_dynamic` instead of `compute_dtype`. \
                    ...
                    of PyTorch.", stacklevel=TO_BE_DETERMINED
            )
