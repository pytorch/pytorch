...
def get_node_first_input_and_output_type(
    ...
    if node.op == "call_function":
        ...
        is_known_fp32_or_int8_input_module = any(
            isinstance(mod, target_type)
            for target_type in MODS_IO_TYPE_FP32_OR_INT8  # type: ignore[arg-type]
        )
        if (
        ...
        is_known_fp32_input_module = any(
            isinstance(mod, target_type)
            for target_type in MODS_IO_TYPE_FP32  # type: ignore[arg-type]
        )
        is_known_int8_input_module = any(
            isinstance(mod, target_type)
            for target_type in MODS_IO_TYPE_INT8  # type: ignore[arg-type]
        )
...
def get_node_input_qparams(
    ...
    if prev_node.op == "call_function":
        ...
        is_known_fp32_or_int8_input_module = any(
            isinstance(module_obj, target_type)
            for target_type in MODS_IO_TYPE_FP32_OR_INT8  # type: ignore[arg-type]
        )
