...
class ConvReLU1d(nnq.Conv1d):
    ...
    def from_float(cls, mod):
        if type(mod) == torch.ao.nn.quantization.intrinsic.qat.ConvBnReLU1d:
...
class ConvReLU2d(nnq.Conv2d):
    ...
    @classmethod
    def from_float(cls, mod):
        if type(mod) == torch.ao.nn.quantization.intrinsic.qat.ConvBnReLU2d:
            mod.weight, mod.bias = fuse_conv_bn_weights(
                mod.weight, mod.bias, mod.bn.running_mean, mod.bn.running_var,
                mod.bn.eps, mod.bn.weight, mod.bn.bias)
...
class ConvReLU3d(nnq.Conv3d):
    ...
    @classmethod
    def from_float(cls, mod):
        if type(mod) == torch.ao.nn.quantization.intrinsic.qat.ConvBnReLU3d:
            mod.weight, mod.bias = fuse_conv_bn_weights(
                mod.weight,
                mod.bias,
                mod.bn.running_mean,
