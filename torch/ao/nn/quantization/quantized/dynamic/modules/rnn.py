...
class RNNBase(torch.nn.Module):
    ...
    def from_float(cls, mod):
        assert type(mod) in set(
            [torch.nn.LSTM,
             torch.nn.GRU]
        ), 'ao.nn.quantization.quantized.dynamic.RNNBase.from_float only works for nn.LSTM and nn.GRU'
        assert hasattr(
            mod,
            'qconfig'
...
class RNNCellBase(torch.nn.Module):
    ...
    def from_float(cls, mod):
        assert type(mod) in set([torch.nn.LSTMCell,
                                 torch.nn.GRUCell,
                                 torch.nn.RNNCell]), 'ao.nn.quantization.quantized.dynamic.RNNCellBase.from_float \
