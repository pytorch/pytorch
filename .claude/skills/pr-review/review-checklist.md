...
`pull.yml` and `trunk.yml` gate every commit, so a red or flaky job costs every author, not just the one who broke it — `docs/source/community/viable_strict.md` is the policy for keeping the `viable/strict` blocking set healthy. The items below are advisory and warrant **Needs Discussion**, not **Request Changes**; skip them if the PR does not modify one of these workflows.

- [ ] **New jobs prove reliability before gating** - PR adds a job with no run history: a new hardware target, a new test config, or a job with no analog already in the workflow. Prefer jobs relocated from an existing workflow with history, or a variation of a job already in the workflow; otherwise recommend `unstable.yml`, which runs on every main push, to get an initial read. Hold this line hardest for scarce or partner-operated runners (`linux.aws.h100`, `linux.dgx.b200`, `linux.rocm.gpu.*`) — those pools are fixed inventory, so a flaky job there also queues every other job behind it. Skip when the job replaces one deleted in the same diff (rename, OS or toolchain bump) or is build-only — neither adds a new flake surface
