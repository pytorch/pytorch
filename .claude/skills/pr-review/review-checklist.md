...
### Device Portability (CUDA / XPU / other accelerators)

Much PyTorch code is shared across accelerator backends (CUDA, XPU, ROCm, MPS), where device-specific code can leak into shared paths and only surface later on another accelerator. When a PR writes device-agnostic code (i.e. **not** under a device-specific directory like `aten/src/ATen/native/cuda/`, `*_cuda.cpp`, or files clearly behind `USE_CUDA`), treat any hardcoded device-specific assumption as a must-fix unless it is guarded by an explicit device check that makes it reachable only on that device (e.g. `if device.type == "cuda":`, `torch.cuda.is_available()`, dispatch keyed on `device_type == "cuda"`, or a `cuda` branch). Confirm reachability before flagging.

- [ ] **No hardcoded `torch.cuda.*` in device-agnostic code** — PR uses `torch.cuda.synchronize()`, `torch.cuda.Stream`, `torch.cuda.Event`, `torch.cuda.current_device()`, `torch.cuda.is_available()`, etc. in code (or tests) that is meant to run on any accelerator. Use the device-agnostic `torch.accelerator.*` API or `torch.get_device_module(device)` instead, and derive the device from the input rather than assuming CUDA
- [ ] **No hardcoded `"cuda"` device strings** — PR creates tensors with `device="cuda"`, `.cuda()`, `torch.device("cuda")`, or `torch.zeros(..., device="cuda")` in shared code or in device-agnostic tests. Tests must take `device` as a parameter (via `instantiate_device_type_tests`) and allocate with `device=device`; non-test code must derive the device from inputs
- [ ] **No CUDA-only codegen/cache classes in shared Inductor/compile code** — PR references `CUDACodeCache`, `CUDACompileError`, `nvcc`, or other CUDA-specific backend classes in code shared across backends instead of going through the device-agnostic abstraction (e.g. selecting the cache/compiler by `device_type`). XPU has parallel classes (e.g. an XPU code cache); shared logic must dispatch on the device type rather than hardcoding the CUDA implementation. Verify reachability transitively: a modified class may appear CUDA-specific by filename but be instantiated from a combined scheduler in `codegen/xpu/` or `codegen/rocm/`. Check both the modified file and any backend-specific scheduler that delegates to it (e.g. `grep -r "CUTLASSScheduling\|<ClassName>" torch/_inductor/codegen/xpu/ torch/_inductor/codegen/rocm/`)
- [ ] **No hardcoded `"cuda"` in expected-string test assertions** — PR adds tests that assert on generated code, repr, or error messages containing a hardcoded `cuda` substring, kernel names containing `cuda`). These fail on XPU where the same string contains `xpu`. Derive the expected device substring from the test `device` (e.g. compute `GPU_TYPE` / `self.device` and interpolate it) instead of hardcoding `cuda`
