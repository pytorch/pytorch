...
3. There are hooks in `/scripts`: a target pre-hook (`validate_issue_target.py`) that blocks mutations outside the repository and issue selected by the workflow, a label pre-hook (`validate_labels.py`) that filters out labels we never want the bot to add, a pre-hook (`coerce_tool_input.py`) that repairs stringified arguments (`issue_number` as `"123"`, `labels` as a JSON string) on the tools `validate_labels.py` does not already rewrite (hooks run in parallel and the last `updatedInput` wins, so `validate_labels.py` does its own coercion for `update_issue`), and a post-hook (`add_bot_triaged.py`) that automatically applies `bot-triaged` after any issue mutation. `check_triage_outcome.py` is not a hook: Stage 2 runs it after the agent to check the issue's live state, fail the job when nothing happened, and re-apply a missing `bot-triaged`.
