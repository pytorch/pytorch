...
SQL_CASES = ["snapshots", "same_job", "different_jobs", "zero_time", "paths"]
...
class TestCost(TestCase):
    ...
    def test_files_sql(self, case):
        ...
        job_values = ", ".join(repr((job, job * 10)) for job in jobs)
        values = ", ".join(
            repr(
                (
                    job,
        ...
        fixture = f"""
WITH jobs AS (
  SELECT * FROM VALUES('id UInt64, run_id UInt64', {job_values})
),
labeled AS (
  SELECT id, 'linux.2xlarge' AS runner, 'pull' AS workflow, 'pr' AS trigger,
