...
GreenLight is an automated reviewer for pull requests authored by contributors authorized to merge by [merge_rules.yaml](https://github.com/pytorch/pytorch/blob/main/.github/merge_rules.yaml). It reads the introduced changes and decides whether the it is safe to land as-is; when it is, GreenLight approves the PR, and that approval alone satisfies a merge rule. The criteria it applies are in [its review skill](https://github.com/pytorch/test-infra/tree/main/.claude/skills/greenlight-review). There is nothing to opt into; a scan picks up eligible open PRs every few minutes.
